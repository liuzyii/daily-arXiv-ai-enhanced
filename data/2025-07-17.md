...
TL;DR: 提出了一种硬件友好的优化框架，用于解决可变形注意力变换器（DAT）在硬件部署中的内存访问问题，通过神经架构搜索（NAS）和FPGA验证系统，实现了低精度损失和高效率。
...
TL;DR: 论文提出了一种名为DDCN的动态可变形卷积网络，用于高效且准确的时空交通预测，解决了传统GNN和CNN在捕捉异构性和扩展性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉时空交通数据的异构性和扩展性方面存在不足，尤其是GNN需要预定义邻接矩阵且难以扩展，而CNN难以建模非欧几里得结构。

Method: DDCN通过动态应用基于偏移的可变形滤波器，分解为编码器-解码器结构，并在编码器中引入空间和时空注意力块以突出重要特征。

Result: 在四个真实数据集上的实验表明，DDCN具有竞争性的性能，验证了CNN方法在时空交通预测中的潜力。

Conclusion: DDCN通过动态可变形卷积和注意力机制，实现了高效且准确的时空交通预测，为CNN在该领域的应用提供了新思路。
...
TL;DR: Inversion-DPO是一种新的扩散模型对齐框架，通过DDIM反演优化直接偏好优化（DPO），避免了奖励建模，提高了训练精度和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的对齐方法需要计算密集的奖励模型训练，导致计算开销大且可能影响模型精度和效率。

Method: 提出Inversion-DPO框架，利用DDIM反演将DPO应用于扩散模型，避免了奖励建模，直接从样本反演到噪声进行后训练。
...
TL;DR: ST-VFM是一种新框架，通过重新编程视觉基础模型（VFMs）来解决时空预测任务中的时空建模和模态差距问题，采用双分支架构和专用重编程阶段，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在自然语言处理和计算机视觉中表现出色，但大型语言模型（LLMs）在时空预测中难以捕捉丰富的时空相关性。ST-VFM旨在利用VFMs的空间先验，解决其缺乏时间建模能力和视觉与时空数据之间的模态差距。

Method: ST-VFM采用双分支架构，结合原始时空输入和辅助时空流输入，并通过预VFM和后VFM重编程阶段嵌入时间上下文和动态交互。

Result: 在十个时空数据集上的实验表明，ST-VFM优于现有基线，表现出对不同VFM主干（如DINO、CLIP、DEIT）的有效性和鲁棒性。

Conclusion: ST-VFM是一个强大的通用框架，能够有效利用VFMs进行时空预测，解决了时间建模和模态差距的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 步态识别年龄在医疗、安全和人机交互中有重要应用，需提高其准确性。

Method: 结合元分析、大规模数据集实验和可解释性可视化，评估不同传感器和模型的性能。

Result: 卷积神经网络平均误差4.2年，多传感器融合误差低至3.4年；深度网络准确率达96%。

Conclusion: 通过综合方法，论文为实际应用中降低步态年龄误差提供了基准和指南。
...
TL;DR: 本文探讨了利用改进的PPGNet模型（PPGNet-Cat）通过图像重识别技术监测澳大利亚野猫的方法，取得了高精度结果。


<details>
  <summary>Details</summary>
Motivation: 野猫对澳大利亚野生动物造成严重威胁，需要有效监测手段以减少其影响。

Method: 改进PPGNet模型（原用于东北虎重识别）以适应野猫图像特征，并探索对比学习方法如ArcFace损失。

Result: PPGNet-Cat表现优异，平均精度（mAP）达0.86，rank-1准确率为0.95。

Conclusion: PPGNet-Cat是一种高效的野猫重识别模型，适用于野生动物监测。
...
Method: 采用VAE作为特征编码器，结合MLP，应用于168例未接受新辅助治疗的患者的MRI数据集，以术后病理N分期为金标准评估模型。

Result: VAE-MLP模型在MRI数据集上达到AUC 0.86 +/- 0.05、敏感性0.79 +/- 0.06、特异性0.85 +/- 0.05的优异性能。

Conclusion: VAE-MLP模型在直肠癌LNM分期中表现优于现有方法，具有更高的诊断准确性和可解释性。
...
TL;DR: 论文提出了一种基于姿势的心理状态推断方法，通过板球运动验证其有效性，实现了较高的意图识别准确率。


<details>
  <summary>Details</summary>
Motivation: 姿势推断在疲劳诊断、伤害预防和性能提升方面潜力巨大，但面临数据敏感性问题。体育场景为获取多样化情绪数据提供了可行方案。

Method: 利用板球比赛的活动视频，通过运动分析识别攻击性和防守性击球意图。

Result: 方法在区分意图时达到75%以上的F1分数和80%以上的AUC-ROC。

Conclusion: 姿势分析为意图推断提供了强信号，弱监督验证方法为数据标注限制提供了解决方案，推动了体育分析和行为分析的通用技术发展。
...
TL;DR: VISTA是一种新颖的全局定位框架，通过分割和跟踪结合几何一致性，解决了视角和季节变化带来的挑战，显著提升了定位性能。


<details>
  <summary>Details</summary>
Motivation: 解决在无结构环境中因视角变化、季节变化等因素导致的传统定位方法失效问题。

Method: 结合前端对象分割与跟踪管道，以及子图对应搜索，利用几何一致性对齐参考帧。

Result: 在季节和斜视角数据集上，召回率提升69%，地图大小仅为基线方法的0.6%。

Conclusion: VISTA无需特定领域训练，适用于资源受限平台，实现了跨视角和季节的稳定定位。
...
<details>
  <summary>Details</summary>
Motivation: 解决户外广告文本在复杂环境下识别准确性的挑战，探索VLMs与传统OCR方法的优劣。

Method: 系统性地评估了Qwen 2.5 VL 3B、InternVL3和SmolVLM2等VLMs与PaddleOCRv4在公开数据集（ICDAR 2015和SVT）上的表现，并加入合成天气失真模拟真实场景。

Result: VLMs在全场景理解上表现优异，但轻量级CNN在裁剪文本识别上仍具竞争力且计算成本更低。

Conclusion: VLMs适合复杂场景，而轻量级CNN更适合边缘部署；公开了天气增强的基准和评估代码以促进未来研究。
...
TL;DR: 本文提出了一种统一的条件生成求解器（UCGS），旨在通过单一框架解决多种抽象视觉推理（AVR）任务，避免了任务特定设计和重新训练的需求。


<details>
  <summary>Details</summary>
Motivation: 设计具有人类抽象视觉推理能力的智能系统是人工智能领域的长期目标，但现有方法通常需要任务特定设计或重新训练，增加了成本。

Method: 将AVR任务重新表述为目标图像可预测性问题，并通过训练一个条件生成模型统一解决多种任务。

Result: 实验表明，UCGS通过多任务训练展示了跨任务的抽象推理能力，并具备零样本推理能力。

Conclusion: UCGS为AVR任务提供了一种高效统一的解决方案，减少了重新训练的需求。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在多样场景结构和跨域变化中鲁棒性不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅利用单张图像的视觉信息，忽略了上下文信息的潜在价值。

Method: 提出ProtoConNet，包含聚类数据选择（CDS）、上下文增强语义细化（CSR）和原型对齐（PA）三个模块。

Result: 在两个数据集上验证了ProtoConNet在少样本场景中的有效性，优于现有方法。
...
Method: GRACE结合动态运动建模、语义文本细化（CATE模块）和跨模态对齐，通过熵正则最优传输实现。
...
TL;DR: 该论文提出了一种新颖的空间频率调制（SFM）方法，通过调制高频特征到低频以减轻下采样时的混叠效应，并通过解调恢复细节。
...
Method: 采用自适应重采样（ARS）调制高频特征，并设计多尺度自适应上采样（MSAU）解调以恢复高频信息。

Result: SFM有效减轻了混叠并保留了细节，适用于多种任务和架构。

Conclusion: SFM在语义分割及其他视觉任务中表现出广泛适用性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 事件传感器在行人监控中表现优异，但相关数据稀缺。SEPose旨在提供全面的合成数据集以支持研究。

Method: 利用CARLA模拟器和动态视觉传感器生成SEPose数据集，包含多种环境和条件下的行人姿态数据。

Result: SEPose包含近350K标注行人姿态数据，支持现有模型（如RVT和YOLOv8）的训练，并展示了模拟到现实的泛化能力。

Conclusion: SEPose为事件传感器在行人监控中的应用提供了宝贵的数据资源，验证了其模拟到现实的潜力。
...
TL;DR: 论文提出Dark-EvGS框架，利用事件相机和3D高斯溅射技术，在低光条件下重建多视角明亮图像，解决了噪声、帧质量差和色调不一致问题。


<details>
  <summary>Details</summary>
Motivation: 传统相机在低光环境下难以捕捉清晰多视角图像，事件相机的高动态范围和高速度特性可能解决此问题，但现有方法仍面临噪声和色调不一致的挑战。

Method: 提出Dark-EvGS框架，结合事件相机和3D高斯溅射技术，引入三重监督和色调匹配模块，确保渲染帧的细节和颜色一致性。

Result: 实验表明，该方法在低光条件下优于现有方法，成功重建辐射场并合成明亮帧。

Conclusion: Dark-EvGS是首个事件辅助的3D高斯溅射框架，有效解决了低光环境下的多视角图像重建问题，并提供了首个真实捕获的数据集。
...
TL;DR: 论文介绍了Hyperphantasia基准，用于评估多模态大语言模型的主动心理可视化能力，发现当前模型与人类表现存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注被动视觉感知，而心理可视化是核心认知能力，需要主动构建视觉表征以支持问题解决。

Method: 设计了四个程序生成的谜题任务，分三个难度级别，评估模型表现，并探索强化学习提升视觉模拟能力的潜力。

Result: 评估显示当前模型在心理可视化能力上与人类存在显著差距，部分模型仅能识别视觉模式。

Conclusion: 当前多模态大语言模型在心理可视化方面仍面临挑战，需进一步研究提升其主动视觉模拟能力。
...
TL;DR: RaDL框架通过关系感知解耦学习解决多实例图像生成中的关系差异和属性泄漏问题，显著提升了位置准确性和实例间关系表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多实例图像生成中难以处理关系差异和多个属性泄漏问题，需要一种更有效的方法。

Method: 提出RaDL框架，利用可学习参数增强实例特定属性，并通过关系注意力生成关系感知图像特征。

Result: 在COCO-Position、COCO-MIG和DrawBench等基准测试中，RaDL表现优于现有方法，显著提升了位置准确性和实例关系。
...
TL;DR: 提出了一种名为PPAR的新框架，通过CLIP模型的原型对齐和重加权策略，解决了通用语义分割中的三个挑战，实现了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 通用语义分割在未见目标域上的表现是关键挑战，现有方法存在原型对齐粗糙、原型计算易过拟合以及忽略特征适应难度差异的问题。

Method: 定义了OTP和VTP两种原型，采用渐进对齐策略和原型重加权机制，逐步减少域差距并优化源数据贡献。

Result: 在多个基准测试中达到最先进性能，验证了PPAR的有效性。

Conclusion: PPAR通过结合CLIP的泛化能力和渐进对齐策略，显著提升了通用语义分割的性能。
...
TL;DR: 提出LG-CAV-MAE模型，通过结合预训练文本编码器和对比音频-视觉掩码自编码器，实现跨模态学习，显著提升音频-视觉检索和分类任务性能。
...
Method: 结合预训练文本编码器和对比音频-视觉掩码自编码器，利用自动生成的音频-视觉-文本三元组进行训练。

Result: 在音频-视觉检索任务中，召回率提升5.6%；在分类任务中，准确率提升3.2%。

Conclusion: LG-CAV-MAE通过跨模态学习和自动生成高质量三元组，显著提升了音频-视觉任务的性能。
...
TL;DR: 该论文提出了一个评估多模态大语言模型（MLLMs）在短视频内容审核中三模态安全性的框架，包括数据集SVMA和攻击策略ChimeraBreak，揭示了模型的高攻击成功率及其偏见。


<details>
  <summary>Details</summary>
Motivation: 当前的安全评估多基于单模态攻击，未能全面应对多模态联合攻击的脆弱性，尤其在短视频内容审核领域缺乏研究。

Method: 论文提出了SVMA数据集和ChimeraBreak攻击策略，通过视觉、听觉和语义推理三模态同时攻击MLLMs。

Result: 实验显示MLLMs存在显著漏洞，攻击成功率高，且模型对良性或违规内容的分类存在偏见。
...
TL;DR: GS-Bias是一种高效的测试时适应（TTA）方法，通过全局和空间偏置学习提升视觉语言模型的零样本泛化能力，性能优于现有方法且内存消耗低。
...
Method: 提出GS-Bias，通过全局偏置（学习增强视图一致性）和空间偏置（学习区域语义连贯性）直接调整预训练模型的输出，避免全反向传播。

Result: 在15个基准数据集上达到SOTA性能，例如在跨数据集泛化和域泛化上分别提升2.23%和2.72%，内存消耗仅为TPT的6.5%。
...
TL;DR: EC-Diff通过梯度噪声估计和最优切换点选择，在边缘-云协作框架中提升扩散模型的生成质量和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有边缘-云协作框架中，过多的云去噪步骤延长推理时间，而步骤不足则导致语义模糊，影响边缘模型输出的一致性。

Method: 提出K步噪声近似策略减少云推理频率，并设计两阶段贪心搜索算法优化噪声近似和边缘模型切换参数。

Result: 实验表明，EC-Diff在生成质量上优于边缘推理，推理速度比云推理平均提升2倍。

Conclusion: EC-Diff有效平衡了生成质量与推理速度，为扩散模型的边缘-云协作提供了高效解决方案。
...
TL;DR: MPAE提出了一种无监督部件发现的新方法，通过掩码自编码器学习部件描述符，并在复杂场景中实现鲁棒的部件匹配。
...
Method: MPAE通过掩码自编码器学习部件描述符，利用局部特征与描述符的相似性填充掩码区域，实现部件对齐。

Result: 实验表明，MPAE能在多种类别和场景中鲁棒地发现有意义部件。
...
TL;DR: 提出一种零样本扩散管道，通过融合不同风格模型的去噪潜在空间，实现区域特异性风格混合。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在风格混合中难以控制区域风格的问题。

Method: 在去噪潜在空间中进行风格融合，利用空间掩码和深度图条件控制。

Result: 成功实现区域特异性风格混合，保持各风格保真度。
...
Result: ID-EA在身份保持指标上显著优于现有方法，且生成速度提升约15倍。

Conclusion: ID-EA通过改进嵌入对齐，实现了高效且高保真的个性化肖像生成。
...
TL;DR: 论文提出了AUBlendSet数据集和AUBlendNet方法，用于细粒度的3D面部表情风格化操控，填补了相关数据集的空白。


<details>
  <summary>Details</summary>
Motivation: 当前3D面部动画缺乏细粒度风格化表情操控的合适数据集。

Method: 基于32种标准面部动作单元（AUs）构建AUBlendSet数据集，并提出AUBlendNet网络学习风格化的AU-Blendshape基向量。
...
TL;DR: 论文将掩码图像建模（MIM）重新定义为稀疏信号重建问题，提出MaskTwins框架，通过互补掩码增强特征提取，无需单独预训练即可实现领域自适应分割。
...
Method: 将掩码重建视为稀疏信号重建问题，提出MaskTwins框架，通过互补掩码一致性增强跨领域特征提取。

Result: 实验证明MaskTwins在自然和生物图像分割中优于基线方法，能有效提取领域不变特征。

Conclusion: MaskTwins为领域自适应分割提供了新范式，无需预训练即可实现高性能。
...
Method: 使用时间卷积层处理连续电影帧的输入，预测视觉皮层及其周围体素的活动，并通过显著性图分析贡献最大的脑区。

Result: 模型成功重建了边缘、面部和对比度等视觉特征，贡献最大的脑区包括中枕区、梭状回和距状沟。

Conclusion: 该模型为探索视觉处理机制提供了新工具，表明深度学习模型可以作为理解视觉处理的代理。
...
TL;DR: 论文提出了一种基于解耦-耦合策略的SS-DC框架，用于解决可见光到红外的无监督域自适应目标检测问题，通过光谱分解和空间-光谱耦合方法显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将RGB域视为统一域，忽略了其内部的多个子域（如白天、夜晚、雾天场景），解耦域不变和域特定特征有助于提升RGB-IR域自适应效果。

Method: 设计了光谱自适应幂等解耦（SAID）模块，通过基于滤波器组的光谱处理范式和自蒸馏驱动的解耦损失，实现更准确的光谱域解耦；并提出空间-光谱耦合方法，结合解耦的域特定特征减少域偏差。

Result: 实验表明，该方法显著提升了基线性能，并在多个RGB-IR数据集上优于现有UDAOD方法。

Conclusion: 通过解耦-耦合策略，论文提出的SS-DC框架有效解决了RGB-IR域自适应问题，为多子域场景下的目标检测提供了新思路。
...
TL;DR: 提出了一种名为DOV4MM的新方法，用于验证掩码模型是否使用了特定未标记数据集进行预训练，解决了现有技术无法适用于掩码模型的问题。


<details>
  <summary>Details</summary>
Motivation: 高质量开源数据集面临被滥用的风险，现有验证技术主要针对监督模型和对比预训练模型，无法直接应用于掩码模型，因此需要一种新的验证方法。

Method: 基于掩码模型在目标数据集上预训练后，重建掩码信息的难度与其他模型不同的现象，提出DOV4MM方法。

Result: 在ImageNet-1K和WikiText-103上的实验表明，DOV4MM显著优于现有方法，p值远低于0.05。

Conclusion: DOV4MM是首个针对掩码模型的数据集所有权验证方法，有效解决了这一关键问题。
...
TL;DR: 论文提出了一种多变量自回归空气污染物预测模型（MVAR），解决了现有研究中单污染物预测的局限性，并提升了数据利用效率。
...
Method: MVAR模型通过多变量自回归训练范式，减少对长时间窗口输入的依赖，并设计了气象耦合空间变换器模块，结合AI气象预测。

Result: 实验结果表明，MVAR模型优于现有方法，并验证了其架构的有效性。
...
TL;DR: 3D-MoRe是一种新范式，利用基础模型生成大规模3D-语言数据集，显著提升3D场景任务性能。
...
Conclusion: 3D-MoRe有效提升3D场景任务性能，代码和数据集将公开。
...
Method: 提出IICMVNCD框架，通过矩阵分解在视图内捕获已知类和新类的分布相似性，并在视图间利用已知类的关系指导新类聚类。

Result: 实验验证了该方法的有效性。

Conclusion: IICMVNCD首次在多视图数据中实现新类发现，解决了现有方法的局限性。
...
TL;DR: MoViAD是一个模块化库，用于快速访问先进的视觉异常检测（VAD）模型和工具，支持多种场景和部署需求。
...
Method: 提供模块化库，集成多种模型、数据集、工具和优化技术，支持灵活扩展。

Result: MoViAD支持多种场景和部署需求，包括边缘计算和物联网，提供高效模型和评估工具。

Conclusion: MoViAD为工程师和研究人员提供了快速部署和灵活开发的工具，推动了VAD领域的发展。
...
TL;DR: InstructFLIP是一种基于视觉语言模型（VLM）的指令调优框架，通过文本指导增强跨域泛化能力，显著减少训练冗余并提升人脸反欺骗（FAS）的准确性。
...
Result: 在多个域上优于现有最佳模型（SOTA），显著减少训练冗余。
...
TL;DR: MS-DETR框架通过统一学习运动-语义特征，提升视频时刻检索和高光检测任务性能，利用解耦的模态内相关性和任务间相关性，并通过生成策略和对比去噪学习解决数据稀疏问题。
...
Method: 提出MS-DETR框架，编码器解耦运动与语义模态内相关性，解码器利用任务间相关性进行定位与边界划分，并通过生成策略和对比去噪学习解决数据稀疏问题。

Result: 在四个基准测试中，MS-DETR优于现有最优模型。

Conclusion: MS-DETR通过运动-语义特征统一学习和数据增强策略，显著提升了视频时刻检索和高光检测的性能。
...
TL;DR: 论文提出了一种基于规划视角的交通参与者运动预测方法，通过先推理行为意图再预测轨迹的策略，结合逆向强化学习和层次化解码器，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法直接预测未来轨迹，忽略了行为意图的重要性，而行为意图对自动驾驶系统的安全性至关重要。

Method: 采用逆向强化学习（IRL）推理行为意图，生成奖励分布作为空间引导，结合层次化解码器和双向选择性状态空间模型预测轨迹。

Result: 在Argoverse和nuScenes数据集上，方法显著提升了轨迹预测的置信度和性能，优于现有技术。

Conclusion: 通过先推理意图再预测轨迹的策略，结合IRL和层次化解码器，方法在运动预测任务中表现出色，为自动驾驶系统提供了更可靠的预测结果。
...
Result: 在SMOT4SB公开测试集上达到SO-HOTA分数55.205，验证了框架的有效性和先进性。

Conclusion: 该方法在复杂现实SMOT问题中表现出色，代码开源。
...
TL;DR: 提出了一种基于稀疏视图输入的车辆3D重建方法，结合深度图和鲁棒的姿态估计架构，改进了高斯溅射技术，并在多个基准测试中取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法如NeRF和高斯溅射依赖密集输入视图，限制了实际应用。本文旨在解决稀疏视图输入的车辆重建问题。

Method: 通过选择性光度损失（仅用于高置信像素）和改进的DUSt3R姿态估计架构增强高斯溅射技术，并提出了包含合成和真实公共交通工具的新数据集。

Result: 在多个基准测试中表现优异，即使在输入受限条件下也能实现高质量重建。

Conclusion: 该方法在稀疏视图输入下实现了高质量的车辆3D重建，具有实际应用潜力。
...
TL;DR: 论文提出了一种名为DeepShade的扩散模型，用于学习和合成阴影变化，并结合对比学习捕捉阴影的时间变化规律。通过构建包含多样地理区域的模拟阴影数据集，改进了阴影图像的生成性能，并应用于实际路线规划。


<details>
  <summary>Details</summary>
Motivation: 热浪对公共健康构成威胁，但现有路线规划系统缺乏阴影信息。论文旨在解决从卫星图像直接估计阴影的困难以及生成模型训练数据不足的问题。

Method: 1. 构建包含多样地理区域的模拟阴影数据集；2. 提出DeepShade模型，结合RGB和Canny边缘层，利用对比学习捕捉阴影时间变化规律。

Result: DeepShade模型在生成阴影图像方面表现出色，并成功应用于亚利桑那州坦佩的实际路线规划中。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割网络在有限和不完美数据集上容易误分类，OoD数据在其他视觉任务中表现优异，启发作者探索其在医学分割中的应用。

Method: 提出Med-OoD框架，利用OoD数据监督完全监督的医学分割，无需额外资源或网络架构修改。

Result: 实验表明Med-OoD显著减少误分类，在Lizard数据集上性能提升，且仅用OoD数据训练也能达到76.1% mIoU。
...
TL;DR: 提出了一种新方法生成对抗性人脸图像，利用FRS特征空间的结构特性，通过属性子球体实现非适应性和低查询需求，攻击成功率高。


<details>
  <summary>Details</summary>
Motivation: 对抗性攻击对FRS构成严重威胁，现有方法依赖迭代优化或迁移性，效率低且不适用于商业FRS。

Method: 利用FRS特征空间中的属性子球体结构，生成对抗性人脸图像，无需迭代优化或多次查询。

Result: 仅需100张非适应性查询图像，攻击AWS CompareFaces API的成功率超过93%。

Conclusion: 该方法高效且灵活，能生成具有特定属性的对抗性人脸，适用于商业FRS攻击。
...
Method: 提出LidarPainter，利用一步扩散模型从稀疏LiDAR数据和损坏渲染中恢复一致驾驶视图。

Result: 实验显示LidarPainter在速度、质量和资源效率上优于现有方法，速度快7倍且GPU内存需求低。

Conclusion: LidarPainter能高效提升驾驶场景重建质量，并支持多样化风格生成。
...
TL;DR: OVIGo-3DHSG是一种基于3D分层场景图的开源词汇室内物体定位方法，利用RGB-D帧序列和开放词汇基础模型，结合大语言模型进行多步推理，提升空间上下文理解。


<details>
  <summary>Details</summary>
Motivation: 解决复杂查询中涉及空间关系的物体定位问题，并增强室内环境的空间推理能力。

Method: 通过分层场景图建模空间关系（如楼层、房间、位置和物体），并集成大语言模型进行多步推理。
...
TL;DR: BSPF-ViT是一种新的ViT剪枝方法，通过联合优化Q/K令牌的剪枝和融合，显著降低了计算成本并提升了性能。
...
Method: 提出块级对称剪枝与融合（BSPF-ViT），联合评估令牌及其邻居，保留关键令牌并通过相似性融合压缩。

Result: 在DeiT-T和DeiT-S上分别提升1.3%和2.0%的ImageNet分类准确率，计算开销减少50%，速度提升40%。

Conclusion: BSPF-ViT通过考虑令牌交互，在性能和效率上均优于现有方法。
...
TL;DR: 提出了一种结合双边网格和MLP的BPAM框架，解决了现有方法无法处理复杂颜色关系和局部变化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于双边网格的方法仅限于线性变换，而传统MLP方法难以处理局部变化，因此需要一种新方法结合两者的优势。

Method: 通过生成包含MLP参数的双边网格，每个像素动态获取独特的变换参数，并采用网格分解策略和多通道引导图优化参数生成。

Result: 在公开数据集上的实验表明，该方法在性能上优于现有方法，同时保持实时处理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法依赖昂贵的手动标注或自监督方法无法准确捕捉动态对象运动和场景分解，导致渲染伪影。

Method: 结合局部感知B样条曲线和全局感知三角函数的学习运动模型，动态高斯表示对象，双向时间可见性掩模和物理刚性正则化增强鲁棒性。
...
TL;DR: 论文提出了一种无需依赖预训练模型的高性能细粒度图像识别（FGIR）框架TGDA，通过数据增强和知识蒸馏实现任务特定架构设计，显著提升了低分辨率和高分辨率输入下的性能。
...
Result: 在多个FGIR基准测试中，TGDA框架性能优于或匹配现有预训练模型，尤其在低分辨率设置下，LRNets提升准确率23%，参数和训练数据需求大幅减少。
...
<details>
  <summary>Details</summary>
Motivation: 低光图像的复杂退化问题导致现有方法特征高度纠缠，易陷入捷径学习。小波变换能独立处理低频和高频信息，为解耦提供可能。

Method: 使用小波变换将特征空间分解为低频分支（用于光照调整）和多个高频分支（用于纹理增强），并设计高频引导的跨视图交互模块（HF-CIM）和细节纹理增强模块（DTEM）。
...
TL;DR: 提出一种双集成框架，结合预训练深度学习模型和超参数调优的机器学习模型，以提高脑肿瘤MRI诊断的准确性。
...
Method: 采用预训练深度学习模型提取特征，结合超参数调优的机器学习模型进行分类，并进行数据预处理和增强。

Result: 实验表明，特征融合和分类器融合优于现有方法，超参数调优显著提升性能。
...
TL;DR: 论文提出了三种创新技术（分形卷积、自敏感瓷砖填充和超分辨率）用于印度古迹的保护与修复，结合计算机视觉和机器学习，实现了高效且低成本的自动化解决方案。
...
Method: 1. 分形卷积方法用于图像分割，揭示建筑细节；2. 自敏感瓷砖填充（SSTF）结合MosaicSlice数据增强技术；3. 超分辨率技术提升图像质量。

Result: 方法实现了高精度区域填充和瓷砖生成，同时保持低成本与自动化，平衡传统与创新。
...
Conclusion: RODS在不增加额外训练成本的情况下，有效减少了扩散模型采样中的幻觉现象。
...
TL;DR: 论文提出了一种新的伪造检测框架MGFFD-VLM，通过扩展数据集和引入多粒度提示学习等方法，提升了视觉大语言模型在伪造检测和解释性方面的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法未充分利用人脸质量相关属性，且缺乏有效的训练策略，限制了伪造检测和解释性。

Method: 扩展了VQA数据集为DD-VQA+，引入Attribute-Driven Hybrid LoRA策略、多粒度提示学习和伪造感知训练策略。

Result: 实验表明，该方法在文本伪造判断和分析方面优于现有方法，准确性更高。

Conclusion: MGFFD-VLM框架显著提升了伪造检测的性能和解释性。
...
TL;DR: 论文提出了一种基于生成式文本到图像扩散模型的短语定位方法，在医学影像中通过临床报告实现疾病定位，性能优于当前的自监督对比模型。
...
Method: 使用文本到图像扩散模型，结合跨注意力图，并引入Bimodal Bias Merging（BBM）后处理技术优化定位。

Result: 方法在零样本短语定位中表现优异，mIoU分数是当前判别方法的两倍。

Conclusion: 生成式模型在医学影像短语定位中具有显著优势，为临床应用提供了更鲁棒和可解释的解决方案。
...
TL;DR: 该研究开发了一种基于深度学习的模型，利用出生24小时内的胸部X光片预测极低出生体重婴儿的支气管肺发育不良（BPD）结局，模型表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: BPD是一种常见的慢性肺病，预防干预措施存在严重风险，因此早期预测BPD结局对避免不必要的毒性至关重要。

Method: 研究使用163名极低出生体重婴儿的胸部X光片，通过微调ResNet-50模型，采用渐进层冻结和CutMix增强技术，结合线性探测。

Result: 最佳模型的AUROC为0.78，平衡准确度为0.69，F1分数为0.67，显著优于ImageNet初始化和传统IRDS分级。

Conclusion: 研究表明，领域特定的预训练结合渐进冻结和线性探测，能够从常规X光片中准确预测BPD结局，且计算效率高，适合实际应用。
...
TL;DR: 该论文提出了一种自动化工具，用于从视频中识别和分割静态体操技能，并建立了一个标注数据集。


<details>
  <summary>Details</summary>
Motivation: 体操技能的评估需要自动化工具辅助训练和比赛评分，但目前缺乏针对静态技能时间分割的研究。

Method: 构建了一个标注视频数据集，并提出了一种基线方法进行技能时间分割。

Result: 结果显示该方法是可行的，但仍有改进空间。

Conclusion: 研究为体操领域的自动化工具开发迈出了第一步。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习框架在医学图像分类中广泛应用，但不同框架在性能上的比较研究较少。

Method: 通过PathMNIST数据集，对CNN在不同框架（Keras、PyTorch、JAX）中的实现进行全面分析。

Result: 研究揭示了计算速度与模型准确性之间的权衡。

Conclusion: 结果为医学图像分析的研究者和实践者提供了有价值的参考。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面表现出色，但也存在隐私泄露和偏见传播的风险，需要一种有效的方法来移除敏感或有害概念。

Method: FADE结合了轨迹感知的微调策略和对抗性目标，确保概念被可靠移除，同时保持模型整体保真度。

Result: FADE在概念移除性能和图像质量上优于现有方法（如ESD、UCE等），并在移除效果和保真度的调和平均上提升了5-10%。
...
TL;DR: 提出一种基于深度估计和运动员区域检索的健美操技能分类方法，避免了传统姿态估计的高计算成本，显著提升了效率和分类精度。
...
Result: 方法在RGB图像块上推理速度快38.3倍，深度图像块的分类精度提升至0.837（传统方法为0.815）。

Conclusion: 该方法高效、灵活，适用于实时应用，且模块化设计便于未来改进。
...
TL;DR: 本文探讨了扩散模型成功的关键在于输入条件，并提出了离散潜在代码（DLC）作为图像表示，提高了生成质量并支持组合性。


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型输入条件的理想表示应具备的特性：提高样本保真度、易于生成和组合性。

Method: 引入离散潜在代码（DLC），一种基于自监督学习的图像表示，用于扩散模型训练。

Result: DLC显著提高了生成质量，在ImageNet上达到新SOTA，并支持生成超出训练分布的样本。

Conclusion: DLC为扩散模型提供了高效的表示方法，支持组合性和文本到图像生成。
...
<details>
  <summary>Details</summary>
Motivation: 受近期进展启发，旨在结合对比学习和聚类的优势，以提升无监督视觉表示学习的性能。
...
TL;DR: KeyDiff3D是一个无监督的单目3D关键点估计框架，利用预训练的多视角扩散模型生成多视角图像作为监督信号，无需手动标注或多视角校准图像。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的手动标注或多视角校准图像，KeyDiff3D旨在仅用单视角图像实现准确的3D关键点估计。

Method: 利用预训练的多视角扩散模型生成多视角图像，提取2D多视角特征并构建3D特征体积，将隐式3D先验转化为显式3D特征。

Result: 在Human3.6M、Stanford Dogs等数据集上表现出高准确性和泛化能力，并能对扩散模型生成的3D对象进行操控。

Conclusion: KeyDiff3D通过利用扩散模型的几何先验，实现了无需标注的单目3D关键点估计，并支持3D对象操控。
...
TL;DR: 论文研究了通道知识蒸馏（CWD）和掩码生成蒸馏（MGD）在轻量级模型中的应用，以提升实时智能喷洒系统中的杂草检测性能。实验表明，CWD和MGD显著提高了模型的准确率，且不影响模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 精准农业中杂草检测对减少环境影响至关重要，但在资源有限的平台上部署高精度模型仍具挑战性，尤其是区分视觉相似的杂草种类。

Method: 使用YOLO11x作为教师模型，YOLO11n作为参考和学生模型，应用CWD和MGD进行知识蒸馏。实验基于真实数据集，包含甜菜作物和四种杂草。

Result: CWD学生模型在mAP50上提升了2.5%，MGD提升了1.9%。模型在嵌入式设备上验证了实时部署的可行性。
...
Conclusion: Text-SemiSeg框架通过文本驱动的方式显著提升了半监督医学图像分割的效果，为相关领域提供了新思路。
...
TL;DR: 论文提出了一种名为QuRe的方法，通过硬负采样减少假阴性，优化检索结果，并创建了HP-FashionIQ数据集以评估用户满意度。
...
Method: 提出QuRe方法，采用奖励模型目标和硬负采样策略，减少假阴性。

Result: QuRe在FashionIQ和CIRR数据集上达到最优性能，并在HP-FashionIQ数据集上表现出与人类偏好最强的对齐。

Conclusion: QuRe通过优化检索策略和引入新数据集，显著提升了CIR任务的性能和用户满意度。
...
TL;DR: 论文介绍了两个视觉目标检测基准OD-VIRAT Large和OD-VIRAT Tiny，用于提升监控图像中的视觉理解任务，并评估了多种先进目标检测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 开发可靠的监控系统需要真实且多样化的数据集来训练和评估计算机视觉模型，以应对复杂环境中的挑战。

Method: 创建了两个大规模标注数据集（OD-VIRAT Large和OD-VIRAT Tiny），并在这些数据集上测试了RETMDET、YOLOX、RetinaNet、DETR和Deformable-DETR等先进目标检测模型。

Result: OD-VIRAT Large包含8.7百万标注实例，OD-VIRAT Tiny包含288,901标注实例。实验为复杂监控场景下的模型性能提供了新见解。

Conclusion: 该研究为开发更高效和鲁棒的目标检测架构奠定了基础，并填补了现有技术在真实监控场景中的性能评估空白。
...
TL;DR: 提出了一种新的损失函数InterpIoU，通过插值框解决IoU非重叠情况下的梯度问题，并避免传统几何惩罚导致的框扩大问题。进一步提出Dynamic InterpIoU，动态调整插值系数，实验证明其在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有IoU损失函数依赖手工几何惩罚，对小物体优化不佳且可能导致框扩大。

Method: 提出InterpIoU，利用插值框与目标框的IoU作为损失项；进一步提出Dynamic InterpIoU，动态调整插值系数。

Result: 在COCO、VisDrone和PASCAL VOC上表现优于现有方法，尤其在小物体检测中效果显著。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在视频识别中表现出色，但其计算成本高，难以在设备上实时部署。因此，需要一种轻量化的解决方案。

Method: DVFL-Net利用知识蒸馏和时空焦点调制技术，从大型预训练教师模型中提取知识，并采用前向KL散度传递局部和全局上下文信息。

Result: 在多个基准测试中，DVFL-Net在性能和效率之间取得了最佳平衡，表现出低内存占用、低计算量和强准确性。
...
TL;DR: 论文提出了一种深度学习模型RadGazeIntent，用于预测放射科医生在解读医学图像时的意图驱动的注视行为。


<details>
  <summary>Details</summary>
Motivation: 现有模型未能捕捉放射科医生注视背后的意图，而理解这种意图对提高医学图像解读的准确性至关重要。

Method: 采用基于Transformer的架构，处理注视数据的时间和空间维度，将其转化为诊断意图的粗粒度表示。模型基于三个意图标记的数据集（RadSeq、RadExplore、RadHybrid）进行训练。

Result: RadGazeIntent在预测放射科医生注视意图方面优于基线方法，在所有意图标记数据集上表现优异。

Conclusion: RadGazeIntent能够有效建模放射科医生的意图驱动行为，为医学图像解读提供了新的工具。
...
TL;DR: 提出了一种结合交通信号状态和行人意图预测的TA-STGCN模型，显著提高了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有模型未能充分考虑动态交通信号和场景信息，而这对自动驾驶车辆的安全导航至关重要。

Method: 采用时空图卷积网络（TA-STGCN），整合交通信号状态和边界框大小作为关键特征。

Result: 在PIE数据集上，TA-STGCN比基线模型准确率提高了4.75%。

Conclusion: TA-STGCN有效提升了行人意图预测的准确性，适用于复杂城市环境。
...
TL;DR: DAM-QA利用区域感知视觉语言模型DAM的能力，通过多区域视图聚合答案，显著提升了文本密集图像VQA任务的性能。
...
Result: 在六个VQA基准测试中表现优于基线DAM，DocVQA提升7+分，参数更少且性能最佳。

Conclusion: DAM-QA展示了DAM类模型在文本密集VQA任务中的潜力，通过高效使用和集成策略缩小了与通用VLMs的差距。
...
Method: 使用摄像头感知模块（YOLOv11和目标距离估计模型）结合Frenet-Pure Pursuit规划策略。

Result: 在多样化的校园场景中验证了系统的有效性，能够处理多种障碍物。

Conclusion: 该系统为自动驾驶车辆的障碍物避障提供了一种高效且实用的解决方案。
...
Conclusion: SENTINEL是一种高效且无需人工标注的幻觉缓解方法，具有优越性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 理解人类行为对开发安全的驾驶系统至关重要，但目前缺乏一个全面的基准来评估自动驾驶中的人类行为理解。

Method: 提出了MMHU基准，包含57k人类运动片段和1.73M帧数据，涵盖多种来源，并开发了人工参与的注释流程。

Result: 提供了丰富的数据集分析和多任务基准，包括运动预测、运动生成和行为问答。
...
TL;DR: CytoSAE是一种稀疏自编码器，用于医学图像分析，可发现视觉概念并解释模型推理。


<details>
  <summary>Details</summary>
Motivation: 医学影像领域缺乏解释模型推理的工具，稀疏自编码器（SAEs）在视觉领域的成功应用启发了其在医学图像中的探索。

Method: 提出CytoSAE，基于40,000多张外周血单细胞图像训练，适用于多样化和域外数据集。

Result: CytoSAE能识别形态相关概念，生成患者和疾病特异性概念，并在AML亚型分类任务中表现优异。

Conclusion: CytoSAE为医学影像提供了可解释性工具，性能与现有技术相当，同时支持亚细胞级解释。
...
TL;DR: 论文提出PhysX，一种物理基础的3D资产生成方法，包括数据集PhysXNet和生成框架PhysXGen，填补了现有3D生成模型忽视物理属性的空白。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI是否能够作为媒介，推动人与自然关系的转变，实现真正的相互依存。
...
TL;DR: 提出了一种模块化的LLM代理设计，包含感知、记忆和推理组件，适用于多轮游戏环境，无需领域特定工程。


<details>
  <summary>Details</summary>
Motivation: 通过游戏环境作为测试平台，分析模块化设计对动态交互性能的影响，推动通用代理的发展。

Method: 采用经典和现代游戏套件，通过统一工作流评估各模块在不同环境中的表现。

Result: 实验表明，模块化设计显著提升游戏性能，不同模块在不同场景中贡献各异（如记忆在长时谜题中占优，感知在视觉噪声环境中关键）。

Conclusion: 模块化设计有效推动通用代理的进步，游戏环境的熟悉性和普遍性为其提供了理想测试场景。
...
TL;DR: MLLMs作为验证器在复杂任务中存在一致性偏差问题，提出的SGV方法显著提升了验证效果。


<details>
  <summary>Details</summary>
Motivation: 在缺乏明确成功标准的领域（如计算机使用）中，如何扩展AI验证器的能力是一个挑战。MLLMs因其知识和对齐人类偏好的能力成为潜在解决方案。

Method: 提出Self-Grounded Verification (SGV)方法，通过无条件生成和条件生成两步，利用MLLMs自身的采样机制优化验证过程。

Result: SGV显著提升了验证准确性（最高20点）和失败检测率，并在多个任务中实现实时监督，性能超越之前最佳方法48%。

Conclusion: SGV方法有效解决了MLLMs作为验证器的一致性偏差问题，显著提升了其在复杂任务中的表现。
...
TL;DR: ClarifAI是一种结合案例推理和本体驱动的方法，旨在提升AI的透明度和可解释性，适用于高风险的决策环境。
...
Method: 结合案例推理（CBR）和本体驱动的方法，设计理论框架和架构蓝图。

Result: ClarifAI能够显著提升AI系统的可解释性，适用于多领域和高风险环境。

Conclusion: ClarifAI为AI系统的可解释性提供了新途径，有望在关键决策过程中广泛应用。
...
TL;DR: 论文提出了一种名为DPLM的7B参数模型，用于自动化动态规划（DP）问题建模，并通过DualReflect数据生成方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统DP建模需要专家知识，而现有LLM难以直接应用于DP问题，因此需要专门的方法和基准。

Method: 提出DPLM模型和DualReflect数据生成方法，结合前向和后向生成以扩展训练数据。

Result: DPLM在DP-Bench基准上表现优异，超越现有LLM，尤其在复杂问题上。

Conclusion: DualReflect方法展示了前向和后向生成的互补性，为低数据和高数据场景提供了有效解决方案。
...
TL;DR: 综述探讨了基于群体智能的语义相似性文档搜索的最新进展及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 群体智能在人工智能中日益流行，其通过模拟自然行为解决优化问题，尤其在文档搜索领域具有潜力。

Method: 综述分析了群体智能算法在语义相似性文档搜索中的应用。

Result: 总结了群体智能在该领域的有效性及最新发展。
...
TL;DR: 本文提出了一种利用GPU并行计算能力优化深度优先搜索（DFS）的方法，特别是成本受限深度优先搜索（CB-DFS），并扩展了Batch IDA*和Batch BTS算法。实验在3x3魔方和4x4滑块拼图上验证了其高效性。


<details>
  <summary>Details</summary>
Motivation: GPU技术的快速发展为优化经典搜索算法提供了新机会，但目前很少有算法在搜索过程中充分利用GPU。本文旨在填补这一空白。

Method: 提出了一种基于成本受限深度优先搜索（CB-DFS）的方法，结合CPU和GPU的并行计算能力，扩展了Batch IDA*和Batch BTS算法。

Result: 在3x3魔方和4x4滑块拼图上的实验表明，GPU操作可以在DFS中高效批处理。

Conclusion: 该方法成功利用GPU并行计算优化了DFS，同时保持了最优性保证，为未来研究提供了新方向。
...
TL;DR: Aime是一个新型多智能体框架，通过动态反应式规划和执行解决传统框架的局限性，显著提升多智能体系统的适应性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统规划和执行框架在多智能体系统中存在刚性执行、静态能力和低效通信等问题，限制了系统的动态环境适应性。

Method: Aime采用动态规划器、动态执行器工厂和集中式进度管理模块，实现实时策略调整、按需生成特化智能体和全局状态一致性。

Result: 在多个基准测试中，Aime表现优于领域内最先进的智能体，展现出更高的适应性和任务成功率。
...
TL;DR: 论文研究了基于光流的强化学习代理在无人机导航中的应用，发现代理主要关注光流中的不连续区域和大光流区域，行为类似昆虫飞行。


<details>
  <summary>Details</summary>
Motivation: 生物系统（如蜜蜂）在有限感知和计算能力下仍能飞行和避障，启发研究者利用光流训练无人机导航代理。

Method: 使用强化学习训练代理在隧道中导航，仅依赖光流作为感知输入，并分析其注意力模式。

Result: 代理主要关注光流的不连续区域和大光流区域，行为类似昆虫，且在不同代理中表现一致。
...
TL;DR: 论文提出了一种基于拓扑增强的多智能体强化学习方法（TPE-MARL），用于优化混合交通中联网自动驾驶车辆（CAV）的协同决策。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体强化学习中探索-利用权衡的挑战，特别是在高维联合状态-动作空间中。

Method: 构建动态交通流的游戏拓扑张量以压缩高维信息，并基于QMIX算法设计拓扑增强的MARL框架，结合访问计数和智能体互信息。

Result: TPE-MARL在多种交通密度和CAV渗透率下表现出色，平衡了探索与利用，提升了交通效率、安全性和决策合理性。

Conclusion: TPE-MARL在混合和全自动驾驶场景中表现优于或接近人类驾驶员，验证了其有效性。
...
Conclusion: 算法在理论和实践中均表现出色，适用于复杂动态环境。
...
TL;DR: BuildEvo利用大型语言模型（LLM）自动设计高效且可解释的建筑能耗预测启发式方法，结合物理原理，实现了先进的性能和透明性。
...
Method: 通过进化过程引导LLM构建和优化启发式方法，结合建筑特征和运行数据（如Building Data Genome Project 2）。

Result: BuildEvo在基准测试中达到最先进性能，泛化能力更强且预测逻辑透明。

Conclusion: 该研究推动了自动化设计鲁棒、基于物理的启发式方法，为复杂能源系统提供了可信赖的模型。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在复杂棋盘游戏中的战略推理能力，填补其在空间战略推理方面的研究空白。

Method: 采用多阶段训练框架：1）微调合法移动预测；2）融入战略注释；3）通过GRPO强化学习优化决策。
...
### [100] [Recurrent U-Net-Based Graph Neural Network (RUGNN) for Accurate Deformation Predictions in Sheet Material Forming](https://arxiv.org/abs/2507.11547)
*Yingxue Zhao,Qianyi Chen,Haoran Li,Haosu Zhou,Hamid Reza Attar,Tobias Pfaff,Tailin Wu,Nan Li*

Main category: cs.LG

TL;DR: 提出了一种名为RUGNN的图神经网络替代模型，用于预测材料成形过程中的变形场，结合GRU和U-Net机制，显著提升了计算效率和预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于AI的替代模型难以捕捉复杂的3D空间关系，且缺乏排列不变性，因此开发了基于图神经网络的RUGNN模型来解决这些问题。

Method: RUGNN结合了门控循环单元（GRU）建模时间动态，以及受U-Net启发的图上下采样机制处理空间长程依赖，并提出了一种新颖的'节点到表面'接触表示方法。
...
### [101] [SurgeryLSTM: A Time-Aware Neural Model for Accurate and Explainable Length of Stay Prediction After Spine Surgery](https://arxiv.org/abs/2507.11570)
...
Method: 比较传统ML模型（如线性回归、随机森林、SVM、XGBoost）与SurgeryLSTM（带注意力的BiLSTM），使用电子健康记录数据评估性能。

Result: SurgeryLSTM预测准确率最高（R2=0.86），注意力机制提升了可解释性，关键预测因素包括骨病、慢性肾病和腰椎融合。

Conclusion: SurgeryLSTM为脊柱手术住院时间预测提供了高精度和可解释的解决方案，支持临床决策系统的整合。
...
### [102] [Distribution-Free Uncertainty-Aware Virtual Sensing via Conformalized Neural Operators](https://arxiv.org/abs/2507.11574)
...
<details>
  <summary>Details</summary>
Motivation: 在稀疏、噪声或非共位传感器数据的高风险领域中，深度学习的不确定性量化（UQ）是实现安全部署的关键障碍。

Method: CMCO通过将蒙特卡洛dropout与分形保形预测结合在DeepONet架构中，实现空间分辨的不确定性估计。

Result: 在湍流、弹塑性变形和全球宇宙辐射剂量估计三个应用中，CMCO始终达到接近名义的经验覆盖范围。

Conclusion: CMCO为神经算子提供了一个通用、即插即用的UQ解决方案，为科学机器学习奠定了可扩展、泛化和不确定性感知的新基础。
...
### [103] [Einstein Fields: A Neural Perspective To Computational General Relativity](https://arxiv.org/abs/2507.11589)
*Sandeep Suresh Cranganore,Andrei Bodnar,Arturs Berzins,Johannes Brandstetter*

Main category: cs.LG

TL;DR: Einstein Fields是一种神经表示方法，用于压缩计算密集的四维数值相对论模拟为紧凑的隐式神经网络权重。通过建模广义相对论的核心张量场——度规，Einstein Fields能够通过自动微分推导物理量。


<details>
  <summary>Details</summary>
Motivation: 传统数值相对论模拟计算成本高，Einstein Fields旨在提供一种更高效、存储友好且易于使用的方法。

Method: 提出Neural Tensor Fields（神经张量场），将时空几何编码为神经网络表示，动态特性自然涌现。

Result: Einstein Fields在4D时空连续建模、网格无关性、存储效率、导数精度和易用性方面表现出潜力。

Conclusion: Einstein Fields为数值相对论提供了更具扩展性和表达力的方法，并开源了JAX库支持。
...
### [104] [Synthetic Tabular Data Generation: A Comparative Survey for Modern Techniques](https://arxiv.org/abs/2507.11590)
...
Method: 提出了一种基于实际生成目标（如下游应用、隐私保证和数据效用）的新分类法，并设计了基准框架以将技术创新与实际需求对齐。

Result: 综述为未来研究提供了路线图，并为隐私关键环境中实施合成表格数据提供了指南。

Conclusion: 本文通过连接理论基础与实际部署，推动了合成表格数据生成的研究和应用。
...
### [105] [Learning Representations of Event Time Series with Sparse Autoencoders for Anomaly Detection, Similarity Search, and Unsupervised Classification](https://arxiv.org/abs/2507.11620)
*Steven Dillmann,Juan Rafael Martínez-Galarza*

Main category: cs.LG

TL;DR: 论文提出了一种用于事件时间序列的新型张量表示方法，结合稀疏自编码器提取物理意义明确的潜在表示，支持多种下游任务。


<details>
  <summary>Details</summary>
Motivation: 事件时间序列的非结构化和不规则性使其难以用传统技术提取有意义模式，需新方法解决这一问题。

Method: 提出二维和三维张量表示，结合稀疏自编码器学习潜在表示。

Result: 在X射线天文学数据集中成功捕获时间和光谱特征，并分类X射线瞬变事件。

Conclusion: 该框架为跨领域复杂事件时间序列分析提供了灵活、可扩展的通用解决方案。
...
### [106] [Deep Generative Methods and Tire Architecture Design](https://arxiv.org/abs/2507.11639)
...
<details>
  <summary>Details</summary>
Motivation: 解决工业实践中深度生成模型在复杂制造设计任务中的适用性问题。

Method: 评估了五种模型（VAE、GAN、MMVAE、DDPM、MDM）在三种工业场景下的表现，并提出类别修复方法。

Result: 扩散模型整体表现最佳，MDM在分布内表现最好，DDPM在分布外泛化能力更强。

Conclusion: 扩散模型在工业设计任务中具有优势，尤其是DDPM在分布外约束下的表现。
...
### [107] [Tracing the Path to Grokking: Embeddings, Dropout, and Network Activation](https://arxiv.org/abs/2507.11645)
...
<details>
  <summary>Details</summary>
Motivation: 研究神经网络中“grokking”现象（即测试精度延迟提升）的预测方法，以理解其行为机制。

Method: 通过Dropout鲁棒性曲线（DRC）、测试精度方差、神经元稀疏性和嵌入相似性等指标分析模型从记忆到泛化的过渡。

Result: 发现Dropout鲁棒性曲线和测试精度方差在“grokking”期间出现局部最大值，神经元稀疏性减少，嵌入分布趋于双峰。

Conclusion: 提出的指标能有效预测“grokking”行为，并揭示了其与数据集对称性和模型动态的关联。
...
### [108] [ZKP-FedEval: Verifiable and Privacy-Preserving Federated Evaluation using Zero-Knowledge Proofs](https://arxiv.org/abs/2507.11649)
...
Method: 使用零知识证明生成简洁证明，断言本地损失低于预设阈值，无需依赖外部API，实现自包含模块化设计。

Result: 在MNIST和HAR数据集上测试了CNN和MLP模型，评估了计算开销、通信成本和可验证性。

Conclusion: 提出的协议在保护隐私的同时实现了可验证的联邦学习评估，适用于实际应用。
...
### [109] [STAGED: A Multi-Agent Neural Network for Learning Cellular Interaction Dynamics](https://arxiv.org/abs/2507.11660)
...
TL;DR: 论文提出了一种结合深度学习和基于代理的建模（ABM）的方法STAGED，用于模拟细胞间的时空动态交互及其对基因调控网络的影响。


<details>
  <summary>Details</summary>
Motivation: 单细胞技术虽然提升了我们对细胞状态的理解，但传统方法将细胞视为独立数据点，忽略了细胞间的动态交互。空间转录组学提供了细胞组织信息，但需要新的计算方法来学习复杂的细胞动态交互。

Method: STAGED方法将ABM与深度学习结合，使用图ODE网络（GDEs）和注意力机制动态学习基因间的相互作用强度。

Result: 模型能够匹配模拟和空间转录组学数据的连续轨迹，捕捉细胞间和细胞内的相互作用。
...
### [110] [Composing Linear Layers from Irreducibles](https://arxiv.org/abs/2507.11688)
*Travis Pence,Daisuke Yamada,Vikas Singh*

Main category: cs.LG

TL;DR: 论文研究了线性层中的组合结构，提出用Clifford代数和双向量表达线性变换，并通过可微分算法分解为转子，参数效率高，性能媲美基线方法。


<details>
  <summary>Details</summary>
Motivation: 探索大模型中低层几何原语的组合方式，以理解其如何构建更高层功能。

Method: 使用Clifford代数将线性层表达为双向量组合，并设计可微分算法分解为转子，参数复杂度为O(log²d)。

Result: 转子基线性层在LLM注意力层中性能与基线方法（如块Hadamard和低秩近似）相当。

Conclusion: 研究为深度模型中几何原语的组合提供了代数视角，揭示了其构建高层功能的方式。
...
### [111] [The Impact of Coreset Selection on Spurious Correlations and Group Robustness](https://arxiv.org/abs/2507.11690)
*Amaya Dharmasiri,William Yang,Polina Kirichenko,Lydia Liu,Olga Russakovsky*

Main category: cs.LG

TL;DR: 本文研究了核心集选择方法对数据偏见的影响，发现某些方法可能加剧偏见，但并不能保证下游模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 探讨核心集选择方法是否会延续、放大或缓解数据集中的偏见，以及这些方法对下游模型鲁棒性的影响。

Method: 在十个不同的虚假相关性基准上，使用五种评分指标和五种数据选择策略，分析核心集选择对偏见和模型鲁棒性的影响。

Result: 发现基于嵌入的样本评分方法比基于学习动态的方法更不易加剧偏见，但优先选择困难样本的方法并不能可靠地保证下游鲁棒性。

Conclusion: 核心集选择方法需要谨慎设计以避免加剧偏见，且仅降低偏见水平并不足以确保模型鲁棒性。
...
### [112] [Time series classification of satellite data using LSTM networks: an approach for predicting leaf-fall to minimize railroad traffic disruption](https://arxiv.org/abs/2507.11702)
*Hein de Wilde,Ali Mohammed Mansoor Alsahag,Pierre Blanchet*

Main category: cs.LG

TL;DR: 研究利用LSTM网络和卫星数据预测落叶时间，为铁路行业优化落叶缓解措施提供支持。


<details>
  <summary>Details</summary>
Motivation: 英国铁路因落叶每年损失3亿英镑，现有预测方法在可扩展性和可靠性上存在局限，亟需改进。
...
### [113] [Reinforcement Learning from Adversarial Preferences in Tabular MDPs](https://arxiv.org/abs/2507.11706)
*Taira Tsuchiya,Shinji Ito,Haipeng Luo*

Main category: cs.LG

TL;DR: 论文提出了一种基于偏好的MDP框架（PbMDPs），研究了Borda分数下的遗憾下界，并提出了两种算法，分别基于全局优化和策略优化，实现了次线性遗憾。


<details>
  <summary>Details</summary>
Motivation: 研究在偏好反馈而非直接损失观测下的MDP问题，扩展了传统对抗性MDP的框架，适用于更广泛的决策场景。

Method: 1. 建立了Borda分数下PbMDPs的遗憾下界；2. 提出了基于全局优化的算法和策略优化算法，分别适用于已知和未知转移概率的情况。

Result: 1. 证明了PbMDPs的遗憾下界为Ω((H²SK)^{1/3}T^{2/3})；2. 提出的算法实现了O(T^{2/3})的遗憾上界。

Conclusion: 论文为偏好反馈下的MDP问题提供了理论框架和实用算法，扩展了对抗性MDP的研究范围。
...
### [114] [Subgraph Generation for Generalizing on Out-of-Distribution Links](https://arxiv.org/abs/2507.11710)
...
<details>
  <summary>Details</summary>
Motivation: 解决图神经网络在分布外场景中性能下降的问题，并扩展图生成模型的应用范围。
...
### [115] [Globalization for Scalable Short-term Load Forecasting](https://arxiv.org/abs/2507.11729)
*Amirhossein Ahmadi,Hamidreza Zareipour,Henry Leung*

Main category: cs.LG

TL;DR: 本文探讨了电力传输网络中负载预测的全局模型（GFMs）与传统局部模型（LFMs）的对比，提出基于时间序列聚类的方法以解决数据异质性和数据漂移问题。


<details>
  <summary>Details</summary>
Motivation: 传统局部预测模型在泛化性、过拟合、数据漂移和冷启动问题上表现不佳，而全局模型通过全局化和交叉学习提供了更好的解决方案。

Method: 研究了特征转换和目标转换模型，提出基于时间序列聚类的方法（模型基于和加权实例基于）以处理数据异质性。

Result: 实验表明，全局目标转换模型在全局特征和聚类技术支持下表现优于局部模型，而特征转换模型需依赖聚类技术平衡局部与全局动态。

Conclusion: 全局目标转换模型在负载预测中更具优势，而特征转换模型需进一步优化以应对数据异质性。
...
### [116] [Graph Neural Networks Powered by Encoder Embedding for Improved Node Learning](https://arxiv.org/abs/2507.11732)
*Shiyu Chen,Cencheng Shen,Youngser Park,Carey E. Priebe*

Main category: cs.LG

TL;DR: 本文提出了一种基于统计方法（GEE）的图神经网络（GNN）初始特征生成方法，显著提升了GNN的性能和收敛速度。


<details>
  <summary>Details</summary>
Motivation: GNN的性能受限于随机或低信息初始特征表示，导致收敛慢和次优解。本文旨在通过统计方法改进初始特征。

Method: 利用one-hot图编码器嵌入（GEE）生成高质量初始节点特征，并将其集成到GNN框架中（GG）。在无监督和有监督任务中验证其有效性。

Result: 在节点聚类任务中，GG在所有评估的真实数据集上表现最佳，且收敛更快。在节点分类任务中，改进版GG-C进一步提升了性能。

Conclusion: 结构感知的特征初始化对发挥GNN的潜力至关重要，GEE方法为GNN提供了有效的初始特征解决方案。
...
### [117] [Sparse Identification of Nonlinear Dynamics with Conformal Prediction](https://arxiv.org/abs/2507.11739)
*Urban Fasel*

Main category: cs.LG

TL;DR: 论文提出将Conformal Prediction框架与Ensemble-SINDy结合，用于量化SINDy模型的不确定性，并在时间序列预测、模型选择和系数不确定性量化中验证其有效性。
...
Method: 通过Conformal Prediction框架与Ensemble-SINDy结合，提出三种应用：时间序列预测不确定性量化、基于特征重要性的模型选择、以及模型系数不确定性的量化。

Result: 在随机捕食者-猎物动力学和混沌系统中验证了方法的有效性，能够可靠地实现目标覆盖范围，并生成更稳健的不确定性区间。

Conclusion: Conformal Prediction与E-SINDy结合的方法在非高斯噪声下仍能提供可靠的不确定性量化，优于标准E-SINDy估计。
...
### [118] [A Graph-in-Graph Learning Framework for Drug-Target Interaction Prediction](https://arxiv.org/abs/2507.11757)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图神经网络（GNN）的DTI预测方法难以有效整合药物、靶标及其相互作用的多样化特征，限制了预测性能。

Method: 引入GiG模型，将药物和靶标的分子结构图表示为药物-靶标相互作用图中的元节点，结合转导学习和归纳学习，探索其复杂关系。

Result: GiG模型在多个评估指标上显著优于现有方法，证明了整合不同学习范式和交互数据的优势。

Conclusion: GiG框架通过结合分子水平和网络水平的特征，为DTI预测提供了更高效和准确的解决方案。
...
### [119] [Torsional-GFN: a conditional conformation generator for small molecules](https://arxiv.org/abs/2507.11759)
...
<details>
  <summary>Details</summary>
Motivation: 分子构象的稳定生成在药物发现中至关重要，传统方法如分子动力学效率较低，而生成式机器学习方法提供了更高效的替代方案。

Method: Torsional-GFN是一种条件GFlowNet，通过旋转扭转角采样分子构象，仅依赖分子图和局部结构（键长和键角）作为输入。

Result: 实验表明，Torsional-GFN能近似玻尔兹曼分布采样构象，并实现零样本泛化到未见过的键长和键角。

Conclusion: Torsional-GFN为大规模分子系统提供了一种有前景的方法，未来可扩展至生成局部结构。
...
### [120] [Scaling laws for activation steering with Llama 2 models and refusal mechanisms](https://arxiv.org/abs/2507.11771)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）复杂性和能力的提升，较少广泛部署的对齐技术的有效性尚不确定。本文旨在探索CAA在不同模型规模下的表现。

Method: 通过对比对（如从仇恨到爱）在模型的残差流向量空间中寻找理想方向，并在前向传播时将该方向添加到残差流中，直接操纵残差流以更好地控制模型输出。

Result: 1) CAA在早期到中期层最有效；2) CAA效果随模型规模增大而减弱；3) 负向引导在所有模型规模中效果更显著。

Conclusion: CAA的有效性受模型规模和应用层的影响，负向引导更具潜力。
...
### [121] [Predicting Delayed Trajectories Using Network Features: A Study on the Dutch Railway Network](https://arxiv.org/abs/2507.11776)
*Merel Kampere,Ali Mohammed Mansoor Alsahag*

Main category: cs.LG

TL;DR: 该研究使用XGBoost分类器结合拓扑特征预测荷兰铁路网络的延误，发现现有方法在非同步测试中表现有限，需进一步优化。


<details>
  <summary>Details</summary>
Motivation: 荷兰铁路网络是全球最繁忙的之一，延误问题严重。现有研究多关注短期预测，忽视了网络范围的模式，本研究填补了这一空白。

Method: 改进现有方法，集成节点中心性度量，并比较多种分类器（如XGBoost、RandomForest等）以预测延误轨迹。

Result: 结果显示性能有限，尤其在非同步测试中，表明需要更多针对具体场景的优化。
...
### [122] [Enforcing Latent Euclidean Geometry in Single-Cell VAEs for Manifold Interpolation](https://arxiv.org/abs/2507.11789)
*Alessandro Palma,Sergei Rybakov,Leon Hetzel,Stephan Günnemann,Fabian J. Theis*

Main category: cs.LG

TL;DR: FlatVI是一种新的训练框架，通过正则化离散似然变分自编码器的潜在流形，使其更接近欧几里得几何，从而改善单细胞RNA测序数据的建模。
...
Method: FlatVI通过正则化潜在流形，使其更接近欧几里得几何，并优化潜在空间中的直线以近似解码后的单细胞流形上的测地插值。

Result: 在合成数据和实际单细胞RNA测序数据上的实验表明，FlatVI提高了轨迹重建和流形插值的准确性。

Conclusion: FlatVI通过优化潜在流形的几何特性，显著提升了单细胞RNA测序数据的建模效果。
...
### [123] [CLID-MU: Cross-Layer Information Divergence Based Meta Update Strategy for Learning with Noisy Labels](https://arxiv.org/abs/2507.11807)
*Ruofan Hu,Dongyu Zhang,Huayi Zhang,Elke Rundensteiner*

Main category: cs.LG

TL;DR: 论文提出了一种无需干净标签数据集的元学习方法CLID-MU，通过跨层信息差异评估模型性能，并在噪声标签场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有元学习方法依赖干净标签数据集，但实际中难以获取。本文旨在解决无干净标签数据集时的元学习问题。

Method: 提出CLID-MU策略，利用数据结构的跨层一致性（干净样本保持一致性，噪声样本破坏一致性）来指导训练。

Result: 在合成和真实噪声的基准数据集上，CLID-MU优于现有方法。

Conclusion: CLID-MU为无干净标签数据集的噪声标签学习提供了有效解决方案。
...
### [124] [SynCoGen: Synthesizable 3D Molecule Generation via Joint Reaction and Coordinate Modeling](https://arxiv.org/abs/2507.11818)
...
Result: 在无条件小分子图和构象生成中达到最优性能，并在零射击分子连接设计中表现优异。
...
### [125] [MNIST-Gen: A Modular MNIST-Style Dataset Generation Using Hierarchical Semantics, Reinforcement Learning, and Category Theory](https://arxiv.org/abs/2507.11821)
*Pouya Shaeri,Arash Karimi,Ariane Middel*

Main category: cs.LG

TL;DR: MNIST-Gen是一个自动化框架，用于生成针对特定领域的MNIST风格数据集，结合了语义理解和强化学习，显著提高了效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有标准数据集（如MNIST）对领域特定任务不适用，而创建自定义数据集耗时且复杂。

Method: 结合CLIP语义理解、强化学习和人类反馈，采用分层语义分类和可组合的数据转换模型。
...
### [126] [HyperEvent:Learning Cohesive Events for Large-scale Dynamic Link Prediction](https://arxiv.org/abs/2507.11836)
...
Method: 提出HyperEvent框架，动态构建关联序列，利用事件相关性向量量化历史事件与查询事件的依赖关系。

Result: 在5个数据集中的4个上优于现有方法，在大规模Flight数据集上平均倒数排名提升6.95%，训练时间仅需10.17%。

Conclusion: HyperEvent框架在动态链接预测中表现出更高的准确性和效率，适用于大规模图数据。
...
### [127] [Protenix-Mini: Efficient Structure Predictor via Compact Architecture, Few-Step Diffusion and Switchable pLM](https://arxiv.org/abs/2507.11839)
...
<details>
  <summary>Details</summary>
Motivation: 解决生物分子结构预测中模型效率与预测精度之间的平衡问题，适用于资源有限但需要高精度预测的场景。

Method: 1) 用少步ODE采样替代多步AF3采样；2) 剪枝冗余的Transformer模块；3) 用ESM模块替代传统MSA模块。

Result: 在基准数据集上，Protenix-Mini仅损失1-5%的性能，但显著降低了计算复杂度。

Conclusion: Protenix-Mini是资源有限场景下高效且准确的蛋白质结构预测解决方案。
...
### [128] [Generalized Linear Bandits: Almost Optimal Regret with One-Pass Update](https://arxiv.org/abs/2507.11847)
*Yu-Jie Zhang,Sheng-An Xu,Peng Zhao,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 论文提出了一种联合高效的算法，用于解决广义线性老虎机问题，通过在线镜像下降估计器实现近乎最优的遗憾边界，同时保持每轮时间和空间复杂度为常数。


<details>
  <summary>Details</summary>
Motivation: 广义线性老虎机（GLB）问题因其非线性特性在计算和统计效率上存在挑战，现有方法通常需要在两者之间权衡。本文旨在设计一种同时满足高效计算和统计性能的算法。

Method: 提出了一种基于在线镜像下降（OMD）估计器的算法，通过混合损失的新颖分析构建紧密置信集，实现统计效率与最大似然估计相当。

Result: 算法在每轮仅需常数时间和空间复杂度的情况下，达到了近乎最优的遗憾边界。

Conclusion: 通过在线镜像下降估计器和混合损失分析，本文提出的方法在广义线性老虎机问题中实现了计算和统计效率的联合优化。
...
### [129] [OrdShap: Feature Position Importance for Sequential Black-Box Models](https://arxiv.org/abs/2507.11855)
...
<details>
  <summary>Details</summary>
Motivation: 现有特征归因方法假设特征顺序固定，无法区分特征值和位置对预测的影响，限制了模型解释的准确性。

Method: 提出OrdShap方法，通过置换特征位置量化其对预测的影响，并基于博弈论与Sanchez-Bergantiños值建立理论联系。

Result: 在健康、自然语言和合成数据集上的实验表明，OrdShap能有效捕捉特征值和位置的影响，提供更深入的模型行为洞察。

Conclusion: OrdShap为理解序列深度学习模型的预测提供了更精确的特征归因方法，解决了特征值与位置混淆的问题。
...
### [130] [A Policy-Improved Deep Deterministic Policy Gradient Framework for the Discount Order Acceptance Strategy of Ride-hailing Drivers](https://arxiv.org/abs/2507.11865)
*Hanwen Dai,Chang Gao,Fang He,Congyuan Ji,Yanni Yang*

Main category: cs.LG

TL;DR: 研究提出了一种动态管理司机接受折扣快车服务的方法，通过pi-DDPG框架提升学习效率和早期训练性能。


<details>
  <summary>Details</summary>
Motivation: 平台整合虽能缓解市场碎片化，但折扣快车服务可能导致利润下降，需动态管理司机参与行为。

Method: 采用pi-DDPG框架，结合改进模块、卷积LSTM网络和优先经验回放机制。

Result: pi-DDPG在学习效率和减少早期训练损失方面表现优异。

Conclusion: pi-DDPG框架能有效解决动态管理问题，适用于实际部署。
...
### [131] [Imbalanced Regression Pipeline Recommendation](https://arxiv.org/abs/2507.11901)
*Juscimara G. Avelino,George D. C. Cavalcanti,Rafael M. O. Cruz*

Main category: cs.LG

TL;DR: 提出Meta-IR框架，通过元学习推荐最佳的数据重采样和学习模型组合，解决回归任务中的不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 回归任务中目标值稀少导致的不平衡问题，现有方法需测试多种组合，效率低。

Method: 训练元分类器，基于元特征推荐最佳管道（重采样策略和学习模型），提出独立和链式两种形式。

Result: 链式形式表现更优，Meta-IR优于AutoML框架和42种基线配置。

Conclusion: Meta-IR能有效解决回归任务中的不平衡问题，链式形式更优。
...
### [132] [Resampling strategies for imbalanced regression: a survey and empirical analysis](https://arxiv.org/abs/2507.11902)
*Juscimara G. Avelino,George D. C. Cavalcanti,Rafael M. O. Cruz*
...
<details>
  <summary>Details</summary>
Motivation: 解决现实世界中不平衡回归问题，填补分类任务中不平衡问题研究之外的空白。

Method: 通过实验研究多种平衡和预测模型，并提出基于三个关键标准的分类法。

Result: 验证了平衡策略在回归任务中的优势，并提供了进一步研究的方向。

Conclusion: 不平衡回归策略对模型学习过程有显著帮助，未来研究可基于此分类法展开。
...
### [133] [From Generative to Episodic: Sample-Efficient Replicable Reinforcement Learning](https://arxiv.org/abs/2507.11926)
*Max Hopkins,Sihan Liu,Christopher Ye,Yuichi Yoshida*

Main category: cs.LG

TL;DR: 论文研究了可复现强化学习算法的样本效率，填补了生成模型与非生成模型之间的样本复杂度差距。


<details>
  <summary>Details</summary>
Motivation: 解决可复现学习在强化学习中的样本效率问题，探索是否可复现探索比批量学习更昂贵。

Method: 提出了一种可复现强化学习算法，适用于低水平表格MDP，样本复杂度为$	ilde{O}(S^2A)$。

Result: 算法在生成模型和情景设置下均表现优异，样本复杂度接近最优。

Conclusion: 可复现探索并非显著障碍，样本高效的强化学习是可实现的。
...
### [134] [Accelerating RF Power Amplifier Design via Intelligent Sampling and ML-Based Parameter Tuning](https://arxiv.org/abs/2507.11928)
...
Result: 验证显示平均R²为0.901，仿真时间减少58.24%至77.78%。

Conclusion: 该框架通过自动化GUI工作流实现快速设计迭代，同时满足生产级RF电路的精度要求。
...
### [135] [Kevin: Multi-Turn RL for Generating CUDA Kernels](https://arxiv.org/abs/2507.11948)
...
Result: Kevin在生成内核的正确性（从56%提升到82%）和平均加速（从0.53x提升到1.10x）上显著优于基准模型和前沿模型。

Conclusion: 多轮RL在CUDA内核优化中表现出色，尤其是串行细化比并行采样更有效，随着细化轮次增加，性能提升更显著。
...
### [136] [Online Training and Pruning of Deep Reinforcement Learning Networks](https://arxiv.org/abs/2507.11975)
*Valentin Frank Ingmar Guenter,Athanasios Sideris*

Main category: cs.LG

TL;DR: 论文提出了一种在强化学习（RL）中同时训练和剪枝神经网络的方法，通过变分伯努利分布和正则化技术，显著减少计算和内存开销，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在强化学习中性能提升的同时带来了高计算和内存开销，而剪枝方法在监督学习中已成功应用，但在RL中尚未充分探索。

Method: 提出了一种结合训练和剪枝的方法，利用变分伯努利分布和正则化技术，自动选择超参数，优化网络结构。

Result: 在MuJoCo连续控制基准测试中，证明了OFENet可大幅剪枝且性能损失极小，剪枝后的网络比从头训练的小网络更高效且性能更高。

Conclusion: 该方法有效结合了RL目标和网络压缩，为RL中的神经网络剪枝提供了可行方案。
...
### [137] [Can LLMs Find Fraudsters? Multi-level LLM Enhanced Graph Fraud Detection](https://arxiv.org/abs/2507.11997)
*Tairan Huang,Yili Wang*

Main category: cs.LG

TL;DR: MLED是一个多级LLM增强的图欺诈检测框架，通过利用LLM提取文本信息的外部知识，结合图结构信息，提升欺诈检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有图欺诈检测方法忽略原始文本信息的丰富语义线索，且难以将LLM处理的文本嵌入与图结构进行多模态融合。

Method: 设计多级LLM增强框架（类型级和关系级增强器），分别增强欺诈者与良性实体的差异以及欺诈者在不同关系中的重要性。

Result: 在四个真实数据集上，MLED作为通用框架，实现了最先进的图欺诈检测性能。

Conclusion: MLED通过结合LLM与图结构信息，有效提升了欺诈检测能力，可作为现有方法的通用增强框架。
...
### [138] [Detecting In-Person Conversations in Noisy Real-World Environments with Smartwatch Audio and Motion Sensing](https://arxiv.org/abs/2507.12002)
...
Method: 结合机器学习和深度学习方法，融合音频和惯性数据，分析对话中的语言和非语言线索。

Result: 在实验室和半自然环境中分别达到82.0%和77.2%的宏F1分数。

Conclusion: 多模态数据融合在复杂环境中检测对话具有显著优势。
...
### [139] [DUSE: A Data Expansion Framework for Low-resource Automatic Modulation Recognition based on Active Learning](https://arxiv.org/abs/2507.12011)
*Yao Lu,Hongyu Gao,Zhuangzhi Chen,Dongwei Xu,Yun Lin,Qi Xuan,Guan Gui*

Main category: cs.LG

TL;DR: 论文提出了一种名为DUSE的数据扩展框架，通过不确定性评分函数和主动学习策略解决自动调制识别中的数据稀缺问题。
...
Method: DUSE框架使用不确定性评分函数筛选有用样本，并结合主动学习策略持续优化评分器。

Result: 实验表明DUSE在类别平衡和不平衡场景下均优于8种核心集选择基线方法，并展现出对未见模型的强泛化能力。

Conclusion: DUSE能有效解决数据稀缺问题，提升模型性能，具有广泛适用性。
...
### [140] [Granular feedback merits sophisticated aggregation](https://arxiv.org/abs/2507.12041)
*Anmol Kagrecha,Henrik Marklund,Potsawee Manakul,Richard Zeckhauser,Benjamin Van Roy*

Main category: cs.LG

TL;DR: 论文探讨了如何利用有限个体反馈更准确地预测群体反馈分布，发现反馈粒度越高，复杂方法比正则化平均效果更好。


<details>
  <summary>Details</summary>
Motivation: 研究如何在成本限制下，通过小规模个体反馈更准确地估计群体反馈分布，尤其是高粒度反馈的情况。

Method: 比较正则化平均与更复杂的方法在不同反馈粒度下的表现，通过社会态度问题的实证分析验证。

Result: 高粒度反馈（如五点反馈）下，复杂方法仅需正则化平均一半的个体即可达到相同性能，而二元反馈下差异不明显。

Conclusion: 反馈粒度影响预测方法的有效性，高粒度反馈更适合采用复杂方法以提高效率。
...
### [141] [Information-Theoretic Generalization Bounds of Replay-based Continual Learning](https://arxiv.org/abs/2507.12043)
...
<details>
  <summary>Details</summary>
Motivation: 持续学习（CL）在避免灾难性遗忘的同时从顺序任务中获取知识，但现有方法缺乏理论理解，尤其是基于回放的方法。

Method: 建立统一的理论框架，推导信息论界限，分析内存缓冲区与当前任务的交互对泛化的影响，并以SGLD为例验证。

Result: 理论界限表明，有限回放与当前任务数据结合可改善泛化并减少遗忘，预测界限通过低维变量提供更紧的上界。

Conclusion: 实验验证了理论界限的有效性，为基于回放的持续学习方法提供了理论支持。
...
### [142] [FloGAN: Scenario-Based Urban Mobility Flow Generation via Conditional GANs and Dynamic Region Decoupling](https://arxiv.org/abs/2507.12053)
*Seanglidet Yean,Jiazu Zhou,Bu-Sung Lee,Markus Schläpfer*

Main category: cs.LG

TL;DR: 提出了一种基于数据驱动的方法，结合动态区域大小和土地用途原型，利用cGAN生成模拟城市场景中的出行流量。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型依赖历史轨迹，忽视动态因素；机械方法假设静态场景，无法适应未来预测。

Method: 结合动态区域大小和土地用途原型，利用cGAN融合历史数据与动态参数。

Result: 在新加坡手机数据上验证了方法的有效性，并优于现有方法。

Conclusion: 该方法无需大量校准数据或复杂行为建模，能快速生成可调空间粒度的出行流量。
...
### [143] [Emergence of Quantised Representations Isolated to Anisotropic Functions](https://arxiv.org/abs/2507.12070)
*George Bird*

Main category: cs.LG

TL;DR: 论文提出了一种基于Spotlight Resonance方法的新方法，用于确定表征对齐，发现网络原语的代数对称性是任务无关表征结构的强预测因子。


<details>
  <summary>Details</summary>
Motivation: 研究功能形式选择如何无意中引入归纳偏差，导致表征中出现任务无关的人工结构，特别是当代形式导致连续结构的离散化。

Method: 通过改变激活函数进行消融研究，分析离散表征在自编码器模型中的形成和排列。

Result: 发现当激活函数具有离散代数置换等变性时，表征倾向于离散化；而连续代数正交等变性时，表征保持连续。

Conclusion: 功能形式的选择会影响表征结构，可能导致离散化效应，这对下游可解释性研究有重要意义。
...
### [144] [Measuring Informativeness Gap of (Mis)Calibrated Predictors](https://arxiv.org/abs/2507.12094)
*Yiding Feng,Wei Tang*

Main category: cs.LG

TL;DR: 论文提出了一种衡量两个预测模型之间信息差距的框架，称为“信息差距”，并展示了其与现有概念的泛化关系。
...
Method: 引入信息差距的概念，并通过双重表征推导出一种自然的信息度量方法。

Result: 该框架泛化了现有概念，并满足完整性、可靠性和样本高效性。

Conclusion: 信息差距框架为预测模型的选择提供了理论基础和实用工具。
...
### [145] [Self-Adaptive and Robust Federated Spectrum Sensing without Benign Majority for Cellular Networks](https://arxiv.org/abs/2507.12127)
*Ngoc Duy Pham,Thusitha Dayaratne,Viet Vo,Shangqi Lai,Sharif Abuadbba,Hajime Suzuki,Xingliang Yuan,Carsten Rudolph*

Main category: cs.LG

TL;DR: 论文探讨了分布式机器学习（如联邦学习）在频谱感知中的应用，解决了标记数据稀缺和安全漏洞问题，提出了一种半监督联邦学习和新型防御机制。


<details>
  <summary>Details</summary>
Motivation: 无线设备快速增长导致频谱稀缺，动态频谱分配（DSA）成为解决方案，但集中式机器学习方法存在隐私和带宽限制，分布式方法如联邦学习（FL）更具潜力。

Method: 采用半监督联邦学习结合能量检测解决标记数据稀缺问题；提出基于疫苗接种启发的新型防御机制应对数据投毒攻击。

Result: 实验验证了方法在未标记数据集上的高准确性和对数据投毒攻击的鲁棒性。

Conclusion: 联邦学习频谱感知（FLSS）在未标记数据和安全性方面表现优异，为动态频谱分配提供了可行方案。
...
### [146] [HyDRA: A Hybrid Dual-Mode Network for Closed- and Open-Set RFFI with Optimized VMD](https://arxiv.org/abs/2507.12133)
*Hanwen Liu,Yuhe Huang,Yifeng Gong,Yanjie Zhai,Jiaxuan Lu*

Main category: cs.LG

TL;DR: HyDRA是一种混合双模射频架构，结合优化的VMD和新型CNN、Transformer与Mamba融合架构，支持闭集和开集分类任务，实现高效无线设备识别。


<details>
  <summary>Details</summary>
Motivation: 无线通信系统中设备识别对安全至关重要，RFFI提供了一种非加密解决方案。本文旨在通过HyDRA提升识别效率和准确性。

Method: HyDRA结合优化的VMD预处理和CNN、Transformer、Mamba融合架构，采用TDSE和MLFE分别建模全局依赖和线性复杂度处理。

Result: 在公开数据集上，HyDRA在闭集场景中达到SOTA精度，开集分类表现稳健，并在NVIDIA Jetson Xavier NX上实现毫秒级推理速度和低功耗。

Conclusion: HyDRA为实时无线认证提供了高效、实用的解决方案，适用于实际环境。
...
### [147] [RiemannLoRA: A Unified Riemannian Framework for Ambiguity-Free LoRA Optimization](https://arxiv.org/abs/2507.12142)
*Vladimir Bogachev,Vladimir Aletov,Alexander Molozhavenko,Denis Bobkov,Vera Soboleva,Aibek Alanov,Maxim Rakhuba*

Main category: cs.LG

TL;DR: 提出了一种名为RiemannLoRA的新方法，通过将LoRA矩阵视为光滑流形上的元素，解决了低秩矩阵分解中的过参数化和初始化问题，提升了收敛速度和性能。
...
Method: 将固定秩的LoRA矩阵视为光滑流形，通过流形上的优化确定初始化方向，并采用数值稳定和计算高效的实现。

Result: 在LLM和扩散模型上的实验表明，RiemannLoRA在收敛速度和最终性能上优于标准LoRA及其先进变体。
...
### [148] [FourCastNet 3: A geometric approach to probabilistic machine-learning weather forecasting at scale](https://arxiv.org/abs/2507.12144)
...
Conclusion: FourCastNet 3在计算效率、概率技能和光谱保真度上表现优异，适合气象预报和预警系统。
...
### [149] [PRISM: Distributed Inference for Foundation Models at Edge](https://arxiv.org/abs/2507.12145)
*Muhammad Azlan Qazi,Alexandros Iosifidis,Qi Zhang*

Main category: cs.LG

TL;DR: PRISM是一种通信高效且计算感知的策略，用于在边缘设备上部署Transformer模型，显著减少通信和计算开销。
...
Method: 采用Segment Means表示近似中间特征，重构自注意力机制以减少冗余计算，设计分区感知的因果掩码。

Result: 在BERT等模型上，通信开销减少99.2%，计算减少51.24%，精度损失小。
...
### [150] [Multi-Component VAE with Gaussian Markov Random Field](https://arxiv.org/abs/2507.12165)
...
Result: LEOPARD在自然语言处理基准测试中实现了最先进的离散属性非线性擦除性能，并有效减少了深度非线性分类器的偏见。
...
### [152] [RadioDiff-3D: A 3D$\times$3D Radio Map Dataset and Generative Diffusion Based Benchmark for 6G Environment-Aware Communication](https://arxiv.org/abs/2507.12166)
...
<details>
  <summary>Details</summary>
Motivation: 现有无线电地图构建方法局限于2D平面和静态学习范式，忽略了方向、时间和垂直空间变化等关键参数。

Method: 通过射线追踪构建UrbanRadio3D数据集，并提出基于3D卷积的UNet和扩散模型RadioDiff-3D来生成3D无线电地图。

Result: RadioDiff-3D在UrbanRadio3D数据集上表现优异，支持多种环境动态下的高维无线电地图构建。
...
### [153] [Explainable Evidential Clustering](https://arxiv.org/abs/2507.12192)
*Victor F. Lopes de Souza,Karima Bakhti,Sofiane Ramdani,Denis Mottet,Abdelhak Imoussaten*

Main category: cs.LG

TL;DR: 本文探讨了基于Dempster-Shafer理论的证据聚类解释问题，提出了一种通过代表性条件和效用函数构建解释器的方法，并设计了IEMM算法，验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现实数据常存在不确定性和不精确性，传统方法难以处理。证据聚类虽能解决这些问题，但其结果解释尚未充分研究，尤其在医疗等高风险领域。

Method: 提出代表性条件是决策树作为解释器的充要条件，并扩展至部分标记数据。定义证据错误性为解释成本，构建定制解释器，并提出IEMM算法。

Result: 在合成和真实数据上验证了IEMM算法，根据决策者偏好，93%的情况下能提供满意解释。

Conclusion: 通过代表性条件和IEMM算法，实现了对证据聚类结果的解释，满足了高风险领域的需求。
...
### [154] [Selective Quantization Tuning for ONNX Models](https://arxiv.org/abs/2507.12196)
...
<details>
  <summary>Details</summary>
Motivation: 解决全量化模型性能下降和低端硬件部署问题。

Method: 选择性量化ONNX模型，结合多目标优化和性能分析。

Result: 相比全量化模型，精度损失减少54.14%，模型大小减少72.9%。

Conclusion: TuneQn有效优化模型量化，提升部署效率。
...
### [155] [Physics-Informed Linear Model (PILM): Analytical Representations and Application to Crustal Strain Rate Estimation](https://arxiv.org/abs/2507.12218)
*Tomohisa Okazaki*

Main category: cs.LG

TL;DR: 论文提出了一种基于线性基函数组合的物理信息线性模型（PILM），用于解决偏微分方程（PDEs）的正反问题，并在不确定边界条件下验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统方法解决PDEs及其参数估计问题复杂，而物理信息神经网络（PINN）虽然有效但缺乏解析解。PILM旨在提供一种可解析求解的线性模型框架。

Method: PILM通过线性基函数组合表示解，最小化PDE残差、边界条件和数据误差，适用于线性和欠定问题。

Result: PILM在正反问题中表现良好，数学正则化在贝叶斯视角下优于物理正则化。

Conclusion: PILM为线性问题提供了一种解析可解的框架，适用于物理正则化和欠定系统。
...
### [156] [Optimizers Qualitatively Alter Solutions And We Should Leverage This](https://arxiv.org/abs/2507.12224)
...
<details>
  <summary>Details</summary>
Motivation: 早期对DNNs的可行性持怀疑态度，但实践表明其优化动态良好。然而，社区过于关注收敛效率，忽视了优化器对学习解性质的影响。

Method: 通过分析优化器在DNNs中的作用，提出优化器不仅影响收敛速度，还编码了归纳偏见和改变模型表达能力。

Result: 优化器设计可以影响学习解的性质，而不仅仅是收敛速度，这为模型性能提供了新的调控手段。

Conclusion: 呼吁社区关注优化器的偏见和设计，将其视为与架构和数据同等重要的工具，以塑造模型结果。
...
### [157] [Robust Causal Discovery in Real-World Time Series with Power-Laws](https://arxiv.org/abs/2507.12257)
...
Method: 利用时间序列的幂律谱分布特征，提取因果信号，构建鲁棒方法。
...
### [158] [A Framework for Nonstationary Gaussian Processes with Neural Network Parameters](https://arxiv.org/abs/2507.12262)
...
Result: 在多个数据集上表现优于平稳模型和变分推断的分层模型，且能有效恢复非平稳参数。

Conclusion: 该方法灵活、高效，适用于不同非平稳核，且易于扩展到大规数据。
...
### [159] [RegCL: Continual Adaptation of Segment Anything Model via Model Merging](https://arxiv.org/abs/2507.12297)
...
Result: 实验表明，RegCL在多个下游数据集上表现优异，验证了其在动态场景中的有效性。

Conclusion: RegCL通过参数高效的多领域知识整合，提升了模型的持续学习能力。
...
### [160] [PROL : Rehearsal Free Continual Learning in Streaming Data via Prompt Online Learning](https://arxiv.org/abs/2507.12305)
*M. Anwar Ma'sum,Mahardhika Pratama,Savitha Ramasamy,Lin Liu,Habibullah Habibullah,Ryszard Kowalczyk*

Main category: cs.LG

TL;DR: 提出了一种新颖的基于提示的在线持续学习方法，解决了数据隐私约束和参数增长问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在线持续学习（OCL）中数据隐私约束和灾难性遗忘问题，以及现有方法（如记忆重放和提示方法）的局限性。

Method: 包含四个主要组件：轻量级提示生成器、可训练的缩放器和移位器、预训练模型泛化保持、硬软更新机制。
...
### [161] [Thought Purity: Defense Paradigm For Chain-of-Thought Attack](https://arxiv.org/abs/2507.12314)
*Zihao Xue,Zhen Bi,Long Ma,Zhenlin Hu,Yan Wang,Zhenfang Liu,Qing Sheng,Jie Xiao,Jungang Lou*

Main category: cs.LG

TL;DR: 论文提出Thought Purity（TP）防御范式，针对强化学习训练的大型推理模型（LRMs）在Chain-of-Thought（CoT）生成过程中的安全漏洞，通过优化数据处理、强化规则约束和自适应监控，提升安全性和功能性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（如Deepseek-R1）在Chain-of-Thought（CoT）生成过程中易受安全威胁（如后门提示攻击），导致推理机制被破坏，亟需解决安全与性能的双重脆弱性。

Method: 提出Thought Purity（TP）防御范式，包含三个协同组件：安全优化的数据处理管道、强化学习增强的规则约束和自适应监控指标。

Result: TP成为首个针对CoTA漏洞的全面防御机制，显著提升了安全性与功能性的平衡。

Conclusion: TP为下一代AI架构的安全与性能提供了有效解决方案，推动了强化学习对齐推理系统的安全性发展。
...
### [162] [Heat Kernel Goes Topological](https://arxiv.org/abs/2507.12380)
...
<details>
  <summary>Details</summary>
Motivation: 拓扑神经网络虽然强大，但高阶消息传递计算成本高，需要一种更高效的方法。
...
### [163] [Improving Reinforcement Learning Sample-Efficiency using Local Approximation](https://arxiv.org/abs/2507.12383)
...
<details>
  <summary>Details</summary>
Motivation: 现有文献中的样本复杂度界限不够精确，研究旨在通过状态间的距离相关性优化样本复杂度。

Method: 通过构建原始MDP的子集近似MDP，减少样本复杂度至O(SA log A)，并扩展到无模型设置。

Result: 样本复杂度显著降低，实验验证了算法的优越性。

Conclusion: 提出的方法在样本复杂度上优于现有工作，适用于无限时间MDP和无模型设置。
...
### [164] [Trustworthy Tree-based Machine Learning by $MoS_2$ Flash-based Analog CAM with Inherent Soft Boundaries](https://arxiv.org/abs/2507.12384)
...
<details>
  <summary>Details</summary>
Motivation: 人工智能的快速发展引发了对可信赖性的担忧，尤其是树模型在可解释性和鲁棒性方面的挑战。传统方法因硬件性能不足和对抗攻击脆弱性而受限。

Method: 采用$MoS_2$闪存模拟CAM，结合软边界设计，实现高效的软树模型推理。

Result: 在WDBC数据库上达到96%的准确率，MNIST数据集在10%设备阈值变化下仅下降0.6%，显著优于传统决策树。
...
### [165] [ROC-n-reroll: How verifier imperfection affects test-time scaling](https://arxiv.org/abs/2507.12399)
*Florian E. Dorner,Yatong Chen,André F. Cruz,Fanny Yang*

Main category: cs.LG

TL;DR: 本文研究了测试时扩展（test-time scaling）中验证器不完美对性能的影响，揭示了验证器的ROC曲线几何特性如何精确影响实例级准确率。
...
Method: 通过分析验证器的ROC曲线几何特性，理论推导了Best-of-N和拒绝采样的性能表现。

Result: 拒绝采样在固定计算量下优于Best-of-N，但在无限计算量下两者性能趋同，由ROC曲线原点斜率决定。

Conclusion: 验证器的ROC曲线几何特性是决定测试时扩展性能的关键因素。
...
### [166] [NOCTA: Non-Greedy Objective Cost-Tradeoff Acquisition for Longitudinal Data](https://arxiv.org/abs/2507.12412)
*Dzung Dinh,Boqi Chen,Marc Niethammer,Junier Oliva*

Main category: cs.LG

TL;DR: NOCTA是一种非贪婪目标成本权衡获取方法，用于在资源受限的预测任务中动态选择最具信息量的特征，同时考虑时间动态和获取成本。


<details>
  <summary>Details</summary>
Motivation: 在医疗等关键应用中，资源限制使得预测任务需要高效获取信息，同时避免不必要的成本或风险。

Method: 提出NOCTA方法，包括非参数（NOCTA-NP）和参数（NOCTA-P）两种互补的估计器，用于动态选择特征。

Result: 在合成和真实医疗数据集上，NOCTA优于现有基线方法。

Conclusion: NOCTA能有效解决资源受限下的动态特征选择问题，具有实际应用价值。
...
### [167] [Mixture of Raytraced Experts](https://arxiv.org/abs/2507.12419)
*Andrea Perin,Giacomo Lagomarsini,Claudio Gallicchio,Giuseppe Nuti*

Main category: cs.LG

TL;DR: 提出了一种动态选择专家序列的混合专家架构（Mixture of Raytraced Experts），通过可变宽度和深度的计算图逐步提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有混合专家架构通常对每个样本的计算量固定，无法动态调整。本文旨在设计一种能够逐步提升精度的动态计算架构。

Method: 采用类似循环神经网络的训练方式，迭代采样候选专家序列，无需负载均衡机制。
...
### [168] [Targeted Deep Architectures: A TMLE-Based Framework for Robust Causal Inference in Neural Networks](https://arxiv.org/abs/2507.12435)
...
<details>
  <summary>Details</summary>
Motivation: 现代深度神经网络在预测方面强大，但在因果参数（如处理效应或生存曲线）的推断上缺乏有效性。现有方法要么无法保证解决高效影响函数方程，要么计算成本高。

Method: TDA通过冻结大部分参数并迭代更新一小部分“目标”参数，利用目标梯度消除一阶偏差，支持多维度因果估计。

Result: 在IHDP数据集和模拟生存数据上，TDA减少了偏差并提高了覆盖率，优于标准神经网络估计器和现有后处理方法。

Conclusion: TDA为复杂多参数目标的深度架构提供了直接、可扩展的严格因果推断路径。
...
TL;DR: 论文提出了一种基于经济激励的轻量级防御机制，通过贝叶斯博弈设计，使恶意行为在经济上不理性，从而保护联邦学习免受数据投毒攻击。


<details>
  <summary>Details</summary>
Motivation: 联邦学习的开放参与性使其容易受到数据投毒攻击，现有防御方法多为被动且计算昂贵，且通常假设大多数参与者诚实。

Method: 设计了一个轻量级的贝叶斯激励机制，将每轮训练建模为不完全信息的贝叶斯博弈，服务器通过小型验证数据集验证更新质量并发放奖励。

Result: 在MNIST和FashionMNIST的非独立同分布数据上，即使50%的标签翻转攻击下，机制仍能保持96.7%的准确率，显著优于标准FedAvg。

Conclusion: 该机制计算轻量、预算可控，易于集成到现有联邦学习框架中，为构建经济上稳健和可持续的联邦学习生态系统提供了实用方案。
...
### [170] [Cost-aware Stopping for Bayesian Optimization](https://arxiv.org/abs/2507.12453)
...
Method: 提出一种成本感知停止规则，与PBGI和log EI per cost获取函数结合，提供理论成本保证。

Result: 实验表明，该规则与PBGI结合在成本调整简单后悔指标上表现优于其他方法。

Conclusion: 该停止规则在高成本优化任务中具有实际应用价值。
...
### [171] [Real-Time Cloth Simulation Using WebGPU: Evaluating Limits of High-Resolution](https://arxiv.org/abs/2507.11794)
*Nak-Jun Sung,Jun Ma,TaeHeon Kim,Yoo-joo Choi,Min-Hyung Choi,Min Hong*

Main category: cs.GR

TL;DR: WebGPU显著提升了实时布料模拟的性能，优于WebGL，支持高分辨率模拟和实时碰撞处理。
...
Method: 基于WebGPU框架实现质量-弹簧方法的布料模拟系统，集成碰撞检测和响应处理。

Result: WebGPU在高分辨率模拟中保持60fps，支持实时处理4K至100K节点的碰撞。

Conclusion: WebGPU在实时布料模拟中表现优异，平衡了性能与真实感渲染。
...
### [172] [Measuring and predicting visual fidelity](https://arxiv.org/abs/2507.11857)
*Benjamin Watson,Alinda Friedman,Aaron McGaffey*

Main category: cs.GR

TL;DR: 研究测量和预测视觉保真度的技术，使用多边形模型作为视觉刺激，通过两种简化算法和两种对象类型（动物和人造物）进行实验，比较三种测量方法（命名时间、评分和偏好）的效果，并评估自动预测技术的表现。
...
Method: 使用两种模型简化算法处理多边形模型，分为动物和人造物两类，通过命名时间、评分和偏好三种实验方法测量保真度变化，并尝试用基于图像和模型的自动技术预测实验结果。

Result: 实验测量方法对简化类型和程度敏感，但对对象类型反应不同；自动预测技术在评分上表现较好，偏好次之，命名时间预测失败。

Conclusion: 提出改进实验和自动测量技术的建议，以提升视觉保真度评估的准确性。
...
### [173] [MOSPA: Human Motion Generation Driven by Spatial Audio](https://arxiv.org/abs/2507.11949)
...
Result: MOSPA能根据不同的空间音频输入生成多样且真实的人体运动，并在实验中达到最先进的性能。

Conclusion: 论文填补了空间音频驱动运动建模的空白，并开源了模型和数据集。
...
### [174] [HPR3D: Hierarchical Proxy Representation for High-Fidelity 3D Reconstruction and Controllable Editing](https://arxiv.org/abs/2507.11971)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D表示方法（如网格、体素、点云和NeRF）在通用性、编辑性和渲染质量上存在显著不足，无法同时满足重建、生成、编辑和驱动等任务的需求。

Method: 通过稀疏的分层组织（树状结构）代理节点表示物体的形状和纹理，每个节点存储局部形状和纹理信息，并通过轻量级解码实现高效查询。

Result: 实验表明，该方法在3D重建和编辑中表现出高效表达能力、高保真渲染质量和卓越的可编辑性。

Conclusion: 提出的3D分层代理节点表示方法在通用性、编辑性和渲染质量上优于现有方法，具有广泛应用潜力。
...
### [175] [SmokeSVD: Smoke Reconstruction from A Single View via Progressive Novel View Synthesis and Refinement with Diffusion Models](https://arxiv.org/abs/2507.12156)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图下动态流体重建因3D信息不足而困难，现有方法优化耗时且受限于不适定条件。

Method: 结合扩散模型生成侧视图，迭代优化多视角图像序列，利用Navier-Stokes方程重建精细密度和速度场。

Result: 实验表明，SmokeSVD在重建质量上优于现有技术。

Conclusion: SmokeSVD通过生成模型与物理约束的结合，实现了高效且高质量的动态烟雾重建。
...
### [176] [Shape Adaptation for 3D Hairstyle Retargeting](https://arxiv.org/abs/2507.12168)
*Lu Yu,Zhong Ren,Youyi Zheng,Xiang Chen,Kun Zhou*

Main category: cs.GR

TL;DR: 提出了一种自动形状适应方法，用于将3D发型重定向到新角色上，通过多尺度策略和约束优化解决高分辨率发型的复杂几何问题。


<details>
  <summary>Details</summary>
Motivation: 在游戏和VR应用中，为角色设计发型是一项复杂且耗时的任务，需要保留发型的几何和空间关系。

Method: 将发型适应过程建模为约束优化问题，采用多尺度策略从粗到细计算发丝位置，并引入发际线编辑工具。

Result: 通过定量和定性实验验证了方法的有效性，能够处理多种发型和角色。

Conclusion: 该方法为发型重定向提供了一种高效且自动化的解决方案，支持用户定制。
...
### [177] [Subjective Evaluation Profile Analysis of Science Fiction Short Stories and its Critical-Theoretical Significance](https://arxiv.org/abs/2507.11582)
*Kazuyoshi Otsuka*

Main category: cs.CL

TL;DR: 研究将大语言模型（LLMs）作为“主观文学评论家”，探讨其在文学评估中的美学偏好和评价模式。通过分析十篇日本科幻短篇小说的翻译文本，发现LLMs的评价存在显著差异和独特模式。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在文学评估中的表现，揭示其是否具有类似人类批评家的个体化评价特征。

Method: 使用六种先进LLMs对十篇日本科幻短篇小说进行七次独立评估，结合主成分分析和聚类技术分析评价模式。

Result: 发现评价一致性差异显著（α值1.00至0.35），识别出五种独特评价模式，且不同模型的评价词汇差异明显。
...
### [178] [MapIQ: Benchmarking Multimodal Large Language Models for Map Question Answering](https://arxiv.org/abs/2507.11625)
...
<details>
  <summary>Details</summary>
Motivation: 当前Map-VQA研究主要局限于等值区域图（choropleth maps），覆盖的主题和视觉分析任务有限，需要更全面的评估基准。

Method: 提出MapIQ数据集，包含14,706个问答对，涵盖三种地图类型和六个主题，评估多种MLLMs在六种视觉分析任务中的表现，并与人类基准对比。

Result: 实验表明MLLMs在不同地图类型和任务中的表现差异，并揭示了模型对地图设计变化的敏感性和依赖内部地理知识的程度。
...
### [179] [Cross-lingual Few-shot Learning for Persian Sentiment Analysis with Incremental Adaptation](https://arxiv.org/abs/2507.11634)
...
Method: 采用三种预训练多语言模型（XLM-RoBERTa、mDeBERTa和DistilBERT），通过少样本和增量学习方法在波斯语数据上进行微调。
...
### [180] [Partitioner Guided Modal Learning Framework](https://arxiv.org/abs/2507.11661)
*Guimin Hu,Yi Xin,Lijie Hu,Zhihong Zhu,Hasti Seifi*

Main category: cs.CL

TL;DR: PgM框架通过分区器将多模态表示分为单模态和配对模态特征，分别学习并灵活调整，适用于多种任务。
...
Method: 提出PgM框架，包含模态分区器、单模态学习器、配对模态学习器和解码器，分别处理特征。

Result: 实验证明PgM在四种多模态任务中有效，并具有迁移性。

Conclusion: PgM能全面学习特征并灵活调整，为多模态任务提供新思路。
...
### [181] [ExpliCIT-QA: Explainable Code-Based Image Table Question Answering](https://arxiv.org/abs/2507.11694)
...
### [182] [CRABS: A syntactic-semantic pincer strategy for bounding LLM interpretation of Python notebooks](https://arxiv.org/abs/2507.11742)
...
Method: 提出CRABS方法，结合浅层语法分析（如AST解析）和LLM的零样本学习，捕获笔记本的输入输出集，并通过LLM解决剩余歧义。

Result: 在50个Kaggle笔记本上测试，LLM解决了98%的歧义，CRABS在识别信息流和依赖关系上的F1分数分别达到98%和99%。
...
### [183] [AI Wizards at CheckThat! 2025: Enhancing Transformer-Based Embeddings with Sentiment for Subjectivity Detection in News Articles](https://arxiv.org/abs/2507.11764)
*Matteo Fasulo,Luca Babboni,Luca Tedeschini*

Main category: cs.CL

TL;DR: AI Wizards团队在CLEF 2025 CheckThat! Lab Task 1中，通过结合情感分数与句子表示，提升了基于Transformer的分类器在主观性检测任务中的性能，尤其在零样本和多语言场景下表现突出。


<details>
  <summary>Details</summary>
Motivation: 研究旨在改进新闻文章中句子主观性检测的性能，特别是在多语言和零样本设置下，以应对不同语言的类不平衡问题。

Method: 通过将情感分数（来自辅助模型）与句子表示结合，增强Transformer分类器（如mDeBERTaV3-base、ModernBERT-base和Llama3.2-1B），并使用决策阈值校准处理类不平衡。

Result: 情感特征显著提升了性能，特别是在主观F1分数上，团队在希腊语任务中排名第一（Macro F1 = 0.51）。

Conclusion: 结合情感特征的框架在多语言和零样本主观性检测任务中表现优异，为未来研究提供了有效方法。
...
### [184] [Tracing Facts or just Copies? A critical investigation of the Competitions of Mechanisms in Large Language Models](https://arxiv.org/abs/2507.11809)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是验证和调和三项近期研究（Ortu等人、Yu、Merullo和Pavlick以及McDougall等人）的发现，这些研究通过机制解释工具探讨了模型学习事实与矛盾上下文信息之间的竞争。

Method: 研究方法包括分析注意力头强度与事实输出比率的关系，评估关于注意力头抑制机制的竞争性假设，并研究这些注意力模式的领域特异性。

Result: 研究结果表明，促进事实输出的注意力头通过通用复制抑制而非选择性反事实抑制发挥作用，且其行为具有领域依赖性，较大模型表现出更专业和类别敏感的模式。

Conclusion: 结论是注意力头的行为机制和领域依赖性对LLMs处理竞争性信息具有重要影响，为未来研究提供了新的方向。
...
### [185] [ILID: Native Script Language Identification for Indian Languages](https://arxiv.org/abs/2507.11832)
...
<details>
  <summary>Details</summary>
Motivation: 语言识别是NLP的基础任务，尤其在嘈杂、简短和混合代码的环境中更具挑战性，特别是对于印度语言，它们具有相似性但又有显著差异。

Method: 论文创建了一个新数据集，并采用机器学习和深度学习的最新方法开发了基线模型。

Result: 基线模型在语言识别任务中表现与最先进模型相当。

Conclusion: 该研究为印度语言识别提供了数据集和基线模型，推动了该领域的研究。
...
### [186] [Your LLM Knows the Future: Uncovering Its Multi-Token Prediction Potential](https://arxiv.org/abs/2507.11851)
...
### [187] [Cross-Domain Transfer and Few-Shot Learning for Personal Identifiable Information Recognition](https://arxiv.org/abs/2507.11862)
...
<details>
  <summary>Details</summary>
Motivation: 探索PII识别的跨领域模型迁移、数据融合和样本高效学习方法，以提升文本匿名化的准确性。

Method: 使用医疗、法律和传记领域的标注语料库，评估模型在域内性能、跨领域迁移性、融合和少样本学习四个维度的表现。

Result: 法律领域数据对传记文本迁移效果好，医疗领域迁移效果差；融合效果因领域而异；低专业化领域仅需10%训练数据即可实现高质量识别。

Conclusion: 跨领域迁移、数据融合和样本高效学习对PII识别的效果因领域而异，需根据领域特点选择合适方法。
...
### [188] [COLA-GEC: A Bidirectional Framework for Enhancing Grammatical Acceptability and Error Correction](https://arxiv.org/abs/2507.11867)
*Xiangyu Yang,Xinying Qiu*

Main category: cs.CL

TL;DR: COLA-GEC框架通过双向知识转移提升语法错误修正（GEC）和语法可接受性判断（COLA）任务，实现多语言性能提升。


<details>
  <summary>Details</summary>
Motivation: GEC和COLA任务共享语法知识但独立发展，需探索其协同优化潜力。

Method: 双向框架：1）用GEC数据增强语法可接受性模型；2）通过动态损失函数将语法可接受性信号融入GEC训练。

Result: 在多语言基准测试中取得最优结果，但标点错误修正仍有挑战。
...
### [189] [DualReward: A Dynamic Reinforcement Learning Framework for Cloze Tests Distractor Generation](https://arxiv.org/abs/2507.11875)
...
### [190] [LLMs Encode Harmfulness and Refusal Separately](https://arxiv.org/abs/2507.11878)
*Jiachen Zhao,Jing Huang,Zhengxuan Wu,David Bau,Weiyan Shi*

Main category: cs.CL

TL;DR: 研究发现LLMs内部对有害性的理解与拒绝行为是分离的，提出了‘有害性方向’概念，并开发了‘Latent Guard’作为安全应用。
...
Method: 识别有害性方向，分析其与拒绝方向的差异，并通过实验验证。

Result: 有害性方向独立于拒绝方向，某些越狱方法仅降低拒绝信号而不改变有害性判断。
...
### [191] [Marco-Bench-MIF: On Multilingual Instruction-Following Capability of Large Language Models](https://arxiv.org/abs/2507.11882)
...
<details>
  <summary>Details</summary>
Motivation: 现有指令跟随评测数据集多为英语或简单机器翻译，无法满足多语言需求。

Method: 通过结合翻译与验证的混合流程，创建了本地化的多语言评测基准。

Result: 评测发现高低资源语言间存在25-35%的准确率差距，模型规模影响性能45-60%，机器翻译数据低估准确率7-22%。

Conclusion: Marco-Bench-MIF揭示了多语言指令跟随的挑战，如关键词一致性和跨语言约束遵循，并提供了公开可用的评测工具。
...
### [192] [A Survey of Deep Learning for Geometry Problem Solving](https://arxiv.org/abs/2507.11936)
...
<details>
  <summary>Details</summary>
Motivation: 几何问题解决是数学推理的关键领域，涉及教育、人工智能能力评估等多领域。深度学习的发展推动了该领域的研究热潮。

Method: 综述了深度学习在几何问题解决中的应用，包括任务分类、方法回顾、评估指标分析。

Result: 提供了深度学习在几何问题解决中的全面参考，并创建了持续更新的论文列表。

Conclusion: 本文旨在促进几何问题解决领域的进一步发展，并提供了未来研究方向。
...
### [193] [POLYCHARTQA: Benchmarking Large Vision-Language Models with Multilingual Chart Question Answering](https://arxiv.org/abs/2507.11939)
*Yichen Xu,Liangyu Chen,Liang Zhang,Wenxuan Wang,Qin Jin*

Main category: cs.CL

TL;DR: PolyChartQA是首个大规模多语言图表问答基准，涵盖10种语言的22,606张图表和26,151个问答对，旨在解决现有基准的英语中心问题。


<details>
  <summary>Details</summary>
Motivation: 现有图表理解基准主要基于英语，限制了全球受众的可访问性和适用性。

Method: 采用解耦管道，分离图表数据和渲染代码，通过翻译数据和重用代码生成多语言图表，并利用LLM翻译和严格质量控制。
...
### [194] [BlockBPE: Parallel BPE Tokenization](https://arxiv.org/abs/2507.11941)
*Amos You*
...
<details>
  <summary>Details</summary>
Motivation: 现有CPU实现的BPE在批量推理中效率低下，无法充分利用GPU并行能力。
...
Conclusion: BlockBPE显著提升了BPE在GPU上的效率，适合高吞吐量应用。
...
### [195] [DAC: A Dynamic Attention-aware Approach for Task-Agnostic Prompt Compression](https://arxiv.org/abs/2507.11942)
...
Result: 在多个领域和任务中实验表明，DAC能显著提升性能。

Conclusion: DAC是一种高效的任务无关提示压缩方法。
...
### [196] [IAM: Efficient Inference through Attention Mapping between Different-scale LLMs](https://arxiv.org/abs/2507.11953)
*Yi Zhao,Zuchao Li,Hai Zhao*

Main category: cs.CL

TL;DR: IAM框架通过利用不同规模LLM间注意力矩阵的高相似性，优化注意力计算和KV缓存使用，实现了15%的预填充加速和22.1%的KV缓存减少，且不影响性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在长上下文中的资源消耗问题严重，现有方法主要依赖模型内部稀疏性，未充分利用外部信息优化。

Method: 通过分析注意力矩阵相似性、选择映射层并验证一致性，提出IAM框架，实现小规模与大规模LLM间的注意力映射。

Result: IAM在实验中实现了15%的预填充加速和22.1%的KV缓存减少，且性能无明显下降。
...
### [197] [The benefits of query-based KGQA systems for complex and temporal questions in LLM era](https://arxiv.org/abs/2507.11954)
...
Conclusion: 基于查询的多阶段框架能够有效提升多跳和时间问题的问答性能。
...
### [198] [PoTPTQ: A Two-step Power-of-Two Post-training for LLMs](https://arxiv.org/abs/2507.11959)
...
<details>
  <summary>Details</summary>
Motivation: LLM部署因计算资源需求高而困难，现有PoT量化在GPU上效率不足。

Method: 采用两步后训练算法：初始化量化尺度并校准优化。

Result: 在2-和3-bit格式下超越现有整数量化，GPU上推理速度显著提升。
...
### [199] [Toxicity-Aware Few-Shot Prompting for Low-Resource Singlish Translation](https://arxiv.org/abs/2507.11966)
*Ziyu Ge,Gabriel Chua,Leanne Tan,Roy Ka-Wei Lee*

Main category: cs.CL

TL;DR: 提出了一种两阶段框架，用于在低资源语言对中保留毒性内容的翻译，以新加坡英语为例，通过人工验证的少样本提示工程和模型优化，提升了翻译质量和安全性。


<details>
  <summary>Details</summary>
Motivation: 在线交流中低资源语言和方言的翻译系统常无法保留本地俚语、混合语和文化嵌入的有害言论标记，且缺乏平行数据和安全性过滤。

Method: 1. 人工验证的少样本提示工程，筛选和排序新加坡英语目标示例；2. 通过语义相似度基准测试优化模型-提示对。

Result: 定量人工评估验证了框架的有效性和效率，提升了翻译质量并支持文化敏感的审核。

Conclusion: 该框架强调了在内容审核和区域平台治理中保留社会语言细微差别的重要性，为包容性NLP提供了测试平台。
...
### [200] [Graph Representations for Reading Comprehension Analysis using Large Language Model and Eye-Tracking Biomarker](https://arxiv.org/abs/2507.11972)
...
Method: 使用基于LLM的AI代理将文本分组为节点和边，构建图结构，并通过眼动数据验证重要节点和边的注视分布。

Result: LLMs在图拓扑结构层面表现出高度一致的语言理解能力。

Conclusion: 研究结果支持人机协同学习的有效性，并为未来优化提供了方向。
...
### [201] [Value-Based Large Language Model Agent Simulation for Mutual Evaluation of Trust and Interpersonal Closeness](https://arxiv.org/abs/2507.11979)
...
<details>
  <summary>Details</summary>
Motivation: 探索价值相似性在人工社会（由LLM代理组成）中是否与人类社会一样影响关系建立。

Method: 通过两个实验：初步实验评估LLM中价值的可控性；主实验生成具有特定价值的代理对，分析对话后的信任和亲密感。

Result: 价值相似性高的代理对表现出更强的信任和亲密感，且结果在英语和日语中一致。

Conclusion: LLM代理模拟可作为社会科学理论的有效测试平台，揭示了价值影响关系建立的机制，并为新理论提供了基础。
...
### [202] [Simplifications are Absolutists: How Simplified Language Reduces Word Sense Awareness in LLM-Generated Definitions](https://arxiv.org/abs/2507.11981)
...
<details>
  <summary>Details</summary>
Motivation: 探讨简化定义对不同目标群体（如儿童或语言学习者）的影响，尤其是多义词定义可能因简化而丢失关键信息，误导用户。

Method: 使用两种新的多语言评估数据集，测试多个LLM模型（如DeepSeek v3、Llama 4 Maverick等），结合LLM-as-Judge和人工标注。通过微调Llama 3.1 8B模型优化结果。

Result: 简化定义会显著降低多义词定义的完整性，而微调模型能显著提升定义质量。
...
### [203] [Improving Data and Parameter Efficiency of Neural Language Models Using Representation Analysis](https://arxiv.org/abs/2507.12004)
*Josip Jukić*

Main category: cs.CL

TL;DR: 该论文针对神经语言模型的数据和参数效率问题，提出基于表示平滑性的新优化技术，结合主动学习和参数高效微调，显著提升性能、稳定性和效率。
...
Conclusion: 论文提出的技术有效提升了语言模型的数据和参数效率，增强了模型的适应性和鲁棒性。
...
### [204] [A Comparative Approach to Assessing Linguistic Creativity of Large Language Models and Humans](https://arxiv.org/abs/2507.12039)
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在比较人类和LLMs在语言创造力方面的表现，探索两者的差异。

Method: 通过多种任务测试人类和LLMs的创造力，使用OCSAI工具自动评估答案的原创性、精细度和灵活性。

Result: LLMs在所有评估标准中表现优于人类，并在八项任务中的六项中表现更好。人类更倾向于扩展性创造力，而LLMs偏向固定创造力。

Conclusion: LLMs在语言创造力方面具有显著优势，但人类和LLMs的创造力类型存在差异。
...
### [205] [Evaluating the Ability of Large Language Models to Reason about Cardinal Directions, Revisited](https://arxiv.org/abs/2507.12059)
*Anthony G Cohn,Robert E Blackwell*

Main category: cs.CL

TL;DR: 研究评估了28种大型语言模型（LLMs）在方向推理任务中的表现，发现即使是新型的大型推理模型也无法完全正确解决所有问题。
...
Method: 使用基于模板生成的基准测试，测试LLM在不同场景下确定正确方向的能力，包括代理的运动方式和人称视角的变化。

Result: 研究发现，即使是新型的大型推理模型也无法在所有问题上可靠地确定正确方向。

Conclusion: 论文总结了在COSIT-24上展示的早期工作，并进一步扩展了研究内容。
...
### [206] [StylOch at PAN: Gradient-Boosted Trees with Frequency-Based Stylometric Features](https://arxiv.org/abs/2507.12064)
*Jeremi K. Ochab,Mateusz Matias,Tymoteusz Boba,Tomasz Walkowiak*

Main category: cs.CL

TL;DR: 基于模块化风格度量管道的二进制AI检测方法，使用spaCy模型预处理文本并提取特征，采用轻量级梯度提升机作为分类器，训练集包含50万+机器生成文本。
...
Result: 训练了一个包含50万+机器生成文本的分类器，通过参数优化提升了分类能力。

Conclusion: 该方法延续了先前有效的非神经网络、低成本且可解释的路径，适用于二进制AI检测任务。
...
### [207] [BOOKCOREF: Coreference Resolution at Book Scale](https://arxiv.org/abs/2507.12075)
...
<details>
  <summary>Details</summary>
Motivation: 现有共指消解基准在长文本评估上存在局限性，无法充分评估书籍规模文本的能力。

Method: 提出自动标注管道，创建BOOKCOREF基准，并进行实验验证其鲁棒性和价值。

Result: BOOKCOREF使系统性能提升高达20 CoNLL-F1点，同时揭示了当前模型在书籍规模上的不足。

Conclusion: BOOKCOREF填补了书籍规模共指消解评估的空白，并推动了相关研究的发展。
...
### [208] [Findings of MEGA: Maths Explanation with LLMs using the Socratic Method for Active Learning](https://arxiv.org/abs/2507.12079)
*Tosin Adewumi,Foteini Simistira Liwicki,Marcus Liwicki,Viktor Gardelli,Lama Alkhaled,Hamam Mokayed*

Main category: cs.CL

TL;DR: 研究比较了结合苏格拉底法、思维链推理、简化游戏化和形成性反馈的MEGA方法与传统的逐步思维链方法在数学学习中的效果，结果显示MEGA方法更受学生认可。
...
Method: 采用组内设计，随机分配问题，比较MEGA方法和传统逐步思维链方法，使用GSM8K和MATH数据集评估两种大型语言模型（GPT4o和Claude 3.5 Sonnet）。

Result: 学生更认可MEGA方法，尤其在较难的MATH数据集中表现更优（47.5% vs 26.67%）。

Conclusion: MEGA方法在解释复杂数学问题时更有效。
...
### [209] [Iterative Augmentation with Summarization Refinement (IASR) Evaluation for Unstructured Survey data Modeling and Analysis](https://arxiv.org/abs/2507.12126)
*Payal Bhattad,Sai Manoj Pudukotai Dinakarrao,Anju Gupta*

Main category: cs.CL

TL;DR: 本文提出了一种评估基于大语言模型（LLM）的文本增强的框架，包括可扩展性分析和迭代增强与摘要细化（IASR），验证了GPT-3.5 Turbo在语义保真度、多样性和生成效率上的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 解决现有文本增强技术在语义保留方面的不足，特别是在大规模或迭代生成中导致的冗余和不稳定性。
...
### [210] [Overview of the Sensemaking Task at the ELOQUENT 2025 Lab: LLMs as Teachers, Students and Evaluators](https://arxiv.org/abs/2507.12143)
*Pavel Šindelář,Ondřej Bojar*

Main category: cs.CL

TL;DR: ELOQUENT的Sensemaking任务通过三步评估生成模型的能力：问题生成、问题回答和答案评分，发现现有方法在问题生成和答案限制上仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 旨在为生成语言模型提供可测试的高层次评估标准，特别是在多语言和多源材料的背景下。

Method: 采用三步评估框架（问题生成、回答、评分），结合自动与人工评估，测试了多种语言和材料来源。

Result: 发现现有方法在问题生成质量评估和答案限制上存在问题，LLM-as-a-Judge范式在评分中存在错误。

Conclusion: 需要改进问题生成评估策略，并解决答案限制和评分准确性问题。
...
### [211] [Toward a Behavioural Translation Style Space: Simulating the Temporal Dynamics of Affect, Behaviour, and Cognition in Human Translation Production](https://arxiv.org/abs/2507.12208)
*Michael Carl,Takanori Mizowaki,Aishvarya Ray,Masaru Yamada,Devi Sri Bandaru,Xinyue Ren*

Main category: cs.CL

TL;DR: 论文提出了一种行为翻译风格空间（BTSS），用于描述可能的翻译行为模式，并通过多层嵌入结构分析眼动和手指运动数据，揭示隐藏的认知过程。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过观察翻译行为（如眼动和手指运动）来理解其背后的高阶认知和情感状态，从而构建一个计算翻译代理。

Method: 通过分析击键和注视数据，构建多层嵌入的行为翻译风格空间（BTSS）。

Result: BTSS能够模拟人类翻译过程中的情感、自动化行为和认知的动态变化。

Conclusion: BTSS为计算翻译代理提供了理论基础，有助于模拟翻译行为中的复杂动态过程。
...
### [212] [Towards few-shot isolated word reading assessment](https://arxiv.org/abs/2507.12217)
...
<details>
  <summary>Details</summary>
Motivation: 在低资源环境中，缺乏自动语音识别（ASR）系统的情况下，探索一种无需ASR的孤立词阅读评估方法。

Method: 使用SSL模型的中间层编码输入儿童语音和少量成人参考模板，并研究离散化SSL特征和模板的质心平均等设计选项。
...
### [213] [Improving Contextual ASR via Multi-grained Fusion with Large Language Models](https://arxiv.org/abs/2507.12252)
...
<details>
  <summary>Details</summary>
Motivation: 现有ASR模型在识别上下文相关关键词（如专有名词或用户特定实体）时表现不佳，而现有方法在粒度上各有局限性。
...
### [214] [Translationese-index: Using Likelihood Ratios for Graded and Generalizable Measurement of Translationese](https://arxiv.org/abs/2507.12260)
*Yikang Liu,Wanyang Zhang,Yiming Wang,Jialong Tang,Pei Zhang,Baosong Yang,Fei Huang,Rui Wang,Hai Hu*

Main category: cs.CL

TL;DR: 提出了一种名为T-index的定量测量方法，用于评估翻译文本的翻译风格（translationese），并通过对比微调的语言模型计算。实验表明T-index在跨域设置中具有普适性，并与人类判断一致。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对翻译风格的定量测量，T-index填补了这一空白。

Method: 使用对比微调的语言模型计算T-index，并通过合成数据和真实翻译数据验证其有效性。

Result: T-index在仅用1-5k对合成数据微调的0.5B语言模型上表现良好，与人类标注的翻译风格相关性高（Pearson's r=0.568）。

Conclusion: T-index是一种高效且稳健的翻译风格测量方法，可作为机器翻译质量评估的补充指标。
...
### [215] [Infherno: End-to-end Agent-based FHIR Resource Synthesis from Free-form Clinical Notes](https://arxiv.org/abs/2507.12261)
*Johann Frei,Nils Feldhus,Lisa Raithel,Roland Roller,Alexander Meyer,Frank Kramer*

Main category: cs.CL

TL;DR: Infherno框架利用LLM代理和医疗术语数据库，将非结构化临床笔记转换为结构化FHIR资源，解决了现有方法的泛化性和结构一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将自由格式临床笔记转换为FHIR资源时存在泛化性不足和结构不一致的问题，需要更高效的解决方案。

Method: 提出Infherno框架，结合LLM代理、代码执行和医疗术语数据库工具，实现端到端的转换。

Result: Infherno在预测FHIR资源时表现接近人类基准，并支持自定义数据和多种模型。

Conclusion: Infherno为临床数据集成和跨机构互操作性提供了有效工具。
...
### [216] [Text-ADBench: Text Anomaly Detection Benchmark based on LLMs Embedding](https://arxiv.org/abs/2507.12295)
*Feng Xiao,Jicong Fan*

Main category: cs.CL

TL;DR: 该论文提出了一个文本异常检测的基准测试，通过评估不同预训练语言模型的嵌入效果，发现嵌入质量对异常检测性能至关重要，且深度学习模型在LLM嵌入下并未优于传统算法。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏标准化的文本异常检测基准，限制了方法的比较和创新。

Method: 利用多种预训练语言模型（如GloVe、BERT、LLaMa等）的嵌入，结合多领域文本数据集和评估指标（AUROC、AUPRC）进行系统性评估。

Result: 嵌入质量显著影响异常检测效果，深度学习模型在LLM嵌入下与传统算法表现相当；跨模型性能矩阵呈现低秩特性。
...
### [217] [Chain-of-Descriptions: Improving Code LLMs for VHDL Code Generation and Summarization](https://arxiv.org/abs/2507.12308)
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在通用代码任务中广泛应用，但在硬件描述语言（如VHDL）领域的研究较少，存在性能不足的问题。
...
### [218] [Exploring Gender Bias in Alzheimer's Disease Detection: Insights from Mandarin and Greek Speech Perception](https://arxiv.org/abs/2507.12356)
...
### [219] [Beyond Single Models: Enhancing LLM Detection of Ambiguity in Requests through Debate](https://arxiv.org/abs/2507.12370)
*Ana Davila,Jacinto Colan,Yasuhisa Hasegawa*

Main category: cs.CL

TL;DR: 论文提出了一种多代理辩论框架，用于提升大语言模型（LLMs）在检测和解决用户请求模糊性方面的能力，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在理解和生成人类语言方面表现出色，但在处理用户请求时存在模糊性问题，需要更有效的解决方案。

Method: 研究采用了三种LLM架构（Llama3-8B、Gemma2-9B和Mistral-7B变体）和一个包含多样模糊性的数据集，设计并评估了多代理辩论框架。

Result: 辩论框架显著提升了Llama3-8B和Mistral-7B变体的性能，其中Mistral-7B主导的辩论达到了76.7%的成功率，尤其擅长处理复杂模糊性和达成共识。

Conclusion: 研究表明，结构化辩论是增强LLM能力的有效方法，为开发更鲁棒和自适应的语言理解系统提供了重要启示。
...
### [220] [Web-Browsing LLMs Can Access Social Media Profiles and Infer User Demographics](https://arxiv.org/abs/2507.12372)
...
<details>
  <summary>Details</summary>
Motivation: 传统LLMs依赖静态数据，但新功能使其能实时获取网络信息。然而，LLMs直接分析社交媒体数据的能力尚未被研究。

Method: 使用合成数据集（48个X/Twitter账户）和调查数据集（1,384名国际参与者），评估LLMs预测用户 demographic 属性的能力。

Result: LLMs能访问社交媒体内容并较准确地预测用户 demographic 属性，但也可能引入性别和政治偏见。

Conclusion: 该能力对计算社会科学有潜力，但也存在滥用风险，建议限制公开应用，保留研究用途的受控访问。
...
### [221] [Probing for Arithmetic Errors in Language Models](https://arxiv.org/abs/2507.12379)
*Yucheng Sun,Alessandro Stolfo,Mrinmaya Sachan*

Main category: cs.CL

TL;DR: 研究发现，语言模型的内部激活可用于检测算术错误，通过简单探针解码预测输出和正确答案，训练轻量级错误检测器，并扩展到复杂任务，提升任务准确性。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型内部激活是否能用于检测算术错误，以实现模型的自我纠正。

Method: 从3位数加法开始，训练探针解码隐藏状态，扩展到结构化思维链任务，并利用探针指导选择性重新提示。
...
### [222] [Advancing Retrieval-Augmented Generation for Structured Enterprise and Internal Data](https://arxiv.org/abs/2507.12425)
*Chandana Cheerla*
...
### [223] [Can We Predict Alignment Before Models Finish Thinking? Towards Monitoring Misaligned Reasoning Models](https://arxiv.org/abs/2507.12428)
*Yik Siu Chan,Zheng-Xin Yong,Stephen H. Bach*

Main category: cs.CL

TL;DR: 研究发现，通过线性探针分析链式思维（CoT）的激活状态，可以更准确地预测最终回答的安全性，优于基于文本的方法。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以利用链式思维（CoT）预测最终回答的对齐风险，以改进实时安全监控。

Method: 评估多种监控方法，包括人类、大型语言模型和文本分类器，分析CoT文本或激活状态。

Result: 线性探针在CoT激活状态上的表现显著优于文本方法，且能早期预测安全性。

Conclusion: 轻量级探针可用于实时安全监控，适用于不同模型和任务。
...
### [224] [S2WTM: Spherical Sliced-Wasserstein Autoencoder for Topic Modeling](https://arxiv.org/abs/2507.12451)
*Suman Adhya,Debarshi Kumar Sanyal*

Main category: cs.CL

TL;DR: S2WTM提出了一种新的主题建模方法，通过球面切片Wasserstein距离解决VAE-NTMs中的后验崩溃问题，生成更一致和多样的主题。
...
Result: 实验表明，S2WTM优于现有主题模型，生成的主题更一致和多样，并在下游任务中表现更好。
...
### [225] [Language Models Improve When Pretraining Data Matches Target Tasks](https://arxiv.org/abs/2507.12466)
...
TL;DR: 论文提出了一种显式优化数据选择目标的方法BETR，通过将预训练数据与评估基准对齐，显著提升了模型性能。
...
Result: BETR在10个任务中的9个上表现优于基线，计算效率提升2.1倍，且在不同基准上泛化良好。
...
### [226] [CompressedVQA-HDR: Generalized Full-reference and No-reference Quality Assessment Models for Compressed High Dynamic Range Videos](https://arxiv.org/abs/2507.11900)
*Wei Sun,Linhan Cao,Kang Fu,Dandan Zhu,Jun Jia,Menghan Hu,Xiongkuo Min,Guangtao Zhai*

Main category: eess.IV

TL;DR: 论文提出了一种名为CompressedVQA-HDR的视频质量评估框架，针对高动态范围（HDR）视频压缩质量评估问题，采用Swin Transformer和SigLip 2作为骨干网络，分别设计了全参考（FR）和无参考（NR）模型，并在多个数据集上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估方法难以泛化到多样化的视频类型，尤其是HDR内容，因此需要一种更通用的评估框架。

Method: 采用Swin Transformer和SigLip 2作为骨干网络，分别设计FR和NR模型；通过预训练和微调策略解决HDR数据不足问题。

Result: 模型在多个数据集上表现优异，CompressedVQA-HDR-FR在IEEE ICME 2025挑战赛中FR赛道排名第一。

Conclusion: CompressedVQA-HDR框架有效解决了HDR视频质量评估问题，为视频压缩技术的发展提供了实用工具。
...
### [227] [Identifying Signatures of Image Phenotypes to Track Treatment Response in Liver Disease](https://arxiv.org/abs/2507.12012)
...
Result: 方法在非酒精性脂肪性肝炎患者的随机对照试验中验证，能区分治疗组与安慰剂组的组织变化，并预测活检特征。
...
### [228] [Benchmarking and Explaining Deep Learning Cortical Lesion MRI Segmentation in Multiple Sclerosis](https://arxiv.org/abs/2507.12092)
...
TL;DR: 论文提出了一种基于nnU-Net框架的皮质病变检测和分割方法，通过多中心MRI数据验证了其性能，并分析了影响模型表现的因素。


<details>
  <summary>Details</summary>
Motivation: 皮质病变在MS诊断中具有重要价值，但由于MRI表现不明显、专家标注困难以及缺乏标准化方法，临床应用中存在限制。

Method: 使用656个MRI扫描数据，基于nnU-Net框架进行改进，并进行了跨域测试。

Result: 模型在域内和域外的F1分数分别为0.64和0.5，展示了较强的检测能力。

Conclusion: 研究分析了数据变异性、病变模糊性和协议差异对模型的影响，并提出了未来改进建议，同时公开了代码和模型。
