...
Motivation: 现有方法面临几何不一致、幻觉问题导致身份保持能力下降，以及合成数据训练模型无法捕捉高频细节如皮肤皱纹和细发，限制了真实感。

Method: 引入两个关键贡献：(1)生成式规范化模块处理多个非结构化视图为标准化一致表示；(2)基于transformer的模型在从真实人物圆顶捕捉得到的高斯溅射头像大规模数据集上训练。采用"捕捉、规范化、溅射"流水线。

Result: 从非结构化照片产生具有引人注目的真实感和强大身份保持能力的静态四分之一身体头像。

Conclusion: 该方法通过新颖的零样本流水线成功解决了3D头像创建中的几何一致性和高频细节捕捉问题，实现了超真实且身份保持的头像生成。
...
<details>
  <summary>Details</summary>
Motivation: 高斯溅射在大规模数据集上表现良好，但在稀疏3D训练数据区域难以捕捉精细细节或保持真实感，需要解决稀疏覆盖区域的局限性。
...
Conclusion: 混合2D-3D方法具有显著潜力，2D基础模型与3D重建管道的精心结合能够克服各自方法的固有局限性。
...
Result: 在1000个测试样本上，该方法相比逆距离加权、克里金插值和纳维-斯托克斯算法，重建精度提高4-15%（RMSE），感知相似度提高29-81%（LPIPS）。

Conclusion: 无条件扩散模型能够有效重建火星表面地形，在填补高度图缺失值方面优于传统方法，为火星探索任务提供了更可靠的虚拟现实模拟基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有监督模型依赖大量标注数据且泛化能力有限，需要开发无需标注即可识别新食物类别的零样本方法。

Method: 集成视觉-语言模型和大语言模型，通过多轮视觉-文本对话进行协作推理，使用对象感知令牌捕获细粒度视觉属性，交互式推理代理动态解释上下文线索。
...
<details>
  <summary>Details</summary>
Motivation: 近距离人-人交互姿态蕴含丰富的交互动态上下文信息，人类能凭此直观推断上下文并预测可能的过去和未来动态。受此启发，作者希望开发一个基于交互姿态先验的通用交互动画框架。

Method: 使用两个条件扩散模型：(1)姿态动画器利用时间先验从交互姿态生成动态运动序列；(2)姿态生成器应用空间先验从单姿态、文本或两者合成交互姿态。训练数据来自动捕交互数据集中的近距离双人姿态及其周围时间上下文。
...
<details>
  <summary>Details</summary>
Motivation: 子宫内膜异位症在不同体内位置呈现多种视觉表现，使其识别困难且容易出错，特别是对于非专科医生。该系统旨在为妇科医生提供辅助诊断支持。

Method: 训练了一个能够分割深色子宫内膜植入物的系统，该系统能够分析腹腔镜手术视频，用多色覆盖层标注识别出的植入区域，并显示检测摘要以改进视频浏览。

Result: 系统成功实现了对深色子宫内膜植入物的自动分割和标注功能，能够辅助医生在腹腔镜手术视频中识别病变区域。

Conclusion: 该系统为子宫内膜异位症的诊断提供了有效的计算机辅助工具，有助于提高诊断准确性和效率，特别是对于非专科医生而言。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉模型需要大量领域特定标注数据且难以理解复杂环境上下文，而视觉语言模型在遥感领域应用不足。研究旨在结合两者优势，提升遥感图像分析的准确性和上下文感知能力。

Method: 集成YOLO目标检测模型与多种视觉语言模型（LLaVA、ChatGPT、Gemini），在标记和未标记遥感数据以及图像退化场景下进行性能评估。

Result: 在飞机检测和计数准确性方面，所有模型平均MAE提升48.46%，在挑战性条件下表现尤其突出；遥感图像综合理解的CLIPScore提升6.17%。
...
<details>
  <summary>Details</summary>
Motivation: 前列腺癌中的筛状形态是一个预后不良的组织学特征，但目前存在报告不足和观察者间变异性大的问题。

Method: 使用EfficientNetV2-S编码器和多实例学习的深度学习模型，在640个前列腺穿刺活检切片上进行训练，并在多个队列中进行内部和外部验证。

Result: 模型在内部验证中AUC为0.97，Cohen's kappa为0.81；在外部验证中AUC为0.90，Cohen's kappa为0.55；在病理学家间比较中表现最佳（kappa: 0.66）。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗净化方法主要针对加性扰动设计，对非加性扰动（模糊、遮挡、扭曲等）效果不佳，需要扩展框架来处理这类常见于现实世界的扰动。

Method: 建立对抗图像生成过程，通过似然最大化方法分离底层干净图像和扰动参数，实现非加性扰动的净化。

Result: 在GTSRB和CIFAR-10数据集上的实验表明，NAPPure显著提升了图像分类模型对非加性扰动的鲁棒性。

Conclusion: NAPPure框架成功扩展了对抗净化的适用范围，有效处理非加性对抗扰动，提升了模型在现实世界扰动下的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决大视频语言模型在处理长视频时面临的困难：超出上下文窗口的视频token处理和长期序列信息保留问题，以及现有视频RAG方法中时间依赖关系破坏和无关信息干扰的挑战。

Method: 提出两个关键创新：(1) 使用结构化图表示视频，保持视频片段间的语义关系；(2) 引入中间推理步骤，通过结构化验证减少检索噪声，促进跨片段相关信息的显式聚合。
...
<details>
  <summary>Details</summary>
Motivation: 同步来自不同场景或生成AI的视频具有挑战性，因为存在多样化的主题、背景和非线性时间错位。

Method: TPL从预训练模型提取的高维嵌入中构建共享紧凑的一维表示，通过学习统一的原型序列来锚定关键动作阶段。
...
<details>
  <summary>Details</summary>
Motivation: 现代显微镜产生的大规模2D和3D数据集需要更高效的计算方法，现有工具在可扩展性、GPU加速和现代科学计算工作流集成方面存在局限。

Method: 基于CuPy和RAPIDS cuCIM，提供设备无关的API，自动调度GPU或CPU执行，加速图像处理流程。

Result: 在去卷积和分割流程中实现了显著加速，同时保持算法保真度。
...
TL;DR: 提出了一个框架，通过定制化数据流水线实现视频扩散模型中的多视角角色一致性和3D相机控制，支持虚拟制作的核心能力。
...
Method: 使用4D高斯泼溅技术重新渲染体积捕捉表演，结合视频重光照模型获得光照变化，在开源视频扩散模型上微调。
...
<details>
  <summary>Details</summary>
Motivation: 传统研究主要使用Big Five模型，但忽略了HEXACO模型中的诚实-谦逊特质，该特质与攻击性和支配欲相关。同时，两种模型在机器学习中的关系尚未明确，通过考虑这些关系可以提升对人类行为的理解。

Method: 采用联合优化方法同时识别Big Five和HEXACO人格特质，使用自我介绍视频数据集进行实验验证。

Result: 实验证明，所提出的方法能够有效识别Big Five和HEXACO人格特质。

Conclusion: 联合建模Big Five和HEXACO的方法在多元人格特质识别中表现有效，为理解人类行为提供了新的视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于重建误差的AI图像检测方法计算成本高且无法有效捕捉原始图像中的固有噪声特征，需要一种更高效准确的检测方案。

Method: 使用位平面图像处理提取噪声特征，设计最大梯度补丁选择算法放大噪声信号，并构建轻量级分类器（噪声分类器和噪声引导分类器）。

Result: 在GenImage基准测试中达到98.9%平均准确率（提升11.9%），跨生成器泛化能力强（GAN到Diffusion 98.2%，Diffusion到GAN 99.2%），检测速度比现有方法快近百倍。

Conclusion: 该方法通过位平面噪声分析和高效梯度选择，实现了高精度、高效率的AI生成图像检测，具有良好的实用性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度伪造检测方法依赖手动设计的音素-视素对齐阈值、基本帧级一致性检查或单模态检测策略，无法有效识别由先进生成模型（如GANs、扩散模型、神经渲染技术）生成的现代深度伪造内容。

Method: 使用音素序列、唇部几何数据和先进的面部身份嵌入，构建了一个多模态音频-视觉框架，通过分析跨多个互补模态的不一致性来检测深度伪造。
...
Conclusion: EIM方案能够充分利用事件视觉传感器的异步操作和高动态范围特性，显著提高了基于事件的OCC系统的传输性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模场景中定位和渲染的计算成本高的问题，传统场景坐标回归方法在扩展到大规模场景时受限于单个网络容量。

Method: 引入门控网络隐式分类和选择子网络，确保每次推理只激活单个子网络；提出无辅助损失负载均衡策略提升定位精度。

Result: 在剑桥测试集上仅需10分钟训练即可获得高质量渲染结果，实现了成本显著降低的同时保持更高精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有I2V模型在生成视频时难以保持输入图像中人物的身份一致性，特别是当人脸在图像中占比较小或人物表情动作变化较大时。

Method: 基于强化学习的视频扩散框架，使用人脸身份评分器优化模型，通过反向传播奖励信号和KL散度正则化防止过拟合。

Result: 在Wan 2.2 I2V模型和内部模型上的实验证明了方法的有效性。

Conclusion: IPRO框架能够有效提升图像到视频生成中的身份保持能力，无需改变模型架构。
...
<details>
  <summary>Details</summary>
Motivation: 解决高分辨率图像跨视图匹配的挑战，包括二次复杂度和缺乏显式匹配约束的问题，同时处理跨视图遮挡问题。

Method: 使用MatchAttention机制动态匹配相对位置，通过BilinearSoftmax实现连续可微的滑动窗口注意力采样，设计分层跨视图解码器MatchDecoder，并引入门控跨MatchAttention和一致性约束损失来处理遮挡。
...
Conclusion: 提出的方法结合高精度和低计算复杂度，使实时、高分辨率、高精度的跨视图匹配成为可能。
...
Result: 在室外实验中，首次实现了200米距离下60kbps和400米距离下30kbps的通信，且误码率低于10^{-3}。

Conclusion: 所提出的解调方案有效提升了光学相机通信系统在长距离和高数据速率下的性能，为室外应用提供了可行的解决方案。
...
Method: 基于因果推断的框架，采用前门原则设计转换策略，明确整合中介变量和观察到的组织切片，以利用语义特征并减轻混杂因素的影响。

Result: 在CAMELYON17数据集和私有组织病理学数据集上验证，在未见领域上获得一致性能提升，相比现有基线方法性能提高达7%。
...
Method: 训练免费的三层对比解码加水印方法：选择解码层中的成熟层和业余层；使用水印相关问题识别视觉基础良好的枢纽层；应用三层对比解码生成最终输出。

Result: 在POPE、MME和AMBER等公共基准测试中，该方法在减少LVLM幻觉方面达到了最先进的性能，并生成更具视觉基础的回答。

Conclusion: 提出的三层对比解码加水印方法有效减少了LVLM的幻觉问题，提高了输出的视觉基础性，且无需额外训练。
...
<details>
  <summary>Details</summary>
Motivation: 虹膜生物识别系统易受呈现攻击（如人工眼、打印图像、美瞳镜片），但缺乏足够的训练数据来开发有效的攻击检测方法。

Method: 结合扩散模型和生成对抗网络，采用多域架构在真实眼部图像和不同攻击域之间进行转换，使用针对眼部数据的自适应损失函数。
...
<details>
  <summary>Details</summary>
Motivation: 主流多模态大语言模型仅通过文本token的next-token预测进行监督，忽略了视觉中心的关键信息，这限制了模型的视觉分析能力。

Method: 引入可学习的模块化任务查询和视觉对齐层，在多个视觉基础模型的监督下激活特定视觉信号，并通过令牌网关掩码协调不同视觉基础模型之间的表示冲突。

Result: 大量实验表明，VaCo显著提升了不同MLLMs在多个基准测试上的性能，展示了其在视觉理解方面的卓越能力。

Conclusion: VaCo通过视觉中心激活和协调机制，有效解决了MLLMs中视觉信息整合不足的问题，显著提升了模型的视觉理解能力。
...
Method: 提出空间偏好奖励方法SPR，通过随机选择图像区域和模型生成的区域描述，引入语义和定位评分来全面评估文本质量和定位质量，并通过直接偏好优化来增强与视觉输入的细粒度对齐。

Result: 在标准参考和定位基准测试上的广泛实验表明，SPR能够有效提升多模态大语言模型的空间理解能力，且训练开销极小。

Conclusion: SPR方法通过奖励机制成功增强了多模态大语言模型的细粒度空间感知能力，在最小化训练开销的同时显著提升了空间理解性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于Mamba的状态空间模型在脑肿瘤分割中计算开销大、对BraTS数据分区鲁棒性未充分探索的问题，需要开发更高效可靠的3D分割方法。

Method: 使用双分辨率双向Mamba架构捕获多尺度长程依赖；采用空间填充曲线进行3D到1D特征映射以保持空间局部性；提出门控融合模块自适应整合前向和反向上下文；引入量化块离散化特征提升鲁棒性；建立五个系统性折叠进行严格评估。

Result: 在20%测试集上：全肿瘤Dice提升0.10%，肿瘤核心提升1.75%，增强肿瘤提升0.93%；在五个系统性折叠上：肿瘤核心平均Dice增益0.86%，增强肿瘤增益1.45%；效率提升15倍同时保持高分割精度。

Conclusion: DRBD-Mamba模型在脑肿瘤分割任务中实现了计算效率和分割精度的双重提升，证明了其在处理多尺度长程依赖和保持空间局部性方面的优势，为临床诊断提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有PCB检测研究主要针对裸板或线路级缺陷，而组装级全主板检测研究不足，需要开发可靠的检测方法来确保高产量电子制造的可靠性。

Method: 使用YOLOv7和Faster R-CNN作为基准检测器，提出轻量级集成方法Confidence-Temporal Voting (CTV Voter)，通过可解释规则平衡精确率和召回率，并在MiracleFactory主板数据集上进行评估。

Result: YOLOv7在精确率方面表现优异但在召回率方面表现不佳，Faster R-CNN则相反；CTV Voter集成方法有效平衡了精确率和召回率；在现实扰动条件下评估了鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学在处理千兆像素尺寸输入和缺乏密集手动标注方面面临计算瓶颈，现有方法往往忽略多放大倍数WSI中的细粒度信息和肿瘤微环境的变化。

Method: 采用从易到难的渐进表示学习方法，通过双课程对比多实例学习模型处理WSI，不依赖密集标注，直接将千兆像素图像转换为预后预测。

Result: 在12种癌症类型（5,954名患者，1254万个图块）上的广泛实验表明，DCMIL优于标准的基于WSI的预后模型，能够识别细粒度的预后显著区域，提供稳健的实例不确定性估计，并捕获正常和肿瘤组织之间的形态学差异。

Conclusion: DCMIL模型在癌症预后预测方面表现出色，具有生成新生物学见解的潜力，所有代码已公开。
...
Method: 设计了统一的帧内/帧间编码框架，每个帧由单一模型自适应处理，并提出同时双帧压缩设计以利用前向和后向帧间冗余。

Result: 相比DCVC-RT平均减少10.7%的BD-rate，提供更稳定的比特率和每帧质量，同时保持实时编码/解码性能。

Conclusion: 该方案通过引入帧内编码工具和双向压缩设计，有效解决了神经视频压缩中的关键问题，实现了性能的显著提升。
...
Method: 使用核范数正则化来促进集中于背景的结构化扰动，并采用自适应乐观指数梯度方法进行高效优化，提高可扩展性和收敛性。
...
<details>
  <summary>Details</summary>
Motivation: 解决监督方法需要大量标注数据和局限于已知病理的问题，通过仅使用健康数据训练模型来识别异常，特别适用于罕见或异质性疾病。

Method: 采用PRISMA指导的范围综述方法，系统分析了2018-2025年间49项研究，比较了不同生成模型的性能指标和架构设计选择。

Result: 生成模型在大病灶检测中表现出良好性能，在识别细微异常方面也取得进展，能够生成可解释的伪健康重建图像。

Conclusion: 无监督深度生成模型为异常检测提供了有前景的方向，支持半监督学习、新成像生物标志物发现以及疾病内和跨疾病偏差映射，未来需关注解剖感知建模、基础模型开发和临床验证。
...
<details>
  <summary>Details</summary>
Motivation: 多任务图像修复模型参数量过大导致计算效率低下，需要压缩模型规模同时保持性能

Method: 使用迭代剪枝策略，多轮移除低幅值权重并将剩余权重重置为原始初始化，发现高性能稀疏子网络

Result: 在去雨、去雾、去噪任务上，MIR-L仅保留10%可训练参数但维持高图像修复性能

Conclusion: 提出的压缩策略能有效发现多任务图像修复模型中的高性能稀疏子网络，实现模型轻量化
...
Method: 基于Sentinel-2 L2A时间序列数据，训练CNN-LSTM集成模型，利用多时相反射特征进行二元预测（放牧/未放牧）。

Result: 在五个验证分割中平均F1分数达77%，放牧牧场召回率90%；在仅能检查4%场地的情况下，模型优先预测的非放牧场地确认率比随机检查高17.2倍。

Conclusion: 粗分辨率免费卫星数据可有效指导保护性土地利用合规检查，代码和模型已公开。
...
<details>
  <summary>Details</summary>
Motivation: Vision Mamba相比Vision Transformers具有线性而非二次的计算复杂度，且比传统CNN需要更少的可训练参数，这使其在计算和内存效率方面具有优势，适合用于三维多孔介质渗透率预测。

Method: 使用Vision Mamba作为骨干网络构建神经网络，用于预测三维多孔介质的渗透率，并与ViT和CNN模型进行性能比较，同时进行消融研究评估各组件对准确性的影响。

Result: 在实际应用中验证了Vision Mamba相比ViT和CNN在三维多孔介质渗透率预测中的优势，包括计算效率和内存效率方面的改进。

Conclusion: 提出的框架有潜力整合到大型视觉模型中，用Vision Mamba替代ViT，源代码已公开以促进可重复性和进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 手术器械缺陷会严重影响无菌性、机械完整性和患者安全，而传统人工检查存在误差和不一致性问题。

Method: 使用YOLOv8模型在102,876张高分辨率图像数据集上进行训练，涵盖11种器械类型和5种主要缺陷类别，采用对比度增强预处理。

Result: SurgScan达到99.3%的最高准确率，实时推理速度4.2-5.8毫秒/图像，显著优于现有CNN架构。

Conclusion: SurgScan提供了可扩展、经济高效的AI解决方案，减少对人工检查的依赖，确保符合ISO 13485和FDA标准。
...
Method: 设计噪声投影器，首先采样噪声并用视觉语言模型获得token级反馈，蒸馏到奖励模型中，最后通过准直接偏好优化优化噪声投影器。

Result: 实验表明，提示感知噪声投影能显著提高各种提示下的文本-图像对齐度。

Conclusion: 提出的噪声投影框架有效解决了训练-推理不匹配问题，无需参考图像或手动先验，且推理成本低，仅需单次前向传播。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个既能准确识别复杂文档元素（文本、表格、公式、图表），又保持资源消耗最小化的高效文档解析模型。
...
Method: 使用两个编码器f_E和f_SE分别提取域不变特征z_u和域特定特征z_d，通过解码器重建图像，并采用域预测器进行对抗训练。通过将z_u和z_d的重构相加来重建输入图像。

Result: 与现有方法相比，PL-SE-ADA在图像重建、疾病分类和域识别方面达到同等或更好的性能，并能可视化域无关和域特定的脑部特征。

Conclusion: PL-SE-ADA不仅实现了有效的域协调，还提供了整个框架的高可解释性，能够可视化域独立和域特定的脑部特征组件。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在图像理解方面取得了显著进展，但其内部表示往往不透明，难以解释视觉信息处理过程。而经典视觉描述符对人类来说直观易懂，因此研究现代学习能否从这些经典线索中受益。
...
Conclusion: VisualSplit证明了这种新的学习方法在视觉理解中的有效性，为现代学习从经典视觉线索中受益提供了实证支持。
...
Result: 在多个基准测试和骨干网络上一致产生显著性能提升，特别是在具有挑战性的数据集上表现突出。

Conclusion: FMA具有多步校正能力，相比一步PEFT方法能实现更精确和鲁棒的跨模态特征对齐。
...
TL;DR: 提出了一种名为场景去上下文化(SDeC)的训练免费方法，通过抑制文本到图像生成中固有的场景-身份相关性来解决身份偏移问题。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中的身份偏移问题，特别是当不知道所有目标场景时，传统方法依赖不现实的假设。

Method: 通过SVD方向稳定性量化场景-身份相关性，自适应重新加权特征值来抑制ID提示嵌入中的潜在场景-ID相关性。

Result: 实验表明SDeC显著增强了身份保持，同时保持了场景多样性。

Conclusion: SDeC提供了一种高效、灵活且通用的解决方案，适用于现实世界应用，无需事先了解所有目标场景。
...
TL;DR: 提出ESTP-Bench基准和ESTP-F1指标，用于评估AI在流式视频中主动理解、预测和响应事件的能力，并开发了一个包含数据引擎、多阶段训练策略和主动动态压缩技术的完整技术流程。


<details>
  <summary>Details</summary>
Motivation: 构建能够在人类环境中主动理解、预测和响应事件的AI系统，超越单纯的观察功能，实现同步感知和推理。

Method: 提出包含三个组件的技术流程：(1) 数据引擎，(2) 多阶段训练策略，(3) 主动动态压缩技术，用于处理流式视频输入并主动回答多样化问题。

Result: 所提出的模型有效解决了主动一致性、及时响应和同步效率等关键特性，在多个在线和离线基准测试中优于多个基线方法。

Conclusion: 该工作为实现能够在流式视频环境中主动感知和响应的AI系统提供了有效的基准评估框架和技术解决方案。
...
TL;DR: BalanceGS是一个算法-系统协同设计的3D高斯泼溅训练优化框架，通过密度控制、动态负载分配和内存访问优化，在保持重建质量的同时实现了1.44倍的训练加速。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅训练存在三个关键效率问题：高斯密度分配不均、计算负载不平衡和内存访问碎片化。

Method: 1. 算法层面：启发式负载敏感高斯密度控制；2. 系统层面：基于相似性的高斯采样与合并；3. 映射层面：重排序内存访问映射策略。

Result: 在NVIDIA A100 GPU上实现了1.44倍的训练加速，同时重建质量下降可忽略不计。

Conclusion: BalanceGS通过算法-系统协同设计有效解决了3D高斯泼溅训练的效率瓶颈问题。
...
Conclusion: 将曲率信息整合到深度学习架构中以及多分支特征学习对基于LiDAR点云的车辆重识别是有效的。
...
TL;DR: 提出了一个用于像素级关键点定位的双向框架，包含点描述器和点定位器两个组件，通过自然语言实现精确的关键点理解
...
Method: 开发了包含点描述器（生成关键点上下文描述）和点定位器（从描述回归像素坐标）的双向框架，使用GRPO在AP-10K上优化点描述器，并创建了LlamaPointInPart数据集进行训练

Result: 在LlamaPointInPart数据集上相比基线模型表现出优越性能，建立了新的评估协议来衡量定位精度

Conclusion: 该双向框架为关键点引导的图像理解和语言引导的精确定位应用开辟了新途径
...
Method: 提出两个组件：1) 实例线索 - 将稀疏的用户可编辑提示转换为密集的2.5D运动场；2) 密集RoPE - 在token空间中保持线索显著性，配合RGB+辅助图预测。

Result: 该方法减少了深度模糊性，稳定了优化过程，提高了时间一致性。

Conclusion: STANCE框架有效解决了视频生成中的运动一致性问题，无需复杂的逐帧轨迹脚本。
...
<details>
  <summary>Details</summary>
Motivation: 现有动物分类模型如SpeciesNet使用保守的汇总策略，导致许多动物只能在高级分类级别标记，而非物种级别。

Method: 五阶段管道：高置信度接受、鸟类覆盖、质心构建、三元组损失度量学习和自适应余弦距离评分，结合SpeciesNet预测、CLIP嵌入和度量学习。

Result: 在LILA BC沙漠狮子保护数据集（4,018张图像，15,031个检测）上评估，从"空白"和"动物"标签中恢复了761个鸟类检测，以96.5%的准确率重新分类了456个标记为动物、哺乳动物或空白的检测，实现了64.9%的物种级识别。

Conclusion: 该分层重分类系统能有效将高级分类标签细化为物种级识别，显著提高了动物检测的精确度。
...
TL;DR: 评估零样本方法在野生动物图像分类中的应用，使用自监督视觉变换器，在Animal Detect平台上开发和测试。比较了无监督聚类方法（DBSCAN、GMM）和三种架构（CLIP、DINOv2、MegaDescriptor）结合降维技术（PCA、UMAP），并展示了通过t-SNE投影实现的一维相似性排序。
...
Method: 使用自监督视觉变换器，比较无监督聚类方法（DBSCAN、GMM）与三种架构（CLIP、DINOv2、MegaDescriptor）结合降维技术（PCA、UMAP），并通过t-SNE投影实现一维相似性排序。

Result: 在5个物种测试集上，DINOv2与UMAP和GMM组合达到88.6%准确率（宏F1=0.874）。一维排序在1500张图像中，哺乳动物和鸟类达到88.2%一致性，鱼类达到95.2%一致性。
...
TL;DR: 提出Wiki-PRF三阶段方法，通过处理、检索和过滤阶段改进知识库视觉问答任务，结合强化学习训练视觉语言模型，在基准数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决检索增强生成方法在KB-VQA任务中面临的多模态查询质量差和检索结果相关性不足的问题。

Method: 三阶段方法：处理阶段动态调用视觉工具提取精确多模态信息；检索阶段整合视觉和文本特征实现多模态知识检索；过滤阶段进行相关性过滤和结果浓缩。使用强化学习训练视觉语言模型，以答案准确性和格式一致性作为奖励信号。

Result: 在E-VQA和InfoSeek基准数据集上分别提升36.0和42.8的答案质量，达到最先进性能。

Conclusion: Wiki-PRF方法通过三阶段处理和强化学习训练，显著提升了KB-VQA任务的性能，证明了该方法在整合视觉理解和外部知识检索方面的有效性。
...
Method: 采用双分支设计，包括视觉编码器、时空Transformer编码器和基于Transformer的解码器。引入战术单元检测器和击球级提示引导机制，将预测的战术类型和状态作为提示注入解码器。
...
Method: 引入高效视频采样(EVS)，通过识别和修剪时间上静态的补丁来减少视频令牌冗余。该方法保留位置身份，无需架构更改或重新训练。在推理时应用，并与使用随机修剪率的上训练阶段结合。

Result: EVS将大型语言模型的首令牌时间(TTFT)减少高达4倍，准确率损失最小。结合上训练后，模型对不同程度的压缩具有鲁棒性，在激进修剪下仍保持完整性能。

Conclusion: EVS持续改善效率-准确性权衡，在不牺牲质量的情况下实现可扩展的视频语言理解。
...
<details>
  <summary>Details</summary>
Motivation: 解决2D潜在空间的空间冗余问题，降低训练成本，同时保持自监督学习空间的有益几何特性。
...
Conclusion: SteeringTTA通过Feynman-Kac引导机制有效提升了测试时适应的性能，在保持输入适应优势的同时解决了梯度引导的局限性。
...
TL;DR: 提出了一种低成本预训练策略，通过利用无配对视频剪辑的上下文学习来实现基于指令的视频编辑，显著提升了编辑指令遵循度和编辑质量。


<details>
  <summary>Details</summary>
Motivation: 基于指令的图像编辑发展迅速，但其在视频领域的扩展因大规模配对视频编辑数据集构建成本高且复杂而进展缓慢。

Method: 采用两阶段方法：首先在约100万个真实视频剪辑上进行预训练学习基本编辑概念，然后在少于15万个精选编辑对上进行微调以扩展编辑任务并提升编辑质量。

Result: 实验表明该方法在指令对齐和视觉保真度方面均优于现有基于指令的视频编辑方法，编辑指令遵循度提升12%，编辑质量提升15%。

Conclusion: 该低成本预训练策略能够有效赋予基础视频生成模型通用编辑能力，并通过少量高质量配对数据实现高效精炼。
...
TL;DR: 将去相关反向传播（DBP）集成到MAE预训练中，加速ViT预训练收敛，减少21.1%训练时间、21.4%碳排放，同时提升下游任务性能


<details>
  <summary>Details</summary>
Motivation: MAE预训练在低标签场景表现优异但计算成本高昂，在工业应用中不实用，需要更高效的预训练方法

Method: 在MAE预训练的编码器中选择性应用DBP，通过逐层减少输入相关性来加速收敛，保持训练稳定性

Result: 在ImageNet-1K预训练和ADE20K微调中，DBP-MAE减少21.1%训练时间、21.4%碳排放，分割mIoU提升1.1个百分点；在工业数据上验证了类似效果

Conclusion: DBP能够显著减少大规模ViT预训练的时间和能耗，同时改善下游任务性能，具有实际工业应用价值
...
TL;DR: EuroMineNet是首个基于Sentinel-2多光谱影像的欧洲采矿足迹多时相基准数据集，涵盖2015-2024年133个欧盟采矿点的年度观测和专家验证标注，支持采矿足迹制图和跨时相变化检测任务。
...
Method: 基于Sentinel-2多光谱影像构建多时相基准数据集，提供年度观测和专家验证标注，支持两个任务：(1)多时相采矿足迹制图，(2)跨时相变化检测，并使用新的Change-Aware Temporal IoU指标评估。

Result: 对20个最先进的深度学习模型进行基准测试，发现GeoAI方法能有效识别长期环境变化，但在检测对及时缓解至关重要的短期动态方面仍存在挑战。
...
TL;DR: 提出了WeCKD方法，通过链式知识蒸馏框架解决传统知识蒸馏中的知识退化、监督效率低和数据依赖问题，在有限数据场景下实现高效学习。
...
Method: 采用弱监督链式知识蒸馏网络，构建渐进式蒸馏链，每个模型不仅从前驱学习知识，还进行知识精炼后传递给后续模型。

Result: 在四个耳镜成像数据集上超越现有监督方法，在其他两个数据集上验证了在显微和磁共振成像等医学影像模态上的泛化能力，相比单骨干网络在有限数据下获得最高+23%的累积准确率提升。

Conclusion: WeCKD通过结构化知识转移显著提升了特征学习能力，降低了数据依赖性，缓解了一步知识蒸馏的局限性，具有实际应用潜力。
...
TL;DR: 提出了VTimeCoT框架，通过引入进度条工具和视觉时间链式思维，提升多模态大语言模型在视频时序定位和推理任务中的性能。
...
Method: 提出VTimeCoT训练免费框架，包含两个新颖的进度条视觉工具：即插即用的进度条集成工具和高效高亮工具，并引入视觉时间链式思维过程实现跨模态推理。

Result: 在Qwen2VL-7B和GPT4o基线上，视频时序定位和推理问答任务中均取得显著性能提升。
...
TL;DR: 提出了一种基于学习图像先验的3D高斯溅射压缩框架，通过恢复网络建模压缩伪影，并利用粗渲染残差作为辅助信息，显著提升压缩率和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射压缩方法缺乏学习先验，限制了率失真性能的进一步提升，需要利用学习图像先验来恢复压缩引起的质量退化。

Method: 基于初始压缩的高斯，构建恢复网络在图像空间建模压缩伪影，引入粗渲染残差作为辅助信息，通过恢复图像的监督来优化压缩高斯表示。

Result: 实验验证了框架的有效性，在率失真性能上优于现有最先进的3DGS压缩方法，同时显著减少存储需求。

Conclusion: 该框架兼容现有高斯压缩方法，能够实现高度紧凑的表示并提升渲染性能，为3DGS压缩提供了新的解决方案。
...
TL;DR: 提出了一种半自动的鲸鱼检测方法，使用统计异常检测来标记空间异常点，结合网页标注界面，显著减少了专家需要检查的区域面积。


<details>
  <summary>Details</summary>
Motivation: 传统鲸鱼监测方法昂贵且难以扩展，而现有基于高分辨率卫星图像的自动检测面临标注数据缺乏、图像质量变化和构建机器学习管道成本高等挑战。

Method: 采用统计异常检测方法标记空间异常点（"有趣点"），配合网页标注界面让专家快速标注这些点，无需依赖标注训练数据。

Result: 在三个基准场景上测试，召回率达到90.3%到96.4%，同时将专家需要检查的区域面积减少了99.8%——从超过1000平方公里减少到不到2平方公里。

Conclusion: 该方法不依赖标注训练数据，为未来机器辅助的海洋哺乳动物空间监测提供了可扩展的第一步，相关代码已开源。
...
TL;DR: 本文首次系统评估了深度视频相机运动分类模型在历史档案影片上的表现，发现Video Swin Transformer在HISTORIAN数据集上达到80.25%准确率，证明了现有模型对低质量视频的适应潜力。


<details>
  <summary>Details</summary>
Motivation: 相机运动传递了理解视频内容所需的空间和叙事信息。虽然现有的相机运动分类方法在现代数据集上表现良好，但它们在历史档案素材上的泛化能力尚未被探索。

Method: 总结了代表性方法和数据集，重点分析了模型设计和标签定义的差异。在包含专家标注的二战镜头的HISTORIAN数据集上评估了五种标准视频分类模型。

Result: 性能最佳的Video Swin Transformer模型达到80.25%的准确率，尽管训练数据有限但仍表现出强大的收敛能力。
...
TL;DR: 提出了跨层特征自注意力模块(CFSAM)，通过建模多尺度特征间的局部和全局依赖关系，显著提升目标检测性能，在PASCAL VOC和COCO数据集上分别达到78.6%和52.1%的mAP。


<details>
  <summary>Details</summary>
Motivation: 现有注意力机制大多局限于单层或双层特征优化，忽略了多尺度表示间的跨层依赖关系，限制了检测大尺度变化目标的能力。

Method: CFSAM包含三个组件：卷积局部特征提取器、基于Transformer的全局建模单元（捕获跨层交互）、特征融合机制（恢复和增强原始表示）。

Result: 在SSD300框架中集成CFSAM后，PASCAL VOC mAP从75.5%提升至78.6%，COCO mAP从43.1%提升至52.1%，优于现有注意力模块，且加速训练收敛而不增加显著计算开销。
...
TL;DR: 提出了ImageNet-F基准测试和free-grain学习方法，用于处理现实世界中粒度不一的图像标注，通过视觉语言模型和半监督学习提升分层分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有分层图像分类方法通常假设完整、细粒度的标注，但现实世界的监督标注粒度不一，受图像质量、标注者专业知识和任务需求影响。

Method: 使用CLIP模拟语义模糊性，创建混合粒度的真实标注；提出free-grain学习框架，结合视觉语言模型生成的伪属性和半监督学习来增强语义和视觉指导。

Result: 提出的方法和强基线在混合监督下显著提升了性能。

Conclusion: ImageNet-F基准测试和free-grain学习方法推动了在现实约束下的分层分类研究进展。
...
TL;DR: DEXTER是一个无需数据的框架，利用扩散模型和大型语言模型为视觉分类器生成全局文本解释，通过优化文本提示合成激活目标分类器的类条件图像，然后生成描述分类器决策模式和偏见的自然语言报告。
...
Method: 使用扩散模型和LLM，通过优化文本提示合成类条件图像，这些合成样本用于生成描述类特定决策模式和偏见的自然语言报告。

Result: 在ImageNet、Waterbirds、CelebA和FairFaces上的实验表明，DEXTER在全局模型解释和类级偏见报告方面优于现有方法，定量和定性评估（包括用户研究）显示其输出准确且可解释。

Conclusion: DEXTER提供了一个无需训练数据或真实标签就能生成视觉分类器自然语言解释的灵活框架，在激活最大化、切片发现与去偏、偏见解释等任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统无线电通信在MAV蜂群中存在频谱拥塞、干扰和高功耗问题，需要寻找更可靠的替代通信方式。

Method: 使用四种运动基元（垂直、水平、左-上-右、左-下-右）作为控制符号，通过事件相机捕获运动模式，设计基于事件帧的分割模型和轻量级SNN进行动作识别和解码。

Result: 实验验证了框架的有效性，实现了准确解码和低功耗，在受限环境中表现出良好的通信性能。

Conclusion: 该视觉通信框架为MAV蜂群提供了一种节能高效的通信替代方案，特别适用于无线电受限的环境。
...
TL;DR: CoT-PL框架通过视觉链式思维推理改进开放词汇目标检测，在拥挤和遮挡场景中显著提升新类别伪标签质量，分别比之前最佳方法提高103.4%和168.4%。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法过度依赖直接图像-文本匹配，忽略了语义复杂场景理解所需的中间推理步骤，导致在拥挤或遮挡场景中鲁棒性不足。

Method: 提出CoT-PL框架，将物体理解分解为三个可解释步骤：区域感知、零样本类别识别和背景定位，并引入对比背景学习来促进物体与背景特征解耦。

Result: 在开放词汇COCO数据集上获得+7.7 AP50提升，在LVIS数据集上获得+2.9 mask AP提升，为新颖类别设立了新的最先进水平。

Conclusion: 结构化视觉链式思维推理与对比背景学习的结合，为拥挤和遮挡场景中的鲁棒伪标签生成提供了有效解决方案，显著提升了开放词汇目标检测性能。
...
TL;DR: 开发了PRISM模型，这是一种可解释的AI模型，通过整合连续变异谱来表征结直肠癌的形态多样性，在5年总生存期预测上表现优异，超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的计算病理学基础模型往往忽略器官特异性关键形态模式，这些模式代表不同的生物过程，可能影响肿瘤行为、治疗反应和患者预后。
...
Conclusion: PRISM模型能够有效捕捉结直肠癌的形态多样性，提供优越的预后预测性能，且在不同患者亚群中保持稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 传统AI肿瘤分割模型需要大量手工绘制的肿瘤掩膜，成本高昂且耗时。而临床实践中几乎每个CT扫描都配有描述肿瘤特征的医学报告，这些丰富信息未被充分利用。

Method: R-Super通过训练AI模型来分割与医学报告中描述相匹配的肿瘤，利用101,654份医学报告进行训练，减少对人工标注肿瘤掩膜的依赖。

Result: 仅使用报告训练的AI模型性能与使用723个掩膜训练的模型相当。结合报告和掩膜训练，灵敏度提高13%，特异性提高8%，在7种肿瘤类型中有5种检测性能超过放射科医生。

Conclusion: 该研究挑战了大规模手工肿瘤掩膜创建不可或缺的传统观念，为多种肿瘤类型的早期检测提供了一条可扩展且易于实现的路径。
...
Method: 提出PRTraj框架：1）环境感知模块从POI分布中捕获多粒度环境语义；2）路径选择编码器将路段转换建模为决策序列；3）聚合形成全局轨迹嵌入。

Result: 在3个真实世界数据集和5个下游任务上的实验验证了PRTraj的有效性和泛化能力，在少样本场景下仍保持稳健性能。
...
TL;DR: 提出了一种新的分数位量化方法SHORT，通过渐进式降低模型精度从32位到4位，在优化过程中利用分数位来保持生成质量，成功在智能手机上部署了Sana模型。
...
Method: 提出分数位量化方法SHORT，渐进式降低模型参数精度（从32位到4位），在优化过程中利用分数位来维持高质量生成。

Result: SHORT在多种扩散模型上表现出改进的质量，包括SD3.5-Medium、Sana、Pixart和FLUX.1-schnell，比标准QAT的FiD低4-7%。成功在三星S25U手机上部署和运行Sana模型。

Conclusion: SHORT方法有效解决了量化过程中的质量保持问题，使大容量生成模型能够在资源受限的移动设备上部署，同时保持高质量的生成能力。
...
TL;DR: AI肿瘤分割受限于缺乏大规模体素级标注数据集。研究发现合成数据可显著提升数据扩展效率，仅用500个真实扫描即可达到1500个扫描的性能。基于此创建了AbdomenAtlas 2.0数据集，包含10,135个CT扫描和15,130个肿瘤实例，在六个器官上实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决AI肿瘤分割面临的大规模体素级标注数据稀缺问题，这些数据创建困难且需要医学专家参与。

Method: 利用合成数据增强训练效率，创建了AbdomenAtlas 2.0数据集，包含10,135个CT扫描，15,130个肿瘤实例，覆盖六个器官（胰腺、肝脏、肾脏、结肠、食管、子宫），由23名放射科专家标注。

Result: 合成数据仅用500个真实扫描即可达到1500个扫描的性能。AbdomenAtlas 2.0在分布内测试中DSC提升+7%，分布外测试中提升+16%，显著优于现有公共数据集。

Conclusion: 合成数据可以显著提升数据扩展效率，AbdomenAtlas 2.0为训练多器官肿瘤分割AI提供了强大基础，在性能上实现了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在空间感知和推理方面存在不足，缺乏对3D结构的理解能力，难以完成精细的操作任务。

Method: 提出QDepth-VLA框架，通过辅助深度预测任务增强VLA模型。设计专门的深度专家来预测VQ-VAE编码器获得的深度图的量化潜在标记。

Result: 在仿真基准测试和真实世界任务上的实验结果表明，QDepth-VLA在空间推理方面表现强劲，在操作任务上具有竞争力。

Conclusion: 通过深度感知表示学习，QDepth-VLA有效提升了VLA模型的空间感知和推理能力，为精细操作任务提供了更好的几何线索理解。
...
TL;DR: 提出了ImagerySearch方法，通过动态调整推理搜索空间和奖励函数来提升视频生成模型在想象力场景中的表现，并创建了LDT-Bench基准进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在现实场景表现良好，但在想象力场景中性能显著下降，这些场景包含罕见概念组合和长距离语义关系，超出了训练分布范围。
...
Conclusion: 该方法在挑战性想象力场景中能够生成更连贯和视觉上合理的视频，证明了其有效性。
...
TL;DR: 提出一个深度学习框架，既能对皮肤病变进行分类，又能量化ABCDE特征评分，模拟病变随时间演变过程，为医生提供可解释的AI诊断。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型对皮肤病变分析存在黑盒问题，缺乏对人类可解释特征的解释。需要开发能够模拟ABCDE临床标准并提供量化评分的框架。

Method: 使用深度学习框架同时分类皮肤病变并量化ABCD特征评分，模拟E特征的演变过程，在潜在空间中可视化特征轨迹。实验使用HAM10000数据集。

Result: 分类准确率约89%，黑色素瘤AUC为0.96。特征评估在不对称性、颜色变化和直径预测方面表现良好，但边界不规则性建模仍具挑战。

Conclusion: 该框架使医生能够将机器学习诊断与临床相关标准联系起来，从而改善对皮肤癌进展的理解，为未来研究开辟更多窗口。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界本质上是多模态的，但传统机器学习模型多为单模态或双模态。为了真正理解世界，需要整合所有独立模态，特别是在视觉领域整合多种模态信息。

Method: 使用预训练专家模型和程序化组合方法，构建全自动数据流水线，采用专门设计的PHG-MAE模型来利用多模态数据，并将模型高效蒸馏为低参数版本（<1M）。

Result: 开发的小参数模型（<1M）在性能上可与约300M参数的大型模型竞争，成功部署于手持设备或网络摄像头的实时语义分割任务，并展示了近实时深度估计的应用。

Conclusion: 通过多模态整合和高效蒸馏，可以在保持性能的同时大幅减少模型参数，实现资源受限设备上的实时视觉任务处理，为多模态学习提供了实用解决方案。
...
TL;DR: 本文对开源多模态大语言模型在标准人脸识别数据集上的性能进行了系统评估，发现虽然MLLMs能捕捉丰富的语义线索，但在零样本应用中仍落后于专用人脸识别模型。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在各种视觉语言任务中表现优异，但其在人脸识别领域的潜力尚未充分探索，特别是需要与现有专用人脸识别模型在标准基准上进行性能比较。

Method: 在多个标准人脸识别数据集（LFW、CALFW、CPLFW、CFP、AgeDB、RFW）上系统评估最先进的MLLMs性能，采用相似协议进行比较。

Result: 实验结果显示，MLLMs能够捕捉对人脸相关任务有用的丰富语义线索，但在零样本应用的高精度识别场景中表现落后于专用模型。
...
TL;DR: 提出了Free-Form HOI生成方法，通过细粒度意图控制生成多样化、物理合理的手-物体交互，突破了传统固定抓握模式的限制。


<details>
  <summary>Details</summary>
Motivation: 现有手-物体交互研究局限于固定抓握模式，控制依赖于物理先验或通用意图指令，无法捕捉日常交互的多样性。

Method: 构建WildO2数据集，提出TOUCH三阶段框架，基于多级扩散模型实现细粒度语义控制，利用显式接触建模和物理约束优化。

Result: 方法能够生成可控、多样且物理合理的手交互，代表日常活动中的各种交互模式。

Conclusion: 成功扩展了手-物体交互从抓握到自由形式交互的范围，为多样化日常交互生成提供了有效解决方案。
...
Method: 使用V-JEPA2骨干网络进行端到端训练，提供BADAS-Open（1.5k公开视频）和BADAS1.0（40k专有视频）两个变体，重新标注主要基准数据集以识别自车参与情况。

Result: 在DAD、DADA-2000、DoTA和Nexar数据集上，BADAS实现了最先进的AP/AUC性能，优于前向碰撞ADAS基线，并产生更真实的事故时间估计。

Conclusion: BADAS通过专门的自车中心评估框架有效解决了碰撞预测中的误报问题，并发布了模型权重、代码和重新标注的数据集以促进相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉自回归模型在生成质量和推理效率方面取得了显著进展，但在该范式下实现精确灵活的控制机制仍然未被充分探索，需要弥合这一关键差距。
...
TL;DR: 提出了nlg2choice方法，通过两阶段处理解决多模态大语言模型在细粒度视觉分类任务中处理自由形式响应和大量选项的评估挑战


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注纯语言任务或不超过5个选项的多选题，无法应对细粒度视觉分类中数百到数千个高度相关选项的情况，且在多选项设置下扩展LLM选择提取到检索问题存在计算成本高的问题

Method: nlg2choice两阶段方法：首先向MLLM提出无约束的开放性问题，然后使用纯文本约束解码预测最可能的选择；在检索设置中，采用早期停止方法计算约束响应选择该选项的概率，显著提高吞吐量

Result: 在七个细粒度视觉数据集上的实验显示，该方法在分类和检索评估方面均有改进，且性能在不同自然语言任务实现方式下保持稳定

Conclusion: nlg2choice方法有效解决了MLLM在细粒度视觉分类中处理大量选项和自由形式响应的评估挑战，为多模态大语言模型在复杂视觉任务中的应用提供了实用解决方案
...
TL;DR: 提出了一种基于多模态大语言模型的视频异常检测框架，通过提取和解释对象活动和交互来检测复杂异常，并提供可解释性。
...
Result: 在基准数据集上的实验表明，该方法不仅能有效检测基于交互的复杂异常，还在无交互异常的数据集上达到最先进性能。

Conclusion: 该方法通过MLLM提取高级语义表示，不仅提升了复杂异常检测能力，还提供了可解释性，并能与传统VAD方法结合增强其可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 密集视频对象字幕任务需要联合检测、跟踪和描述对象轨迹，但由于任务复杂性和手动标注成本高，现有方法采用分离训练策略导致性能不佳。
...
Conclusion: 通过合成字幕扩展数据集和端到端训练，MaskCaptioner有效解决了DVOC任务，显著提升了性能并降低了标注成本。
...
TL;DR: 3DScenePrompt是一个从任意长度输入生成后续视频片段的框架，通过双时空条件化和3D场景记忆实现精确相机控制和场景一致性保持。
...
Method: 采用双时空条件化：时间相邻帧用于运动连续性，空间相邻内容用于场景一致性。引入3D场景记忆表示静态几何，通过动态SLAM和动态掩码策略分离静态场景几何与动态元素。

Result: 实验表明，该框架在场景一致性、相机可控性和生成质量方面显著优于现有方法。

Conclusion: 3DScenePrompt能够保持长距离空间一致性和精确相机控制，同时不牺牲计算效率或运动真实性。
...
TL;DR: 提出了一种连续掩码自回归运动变换器，结合门控线性注意力和RMSNorm模块，通过DiT结构增强多模态运动生成，在文本、语音和音乐到运动的任务中表现优于现有方法。
...
TL;DR: RealDPO是一种新的对齐范式，利用真实世界数据作为偏好学习的正样本，通过DPO和定制损失函数提升运动合成的真实感，显著改善了视频质量、文本对齐和运动真实感。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在复杂运动生成方面存在困难，往往无法产生自然、流畅且上下文一致的运动，这限制了它们的实际应用。

Method: 引入RealDPO对齐范式，使用真实世界视频作为正样本进行偏好学习，通过DPO和定制损失函数对比真实视频与错误模型输出，实现迭代自校正。同时创建RealAction-5K高质量视频数据集支持复杂运动合成的后训练。

Result: 广泛实验表明，RealDPO在视频质量、文本对齐和运动真实感方面显著优于最先进模型和现有偏好优化技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在视觉思维链方面存在局限，要么依赖僵化的外部工具，要么无法生成高保真、策略性定时的图表来支持复杂数学问题求解。

Method: 框架包含两个阶段：1）视觉操作阶段，在1520万对语料上预训练模型掌握图表生成和编辑；2）策略性视觉辅助推理阶段，在21.9万例交织视觉-文本推理路径数据集上微调模型，教会模型何时及如何利用视觉辅助。

Result: BAGEL-Canvas模型在MathCanvas-Bench基准上相比强大的LMM基线实现了86%的相对提升，在其他公共数学基准上也表现出优秀的泛化能力。

Conclusion: 该工作提供了一个完整的工具包（框架、数据集和基准），为解锁LMM中复杂、类人的视觉辅助推理能力奠定了基础。
...
TL;DR: C4D是一个从单目视频恢复4D（动态几何和相机姿态）的框架，通过引入短时光流和长时点跟踪的时序对应关系，将现有3D重建方法扩展到动态场景。
...
Method: C4D通过预测点图并捕获两种对应关系：短时光流和长时点跟踪。训练动态感知点跟踪器提供移动性信息，估计运动掩码分离移动元素和静态背景，并引入动态场景优化目标来恢复每帧3D几何和相机参数。

Result: 实验表明，C4D实现了完整的4D恢复，在深度估计、相机姿态估计和点跟踪等多个下游任务中表现出强大性能。

Conclusion: C4D通过时序对应关系成功将3D重建扩展到4D动态场景，解决了运动物体对多视图几何约束的破坏问题。
...
TL;DR: 提出一种集成token-wise attention的扩散模型，用于降水临近预报，无需单独潜在模块即可有效捕捉多尺度时空依赖关系。
...
Conclusion: 提出的方法成功解决了扩散模型在降水临近预报中的可扩展性问题，通过集成注意力机制有效提升了预报性能。
...
TL;DR: 提出了ChangingGrounding基准测试和Mem-ChangingGrounder方法，用于在动态场景中通过记忆驱动的3D视觉定位，减少重复扫描需求并提高定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法假设场景点云是重建且最新的，这在实际动态场景中需要频繁重复扫描，成本高昂且阻碍部署。需要将3DVG重新定义为主动、记忆驱动的问题。

Method: 提出Mem-ChangingGrounder方法：通过跨模态检索识别查询对象类型，检索相关记忆指导行动，在场景中高效探索目标，无效时回退，执行多视角扫描，并将融合证据投影获取精确边界框。

Result: 在ChangingGrounding基准测试中，Mem-ChangingGrounder实现了最高的定位精度，同时大幅降低了探索成本。

Conclusion: 该基准测试和方法推动了面向实际应用的实用、以记忆为中心的3DVG研究发展。
...
TL;DR: 提出了WithAnyone模型，通过构建MultiID-2M数据集和对比身份损失来解决身份一致生成中的copy-paste问题，平衡身份保真度和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有身份一致生成模型依赖重建训练，导致copy-paste问题——直接复制参考人脸而非保持身份在姿态、表情、光照等自然变化中的一致性，这削弱了可控性和生成表现力。

Method: 1) 构建大规模配对数据集MultiID-2M，为每个身份提供多样化参考；2) 提出量化copy-paste伪影和身份保真度-变化权衡的基准；3) 引入基于对比身份损失的新训练范式，利用配对数据平衡保真度与多样性。

Result: WithAnyone模型显著减少copy-paste伪影，提高姿态和表情的可控性，保持强感知质量。用户研究验证该方法在实现高身份保真度的同时支持富有表现力的可控生成。

Conclusion: 提出的方法有效缓解了身份一致生成中的copy-paste问题，在保持身份相似性的同时提升了生成多样性和可控性。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界模型大多依赖像素对齐表示作为世界演进基础，忽略了物理世界固有的3D特性，这会破坏3D一致性并降低建模效率。

Method: 提出点对高斯变分自编码器(P2G-VAE)将3D输入编码为潜在点表示，然后解码为3D高斯基元来联合建模几何和外观；引入稀疏点流匹配网络(SPFlow)生成潜在点表示，同时去噪点的位置和特征。

Result: 在ScanNet v2室内场景上的实验表明，Terra在重建和生成方面实现了最先进的性能，具有高3D一致性。

Conclusion: Terra通过原生3D表示和架构实现了精确的多视角一致性，仅需单次生成过程即可支持任意视角的灵活渲染，并通过点潜在空间中的渐进生成实现了可探索的世界建模。
...
TL;DR: 提出无需配对数据的图像编辑训练新范式，通过扩散模型展开和视觉语言模型反馈进行端到端优化，在无监督情况下达到与有监督方法相当的性能


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型依赖大规模输入-目标配对数据进行监督微调，但这类自然配对数据难以大规模获取，成为关键瓶颈

Method: 直接优化多步扩散模型：训练时展开模型，利用视觉语言模型评估编辑是否遵循指令并保留未改变内容，提供直接梯度进行端到端优化；同时加入分布匹配损失确保视觉保真度

Result: 在标准基准测试中，无需任何配对数据，该方法在少步设置下与使用大量监督配对数据训练的图像编辑扩散模型表现相当；使用相同VLM作为奖励模型时，也优于基于强化学习的技术如Flow-GRPO

Conclusion: 提出的训练范式成功消除了对配对数据的依赖，通过VLM反馈和分布匹配实现了有效的无监督图像编辑训练
...
TL;DR: 本文提出了NEO，一个从第一原理构建的原生视觉语言模型家族，能够与顶级模块化模型竞争，并通过390M图像-文本样本高效开发视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 解决原生视觉语言模型的两个关键挑战：明确其与模块化模型的基本差异限制，以及如何使原生VLM研究更加民主化和易于访问。

Method: 基于三个核心原则构建NEO：在共享语义空间中有效对齐像素和词表示；无缝整合视觉和语言模块优势；内在体现支持统一视觉语言编码、对齐和推理的跨模态特性。

Result: NEO在多样化现实场景中能够与顶级模块化模型竞争，仅使用390M图像-文本样本就高效开发了视觉感知能力，同时在密集单体模型中缓解了视觉语言冲突。

Conclusion: NEO为可扩展和强大的原生视觉语言模型奠定了基础，提供了一套丰富的可重用组件，促进成本效益高且可扩展的生态系统发展。
...
Conclusion: 该方法作为多视角一致编辑的通用解决方案具有潜力，无需显式3D表示优化即可实现跨视角一致性。
...
TL;DR: 提出了结合多语言对比奖励信号的GRPO框架，显著提升跨语言Text-to-SQL系统的执行准确率和语义准确率，使用小规模训练数据即可超越更大模型的性能


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL方法只关注查询可执行性，忽略了语义对齐挑战，且在非英语语言上执行准确率显著下降（平均下降6个百分点）

Method: 在GRPO框架中结合多语言对比奖励信号，通过语义相似度奖励信号增强SQL生成与用户意图的对应关系

Result: 在MultiSpider数据集上，LLaMA-3-3B模型执行准确率提升至87.4%（比零样本提升26pp），语义准确率提升至52.29%（提升32.86pp）；添加对比奖励后平均语义准确率达59.14%（提升6.85pp，越南语最高提升10pp）；3B模型超越8B零样本模型执行准确率（88.86% vs 81.43%）

Conclusion: 通过对比奖励实现定向语义对齐，无需大规模训练数据集即可显著提升Text-to-SQL系统性能
...
TL;DR: 本文提出生成式操作框架，利用LLM作为翻译引擎，将XAI技术输出转化为临床可用的叙述，解决心理健康筛查中技术透明度与临床实用性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 当前XAI技术（如SHAP和LIME）虽然能产生技术忠实的输出，但无法提供临床相关、可操作的见解，导致实验室到临床应用的持续差距。

Method: 提出生成式操作框架，使用LLM作为核心翻译引擎，整合多种XAI工具的技术输出与临床指南（通过RAG），自动生成人类可读、有证据支持的临床叙述。

Result: 该框架直接解决了关键操作障碍，包括工作流集成、偏见缓解和利益相关者特定沟通，推动了从孤立数据点生成向集成、可操作、可信赖AI交付的转变。

Conclusion: 生成式操作框架为将XAI从技术透明度转向临床实用性提供了战略路线图，有望促进AI在临床实践中的真实应用。
...
TL;DR: STELA是一个公开可验证的水印框架，通过语言自由度动态调整水印强度，在语法约束语境中减弱信号以保持文本质量，在语言灵活性高的语境中增强信号以提高可检测性。


<details>
  <summary>Details</summary>
Motivation: 现有水印方法依赖模型特定信号（如logits），阻碍了公开验证。需要一种不依赖模型内部信息的公开可验证水印方法，平衡文本质量与检测鲁棒性。

Method: 利用词性n-元模型建模语言不确定性，根据语言自由度动态调制水印信号强度。检测器无需访问模型logits即可进行公开验证。

Result: 在分析性英语、孤立性汉语和黏着性韩语等多种语言类型上的实验表明，STELA在检测鲁棒性方面优于现有方法。

Conclusion: STELA通过语言自由度动态调整水印强度，实现了公开可验证的水印检测，在保持文本质量的同时提升了检测鲁棒性。
...
TL;DR: 提出一种通过用户比较模式收集成对偏好数据的新方法，利用不同模型生成的两个响应来推断用户数据质量，开发EM算法估计用户潜在质量因子并过滤数据。


<details>
  <summary>Details</summary>
Motivation: 传统成对偏好数据依赖专业标注人员，而用户标注虽然能更好判断自身查询的响应质量，但缺乏质量控制。本文旨在解决用户标注数据的质量问题。

Method: 提出用户行为模型，通过两个不同模型生成的响应不对称性推断用户数据质量，开发期望最大化算法估计用户潜在质量因子，并据此过滤数据。

Result: 下游任务验证了该方法在捕捉用户行为和LLM对齐数据过滤方面的有效性。

Conclusion: 用户比较模式结合质量评估机制能够有效收集高质量成对偏好数据，为LLM对齐提供可靠数据源。
...
TL;DR: 提出了一种名为"知情路由"的新范式，通过轻量级特征预测器评估token的可恢复性，实现执行或近似策略，显著降低计算成本同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有动态token级计算分配方法依赖贪婪路由机制，导致不可逆信息损失和次优token选择，限制了LLMs在实际应用中的部署。

Method: 引入轻量级特征预测器(LFF)来预测单元输出，基于token的重要性和可恢复性评估，采用执行或近似策略进行路由决策。

Result: 在语言建模和推理任务上实现了最先进的效率-性能权衡，无需最终LoRA微调即可匹配或超越需要完整微调的基线方法，训练时间减少50%以上。

Conclusion: 知情路由通过前瞻性评估token的可恢复性，有效解决了贪婪路由的信息损失问题，为LLMs的高效部署提供了可行方案。
...
TL;DR: 提出了HIES（Head Importance-Entropy Score）剪枝标准，结合注意力熵来改进仅基于梯度的重要性评分方法，在保持准确性的同时实现更好的模型压缩效果。
...
Conclusion: HIES通过整合重要性分数和注意力熵，克服了传统方法的局限性，为Transformer模型提供了更有效的剪枝方案。
...
Method: 使用多智能体工作流从描述公共数据集见解的文章中生成真实基准测试，创建了1420个对话式数据分析问题，并开发了评估工具。
...
TL;DR: 本文研究了大型语言模型的不确定性量化方法，提出了基于相似性的黑盒框架来评估LLM生成内容的置信度，并在问答、摘要和文本转SQL等任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在可信AI系统中的重要性日益增加，需要有效的不确定性量化方法来衡量模型对其生成输出的置信度。黑盒方法具有适应性强、成本低等优势，但现有方法在复杂生成任务中的效果有待改进。

Method: 提出了一个高层次的非语言化相似性聚合框架，该框架包含多种适用于复杂生成任务的不确定性量化方法，并引入了使用小训练集训练置信度估计模型的新技术。

Result: 通过在问答、摘要和文本转SQL等多个任务数据集上的实证研究，表明提出的基于相似性的方法相比基线方法能够产生更好校准的置信度。

Conclusion: 基于相似性的黑盒不确定性量化方法能够有效评估LLM生成内容的置信度，在多种复杂生成任务中表现优于现有基线方法，为构建可信AI系统提供了实用工具。
...
Conclusion: 提出的文化感知框架能有效解决仇恨言论检测中的数据稀疏性、文化纠缠和模糊标签问题，提升分类性能。
...
TL;DR: 本文开发了一种利用大语言模型从原始评论文本中自动提取产品本体（以部分关系形式）的方法，该方法优于现有的基于BERT的基线。
...
Method: 利用大语言模型的最新进展，开发了从原始评论文本中自动提取产品本体（以部分关系形式）的完全自动化方法。

Result: 通过LLM作为评判者的评估，证明该方法产生的本体优于现有的基于BERT的基线。

Conclusion: 本研究为在大语言模型更广泛地应用于（产品或其它）本体提取奠定了基础。
...
Method: 提出ADMIT攻击方法：少样本、语义对齐的投毒攻击，无需访问目标LLM、检索器或令牌级控制。

Result: 在4个检索器、11个LLM和4个跨领域基准测试中，ADMIT平均攻击成功率达86%，投毒率仅为0.93×10^-6，且对强反证据保持鲁棒性。

Conclusion: ADMIT暴露了现实世界基于RAG的事实核查系统的显著脆弱性，相比现有最优攻击方法将ASR提升了11.2%。
...
TL;DR: SerialBEHRT是一个针对医疗领域的预训练基础模型，通过将电子健康记录序列化来更好地捕捉临床事件间的时序关系，在抗生素敏感性预测任务中表现优异。
...
Method: 基于SciBERT进行额外预训练，设计领域对齐的基础模型，对结构化EHR序列进行编码以捕捉临床事件的时序和上下文关系。

Result: 在抗生素敏感性预测任务中，SerialBEHRT相比现有EHR表示策略实现了更优越和一致的表现。

Conclusion: 时序序列化在医疗基础模型预训练中具有重要意义，SerialBEHRT证明了其在捕捉临床事件间时间关系方面的有效性。
...
TL;DR: DynaSpec提出了一种上下文相关的动态短列表机制，通过轻量级元分类器将上下文路由到少量token聚类中，解决传统固定词汇子集方法在推测解码中的局限性，提高起草效率并保持验证的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统推测解码中，起草器使用固定词汇子集存在两个问题：(1) 频率列表依赖语料库且需要重新调整，(2) 静态短列表抑制罕见或领域特定token，降低每次验证步骤的预期token数量。

Method: 引入轻量级粗粒度元分类器，将上下文路由到少量token聚类中；选择的前k个聚类联合形成起草器的短列表，同时验证保留完整词汇和精确性；通过并行执行起草编码和元短列表计算实现提前完成。

Result: 在标准推测解码基准测试中，相比固定短列表基线，观察到平均接受长度的一致增益，同时上下文相关选择使得更小的短列表不会降低接受率。

Conclusion: DynaSpec提供了一种稳健、加速起草并能泛化到多样化任务的动态短列表机制，解决了传统方法在词汇扩展背景下的瓶颈问题。
...
TL;DR: 提出一种针对组合多任务（如翻译摘要）的新方法，通过在组合适配器上添加可学习投影层来实现高效集成，在云端和移动设备上均表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调技术（如LoRA）在处理复杂组合任务（如从长对话生成翻译摘要）时表现不佳，需要一种能同时执行多个复杂任务的高效解决方案。

Method: 在组合的摘要和翻译适配器之上添加可学习投影层，实现有效集成，同时通过减少计算开销保持效率，避免大量重新训练或顺序处理。

Result: 实验结果表明该方法在云端和移动设备实现中表现良好且快速，开发了能够无缝执行组合任务的Android应用。
...
TL;DR: 提出了一种轻量级的推理时方法，通过PCA识别语言方向并引导token嵌入来控制语言身份，有效减少多语言LLM中的意外代码切换。
...
Method: 潜在空间语言引导：在并行翻译数据上使用PCA识别语言方向，然后沿着这些轴引导token嵌入来控制语言身份。

Result: 使用单个主成分实现95-99%的语言分类准确率，在Qwen2.5和Llama-3.2模型上，多个语言对的下一token分布差异减少高达42%。

Conclusion: 语言身份集中在最终层，具有近乎完美的线性可分性，该方法能以可忽略的计算开销缓解代码切换，同时保持语义。
...
TL;DR: 该论文发现成功的LLM推理具有全局非均匀的信息密度特征，与人类通信模式形成鲜明对比，挑战了关于机器推理的假设。


<details>
  <summary>Details</summary>
Motivation: 受心理语言学中均匀信息密度假设的启发，旨在分析LLM推理过程中的信息流，解决中间步骤不忠实或难以解释的问题。

Method: 引入基于熵的指标来分析推理轨迹中的信息流，在三个数学基准测试上进行实验。

Result: 发现成功的LLM推理表现出信息密度的不均匀波动，与人类通信模式相反。
...
Conclusion: 在动态演化信息环境中开发更原则性的LLM设计方法具有必要性，EvoEdit提供了一个简单有效且具有强理论保证的解决方案。
...
Method: 对19个LLM进行测试，每个模型针对15个话题询问5个事实，重复100次并添加不同人物角色上下文。通过句子嵌入计算跨人物角色的余弦相似度来评估事实一致性。

Result: 在100个人物角色的实验中，一致性得分范围从0.9065到0.7896，平均为0.8656。Grok-3最一致，而几个轻量级模型排名最低。不同话题的一致性差异显著。

Conclusion: LLM的事实一致性既受提供商影响，也受话题影响。需要开发人物角色不变的提示策略来提高模型的一致性表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有text-to-SQL研究多基于公开数据集，但在私有企业数据仓库中效果不佳。手动标注SQL日志以构建基准既困难又昂贵。

Method: 给定SQL查询，使用检索增强生成和LLMs提出多个自然语言描述，然后由人类专家选择、排序或编辑以确保准确性和领域对齐。

Result: 在标注的企业SQL日志上评估显示，LLM辅助标注大幅减少了创建高质量基准的时间和精力，结合人类验证提高了标注准确性、基准可靠性和模型评估鲁棒性。

Conclusion: BenchPress为研究人员和从业者提供了评估text-to-SQL模型在特定领域工作负载上的机制，通过简化自定义基准的创建过程。
...
TL;DR: R2T框架通过将语言规则融入神经网络训练目标，使用自适应损失函数处理OOV词汇，在无标注文本上训练的Zarma词性标注达到98.2%准确率，优于有监督基线。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言标注数据稀缺问题，探索基于原则而非纯数据驱动的学习方法（PrL），通过显式任务约束提升模型性能。

Method: 提出Rule-to-Tag框架，将多层级语言规则集成到神经网络训练目标中，使用包含正则化项的自适应损失函数处理OOV词汇的不确定性。

Result: 在Zarma词性标注任务中，仅使用无标注文本训练的R2T-BiLSTM模型达到98.2%准确率，优于在300个标注句子上微调的AfriBERTa；在NER任务中，R2T预训练后在50个标注句子上微调的模型性能优于在300个句子上训练的基线。

Conclusion: R2T框架在低资源场景下表现出色，证明原则学习范式能有效利用语言规则提升模型性能，特别是在数据稀缺的语言处理任务中。
...
TL;DR: CoRE是一种利用模型一致性增强LLM集成鲁棒性的即插即用技术，通过token级和模型级一致性处理集成中的错误信号，提升集成性能。


<details>
  <summary>Details</summary>
Motivation: 不同LLM具有互补能力，但集成方法对错误信号（如异构分词方案和模型专长差异）的鲁棒性关注不足，集成失败通常源于token级和模型级的不一致性。

Method: 提出CoRE技术：token级一致性使用低通滤波器降低高不一致性token的权重；模型级一致性通过提升高自信度且与其他模型差异小的模型输出来增强全局一致性。

Result: 在多种基准测试、模型组合和集成策略上的实验表明，CoRE能持续提升集成性能和鲁棒性。

Conclusion: CoRE通过利用模型一致性有效解决了LLM集成中的鲁棒性问题，可无缝集成到各种集成方法中，显著提升集成效果。
...
TL;DR: MasonNLP系统使用基于检索增强生成(RAG)的通用指令调优大语言模型，在MEDIQA-WV 2025伤口护理VQA任务中获得第三名，证明了轻量级RAG方法在临床多模态NLP任务中的有效性。
...
Method: 采用通用领域指令调优大语言模型，结合检索增强生成(RAG)框架，从领域内数据中检索文本和视觉示例，通过简单索引和融合添加相关示例，无需额外训练或复杂重排序。

Result: 在19个团队51个提交中排名第三，平均得分41.37%，在dBLEU、ROUGE、BERTScore和基于LLM的指标上表现良好。

Conclusion: 轻量级RAG与通用LLMs结合，通过最小推理时间层添加相关示例，为多模态临床NLP任务提供了简单有效的基线方法。
...
Result: ShishuLM在中等上下文场景下，相比父模型内存需求减少25%，训练和推理延迟提升40%。
...
TL;DR: 该研究开发了首个集成LLM框架用于大规模辅导对话中的情感感知，分析了16,986个对话轮次，发现学生在AI辅导中通常呈现轻度积极情感和中等唤醒度，但学习过程并非一帆风顺，困惑和好奇是常见伴随状态。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探讨了大型语言模型在教育环境中的学习影响，但LLM介导的辅导中的情感动态仍未被充分理解。本研究旨在通过关注学习者不断变化的情感状态，推进将生成式AI负责任地整合到教育中的路径。

Method: 分析了两个学期16,986个对话轮次，使用三种前沿LLM（Gemini、GPT-4o、Claude）生成零样本情感标注，包括效价、唤醒度和学习帮助性的标量评分以及自由文本情感标签，通过排名加权模型内池化和跨模型多数共识融合估计值。

Result: 学生通常报告轻度积极情感和中等唤醒度；困惑和好奇是解决问题的常见伴随状态；沮丧虽然较少见但仍会出现；情感状态短暂存在，积极时刻稍长但易被破坏；负面情感通常快速解决，有时直接反弹至积极状态；中性时刻常作为转折点，更多引导向上而非向下。

Conclusion: 中性时刻为辅导者提供了干预机会，情感动态分析揭示了在AI辅导中支持学习者情感需求的重要性，为负责任地整合生成式AI到教育中提供了见解。
...
TL;DR: 提出了模板填充(TI)方法，这是针对扩散语言模型的定制化条件生成方法，通过先生成结构模板再填充掩码段来改进推理策略。
...
Method: 模板填充(TI)方法首先生成目标响应的结构模板，然后填充掩码段；引入动态段分配(DSA)根据生成置信度自适应调整段长度。

Result: 在数学推理和代码生成基准测试中，相比基线方法获得了17.01%的持续改进，并在多标记生成设置中实现了有效的加速。

Conclusion: 模板填充方法为扩散语言模型提供了有效的结构控制，在保持生成质量的同时提升了性能并实现了加速。
...
TL;DR: 该论文研究了将克丘亚语整合到Common Voice平台的过程，重点关注普诺克丘亚语，展示了该平台在支持资源匮乏语言语音技术发展方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 克丘亚语等资源匮乏语言面临数据和资源稀缺问题，阻碍了其语音技术的发展。Common Voice提供了一个开放和社区驱动的语音数据集创建机会。

Method: 详细介绍了17种克丘亚语言，并以普诺克丘亚语为重点案例研究，包括语言上线和语料库收集（朗读和自发语音数据）。

Result: Common Voice现在拥有191.1小时的克丘亚语语音数据（86%已验证），其中普诺克丘亚语贡献了12小时（77%已验证）。

Conclusion: 这项工作为包容性语音技术和资源匮乏语言社区的数字赋权做出了贡献，并提出了解决技术挑战和伦理考虑的研究议程。
...
<details>
  <summary>Details</summary>
Motivation: 法语肿瘤学领域的自然语言处理工具缺乏标注数据集，需要开发专门的法语资源来支持临床文本分析。

Method: 通过将西班牙语CANTEMIST语料库翻译成法语，专家手动标注实体范围，使用ICD-O作为参考标准，结合自动匹配和人工验证进行概念标准化。

Result: 产生了71127个ICD-O标准化标注，包含399个形态学代码、272个解剖学代码和2043个复合表达，为法语肿瘤学文本提供了参考标准。

Conclusion: FRACCO语料库填补了法语肿瘤学标注资源的空白，为法语临床文本的自然语言处理研究提供了重要基础。
...
TL;DR: GateSkip是一种简单的残差流门控机制，通过sigmoid线性门实现解码器语言模型中的token级层跳过，在推理时根据门值对token进行排序并按层预算跳过低重要性token，可节省15%计算成本同时保持90%以上基线准确率。


<details>
  <summary>Details</summary>
Motivation: 解决早期退出或基于路由器的混合深度模型不稳定且需要大量重新训练的问题，提出一种可在预训练模型上稳定微调的平滑可微分门控机制。

Method: 为每个Attention/MLP分支配备sigmoid线性门，在分支输出重新进入残差流之前进行压缩；推理时根据门值对token排序，按层预算跳过低重要性token。

Result: 在长形式推理任务中节省15%计算成本同时保持90%以上基线准确率；在指令调优模型上，全计算时获得准确率提升，50%节省时匹配基线质量。

Conclusion: GateSkip提供了一种稳定高效的层跳过方法，可与量化、剪枝和自推测解码等技术轻松结合，同时学习的门控机制为transformer信息流提供了洞察。
...
TL;DR: 该研究引入了一个纯文本交互的多臂老虎机基准，评估LLMs在不确定性下仅使用自然语言进行顺序决策的能力。Qwen3-4B模型表现最佳，达到89.2%的最佳臂选择率，优于传统算法和其他LLMs。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在仅使用自然语言反馈的情况下进行不确定性顺序决策的能力，现有研究对此关注不足。

Method: 创建纯文本多臂老虎机环境，LLMs仅通过"你获得了一个代币"这样的文本反馈来推断潜在奖励结构，并与传统算法进行比较。

Result: 大多数LLMs表现不如基线算法，但Qwen3-4B达到89.2%的最佳臂选择率，显著优于更大的LLMs和传统方法。

Conclusion: 概率推理能力能够仅从语言中涌现，该基准为评估自然语言环境下的决策能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 让语言模型能够根据输入token的复杂性自主决定所需的计算资源，在不确定时请求更多计算时间以提高准确性。

Method: 提出了Catch Your Breath损失函数类别，包括CYB-AP（随时预测）、CYB-VA（变分方法）和CYB-DP（计算预算惩罚）三种方法，将输出选择建模为带时间成本的序列决策问题。

Result: CYB模型仅需基线模型三分之一的数据量就能达到相同性能，比带暂停的交叉熵损失模型少用一半数据。模型能够根据token复杂性和上下文自适应调整处理时间。

Conclusion: 该方法使语言模型能够智能地分配计算资源，在需要时请求额外计算时间，显著提高了训练效率和模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前自然语言生成模型在特定任务或要求下性能不佳或需要大量额外数据调整
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM社会模拟研究大多局限于选择题或简答格式，忽视了LLM的生成特性，无法真实反映社会现象中的话题、观点和推理过程。

Method: 提出基于开放式文本生成的社会模拟方法，借鉴调查方法学和NLP最新进展，强调自由形式文本的价值。
...
TL;DR: 对10个最先进大语言模型在IAB 2.2分类任务上的比较评估显示，尽管模型快速发展，但在文本分类任务中表现中等，平均准确率34%。研究发现模型集成方法能显著提升性能并消除幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 评估当前大语言模型在非结构化文本分类任务中的实际性能，了解模型在压缩丰富文本到有限分类体系时的能力局限。

Method: 使用8,660个人工标注样本和统一的零样本提示，对10个LLM进行IAB 2.2层次分类评估，采用经典指标和LLM特定指标，并开发了模型集成方法。

Result: 当代LLM平均准确率34%，精度42%，召回率45%，F1分数41%。模型经常过度产生类别。Gemini和GPT系列在成本性能平衡上表现最佳，GPT 120B幻觉率最低。集成方法显著提升准确率并完全消除幻觉。

Conclusion: 单纯的模型规模和架构改进不能确保更好的分类准确性，协调的模型集成方法比单纯扩大规模更有效，是实现超越人类专家性能的最有效途径。
...
TL;DR: 提出了ProofGrader评估器，通过系统化方法开发能够对LLM生成的数学证明进行细粒度评分（0-7分）的评估器，在ProofBench数据集上达到0.926的MAE，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前数学推理LLM主要关注可验证最终答案的任务，但生成和验证自然语言数学证明仍是开放挑战，缺乏可靠的细粒度评估器是关键瓶颈。

Method: 引入ProofBench专家标注数据集，系统探索评估器设计空间（骨干模型、输入上下文、指令和工作流程），开发ProofGrader评估器，结合强推理骨干LM、参考解法和评分方案的丰富上下文，以及简单集成方法。

Result: ProofGrader在专家评分上达到0.926的MAE，在最佳选择任务中（n=16）平均得分4.14（满分7），填补了朴素二元评估器（2.48）与人类专家（4.62）之间78%的差距。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型面临高微调成本、推理延迟、边缘部署受限和可靠性问题，而小语言模型具有紧凑、高效和适应性强的特点，两者协作可以互补优势，满足不同任务和部署场景的多样化目标。

Method: 提出系统性的SLM-LLM协作调查，建立基于四个目标的分类框架：性能增强、成本效益、云边隐私和可信度，并在该框架下回顾代表性方法和总结设计范式。

Result: 构建了完整的SLM-LLM协作分类体系，系统梳理了现有协作方法，为研究者和实践者提供了清晰的指导框架。

Conclusion: SLM-LLM协作是实现高效、安全和可扩展人工智能系统的重要方向，需要进一步解决开放挑战并探索未来发展方向。
...
TL;DR: THTB是一个受认知科学启发的框架，通过质量过滤和内在/外在难度评分来优先选择高级认知指令，仅用5%的数据就能超越全数据集训练效果，并在垂直领域实现高效领域适应。
...
Method: 提出THTB框架，结合质量过滤与内在和外在难度评分，优先选择需要更高认知水平的指令，为SFT提供可解释和可量化的标准。

Result: 仅使用5%数据训练的模型就能超越全数据集训练效果，在垂直领域中仅用2%数据训练的模型就能超越使用更大数据集训练的模型。

Conclusion: THTB在数据选择和标注指导方面表现出色，具有强大的领域适应潜力，能显著降低训练成本同时提升模型性能。
...
TL;DR: 本文通过结构化红队挑战开发了包含50种越狱策略的全面分层分类法，分析了不同攻击类型的流行度和成功率，评估了分类法引导提示对自动检测的改进效果，并创建了包含1364个多轮对抗对话的意大利语数据集。
...
Method: 进行结构化红队挑战，开发包含7大类（冒充、说服、权限提升、认知过载、混淆、目标冲突、数据投毒）的50种越狱策略分层分类法，收集挑战数据进行分析，并创建多轮对抗对话数据集。

Result: 建立了全面的越狱策略分类体系，分析了不同攻击策略的成功率和模型漏洞利用方式，验证了分类法引导提示对自动检测的改进效果，创建了首个意大利语多轮对抗对话数据集。

Conclusion: 该研究推进了对越狱技术有效性的理解，为改进LLM安全防御提供了系统性的分类框架和数据集支持，特别是在多轮对话和跨语言场景下的安全防护。
...
TL;DR: 本文比较了两种作者归属方法：固定风格嵌入和指令调优的LLM法官（GPT-4o），在包含600个平衡实例的Human AI Parallel Corpus上进行测试，发现两种方法在不同领域各有优势，表明作者归属是一个需要混合策略的多维问题。
...
Method: 使用固定风格嵌入和指令调优的LLM法官（GPT-4o）两种方法，在Human AI Parallel Corpus数据集上进行测试，该数据集包含6个领域的600个平衡实例，每个实例都有人类提示、黄金延续和LLM生成的延续。

Result: 风格嵌入在GPT延续上表现更好（82% vs 68%），LLM法官在LLaMA延续上略优（85% vs 81%）。LLM法官在小说和学术散文领域显著优于风格嵌入，而风格嵌入在口语和剧本对话中表现更好。

Conclusion: 作者归属是一个多维问题，需要结合不同方法的混合策略。两种方法在不同领域各有优势：LLM法官具有语义敏感性，而风格嵌入在结构分析方面更强。
...
TL;DR: 窄域微调会在LLM激活中产生强烈偏差，这些偏差可以通过模型差异分析来理解微调领域。在随机文本前几个token上分析激活差异并添加到模型激活中，可以生成类似微调数据格式和内容的文本。
...
Method: 使用模型差异分析工具，在随机文本前几个token上分析激活差异，并通过将这些差异添加到模型激活中来引导文本生成。创建基于LLM的可解释性代理来理解微调领域。

Result: 窄域微调模型在其激活中具有明显的训练目标痕迹。访问这些偏差的代理性能显著优于使用简单提示的基线代理。混合预训练数据到微调语料库可以很大程度上消除这些偏差。

Conclusion: 窄域微调模型在其激活中包含训练目标的显著痕迹，这为改进训练方法提供了思路，同时警告AI安全研究人员使用此类模型作为研究更广泛微调的代理可能不现实。
...
TL;DR: RAID是一个针对大型语言模型的越狱攻击框架，通过优化连续嵌入空间中的对抗性后缀来绕过安全机制，同时保持流畅性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在各种任务中表现出色，但仍然容易受到越狱攻击的威胁，这些攻击会绕过安全机制。需要系统性地探测这些弱点。

Method: RAID将离散标记松弛为连续嵌入，并使用联合目标进行优化：(i)鼓励受限响应，(ii)加入拒绝感知正则化器以避开嵌入空间中的拒绝方向，(iii)应用连贯性术语保持语义合理性和非冗余性。优化后，通过批评引导的解码程序将嵌入映射回标记。
...
TL;DR: 该研究分析了大型语言模型在道德和政治领域的偏见，使用道德基础理论直接比较LLM响应与人类数据，评估模型是否表现出意识形态倾向。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在日常生活中的广泛应用，特别是在医疗、人际关系和法律等关键领域作为建议提供者，需要了解这些模型在政治和道德领域的潜在偏见。

Method: 应用道德基础理论框架，分析LLM的固有响应、明确政治意识形态提示和基于人口统计的角色扮演响应，与现有人类研究数据直接比较。

Result: 研究发现LLM响应存在政治和人口统计依赖性，模型在某些条件下会表现出特定的意识形态倾向。

Conclusion: LLM在道德和政治响应中存在系统性偏见，需要更深入理解这些偏见对AI生成内容的影响。
...
TL;DR: 提出了一种基于认知科学中图式理论的Schema Activated In-Context Learning (SA-ICL)框架，通过从示例中提取抽象推理模式来增强大语言模型的新任务适应能力。


<details>
  <summary>Details</summary>
Motivation: 传统基于示例的上下文学习缺乏在抽象层面进行知识检索和传递的显式模块，而人类认知研究表明人们通过激活已有的心理框架（图式）来理解新信息。

Method: SA-ICL从先前的示例中提取推理过程的认知构建块表示，创建轻量级的结构化模板（抽象图式），包含关键推理步骤及其关系，用于增强模型面对新问题时的推理过程。

Result: 在GPQA数据集的化学和物理问题上，SA-ICL显著提升了性能（最高达36.19%），特别是在单个高质量演示示例的情况下，同时减少了对演示数量的依赖并增强了可解释性。

Conclusion: SA-ICL不仅连接了从模式启动到思维链提示的各种ICL策略，还为增强LLMs中类人推理开辟了新路径，证明显式图式基架能显著提升模型性能。
...
TL;DR: 提出了Prompt Duel Optimizer (PDO)，一个无需标签的提示优化框架，通过成对偏好反馈和双人赌博机设置实现高效优化


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对输入提示高度敏感，但自动提示优化方法通常需要真实标签，而收集高质量标签成本高昂且耗时

Method: 将问题建模为双人赌博机设置，使用Double Thompson Sampling进行信息丰富的提示比较，结合Top-Performer Guided Mutation扩展候选池

Result: 在BIG-bench Hard和MS MARCO上的实验表明，PDO持续优于基线方法，消融研究验证了D-TS和提示突变的有效性

Conclusion: PDO提供了一种无需标签的高效提示优化方法，能够利用LLM判断的成对偏好反馈，在标签稀缺的实际场景中具有优势
...
TL;DR: 该研究探索了LLaMA 3.2-3B模型在算术计算中是否编码运算符优先级，发现中间计算结果存在于残差流中，特别是MLP块之后，且模型在线性编码运算符优先级。
...
Result: 中间计算结果显示在残差流中，特别是在MLP块之后；模型在注意力层后线性编码每个运算符的优先级。

Conclusion: LLM确实在内部表示中编码运算符优先级，这为理解模型算术推理机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM-based KGFMs在处理归纳KGR时存在LLM知识被稀疏KG上下文遮蔽导致的知识扭曲问题，以及难以完全约束LLM生成幻觉的问题

Method: 设计了KRL指令格式和分词器来对齐LLM知识和KG表示，提出KRL注意力层通过动态知识记忆机制协调内在LLM知识和额外KG上下文，使用结构感知的下一个实体预测器严格约束推理结果

Result: 在25个真实世界归纳KGR数据集上的实验结果表明，KRLM在零样本推理和微调场景下均表现出显著优势

Conclusion: KRLM通过统一协调LLM知识和KG上下文，有效解决了知识扭曲和生成幻觉问题，提升了归纳知识图谱推理的可信度
...
TL;DR: 提出了RAGCap-Bench基准，用于细粒度评估代理式RAG工作流中的中间任务能力，发现具有更强中间能力的模型能获得更好的端到端结果。
...
Method: 通过分析先进系统的输出识别常见任务和核心能力，构建典型LLM错误分类法，设计针对性评估问题。
...
TL;DR: 论文研究了AI辩论中语言模型的行为模式，发现在主观问题上，模型倾向于迎合法官的立场而非坚持自身先验信念，同时发现顺序辩论存在显著偏见，且与先验信念不一致的论点在质量评估中得分更高。


<details>
  <summary>Details</summary>
Motivation: 现有AI辩论实验依赖有标准答案的数据集，忽略了主观维度上的说谎问题。本研究旨在探索在主观问题中，语言模型是否会为了说服法官而采取迎合策略，还是坚守自身先验信念。

Method: 在主观问题上测量语言模型的先验信念，然后让辩手在法官立场与其先验信念冲突的情况下进行辩论，比较顺序和同时两种辩论协议，评估模型在辩护与自身信念一致或不一致立场时的说服力和论点质量。

Result: 模型倾向于选择与法官立场一致的立场而非自身先验信念；顺序辩论对第二位辩手存在显著偏见；模型在辩护与自身信念一致的立场时更具说服力；但与信念不一致的论点在成对比较中质量评分更高。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度研究代理在长时程推理方面存在挑战，传统的数据合成方法缺乏对难度和质量的精细控制，且难以评估数据本身的有效性。

Method: 采用两阶段数据合成管道，利用基线网络代理在多个角色中工作：尝试回答问题、验证事实性、检查替代答案和执行过滤。通过蒸馏训练设置评估数据有效性。

Result: 实验表明，尽管数据集规模较小，但训练出的网络代理在多个基准测试中表现优于现有数据集，工具使用多样性翻倍，避免了重复的工具调用行为。

Conclusion: 提出的数据合成方法能够生成更高质量的训练数据，显著提升网络代理在复杂问答任务中的性能，特别是在长时程推理和工具使用多样性方面。
...
TL;DR: 该论文挑战了可读性对小语言模型(SLMs)能力发展起关键作用的观点，发现统计简单性(通过n-gram多样性衡量)才是更强的可学习性预测指标。


<details>
  <summary>Details</summary>
Motivation: 反驳当前研究中将语言模型训练与人类认知发展进行类比的观点，质疑可读性在SLMs能力涌现中的核心作用，主张基于实证证据进行更精确的分析。

Method: 构建具有匹配结构但可读性不同的合成数据集，比较在简化语言和复杂成人级文本上训练的SLMs的表现，分析统计简单性与学习效率的关系。

Result: 在复杂文本上训练的模型与在简化语言上训练的模型表现相当，甚至在训练过程中更快发展出连贯性；统计简单性比可读性更能预测可学习性。
...
TL;DR: 提出Element2Vec方法，通过语言模型从维基百科文本生成化学元素的通用嵌入和属性高亮向量，解决元素特性预测中的复杂关系建模和数据稀疏问题。


<details>
  <summary>Details</summary>
Motivation: 化学元素特性数据对材料设计至关重要，但许多特性难以直接测量。传统数值分析方法无法建模复杂关系，而现有AI方法存在幻觉和缺乏可解释性问题。

Method: 使用语言模型从维基百科文本生成元素的通用嵌入（Global）和属性高亮向量（Local），并设计基于自注意力的测试时训练方法来减少回归误差。

Result: 成功开发了能够有效表示化学元素的向量化方法，解决了文本分布差异和数据稀疏的挑战。

Conclusion: 这项工作为材料科学中AI驱动发现铺平了道路，提供了更准确和可解释的元素特性预测方法。
...
TL;DR: 本文提出了一种优化过程奖励模型（PRM）在测试时扩展（TTS）中使用的理论框架和校准方法，通过加权聚合策略显著提升效率，仅需21.3%的计算量即可超越传统加权多数投票。


<details>
  <summary>Details</summary>
Motivation: 现有PRM在TTS中的表现有时甚至不如简单的多数投票，这引发了对如何有效利用PRM验证信号的关键问题。

Method: 开发理论框架分析LLM和PRM信号的最优组合策略，提出高效的预计算方法来校准加权函数。
...
TL;DR: FACTS是一个快速、准确且隐私合规的表格摘要方法，通过离线模板生成实现查询导向的表格摘要，解决了现有方法在推理能力、效率和隐私保护方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有表格摘要方法存在多个问题：表格到文本模型需要昂贵的微调且难以处理复杂推理；基于提示的LLM方法受限于token限制和效率问题，同时暴露敏感数据；现有的代理流程依赖分解、规划或手动模板，缺乏鲁棒性和可扩展性。

Method: FACTS采用代理工作流程，通过离线生成包含SQL查询和Jinja2模板的模板，这些模板可以在共享相同模式的多个表格中重复使用，实现快速摘要生成、通过可执行SQL查询确保准确性，并通过仅向LLM发送表格模式来保护隐私。

Result: 在广泛使用的基准测试中，FACTS始终优于基线方法，证明了其作为现实世界查询导向表格摘要的实用解决方案的有效性。

Conclusion: FACTS通过离线模板生成方法，提供了一个快速、准确且隐私合规的表格摘要解决方案，解决了现有方法的局限性，并在实际应用中表现出色。
...
TL;DR: 提出了一个基于LLM的AI代理框架，将原始数据包捕获转换为结构化语义表示，用于交互式IoT网络流量分析。


<details>
  <summary>Details</summary>
Motivation: IoT网络产生多样化高流量数据，需要跨层解释行为、协议和上下文，而不仅仅是孤立检测。

Method: 集成特征提取、基于Transformer的异常检测、数据包和流摘要、威胁情报增强以及检索增强问答的AI代理框架。

Result: 在多个IoT捕获和六个开放模型上的实验评估显示，混合检索（结合词汇和语义搜索与重排序）显著改善了BLEU、ROUGE、METEOR和BERTScore结果。系统分析表明CPU、GPU和内存开销低。

Conclusion: 该框架实现了对IoT网络流量的全面高效解释。
...
TL;DR: BioMedSearch是一个基于大语言模型的多源生物医学信息检索框架，通过整合文献检索、蛋白质数据库和网络搜索，结合子查询分解、关键词提取、任务图构建和多源信息过滤，显著提高了复杂生物医学问答的准确性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生物医学领域生成的内容缺乏科学严谨性，无法访问权威生物医学数据库，经常捏造蛋白质功能、相互作用和结构细节等不准确信息。

Method: 构建多源生物医学信息检索框架，整合文献检索、蛋白质数据库和网络搜索，通过子查询分解、关键词提取、任务图构建和多源信息过滤来生成高质量问答结果。

Result: 在包含3000个问题的BioMedMCQs数据集上，BioMedSearch在所有推理级别上都显著优于基线模型：Level 1准确率从59.1%提升到91.9%，Level 2从47.0%提升到81.0%，Level 3从36.3%提升到73.4%。

Conclusion: BioMedSearch通过多源信息整合和结构化处理，有效解决了LLMs在生物医学领域生成内容不准确的问题，显著提升了复杂生物医学问答的准确性。
...
TL;DR: LLM Brain Rot Hypothesis：持续接触垃圾网络文本会导致大语言模型认知能力持续下降。通过控制实验发现，在垃圾数据集上持续预训练会显著损害模型的推理、长文本理解、安全性等能力，并增加"黑暗特质"。


<details>
  <summary>Details</summary>
Motivation: 研究数据质量对LLM能力的因果影响，验证"脑退化"假设——垃圾网络文本是否会导致模型认知能力下降。

Method: 使用Twitter/X语料构建垃圾和控制数据集，采用两种正交操作化方法（M1：参与度，M2：语义质量），在匹配的token规模和训练条件下对4个LLM进行持续预训练实验。

Result: 垃圾数据集训练导致推理能力显著下降（Hedges' g>0.3），ARC-Challenge从74.9降至57.2，RULER-CWE从84.4降至52.3。发现思维跳过是主要错误机制，指令微调和干净数据预训练只能部分恢复能力。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLMs在生物医学领域药物安全预测中的可靠性，特别是它们是否会将临床无关的社会人口学信息纳入不良事件预测。

Method: 使用FDA不良事件报告系统结构化数据和基于角色的评估框架，评估ChatGPT-4o和Bio-Medical-Llama-3.8B模型在不同教育、婚姻、就业等角色定义下的表现。

Result: 发现系统性预测准确性差异，弱势群体被分配更高不良事件预测概率；识别出显性偏见（预测直接引用角色属性）和隐性偏见（预测不一致但未明确提及角色）两种模式。
...
TL;DR: 该论文提出了一种通过指令检索来增强小语言模型推理能力的方法，在推理时检索结构化推理过程而非生成，在多个专业领域任务上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 小语言模型虽然能在本地硬件高效运行，具有隐私保护、低成本、环保等优势，但在需要多步推理或领域知识的任务上表现不佳。

Method: 构建指令语料库，将相似训练问题分组并通过GPT-5创建指令。推理时，小语言模型检索最相关指令并遵循其步骤，提供结构化推理指导。

Result: 在MedQA、MMLU专业法律和MathQA上评估3B到14B参数模型，无需额外微调。指令检索带来一致提升：MedQA提升9.4%，MMLU Law提升7.9%，MathQA提升5.1%。简洁指令优于长指令，改进程度强烈依赖于模型家族和内在推理能力。

Conclusion: 指令检索是一种有效增强小语言模型推理能力的方法，为在本地硬件上实现大规模推理提供了可行方案。
...
TL;DR: 提出了HisRubric评估框架和FinDeepResearch基准，用于系统评估深度研究代理在金融分析中的能力，涵盖8个金融市场、4种语言的64家公司，包含15,808个评分项。
...
Method: 提出HisRubric分层分析框架，模拟专业分析师工作流程：从数据识别到指标计算，再到战略总结和解释。构建FinDeepResearch基准，对16种代表性方法进行广泛实验。

Result: 实验结果揭示了这些方法在不同能力、金融市场和语言方面的优势与局限，为未来研究提供了有价值的见解。

Conclusion: 该研究填补了深度研究代理评估的空白，提出的框架和基准将为未来研究和开发提供重要参考。
...
TL;DR: 通过微调AI模型在作者完整作品上，可以生成比专家人类写作更受读者偏好的文学文本，且成本大幅降低，这为版权法中的合理使用提供了实证证据。


<details>
  <summary>Details</summary>
Motivation: 研究AI模型是否能生成高质量文学文本并模仿作者风格，以回应关于使用受版权保护书籍训练AI模型的法律争议。

Method: 进行预注册研究，比较MFA训练的专业作家与三种前沿AI模型（ChatGPT、Claude、Gemini）在模仿50位获奖作家风格方面的表现，通过159名专家和普通读者的盲评进行对比。

Result: 上下文提示生成的AI文本被专家强烈否定，但经过作者特定微调后，专家反而偏好AI生成的文本，且AI检测器难以识别微调输出。

Conclusion: 作者特定的微调能够消除AI可检测的风格特征，使AI生成的非逐字文本在风格保真度和写作质量上优于专家人类写作，这对版权法的市场影响因素具有重要启示。
...
TL;DR: 论文提出MTI框架，通过仅在不确定位置应用选择性CFG干预和轻量级负提示引导，以最小开销提升推理准确性和稳定性。
...
Result: 在通用、编程和STEM任务上获得一致提升，如Qwen3-8B-Base在八个基准上平均提升1.35%，Qwen3-32B-Reasoning在AIME2024上提升5%，同时保持高效率。

Conclusion: MTI是一种无需训练的方法，能以最小开销有效提升推理性能，证明了局部化干预的有效性。
...
TL;DR: 本文提出了一个针对真实RAG系统中错误类型的新分类法，包括错误示例和实用建议，并创建了带错误类型标注的数据集，同时提出了与分类法一致的自动评估方法。
...
Method: 提出了RAG系统错误类型的新分类法，整理了带错误类型标注的数据集，并开发了与分类法一致的自动评估方法。

Result: 创建了包含错误类型标注的RAG响应数据集，并提供了实用的自动评估工具来跟踪和解决开发过程中的错误。

Conclusion: 该分类法、数据集和评估方法为RAG系统的错误分析和改进提供了实用框架，有助于提高系统部署的稳健性。
...
TL;DR: 德国Commons是迄今为止最大的公开许可德语文本集合，包含1545.6亿个token，涵盖7个领域的41个数据源，解决了德语公开许可训练数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大规模语言模型开发依赖大规模训练语料，但大多数语料许可状态不明确，限制了真正开放模型的开发。对于非英语语言，特别是德语，公开许可文本仍然严重稀缺。

Method: 从41个来源系统收集数据，涵盖法律、科学、文化、政治、新闻、经济和网络文本七个领域。实施全面的质量过滤、去重和文本格式修复处理流程，确保异构文本源的质量一致性。

Result: 构建了包含154.56亿个token的高质量德语文本集合，所有领域子集均采用至少CC-BY-SA 4.0或等效许可，确保模型训练和重新分发的法律合规性。

Conclusion: 德国Commons填补了德语公开许可预训练数据的关键空白，使真正开放的德语语言模型开发成为可能，同时发布了语料构建和德语文本数据过滤代码，确保完全可重现和可扩展。
...
TL;DR: CRaFT是一个基于解释的多语言评估框架，通过四个可解释指标评估LLM在不同文化背景下的推理能力，发现文化意识不是LLM固有的，而是通过语言框架显现的。


<details>
  <summary>Details</summary>
Motivation: 正确回答不一定反映文化理解，需要评估LLM在不同文化背景下的推理能力。

Method: 引入CRaFT框架，使用文化流畅性、偏差、一致性和语言适应性四个指标，对来自世界价值观调查的50个文化相关问题进行评估，涵盖阿拉伯语、孟加拉语和西班牙语。

Result: 结果显示显著的跨语言推理差异：阿拉伯语降低流畅性，孟加拉语增强流畅性，西班牙语保持稳定。GPT跨语言适应更好但一致性较低，FANAR推理稳定但僵化。

Conclusion: 文化意识在LLM中不是固有的，而是通过语言框架显现。CRaFT为评估多语言环境中的跨文化推理提供了新视角，为构建文化适应性语言模型提供了可行见解。
...
TL;DR: 该论文提出了GlobalGroup任务，用于评估大语言模型在多种语言中的抽象推理能力，发现英语模态通常表现更好，且开源和闭源模型之间存在性能差异。
...
Method: 构建了基于纽约时报Connections游戏的基准，涵盖英语、西班牙语、中文、印地语和阿拉伯语五种语言背景，并提出了游戏难度测量方法以进行更受控的比较。

Result: 实验发现英语模态在此抽象推理任务中表现更好，且开源和闭源模型之间存在性能差异。

Conclusion: 研究强调了在抽象推理任务中评估语言偏见的重要性，并展示了不同语言模态和模型类型之间的性能差异。
...
TL;DR: 该研究通过分布方法量化6种语言中音义象似性，发现新的可解释音义对应关系，并对先前假设进行了验证。
...
Method: 使用分布方法分析6种语言（英语、西班牙语、印地语、芬兰语、土耳其语、泰米尔语）中语素的语音和语义相似度空间对齐情况。

Result: 发现了一系列文献中未识别但可解释的音义对应关系，以及跨语言模式；对5个先前假设的音义对应关系获得支持或混合结果。

Conclusion: 语言中存在大规模的音义象似性现象，分布方法能有效识别新的音义对应关系。
...
TL;DR: ERGO通过监测LLM内部不确定性（香农熵）来动态调整多轮对话上下文，当检测到熵值急剧上升时触发提示整合，显著提升增量信息对话的性能和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在增量信息的多轮对话中性能显著下降，这严重影响了实际应用。研究者假设模型不确定性的急剧增加标志着对话不对齐，可以利用这一信号来动态调整上下文。

Method: 提出ERGO方法，通过持续量化下一个token分布的香农熵来监测内部不确定性，当检测到熵值急剧上升时触发自适应提示整合。

Result: 在增量指令的多轮任务中，ERGO相比标准基线平均性能提升56.6%，峰值性能能力提升24.7%，性能变异性降低35.3%。
...
Method: 提出DROID框架，结合通用句子编码器(USE)用于广泛语义泛化和领域自适应Transformer去噪自编码器(TSDAE)用于领域特定上下文区分，通过融合表示和轻量级分支分类器进行意图分离，并采用合成和开放域异常增强来加强边界学习。
...
TL;DR: CyberPal 2.0是一系列专为网络安全设计的小型语言模型(4B-20B参数)，通过SecKnowledge 2.0数据增强管道生成高质量网络安全指令数据集，在多项网络安全基准测试中表现优异，甚至超越了一些前沿大模型。
...
Method: 开发了SecKnowledge 2.0数据增强和格式化管道，集成专家指导和LLM驱动的多步基础验证，生成高质量、任务基础的网络安全推理轨迹。基于此训练了CyberPal 2.0系列小型语言模型。

Result: 在多样化网络安全基准测试中，CyberPal 2.0持续超越基线模型，匹敌或超越各种开源和闭源前沿模型。在核心网络威胁情报任务中排名第二(仅次于Sec-Gemini v1)，在威胁调查任务中20B参数模型排名第一，4B参数模型排名第二。

Conclusion: CyberPal 2.0证明了通过专业领域数据增强和专家指导，小型语言模型可以在网络安全任务中达到甚至超越大型前沿模型的性能，为网络安全领域的AI应用提供了高效解决方案。
...
Method: 通过网页抓取和结构化解析构建数据集，处理异质食材描述（包括单位、数量和描述符标准化），使用点互信息和提升度分析食材共现模式。

Result: 创建了马其顿食谱数据集，通过探索性分析揭示了马其顿菜肴中独特的食材组合特征。
...
TL;DR: 提出RLSR方法替代SFT，使用强化学习框架和语义相似度奖励来提升模型的指令遵循能力，在多个基准测试中表现优于传统SFT方法。


<details>
  <summary>Details</summary>
Motivation: 传统SFT方法依赖下一个token预测目标，而RFT展示了强化学习在领域适应中的有效性。受此启发，作者希望将强化学习应用于大规模SFT数据集，以更有效地提升基础模型的指令遵循能力。

Method: RLSR方法：基础模型为每个提示生成多个响应，在语义嵌入空间中计算生成响应与人工标注响应之间的余弦相似度作为奖励分数，通过强化学习框架进行训练。

Result: RLSR单独使用时，在Qwen-7B模型上AlpacaEval胜率达到26.34%，优于SFT的21.01%；与SFT结合使用时，胜率进一步提升至30.73%。

Conclusion: RLSR不仅能有效替代SFT，还能与SFT结合进一步提升下游任务性能，为模型指令遵循能力的优化提供了新的强化学习框架。
...
TL;DR: 提出了动态角色精炼框架(DPRF)，通过迭代识别认知差异并优化角色配置文件，显著提升大语言模型角色扮演代理的行为对齐效果。
...
Method: DPRF通过自由形式或基于理论的结构化分析，迭代识别生成行为与人类真实行为之间的认知差异，并优化角色配置文件以减少这些差异。

Result: 在四种不同行为预测场景（正式辩论、社交媒体帖子、公共访谈和电影评论）上评估，DPRF能持续显著改善行为对齐，并在不同模型和场景中具有良好泛化性。
...
TL;DR: LiteStage是一个针对多阶段推理的延迟感知层跳过框架，通过阶段化离线搜索和在线置信度早期退出机制，在保持精度的同时实现高达1.70倍加速


<details>
  <summary>Details</summary>
Motivation: 多阶段推理虽然能提升小语言模型的推理能力，但增加了延迟。现有自适应加速技术如层跳过难以平衡效率和精度，面临阶段间跳过敏感度差异和冗余输出token生成两大挑战

Method: 提出LiteStage框架，结合阶段化离线搜索分配最优层预算，以及基于置信度的在线生成早期退出机制来抑制不必要的解码

Result: 在OBQA、CSQA和StrategyQA三个基准测试上，LiteStage实现了高达1.70倍的加速，精度损失小于4.0%，优于先前的免训练层跳过方法

Conclusion: LiteStage有效解决了多阶段推理中的延迟问题，在保持推理精度的同时显著提升了效率
...
Method: F²C包含两个关键组件：Consensus Cross-Entropy (CCE)使用多数投票创建硬伪标签；表示对齐损失将低置信度预测器向多数投票建立的共识对齐。
...
TL;DR: 提出了MoM框架，将RAG从被动分块转向主动理解，模拟人类阅读认知过程，通过多路径采样和多视角评估机制选择最优文档记忆，并融入逆向推理策略训练小语言模型。


<details>
  <summary>Details</summary>
Motivation: 传统RAG范式仅理解相关文本块，限制了知识内化深度和推理能力，需要转向主动理解文档的认知过程。

Method: 使用LLM模拟领域专家生成文档逻辑大纲，指导结构化分块和核心内容提取；采用多路径采样和多视角评估机制选择最优文档记忆；融入逆向推理策略训练SLM；开发三层文档记忆检索机制。

Result: 在三个不同领域的实验表明，MoM框架解决了现有RAG系统的文本分块问题，为LLM提供语义完整的文档记忆，并使SLM能够实现以人为中心的智能文本处理。

Conclusion: MoM框架不仅提升了RAG系统的性能，还为小语言模型实现人类认知水平的文本处理能力开辟了新途径。
...
TL;DR: 提出了一个实验方法来研究训练数据与语言模型行为的关系，通过干预数据批次（"重写历史"）并重新训练模型来测试数据与行为之间的假设
...
Method: 通过选择评估项目、匹配相关文档、修改文档、重新训练并测量效果的步骤来干预数据批次

Result: 现有识别相关训练文档的方法不能完全解释语言模型正确回答知识问题的能力

Conclusion: 提供了一个可复用的实验方法框架，供研究者进一步测试训练数据如何影响模型行为的假设
...
TL;DR: DEG-RAG是一个去噪知识图谱框架，通过实体解析和三元组反思技术来清理LLM生成的知识图谱，从而提高基于图的RAG系统的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的RAG系统依赖LLM自动构建知识图谱，但生成的图谱通常包含冗余实体和不可靠关系，这种噪声会降低检索和生成性能并增加计算成本。

Method: 提出DEG-RAG框架，包含两个核心技术：(1) 实体解析：消除冗余实体；(2) 三元组反思：移除错误关系。同时系统评估了实体解析中的不同策略。

Result: 实验表明该方法不仅显著减小图谱规模，还能在各种流行的基于图的RAG变体中持续提升问答性能。

Conclusion: DEG-RAG通过去噪技术生成更紧凑、高质量的知识图谱，有效解决了LLM生成知识图谱中的噪声问题，提升了基于图的RAG系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决小型多语言模型在检索任务中表现落后于大型模型的问题，探索能否通过特定优化提升小型模型的检索性能。

Method: 分析训练数据规模、负采样策略和数据多样性对多语言嵌入模型效果的影响，重点关注硬负样本的使用和任务多样性。

Result: 发现增加训练数据规模初期有效但收益递减，硬负样本对提升检索准确性至关重要，任务多样性比语言多样性对性能贡献更大。

Conclusion: 成功开发出仅300M参数的紧凑多语言模型，其检索性能可与7B模型相媲美甚至更优，证明小型模型通过针对性优化可在检索任务中表现出色。
...
TL;DR: 提出了Qwen3Guard系列安全护栏模型，包含生成式和流式两种变体，支持多语言和实时安全监控，在安全分类方面达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有护栏模型存在两个主要限制：只能输出二元安全标签，无法适应不同领域的安全容忍度；需要完整模型输出才能进行安全检查，不兼容流式推理。

Method: 开发了两种专门变体：生成式Qwen3Guard将安全分类转换为指令跟随任务，实现三分类判断；流式Qwen3Guard引入token级分类头，支持实时安全监控。

Result: 在英语、中文和多语言基准测试中，Qwen3Guard在提示和响应安全分类方面达到了最先进的性能。

Conclusion: Qwen3Guard提供了全面、可扩展和低延迟的安全调节，适用于全球LLM部署，所有模型都基于Apache 2.0许可证公开发布。
...
TL;DR: 提出了一种基于智能代理的检索系统，通过三个专门代理（问题分析器、选择器和添加器）在结构化循环中迭代检索证据，显著提升了多跳问答中的检索精度和召回率。
...
Result: 在四个多跳问答基准测试（HotpotQA、2WikiMultiHopQA、MuSiQue和MultiHopRAG）上，该方法持续优于强基线，实现了更高的检索精度，同时过滤掉干扰内容。

Conclusion: 该代理检索系统能够生成高质量的证据集，使下游问答模型在依赖更少无关信息的情况下超越全上下文答案的准确性。
...
Method: 结合表优先检索后列选择和列优先检索后表选择两种互补策略，辅以问题分解、关键词提取和关键短语提取技术。

Result: 在BIRD和Spider等基准测试中显著提高模式召回率并减少误报，SQL生成性能优于全模式基线并接近oracle性能，无需查询优化。
...
<details>
  <summary>Details</summary>
Motivation: 学术出版物快速增长给论文分析带来挑战，现有论文数据库仅限于相似性匹配和基本分类，无法深入探索概念间的关系网络。

Method: 基于OpenAlex知识图谱，分析近8000篇开源论文数据，提出基于提示工程的关键概念路径分析方法，利用小语言模型进行概念提取和创新点识别，构建基于知识图谱约束机制的智能体。

Result: 通过对Qwen和DeepSeek模型进行微调，在准确性方面取得了显著提升，模型已在Hugging Face平台公开。

Conclusion: 论文关键概念路径分布模式与创新点和稀有路径存在强相关性，提出的方法能有效提升学术论文分析的深度和准确性。
...
TL;DR: MathMist是一个多语言数学推理基准，包含7种语言的21K+对齐问答对，评估LLM在跨语言数学推理中的表现，发现在低资源语言中性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注英语或高资源语言，缺乏对多语言和跨语言数学推理能力的全面评估，需要填补这一空白。

Method: 构建包含7种语言的平行多语言数据集，系统评估开源和专有LLM在零样本、思维链和代码切换推理范式下的表现。

Result: LLM在不同语言间的数学推理能力存在显著差异，在低资源语言环境中性能明显下降，缺乏一致性和可解释性。

Conclusion: 多语言数学推理仍是LLM的挑战领域，需要更多关注低资源语言的推理能力提升。
...
Method: 构建包含5种语言（印地语、日语、印尼语、越南语、泰米尔语）的多模态数据集，使用LLaMa-2和Aya-23等多语言模型进行基准测试。

Result: 视觉数据的加入提高了实体链接的准确性，特别是在文本上下文模糊或不足的情况下，对于多语言能力较弱的模型效果更明显。

Conclusion: 多模态方法在多语言实体链接任务中具有重要价值，视觉信息能够有效补充文本信息的不足。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在与用户交互时会产生欺骗性输出，无论是故意还是无意的，这带来了严重的安全隐患。模型行为的不可预测性、对幻觉和错误信息的防护不足，使得其滥用成为现实风险。

Method: 提出了信念错位指标来量化欺骗，在四种对话场景中评估欺骗行为，使用五个现有欺骗检测指标和提出的新指标。引入了多轮强化学习方法来微调LLMs以减少欺骗行为。

Result: 新提出的欺骗指标与人类判断更相关；LLMs在约26%的对话轮次中自然表现出欺骗行为；被提示欺骗时，欺骗性可增加31%；RLHF训练模型仍有43%的欺骗率；多轮强化学习方法减少了77.6%的欺骗行为。

Conclusion: 对话中的欺骗行为是随着交互历史发展的，需要超越单轮话语分析。提出的多轮强化学习方法能有效减少LLMs的欺骗行为，为LLM安全提供了重要改进方向。
...
TL;DR: 开发了一种基于混合词嵌入和超参数调优的阿尔茨海默病早期检测方法，通过结合Doc2Vec和ELMo的词向量获得句子困惑度分数，并添加语言特征，最终在逻辑回归模型中实现了91%的分类准确率和97%的AUC值。
...
Method: 创建基于Doc2Vec和ELMo词向量的混合词嵌入来获取句子困惑度分数，识别句子流畅度并捕捉语义上下文。通过添加语言特征丰富词嵌入，分析语法和语义，然后将嵌入特征向量输入逻辑回归模型并微调超参数。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM角色扮演系统在忠实呈现多版本角色方面存在不足，特别是像漫威、DC这样的超级英雄宇宙中同一角色的不同版本具有独特历史、价值观和道德准则。

Method: 创建包含30个标志性英雄90个特定版本的基准测试，包含两个任务：经典事件（事实回忆）和道德困境（伦理场景）。提出Think-Act Matching指标评估推理与行动的一致性。

Result: 实验发现：思维链提示能提升弱模型的叙事连贯性但会降低强模型的准确性；跨版本泛化仍是主要挑战；模型通常擅长思考或行动但很少两者兼备。
...
Result: 在MedQA、MedMCQA和PubMedQA三个医学基准测试中表现优异，特别是PubMedQA达到95.0%，MedMCQA达到78.0%。消融研究证实置信度感知路由与多模型协作显著优于单模型方法和统一推理策略。

Conclusion: 战略性模型合作为改进医学AI系统提供了实用且计算高效的途径，对在资源有限环境中普及先进医学AI具有重要意义。
...
TL;DR: 该研究探讨了现代大语言模型对字符级扰动的鲁棒性，通过在每个输入字符后插入不可见Unicode控制字符来测试模型性能。尽管这种干扰严重破坏了分词并降低了信噪比，许多LLM仍能保持显著性能。


<details>
  <summary>Details</summary>
Motivation: 研究LLM对结构化字符级扰动的鲁棒性，旨在防止LLM在在线考试系统等场景中的滥用，并评估其在不同应用中的部署可靠性。

Method: 提出了NAMESHORT方法，在文本中插入不可见Unicode控制字符来干扰LLM，通过模型、问题和噪声相关配置的全面评估，探索字符级分词处理机制和隐式/显式去噪假设。

Result: 令人惊讶的是，尽管存在强烈的混淆效果，许多LLM仍能保持显著性能，显示出对字符级噪声的强鲁棒性。

Conclusion: 研究揭示了LLM在低层级鲁棒性方面的特性，这对理解LLM滥用的风险和在不同应用中部署的可靠性具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 为美国6800万非英语母语者提供准确、及时的气象信息，建设天气准备国家，确保所有美国人都能获得关键气象预警。

Method: 与LILT合作，利用大型语言模型和神经机器翻译技术，结合GIS地图分析语言需求分布，并融入伦理AI实践确保透明度、公平性和人工监督。

Result: 开发出可扩展的自动翻译系统，显著减少人工翻译时间，减轻工作负担，创建了包含预警、7天预报和教育活动的多语言气象产品网站。

Conclusion: 该项目使美国向覆盖所有公民的国家预警系统迈进了一步，通过技术手段实现了更公平的气象信息服务。
...
TL;DR: 该论文提出了pluri-hop问题（需要聚合所有文档的问题），创建了PluriHopWIND数据集，并开发了PluriHopRAG方法，通过文档级子问题分解和交叉编码器过滤来提高性能。


<details>
  <summary>Details</summary>
Motivation: 现有问答系统在处理需要聚合所有文档的pluri-hop问题时表现不佳，特别是在重复性高、干扰文档多的现实场景中。

Method: 提出PluriHopRAG方法：将查询分解为文档级子问题，使用交叉编码器过滤无关文档，再进行LLM推理。

Result: PluriHopRAG在F1分数上相对提升了18-52%，而传统方法最高不超过40%。

Conclusion: PluriHopWIND暴露了当前QA系统在处理重复性语料时的局限性，PluriHopRAG证明了穷尽检索和早期过滤的价值。
...
Result: 统计分析和LLM实验结果表明，纳入评论树数据能显著增强用户自杀风险水平的区分和预测能力。
...
TL;DR: 提出"Your Next Token Prediction (YNTP)"任务，通过受控的人机对话建模用户精确的词汇选择，构建多语言基准数据集，评估基于提示和微调的个性化方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用下一个词预测方面表现出色，但难以生成反映个人真实沟通风格的回复，如回复邮件或社交消息。真实SNS或邮件历史因隐私问题难以收集。
...
Conclusion: YNTP任务能够有效建模用户的精确词汇选择，通过受控对话解决了隐私数据收集问题，为开发更符合用户个人风格的语言模型提供了重要基础。
...
TL;DR: 提出MedTrust-Guided Iterative RAG框架，通过引用感知推理、迭代检索验证和MedTrust对齐模块，显著减少医学问答中的幻觉问题，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于RAG的医学问答系统存在幻觉问题，主要源于检索后噪声和证据验证不足，影响了回答的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型在处理对现实应用至关重要的多约束指令时表现不佳，现有强化学习方法依赖外部监督且多约束任务奖励信号稀疏。

Method: 采用约束分解策略和高效的逐约束二元分类，直接从指令推导奖励信号并生成伪标签进行奖励模型训练。

Result: 在3个领域内和5个领域外数据集上均取得显著改进，包括具有挑战性的代理和多轮指令跟随任务。

Conclusion: 该方法能够很好地泛化，有效解决了多约束指令跟随问题，且数据和代码已公开。
...
TL;DR: 提出了Explore to Evolve范式来构建可验证的Web代理训练数据，开发了WebAggregatorQA数据集和WebAggregator模型系列，在信息聚合能力上超越GPT-4.1和接近Claude-3.7-sonnet。
...
Method: 采用Explore to Evolve范式：先通过主动在线探索从真实网络获取基础信息，然后使用收集的证据通过选择和组合12种高级逻辑操作来自我演化聚合程序，生成可验证的QA对。

Result: 构建了包含10K样本、覆盖50K网站和11个领域的WebAggregatorQA数据集。WebAggregator-8B模型性能与GPT-4.1相当，32B版本在GAIA-text上超越GPT-4.1超过10%，接近Claude-3.7-sonnet。在WebAggregatorQA评估集上，Claude-3.7-sonnet仅得28%，GPT-4.1得25.8%。

Conclusion: 即使代理能够检索所有参考资料，在WebAggregatorQA上仍然表现不佳，突显了加强Web代理基础信息聚合能力的必要性。
...
TL;DR: NLT框架用自然语言输出替代LLMs中的程序化JSON工具调用，通过解耦工具选择和响应生成，显著提升工具调用性能
...
Conclusion: NLT框架显著改善工具调用性能，这些改进在提示扰动下持续存在，并能扩展缺乏原生支持的模型的工具调用能力
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在低资源语言（如乌尔都语、泰语）上性能不足的问题，这些语言由于训练数据有限、机器翻译噪声和跨语言对齐不稳定而表现较差。

Method: 提出LiRA训练框架，包含两个模块：Arca通过锚点对齐和多智能体协作编码将低资源语言锚定到英语语义空间；LaSR在Arca的多语言表示基础上添加语言感知轻量级推理头，使用一致性正则化统一训练目标。

Result: 在低资源基准测试（跨语言检索、语义相似性和推理）中，在少样本和噪声放大设置下均表现出持续增益和鲁棒性，消融实验验证了Arca和LaSR的贡献。

Conclusion: LiRA框架有效提升了低资源语言的跨语言表示能力，同时增强了检索和推理的鲁棒性，为低资源语言处理提供了有效解决方案。
...
Result: 在OntoNotes上，最佳模型仅比全前缀增量基线低0.6个CoNLL F1点，压缩比为1.8；在LitBank上达到最先进性能。

Conclusion: 在序列到序列共指消解器中丢弃大量标记是增量共指消解的一种可行策略。
...
TL;DR: 本文构建了一个新的数据集和分析框架，用于评估主权LLMs的社会文化元素和技术稳健性，发现虽然主权LLMs在支持低资源语言方面有意义，但并未完全满足服务目标用户的主张，且可能低估安全等关键质量属性。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏验证主权LLMs是否与用户社会文化背景对齐以及是否保持安全性和技术稳健性的框架和数据集，需要填补这一空白。

Method: 构建新数据集并引入分析框架，提取和评估主权LLMs的社会文化元素，同时评估其技术稳健性。

Result: 实验结果显示，主权LLMs在支持低资源语言方面有意义，但并未完全满足服务目标用户的主张，且追求未经检验的主张可能导致低估安全等关键质量属性。
...
TL;DR: 提出了WritingPreferenceBench数据集，发现在创意写作偏好学习中，传统序列奖励模型表现不佳（52.7%准确率），而生成式奖励模型通过显式推理链达到81.8%准确率。模型在不同写作类型间存在显著性能差异，表明当前RLHF方法主要学习检测客观错误而非主观质量偏好。


<details>
  <summary>Details</summary>
Motivation: 现有偏好学习方法在标准基准上表现良好，但当移除客观质量信号时性能显著下降。需要评估模型在主观质量偏好（如创意、风格、情感共鸣）上的能力。

Method: 构建了包含1,800个人工标注偏好对的数据集（1,200英文，600中文），涵盖8种创意写作类型，确保回答在客观正确性、事实准确性和长度上匹配。比较了序列奖励模型、零-shot语言模型评判和生成式奖励模型的性能。

Result: 序列奖励模型平均准确率52.7%，零-shot语言模型53.9%，而生成式奖励模型达到81.8%准确率。模型在不同写作类型间性能差异显著（18.2%-81.8%），标准差平均10.1%。模型规模（8B vs 27B）未带来一致改进。

Conclusion: 当前RLHF方法主要学习检测客观错误而非捕捉主观质量偏好。成功的偏好建模可能需要中间推理表示而非直接分类，生成式奖励模型通过显式推理链表现出色。
...
TL;DR: 本文提出了CodeSeq数据集，通过将数字序列包装成算法问题来训练LLMs进行归纳推理，结合反思机制和强化学习，提升模型在各种推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有归纳推理研究面临两个挑战：现有数据主要关注表面规律而缺乏复杂内部模式；当前方法仅通过提示或简单微调，未提供精确思维过程或难度控制。

Method: 构建CodeSeq合成后训练数据集，将数字序列转化为算法问题来发现通项公式；通过反思失败测试用例和迭代修正生成监督微调数据；使用基于可解性和自生成案例成功率的强化学习奖励机制。
...
TL;DR: 提出了RLAIF-SPA框架，使用强化学习从AI反馈来提升情感语音合成的表现力和自然度，通过ASR和LLM技术分别评估语义准确性和韵律情感对齐。
...
Method: 采用RLAIF机制，利用ASR和LLM分别判断语义准确性和韵律情感标签对齐，作为表达力和清晰度的直接奖励。通过韵律标签对齐在结构、情感、速度和音调四个维度增强表达质量。

Result: 在Libri Speech数据集上的实验表明，RLAIF-SPA优于Chat-TTS，WER降低26.1%，SIM-O提高9.1%，人工评估提升超过10%。

Conclusion: RLAIF-SPA框架有效提升了情感语音合成的表达力和自然度，通过直接优化语义准确性和韵律情感对齐取得了显著改进。
...
TL;DR: 提出了“nugget-as-rubric”统一可验证范式，将原子信息点作为结构化评估标准，并开发了Search-Gen-V验证器，用于增强搜索增强型大语言模型的奖励建模。


<details>
  <summary>Details</summary>
Motivation: 现有搜索增强型LLM的奖励建模存在局限：基于规则的奖励对表达变化敏感且不适用于长文本任务，而生成式奖励在动态语料库中设计可验证且稳定的奖励具有挑战性且计算成本高。

Method: 提出了“nugget-as-rubric”范式，将原子信息点作为结构化评估标准；设计了基于查询重写的自动标准构建流程；开发了4B参数的Search-Gen-V生成式验证器，采用蒸馏思想和两阶段训练策略。

Result: 实验结果表明，Search-Gen-V在不同工作负载下实现了强大的验证准确性，成为搜索增强型LLM的可扩展、鲁棒且高效的可验证奖励构建器。

Conclusion: 提出的nugget-as-rubric范式和Search-Gen-V验证器为搜索增强型LLM提供了一种统一、可验证且高效的奖励建模解决方案，克服了现有方法的局限性。
...
TL;DR: 提出了一种教机器翻译模型理解语义韵律的方法，特别关注中文"被"字句的负面语义韵律，通过构建数据集和微调模型来改善翻译准确性。


<details>
  <summary>Details</summary>
Motivation: 语义韵律是语言单位与共现搭配形成的意义，不同于语义意义。当前机器翻译模型无法处理语义韵律问题，导致翻译不准确，特别是对于字面翻译相同但语义韵律不同的词语。

Method: 构建了英汉句子对数据集，展示"被"字句的负面语义韵律。然后对OPUS-MT、NLLB-600M和mBART50模型进行微调，用于英汉翻译任务。

Result: 微调后的机器翻译模型在翻译不利内容时能更好地使用"被"字句，在中性和有利内容中避免使用。NLLB-600M模型还能将语义韵律知识从英汉翻译迁移到其他语言对（如西班牙语-中文）。

Conclusion: 通过专门的数据集和微调，可以有效教机器翻译模型理解特定结构的语义韵律，提高翻译准确性，且这种知识在多语言模型中具有可迁移性。
...
TL;DR: 提出了一个以人为中心的框架，通过生成用户故事和多智能体讨论，帮助人们在AI系统部署前创造性思考潜在的利益和危害。


<details>
  <summary>Details</summary>
Motivation: AI在医疗领域的快速发展可能带来偏见、隐私侵犯和不平等访问等风险，而现有自动风险检测方法可能减少人类参与理解危害产生的方式和受影响人群。

Method: 开发了一个人类中心框架，生成用户故事并支持多智能体讨论，在用户研究中让参与者阅读故事并评估AI系统的潜在影响。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型从感知任务发展到复杂多步推理，但仅奖励最终答案正确性的强化学习方法会导致虚假推理，需要过程级监督来改进。

Method: 提出可扩展的自聚合方法，从成功轨迹中提取一致推理检查点，无需人工标注或更强教师模型即可构建问题特定量规，联合利用量规奖励和结果奖励。

Result: 在六个多模态推理基准测试中达到最先进性能，并在专门评估中显著提高推理忠实度。

Conclusion: AutoRubric-R1V通过自动量规生成和过程监督，有效提升了多模态推理的准确性和忠实度。
...
TL;DR: Pluto是一个用于评估LLM生成Verilog代码效率的基准测试框架，包含114个问题、自检测试平台和帕累托最优参考实现。实验显示LLM功能正确性达78.3%，但综合效率仅约64%，远低于专家实现。
...
Result: 最先进LLM在功能正确性上达到78.3%（pass@1），但在综合效率上表现较差：面积效率63.8%、延迟效率65.9%、功耗效率64.0%（eff@1）。

Conclusion: 需要像Pluto这样的效率感知评估框架来推动硬件导向的LLM研究，当前LLM在硬件设计效率方面仍有显著差距。
...
### [209] [COIG-Writer: A High-Quality Dataset for Chinese Creative Writing with Thought Processes](https://arxiv.org/abs/2510.14763)
...
TL;DR: COIG-Writer是一个中文创意写作数据集，通过逆向工程高质量文本捕获创作过程和结果，包含1,665个精心策划的三元组，涵盖51种文体。研究发现创意写作需要过程监督和通用数据的结合，创意能力具有文化特异性且无跨语言迁移，词汇多样性与创意质量呈负相关。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在创意写作方面存在系统性缺陷，特别是在非英语环境中，训练数据稀缺且缺乏过程级监督。为了解决这个问题，需要开发能够捕捉创作过程的中文创意写作数据集。

Method: 通过系统逆向工程高质量文本构建COIG-Writer数据集，包含1,665个三元组（逆向工程提示、详细创作推理、最终文本），涵盖51种文体。通过综合实验识别创意写作的两组件模型：叙事逻辑（过程监督提供）和语言表达（通用数据维持）。

Result: 研究发现：(1)过程监督非常有效但需要通用数据稳定，至少1:12的创意与通用样本比例才能达到最佳性能；(2)创意能力具有文化特异性，中文和英文表现差距达89.26个百分点；(3)词汇多样性与创意质量呈负相关（TTR悖论），高多样性暗示逻辑缺陷的补偿行为。

Conclusion: 创意卓越源于逻辑支架和语言基础的相互作用，类似于数学推理增强但不能取代基础模型中的语言能力。过程监督和通用数据的适当结合是提升创意写作质量的关键。
...
### [210] [Supervised Fine-Tuning or Contrastive Learning? Towards Better Multimodal LLM Reranking](https://arxiv.org/abs/2510.14824)
*Ziqi Dai,Xin Zhang,Mingxin Li,Yanzhao Zhang,Dingkun Long,Pengjun Xie,Meishan Zhang,Wenjie Li,Min Zhang*

Main category: cs.CL

TL;DR: 本文比较了对比学习(CL)和监督微调(SFT)在LLM重排序中的效果，发现SFT在权重更新方面显著优于CL，从而在LLM重排序任务中表现更好。
...
Method: 将目标函数分解为权重和方向两个组件，在通用多模态检索(UMR)平台上进行对比实验，并通过大规模训练验证发现。

Result: SFT提供了比CL更强的权重更新方案，而评分方向没有明显差异，SFT在LLM重排序中具有一致优势，在MRB基准测试中创造了新的最先进结果。

Conclusion: 监督微调(SFT)在LLM重排序中比对比学习(CL)更具优势，主要原因是SFT提供了更强的权重更新机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型评估方法对答案提取算法敏感，导致结果不稳定，需要一种更可靠的评估方法。

Method: 提出Answer Regeneration框架：在原始输入输出前添加"Answer:"提示，通过额外模型推理重新生成答案，然后从中提取最终答案。
...
TL;DR: 该研究发现当前多模态生成模型在处理方言文本输入时存在显著性能下降，构建了包含6种英语方言的大规模基准测试，并提出了一种编码器缓解策略来同时提升方言性能并保持标准英语性能。


<details>
  <summary>Details</summary>
Motivation: 研究多模态生成模型是否能有效处理方言文本输入，因为方言使用者经常与生成模型交互，但当前模型对方言的适应性尚未得到充分研究。

Method: 构建包含6种英语方言的4200多个提示的大规模基准，评估17个图像和视频生成模型，设计基于编码器的缓解策略来教模型识别新方言特征同时保持标准英语性能。

Result: 当前最先进的多模态生成模型在使用单个方言词时性能下降32.26%至48.17%，提出的方法能将5种方言性能提升至与标准英语相当（+34.4%），且对标准英语性能影响极小。

Conclusion: 多模态生成模型在处理方言输入时存在显著性能差距，但通过专门的编码器缓解策略可以有效解决这一问题，实现方言与标准英语性能的平衡。
...
Method: 采用两阶段循环：在预填充阶段和定期间隔中，基于已生成序列通过自我监督优化路由决策；然后正常生成文本，保持修改后的路由器直到下一次适应。通过轻量级加法向量仅更新选定层的路由器对数，保持计算效率。

Result: 在挑战性推理任务上获得持续性能提升，同时保持对上下文偏移的鲁棒性。例如，在HumanEval上获得5.5%的改进，与自一致性结合时在DeepSeek-V2-Lite上获得6%的平均增益。

Conclusion: 该方法提供了一种无需数据、即插即用的测试时适应方案，能够有效改善MoE模型的路由决策，并自然补充现有的测试时缩放技术。
...
<details>
  <summary>Details</summary>
Motivation: 尽管中期训练在实践中很流行，但对其有效性的科学理解很少。本研究旨在通过对照实验系统探究中期训练阶段的作用机制和效果。

Method: 通过从头预训练语言模型并在不同领域的有监督微调数据集上进行对照实验，研究了中期训练的时间安排和混合权重等关键参数。

Result: 中期训练在数学和代码领域效果最显著，能减少预训练与后训练数据之间的语法差距，在领域内验证损失和预训练数据遗忘方面均优于持续预训练。时间安排比混合权重影响更大，较早引入专业数据能带来更大收益。
...
TL;DR: MLIR-AIR是一个基于MLIR的开源编译器栈，专门为AMD NPU等空间架构设计，通过AIR方言提供异步和分层操作的结构化表示，实现编译器管理的空间调度和计算通信重叠。


<details>
  <summary>Details</summary>
Motivation: 通用编译器在抽象并行性、局部性和同步方面存在局限，无法充分利用现代空间架构的细粒度数据移动和执行控制能力，需要新的编译器基础设施来协调计算和数据。

Method: 基于MLIR构建MLIR-AIR编译器栈，定义AIR方言提供异步和分层操作的结构化表示，支持空间调度、计算分布和通信计算重叠，无需依赖临时运行时协调或手动调度。

Result: 在矩阵乘法中达到78.7%的计算效率，性能接近手动优化的MLIR-AIE实现；多头注意力仅用约150行代码实现融合实现，高效映射到空间硬件。
...
TL;DR: 提出一种结合系统化合成数据生成、三元组损失和分层模型合并的新方法，显著提升了RAG系统中LLM输出的一致性，响应相似度提高约47.5%。


<details>
  <summary>Details</summary>
Motivation: LLM在语义等价输入下经常产生不一致输出，且缺乏一致性训练数据和现有微调技术存在局限性，影响了RAG系统的可靠性。

Method: 结合系统化合成数据生成、三元组损失优化嵌入表示，以及基于中间层激活的一致性感知权重的分层模型合并方法。

Result: 合并后的模型显著提升了输出一致性，响应相似度相比基线提高了约47.5%。
...
TL;DR: 提出了一个可解释的框架，联合建模下游任务性能与训练计算量和上下文长度的关系，在三个推理任务上验证了该框架的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统的扩展定律无法捕捉下游任务性能，而上下文在下游任务中起着关键作用，需要建立能够关联训练计算量和上下文利用的框架。

Method: 提出了一个简单可解释的框架，通过拟合Llama-2-7B和Llama-2-13B在三个任务上的65,500个实例的下游性能数据，建模性能与训练计算量和上下文的关系。
...
TL;DR: 该研究评估了机器学习模型在心理健康筛查中的有效性，使用553个真实世界半结构化访谈数据，在抑郁症、焦虑症和PTSD诊断中达到超过80%的准确率，特别是PTSD检测准确率高达89%。


<details>
  <summary>Details</summary>
Motivation: 心理健康障碍是全球残疾的主要原因，但在初级保健环境中，抑郁症和焦虑症的误诊率超过60%，迫切需要可扩展、易获取且情境感知的诊断工具来支持早期检测和干预。

Method: 使用553个真实世界半结构化访谈数据集，评估了多种机器学习模型，包括GPT-4.1 Mini和MetaLLaMA的零样本提示，以及使用LoRA微调的RoBERTa模型。

Result: 模型在所有诊断类别中达到超过80%的准确率，PTSD检测表现尤为突出（准确率89%，召回率98%）。使用较短、聚焦的上下文片段能提高召回率，LoRA微调在低秩配置下保持竞争力。

Conclusion: 基于LLM的模型相比传统自报告筛查工具能提供显著改进，为在资源匮乏或高污名化环境中整合机器学习到临床工作流程奠定了基础。
...
TL;DR: 提出LaSeR算法，通过最后一词自奖励机制简化强化学习中的自我验证过程，仅需一个额外token推理即可获得奖励分数，提升模型推理性能和自奖励能力。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法需要模型使用两个独立提示模板顺序生成解决方案和自我验证，效率低下。

Method: 理论推导发现自我验证的RL目标闭式解可简化为最后一词自奖励分数，提出LaSeR算法，在原RLVR损失基础上增加MSE损失对齐最后一词自奖励分数与验证器推理奖励。

Result: 实验表明该方法不仅提升模型推理性能，还赋予其显著的自奖励能力，增强推理时扩展性能。

Conclusion: LaSeR通过最后一词自奖励机制高效统一了推理和验证能力，仅需最小额外计算成本即可实现性能提升。
...
TL;DR: MetaBench是首个代谢组学评估基准，用于系统评估大语言模型在代谢组学领域的五种关键能力：知识、理解、基础、推理和研究。评估25个开源和闭源模型发现，模型在文本生成任务上表现良好，但在跨数据库标识符基础和长尾代谢物方面仍有挑战。
...
Method: 从权威公共资源中策划MetaBench基准，评估代谢组学研究的五种关键能力：知识、理解、基础、推理和研究。对25个开源和闭源大语言模型进行系统评估。

Result: 模型在代谢组学任务中表现出不同的性能模式：文本生成任务表现良好，但跨数据库标识符基础即使使用检索增强仍然具有挑战性。对于注释稀疏的长尾代谢物，模型性能下降。
...
TL;DR: 提出IGPO框架，通过信息增益为多轮交互的LLM智能体提供密集的内在奖励，解决传统结果奖励稀疏导致的优势崩溃和信用分配问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的LLM智能体训练依赖最终结果奖励，在多轮交互中面临奖励稀疏问题，导致优势崩溃和细粒度信用分配困难。
...
Conclusion: IGPO通过内在奖励机制有效解决了多轮LLM智能体训练中的奖励稀疏问题，为复杂交互任务提供了更高效的训练框架。
...
<details>
  <summary>Details</summary>
Motivation: 数字代理需要多样化的大规模UI轨迹来泛化真实世界任务，但通过人工标注、基础设施和工程角度收集此类数据成本过高。

Method: 整合数字世界模拟器生成多样化UI状态，引导展开过程实现连贯探索，以及轨迹包装器生成高质量多样化训练轨迹。还提出了UI-Simulator-Grow目标缩放策略，通过优先处理高影响任务和合成信息丰富的轨迹变体来实现更快速和数据高效的缩放。
...
TL;DR: 论文提出了TokDrift框架，发现代码LLMs中的子词分词器由于统计驱动而非语法驱动，导致语义相同的代码片段因格式差异而产生不同的分词结果，严重影响模型行为的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有代码LLMs使用的子词分词器（如BPE）基于统计而非语法，导致语义相同的代码可能因空格、标识符命名等表面因素被不同分词，影响模型可靠性。

Method: 引入TokDrift框架，应用语义保持的重写规则创建仅分词不同的代码变体，分析九个代码LLMs（包括超过300亿参数的大模型）的行为变化。

Result: 即使微小的格式变化也会导致模型行为显著偏移，分层分析显示问题源于早期嵌入层，子词分词未能捕捉语法标记边界。

Conclusion: 错误对齐的分词是代码理解和生成可靠性的隐藏障碍，未来代码LLMs需要语法感知的分词方法。
...
TL;DR: Elastic-Cache是一种自适应KV缓存重计算策略，通过注意力感知漂移测试和深度感知调度，在保持生成质量的同时显著加速扩散大语言模型的解码过程。
...
Method: 提出Elastic-Cache策略：1) 基于最关注token的注意力感知漂移测试决定何时刷新；2) 深度感知调度从选定层开始重新计算，同时重用浅层缓存和窗口外MASK缓存；3) 缓存块状MASK token作为长度偏置。

Result: 在数学推理和代码生成任务上实现显著加速：GSM8K上8.7倍，长序列上45.1倍，HumanEval上4.8倍，同时保持比基线更高的准确率，吞吐量比现有方法高6.8倍。

Conclusion: Elastic-Cache通过自适应、层感知的缓存更新，有效减少冗余计算，在保持生成质量的同时显著加速扩散LLM的解码，实现实际部署。
...
TL;DR: 提出了一种基于语义推理的物联网设备识别方法，将设备识别重新定义为网络元数据的语言建模任务，使用指令调优的LLaMA3.18B模型在真实世界IoT流量数据集上实现了高精度识别。


<details>
  <summary>Details</summary>
Motivation: 物联网设备的快速扩张超过了现有识别方法的能力，在开放世界环境中，流量元数据通常不完整、有噪声或被故意混淆，导致安全、隐私和网络问责方面的重大风险。

Method: 构建语义推理管道，将设备识别重新定义为异构网络元数据的语言建模任务；使用基于互信息和熵稳定性得分的LLM集成方法为IoT Inspector数据集生成高保真厂商标签；采用课程学习对量化LLaMA3.18B模型进行指令调优，支持稀疏性和长尾厂商分布下的泛化。

Result: 模型在2,015个厂商上实现了98.25%的top-1准确率和90.73%的宏观准确率，同时对缺失字段、协议漂移和对抗性操作保持鲁棒性。在独立IoT测试平台上的评估显示指令调优的LLM为大规模真实世界设备识别提供了可扩展且可解释的基础。

Conclusion: 指令调优的大型语言模型为大规模真实世界设备识别提供了可扩展且可解释的基础，能够有效应对开放世界环境中的设备识别挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统渐进域适应方法存在知识迁移效率低或中间数据不完整的问题，需要更平滑的知识迁移机制来应对域偏移挑战。

Method: 引入动态加权机制，通过时变超参数ϱ控制域特定学习强度，结合自训练生成伪标签，优化加权目标函数进行迭代模型更新。
...
<details>
  <summary>Details</summary>
Motivation: 现有最先进的持续学习方法如LFS-GAN在每次训练迭代中引入大量新权重，长期来看参数负担显著。需要一种既能处理少样本学习又能进行持续学习的高效方法。

Method: 提出CoLoR-GAN框架，利用低秩张量高效适配模型到目标任务。引入LLoRA（LoRA中的LoRA）技术进一步优化卷积层适配器大小，并提供超参数选择的经验研究。

Result: 在多个基准持续学习和少样本任务上的实验表明，CoLoR-GAN高效且达到SOTA性能，同时显著减少所需资源量。

Conclusion: CoLoR-GAN通过低秩适配技术成功解决了GAN中持续少样本学习的挑战，在保持高性能的同时大幅降低了参数需求。
...
<details>
  <summary>Details</summary>
Motivation: 传统联合能量模型在同时实现鲁棒分类和高质量生成方面存在局限性，特别是基于SGLD的训练方法存在不稳定性和样本质量差的问题。

Method: 采用对抗训练原则，用稳定的AT方法替代SGLD，通过区分真实数据和PGD生成的对比样本来优化能量函数；结合对抗训练增强分类鲁棒性；使用两阶段训练解决批归一化与EBM训练的不兼容性。

Result: 在CIFAR-10、CIFAR-100和ImageNet上的实验表明，该方法显著提高了对抗鲁棒性，同时保持竞争力的生成性能。在ImageNet上，生成质量超过BigGAN并接近扩散模型。
...
TL;DR: K-frames是一种基于场景驱动的关键帧选择新范式，通过预测语义连贯的查询相关片段来保持时间连续性，支持任意数量关键帧选择以适应不同用户预算。
...
Method: 首先构建PeakClips数据集（20万个基于查询的视频高光片段），然后采用三阶段渐进式课程学习：两个监督微调阶段用于时间定位和关键片段感知，以及一个强化学习阶段直接优化场景驱动预测策略。

Result: 在主要长视频理解基准测试上的广泛实验表明，K-frames在不同尺度下提供了有效、可解释且即插即用的关键帧选择解决方案。

Conclusion: K-frames通过场景驱动的关键片段选择方法，解决了长视频理解中的关键帧选择问题，在保持时间连续性的同时支持多尺度选择，为下游任务提供了无需额外标注的有效解决方案。
...
TL;DR: 提出了多视图半监督标签分布学习方法MVSS-LDL，通过利用各视图的局部最近邻结构并强调多视图间的互补性，解决了多视图标签分布学习问题。
...
Method: 首先计算每个视图的k最近邻结构，然后通过整合其他视图的最近邻信息来补充当前视图的最近邻集，最后基于补充后的最近邻集构建基于图学习的多视图半监督标签分布学习模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法严重依赖缩放超参数λ，但缺乏无需数据的原则性设置方法，导致研究者不得不用评估数据调优λ，这在实际中不可行

Method: Weight Weaving技术，使用平均、随机选择或现有模型合并方法等池化函数在λ搜索空间中整合模型权重，具有高模块化且与现有方法正交

Result: 在三个ViT变体和三个实验设置中验证，持续改进多个模型合并方法性能，在无数据设置下平均准确率提升高达15.9个百分点

Conclusion: Weight Weaving是一种即插即用的数据无关模型合并技术，能显著提升现有方法性能，具有实用性和普适性
...
TL;DR: 本文提出了一种新的ICD编码自动分配方法，将问题视为分类和排序任务而非单纯分类，以考虑ICD代码的顺序重要性。
...
Method: 从检索系统角度处理该任务，将其制定为分类和排序任务，同时考虑代码分配和优先级排序。

Result: 在主要诊断代码排序准确率达到47%（相比现有最佳方法的20%），在分类指标上微平均F1为0.6065，宏平均F1为0.2904，均优于之前最佳模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统的数据保真度损失函数如均方误差或负对数似然寻求与噪声测量的逐点一致性，往往导致对噪声的过拟合。需要一种能够从统计角度评估数据保真度的方法。

Method: 引入分布一致性损失，使用基于模型的概率分数对每个测量值进行分布级校准，作为标准数据一致性项的直接替代品，与现代正则化器兼容且优化方式相同。

Result: 在图像去噪中，使用DC损失替代MSE损失无需提前停止训练即可获得更高PSNR；在医学图像重建中，DC损失减少了高度迭代重建中的伪影并增强了手工正则化的效果。

Conclusion: DC损失为逆问题提供了一种统计基础扎实、性能提升的替代传统保真度损失的方法，特别适用于测量噪声分布已知且数据集包含大量独立噪声值的实际逆问题。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大语言模型在部署时的计算和内存开销问题，同时保持特定任务上的高性能，需要开发能够将全精度模型高效压缩为极低精度模型的方法。

Method: BitDistill结合了三个关键技术：SubLN模块（来自BitNet）、基于MiniLM的多头注意力蒸馏，以及作为关键预热步骤的持续预训练，以缓解全精度和1.58位模型在特定任务上的性能差距。

Result: 实验结果显示，BitDistill在不同模型规模下都能达到与全精度对应模型相当的性能，同时实现高达10倍的内存节省和2.65倍的CPU推理加速。

Conclusion: BitDistill提供了一种有效的模型压缩方法，能够在保持任务特定性能的同时，显著降低大语言模型的部署成本，为边缘设备上的高效推理提供了可行方案。
...
TL;DR: 提出REAP方法，通过考虑路由器门值和专家激活范数的剪枝策略，在生成任务中优于专家合并方法，能在50%压缩率下实现接近无损的压缩效果。


<details>
  <summary>Details</summary>
Motivation: 稀疏激活的专家混合模型参数数量庞大导致内存开销显著，需要研究专家压缩方法。现有研究偏向专家合并，但本文证明对于生成任务，专家剪枝是更优策略。

Method: 提出Router-weighted Expert Activation Pruning (REAP)方法，结合路由器门值和专家激活范数作为剪枝准则，避免专家合并导致的功能子空间崩溃问题。

Result: 在20B到1T参数的SMoE模型上，REAP在生成基准测试中始终优于合并和其他剪枝方法，特别是在50%压缩率下。在代码生成和工具调用任务中，即使剪枝50%专家也能实现接近无损的压缩。

Conclusion: 专家剪枝比专家合并更适合生成任务，REAP方法通过考虑路由器控制机制有效避免了功能子空间崩溃，实现了高效的专家压缩。
...
TL;DR: 提出了条件Clifford-可导向核来增强CSCNNs的表达能力，通过输入特征场计算等变表示来扩充核函数，在多个PDE预测任务中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的Clifford-可导向CNNs的核基不完整，限制了模型的表达能力，需要解决这个问题以提升在物理系统建模中的性能。

Method: 提出条件Clifford-可导向核，通过输入特征场计算等变表示来扩充核函数，推导了这些输入依赖核的等变约束，并通过隐式参数化高效求解。

Result: 在流体动力学和相对论电动力学等多个PDE预测任务中，该方法始终优于基线方法，证明了改进的表达能力。

Conclusion: 条件Clifford-可导向核有效解决了CSCNNs核基不完整的问题，显著提升了模型在物理系统建模中的表达能力和性能。
...
TL;DR: 提出了基于几何感知优化算法的噪声自适应分层学习率方案，通过估计梯度方差动态调整组内各层学习率，显著加速深度神经网络训练


<details>
  <summary>Details</summary>
Motivation: 现有几何感知优化器在相同范数组内对层施加固定学习率，但实际训练中不同层的局部曲率存在异质性且动态变化，这种固定学习率策略可能效率低下

Method: 在几何感知优化算法基础上引入噪声自适应分层学习率方案，实时估计由所选LMO诱导的对偶范数中的梯度方差，并据此为组内各层分配时变的噪声自适应学习率

Result: 理论分析表明算法达到锐利收敛率，在LLaMA和GPT等transformer架构上的实证结果显示比最先进优化器收敛更快

Conclusion: 所提出的噪声自适应分层学习率方案能有效加速深度神经网络训练，特别是在transformer架构上表现出优越性能
...
TL;DR: COFFEE模型是一种新颖的时变状态空间模型，通过引入状态反馈机制实现上下文依赖的选择性，在保持并行实现能力的同时，显著提升了长序列依赖建模能力，并在多个基准测试中超越了S6模块。


<details>
  <summary>Details</summary>
Motivation: Transformer模型存在二次复杂度问题和长序列依赖建模困难，而现有的状态空间模型（如S6）的选择性机制仅依赖当前输入，缺乏对序列历史上下文的利用。

Method: 提出COFFEE模型，通过状态反馈机制从内部状态计算选择性，内部状态作为序列历史的紧凑表示；采用高效模型参数化方法消除S6中的冗余。

Result: 在induction head任务中，COFFEE以两个数量级更少的参数和训练序列达到接近完美的准确率；在MNIST上仅用3585个参数就达到97%准确率，大幅超越S6。
...
### [240] [CausalVerse: Benchmarking Causal Representation Learning with Configurable High-Fidelity Simulations](https://arxiv.org/abs/2510.14049)
...
<details>
  <summary>Details</summary>
Motivation: 现有因果表示学习评估面临困境：要么使用过于简化的合成数据集，要么依赖现实任务的下游性能，难以在真实性和评估精度之间取得平衡。

Method: 创建基于高保真模拟视觉数据的基准数据集，包含静态图像生成、动态物理模拟、机器人操作和交通情境分析四个领域，提供对底层因果结构的灵活访问。

Result: 构建了包含丰富场景的综合性测试平台，允许用户修改因果结构以符合CRL假设，并评估了代表性CRL方法。

Conclusion: 该基准为实践者和新研究者提供了选择或扩展适当CRL框架的实证见解，以解决能从CRL视角受益的特定现实问题。
...
### [241] [FedHFT: Efficient Federated Finetuning with Heterogeneous Edge Clients](https://arxiv.org/abs/2510.14054)
...
<details>
  <summary>Details</summary>
Motivation: 解决个性化自然语言理解应用中的两个主要挑战：由于数据保密或隐私要求导致的有限和/或异构数据，以及边缘设备等参与客户端之间不同的计算资源。
...
### [242] [On the expressivity of sparse maxout networks](https://arxiv.org/abs/2510.14068)
*Moritz Grillo,Tobias Hofmann*

Main category: cs.LG

TL;DR: 该论文研究了稀疏maxout网络的表达能力，建立了网络计算函数与虚拟多面体之间的对偶关系，并证明了在固定入度约束下，深度不足时仅靠宽度无法弥补稀疏性。


<details>
  <summary>Details</summary>
Motivation: 研究稀疏maxout网络的表达能力，这类网络捕捉了卷积神经网络或图神经网络的关键特征，旨在理解其深度和宽度在表达复杂函数时的作用。

Method: 建立稀疏maxout网络计算函数与一类虚拟多面体之间的对偶关系，利用多面体几何分析网络表达能力，推导关联多面体维度的紧致界。

Result: 证明了足够深的稀疏maxout网络具有通用性，但在固定入度约束下，如果深度不足，仅增加宽度无法弥补稀疏性带来的限制。

Conclusion: 稀疏maxout网络的表达能力受深度和稀疏性共同影响，深度不足时宽度无法完全补偿固定入度约束带来的限制。
...
### [243] [Exploratory Causal Inference in SAEnce](https://arxiv.org/abs/2510.14073)
*Tommaso Mencattini,Riccardo Cadei,Francesco Locatello*

Main category: cs.LG

TL;DR: 提出Neural Effect Search方法，直接从数据中发现未知的因果效应，无需预设假设，在真实科学试验中成功识别因果效应
...
Method: 使用预训练基础模型将非结构化数据转换为有意义的表示，通过稀疏自编码器解释，采用递归分层方法解决多重检验和效应纠缠问题

Result: 在半合成实验中验证了算法的鲁棒性，在实验生态学背景下首次在真实世界科学试验中成功进行无监督因果效应识别

Conclusion: 该方法能够直接从数据中发现未知因果效应，为大规模因果发现提供了新途径
...
### [244] [Neural Network approximation power on homogeneous and heterogeneous reaction-diffusion equations](https://arxiv.org/abs/2510.14094)
*Haotian Feng*
...
<details>
  <summary>Details</summary>
Motivation: 尽管神经网络被广泛用于求解微分方程，但其能够有效逼近这类解的理论基础尚未得到充分探索。

Method: 基于通用逼近定理，对一维和二维反应-扩散方程在均匀和非均匀介质中的神经网络逼近能力进行理论分析。

Result: 证明了两层神经网络足以逼近一维反应-扩散方程，三层神经网络足以逼近二维反应-扩散方程。

Conclusion: 这项工作强调了神经网络在逼近反应-扩散方程及相关偏微分方程解方面的表达能力，为基于神经网络的微分方程求解器提供了理论基础，该框架可进一步扩展到椭圆和抛物型方程。
...
### [245] [Unlocking Out-of-Distribution Generalization in Transformers via Recursive Latent Space Reasoning](https://arxiv.org/abs/2510.14095)
*Awni Altabaa,Siyu Chen,John Lafferty,Zhuoran Yang*

Main category: cs.LG

TL;DR: 该论文研究了Transformer网络在分布外泛化问题，提出了四种架构机制来增强算法推理能力，并在模块化算术任务上验证了这些机制的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决机器学习中系统性、组合性泛化超越训练分布的核心挑战，特别是现代语言模型在推理能力方面的关键瓶颈。

Method: 使用GSM8K风格的模块化算术计算图任务作为测试平台，引入了四种架构机制：输入自适应递归、算法监督、通过离散瓶颈的锚定潜在表示、显式纠错机制。

Result: 这些机制共同形成了一种在Transformer网络中实现原生且可扩展的潜在空间推理的架构方法，具有强大的算法泛化能力。

Conclusion: 通过机制可解释性分析揭示了这些机制如何产生稳健的分布外泛化能力，为Transformer网络中的算法推理提供了新的架构解决方案。
...
### [246] [TENDE: Transfer Entropy Neural Diffusion Estimation](https://arxiv.org/abs/2510.14096)
...
<details>
  <summary>Details</summary>
Motivation: 现有传递熵估计方法存在维度灾难、需要严格分布假设或需要指数级大数据集才能可靠收敛的问题。

Method: TENDE方法利用基于分数的扩散模型学习相关条件分布的函数，通过条件互信息估计传递熵，对底层数据生成过程假设最小。

Result: 在合成基准测试和真实数据上，TENDE相比现有神经估计器和其他最先进方法展现出更优的准确性和鲁棒性。

Conclusion: TENDE提供了一种灵活、可扩展的传递熵估计方法，克服了现有方法的局限性，在多个应用领域具有重要价值。
...
### [247] [Near-Optimal Regret-Queue Length Tradeoff in Online Learning for Two-Sided Markets](https://arxiv.org/abs/2510.14097)
*Zixian Yang,Sushil Mahavir Varma,Lei Ying*

Main category: cs.LG

TL;DR: 该论文研究双边市场中的定价和匹配算法，设计了一种基于在线学习的定价策略，在需求供给曲线未知的情况下实现近最优利润，同时控制队列长度。


<details>
  <summary>Details</summary>
Motivation: 在双边市场中，平台需要同时考虑利润最大化和队列长度控制。由于实际中需求供给曲线可能未知，需要设计能够在未知环境下有效运作的在线学习算法。

Method: 提出了一种新颖的在线学习定价策略，包含动态组件优化遗憾与队列长度的权衡，以及概率组件解决快速学习样本获取与队列长度控制之间的张力。

Result: 证明了性能指标之间的权衡关系：对于γ∈(0,1/6]，实现了Õ(T^(1-γ))遗憾、Õ(T^(γ/2))平均队列长度和Õ(T^γ)最大队列长度，显著优于现有结果。

Conclusion: 所提出的定价策略在需求供给曲线未知的情况下实现了近最优性能，证明了遗憾与平均队列长度之间的权衡在允许的γ范围内是最优的。
...
### [248] [Briding Diffusion Posterior Sampling and Monte Carlo methods: a survey](https://arxiv.org/abs/2510.14114)
*Yazid Janati,Alain Durmus,Jimmy Olsson,Eric Moulines*

Main category: cs.LG

TL;DR: 这篇综述论文探讨了如何利用预训练扩散模型和蒙特卡洛方法解决贝叶斯逆问题，无需额外训练。


<details>
  <summary>Details</summary>
Motivation: 扩散模型能够从复杂分布中合成高精度样本，在生成建模中已成为基础。最近研究表明它们有潜力作为先验来解决贝叶斯逆问题。

Method: 主要采用扩散过程中间分布的扭曲机制，引导模拟朝向后验分布，然后使用各种蒙特卡洛方法从这些扭曲分布中采样。

Result: 展示了预训练扩散模型与蒙特卡洛方法结合的有效性，能够解决贝叶斯逆问题而无需额外训练。

Conclusion: 扩散模型作为先验与蒙特卡洛方法结合，为贝叶斯逆问题提供了一种有效的解决方案框架。
...
### [249] [Neural Network-enabled Domain-consistent Robust Optimisation for Global CO$_2$ Reduction Potential of Gas Power Plants](https://arxiv.org/abs/2510.14125)
*Waqar Muhammad Ashraf,Talha Ansar,Abdulelah S. Alshehri,Peipei Chen,Ramit Debnath,Vivek Dua*

Main category: cs.LG

TL;DR: 提出了一种神经网络驱动的鲁棒优化框架，将数据驱动域作为约束整合到非线性规划中，解决了参数化神经网络模型与优化求解器交互产生的域不一致解问题。
...
Method: 开发了神经网络驱动的鲁棒优化框架，将数据驱动域作为约束整合到非线性规划技术中。

Result: 应用于1180 MW联合循环燃气电厂，实现了域一致的鲁棒最优解，验证了能源效率平均提升0.76个百分点。全球范围内估计每年可减少2600万吨CO₂排放。

Conclusion: 该研究强调了机器学习在提供近期的、可扩展的全球气候行动脱碳路径中的协同作用。
...
### [250] [Demystifying the Mechanisms Behind Emergent Exploration in Goal-conditioned RL](https://arxiv.org/abs/2510.14129)
*Mahsa Bastankhah,Grace Liu,Dilip Arumugam,Thomas L. Griffiths,Benjamin Eysenbach*

Main category: cs.LG

TL;DR: SGCRL是一种无监督强化学习算法，通过隐式奖励和表示学习实现探索，无需外部奖励或课程安排即可解决长时程目标达成任务。
...
Method: 结合理论分析和受控实验，研究SGCRL的目标函数，分析其如何通过学习的表示来塑造隐式奖励。

Result: SGCRL通过学习的表示自动修改奖励环境，在达成目标前促进探索，之后促进利用；探索动态源于状态空间的低秩表示学习而非神经网络函数逼近。

Conclusion: 对SGCRL探索机制的理解使其能够适配为执行安全感知的探索。
...
### [251] [Learning Wireless Interference Patterns: Decoupled GNN for Throughput Prediction in Heterogeneous Multi-Hop p-CSMA Networks](https://arxiv.org/abs/2510.14137)
*Faezeh Dehghan Tarzjani,Bhaskar Krishnamachari*

Main category: cs.LG

TL;DR: 提出解耦图卷积网络(D-GCN)用于异构多跳无线网络中p-persistent CSMA协议的吞吐量预测，通过分离节点自身传输概率和邻居干扰效应，显著提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统简化模型在稀疏拓扑中低估吞吐量48-62%，精确马尔可夫链分析计算复杂度指数级增长，标准GNN因对称归一化混淆直接干扰和级联效应导致63.94%误差。

Method: D-GCN架构显式分离节点自身传输概率和邻居干扰效应，用可学习注意力替代均值聚合，捕获复杂多跳干扰模式。

Result: D-GCN达到3.3%归一化平均绝对误差，优于强基线方法，在精确分析方法不可行时仍可处理，基于梯度的网络优化达到理论最优值的1%以内。

Conclusion: D-GCN通过解耦干扰效应实现了准确且可扩展的吞吐量预测，为大规模无线网络性能分析提供了实用解决方案。
...
### [252] [Inferred global dense residue transition graphs from primary structure sequences enable protein interaction prediction via directed graph convolutional neural networks](https://arxiv.org/abs/2510.14139)
...
<details>
  <summary>Details</summary>
Motivation: 现有蛋白质相互作用预测方法计算密集，本研究探索计算强度较低的替代方案，通过链接预测进行下游PPI预测。

Method: 两阶段图表示学习框架：1) ProtGram将蛋白质一级结构建模为全局推断的n-gram层次图，边权重由残基转移概率定义；2) DirectGCN定向图卷积网络，通过路径特定变换和可学习门控机制处理信息。

Result: DirectGCN在标准节点分类基准上表现与现有方法相当，在复杂定向图和异质结构上表现优异；完整框架在PPI预测中展现出强大预测能力，即使训练数据有限。

Conclusion: ProtGram-DirectGCN框架为蛋白质相互作用预测提供了计算效率高的有效解决方案，在数据稀缺情况下仍能保持强劲性能。
...
### [253] [On Evaluating Loss Functions for Stock Ranking: An Empirical Analysis With Transformer Model](https://arxiv.org/abs/2510.14156)
*Jan Kwiatkowski,Jarosław A. Chudziak*

Main category: cs.LG

TL;DR: 本文系统评估了多种先进的损失函数（点式、配对式、列表式）在Transformer模型中对股票收益率排序的影响，为基于排序的投资组合选择提供实用指导。


<details>
  <summary>Details</summary>
Motivation: 量化交易策略依赖准确的股票排序来识别盈利投资。传统损失函数注重预测准确性，但未能直接教导模型学习股票收益率的正确排序顺序，且缺乏对现代Transformer模型在金融排序任务中不同损失函数效果的全面比较。

Method: 在S&P 500数据上，系统评估了包括点式、配对式、列表式在内的多种先进损失函数，用于每日股票收益率预测，重点关注每种损失函数如何影响模型识别资产间盈利相对排序的能力。

Result: 研究提供了一个全面的基准，揭示了不同损失函数如何影响模型学习对投资组合选择至关重要的横截面和时间模式的能力。

Conclusion: 为优化基于排序的交易策略提供了实用指导，填补了现代Transformer模型在金融收益率排序任务中损失函数效果比较的研究空白。
...
### [254] [Data Understanding Survey: Pursuing Improved Dataset Characterization Via Tensor-based Methods](https://arxiv.org/abs/2510.14161)
*Matthew D. Merris,Tim Andersen*

Main category: cs.LG

TL;DR: 该论文综述了传统数据集表征方法的局限性，并探讨了基于张量的方法作为更强大的替代方案，能够揭示数据的细微特征并提供更好的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习数据集表征方法（统计、结构、模型分析）无法提供深度理解和解释性，限制了创新和洞察力。

Method: 调查当前最先进的数据分析技术，分析其局限性，并讨论各种基于张量的方法如何提供更强大的数据集表征替代方案。

Result: 通过实例说明张量方法能够揭示数据的细微特征，提供增强的可解释性和可操作的情报。
...
### [255] [Towards Reversible Model Merging For Low-rank Weights](https://arxiv.org/abs/2510.14163)
*Mohammadsajad Alipour,Mohammad Mohammadi Amiri*

Main category: cs.LG

TL;DR: 提出可逆模型合并(RMM)方法，通过构建紧凑基空间来恢复原始任务特定模型，而非传统合并为单一权重，解决了低秩压缩模型合并时的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 传统模型合并方法在应用于低秩权重（如LoRA或SVD压缩模型）时会导致严重的性能下降，需要新的合并策略。

Method: 构建紧凑基空间，通过线性组合恢复原始任务特定模型，提供闭式解选择最优基权重和任务特定系数。

Result: 在多样化数据集和模型规模上的实验表明，RMM显著优于现有合并方法，大幅保留了低秩压缩模型的性能。

Conclusion: 将合并重新定义为生成可重构模型空间而非单一合并模型，允许在需要时恢复到各个特定任务模型，认识到没有合并模型能始终优于专门化模型。
...
### [256] [Optimal Control Theoretic Neural Optimizer: From Backpropagation to Dynamic Programming](https://arxiv.org/abs/2510.14168)
*Guan-Horng Liu,Tianrong Chen,Evangelos A. Theodorou*

Main category: cs.LG

TL;DR: 将深度神经网络训练重新解释为最优控制问题，提出基于动态规划Bellman方程高阶展开的OCNOpt优化器，在保持计算复杂度的同时提升鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 观察到反向传播算法与动态系统最优性条件（动态规划）的算法相似性，希望通过最优控制理论为DNN优化提供新的理论框架和算法设计。

Method: 将DNN视为动力系统，通过求解Bellman方程的一阶展开近似动态规划，开发OCNOpt优化器，支持层间反馈策略、博弈论应用和连续时间模型的高阶训练。

Result: 大量实验表明OCNOpt在鲁棒性和效率方面优于现有方法，同时保持可管理的计算复杂度。

Conclusion: OCNOpt为基于动力系统和最优控制理论的原理性算法设计开辟了新途径，证明了最优控制视角在DNN优化中的价值。
...
TL;DR: MAFA是一个生产级部署的多智能体标注框架，通过可配置的多智能体协作解决金融服务中的大规模标注积压问题，在JP Morgan Chase成功消除了100万条话语积压，平均与人工标注者达成86%一致，每年节省5000多小时人工标注工作。
...
Result: 在JP Morgan Chase部署中消除了100万条话语积压，平均与人工标注者达成86%一致，标注置信度分类为85%高、10%中、5%低，在内部意图分类数据集上比传统方法提升13.8% Top-1准确率、15.1% Top-5准确率和16.9% F1分数。
...
### [258] [Contrastive Diffusion Alignment: Learning Structured Latents for Controllable Generation](https://arxiv.org/abs/2510.14190)
...
TL;DR: ConDA框架通过对比学习在扩散嵌入中对齐潜在几何与系统动态，实现可解释的控制生成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型擅长生成，但其潜在空间未明确组织以实现可解释控制。

Method: 在扩散嵌入中应用对比学习，使潜在几何与系统动态对齐，支持非线性轨迹遍历。

Result: 在流体动力学、神经钙成像、治疗性神经刺激和面部表情等基准测试中，ConDA相比线性遍历和基于条件的方法产生更可解释的潜在表示和更好的可控性。
...
### [259] [Incentive-Based Federated Learning](https://arxiv.org/abs/2510.14208)
*Chanuka A. S. Hewa Kaluannakkage,Rajkumar Buyya*

Main category: cs.LG

TL;DR: 本文探讨了联邦学习中的激励机制设计挑战，分析了经济学和博弈论概念在联邦学习中的应用，以及区块链和深度强化学习等技术解决方案，提出了覆盖集中式和去中心化架构的综合分类法。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然能保护数据隐私，但面临参与困境问题——参与实体不愿贡献资源或可能搭便车，因此需要设计有效的激励机制来确保系统的实际可行性。

Method: 应用经济学和博弈论的基础概念，结合区块链和深度强化学习等技术驱动解决方案，构建了基于理论概念的综合分类法，涵盖集中式和去中心化架构。

Result: 提出了一个全面的激励机制分类法，展示了从应用角度描述的概念，覆盖医疗保健、智能基础设施、车联网和基于区块链的去中心化系统等新兴工业应用。

Conclusion: 精心设计的激励机制不是可选功能，而是联邦学习实际成功的关键组成部分，分析揭示了有前景的解决方案和构建可持续、公平、鲁棒联邦学习生态系统的重大挑战。
...
### [260] [Spectral Analysis of Molecular Kernels: When Richer Features Do Not Guarantee Better Generalization](https://arxiv.org/abs/2510.14217)
*Asma Jamali,Tin Sum Cheng,Rodrigo A. Vargas-Hernández*

Main category: cs.LG

TL;DR: 对分子核方法进行首次全面谱分析，发现更丰富的谱特征并不总是提高预测精度，甚至在某些情况下与性能呈负相关。


<details>
  <summary>Details</summary>
Motivation: 尽管深度模型在分子性质预测中达到最先进精度，但核方法在低数据场景下因其鲁棒性和理论透明性仍被广泛使用。目前缺乏对分子核的系统谱分析。

Method: 在QM9数据集上对分子指纹、预训练transformer、全局和局部3D表示等七种分子性质进行核岭回归的谱分析，使用四种谱度量指标，并通过截断核来探究谱与预测性能的关系。

Result: 更丰富的谱特征并不一致提高精度；对于transformer和局部3D表示，谱丰富度与性能呈负相关；许多核中仅保留前2%特征值即可恢复几乎所有性能。

Conclusion: 研究结果挑战了"更丰富的谱带来更好泛化"的常见启发式，揭示了表示、核特征与预测性能之间的微妙关系，对数据有限的科学和实际任务中的核与自监督学习方法评估具有指导意义。
...
### [261] [When Flatness Does (Not) Guarantee Adversarial Robustness](https://arxiv.org/abs/2510.14231)
*Nils Philipp Walter,Linara Adilova,Jilles Vreeken,Michael Kamp*

Main category: cs.LG

TL;DR: 论文通过严格形式化平坦性与对抗鲁棒性的关系，发现平坦最小值只能提供局部而非全局的对抗鲁棒性，并揭示了对抗样本通常位于模型自信但错误的平坦区域。


<details>
  <summary>Details</summary>
Motivation: 尽管神经网络经验上成功，但仍易受微小对抗扰动影响。长期假设认为平坦最小值（损失景观中低曲率区域）能提供更强的鲁棒性，但这种联系缺乏严格形式化。

Method: 首先推导了倒数第二层相对平坦度的闭式表达式，然后利用该表达式约束输入空间中损失的变化，从而形式化分析整个网络的对抗鲁棒性。

Result: 平坦性仅能保证局部对抗鲁棒性，要实现全局鲁棒性，损失需要在数据流形外急剧弯曲。实验验证了理论预测，发现对抗样本常位于模型自信但错误的平坦区域。
...
### [262] [Scaling Test-Time Compute to Achieve IOI Gold Medal with Open-Weight Models](https://arxiv.org/abs/2510.14232)
...
TL;DR: GenCluster是一个可扩展的测试时计算框架，使用开源模型实现了IOI金牌水平的性能，通过大规模生成、行为聚类、排名和轮询提交策略来高效探索解决方案空间。


<details>
  <summary>Details</summary>
Motivation: 竞争性编程已成为评估大型语言模型推理和问题解决能力的重要基准，但开源模型在IOI等顶级竞赛中达到金牌水平仍面临挑战。

Method: 结合大规模生成、行为聚类、排名和轮询提交策略，在有限验证预算下高效探索多样化的解决方案空间。

Result: GenCluster能够首次使用开源模型gpt-oss-120b在IOI 2025中获得金牌，性能随可用计算资源一致扩展，缩小了开源与闭源系统之间的差距。

Conclusion: 该框架为LLM推理能力的透明和可复现评估设立了新基准，证明了开源模型在竞争性编程中达到顶尖水平的可行性。
...
### [263] [Policy Regularized Distributionally Robust Markov Decision Processes with Linear Function Approximation](https://arxiv.org/abs/2510.14246)
*Jingwen Gu,Yiting He,Zhishuai Liu,Pan Xu*

Main category: cs.LG

TL;DR: 提出DR-RPO算法，一种模型无关的在线策略优化方法，用于在分布偏移下的强化学习，通过参考策略正则化和线性函数逼近实现高效学习和鲁棒策略。
...
Method: DR-RPO算法结合参考策略正则化，将鲁棒MDP转化为在转移和策略上双重约束的变体，采用d-矩形线性MDP公式和带置信上界的线性函数逼近进行乐观探索。

Result: 理论保证显示策略优化在鲁棒RL中可实现多项式次优界和样本效率，与基于价值的方法性能相当，实证结果验证了DR-RPO的鲁棒性。

Conclusion: DR-RPO成功弥合了策略优化在鲁棒RL中的理论空白，证明了其在线设置下的有效性和可扩展性。
...
### [264] [A Physics Prior-Guided Dual-Stream Attention Network for Motion Prediction of Elastic Bragg Breakwaters](https://arxiv.org/abs/2510.14250)
*Lianzi Jiang,Jianxin Zhang,Xinyu Han,Huanhe Dong,Xiangrong Wang*

Main category: cs.LG

TL;DR: 提出PhysAttnNet模型，通过物理先验引导的双流注意力网络改进弹性布拉格防波堤运动响应预测，解决传统深度学习模型在未知海况下泛化能力不足的问题。
...
Method: 提出PhysAttnNet模型，包含衰减双向自注意力模块（模拟自然衰减）、相位差引导双向交叉注意力模块（捕捉波-结构相互作用），以及全局上下文融合模块，并采用混合时频损失函数进行训练。

Result: 在波浪水槽数据集上的实验表明，PhysAttnNet显著优于主流模型，跨场景泛化测试验证了模型在未见环境中的鲁棒性和适应性。

Conclusion: PhysAttnNet作为海洋工程中复杂系统预测模型的开发框架具有巨大潜力，能够有效提升弹性布拉格防波堤运动响应的预测精度和泛化能力。
...
### [265] [Generalist vs Specialist Time Series Foundation Models: Investigating Potential Emergent Behaviors in Assessing Human Health Using PPG Signals](https://arxiv.org/abs/2510.14254)
...
TL;DR: 该论文对通用型和专用型时间序列基础模型在PPG信号处理上的性能进行了全面基准测试，发现专用模型在完全微调场景下胜率得分高出27%。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型在时间序列分析中日益受到关注，特别是生理信号处理领域，需要比较通用型模型（如MOMENT）和专用型模型在PPG信号处理上的性能差异。

Method: 通过包含51个任务的综合测试套件，评估两种模型在7个维度上的表现：胜率得分、平均性能、特征质量、调优增益、性能方差、可迁移性和可扩展性。

Result: 在完全微调场景下，专用型模型比通用型模型获得27%更高的胜率得分。

Conclusion: 专用型时间序列基础模型在PPG信号处理任务中表现更优，研究还提供了关于泛化性、公平性、注意力可视化和训练数据选择重要性的深入分析。
...
### [266] [CAST: Compositional Analysis via Spectral Tracking for Understanding Transformer Layer Functions](https://arxiv.org/abs/2510.14262)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然取得了显著成功，但其内部机制仍然是不透明的黑箱。现有可解释性方法各有局限性，需要补充性的新视角来深入理解模型内部运作。

Method: CAST框架使用Moore-Penrose伪逆直接估计每层的实现变换矩阵，并应用频谱分析，使用六个可解释指标来表征层行为，包括核分析来识别层间功能关系。

Result: 分析发现解码器模型表现出压缩-扩展循环，而编码器模型保持一致的高秩处理。核相似性矩阵将层划分为三个阶段：特征提取、压缩和专业化。

Conclusion: CAST提供了一种补充现有方法的视角，通过直接矩阵估计和频谱分析揭示了Transformer层的功能特性，有助于更好地理解语言模型的内部机制。
...
### [267] [Nonparametric Data Attribution for Diffusion Models](https://arxiv.org/abs/2510.14269)
*Yutian Zhao,Chao Du,Xiaosen Zheng,Tianyu Pang,Min Lin*

Main category: cs.LG

TL;DR: 提出了一种用于扩散模型的数据归因非参数方法，通过图像块级相似性衡量训练样本对生成输出的影响，无需模型梯度或重新训练。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型归因方法需要模型梯度或重新训练，限制了在专有或大规模设置中的适用性，需要一种仅基于数据的归因方法。

Method: 基于最优得分函数的解析形式，通过图像块级相似性测量生成图像与训练图像之间的影响，支持多尺度表示并通过卷积加速计算。

Result: 实验表明该方法实现了强大的归因性能，与基于梯度的方法接近，并显著优于现有非参数基线方法。

Conclusion: 该方法提供了一种计算高效、空间可解释的归因框架，能够揭示训练数据与输出之间的内在关系，且不依赖特定模型。
...
### [268] [Stable Prediction of Adverse Events in Medical Time-Series Data](https://arxiv.org/abs/2510.14286)
*Mayank Keoliya,Seewon Choi,Rajeev Alur,Mayur Naik,Eric Wong*

Main category: cs.LG

TL;DR: CAREBench是一个早期事件预测基准，评估多模态输入下的预测准确性和时间稳定性，提出稳定性指标来量化风险轨迹的短期变化。


<details>
  <summary>Details</summary>
Motivation: 当前早期事件预测系统忽略风险评分的稳定性评估，且主要基于表格数据，缺乏对多模态输入下风险轨迹行为的测试。

Method: 引入CAREBench基准，使用多模态输入（表格EHR、ECG波形、临床文本），提出基于局部Lipschitz常数的稳定性指标来惩罚风险评分的突然波动。

Result: 现有方法（特别是LLMs）在准确性和稳定性联合优化方面表现不佳，在高精度操作点召回率较差。
...
### [269] [Enhancing Time-Series Anomaly Detection by Integrating Spectral-Residual Bottom-Up Attention with Reservoir Computing](https://arxiv.org/abs/2510.14287)
*Hayato Nihei,Sou Nobukawa,Yusuke Sakemi,Kazuyuki Aihara*

Main category: cs.LG

TL;DR: 提出了一种结合谱残差方法和储备池计算的时间序列异常检测方法SR-RC，在不牺牲学习效率的情况下提高了检测性能。


<details>
  <summary>Details</summary>
Motivation: 储备池计算在边缘AI应用中具有优势，但单独使用时需要过大的储备池才能达到足够的异常检测性能，而注意力机制虽然能提高精度但计算量大。

Method: 将谱残差方法（一种无需学习的自底向上注意力机制）与储备池计算相结合，形成SR-RC方法。

Result: SR-RC在基准任务和真实世界时间序列数据集上优于传统RC和基于谱残差特征提取的逻辑回归模型。

Conclusion: SR-RC为在边缘AI中部署储备池计算进行时间序列异常检测提供了实用方向，因为谱残差方法也适合硬件实现。
...
### [270] [TED++: Submanifold-Aware Backdoor Detection via Layerwise Tubular-Neighbourhood Screening](https://arxiv.org/abs/2510.14299)
...
Method: TED++围绕每个类的隐藏特征流形构建管状邻域，从少量干净激活估计局部"厚度"，然后应用局部自适应排名(LAR)检测漂移到允许管外的激活。通过聚合所有层的LAR调整排名，捕捉输入在演化类子流形上的忠实度。

Result: 在基准数据集和任务上的广泛实验表明，TED++在自适应攻击和有限数据场景下实现最先进的检测性能。即使每类只有5个保留样本，TED++仍能实现近乎完美的检测，AUROC比次优方法提升高达14%。

Conclusion: TED++通过管约束行为特征有效检测隐蔽后门攻击，在有限数据场景下表现出色，为解决深度神经网络安全风险提供了有效解决方案。
...
### [271] [Active Measuring in Reinforcement Learning With Delayed Negative Effects](https://arxiv.org/abs/2510.14315)
...
<details>
  <summary>Details</summary>
Motivation: 在现实世界的强化学习环境中，测量状态可能成本高昂且对后续结果产生负面影响，需要一种能够权衡测量成本和收益的框架。

Method: 将AOMDP建模为周期性部分可观测MDP，提出基于信念状态的在线RL算法，并使用序列蒙特卡洛方法联合近似未知静态环境参数和未观测潜在状态的后验分布。

Result: 研究表明减少不确定性可以证明提高样本效率并增加最优策略的价值，尽管存在测量成本。在数字健康应用中评估了该算法。

Conclusion: AOMDP框架能够有效处理测量成本问题，通过主动选择何时测量状态来平衡信息获取和成本，在数字健康等应用中具有实用价值。
...
### [272] [LLM-ERM: Sample-Efficient Program Learning via LLM-Guided Search](https://arxiv.org/abs/2510.14331)
...
<details>
  <summary>Details</summary>
Motivation: 解决程序学习中样本效率和计算可行性之间的平衡问题。传统方法要么搜索成本高（程序枚举），要么需要大量样本（梯度训练）

Method: 提出LLM-ERM框架：使用预训练LLM生成候选程序，在保留数据上编译验证，选择最佳假设，无需反馈或梯度

Result: 理论证明SGD需要大量样本学习某些短程序；实证显示LLM-ERM仅需200样本即可解决奇偶校验、模式匹配等任务，而SGD训练即使10万样本也会过拟合
...
### [273] [DARTS-GT: Differentiable Architecture Search for Graph Transformers with Quantifiable Instance-Specific Interpretability Analysis](https://arxiv.org/abs/2510.14336)
*Shruti Sarika Chakraborty,Peter Minary*

Main category: cs.LG

TL;DR: 提出DARTS-GT方法，通过不对称注意力设计和可微分架构搜索实现深度异构的图Transformer，同时开发了首个量化可解释性框架来理解模型决策。


<details>
  <summary>Details</summary>
Motivation: 当前图Transformer存在设计僵化、缺乏量化可解释性的问题，固定GNN类型无法充分利用深度特定组件优势，复杂架构难以区分性能提升是来自有意义的模式还是虚假相关性。

Method: 重新设计图Transformer注意力为不对称结构，将结构编码与特征表示解耦：查询来自节点特征，键值来自GNN变换。使用DARTS为每层选择最优GNN算子，实现注意力内部的深度异构。开发因果消融的量化可解释性框架。

Result: 在8个基准测试中，DARTS-GT在4个数据集上达到最先进水平，在其他数据集上保持竞争力。发现的可解释性分析显示视觉注意力显著性与因果重要性并不总是相关，异构架构产生的模型比基线更具可解释性。

Conclusion: 图Transformer不需要在性能和可解释性之间做出选择，DARTS-GT通过深度异构设计和量化可解释性框架同时实现了高性能和可解释性。
...
### [274] [Stop-RAG: Value-Based Retrieval Control for Iterative RAG](https://arxiv.org/abs/2510.14337)
...
Method: 将迭代RAG建模为有限时域马尔可夫决策过程，引入基于价值的控制器Stop-RAG，使用完整轨迹的全宽度前向视图Q(λ)目标进行训练，兼容黑盒API和现有流程。
...
### [275] [Jet Functors and Weil Algebras in Automatic Differentiation: A Geometric Analysis](https://arxiv.org/abs/2510.14342)
*Amandip Sangha*

Main category: cs.LG

TL;DR: 本文提出了基于jet bundles和Weil代数的自动微分几何公式化方法，将反向模式AD视为余切拉回，泰勒模式对应Weil代数中的求值，并推导了正确性、稳定性和复杂性的简洁陈述。
...
Method: 使用jet bundles和Weil代数的几何框架，将反向模式AD表述为余切拉回，泰勒模式表述为Weil代数中的求值操作。

Result: 推导了反向模式的函子恒等式、高阶导数的代数精确性、截断误差的显式界，并证明张量化Weil代数可以一次性计算所有混合导数，避免嵌套JVP/VJP调度的组合爆炸。

Conclusion: 该框架为自动微分理论提供了几何解释，并为开发结构保持的微分方法奠定了基础，在深度学习和科学计算中具有应用价值。
...
### [276] [Are My Optimized Prompts Compromised? Exploring Vulnerabilities of LLM-based Optimizers](https://arxiv.org/abs/2510.14381)
*Andrew Zhao,Reshmi Ghosh,Vitor Carvalho,Emily Lawton,Keegan Hines,Gao Huang,Jack W. Stokes*

Main category: cs.LG

TL;DR: 本文首次系统分析了基于LLM的提示优化中的投毒风险，发现基于反馈的攻击比注入查询攻击更危险，提出了无需访问奖励模型的虚假奖励攻击，并设计了高亮防御方法显著降低风险。


<details>
  <summary>Details</summary>
Motivation: 随着LLM系统在日常AI应用中的广泛使用，提示优化阶段的安全性研究不足，需要系统评估投毒风险并建立防护措施。

Method: 使用HarmBench评估系统脆弱性，比较反馈攻击与查询注入攻击的效果，设计无需奖励模型访问的虚假奖励攻击，并提出高亮防御方法。

Result: 反馈攻击将攻击成功率提升高达ΔASR=0.48，虚假奖励攻击的ΔASR为0.23，而高亮防御可将其降至0.07且不影响实用性。

Conclusion: 提示优化流程是重要的攻击面，需要加强对反馈通道和优化框架的安全防护。
...
### [277] [SHaRe-SSM: An Oscillatory Spiking Neural Network for Target Variable Modeling in Long Sequences](https://arxiv.org/abs/2510.14386)
*Kartikay Agrawal,Abhijeet Vikram,Vedant Sharma,Vaishnavi N.,Ayon Borthakur*

Main category: cs.LG

TL;DR: 提出SHaRe-SSM（脉冲谐波共振点火状态空间模型），一种用于超长序列建模的第二阶脉冲神经网络状态空间模型，在保持性能的同时显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 结合脉冲神经网络（SNN）的能效优势和状态空间模型（SSM）在长序列建模中的能力，为资源受限应用开发高效的序列建模方法。

Method: 设计基于共振点火神经元的二阶脉冲SSM，利用并行扫描实现动态系统的稳定高效实现，并提出基于核的脉冲回归器。

Result: 在18k序列上比二阶基于ANN的SSM能耗降低73倍，在50k序列上仍保持优越性能，平均表现优于transformer和一阶SSM。

Conclusion: SHaRe-SSM为超长序列建模提供了一种高效节能的解决方案，同时系统分析了共振点火SSM中异质性、耗散和守恒的影响。
...
### [278] [Revisit Modality Imbalance at the Decision Layer](https://arxiv.org/abs/2510.14411)
*Xiaoyu Ma,Hao Chen*

Main category: cs.LG

TL;DR: 本文揭示了多模态学习中不仅存在表示层的不平衡，决策层也存在显著偏差。研究发现即使经过充分预训练和平衡优化，模型仍会系统性地偏向某些模态（如音频），这种偏差源于特征空间和决策权重分布的内在差异。作者建议未来多模态系统应在决策层引入自适应权重分配机制。


<details>
  <summary>Details</summary>
Motivation: 多模态学习虽然能整合不同模态信息提升性能，但经常遭受模态不平衡问题，即主导模态在联合优化过程中压制较弱模态。本文旨在揭示这种不平衡不仅发生在表示学习阶段，在决策层也显著存在。

Method: 通过在音频-视觉数据集（CREMAD和Kinetic-Sounds）上进行实验，分析模型在充分预训练和平衡优化后仍表现出的系统性模态偏差。进一步分析特征空间和决策权重分布的差异。

Result: 实验表明，即使经过广泛预训练和平衡优化，模型仍然表现出对某些模态（如音频）的系统性偏好。这种偏差源于特征空间和决策权重分布的内在差异，而非仅由优化动态引起。

Conclusion: 在融合阶段聚合未校准的模态输出会导致决策层权重偏差，阻碍较弱模态的有效贡献。未来多模态系统应更多关注在决策层纳入自适应权重分配机制，根据各模态能力实现相对平衡。
...
### [279] [Interaction Concordance Index: Performance Evaluation for Interaction Prediction Methods](https://arxiv.org/abs/2510.14419)
*Tapio Pahikkala,Riikka Numminen,Parisa Movahedi,Napsu Karmitsa,Antti Airola*

Main category: cs.LG

TL;DR: 提出了交互一致性指数(IC-index)来评估药物-靶点亲和力预测中交互方向的准确性，补充现有的预测性能评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有的药物-靶点亲和力预测方法主要关注预测值的准确性，但忽略了交互效应的方向性，而正确捕捉交互方向对于药物分配决策至关重要。

Method: 引入IC-index评估交互方向预测性能，分析预测器对交互效应的捕捉能力，并研究机器学习算法的置换等变性对交互学习的影响。

Result: 实验表明IC-index能够有效评估不同预测方法在交互方向预测上的表现，补充现有评估指标的不足。
...
### [280] [MergeMoE: Efficient Compression of MoE Models via Expert Output Merging](https://arxiv.org/abs/2510.14436)
...
<details>
  <summary>Details</summary>
Motivation: MoE技术虽然能有效扩展模型规模，但其巨大的内存开销使得模型压缩成为重要研究方向。专家合并作为一种新兴的MoE压缩技术，需要从新的理论角度进行理解。

Method: 提出MergeMoE方法，将专家合并过程解释为在前向计算中插入额外矩阵，并基于此建立优化公式，利用数学优化构建压缩矩阵。
...
### [281] [A Free Lunch in LLM Compression: Revisiting Retraining after Pruning](https://arxiv.org/abs/2510.14444)
*Moritz Wagner,Christophe Roux,Max Zimmer,Sebastian Pokutta*

Main category: cs.LG

TL;DR: 该论文研究发现，在大型语言模型剪枝后，逐层重构注意力机制和MLP组件比完整重训练更有效，不仅计算资源需求更少，还能获得更好的性能表现。


<details>
  <summary>Details</summary>
Motivation: 挑战传统观点，即认为LLM剪枝后应避免重训练，研究剪枝后权重重构与重训练的关键设计选择。

Method: 在GPT架构上进行广泛计算研究，比较不同重构粒度（单矩阵、组件级、块级）与重训练的效果。

Result: 发现组件级重构（分别重构注意力和MLP）是帕累托最优方案，比完整重训练性能更好且内存需求更少。

Conclusion: 重构步骤的重要性超过复杂剪枝标准，挑战了"不惜一切代价避免重训练"的传统观念。
...
### [282] [Towards geological inference with process-based and deep generative modeling, part 1: training on fluvial deposits](https://arxiv.org/abs/2510.14445)
*Guillaume Rongier,Luk Peeters*

Main category: cs.LG

TL;DR: 本研究探索使用生成对抗网络(GAN)来复制过程模型模拟的河流沉积物，通过消融实验验证了深度学习技术在3D地质图像生成中的可转移性，并展示了如何利用沉积时间等地质属性来监控GAN性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法难以准确再现地质结构特别是河流沉积物的连续性特征，而过程模型虽然能模拟地质过程但计算成本高昂，因此需要探索更高效的生成模型。

Method: 使用生成对抗网络(GAN)作为深度生成模型，训练其复制过程模型模拟的河流沉积物，通过消融研究验证2D图像生成技术向3D地质图像的迁移性。

Result: GAN训练保持稳定，生成的样本能够再现沉积物的非平稳性和细节特征，没有出现模式崩溃或对训练数据的纯粹记忆，且能够遵循地层叠置律等地质原理。

Conclusion: GAN在针对特定地质结构的训练数据集上表现出比预期更强的鲁棒性，利用地质原理如地层叠置律来指导深度生成模型具有很大潜力，但在更大3D图像和多模态数据集上的适用性仍需进一步验证。
...
### [283] [Feature Selection and Regularization in Multi-Class Classification: An Empirical Study of One-vs-Rest Logistic Regression with Gradient Descent Optimization and L1 Sparsity Constraints](https://arxiv.org/abs/2510.14449)
...
Result: 手动梯度下降达到92.59%测试准确率，scikit-learn提供24倍训练加速和98.15%准确率。L1正则化实现54-69%特征减少，仅损失4.63%准确率。最优5特征子集实现62%复杂度降低和92-94%准确率。

Conclusion: 研究为资源受限环境中的化学分析提供了可操作指南，平衡全面化学分析与针对性特征测量，实现成本效益部署。
...
### [284] [Coder as Editor: Code-driven Interpretable Molecular Optimization](https://arxiv.org/abs/2510.14455)
*Wenyu Zhu,Chengzhu Li,Xiaohe Tian,Yifan Wang,Yinjun Jia,Jianhui Wang,Bowen Gao,Ya-Qin Zhang,Wei-Ying Ma,Yanyan Lan*

Main category: cs.LG

TL;DR: MECo是一个将分子优化重新定义为级联框架的方法：首先生成人类可理解的编辑意图，然后通过代码生成将这些意图转化为可执行的结构编辑，从而解决LLMs在SMILES表示上执行修改的困难。


<details>
  <summary>Details</summary>
Motivation: 分子优化是药物发现的核心任务，需要精确的结构推理和领域知识。虽然LLMs在生成高层次编辑意图方面表现出潜力，但在执行这些修改时经常遇到困难，特别是在处理非直观的SMILES表示时。

Method: MECo将分子优化重新定义为级联框架：1）从分子和属性目标生成人类可理解的编辑意图；2）通过代码生成将这些意图转化为可执行的结构编辑。

Result: 在重现来自化学反应和靶点特异性化合物对的真实编辑时达到超过98%的准确率。在下游优化基准测试中，一致性提高了38-86个百分点至90%以上，相比基于SMILES的基线方法获得更高的成功率，同时保持结构相似性。
...
### [285] [Holdout-Loss-Based Data Selection for LLM Finetuning via In-Context Learning](https://arxiv.org/abs/2510.14459)
*Ling Zhang,Xianliang Yang,Juwon Yu,Park Cheonyoung,Lei Song,Jiang Bian*

Main category: cs.LG

TL;DR: 提出了一种基于上下文近似(ICA)的数据选择和重加权框架，通过少量精心选择的验证集来评估候选训练数据的价值，无需参考模型或额外微调，在多种对齐方法中都能有效提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前微调大型预训练语言模型时，噪声或偏离目标的训练样本会稀释监督信号。虽然小规模精选数据集往往能达到大规模数据集的性能，但系统高效识别高价值训练数据的方法仍待探索。现有方法多依赖启发式规则或昂贵的重新训练。

Method: 核心是上下文近似(ICA)方法，通过在上下文中条件化少量精选验证集，估计模型在候选样本上训练后的验证损失。ICA无需参考模型和额外微调，在局部线性化下等价于向验证最优解的一阶更新。从ICA分数导出每个样本的权重，在模型参数演化时动态重加权梯度更新。

Result: 在SFT、DPO和SimPO等多种对齐方法上，以及不同骨干网络和数据集上，基于ICA的重加权方法都能以最小开销持续改善模型对齐性能。分析了分数更新频率和验证集大小k的影响，并指出了在快速漂移的在线策略更新中的局限性。

Conclusion: ICA提供了一个理论上有依据、资源高效的数据选择和重加权框架，能够有效识别高价值训练数据，提升模型对齐效果，为未来工作指明了方向。
...
### [286] [From Guess2Graph: When and How Can Unreliable Experts Safely Boost Causal Discovery in Finite Samples?](https://arxiv.org/abs/2510.14488)
...
### [287] [Learning to Undo: Rollback-Augmented Reinforcement Learning with Reversibility Signals](https://arxiv.org/abs/2510.14503)
...
Method: 使用经验推导的状态-动作可逆性度量Phi(s,a)来量化返回先前状态的可能性，并动态调整时间差分更新中的惩罚项；同时引入选择性状态回滚操作，当预期回报显著低于瞬时估计值时回滚到前一个状态。

Result: 在CliffWalking v0环境中减少99.8%的灾难性跌落，平均回合回报提升55%；在Taxi v3环境中抑制99.9%的非法动作，累积奖励提升65.7%，同时显著降低两个环境中的奖励方差。

Conclusion: 回滚机制是实现安全和性能提升的关键组件，为安全可靠的顺序决策迈出了稳健的一步。
...
### [288] [Enhancing Time Series Forecasting through Selective Representation Spaces: A Patch Perspective](https://arxiv.org/abs/2510.14510)
*Xingjian Wu,Xiangfei Qiu,Hanyin Cheng,Zhengyu Li,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: 该论文提出选择性表示空间(SRS)模块，通过可学习的选择性分块和动态重组技术，自适应地选择和重排时间序列片段，提升基于分块的时间序列预测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统分块方法将时间序列划分为相邻片段，导致表示空间固定，表达能力不足。需要构建选择性表示空间来灵活包含最具信息量的片段。

Method: 提出SRS模块，包含选择性分块和动态重组技术，自适应地从上下文时间序列中选择和重排片段。构建SRSNet模型，由SRS模块和MLP头组成。

Result: SRSNet在多个领域的真实世界数据集上实现了最先进的性能。SRS模块作为即插即用组件，也能提升现有基于分块的模型的性能。

Conclusion: 选择性表示空间模块通过自适应选择和重排片段，有效提升了时间序列预测模型的性能，具有广泛的适用性。
...
### [289] [On the Identifiability of Tensor Ranks via Prior Predictive Matching](https://arxiv.org/abs/2510.14523)
...
<details>
  <summary>Details</summary>
Motivation: 张量分解中潜在维度（秩）的选择通常依赖启发式方法，需要更严格的理论方法来确定秩的可识别性。

Method: 将矩匹配条件转化为对数线性方程组，建立秩可识别性与方程组可解性之间的等价关系，应用于PARAFAC/CP、Tensor Train、Tensor Ring和Tucker四种张量模型。

Result: PARAFAC/CP、Tensor Train和Tensor Ring模型的秩是可识别的，而Tucker模型的秩由于对称拓扑导致系统欠定而不可识别。对于可识别模型，推导了基于观测数据矩的显式闭式秩估计器。

Conclusion: 该方法为张量分解中的秩选择提供了理论依据，证明了不同张量模型结构的可识别性差异，并开发了实用的秩估计器。
...
### [290] [Agentic Entropy-Balanced Policy Optimization](https://arxiv.org/abs/2510.14545)
...
Result: 在14个挑战性数据集上，AEPO始终优于7种主流RL算法。仅使用1K RL样本，Qwen3-14B结合AEPO在GAIA上达到47.6% Pass@1和65.0% Pass@5，在Humanity's Last Exam上达到11.2% Pass@1和26.0% Pass@5，在WebWalker上达到43.0% Pass@1和70.0% Pass@5。
...
### [291] [MX+: Pushing the Limits of Microscaling Formats for Efficient Large Language Model Serving](https://arxiv.org/abs/2510.14557)
*Jungi Lee,Junyong Park,Soohyun Cha,Jaehoon Cho,Jaewoong Sim*

Main category: cs.LG

TL;DR: 提出MX+格式，一种基于微缩放格式的非侵入式扩展，通过重新利用异常值的指数字段作为扩展尾数来提高精度，在4位精度下显著提升语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有低精度数据格式要么需要侵入式软件修改，要么难以在硬件厂商间广泛采用。行业驱动的块浮点格式在超低位宽下因块内异常值问题而性能不佳。

Method: 基于微缩放格式，提出MX+扩展，关键洞察是异常值不需要使用其指数字段，可将其重新用作扩展尾数来提高异常值元素的精度。

Result: MX+相比4位MX格式显著提升模型性能，存储开销和速度损失可忽略，为高效LLM推理提供了比MXFP4或MXFP6更优的选择。

Conclusion: MX+是一种成本效益高且非侵入式的解决方案，通过巧妙处理异常值问题，在超低位宽下实现了可接受的语言模型性能。
...
### [292] [Redundancy-Aware Test-Time Graph Out-of-Distribution Detection](https://arxiv.org/abs/2510.14562)
*Yue Hou,He Zhu,Ruomei Liu,Yingke Su,Junran Wu,Ke Xu*

Main category: cs.LG

TL;DR: RedOUT是一个无监督的图OOD检测框架，通过结构熵和Redundancy-aware Graph Information Bottleneck来减少结构冗余，提升图分类中的分布外检测性能。


<details>
  <summary>Details</summary>
Motivation: 训练数据和测试数据之间的分布差异导致模型在遇到分布外样本时预测不准确，现有图OOD检测方法因结构冗余导致的语义偏移而性能受限。

Method: 提出RedOUT框架，集成结构熵到测试时OOD检测中，引入Redundancy-aware Graph Information Bottleneck，将目标分解为本质信息和无关冗余，通过最小化结构熵减少解耦的冗余。

Result: 在真实世界数据集上的广泛实验显示RedOUT在OOD检测上表现优异，平均提升6.7%，在ClinTox/LIPO数据集对上显著超越最佳竞争对手17.3%。

Conclusion: RedOUT通过结构熵和信息瓶颈有效减少图结构冗余，显著提升了图分类中的OOD检测性能。
...
### [293] [State-Space Models for Tabular Prior-Data Fitted Networks](https://arxiv.org/abs/2510.14573)
*Felix Koch,Marcel Wever,Fabian Raisch,Benjamin Tischler*

Main category: cs.LG

TL;DR: 本文探讨了使用Hydra（一种双向线性时间结构化状态空间模型）替代TabPFN中的Transformer架构，以解决Transformer在序列长度上的二次复杂度问题，同时保持竞争性的预测性能。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在处理表格数据时存在序列长度的二次复杂度问题，而状态空间模型（SSM）虽然效率更高，但对输入标记顺序敏感，这在表格数据中是不期望的特性。

Method: 使用Hydra双向线性时间结构化状态空间模型作为Transformer的替代方案，研究双向方法如何减少顺序依赖性并实现对称上下文聚合。

Result: 实验表明该方法有效降低了顺序依赖性，预测性能与原始TabPFN模型相当。

Conclusion: Hydra模型可以作为Transformer在表格数据基础模型中的高效替代方案，在保持性能的同时解决了计算复杂度问题。
...
### [294] [Selective Labeling with False Discovery Rate Control](https://arxiv.org/abs/2510.14581)
*Huipeng Huang,Wenbo Liao,Huajun Xi,Hao Zeng,Mengchen Zhao,Hongxin Wei*

Main category: cs.LG

TL;DR: 提出了一种名为Conformal Labeling的新方法，通过控制AI标签的假发现率(FDR)来识别可被证明可信的AI预测实例，确保AI分配标签的质量。


<details>
  <summary>Details</summary>
Motivation: 解决AI模型标注大规模数据集时标签质量不可靠的问题，现有选择性标注方法缺乏对AI标签质量的理论保证，导致AI标注子集中错误率过高。

Method: 构建每个测试实例的conformal p值，通过比较AI模型的预测置信度与校准实例的误标置信度，选择p值低于数据依赖阈值的实例，证明AI预测可信。

Result: 实验证明该方法在各种任务（图像和文本标注、LLM问答）中实现了严格的FDR控制和高功率。

Conclusion: Conformal Labeling方法能够可靠地控制AI标签的假发现率，确保AI分配标签的质量，为大规模数据集标注提供了理论保证的解决方案。
...
### [295] [Matcha: Multi-Stage Riemannian Flow Matching for Accurate and Physically Valid Molecular Docking](https://arxiv.org/abs/2510.14586)
...
Method: 采用三阶段流匹配模型（在R3、SO3和SO2几何空间上运行），结合学习评分模型和无监督物理有效性过滤
...
### [296] [Multimodal RAG for Unstructured Data:Leveraging Modality-Aware Knowledge Graphs with Hybrid Retrieval](https://arxiv.org/abs/2510.14592)
*Rashmi R,Vidyadhar Upadhya*

Main category: cs.LG

TL;DR: 提出MAHA，一种用于多模态问答的模态感知混合检索架构，通过结合密集向量检索和结构化图遍历来增强多模态文档的检索能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成(RAG)的系统主要处理单模态文本数据，在处理包含文本、图像、表格、方程和图形的非结构化多模态文档时效果有限。

Method: MAHA采用模态感知知识图谱，将密集向量检索与结构化图遍历相结合，知识图谱编码跨模态语义和关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer设计在张量并行训练中存在显著的通信开销，特别是每个块的MHA-MLP连接需要all-reduce通信。研究发现MHA-MLP连接可以被绕过以提高效率。

Method: 提出FAL架构，将第一层注意力输出重定向到后续层的MLP输入，消除每块的MHA-MLP连接。FAL+版本还添加归一化的第一注意力输出到后续层MHA输出中，以增强MLP输入质量。

Result: FAL将多GPU训练时间减少高达44%，单GPU吞吐量提高1.18倍，相比基线GPT获得更好的困惑度。FAL+在不增加训练时间的情况下实现更低的困惑度。

Conclusion: FAL和FAL+通过重新设计Transformer架构，有效减少了通信开销，提高了训练效率，同时保持或改进了模型质量。
...
TL;DR: LeapFactual是一种基于条件流匹配的新型反事实解释算法，能够生成可靠且信息丰富的反事实解释，克服了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在高风险领域的应用增加，需要不仅准确而且可解释的模型。现有反事实解释方法存在梯度消失、潜在空间不连续等局限性。

Method: 基于条件流匹配的反事实解释算法，采用模型无关的方法，不限于可微分损失函数的模型，还能处理人机协同系统。

Result: 在基准和真实数据集上的实验表明，LeapFactual能生成准确且符合数据分布的反事实解释，提供可操作的见解，并能作为新训练数据提升模型性能。
...
Conclusion: 所提出的可逆流增强架构在保持预测准确性的同时，显著提高了星系形态分类模型的可解释性。
...
Method: 采用黎曼距离进行分布对齐，将一阶和二阶矩通过Siegel嵌入表示为单个对称正定矩阵，在SPD矩阵共享流形上使用自然几何距离同时适应两个矩。

Result: 在图像去噪和图像分类基准测试中验证了方法的有效性，代码已公开。

Conclusion: 该方法通过保持源域和目标域的均值和协方差结构，为跨域比较提供了更忠实的度量，并将黎曼流形距离与目标域误差界联系起来。
...
TL;DR: 提出了一种基于神经形态无线传感器网络的低功耗在线异常检测框架，采用事件驱动的神经形态传感器和脉冲无线电传输，通过在线假设检验和多臂老虎机优化传感器查询策略，在严格控制误报率的同时实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 针对脑机接口和远程环境监测等应用场景，需要低功耗、高效率的异常检测系统。传统方法难以在严格控制误报率的同时实现低延迟检测和节能通信。

Method: 使用事件驱动的神经形态传感器节点，产生脉冲信号响应环境变化；中央读取器节点主动查询传感器子集，采用基于e值的在线假设检验方法控制误报率，并将传感器查询策略建模为多臂老虎机问题动态优化。

Result: 性能评估表明，该方法能够在严格误报率要求下可靠检测异常，同时高效调度传感器通信并实现低检测延迟。

Conclusion: 该框架为低功耗在线异常检测提供了有效解决方案，在误报率控制、通信效率和检测延迟方面均表现出色，适用于多种实时监测应用。
...
<details>
  <summary>Details</summary>
Motivation: 现实联邦学习场景中客户端存在计算资源异构性和非独立同分布数据，现有个性化联邦学习方法通常忽视模型和数据异构性的共存问题

Method: FedPPA方法：1) 渐进对齐客户端通用层权重与全局模型权重；2) 集成基于熵的加权平均来增强全局模型性能

Result: 在MNIST、FMNIST和CIFAR-10三个图像分类数据集上的实验表明，FedPPA始终优于现有联邦学习算法，在个性化适应方面表现更优

Conclusion: FedPPA通过渐进参数对齐有效缓解全局和局部模型不一致性，同时保留客户端本地知识，在非独立同分布设置下增强了个性化鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 在自适应优化器如Adam中，批次大小增加的策略不明确，通常需要启发式调优，需要开发有原则的批次大小调度方法。
...
Conclusion: Seesaw提供了一种有原则的批次大小调度方法，能显著加速大语言模型预训练，同时保持训练动态和最终性能。
...
TL;DR: Tawa是一个自动化编译器，能从高级瓦片化程序自动生成高性能的warp专用代码，通过异步引用抽象简化GPU数据流编程，在H100 GPU上实现优于cuBLAS和Triton的性能。


<details>
  <summary>Details</summary>
Motivation: 现代GPU的专用硬件单元支持高性能异步数据流执行，但传统的SIMT编程模型与这种任务并行硬件不匹配，导致编程复杂性高，开发者需要手动编排复杂的低级通信和软件管道。

Method: 提出Tawa编译器，使用新型IR抽象——异步引用(aref)来表达warp级通信，自动将程序划分为生产者-消费者角色并管理复杂的数据流管道，无需开发者进行侵入式内核重写。

Result: 在NVIDIA H100 GPU上评估代表性LLM内核，Tawa实现了高硬件利用率，比高度优化的cuBLAS GEMM内核快1.1倍，注意力工作负载比Triton快1.2倍，性能与手工优化的CUTLASS C++ FlashAttention-3内核相当。

Conclusion: Tawa通过自动化warp专用代码生成，显著降低了GPU数据流编程的复杂性，在保持高性能的同时大幅减少了编程工作量。
...
TL;DR: INDAGO-Nexus是一种多目标搜索方法，通过联合优化故障可能性和测试场景多样性，显著提高了深度强化学习代理的故障发现效果和效率。


<details>
  <summary>Details</summary>
Motivation: 现有工具如INDAGO专注于单一目标优化，仅最大化故障数量，但无法确保发现的场景具有多样性或揭示不同的错误类型。

Method: 使用多目标进化算法，结合多种多样性度量和帕累托前沿选择策略，联合优化故障可能性和测试场景多样性。

Result: 在三个DRL代理上评估：人形步行者、自动驾驶汽车和停车代理。平均而言，INDAGO-Nexus在SDC和停车场景中分别比INDAGO多发现83%和40%的独特故障，同时所有代理的故障发现时间减少高达67%。

Conclusion: INDAGO-Nexus通过多目标优化方法显著提高了测试深度强化学习代理的有效性和效率，能够发现更多样化和独特的故障场景。
...
TL;DR: 提出了未来摘要预测（FSP）方法，通过预测序列的长期未来摘要来改进语言模型的长期推理能力，相比传统的下一个词预测和多词预测有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统下一个词预测（NTP）在长期推理、规划和创意写作方面存在局限，多词预测（MTP）只能捕捉短期依赖关系，改进有限。需要一种能够处理长期依赖关系的方法。

Method: 提出未来摘要预测（FSP），训练一个辅助头来预测长期未来的紧凑表示。探索两种变体：手工制作的摘要（如词袋摘要）和学习的摘要（使用从右到左训练的反向语言模型生成的嵌入）。

Result: 在大规模预训练实验（3B和8B参数模型）中，FSP在数学、推理和编码基准测试上相比NTP和MTP都提供了改进。

Conclusion: FSP是一种有效的替代方法，能够提升语言模型在长期推理任务上的表现，为改进语言模型的长期能力提供了新思路。
...
TL;DR: 提出了一种新算法，用于在线性非高斯有向无环图模型中识别包含潜在混杂变量的因果结构，允许潜在变量之间、观测变量之间以及两者之间存在因果关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设潜在混杂变量相互独立，或者无法正确处理观测变量之间存在因果关系的模型，存在局限性。

Method: 利用观测数据的高阶累积量来识别因果结构，允许潜在变量之间、观测变量之间以及两者之间存在因果关系。

Result: 广泛的模拟实验和真实世界数据实验证明了所提算法的有效性和实际应用价值。

Conclusion: 提出的新算法能够有效识别包含潜在混杂变量的因果有向无环图，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统的被动众包方法效率较低，需要开发能够主动引导移动代理收集高质量测量值的定位方法，同时考虑实际城市环境中的障碍和移动约束。

Method: 修改A*算法为A-UCB*，将采集值纳入轨迹成本计算，生成高采集值的规划路径，结合贝叶斯优化来指导移动代理收集接收信号强度测量值。

Result: 在真实城市场景模拟中，该方法相比无信息基线方法，能够用更少的测量值实现准确的定位，在不同环境下都表现出稳定的性能。

Conclusion: 所提出的主动干扰源定位框架通过结合贝叶斯优化和采集感知路径规划，能够高效准确地定位干扰源，为城市环境中的干扰源定位提供了有效的解决方案。
...
TL;DR: 提出SPHeRe方法，通过整合正交性和结构信息保留来解决传统Hebbian学习在机器学习中的局限性，在无监督学习中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统Hebbian学习在机器学习应用中存在连接更新无约束和缺乏反馈调节的问题，限制了其在复杂网络架构和任务中的有效扩展。
...
Conclusion: 证明了Hebbian无监督学习规则在现代深度学习框架中的竞争力和潜力，展示了无需严格依赖反向传播的高效生物启发学习算法的可能性。
...
TL;DR: 提出了置换增强结构化稀疏训练(PA-DST)方法，通过学习每层的置换矩阵与结构化权重矩阵，在保持高精度的同时显著提升训练和推理速度。


<details>
  <summary>Details</summary>
Motivation: 结构化稀疏在现代GPU上能加速训练和推理，但在准确性上仍落后于非结构化动态稀疏训练(DST)，主要原因是表达能力受限。

Method: 为每个层学习单个置换矩阵与结构化权重矩阵，应用于块、N:M和对角线三种典型结构。

Result: 在ImageNet-1K(ViT-B/16)和WikiText-103(GPT-2)上，PA-DST在90-95%稀疏度下与非结构化基线(RigL, SET)精度相当，但训练快1.21倍，推理快2.9倍。

Conclusion: 结构+学习置换在准确性和效率之间找到了最佳平衡点。
...
TL;DR: 提出了ShifTS框架，先处理时间序列中的时间偏移，再处理概念漂移，通过软注意力机制从回看和预测时间序列中寻找不变模式，提升预测准确性。
...
Method: 提出ShifTS框架：1）先处理时间偏移作为预处理步骤；2）使用软注意力机制从回看和预测时间序列中寻找不变模式来处理概念漂移。该方法与具体模型无关。
...
Method: 使用LLM合成特征函数（将数据点映射到标量的代码），然后与决策树结合。提出两种算法：FunSearch的变体学习特征，以及ID3的变体在分裂节点时按需生成新特征。
...
TL;DR: SSMs在长序列建模中具有效率优势，但理论证明无法解决真正的长序列生成问题。通过引入外部工具交互，SSMs能够克服这一限制，实现长度泛化。


<details>
  <summary>Details</summary>
Motivation: SSMs作为Transformer的替代方案，在长上下文和长序列生成中具有效率优势，但存在理论上的局限性，需要探索如何克服这些限制。

Method: 通过为SSMs提供外部工具交互能力，结合问题相关的训练数据，使SSMs能够学习解决任何可处理的问题。

Result: 工具增强的SSMs在算术、推理和编程任务上实现了显著的序列长度泛化能力。

Conclusion: SSMs在交互式工具和智能体场景中，可能成为Transformer的高效替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 6G多RAT网络中，蜂窝和WiFi发射器共存，需要可靠的移动性决策来应对快速信道动态、干扰和异构覆盖。现有切换机制仍高度被动，依赖瞬时测量和阈值事件。

Method: 提出模型驱动、短时域信号质量预测的ML辅助预测性条件切换框架，使用RAT感知LSTM网络预测移动用户信号质量指标，包含数据收集、并行预测、决策逻辑和CHO执行的标准化工作流。

Result: 在软硬切换设置下测试，显示带滞后的P-CHO方案能够减少切换失败和乒乓事件，实现准确、低延迟的主动切换。

Conclusion: 提出的P-CHO框架适用于6G多RAT部署中的ML辅助切换控制，能够实现准确、低延迟的主动切换。
...
TL;DR: 提出了一种新型的随机奖励机及其学习算法，用于处理强化学习中存在噪声的稀疏奖励问题，相比现有方法表现更优。
...
Result: 在两个案例研究中证明该算法有效，在存在噪声的奖励函数处理上优于现有方法和朴素方法。

Conclusion: 该算法能够学习随机奖励机，保证在极限情况下收敛到最优策略，解决了奖励噪声问题。
...
TL;DR: 本文对梯度上升方法在机器遗忘中的有效性进行了理论分析，证明该方法能在不重新训练的情况下有效移除特定数据点的影响，同时保持模型在保留数据上的性能。
...
Method: 利用梯度下降的隐式偏差，通过梯度上升步骤逆转特定数据点的影响，提出新的成功标准(ε,δ,τ)-successful unlearning来衡量遗忘质量。

Result: 在线性模型和高维数据的两层神经网络中，适当缩放的梯度上升步骤能满足成功标准，产生与在保留数据上重新训练的解相近的模型。在合成高斯混合设置中，梯度上升在成功遗忘的同时仍保持泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型容易受到对抗性扰动和后门攻击的影响，现有移除方法需要昂贵的微调且会降低其他任务性能，因此需要找到不损害模型通用能力的后门移除方案。

Method: 研究发现后门在模型权重空间中与其他良性任务是解缠的，基于此提出利用这种解缠特性来隔离和擦除后门影响的反学习方法。

Result: 在CLIP模型和常见对抗触发器的实验中，该方法实现了近乎完美的反学习效果，平均保持96%的干净准确率，即使攻击未知时也能通过逆向工程触发器成功移除后门。

Conclusion: 该方法在反学习和干净准确率的权衡上优于现有最先进的防御方法，证明了后门可以在不损害模型通用能力的情况下被有效移除。
...
TL;DR: 提出Hermite特征结构假设(HEA)，通过分析核特征值和特征函数来预测真实数据集上的学习曲线，证明该框架可用于端到端理论建模。
...
Method: 使用HEA近似核特征值和特征函数，基于经验数据协方差矩阵和目标函数的多项式分解来预测学习曲线。

Result: HEA在真实图像数据上表现良好，能够准确预测学习曲线，并发现MLP在特征学习阶段也按HEA预测的顺序学习Hermite多项式。

Conclusion: HEA框架证明了在真实数据集上建立从数据到性能的端到端学习理论的可行性，为理解复杂学习算法提供了新视角。
...
TL;DR: 提出了一种在无导师情况下处理无界奖励的上下文赌博机模型，通过谨慎探索避免不可逆错误


<details>
  <summary>Details</summary>
Motivation: 在高风险AI应用中，传统算法假设所有错误都可恢复，但实际中单个错误可能造成不可逆损害。现有方法依赖导师帮助，但导师并非总是可用

Method: 使用包含弃权选项的双动作上下文赌博机模型，提出基于谨慎的算法，仅在有足够证据证明无害时才执行任务策略

Result: 在i.i.d.输入条件下建立了次线性遗憾保证，理论上证明了谨慎探索在高风险环境中部署学习智能体的有效性

Conclusion: 谨慎探索策略能够在不依赖导师的情况下安全部署学习智能体，避免不可逆损害
...
TL;DR: 提出一种无需额外训练的推理算法，通过纯采样方法在推理时提升基础模型的推理能力，性能接近甚至超过RL训练后的模型


<details>
  <summary>Details</summary>
Motivation: 探索是否能在不进行额外训练的情况下，仅通过推理时的纯采样方法就能激发基础模型的可比推理能力，而不是依赖RL训练

Method: 受MCMC技术启发，提出基于基础模型自身似然的简单迭代采样算法

Result: 在多种单次任务上大幅提升推理能力，性能接近甚至超过RL训练，同时避免了RL训练导致的多样性崩溃问题
...
TL;DR: 提出了WeightLens和CircuitLens两种方法，通过分析权重和特征交互来增强神经网络的可解释性，超越传统的基于激活的方法。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法依赖人工检查且局限于简单任务，自动化方法虽然可扩展但容易忽略特征间交互，且依赖外部LLM和数据集质量。
...
TL;DR: 本文提出了一种新的扩散强制采样器，用于加速循环深度语言模型的生成，通过在每个前向传递中解码新token，同时并行优化这些token的潜在状态，实现了比自回归基线更高效的生成。


<details>
  <summary>Details</summary>
Motivation: 探索循环深度模型与扩散语言模型之间的关系，利用它们的相似性来开发更高效的生成方法，解决循环深度模型在推理时额外计算并行化的问题。

Method: 基于扩散文献原理开发扩散强制采样器，在每个模型前向传递中解码新token，同时通过循环并行优化这些token的潜在状态，无需对现有3.5B循环深度transformer进行调优。

Result: 该采样器在相同时间预算下比自回归生成更具表达能力，直接应用于现有模型可实现高达5倍的加速，同时表明循环深度模型可被视为强大的连续因果扩散语言模型。

Conclusion: 提出的采样器不仅为循环深度模型在推理时提供了高效的并行计算机制，还揭示了此类模型可自然地视为强大的连续因果扩散语言模型。
...
TL;DR: 论文提出使用恒等链接函数而非传统的logit/probit链接来保持TVD-MI成对比较的加性结构，通过Gini熵最大化推导出箱约束最小二乘模型，在33%覆盖率下实现高效的大语言模型评估。
...
Method: 从Gini熵最大化推导出剪裁线性模型，采用箱约束最小二乘公式处理边界饱和问题，使用恒等链接函数而非非线性链接。

Result: 在33%覆盖率下获得0.117±0.008的留一均方根误差，保持代理排名（Spearman ρ=0.972±0.015），比全密集评估减少三倍评估次数。法官稳健性分析显示强一致性（ρ=0.872）。

Conclusion: TVD-MI的几何特性通过恒等映射得到最佳保持，适用于高效LLM评估和其他有界响应领域。
...
TL;DR: 本文扩展了生物信息神经网络(BINNs)在作物基因组预测和选择中的应用，通过整合SNPs、多组学数据和先验生物知识，显著提高了预测准确性，并能发现传统方法无法识别的非线性生物关系。


<details>
  <summary>Details</summary>
Motivation: 传统基因型到表型(G2P)模型准确度有限，依赖大量昂贵田间试验。现有结合分子表型的模型虽能提高预测精度，但在实际育种中不实用，因为部署时无法获得这些数据。

Method: BINNs通过编码通路级归纳偏置，仅在训练时使用多组学数据，在推理时仅需基因型数据。该方法整合了SNPs、多组学测量和先验生物知识。

Result: 在玉米基因表达和多环境田间试验数据中，BINNs在稀疏数据条件下将排序相关准确性提高了56%，并能非线性识别GWAS/TWAS无法发现的基因。在合成代谢组学基准测试中，相比传统神经网络减少了75%的预测误差。

Conclusion: BINNs建立了一个利用中间领域信息提高基因组预测准确性的框架，能揭示非线性生物关系，指导基因组选择、候选基因选择、通路富集和基因编辑优先排序。
...
TL;DR: 提出基于策略的流模型（π-Flow），通过修改学生流模型的输出层来预测无网络策略，在子步骤上产生动态流速度，实现快速准确的ODE积分，避免质量-多样性权衡问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统少步扩散或流模型蒸馏过程中教师模型预测速度与学生模型预测捷径之间的格式不匹配问题，以及由此导致的复杂蒸馏过程和质量-多样性权衡。

Method: 修改学生流模型的输出层，使其预测一个无网络策略，该策略在后续子步骤产生动态流速度；引入模仿蒸馏方法，使用标准ℓ2流匹配损失将策略的速度与教师模型沿策略轨迹的速度相匹配。

Result: 在ImageNet 256²上达到1-NFE FID为2.85，优于相同DiT架构的MeanFlow；在FLUX.1-12B和Qwen-Image-20B上，4 NFEs时比最先进的少步方法获得显著更好的多样性，同时保持教师级别的质量。
...
### [326] [PoissonNet: A Local-Global Approach for Learning on Surfaces](https://arxiv.org/abs/2510.14146)
*Arman Maesumi,Tanish Makadia,Thibault Groueix,Vladimir G. Kim,Daniel Ritchie,Noam Aigerman*

Main category: cs.GR

TL;DR: PoissonNet是一种新颖的神经架构，通过局部-全局学习方案解决网格学习中的高频特征学习、感受野不足、离散化敏感性和计算开销等问题，在语义分割和表面参数化方面达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有网格学习架构在学习高频特征、感受野范围、离散化敏感性和计算效率之间存在微妙的权衡，需要一种能够克服所有这些缺陷的新方法。

Method: 使用泊松方程作为特征传播的主要机制，在网格的梯度域中应用学习的局部特征变换，然后求解泊松系统以在表面上全局传播标量特征更新。

Result: 在各种实验中，与之前的固有架构相比，在语义分割和参数化高度详细的动画表面方面达到了最先进的性能，并且在学习变形方面显著优于现有的表面学习架构。

Conclusion: PoissonNet的局部-全局学习框架保留了特征的完整频率谱，提供真正的全局感受野，同时对网格三角化保持不可知性，计算效率高，具有可扩展性。
...
### [327] [Decision Oriented Technique (DOTechnique): Finding Model Validity Through Decision-Maker Context](https://arxiv.org/abs/2510.13858)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的有效性框架，但这些框架可能不可用或不充分。模型有效性对决策过程至关重要，需要更灵活的有效性验证方法。
...
Conclusion: DOTechnique技术有潜力通过决策者上下文来支持模型有效性的发现，为模型验证提供了新的思路。
...
### [328] [Do Slides Help? Multi-modal Context for Automatic Transcription of Conference Talks](https://arxiv.org/abs/2510.13979)
*Supriti Sinhamahapatra,Jan Niehues*

Main category: cs.AI

TL;DR: 该研究提出了一种融合视觉信息（演讲者图像和演示幻灯片）的多模态语音识别方法，在科学演讲场景中显著降低了词错误率，特别是对领域特定术语的识别效果提升明显。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的自动语音识别系统主要依赖声学信息，忽略了多模态上下文。视觉信息在消歧和适应方面至关重要，特别是在科学演讲场景中，演示幻灯片包含重要信息。

Method: 创建多模态演讲基准并自动分析领域特定术语；探索用多模态信息增强语音模型的方法；通过数据增强方法解决缺少配套幻灯片数据集的问题；使用增强数据集训练模型。

Result: 与基线模型相比，训练后的模型在所有词汇上的词错误率相对降低了约34%，在领域特定术语上的词错误率相对降低了约35%。

Conclusion: 集成视觉信息（特别是演示幻灯片）能显著提升语音识别性能，特别是在科学演讲等特定领域中，多模态方法具有重要价值。
...
### [329] [Agentic Design of Compositional Machines](https://arxiv.org/abs/2510.14980)
*Wenqian Zhang,Weiyang Liu,Zhen Liu*

Main category: cs.AI

TL;DR: 本文研究了大型语言模型在组合式机器设计任务中的能力，通过BesiegeField测试平台评估LLMs在空间推理、策略性组装和指令遵循方面的表现，并探索了强化学习作为改进途径。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能够学习创造性设计，特别是在组合式机器设计领域，这既是人类智能的标志，也是工程实践的基础。

Method: 引入BesiegeField测试平台，基于机器建造游戏Besiege构建，支持基于部件的构造、物理模拟和奖励驱动评估。使用代理工作流对最先进的LLMs进行基准测试，并探索强化学习微调实验。

Result: 当前开源模型在组合式机器设计任务中表现不足，特别是在空间推理、策略性组装和指令遵循方面存在挑战。

Conclusion: 在语言、机器设计和物理推理的交叉领域仍存在开放挑战，强化学习可能是一条改进路径，但需要进一步研究。
...
### [330] [Do Large Language Models Show Biases in Causal Learning? Insights from Contingency Judgment](https://arxiv.org/abs/2510.13985)
...
TL;DR: 研究发现大型语言模型在零相关情境下会系统性地产生因果错觉，错误推断不存在因果关系的变量之间存在因果关系。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否像人类一样容易产生因果错觉，特别是在医学等需要准确因果推理的领域中。

Method: 构建了1000个医学领域的零相关情境数据集，使用经典认知科学范式——列联判断任务来测试LLMs的因果推理能力。
...
### [331] [GammaZero: Learning To Guide POMDP Belief Space Search With Graph Representations](https://arxiv.org/abs/2510.14035)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要特定领域的神经网络架构且难以扩展，GammaZero旨在通过统一的图表示框架解决这些问题，实现从小规模问题到大规模问题的知识迁移。

Method: 使用图神经网络和编码器-解码器架构，将信念状态转换为动作中心图，从小规模问题的专家演示中学习价值函数和策略，然后将学习到的启发式方法应用于大规模问题的蒙特卡洛树搜索。

Result: 在标准POMDP基准测试中，GammaZero在相同规模问题上与BetaZero性能相当，同时能够零样本泛化到训练时未见的大2-4倍规模问题，在保持解质量的同时减少搜索需求。

Conclusion: GammaZero通过动作中心图表示实现了POMDP规划学习的可扩展性和泛化能力，为处理大规模部分可观测环境提供了有效解决方案。
...
### [332] [Position: Require Frontier AI Labs To Release Small "Analog" Models](https://arxiv.org/abs/2510.14053)
*Shriyash Upadhyay,Chaithanya Bandi,Narmeen Oozeer,Philip Quirke*

Main category: cs.AI

TL;DR: 本文提出一种替代性监管方法：要求大型AI实验室发布小型、开放的类比模型，这些模型是从其最大专有模型训练和蒸馏而来的缩小版本，旨在确保AI安全的同时促进创新。


<details>
  <summary>Details</summary>
Motivation: 当前前沿AI模型监管提案因安全与创新的权衡而被搁置，需要一种既能确保AI安全又积极促进创新的监管方法。

Method: 强制要求大型AI实验室发布小型、可公开访问的类比模型，这些模型以类似方式训练并从其最大专有模型蒸馏而来，作为公共代理进行安全验证、可解释性研究和算法透明度分析。

Result: 类比模型使广泛的研究社区能够直接调查和创新，显著减少监管负担并加速安全进展，同时最小化额外成本。

Conclusion: 这种监管方法不仅应该被采纳，而且说明了一个更广泛的原则：对模型的深入理解可以缓解安全与创新的权衡，让我们同时获得更多安全性和创新。
...
### [333] [Generating Fair Consensus Statements with Social Choice on Token-Level MDPs](https://arxiv.org/abs/2510.14106)
*Carter Blair,Kate Larson*

Main category: cs.AI

TL;DR: 提出基于多目标马尔可夫决策过程的共识声明生成框架，通过社会选择理论方法确保公平性保证
...
Method: 将任务建模为多目标、令牌级的马尔可夫决策过程，每个目标对应一个代理的偏好。提出两种基于社会选择理论的方法：保证在事前核心的随机生成策略，以及针对单一声明最大化平等福利的搜索算法

Result: 实验表明，基于平等目标的搜索算法生成的共识声明在代理对齐的最坏情况下优于基线方法，包括Habermas Machine

Conclusion: 该框架为共识声明生成提供了具有可证明公平性保证的形式化结构，通过社会选择理论原则实现了公平的文本生成
...
### [334] [STEMS: Spatial-Temporal Enhanced Safe Multi-Agent Coordination for Building Energy Management](https://arxiv.org/abs/2510.14112)
...
<details>
  <summary>Details</summary>
Motivation: 解决多建筑能源系统中的三个关键挑战：空间-时间信息利用不足、缺乏严格安全保证、系统复杂性，以实现碳减排、提高舒适度和降低能源成本。

Method: 集成两个核心组件：(1)使用GCN-Transformer融合架构的空间-时间图表示学习框架，捕捉建筑间关系和时序模式；(2)结合控制屏障函数的安全约束多智能体强化学习算法，提供数学安全保证。

Result: 在真实建筑数据集上的实验显示，STEMS相比现有方法实现21%成本降低、18%排放减少，安全违规从35.1%降至5.6%，仅0.13的不适比例保持最优舒适度，在极端天气下表现出强鲁棒性。

Conclusion: STEMS框架在多建筑能源管理中表现出卓越性能，有效平衡了能源效率、安全性和舒适度需求，具有实际应用价值。
...
### [335] [Formalizing the Safety, Security, and Functional Properties of Agentic AI Systems](https://arxiv.org/abs/2510.14133)
*Edoardo Allegrini,Ananth Shreekumar,Z. Berkay Celik*

Main category: cs.AI

TL;DR: 提出一个用于多AI代理系统的统一建模框架，包含主机代理模型和任务生命周期模型，通过17+14个形式化属性实现系统行为的严格验证。


<details>
  <summary>Details</summary>
Motivation: 当前代理间通信协议碎片化导致语义鸿沟，无法对系统属性进行严格分析，存在架构错位和可被利用的协调问题等风险。

Method: 引入两个基础模型：主机代理模型（负责与用户交互、任务分解和协调执行）和任务生命周期模型（详细描述子任务从创建到完成的状态转换）。

Result: 定义了17个主机代理属性和14个任务生命周期属性，分为活性、安全性、完整性和公平性类别，使用时序逻辑表达，支持形式化验证。
...
### [336] [A Multimodal Approach to Heritage Preservation in the Context of Climate Change](https://arxiv.org/abs/2510.14136)
...
<details>
  <summary>Details</summary>
Motivation: 文化遗产地因气候变化加速退化，传统单模态监测方法无法捕捉环境应力与材料劣化之间的复杂相互作用。

Method: 改进PerceiverIO架构，采用简化编码器（64D潜在空间）防止小数据集过拟合，并使用自适应Barlow Twins损失鼓励模态互补性。

Result: 在37个训练样本上，模型准确率达76.9%，比标准多模态架构提升43%，比传感器单独使用（61.5%）和图像单独使用（46.2%）均有显著改进。
...
### [337] [CodeEvolve: An open source evolutionary coding agent for algorithm discovery and optimization](https://arxiv.org/abs/2510.14150)
*Henrique Assumpção,Diego Ferreira,Leandro Campos,Fabricio Murai*

Main category: cs.AI

TL;DR: CodeEvolve是一个开源进化编码代理，结合大型语言模型和遗传算法解决复杂计算问题，在数学基准测试中超越了Google DeepMind的AlphaEvolve。
...
### [338] [Combining Reinforcement Learning and Behavior Trees for NPCs in Video Games with AMD Schola](https://arxiv.org/abs/2510.14154)
*Tian Liu,Alex Cann,Ian Colbert,Mehdi Saeedi*

Main category: cs.AI

TL;DR: 论文探讨了在商业视频游戏中应用强化学习（RL）的挑战，提出了将RL与传统行为树（BTs）结合的方法，并通过AMD Schola插件在Unreal Engine中展示了该方法的可行性。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习研究进展迅速，但在商业视频游戏中的应用仍然缓慢。论文旨在解决游戏AI社区在使用RL驱动NPC时面临的常见挑战，并探索RL与行为树的结合点。
...
Conclusion: RL与行为树的交集是一个值得进一步探索的关键节点，虽然之前有研究提出，但实际应用仍然罕见，本文展示了该方法的实际可行性。
...
### [339] [JEDA: Query-Free Clinical Order Search from Ambient Dialogues](https://arxiv.org/abs/2510.14169)
*Praphul Singh,Corey Barrett,Sumana Srivasta,Amitabh Saikia,Irfan Bulu,Sri Gadde,Krishnaram Kenthapadi*

Main category: cs.AI

TL;DR: JEDA是一个联合嵌入模型，可直接检索规范临床医嘱，并通过无查询模式编码滚动对话窗口来触发检索，实现实时临床医嘱链接。


<details>
  <summary>Details</summary>
Motivation: 临床对话混合了显性指令和隐性推理，现有LLM重写方法存在延迟、不稳定和不透明问题，阻碍实时医嘱处理。

Method: 基于PubMedBERT初始化，使用重复安全对比目标微调，通过受限LLM指导将签名医嘱与互补表述对齐，采用无查询模式减少噪声敏感性。

Result: JEDA在实践中取得显著提升，大幅优于基础编码器和近期开源嵌入模型，提供快速、可解释、无LLM的检索层。

Conclusion: JEDA实现了将环境上下文与可操作临床医嘱实时链接的快速、可解释检索解决方案。
...
### [340] [ARM-FM: Automated Reward Machines via Foundation Models for Compositional Reinforcement Learning](https://arxiv.org/abs/2510.14176)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习算法对奖励函数规范高度敏感，这限制了其广泛应用。需要一种自动化的方法来设计组合式奖励，减少人工设计奖励函数的负担。

Method: 使用基础模型从自然语言规范自动生成奖励机器；为每个自动机状态关联语言嵌入以实现跨任务泛化；利用奖励机器的结构化形式化实现有效任务分解。

Result: 在多个具有挑战性的环境中验证了ARM-FM的有效性，包括展示了零样本泛化能力。

Conclusion: ARM-FM通过结合基础模型和奖励机器，为强化学习的自动化奖励设计提供了一种有前景的方法，能够从自然语言规范生成有效的奖励函数。
...
### [341] [Implementation of AI in Precision Medicine](https://arxiv.org/abs/2510.14194)
*Göktuğ Bender,Samer Faraj,Anand Bhardwaj*

Main category: cs.AI

TL;DR: 对2019-2024年精准医学中AI实施文献的范围综述，识别关键障碍和促进因素，提出基于生态系统的框架支持可信赖和可持续实施
...
Method: 采用范围综述方法，分析2019-2024年文献，基于生态系统框架分析数据质量、临床可靠性、工作流整合和治理等维度

Result: 识别了精准医学AI实施的关键障碍和促进因素，强调了各要素间的相互依赖关系

Conclusion: 提出了支持可信赖和可持续实施AI的未来方向，强调需要生态系统视角来推动真实世界转化
...
### [342] [Echoes of Human Malice in Agents: Benchmarking LLMs for Multi-Turn Online Harassment Attacks](https://arxiv.org/abs/2510.14207)
*Trilok Padhi,Pinxian Lu,Abdulkadir Erol,Tanmay Sutar,Gauri Sharma,Mina Sonmez,Munmun De Choudhury,Ugur Kursuncu*

Main category: cs.AI

TL;DR: 提出了一个在线骚扰代理基准，包含多轮骚扰对话数据集、基于重复博弈论的多代理模拟、三种越狱攻击方法以及混合评估框架。研究发现越狱调优显著提高了骚扰成功率，降低了拒绝率，并揭示了封闭源和开源模型在骚扰升级轨迹上的差异。
...
Method: 构建了包含多轮骚扰对话数据集、基于重复博弈论的多代理模拟、针对记忆、规划和微调的三种越狱攻击方法，以及混合评估框架。使用LLaMA-3.1-8B-Instruct和Gemini-2.0-flash进行实验。

Result: 越狱调优使骚扰成功率从57.25-64.19%提升到95.78-96.89%(Llama)，从98.46%提升到99.33%(Gemini)，拒绝率降至1-2%。侮辱行为从44.2-50.8%增至84.9-87.8%，谩骂行为从31.5-38.8%增至81.2-85.1%。封闭源模型表现出显著脆弱性。

Conclusion: 多轮和理论基础的攻击不仅成功率高，还能模拟人类骚扰动态，强调了开发强大安全防护措施的必要性，以保护在线平台的安全和负责任性。
...
### [343] [LiveResearchBench: A Live Benchmark for User-Centric Deep Research in the Wild](https://arxiv.org/abs/2510.14240)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在评估深度研究系统时存在不足，往往聚焦于狭窄领域或提出模糊问题，无法公平比较。需要遵循用户中心、动态性、明确性和多面性搜索强度四个原则来构建更合适的评估框架。
...
Result: 使用LiveResearchBench和DeepEval对17个前沿深度研究系统进行了全面评估，包括单代理网络搜索、单代理深度研究和多代理系统，揭示了当前的优势、常见失败模式和推进可靠深度研究所需的关键系统组件。

Conclusion: LiveResearchBench和DeepEval为系统评估深度研究能力提供了严格基础，分析结果指出了当前系统的局限性和未来发展方向，对推进可靠、有洞察力的深度研究具有重要意义。
...
### [344] [Towards Agentic Self-Learning LLMs in Search Environment](https://arxiv.org/abs/2510.14253)
*Wangtao Sun,Xiang Cheng,Jialin Fan,Yao Xu,Xing Yu,Shizhu He,Jun Zhao,Kang Liu*

Main category: cs.AI

TL;DR: 本文提出了Agentic Self-Learning (ASL)框架，通过多角色协同进化的强化学习方法实现无需人工标注数据的智能体自我学习，发现奖励信号来源和任务数据规模是影响智能体训练可扩展性的关键因素。


<details>
  <summary>Details</summary>
Motivation: 研究如何在不依赖人工标注数据集或预定义规则奖励的情况下，通过自我学习扩展基于大语言模型的智能体能力，解决传统方法在开放领域学习中存在的局限性。

Method: 提出了ASL框架，包含提示生成器、策略模型和生成式奖励模型三个角色，在共享工具环境和LLM骨干网络中形成任务生成、策略执行和评估的闭环强化学习循环。

Result: ASL实现了稳定持续的轮次间性能提升，超越了会达到平台期或性能下降的强基线方法，在零标注数据条件下仍能持续改进，表现出优异的样本效率和鲁棒性。

Conclusion: 奖励来源和数据规模是开放领域智能体学习的关键杠杆，多角色协同进化方法能够实现可扩展的自我改进智能体，其中生成式奖励模型的验证能力是主要瓶颈。
...
### [345] [MorphoBench: A Benchmark with Difficulty Adaptive to Model Reasoning](https://arxiv.org/abs/2510.14265)
...
Method: 从现有基准和奥林匹克竞赛等来源收集复杂推理问题，利用模型推理过程中的关键陈述自适应修改分析挑战，使用仿真软件生成问题以动态调整难度。
...
### [346] [A Guardrail for Safety Preservation: When Safety-Sensitive Subspace Meets Harmful-Resistant Null-Space](https://arxiv.org/abs/2510.14301)
*Bingjie Zhang,Yibo Yang,Renzhe,Dandan Guo,Jindong Gu,Philip Torr,Bernard Ghanem*

Main category: cs.AI

TL;DR: GuardSpace是一个在微调过程中保护大语言模型安全对齐的框架，通过安全敏感子空间和抗有害空空间来防止安全行为退化。
...
Method: 使用协方差预条件奇异值分解将预训练权重分解为安全相关和安全无关组件，初始化低秩适配器时冻结安全相关部分；构建空空间投影器限制适配器更新对有害提示的安全输出。

Result: 在多个下游任务上的实验表明，GuardSpace优于现有方法。对于Llama-2-7B-Chat在GSM8K上的微调，有害评分从14.4%降至3.6%，准确率从26.0%提升至28.0%。

Conclusion: GuardSpace能够有效保护大语言模型在微调过程中的安全对齐，同时保持任务性能，是解决安全对齐脆弱性的有效方案。
...
### [347] [Terrarium: Revisiting the Blackboard for Multi-Agent Safety, Privacy, and Security Studies](https://arxiv.org/abs/2510.14312)
...
Method: 重新利用黑板设计创建模块化、可配置的多智能体协作测试床，识别关键攻击向量，并在三个协作场景中实施四种代表性攻击。

Result: 开发了Terrarium框架，提供快速原型设计、评估和迭代防御方案的工具，展示了框架的灵活性。

Conclusion: Terrarium旨在加速可信多智能体系统的进展，通过提供工具来快速原型化、评估和迭代防御方案。
...
### [348] [Metacognitive Self-Correction for Multi-Agent System via Prototype-Guided Next-Execution Reconstruction](https://arxiv.org/abs/2510.14319)
...
Method: MASC采用两种互补设计：1）下一执行重构，从查询和交互历史预测下一步的嵌入以捕捉因果一致性；2）原型引导增强，学习正常步骤嵌入的原型先验，在稀疏上下文下稳定重构和异常评分。检测到异常时触发校正智能体修正输出。

Result: 在Who&When基准测试中，MASC始终优于所有基线方法，步骤级错误检测AUC-ROC提升高达8.47%；集成到不同多智能体框架中，在各种架构上都能带来一致的端到端性能提升。
...
### [349] [AI for Service: Proactive Assistance with AI Glasses](https://arxiv.org/abs/2510.14359)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI服务大多是被动响应，而真正智能的助手应该能够预见用户需求并在适当时机主动采取行动。

Method: 基于冯·诺依曼计算机架构，在AI眼镜上构建包含输入单元、中央处理单元、算术逻辑单元、内存单元和输出单元的Alpha-Service框架，采用多智能体系统实现。

Result: 通过实时21点顾问、博物馆导览和购物搭配助手等案例研究，展示了系统能够无缝感知环境、推断用户意图并提供及时有用的帮助。

Conclusion: Alpha-Service框架为实现主动式AI助手提供了可行方案，能够在不需明确提示的情况下提供实时个性化服务。
...
### [350] [Can MLLMs Absorb Math Reasoning Abilities from LLMs as Free Lunch?](https://arxiv.org/abs/2510.14387)
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数研究专注于提升纯文本大语言模型(LLM)的数学推理能力，而多模态大语言模型(MLLM)的数学推理性能仍然落后。研究者希望探索MLLM能否直接从现成的数学LLM中吸收数学推理能力，而无需进行调优训练。
...
Conclusion: IP-Merging是一种有效的免调优方法，能够成功地将数学LLM的推理能力迁移到MLLM中，解决了参数空间对齐问题，显著提升了MLLM的数学推理性能。
...
### [351] [Hi-Agent: Hierarchical Vision-Language Agents for Mobile Device Control](https://arxiv.org/abs/2510.14388)
...
Method: 提出分层架构：高层推理模型和低层动作模型联合优化。将多步决策重新表述为单步子目标序列，并提出前瞻优势函数，利用低层模型的执行反馈指导高层优化，缓解GRPO在长时任务中的路径爆炸问题。

Result: 在Android-in-the-Wild基准测试中达到87.9%的任务成功率，显著优于基于提示的方法(AppAgent: 17.7%)、监督学习(Filtered BC: 54.5%)和强化学习(DigiRL: 71.9%)。在ScreenSpot-v2基准测试中表现出竞争力的零样本泛化能力。

Conclusion: Hi-Agent通过分层设计和联合优化策略，在移动设备控制任务中实现了最先进的性能，展示了在高复杂度场景中的强适应性和可扩展性。
...
### [352] [IMAGINE: Integrating Multi-Agent System into One Model for Complex Reasoning and Planning](https://arxiv.org/abs/2510.14406)
*Xikai Zhang,Bo Wang,Likang Xiao,Yongzhi Li,Quan Chen,Wenju Wu,Liu Liu*

Main category: cs.AI

TL;DR: 提出了IMAGINE框架，将多智能体系统的推理规划能力集成到单一紧凑模型中，显著超越MAS性能，在TravelPlanner基准上达到82.7%通过率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理和规划方面仍面临挑战，多智能体系统虽能改善集体推理但存在推理成本高、延迟长、端到端训练困难等问题

Method: IMAGINE框架通过端到端训练将多智能体系统的能力集成到单一模型中，使用Qwen3-8B-Instruct作为基础模型进行训练

Result: 训练后的模型在TravelPlanner基准上达到82.7%最终通过率，远超DeepSeek-R1-671B的40%，同时保持更小的模型规模

Conclusion: IMAGINE框架成功将多智能体系统的优势整合到单一模型中，在显著提升性能的同时降低了模型规模和推理成本
...
### [353] [Eliminating Negative Occurrences of Derived Predicates from PDDL Axioms](https://arxiv.org/abs/2510.14412)
...
<details>
  <summary>Details</summary>
Motivation: PDDL标准限制公理体中谓词的负出现只能针对由动作直接设置的谓词，而非由公理派生的谓词。但文献中常偏离此限制，仅要求公理集可分层。本文旨在证明这两种变体实际上表达能力相同。

Method: 提出了一种转换方法，通过消除派生谓词的负出现，将包含此类负出现的公理转换为符合PDDL标准限制的形式。

Result: 证明了包含派生谓词负出现的公理可以通过转换消除这些负出现，表明PDDL标准限制实际上不会减少表达能力。

Conclusion: PDDL标准对公理中派生谓词负出现的限制是多余的，因为通过转换可以消除这些负出现，两种变体都能表达与最小不动点逻辑相同的查询。
...
### [354] [Helmsman: Autonomous Synthesis of Federated Learning Systems via Multi-Agent Collaboration](https://arxiv.org/abs/2510.14512)
*Haoyuan Li,Mathias Funk,Aaqib Saeed*

Main category: cs.AI

TL;DR: Helmsman是一个多智能体系统，通过模拟研发流程自动化合成联邦学习系统，包括交互式规划、模块化代码生成和自主评估优化。


<details>
  <summary>Details</summary>
Motivation: 联邦学习系统设计复杂，需要处理数据异构性和系统约束等多方面挑战，导致解决方案脆弱且定制化。

Method: 采用三阶段协作方法：交互式人机规划制定研究计划，监督智能体团队进行模块化代码生成，在沙盒环境中进行自主评估和优化的闭环流程。

Result: 实验表明，该方法生成的解决方案与手工构建的基线方法竞争力相当，甚至更优。

Conclusion: 这项工作代表了向复杂分布式AI系统自动化工程迈出的重要一步。
...
### [355] [JSPLIT: A Taxonomy-based Solution for Prompt Bloating in Model Context Protocol](https://arxiv.org/abs/2510.14537)
*Emanuele Antonioni,Stefan Markovic,Anirudha Shankar,Jaime Bernardo,Lovro Markovic,Silvia Pareti,Benedetto Proietti*

Main category: cs.AI

TL;DR: JSPLIT是一个基于分类学的框架，用于解决AI代理在使用大量MCP工具时出现的提示膨胀问题，通过分层分类和工具选择算法显著减少提示大小，同时提高工具选择准确性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统发展，用户期望从简单的文本交互转向更复杂的代理系统，需要LLM与外部工具交互。MCP等标准使代理能访问工具，但随着工具数量增加，提示变得冗长，导致高token成本、延迟增加和任务成功率下降。

Method: JSPLIT将工具组织成层次化分类学，使用用户提示基于查询和分类结构识别并仅包含最相关工具，包括分类设计、工具选择算法和评估数据集。

Result: JSPLIT显著减少了提示大小，且未显著影响代理响应能力。当可用工具数量大幅增加时，JSPLIT甚至提高了代理的工具选择准确性，在高度复杂环境中有效降低成本同时提高任务成功率。

Conclusion: JSPLIT框架通过分类学驱动的方法有效管理提示大小，解决了MCP工具使用中的提示膨胀问题，在保持性能的同时降低成本，适用于复杂代理环境。
...
### [356] [Symbol Grounding in Neuro-Symbolic AI: A Gentle Introduction to Reasoning Shortcuts](https://arxiv.org/abs/2510.14538)
...
TL;DR: 神经符号AI（NeSy）结合神经网络和符号推理，但存在推理捷径（RSs）问题：模型可能通过错误地表示概念来获得高准确率，这会影响模型的可解释性和可靠性。本文概述了RSs的成因、后果、理论特征及应对方法。


<details>
  <summary>Details</summary>
Motivation: 神经符号AI旨在开发符合先验知识（如安全或结构约束）的深度神经网络，是实现可靠和可信AI的重要途径。然而，当概念未被直接监督时，NeSy模型可能受到推理捷径（RSs）的影响，即模型通过错误地表示概念来获得高准确率，这会影响模型的可解释性、在分布外场景中的性能以及可靠性。目前关于RSs的文献分散，使得研究人员和实践者难以理解和解决这一问题。

Method: 本文提供了对推理捷径（RSs）的温和介绍，直观地讨论了其成因和后果。回顾并阐明了现有对这一现象的理论特征。详细介绍了处理RSs的方法，包括缓解和意识策略，并分析了它们的优缺点。通过以易于理解的形式重新阐述高级材料，本文旨在提供关于RSs的统一视角，降低解决这一问题的门槛。

Result: 本文系统地概述了神经符号AI中的推理捷径问题，包括其定义、成因、影响以及现有的应对策略。通过整合分散的文献，为研究人员和实践者提供了理解和解决RSs问题的全面框架。

Conclusion: 推理捷径（RSs）是神经符号AI中一个关键但常被忽视的问题，它可能严重损害模型的可解释性和可靠性。本文通过提供统一的视角和实用的应对策略，旨在促进更可靠和可信的神经符号AI模型的发展。最终，希望这一概述能为开发可靠的NeSy和可信AI模型做出贡献。
...
### [357] [LLM Agents Beyond Utility: An Open-Ended Perspective](https://arxiv.org/abs/2510.14548)
*Asen Nachkov,Xi Wang,Luc Van Gool*

Main category: cs.AI

TL;DR: 本文研究如何将预训练LLM代理转化为能够自主生成任务、积累知识并追求模糊长期目标的开放式智能体，探讨了其潜力和当前局限。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理能力的增强，研究者希望探索其是否能成为具有自主规划、任务设计和模糊目标推理能力的独立实体，而不仅仅是问题解决工具。

Method: 采用开放式实验设置，增强预训练LLM代理的能力，使其能够生成自己的任务、积累知识并与环境进行广泛互动，然后对结果进行定性研究。

Result: 该代理能够可靠地遵循复杂多步骤指令、跨运行存储和重用信息、提出并解决自己的任务，但对提示设计敏感、容易重复生成任务、无法形成自我表征。

Conclusion: 研究展示了将预训练LLM转向开放式的潜力与当前限制，为未来训练代理管理记忆、进行高效探索和追求抽象长期目标指明了方向。
...
### [358] [ColorBench: Benchmarking Mobile Agents with Graph-Structured Framework for Complex Long-Horizon Tasks](https://arxiv.org/abs/2510.14621)
...
TL;DR: 提出了ColorBench图结构基准框架，用于评估移动代理在复杂长任务中的表现，支持多有效路径验证和准动态交互。


<details>
  <summary>Details</summary>
Motivation: 解决现有移动代理评估方法的不足：离线基准只能验证单一预设路径，而在线测试受限于真实设备的复杂性和不可重现性。

Method: 通过建模真实设备交互中的有限状态，实现动态行为的静态模拟，构建包含175个任务的ColorBench基准，支持多有效路径和错误路径分析。

Result: 评估发现现有模型存在局限性，提出了改进方向和可行技术路径来提升代理在复杂长任务中的性能。

Conclusion: ColorBench填补了离线与在线评估之间的差距，提供了稳定、全面的移动代理能力评估框架。
...
### [359] [Beyond Hallucinations: The Illusion of Understanding in Large Language Models](https://arxiv.org/abs/2510.14665)
*Rikard Rosenbacke,Carl Rosenbacke,Victor Rosenbacke,Martin McKee*

Main category: cs.AI

TL;DR: 该论文提出了Rose-Frame框架，用于诊断人机交互中的认知和认识论漂移，将AI对齐重新定义为认知治理问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然流畅且情感共鸣强，但基于统计预测而非有根据的推理，存在幻觉风险，需要解决其认知局限性。

Method: 引入三维框架Rose-Frame，包含地图vs领土（认识论vs本体论）、直觉vs理性（双过程理论）、冲突vs确认（批判性测试vs相互验证）三个维度。
...
### [360] [Machine Learning and Public Health: Identifying and Mitigating Algorithmic Bias through a Systematic Review](https://arxiv.org/abs/2510.14669)
*Sara Altamirano,Arjan Vreeken,Sennay Ghebreab*

Main category: cs.AI

TL;DR: 本文系统回顾了2021-2025年荷兰公共卫生机器学习研究中的算法偏见问题，开发了RABAT评估工具，发现普遍存在公平性框架缺失等问题，并提出了ACAR四阶段公平性框架。
...
### [361] [TITAN: Graph-Executable Reasoning for Cyber Threat Intelligence](https://arxiv.org/abs/2510.14670)
...
Method: 集成路径规划模型预测文本中的逻辑关系链，图执行器遍历TITAN本体图检索事实答案和证据，使用从MITRE衍生的类型化双向图进行推理。

Result: 创建了包含88209个示例的TITAN数据集，实证评估显示TITAN能生成语法有效且语义连贯的推理路径，可在底层图上确定性执行。

Conclusion: TITAN框架成功实现了自然语言威胁查询与结构化知识图谱推理的连接，为威胁情报分析提供了有效的自动化解决方案。
...
### [362] [NAEL: Non-Anthropocentric Ethical Logic](https://arxiv.org/abs/2510.14676)
*Bianca Maria Lerma,Rafael Peñaloza*

Main category: cs.AI

TL;DR: 提出了NAEL（非人类中心主义伦理逻辑）框架，结合主动推理和符号推理，为人工智能系统构建非人类中心的伦理行为模型。


<details>
  <summary>Details</summary>
Motivation: 传统AI伦理方法过于人类中心主义，需要开发不预设人类道德直觉的、能适应动态多智能体环境的伦理框架。

Method: 采用神经符号架构，通过最小化全局期望自由能量来形式化伦理行为，使智能体能在不确定环境中评估行为后果。

Result: 案例研究表明NAEL能动态平衡自我保存、认知学习和集体福利，实现上下文敏感、自适应和关系性的伦理行为。

Conclusion: NAEL为AI伦理提供了新的理论基础，能够克服现有伦理模型的局限性，发展出非人类中心主义的适应性伦理系统。
...
### [363] [Practical, Utilitarian Algorithm Configuration](https://arxiv.org/abs/2510.14683)
...
<details>
  <summary>Details</summary>
Motivation: COUP算法配置方法虽然具有理论保证，但实际性能不如启发式配置方法。本文旨在弥合这一差距，使基于效用的算法配置在实践中具有竞争力。

Method: 提出了一系列对COUP的改进措施，这些改进在不降低理论保证的前提下提升了算法的实证性能，并通过实验验证了这些改进的效果。

Result: 改进后的COUP算法配置方法在实证性能上达到了与广泛使用的启发式配置方法相当的水平，同时保持了理论性能保证。

Conclusion: 通过本文的改进，基于效用的算法配置方法COUP现在既具有理论保证，又在实际性能上具有竞争力，为算法配置提供了更可靠的选择。
...
### [364] [Purifying Task Vectors in Knowledge-Aware Subspace for Model Merging](https://arxiv.org/abs/2510.14697)
*Bang An,Yibo Yang,Philip Torr,Bernard Ghanem*

Main category: cs.AI

TL;DR: 提出了PAVE方法，通过在知识感知子空间中净化任务向量，去除任务向量中的冗余成分，从而提升模型融合性能。


<details>
  <summary>Details</summary>
Motivation: 现有的模型融合方法使用任务向量作为基础构建块，但合并模型常因任务向量中的任务无关冗余导致性能下降。现有方法随机丢弃参数缺乏知识感知。

Method: 通过从每个任务采样训练样本，获取微调模型线性层前的协方差矩阵，进行上下文导向的奇异值分解，识别最相关的权重成分，在知识感知子空间中将微调模型权重分为任务相关和冗余组件，修剪冗余组件净化任务向量。

Result: PAVE作为即插即用方案适用于各种基于任务向量的融合方法，在多种融合方法、任务和模型架构上验证了有效性。

Conclusion: PAVE方法能够有效净化任务向量中的冗余成分，显著提升模型融合性能，且具有广泛的适用性。
...
### [365] [Cognitive-Aligned Spatio-Temporal Large Language Models For Next Point-of-Interest Prediction](https://arxiv.org/abs/2510.14702)
*Penglong Zhai,Jie Li,Fanyi Di,Yue Liu,Yifang Yuan,Jie Huang,Peng Wu,Sicong Wang,Mingyang Yin,Tingting Hu,Yao Xu,Xin Li*

Main category: cs.AI

TL;DR: CoAST框架通过两阶段方法解决大语言模型在POI推荐中缺乏地理实体理解和时空模式认知的问题：第一阶段通过持续预训练获取推荐知识，第二阶段通过监督微调和强化学习实现认知对齐。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型主要基于非结构化文本预训练，缺乏对结构化地理实体和序列移动模式的理解，且工业级POI预测需要融入世界知识和人类认知对齐（如季节、天气、节假日、用户画像等）来提升推荐性能。

Method: CoAST框架包含两个主要阶段：(1) 推荐知识获取：在脱敏用户的丰富时空轨迹数据上进行持续预训练；(2) 认知对齐：通过监督微调和强化学习，使用丰富训练数据将认知判断与人类偏好对齐。

Result: 在多个真实世界数据集上的离线实验和在AMAP App首页"猜你想去"功能中的在线实验证明了CoAST的有效性。

Conclusion: CoAST框架通过自然语言接口成功整合了世界知识、时空轨迹模式、用户画像和情境信息，有效提升了POI推荐的性能。
...
### [366] [ToolPRM: Fine-Grained Inference Scaling of Structured Outputs for Function Calling](https://arxiv.org/abs/2510.14703)
...
Method: 构建了首个细粒度的调用内过程监督数据集，使用函数掩码技术自动标注步骤级奖励，训练ToolPRM过程奖励模型来评分单个函数调用的内部步骤。
...
### [367] [SimKO: Simple Pass@K Policy Optimization](https://arxiv.org/abs/2510.14807)
*Ruotian Peng,Yi Ren,Zhouliang Yu,Weiyang Liu,Yandong Wen*

Main category: cs.AI

TL;DR: 本文提出SimKO方法解决RLVR中的过度集中问题，通过不对称设计提升探索能力，改善pass@K性能
...
Method: 提出SimKO方法：对验证正确的响应提升top-K候选概率，对验证错误的响应对top-1候选施加更强惩罚，特别在高熵token上应用

Result: 在多个数学和逻辑推理基准测试中，SimKO持续提高各种K值的pass@K性能

Conclusion: SimKO通过缓解概率过度集中问题，有效提升RLVR的探索能力，为改进RLVR提供简单有效的方法
...
### [368] [Agentic NL2SQL to Reduce Computational Costs](https://arxiv.org/abs/2510.14808)
*Dominik Jehle,Lennart Purucker,Frank Hutter*

Main category: cs.AI

TL;DR: Datalake Agent是一个代理系统，通过交互式循环选择性请求必要信息，大幅减少NL2SQL任务中的token使用量，降低成本同时保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 传统NL2SQL方法需要处理大量数据库元信息，导致提示过长、token使用量高、处理成本昂贵。

Method: 采用代理系统设计，使用交互式循环和推理框架，让LLM选择性请求必要信息，而非一次性提供所有元信息。

Result: 在23个数据库和100个表问答任务上的评估显示，token使用量减少高达87%，成本显著降低，同时保持竞争力。

Conclusion: Datalake Agent通过选择性信息请求策略，有效解决了NL2SQL任务中的高成本问题，实现了效率与性能的平衡。
...
### [369] [RoboGPT-R1: Enhancing Robot Planning with Reinforcement Learning](https://arxiv.org/abs/2510.14828)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型和视觉语言模型在长视野操作任务中面临常识和推理能力受限的问题，监督微调存在泛化性差和物理理解不足的挑战。

Method: 两阶段微调框架：监督训练获取基础知识，强化学习解决视觉空间理解和推理不足；设计基于规则的奖励函数同时考虑长视野性能和动作约束。

Result: 在Qwen2.5-VL-3B上训练的推理模型，在EmbodiedBench基准上比GPT-4o-mini提升21.33%，比其他基于Qwen2.5-VL-7B的工作提升20.33%。

Conclusion: RoboGPT-R1框架通过结合监督训练和强化学习，有效提升了具身智能体的物理理解和多步推理能力，在复杂真实环境任务中表现出色。
...
### [370] [Boosting Instruction Following at Scale](https://arxiv.org/abs/2510.14842)
...
Method: 提出Instruction Boosting后生成方法，并引入SCALEDIF基准测试，包含最多10条指令的数据样本，同时开发定量冲突评分工具分析指令间冲突。

Result: Instruction Boosting将指令遵循率提升高达7个百分点（2条指令）和4个百分点（10条指令），发现性能下降与指令间冲突程度相关。

Conclusion: Instruction Boosting有效提高LLM指令遵循可靠性，指令冲突是性能下降的重要因素，冲突评分工具可为开发者提供反馈。
...
### [371] [Where to Search: Measure the Prior-Structured Search Space of LLM Agents](https://arxiv.org/abs/2510.14846)
*Zhuo-Yang Song*

Main category: cs.AI

TL;DR: 提出了一种紧凑的形式理论来描述和测量基于领域先验指导的LLM辅助迭代搜索，通过模糊关系算子表示智能体，引入覆盖生成函数来衡量可达性难度，为LLM构建的迭代搜索提供系统化形式描述。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的生成-过滤-精炼迭代范式在推理、编程和科学发现方面取得进展，但搜索效果依赖于如何将领域先验编码为操作化的结构化假设空间。

Method: 将智能体表示为输入输出的模糊关系算子，通过安全包络约束智能体；使用单一延续参数加权所有可达路径，求和得到覆盖生成函数；提供可测试的推断并通过多数投票实例进行验证。

Result: 该理论提供了可操作的语言和工具来测量智能体及其搜索空间，为LLM构建的迭代搜索提供了系统化的形式描述框架。

Conclusion: 提出的形式理论为LLM辅助的迭代搜索提供了工作语言和操作工具，能够系统性地描述和测量智能体在领域先验指导下的搜索过程。
...
### [372] [LabOS: The AI-XR Co-Scientist That Sees and Works With Humans](https://arxiv.org/abs/2510.14861)
...
TL;DR: LabOS是首个将计算推理与物理实验相结合的AI共同科学家，通过多模态感知、自进化代理和XR技术支持的人机协作，将实验室转变为智能协作环境。
...
Result: 在癌症免疫治疗靶点发现和干细胞工程等应用中，LabOS展示了AI能够超越计算设计，直接参与实验过程。
...
### [373] [The Gatekeeper Knows Enough](https://arxiv.org/abs/2510.14881)
*Fikresilase Wondmeneh Abebayew*

Main category: cs.AI

TL;DR: 提出了Gatekeeper Protocol框架，通过低保真度潜在状态表示和按需请求高保真度上下文的方法，解决LLM代理在复杂知识系统中因上下文窗口限制和状态不同步导致的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: LLM作为自主代理部署时，受限于有限的上下文窗口和状态不同步问题，导致与代码库、文档等大型结构化知识系统交互时输出不可靠、行为不可预测和资源使用效率低下。

Method: 设计Gatekeeper Protocol框架，要求代理先在低保真度的潜在状态表示上进行操作和推理，然后按需策略性地请求高保真度上下文。所有交互通过统一的JSON格式进行中介，作为声明式、状态同步的协议。
...
### [374] [Mapping Smarter, Not Harder: A Test-Time Reinforcement Learning Agent That Improves Without Labels or Model Updates](https://arxiv.org/abs/2510.14900)
...
<details>
  <summary>Details</summary>
Motivation: 企业智能平台需要整合来自众多第三方供应商的日志，但供应商文档在测试时往往不可用、不匹配、格式混乱或不完整，这给模式映射带来了挑战。

Method: 使用强化学习代理，在推理过程中：1)识别模糊的字段映射尝试；2)生成针对性网络搜索查询收集外部证据；3)应用基于置信度的奖励来迭代优化映射。

Result: 在将Microsoft Defender for Endpoint日志转换为通用模式的任务中，该方法将映射准确率从56.4%(仅LLM)提升到72.73%(RAG)，最终达到93.94%(100次迭代后)，同时将需要专家审查的低置信度映射减少了85%。

Conclusion: 这种新方法提供了一种证据驱动、透明的解决方案，为更稳健、可问责、可扩展、高效、灵活、适应性强和协作的解决方案铺平了道路。
...
### [375] [Budget-aware Test-time Scaling via Discriminative Verification](https://arxiv.org/abs/2510.14913)
...
<details>
  <summary>Details</summary>
Motivation: 现有生成式验证器虽然性能优秀但计算成本过高，限制了实际应用，需要更高效的验证策略。

Method: 采用判别式验证器与自一致性结合的混合方法，在固定计算预算下进行测试时扩展。

Result: 在AIME2025上获得高达15.3%的准确率提升，显著优于最先进的生成式验证方法。
...
### [376] [TRI-DEP: A Trimodal Comparative Study for Depression Detection Using Speech, Text, and EEG](https://arxiv.org/abs/2510.14922)
*Annisaa Fitri Nurfidausi,Eleonora Mancini,Paolo Torroni*

Main category: cs.AI

TL;DR: 该论文系统探索了使用EEG、语音和文本进行多模态抑郁症检测的方法，比较了手工特征与预训练嵌入、不同神经网络编码器、单模态/双模态/三模态配置，以及融合策略。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症自动检测研究范围有限，缺乏系统性的特征比较和一致的评估协议，需要填补这些空白。

Method: 使用EEG、语音和文本三种模态，系统评估手工特征与预训练嵌入、不同神经网络编码器、单模态/双模态/三模态配置，以及融合策略，采用一致的受试者独立分割进行稳健评估。

Result: 结果显示：(i) EEG、语音和文本的组合增强了多模态检测；(ii) 预训练嵌入优于手工特征；(iii) 精心设计的三模态模型达到了最先进的性能。
...
### [377] [Stable but Miscalibrated: A Kantian View on Overconfidence from Filters to Large Language Models](https://arxiv.org/abs/2510.14925)
*Akira Okutomi*

Main category: cs.AI

TL;DR: 将康德的《纯粹理性批判》重新解释为反馈稳定性理论，提出复合不稳定性指数(H-Risk)来衡量推理系统的稳定性，发现在线性高斯模拟和LLMs中，更高的H-Risk预测过度自信错误和幻觉问题。
...
Result: 在线性高斯模拟中，更高的H-Risk预测过度自信错误；在LLMs中，脆弱的内部动态与校准错误和幻觉相关，而批判式提示对校准和幻觉的影响不一致。
...
### [378] [GroundedPRM: Tree-Guided and Fidelity-Aware Process Reward Modeling for Step-Level Reasoning](https://arxiv.org/abs/2510.14942)
*Yao Zhang,Yu Wu,Haowei Zhang,Weiguo Li,Haokun Chen,Jingpei Wu,Guohao Li,Zhen Han,Volker Tresp*

Main category: cs.AI

TL;DR: 提出了GroundedPRM框架，通过蒙特卡洛树搜索构建结构化推理路径，使用外部工具验证中间步骤，结合步骤级验证和全局结果评估，仅用10%的数据就实现了26%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型面临三大挑战：噪声奖励、低事实保真度以及与步骤级推理目标不对齐，主要由于缺乏可扩展的高质量标注，现有方法依赖昂贵的人工标注或存在幻觉问题的LLM自评估。

Method: 使用蒙特卡洛树搜索构建结构化推理路径以减少奖励噪声，通过外部工具验证中间步骤消除幻觉监督，设计混合奖励聚合机制融合工具验证和MCTS反馈，采用理由增强的生成结构提高可解释性。

Result: 仅用4万个自动标注样本（最佳PRM所需数据的10%），在ProcessBench上实现高达26%的相对性能提升，在奖励引导的贪婪搜索中甚至优于使用人工标注监督训练的PRM。
...
### [379] [Towards Unified Multimodal Misinformation Detection in Social Media: A Benchmark Dataset and Baseline](https://arxiv.org/abs/2509.25991)
*Haiyang Li,Yaxiong Wang,Shengeng Tang,Lianwei Wu,Lechao Cheng,Zhun Zhong*

Main category: cs.AI

TL;DR: 提出了统一的多模态虚假内容检测框架UMFDet，能够同时处理人工制作的虚假信息和AI生成内容，解决了现有方法只能处理单一类型虚假内容的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多模态虚假内容类型未知，而现有方法通常只针对单一类型（人工虚假信息或AI生成内容），限制了实际应用效果。

Method: 使用VLM骨干网络，结合类别感知的MoE适配器捕获类别特定线索，以及归因链式思维机制提供隐式推理指导来定位关键欺骗信号。

Result: 在包含12.7万样本的OmniFake数据集上，UMFDet在两种虚假内容类型上都表现出稳健一致的性能，优于专用基线方法。
