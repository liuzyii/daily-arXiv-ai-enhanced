...
Motivation: 遥感数据量日益增长，自动信息提取对于解决社会、经济和环境挑战至关重要，但高维数据带来的稀疏性、低效率和维度灾难限制了机器学习模型的有效性。

Method: 采用维度约简技术，特别是特征提取方法，在保持数据关键特性的同时降低复杂度，提升数据压缩、清洗、融合、可视化、异常检测和预测等任务的效果。

Result: 为遥感数据价值链中的维度约简应用提供了系统指导，识别了尚未充分探索的维度约简算法及其在未来研究中的应用机会。

Conclusion: 维度约简技术是解决遥感高维数据挑战的关键方法，本综述为相关研究提供了实用指南，并指出了未来研究方向。
...
Method: 使用端到端的深度隐写方法，联合训练编码器网络将任意图像转换为Ninja Codes，并训练检测模块进行识别。

Result: 实验表明Ninja Codes在常见室内光照条件下能提供可靠的位置跟踪，同时成功隐藏在多样环境纹理中。

Conclusion: Ninja Codes在需要隐蔽跟踪的场景中具有重要价值，可以使用普通彩色打印机打印，并通过现代RGB摄像头检测。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶场景中高级感知、预测和规划问题的准确回答需求，利用预训练视觉语言模型但需要更好的提示工程和上下文信息。

Method: 两阶段方法：第一阶段使用大型多模态LLM（Qwen2.5-VL-32B）处理六摄像头输入、历史时序信息和思维链提示；第二阶段加入场景元数据和任务特定指令。使用自一致性集成提高可靠性。

Result: 在驾驶QA基准测试中显著优于基线模型：第一阶段65.1%准确率（零样本为62.61%），自一致性提升至66.85%，第二阶段达67.37%。在严重视觉干扰下仍保持96%准确率。

Conclusion: 精心设计的提示工程和上下文基础可以大幅提升预训练视觉语言模型在高级驾驶问答任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 纵向序列放射影像分析面临数据挑战：如何有效建模高分辨率图像序列在非均匀时间间隔下的捕获。现有方法要么压缩空间信息为向量，要么使用计算效率低且不兼容非均匀时间步的时空模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散模型在生成复杂运动时往往缺乏时间一致性和物理合理性，主要原因是模型对自然视频中复杂运动的理解不足。现有方法通过将扩散模型特征与预训练视频编码器特征对齐来解决此问题，但这些编码器的特征混合了视频外观和动态信息，限制了对齐效果。

Method: 1. 从预训练视频编码器中学习解耦的运动子空间，该子空间经过优化以预测真实光流，确保捕捉真实的运动动态。2. 将文本到视频扩散模型的潜在特征与这个新的运动子空间对齐，使生成模型能够内化运动知识。

Result: 该方法在VideoPhy、VideoPhy2、VBench和VBench-2.0等基准测试上进行了实证评估，并进行了用户研究。结果表明，该方法显著提升了最先进视频扩散模型的物理常识性，同时保持了与文本提示的契合度。

Conclusion: 提出的运动中心对齐框架有效解决了文本到视频扩散模型在运动生成方面的局限性，通过解耦运动特征并与扩散模型对齐，显著提升了生成视频的时间一致性和物理合理性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的评估指标（如CIDEr、SPICE）是为短文本设计的，无法有效评估长文本的详细图像描述，特别是对属性和关系附件的敏感性不足。

Method: PoSh使用场景图作为结构化评分标准，指导大型语言模型作为评判者，产生基于细粒度错误的聚合分数。同时构建了DOCENT数据集，包含艺术品、专家撰写的参考描述和模型生成的描述，并由艺术史学生进行质量评估。

Result: PoSh在DOCENT数据集上与人类评判的相关性比最佳开源替代方案强0.05 Spearman ρ，对图像类型具有鲁棒性，并且能够作为有效的奖励函数，优于标准监督微调。

Conclusion: PoSh是一个可复制、可解释且比现有指标更好地代理人类评判者的评估指标。通过PoSh和DOCENT，可以推动辅助文本生成等重要领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏对多种人体姿态表示（图像、2D关键点、3D骨架等）之间相关性的系统性研究，特别是在对比学习范式下。

Method: 提出UniHPR统一人体姿态表示学习框架，使用基于奇异值的对比学习损失来同时对齐多种模态数据表示。

Result: 在Human3.6M数据集上MPJPE达到49.9mm，在3DPW数据集上PA-MPJPE达到51.6mm；在Human3.6M数据集上实现2D和3D姿态检索，检索误差为9.24mm MPJPE。

Conclusion: UniHPR通过统一的多模态对齐框架有效提升了人体姿态表示的质量，在姿态估计和检索任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 标准U-Net模型在脑肿瘤分割中面临挑战，特别是在处理不规则形状和模糊边界时准确度不足，同时高分辨率MRI数据训练需要大量计算资源且存在类别不平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌是女性主要死因之一，需要早期检测和准确诊断。医疗数据的敏感性使得开发准确且保护隐私的AI模型具有挑战性。联邦学习虽然能保护隐私，但在非独立同分布数据上训练会影响模型准确性和泛化能力。

Method: 采用联邦近端算法(FedProx)处理非独立同分布的超声乳腺癌图像数据集，并整合改进的U-Net模型和注意力机制来增强肿瘤分割准确性。

Result: 该方法训练出的全局模型达到了96%的准确率，有效提高了肿瘤分割精度同时保护了患者隐私。

Conclusion: FedProx在非独立同分布的本地医疗数据集上训练精确机器学习模型方面具有潜力，是解决医疗数据隐私和模型性能平衡问题的有前景方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频理解研究主要依赖第三人称视角，忽视了同步的、以自我为中心的多智能体学习特性，需要开发能够从个体视角理解团队战术的方法。

Method: 构建X-Ego-CS数据集包含124小时专业比赛的第一人称视角视频流，并提出跨视角对比学习(CECL)方法，通过对齐队友的第一人称视觉流来培养团队战术意识。

Result: 在队友-对手位置预测任务中，CECL方法有效提升了智能体从单一第一人称视角推断队友和对手位置的能力，使用最先进的视频编码器取得了良好效果。

Conclusion: X-Ego-CS和CECL为电子竞技中的跨视角多智能体基准测试奠定了基础，将游戏理解定位为多智能体建模和战术学习的测试平台，对时空推理和人机协作具有广泛意义。
...
Result: 在多个数据集上，FootFormer在足底压力分布、足部接触图和质心估计方面显著优于或等同于现有方法，在经典运动学指标（压力中心、质心、支撑面）估计方面达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 疟疾主要通过雌性按蚊传播，常导致0-5岁儿童死亡。传统显微镜观察血涂片图像的方法依赖人工操作，容易误诊，因此需要计算机辅助的自动诊断系统。
...
Conclusion: 这些发现高度支持深度学习驱动的方法在疟疾自动可靠检测中的现实可行性，同时最大限度地减少了直接人工参与。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的幻觉问题严重，现有解决方案需要额外计算成本。研究发现幻觉与视觉token注意力不足相关，冗余视觉token分散了模型注意力。

Method: 提出PruneHal方法，利用自适应KV缓存修剪增强模型对关键视觉信息的关注，无需额外训练，几乎不增加推理成本。

Result: 在多个主流MLLM和幻觉评估基准上测试，取得了稳健且优异的结果，证明了方法的有效性和优越性。

Conclusion: PruneHal是首个将token修剪应用于MLLM幻觉缓解的方法，具有模型无关性，可与不同解码策略无缝集成，无需训练且成本极低。
...
<details>
  <summary>Details</summary>
Motivation: 传统的基于奖励的微调方法主要关注整体视频质量，但在图像到视频生成任务中时序一致性经常受到影响，需要专门针对时序一致性的改进方法。

Method: 提出VCD指标，在视频帧特征的频域空间中定义，通过频域分析有效捕捉帧间信息，并基于奖励微调框架对模型进行微调。

Result: 在多个I2V数据集上的实验结果表明，使用VCD微调的视频生成模型显著增强了时序一致性，且不降低其他性能指标。

Conclusion: VCD是一种有效的时序一致性增强方法，能够在不牺牲其他质量指标的前提下，显著改善图像到视频生成的时间连贯性。
...
<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界模型主要关注生成质量和可控性指标，但忽视了对于自动驾驶性能至关重要的下游感知任务的评估。现有方法需要先在合成数据上预训练再在真实数据上微调，导致训练周期加倍，当基线方法也加倍训练周期时，合成数据的优势变得不明显。

Method: Dream4Drive首先将输入视频分解为多个3D感知引导图，然后将3D资产渲染到这些引导图上，最后微调驾驶世界模型生成编辑后的多视角逼真视频，用于训练下游感知模型。

Result: 该框架能够大规模生成多视角角案例，显著提升自动驾驶中的角案例感知能力。作者还贡献了DriveObj3D大规模3D资产数据集，涵盖驾驶场景中的典型类别。

Conclusion: 综合实验表明，Dream4Drive在不同训练周期下都能有效提升下游感知模型的性能，为自动驾驶感知任务提供了有效的合成数据增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态场景重建方法在不同场景下表现不一致，没有单一方法能有效处理所有动态挑战，需要更统一的框架来提升渲染质量和适应性。

Method: 提出MoE-GS框架，集成多个专门专家，通过体积感知像素路由器将体积高斯级权重投影到像素空间进行自适应融合；同时探索单次多专家渲染、门控感知高斯剪枝和蒸馏策略来提高效率。

Result: 在N3V和Technicolor数据集上的广泛实验表明，MoE-GS在保持改进效率的同时，持续优于最先进方法。

Conclusion: MoE-GS是首个将混合专家技术融入动态高斯泼溅的方法，通过专家融合和效率优化策略，在动态场景重建中实现了更好的渲染质量和适应性。
...
Method: 通过从图像和雷达数据估计物体的二次表面参数，使用显式表面拟合模型增强空间表示和跨模态交互。预测的深度用于：1）在图像分支中引导特征从透视视图转换到统一的鸟瞰视图；2）在表面伪点分支中生成密集伪点云。两个点云分支采用基于柱体的方法，最终使用标准2D骨干网络和检测头进行目标检测。

Result: 在TJ4DRadSet和view-of-delft (VoD)目标检测基准测试中，SFGFusion有效融合了相机和4D雷达特征，实现了优越的性能。

Conclusion: SFGFusion通过表面拟合引导的方法，成功解决了4D雷达点云稀疏性问题，实现了相机和4D雷达的有效融合，在3D目标检测任务中表现出色。
...
Method: 使用小波变换增强单帧图像中的高频特征并抑制背景噪声；提出基于匈牙利算法的多帧时序轨迹补全方案进行全局最优跨帧匹配；后处理流程包括时序匹配插值补全、基于时序一致性的噪声滤波和渐进式轨迹优化。

Result: 在公开SpotGEO数据集上的实验结果表明，该方法有效，F1分数达到90.14%。

Conclusion: 该方法通过小波变换特征增强和多帧时序轨迹优化，成功解决了GEO空间目标的检测难题，具有较高的检测精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法受限于车载网络带宽，只能传输前景区域特征而丢弃包含重要上下文的背景信息，这限制了感知性能。

Method: 采用课程学习策略，早期利用背景线索但逐步剪除，迫使模型将上下文信息内化到前景表示中，而不需要传输背景本身。

Result: 在模拟和真实世界基准测试中，FadeLead在不同带宽设置下均优于先前方法。

Conclusion: 通过上下文丰富的前景共享，FadeLead证明了在带宽受限环境中有效利用背景上下文的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 随着神经场、几何与运动深度学习以及3D生成AI的快速发展，4D生成与重建领域迅速发展。论文旨在从4D表示的独特视角，为研究者提供选择合适表示方法的指导。

Method: 采用选择性方法，重点分析代表性工作，而非详尽列举。将4D表示分为几何、运动和交互三个支柱进行组织，涵盖NeRF、3D高斯泼溅等流行方法以及结构化模型、长程运动等较少探索的表示。

Result: 提供了4D表示方法的系统性分类和分析框架，强调了不同表示方法在不同计算、应用和数据场景下的优缺点。讨论了LLM和视频基础模型在4D应用中的作用及局限性。

Conclusion: 论文为4D生成与重建领域提供了基于表示方法的独特视角，帮助研究者根据具体任务需求选择和定制合适的4D表示方法，并指出了当前数据集不足的问题和未来发展方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决真实图像超分辨率中复杂退化问题和重建模糊性，在保持生成模型感知质量的同时平衡计算成本，避免单步扩散模型因蒸馏伪影导致的结构不准确问题。

Method: 使用ControlNet机制进行语义边缘引导，为单步推理提供动态结构控制；采用混合损失函数结合L2、LPIPS和边缘感知AME损失，优化像素精度、感知质量和几何精度。

Result: 实验表明该方法有效提升了结构完整性和真实感，同时保持了单步生成的高效性，在输出质量和推理速度之间实现了优越的平衡。

Conclusion: 提出的框架成功解决了单步扩散模型在超分辨率任务中的结构不准确问题，实现了感知质量与计算效率的良好平衡。
...
Conclusion: MobiAct框架实现了低能耗、低计算的MAV动作识别，在保持高精度的同时显著提升了推理效率。
...
Result: 在多个基准测试中显著提升对象计数准确率（例如在D2D-Small基准上提升13.7%），同时图像质量和计算开销几乎没有下降。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像标注数据稀缺、疾病复杂性和数据隐私限制等挑战，提高阿尔茨海默病检测的准确性。

Method: 使用原型网络框架，集成多个预训练CNN作为编码器，结合类感知损失和熵损失进行精确分类。

Result: 在Kaggle阿尔茨海默数据集上达到99.72%准确率，在ADNI数据集上达到99.86%准确率，优于现有最先进方法。

Conclusion: 该方法在阿尔茨海默病早期检测中具有有效性和实际应用潜力，能够处理标注数据稀缺的问题。
...
Method: 利用计算机视觉技术，包括动作识别、预测和活动理解，通过分析程序结构和执行偏差来检测错误。

Result: 建立了基于程序结构、监督水平和学习策略的分类框架，并识别了现有数据集、评估指标和先进方法。

Conclusion: 该研究为基于视觉的程序性活动错误分析提供了统一视角，指出了神经符号推理和反事实状态建模等未来方向。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)虽然取得了显著进展，但其大规模特性使得在资源受限环境中部署不切实际，需要开发高效的训练方法来创建轻量级但强大的VLMs。

Method: RIL算法结合强化学习和对抗模仿学习，使用基于LLM的判别器区分学生和教师模型输出，并利用多个大型教师VLM提供多样化指导，通过统一学习策略提升学生模型的生成能力。

Result: 在多个视觉语言基准测试上的广泛实验表明，RIL显著缩小了与最先进开源和闭源VLMs的性能差距，并在多个实例中超越了它们。

Conclusion: RIL算法通过统一强化与模仿学习，能够有效训练轻量级视觉语言模型，使其在保持小规模的同时达到与大型模型竞争的性能水平。
...
Method: TS-GATR将签名表示为图结构，节点捕获动态特征，使用注意力机制建模复杂关系。采用双图注意力变换器模块分别建模局部和全局空间特征，并集成GRU捕获长期时间依赖关系。
...
Method: 采用双分支编码器分别处理水深估计和像素级海底分类，通过注意力特征融合模块和Swin-Transformer融合块整合跨任务特征，并使用动态任务不确定性加权平衡目标。

Result: 在两个异质沿海站点的评估中，相比传统经验模型和机器学习方法，RMSE降低达75%；与最先进的单任务和多任务基线相比，水深RMSE降低10-30%，海底分类精度提升达8%。

Conclusion: 联合建模水深与海底底质和栖息地能产生协同增益，为集成浅水测绘提供了鲁棒的开放解决方案。
...
Method: 系统研究尺度偏移对性能的影响，建立ScaleBench基准测试20种先进DG算法，提出Causal Feature Decomposition and Anisotropic Processing (Catto)算法来缓解尺度偏移影响。

Result: 实验表明现有算法存在局限性，尺度偏移问题复杂且重要，Catto算法能有效缓解尺度偏移的影响。

Conclusion: 尺度偏移领域泛化是一个新颖且实用的研究方向，需要进一步深入研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将fMRI映射到CLIP的最终语义层，忽略了CLIP中间层的丰富物体信息，且与大脑功能层次结构不符。许多方法添加参数密集的VAE通路来捕捉细节，但效率低下。

Method: 基于人类视觉系统功能层次结构，将不同视觉区域（低/高层次）的fMRI信号对齐到对应的CLIP中间层和最终层，引入跨重建策略和多粒度损失函数。

Result: 在高级语义指标上达到或超越SOTA方法性能，同时参数数量减少71.7%，有效平衡语义准确性和细节保真度。

Conclusion: 通过利用CLIP中间特征，BrainMCLIP无需VAE通路即可实现竞争性性能，在参数效率和性能间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统图像分割方法需要大量标注数据且无法识别未见类别的问题，开发一个无需训练、能够处理任意类别词汇的开集识别框架。

Method: 采用两阶段流水线：1) 使用EfficientNetB0提取像素特征，通过SVD分解和层次聚类进行无监督分割；2) 使用CLIP的ViT编码分割区域，与文本嵌入在共享潜在空间中进行跨模态对齐和识别。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在处理多个移动手机任务时性能有限，传统多任务监督微调方法难以确定最优训练数据组合。

Method: 使用可训练网络预测下游任务性能，从而确定最优数据混合配置，并引入PhoneAgentBench基准进行综合评估。

Result: DaMo在PhoneAgentBench上提升3.38%，在其他基准上平均提升2.57%，在BFCL-v3任务上提升12.47%，且具有良好的可扩展性。

Conclusion: DaMo能有效优化多任务学习的数据混合策略，显著提升移动手机代理的性能，并具有良好的泛化能力和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法忽视正则化在确保鲁棒性和解剖合理性中的关键作用，需要平衡配准精度与物理合理性

Method: 基于变形场梯度范数动态调整弹性正则化，整合应变和剪切能量项，包含折叠预防机制惩罚负变形雅可比区域

Result: 减轻非物理伪影如折叠，避免过度平滑，提高配准精度和解剖合理性

Conclusion: DARE框架在医学图像配准中实现了稳定性和灵活性的平衡，确保物理合理的变换
...
Method: 提出AegisRF框架，包含扰动场和敏感度场。扰动场在NeRF预渲染输出中注入对抗性扰动，敏感度场学习空间变化的几何扰动敏感度来约束扰动，保持渲染质量。

Result: 实验证明AegisRF在多种下游任务和模态中具有通用适用性，包括多视图图像分类和基于体素的3D定位，同时保持高视觉保真度。

Conclusion: AegisRF能有效保护NeRF知识产权，在保持渲染质量的同时干扰未经授权的使用。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数VLM评估集中在单视角设置，而多摄像头设置已成为机器人平台标准配置，但VLM能否有效利用多视角输入进行机器人推理仍是一个开放问题。

Method: 构建MV-RoboBench基准测试，包含空间理解和机器人执行两个主要类别，评估包括开源和闭源模型在内的多种VLM，并采用CoT启发技术增强版本。

Result: 最先进模型的表现远低于人类水平，揭示了VLM在多视角机器人感知中面临的重大挑战。空间智能与机器人任务执行在多视角场景中呈正相关，但现有通用单视角空间理解基准的强表现不能可靠转化为机器人空间任务的成功。
...
<details>
  <summary>Details</summary>
Motivation: 随着电子商务的普及，物流仓库运营效率提升变得至关重要。数字孪生技术需要准确收集工人在仓库中的位置，但单摄像头视野有限，需要多摄像头系统。

Method: 在仓库天花板上安装19个广角摄像头俯视地面，基于地面进行坐标对齐。针对广角摄像头边缘畸变问题，通过检测工人脚部位置进行对齐，减少图像畸变影响。

Result: 确认跟踪精度提高了20%以上，并比较了多种外观特征利用方法，验证了所提方法的有效性。

Conclusion: 基于脚部位置对齐的方法能有效减少广角摄像头畸变影响，在多摄像头系统中实现准确的工人位置追踪，为物流仓库数字孪生系统提供了可行的技术方案。
...
Method: 将包含多个实例的绘图分解为语义子图，构建层次表示；分析各层次子图提取心理洞察；引入HTP知识库和特征提取模块生成心理档案；整合多层面信息进行综合评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决地下表征中孔隙尺度图像代表性不足的问题，因为自然空间异质性导致提取的子图像与岩心测量值显著偏离，且物理样本仅在稀疏井位可用。

Method: 使用多条件生成对抗网络框架，在碳酸盐岩地层的四个深度薄片样本上训练，同时以孔隙度值和深度参数为条件，捕捉通用孔隙网络原理和深度特定地质特征。

Result: 模型在所有地层中实现了出色的孔隙度控制(R^2=0.95)，平均绝对误差为0.0099-0.0197。生成的图像表现出优越的代表性，双约束误差为1.9-11.3%，而随机提取的真实子图像误差为36.4-578%。
...
<details>
  <summary>Details</summary>
Motivation: MRI采集时间过长限制了临床效率，传统图像重建方法面临挑战，需要创新方法来加速MRI扫描过程。

Method: 使用生成模型预测目标对比度图像作为数据驱动先验，条件输入包括其他对比度图像、历史扫描图像、采集参数和患者信息，然后基于该先验重建高度欠采样的k空间数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频方法仅处理单个序列，未能利用跨序列中存在的强结构规律性和重复运动模式，限制了3D姿态估计的性能。

Method: PRGCN包含图记忆库存储姿态原型，通过注意力机制检索，结合Mamba状态空间模型的自注意力的双流混合架构提取时空特征。
...
Method: 评估了多种插补方法，并提出了在训练期间使用加权重采样方案，通过在每个覆盖区间强制类别平衡来消除标签与覆盖率之间的关联。
...
TL;DR: 提出Cauvis方法解决单源域泛化目标检测中的伪相关性问题，通过交叉注意力提示和双分支适配器分离因果-伪特征，在SDGOD数据集上取得15.9-31.4%的性能提升。
...
Conclusion: Cauvis方法有效解决了单源域泛化目标检测中的伪相关问题，实现了优异的泛化性能和鲁棒性。
...
Method: 使用紧凑VLM（350M）提取特征并预测目标预训练VLM响应何时收敛到其正确回答能力的峰值，训练为离散分类器但在推理时插值连续分辨率以进行细粒度控制。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理小重叠或无重叠图像对时表现不佳，视频插值生成的中间帧模糊且选择策略效率低，需要更清晰中间帧和与位姿估计对齐的选择方法。

Method: 使用混合视频生成(HVG)结合视频插值和位姿条件新视角合成模型生成清晰中间帧，提出基于特征匹配的选择器(FMS)选择适合位姿估计的中间帧。
...
TL;DR: 本文分析了视觉语言模型在计数任务中的局限性，发现模型对物体数量、类型、空间布局和干扰物高度敏感，通过仅微调输出层可将准确率提升高达21%。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在下游任务中表现良好，但在视觉推理方面仍存在限制，特别是在识别关系、理解时间序列和计数对象方面。本文旨在深入探究这些失败的根本原因并针对性提升其推理能力。

Method: 在受控实验条件下研究了7个最先进视觉语言模型的计数能力，进行分层分析，并针对性地仅微调输出层进行训练。

Result: 实验显示视觉语言模型对物体数量、类型、空间布局和干扰物高度敏感，错误主要源于最后一层表示到输出空间的映射不正确。仅微调输出层可将准确率提升高达21%，并在真实数据集上获得一致改进。
...
TL;DR: 本文研究了文本到图像(T2I)模型中提示复杂度对合成数据效用的影响，发现增加提示复杂度会降低条件多样性和提示一致性，但减少合成与真实数据分布差异。


<details>
  <summary>Details</summary>
Motivation: 虽然提示工程是T2I模型的主要交互方式，但提示复杂度对合成数据质量、多样性和一致性等关键效用维度的系统性影响尚未充分探索。

Method: 首先进行合成实验验证提示复杂度泛化难度，提出理论推导解释观察到的困难；然后引入新评估框架比较真实数据和合成数据效用，分析提示复杂度对常用T2I模型生成合成数据的影响。

Result: 增加提示复杂度导致条件多样性和提示一致性降低，但减少合成与真实数据分布差异；当前推理时干预方法可以增加生成多样性但会超出真实数据支持范围；提示扩展方法在图像多样性和美学方面表现最佳。

Conclusion: 提示复杂度对T2I模型生成合成数据的效用有显著影响，需要平衡复杂度和数据质量，提示扩展方法在保持多样性和美学方面表现优异。
...
TL;DR: 本文研究了大规模视觉语言模型的时间感知能力，提出了TIME10k基准数据集，并发现时间信息在VLM嵌入空间中沿着低维非线性流形结构存在。基于这一发现，作者提出了从嵌入空间提取显式时间线表示的方法。


<details>
  <summary>Details</summary>
Motivation: 研究大规模视觉语言模型（如CLIP）是否具备时间感知能力，即能否将视觉内容定位到时间维度上。

Method: 构建了TIME10k基准数据集（包含10,000多张带时间标签的图像），评估了37个VLM的时间感知能力，并提出从VLM嵌入空间中提取时间线表示的新方法。

Result: 发现时间信息在VLM嵌入空间中沿着低维非线性流形结构存在。提出的时间线方法在时间推理任务中达到了与基于提示的基线方法相当或更优的准确率，同时计算效率更高。

Conclusion: 视觉语言模型确实具备时间感知能力，时间信息在其嵌入空间中具有结构化特征。提出的时间线表示方法能够有效建模时间及其时序进展，为时间推理任务提供了新思路。
...
Conclusion: HAD成功解决了RGB和事件相机之间的时空不对称问题，为多模态目标跟踪提供了有效的解决方案。
...
TL;DR: α-Cloak是首个在无盒设置下通过RGBA视频的alpha通道对目标检测器进行对抗攻击的方法，利用alpha通道融合恶意和良性视频，在人类看来无害但能持续欺骗目标检测器。


<details>
  <summary>Details</summary>
Motivation: 随着目标检测模型在自动驾驶车辆等系统中的部署增加，确保其安全性至关重要。现有研究主要关注图像域攻击，视频域攻击特别是无盒设置下的攻击研究不足。

Method: 利用RGBA视频的alpha通道将恶意目标视频与良性视频融合，设计融合算法确保视觉隐蔽性和兼容性，无需访问模型架构、参数或输出。

Result: 在5个最先进的目标检测器、一个视觉语言模型和一个多模态大语言模型上测试，攻击成功率100%，且不引入可感知的伪影。

Conclusion: 揭示了视频感知系统中一个未被探索的漏洞，强调了在对抗环境中考虑alpha通道的防御需求的紧迫性。
...
TL;DR: VGD是一个前馈端到端学习框架，通过显式学习几何信息并利用几何特征提升语义质量，解决环视自动驾驶场景重建中的泛化性和新视角质量问题。


<details>
  <summary>Details</summary>
Motivation: 环视自动驾驶场景重建面临泛化性和新视角质量提升的核心挑战，现有方法在最小重叠区域的环视设置下难以保证几何一致性和重建质量。

Method: 设计轻量级VGGT变体从预训练VGGT中提取几何先验，构建高斯头融合多尺度几何token预测高斯参数，并整合多尺度特征通过特征一致性学习优化渲染质量。
...
TL;DR: 提出了一种新的多模态协同学习框架，能够泛化到各种任务而不针对特定推理模态，通过结合对比学习和模态判别学习来指导单模态模型构建模态共享和模态特定的内部模型流形。


<details>
  <summary>Details</summary>
Motivation: 地球观测领域产生大量多模态数据，但在实际应用中训练和推理阶段获取相同传感器模态变得越来越困难。多模态协同学习可以利用训练阶段的大量传感器数据来改进推理时部署的单模态模型。

Method: 结合对比学习和模态判别学习，引导单模态模型将内部模型流形结构化为模态共享和模态特定信息，实现跨任务泛化而不针对特定推理模态。

Result: 在四个地球观测基准测试（涵盖分类和回归任务）上评估，相比最先进的机器学习和计算机视觉方法以及EO特定方法，实现了持续的预测改进。

Conclusion: 该框架在单模态推理场景下，在多样化的地球观测应用中得到了验证，证明了其有效性。
...
TL;DR: 提出了VAEEDOF方法，使用蒸馏变分自编码器进行多焦点图像融合，解决了光学镜头景深限制问题，并创建了新的合成数据集MattingMFIF。


<details>
  <summary>Details</summary>
Motivation: 解决多焦点图像融合中存在的训练数据有限、合成数据集与真实场景存在域差距、以及信息缺失区域处理困难等挑战。

Method: 使用蒸馏变分自编码器进行高保真图像重建，融合模块可同时处理多达7张图像，并创建了MattingMFIF合成4K数据集模拟真实景深效果。
...
Result: 通过实验评估了不确定性度量在识别预测错误和噪声污染图像区域方面的实际效用，基于不同数据集特性进行了对比分析。

Conclusion: 基于研究结果提出了实用的建议，为遥感应用中语义分割不确定性估计提供了指导。
...
TL;DR: 提出一个全自动模块化框架，将扫描或拍摄的心电图图像转换为数字信号，在多个数据集上验证优于现有技术，并开源软件以促进心电图档案的数字化和AI诊断的普及。
...
Method: 开发全自动模块化框架，处理扫描或拍摄的心电图图像，包括处理常见伪影、透视畸变、褶皱和污渍等问题。

Result: 在37,191张心电图图像上验证，在Akershus大学医院的1,596张图像上获得19.65 dB的平均信噪比，在Emory数据集的所有子类别中都优于现有技术。

Conclusion: 该框架成功将纸质心电图转换为数字信号，开源软件将有助于解锁回顾性心电图档案并普及AI驱动的诊断。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解方面表现强劲，但其原始注意力图噪声大且与物体区域对齐差，需要一种无需训练的方法来精炼这些注意力图以实现定位。

Method: 提出分解注意力融合（DecAF），包含对比性物体-背景融合和互补性视频-帧融合两种机制，结合注意力引导的SAM2提示来获得细粒度掩码。

Result: DecAF在无需训练的方法中表现优异，在指代和推理VOS基准测试中达到与训练方法相当的性能。

Conclusion: 该方法证明了无需重新训练MLLMs，仅通过注意力图精炼就能实现有效的视频分割，为训练自由的视频定位提供了可行方案。
...
TL;DR: 提出CBDiff模型用于图像伪造定位，通过生成多个可能的伪造区域分布图来增强预测可信度，在多个数据集上优于现有方法。
...
Method: 提出条件伯努利扩散模型(CBDiff)，在扩散过程中引入伯努利噪声以更好反映伪造掩码的二元稀疏特性，并设计时间步交叉注意力机制(TSCAttention)利用语义特征指导改进检测。

Result: 在8个公开基准数据集上的广泛实验表明，CBDiff显著优于现有最先进方法。

Conclusion: CBDiff通过生成多个多样化的定位图来更好地表示伪造分布，解决了篡改区域固有的不确定性和变异性，具有实际部署的强潜力。
...
TL;DR: 本文提出了首个系统性基准XBench，用于评估胸片中的跨模态可解释性，发现当前视觉语言模型在医学图像中的定位能力仍有不足，特别是对于小病灶或弥散性病变。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学图像理解方面表现出色，但其在医学领域中的定位能力（文本概念与视觉证据的对齐程度）尚未充分探索，而可靠的定位对于可解释性和临床应用至关重要。

Method: 使用交叉注意力和基于相似性的定位图生成视觉解释，并在多个病理学上定量评估其与放射科医生标注区域的对应关系，评估了七种CLIP风格的视觉语言模型变体。

Result: 发现所有模型变体对于大而明确的病变表现出合理的定位能力，但对于小或弥散性病变性能显著下降；在胸片特定数据集上预训练的模型比通用领域训练的模型具有更好的对齐效果；模型的整体识别能力与定位能力强相关。

Conclusion: 当前视觉语言模型尽管具有很强的识别能力，但在临床可靠的定位方面仍有不足，强调了在医学实践中部署前需要进行有针对性的可解释性基准测试。
...
TL;DR: 本文提出了一种基于散射系数的去噪估计器，通过联合最小化和最大化不同散射系数子集的ℓ¹范数来抑制高斯噪声，达到了卡通图像在所有Lipschitz指数α≤2下的渐近极小极大界。


<details>
  <summary>Details</summary>
Motivation: 传统的基于帧的稀疏估计器（如小波、曲波等）无法充分适应复杂信号的正则性，特别是在卡通图像边缘为分段C^α曲线且α≤2为未知参数时表现不佳。深度卷积神经网络虽然取得了更好的数值结果，但需要更简化的模型来建立谐波分析与去噪估计器之间的数学桥梁。

Method: 使用散射系数作为简化的卷积神经网络模型，通过对小波系数模进行第二次小波变换计算散射系数。通过联合最小化和最大化不同散射系数子集的ℓ¹范数来构建去噪估计器。

Result: 数值实验表明，该去噪估计器在所有Lipschitz指数α≤2的情况下都能达到卡通图像的渐近极小极大界。这些ℓ¹范数能够捕捉不同类型的几何图像正则性。

Conclusion: 该方法为抑制信号噪声和指定函数几何正则性提供了不同的谐波分析方法，并在谐波分析与深度卷积网络去噪估计器之间建立了数学桥梁。数值结果被表述为一个数学猜想。
...
TL;DR: 提出GenComm生成式通信机制，通过特征生成实现异构多智能体系统的无缝感知，无需修改原有网络，通过轻量级空间信息数值对齐以最小成本集成新智能体。


<details>
  <summary>Details</summary>
Motivation: 现实应用中异构智能体在传感器和模型上的差异导致协作时的领域差距，现有基于适应和重构的方法存在两个关键限制：侵入式重训练破坏语义一致性，集成新智能体计算成本高限制可扩展性。

Method: 设计可变形消息提取器提取空间消息，使用条件扩散模型的空间感知特征生成器生成与自我智能体语义空间对齐的特征，通过通道增强器优化特征后进行融合。

Result: 在OPV2V-H、DAIR-V2X和V2X-Real数据集上的实验表明，GenComm优于现有最先进方法，集成新智能体时计算成本和参数数量减少81%。

Conclusion: GenComm通过生成式通信机制有效解决了异构多智能体协作中的领域差距问题，实现了高效可扩展的感知协作。
...
TL;DR: 提出AMR框架解决时刻检索的三个瓶颈：数据稀缺、边界模糊和语义区分不足，通过零外部依赖的增强训练和两阶段学习策略提升性能


<details>
  <summary>Details</summary>
Motivation: 解决现有时刻检索方法面临的三个关键瓶颈：数据稀缺导致模型陷入浅层关键词特征关联、相邻事件间边界模糊、以及细粒度语义区分能力不足

Method: 提出AMR框架，包含两阶段训练：冷启动阶段使用课程学习在增强数据上建立基础边界/语义感知；蒸馏阶段引入双查询集（原始查询和动态查询），通过跨阶段蒸馏损失保持知识一致性

Result: 在多个基准测试中，AMR相比之前的最先进方法实现了性能提升

Conclusion: AMR框架通过零外部依赖的增强训练和两阶段学习策略，有效解决了时刻检索中的数据稀缺、边界模糊和语义区分问题，显著提升了模型性能
...
TL;DR: ReAct-GS通过重要性感知的致密化准则和参数扰动机制，解决了3D高斯溅射中的梯度稀释和基元冻结问题，有效消除过度重建伪影，在保持几何细节的同时实现最先进的视图合成性能。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在复杂场景中会出现过度重建伪影（局部模糊和针状扭曲），现有方法归因于大尺度高斯分布分裂不足，但本文发现两个根本限制：致密化过程中的梯度幅度稀释和基元冻结现象。

Method: 提出ReAct-GS方法，基于重新激活原则：1）重要性感知致密化准则，结合多视角的α混合权重重新激活复杂区域停滞的基元增长；2）重新激活机制，通过自适应参数扰动激活冻结的基元。

Result: 在多样化真实世界数据集上的综合实验表明，ReAct-GS有效消除过度重建伪影，在标准新视图合成指标上达到最先进性能，同时保持精细几何细节。重新激活机制在其他3D-GS变体中也带来一致改进。

Conclusion: ReAct-GS通过重新激活机制成功解决了3D高斯溅射中的基元冻结和梯度稀释问题，在复杂场景中实现了更好的重建质量，且该方法具有广泛的适用性。
...
TL;DR: 提出了Policy World Model (PWM)新范式，将世界建模与轨迹规划统一，通过无动作未来状态预测方案利用学习的世界知识提升规划性能。
...
Method: 提出PWM统一架构，通过协作状态-动作预测实现类人预期感知，引入动态增强并行令牌生成机制、上下文引导分词器和自适应动态焦点损失。

Result: 仅使用前视摄像头输入，性能匹配或超越依赖多视角和多模态输入的最先进方法。

Conclusion: PWM成功整合世界建模与规划，通过知识共享机制显著提升自动驾驶系统的规划可靠性。
...
TL;DR: 提出CST框架，在无配对图像翻译中显式保护细长曲线结构，通过集成结构一致性训练，可无缝整合到现有方法中


<details>
  <summary>Details</summary>
Motivation: 现有无配对图像翻译方法会扭曲细长曲线结构（如微血管），影响诊断可靠性和定量分析，这在眼科和血管成像中尤为关键

Method: CST框架通过添加曲线提取模块提供拓扑监督，可集成到CycleGAN和UNSB等现有方法中，在训练中融入结构一致性

Result: 在OCT血管成像、彩色眼底和X射线冠脉造影三种成像模态上的评估表明，CST提高了翻译保真度并达到最先进性能

Conclusion: 通过增强学习映射中的几何完整性，CST为医学成像中曲线结构感知的跨域翻译建立了原则性途径
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习活体检测系统多为黑盒模型，决策过程不透明。需要解释性技术来理解系统为何将生物特征图像判断为真实或伪造。

Method: 提出Ensemble-CAM技术，为基于深度学习的面部活体检测系统提供视觉解释，识别导致判断的关键区域。

Result: 通过视觉解释技术，能够更好地理解深度学习活体检测系统的决策过程，确定影响判断的关键图像区域。

Conclusion: Ensemble-CAM技术能够提高基于深度学习的面部活体检测系统的透明度和可信度，帮助用户理解模型行为。
...
<details>
  <summary>Details</summary>
Motivation: 从高维视频中提取真实动态变量具有挑战性，因为存在背景运动、遮挡和纹理变化等干扰因素。需要开发能够抑制干扰变化、学习鲁棒潜在表示的框架。

Method: 第一阶段使用时序变换器自编码器，利用全局注意力关注动态相关区域；第二阶段通过线性相关分析选择物理意义维度，并使用Lyapunov稳定性正则化器精炼转移动态。

Result: 在五个合成基准和四个真实世界动态系统上的实验表明，LyTimeT在互信息和内在维度估计方面最接近真实值，对背景扰动保持不变性，在CNN和变换器基线中具有最低的分析均方误差。
...
TL;DR: 提出ADQ自适应分布感知量化框架，解决激活值分布高度不均匀和权重量化码本不匹配的问题，通过自适应权重量化方案和硬件友好的非均匀到均匀映射，在ImageNet上仅用2.81位平均位宽实现71.512%的Top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 现有量化感知训练方法面临两个主要挑战：激活值的高度不均匀分布和权重量化中使用的静态、不匹配码本，这限制了在资源受限设备上的部署效果。

Method: 提出自适应分布感知量化(ADQ)框架，包含三个关键创新：1)基于分位数的码本初始化方法；2)基于指数移动平均的在线码本自适应机制；3)灵敏度感知的混合精度分配策略。对于激活值，集成硬件友好的非均匀到均匀映射方案。

Result: 在ImageNet上，ADQ使ResNet-18仅用2.81位平均位宽就达到71.512%的Top-1准确率，优于同类条件下的最先进方法。在CIFAR-10上的消融研究验证了各创新组件的独立贡献。

Conclusion: ADQ框架通过自适应权重量化和硬件友好的激活值处理，有效解决了量化中的分布不均匀和码本不匹配问题，在保持高精度的同时显著降低了模型位宽。
...
Method: 提出使用参考运动作为条件信号，采用渐进式弱到强混合条件训练策略处理多模态冲突，构建了最大的统一多模态运动数据集OmniMoCap-X。

Result: 实验证明OmniMotion-X在多个多模态任务上显著超越现有方法，实现了最先进的性能，能够生成逼真、连贯且可控的长时运动。

Conclusion: OmniMotion-X提供了一个统一的多模态运动生成框架，通过创新的条件信号和训练策略，在多任务场景下实现了优越的性能。
...
Method: 使用类别感知原型缓存模块动态调整每类容量，包含激活历史记录和复苏机制；负对比学习机制识别和约束困难负样本；采用非对称优化，仅优化文本原型。

Result: 在15个基准测试中，CPL-NC在ResNet-50和ViT-B/16骨干网络上均优于现有测试时适应方法。

Conclusion: CPL-NC是轻量级测试时适应框架，能有效提升视觉语言模型在分布偏移下的泛化能力。
...
TL;DR: Pico-Banana-400K是一个包含40万张图像的大规模指令图像编辑数据集，通过系统化质量控制和多样化编辑类型构建，支持单轮和多轮编辑研究。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型的文本引导图像编辑能力受限，缺乏大规模、高质量、开放的真实图像数据集，阻碍了研究进展。

Method: 利用Nano-Banana从OpenImages真实照片生成多样化编辑对，采用细粒度图像编辑分类法确保全面覆盖，通过MLLM质量评分和精心筛选保证内容保持和指令忠实度。

Result: 构建了包含三个专业子集的综合数据集：7.2万示例的多轮编辑集、5.6万示例的偏好子集，以及长短指令配对集，为复杂编辑场景研究提供资源。

Conclusion: Pico-Banana-400K为训练和评估下一代文本引导图像编辑模型建立了坚实基础，推动了该领域的研究发展。
...
TL;DR: 本文分析了单目深度估计的评估指标问题，发现现有指标对曲率扰动严重不敏感，提出了基于相对表面法线的新指标、新的深度可视化工具和构建更好人类对齐复合指标的原理性方法。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计任务进展迅速，但如何评估仍是一个开放问题，现有文献缺乏标准化，大量评估指标的权衡和行为未被充分理解。

Method: 对现有指标进行定量分析，评估它们对地面真实值各种扰动的敏感性，特别强调与人类判断的比较。引入基于相对表面法线的新指标、新的深度可视化工具和构建复合指标的原理性方法。

Result: 分析显示现有指标对曲率扰动（如使平坦表面波浪化）严重不敏感。新提出的指标和工具能够更好地与人类判断对齐。

Conclusion: 提出了改进单目深度估计评估的新指标和方法，解决了现有指标对曲率扰动不敏感的问题，提供了更好的评估标准化方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发更强大的OCR系统，将数字化打印文档（如PDF）转换为干净、自然排序的纯文本，特别提升在复杂布局下的性能。

Method: 使用强化学习与可验证奖励（RLVR）训练专门的7B视觉语言模型，通过生成具有多样化布局的合成文档来创建单元测试，并以此作为奖励信号。

Result: 在olmOCR-Bench英文OCR基准测试中达到最先进性能，在数学公式转换、表格解析和多列布局方面相比之前版本有最大改进。

Conclusion: olmOCR 2通过强化学习和合成数据训练，显著提升了OCR在复杂文档布局下的性能，模型、数据和代码均已开源发布。
...
TL;DR: ITTO是一个新的点跟踪基准测试套件，从现有数据集和真实世界第一人称视角录制中收集视频，通过多阶段流程获得高质量人工标注，捕捉真实场景的运动复杂性、遮挡模式和物体多样性。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试缺乏真实世界场景的运动复杂性、遮挡模式和物体多样性，需要一个新的基准来评估和诊断点跟踪方法的能力和局限性。

Method: 从现有数据集和真实世界第一人称视角录制中收集视频，通过多阶段流程获得高质量人工标注，构建包含真实场景复杂性的基准测试套件。

Result: 对最先进的跟踪方法在ITTO上进行严格分析，发现现有跟踪器在这些挑战上表现不佳，特别是在遮挡后重新识别点方面，揭示了关键失败模式。

Conclusion: ITTO作为点跟踪的基础测试平台，指出了需要针对真实世界动态开发新建模方法的必要性，将指导开发更鲁棒的跟踪算法。
...
TL;DR: 提出了一种联合实体识别与消歧的统一框架，利用大语言模型丰富实体上下文，在领域外数据集上达到最优性能
...
Method: 使用微调模型统一整合实体识别和消歧，并利用大语言模型增强实体提及的上下文信息

Result: 在基准数据集上评估，与多个基线方法比较，在领域外数据集上取得了最优性能

Conclusion: 联合实体识别与消歧的统一框架结合大语言模型能够有效提升实体链接性能，特别是在领域外数据集上表现优异
...
TL;DR: 开发了基于小型语言模型(MiniLMs)的框架，用于从地球科学文献中精确、快速、经济地检索信息，相比大型语言模型具有更高的效率和专业性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学文献处理中存在信息偏见和计算成本高的问题，需要开发更精确、经济高效的替代方案。

Method: 构建了包含约7700万高质量句子的地球科学文献语料库，使用MiniLMs通过语义搜索和句子级索引进行高效信息提取，并结合情感分析和无监督聚类分析。

Result: MiniLMs能够有效提取经过专家验证的领域特定信息，特别擅长处理定量研究结果，并能追踪科学结论演变、研究重点和新兴问题。

Conclusion: MiniLM在地球科学社区具有广泛应用潜力，包括事实检索、趋势分析、矛盾分析和教育用途。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模基准测试需要大量计算资源，且新模型可能在现有基准上训练导致记忆性能而非真实能力评估。需要新颖的评估方法来诊断特定的指令遵循模式。

Method: 开发了一个流线型评估框架，使用20个精心设计的提示评估LLM指令遵循能力，涵盖格式合规、内容约束、逻辑排序和多步骤任务执行等方面。在256个验证可用的模型上进行大规模实证研究。

Result: 研究揭示了模型在指令遵循方面的一致失败模式，识别出特定指令类型对模型构成特别挑战，提供了主要提供商和新兴实现模型的比较性能分析。

Conclusion: 这项工作既贡献了一个实用的评估工具，也提供了当代LLM生态系统中指令遵循能力最全面的实证分析之一，为研究人员和从业者提供了实用的诊断工具。
...
TL;DR: 该论文提出通过微调编码器专用的小型语言模型（SLMs）来检测机器生成内容，相比大型语言模型（LLMs）在准确性和效率上都有显著提升。
...
Result: 提出的编码器在AUROC指标上达到0.97-0.99，宏F1分数达到0.89-0.94，同时延迟降低8-12倍，峰值VRAM使用减少3-5倍。在跨生成器偏移和对抗性变换下，性能保持≥92%的清洁AUROC。
...
TL;DR: TM-Rephrase框架使用LLMs将原始推文重写为更正式的语言，显著提升了COVID-19相关推文主题建模的效果，特别是使用口语转正式策略时LDA算法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 社交媒体短文本的简短性、非正式性和噪声限制了传统主题建模的效果，产生难以解释的冗余主题。

Method: 开发了TM-Rephrase框架，利用LLMs对原始推文进行重写，研究两种重写策略（通用重写和口语转正式重写）对多种主题建模方法的影响。

Result: TM-Rephrase提高了主题连贯性、独特性和多样性三个指标，减少了大多数主题建模算法的主题冗余，口语转正式策略对LDA算法效果提升最显著。

Conclusion: 该研究为公共卫生相关社交媒体分析提供了一种模型无关的主题建模增强方法，对改善健康危机及其他重要领域的公共话语理解具有广泛意义。
...
TL;DR: 提出了ODiS算法，通过主成分分析将多维度评分解耦为正交特征维度，在保持数据质量的同时确保多样性，显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分的预训练数据选择方法存在维度相关性偏差，导致选择的数据虽然评分高但缺乏多样性，影响模型性能。需要将相关指标分解为正交特征维度。

Method: 1. 从语言质量、知识质量和理解难度等多维度评估数据；2. 使用PCA对多维度评分进行解相关，得到正交评估维度；3. 为每个维度训练Roberta评分器；4. 在每个正交维度中选择评分最高的数据构建训练集。
...
Method: 采用动态可逆剪枝框架，检测上下文感知的神经元激活，应用自适应掩码来调节生成过程中的神经元影响，实现推理时干预。

Result: 在多语言单轮和多轮对话中实现了细粒度、内存感知的偏见缓解，同时保持了知识完整性和行为一致性。

Conclusion: 该方法为现实世界对话AI提供了动态公平性控制，能够在推理时灵活适应不同对话上下文，比静态方法更具适应性。
...
TL;DR: 提出MMAOBench基准，用于评估多模态大语言模型在单模态和全模态理解能力，包含1880个人工标注样本和44种任务类型，揭示了跨模态与单模态性能之间的组合规律。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型正从单模态理解向视觉、音频和语言模态的统一发展，但单模态与全模态之间的相关性尚不清楚，需要全面评估来推动全模态模型的智能进化。

Method: 提出MMAOBench基准，包含1880个人工标注样本，涵盖44种任务类型，并引入创新的多步骤开放式问题类型来更好评估复杂推理任务。

Result: 实验结果显示跨模态与单模态性能之间存在组合规律，全模态能力在弱模型上表现为瓶颈效应，而在强模型上表现出协同促进作用。

Conclusion: MMAOBench基准有效评估了单模态和全模态理解能力，揭示了全模态能力在不同模型上的不同表现模式，为多模态模型的发展提供了重要评估工具。
...
TL;DR: 提出一种基于transformer预训练语言模型的可解释性虚假信息检测流程，使用RoBERTa和DistilBERT模型，通过两阶段微调策略和可解释性方法，在保持性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 在线平台虚假信息的快速传播破坏信任并阻碍知情决策，需要开发高效且可解释的检测方法。

Method: 使用transformer预训练语言模型，采用两阶段微调策略：先冻结主干网络训练分类头，然后逐步解冻主干层并应用分层学习率衰减。集成LIME进行token级解释和SHAP进行全局特征归因。
...
TL;DR: 提出了一种通过分析叙事中的符号化角色网络来评估LLM故事生成能力的新方法，发现LLM生成的故事倾向于紧密团结的积极关系
...
Method: 通过符号化角色网络分析叙事中的社会结构，对四个领先LLM生成的1200多个故事进行大规模比较分析

Result: LLM生成的故事在密度、聚类和符号边权重等网络属性上表现出强烈偏向紧密团结的积极关系，与先前人工评估研究结果一致
...
<details>
  <summary>Details</summary>
Motivation: 现有的代理搜索框架在处理长轨迹时面临上下文限制问题，包括积累冗长嘈杂内容、超出上下文窗口和工具预算、或过早停止搜索。

Method: 提出SLIM框架，将检索分离为独立的搜索和浏览工具，并定期总结轨迹，保持上下文简洁的同时支持更长、更专注的搜索。

Result: 在长视野任务中，SLIM以显著更低的成本和更少的工具调用实现了与强开源基线相当的性能。使用o3作为基础模型时，在BrowseComp上达到56%，在HLE上达到31%，分别比所有开源框架高出8和4个绝对百分点，同时工具调用减少4-6倍。

Conclusion: SLIM框架在长轨迹代理搜索中表现出更少的幻觉，作者还发布了自动细粒度轨迹分析流程和错误分类法，为未来长视野代理系统提供参考。
...
TL;DR: ProfBench是一个包含7000多个专家评估响应的基准测试，用于评估LLM在处理专业文档、信息综合和生成报告方面的能力，覆盖物理、化学、金融和咨询领域。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估主要局限于数学、编程等易于验证的任务，但现实应用需要评估LLM处理专业文档和生成综合报告的能力。

Method: 构建ProfBench数据集，包含人类专家评估的响应-标准对，并开发经济高效的LLM-Judges来评估，通过减少自我增强偏见将评估成本降低2-3个数量级。

Result: ProfBench对最先进的LLM构成显著挑战，GPT-5-high仅达到65.9%的总体性能，发现专有模型和开源模型之间存在明显性能差距。
...
Method: 开发动态生成模型特定挑战数据集的框架，捕捉新兴防御模式并与每个模型的独特行为对齐，构建OVERBENCH基准。

Result: 创建了包含45万个样本、覆盖25个模型的OVERBENCH基准，提供了对过度敏感性的动态演进视角。

Conclusion: OVERBENCH能够持续监测模型防御触发器，突出静态数据集忽略的漏洞，为模型过度敏感性提供更好的评估工具。
...
TL;DR: SCRIPTS数据集评估LLM在人际社交推理能力，发现当前模型在英语上表现尚可(75-80%)，但在韩语上显著下降(58-69%)，且经常选择不可能的关系(10-25%)。思维链提示对社交推理帮助有限，甚至可能放大社会偏见。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在人类-AI交互中广泛应用，评估其在人际语境中的社交推理能力变得至关重要。

Method: 使用从电影剧本中收集的1k对话数据集(英语和韩语)，要求模型推断说话者之间的人际关系，并由母语者标注概率性关系标签。

Result: 评估9个模型发现：专有LLM在英语数据集上达到75-80%准确率，但在韩语上降至58-69%；模型在10-25%的情况下选择不可能的关系；思维链提示对社交推理帮助有限且可能放大偏见。
...
TL;DR: 提出了两个用于心理健康支持LLM评估的基准：MentalBench-100k包含10万对对话响应，MentalAlign-70k通过7万次评分比较LLM与人类专家的表现，揭示了LLM评估者的系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 现有心理健康支持LLM评估基准规模有限、可靠性不足，缺乏评估自动化评判者可信度的框架，需要大规模对话数据集和评判可靠性评估方法。

Method: 使用MentalBench-100k整合真实场景对话和LLM生成响应，MentalAlign-70k通过情感认知一致性框架，采用组内相关系数统计方法量化LLM评判者与人类专家的一致性和偏差。

Result: 分析显示LLM评判者存在系统性评分膨胀，认知属性如指导性和信息性可靠性强，同理心精确度降低，安全性和相关性存在不可靠性。

Conclusion: 为心理健康领域LLM的可靠大规模评估建立了新的方法论和实证基础，发布了基准数据集和代码。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学数据集成需要将自然语言术语映射到标准化标识符，大语言模型在此任务中表现不一，需要系统评估其在不同术语体系中的记忆和泛化能力。

Method: 对Llama 3.1 8B进行微调，评估多个生物医学本体（GO、HPO、GENE）的术语到标识符映射性能，分析记忆（训练术语）和泛化（验证术语）表现，并进行嵌入分析。

Result: GO术语显示强记忆增益（准确率提升77%），HPO改进最小；仅基因映射有泛化增益（13.9%）；GPT-4o在所有术语中优于Llama变体；基因符号与蛋白质名称语义对齐紧密，GO和HPO对齐弱。

Conclusion: 微调成功取决于标识符流行度和词汇化程度：流行标识符增强记忆，词汇化标识符（如基因符号）支持语义泛化，而任意标识符（如GO和HPO）限制模型为机械学习。
...
TL;DR: 该论文研究了大型语言模型在参数知识与提示中冲突信息出现矛盾时的行为，特别关注代码生成领域中的知识冲突检测与激活引导方法。
...
Result: 实验表明足够大的LLMs在其参数中编码了知识冲突的概念，知识冲突检测准确率高达80.65%，激活引导相比随机基线提升了12.6%的引导成功率。

Conclusion: 模型大小、任务领域和引导方向之间的平衡对激活引导的有效性至关重要，成功检测和引导知识冲突需要综合考虑这些因素。
...
TL;DR: 提出基于频谱分析的框架，将transformer层建模为由注意力机制诱导的动态图，通过图信号处理定义诊断指标，用于检测大语言模型中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在取得显著成果的同时，区分事实推理与幻觉仍然具有挑战性，需要开发新的检测方法。

Method: 将transformer层建模为注意力诱导的动态图，token嵌入作为图上的信号，通过图信号处理定义Dirichlet能量、频谱熵和高频能量比等诊断指标。

Result: 实验发现事实陈述呈现一致的"能量山"行为，而不同类型的幻觉显示不同特征：逻辑矛盾使频谱不稳定，语义错误保持稳定但连接性漂移，替换幻觉显示中间扰动。基于频谱特征的检测器准确率达88.75%，优于基于困惑度的基线方法。

Conclusion: 频谱几何可能捕捉推理模式和错误行为，为检测大语言模型中的幻觉提供了潜在框架。
...
TL;DR: 通过图信号处理分析注意力诱导的token图，研究不同transformer架构在语音交替任务中的计算指纹，发现架构特定的光谱特征与行为差异强相关。
...
Method: 使用图信号处理分析注意力诱导的token图，跟踪20种语言和三个模型家族在语音交替下代数连通性（Fiedler值）的变化，重点关注早期层（2-5层）。

Result: 发现清晰的架构特征：Phi-3-Mini在英语中显示显著的早期层中断，Qwen2.5-7B对形态丰富语言显示分布式小变化，LLaMA-3.2-1B显示系统但温和的响应。光谱特征与行为差异强相关（Phi-3: r=-0.976）。

Conclusion: 训练重点可以留下可检测的计算印记，表现为在句法转换期间可测量的连接模式。该框架可作为简单、无需训练的诊断工具，揭示架构偏见并支持模型可靠性分析。
...
TL;DR: 本文对藏语AI研究现状进行全面调查，涵盖数据资源、NLP任务、机器翻译、语音识别和大语言模型等领域，旨在为未来研究提供基础参考。
...
Conclusion: 本调查旨在为藏语AI研究的未来工作提供基础参考，鼓励合作努力为低资源语言构建包容和可持续的AI生态系统。
...
TL;DR: 本文研究大型语言模型能否通过科技公司的招聘评估，发现所有评估的LLMs都无法通过标准化招聘考试。


<details>
  <summary>Details</summary>
Motivation: 科技公司需要高效筛选大量软件工程师申请人，而LLMs在编程和推理任务中表现出色，因此研究LLMs能否通过专业招聘评估具有重要意义。

Method: 使用最先进的LLMs生成对广泛使用的专业评估问卷的回答，并将其表现与公司参考解决方案进行比较评估。

Result: 模型生成的答案与公司参考解决方案之间存在显著不一致，所有评估的LLMs都未能通过招聘评估。

Conclusion: 尽管LLMs在编程任务中表现出色，但它们无法通过实际的科技公司招聘评估，表明当前LLMs在专业工程能力评估方面仍存在局限性。
...
TL;DR: TSSS是一个高效的多跳检索增强生成框架，通过模板化推理和基于检索器的终止机制，减少令牌使用并提高推理稳定性。
...
Method: 提出TSSS框架，包含：(1) 模板化推理，缓存重复前缀并将子查询锚定到主问题；(2) 基于检索器的终止器，当子查询重复时确定性地停止推理。

Result: 在HotpotQA、2WikiMultiHop和MuSiQue数据集上，TSSS实现了最先进的准确率和竞争性的效率。
...
Method: 从AWS更新、Azure变更和WHO疾病爆发报告3个真实世界时间戳语料库构建evolveQA基准，识别自然发生的知识演化，生成针对不同LLM知识截止日期的问题

Result: 在12个开源和闭源LLMs的3种知识探测格式评估中，evolveQA相比静态知识问题性能显著下降达31%

Conclusion: evolveQA基准能有效评估LLMs处理时间演化知识的能力，揭示了现有模型在动态知识理解方面的局限性
...
TL;DR: 提出了一种不依赖LLM检索增强生成的知识图谱问答系统，使用小型复述模型处理从知识图谱检索的实体关系边，在CRAG基准测试中达到71.9%和54.4%的准确率。


<details>
  <summary>Details</summary>
Motivation: 开发不依赖大型语言模型检索增强生成(RAG)的问答系统，通过知识图谱检索和实体关系边的复述来生成答案，减少对LLM的依赖。
...
Conclusion: 证明了不依赖LLM RAG的知识图谱问答系统的可行性，通过小型复述模型处理知识图谱检索结果可以获得良好的问答性能。
...
Conclusion: 该工作为评估工具增强对话系统提供了更全面的框架，能够发现传统评估方法可能遗漏的关键错误。
...
TL;DR: DiSRouter是一个分布式自路由系统，通过让LLM代理基于自我认知能力自主决定是否回答或转发查询，实现灵活高效的查询路由。
...
Method: 提出分布式路由范式，每个LLM代理独立决策；开发两阶段自我认知训练流程增强LLM的自我认知能力。
...
TL;DR: 提出了一个类型匹配的语言距离框架，通过结构感知的表示方法改进现有的语言知识库，并开发统一的多信号综合距离度量，在跨语言迁移任务中取得更好效果。


<details>
  <summary>Details</summary>
Motivation: 现有语言知识库（如URIEL+）存在两个关键限制：一是通用向量表示不适合语言数据的多样结构，二是缺乏将多种信号聚合为单一综合得分的原理性方法。

Method: 为每种距离类型提出新颖的结构感知表示：地理距离使用说话者加权的分布表示，谱系距离使用双曲嵌入，类型学距离使用潜变量模型，并将这些信号统一为稳健的任务无关综合距离。

Result: 在跨语言迁移任务的语言选择中，提出的表示方法和综合距离在各种NLP任务中持续提升性能。

Conclusion: 该框架为多语言研究提供了更原理化和有效的工具包，改进了跨语言迁移的语言选择效果。
...
TL;DR: SheetBrain是一个神经符号双工作流代理框架，用于在表格数据上进行准确推理，支持电子表格问答和操作任务，通过理解、执行和验证三个核心模块显著提高了推理准确性。
...
Conclusion: SheetBrain框架通过神经符号双工作流设计，有效解决了LLMs在复杂电子表格推理中的局限性，为表格数据处理提供了准确可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有难度可控问题生成方法存在两个主要局限：无法直接生成选择题（教育场景中最常用的问题类型），以及没有明确优化难度控制的准确性。

Method: 使用大语言模型，并采用直接偏好优化技术来训练模型，以提高难度控制的准确性。

Result: 该方法能够直接生成难度可控的多项选择题，解决了传统方法的局限性。

Conclusion: 提出的方法在难度可控选择题生成方面具有优势，为自适应学习支持提供了更有效的工具。
...
TL;DR: TheMCPCompany是一个用于评估工具调用代理在真实世界服务交互任务中的基准，包含超过18,000个工具。实验表明，先进推理模型在简单环境中能有效发现工具，但在复杂企业环境中表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前通用代理主要依赖网页浏览器与环境交互，而MCP协议提供了大量任务特定工具集，这些工具比通用工具更易开发维护。需要评估工具调用代理在实际服务交互任务中的表现。

Method: 使用真实世界服务的REST API创建MCP服务器，包含18,000多个工具，并为每个任务提供手动标注的真实工具。通过真实工具和工具检索两种方式评估代理性能。

Result: 使用真实工具时，工具调用代理在性能和成本方面都有提升。通过工具检索时，所有模型表现相似或优于基于浏览器的代理，但小模型无法充分利用可用工具。GPT-5在工具检索下的表现接近其使用真实工具的表现。

Conclusion: 在数万种工具中导航并以非平凡方式组合它们来解决复杂问题，对当前模型仍然具有挑战性，需要更好的推理和检索模型。
...
TL;DR: 提出了JointCQ框架，通过联合生成声明和查询来改进幻觉检测流程，在多个开放域QA基准测试中表现优于现有方法。
...
Method: 设计了联合声明-查询生成框架，利用精心设计的评估标准筛选合成训练数据，微调语言模型进行联合声明提取和查询生成。

Result: 在多个开放域QA幻觉检测基准测试中表现优于先前方法。

Conclusion: 该方法为实现更可信和透明的语言模型系统提供了有效的幻觉检测解决方案。
...
TL;DR: KORE是一种协同方法，通过知识导向的增强和约束，在向大型多模态模型注入新知识的同时保留旧知识，解决了现有方法在学习新知识和避免灾难性遗忘方面的困难。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在预训练权重中编码了大量事实知识，但这些知识是静态的，无法跟上现实世界的发展，阻碍了持续知识获取。现有方法在学习新知识时往往表现不佳且容易遭受灾难性遗忘。
...
Conclusion: KORE通过知识导向的增强和约束，成功解决了大型多模态模型在知识注入过程中面临的知识适应和知识保留双重挑战。
...
TL;DR: 提出了一个包含11个类别的幻觉综合分类法，并开发了HAD模型，将幻觉检测、跨度级识别和纠正集成到单一推理过程中。在约90K合成样本上训练，在多个基准测试中达到最先进水平。
...
Method: 提出幻觉综合分类法，开发HAD模型集成检测、识别和纠正功能，使用约90K合成样本训练，并构建包含2,248样本的HADTest测试集。
...
TL;DR: 提出了一种基于超体积优化的强化学习方法HVO，用于优化文本摘要的多目标问题，在多个数据集上表现优于GRPO方法，7B模型增强后性能接近GPT-4。
...
Result: 在多个代表性摘要数据集上，HVO方法在总体得分上优于GRPO，在不同维度上表现更均衡；7B基础模型增强后性能与GPT-4相当，同时保持更短的生成长度。

Conclusion: HVO方法有效解决了文本摘要的多目标优化问题，能够生成跨多个目标的平衡摘要，且在小模型上实现了与大模型相当的性能。
...
TL;DR: 将推理能力集成到语音大语言模型中用于端到端的槽位填充任务，通过思维链框架分解任务步骤，创建推理数据集并进行监督微调。


<details>
  <summary>Details</summary>
Motivation: 受推理大语言模型最新发展的启发，希望将推理能力整合到语音LLMs中，提升槽位填充任务的性能。

Method: 使用思维链框架将槽位填充任务分解为多个推理步骤，创建推理数据集，对语音LLM进行监督微调，并区分常规和推理型语音LLM。

Result: 引入推理步骤带来了性能提升，但主要用于数学、逻辑和编码领域的推理文本LLM作为语音LLM基础模型效果较差。混合语音LLM在保留直接和推理两种操作模式时性能更好。

Conclusion: 推理步骤能改善语音LLM的槽位填充性能，但需要选择合适的文本基础模型，混合操作模式的语音LLM比单一模式表现更优。
...
TL;DR: 该研究探讨了通过个性化大语言模型（Persona-LLMs）使用标注者身份来影响其对仇恨言论的敏感性，特别关注标注者与目标群体身份相似或不同时的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 研究旨在将心理学中的群体身份理论与先进NLP技术相结合，探索如何通过融入社会人口属性来减少自动仇恨言论检测中的偏见。

Method: 使用Google Gemini和OpenAI GPT-4.1-mini模型，采用两种身份提示方法：浅层身份提示和基于检索增强生成（RAG）的深度情境化身份开发，以纳入更丰富的身份档案。

Result: 研究分析了使用内群体和外群体标注者身份对模型检测性能和跨不同社会群体公平性的影响，揭示了基于身份的方法在减少偏见方面的潜力和局限性。

Conclusion: 将社会人口属性融入LLMs可以解决自动仇恨言论检测中的偏见问题，基于身份的方法为开发更公平的仇恨言论检测系统提供了有价值的见解。
...
TL;DR: LOGICAL是一个基于微调GLiNER模型的高效本地可部署PII移除系统，在临床笔记中实现了95%的文档完全净化，性能优于Gemini-Pro-2.5等模型，且可在标准笔记本电脑上运行。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在临床笔记PII移除中计算成本高、数据隐私风险大的问题，特别是在资源受限环境中。

Method: 使用1515份临床文档，定义9个PII类别，对modern-gliner-bi-large-v1.0模型在2849个文本实例上进行微调，并与多个基准系统进行比较。

Result: 微调GLiNER模型获得0.980的F1分数，显著优于Gemini-Pro-2.5（0.845），95%文档完全净化，但存在2%的实体级假阴性率。
...
TL;DR: 本文介绍了DnD Gesture++数据集，通过整合语义手势信息改进了多模态对话中的轮次转换预测性能。


<details>
  <summary>Details</summary>
Motivation: 研究人类对话中多模态线索（语音、手势、注视）对轮次转换的作用，特别是手势提供的补充性信息。

Method: 扩展DnD Gesture语料库，添加2,663个语义手势标注；采用专家混合框架整合文本、音频和手势信息进行轮次转换预测。

Result: 实验表明，加入语义引导的手势信息相比基线模型带来了持续的性能提升。

Conclusion: 手势在多模态轮次转换中发挥着补充性作用，语义手势信息能有效提升预测性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前模型在语音和文本理解方面表现良好，但在自然对话中理解谁在什么时候说了什么的能力（说话人归属推理）仍然存在困难。

Method: 从四个开放语料库（CHiME-6、MELD、MultiDialog、AMI）构建，包含超过12,000个经过验证的实例，配有音频、转录文本和元数据。包含两个任务：说话人归属问答和通过话语匹配的说话人归属。
...
TL;DR: AgenticMath是一个用于生成高质量数学问答对的智能代理流程，通过四阶段方法提升LLM的数学推理能力，仅需3-6万样本就能在3B-8B参数模型上实现优于更大规模数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法在生成高质量数据集时面临答案质量低、信息丰富度有限的问题，需要更有效的数据生成方法来提升LLM的数学推理能力。

Method: 四阶段流程：1)种子问题筛选；2)多代理问题重述；3)思维链答案增强；4)问答对评估筛选。

Result: 在3B-8B参数模型上，仅使用3-6万数学样本就能在多个数学推理基准测试中达到或超越使用40万-230万样本的基线模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 长上下文推理对大型语言模型至关重要，但现有强化学习方法主要针对短上下文，缺乏对高级长上下文推理模式的研究，且高难度RL数据稀缺。

Method: 提出LoongRL方法，核心是KeyChain合成技术：通过插入UUID链将短多跳问答转化为长上下文任务，在大量干扰文档中隐藏真实问题，要求模型逐步追踪正确链、识别真实问题并推理回答。

Result: 在Qwen2.5-7B和14B上，LoongRL将长上下文多跳问答准确率分别提升23.5%和21.1%。LoongRL-14B达到74.2分，媲美更大前沿模型。同时提升长上下文检索能力，通过所有128K压力测试，并保持短上下文推理能力。

Conclusion: LoongRL通过数据驱动的强化学习成功诱导出计划-检索-推理-复查的推理模式，该模式能泛化远超训练长度，为长上下文推理提供了高效解决方案。
...
Conclusion: MLEB为法律信息检索领域提供了最全面、最多样化的开源基准，填补了现有资源的空白，有助于推动该领域的研究和发展。
...
TL;DR: MoE-Prism通过模型-系统协同设计将刚性MoE模型转变为弹性服务，提供细粒度控制能力，在保持质量的同时实现更好的资源利用。
...
Method: 采用两阶段方法：离线重构引擎将单体专家分解为细粒度"子专家"，使用元启发式分区优化算法分组神经元；在线调度引擎实现QoS感知调度，包含云部署吞吐量最大化和内存受限设备延迟优化卸载等策略。

Result: 在三种不同MoE模型上的评估显示，MoE-Prism提供比基线多4倍以上的稳定操作点，在严格延迟预算下可将吞吐量提升19.9%，在有限资源下可将延迟降低10.36%。
...
TL;DR: 提出了一种新的手语翻译方法，使用目标句子的句子嵌入代替传统的手势注释，无需手动标注，在德语和美国手语数据集上表现优异


<details>
  <summary>Details</summary>
Motivation: 传统手语翻译系统依赖手势注释，但这类标注数据通常规模有限且不同数据集间差异很大，需要一种无需手动标注的新监督方法

Method: 使用目标句子的句子嵌入作为训练时的监督信号，这种监督不需要手动标注，而是从原始文本数据中学习，支持单语和多语言句子嵌入及翻译系统

Result: 在德语(PHOENIX-2014T)和美国手语(How2Sign)数据集上显著优于其他无手势注释方法，在没有额外手语翻译数据集预训练的情况下，缩小了无手势注释系统与依赖手势注释系统之间的差距

Conclusion: 该方法为手势注释不可用的数据集设立了新的最先进水平，通过使用句子嵌入作为监督信号，有效解决了手势注释数据稀缺和标注不一致的问题
...
TL;DR: 该论文提出使用语言无关的多模态嵌入来监督手语翻译，结合多语言目标增强和视频级扰动，实现了直接的多语言翻译并提升了模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统手语翻译通常使用单一语言的文本进行训练，这限制了可扩展性和跨语言泛化能力。现有方法虽然用文本句子嵌入替代了注释监督，但仍局限于特定语言和模态。

Method: 采用在多种语言的文本和语音上训练的语言无关多模态嵌入来监督SLT；提出耦合增强方法，结合多语言目标增强（翻译成多种语言）和视频级扰动。

Result: 实验显示在BLEURT指标上相比仅使用文本句子嵌入监督有持续提升，在低资源设置下改进更显著。
...
TL;DR: ToMMeR是一个轻量级模型（<300K参数），通过探测早期LLM层来识别文本中的实体提及，在13个NER基准测试中达到93%的零样本召回率，90%以上的精确度，证明了结构化实体表示存在于早期transformer层中。
...
Method: 引入ToMMeR模型，通过探测早期LLM层来检测实体提及，使用轻量级参数（<300K），并扩展到包含跨度分类头。

Result: 在13个NER基准测试中达到93%的零样本召回率，90%以上精确度；跨模型分析显示不同架构（14M-15B参数）在提及边界上高度一致（DICE >75%）；扩展后达到接近SOTA的NER性能（80-87% F1）。

Conclusion: 实体提及检测自然地从语言建模中涌现，结构化实体表示存在于早期transformer层中，可以用最少的参数高效恢复。
...
TL;DR: 提出了一种端到端的手语翻译系统，从瑞士德语手语视频直接翻译成德语文本，使用时空特征表示和翻译的单一模型。


<details>
  <summary>Details</summary>
Motivation: 现有手语翻译系统通常使用通用序列到序列架构，但往往无法充分利用时间特征，需要改进特征表示方法。

Method: 开发了一个学习时空特征表示和翻译的单一模型，实现真正的端到端架构，预期能更好地泛化到新数据集。

Result: 在开发集上达到5±1 BLEU分，但在测试集上性能显著下降至0.11±0.06 BLEU分。

Conclusion: 虽然端到端方法在开发集上表现良好，但在测试集上泛化能力有限，表明该方法仍需进一步改进。
...
Method: 从280万自然学习者句子构建136,867个控制三元组（修正版、学习者版、人工版），测试模型的选择性容忍能力。

Result: 实验表明选择性容忍度与标准语法性是不同能力，模型性能按训练范式聚类，验证了BLiSS的有效性。

Conclusion: BLiSS是测量不同训练目标如何影响模型与人类语言习得系统模式对齐的强大工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试受限于静态设计，无法充分评估LMMs对时间敏感知识的理解能力，需要专门的时间感知评估基准。

Method: 从维基百科构建MINED基准，包含2104个时间敏感知识样本，涵盖6种知识类型，由两名专业标注者完成。评估了15个广泛使用的LMMs，并研究了通过知识编辑方法更新时间敏感知识的可行性。

Result: Gemini-2.5-Pro获得最高平均CEM分数63.07，大多数开源LMMs缺乏时间理解能力。LMMs在组织知识上表现最佳，在体育知识上表现最弱。知识编辑方法在单次编辑场景中能有效更新LMMs的知识。
...
TL;DR: MBR解码在语音转文本任务中表现优于波束搜索，在自动语音识别和语音翻译任务中提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 鉴于MBR解码在文本到文本生成任务中有效，作者期望它在语音到文本任务中也能有效，但目前波束搜索仍是语音转文本任务的主流方法。

Method: 使用Whisper及其衍生模型在英语和日语上评估MBR解码在ASR和ST任务中的表现。

Result: 在大多数实验设置中，MBR解码的准确性优于波束搜索。

Conclusion: MBR解码是离线ASR和ST任务中一种有前景的方法，特别适用于需要高准确性的场景。
...
TL;DR: VideoAgentTrek是一个从公开屏幕录制视频自动挖掘训练数据的可扩展管道，无需手动标注。通过Video2Action模块检测GUI动作并提取结构化参数，从39,000个YouTube教程视频生成了152万交互步骤，显著提升了计算机使用代理的性能。


<details>
  <summary>Details</summary>
Motivation: 训练计算机使用代理需要大量GUI交互数据，但手动标注成本过高。利用公开的屏幕录制视频作为数据源，可以解决数据稀缺问题。

Method: 开发了Video2Action逆动力学模块，包含视频定位模型（检测GUI动作的时间边界和上下文）和动作内容识别器（提取点击坐标和输入文本等结构化参数）。通过持续预训练和监督微调利用生成的数据。

Result: 在OSWorld-Verified上，任务成功率从9.3%提升到15.8%（相对提升70%）；在AgentNetBench上，步骤准确率从64.1%提升到69.3%。

Conclusion: 被动互联网视频可以转化为高质量的训练数据，为计算机使用代理提供了可扩展的替代方案，避免了昂贵的手动标注。
...
TL;DR: 该论文研究了成员推理攻击和机器生成文本检测之间的可迁移性，证明两者最优性能的度量标准相同，并通过大规模实验验证了跨任务性能的强相关性。


<details>
  <summary>Details</summary>
Motivation: 尽管成员推理攻击和机器生成文本检测目标不同，但方法都基于语言模型的概率分布信号。由于两个任务被独立研究，可能忽略了对方领域更强的方法和见解。

Method: 通过理论和实证研究，证明了两个任务的最优性能度量标准相同，并假设方法近似该度量的准确性与可迁移性直接相关。进行了大规模实验，包括7种最先进的MIA方法和5种机器文本检测器，涵盖13个领域和10个生成器。

Result: 实验显示跨任务性能存在很强的等级相关性（rho > 0.6）。特别发现，原本为机器文本检测设计的Binoculars在MIA基准测试中也达到了最先进性能。

Conclusion: 研究结果强调了两个研究社区需要更多的跨任务意识和合作。为此，作者引入了MINT，一个统一的评估套件，包含两个任务的15种最新方法实现。
...
<details>
  <summary>Details</summary>
Motivation: 现有路由方法仅基于输入查询进行分类，忽略了输出信息和上下文细微差别，导致复杂或模糊查询的路由决策不理想

Method: 提出Lookahead框架，通过预测模型输出的潜在表示来"预见"可能输出，实现无需完整推理的知情路由，基于因果和掩码语言模型实现两种方法

Result: 在七个公共基准测试（指令遵循、数学推理和代码生成）上，Lookahead始终优于现有路由基线

Conclusion: Lookahead框架通过预见潜在输出实现了更明智的路由决策，显著提升了多模型系统的效率
...
<details>
  <summary>Details</summary>
Motivation: 语音基础模型在不同任务上表现出色，但评估方法分散且不一致，不同模型在不同语音处理方面各有所长，需要统一的评估分类方法。

Method: 提出三轴分类法：评估方面（测量内容）、模型能力（完成任务所需能力）、任务要求（执行任务所需协议），对现有评估和基准进行分类映射。

Result: 分类法揭示了系统性空白，如韵律、交互和推理方面的评估覆盖有限，为未来基准设计指明了优先方向。

Conclusion: 这项工作为选择和解释语音模型评估提供了概念基础和实践指南，有助于更系统地评估不同语音基础模型。
...
TL;DR: 本文通过系统实证研究发现，多语言模型微调中的灾难性遗忘主要由模型与数据规模的相对比例决定，指令跟随能力比架构更重要，参数高效微调在缓解遗忘方面无明显优势，而跨语言对齐可有效缓解遗忘并促进正向迁移。


<details>
  <summary>Details</summary>
Motivation: 多语言基础模型在特定语言上微调时常引发灾难性遗忘，降低未见语言的性能，但文献对此现象的发生条件存在分歧，需要系统研究来澄清。

Method: 使用机器翻译作为测试平台，在不同模型架构、数据规模和微调方法下进行受控实验，系统分析灾难性遗忘的触发条件。

Result: 发现模型与数据规模的相对比例是遗忘的主要决定因素；指令跟随能力对保持多语言知识比架构更重要；参数高效微调相比全参数微调在缓解遗忘方面无明确优势；跨语言对齐可缓解遗忘并促进正向迁移。

Conclusion: 多语言微调中的灾难性遗忘受模型-数据规模比例主导，指令能力是关键因素，跨语言对齐是有效的缓解策略。
...
<details>
  <summary>Details</summary>
Motivation: 解决混合语言历史文档中拉丁语片段提取的问题，填补该领域研究的空白。

Method: 使用724页标注的多模态数据集，对大型基础模型进行基准测试和性能评估。

Result: 结果显示当代模型能够实现可靠的拉丁语检测，为任务提供了可行性验证。

Conclusion: 本研究首次全面分析了这些模型在该任务上的能力和局限性，为相关研究奠定了基础。
...
TL;DR: 提出了PBBQ基准数据集，用于评估波斯语言模型中的社会偏见，包含16个文化类别和37,000多个问题。
...
Method: 通过与社会科学专家合作，让250名不同人口统计学背景的个体填写问卷，开发了包含16个文化类别的综合基准数据集。

Result: 评估发现当前LLMs在波斯文化中表现出显著的社会偏见，且模型输出往往复制人类偏见模式。

Conclusion: PBBQ数据集为波斯语言模型的偏见评估和缓解提供了基础，揭示了学习表示与文化刻板印象之间的复杂关系。
...
TL;DR: 提出了一个可扩展、可解释的众包流程来收集跨语言新闻相似性评估数据集CrossNews-UA，包含乌克兰语与波兰语、俄语、英语的新闻对，并测试了多种模型的性能。


<details>
  <summary>Details</summary>
Motivation: 在社交媒体和错误信息快速传播的时代，跨语言假新闻检测面临挑战。现有数据集由专家手动整理，缺乏可扩展性和对新语言的适应性。

Method: 开发了可扩展的众包流程收集乌克兰语与相关语言（波兰语、俄语、英语）的新闻对，基于4W标准（Who、What、Where、When）进行语义相似性标注和详细解释。测试了词袋模型、Transformer架构和大型语言模型。

Result: 收集了CrossNews-UA数据集，揭示了多语言新闻分析的挑战，并提供了不同模型性能的见解。

Conclusion: 提出的众包流程能够有效收集跨语言新闻相似性数据，为多语言新闻分析提供了有价值的资源和模型性能评估。
...
TL;DR: 本文提出了一种基于字体风格的攻击方法SAD，利用人类与NLP模型对风格化文本的感知差异，在保持人类可读性的同时干扰模型处理。


<details>
  <summary>Details</summary>
Motivation: 社交媒体用户使用风格化字体和字体式表情符号表达个性，但这些字符在NLP模型中被视为不同token，造成人机感知差异，从而引入安全漏洞。

Method: 设计了SAD攻击方法，包含轻量版（查询高效）和强力版（攻击性能优越），利用风格化文本字符干扰模型处理。

Result: 在情感分类和机器翻译任务上对传统模型、LLM和商业服务进行测试，SAD展现出强大的攻击性能，并在多模态任务（文本到图像、文本到语音）中显示潜在威胁。

Conclusion: 风格化文本在人机感知差异中隐藏着安全风险，SAD攻击方法有效利用了这种差异，对NLP系统构成实际威胁。
...
TL;DR: LlavaCode框架通过将代码压缩成紧凑的语义表示来提升代码补全效率，在保持生成质量的同时显著减少检索上下文，实现20-38%的TTFT降低。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成虽然能有效提升代码补全质量，但引入上下文会显著增加序列长度，导致推理速度变慢，这在IDE等交互式环境中是严重限制。

Method: 使用小型投影器模块将代码压缩成紧凑的语义表示，使检索上下文仅需几个压缩后的单令牌向量，从而减少序列长度。

Result: 实验表明压缩上下文能在保持生成质量的同时，在线补全任务中实现20-38%的TTFT降低，且EM和ES指标有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 探索在开放文本中进行自动情感分析的挑战，包括上下文模糊性、语言变异性和复杂情感表达理解困难，这些限制使得通用模型的直接应用变得困难。

Method: 比较微调预训练模型与通用LLMs在三种场景下的效果：(i)微调模型与简单提示LLMs的性能；(ii)不同情感提示设计的有效性；(iii)情感分组技术对模型的影响。
...
TL;DR: 论文提出DiffAdapt框架，通过分析推理轨迹的熵模式来优化LLM推理效率，减少过度思考现象，在保持准确率的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有推理大语言模型虽然表现出色，但会产生冗长的思维轨迹，计算效率低下。研究发现模型在简单问题上存在'过度思考'现象，导致不必要的计算开销。

Method: 提出DiffAdapt轻量级框架，基于问题难度和推理轨迹熵为每个问题选择Easy/Normal/Hard推理策略（包含固定提示、温度和最大token长度）。该方法不微调基础LLM，而是训练一个小型探针来分类LLM的最终隐藏状态。

Result: 在5个模型和8个基准测试上的评估显示，该方法在保持或提高准确率的同时，将token使用量减少了高达22.4%。

Conclusion: DiffAdapt为计算高效的推理提供了一条实用路径，通过动态调整推理策略显著提升了LLM的推理效率。
...
TL;DR: CoSense-LLM是一个边缘优先框架，将多模态传感器流转换为紧凑的语义令牌，在延迟、能耗、带宽和隐私约束下与大语言模型协同工作。


<details>
  <summary>Details</summary>
Motivation: 解决在干扰环境中部署大模型时面临的语义理解、隐私保护和可预测延迟等挑战，实现边缘设备上的高效多模态感知。

Method: 包含四个核心组件：SenseFusion轻量编码器、Edge-RAG本地检索层、PromptRouter成本感知策略和Secure Execution安全执行机制，支持现代服务优化技术。

Result: 在家庭、办公室和诊所部署中实现亚秒级延迟，显著降低跨层令牌和带宽成本，通过本地检索提高事实一致性并减少矛盾。

Conclusion: 支持边缘优先设计，将语义理解、隐私保护和可预测延迟视为在干扰环境中部署大模型的同等重要目标。
...
TL;DR: 该论文研究大型语言模型是否具备动机警觉能力，即能否像人类一样考虑信息来源的动机来评估内容可信度。研究发现LLMs在受控实验中表现良好，但在真实广告场景中表现不佳，不过简单的干预可以显著提升其表现。


<details>
  <summary>Details</summary>
Motivation: 人类沟通具有动机性，而LLMs处理的信息也受到人类意图和动机的影响。为了让LLMs在现实世界中有效运作，它们必须能够批判性地评估内容，考虑信息来源的动机，比如判断销售宣传中声明的可信度。

Method: 首先使用认知科学中的受控实验验证LLMs行为是否与理性模型一致，然后扩展到赞助在线广告等更自然的环境。最后测试简单的干预措施是否能提高LLMs对动机的警觉性。
...
TL;DR: 本文通过实验研究发现，提示词对语言模型内部表征质量有影响，但这种变化与提示词与目标任务的相关性并不一致相关，挑战了"更相关的提示词必然产生更好表征"的假设。


<details>
  <summary>Details</summary>
Motivation: 研究提示词与语言模型内部表征质量的关系，揭示预训练嵌入如何支持上下文任务解决，理解语言模型在零样本设置下执行多样化任务的机制。

Method: 进行一系列探测实验，分析零样本分类中不同提示模板组合的嵌入表示，研究提示词对表征质量的影响。

Result: 发现提示词确实影响表征质量，但这些变化与提示词和目标任务的相关性并不一致相关，更相关的提示词不一定产生更好的表征。

Conclusion: 挑战了提示词相关性与表征质量直接相关的假设，需要进一步分析导致这种意外行为的潜在因素。
...
<details>
  <summary>Details</summary>
Motivation: 欧盟法律信息虽然公开，但对普通民众来说难以理解和应用，存在可访问性挑战

Method: 基于204个博客构建880轮对话数据集，提出LexGuide框架，使用检索增强生成和分层主题组织来结构化对话进程
...
<details>
  <summary>Details</summary>
Motivation: 现有方法无法通过提示工程确保LLM按照特定文化规范生成内容，而直接微调LoRA权重的方法引入了大量参数。

Method: 提出Zhyper框架，使用因子化超网络从文本描述生成上下文感知的LoRA适配器，实现参数高效的文化对齐。

Result: 在多个基准测试中，Zhyper达到与最先进基线相当的性能，同时参数最多减少26倍，在文化对齐任务中展现出更好的泛化能力和细粒度上下文价值捕捉。

Conclusion: Zhyper提供了一种参数高效的方法来实现大语言模型的文化对齐，在减少参数的同时保持性能，并改善了泛化能力。
...
TL;DR: 提出SmartSwitch推理框架解决大语言模型在复杂推理中的"欠思考"问题，通过监控推理过程检测过早放弃的高潜力思路并引导深入探索。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在长链推理中存在的"欠思考"问题，即模型频繁切换思路而缺乏充分探索，限制了性能和token效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法最小化所有token的KL散度，这与推测解码最大化token接受率的目标不一致，导致草稿模型难以充分吸收目标模型知识。

Method: AdaSPEC引入选择性token过滤，使用参考模型识别并过滤难以拟合的token，使草稿模型在简单token上更好地与目标模型对齐。

Result: 在算术推理、指令跟随、编码和摘要等任务上，AdaSPEC始终优于最先进的DistillSpec方法，接受率提升高达15%。

Conclusion: AdaSPEC通过选择性蒸馏策略有效提升了推测解码的性能，在不影响生成质量的情况下提高了token接受率。
...
TL;DR: 该论文提出了一种通过模型合并来适应代码混合NLP任务的方法，相比传统全微调或持续预训练方法，在英语-印地语和英语-西班牙语的情感分析和仇恨言论检测任务中表现更优，并能更好地迁移到低资源语言对。


<details>
  <summary>Details</summary>
Motivation: 研究模型合并作为代码混合NLP任务中传统适应策略的实用替代方案，旨在更有效地利用未标记数据并提升在低资源语言对上的迁移能力。

Method: 从多语言基础模型出发：(i)在未标记的代码混合文本上进行持续预训练获得适应检查点，(ii)将该检查点与基础模型合并，(iii)在下游任务数据上进行微调。使用XLM-R和Llama-3.2-1B模型在英语-印地语和英语-西班牙语上进行评估。

Result: 合并模型始终优于全微调和CPT->FT方法，在F1分数上比全微调高出2-5分，比CPT->FT高出约1-2分。在跨语言对迁移测试中，合并检查点比单语英语基线表现更好，表明代码混合知识是更可靠的低资源对知识基础。

Conclusion: 模型合并能更有效地利用未标记数据，在代码混合NLP任务中表现优于传统方法，并为低资源语言对提供了更可靠的迁移基础。论文还提供了针对不同数据场景的适应方案，并讨论了在大模型和更广泛任务中的扩展考虑。
...
TL;DR: ToolDreamer框架通过让LLM生成假设性的工具描述来改进工具检索，使检索器能更好地匹配用户查询与工具，从而支持大规模工具集的使用。


<details>
  <summary>Details</summary>
Motivation: 现有检索模型基于用户查询与工具描述的相似性进行工具检索，但用户请求往往与工具描述语言不匹配，导致检索效果不佳。

Method: 提出ToolDreamer框架，使用LLM生成假设性的工具描述，让检索器基于这些描述来检索工具，实现查询与工具在语言空间上的更好对齐。

Result: 在ToolRet数据集上的实验表明，该方法能提升稀疏和密集检索器的性能，无论是否进行训练都有效。

Conclusion: 该框架将部分推理负担转移到检索器上，使LLM能够有效处理大规模工具集而不会超出上下文窗口限制。
...
TL;DR: 提出了一种轻量级框架，通过优化提示空间（自然性、文化适应和难度增强）来改进多语言LLM性能，相比仅翻译的基线方法取得了显著提升。
...
Method: 引入轻量级提示空间优化框架，系统地将翻译后的提示进行自然性、文化适应和难度增强的转换，应用于12种语言的提示优化。

Result: 在相同数据条件下，相比仅翻译基线方法，在Global-MMLU准确率上提升4.7%，Flores XCometXL上提升2.4%，mArenaHard偏好测试中胜出35.3%。

Conclusion: 提示空间优化是构建更稳健、文化基础和全球化能力更强的多语言LLM的简单而强大的范式。
...
TL;DR: Scaf-GRPO通过渐进式训练框架解决强化学习中的"学习悬崖"问题，在模型学习停滞时提供分层提示，显著提升模型在复杂数学问题上的表现。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法面临"学习悬崖"问题：当问题远超模型当前能力时，模型持续失败导致奖励信号为零，优势计算崩溃，学习梯度消失，难以解决超出能力范围的问题。

Method: 提出Scaf-GRPO框架，首先诊断学习停滞，然后在必要时注入分层提示（从抽象概念到具体步骤），使模型能够自行构建有效解决方案。

Result: 在AIME24基准测试中，将Qwen2.5-Math-7B模型的pass@1分数相比基础GRPO方法相对提升了44.3%。

Conclusion: 该框架为解锁模型解决超出其能力范围问题的能力提供了稳健有效的方法，是扩展LLM自主推理前沿的关键一步。
...
TL;DR: Hubble是一套用于研究LLM记忆机制的开源模型套件，包含标准版和扰动版，通过控制文本插入来模拟记忆风险，揭示了训练语料大小和敏感数据出现时机对记忆的影响。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型的记忆机制，特别是敏感数据的记忆风险，为理解和管理LLM记忆提供科学基础。

Method: 开发标准模型和扰动模型，在训练过程中控制插入特定文本（如书籍段落、传记、测试集），比较不同参数规模（1B/8B）和训练数据量（100B/500B tokens）下的记忆效果。

Result: 发现记忆风险取决于敏感数据在训练语料中的相对频率，增加语料规模可以稀释敏感数据，而让敏感数据在训练早期出现能增强记忆。扰动模型还显示没有持续暴露的敏感数据会被遗忘。

Conclusion: 提出了应对记忆风险的最佳实践：通过增加训练语料规模稀释敏感数据，以及合理安排敏感数据在训练中的出现时机。Hubble套件为记忆研究、成员推断和机器遗忘提供了理想的测试平台。
...
### [153] [A New Type of Adversarial Examples](https://arxiv.org/abs/2510.19347)
*Xingyang Nie,Guojie Xiao,Su Pan,Biao Wang,Huilin Ge,Tao Fang*

Main category: cs.LG

TL;DR: 提出了一种与常规对抗样本相反的新型对抗样本生成方法，这些样本与原始样本显著不同但产生相同输出，并开发了NI-FGSM、NI-FGM等算法。


<details>
  <summary>Details</summary>
Motivation: 传统对抗样本是在原始样本附近添加微小扰动导致模型错误分类，本文探索相反情况：生成与原始样本显著不同但模型输出相同的样本。

Method: 提出了负迭代快速梯度符号法(NI-FGSM)、负迭代快速梯度法(NI-FGM)及其动量变体(NMI-FGSM、NMI-FGM)来生成这类对抗样本。

Result: 生成的对抗样本不仅分布在数据集样本附近，而是广泛分布在样本空间中，可用于特定场景下对机器学习系统的攻击。

Conclusion: 对抗样本的分布范围比传统认知更广泛，这种新型对抗样本为机器学习安全研究提供了新的视角和攻击手段。
...
### [154] [3D Optimization for AI Inference Scaling: Balancing Accuracy, Cost, and Latency](https://arxiv.org/abs/2510.18905)
...
<details>
  <summary>Details</summary>
Motivation: 现有的AI推理扩展方法通常使用1D启发式或2D双变量权衡，未能考虑成本和延迟约束。

Method: 使用蒙特卡洛模拟在三个代表性场景和九个模拟大语言模型上评估四种优化方法，解决3D多目标优化问题。

Result: 膝点优化实现了最佳平衡，当精度优先时准确性最大化仍然有利。该框架为不同操作环境下的部署感知推理扩展建立了理论基础。

Conclusion: 将推理扩展构建为多目标优化问题形成了1D和2D优化无法捕获的可行空间，实现了环境自适应的推理扩展k值选择。
...
### [155] [Large Connectome Model: An fMRI Foundation Model of Brain Connectomes Empowered by Brain-Environment Interaction in Multitask Learning Landscape](https://arxiv.org/abs/2510.18910)
*Ziquan Wei,Tingting Dan,Guorong Wu*

Main category: cs.LG

TL;DR: 该论文提出了一种基于多任务学习的功能性神经影像基础模型，通过将大脑-环境交互(BEI)标记化进行预训练，并使用伪标签进行半监督微调，在多种神经影像应用中取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 当前功能性神经影像AI模型受限于样本量不足，而现有的基础模型由于自监督学习与大脑-结果关系不一致，对下游任务不够优化。

Method: 利用丰富的环境变量和人口统计数据，将大脑建模为多任务学习，提出可扩展的模型架构：(i)通过标记化大脑-环境交互进行多任务预训练；(ii)使用预训练BEI的伪标签进行半监督微调。

Result: 在性别预测、人类行为识别以及自闭症、帕金森病、阿尔茨海默病和精神分裂症的早期诊断等多种应用中评估，结果显示有促进当前神经影像临床应用的良好潜力。

Conclusion: 该基础模型通过多任务预训练和半监督微调方法，能够有效提升功能性神经影像在临床常规应用中的性能。
...
### [156] [ADPO: Anchored Direct Preference Optimization](https://arxiv.org/abs/2510.18913)
*Wang Zixian*

Main category: cs.LG

TL;DR: ADPO是一个统一框架，扩展了DPO，引入软偏好概率、任意参考策略锚定和列表式偏好建模，在噪声环境下表现优于标准DPO。


<details>
  <summary>Details</summary>
Motivation: 标准DPO假设硬二元标签和成对比较，存在梯度漂移和训练不稳定的问题。ADPO旨在通过软偏好概率、参考策略锚定和列表式建模来解决这些问题。

Method: ADPO引入：(i)软偏好概率编码不确定性；(ii)任意参考策略锚定通过组间平移不变性和隐式KL正则化稳定训练；(iii)通过Plackett-Luce分布进行列表式偏好建模。

Result: 在上下文赌博机中，锚定使WinMass比标准DPO提高38-63%；在重尾噪声污染下，KDE平滑达到0.68 vs 0.32（相对增益112%）。在顺序强化学习中，锚定在噪声偏好下性能提高15-29%。

Conclusion: ADPO是DPO的泛化框架，在噪声环境下表现更优。建议在清洁或中等噪声下使用成对锚定Soft-DPO，在极端污染下使用基于KDE的列表式ADPO。
...
Method: 创建MLX-transformers框架，包含不同的Transformer实现，将PyTorch模型检查点转换为MLX格式，并在两个苹果硅MacBook设备上对BERT、RoBERTa和XLM-RoBERTa模型进行基准测试。

Result: 研究结果突显了MLX在苹果生态系统中实现高效和更易访问的设备上机器学习应用的潜力。

Conclusion: MLX框架在苹果硅设备上表现出良好的性能，为设备上机器学习应用提供了有效的解决方案。
...
### [158] [Noise-corrected GRPO: From Noisy Rewards to Unbiased Gradients](https://arxiv.org/abs/2510.18924)
...
<details>
  <summary>Details</summary>
Motivation: RLHF和RLVR对不一致或错误的奖励噪声高度敏感，但噪声与基于群体的策略优化方法的相互作用研究不足。

Method: 引入GRPO和Dr.GRPO框架，将奖励腐败建模为伯努利噪声，通过估计奖励翻转概率进行噪声校正，得到可证明无偏的梯度估计。
...
### [159] [Application of Reduced-Order Models for Temporal Multiscale Representations in the Prediction of Dynamical Systems](https://arxiv.org/abs/2510.18925)
*Elias Al Ghazal,Jad Mounayer,Beatriz Moya,Sebastian Rodriguez,Chady Ghnatios,Francisco Chinesta*

Main category: cs.LG

TL;DR: 提出了三种多尺度学习方法：基于分区统一法的神经网络分解、奇异值分解提取主导模态、稀疏高阶SVD从有限测量重建多尺度动力学，以准确捕捉复杂多尺度系统的宏观和微观行为。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法难以捕捉复杂多尺度系统的高频行为和非线性动态，且对初始条件敏感，需要开发能同时处理宏观和微观尺度的新方法。

Method: 1) 分区统一法结合神经网络分解动力学；2) 奇异值分解提取主导模态分离尺度；3) 稀疏高阶SVD从有限测量重建多尺度动态。

Result: 这些方法能准确捕捉粗粒度和细粒度动态，适用于现实世界中涉及复杂多尺度现象的应用，并能适应高维系统和不完整观测。

Conclusion: 提出的多尺度学习框架能有效处理复杂多尺度系统的动态建模和预测，在所有时间尺度上提供近似和解释，具有实际应用价值。
...
### [160] [BAPO: Stabilizing Off-Policy Reinforcement Learning for LLMs via Balanced Policy Optimization with Adaptive Clipping](https://arxiv.org/abs/2510.18927)
...
TL;DR: BAPO是一种针对离策略强化学习的平衡优化方法，通过动态调整裁剪边界来解决策略熵下降和优化不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 离策略强化学习在训练中使用历史数据提高样本效率，但会导致策略熵急剧下降、优化不稳定甚至崩溃的问题。

Method: 提出BAPO方法，基于熵裁剪规则动态调整裁剪边界，自适应平衡正负贡献，保持策略熵并稳定优化过程。

Result: 在AIME 2024和2025基准测试中，7B BAPO模型超越开源对手，32B BAPO模型在同规模模型中达到最先进水平，甚至超过o3-mini和Gemini-2.5-Flash-Thinking等专有系统。

Conclusion: BAPO方法在多样化离策略场景中实现了快速、稳定且数据高效的训练，解决了离策略强化学习中的关键挑战。
...
### [161] [Position: Many generalization measures for deep learning are fragile](https://arxiv.org/abs/2510.18934)
*Shuofeng Zhang,Ard Louis*

Main category: cs.LG

TL;DR: 本文指出许多用于深度神经网络的事后泛化度量是脆弱的——微小的训练修改（如超参数调整）会显著改变度量的值、趋势或缩放行为，即使这些修改几乎不影响网络本身。


<details>
  <summary>Details</summary>
Motivation: 虽然深度神经网络的泛化度量难以获得紧致边界，但通常被认为能反映定性泛化趋势。本文旨在揭示这些事后度量的脆弱性，即它们对训练细节的敏感性。

Method: 通过实验分析多种泛化度量（如路径范数、谱范数、Frobenius范数、平坦度代理和确定性PAC-Bayes替代）在不同训练条件下的行为变化。

Result: 发现许多泛化度量对超参数调整高度敏感，例如学习率调整或SGD变体切换可以反转学习曲线的斜率。PAC-Bayes原点度量虽然对超参数变化较不敏感，但无法捕捉数据复杂度的差异。

Conclusion: 许多泛化度量具有脆弱性，新度量开发者应明确审核其脆弱性，并建议使用基于函数的边际似然PAC-Bayes边界来更好地捕捉数据复杂度差异。
...
### [162] [NeuroAda: Activating Each Neuron's Potential for Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2510.18940)
...
Method: 首先识别网络中的重要参数（连接），然后为这些选定参数引入旁路连接。微调期间仅更新旁路连接，原始模型参数保持冻结

Result: 在23+个自然语言生成和理解任务上，仅需≤0.02%可训练参数即可达到最先进性能，CUDA内存使用减少高达60%
...
### [163] [Towards Universal Solvers: Using PGD Attack in Active Learning to Increase Generalizability of Neural Operators as Knowledge Distillation from Numerical PDE Solvers](https://arxiv.org/abs/2510.18989)
*Yifei Sun*

Main category: cs.LG

TL;DR: 提出对抗性师生蒸馏框架，用可微数值求解器监督紧凑神经算子，通过PGD风格主动采样搜索最坏情况输入来扩展训练集，显著提升神经算子的分布外鲁棒性。
...
Method: 使用可微数值求解器作为教师模型监督学生神经算子，结合PGD风格主动采样循环在平滑性和能量约束下搜索最坏情况输入来扩展训练集，可微谱求解器支持基于梯度的对抗搜索。

Result: 在Burgers和Navier-Stokes系统上的实验表明，对抗蒸馏显著提高了分布外鲁棒性，同时保持了神经算子的低参数成本和快速推理优势。

Conclusion: 对抗蒸馏框架有效解决了神经算子在分布外泛化方面的局限性，为PDE求解提供了既高效又鲁棒的解决方案。
...
### [164] [An Encode-then-Decompose Approach to Unsupervised Time Series Anomaly Detection on Contaminated Training Data--Extended Version](https://arxiv.org/abs/2510.18998)
*Buang Zhang,Tung Kieu,Xiangfei Qiu,Chenjuan Guo,Jilin Hu,Aoying Zhou,Christian S. Jensen,Bin Yang*

Main category: cs.LG

TL;DR: 提出了一种新的编码-分解范式，通过将编码表示分解为稳定和辅助表示，增强在训练时间序列被污染时的鲁棒性，并使用基于互信息的新指标替代重构误差来识别异常。


<details>
  <summary>Details</summary>
Motivation: 现有的无监督时间序列异常检测方法中，自编码器对训练时间序列中的异常敏感，导致准确性降低。需要提高在训练数据被污染情况下的鲁棒性。
...
### [165] [Prior-informed optimization of treatment recommendation via bandit algorithms trained on large language model-processed historical records](https://arxiv.org/abs/2510.19014)
*Saman Nessari,Ali Bozorgi-Amiri*

Main category: cs.LG

TL;DR: 开发了一个集成LLM、CTGAN、T-learner和上下文bandit的个性化医疗推荐系统，在结肠癌数据集上表现优于基准方法
...
Method: 使用LLM处理非结构化医疗文本，CTGAN生成合成患者数据，T-learner预测治疗反应，上下文bandit优化治疗选择
...
### [166] [Category learning in deep neural networks: Information content and geometry of internal representations](https://arxiv.org/abs/2510.19021)
*Laurent Bonnasse-Gahot,Jean-Pierre Nadal*

Main category: cs.LG

TL;DR: 该论文从理论角度解释了人工神经网络中类别感知现象，证明了最小化贝叶斯成本会导致在决策边界附近扩展神经空间，这与最大互信息原则一致。


<details>
  <summary>Details</summary>
Motivation: 研究动物和人工神经网络中类别学习增强类别边界附近刺激区分能力的现象（类别感知），从理论框架解释这一现象是高效学习的必然结果。

Method: 扩展理论框架到人工网络，证明最小化贝叶斯成本等价于最大化类别与决策层前神经活动之间的互信息，通过Fisher信息矩阵分析神经表示的最优度量。

Result: 发现最优学习使神经Fisher信息遵循类别特定的Fisher信息，其最大值通常不在类别边界上而是在边界附近，在玩具模型和MNIST数据集上验证了两者匹配。

Conclusion: 类别学习在决策边界附近诱导神经空间扩展，建立了与信息瓶颈方法的联系，并提出了贝叶斯成本的偏差-方差分解。
...
### [167] [Empowering Decision Trees via Shape Function Branching](https://arxiv.org/abs/2510.19040)
*Nakul Upadhya,Eldan Cohen*

Main category: cs.LG

TL;DR: 提出了一种新型决策树变体——形状广义树(SGT)，通过在每个内部节点应用可学习的轴对齐形状函数，实现了非线性分割，同时保持可解释性。
...
Method: 提出了ShapeCART算法来学习SGTs，并扩展到双变量形状函数(S²GT)和多路树(SGT_K)，分别对应Shape²CART和ShapeCART_K算法。

Result: 在多个数据集上的实验表明，SGTs相比传统的轴对齐线性树实现了更优的性能，同时减少了模型规模。

Conclusion: SGTs提供了一种既保持可解释性又提升性能的决策树改进方案，通过形状函数实现非线性分割，为模型决策机制提供直观的可视化解释。
...
### [168] [POLAR: Policy-based Layerwise Reinforcement Learning Method for Stealthy Backdoor Attacks in Federated Learning](https://arxiv.org/abs/2510.19056)
*Kuai Yu,Xiaoyu Wu,Peishen Yan,Qingqian Yang,Linshan Jiang,Hao Wang,Yang Hua,Tao Song,Haibing Guan*

Main category: cs.LG

TL;DR: POLAR是首个使用强化学习解决联邦学习中后门攻击层选择问题的框架，通过轻量级伯努利采样和策略梯度优化，在保持隐蔽性的同时显著提升攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护了数据隐私，但其分布式特性使其容易受到后门攻击。现有的后门关键层选择方法基于规则，未考虑层间相互关系，导致效果不佳且容易被先进防御检测。

Method: POLAR采用强化学习框架，使用伯努利采样进行轻量级层选择，通过策略梯度更新基于后门成功率改进优化攻击策略，并引入正则化约束限制修改层数以保持隐蔽性。

Result: 在对抗六种最先进防御方法的实验中，POLAR相比最新攻击方法性能提升高达40%。

Conclusion: POLAR证明了强化学习在后门攻击层选择问题上的有效性，能够动态学习攻击策略，在保持隐蔽性的同时实现高攻击成功率。
...
### [169] [Weight Decay may matter more than muP for Learning Rate Transfer in Practice](https://arxiv.org/abs/2510.19093)
*Atli Kosson,Jeremy Welborn,Yang Liu,Martin Jaggi,Xi Chen*

Main category: cs.LG

TL;DR: 论文通过大规模实证研究发现，Maximal Update Parameterization (muP) 的学习率缩放规则仅在训练初期有效，而权重衰减才是实现跨宽度学习率转移的关键因素。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何将小网络的最优学习率有效转移到大型神经网络中，以解决超参数调优在大型模型训练中的高昂成本问题。

Method: 采用大规模实证研究方法，分析muP在不同训练阶段的有效性，特别关注权重衰减对内部表示更新动态的影响。

Result: 研究发现muP的假设仅在训练初期成立，而权重衰减才是稳定跨宽度更新动态的主要机制，muP主要起到隐式学习率预热的作用。

Conclusion: 这些发现挑战了关于学习率转移的普遍认知，解释了为什么muP需要独立权重衰减变体才能成功实现转移。
...
### [170] [What Makes a Good Curriculum? Disentangling the Effects of Data Ordering on LLM Mathematical Reasoning](https://arxiv.org/abs/2510.19099)
*Yaning Jia,Chunhui Zhang,Xingjian Diao,Xiangchi Yuan,Zhongyu Ouyang,soroush vosoughi*

Main category: cs.LG

TL;DR: 课程学习（CL）在LLM推理中的效果取决于模型能力和任务复杂性，没有统一的策略。研究发现前向和后向课程学习的相对有效性因模型和任务而异，不同难度样本在不同任务中产生不同收益。


<details>
  <summary>Details</summary>
Motivation: 解决课程学习中关于何时有效、哪种方向更好以及答案是否依赖于测量指标的基本问题，通过统一的离线评估框架分析课程难度。

Method: 使用统一的离线评估框架，将课程难度分解为五个维度：问题难度、模型惊讶度、置信度边际、预测不确定性和决策变异性。在数学推理基准上对Llama3.1-8B、Mistral-7B和Gemma3-4B进行受控后训练实验。

Result: 发现没有课程策略普遍占优，前向与后向CL的相对有效性取决于模型能力和任务复杂性；即使在同一指标内，不同难度样本在不同任务中产生不同收益；任务对齐课程关注最终表示和泛化，而内部状态课程调节置信度和不确定性。

Conclusion: 挑战了通用课程策略的概念，为不同模型和任务提供可操作指导，某些指标显示优先处理决策不确定样本可进一步改善学习效果。
...
### [171] [MetaCluster: Enabling Deep Compression of Kolmogorov-Arnold Network](https://arxiv.org/abs/2510.19105)
*Matthew Raffel,Adwaith Renjith,Lizhong Chen*

Main category: cs.LG

TL;DR: MetaCluster框架通过元学习器和聚类技术，使Kolmogorov-Arnold Networks (KANs)在不损失精度的情况下实现高达80倍的参数压缩。


<details>
  <summary>Details</summary>
Motivation: KANs用每边向量替代标量权重提高了表达能力，但导致参数和内存的乘法增长，需要高效的压缩方法。

Method: 使用轻量级元学习器将低维嵌入映射到系数向量，通过K-means聚类用共享质心替代每边向量，然后微调质心码本恢复精度。

Result: 在MNIST、CIFAR-10和CIFAR-100数据集上，MetaCluster实现了高达80倍的参数存储减少，且无精度损失。

Conclusion: MetaCluster成功解决了KANs参数过多的问题，通过向量化参数特性实现了高效的存储分摊，为KANs的实际应用提供了可行的压缩方案。
...
### [172] [Learning Peer Influence Probabilities with Linear Contextual Bandits](https://arxiv.org/abs/2510.19119)
...
<details>
  <summary>Details</summary>
Motivation: 在社交网络中，用户间的推荐采纳率高度依赖于上下文因素，准确估计这些影响力概率对于理解信息传播和改进病毒营销策略至关重要。

Method: 采用上下文线性赌博机框架，提出不确定性引导的探索算法，通过调节参数在遗憾最小化与估计误差之间实现任意权衡。

Result: 在半合成网络数据集上的实验表明，该方法优于静态方法和忽略这种权衡的上下文赌博机方法。

Conclusion: 同伴影响力概率学习存在基本权衡，所提出的算法能够灵活平衡探索与利用，有效提升影响力概率的估计精度。
...
### [173] [Steering Autoregressive Music Generation with Recursive Feature Machines](https://arxiv.org/abs/2510.19127)
*Daniel Zhao,Daniel Beaglehole,Taylor Berg-Kirkpatrick,Julian McAuley,Zachary Novack*

Main category: cs.LG

TL;DR: MusicRFM是一个基于递归特征机器(RFMs)的框架，能够在无需重新训练的情况下对预训练音乐模型进行细粒度、可解释的控制，通过引导模型内部激活来实现对音符、和弦等音乐属性的精确控制。


<details>
  <summary>Details</summary>
Motivation: 现有可控音乐生成方法通常需要模型重新训练或会产生可听伪影，因此需要一种能够在不影响生成质量的前提下对冻结预训练模型进行实时细粒度控制的方法。

Method: 训练轻量级RFM探针来发现MusicGen隐藏状态中的概念方向，然后在推理过程中将这些方向注入回模型，通过动态时间变化调度和多重音乐属性同时强制执行机制来引导生成过程。

Result: 能够将目标音符生成的准确率从0.23提升到0.82，同时文本提示的遵循度保持在未引导基线的约0.02范围内，实现了有效控制且对提示保真度影响最小。

Conclusion: MusicRFM成功平衡了控制与生成质量之间的权衡，展示了RFMs在音乐领域的应用潜力，为无需重新训练的可控音乐生成提供了有效解决方案。
...
### [174] [InvarGC: Invariant Granger Causality for Heterogeneous Interventional Time Series under Latent Confounding](https://arxiv.org/abs/2510.19138)
...
<details>
  <summary>Details</summary>
Motivation: 传统Granger因果性测试基于线性模型，往往无法检测到轻微的非线性因果关系。现有非线性方法依赖两个关键假设：因果充分性和已知干预目标，但在实际时间序列数据中，这些假设往往不成立。

Method: 提出不变Granger因果性(InvarGC)，利用跨环境异质性来缓解潜在混杂因素的影响，并在边缘级别粒度上区分干预和非干预环境。

Result: 在合成和真实世界数据集上的广泛实验表明，该方法相比最先进方法具有竞争性性能。

Conclusion: InvarGC能够有效处理存在潜在混杂因素和未知干预环境的情况，恢复不变的因果关系，并在这些条件下建立了可识别性。
...
### [175] [Subliminal Corruption: Mechanisms, Thresholds, and Interpretability](https://arxiv.org/abs/2510.19152)
*Reya Vir,Sarvesh Bhatnagar*

Main category: cs.LG

TL;DR: 本文研究了潜意识污染现象，即不良特性通过语义中性数据传播，绕过标准安全检查，导致AI系统行为退化。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型越来越多地使用合成数据进行微调，存在微妙错位通过互联AI系统传播的关键风险，需要量化理解其动态机制。

Method: 使用GPT-2的师生设置，系统研究潜意识污染的缩放规律、阈值和机制，包括实验和可解释性分析。

Result: 发现三个关键结果：潜意识污染导致行为交叉退化；对齐在污染数据临界阈值处发生急剧相变而非逐渐退化；污染机制模仿自然微调过程难以检测。

Conclusion: 研究揭示了依赖合成数据的AI系统的关键脆弱性，强调需要能够应对潜在威胁的新安全协议。
...
### [176] [Feature Space Adaptation for Robust Model Fine-Tuning](https://arxiv.org/abs/2510.19155)
*Peng Wang,Minghao Gu,Qiang Huang*

Main category: cs.LG

TL;DR: 提出了两种在特征空间进行微调的方法LoRFA和VeFA，通过轻量级的特征级变换来补偿下游潜伏变量的影响，在保持预训练表示的同时提高模型在分布偏移下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决模型微调中的灾难性遗忘问题，特别是当下游领域标记数据有限或与预训练分布差异较大时。现有参数高效微调方法在权重空间操作，可能导致模型过度专门化，覆盖预训练知识。

Method: 提出特征空间适应方法：LoRFA（低秩特征适应）和VeFA（基于向量的特征适应）。基于效应等价建模理论，通过轻量级特征变换补偿下游潜伏变量的影响。

Result: 在图像分类、自然语言理解和生成任务上评估，特征空间适应方法在标准微调指标上获得可比结果，在鲁棒性方面表现更优。

Conclusion: 特征空间适应能够有效保持预训练表示，提高模型在分布偏移下的泛化能力，相比权重空间方法具有更强的鲁棒性。
...
### [177] [Instance-Dependent Regret Bounds for Nonstochastic Linear Partial Monitoring](https://arxiv.org/abs/2510.19158)
*Federico Di Gennaro,Khaled Eldowa,Nicolò Cesa-Bianchi*

Main category: cs.LG

TL;DR: 本文提出了一种针对线性部分监控问题的非随机对抗性算法，通过探索优化方法实现了高效的实现，并在不同游戏结构下获得了更透明的遗憾边界。


<details>
  <summary>Details</summary>
Motivation: 线性部分监控可以建模无限结果空间，同时在线性结构和观测上施加约束，这比经典部分监控更灵活。该设置可以看作是线性赌博机的推广，其中损失和反馈以灵活方式解耦。

Method: 使用探索优化方法的简单实例，该方法易于高效实现。针对有限动作的非随机对抗性版本问题，推导了依赖于游戏结构的遗憾边界。

Result: 获得了包含实例特定量的遗憾边界，这些量反映了观测与损失之间的对齐程度。在简单游戏中达到√T速率，在困难游戏中达到T^{2/3}速率。

Conclusion: 该算法在各种部分信息设置中实例化了这些边界，并证明所实现的游戏结构依赖在有趣情况下可能是紧的。
...
### [178] [Preliminary Use of Vision Language Model Driven Extraction of Mouse Behavior Towards Understanding Fear Expression](https://arxiv.org/abs/2510.19160)
*Paimon Goulart,Jordan Steinhauser,Kylene Shuler,Edward Korzus,Jia Chen,Evangelos E. Papalexakis*

Main category: cs.LG

TL;DR: 该研究开发了一个视觉语言模型，通过文本输入对视频中的小鼠行为进行分类，生成随时间变化的行为向量，无需模型微调即可获得高准确率。


<details>
  <summary>Details</summary>
Motivation: 整合多样化数据对改善科学探索至关重要，该研究旨在建立能够编码视频和文本输入的模型，用于分类小鼠在环境中的各种行为。

Method: 使用开源Qwen2.5-VL模型，通过提示工程、上下文学习（ICL）和帧级预处理来增强性能，无需模型微调。

Result: 每种方法都改善了分类性能，组合使用后在所有行为类别（包括罕见的冻结和逃跑行为）上都获得了较高的F1分数。

Conclusion: 该模型将支持跨学科研究人员研究小鼠行为，使他们能够将多个时间点和环境中测量的多样化行为特征整合到综合数据集中，以解决复杂的研究问题。
...
### [179] [Natural Gradient VI: Guarantees for Non-Conjugate Models](https://arxiv.org/abs/2510.19163)
*Fangyuan Sun,Ilyas Fatkhullin,Niao He*

Main category: cs.LG

TL;DR: 本文对随机自然梯度变分推断(NGVI)进行了理论分析，特别关注非共轭似然情况下的均值场参数化，提出了相对平滑性条件和改进算法，并建立了收敛性保证。


<details>
  <summary>Details</summary>
Motivation: 尽管NGVI在变分推断中经验成功且具有基础性作用，但其理论基础仍然有限，特别是在非共轭似然情况下，变分损失变得非凸且难以分析。

Method: 首先推导变分损失满足相对平滑性的充分条件；其次提出包含非欧几里得投影的改进NGVI算法；最后在额外结构假设下揭示变分损失的隐藏凸性。
...
<details>
  <summary>Details</summary>
Motivation: 发现多任务后训练中梯度大小不平衡的问题，大梯度任务主导优化过程，但这种梯度差异与任务学习收益不匹配。

Method: 通过分析RL后训练中不同任务的梯度大小和学习收益关系，研究梯度不平衡现象。

Result: 大梯度任务的学习收益可能相似甚至低于小梯度任务，梯度不平衡不能通过训练奖励或优势值等典型统计量解释。

Conclusion: 需要避免简单的数据集混合，未来应研究基于梯度的修正方法来解决LLM训练中的梯度不平衡问题。
...
TL;DR: 提出了一种去中心化的actor-critic强化学习框架，在多智能体系统中通过局部策略更新减少通信负担，同时保持网络协调。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体系统中通信受限的强化学习问题，通过减少通信频率来降低通信负担。

Method: 使用去中心化actor-critic框架，每个智能体在邻居交换信息前进行多次局部策略和价值函数更新，价值函数由多层神经网络近似。

Result: 在马尔可夫采样下建立了有限时间收敛分析，达到ε精度稳定点的样本复杂度为O(ε⁻³)，通信复杂度为O(ε⁻¹τ⁻¹)，其中τ为局部训练步数。

Conclusion: 该框架在减少通信的同时保持了协调性能，数值实验验证了理论结果的有效性。
...
TL;DR: 提出了基于主动扩散的图神经网络ADGNN，通过整合外部信息源动态影响扩散过程，克服传统被动扩散的过平滑问题，实现真正的无限扩散。


<details>
  <summary>Details</summary>
Motivation: 传统基于热扩散的GNN模拟被动热扩散，存在过平滑问题且难以捕获全局图信息。受宇宙热寂启发，认识到没有外部输入时节点表示会收敛到相同特征向量。

Method: ADGNN通过整合多个外部信息源实现主动扩散，动态影响扩散过程。直接计算主动扩散迭代公式的闭式解，实现真正的无限扩散。

Result: 在多种图任务上与最先进GNN模型对比，ADGNN在准确性和效率上均有显著提升，能有效捕获全局图信息并保持节点独特性。

Conclusion: ADGNN通过主动扩散机制成功克服了传统扩散方法的过平滑问题，在保持节点特征独特性的同时实现了对图全局结构的全面理解。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法需要预训练的复杂教师模型，本研究探索在没有教师模型的情况下，通过简单浅层GNN架构的协同学习来获得更好的推理性能。

Method: 提出协作学习框架，让GNN学生模型集合在训练过程中相互教学，引入自适应logit加权单元促进模型间知识交换，以及熵增强技术改进相互学习。
...
TL;DR: 该论文将机器遗忘重新定义为多目标优化问题，提出了CUP算法，通过独特的枢轴机制可控地导航整个帕累托前沿，使用单一超参数"遗忘强度"来精确选择权衡点。
...
Method: 提出CUP算法，采用枢轴梯度机制，不同于传统多目标优化方法收敛到单一解，而是可控地导航整个帕累托前沿，通过"遗忘强度"超参数实现精确控制。

Result: 实验结果表明，CUP产生了更优的帕累托最优解集，在各种视觉任务中持续优于现有方法，使用超体积指标评估显示其解决方案的质量和多样性更佳。

Conclusion: 将机器遗忘重新构建为多目标优化问题，并通过CUP算法的枢轴机制实现了对遗忘过程的精确控制，解决了传统方法的权衡挑战。
...
TL;DR: 提出了configurations框架，这是一个受大脑启发的有限分辨率聚类方法，使用单一分辨率参数和吸引-排斥动力学来实现层次组织、新颖性检测和灵活适应。


<details>
  <summary>Details</summary>
Motivation: 婴儿能够在无监督情况下发现类别、检测新颖性并适应新环境，这对当前机器学习构成了挑战。

Method: 使用单一分辨率参数和吸引-排斥动力学，通过mheatmap评估方法提供比例热图和重新分配算法来公平评估多分辨率和动态行为。

Result: 在多个数据集上，configurations在标准聚类指标上具有竞争力，新颖性检测AUC达到87%，在动态类别演化中稳定性提高35%。

Conclusion: configurations为早期认知分类提供了一个原则性计算模型，是迈向受大脑启发的AI的重要一步。
...
TL;DR: 该论文分析了时间序列基础模型(TSFMs)设计中的隐性偏差，通过理论和实证研究揭示了训练过程中的各种设计选择如何影响模型的基本特性，包括时间行为、几何结构等。


<details>
  <summary>Details</summary>
Motivation: 理解时间序列基础模型训练过程中各种设计参数如何影响模型质量，而不是开发新模型并声称其在现有基准测试中表现更好。

Method: 使用理论分析和受控实证评估相结合的方法，研究补丁大小、嵌入选择、训练目标等设计选择对模型特性的影响。

Result: 识别了多个设计选择如何导致模型基本特性的隐性偏差，这些偏差可能是直观的或非常反直觉的，取决于模型和数据的特性。

Conclusion: 通过异常值处理的案例研究展示了多种偏差如何复杂交互，并讨论了研究结果对学习苦涩教训和构建TSFMs的影响。
...
TL;DR: SPOT是一种用于计算可解释决策树策略的新方法，通过混合整数线性规划和降维分支定界方法，在保持性能的同时显著提升可扩展性和运行效率。
...
Result: 在标准基准测试中，SPOT实现了显著加速，可扩展到状态数更多的更大MDP，生成的决策树策略既可解释又紧凑。
...
TL;DR: 提出了SILVER with RL-guided labeling，通过将RL策略自身的动作输出纳入边界点识别，将SILVER框架扩展到多动作和高维环境，显著提高了深度强化学习的可解释性。
...
Conclusion: 这项工作通过将SILVER转变为可扩展且行为感知的框架，推动了可解释RL的发展，能够在高维多动作设置中解释深度RL智能体。
...
TL;DR: 该论文提出了GraMixC模块，通过提取配置结构、对齐和融合来改进下游任务的预测性能，在多个数据集上取得了显著提升。
...
Method: 引入GraMixC模块，提取配置结构，使用反向合并/拆分(RMS)技术对齐它们，并通过注意力头融合它们，然后传递给任何下游预测器。

Result: 在DSN1 16S rRNA培养介质预测任务中，GraMixC将R2分数从0.6提高到0.9，在多个方法中创造了新的最先进水平。在标准表格基准测试中也持续优于单分辨率和静态特征基线。

Conclusion: 配置结构比寄存器令牌具有更低的冗余性，且无需临时选择。GraMixC模块能够有效提取、对齐和融合这些配置结构，显著提升下游任务的性能。
...
Conclusion: FnRGNN为图神经网络节点回归任务提供了一个有效的公平性处理框架，通过多层次策略在复杂图拓扑下确保鲁棒公平性。
...
<details>
  <summary>Details</summary>
Motivation: 状态空间模型虽然具有可扩展性优势，但训练成本高且生态系统不成熟，难以从预训练注意力模型中有效蒸馏知识

Method: 使用轻量级桥接和灵活的层级对齐策略，实现token级监督，克服架构异构性

Result: 在视觉和语言领域的广泛实验中，该方法在有限训练数据下持续提升状态空间模型性能，优于标准和跨架构蒸馏方法

Conclusion: 注意力知识可以有效转移到循环模型中，为构建更强的SSM社区快速利用Transformer专业知识
...
TL;DR: 提出高斯蒸馏方法，通过深度潜变量因子模型将教师集成压缩为学生分布而非学生集成，解决了知识蒸馏中不确定性保持不足的问题


<details>
  <summary>Details</summary>
Motivation: 深度集成虽然能提供最先进的不确定性量化，但计算和内存需求大，难以部署到实际应用中。现有知识蒸馏技术难以保持不确定性，因为减小DNN尺寸通常会导致变异减少

Method: 使用深度潜变量因子模型将教师集成视为随机过程的实现，通过期望最大化算法稳定估计均值和协方差函数，实现分布蒸馏

Result: 在多个基准数据集上，高斯蒸馏优于现有基线方法，在语言模型微调和分布偏移问题上也表现良好

Conclusion: 高斯蒸馏能有效压缩教师集成并保持不确定性，为实际部署提供了可行的解决方案
...
TL;DR: 提出QiMeng-SALV方法，通过利用功能正确输出信号的代码段来优化Verilog代码生成的强化学习训练，解决了功能奖励不足的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在Verilog代码生成方面有显著进展，但缺乏有意义的功能奖励阻碍了基于强化学习的偏好优化，无法生成功能正确的Verilog代码。

Method: 通过比较生成模块与参考模块的信号功能正确性，使用抽象语法树识别信号感知代码段，引入信号感知DPO在正确的信号级代码段上进行优化。

Result: 在VerilogEval和RTLLM上达到最先进性能，7B参数模型匹配DeepSeek v3 671B模型的性能，显著优于在相同数据集上训练的开源模型CodeV。

Conclusion: QiMeng-SALV标志着从传统模块级优化向细粒度信号级优化的范式转变，有效解决了Verilog代码生成中功能奖励不足的问题。
...
TL;DR: 提出Loopholing机制，通过确定性潜在路径保留离散扩散模型中的分布信息，显著提升文本生成质量，缩小与自回归模型的差距。


<details>
  <summary>Details</summary>
Motivation: 解决离散扩散模型在采样过程中分布信息丢失的问题，一旦进行类别采样，丰富分布信息会坍缩为one-hot向量，导致后续步骤信息受限。

Method: 引入Loopholing机制，通过确定性潜在路径保留分布信息，结合自条件策略进行高效训练，构建Loopholing离散扩散模型(LDDMs)。

Result: 生成困惑度降低高达61%，在文本生成质量上接近甚至超越自回归模型，在推理任务中提升算术基准性能，减少空闲步骤和振荡。

Conclusion: Loopholing机制为高质量非自回归文本生成提供了可扩展路径，有效缓解了离散扩散模型的信息丢失问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统物种分布监测方法覆盖范围有限，物种分布模型(SDM)有助于填补这些空白。本研究旨在通过深度学习和数据预处理技术提高SDM在蛙类分布预测中的准确性。

Method: 应用深度学习和数据插补技术，使用数据平衡方法，进行特征选择识别关键环境因素，构建多模态集成模型整合土地覆盖、NDVI等环境输入。

Result: 数据平衡使蛙类计数任务的MAE从189降至29；多模态集成模型优于单个模型，在未见区域表现出良好泛化能力；图像和表格数据融合在栖息地分类中达到84.9%准确率和0.90 AUC。

Conclusion: 多模态学习和数据预处理技术（如平衡和插补）在数据稀疏或不完整时能显著改善生态预测建模，为更精确和可扩展的生物多样性监测做出贡献。
...
TL;DR: 提出了一种新颖的校准流程，通过在学习表示样本的聚类上训练校准函数集成来增强整体校准性能，显著提升校准分数并引入匹配度量来优化模型选择。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在关键决策中需要高度可靠的预测，校准虽然常被忽视但在临床预测等关键应用中至关重要。

Method: 使用学习表示样本的聚类来训练校准函数集成，构建通用校准方案，适应任何底层表示、聚类、校准方法和度量。

Result: 将各种方法的校准分数从82.28%提升到100%，并引入独特的匹配度量确保模型选择同时优化区分度和校准。

Conclusion: 该通用方案具有灵活性，在常用校准方法上均表现出优越性能，为关键决策提供更可靠的预测校准。
...
<details>
  <summary>Details</summary>
Motivation: 解决长上下文推理场景中I/O和计算开销过大的问题，通过混合注意力机制优化推理效率。

Method: 采用线性注意力和softmax注意力的混合架构，系统探索不同注意力机制的比例，并使用自研的高性能FP8算子库linghe提升训练效率。

Result: 相比32B参数密集模型，推理成本降低到1/10；相比原Ring系列，成本降低超过50%；在多个复杂推理基准测试中保持SOTA性能。
...
TL;DR: 该论文指出时间序列基础模型(TSFMs)虽然预测精度高，但预测形式(点预测、分位数预测、参数预测或轨迹集成)决定了其实际应用价值。研究发现三分之二的TSFMs只产生点预测或参数预测，而许多运营任务需要保留时间依赖性的轨迹集成预测。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列基础模型主要关注预测精度，但忽略了预测形式对实际应用价值的关键影响。不同类型的运营任务需要不同形式的预测支持，而现有模型大多无法提供所需的轨迹集成预测。

Method: 通过调查近期TSFMs，分析不同预测类型之间的转换可能性，证明边际分布无法确定路径依赖事件概率，并建立六种基本预测任务与最小充分预测类型的映射关系。

Result: 研究发现轨迹集成可以简化为其他形式，但反向转换需要额外假设；边际分布相同的无限多个联合分布对运营问题给出不同答案；提出了任务对齐的评估框架。

Conclusion: 预测类型而非精度决定了实际效用，需要根据具体任务选择合适的预测形式，现有模型在提供轨迹集成预测方面存在不足。
...
### [199] [A Markov Decision Process for Variable Selection in Branch & Bound](https://arxiv.org/abs/2510.19348)
*Paul Strang,Zacharie Alès,Côme Bissuel,Olivier Juan,Safia Kedad-Sidhoum,Emmanuel Rachelson*

Main category: cs.LG

TL;DR: 提出了BBMDP，一种用于分支定界法中变量选择的MDP公式，通过强化学习学习最优分支启发式策略，在四个标准MILP基准测试中优于现有最先进RL方法。


<details>
  <summary>Details</summary>
Motivation: 分支定界法求解MILP时，变量选择启发式对性能影响很大。现有RL方法采用特定MDP公式和收敛定理，需要更通用、规范的方法来学习最优分支策略。

Method: 提出BBMDP，一个规范化的MDP公式用于B&B中的变量选择，能够利用广泛的RL算法学习最优分支启发式。

Result: 计算实验验证了模型有效性，提出的分支代理在四个标准MILP基准测试中优于现有最先进RL代理。

Conclusion: BBMDP提供了一个原则性的MDP框架，能够有效学习B&B中的最优分支策略，在多个基准测试中表现出色。
...
### [200] [Scalable LinUCB: Low-Rank Design Matrix Updates for Recommenders with Large Action Spaces](https://arxiv.org/abs/2510.19349)
*Evgenia Shustova,Marina Sheshukova,Sergey Samsonov,Evgeny Frolov*

Main category: cs.LG

TL;DR: 提出Scalable LinUCB算法，通过动态低秩参数化逆Cholesky因子，实现快速内存高效的线性上下文多臂老虎机操作


<details>
  <summary>Details</summary>
Motivation: LinUCB在推荐系统中广泛应用，但其训练、推理和内存成本随特征维度和动作空间大小增长，关键瓶颈是需要更新、求逆和存储吸收交互历史上下文信息的设计矩阵

Method: 采用动态低秩参数化逆Cholesky因子，推导数值稳定的秩1和批量更新方法，使用投影器-分裂积分器控制内存增长

Result: 实现平均每步更新成本O(dr)和内存O(dr)，推理复杂度为每个动作评估O(dr)，在推荐系统数据集上验证有效性

Conclusion: 提出的Scalable LinUCB算法能够显著降低线性上下文多臂老虎机的计算和内存开销，同时保持良好性能
...
### [201] [ConvXformer: Differentially Private Hybrid ConvNeXt-Transformer for Inertial Navigation](https://arxiv.org/abs/2510.19352)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习惯性跟踪系统易受隐私泄露攻击，现有差分隐私解决方案会因引入过多噪声而损害模型性能，特别是在高频惯性测量中。

Method: 使用ConvXformer混合架构，融合ConvNeXt块和Transformer编码器；提出自适应梯度裁剪和梯度对齐噪声注入的差分隐私机制；利用截断奇异值分解进行梯度处理。

Result: 在OxIOD、RIDI、RoNIN基准数据集上，ConvXformer超越最先进方法，定位精度提升超过40%，同时确保(ε,δ)-差分隐私保证。在KAIST机械工程大楼收集的Mech-IO数据集上验证了在严重环境干扰下的鲁棒性。

Conclusion: 该框架在恶劣环境干扰下表现出鲁棒性，适用于网络物理系统中的安全和智能导航。
...
### [202] [Optimization Benchmark for Diffusion Models on Dynamical Systems](https://arxiv.org/abs/2510.19376)
*Fabian Schaipp*
...
### [203] [LMFD: Latent Monotonic Feature Discovery](https://arxiv.org/abs/2510.19383)
...
<details>
  <summary>Details</summary>
Motivation: 许多系统会随时间老化或退化，但传感器数据中可能没有直接反映'年龄'的信息。需要从现有传感器数据中发现能够代表系统老化的潜在代理特征。

Method: 使用精心定义的语法生成候选公式，通过优化公式的单调性（基于Spearman秩相关系数）来发现能够组合多个传感器的潜在特征。

Result: 在人工生成数据集和两个真实世界数据集上的实验表明，该方法能够将单调性较低的单传感器特征组合成具有高单调性的潜在特征。在InfraWatch结构健康监测项目中，将两个Spearman ρ值分别为0.13和0.09的特征组合成ρ值为0.95的代理特征。

Conclusion: 该方法能够发现可解释的公式，这些公式可以作为系统'年龄'的有效代理特征，即使单个传感器的单调性很低。
...
### [204] [Learning Noise-Resilient and Transferable Graph-Text Alignment via Dynamic Quality Assessment](https://arxiv.org/abs/2510.19384)
*Yuhang Liu,Minglai Shao,Zengyi Wo,Yunlong Chu,Bing Hao,Shengzhong Liu,Ruijie Wang,Jianxin Li*

Main category: cs.LG

TL;DR: ADAligner是一个动态、质量感知的图文本对齐框架，通过实时评估监督质量，在表达性多对多对齐和保守一对一对齐之间动态调整，解决了现有图文本对齐方法在真实世界图中多对多关系和噪声监督下的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的CLIP风格图文本对齐器存在两个关键限制：假设节点和文本之间存在严格的一对一对应关系，忽略了真实世界图中固有的多对多关系；依赖静态对齐目标无法适应变化的数据质量，在噪声监督下变得脆弱。

Method: ADAligner框架实时估计批次级别的对齐可靠性，并相应调整优化策略：当监督干净时促进软性、子图级别的多对多对齐；在噪声情况下通过动态过滤低置信度对来强调可靠的一对一对齐。

Result: 在九个不同的TAG数据集上的综合实验表明，ADAligner在零样本/少样本节点分类、链接预测和跨模态检索任务上持续优于先前的图文本对齐器。在噪声监督下保持强鲁棒性，与多模态基线相比加速预训练约2到3倍。

Conclusion: ADAligner为真实世界网络环境中的图文本表示学习建立了可扩展且可靠的基础，通过动态对齐机制有效解决了多对多关系和噪声监督的核心困境。
...
### [205] [CPSVD: Enhancing Large Language Model Compression via Column-Preserving Singular Value Decomposition](https://arxiv.org/abs/2510.19385)
*Lin Xv,Jingsheng Gao,Xian Gao,Ting Li,Yuzhuo Fu*

Main category: cs.LG

TL;DR: 提出CPSVD方法，通过智能分割参数矩阵，直接保留高分解误差的列，仅对低分解误差的列应用SVD，实现更优的LLM压缩效果。


<details>
  <summary>Details</summary>
Motivation: 现有SVD方法统一处理整个参数矩阵，忽略了不同矩阵部分的分解误差差异，导致压缩效果不佳。

Method: CPSVD识别并直接保留高分解误差的矩阵列，仅对低分解误差的列应用SVD，并自适应分配非均匀压缩率。

Result: 实验表明CPSVD在困惑度和零样本任务准确率上优于现有SVD压缩方法。
...
### [206] [ARA: Adaptive Rank Allocation for Efficient Large Language Model SVD Compression](https://arxiv.org/abs/2510.19389)
*Lin Xv,Jingsheng Gao,Xian Gao,Ting Liu,Yuzhuo Fu*

Main category: cs.LG

TL;DR: 提出自适应秩分配(ARA)方法，解决SVD在LLM压缩中全局压缩比约束下的秩分配问题，通过专用掩码设计和额外损失函数实现更优性能。


<details>
  <summary>Details</summary>
Motivation: 现有SVD压缩方法存在局限性：启发式算法搜索空间受限，掩码训练难以有效捕捉奇异值谱与参数关系，且忽略增益函数在压缩比为1时的非光滑特性，导致训练陷入局部最优。
...
### [207] [Iterative Training of Physics-Informed Neural Networks with Fourier-enhanced Features](https://arxiv.org/abs/2510.19399)
*Yulun Wu,Miguel Aguiar,Karl H. Johansson,Matthieu Barreau*

Main category: cs.LG

TL;DR: 提出了IFeF-PINN算法，通过傅里叶特征增强来克服PINNs中的频谱偏差问题，在经典基准问题上表现出优于现有算法的性能。


<details>
  <summary>Details</summary>
Motivation: 解决物理信息神经网络(PINNs)中的频谱偏差问题，即神经网络倾向于先学习低频特征，难以准确逼近高频偏微分方程。

Method: 使用随机傅里叶特征增强潜在空间，创建两阶段训练：在特征空间中估计基函数，然后进行回归确定增强基函数的系数。

Result: 对于线性模型证明了迭代训练方案的收敛性，并通过数值评估显示该方法在经典基准问题上优于现有算法，在整个频域具有改进的逼近能力。

Conclusion: IFeF-PINN通过傅里叶特征增强有效克服了PINNs的频谱偏差问题，能够准确逼近高频PDEs，在多个基准测试中表现出优越性能。
...
### [208] [FairNet: Dynamic Fairness Correction without Performance Loss via Contrastive Conditional LoRA](https://arxiv.org/abs/2510.19421)
*Songqi Zhou,Zeyuan Liu,Benben Jiang*

Main category: cs.LG

TL;DR: FairNet是一个动态的实例级公平性校正框架，通过偏置检测器和条件LoRA实现选择性公平校正，在保持整体性能的同时提升最差群体的表现。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法通常牺牲性能、使用静态校正策略、难以处理数据稀疏问题，且在敏感属性利用上存在不足。

Method: 集成偏置检测器和条件低秩适应(LoRA)，仅对识别为偏置的实例激活公平校正机制，使用对比损失函数减少跨敏感群体的类内表示差异。

Result: 理论分析表明在适当检测器性能下，FairNet能提升最差群体性能而不降低整体性能，甚至可能带来轻微性能提升。多基准测试验证了其有效性。

Conclusion: FairNet提供了一种灵活处理不同敏感属性标签场景的动态公平校正方法，在保持性能的同时有效提升公平性。
...
### [209] [LLM Unlearning with LLM Beliefs](https://arxiv.org/abs/2510.19422)
*Kemou Li,Qizhou Wang,Yue Wang,Fengpeng Li,Jun Liu,Bo Han,Jiantao Zhou*

Main category: cs.LG

TL;DR: 提出了一种新的语言模型遗忘方法，通过联合抑制目标响应和模型信念来对抗遗忘过程中的挤压效应，实现更彻底的遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法依赖梯度上升来降低特定目标响应的概率，但会导致概率质量重新分配到高似然区域，产生语义相关的重述，形成挤压效应，造成虚假遗忘。

Method: 提出引导框架，将挤压效应与模型自身高置信度生成（模型信念）联系起来，通过联合抑制目标响应和模型信念来实现更彻底的遗忘。包括BS-T（抑制高概率词元）和BS-S（移除整个高置信度生成）两种方法。

Result: 在多个基准测试和不同模型家族上的广泛实验证实了该方法的有效性，能够实现更彻底的遗忘同时保持实用性。
...
### [210] [Neural Variational Dropout Processes](https://arxiv.org/abs/2510.19425)
*Insu Jeon,Youngjin Park,Gunhee Kim*

Main category: cs.LG

TL;DR: 提出了一种新的贝叶斯元学习方法NVDPs，通过任务特定的dropout建模条件后验分布，使用低秩伯努利专家元模型进行内存高效的dropout率映射，能够在多任务少样本学习中快速重新配置共享神经网络。


<details>
  <summary>Details</summary>
Motivation: 学习推断条件后验模型是鲁棒元学习的关键步骤，需要解决少样本学习中的功能模糊性和不确定性。

Method: 基于任务特定dropout建模条件后验分布，使用低秩伯努利专家元模型进行dropout率映射，采用基于整个任务数据的新型先验来优化条件dropout后验的摊销变分推断。

Result: 在1D随机回归、图像修复和分类等少样本学习任务中，与其他元学习方法相比表现出优异性能。

Conclusion: NVDPs能够稳健地近似任务特定的dropout率，处理广泛的功能模糊性和不确定性，在少样本学习中表现卓越。
...
### [211] [Revisiting the Relation Between Robustness and Universality](https://arxiv.org/abs/2510.19427)
*M. Klabunde,L. Caspari,F. Lemmerich*

Main category: cs.LG

TL;DR: 重新审视Jones等人(2022)提出的修正普适性假设，发现在特定设置下确实存在高表示相似性，但结果在不同数据集间不一致。预测行为不会随鲁棒性增加而收敛，但可通过重新训练分类器实现更普适的预测行为。
...
Method: 在不同数据集上验证表示相似性，分析预测行为随鲁棒性的变化，通过重新训练分类器来改进预测普适性。

Result: 在特定设置下验证了高表示相似性，但结果在不同数据集间不一致；预测行为不会随鲁棒性增加而收敛；重新训练分类器可以改善预测普适性。
...
### [212] [g-DPO: Scalable Preference Optimization for Protein Language Models](https://arxiv.org/abs/2510.19474)
*Constance Ferragu,Jonathan D. Ziegler,Nicolas Deutschmann,Arthur Lindoulsi,Eli Bixby,Cradle ML Team*

Main category: cs.LG

TL;DR: g-DPO通过序列空间聚类和分组近似方法，解决了DPO在蛋白质语言模型训练中的可扩展性瓶颈，在保持性能的同时显著加速训练。
...
Method: g-DPO框架采用两种策略：(1)使用序列空间聚类修剪冗余训练对，同时保留训练信号；(2)通过分组近似分摊似然计算成本。

Result: 在三个蛋白质工程任务中，g-DPO保持了与标准DPO在计算和实验性能上的统计无差别性，同时训练速度提升了1.8到3.7倍，且随着数据集增大，加速效果更明显。

Conclusion: g-DPO有效解决了DPO的可扩展性瓶颈，在保持性能的同时显著加速训练，为大规模蛋白质工程应用提供了实用解决方案。
...
### [213] [A Concrete Roadmap towards Safety Cases based on Chain-of-Thought Monitoring](https://arxiv.org/abs/2510.19476)
*Julian Schulz*
...
Method: 提出两部分安全案例：(1)建立模型在无思维链时缺乏危险能力；(2)确保思维链启用的危险能力可被监控检测。分析监控性威胁（神经语言和编码推理），评估保持思维链忠实性的技术，探索从不可监控思维链中提取可监控思维链的可能性。

Result: 系统分析了神经语言和编码推理的三种形式（语言漂移、隐写术、异质推理）及其驱动因素。建立了预测市场来评估思维链监控安全案例的可行性。

Conclusion: 思维链监控为构建AI安全案例提供了有前景的路径，但需要解决监控性威胁并开发有效的监控技术。
...
### [214] [Graph Unlearning Meets Influence-aware Negative Preference Optimization](https://arxiv.org/abs/2510.19479)
...
<details>
  <summary>Details</summary>
Motivation: 现有的图遗忘模型虽然通过保持节点表示基本不变和使用梯度上升在遗忘集上实现遗忘，但由于梯度上升的快速发散速度导致模型效用在遗忘过程中急剧下降。

Method: 提出影响力感知消息传递函数来放大被遗忘边的影响力并减轻遗忘集与保留集之间的紧密拓扑耦合，使用基于移除的方法快速估计每条边的影响力，并提出拓扑熵损失来避免遗忘过程中局部结构的过度信息损失。
...
### [215] [ELUTQ: Efficient LUT-Aware Quantization for Deploying Large Language Models on Edge Devices](https://arxiv.org/abs/2510.19482)
*Xin Nie,Liang Dong,HaiCheng Zhang,JiaWang Xiao,G. Sun*

Main category: cs.LG

TL;DR: ELUTQ是一个高效的量化框架，引入分层线性量化(HLQ)格式，在CPU边缘设备上优化LLM部署，显著降低内存使用和延迟，消除反量化开销。


<details>
  <summary>Details</summary>
Motivation: 在CPU边缘设备上部署LLM面临内存和计算资源有限的挑战，现有均匀量化方法不适合权重分布且低比特位时反量化开销大。

Method: 提出HLQ量化格式，更好地捕捉权重统计特性而不增加位串行LUT GEMM操作的计算成本，可无缝集成到各种量化流程中，并提供优化的CPU内核。

Result: LLaMA3-8B在3位精度下困惑度降低约8%，2位精度下降低85%；2位LLaMA2-7B在Apple M2芯片上达到25+ tokens/s推理速度。

Conclusion: ELUTQ框架有效解决了边缘设备上LLM部署的内存和延迟瓶颈，在低比特量化下保持高性能，实现高效推理。
...
### [216] [Energy-Efficient and Dequantization-Free Q-LLMs: A Spiking Neural Network Approach to Salient Value Mitigation](https://arxiv.org/abs/2510.19498)
*Chenyu Wang,Zhanglu Yan,Zhi Zhou,Xu Chen,Weng-Fai Wong*

Main category: cs.LG

TL;DR: SpikeQuant是一种将脉冲神经网络与混合精度量化相结合的方法，通过二进制脉冲表示和IF机制实现高效能LLM部署，在W4A4量化下保持接近FP16的困惑度，同时能耗降低4.6倍。


<details>
  <summary>Details</summary>
Motivation: 解决传统量化方法的三个挑战：MAC操作能耗高、反量化引入额外开销、统一比特宽度会裁剪重要值。受SNN二进制脉冲表示和IF机制的启发，寻求更节能的LLM部署方案。

Method: 选择性对重要激活值应用混合精度量化，重新编码为二进制脉冲计数，实现动态混合存储；将量化尺度嵌入IF机制的阈值中，避免显式反量化操作。

Result: 在W4A4量化设置下，SpikeQuant能够持续实现接近FP16的困惑度，同时相比现有方法能耗降低高达4.6倍。

Conclusion: SpikeQuant通过结合SNN特性和混合精度量化，为准确且节能的LLM部署提供了有效解决方案，特别适合边缘设备应用。
...
### [217] [Teaming LLMs to Detect and Mitigate Hallucinations](https://arxiv.org/abs/2510.19507)
*Demian Till,John Smeaton,Peter Haubrick,Gouse Saheb,Florian Graef,David Berman*

Main category: cs.LG

TL;DR: 提出多模型一致性方法，通过结合不同LLM的响应来显著改进幻觉检测和缓解能力，同时降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有单模型一致性方法存在局限性，需要结合多个具有不同训练数据、训练方案和架构的LLM来进一步提升幻觉检测效果。

Method: 采用联盟一致性方法，从15个LLM池中组合多个模型团队，探索不同LLM组合的条件和效果。

Result: 多模型一致性方法在幻觉检测和缓解方面取得了实质性改进，且通常伴随推理成本的降低。

Conclusion: 多模型一致性方法优于单模型一致性方法，既能提升性能又能减少推理成本，为LLM幻觉问题提供了有效解决方案。
...
### [218] [From Prototypes to Sparse ECG Explanations: SHAP-Driven Counterfactuals for Multivariate Time-Series Multi-class Classification](https://arxiv.org/abs/2510.19514)
*Maciej Mozolewski,Betül Bayrak,Kerstin Bach,Grzegorz J. Nalepa*

Main category: cs.LG

TL;DR: 提出了一种基于原型的心电图分类模型反事实解释框架，通过SHAP阈值识别关键信号段，使用DTW和聚类提取原型，生成仅修改78%原始信号的反事实解释，在保持81.3%有效性的同时提升43%时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决时间序列AI模型可解释性挑战，特别是在医疗领域如心电图分类中，需要提供可操作且可解释的洞察。

Method: 使用SHAP阈值识别关键信号段并转换为区间规则，采用动态时间规整和聚类提取代表性原型，通过R峰对齐确保与待解释样本的一致性。

Result: 生成的反事实解释仅修改78%原始信号，所有类别平均有效性达81.3%，时间稳定性提升43%。心肌梗死类别有效性达98.9%，但肥厚型心肌病检测面临挑战（13.2%）。
...
TL;DR: 提出Bi-level Decision-Focused Causal Learning (Bi-DFCL)方法，解决在线平台营销中的预测-决策错位和偏差-方差困境问题，通过双层优化框架结合实验和观测数据，在美团平台成功部署。


<details>
  <summary>Details</summary>
Motivation: 传统两阶段方法存在预测-决策错位问题，且观测数据存在多种偏差，实验数据稀缺且成本高，导致高方差估计。

Method: 开发基于实验数据的无偏OR决策质量估计器，通过代理损失函数连接离散优化梯度；建立双层优化框架，通过隐式微分联合利用观测和实验数据。

Result: 在公共基准、工业营销数据集和大规模在线A/B测试中表现出显著改进，统计显著优于现有最优方法。

Conclusion: Bi-DFCL方法有效解决了营销优化中的关键挑战，实现了最优的偏差-方差权衡，已在美团平台成功应用。
...
TL;DR: 提出REBMBO方法，结合高斯过程局部引导和能量基模型全局结构信息，使用PPO算法进行自适应多步前瞻，有效克服传统贝叶斯优化的单步偏差问题。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯优化方法存在显著的单步偏差，容易收敛到局部最优，在复杂或高维任务中表现不佳。黑盒优化在科学和工程领域取得成功，特别是在函数评估成本高且梯度不可用的情况下。

Method: 将贝叶斯优化迭代定义为马尔可夫决策过程，集成高斯过程进行局部引导和能量基模型捕获全局结构信息，使用近端策略优化算法实现自适应多步前瞻。

Result: 在合成和真实世界基准测试中进行了广泛实验，证实REBMBO具有优越性能。不同高斯过程配置下的额外分析进一步突显了其适应性和鲁棒性。

Conclusion: REBMBO方法通过动态调整探索深度和方向，有效克服了传统贝叶斯优化方法的局限性，在复杂优化任务中表现出色。
...
Method: 利用LQR策略结构、灵敏度和稳定性分析，开发MED-LQ控制策略，将混淆实例原则和最小经验散度算法扩展到大规模设置。
...
TL;DR: 本文研究了联邦聚类方法在分布式分子数据上的表现，提出了化学信息评估指标SF-ICF，并通过大规模基准测试验证了领域知识在联邦多样性分析中的重要性。


<details>
  <summary>Details</summary>
Motivation: AI方法在药物发现中的应用受到公共数据集规模和多样性限制，联邦学习虽然能整合私有数据，但难以进行数据多样性评估和化学空间结构理解。

Method: 比较了三种联邦聚类方法（Fed-kMeans、Fed-PCA+Fed-kMeans、Fed-LSH）与其集中式对应方法在八个分子数据集上的表现，使用标准数学指标和新提出的SF-ICF化学指标进行评估。

Result: 大规模基准测试结合深入可解释性分析表明，通过化学信息指标和客户端可解释性分析来融入领域知识对于分子数据的联邦多样性分析至关重要。

Conclusion: 联邦聚类方法能够有效处理分布式分子数据，但需要结合领域特定的评估指标和可解释性分析来获得有意义的化学空间理解。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习在COVID-19预测中显示出价值，但在地方性传染病预测方面仍待探索，特别是RSV等疾病的长期预测。

Method: 基于XGBoost+CNN+BiLSTM的集成模型，结合高分辨率特征学习和长期时间依赖捕捉机制，通过自回归模块训练气候控制的滞后关系。

Result: 在美国34个州的评估中，ForecastNet-XCL在州内和跨州场景中均可靠地优于统计基线、单个神经网络和传统集成方法，在长期预测中保持准确性。

Conclusion: ForecastNet-XCL的效率、性能和不确定性感知设计使其成为可部署的早期预警工具，特别适用于气候压力增加和监测资源受限的情况。
...
TL;DR: DiffEvac是一种基于生成模型的建筑疏散模拟新方法，通过扩散模型学习疏散模式，显著提高了模拟效率和设计迭代速度。


<details>
  <summary>Details</summary>
Motivation: 传统疏散模拟依赖精细建模和大量参数，难以在早期设计阶段进行快速迭代。需要一种更高效的疏散模拟方法来支持快速设计过程。

Method: 建立399个不同功能布局和对应疏散热力图的数据集，提出解耦特征表示来嵌入物理特征，使用基于图像提示的扩散模型学习疏散模式。

Result: 相比现有条件GAN方法，DiffEvac在SSIM指标上提升37.6%，PSNR提升142%，速度提升16倍，模拟时间缩短至2分钟。

Conclusion: 该方法显著提升了快速设计迭代效率，为智能建筑安全优化提供了新思路，降低了建模负担，支持大规模假设探索，并能与多目标设计工具耦合。
...
TL;DR: 最小二乘法在现代机器学习中具有巨大潜力，通过自定义梯度将其转化为可微分算子，实现多种应用。
...
Method: 推导自定义梯度，将最小二乘求解器转化为可微分算子，类似神经网络层。

Result: 实证展示了三个应用：在5000万参数模型中强制权重稀疏性、在基于分数的生成模型中施加保守性约束、基于预测性能调整高斯过程超参数。
...
TL;DR: 提出了FVAE-LoRA方法，使用变分自编码器学习两个不同的潜在空间，一个用于任务显著特征，另一个用于残差信息，在文本、音频和图像任务上优于标准LoRA。


<details>
  <summary>Details</summary>
Motivation: 现有的LoRA变体缺乏显式区分任务相关信息与残差信息的机制，限制了在下游任务中的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 时变环境中处理序列的观察概率随预测时间范围呈指数下降，导致数据支持不足和严重重叠问题，现有元学习器在低重叠情况下估计方差爆炸。

Method: 开发了Neyman正交总体风险函数，最小化重叠加权的oracle风险，构建具有Neyman正交性的重叠加权正交元学习器，对干扰函数误设具有鲁棒性。

Result: WO学习器能够抵消现有元学习器的不稳定性，获得更可靠的异质处理效应估计，在transformer和LSTM骨干网络上的实验验证了其优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统策略学习算法在预测不确定时仍强制决策，这在高风险场景中存在风险。需要开发能够弃权的策略学习方法。

Method: 提出两阶段学习器：首先识别一组近似最优策略，然后基于这些策略的分歧构建弃权规则。使用双重稳健目标处理未知倾向得分情况。

Result: 在已知倾向得分时获得O(1/n)型快速后悔保证，在未知倾向得分时通过DR目标扩展这些保证。弃权机制还能改善边界条件下的性能。

Conclusion: 弃权是策略学习中的多功能工具，能够改善边界条件性能、连接分布鲁棒策略学习，并支持安全策略改进。
...
TL;DR: TraDy是一种内存高效的深度学习训练方法，通过动态随机通道选择和基于架构的层重要性分析，实现高稀疏度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模增大而部署环境资源受限，需要内存高效的训练方法。

Method: 提出动态随机通道选择方法，在预选层中跨epoch随机重采样通道，结合架构相关的层重要性分析。

Result: 在各种下游任务和架构上达到最先进性能，实现99%激活稀疏度、95%权重导数稀疏度，权重导数计算FLOPs减少97%。

Conclusion: TraDy在严格内存约束下实现了高效训练，为大规模模型部署提供了可行解决方案。
...
TL;DR: 提出分层推测解码(HSD)算法，通过构建多级草稿模型层次结构来加速Transformer语言模型的推理过程，相比单草稿模型基线可获得1.2倍加速。


<details>
  <summary>Details</summary>
Motivation: 传统推测解码使用单一草稿模型，但实践中存在多个潜在草稿模型选择，从快速但不准确到慢速但可靠不等。为了充分利用这些模型并进一步降低推理延迟。

Method: 将草稿模型堆叠成层次结构，每个模型提出token，下一个更大的模型在单次前向传播中验证它们，最终由目标模型验证token。推导了此类层次结构的期望延迟表达式，并证明可以在多项式时间内选择延迟最优的层次结构。

Result: HSD相比最佳单草稿基线可获得高达1.2倍的加速，证明该算法在减少生成延迟方面超越了以往技术。

Conclusion: 分层推测解码通过构建多级草稿模型层次结构，有效降低了Transformer语言模型的推理延迟，且能在多项式时间内找到最优配置，具有实际应用价值。
...
TL;DR: SEMPO是一个轻量级时间序列基础模型，通过能量感知频谱分解和混合提示Transformer，在减少预训练数据量和模型大小的同时，实现了强大的零样本和少样本预测性能。
...
Result: 在包含16个数据集的2个大规模基准测试中，SEMPO在零样本和少样本预测场景下均优于最先进方法，显著减少了预训练数据规模和模型大小。
...
TL;DR: 提出Enhanced TimeAutoDiff框架，通过合成ICU时间序列数据来训练和评估预测模型，显著减少真实数据与合成数据评估之间的差距，在细粒度人口亚组中提供更可靠的性能分析。


<details>
  <summary>Details</summary>
Motivation: 在重症监护领域，由于患者隐私和数据敏感性，获取足够真实数据来全面评估AI模型在人口亚组中的性能具有挑战性。需要一种隐私保护的方法来支持可信赖的细粒度模型评估。

Method: 基于先前的扩散和VAE生成器，引入Enhanced TimeAutoDiff，在潜在扩散目标中增加分布对齐惩罚项。在MIMIC-III和eICU数据集上进行24小时死亡率和住院时间预测任务的基准测试。

Result: Enhanced TimeAutoDiff将真实-合成评估差距减少70%以上，达到Δ_TRTS ≤ 0.014 AUROC，同时保持训练效用。在32个交叉亚组中，大型合成队列将亚组级AUROC估计误差减少达50%，在72-84%的亚组中优于小型真实测试集。

Conclusion: 该工作为重症监护领域提供了实用的隐私保护路线图，支持跨多样化患者群体的可信赖细粒度模型评估，无需暴露敏感电子健康记录数据，有助于提升医疗AI的整体可信度。
...
### [233] [A Survey on Cache Methods in Diffusion Models: Toward Efficient Multi-Modal Generation](https://arxiv.org/abs/2510.19755)
...
Method: 通过特征级跨步重用和层间调度机制，识别扩散过程中的内在计算冗余并进行重用，从静态重用发展到动态预测。

Result: 扩散缓存能够在不修改模型参数的情况下减少计算量，提高推理效率，并能与其他加速技术（如采样优化和模型蒸馏）集成。
...
### [234] [Statistical Inference for Linear Functionals of Online Least-squares SGD when $t \gtrsim d^{1+δ}$](https://arxiv.org/abs/2510.19734)
*Bhavya Agrawalla,Krishnakumar Balasubramanian,Promit Ghosal*

Main category: cs.LG

TL;DR: 本文建立了在线最小二乘SGD的线性泛函的非渐近Berry-Esseen界，在增长维度机制下提供高斯中心极限定理。相比现有方法需要t ≳ d^{3/2}次迭代，本文方法仅需t ≳ d^{1+δ}次，显著扩展了维度机制并提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 在关键应用中部署SGD需要对其内在不确定性进行严格量化，现有高维推理方法计算昂贵且维度扩展受限。

Method: 提出基于在线SGD的程序，开发在线方差估计器，并建立该估计器的高概率偏差界。

Result: 实现了在t ≳ d^{1+δ}次迭代下的CLT，运行时间为O(td)，内存需求为O(d)，相比协方差逆方法的O(td² + d³)显著更优。

Conclusion: 建立了第一个完全在线和数据驱动的框架，用于在接近最优的扩展机制t ≳ d^{1+δ}下为SGD迭代构建置信区间。
...
### [235] [BATIS: Bayesian Approaches for Targeted Improvement of Species Distribution Models](https://arxiv.org/abs/2510.19749)
*Catherine Villeneuve,Benjamin Akera,Mélisande Teng,David Rolnick*

Main category: cs.LG

TL;DR: BATIS是一个基于贝叶斯深度学习的物种分布模型框架，通过迭代更新先验预测来结合有限观测数据，有效处理空间数据偏差，提高在数据稀缺区域的可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在物种分布建模中受限于数据的空间偏差，无法有效结合局部观测与生态模式，需要更好的不确定性量化方法来提升模型可靠性。

Method: 提出BATIS框架，从贝叶斯视角重新审视深度物种分布模型，使用迭代更新方法结合先验预测和有限观测数据，并评估多种不确定性量化方法。

Result: 在包含eBird公民科学观测数据的新数据集上测试表明，贝叶斯深度学习方法能显著提高物种分布模型在数据稀缺位置的可靠性。

Conclusion: 贝叶斯深度学习可以大大提升物种分布模型的可靠性，特别是在数据稀缺区域，有助于生态理解和保护工作。
...
### [236] [When Do Transformers Learn Heuristics for Graph Connectivity?](https://arxiv.org/abs/2510.19753)
*Qilin Ye,Deqing Fu,Robin Jia,Vatsal Sharan*

Main category: cs.LG

TL;DR: 本文通过图连通性问题研究Transformer学习算法能力，发现模型容量限制导致其选择学习精确算法或简单启发式策略。


<details>
  <summary>Details</summary>
Motivation: Transformer模型经常学习脆弱的启发式方法而非通用算法，本文旨在从理论和实验角度解释这一现象。

Method: 使用简化解耦Transformer架构，分析其解决图连通性问题的能力，研究训练动态和模型容量对学习策略的影响。

Result: L层模型能解决直径不超过3^L的图问题，实现类似邻接矩阵幂运算的算法；训练数据在容量内时学习精确算法，超出时学习基于节点度的启发式方法。

Conclusion: 限制训练数据在模型容量范围内可使Transformer学习精确算法而非启发式方法，为提升模型算法泛化能力提供了指导。
...
### [237] [CONFEX: Uncertainty-Aware Counterfactual Explanations with Conformal Guarantees](https://arxiv.org/abs/2510.19754)
*Aman Bilkhoo,Milad Kazemi,Nicola Paoletti,Mehran Hosseini*

Main category: cs.LG

TL;DR: 提出CONFEX方法，使用共形预测和混合整数线性规划生成具有不确定性感知的反事实解释，提供局部覆盖保证和最优性保证。


<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法往往忽略预测不确定性，缺乏将不确定性纳入解释的机制，导致解释可能不可靠或误导。

Method: 结合共形预测和混合整数线性规划，通过离线树基分区开发局部化CP程序，实现高效MILP编码。

Result: 在多个基准测试和指标上评估，显示该方法能产生鲁棒且合理的解释。

Conclusion: CONFEX方法为反事实解释提供了严格的不确定性和最优性保证，解决了CFX生成违反可交换性的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有最先进的成员推理攻击需要训练大量计算昂贵的参考模型，限制了实用性。需要一种更高效的方法来评估模型隐私风险。

Method: 利用训练和测试分布的不对称性，通过简单损失攻击的真负率来估计模型级脆弱性，特别关注高损失区域异常值的缺失作为风险预测指标。

Result: 该方法在多种架构和数据集上都能准确估计模型对最先进MIA攻击(LiRA)的脆弱性，优于低成本攻击方法和其他分布差异度量方法。

Conclusion: 该方法为评估模型隐私风险提供了一种高效实用的替代方案，特别是在大型语言模型的风险评估方面显示出良好前景。
...
TL;DR: GaLLoP是一种基于梯度的稀疏微调方法，只微调具有最大梯度幅度和最小预训练幅度的参数，在保持性能的同时减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏微调技术的有效性取决于最优选择要微调的模型参数，需要一种能优先选择高任务相关性但最小化预训练知识破坏的参数的方法。

Method: GaLLoP基于梯度幅度选择参数，只微调在下游任务中梯度幅度最大且预训练幅度最小的参数。

Result: 在LLaMA3 8B和Gemma 2B上的实验表明，GaLLoP在分布内和分布外性能上持续优于或匹配其他参数高效微调技术，如LoRA、DoRA和SAFT。

Conclusion: GaLLoP能减轻灾难性遗忘和任务数据记忆，稳定性能，并在不同随机种子下稳健泛化。
...
TL;DR: DynaInfer是一种无需环境标签的新方法，通过分析固定神经网络的预测误差来推断环境规格，在无标签场景下有效解决交替优化问题，并在多种动态系统中优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 数据驱动方法依赖I.I.D.数据假设，现有泛化技术需要环境标签，但实际中由于数据采集挑战、隐私问题和环境变异性，环境标签往往不可用。

Method: 通过分析每个训练轮次中固定神经网络的预测误差来推断环境规格，直接从数据中进行环境分配，无需环境标签。

Result: DynaInfer在多种动态系统中优于现有环境分配技术，能快速收敛到真实标签，甚至在环境标签可用时也表现出更优性能。

Conclusion: DynaInfer提供了一种无需环境标签的有效解决方案，解决了数据驱动方法在现实场景中的泛化挑战。
...
TL;DR: 该论文提出了一种检测语言模型衍生模型是否源自特定训练模型的方法，通过测试模型输出与训练数据顺序的相关性来证明模型归属权。


<details>
  <summary>Details</summary>
Motivation: 研究如何证明黑盒衍生语言模型是否源自特定开源模型，解决模型归属权的可验证性问题。

Method: 将问题建模为独立性测试，利用训练数据随机排序的特性，通过相关性分析检测模型输出与训练数据顺序的关联。在查询设置中直接估计模型对训练数据的似然度，在观察设置中通过文本重叠度或重新训练模型来检测。

Result: 在查询设置中，对40多个Pythia和OLMo微调模型进行测试，p值大多低于1e-8。在观察设置中，第二种方法仅需几百个token就能可靠区分，第一种方法需要数十万个token。

Conclusion: 该方法能够有效检测语言模型的衍生关系，为模型归属权验证提供了可行的统计证据。
...
<details>
  <summary>Details</summary>
Motivation: 研究Transformer是否能够作为元学习器，仅使用少量上下文示例适应新任务，而无需进一步训练。

Method: 使用简化的Transformer架构，在类条件高斯混合模型的任务族上进行训练，其中均值向量位于共享的k维子空间中。

Result: 训练后，Transformer仅需O(k/R^4)个上下文示例就能泛化到新任务，性能接近知道共享子空间的最优学习器，显著优于仅使用上下文数据的学习器。

Conclusion: Transformer可以作为有效的元学习器，其所需的训练任务和每任务示例数量与环境维度d无关。
...
TL;DR: 该研究分析了巴西和墨西哥在硬件受限条件下训练10万亿token语言模型的技术和经济可行性，发现H100配置成本为800-1400万美元，A100配置成本为1900-3200万美元，建议通过延长训练时间来缓解硬件约束。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练的计算需求加剧了全球南北之间的结构性不对称，研究旨在探索全球南方国家在硬件、能源和财政约束下实现技术主权的可行性。

Method: 采用双轴设计，比较不同加速器（H100 vs A100）和训练时长（90天 vs 150天），评估计算需求、能耗、资本支出和监管兼容性。

Result: 所有配置都低于出口管制和电力基础设施阈值，H100方案成本为800-1400万美元，A100方案为1900-3200万美元，硬件效率决定财政可行性。

Conclusion: 延长训练时间应作为政策工具来缓解硬件约束，使中等收入国家能够建立可持续且战略充足的AI能力，生产可用、可审计且本地对齐的模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统的世界模型通过预测未来像素帧来规划，但像素重建目标与规划目标不一致，强像素重建不一定对应好的规划决策。

Method: 将世界建模转化为视觉问答问题，使用视觉语言模型作为语义世界模型，通过监督微调在图像-动作-文本数据上训练。

Result: 该方法在开放端机器人任务上实现了策略改进，相比基于重建的世界模型获得了显著泛化性能提升。

Conclusion: 语义世界模型通过预测任务相关语义信息而非像素，能更好地支持决策规划，并继承预训练视觉语言模型的泛化和鲁棒性。
...
### [245] [Test-time Verification via Optimal Transport: Coverage, ROC, & Sub-optimality](https://arxiv.org/abs/2510.18982)
*Arpan Mukherjee,Marcello Bullo,Debabrota Basu,Deniz Gündüz*

Main category: cs.AI

TL;DR: 本文提出了一个统一框架，将可验证的测试时缩放建模为传输问题，揭示了生成器覆盖率、验证器收敛区域和采样算法次优性之间的几何交互关系。


<details>
  <summary>Details</summary>
Motivation: 虽然测试时缩放与验证已显示出提升大语言模型性能的潜力，但验证器的作用及其不完善性仍未得到充分探索。现有研究只捕捉了这些因素的部分子集，缺乏量化它们交互几何的统一框架。

Method: 将可验证的测试时缩放构建为传输问题，分析覆盖率、收敛区域和次优性之间的相互作用。提出并分析两类采样算法——顺序和批量采样，考察其计算复杂度如何影响这些权衡。

Result: 发现次优性-覆盖率曲线呈现三个区域：传输区域（次优性随覆盖率增加而增加）、策略改进区域（次优性可能随覆盖率减少，取决于验证器的收敛区域）和饱和区域（次优性趋于平稳，不受覆盖率影响）。在Qwen、Llama和Gemma模型上的实证结果验证了理论发现。

Conclusion: 通过传输问题框架揭示了验证器在测试时缩放中的关键作用，为理解覆盖率、收敛区域和采样次优性之间的复杂交互提供了理论基础，并指导了采样算法的设计选择。
...
### [246] [Timely Clinical Diagnosis through Active Test Selection](https://arxiv.org/abs/2510.18988)
*Silas Ruhrberg Estévez,Nicolás Astorga,Mihaela van der Schaar*

Main category: cs.AI

TL;DR: ACTMED是一个结合贝叶斯实验设计和大型语言模型的临床诊断框架，通过自适应测试选择来优化诊断准确性、可解释性和资源使用。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习诊断方法依赖静态数据集，无法模拟临床医生在实践中的顺序性、资源感知推理，特别是在高压或资源有限环境中需要及时且经济有效的决策支持。

Method: 整合贝叶斯实验设计与大型语言模型，在每一步选择能最大程度减少诊断不确定性的测试，LLM作为灵活模拟器生成患者状态分布并支持信念更新，无需结构化任务特定训练数据。

Result: 在真实世界数据集上评估显示，ACTMED能够优化测试选择，提高诊断准确性、可解释性和资源利用效率。

Conclusion: 这是向透明、自适应且与临床医生对齐的诊断系统迈出的一步，能够在减少对领域特定数据依赖的情况下跨环境泛化。
...
### [247] [Rectifying Shortcut Behaviors in Preference-based Reward Learning](https://arxiv.org/abs/2510.19050)
*Wenqian Ye,Guangtao Zheng,Aidong Zhang*

Main category: cs.AI

TL;DR: 本文提出PRISM方法来解决基于人类偏好的奖励模型中的捷径行为问题，通过学习组不变核函数来减少对虚假特征的依赖，提高奖励模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基于人类偏好的奖励模型容易受到奖励破解和泛化能力差的影响，它们通过利用训练数据中的虚假特征（如回答冗长、讨好语气等）来获得高奖励分数，而非真正反映预期目标。

Method: 受核视角不变性理论启发，提出PRISM方法，通过闭式学习目标学习具有特征映射的组不变核函数，以缓解偏好奖励学习中的捷径行为。
...
TL;DR: MUSE基准测试揭示当前多模态大语言模型在音乐关系推理方面存在严重缺陷，与人类专家存在显著差距，而思维链提示效果不稳定甚至有害。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的音频理解评估可能掩盖了其在关系推理方面的基本弱点，需要更深入评估其音乐感知能力。

Method: 开发了MUSE基准测试，包含10个任务来探测基础音乐感知技能，评估了4个SOTA模型并与200人的人类基线进行对比。

Result: SOTA模型能力差异很大，与人类专家存在持续差距。Gemini Pro在基础感知上表现良好，但Qwen和Audio Flamingo 3表现接近随机水平，思维链提示效果不稳定且通常有害。

Conclusion: MUSE基准为评估不变音乐表征提供了关键工具，将推动开发更鲁棒的AI系统。
...
### [249] [A Multi-faceted Analysis of Cognitive Abilities: Evaluating Prompt Methods with Large Language Models on the CONSORT Checklist](https://arxiv.org/abs/2510.19139)
*Sohyeon Jeon,Hyung-Chul Lee*

Main category: cs.AI

TL;DR: 本研究评估了大型语言模型在根据CONSORT标准评估临床试验报告方面的表现，发现模型在推理风格、不确定性表达和替代解释方面存在明显差异，突显了当前系统在临床合规自动化方面的局限性。
...
Conclusion: 结果强调了当前系统在临床合规自动化方面的局限性，并突出了理解其认知适应和策略行为对于开发更可解释和可靠的医疗AI的重要性。
...
### [250] [The Zero-Step Thinking: An Empirical Study of Mode Selection as Harder Early Exit in Reasoning Models](https://arxiv.org/abs/2510.19176)
*Yuqiao Tan,Shizhu He,Kang Liu,Jun Zhao*

Main category: cs.AI

TL;DR: 本文研究了推理模型中的模式选择问题，将其视为早期退出的更具挑战性变体，旨在通过零步思维在推理开始前决定使用长链思维还是短链思维模式，以减少计算开销。
...
Method: 将模式选择识别为早期退出问题的变体，通过零步思维在推理过程开始前做出决策。在九个基线方法上进行实证研究，比较基于提示的方法和利用内部信息的方法。

Result: 基于提示的方法由于分类能力有限且仅提供少量手工制作信息而经常失败。利用内部信息的方法在大多数场景中表现更好，但仍存在稳定性问题。
...
### [251] [WebGraphEval: Multi-Turn Trajectory Evaluation for Web Agents using Graph Representation](https://arxiv.org/abs/2510.19205)
...
TL;DR: WebGraphEval是一个将多个智能体的网页交互轨迹抽象为统一加权动作图的框架，支持多路径、跨智能体和效率感知的评估，超越了传统的二元成功指标。
...
Method: 将多个智能体的轨迹抽象为统一的加权动作图，兼容WebArena等基准，无需修改环境。框架规范编码动作，合并重复行为，应用奖励传播和成功加权边统计等结构分析。

Result: 在六个网页智能体的数千条轨迹评估中，图抽象捕捉了跨模型规律性，突出了冗余和低效行为，识别了基于结果指标忽略的关键决策点。
...
### [252] [ChatGPT Unveils Its Limits: Principles of Law Deliver Checkmate](https://arxiv.org/abs/2510.19261)
*Marianna Molinari,Ilaria Angela Amantea,Marinella Quaranta,Guido Governatori*

Main category: cs.AI

TL;DR: 本研究通过实验评估ChatGPT在法律领域的表现，与正则表达式基准对比，发现ChatGPT即使具备必要知识也无法整合推理得出全面结果，揭示了其在复杂问题分解和综合解决能力上的重大局限。


<details>
  <summary>Details</summary>
Motivation: 评估ChatGPT在法律领域的能力，特别是提取法律原则关键段落的任务，通过与正则表达式基准对比而非仅与人类表现对比，揭示其推理能力的根本局限。

Method: 在法律领域进行实验，将ChatGPT的表现与基于正则表达式的基准方法进行对比，分析其在法律决策中提取关键法律原则段落的能力。

Result: ChatGPT即使拥有必要的知识和能力，也无法有效整合和推理以产生全面结果，在复杂问题分解和多能力综合解决方面存在重大缺陷。

Conclusion: 真正的智能需要分解复杂问题并根据多种所需能力进行综合解决的能力，在法律领域，这种全面理解和推理能力仍然是人类独有的特质。
...
### [253] [An Argumentative Explanation Framework for Generalized Reason Model with Inconsistent Precedents](https://arxiv.org/abs/2510.19263)
...
<details>
  <summary>Details</summary>
Motivation: 传统案例推理假设先例必须一致，而广义理由模型放宽了这一假设。虽然已有基于传统一致理由模型的论证解释方法，但缺乏针对容纳不一致先例的广义推理框架的相应论证解释方法。

Method: 扩展推导状态论证框架(DSA-framework)，以解释基于广义理由模型的推理过程。

Result: 开发了能够解释广义理由模型推理的论证框架扩展。

Conclusion: 成功扩展了DSA框架，为处理不一致先例的广义理由模型提供了论证性解释能力。
...
### [254] [Learning to Make Friends: Coaching LLM Agents toward Emergent Social Ties](https://arxiv.org/abs/2510.19299)
*Philipp J. Schneider,Lin Tian,Marian-Andrei Rizoiu*

Main category: cs.AI

TL;DR: 本文提出了一个多智能体LLM模拟框架，通过行为奖励函数和上下文学习来研究LLM智能体是否能重现人类在线社交动态。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型智能体是否能重现人类在线行为的复杂社交动态，包括同质性、互惠性和社会验证等特征，以及什么记忆和学习机制促使这些动态出现。

Method: 设计了一个多智能体LLM模拟框架，智能体通过重复互动、相互评估和行为适应，结合行为奖励函数和上下文学习来模拟人类社交行为。
...
### [255] [Continual Knowledge Adaptation for Reinforcement Learning](https://arxiv.org/abs/2510.19314)
*Jinwu Hu,Zihao Lian,Zhiquan Wen,Chenghao Li,Guohao Chen,Xutao Wen,Bin Xiao,Mingkui Tan*

Main category: cs.AI

TL;DR: 提出CKA-RL方法解决持续强化学习中的灾难性遗忘和知识利用效率问题，通过持续知识适应策略和自适应知识合并机制，在三个基准测试中性能提升4.20%，前向迁移提升8.02%。
...
### [256] [MSC-Bench: A Rigorous Benchmark for Multi-Server Tool Orchestration](https://arxiv.org/abs/2510.19423)
*Jia-Kai Dong,I-Wei Huang,Chun-Tin Wu,Yi-Tien Tsai*

Main category: cs.AI

TL;DR: MSC-Bench是一个大规模基准测试，用于评估LLM代理在分层MCP生态系统中的多跳端到端工具编排能力，通过五级课程设计系统测试代理从单工具编排到复杂跨服务器规划的能力。
...
Method: 通过构建'等函数集'作为真实基准，使用F1分数等客观指标，减少对LLM作为评判的依赖。采用五级课程设计，从单工具编排到复杂跨服务器规划，包括对超出范围请求的鲁棒性测试。

Result: 实验表明，刚性层次结构在没有协同设计策略时会阻碍性能，即使最先进的代理在鲁棒性方面也存在系统性弱点。

Conclusion: MSC-Bench提供了一个诊断框架，能够暴露现有代理的局限性，并指导开发更强大、更高效的工具使用代理。基准测试和资源已公开可用。
...
### [257] [NeSyPr: Neurosymbolic Proceduralization For Efficient Embodied Reasoning](https://arxiv.org/abs/2510.19429)
*Wonje Choi,Jooyoung Kim,Honguk Woo*

Main category: cs.AI

TL;DR: NeSyPr是一个新颖的具身推理框架，通过神经符号程序化将符号知识编译为可组合的过程表示，使语言模型能够在单步推理中执行复杂推理，适合延迟敏感和资源受限的物理系统。


<details>
  <summary>Details</summary>
Motivation: 解决在动态环境中为具身任务采用语言模型时面临的挑战，包括大规模推理引擎或符号规划器的在线访问受限于延迟、连接性和资源限制的问题。
...
### [258] [DAIL: Beyond Task Ambiguity for Language-Conditioned Reinforcement Learning](https://arxiv.org/abs/2510.19562)
*Runpeng Xie,Quanwei Wang,Hao Hu,Zherui Zhou,Ni Mu,Xiyun Li,Yiqin Yang,Shuang Xu,Qianchuan Zhao,Bo XU*

Main category: cs.AI

TL;DR: 提出DAIL方法解决语言指令的模糊性问题，通过分布策略和语义对齐来增强任务可区分性


<details>
  <summary>Details</summary>
Motivation: 自然语言指令的灵活性导致语言条件任务中存在显著歧义，严重降低算法性能

Method: DAIL方法包含两个关键组件：分布策略（通过值分布估计机制增强任务可区分性）和语义对齐模块（捕捉轨迹与语言指令的对应关系）

Result: 在结构化和视觉观察基准测试上的广泛实验结果表明，DAIL有效解决了指令歧义问题，性能优于基线方法

Conclusion: DAIL方法通过分布对齐学习有效处理语言指令的模糊性，提升了智能代理在语言条件任务中的表现
...
### [259] [HSCodeComp: A Realistic and Expert-level Benchmark for Deep Search Agents in Hierarchical Rule Application](https://arxiv.org/abs/2510.19631)
*Yiqian Yang,Tian Lan,Qianghuai Jia,Li Zhu,Hui Jiang,Hang Zhu,Longyue Wang,Weihua Luo,Kaifu Zhang*

Main category: cs.AI

TL;DR: HSCodeComp是首个评估深度搜索代理在分层规则应用能力的电商基准测试，要求代理根据商品描述预测10位协调制度编码(HSCode)，现有最佳代理准确率仅46.8%，远低于人类专家的95.0%。


<details>
  <summary>Details</summary>
Motivation: 当前代理基准测试忽视了代理应用复杂规则（如法律条款、医疗手册、关税规则）的关键能力，这些规则具有模糊边界和隐含逻辑关系，对代理构成挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有的城市感知系统在多样化城市场景中的泛化能力有限，决策过程缺乏可解释性，需要开发能够适应动态条件并提供透明决策解释的系统。
...
Conclusion: AgentSense代表了向部署自适应和可解释的城市感知系统迈出的重要进展，为基于网络的城市管理提供了有效的解决方案。
...
TL;DR: 开发了一个基于图论的引擎，为吉他学生提供和弦音即兴演奏建议，通过构建和弦音琶音图并寻找最优路径来生成和弦音独奏线。


<details>
  <summary>Details</summary>
Motivation: 和弦音即兴演奏是爵士吉他理论的基础练习，但学习和实践难度较大，需要专门的工具来辅助学生练习。

Method: 首先生成和弦音琶音，构建加权图（节点代表和弦琶音，边权重基于最优过渡音计算），然后寻找最短路径并重构和弦音独奏线。

Result: 开发了一个用户友好的系统，能够处理输入输出，帮助吉他学生练习和弦音即兴演奏。

Conclusion: 该基于图论的引擎为吉他学生提供了有效的和弦音即兴演奏练习工具，简化了学习过程。
...
Method: 采用可解释的胜率预测分类方法，在流式数据中使用多个滑动窗口控制输入数据，以捕捉游戏中的相关变化。
...
TL;DR: RLIE是一个集成大语言模型与概率建模的统一框架，通过四个阶段学习加权规则集，在真实数据集上验证了直接使用学习权重优于将规则注入LLM的方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法忽视规则间的相互作用，且未充分利用LLM与概率规则学习的结合潜力，需要开发更可靠的神经符号推理方法。

Method: 四阶段框架：1) LLM生成和过滤候选规则；2) 逻辑回归学习概率权重；3) 基于预测误差迭代优化规则集；4) 评估加权规则集作为直接分类器的性能。

Result: 直接使用学习权重的规则集表现更优，而将规则、权重和逻辑模型输出注入LLM反而降低准确性，表明LLM擅长语义生成但概率整合能力有限。
...
TL;DR: Memo是一个基于transformer的架构和训练方法，用于解决强化学习中记忆密集型、长时程任务的记忆创建和检索问题，通过插入周期性总结标记来压缩视觉输入，提高计算和存储效率。


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的具身智能体策略训练中，视觉输入经常超出transformer的上下文限制，而人类能够将终身经验压缩为记忆进行利用。现有方法要么依赖固定大小的循环记忆，要么完全依赖完整上下文。

Method: Memo在训练过程中将周期性总结标记与模型输入交错，实现记忆的创建和检索。这种方法能够压缩不相关的输入信息，形成抽象记忆。

Result: 在网格世界元强化学习基准和照片级真实室内环境的多目标导航任务中，Memo优于简单的长上下文transformer基线，同时计算和存储效率更高。Memo在推理时对更长上下文具有更好的泛化能力，在流式设置中保持鲁棒性。

Conclusion: Memo提供了一种有效的记忆机制，使具身智能体能够在长时间范围内保持环境上下文，解决了transformer在长时程任务中的上下文限制问题。
...
TL;DR: 论文介绍了"错位悬赏"项目，收集了295个AI系统与人类意图不符的案例，最终评选出9个获奖案例。


<details>
  <summary>Details</summary>
Motivation: 收集AI系统与人类意图不符的清晰、可复现案例，以识别和解决AI系统的安全问题。

Method: 通过众包项目"错位悬赏"收集案例，使用明确的评估标准对295个提交进行评审。

Result: 从295个提交中评选出9个获奖案例，展示了AI系统追求非预期或不安全目标的具体情况。

Conclusion: 该项目成功收集了AI系统错位的实证案例，为理解和解决AI安全问题提供了重要参考。
...
TL;DR: PROBE是一个评估LLM智能体主动性的新基准，通过搜索未指定问题、识别具体瓶颈和执行解决方案三个核心能力来分解主动性评估。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM智能体主动性的基准局限于局部上下文，无法测试跨来源和长时间跨度的推理能力，需要更全面的评估框架。

Method: 将主动性分解为三个核心能力管道：(1) 搜索未指定问题，(2) 识别具体瓶颈，(3) 执行适当解决方案，并应用PROBE评估领先的LLM和智能体框架。

Result: 即使是前沿模型也难以解决该基准，GPT-5和Claude Opus-4.1的最佳端到端性能仅为40%，揭示了自主行动能力的当前局限性。
...
TL;DR: WorldTest是一个评估模型学习智能体的协议，将无奖励交互与不同环境中的评分测试阶段分离，支持多种未知任务，并比较了人类与前沿模型在AutumnBench基准上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型的学习和评估方法偏离了目标：训练和评估都锚定在下一帧预测上，成功标准是在同一环境中最大化奖励。需要一种更开放、支持多种下游任务的评估方法。

Method: 提出WorldTest协议，包含无奖励探索、衍生测试和行为评分三个环节。实例化为AutumnBench基准套件，包含43个交互式网格世界环境和129个任务，涵盖掩码帧预测、规划和因果动态变化预测三个类别。

Result: 比较了517名人类参与者和三个前沿模型，发现人类表现优于模型，计算规模扩展只在某些环境中提升性能而非所有环境。

Conclusion: WorldTest提供了一个评估智能体对环境动态学习情况的新模板，AutumnBench揭示了世界模型学习方面存在显著改进空间。
...
### [268] [Visually Comparing Graph Vertex Ordering Algorithms through Geometrical and Topological Approaches](https://arxiv.org/abs/2510.19009)
*Karelia Salinas,Victor Barella,Thales Viera,Luis Gustavo Nonato*

Main category: cs.GR

TL;DR: 该论文提出了一个可视化辅助工具，用于评估图顶点排序技术，特别关注城市分析应用。通过整合现有和新提出的指标，该工具帮助用户选择合适的顶点排序方法、调整超参数，并识别排序失真严重的区域。


<details>
  <summary>Details</summary>
Motivation: 图顶点排序在空间数据分析中广泛应用，特别是在城市分析中，但现实世界图的结构复杂性常常引入失真。现有评估指标通常关注全局质量，难以识别局部失真，而可视化评估能够帮助分析人员比较方法、评估失真并识别异常区域。

Method: 开发了一个可视化辅助工具，整合现有和新提出的指标来评估几何和拓扑排序方法。使用城市街道图进行实验验证，并在多个城市数据集上进行测试。

Result: 实验结果表明，所提出的方法有效支持用户选择合适的顶点排序技术、调整超参数，并识别出排序失真较高的区域。

Conclusion: 该可视化辅助工具为顶点排序技术的评估提供了有效支持，特别适用于城市分析场景，能够帮助用户做出更好的技术选择和参数调优决策。
