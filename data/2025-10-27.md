...
Motivation: 现有方法难以同时生成具有精细几何、准确关节运动和真实外观的3D物体，特别是在处理关节依赖的可见性变化时存在挑战。

Method: 联合建模部件几何和关节动力学，使用变分自编码器将稀疏体素表示和关节属性嵌入统一潜空间，训练潜扩散模型进行采样，并引入关节感知的高斯解码器来重建逼真3D形状。

Result: 在PartNet-Mobility和ACD数据集上的实验表明，ArtiLatent在几何一致性和外观保真度方面优于现有方法，能够生成具有物理合理性的多样化3D物体。

Conclusion: ArtiLatent为关节化3D物体合成和操作提供了可扩展的解决方案，显著提升了跨关节配置的视觉真实感。
...
<details>
  <summary>Details</summary>
Motivation: 纯IMU方法存在局限性：惯性线索本质上是自参考的，无法提供他人的直接空间参考，导致平移估计和个体间精确定位不准确。需要结合其他技术来克服这些限制。

Method: 利用稀疏可穿戴传感器之间的距离（个体内部和跨个体），通过UWB测距估计绝对距离，将距离信息与惯性观测融合到结构化状态空间模型中，集成时间运动模式进行3D姿态估计。采用新颖的两步优化方法利用估计距离准确跟踪全局轨迹。

Result: 在合成和真实世界数据评估中，Group Inertial Poser在准确性和鲁棒性方面优于先前最先进的方法。创建了首个IMU+UWB双人跟踪数据集GIP-DB，包含14名参与者的200分钟运动记录。

Conclusion: IMU+UWB组合在野外多人运动捕捉方面展现出巨大潜力，为克服纯IMU方法的局限性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法面临几何和外观不一致、难以扩展以及对象中心化等问题，需要开发能够生成无限扩展且结构一致的3D场景的方法。

Method: 采用分层框架，包括高质量场景块数据提取、3D块修复机制实现上下文感知的场景扩展，以及从粗到细的生成策略确保全局布局合理性和局部细节保真度。

Result: 在3D-FRONT数据集上实现最先进的几何重建性能，支持无限场景生成，输出具有照片级真实感和结构一致性。

Conclusion: WorldGrow展示了构建大规模虚拟环境的能力，为未来世界模型的构建提供了潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有适配器训练方法容易将目标属性与姿势、表情、光照等偶然因素纠缠，限制了泛化能力和文本提示的遵循度。

Method: 在适配器训练期间，通过ControlNet或LoRA等辅助模块为混杂因素建立捷径通道，消除适配器内部化这些因素的动力，推理时移除辅助模块。

Result: 应用于面部和全身身份注入任务时，该方法提高了生成质量、多样性和提示遵循度。
...
Method: VAP 将问题重新定义为上下文生成，利用参考视频作为直接语义提示，通过即插即用的混合变换器专家引导冻结的视频扩散变换器，采用时间偏置位置嵌入消除虚假映射先验。

Result: VAP 在开源方法中达到了新的最先进水平，获得了38.7%的用户偏好率，可与领先的条件特定商业模型相媲美，并展示了强大的零样本泛化能力。

Conclusion: VAP 在通用可控视频生成方面取得了显著进展，支持各种下游应用，标志着向通用目的可控视频生成的重要进步。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割对临床诊断和治疗规划至关重要，但传统卷积神经网络难以捕捉全局上下文信息和长程依赖关系，限制了其对复杂边界和多种尺寸结构的精确分割能力。

Method: 结合卷积和transformer架构，采用焦点调制注意力机制增强上下文感知，引入双向特征融合模块实现编码器和解码器跨尺度表示的高效交互。

Result: 在8个公开数据集上的实验表明，FM-BFF-Net在Jaccard指数和Dice系数上持续超越现有最先进方法，验证了其有效性和适应性。

Conclusion: FM-BFF-Net通过融合卷积和transformer的优势，有效解决了医学图像分割中的全局上下文建模和边界精确分割问题，在多种医学成像场景中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前判别式模型在点跟踪中只能回归到均值或众数，无法捕捉轨迹的多模态特性，特别是在视觉遮挡和外观变化导致的不确定性情况下。

Method: 提出生成式点跟踪器(GenPT)，使用流匹配公式结合判别式跟踪器的迭代优化、跨窗口一致性的窗口相关先验，以及针对点坐标调整的方差调度。

Result: 在PointOdyssey、Dynamic Replica和TAP-Vid基准测试中评估，在遮挡点跟踪上达到最先进水平，同时在可见点跟踪上保持竞争力。

Conclusion: GenPT能够捕捉点轨迹的多模态特性，显著提升了遮挡点跟踪的准确性，同时保持了可见点跟踪的竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 当前的视觉语言模型难以在3D医学图像中定位解剖区域并进行逐步推理，这限制了模型输出与临床诊断工作流程的对齐，影响了临床医生与AI的信任合作。

Method: 创建了3DReasonKnee数据集，每个样本包含：3D MRI体积、针对特定解剖区域的诊断问题、定位相关解剖结构的3D边界框、临床医生生成的详细3D推理步骤、相关解剖区域的结构化严重程度评估。

Result: 建立了ReasonKnee-Bench评估基准，评估了五个最先进的视觉语言模型在定位和诊断准确性方面的表现，为ReasonKnee-Bench提供了基线性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法依赖光照和材质特性，存在局限性。热偏振成像不受这些因素影响，能够更好地处理透明、半透明和异质物体。

Method: 建立了偏振观测的一般理论，证明长波红外偏振成像不受可见光偏振分析模糊性影响，提出了基于多视角热偏振图像的3D形状恢复方法。

Result: 实验结果表明，该方法能有效重建透明、半透明和异质物体的精细细节，性能优于现有技术。

Conclusion: 热偏振成像为3D形状重建提供了新的有效途径，特别适用于传统方法难以处理的材料类型。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在分布偏移和标注稀缺场景下性能下降的问题，传统监督微调不可行，而自监督学习在视觉编码器模型中效果不佳。
...
Conclusion: VESSA能够有效利用无标注多视角对象视频进行视觉基础模型的自监督域适应，提升模型在目标域的鲁棒性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有6D位姿估计流程在工业环境中的杂乱、光照差和复杂背景等挑战条件下性能下降，检测成为关键瓶颈，需要提升检测可靠性。

Method: 基于当前SOTA基线，采用低光图像增强和基于开放词汇检测的基础模型引导的背景移除，抑制原始SAM输出的误报，为下游位姿估计提供更可靠的检测结果。

Result: 在真实世界工业抓取基准测试中，该方法显著提升了检测精度，同时推理开销可忽略不计。

Conclusion: 该方法在提升检测精度的同时保持了高效性，证明了其有效性和实用性。
...
Method: 基于深度学习框架，使用YOLOv11模型检测开裂、剥落和暴露钢筋，通过微调、数据增强和公开数据集测试，建立自动分类和损伤等级评估的混合框架。

Result: 开发了能够自动识别建筑物内外和结构组件，并可靠确定损伤等级的自动化框架，在多样化损伤场景中实现快速损伤检测。

Conclusion: 通过图像数据收集、标注和深度学习方法，证明了灾后快速自动化损伤检测在不同损伤情境下的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景图生成方法局限于单视图、不支持增量更新、缺乏3D空间几何定位，而这些对具身场景至关重要。

Method: 利用VLM推理生成丰富的2D场景图，通过深度信息将其定位到3D空间，节点包含开放词汇对象特征、3D位置和语义上下文，边捕获空间语义关系和对象间距离。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义分割网络大多采用简单的解码器，导致语义上下文和细粒度细节保留之间的权衡不佳。

Method: 提出WaveSeg解码器：1) 从输入图像学习高频分量作为显式先验；2) 双域操作(DDO)进行多尺度融合；3) 频谱分解注意力(SDA)块利用Mamba增强高频细节；4) 重参数化卷积在小波域保持低频语义完整性；5) 残差引导融合整合多尺度特征。

Result: 在标准基准测试中，WaveSeg在定量和定性评估上都优于最先进方法，实现了高效精确的分割。

Conclusion: WaveSeg通过结合小波域频率先验和基于Mamba的注意力机制，在语义分割任务中取得了优异性能，平衡了语义上下文和细节保留。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在神经丛分类任务中表现良好但缺乏可解释性，且不符合医生的决策方式。需要将专家知识融入模型，使其更符合临床实践。

Method: 使用对比语言-图像预训练模型，整合专家来源的文本概念（医学教科书和论文），通过大语言模型生成提示词，用QuiltNet编码，将临床相关语义线索与视觉特征对齐。

Result: 模型在各项分类指标上表现出优越的判别能力，准确率达83.9%，精确率86.6%，特异性87.6%，优于VGG-19、ResNet-18和ResNet-50等CNN模型。

Conclusion: 多模态学习在组织病理学中具有潜力，整合专家知识可以产生更符合临床需求的模型输出。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Retinex方法在大尺寸图像上处理耗时较长，需要一种更高效的增强方法。

Method: 定义直方图位置矩阵和计数矩阵，建立光照、反射率和低光照图像直方图之间的关系；构建两级优化模型，通过求解得到光照和反射率直方图的迭代公式；通过直方图匹配实现图像增强。

Result: 在1000*664分辨率图像上仅需1.86秒执行时间，比现有方法至少节省6.67秒，在可见性和性能指标上均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM主要在静态、完全可观察环境中研究视觉推理，而真实世界信息往往不完整。受人类主动探索环境能力的启发，需要开发能够主动获取和整合信息的智能体。

Method: 引入CLEVR-AVR仿真基准和AVR-152k大规模数据集，包含丰富的思维链标注。开发了PhysVLM-AVR模型，在马尔可夫决策过程中实现不确定性识别、信息增益预测和动作选择。

Result: PhysVLM-AVR在CLEVR-AVR、具身推理(OpenEQA、RoboVQA)和被动视觉推理(GeoMath、Geometry30K)任务上达到最先进性能。分析发现当前具身MLLM虽能检测信息不完整性，但难以通过交互主动获取和整合新信息。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法对垂直偏差、地面坡度和视角不匹配敏感，且缺乏对象身份信息；基于点云的神经网络方法内存需求大、需要完美预对齐、难以处理薄结构且忽略不确定性。

Method: 使用多分辨率NDT和点面ICP进行时期对齐，高度归一化，基于配准协方差和表面粗糙度计算检测水平，结合几何、语义和实例分割进行跨时期关联，采用类别约束二分图分配处理分裂合并。

Result: 在15个Subiaco区块上达到95.2%准确率、90.4% mF1和82.6% mIoU，优于Triplet KPConv方法，在Decreased类别上IoU达到74.8%，提升7.6个百分点。

Conclusion: 该方法能够有效处理城市规模LiDAR数据的变化检测，在保持高精度的同时处理分裂合并情况，对部分重叠和采样变化具有稳定性。
...
Method: 构建轻量级动态混合局部先验提取器，通过异构卷积捕获多样局部先验，使用门控网络动态输出专家先验。设计双向交互适配器，采用余弦对齐可变形注意力和通道自适应尺度增强来交互和重构特征。

Result: 在31个SOTA方法比较中表现出优异的分割性能，适应多种二元对象分割任务。

Conclusion: Controllable-LPMoE方法通过动态先验微调范式，在减少可训练参数的同时实现了高效微调和优秀的分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像安全数据集粗糙且模糊，仅提供广泛的安全标签而无法隔离驱动安全差异的具体特征，难以系统区分良性图像和有问题的图像。

Method: 利用图像编辑模型对图像进行针对性修改，仅改变与安全策略相关的特征从而翻转安全标签，同时保持安全无关细节不变，构建反事实图像对。

Result: SafetyPairs可作为强大的评估数据源，揭示视觉语言模型在区分细微差异图像方面的弱点，同时作为数据增强策略有效提高轻量级防护模型的样本效率。

Conclusion: SafetyPairs提供了首个系统研究细粒度图像安全差异的资源，包含9个安全类别的3020张图像对，为图像安全研究开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习框架在提升通用思维链推理时难以超越训练分布进行泛化

Method: 使用噪声注入探索策略（高斯噪声扰动视觉输入）和贝叶斯优势估计（将优势估计建模为贝叶斯推理问题）

Result: 在标准CoT质量、通用能力和幻觉基准测试中显著提升了泛化性和鲁棒性，特别是在小规模MLLMs上

Conclusion: NoisyGRPO通过系统性的多模态RL框架有效解决了MLLMs在CoT推理中的泛化问题
...
Result: 在定量指标上表现最佳（MAE: 156.28-165.12，PSNR: 20.27-20.71，SSIM: 0.98），血管分割Dice系数达0.70，血管体积ICC从0.70提升至0.81。

Conclusion: 该方法能有效生成高质量的DCCTPA图像，显著改善血管增强效果，特别是在小血管方面，为无对比剂肺血管疾病诊断提供了可行方案。
...
TL;DR: 提出了Spatial Intelligence Grid (SIG)框架，用于在基础模型中集成和验证空间智能，通过网格化结构编码物体布局和关系，分离空间能力与语言先验。


<details>
  <summary>Details</summary>
Motivation: 当前实践中常使用纯文本提示和VQA式评分来代理视觉空间智能(VSI)，但这会模糊几何信息、引入语言捷径，并削弱对真正空间技能的归因。

Method: 引入SIG结构化网格模式，明确编码物体布局、物体间关系和物理基础先验，作为文本的补充通道，为基础模型推理提供忠实、组合式的场景结构表示。

Result: 在少样本上下文学习中，SIG相比仅使用VQA表示，在所有VSI指标上都产生了更一致、更稳定和更全面的增益。发布了包含1.4K驾驶帧的SIGBench基准数据集。

Conclusion: SIG作为数据标注和训练模式，在学习和评估VSI方面具有前景，能够量化模型的内在空间智能并分离语言先验的影响。
...
Method: BFM将生成轨迹划分为多个时间片段，每个片段由专门的速度块建模；引入语义特征引导模块，使速度块基于预训练表示进行条件生成；提出轻量级特征残差近似策略，在保持语义质量的同时降低推理成本。

Result: 在ImageNet 256x256上的实验表明，BFM在保持相似生成性能的同时，实现了2.1倍到4.9倍的推理复杂度加速，建立了明显优于现有Flow Matching方法的Pareto前沿。

Conclusion: BFM通过块状设计和语义特征引导，有效解决了传统Flow Matching模型的局限性，在推理效率和生成质量之间取得了更好的平衡。
...
Method: 将token无关的文本空间扩展为一组正交子空间，基于语义亲和性动态分配每个token到子空间组合，将动态对齐建模为最优传输问题，通过top-k掩码稀疏化传输计划并专门化不同视觉区域的子空间。

Result: 大量实验证明了TokenCLIP的优越性。

Conclusion: TokenCLIP通过token-wise的动态对齐机制，有效提升了异常检测的准确性和细粒度语义捕捉能力。
...
Method: 使用图表示VQA样本，通过知识增强基准演化(KBE)框架，重构问题和扩展外部知识，实现难度可控的评估。
...
<details>
  <summary>Details</summary>
Motivation: 为了在细粒度食品图像检索任务中取得更好的性能，需要提升特征表示能力和检索精度。
...
Conclusion: 该方法在细粒度食品检索任务中表现优异，证明了多模型集成和新型重排序策略的有效性。
...
Conclusion: Arcface损失和Circle损失的适当组合能提升细粒度食品识别性能
...
<details>
  <summary>Details</summary>
Motivation: 解决shortcut模型在实际应用中的性能瓶颈问题，包括复合引导导致的图像伪影、固定引导缺乏灵活性、频率偏差、自一致性冲突和收敛困难等五个核心限制。

Method: 提出iSM统一训练框架，包含四个关键改进：内在引导机制提供动态控制，多级小波损失缓解频率偏差，缩放最优传输减少训练方差，双EMA策略平衡训练稳定性与自一致性。

Result: 在ImageNet 256×256数据集上的实验表明，该方法在单步、少步和多步生成任务中都显著优于基线shortcut模型，FID指标大幅提升。

Conclusion: iSM框架使shortcut模型成为可行且具有竞争力的生成模型类别，解决了长期存在的性能瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 实现高精度的token预测并支持并行生成，相比顺序自回归方法具有显著优势，通过同时观察所有网格token来提高效率和可控性。

Method: 采用解耦训练和混合推理，将生成过程分为拓扑雕刻和形状细化两个阶段；改进的沙漏架构结合双向注意力和旋转位置编码；引入连接损失作为拓扑约束。

Result: 在复杂数据集上的实验表明，TSSR能够生成高质量的3D艺术风格网格，支持高达10,000个面的生成，空间分辨率达到1024^3。

Conclusion: TSSR通过并行生成和创新的架构设计，在3D网格生成任务中实现了高效和高保真度的结果，为艺术风格3D内容创作提供了有力工具。
...
Method: 提出SAGE-3D方法：1)对象中心语义标注，为3DGS添加对象级细粒度标注；2)物理感知执行连接，嵌入碰撞对象并构建丰富的物理接口。

Result: 发布了包含1K对象标注3DGS室内场景数据的InteriorGS，以及首个基于3DGS的VLN基准测试SAGE-Bench(含2M VLN数据)。实验显示3DGS场景数据更难收敛但泛化性强，在VLN-CE Unseen任务上提升基线性能31%。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理高分辨率图像时，由于输入分辨率受限，难以精确理解和定位视觉细节，特别是在处理嵌入复杂背景中的超小物体时面临挑战

Method: 采用从粗到精的两阶段流程：全局语义探索(GSE)进行指令引导推理生成文本响应和粗略目标区域，局部感知细化(LPR)精炼该区域生成精确边界框和分割掩码，并引入定位知情回溯奖励来耦合两个阶段

Result: 在FineRS-4k和公共数据集上的实验结果表明，该方法在指令引导分割和视觉推理任务上持续优于最先进的MLLM方法

Conclusion: FineRS框架有效解决了MLLMs在高分辨率场景中处理极小物体的挑战，在分割精度和推理能力方面均表现出色
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型的多模态对齐组件可解释性研究不足，因为难以将多模态表示的语义映射到统一概念集中。

Method: 使用基于距离的编码器和两个模态特定解码器构建VL-SAE，通过余弦相似度显式对齐多模态表示，确保语义相似表示具有一致的神经元激活。

Result: 在多个VLM（如CLIP、LLaVA）上的实验表明，VL-SAE在解释和增强视觉-语言对齐方面具有优越能力，在下游任务中提升了零样本图像分类和幻觉消除的性能。

Conclusion: VL-SAE能够通过概念级对齐增强视觉-语言表示，为多模态对齐提供可解释性，并提升下游任务性能。
...
Result: FORM支持无条件形态合成和条件模拟扰动细胞状态，能预测下游信号活性、模拟组合扰动效应，并建模未见扰动的形态动态转换。
...
<details>
  <summary>Details</summary>
Motivation: 传统多尺度特征融合方法在复杂果园环境中面临挑战，无法充分处理局部和全局特征之间的关系，难以应对苹果叶病害的表型异质性和病变形态多样性。

Method: 使用CNN提取局部病变细节特征，Vision Transformer捕获全局结构关系，自适应特征融合模块（AFFM）动态融合特征，并采用多模态图像-文本学习方法，利用预训练CLIP权重实现视觉特征与病害语义描述的深度对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机跟踪方法主要关注速度而缺乏语义感知探索，这阻碍了搜索区域从模板中提取精确定位信息，导致在典型无人机跟踪挑战下性能不佳。
...
Conclusion: 该方法通过增强语义感知能力有效提高了无人机跟踪的准确性和鲁棒性，同时通过剪枝实现了速度优化，为不同计算资源提供了灵活的部署选择。
...
<details>
  <summary>Details</summary>
Motivation: 眼动信号包含注意力、短期意图和未来行动的有价值线索，是建模自我中心行为的强大信号。现有方法要么仅使用视觉输入，要么将注视作为辅助输入信号，本工作探索仅在训练时使用注视信号的方法。

Method: 引入注视正则化注意力机制，将模型焦点与人类视觉注视对齐。该设计灵活且模块化，可泛化到多个使用注意力的VLM架构。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统基于互信息的配准方法在合成CT生成中可能引入的解剖不一致性问题，开发更稳健的合成CT合成模型。

Method: 使用2.5D U-Net++网络，ResNet-34编码器，联合训练不同解剖区域并微调。损失函数结合L1损失和IMPACT-Synth感知损失。比较了Elastix互信息配准和IMPACT特征相似性配准两种策略。

Result: 在本地测试集上，IMPACT配准实现了更准确和一致的对齐，降低了MAE并生成更真实的解剖结构。但在公共验证集上，使用Elastix配准数据的模型得分更高，反映了配准偏差问题。

Conclusion: 配准误差会传播到监督学习中，影响训练和评估。IMPACT通过促进解剖一致性对齐有助于减轻这种偏差，支持开发更稳健的合成CT合成模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型使用固定去噪步数生成高保真图像，但在实际云到设备场景中，有限带宽需要重度压缩，导致纹理丢失和计算浪费。

Method: 引入联合端到端训练策略，扩散模型以可用带宽推导的目标质量水平为条件，学习自适应调制去噪过程，实现保持感知质量的早停采样。

Result: 实验结果表明，相比简单的早停方法，该方法显著提高了带宽自适应生成的视觉保真度。
...
Method: TerraGen引入地理空间布局编码器，统一边界框和分割掩码输入，结合多尺度注入方案和掩码加权损失，从全局结构到细节显式编码空间约束。

Result: 实验结果显示TerraGen在多样化任务中实现最佳生成图像质量，可作为通用数据增强生成器，显著提升下游任务性能，在全数据和少样本场景中展示强大的跨任务泛化能力。

Conclusion: TerraGen解决了遥感视觉任务中生成数据增强的局限性，提供了一个统一的、空间可控的生成框架，有效提升了多种下游任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决图像拼接中因物体深度变化导致的视差问题，这会引发重影和错位，影响拼接质量。

Method: 采用多阶段机制结合全局深度正则化约束来增强不同深度范围目标的配准精度；在融合过程中使用图基计算确定最优拼接缝，并通过软缝区域扩散精确定位过渡区域；引入重参数化策略优化结构设计。

Result: 实验证明该方法在性能上优于现有方法，能有效减轻视差引起的对齐误差，实现自然无缝的拼接效果。

Conclusion: 该方法通过深度一致性约束和优化的拼接策略，成功解决了视差引起的图像拼接问题，同时保持了较高的计算效率。
...
Method: 构建了基于核心视频内容的多层次视觉对应关系（复制、事件、场景、实例、动作等六个级别），开发了三个版本的MUVR基准（Base、Filter、QA）。

Result: 对3种最先进的视频检索模型、6种基于图像的VLM和10种MLLM进行了广泛评估，揭示了这些方法在处理未剪辑视频和多模态查询方面的局限性。

Conclusion: MUVR基准填补了长视频平台多模态检索的空白，为评估检索模型和MLLM在多视频理解和重排序能力方面提供了重要工具。
...
TL;DR: 本文提出用各向异性池化方法替代平均池化，以改进基于查找表(LUT)的CNN图像恢复方法，通过广义中值池化和学习数据依赖的池化系数来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LUT-based CNN图像恢复方法使用平均池化融合不同方向的像素块结果，但这种方法不适合处理各向异性信号结构，导致性能受限。

Method: 提出两种各向异性池化方法：1）广义中值池化；2）学习数据依赖的池化系数，自适应地权衡不同方向像素块的贡献。

Result: 在各种恢复基准测试中，各向异性池化策略在感知质量和数值指标上都优于现有LUT-realizable CNN方法。

Conclusion: 各向异性池化能有效提升LUT-based CNN图像恢复方法的性能，解决了平均池化对各向异性结构处理不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 建模3D对象和场景的层次结构对于自主代理的环境理解至关重要，但现有隐式表示方法在此方面存在挑战。现有显式建模方法要么需要多次渲染增加推理时间，要么依赖预定义的离散层次结构，难以泛化到真实世界的多样化结构。

Method: 使用连续双曲潜在空间表示场景层次结构，利用双曲几何的特性自然编码多尺度关系，并通过潜在空间中的测地路径实现层次结构的平滑遍历。

Result: 在标准基准测试中优于最先进方法，在3D场景理解方面展现出更高的效率和适应性。

Conclusion: OpenHype通过双曲几何表示有效解决了3D场景层次结构建模的挑战，为自主代理的环境理解提供了更高效的解决方案。
...
TL;DR: PhysWorld是一个利用模拟器合成物理演示来学习高效世界模型的框架，用于变形物体的动态预测，推理速度比现有方法快47倍


<details>
  <summary>Details</summary>
Motivation: 从有限的真实世界视频数据中学习物理一致的动态模型是一个重大挑战，特别是对于具有空间变化物理特性的变形物体

Method: 首先在MPM模拟器中构建物理一致的数字孪生，通过物理属性扰动生成多样化运动模式，然后训练轻量级GNN世界模型

Result: PhysWorld实现了对各种变形物体的准确快速未来预测，并能很好地泛化到新的交互场景

Conclusion: PhysWorld在保持竞争力的同时实现了47倍于现有方法的推理速度
...
TL;DR: 提出MoniTor方法，一种基于内存的在线评分队列方案，用于无训练的视频异常检测，通过结合预训练视觉语言模型和LSTM启发机制来有效识别异常行为。


<details>
  <summary>Details</summary>
Motivation: 在线视频异常检测因实时性和计算强度限制而研究较少，现有方法难以有效处理时序依赖关系。

Method: 使用流式输入到视觉语言模型，结合LSTM启发的预测机制捕获时序依赖，设计评分队列和异常先验动态存储最近分数。

Result: 在UCF-Crime和XD-Violence数据集上评估，MoniTor优于现有最先进方法，且与弱监督方法竞争无需训练。

Conclusion: MoniTor为在线视频异常检测提供了有效的无训练解决方案，能够有效建模时序依赖并识别异常行为。
...
Method: 提出VidSplice框架，包含CoSpliced模块进行首帧传播策略，以及上下文控制模块在帧复制后编码一致先验，并将拼接视频注入I2V生成主干。

Result: 广泛评估表明VidSplice在不同视频修复场景中达到竞争性性能，显著改善前景对齐和运动稳定性。

Conclusion: VidSplice通过解耦方法和间隔帧先验，有效解决了视频修复中的时空稳定性问题，优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在胸部X光诊断中预测不可解释的问题，为临床医生提供透明、可验证的解释以增强信任并识别潜在失败模式。

Method: 在BiomedCLIP诊断分类器上训练基于转码器的稀疏自编码器，将医学图像表示分解为可解释的视觉模式，通过在MIMIC-CXR数据集上训练100个转码器集成，发现约5000个单语义模式。

Result: 在五个关键发现上达到竞争性诊断准确性，每个预测可分解为20-50个可解释模式，提供可验证的激活图库，模式涵盖心脏、肺部、胸膜、结构、设备和伪影类别。

Conclusion: 医学AI系统可以同时实现准确性和可解释性，通过从针对特定诊断目标训练的分类器中提取可解释特征，确保发现的模式与临床决策直接相关，支持更安全的临床部署。
...
### [50] [Head Pursuit: Probing Attention Specialization in Multimodal Transformers](https://arxiv.org/abs/2510.21518)
*Lorenzo Basile,Valentino Maiorca,Diego Doimo,Francesco Locatello,Alberto Cazzaniga*

Main category: cs.CV

TL;DR: 本文提出了一种基于信号处理视角的方法来分析文本生成模型中注意力头的语义和视觉属性专业化，发现仅编辑1%的注意力头就能可靠地抑制或增强目标概念。


<details>
  <summary>Details</summary>
Motivation: 语言和视觉语言模型在各种任务中表现出色，但其内部机制仍不完全清楚。本研究旨在理解文本生成模型中单个注意力头如何专门处理特定的语义或视觉属性。

Method: 基于已有的可解释性方法，从信号处理的角度重新解释使用最终解码层探测中间激活的实践，通过原则性方式分析多个样本，并根据注意力头与目标概念的相关性进行排序。

Result: 发现在单模态和多模态transformer中，注意力头层面存在一致的专业化模式。仅使用该方法选择的1%注意力头进行编辑，就能可靠地抑制或增强模型输出中的目标概念。
...
TL;DR: 提出双流架构，通过组织-细胞交互建模提升病理图像细粒度分类性能


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型缺乏细胞级特征建模，限制了癌症亚型分类等细粒度任务的性能

Method: 双流架构结合组织特征和细胞表示，使用循环变压器进行细胞信息聚合，双向组织-细胞交互模块实现相互注意力
...
<details>
  <summary>Details</summary>
Motivation: 解决封闭区域多摄像头重叠视角下频繁发生的人员遮挡问题，提升多目标跟踪性能。

Method: 采用图加权解决方案，结合特征提取、跟踪和社区搜索，配备人员位置估计模块，在线更新人员识别标签。

Result: 在封闭区域模型和真实拥挤数据集上测试，显示优于无位置数据的方法，证明了方法的优越性。

Conclusion: GRAP-MOT在拥挤场景中表现优异，IDF1指标比MOTA更适合此类比较，代码和数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 胎盘早剥是妊娠期严重并发症，传统超声诊断依赖医生经验，存在主观偏见和诊断不一致性问题，需要自动准确的检测方法。

Method: 基于小样本学习改进YOLOv11n模型：集成小波卷积和坐标卷积增强频率和空间特征提取；引入级联分组注意力机制抑制超声伪影和遮挡干扰，提高边界框定位精度。
...
Method: 构建Gran-29M数据集，包含200万张图像和1.8亿个高质量区域级标注；采用预训练-适应框架和自蒸馏机制；使用边界框到标题回归和标题到边界框回归来增强视觉表示和定位能力。

Result: GranViT在细粒度识别、多模态VQA和OCR理解等任务上取得了最先进的结果，并展现出对不同LLM的强迁移性。
...
TL;DR: 本文提出了前瞻引导（FSG），一种改进的文本到图像扩散模型条件引导方法，通过统一视角将条件引导重新定义为定点迭代，解决了现有方法效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 现有分类器自由引导（CFG）方法基于不同的理论解释，限制了设计空间并模糊了关键设计选择，需要统一的理论框架来改进条件引导机制。

Method: 将条件引导重新定义为定点迭代问题，寻找潜在空间中的黄金路径；提出FSG方法，在早期扩散阶段优先解决长间隔子问题并增加迭代次数。

Result: 在多种数据集和模型架构上的广泛实验验证了FSG在图像质量和计算效率方面优于现有最先进方法。

Conclusion: 该工作为条件引导提供了新的视角，并释放了自适应设计的潜力，推动了文本到图像扩散模型的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要端到端重新训练多模态系统，计算成本高且缺乏模块化。Foley Control旨在保持预训练模型能力的同时，通过最小化训练实现视频音频同步。

Method: 将V-JEPA2视频嵌入连接到冻结的Stable Audio Open DiT文本到音频模型，在现有文本交叉注意力后插入紧凑的视频交叉注意力。使用视频token池化减少内存消耗并稳定训练。

Result: 在精选的视频音频基准测试中，Foley Control以远少于最近多模态系统的可训练参数，实现了竞争性的时间和语义对齐，同时保持提示驱动的可控性和生产友好的模块化。

Conclusion: 该方法提供了一种高效、模块化的视频到Foley音效生成解决方案，相同的桥接设计可扩展到其他音频模态（如语音）。
...
TL;DR: 提出了Chunk-GRPO方法，将GRPO优化从步级提升到块级，通过将连续步骤分组为块来捕捉流匹配的时间动态，解决了优势归因不准确和忽略生成时间动态的问题。


<details>
  <summary>Details</summary>
Motivation: GRPO在基于流匹配的文本到图像生成中表现良好，但面临两个关键限制：优势归因不准确，以及忽略生成的时间动态。

Method: 提出Chunk-GRPO方法，将连续步骤分组为捕捉流匹配内在时间动态的'块'，在块级别优化策略，并引入可选的加权采样策略进一步提升性能。

Result: 广泛实验表明，Chunk-GRPO在偏好对齐和图像质量方面均取得了优越结果。
...
TL;DR: MATrack是一种专门用于夜间无人机跟踪的多尺度自适应系统，通过三个核心模块解决夜间跟踪的主要技术挑战，在UAVDark135基准测试中性能显著优于现有方法，同时保持81 FPS的实时处理速度。
...
TL;DR: TIGER是一个两阶段的文本图像超分辨率框架，通过"文本优先、图像后处理"范式，先重建精确的文本结构，再用其指导全图像超分辨率，解决了传统方法在图像质量和文本可读性之间的权衡问题。
...
Method: TIGER采用两阶段框架：第一阶段重建精确的字形结构，第二阶段使用重建的文本结构来指导全图像超分辨率，实现字形到图像的引导。

Result: TIGER在UltraZoom-ST数据集上实现了最先进的性能，在增强可读性的同时保持了整体图像质量。

Conclusion: TIGER通过解耦字形恢复和图像增强，成功打破了图像质量和文本可读性之间的权衡，为场景文本超分辨率提供了有效解决方案。
...
TL;DR: 使用深度学习模型自动检测MEG记录中的发作间期癫痫棘波，提出了基于特征的ANN和CNN两种架构，在仅有时间标记和单专家标注的临床数据上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 药物难治性癫痫的术前评估需要定位癫痫灶，MEG中的发作间期棘波是关键生物标志物。手动检测耗时且易出错，现有自动化方法不适合临床实践，需要大量标注数据或缺乏鲁棒性。

Method: 提出两种深度学习架构：基于特征的人工神经网络(ANN)和卷积神经网络(CNN)，在59名患者数据库上训练，采用交互式机器学习策略迭代改进数据标注质量。

Result: 两种模型在10名测试患者上均优于现有最优模型(F1分数：CNN=0.46，ANN=0.44)，交互式学习策略证明模型对噪声标注具有鲁棒性。

Conclusion: 简单架构的模型在分析复杂和不完美标注数据时具有鲁棒性，交互式机器学习为快速数据标注提供了潜力，所提模型是自动检测发作间期棘波的有效工具。
...
Conclusion: 大规模合成数据生成和模糊感知架构能够有效解决显著目标检测中的数据限制问题，显著提升模型泛化能力。
...
TL;DR: 提出了Modest-Align轻量级跨模态对齐框架，通过随机扰动和嵌入平滑策略解决资源受限场景下模型的过度自信和性能下降问题，在少量数据和计算资源下实现竞争性性能。
...
Method: 采用两种互补策略：随机扰动（引入可控噪声模拟不确定性）和嵌入平滑（校准嵌入空间中的相似度分布）。

Result: 在多个基准数据集上，Modest-Align在检索任务中优于最先进方法，仅用CLIP 1/100的训练数据和1/600的GPU时间就达到竞争性结果。
...
TL;DR: 通过基于偏好的优化方法将成对极几何约束对齐到视频扩散模型中，解决几何不一致性和相机轨迹不稳定问题，无需端到端可微分就能有效实施几何原则。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型虽然通过大型潜在扩散变换器和整流流技术取得进展，但仍存在几何不一致、运动不稳定和视觉伪影等问题，破坏了真实3D场景的幻觉。3D一致视频生成对生成和重建任务的下游应用有重要影响。

Method: 使用基于偏好的优化方法将成对极几何约束对齐到扩散模型中，通过数学原理的几何强制直接解决不稳定的相机轨迹和几何伪影问题，无需端到端可微分。在静态场景和动态相机上训练以确保高质量测量，同时模型能有效泛化到多样化的动态内容。

Result: 评估表明经典几何约束比现代学习指标提供更稳定的优化信号，后者会产生噪声目标从而损害对齐质量。该方法在保持视觉质量的同时生成空间一致的视频。
...
TL;DR: 提出DAP-MAE方法，通过自适应整合跨域点云数据集知识来解决点云数据稀缺问题，在预训练中使用异质域适配器，在微调中使用融合模式，仅需一次预训练即可在多个点云分析任务中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 点云数据在不同领域的训练数据规模有限，现有方法尝试结合不同领域数据进行MAE预训练，但学到的先验知识可能与下游任务不匹配，导致性能下降。
...
Result: 在CIFAR-10和CIFAR-100数据集上，使用ResNet50和MobileNet_v2等架构，Gompertz-CNN比传统方法提升准确率高达8%和4%。

Conclusion: Gompertz-CNN通过动态调整蒸馏策略有效提升了知识蒸馏性能，证明了考虑学生模型学习进展的重要性。
...
TL;DR: 该论文系统研究了长尾识别方法在野生动物物种识别中的应用，在NACTI数据集上实现了99.40%的Top-1准确率，显著优于95.51%的基线，并展示了在域迁移场景下的更强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决自然世界数据集中普遍存在的严重长尾类别不平衡问题，特别是在NACTI数据集中，最大的'Head'类别占据了超过50%的370万张图像。

Method: 基于PyTorch Wildlife模型，系统研究长尾识别方法，包括各种LTR损失函数和LTR敏感正则化，构建了减少偏差的测试集来评估域迁移。

Result: 最佳配置在NACTI测试集上达到99.40% Top-1准确率，在ENA-Detection数据集上达到52.55%准确率，均优于基线方法。

Conclusion: LTR增强调度器选择与先进LTR损失函数结合在野生动物领域表现一致改善，但LTR方法仍无法充分解决某些问题，包括在严重域迁移下'Tail'类别的灾难性失效。
...
<details>
  <summary>Details</summary>
Motivation: 传统突触分类需要监督学习，但该方法旨在仅利用同一神经元内突触相似性这一生物学观察，实现无监督分类，无需先验知识。

Method: 基于同一神经元内相邻突触比随机突触更相似的原理，开发无监督分类方法，应用于果蝇数据。

Result: 成功对果蝇突触进行了分类，无需预先指定类型数量。

Conclusion: 该方法为突触分类提供了无监督的解决方案，可能为选择覆盖突触结构范围的真实数据提供原则性方法。
...
TL;DR: 评估UNI和Virchow2两种基础模型在皮肤病理学全玻片图像分类中的性能，使用均值聚合策略将补丁级特征转换为玻片级特征，并通过多种机器学习分类器进行分类。


<details>
  <summary>Details</summary>
Motivation: 皮肤病理学中全玻片图像的快速生成需要自动化方法进行高效处理和准确分类，以应对日益增长的数据量。

Method: 使用UNI和Virchow2基础模型提取补丁级嵌入特征，通过均值聚合策略转换为玻片级特征，然后训练逻辑回归、梯度提升树和随机森林等分类器。

Result: Virchow2在大多数玻片级分类器中表现优于UNI，逻辑回归在Virchow2上达到最高准确率90%，但差异无统计学意义。均值聚合方法提供了可靠的玻片级特征表示。
...
TL;DR: 该研究将计算机视觉应用于生态监测，通过后处理解释方法增强神经网络模型的可信度，使用多种解释技术评估检测结果，发现模型能准确识别海豹但存在与黑冰和岩石的混淆问题。
...
Method: 使用Glacier Bay国家公园的航空图像训练Faster R-CNN检测海豹，并应用梯度类激活映射（HiResCAM、LayerCAM）、LIME和基于扰动的解释方法生成解释。

Result: 解释集中在海豹躯干和轮廓而非周围环境，移除海豹会降低检测置信度；发现模型会混淆海豹与黑冰和岩石等系统性错误源。

Conclusion: 通过将目标检测与后处理可解释性结合，可以超越"黑盒"预测，开发出可审计的决策支持工具用于保护监测，为模型开发提供针对性数据整理和增强的具体方向。
...
<details>
  <summary>Details</summary>
Motivation: 探索视觉扩散模型作为几何求解器的潜力，证明其能直接处理像素空间中的几何问题，而不需要专门的架构或领域特定适配。

Method: 将每个几何问题实例视为图像，训练标准视觉扩散模型从高斯噪声生成表示有效近似解的图像，模型学习将噪声几何结构转换为正确配置。

Result: 模型成功生成了与精确解密切匹配的有效近似解，在三个著名几何问题上展示了可行性。
...
TL;DR: 提出了一个自动化框架，通过自反思代理来检测训练好的视觉模型对特定视觉属性的依赖关系，该代理能系统生成和测试假设，并通过迭代实验和自我评估来改进发现。
...
Result: 在包含130个模型的新基准测试中，自反思代理的性能持续提升，显著优于非反思基线，并成功识别了CLIP视觉编码器和YOLOv8目标检测器等先进模型中的真实视觉属性依赖。
...
TL;DR: 该论文开发了基于贝叶斯实验设计的蒙特卡洛推理策略，显著提升了语言模型在战略决策对话任务中的信息寻求能力，使较弱模型在协作战舰游戏中超越人类和前沿模型。


<details>
  <summary>Details</summary>
Motivation: 许多高风险AI应用需要形成数据驱动假设并进行针对性猜测，如科学和诊断场景。在资源有限的情况下，需要评估语言模型代理的理性程度并提升其信息寻求能力。

Method: 引入协作战舰战略对话任务，开发基于贝叶斯实验设计的蒙特卡洛推理策略，分别优化观察员代理的准确性和船长代理的信息增益。

Result: 观察员代理准确率提升14.7%，船长代理信息增益提升0.227比特（达到噪声上限的94.2%），较弱模型Llama-4-Scout胜率从8%提升至82%，成本仅为GPT-5的1%。

Conclusion: 基于贝叶斯实验设计的推理方法能显著提升语言模型代理的理性信息寻求能力，在多个任务中表现出通用性。
...
TL;DR: CodeAdapt方法通过结合代码执行和少量样本学习，使标准指令语言模型无需微调就能达到或超越专门训练的推理模型性能，同时提高计算效率。
...
Method: 提出CodeAdapt方法，结合CodeAct框架（语言模型在多步推理中交替使用自然语言和代码执行）和少量样本上下文学习，仅需5个训练问题即可实现。

Result: 在4对匹配的语言模型和推理模型比较中，CodeAdapt使3个语言模型在8个任务上平均表现优于对应推理模型（最高提升22.9%），同时token效率提高10-81%；在6个任务上4个模型平均表现更优（最高提升35.7%）。

Conclusion: CodeAdapt式学习和推理具有鲁棒性和领域通用性，代码增强的语言模型是认知基础良好的强大系统，为权重内强化学习提供了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 随着语言模型处理长文本能力的增强，需要评估其在计算文学研究中的实用性，但现有数据集不适合文学领域任务。

Method: 构建了FICSIM数据集，包含长篇现代小说，基于作者提供的元数据定义了12个相似性维度，并由数字人文学者验证。

Result: 评估了多种嵌入模型，发现它们倾向于关注表面特征而非对计算文学研究有用的语义类别。

Conclusion: FICSIM数据集填补了文学领域评估的空白，在数据收集过程中优先考虑作者权益和知情同意。
...
TL;DR: 评估大语言模型在识别美国最高法院案例推翻关系方面的表现，发现存在时代敏感性、浅层推理和上下文依赖推理失败等关键局限性。
...
Method: 使用236对案例数据集评估最先进的大语言模型识别最高法院案例中的推翻关系，分析模型在历史与现代案例上的表现差异。

Result: 模型在历史案例上表现较差，显示训练数据存在时间偏见；依赖浅层逻辑启发式而非深度法律理解；在复杂开放任务中产生时间上不可能的关系。

Conclusion: 这项工作填补了现实长上下文评估的关键空白，提供了反映实际法律推理任务复杂性和重要性的基准环境。
...
Result: 人类在所有语言特征上表现优于所有模型，平均准确率高16.6%；开源和闭源LLM之间存在18.1%的性能差距；最强模型(gpt-5)准确率仅73.5%，而人类达90.1%。

Conclusion: 爱尔兰-BLiMP为评估LLM在爱尔兰语中的语法能力提供了首个系统性框架，是推进低资源语言理解研究的重要基准。
...
TL;DR: 本文提出置信度门控的思维链方法，让模型仅在直接回答置信度低时才进行推理，以减少不必要的token消耗。通过系统评估四种无需训练的置信度估计方法，发现它们能减少冗余思维链，但效果因数据集和模型而异。


<details>
  <summary>Details</summary>
Motivation: 思维链提示能提升大语言模型的推理能力，但过度推理会增加token使用量，限制实际应用。现有模型允许调整思维链长度，但缺乏明确的使用时机指导。

Method: 提出置信度门控思维链方法，使用四种无需训练的置信度估计方法（包括随机基线和理想情况）来决定是否触发思维链推理。

Result: 实验表明现有置信度方法能减少冗余思维链，优于随机触发，但不同置信度方法的效果在数据集和模型间存在不一致性。

Conclusion: 当前置信度门控思维链方法具有潜力但存在局限性，需要开发更可靠的自适应门控机制。
...
TL;DR: 该研究量化了输入数据结构对LLM生成NBA比赛摘要中幻觉和事实错误的影响，发现JSON格式相比非结构化输入能减少约65-69%的错误率，行结构化格式能减少约51-54%的错误率。


<details>
  <summary>Details</summary>
Motivation: 在体育报道等准确性要求高的领域部署LLM时，生成的文本可能无法忠实反映输入数据，因此需要量化输入结构对事实错误的影响。

Method: 使用三种输入格式（行结构化、JSON和非结构化）生成NBA比赛摘要，手动标注3,312个事实错误，涉及180场比赛摘要和两个模型（Llama-3.1-70B和Qwen2.5-72B）。

Result: 输入结构对错误率有显著影响：JSON输入相比非结构化输入分别减少Llama和Qwen模型69%和65%的错误率，行结构化输入分别减少54%和51%的错误率。双向重复测量ANOVA显示输入结构解释了错误率80%以上的方差。
...
TL;DR: 研究发现推理增强的LLMs在低误报率(FPR)场景下表现不佳，尽管能提升整体准确率。无推理模式在精度敏感任务中更优，而简单集成方法能结合两者优势。


<details>
  <summary>Details</summary>
Motivation: 探究推理增强的LLMs在精度敏感分类任务中的适用性，特别是在严格低误报率要求下的表现。

Method: 系统研究推理在安全检测和幻觉检测任务中的效果，比较微调和零样本设置下的标准LLMs与大型推理模型(LRMs)，分析推理开启(Think On)与推理关闭(Think Off)两种模式。

Result: 推理增强模式提升整体准确率但在低FPR阈值下表现较差；无推理模式在精度敏感场景中占优；基于token的评分优于自我表达置信度；简单集成方法能恢复两种模式的优势。

Conclusion: 推理是一把双刃剑：对平均准确率有益，但在需要严格精度的应用中往往不适用。
...
Method: 训练BERT检索器使用REINFORCE算法根据编辑奖励对演示进行排序，并采用可学习阈值修剪低价值示例。

Result: 在COUNTERFACT基准测试中，编辑成功率提高17.1%，延迟降低41.6%，同时保持无关查询的准确性。

Conclusion: DR-IKE展示了可扩展和自适应的知识编辑能力，为黑盒LLM提供了一种有效的知识更新方法。
...
TL;DR: 本研究将自适应RAG系统应用于印尼语问答任务，通过问题复杂度分类器和机器翻译数据增强方法，探索在低资源语言中构建问答系统的可行性与挑战。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的问答系统主要针对英语，存在语言鸿沟问题。本研究旨在通过自适应RAG系统将问答能力扩展到印尼语，解决低资源语言环境下的问答需求。

Method: 采用自适应RAG系统，包含问题复杂度分类器来区分问题难度并决定回答策略；使用机器翻译进行数据增强以克服印尼语数据集有限的挑战。

Result: 问题复杂度分类器表现可靠，但多检索回答策略存在显著不一致性，当应用该策略时对整体评估产生了负面影响。

Conclusion: 研究揭示了在低资源语言中构建问答系统的潜力和挑战，为未来改进指明了方向，特别是在多检索策略的稳定性和一致性方面需要进一步优化。
...
<details>
  <summary>Details</summary>
Motivation: 解决电子健康记录中药物推荐系统发展受限于公开数据集稀缺的问题，特别是非英语语言的数据集。

Method: 构建包含3,190名患者5,894条记录的CDrugRed数据集，包含人口统计、病史、临床过程和出院诊断等信息，并基于该数据集对多个大语言模型进行基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 监督微调(SFT)作为对齐大语言模型的重要方法，存在过拟合和领域外泛化能力差的问题，特别是在数据有限的情况下。需要一种能够提高泛化性能的在线策略微调方法。

Method: 提出Self-Rewarding PPO方法，结合SFT和近端策略优化(PPO)。核心是设计奖励函数作为SFT模型与预训练基础模型的对数策略比率，使用预训练策略作为基线，SFT策略作为目标，实现无需人工偏好标注的在线策略微调。

Result: 在多个自然语言处理任务上的实证评估表明，Self-Rewarding PPO始终优于传统SFT方法，特别是在高质量标注数据稀缺的情况下，表现出更好的泛化能力、数据效率和鲁棒性。

Conclusion: Self-Rewarding PPO方法有效解决了SFT的局限性，通过在线策略微调提高了语言模型的泛化性能，为在标注数据稀缺场景下对齐大语言模型提供了有效解决方案。
...
TL;DR: 提出了VeriGray基准，通过引入Out-Dependent类别来解决摘要忠实性检测中的标注模糊问题，发现即使是SOTA LLM在摘要任务中也会产生约6%的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有忠实性基准存在标注模糊问题，主要是由于生成输出中允许的外部知识边界定义不清，导致标注不一致。

Method: 提出新的忠实性标注框架，引入中间类别Out-Dependent来分类需要外部知识验证的情况，并构建VeriGray基准。

Result: 统计显示SOTA LLM在摘要任务中产生约6%的幻觉，约8%的生成句子属于Out-Dependent类别，基准对多种基线方法构成显著挑战。
...
TL;DR: 该调查论文系统回顾了大型语言模型与文本属性图的集成，提出了从编排视角的新分类法，涵盖LLM增强图任务和图结构提升LLM推理两个方向，并讨论了三种编排策略及具体技术方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型具有强大的语义理解能力但缺乏结构化推理，文本属性图提供显式关系结构但缺乏语义深度，两者结合可以产生互补优势。

Method: 提出了基于编排视角的新分类法：LLM for TAG（LLM增强图任务）和TAG for LLM（图结构提升LLM推理），并将编排策略分为顺序、并行和多模块框架，讨论了TAG特定预训练、提示和参数高效微调技术。

Result: 总结了实证研究见解，整理了可用数据集，展示了在推荐系统、生物医学分析和知识密集型问答等领域的多样化应用。

Conclusion: 为语言和图学习的交叉领域提供了系统指导，指出了开放挑战和未来研究方向。
...
TL;DR: 本文提出一个系统框架分析LLMs在社交模拟中的行为，发现LLMs不能忠实再现真实人类行为，而是反映过度理想化的版本，存在社会期望偏差，导致形成缺乏真实人类互动复杂性的"乌托邦"社会。


<details>
  <summary>Details</summary>
Motivation: 可靠的人类行为模拟对于解释、预测和干预社会至关重要。虽然LLMs在模拟人类行为方面显示出潜力，但它们在社交情境中与真实人类行为的差异程度仍未充分探索，存在科学研究和实际应用中的误解风险。

Method: 引入一个系统框架，通过聊天室式对话模拟多智能体互动，并在五个语言维度上进行分析，提供简单有效的方法来检验新兴的社会认知偏差。对三个家族的八个代表性LLMs进行了广泛实验。

Result: 研究发现LLMs不能忠实再现真实人类行为，而是反映过度理想化的版本，受到社会期望偏差的影响。具体表现为社会角色偏差、首因效应和积极性偏差，形成缺乏真实人类互动复杂性的"乌托邦"社会。

Conclusion: 这些发现呼吁开发更具社会基础的LLMs，以捕捉人类社交行为的多样性。
...
TL;DR: 提出了一个针对爱沙尼亚语的新基准测试，基于7个多样化数据集，评估了基础模型、指令调优开源模型和商业模型在爱沙尼亚语任务上的表现。
...
Method: 创建基于7个爱沙尼亚语原生数据集的基准测试，不使用机器翻译。评估了6个基础模型和26个指令调优模型，结合人工评估和LLM-as-a-judge方法。

Result: 人工评估分数与基准测试评估呈中度到高度相关。Claude 3.7 Sonnet作为LLM评判者与人工评分高度一致，表明高性能LLM能有效支持爱沙尼亚语模型评估。

Conclusion: 成功建立了爱沙尼亚语LLM评估基准，证明了高性能LLM可以作为有效的评估工具，填补了爱沙尼亚语LLM评估的空白。
...
Method: 结合最先进的自然语言处理技术和社会学见解，分析英语和法语极右翼推特语料库MIGR-TWIT

Result: 整合语言学、社会学和计算方法，揭示极右翼行为者使用的移民话语、仇恨言论和说服技巧模式

Conclusion: 通过跨学科方法提供对社会动态的洞察，有助于更好地理解社交媒体平台上右翼极端主义带来的当代挑战
...
TL;DR: 开发了一个基于临床分类学和大语言模型的多智能体系统，用于模拟真实的紧急医疗调度场景，通过医生评估和自动化分析验证了系统的高性能。


<details>
  <summary>Details</summary>
Motivation: 紧急医疗调度面临呼叫者情绪困扰、信息模糊和认知负荷等挑战，大语言模型和多智能体系统为增强调度员能力提供了机会。

Method: 构建了包含32种主诉和6种呼叫者身份的临床分类学，开发了基于AutoGen的多智能体系统，包含呼叫者和调度员智能体，使用混合评估框架进行验证。

Result: 系统表现出色：调度有效性达94%，指导有效性达91%，情感分析显示73.7%为中性情感，可读性高（Flesch 80.9），60%为礼貌风格。

Conclusion: 该分类学基础的多智能体系统能够高保真地模拟多样化的临床调度场景，可用于调度员培训、协议评估和实时决策支持，为AI安全集成到应急响应工作流程提供了路径。
...
TL;DR: 该论文引入相关性维度这一分形几何度量来量化语言模型感知的文本认识论复杂性，揭示了LLM训练过程中的三个不同阶段，并能检测幻觉和文本退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标强调局部预测准确性但忽略了长程结构复杂性，无法解释LLM即使困惑度低时仍出现重复和不连贯等令人困惑的行为。

Method: 使用相关性维度这一分形几何度量来量化文本的自相似性，捕捉语言的分层递归结构，在统一框架中桥接局部和全局属性。

Result: 相关性维度揭示了预训练中的三个不同阶段，反映了上下文依赖的复杂性，指示了模型产生幻觉的倾向，并能可靠检测生成文本中的多种退化形式。
...
TL;DR: 提出PBS-Attn方法，通过利用注意力的置换特性来增加块级稀疏性，提高LLM预填充的计算效率，在长上下文任务中达到2.75倍加速。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制的O(N^2)复杂度是扩展LLM上下文长度的主要瓶颈，虽然注意力矩阵通常是稀疏的，但现有块稀疏注意力方法在块级稀疏性方面存在不足。

Method: 提出Permuted Block-Sparse Attention (PBS-Attn)，利用注意力的置换特性来增加块级稀疏性，并开发了自定义的permuted-FlashAttention内核。

Result: 在真实世界长上下文数据集上的实验表明，PBS-Attn在模型准确性上持续优于现有块稀疏注意力方法，并接近全注意力基线，预填充端到端加速达2.75倍。

Conclusion: PBS-Attn是一种即插即用的高效方法，能够显著提升长上下文LLM预填充的计算效率，同时保持模型性能。
...
TL;DR: PARL方法使用LLMs作为强化学习智能体，通过提示而非微调在非语言环境中执行任务，在简单环境中表现与传统RL智能体相当或更好，但在需要复杂数学运算的任务中存在局限。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在强化学习任务中作为智能体的能力，特别是在不依赖自然语言的结构化环境中，如网格世界位置解释。

Method: 提出PARL方法，通过提示将动作、状态和奖励编码，让LLMs通过试错交互学习，无需微调。

Result: 在三个标准RL任务中，PARL在简单环境中能匹配或超越传统RL智能体，但在需要复杂数学运算或状态动作解码的任务中表现受限。

Conclusion: LLMs可以作为RL智能体在非语言环境中工作，但当前方法在处理复杂数学运算方面存在局限性。
...
Method: 使用自然语言推理模型在解码过程中注入连续语义相似性惩罚，通过重要性重加权和方差缩减技术来去偏和优化不确定性估计。
...
TL;DR: 该论文研究英语语言中typoglycemia现象（单词内部字母乱序）对NLP模型的影响，发现模型对此类扭曲具有较强鲁棒性，主要原因是英语中真正混淆的单词数量较少，且混淆单词的上下文差异明显便于消歧。


<details>
  <summary>Details</summary>
Motivation: 研究人类阅读字母乱序单词的能力（typoglycemia现象）以及NLP模型对此类扭曲的鲁棒性，探索模型在多个不同单词（如form和from）在typoglycemia下变为相同表示时仍能良好表现的根本原因。

Method: 分析英国国家语料库量化typoglycemia下的单词混淆和歧义情况；评估BERT模型消歧混淆形式的能力；通过对比在干净文本和typoglycemia文本上从头训练的BERT变体进行探测实验。

Result: 研究发现英语中真正混淆的单词数量相对较少，混淆单词的上下文差异明显便于消歧，字母乱序导致的性能下降比预期要小。

Conclusion: NLP模型对typoglycemia具有较强鲁棒性，主要归因于英语语言本身的特性——混淆单词数量有限且上下文差异显著，使得消歧变得相对容易。
...
TL;DR: TripTide是首个评估LLM在现实旅行中断情况下修订行程能力的基准，通过建模中断严重性和旅行者容忍度等维度，系统评估LLM对航班取消、天气关闭等事件的适应能力。
...
Result: LLM在序列一致性和语义稳定性方面表现良好，空间偏差在短途旅行中较大但随行程延长而减小。然而，随着行程长度增加，LLM处理中断的能力下降，显示出稳健性限制。
...
TL;DR: 研究发现单轮训练比多轮训练在推理任务中更有效，单轮训练的模型能很好泛化到单轮和多轮评估，而多轮训练的模型在单轮推理性能上显著下降。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在推理任务中是否需要多轮训练，因为实际应用通常涉及多轮人机交互，而传统训练是单轮的，存在训练与部署条件不匹配的问题。

Method: 比较传统单轮训练与三种多轮策略，评估模型在单轮和多轮推理任务上的表现。

Result: 单轮训练的模型在单轮和多轮评估中都表现良好，而多轮训练的模型在单轮推理性能上显著下降。

Conclusion: 对于信息完整的任务，稳健的单轮训练比多轮训练更有效可靠，多轮训练提供的收益有限甚至可能降低推理能力。
...
TL;DR: 本文介绍了一个专门针对瑞典相关人物和事件的手动编写问答基准数据集，用于测试多语言模型在瑞典特定知识上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的瑞典基准多是翻译自美国中心的数据集，不适合测试瑞典特有的相关知识。需要专门针对瑞典本地人物和事件的评估工具。

Method: 手动编写问答基准，灵感来自瑞典流行广播节目中的文化媒体公众人物和主要体育赛事。数据集包含英文翻译，可用于测试跨语言事实一致性。

Result: 发现具有更强瑞典覆盖度的小模型在回忆瑞典相关事实方面表现与三倍大的多语言模型相当。继续在瑞典语上进行预训练通常能提高事实知识，但也会导致部分先前已知信息的遗忘。
...
TL;DR: SindBERT是首个针对土耳其语的大规模RoBERTa编码器模型，在土耳其NLP任务中表现具有竞争力，但大规模版本并未显示出持续的性能优势，表明当前土耳其语基准可能已饱和。


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer模型在NLP领域取得了革命性进展，但许多形态丰富的语言在大规模预训练中仍然代表性不足。作者旨在为土耳其语开发首个大规模RoBERTa编码器模型。

Method: 使用312GB土耳其语文本（mC4、OSCAR23、维基百科）从头训练SindBERT模型，发布基础和大型两种配置，并在词性标注、命名实体识别、冒犯性语言检测和TurBLiMP语言可接受性基准上进行评估。

Result: SindBERT在土耳其语和多语言模型中表现具有竞争力，大型变体在四项任务中的两项获得最佳分数，但整体未显示出一致的扩展优势。与BERTurk等更小但更精选的模型比较表明，语料库质量和多样性可能超过纯粹的数据量。

Conclusion: SindBERT作为土耳其NLP的开放资源，同时作为关于形态丰富语言中扩展限制和语料库组成核心作用的实证案例研究。模型以MIT许可证发布，提供fairseq和Huggingface两种格式。
...
TL;DR: 提出了HalleluBERT，一个从零开始训练的大规模希伯来语RoBERTa编码器家族，在49.1GB去重希伯来语语料上训练，在NER和情感分类任务上超越了现有模型。


<details>
  <summary>Details</summary>
Motivation: 希伯来语缺乏大规模、充分训练的RoBERTa编码器，现有模型如HeBERT、AlephBERT和HeRo在语料库规模、词汇表或训练深度方面存在局限。

Method: 使用49.1GB去重希伯来语网页文本和维基百科语料，采用希伯来语特定的字节级BPE词汇表，从头训练RoBERTa基础模型和大模型。

Result: 在命名实体识别和情感分类基准测试中，HalleluBERT超越了单语和多语言基线模型，为希伯来语建立了新的最先进水平。

Conclusion: HalleluBERT展示了完全收敛的单语预训练的优势，为希伯来语NLP提供了更强大的编码器。
...
TL;DR: 本文研究了视觉语言模型在人类活动识别中的潜力，提出了新的评估方法和数据集，发现VLMs在准确性上可与传统深度学习方法相当甚至更好。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的发展，视觉语言模型在医疗健康应用中展现出潜力，但在人类活动识别领域应用相对较少，且缺乏有效的评估方法。
...
TL;DR: 提出了UDCG指标，针对RAG系统中LLM处理检索结果的特点，改进了传统IR指标，提高了与端到端答案准确性的相关性。


<details>
  <summary>Details</summary>
Motivation: 传统IR指标基于人类用户顺序阅读的假设，不适用于RAG系统中LLM整体处理检索结果的情况，且未考虑干扰文档对生成质量的负面影响。
...
Method: 系统整合MLLMs、IoT和可穿戴设备，自动收集多模态数据，使用异常检测模块（包括跌倒检测模型）和基于MLLMs的自然语言处理组件，通过提示工程整合患者信息。

Result: 开发了完整原型系统并正在进行测试，实验表明系统在现实场景中可实施且可扩展，能够实时提供患者生命体征、状态和情绪信息。

Conclusion: REMONI系统有望减轻医疗专业人员工作负担并降低医疗成本，通过智能代理和用户友好的Web应用实现高效的远程健康监测。
...
Method: 使用多奖励优化方法，结合测试时缩放、拒绝采样和强化学习来直接优化token相关性，并引入组步和重要性采样策略来降低奖励方差
...
TL;DR: 提出了一种可扩展的通用脑调优方法，通过微调预训练语音语言模型来联合预测多个参与者的fMRI响应，显著提高了脑对齐性能并减少了对个体数据的需求。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖个体参与者数据，难以泛化到新参与者并进行群体分析，限制了脑对齐研究的可扩展性。

Method: 多参与者脑调优方法，通过微调预训练语言模型来联合预测多个参与者的fMRI脑响应数据。

Result: 方法实现了：1）预测新参与者脑数据所需fMRI数据减少5倍；2）整体脑对齐提高达50%；3）对新数据集有强泛化能力；4）下游语义任务性能提升。

Conclusion: 多参与者脑调优在神经科学和AI之间建立了双向受益关系，通过群体脑数据训练可获得更通用的语义表示。
...
TL;DR: 该论文提出了一种基于机制检测的方法来识别RAG系统中的幻觉问题，通过分离外部上下文分数和参数知识分数来训练回归分类器，在Qwen3-0.6b上验证并推广到GPT-4.1-mini。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然整合外部知识来减少幻觉，但模型经常生成与检索内容不一致的输出。现有方法通常混淆外部上下文和参数知识的贡献，需要准确分离这两者来检测幻觉。

Method: 研究RAG幻觉机制，发现当后续层FFN模块不成比例地将参数知识注入残差流时会产生幻觉。基于外部上下文分数和参数知识分数，在Qwen3-0.6b上计算各层和注意力头的分数，训练回归分类器预测幻觉。

Result: 方法在先进LLM（GPT-5、GPT-4.1）和检测基线（RAGAS、TruLens、RefChecker）上评估。在Qwen3-0.6b上训练的classifier能泛化到GPT-4.1-mini响应，展示了代理模型评估的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 开发数学框架来理解文档结构，实现信息量化、内容摘要和扩展，以及提升预训练模型性能。

Method: 使用范畴论将文档表示为问答对类别，开发正交化程序分割信息，构建信息度量方法，并利用RLVR进行自监督改进。

Result: 实现了文档信息量化、新型摘要技术、文档扩展方法，以及预训练模型的自我改进。

Conclusion: 范畴论为文档分析提供了强大的数学基础，支持信息理论分析、内容操作和模型优化，具有广泛的应用前景。
...
TL;DR: 本文研究了大型语言模型对语言谱系结构的敏感性，发现模型在语言切换时倾向于选择谱系相关语言，且知识一致性在谱系内部保持更好，但训练数据不平衡仍是主要影响因素。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在跨语言行为中是否对语言谱系结构敏感，填补了该领域的研究空白。

Method: 基于MultiQ数据集扩展分析，检查模型在提示语言保真度不保持时是否倾向于切换到谱系相关语言，并调查知识一致性在谱系内部和跨谱系的差异。

Result: 发现谱系级别效应存在但强烈受训练资源可用性影响，不同LLM家族表现出不同的多语言策略。

Conclusion: LLM编码了谱系级别结构的某些方面，但训练数据不平衡仍然是塑造其多语言性能的主要因素。
...
TL;DR: 该论文介绍了斯洛文尼亚语的第一个语用理解基准SloPragEval和SloPragMega，包含405个选择题，用于评估LLM在语用理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的提升，需要更挑战性的评估来测试超越表面语言能力的语用理解，即理解情境意义、语境以及语言文化规范的能力。

Method: 构建了斯洛文尼亚语的语用理解基准，包含405个选择题，通过翻译活动建立人类基线，并进行LLM的试点评估。
...
TL;DR: 提出无监督异常检测方法识别词表中的音系异常，应用于Kokborok语言与孟加拉语的多语言数据集，以发现转录错误和借词。
...
Method: 使用字符级和音节级音系特征的无监督异常检测算法，识别音系不一致性。

Result: 音节感知特征显著优于字符级基线方法，虽然精确率和召回率因异常性质微妙而保持适中，但高召回方法能为田野工作者提供系统化标记需验证条目的方法。

Conclusion: 该方法支持低资源语言文档中数据质量的改进，为田野工作者提供识别需验证条目的系统方法。
...
TL;DR: 提出Reflective Evidence Tuning (RETuning)方法，通过动态构建分析框架、组织和评分证据来增强大语言模型在股票预测中的推理能力，解决了现有模型过度依赖分析师意见而缺乏独立逻辑分析的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在数学和编程任务中表现出色，但在股票预测等金融任务中应用不足。现有模型过度依赖分析师意见，缺乏系统性的独立分析逻辑，未能充分利用推理能力。

Method: 提出RETuning方法，在强化学习前进行冷启动训练。该方法鼓励模型从多样化信息源动态构建分析框架，基于框架而非上下文观点组织和评分证据，最终通过反思得出预测。

Result: 构建了大规模数据集（2024年全年5123只A股，32K上下文，20万+样本），实验表明RETuning成功解锁了模型在金融领域的推理能力，即使在6个月后或分布外股票上推理时缩放仍然有效。

Conclusion: RETuning方法使模型获得有价值的股票预测洞察，确保独立逻辑推理，减少上下文的不当影响，显著提升了模型在金融任务中的表现。
...
TL;DR: 提出了一种物理嵌入的条件神经本构模型框架，用于弹塑性材料的建模，结合偏微分方程和神经网络，能够泛化到不同物理参数场景并支持逆向参数估计。


<details>
  <summary>Details</summary>
Motivation: 解决数据驱动方法在复杂材料建模中泛化能力差和物理一致性不足的问题，开发一个能够适应多种物理场景的统一框架。

Method: 使用两个独立的神经网络分别建模弹性和塑性本构关系，将物理参数作为条件输入，在包含多种场景的综合数据集上训练，实现无需重新训练即可泛化到不同物理参数。

Result: 实验结果表明在运动重建、鲁棒长期预测、几何泛化和精确参数估计方面达到最先进性能，展现了作为统一模拟器和逆向分析工具的通用性。

Conclusion: 该框架成功地将物理原理与神经网络相结合，为弹塑性材料提供了一个通用的建模和逆向分析工具，能够处理未知物理参数的情况。
...
### [114] [SViM3D: Stable Video Material Diffusion for Single Image 3D Generation](https://arxiv.org/abs/2510.08271)
*Andreas Engelhardt,Mark Boss,Vikram Voletti,Chun-Han Yao,Hendrik P. A. Lensch,Varun Jampani*

Main category: cs.GR

TL;DR: SViM3D是一个从单张图像预测多视角一致PBR材质的框架，通过扩展视频扩散模型来联合输出空间变化的PBR参数和表面法线，支持重光照和可控外观编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么使用简单材质模型，要么需要额外步骤来估计反射率以实现重光照和外观编辑，限制了在AR/VR、电影、游戏等视觉媒体中的应用。

Method: 扩展潜在视频扩散模型，基于显式相机控制联合输出空间变化的PBR参数和表面法线，引入多种机制改善这个不适定设置下的质量。
...
### [115] [Sketch2BIM: A Multi-Agent Human-AI Collaborative Pipeline to Convert Hand-Drawn Floor Plans to 3D BIM](https://arxiv.org/abs/2510.20838)
...
### [116] [Cultural Alien Sampler: Open-ended art generation balancing originality and coherence](https://arxiv.org/abs/2510.20849)
...
Method: 使用两个在WikiArt概念上微调的GPT-2模型：概念连贯性模型评估概念组合的合理性，文化背景模型评估组合在艺术家作品中的典型性，选择高连贯性低典型性的组合。

Result: 在人类评估中优于随机选择和GPT-4o基线，与艺术学生表现相当；定量研究显示能产生更多样化输出和探索更广的概念空间。

Conclusion: 人工文化异化能够释放自主代理的创意潜力，在保持连贯性的同时生成新颖想法。
...
### [117] [Fuzzy numbers revisited: operations on extensional fuzzy numbers](https://arxiv.org/abs/2510.20861)
*Krzysztof Siminski*

Main category: cs.AI

TL;DR: 本文提出了一种新的模糊数表示方法——外延模糊数，解决了传统模糊数运算中的计算复杂性和结果形状变化问题。


<details>
  <summary>Details</summary>
Motivation: 传统模糊数运算存在计算复杂度高、运算结果可能改变模糊集特征、模糊扩散等问题，限制了模糊数的应用范围。

Method: 定义了外延模糊数的运算和关系运算符（=、>、>=、<、<=），并通过应用实例进行说明。

Result: 开发了C++实现，并在GitHub上公开可用。

Conclusion: 外延模糊数方法为解决传统模糊数运算问题提供了新的有效途径。
...
### [118] [Customizing Open Source LLMs for Quantitative Medication Attribute Extraction across Heterogeneous EHR Systems](https://arxiv.org/abs/2510.21027)
...
Method: 定制开源大语言模型（Llama、Qwen、Gemma、MedGemma）来提取处方属性，使用固定JSON模式处理记录，并进行轻量级归一化和跨字段一致性检查。
...
### [119] [Epistemic Deference to AI](https://arxiv.org/abs/2510.21043)
*Benjamin Lange*

Main category: cs.AI

TL;DR: 该论文探讨了何时应该优先采用AI输出而非人类专家判断，提出了AI优先主义观点及其替代方案——全面证据观，认为AI输出应作为补充性理由而非完全替代人类独立认知。


<details>
  <summary>Details</summary>
Motivation: 基于社会认识论，探讨AI系统作为人工认知权威的地位，以及如何合理地在AI输出与人类判断之间进行权衡，特别是在高风险情境下。

Method: 通过分析AI优先主义的经典反对意见（如盲目服从、认知固化等），并对比提出全面证据观的替代方案，强调AI输出应作为贡献性理由而非替代性理由。

Result: 全面证据观具有三个关键优势：防止专业知识萎缩、为人机监督提供认知依据、解释在可靠性条件不满足时对AI的合理不信任。

Conclusion: 虽然实践要求较高，但全面证据观提供了一种原则性方法来确定何时AI服从是合理的，特别是在需要严格可靠性的高风险情境中。
...
### [120] [From Questions to Queries: An AI-powered Multi-Agent Framework for Spatial Text-to-SQL](https://arxiv.org/abs/2510.21045)
*Ali Khosravi Kazazi,Zhenlong Li,M. Naser Lessani,Guido Cervone*

Main category: cs.AI

TL;DR: 提出了一个多智能体框架，用于将自然语言问题准确翻译为空间SQL查询，通过知识库、语义增强和协作式多智能体流水线（包括实体提取、元数据检索、查询逻辑制定、SQL生成和审查智能体）来解决空间查询的复杂性。
...
Method: 采用多智能体框架，集成了知识库（包含程序化模式分析和语义增强）、上下文检索嵌入以及协作式多智能体流水线，该流水线包含实体提取、元数据检索、查询逻辑制定、SQL生成和审查智能体（进行程序化和语义验证）。
...
TL;DR: MedAlign是一个用于医学视觉问答的新框架，通过多模态直接偏好优化、检索感知专家混合架构和联邦治理机制，解决大视觉语言模型在医疗应用中的幻觉、推理效率和多机构协作问题。


<details>
  <summary>Details</summary>
Motivation: 解决大视觉语言模型在临床服务部署中的三个关键挑战：幻觉回答、固定深度推理效率低下以及多机构协作困难。

Method: 提出多模态直接偏好优化目标，设计检索感知专家混合架构，采用联邦治理机制进行自适应推理。

Result: 在三个代表性Med-VQA数据集上达到最先进性能，比强检索增强基线F1分数提升高达11.85%，平均推理长度比固定深度CoT方法减少51.60%。

Conclusion: MedAlign框架有效解决了LVLM在医疗应用中的关键挑战，实现了视觉准确的响应和高效推理。
...
TL;DR: 提出一种针对观测数据中存在未观测混杂偏倚的深度强化学习算法，在12个有混杂的Atari游戏中相比标准DQN表现更优


<details>
  <summary>Details</summary>
Motivation: 在复杂高维环境中，当存在未观测混杂因素时，传统的离策略学习方法可能失效，需要开发对混杂偏倚具有鲁棒性的算法

Method: 基于深度Q网络(DQN)，提出一种寻找与观测数据兼容的最坏情况下安全策略的算法，通过处理行为策略和目标策略输入不匹配问题

Result: 在12个有混杂的Atari游戏中，该算法在所有存在未观测混杂因素且行为策略与目标策略输入不匹配的游戏中均优于标准DQN

Conclusion: 所提出的算法能有效处理观测数据中的混杂偏倚，在存在未观测混杂因素的环境中具有更好的鲁棒性和性能
...
TL;DR: 该研究首次实证分析了AI代理在去中心化治理中作为自主决策者的表现，通过构建AI投票代理在真实DAO环境中进行投票决策评估。


<details>
  <summary>Details</summary>
Motivation: 研究AI代理在去中心化治理中作为自主决策者的可行性，探索AI如何增强集体决策过程。

Method: 使用3000多个主要协议提案，构建AI投票代理，通过模块化可组合程序工作流在基于可验证区块链数据的金融模拟环境中运行，代理能够解读提案背景、检索历史审议数据并独立确定投票立场。

Result: AI代理的决策与人类和代币加权结果高度一致，通过精心设计的评估指标显示出强对齐性。

Conclusion: 研究表明AI代理能够通过产生可解释、可审计且基于实证的信号来增强集体决策，为去中心化金融系统中可解释且经济严谨的AI代理设计做出贡献。
...
TL;DR: 提出了PACE数据集和PACER咨询模型，用于恐慌发作时的心理急救支持，通过多维度评估框架PanicEval验证其有效性。
...
Method: 引入PACE数据集（基于第一人称叙事构建的高痛苦发作数据），训练PACER咨询模型（结合监督学习和模拟偏好对齐），提出PanicEval多维度评估框架。

Result: PACER在咨询师端指标和客户情感改善方面均优于强基线模型，人类评估中在恐慌场景下持续优于通用、CBT基础和GPT-4驱动的模型。

Conclusion: PACER模型在恐慌发作干预中表现出色，具有实际应用价值，代码已开源。
...
Method: 首先识别与上下文中毒知识强相关的毒害响应神经元，然后使用遗传算法进化对抗性段落以最大化激活这些神经元，并通过观察到的归因信号重用有潜力但初始不成功的外部知识变体。

Result: 在多个模型和数据集上的实验结果显示，该方法持续实现超过90%的人口覆盖成功率，同时保持流畅性。
...
TL;DR: EGO-Prompt是一个自动化框架，通过进化图优化方法自动设计更好的提示和推理过程，在领域特定任务上显著提升LLM性能。


<details>
  <summary>Details</summary>
Motivation: 在真实应用中，为大型语言模型设计领域特定任务的最优提示和推理过程既必要又具挑战性，需要解决如何整合领域知识、提升推理效率以及为领域专家提供知识整合指导等关键问题。

Method: EGO-Prompt从专家构建的初始语义因果图开始，通过两阶段的因果引导文本梯度过程：首先生成确定性推理指导，然后让LLM适应使用该指导与原始输入。迭代优化算法使用文本梯度和真实标签来精炼语义因果图和推理机制。

Result: 在公共卫生、交通和人类行为任务上测试，EGO-Prompt比前沿方法F1分数提高7.32%-12.61%，让小模型以不到20%的成本达到大模型性能，并输出精炼的领域特定语义因果图提升可解释性。

Conclusion: EGO-Prompt框架能有效自动化提示设计和推理过程优化，显著提升LLM在领域特定任务上的性能，同时提供可解释的因果图结构。
...
<details>
  <summary>Details</summary>
Motivation: LLM在需要单一确定性答案的任务上表现优秀，但在概率指令跟随(PIF)任务上表现不佳，存在偏见问题，影响需要非确定性行为的应用如人类行为模拟、内容多样化和多人游戏。

Method: SSoT提示方法：首先让LLM输出随机字符串生成足够熵，然后通过操作该字符串提取随机性来推导最终答案，在保持多样性的同时遵守特定约束。

Result: SSoT显著提高了LLM的PIF性能，接近伪随机数生成器的理想性能。在NoveltyBench上的实验显示SSoT还能增强开放任务的响应多样性。
...
<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型在现实部署中面临分布偏移和新任务挑战，静态零样本能力不足，需要持续学习方法在适应新任务时避免遗忘原有知识。

Method: 采用低秩适应技术，将任务特定的权重更新约束在模型参数的近似零空间内，最小化对已学知识的干扰。
...
<details>
  <summary>Details</summary>
Motivation: 现有因果关系发现方法依赖人工经验、统计方法或图形准则，容易出错、依赖理想化假设和大规模数据。多变量时间序列数据获取困难，现有方法容易过拟合。

Method: 使用组膨胀卷积和共享核指数级减少参数数量，学习带时间延迟的变量表示，结合全局和局部约束实现网络间信息共享。

Result: 在常用基准测试和生成数据集上的广泛实验表明，Shylock在少样本和正常多变量时间序列上都优于两种现有最先进方法。

Conclusion: Shylock方法有效解决了多变量时间序列因果关系发现中的过拟合问题，在少样本场景下表现优异，并开发了Tcausal库便于使用。
...
TL;DR: 提出了OutboundEval基准，用于评估LLM在专家级智能外呼场景中的表现，解决了现有方法在数据集多样性、用户模拟真实性和评估指标准确性方面的不足。
...
Method: 1) 设计涵盖6个主要业务领域和30个子场景的基准；2) 开发大模型驱动的用户模拟器；3) 引入动态评估方法，结合自动和人工评估。

Result: 在12个最先进的LLM上实验显示，专家级任务完成度和交互流畅性之间存在明显权衡，为构建可靠、类人的外呼AI系统提供了实用见解。
...
TL;DR: 这篇综述论文分析了自主系统中OOD检测技术，重点关注安全关键领域的应用，探讨了OOD检测在ML开发生命周期中的集成方法及其对安全保证的贡献。


<details>
  <summary>Details</summary>
Motivation: 随着AI自主系统在安全关键领域的应用扩展，需要可靠的方法来处理系统生命周期中的新情况和不确定情况，特别是检测分布外数据，以确保系统的安全运行。

Method: 采用系统性综述方法，分析OOD检测技术，定义相关概念，探讨OOD产生原因，识别ML开发生命周期中可用的技术，并提出在生命周期中集成这些技术的建议。

Result: 识别了多种可在ML开发生命周期中使用的OOD检测技术，提出了在系统生命周期中集成OOD检测以支持安全保证论证的具体方法，并指出了系统工程师需要注意的注意事项。
...
TL;DR: 本文提出并评估了多种自适应选择UCT探索常数λ的策略，使其对游戏奖励尺度不敏感，并推荐使用2σ作为λ值的新策略。


<details>
  <summary>Details</summary>
Motivation: UCT算法对游戏奖励尺度敏感，在具有密集奖励且奖励尺度人为设定的游戏中，不同游戏的Q值范围差异很大，这会影响算法性能。

Method: 评估了文献中已有的以及五种新的λ选择策略，包括使用Q值经验标准差的方法。

Result: 实验结果表明，选择λ=2σ（σ为搜索树中所有状态-动作对Q值的经验标准差）的新策略在各种任务中表现最优。

Conclusion: 推荐使用λ=2σ策略，该策略在单参数值和优化所有可用参数后的峰值性能方面均优于现有λ策略。
...
TL;DR: 论文提出Chain-of-Guardrail训练框架，通过重组或回溯不安全的推理步骤，在保持有效推理链的同时引导模型回到安全轨迹，显著提升大型推理模型的安全性而不损害推理能力。
...
Method: 提出Chain-of-Guardrail训练框架，分析模型推理轨迹发现Self-Jailbreak现象，通过重组或回溯不安全推理步骤，引导模型回到安全轨迹。

Result: 在多个推理和安全基准测试中，CoG显著提升了当前大型推理模型的安全性，同时保持了相当的推理能力，显著优于先前存在严重安全-推理权衡的方法。

Conclusion: CoG框架有效解决了大型推理模型的安全与推理权衡问题，证明了在不损害推理能力的前提下提升模型安全性的可行性。
...
TL;DR: 本文通过范围综述分析了AIES和FAccT会议中可信AI的研究现状，发现当前研究过度关注技术属性而忽视社会技术维度，提出了结合技术严谨性与社会文化因素的综合框架建议。


<details>
  <summary>Details</summary>
Motivation: 当前可信AI研究存在技术中心主义倾向，过度关注技术属性而忽视社会技术维度，需要全面理解AI可信度在真实世界环境中的含义。

Method: 对AIES和FAccT会议论文集进行范围综述，系统分析可信度的定义、操作化和应用方式，重点关注概念化方法、测量方法、验证技术、应用领域和基础价值观。

Result: 研究发现虽然技术属性定义取得进展，但存在关键差距：当前研究过度强调技术精度而牺牲社会伦理考量，AI系统的社会技术性质探索不足，可信度成为由有权定义者塑造的争议概念。

Conclusion: 需要结合技术严谨性与社会、文化、制度考量的跨学科方法推进可信AI，为AI伦理社区提出采用真正解决AI系统与社会复杂互动的整体框架的行动措施。
...
TL;DR: 提出了一种融合符号验证和交互验证的神经符号具身任务规划框架，通过在代码生成过程中进行环境交互验证来增强环境感知，显著提高了动态环境中的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的代码策略方法在动态或部分可观测环境中存在环境感知不足的问题，导致代码生成不准确或不完整，影响任务成功率。

Method: 在代码生成过程中加入显式符号验证和交互验证，生成探索性代码主动与环境交互获取缺失观测，同时保持任务相关状态。

Result: 在RLBench和真实世界环境中评估，任务成功率比Code-as-Policies基线提高46.2%，任务相关动作的可执行性达到86.8%以上。

Conclusion: 该神经符号框架通过增强环境感知显著提高了动态环境中任务规划的可靠性。
...
TL;DR: CXRAgent是一个基于LLM的胸部X光分析智能体，通过导演协调的多阶段框架整合工具调用、诊断规划和协作决策，提供视觉证据支持的可信诊断。
...
Method: 采用导演协调的三阶段框架：1)工具调用与证据驱动验证；2)基于任务需求的诊断规划和专家团队组建；3)协作决策与证据支持结论合成。

Result: 在多种CXR解读任务上表现优异，能提供视觉证据，并能良好泛化到不同复杂度的临床任务。

Conclusion: CXRAgent通过多阶段协作框架显著提升了CXR分析的适应性、可靠性和推理能力，为临床诊断提供了有效的AI支持。
...
TL;DR: 提出了Magellan框架，通过蒙特卡洛树搜索和分层引导系统，将创意生成重新定义为对LLM潜在概念空间的有原则探索，显著提升了生成想法的创新性和合理性。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在生成创新想法时倾向于高概率、熟悉概念的局限性，以及现有搜索方法依赖不可靠自评估启发式的问题。

Method: 使用蒙特卡洛树搜索(MCTS)和分层引导系统：语义罗盘向量进行长程方向引导，景观感知价值函数替代有缺陷的自评估，平衡内在连贯性、外在新颖性和叙事进展。
...
TL;DR: 提出融合强化学习的框架，提升1.5B模型在数学推理中的性能，显著减少40%以上的token使用量。


<details>
  <summary>Details</summary>
Motivation: 预算强制等测试时缩放方法依赖长上下文推理轨迹的监督微调，导致小模型因冗长响应而性能下降。

Method: 集成强化学习框架，仅使用1.5K训练样本，结合监督微调和强化学习提升token效率。

Result: 在GSM8K数据集上，SFT+RL模型在不同计算预算下表现更好，准确率更高，token使用量减少40%以上。
...
TL;DR: 本文系统回顾了神经符号AI在LLMs中的应用现状，提出了新的符号集成分类框架，并制定了未来研究路线图，旨在解决LLMs透明性不足的问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在关键领域表现出色但缺乏透明度，现有神经符号AI方法主要针对传统神经网络，不适用于LLMs的独特特性，需要系统性的符号集成方法。

Method: 首先回顾现有神经符号AI方法，然后提出LLMs中符号集成的新分类法，构建包含四个维度的分类框架：LLM不同阶段的符号集成、耦合机制、架构范式、算法和应用层面。

Result: 建立了全面的分类框架，系统组织了现有文献，识别了当前基准、前沿进展和关键差距，为增强LLMs透明度提供了实践指导。

Conclusion: 通过提出符号集成路线图，为未来研究指明方向，有助于开发增强LLMs透明度的框架，填补了符号AI与LLMs有效结合的系统性理解空白。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统优化问题求解需要人工建模和编程的繁琐过程，实现从图像输入到优化求解的全自动化，降低优化问题求解的技术门槛。

Method: 开发三模块框架：M1使用深度学习进行数学表达式识别生成LaTeX代码；M2使用微调LLM将LaTeX转换为PYOMO脚本；M3使用双层优化分解方法求解优化问题。

Result: MER模型在BLEU分数上优于ChatGPT、Gemini和Nougat；BOBD方法在复杂测试问题上比内点法和遗传算法表现更好。

Conclusion: AutoOpt框架实现了优化问题从图像到求解的全自动化，在复杂优化问题上表现出色，为优化问题求解提供了新的自动化解决方案。
...
TL;DR: 提出MoSES框架，通过状态可分解MDP和专家混合机制，让统一求解器能够重用基础VRP变体的专用求解器，解决现有方法忽视VRP组合结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多任务VRP神经方法学习统一求解器，但未能充分利用VRP变体的组合结构，每个变体都可由一组基础VRP变体推导而来，这导致统一求解器错失了专用基础求解器的优势。

Method: 提出状态可分解MDP(SDMDP)，将状态空间表示为与基础VRP变体相关的基础状态空间的笛卡尔积；开发基于潜在空间的SDMDP扩展，结合最优基础策略和可学习混合函数；实现MoSES求解器，通过专用LoRA专家实现基础策略，使用自适应门控机制实现混合函数。

Result: 在多个VRP变体上的广泛实验表明，MoSES优于先前的方法。

Conclusion: MoSES框架通过重用基础求解器，有效解决了统一求解器忽视VRP组合结构的问题，在多个VRP变体上表现出优越性能。
...
TL;DR: EU-Agent-Bench是一个用于评估LLM代理在欧盟法律框架下合规性的基准测试，通过验证性人工标注测试代理在良性用户输入可能导致非法行为时的法律遵从性。
...
TL;DR: Co-Sight通过冲突感知元验证和可信推理结构化事实机制，将推理转化为可证伪和可审计的过程，解决了LLM智能体长时程推理中的验证不足问题。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时程推理中失败往往不是因为生成能力弱，而是因为对中间推理的验证不足。

Method: 采用冲突感知元验证(CAMV)和可信推理结构化事实(TRSF)两种互补机制。CAMV将验证重新定义为冲突识别和针对性证伪，TRSF通过结构化事实模块持续组织、验证和同步证据。

Result: 在GAIA上达到84.4%的准确率，Humanity's Last Exam上达到35.5%，Chinese-SimpleQA上达到93.8%，实现了最先进的性能。

Conclusion: Co-Sight为LLM智能体的可靠长时程推理提供了一个可扩展的范式，结构化事实基础和冲突感知验证的协同作用驱动了这些改进。
...
TL;DR: 该论文提出了一种使用模仿学习(ICL)训练神经控制屏障函数(CBF)的方法，无需明确指定失败状态集，而是通过专家演示数据来学习安全约束，在四个环境中验证了其优于现有基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 在关键领域运行的自主系统中，安全是基本要求。传统控制屏障函数(CBF)的优化合成计算昂贵，且失败状态集往往难以形式化指定，而专家演示数据更容易生成。

Method: 使用模仿学习(ICL)训练约束函数来分类系统状态为安全或非安全，然后用该函数标注新的模拟轨迹来训练神经CBF。

Result: 在四个不同环境中的实证评估表明，该方法优于现有基线，且与使用真实安全标签训练的神经CBF性能相当。

Conclusion: 该方法提供了一种数据驱动的神经CBF学习方案，能够从专家演示中学习安全约束，无需明确指定失败状态集，在保持安全性的同时实现了良好的性能。
...
TL;DR: 本文提出Huxley-Gödel Machine (HGM)方法，通过估计代理的自我改进潜力(CMP指标)来指导代码修改树的搜索，在SWE-bench等基准测试中优于现有方法，并实现了人类水平的性能。


<details>
  <summary>Details</summary>
Motivation: 现有编码代理通过软件工程基准性能来指导自我改进，但作者发现代理的自我改进潜力与基准性能之间存在不匹配问题，需要更好的指标来评估代理的元生产力。

Method: 提出CMP指标来聚合代理后代的基准性能，作为自我改进潜力的指示器。基于此构建Huxley-Gödel Machine (HGM)，通过估计CMP来指导自我修改树的搜索。

Result: 在SWE-bench Verified和Polyglot基准测试中，HGM优于现有方法且使用更少的计算时间。优化后的代理在SWE-bench Lite上达到人类水平性能，匹配人类工程化编码代理的最佳结果。

Conclusion: CMP指标能有效评估编码代理的自我改进潜力，HGM方法在多个基准测试中表现出色，具有良好的迁移性和实用性。
...
TL;DR: DeepAgent是一个端到端的深度推理智能体，通过自主思考、工具发现和行动执行在单一连贯推理过程中完成任务，解决了长时交互中的上下文爆炸和错误累积问题。


<details>
  <summary>Details</summary>
Motivation: 现有智能体框架通常遵循预定义工作流，限制了自主性和全局任务完成能力。现实世界任务需要外部工具和长时交互，但现有方法存在上下文长度爆炸和交互历史累积的问题。

Method: 提出自主记忆折叠机制压缩过去交互为结构化记忆，减少错误累积；开发端到端强化学习策略ToolPO，利用LLM模拟API并通过工具调用优势归因分配细粒度信用。
...
TL;DR: 提出了AstaBench基准套件，用于更严谨地评估AI科学代理的能力，包含2400+个涵盖整个科学发现过程的问题，并提供可复现的评估环境和基线代理。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理评估基准存在不足：缺乏真实科学研究的整体衡量、缺乏可复现的工具、未考虑模型成本和工具访问等混杂变量、缺乏标准化接口、缺少全面基线代理。

Method: 定义基准原则和工具，开发AstaBench套件，包含科学问题集、生产级搜索工具环境，以及9类科学优化代理和多个基线。

Result: 对57个代理在22个代理类别上的广泛评估显示，AI在科学研究的某些方面取得进展，但距离解决科学研究辅助的挑战还很远。

Conclusion: 需要更严谨的基准来评估AI科学代理，AstaBench提供了这样的框架，但AI在科学辅助方面仍有很大提升空间。
...
<details>
  <summary>Details</summary>
Motivation: 简单的成对等价映射无法提供相关但不相交本体的完整语义集成，需要更复杂的多本体匹配来建立更细致的等价关系和本体层次上的溯源。

Method: 使用检索增强生成方法选择相关类来组成映射，并过滤匹配参考映射作为示例来增强上下文学习。
...
TL;DR: 提出了一个用于评估视觉语言模型的多模态框架分析数据集，包含专家标注的视频广告，涵盖13种框架类型、50多家公司和20个国家，特别关注能源行业的绿色洗白现象。


<details>
  <summary>Details</summary>
Motivation: 企业公关活动存在言行不一的问题，特别是石油天然气公司的绿色洗白现象。理解大规模框架及其变化有助于分析公关活动的目标和性质。

Method: 构建专家标注的视频广告数据集，从Facebook和YouTube获取，包含13种框架类型的标注，专门用于评估视觉语言模型。

Result: 基线实验显示GPT-4.1检测环境信息的F1分数为79%，而最佳模型在识别绿色创新框架方面仅达到46% F1分数。

Conclusion: 该数据集有助于能源行业战略沟通的多模态分析研究，同时揭示了视觉语言模型在处理隐含框架、不同长度视频和隐含文化背景等方面的挑战。
...
TL;DR: 提出了一个基于知识图谱的框架，用于解决自主代理在动态环境中高层任务目标与低层规划输入之间的语义鸿沟问题。
...
Method: 采用双平面架构的知识图谱作为智能翻译层，将声明性事实编译为每个代理的任务感知"世界观"和物理感知遍历规则，使任务语义与领域无关的规划器解耦。
...
### [151] [Incentivizing Consistent, Effective and Scalable Reasoning Capability in Audio LLMs via Reasoning Process Rewards](https://arxiv.org/abs/2510.20867)
...
<details>
  <summary>Details</summary>
Motivation: 音频大语言模型中的推理作用未被充分探索，引入推理过程往往会在推理时降低性能，出现测试时间逆缩放现象，即更长的推理链产生更差的结果。

Method: 提出CESAR框架，采用在线强化学习，使用组相对策略优化和多方面奖励机制，激励正确性、格式、一致性、结构化分析模式、因果推理、领域知识整合和校准推理深度。

Result: 解决了测试时间逆缩放问题，在MMAU Test-mini上达到最先进结果，显著优于Gemini 2.5 Pro和GPT-4o Audio，在MMSU推理任务上接近人类水平。
...
### [152] [Crisis-Resilient Portfolio Management via Graph-based Spatio-Temporal Learning](https://arxiv.org/abs/2510.20868)
*Zan Li,Rui Fan*

Main category: cs.LG

TL;DR: CRISP是一个基于图时空学习的金融预测框架，通过自适应图注意力机制发现危机相关的资产依赖关系，在通胀驱动市场等不同危机机制下实现稳健的资产配置预测。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的时空学习方法依赖预定义的图拓扑结构（如相关性阈值、行业分类），无法适应不同危机机制下市场动态的变化。

Method: 使用图卷积网络编码空间关系，BiLSTM与自注意力编码时间动态，通过多头图注意力网络学习稀疏结构，过滤92.5%的噪声连接。

Result: 在2005-2021年数据训练，在2022-2024年通胀驱动市场（完全不同机制）上实现稳健泛化，夏普比率3.76，比等权重基准提升707%，比静态图方法提升94%。

Conclusion: CRISP通过自适应学习危机相关依赖关系，无需强加假设即可实现跨危机机制的稳健预测，注意力权重提供可解释的机制检测。
...
### [153] [MOBO-OSD: Batch Multi-Objective Bayesian Optimization via Orthogonal Search Directions](https://arxiv.org/abs/2510.20872)
*Lam Ngo,Huong Ha,Jeffrey Chan,Hongyu Zhang*

Main category: cs.LG

TL;DR: MOBO-OSD是一种多目标贝叶斯优化算法，通过沿正交搜索方向求解多个约束优化问题来生成多样化的帕累托最优解集，在2-6目标问题上优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 多目标贝叶斯优化问题仍然具有挑战性，现有方法在生成多样化帕累托最优解集方面存在不足。

Method: 使用基于个体目标最小值近似凸包的正交搜索方向定义多个约束优化子问题，结合帕累托前沿估计技术生成邻域解，支持批量优化并行评估。

Result: 在多种合成和现实基准函数上，MOBO-OSD在解集多样性和超体积性能方面一致优于最先进算法。

Conclusion: MOBO-OSD通过正交搜索方向和帕累托前沿估计技术，有效解决了多目标贝叶斯优化中解集多样性和密度的问题。
...
TL;DR: CC-GRMAS是一个利用卫星观测和环境信号的滑坡预测框架，通过三个相互关联的智能体（预测、规划、执行）实现实时态势感知、响应规划和干预。


<details>
  <summary>Details</summary>
Motivation: 随着气候变化，滑坡灾害日益严重，特别是在高亚洲山区。尽管卫星和时序数据集日益丰富，但及时检测和灾害响应仍然不完善且分散。

Method: 采用多智能体协调框架，整合卫星观测和环境信号，通过预测、规划、执行三个智能体的协作来增强滑坡预测准确性。

Result: 该方法提供了一个可扩展的主动解决方案，能够提高滑坡预测的准确性，并实现实时态势感知和响应规划。

Conclusion: CC-GRMAS框架通过整合当地环境因素和多智能体协调，为脆弱山区的气候韧性灾害准备提供了有效的解决方案。
...
### [155] [Multimodal Negative Learning](https://arxiv.org/abs/2510.20877)
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态学习方法强制弱模态与主导模态对齐，这会抑制弱模态的独特信息。为了解决模态不平衡问题，需要一种新范式来保护弱模态的特定信息。

Method: 提出多模态负学习框架，主导模态动态引导弱模态抑制非目标类，稳定决策空间并保留模态特定信息。

Result: 实验证明该方法在多个基准测试中有效且具有通用性，特别是在噪声和不平衡场景下能减少弱模态的实证误差。

Conclusion: 负学习范式通过增加单模态置信度边界，从理论上收紧多模态学习的鲁棒性下界，有效解决了模态不平衡问题。
...
### [156] [HA-RAG: Hotness-Aware RAG Acceleration via Mixed Precision and Data Placement](https://arxiv.org/abs/2510.20878)
*Danying Ge,Jianhua Gao,Yixue Yang,Weixing Ji*

Main category: cs.LG

TL;DR: 提出了一种基于访问热度的RAG推理优化系统HA-RAG，通过混合精度压缩和热感知数据布局策略，显著降低了TTFT延迟。


<details>
  <summary>Details</summary>
Motivation: RAG通过引入外部知识库提高LLM输出准确性，但带来了长上下文处理的内存消耗和推理延迟问题。现有方法通过预计算KV缓存来加速推理，但仍有优化空间。

Method: 1. 基于KV块访问频率的热感知混合精度压缩和加载方法；2. 热感知数据布局策略，将高频访问KV块优先存储在高速内存中。

Result: 相比TurboRAG，HA-RAG在TTFT上平均加速2.10倍，最大加速10.49倍，准确率损失可忽略不计。

Conclusion: HA-RAG系统通过热感知优化策略，有效解决了RAG推理中的性能瓶颈，在保持准确性的同时显著提升了推理速度。
...
### [157] [Global Dynamics of Heavy-Tailed SGDs in Nonconvex Loss Landscape: Characterization and Control](https://arxiv.org/abs/2510.20905)
*Xingyu Wang,Chang-Han Rhee*

Main category: cs.LG

TL;DR: 本文研究了重尾SGD的全局动力学，发现通过注入并截断重尾噪声，SGD几乎可以完全避免尖锐局部极小值，从而获得更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 理解SGD避免尖锐局部极小值的能力，并进一步强化这种能力，需要超越传统的局部收敛分析，全面理解SGD的全局动力学。

Method: 基于大偏差和亚稳态分析的技术工具，对重尾SGD的全局动力学进行精确刻画，通过注入和截断重尾噪声来训练模型。

Result: 重尾SGD能够几乎完全避免尖锐极小值，找到几何更平坦的局部极小值，并在测试数据上实现更好的泛化性能。

Conclusion: 重尾SGD结合梯度裁剪能够有效避免尖锐极小值，提升深度学习模型的泛化能力，理论和实验均证实了这一发现。
...
### [158] [Learning from Interval Targets](https://arxiv.org/abs/2510.20925)
*Rattana Pukdee,Ziqi Ke,Chirag Gupta*

Main category: cs.LG

TL;DR: 提出了处理区间目标回归问题的新方法，包括兼容区间目标的损失函数和最小-最大学习框架，在真实数据集上实现了最先进的性能。
...
Method: 1. 使用兼容区间目标的损失函数；2. 提出最小-最大学习框架：在提供的区间内最小化最坏情况（最大化）的目标标签；3. 通过平滑约束解决最大化问题的非凸性。

Result: 在真实世界数据集上的广泛实验表明，所提方法达到了最先进的性能水平。

Conclusion: 所提出的方法能够有效处理区间目标回归问题，通过兼容区间目标的损失函数和最小-最大学习框架，在放松先前假设的同时获得了优异的性能。
...
### [159] [Meta-Learning for Cross-Task Generalization in Protein Mutation Property Prediction](https://arxiv.org/abs/2510.20943)
*Srivathsan Badrinarayanan,Yue Su,Janghoon Ock,Alan Pham,Sanya Ahuja,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 提出了一种结合元学习和新型突变编码策略的蛋白质突变属性预测方法，显著提升了跨数据集泛化能力和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖蛋白质特异性变换器的微调，但在跨数据集泛化方面表现不佳，主要原因是实验条件异质性和目标域数据有限。

Method: 1) 首次将模型无关元学习应用于蛋白质突变属性预测；2) 使用分隔符标记的新型突变编码策略，将突变直接整合到序列上下文中；3) 基于变换器架构与MAML结合，实现快速任务适应。

Result: 在三个不同蛋白质突变数据集上的评估显示显著优于传统微调方法：功能适应性预测准确率提升29%，训练时间减少65%；溶解度预测准确率提升94%，训练时间减少55%。
...
### [160] [LLM-Integrated Bayesian State Space Models for Multimodal Time-Series Forecasting](https://arxiv.org/abs/2510.20952)
*Sungjun Cho,Changho Shin,Suenggwan Jo,Xinya Yan,Shourjo Aditya Chaudhuri,Frederic Sala*

Main category: cs.LG

TL;DR: 提出了LBS框架，将大型语言模型与贝叶斯状态空间模型结合，用于多模态时间序列预测，支持灵活的时间窗口和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界预测中需要整合结构化时间序列数据和非结构化文本信息的问题，现有方法在输入输出时间窗口固定且无法建模不确定性方面存在局限。

Method: LBS包含两个组件：状态空间模型捕捉潜在状态的时间动态，预训练LLM编码文本输入进行后验状态估计并解码与潜在轨迹一致的文本预测。
...
### [161] [Safety Assessment in Reinforcement Learning via Model Predictive Control](https://arxiv.org/abs/2510.20955)
...
<details>
  <summary>Details</summary>
Motivation: 模型自由强化学习缺乏形式化安全保证，现有方法需要详细安全规范知识，而许多难以明确规范的安全问题更适合用不变性来表征

Method: 利用可逆性作为防止训练过程中安全问题的机制，使用模型预测路径积分控制检查学习策略提出的动作安全性，只需查询黑盒动态系统

Result: 实验结果表明，所提算法在所有不安全动作前成功中止，同时达到与允许违反安全的基线PPO方法相当的训练进度
...
### [162] [An Ensembled Penalized Federated Learning Framework for Falling People Detection](https://arxiv.org/abs/2510.20960)
...
<details>
  <summary>Details</summary>
Motivation: 解决老年人跌倒检测中传统方法泛化性差、隐私问题和个体行为差异等挑战。

Method: 集成惩罚联邦学习框架，结合持续学习、个性化建模和专门加权聚合策略，使用可穿戴传感器数据并通过同态加密保护隐私。
...
### [163] [Towards Scalable Oversight with Collaborative Multi-Agent Debate in Error Detection](https://arxiv.org/abs/2510.20963)
*Yongqiang Chen,Gang Niu,James Cheng,Bo Han,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 提出ColMAD协作式多智能体辩论协议，将辩论重构为非零和博弈，通过相互批评而非竞争来提高大语言模型错误检测的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论协议将辩论视为零和博弈，导致辩论黑客行为——辩论者倾向于误导裁判而非寻求真相，从而引入更多错误。

Method: ColMAD协议鼓励多个智能体以支持性方式相互批评，补充彼此缺失的观点，使裁判智能体能够基于更全面的证据做出结论。

Result: ColMAD在错误检测任务上显著优于先前竞争性多智能体辩论方法19%，相比单智能体方法也有显著提升。

Conclusion: 协作式多智能体辩论协议能有效缓解辩论黑客问题，提高大语言模型错误检测的可靠性，为可扩展监督提供更有效的方法。
...
### [164] [Neural Mutual Information Estimation with Vector Copulas](https://arxiv.org/abs/2510.20968)
*Yanzhi Chen,Zijing Ou,Adrian Weller,Michael U. Gutmann*

Main category: cs.LG

TL;DR: 提出了一种基于向量copula理论的互信息估计器，在模型复杂度和容量之间实现更好的平衡
...
Method: 基于近期向量copula理论，在两种极端方法之间进行原则性插值

Result: 在先进的合成基准测试和多种模态的真实世界数据上实验证明了所提估计器的优势

Conclusion: 该方法在复杂度和容量之间实现了更好的权衡，优于现有方法
...
### [165] [On the accuracy of implicit neural representations for cardiovascular anatomies and hemodynamic fields](https://arxiv.org/abs/2510.20970)
...
Result: 在胸主动脉血流场中实现约230的压缩比，压力最大绝对误差1mmHg，速度误差5-10cm/s；在48个胸主动脉解剖中，平均和最大解剖误差分别低于0.5mm和1.6mm。

Conclusion: SIREN、MFN-Gabor和MHE架构表现最佳，INRs在血流动力学场压缩和心血管解剖表示中具有高压缩比和低误差的潜力。
...
### [166] [L^2M^3OF: A Large Language Multimodal Model for Metal-Organic Frameworks](https://arxiv.org/abs/2510.20976)
*Jiyu Cui,Fang Wu,Haokai Zhao,Minggao Feng,Xenophon Evangelopoulos,Andrew I. Cooper,Yejin Choi*

Main category: cs.LG

TL;DR: L2M3OF是首个用于金属有机框架（MOF）的多模态大语言模型，通过整合晶体结构表示学习与语言理解，在材料性能预测和知识生成任务中超越了仅基于文本的闭源LLM。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在自然语言任务中表现出色，但在科学发现领域突破有限，因为理解复杂物理现象需要超越语言的多模态表示。MOF设计尤其依赖难以用文本编码的人类专业知识。

Method: L2M3OF整合了晶体表示学习与语言理解，使用预训练的晶体编码器和轻量级投影层将结构信息压缩到令牌空间，实现与语言指令的高效对齐。

Result: 实验表明L2M3OF在性能预测和知识生成任务中超越了GPT-5、Gemini-2.5-Pro和DeepSeek-R1等领先的闭源文本LLM，且参数量更少。
...
### [167] [Memory Constrained Dynamic Subnetwork Update for Transfer Learning](https://arxiv.org/abs/2510.20979)
...
<details>
  <summary>Details</summary>
Motivation: 设备端神经网络训练面临严重的内存限制，这限制了预训练模型在下游任务中的适配能力。需要开发能够在严格内存预算下有效进行模型微调的方法。

Method: 提出了LaRa（层排序）改进的层重要性度量方法，以及利用通道重要性分布时间稳定性的动态通道采样策略。在训练过程中根据重要性加权概率动态重采样通道。

Result: 在大量任务和架构上的广泛评估表明，MeDyate在极端内存约束下实现了最先进的性能，始终优于现有的静态和动态方法，同时保持高计算效率。

Conclusion: 该方法代表了向高效设备端学习迈出的重要一步，证明了在低至几百kB RAM的内存预算下仍能进行有效的微调。
...
### [168] [Learning Grouped Lattice Vector Quantizers for Low-Bit LLM Compression](https://arxiv.org/abs/2510.20984)
*Xi Zhang,Xiaolin Wu,Jiamang Wang,Weisi Lin*

Main category: cs.LG

TL;DR: 提出了一种分组格点向量量化(GL-VQ)框架，通过为每组权重分配定制化的格点码本，在低比特量化场景下实现更好的性能-模型大小权衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常需要大量计算资源和内存进行推理，标准均匀量化在低比特场景下会导致显著性能下降。

Method: 采用分组格点向量量化框架，为每组权重分配可学习生成矩阵定义的格点码本，使用Babai舍入近似最近格点搜索来解决量化过程的不可微性问题。

Result: 在多个基准测试中，该方法相比现有后训练量化基线实现了更好的模型大小与精度权衡。

Conclusion: GLVQ框架在严格资源约束下部署大模型方面表现出有效性，解码过程简化为简单的矩阵向量乘法，形成高效实用的量化流程。
...
### [169] [GPU Memory Requirement Prediction for Deep Learning Task Based on Bidirectional Gated Recurrent Unit Optimization Transformer](https://arxiv.org/abs/2510.20985)
...
### [170] [AL-CoLe: Augmented Lagrangian for Constrained Learning](https://arxiv.org/abs/2510.20995)
*Ignacio Boero,Ignacio Hounie,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 该论文重新探讨了增广拉格朗日方法在非凸机器学习约束问题中的应用，建立了强对偶性、证明了收敛性，并提供了泛化保证。


<details>
  <summary>Details</summary>
Motivation: 尽管现代机器学习参数化大多是非凸的，拉格朗日对偶已成为处理约束学习问题的流行工具，但增广拉格朗日方法在约束学习环境中相对未被充分探索。

Method: 使用增广拉格朗日方法来缓解非凸设置中的对偶间隙，仅需最小修改，包括建立强对偶条件、证明对偶上升算法的收敛性。

Result: 在温和条件下建立了强对偶结果，证明了对偶上升算法收敛到可行且最优的原始解，并提供了PAC风格的泛化保证。

Conclusion: 该方法在公平约束分类任务中表现出有效性，为约束学习问题提供了理论保证和实用解决方案。
...
### [171] [Exploring Spiking Neural Networks for Binary Classification in Multivariate Time Series at the Edge](https://arxiv.org/abs/2510.20997)
*James Ghawaly,Andrew Nicholson,Catherine Schuman,Dalton Diez,Aaron Young,Brett Witherspoon*

Main category: cs.LG

TL;DR: 提出了一种训练脉冲神经网络(SNNs)进行多元时间序列二分类的通用框架，通过进化算法联合优化网络架构和参数，在低功耗硬件上实现高性能分类。


<details>
  <summary>Details</summary>
Motivation: 开发一个通用框架，用于训练稀疏、状态化的脉冲神经网络，在低误报率下实现高精度分类，特别关注步进预测和硬件部署效率。

Method: 使用EONS进化算法联合优化SNNs的架构和参数，将输入编码为脉冲序列，通过输出神经元的脉冲计数阈值进行预测，并采用投票集成方法提升性能。

Result: 在放射性源检测任务中，仅49个神经元和66个突触的SNNs在1/小时误报率下达到51.8%真阳性率，优于PCA(42.7%)和深度学习(49.8%)基线；三模型集成提升至67.1%。硬件部署功耗仅2mW，延迟20.2ms。在癫痫检测任务中，集成方法达到95%真阳性率和16%假阳性率。

Conclusion: 该框架能够训练出高效、稀疏的SNNs，在多个时间序列分类任务中表现出色，特别适合低功耗硬件部署，展示了脉冲神经网络在实际应用中的潜力。
...
### [172] [Distilled Decoding 2: One-step Sampling of Image Auto-regressive Models with Conditional Score Distillation](https://arxiv.org/abs/2510.21003)
*Enshu Liu,Qian Chen,Xuefei Ning,Shengen Yan,Guohao Dai,Zinan Lin,Yu Wang*

Main category: cs.LG

TL;DR: DD2提出了一种新的条件分数蒸馏方法，用于实现图像自回归模型的一步采样，相比DD1显著减少了性能下降和训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有的图像自回归模型生成速度慢，DD1虽然实现了少步采样但仍存在一步采样性能显著下降和依赖预定义映射的问题。

Method: 将原始AR模型视为教师模型，提出条件分数蒸馏损失来训练一步生成器，在每个token位置基于先前token进行分数蒸馏。

Result: 在ImageNet-256上，DD2将FID从3.40轻微增加到5.43，相比DD1将一步采样与原始AR模型的差距减少了67%，同时训练速度提升12.3倍。

Conclusion: DD2为实现一步AR生成迈出了重要一步，为快速高质量的AR建模开辟了新可能性。
...
### [173] [Fair Representation Learning with Controllable High Confidence Guarantees via Adversarial Inference](https://arxiv.org/abs/2510.21017)
...
Method: 使用优化的对抗模型，提供高置信度公平性保证，确保下游预测的群体差异在用户定义的误差阈值内。

Result: 在三个真实世界数据集上的实验表明，FRG能在一系列下游模型和任务中持续限制不公平性。

Conclusion: FRG框架能有效学习具有高置信度公平性保证的表示，为公平表示学习提供了可靠解决方案。
...
### [174] [More Than Memory Savings: Zeroth-Order Optimization Mitigates Forgetting in Continual Learning](https://arxiv.org/abs/2510.21019)
*Wanhao Yu,Zheng Wang,Shuteng Niu,Sen Lin,Li Yang*

Main category: cs.LG

TL;DR: 本文研究了零阶优化在持续学习中的应用，发现零阶优化能产生更平坦的损失景观从而减少遗忘，但会牺牲可塑性。作者提出ZO-FC方法，在PEFT模块使用零阶优化，分类器使用一阶优化，平衡了稳定性和可塑性。


<details>
  <summary>Details</summary>
Motivation: 探索零阶优化作为解决持续学习中稳定性-可塑性-效率三难问题的新方法，利用其内存效率优势，并研究其在减少遗忘方面的潜力。

Method: 通过理论分析和实证研究评估零阶优化在持续学习中的表现，提出ZO-FC方法：在PEFT模块应用零阶优化，分类器使用一阶优化，以平衡稳定性和可塑性。

Result: 零阶优化能产生更平坦的损失景观，显著减少遗忘，但会削弱可塑性，特别是在训练预算受限时。ZO-FC方法在保持内存效率的同时，有效平衡了稳定性和可塑性。

Conclusion: 零阶优化是持续学习中有前景的方向，ZO-FC提供了一种实用且内存高效的解决方案，特别适用于设备端持续学习场景。
...
### [175] [From Information to Generative Exponent: Learning Rate Induces Phase Transitions in SGD](https://arxiv.org/abs/2510.21020)
*Konstantinos Christopher Tsiolis,Alireza Mousavi-Hosseini,Murat A. Erdogdu*

Main category: cs.LG

TL;DR: 该论文研究了神经网络特征学习动态，分析了学习率对梯度算法样本复杂度的影响，揭示了从信息指数机制到生成指数机制的相变，并提出了一种新的分层训练算法。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络特征学习动态，特别是学习率如何影响梯度算法的样本复杂度，以及如何超越相关查询的限制。

Method: 提出了一个涵盖相关和非相关更新的梯度算法框架，包括单次SGD、批量重用SGD以及新的分层训练算法（使用不同层的学习率）。

Result: 发现学习率选择存在相变：小学习率对应信息指数机制，大学习率对应生成指数机制。分层训练算法可以在不重用样本的情况下超越相关查询。

Conclusion: 学习率选择与算法设计同样重要，对实现统计和计算效率至关重要。
...
### [176] [CIPHER: Scalable Time Series Analysis for Physical Sciences with Application to Solar Wind Phenomena](https://arxiv.org/abs/2510.21022)
...
Method: CIPHER框架整合了iSAX进行可解释的压缩和索引，使用HDBSCAN进行密度聚类以分组重复现象，并加入人机交互步骤进行高效的专家验证。
...
### [177] [Physically consistent and uncertainty-aware learning of spatiotemporal dynamics](https://arxiv.org/abs/2510.21023)
*Qingsong Xu,Jonathan L Bamber,Nils Thuerey,Niklas Boers,Paul Bates,Gustau Camps-Valls,Yilei Shi,Xiao Xiang Zhu*

Main category: cs.LG

TL;DR: 提出物理一致性神经算子(PCNO)和扩散模型增强PCNO(DiffPCNO)，通过物理约束投影和一致性模型实现高精度、物理一致且能量化不确定性的时空预测。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法往往忽略物理定律且无法量化时空预测中的固有不确定性，需要开发既能保持物理一致性又能处理不确定性的预测方法。

Method: PCNO通过物理一致性投影层在傅里叶空间中强制实施质量和动量守恒；DiffPCNO在确定性预测基础上利用一致性模型来量化和减轻不确定性。

Result: PCNO和DiffPCNO在从湍流建模到真实世界洪水/大气预测的多种系统和空间分辨率下，实现了高保真度的时空预测，同时保持了物理一致性和不确定性量化能力。

Conclusion: 该两阶段框架为准确、物理基础扎实且具有不确定性感知能力的时空预测提供了稳健且通用的方法。
...
### [178] [Elementary, My Dear Watson: Non-Invasive Neural Keyword Spotting in the LibriBrain Dataset](https://arxiv.org/abs/2510.21038)
*Gereon Elvers,Gilad Landau,Oiwi Parker Jones*

Main category: cs.LG

TL;DR: 提出了关键词检测(KWS)作为脑机接口的实用中间任务，使用LibriBrain语料库提供标准化基准，开发了紧凑的1-D Conv/ResNet基线模型，在保持会话上达到置换基线AUPRC的13倍。
...
Method: 使用52小时的LibriBrain语料库，采用针对极端类别不平衡的评估协议，使用AUPRC和每小时误报率作为评估指标，开发了基于1-D Conv/ResNet的紧凑基线模型，采用焦点损失和top-k池化。

Result: 参考模型在保持会话上达到置换基线AUPRC的13倍，发现性能随训练时间对数线性提升，词频和持续时间等词级因素系统性影响可检测性。

Conclusion: 关键词检测是脑机接口的可行中间任务，提供了标准化基准和紧凑模型，为实际应用奠定了基础。
...
### [179] [Amortized Active Generation of Pareto Sets](https://arxiv.org/abs/2510.21052)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统多目标优化方法需要显式计算超体积、难以灵活整合用户偏好的问题，实现高效且可定制的帕累托集近似。

Method: 使用类别概率估计器预测非支配关系，结合偏好方向向量调节生成模型，通过帕累托成员资格和偏好对齐更新模型。

Result: 在合成基准和蛋白质设计任务中表现出强样本效率和有效偏好整合能力。

Conclusion: A-GPS是一个简单而强大的方法，能生成高质量帕累托集近似，灵活捕捉用户偏好，无需重新训练即可采样整个帕累托前沿。
...
### [180] [Online Multi-Class Selection with Group Fairness Guarantee](https://arxiv.org/abs/2510.21055)
*Faraz Zargari,Hossein Nekouyan,Lyndon Hallett,Bo Sun,Xiaoqi Tan*

Main category: cs.LG

TL;DR: 提出了一种在线多类选择问题的公平分配算法，通过无损舍入方案和资源预留机制确保群体公平性，同时支持多类别代理的处理。


<details>
  <summary>Details</summary>
Motivation: 解决现有文献中的两个关键限制：缺乏无损舍入方案导致性能损失，以及无法有效处理属于多个类别的代理。

Method: 基于松弛-舍入框架的随机算法，首先使用资源预留机制计算分数解，然后通过无损舍入方案保持公平性。还提出了结合机器学习预测的学习增强变体。

Result: 算法确保积分解与任何分数解具有相同的期望性能，同时保持跨类别的公平性保证。

Conclusion: 该框架成功解决了多类别在线分配中的公平性和效率平衡问题，无损舍入方案是关键创新点。
...
### [181] [On the Sample Complexity of Differentially Private Policy Optimization](https://arxiv.org/abs/2510.21060)
*Yi He,Xingyu Zhou*
...
<details>
  <summary>Details</summary>
Motivation: 随着策略优化在机器人、医疗和LLM训练等敏感领域的应用增加，隐私保护需求日益迫切，需要研究差分隐私下的策略优化理论。

Method: 提出了专门针对策略优化的差分隐私定义，通过统一框架系统分析策略梯度、自然策略梯度等算法在DP约束下的样本复杂度。

Result: 理论结果表明隐私成本通常表现为样本复杂度的低阶项，同时揭示了私有策略优化设置中的重要观察。

Conclusion: 研究为隐私保护策略优化算法提供了有价值的实践见解，隐私成本在样本复杂度中通常占次要地位。
...
### [182] [Scalable Machine Learning Analysis of Parker Solar Probe Solar Wind Data](https://arxiv.org/abs/2510.21066)
*Daniela Martin,Connor O'Brien,Valmir P Moraes Filho,Jinsu Hong,Jasmine R. Kobayashi,Evangelia Samara,Joseph Gallego*

Main category: cs.LG

TL;DR: 提出一个可扩展的机器学习框架，使用分布式处理和量子启发的核密度矩阵方法分析帕克太阳探测器数据，揭示内日球层太阳风参数的特征趋势。
...
Method: 利用Dask进行大规模统计计算，使用核密度矩阵方法估计太阳风参数的单变量和双变量分布，包括太阳风速、质子密度和质子热速度。

Result: 揭示了内日球层的特征趋势：太阳风速随日心距离增加而增加，质子密度减少，速度与密度呈反比关系。太阳风结构在极端空间天气现象中起关键作用。

Conclusion: 该方法为探索复杂物理数据集提供了可处理、可解释和分布式的解决方案，促进大规模原位测量的可重复分析，相关数据产品和工具已公开。
...
### [183] [The Virtues of Brevity: Avoid Overthinking in Parallel Test-Time Reasoning](https://arxiv.org/abs/2510.21067)
*Raul Cavalcante Dinardi,Bruno Yamamoto,Anna Helena Reali Costa,Artur Jordao*

Main category: cs.LG

TL;DR: 研究发现选择最短答案这一简单启发式方法在提升LLM推理能力方面与复杂方法效果相当，但计算成本显著降低。
...
Method: 采用选择最短答案的简单启发式方法，基于模型存在简洁自信和冗长过度思考两种推理模式的假设。

Result: 该方法在两个挑战性基准测试中与自一致性等复杂方法表现相当，同时显著降低计算开销。

Conclusion: 最短答案启发式方法提供了对自一致性的帕累托改进，即使在输出相等性定义不明确的任务中也适用。
...
### [184] [Neural Collapse under Gradient Flow on Shallow ReLU Networks for Orthogonally Separable Data](https://arxiv.org/abs/2510.21078)
...
<details>
  <summary>Details</summary>
Motivation: 理解深度网络成功背后的神经崩溃现象，特别是放松无约束特征的假设，研究数据结构和非线性激活对NC的影响，以及训练动态的隐式偏置作用。

Method: 使用梯度流分析两层ReLU网络在正交可分数据上的训练过程，证明其会表现出神经崩溃现象。

Result: 证明了梯度流在两层ReLU网络上对正交可分数据分类时确实会表现出神经崩溃现象。

Conclusion: 该研究推进了对神经崩溃现象的理论理解，揭示了数据结构和训练动态在NC形成中的重要作用。
...
### [185] [Accelerating Mobile Inference through Fine-Grained CPU-GPU Co-Execution](https://arxiv.org/abs/2510.21081)
...
Conclusion: 提出的轻量级同步机制和机器学习预测模型能有效克服移动设备上CPU-GPU协同执行的障碍，显著提升深度学习推理性能。
...
### [186] [DictPFL: Efficient and Private Federated Learning on Encrypted Gradients](https://arxiv.org/abs/2510.21086)
*Jiaqi Xue,Mayank Kumar,Yuzhang Shang,Shangqian Gao,Rui Ning,Mengxin Zheng,Xiaoqian Jiang,Qian Lou*

Main category: cs.LG

TL;DR: DictPFL是一个实用的联邦学习框架，通过分解模型权重为静态字典和可更新查找表，仅加密传输部分参数，实现全梯度保护同时显著降低计算和通信开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于同态加密的联邦学习方法存在两极化问题：要么加密所有梯度带来高昂开销，要么部分加密存在安全漏洞。需要一种既能保护隐私又实用的解决方案。

Method: 提出DePE模块将模型权重分解为静态字典和可更新查找表，仅加密传输后者；同时使用PrME模块通过加密感知剪枝最小化加密参数数量。
...
### [187] [M-GLC: Motif-Driven Global-Local Context Graphs for Few-shot Molecular Property Prediction](https://arxiv.org/abs/2510.21088)
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法依赖大量标注数据，而少样本分子属性预测面临数据稀缺问题。现有的基于分子-属性关系图的方法结构指导有限，需要更丰富的上下文信息来提升预测性能。

Method: 构建全局三元异质图（基序-分子-属性）捕获长程组合模式，同时为每个分子-属性对构建局部子图，分别编码以聚焦最相关的邻近信息。
...
### [188] [ESCORT: Efficient Stein-variational and Sliced Consistency-Optimized Temporal Belief Representation for POMDPs](https://arxiv.org/abs/2510.21107)
...
<details>
  <summary>Details</summary>
Motivation: 随着环境变得更加真实，信念分布变得复杂，标准数学模型无法准确捕捉，现有POMDP信念近似方法无法准确表示复杂不确定性结构，导致估计误差和次优行为。

Method: 扩展SVGD，引入相关感知投影来建模状态维度间的依赖关系，以及时间一致性约束来稳定更新同时保持相关结构，保留SVGD的吸引-排斥粒子动力学。

Result: 在POMDP领域和合成多模态分布上的广泛评估显示，ESCORT在信念近似精度和下游决策质量方面持续优于最先进方法。
...
### [189] [Distributionally Robust Feature Selection](https://arxiv.org/abs/2510.21113)
*Maitreyi Swaroop,Tamar Krishnamurti,Bryan Wilder*

Main category: cs.LG

TL;DR: 提出一种特征选择方法，通过连续松弛和噪声机制在多个子群体中平衡下游模型性能，无需反向传播训练过程。


<details>
  <summary>Details</summary>
Motivation: 在特征收集成本高昂的场景下，需要选择有限特征来训练能在多个子群体中同时表现良好的模型。

Method: 使用连续松弛的传统变量选择方法，结合噪声机制，通过优化贝叶斯最优预测器的方差来平衡不同群体的下游预测性能。

Result: 在合成数据集和真实世界数据上的实验验证了该方法的有效性。

Conclusion: 该方法提供了一种模型无关的框架，能够在特征收集成本约束下实现跨群体的性能平衡。
...
### [190] [SolarBoost: Distributed Photovoltaic Power Forecasting Amid Time-varying Grid Capacity](https://arxiv.org/abs/2510.21129)
*Linyuan Geng,Linxiao Yang,Xinyue Gu,Liang Sun*

Main category: cs.LG

TL;DR: SolarBoost提出了一种用于分布式光伏系统功率输出预测的新方法，通过将聚合功率输出建模为小电网输出的组合，解决了DPV系统面临的电网级数据缺失、容量变化等挑战。
...
### [191] [Cloud-Fog-Edge Collaborative Computing for Sequential MIoT Workflow: A Two-Tier DDPG-Based Scheduling Framework](https://arxiv.org/abs/2510.21135)
*Yuhao Fu,Yinghao Zhang,Yalin Liu,Bishenghui Tao,Junhong Ruan*

Main category: cs.LG

TL;DR: 提出基于DDPG的两层调度框架，用于医疗物联网中顺序工作流的调度优化，通过分层决策（全局控制器选择计算层，局部控制器分配节点）来最小化工作流完成时间。


<details>
  <summary>Details</summary>
Motivation: 医疗物联网对端到端延迟有严格要求，而调度顺序工作流以最小化完成时间是一个NP难问题，需要有效的调度策略。

Method: 使用两层DDPG强化学习框架：全局控制器负责选择计算层（边缘、雾或云），局部控制器负责在选定层内分配具体节点。
...
### [192] [Uncertainty-Aware Multi-Objective Reinforcement Learning-Guided Diffusion Models for 3D De Novo Molecular Design](https://arxiv.org/abs/2510.21153)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成高质量3D分子结构方面表现出色，但难以有效控制复杂多目标约束，这对实际应用至关重要。

Method: 利用具有预测不确定性估计的代理模型动态调整奖励函数，平衡多个优化目标，指导3D分子扩散模型的优化。

Result: 在三个基准数据集和多种扩散模型架构上的评估表明，该方法在分子质量和属性优化方面持续优于基线方法。分子动力学模拟和ADMET分析显示生成候选分子具有良好药物样行为和结合稳定性。
...
### [193] [Reducing the Probability of Undesirable Outputs in Language Models Using Probabilistic Inference](https://arxiv.org/abs/2510.21184)
...
Conclusion: RePULSe相比标准RL对齐方法和替代方案，在性能权衡和鲁棒性方面表现更优。
...
### [194] [A Unified Matrix Factorization Framework for Classical and Robust Clustering](https://arxiv.org/abs/2510.21172)
*Angshul Majumdar*
...
<details>
  <summary>Details</summary>
Motivation: 现有的聚类方法缺乏统一的数学框架，且对异常值敏感。需要建立经典聚类与矩阵分解的统一理论框架，并开发对异常值鲁棒的聚类方法。

Method: 1. 重新推导k-means与矩阵分解的等价关系；2. 首次形式化模糊c-means的矩阵分解解释；3. 用l1,2范数替换Frobenius范数构建鲁棒变体；4. 开发交替最小化算法和IRLS算法。

Result: 成功建立了经典聚类与矩阵分解的统一框架，提出了鲁棒聚类方法，所有算法都理论证明能收敛到局部最小值。

Conclusion: 该框架为经典和鲁棒聚类提供了统一的数学基础，鲁棒变体能有效处理异常值，算法具有理论收敛保证。
...
### [195] [A visual big data system for the prediction of weather-related variables: Jordan-Spain case study](https://arxiv.org/abs/2510.21176)
*Shadi Aljawarneh,Juan A. Lara,Muneer Bani Yassein*

Main category: cs.LG

TL;DR: 提出一个可视化大数据系统，用于处理气象数据，通过NoSQL数据库融合不同时空聚合级别的数据，进行温度和降雨预测分析。


<details>
  <summary>Details</summary>
Motivation: 气象领域产生大量数据，具有高维度、缺失值频繁和变量高度相关等特点，需要利用大数据和数据挖掘技术来提取有用知识并预测天气现象。

Method: 系统收集开放数据并加载到本地NoSQL数据库，在不同时空聚合级别融合数据，使用单变量和多变量方法进行预测分析，对于高缺失值情况使用邻近站点的训练数据进行预测。

Result: 系统在可用性和预测性能方面得到评估，整体归一化均方误差为0.00013，方向对称性接近0.84。专家对系统各方面（除图形设计外）评分均在3分以上（1-5分制）。
...
<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在特定任务上微调时会记忆敏感数据（如医疗记录、商业数据），存在隐私泄露风险，需要在不重新训练的情况下净化模型。

Method: SANI采用遗忘学习方法，包括：1）重置模型最后几层的特定神经元以破坏细粒度信息记忆；2）在避免记忆敏感信息的同时对模型进行微调。

Result: 仅需少量额外的遗忘学习轮次，模型就能被有效净化，信息泄露数量显著减少。

Conclusion: 该方法对医院等已投入大量资源训练模型的机构特别有用，可以在共享模型前进行净化处理。
...
### [197] [Scalable Principal-Agent Contract Design via Gradient-Based Optimization](https://arxiv.org/abs/2510.21177)
*Tomer Galanti,Aarya Bookseller,Korok Ray*

Main category: cs.LG

TL;DR: 提出了一个双层最大-最大优化框架用于委托-代理合同设计，使用隐式微分和共轭梯度法高效计算超梯度，无需形成或求逆Hessian矩阵。


<details>
  <summary>Details</summary>
Motivation: 委托-代理合同设计是道德风险和合同理论的核心问题，在现实环境中由于非线性效用、随机动态或高维动作，通常没有闭式解。

Method: 采用现代机器学习技术进行双层优化，使用隐式微分结合共轭梯度法，通过Hessian-向量乘积高效计算超梯度。

Result: 在CARA-Normal基准环境中能够恢复已知解析最优解，从随机初始化可靠收敛。框架可扩展到复杂非线性合同，如S形工资计划、相对绩效补偿、多任务合同和CARA-Poisson计数模型。
...
### [198] [FairImagen: Post-Processing for Bias Mitigation in Text-to-Image Models](https://arxiv.org/abs/2510.21363)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型（如Stable Diffusion）经常复制和放大社会偏见，特别是在性别和种族等人口统计属性上。

Method: 集成公平主成分分析，将基于CLIP的输入嵌入投影到最小化群体特定信息同时保留语义内容的子空间中，并通过经验噪声注入和统一的跨人口统计投影方法增强去偏效果。

Result: 在性别、种族和交叉设置上的广泛实验表明，FairImagen显著提高了公平性，在图像质量和提示保真度方面有适度权衡。

Conclusion: 该框架优于现有的后处理方法，为公平的文本到图像生成提供了一个简单、可扩展且模型无关的解决方案。
...
### [199] [PLAN: Proactive Low-Rank Allocation for Continual Learning](https://arxiv.org/abs/2510.21188)
*Xiequn Wang,Zhan Zhuang,Yu Zhang*

Main category: cs.LG

TL;DR: PLAN是一个持续学习框架，通过扩展LoRA来管理任务特定子空间的分配，使用正交基向量和基于扰动的策略来最小化与先前学习参数的冲突，从而减少对过去知识的遗忘。


<details>
  <summary>Details</summary>
Motivation: 持续学习需要模型在不遗忘过去知识的情况下不断适应新任务，现有方法在管理任务特定子空间分配和减少干扰方面存在不足。

Method: PLAN通过引入正交基向量、基于扰动的优化策略和选择机制来主动管理任务特定子空间的分配，识别并分配干扰敏感性最小的基向量。

Result: 在标准持续学习基准测试中，PLAN始终优于现有方法，为基础模型的持续学习建立了新的最先进水平。

Conclusion: PLAN框架通过主动管理任务特定子空间分配和最小化干扰，有效解决了持续学习中的遗忘问题，为基础模型的持续学习提供了高效解决方案。
...
### [200] [Gen-Review: A Large-scale Dataset of AI-Generated (and Human-written) Peer Reviews](https://arxiv.org/abs/2510.21192)
*Luca Demetrio,Giovanni Apruzzese,Kathrin Grosse,Pavel Laskov,Emil Lupu,Vera Rimmer,Philine Widmer*

Main category: cs.LG

TL;DR: 本文介绍了GenReview数据集，这是迄今为止最大的包含LLM生成评审意见的数据集，用于研究LLM在科学同行评审中的影响。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在科学同行评审中的潜在应用日益增多，需要全面了解其效用和影响，但目前缺乏相关数据集。

Method: 构建了包含81K条LLM生成评审意见的GenReview数据集，使用负面、正面和中性三种提示为2018-2025年ICLR会议的所有提交生成评审意见。

Result: 研究发现：LLM在评审中存在偏见；LLM生成的评审意见目前可被自动检测；LLM不总能严格遵循评审指南；LLM提供的评分仅与论文接受决策一致。

Conclusion: GenReview数据集为研究LLM在科学同行评审中的影响提供了重要资源，揭示了LLM评审的局限性和潜在风险。
...
### [201] [Online AUC Optimization Based on Second-order Surrogate Loss](https://arxiv.org/abs/2510.21202)
*JunRu Luo,Difei Cheng,Bo Zhang*

Main category: cs.LG

TL;DR: 提出了一种基于成对铰链损失的新型二阶代理损失函数，用于在线AUC优化，解决了传统方法在非凸性和内存成本方面的挑战。


<details>
  <summary>Details</summary>
Motivation: AUC是分类任务中重要的性能指标，特别是在类别不平衡场景下。但最小化AUC面临非凸不连续的成对0/1损失优化困难，以及大规模应用中实例存储的内存瓶颈问题。

Method: 提出二阶代理损失方法，直接使用训练数据的一阶和二阶统计量构建代理损失函数，替代传统的逐个成对损失近似方法，并开发了高效的在线算法和核方法扩展。

Result: 理论分析显示，现有在线AUC优化算法通常达到O(√T)遗憾界，而该方法获得更紧的O(ln T)界。在多个基准数据集上的实验证明了该方法在优化在线AUC性能方面的优越效率和效果。

Conclusion: 所提出的二阶代理损失方法为在线AUC优化提供了一种新的范式，在理论和实验上都表现出色，有效解决了传统方法的局限性。
...
TL;DR: 提出了一种名为CoD的反事实解释增强蒸馏方法，用于解决小样本任务感知知识蒸馏中的数据稀缺问题。该方法通过反事实解释来精确映射教师模型的决策边界，在仅使用少量样本的情况下显著提升蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有的任务感知知识蒸馏方法通常需要大量数据，但在实际场景中这些数据可能难以获取或成本高昂。本文旨在解决小样本场景下的知识蒸馏挑战。

Method: CoD策略通过系统地注入反事实解释来增强蒸馏过程。反事实解释是指能够以最小扰动翻转教师模型预测输出的输入样本。该方法利用这些CFE来精确映射教师决策边界。

Result: 在多个数据集和LLM上的实验表明，CoD在少样本场景（低至8-512个样本）中优于标准蒸馏方法。CoD仅使用基线方法一半的原始样本，配合对应的CFE仍能提升性能。

Conclusion: CoD方法通过反事实解释有效解决了小样本知识蒸馏问题，从统计和几何角度提供了理论保证，证明了CFE在蒸馏中的重要作用。
...
### [203] [Mitra: Mixed Synthetic Priors for Enhancing Tabular Foundation Models](https://arxiv.org/abs/2510.21204)
...
TL;DR: 本文系统研究了表格基础模型(TFMs)中合成先验分布的关键特性，并提出了Mitra模型，在分类和回归任务上超越了现有最佳TFMs。


<details>
  <summary>Details</summary>
Motivation: 现有的表格基础模型通过纯合成数据集预训练表现出色，但指导合成先验设计的原理尚不明确，需要填补这一研究空白。

Method: 系统研究合成先验的关键特性，基于这些洞察开发了Mitra模型，使用经过筛选的多样化、有区分度的合成先验混合进行训练。

Result: Mitra在分类和回归基准测试中持续优于TabPFNv2和TabICL等最先进的TFMs，并具有更好的样本效率。

Conclusion: 合成先验的设计对表格基础模型的泛化能力至关重要，Mitra证明了精心设计的先验混合能够显著提升模型性能。
...
### [204] [Adaptive Graph Mixture of Residual Experts: Unsupervised Learning on Diverse Graphs with Heterogeneous Specialization](https://arxiv.org/abs/2510.21207)
*Yunlong Chu,Minglai Shao,Zengyi Wo,Bing Hao,Yuhang Liu,Ruijie Wang,Jianxin Li*

Main category: cs.LG

TL;DR: ADaMoRE是一个自适应残差专家混合框架，通过无监督训练解决图神经网络在多样化真实图数据上的适应性问题


<details>
  <summary>Details</summary>
Motivation: 图神经网络面临基本适应性问题：固定的消息传递架构难以应对真实世界图的巨大多样性，其中最优计算策略随局部结构和任务而变化

Method: 采用骨干-残差专家架构，基础编码器提供稳定性，专用残差专家捕获多样化计算模式，结构感知门控网络执行细粒度节点路由，通过统一无监督目标进行端到端训练

Result: 在16个基准测试中验证了ADaMoRE在无监督节点分类和少样本学习中的最先进性能，具有优越的泛化能力、训练效率和更快收敛速度

Conclusion: ADaMoRE提供了一个原则性框架，能够稳健地无监督训练异构专家混合模型，显著提高了图神经网络的适应性和性能
...
### [205] [On the flow matching interpretability](https://arxiv.org/abs/2510.21210)
*Francesco Pivi,Simone Gazza,Davide Evangelista,Roberto Amadini,Maurizio Gabbrielli*

Main category: cs.LG

TL;DR: 提出了一种将物理约束嵌入流匹配生成模型的新框架，通过将流轨迹映射到二维Ising模型的平衡状态，使生成过程的每个步骤都具有物理可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决传统流匹配生成模型中间生成步骤缺乏可解释性的问题，通过将物理过程语义嵌入到生成流程中。

Method: 使用编码器将离散Ising配置映射到连续潜空间，流匹配网络执行温度驱动的扩散，投影器返回离散Ising状态并保持物理约束。

Result: 在多个晶格尺寸上验证了框架的有效性，在保持物理保真度的同时，随着晶格尺寸增大，生成速度优于蒙特卡洛方法。

Conclusion: 将物理语义嵌入生成流可以将不透明的神经轨迹转化为可解释的物理过程，每个向量场都代表了二维Ising模型潜空间中有意义的逐步转变。
...
### [206] [Model Merging with Functional Dual Anchors](https://arxiv.org/abs/2510.21223)
*Kexuan Shi,Yandong Wen,Weiyang Liu*

Main category: cs.LG

TL;DR: 提出Functional Dual Anchors (FDAs)框架，通过在输入表示空间建模来改进模型合并，相比传统参数空间方法更具鲁棒性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法在参数空间操作，受到参数不一致性的限制，需要更有效的知识整合策略。

Method: 使用功能性双锚点(FDAs)作为合成输入，其诱导的梯度与任务向量对齐，捕捉相对于预训练模型的任务特定功能偏移。

Result: 实验证明FDAs在模型合并中有效，且与参数空间模型合并方法互补。

Conclusion: FDAs框架为联合多任务训练和后验合并提供了桥梁，在模型合并中表现出色。
...
### [207] [How Hard is it to Confuse a World Model?](https://arxiv.org/abs/2510.21232)
*Waris Radji,Odalric-Ambrym Maillard*

Main category: cs.LG

TL;DR: 本文提出了针对神经网络世界模型的最混淆实例构造方法，通过对抗训练寻找统计上接近参考模型但使次优策略性能发散的替代模型，为深度基于模型的强化学习提供理论指导。


<details>
  <summary>Details</summary>
Motivation: 在强化学习理论中，最混淆实例对于建立遗憾下界至关重要，但在神经网络世界模型中构造此类实例仍是一个开放问题。

Method: 将问题形式化为约束优化，提出对抗训练程序来寻找统计上接近参考模型但使最优和次优策略性能发散的修改模型。
...
### [208] [Convergence of Stochastic Gradient Langevin Dynamics in the Lazy Training Regime](https://arxiv.org/abs/2510.21245)
...
<details>
  <summary>Details</summary>
Motivation: 连续时间模型为深度学习优化算法的训练动态提供了重要见解。本文旨在分析SGLD在惰性训练机制下的收敛性质，该机制是随机梯度下降在连续时间中的Itô随机微分方程近似。

Method: 在损失函数Hessian矩阵的正则条件下，分析SGLD与乘性和状态相关噪声的性质：(i) 证明训练过程中以高概率保持非退化核；(ii) 证明期望意义上的指数收敛到经验风险最小化器，并建立最优性间隙的有限时间和有限宽度界。

Result: 理论分析表明SGLD在惰性训练机制下具有非退化核和指数收敛性质，数值实验在回归设置中验证了理论发现。

Conclusion: 本文为SGLD在惰性训练机制下的收敛行为提供了严格的理论保证，证明了其能够有效收敛到经验风险最小化器，并通过数值实验验证了理论结果。
...
### [209] [Unified Implementations of Recurrent Neural Networks in Multiple Deep Learning Frameworks](https://arxiv.org/abs/2510.21252)
*Francesco Martinuzzi*

Main category: cs.LG

TL;DR: 提出了三个开源库（torchrecurrent、RecurrentLayers.jl、LuxRecurrentLayers.jl）来集中实现多种RNN变体，解决缺乏统一测试平台和重复实现的问题。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏中心化的RNN变体测试库，重新实现各种架构耗时且容易出错，限制了可重复性和探索性研究。

Method: 在Julia和Python中开发了三个开源库，提供一致的框架来构建和扩展RNN模型，包含内置的自定义和实验机制。

Result: 成功创建了集中化多种循环单元实现和高级循环架构的库，所有包都在GitHub上积极维护，采用MIT许可证。
...
### [210] [PINN Balls: Scaling Second-Order Methods for PINNs with Domain Decomposition and Adaptive Sampling](https://arxiv.org/abs/2510.21262)
*Andrea Bonfanti,Ismael Medina,Roman List,Björn Staeves,Roberto Santana,Marco Ellero*

Main category: cs.LG

TL;DR: 提出了PINN Balls模型，通过局部专家混合和可学习域分解结构，在保持可扩展性的同时提升了物理信息神经网络的训练效果。


<details>
  <summary>Details</summary>
Motivation: 解决二阶方法在训练物理信息神经网络时内存需求大、模型规模扩展性差的问题。

Method: 使用局部专家混合模型结合参数高效的集成模型和稀疏编码，并采用对抗自适应采样实现完全可学习的域分解结构。

Result: 在科学机器学习中达到了比现有技术更好的精度，同时保持了宝贵的可扩展性。

Conclusion: PINN Balls模型成功地将二阶训练方法应用于PINNs，在精度和可扩展性方面都取得了显著改进。
...
### [211] [Relieving the Over-Aggregating Effect in Graph Transformers](https://arxiv.org/abs/2510.21267)
*Junshu Sun,Wanxing Chang,Chenxue Yang,Qingming Huang,Shuhui Wang*

Main category: cs.LG

TL;DR: 提出了Wideformer方法来解决图注意力中的过度聚合问题，通过并行处理和引导机制避免关键信息被稀释。


<details>
  <summary>Details</summary>
Motivation: 图注意力在全局交互学习中面临过度聚合问题，即大量消息聚合到单个节点时会导致关键信息被稀释和信息丢失。

Method: Wideformer将节点聚合划分为并行过程，引导模型关注特定子集，限制每次聚合的输入量，并通过排序和加权优先处理信息丰富的消息。

Result: 评估显示Wideformer能有效缓解过度聚合问题，使骨干方法能专注于信息丰富的消息，相比基线方法获得更优性能。

Conclusion: Wideformer作为一种即插即用方法，通过并行处理和引导机制成功解决了图注意力中的过度聚合问题，提升了模型性能。
...
### [212] [Buffer layers for Test-Time Adaptation](https://arxiv.org/abs/2510.21271)
*Hyeongyu Kim,Geonhui Han,Dosik Hwang*

Main category: cs.LG

TL;DR: 提出基于Buffer层的新范式，解决传统测试时适应方法依赖归一化层更新的局限性，通过保持预训练主干完整性来避免灾难性遗忘，并在多种架构中实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法主要依赖归一化层更新，但存在对小批量大小敏感、受限于预训练模型结构等问题，限制了在显著域偏移下的有效性。

Method: 引入Buffer层的概念，不修改模型核心参数，保持预训练主干完整性，模块化设计可无缝集成到现有TTA框架中。
...
### [213] [Sensor-Specific Transformer (PatchTST) Ensembles with Test-Matched Augmentation](https://arxiv.org/abs/2510.21282)
*Pavankumar Chandankar,Robin Burchard*

Main category: cs.LG

TL;DR: 提出了一种基于PatchTST变换器的噪声感知集成方法，通过在训练时模拟测试噪声的数据增强，结合四个传感器位置的独立模型集成，在噪声条件下实现鲁棒的人类活动识别。


<details>
  <summary>Details</summary>
Motivation: 解决真实世界传感器数据中的噪声问题，通过在训练时模拟测试阶段的噪声模式，提高模型在噪声条件下的泛化能力。

Method: 使用PatchTST变换器架构，为四个惯性传感器位置分别训练独立模型，采用1秒滑动窗口和包含随机抖动、缩放、旋转、通道丢失的数据增强策略，最后通过软概率平均进行模型集成。

Result: 在私有排行榜上取得了显著高于基线的宏F1分数，证明了测试匹配增强与变换器集成策略在噪声HAR任务中的有效性。

Conclusion: 测试匹配的数据增强结合基于变换器的模型集成是在噪声条件下实现鲁棒人类活动识别的有效策略。
...
### [214] [Adaptive Data Selection for Multi-Layer Perceptron Training: A Sub-linear Value-Driven Method](https://arxiv.org/abs/2510.21286)
*Xiyang Zhang,Chen Liang,Haoxuan Qiu,Hongzhi Wang*

Main category: cs.LG

TL;DR: 提出了DVC方法，用于多层感知机训练中的数据选择，通过分层评估数据贡献度，在预算约束下实现更优的性能表现


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法在多层感知机训练中存在局限性：过度简化非线性变换、忽略隐藏层中间表示、计算复杂度高难以扩展到大型网络
...
Conclusion: 这是神经网络中分层数据评估的首个系统处理方法，为大规模机器学习系统提供了理论保证和实际优势
...
### [215] [Additive Models Explained: A Computational Complexity Approach](https://arxiv.org/abs/2510.21292)
*Shahaf Bassan,Michal Moshkovitz,Guy Katz*

Main category: cs.LG

TL;DR: 该论文分析了广义可加模型(GAMs)解释生成的复杂性，发现在不同输入空间结构、组件模型类型、回归与分类任务以及解释方法下，计算复杂度存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 挑战GAMs解释生成计算效率高的假设，研究不同形式GAMs在各种上下文中的解释生成计算复杂性。

Method: 通过计算复杂性分析，在P!=NP等标准复杂性假设下，研究GAMs解释生成在不同输入空间结构、组件模型类型、任务类型和解释方法下的复杂性。

Result: 发现GAMs解释复杂性受输入空间结构影响显著；不同组件模型的复杂性差异仅在特定输入域下显现；回归与分类任务的解释复杂性存在显著区别；神经网络可加化表达在某些解释方法和输入域下更易解释。

Conclusion: 为GAMs多样化解释的计算可行性提供了严格的理论图景，明确了在何种条件下这些计算是可能的或证明是困难的。
...
### [216] [An Evidence-Based Post-Hoc Adjustment Framework for Anomaly Detection Under Data Contamination](https://arxiv.org/abs/2510.21296)
*Sukanya Patra,Souhaib Ben Taieb*

Main category: cs.LG

TL;DR: EPHAD是一个测试时自适应框架，用于处理训练数据被异常值污染的无监督异常检测问题。它通过结合预训练AD模型的知识与多模态基础模型或经典AD方法提供的证据，在测试时更新模型输出。


<details>
  <summary>Details</summary>
Motivation: 现实世界的数据集通常包含未被检测或错误标记的异常值，这会导致无监督异常检测方法性能显著下降。现有解决方案需要访问训练流水线、数据或知道数据中异常比例的先验知识，限制了实际应用。

Method: 提出EPHAD框架，在测试时使用多模态基础模型（如CLIP）、经典AD方法（如潜在异常因子）或领域特定知识收集的证据，更新在污染数据集上训练的AD模型的输出。

Result: 在8个视觉AD数据集、26个表格AD数据集和1个真实工业AD数据集上的综合实验验证了EPHAD的有效性。消融研究分析了超参数影响和对不同污染水平的鲁棒性。

Conclusion: EPHAD展示了在不同AD模型和证据对中的多功能性和鲁棒性，为解决训练数据污染的异常检测问题提供了实用解决方案。
...
### [217] [Amortized Variational Inference for Partial-Label Learning: A Probabilistic Approach to Label Disambiguation](https://arxiv.org/abs/2510.21300)
...
<details>
  <summary>Details</summary>
Motivation: 现实数据通常存在噪声和歧义，部分标签学习处理每个实例与多个候选标签关联的情况。现有方法要么计算量大，要么依赖启发式标签精炼，需要更高效严谨的解决方案。

Method: 使用摊销变分推理，通过神经网络从输入数据预测变分参数来直接近似真实标签的后验分布，结合深度学习的表达能力和概率建模的严谨性。

Result: 在合成和真实数据集上的广泛实验表明，该方法在准确性和效率方面都达到了最先进的性能。

Conclusion: 该方法成功地将深度学习的表达能力与概率建模的严谨性相结合，为部分标签学习提供了高效且准确的解决方案。
...
### [218] [Data as a Lever: A Neighbouring Datasets Perspective on Predictive Multiplicity](https://arxiv.org/abs/2510.21303)
*Prakhar Ganesh,Hsiang Hsu,Golnoosh Farnadi*

Main category: cs.LG

TL;DR: 论文研究了数据对模型多样性的影响，发现数据点间的微小差异会导致模型多样性，并提出了一种邻接数据集框架来分析单数据点差异对多样性的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注建模选择对模型多样性的影响，而忽视了数据在塑造多样性中的关键作用。本文旨在填补这一空白，特别关注单数据点差异对多样性的影响。

Method: 引入邻接数据集框架分析单数据点差异对多样性的影响，通过严格的数学证明验证发现，并扩展应用到主动学习和数据插补两个实际领域。

Result: 发现邻接数据集在类间分布重叠更大时表现出更低的多样性，这与传统预期相反。提出了新颖的多样性感知数据获取策略和多样性感知数据插补技术。

Conclusion: 数据在模型多样性中扮演关键角色，单数据点差异能显著影响多样性。提出的多样性感知方法在主动学习和数据插补中具有实际应用价值。
...
### [219] [Revisiting Social Welfare in Bandits: UCB is (Nearly) All You Need](https://arxiv.org/abs/2510.21312)
*Dhruv Sarkar,Nishant Pandey,Sayak Ray Chowdhury*

Main category: cs.LG

TL;DR: 本文提出了一种简单而有效的算法，通过初始均匀探索阶段后接标准UCB算法，实现了近最优的Nash遗憾，适用于更广泛的奖励分布和公平性度量。


<details>
  <summary>Details</summary>
Motivation: 传统随机多臂老虎机中的遗憾度量无法解决奖励分配公平性问题，特别是在需要公平分配奖励的场景中，如临床试验中的患者分配。

Method: 采用初始均匀探索阶段后接标准上置信界(UCB)算法，仅依赖加性Hoeffding界，可扩展到次高斯奖励分布。

Result: 该算法实现了近最优的Nash遗憾，并推广到更广泛的p-均值遗憾公平度量，在所有p值上获得(近)最优遗憾界。

Conclusion: 相比现有需要强假设和专门设计的算法，该方法更简单且适用范围更广，在保证公平性的同时实现了最优性能。
...
### [220] [A Convergence Analysis of Adaptive Optimizers under Floating-point Quantization](https://arxiv.org/abs/2510.21314)
...
<details>
  <summary>Details</summary>
Motivation: 大规模语言模型的快速发展使得低精度训练变得至关重要，但现有收敛理论假设所有组件都是精确的，忽略了硬件感知量化，无法解释为什么低精度训练仍然有效。

Method: 引入了首个分析自适应优化器在梯度、权重和优化器状态浮点量化下的收敛性理论框架，在平滑非凸目标函数下推导收敛速率，明确量化不同组件的量化误差如何影响收敛。

Result: 理论分析表明Adam对权重和第二矩量化高度敏感，而Muon需要更弱的误差控制，因此可能更鲁棒。数值实验验证了理论结果。

Conclusion: 该研究缩小了低精度训练方法的实证成功与理论理解之间的差距，为低精度自适应优化器的设计提供了理论指导。
...
### [221] [SCORENF: Score-based Normalizing Flows for Sampling Unnormalized distributions](https://arxiv.org/abs/2510.21330)
*Vikas Kanaujia,Vipul Arora*

Main category: cs.LG

TL;DR: 提出ScoreNF框架，结合归一化流与独立Metropolis-Hastings模块，用于从未归一化概率分布中高效采样


<details>
  <summary>Details</summary>
Motivation: 传统MCMC方法收敛慢、存在临界减速和模式混合问题，而基于似然的机器学习方法需要大量数据且易出现模式覆盖和模式崩溃

Method: 基于归一化流架构的评分学习框架，集成独立Metropolis-Hastings模块，支持小训练集高效采样

Result: 在合成2D分布和高维φ⁴晶格场理论分布上验证了方法的有效性，即使小训练集也能保持高性能

Conclusion: ScoreNF提供了一种减少对计算昂贵MCMC训练数据依赖的高效无偏采样方法
...
### [222] [Weak-to-Strong Generalization under Distribution Shifts](https://arxiv.org/abs/2510.21332)
*Myeongho Jeon,Jan Sobotka,Suhwan Choi,Maria Brbić*

Main category: cs.LG

TL;DR: 提出了RAVEN框架，通过动态学习弱模型的最优组合和强模型参数，解决弱到强泛化在分布偏移下的失效问题，在图像分类、文本分类和偏好对齐任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 随着超人类模型日益复杂，人类可能无法有效监督其行为。现有弱到强泛化方法在分布偏移下会失效，导致强模型性能甚至不如弱监督者。

Method: RAVEN框架动态学习弱模型的最优组合和强模型参数，能够自动识别可信监督源。

Result: RAVEN在分布外任务上比基线方法提升30%以上，在分布内任务上匹配或超越现有方法，并能正确分配更高权重给更准确的弱模型。

Conclusion: RAVEN有效解决了弱到强泛化在分布偏移下的鲁棒性问题，为超人类模型的监督提供了可行方案。
...
### [223] [$α$-LoRA: Effective Fine-Tuning via Base Model Rescaling](https://arxiv.org/abs/2510.21345)
*Aymane El Firdoussi,El Mahdi Chayti,Mohamed El Amine Seddik,Martin Jaggi*

Main category: cs.LG

TL;DR: 本文提出了一种新的重参数化方法用于迁移学习，旨在提高微调模型的泛化能力，并通过随机矩阵理论和高维二分类设置验证了其有效性。
...
Method: 引入一类新的重参数化方法，在冻结权重矩阵基础上添加可训练权重矩阵，并通过随机矩阵理论分析其在高维二分类设置中的表现。

Result: 理论分析和实验验证表明，该方法能有效提升微调模型的泛化能力，特别是在LLM微调等实际场景中。

Conclusion: 提出的新重参数化方法在理论和实验上都显示出优于现有方法的泛化性能，为迁移学习提供了更有效的微调策略。
...
### [224] [Robust Yield Curve Estimation for Mortgage Bonds Using Neural Networks](https://arxiv.org/abs/2510.21347)
...
<details>
  <summary>Details</summary>
Motivation: 传统收益率曲线估计方法（如自举法和Nelson-Siegel模型）在债券稀疏、价格波动或噪声难以去除时容易出现过拟合或不稳定性问题，需要更稳健的估计方法。

Method: 使用神经网络框架，每天独立估计收益率曲线，并引入新的损失函数来强制平滑性和稳定性，同时允许集成领域特定约束（如与无风险基准对齐）。

Result: 在瑞典抵押债券上的实证结果表明，该方法比Nelson-Siegel-Svensson和Kernel-Ridge方法产生更稳健和稳定的收益率曲线估计。

Conclusion: 该神经网络框架为小型抵押债券市场提供了更可靠的收益率曲线估计方法，允许从业者在平滑性和准确性之间进行权衡，并集成领域知识约束。
...
### [225] [Compositional Monte Carlo Tree Diffusion for Extendable Planning](https://arxiv.org/abs/2510.21361)
*Jaesik Yoon,Hyeonseo Cho,Sungjin Ahn*

Main category: cs.LG

TL;DR: 提出C-MCTD框架，通过组合式规划提升MCTD方法，解决局部规划限制和训练轨迹长度限制问题


<details>
  <summary>Details</summary>
Motivation: MCTD方法受限于训练轨迹长度，且规划过程局部受限，无法访问全局上下文，需要提升到完整计划组合的推理层面

Method: 引入三个互补组件：在线组合器进行全局感知规划、分布式组合器通过并行探索降低搜索复杂度、预规划组合器利用缓存计划图加速推理

Result: C-MCTD将规划从单个轨迹优化提升到完整计划组合的推理层面，实现更有效的轨迹探索

Conclusion: C-MCTD框架通过组合式方法显著提升了MCTD的规划能力，解决了局部规划限制和轨迹长度限制问题
...
### [226] [Randomized Neural Network with Adaptive Forward Regularization for Online Task-free Class Incremental Learning](https://arxiv.org/abs/2510.21367)
*Junda Wang,Minghui Hu,Ning Li,Abdulaziz Al-Ali,Ponnuthurai Nagaratnam Suganthan*

Main category: cs.LG

TL;DR: 本文提出了一种基于随机神经网络和前向正则化的在线任务无关类增量学习框架，通过集成深度随机向量函数链接网络和贝叶斯自适应调整机制，有效解决非i.i.d数据流和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决类增量学习在实际应用中的两大问题：(1) 非i.i.d批次流且无边界提示的在线任务无关场景；(2) 长任务流中的记忆损失问题，旨在实现高效决策并减少累积遗憾。

Method: 提出随机神经网络与前向正则化框架，集成无监督知识到递归凸优化中；开发edRVFL-kF算法，生成单次闭式增量更新和可变学习率；进一步改进为edRVFL-kF-Bayes，通过贝叶斯学习自适应确定正则化强度。

Result: 在2个图像数据集上的6个指标测试显示，所提OTCIL框架在动态性能、消融测试和兼容性方面均表现出色，有效避免了灾难性遗忘并实现优越性能。

Conclusion: 提出的edRVFL-kF和edRVFL-kF-Bayes框架能够有效应对在线任务无关类增量学习挑战，通过自适应正则化和贝叶斯调整机制显著提升学习性能并缓解遗忘问题。
...
### [227] [Cost-Sensitive Freeze-thaw Bayesian Optimization for Efficient Hyperparameter Tuning](https://arxiv.org/abs/2510.21379)
*Dong Bok Lee,Aoxuan Silvia Zhang,Byungjoo Kim,Junhyeon Park,Steven Adriaensen,Juho Lee,Sung Ju Hwang,Hae Beom Lee*

Main category: cs.LG

TL;DR: 提出了一种基于冻结-解冻贝叶斯优化的成本敏感超参数优化方法，通过效用函数平衡计算成本和性能，并引入新的采集函数和停止准则。


<details>
  <summary>Details</summary>
Motivation: 解决用户在期望性能改进不理想时希望提前停止超参数优化过程的问题，考虑计算成本与性能之间的权衡。

Method: 在冻结-解冻框架中引入效用函数，结合新的采集函数和停止准则，并利用迁移学习提高样本效率。

Result: 在标准多保真度超参数优化基准测试中，算法优于所有考虑的冻结-解冻贝叶斯优化和迁移贝叶斯优化基线方法。

Conclusion: 该方法在成本和性能之间实现了更好的权衡，显著提升了超参数优化的效率。
...
### [228] [Disentangled Representation Learning via Modular Compositional Bias](https://arxiv.org/abs/2510.21402)
*Whie Jung,Dong Hoon Lee,Seunghoon Hong*

Main category: cs.LG

TL;DR: 本文提出了一种解耦的模块化归纳偏置——组合偏置，通过随机重混合潜在变量并配合两种互补目标函数，实现了无需修改架构或目标即可解耦属性、物体甚至两者的表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有的解耦表示学习方法严重依赖特定因子的策略（属性学习目标或物体模型架构），当新变化因子不符合先验假设或多个因子共存时，需要重新设计架构或目标，导致显著开销。

Method: 提出组合偏置框架：1）根据因子特定规则随机重混合潜在变量；2）使用先验损失确保每次重混合都能解码为真实图像；3）使用组合一致性损失对齐复合图像与对应复合潜在表示。

Result: 实验表明该方法在属性和物体解耦方面具有竞争力，并独特地实现了全局风格和物体的联合解耦。

Conclusion: 通过简单调整混合策略，无需修改目标或架构即可实现属性、物体甚至两者的解耦，为解耦表示学习提供了通用框架。
...
### [229] [Assessing the Real-World Utility of Explainable AI for Arousal Diagnostics: An Application-Grounded User Study](https://arxiv.org/abs/2510.21389)
*Stefan Kraft,Andreas Theissler,Vera Wienhausen-Wilke,Gjergji Kasneci,Hendrik Lensch*

Main category: cs.LG

TL;DR: 该研究通过用户实验评估了透明AI辅助在睡眠医学中的效果，发现透明AI作为质量控制步骤能显著提升性能，同时获得临床医生的广泛接受。


<details>
  <summary>Details</summary>
Motivation: AI系统在生物医学信号解读中表现优异，但临床实践中需要解决医生何时以及为何信任算法建议的问题，以实现有效的临床整合。

Method: 对8名专业睡眠医学从业者进行应用导向的用户研究，在三种条件下评估夜间觉醒事件评分：手动评分、黑盒AI辅助和透明白盒AI辅助，并比较不同辅助时机的影响。

Result: AI和人类-AI团队均显著优于无辅助专家，透明AI作为质量控制步骤比黑盒AI提升约30%的事件级性能，质量控制时机进一步改善计数结果，参与者普遍偏好透明性。
...
### [230] [Large Language Models as Model Organisms for Human Associative Learning](https://arxiv.org/abs/2510.21408)
*Camila Kolling,Vy Ai Vo,Mariya Toneva*

Main category: cs.LG

TL;DR: 使用大语言模型研究联想学习中的表征变化，发现中等相似项目在学习后会分化，这种分化受词汇干扰调节。


<details>
  <summary>Details</summary>
Motivation: 测试生物系统中表征变化假说具有挑战性，大语言模型提供了可扩展的替代方案来研究联想学习中的表征动态。

Method: 基于LLMs的上下文学习，改编认知神经科学联想学习范式，研究六个模型中表征如何演化，并分析词汇干扰的影响。

Result: 发现非单调塑性模式，中等相似项目在学习后分化；词汇干扰越高，分化越明显，表明表征变化受项目相似性和全局竞争影响。

Conclusion: LLMs不仅是研究类人学习系统表征动态的强大工具，也是生成关于大脑记忆重组原理新假说的可访问通用计算模型。
...
### [231] [Self-diffusion for Solving Inverse Problems](https://arxiv.org/abs/2510.21417)
*Guanxiong Luo,Shoujin Huang,Yanlong Yang*

Main category: cs.LG

TL;DR: 提出了一种名为self-diffusion的新框架，无需依赖预训练生成模型即可解决逆问题。该方法通过交替进行加噪和去噪步骤来逐步优化解估计。


<details>
  <summary>Details</summary>
Motivation: 传统基于扩散的方法需要先在干净数据集上训练模型来学习反向噪声过程，而self-diffusion旨在开发一种自包含的迭代过程，不依赖预训练模型。

Method: 引入自包含的迭代过程，交替进行加噪和去噪步骤。在每个步骤中，向当前估计添加噪声，然后通过数据保真度损失连续训练一个随机初始化的未训练卷积网络（自去噪器）来从噪声估计中预测解。

Result: 在线性逆问题的各种实验中，self-diffusion取得了与其他方法相当或更优的性能。

Conclusion: self-diffusion通过利用神经网络的频谱偏置并通过调度噪声过程进行调制，无需依赖预训练分数函数或外部去噪器，仍能适应任意前向算子和噪声观测，具有高度灵活性和广泛适用性。
...
### [232] [DreamerV3-XP: Optimizing exploration through uncertainty estimation](https://arxiv.org/abs/2510.21418)
...
Result: 在Atari100k和DeepMind Control Visual Benchmark任务上验证，确认了原始DreamerV3结果，同时显示学习速度更快、动态模型损失更低。

Conclusion: DreamerV3-XP通过提出的扩展方法有效提升了探索和学习效率，尤其在稀疏奖励设置中表现突出。
...
### [233] [A Rapid Physics-Informed Machine Learning Framework Based on Extreme Learning Machine for Inverse Stefan Problems](https://arxiv.org/abs/2510.21426)
*Pei-Zhi Zhuang,Ming-Yue Yang,Fei Ren,Hong-Ya Yue,He Yang*

Main category: cs.LG

TL;DR: 本文提出了一种基于物理信息的极限学习机（PIELM）框架，用于解决逆Stefan问题。该方法用极限学习机网络替代传统的深度神经网络，固定输入权重，通过最小二乘法优化输出权重，显著提高了预测精度和训练效率。
...
Method: PIELM使用极限学习机网络，固定输入权重，通过最小二乘法优化输出权重，将逆Stefan问题转化为寻找Moore-Penrose广义逆的问题。

Result: 与传统的PINNs相比，PIELM将相对L2误差提高了3-7个数量级，同时节省了超过94%的训练时间。

Conclusion: PIELM框架为逆Stefan问题提供了一种快速、准确的物理信息学习方法，在精度和效率方面都显著优于传统方法。
...
### [234] [Causality Meets Locality: Provably Generalizable and Scalable Policy Learning for Networked Systems](https://arxiv.org/abs/2510.21427)
...
Method: 每个智能体学习稀疏局部因果掩码识别影响动态的最小邻域变量，获得紧凑状态和领域因子表示；元actor-critic在多个源域训练共享策略，测试时通过少量轨迹估计新领域因子并部署适应策略。

Result: 建立了因果恢复、actor-critic收敛和适应差距的有限样本保证，GSAC能快速适应并显著优于从头学习和传统适应基线。

Conclusion: GSAC通过因果表示学习和元学习有效解决了大规模网络系统的可扩展性和领域泛化问题。
...
### [235] [Unified token representations for sequential decision models](https://arxiv.org/abs/2510.21448)
*Zhuojing Tian,Yushu Chen*

Main category: cs.LG

TL;DR: 提出统一令牌表示(UTR)方法，将回报目标、状态和动作合并为单个令牌，显著减少序列长度和模型复杂度，提升离线强化学习的效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有方法如决策变换器及其变体存在冗余令牌化和二次注意力复杂度问题，限制了在实时或资源受限环境中的可扩展性。

Method: 开发统一令牌表示(UTR)，将回报目标、状态和动作合并为单个令牌，并基于变换器和门控CNN骨干网络构建UDT和UDC两个变体。

Result: UTR方法在显著降低计算量的同时，实现了与最先进方法相当或更优的性能，理论分析显示其具有更紧的Rademacher复杂度边界。

Conclusion: UTR方法在不同架构上都具有良好的泛化能力，可能为未来大规模决策模型提供高效基础。
...
### [236] [ParaRNN: Unlocking Parallel Training of Nonlinear RNNs for Large Language Models](https://arxiv.org/abs/2510.21450)
*Federico Danieli,Pau Rodriguez,Miguel Sarabia,Xavier Suau,Luca Zappella*

Main category: cs.LG

TL;DR: ParaRNN是一个突破非线性RNN序列并行化障碍的框架，通过将非线性递推关系转化为方程组并使用牛顿迭代和并行归约求解，实现了665倍的加速，成功训练了70亿参数的LSTM和GRU模型。


<details>
  <summary>Details</summary>
Motivation: 传统RNN的序列特性限制了并行计算能力，而现有的并行化架构如Transformers和状态空间模型在线性约束下表达能力有限，无法建模复杂的非线性序列依赖关系。

Method: 将非线性递推关系序列转化为单一方程组，使用牛顿迭代结合自定义并行归约进行并行求解，实现了非线性RNN的高效并行训练。

Result: 实现了高达665倍的加速比，成功训练了70亿参数的LSTM和GRU模型，在困惑度指标上与同等规模的Transformers和Mamba2架构相当。

Conclusion: ParaRNN突破了非线性RNN的并行化障碍，为高效序列建模开辟了新途径，并开源了代码框架以促进相关研究。
...
### [237] [Towards Explainable Personalized Recommendations by Learning from Users' Photos](https://arxiv.org/abs/2510.21455)
...
<details>
  <summary>Details</summary>
Motivation: 解释复杂系统（如推荐系统）的输出对用户和公司都越来越重要。用户上传的照片可以强化或证明他们对物品的评价，因此预测用户会拍摄什么照片可以作为最有说服力的解释方式。

Method: 提出一个形式化框架来估计给定（用户，照片）对的作者概率，使用从TripAdvisor收集的餐厅评论（包含照片）数据来验证方法。

Result: 通过预测用户会为物品拍摄的吸引人的照片，不仅可以解释推荐结果，还能让公司了解客户关注的产品特点。

Conclusion: 将个性化解释作为推荐本身学习的方法能够提高推荐系统的可靠性，并为公司提供关于客户偏好的生动知识。
...
### [238] [Estimating Treatment Effects in Networks using Domain Adversarial Training](https://arxiv.org/abs/2510.21457)
*Daan Caljon,Jente Van Belle,Wouter Verbeke*

Main category: cs.LG

TL;DR: HINet提出了一种结合图神经网络和领域对抗训练的新方法，用于在未知暴露映射下估计网络环境中的异质处理效应，同时减轻网络级协变量偏移的影响。


<details>
  <summary>Details</summary>
Motivation: 现有因果机器学习方法通常假设已知的暴露映射，这在现实中往往不切实际。此外，同质性和处理分配机制之间的相互作用会引发网络级协变量偏移，导致处理效应估计不准确，这一现象尚未被明确研究。

Method: HINet整合了图神经网络与领域对抗训练，能够在未知暴露映射的情况下估计处理效应，同时缓解网络级协变量偏移的影响。

Result: 在合成和半合成网络数据集上的广泛实证评估证明了该方法的有效性。

Conclusion: HINet为解决网络环境中异质处理效应估计的挑战提供了一种有效方法，特别是在处理未知暴露映射和网络级协变量偏移方面表现出色。
...
### [239] [Generative Correlation Manifolds: Generating Synthetic Data with Preserved Higher-Order Correlations](https://arxiv.org/abs/2510.21610)
...
Method: 使用Cholesky分解目标相关矩阵的计算高效方法，生成能完整保留源数据集相关结构的合成数据。

Result: 通过数学证明，该方法能够保留从简单成对关系到高阶交互的整个相关结构。
...
TL;DR: 提出ZEN（零参数超图神经网络），一个完全线性和参数无关的模型，在11个真实超图基准上优于8个基线模型，同时实现高达696倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有的超图神经网络（HNNs）能够有效编码高阶结构，但由于复杂的黑盒架构，往往存在过拟合和可扩展性问题。

Method: 基于线性化HNNs的统一公式，ZEN引入了权重矩阵的可处理闭式解和冗余感知传播方案，避免迭代训练并消除冗余自信息。

Result: 在11个真实世界超图基准上，ZEN在分类准确率上持续优于8个基线模型，同时比最快的竞争对手实现高达696倍的加速。

Conclusion: ZEN实现了表达能力和效率的平衡，其决策过程完全可解释，为数据集特征提供了洞察。
...
TL;DR: 提出了第一个针对联邦持续时间序列预测中灾难性遗忘的基准框架，在12个分散客户端上系统评估了多种缓解策略。


<details>
  <summary>Details</summary>
Motivation: 灾难性遗忘在持续学习中是一个持续挑战，特别是在非独立同分布时间序列数据的联邦学习环境中。现有研究主要关注视觉领域的分类任务，而物联网和边缘应用中普遍存在的基于回归的预测设置尚未得到充分探索。

Method: 使用北京多站点空气质量数据集，在12个分散客户端上系统评估了多种灾难性遗忘缓解策略，包括重放、弹性权重巩固、无遗忘学习和突触智能等方法。

Result: 建立了第一个针对联邦持续时间序列预测中灾难性遗忘的基准框架，并进行了全面的比较分析。
...
TL;DR: 本文扩展了Vapnik和Chervonenkis的均匀收敛理论，提出了均匀均值可估性(UME-learnability)概念，证明了均值向量可分性是UME可学性的充分条件但非必要条件，并解决了关于可数并集可学性的猜想。


<details>
  <summary>Details</summary>
Motivation: 传统Vapnik-Chervonenkis理论局限于经验均值估计器，本文旨在超越这一限制，研究在任意估计器下分布集合是否允许均匀均值估计。

Method: 在分布集合的均值向量空间上工作，分析均值向量的可分性与均匀均值可估性的关系，并构造非可分但UME可学的反例。

Result: 证明了均值向量可分性是UME可学性的充分条件，构造了非可分但UME可学的反例，确立了可数并集的可学性。

Conclusion: 均匀均值可估性比传统经验均值收敛更一般，均值向量可分性是其充分但非必要条件，可数并集保持可学性。
...
### [243] [DEEDEE: Fast and Scalable Out-of-Distribution Dynamics Detection](https://arxiv.org/abs/2510.21638)
...
TL;DR: 提出一种基于多项式混沌展开的代理模型方法，用于量化生成流网络中由于奖励函数不确定性导致的策略不确定性。


<details>
  <summary>Details</summary>
Motivation: 生成流网络从噪声数据中学习奖励函数时存在认知不确定性，需要量化这种不确定性对策略的影响。

Method: 构建基于多项式混沌展开的代理模型，在小规模训练流网络集合上学习奖励函数参数与动作概率分布的关系，然后进行廉价蒙特卡洛采样。

Result: 方法在离散和连续网格世界、符号回归以及贝叶斯结构学习任务中展示了良好性能。

Conclusion: 所提出的代理模型方法能够有效量化生成流网络中由奖励函数不确定性引起的策略不确定性。
...
TL;DR: 提出统一模型(UM)解决8种灾后道路评估变体问题，通过单一神经网络训练实现多配置适应，相比单独模型训练时间减少8倍，在解决方案质量上优于单任务DRL方法6-14%和传统优化方法24-82%。


<details>
  <summary>Details</summary>
Motivation: 灾后道路评估对应急响应至关重要，但现有方法存在扩展性差、需要领域专业知识、单任务模型缺乏适应性等问题，无法满足动态灾害场景需求。

Method: 采用现代transformer编码器-解码器架构，通过轻量级适配器机制实现多问题配置训练，能够有效捕获共享结构知识并适应变体特定约束。

Result: 在多达1000个节点的网络中实现实时解决方案(1-10秒)，微调实验表明可以低成本有效整合未见属性并保持高质量解决方案。

Conclusion: 该统一模型为基于无人机的灾后道路评估提供了计算高效、高质量且适应性强的解决方案，推进了时间关键应用的神经组合优化。
...
TL;DR: 本文研究了在训练过程中使用探针检测器时如何避免Goodhart定律问题，提出了两种基于探针的训练方法，并发现基于偏好的优化能更好地保持探针检测能力，而探针多样性带来的益处有限。


<details>
  <summary>Details</summary>
Motivation: 探针可用于检测模型内部的不良行为，但当探针成为训练目标时可能失去可靠性（Goodhart定律）。本文旨在探索如何在训练中使用探针的同时保持其检测准确性。

Method: 提出了两种基于探针的训练方法：监督微调（SFT）和直接偏好优化（DPO）。在毒性减少测试平台上进行实验，评估训练后探针准确性的下降程度。尝试了三种策略来保持探针准确性：训练对抗探针集成、保留未用于训练的探针、训练后重新训练新探针。

Result: 基于探针的偏好优化意外地比基于分类器的方法更好地保持了探针检测能力，表明偏好学习目标激励模型维持而非混淆相关表征。探针多样性带来的实际益处有限——只需在优化后重新训练探针即可恢复高检测准确率。

Conclusion: 基于探针的训练对于某些对齐方法是可行的，但当重新训练可行时，探针集成基本上是不必要的。
...
### [247] [FrameShield: Adversarially Robust Video Anomaly Detection](https://arxiv.org/abs/2510.21532)
*Mojtaba Nafez,Mobina Poulaei,Nikan Vasei,Bardia Soltani Moakhar,Mohammad Sabokrou,MohammadHossein Rohban*

Main category: cs.LG

TL;DR: 提出了一种名为时空区域扭曲(SRD)的伪异常生成方法，通过在正常视频中应用严重增强来创建合成异常，结合噪声伪标签显著减少标签噪声，从而有效进行对抗训练，显著提升弱监督视频异常检测模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测模型对对抗攻击脆弱，但由于弱监督的固有约束（只有视频级标签但需要帧级预测），传统对抗防御机制如对抗训练效果不佳。伪标签虽然能实现帧级对抗训练，但本身噪声较大，会显著降低性能。

Method: 提出时空区域扭曲(SRD)方法，通过对正常视频中的局部区域应用严重增强来创建合成异常，同时保持时间一致性。将这些精确标注的合成异常与噪声伪标签结合，显著减少标签噪声。

Result: 在多个基准测试中，该方法显著提升了WSVAD模型对对抗攻击的鲁棒性，在整体AUROC性能上平均优于最先进方法71.0%。
...
### [248] [Excision Score: Evaluating Edits with Surgical Precision](https://arxiv.org/abs/2510.21537)
*Nikolai Gruzinov,Ksenia Sycheva,Earl T. Barr,Alex Bezzubov*

Main category: cs.LG

TL;DR: 提出了修订相似性问题，并开发了Excision Score (ES)方法来评估文档修订，该方法通过移除共享内容后比较差异区域，在代码编辑评估中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法（如BLEU）在评估文档修订时存在问题，因为它们被共享内容主导，无法准确反映人类对修订差异的判断。

Method: 提出Excision Score (ES)方法，使用最长公共子序列(LCS)移除现有文档与修订版本之间的共享内容，然后比较剩余的不同区域，并通过近似方法将计算复杂度从立方降至平方。

Result: 在HumanEvalFix上的实验显示，ES比最接近的竞争对手SARI提高了12%的皮尔逊相关性，比标准方法如BLEU提高了>21%。当增加共享上下文时，ES的改进进一步增加到20%和>30%。

Conclusion: ES方法通过关注修订中的差异区域，能更准确地评估文档修订质量，特别是在代码编辑任务中表现出色，解决了现有方法被共享内容主导的问题。
...
### [249] [Cost Minimization for Space-Air-Ground Integrated Multi-Access Edge Computing Systems](https://arxiv.org/abs/2510.21541)
*Weihong Qin,Aimin Wang,Geng Sun,Zemin Sun,Jiacheng Wang,Dusit Niyato,Dong In Kim,Zhu Han*

Main category: cs.LG

TL;DR: 提出了一种基于MADDPG-凸优化和联盟博弈的算法，用于优化空天地一体化边缘计算系统中的任务卸载、资源分配和轨迹规划，显著降低了用户设备成本。


<details>
  <summary>Details</summary>
Motivation: 解决空天地一体化边缘计算在低空经济应用中面临的异构节点协调、移动性建模和部分可观测环境下实时决策等挑战。
...
Conclusion: MADDPG-COCG算法有效解决了SAGIN-MEC系统中的复杂优化问题，为低空经济应用提供了高效的计算服务解决方案。
...
### [250] [Interpretable Multimodal Zero-Shot ECG Diagnosis via Structured Clinical Knowledge Alignment](https://arxiv.org/abs/2510.21551)
...
<details>
  <summary>Details</summary>
Motivation: 当前自动ECG诊断系统在透明性和对未见病症的泛化能力方面存在不足，需要更符合临床工作流程的可解释诊断方法。

Method: 使用LLM辅助、专家验证的过程构建结构化临床观察数据集，利用预训练多模态模型对齐ECG和文本嵌入，无需疾病特定微调。

Result: 实证评估显示ZETA在零样本分类性能上具有竞争力，并在定性和定量上提供了增强的可解释性证据，将预测结果基于具体的临床相关诊断特征。
...
### [251] [Leveraging Classical Algorithms for Graph Neural Networks](https://arxiv.org/abs/2510.21574)
*Jason Wu,Petar Veličković*

Main category: cs.LG

TL;DR: 通过在经典算法上预训练图神经网络，可以提升分子属性预测任务的性能，特别是在HIV抑制和临床毒性预测任务上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 神经网络擅长处理非结构化数据但泛化能力差，经典算法保证正确性但缺乏灵活性。研究是否可以通过在经典算法上预训练GNN来提升其在分子属性预测任务上的表现。

Method: 使用CLRS算法推理基准中的24个经典算法预训练GNN，然后将预训练模型的选定层用于初始化并冻结分子预测GNN的相应层。

Result: 相比随机初始化基线，预训练模型在ogbg-molhiv任务上取得6%绝对增益，在ogbg-molclintox任务上取得3%增益，表现出一致的优势或持平。

Conclusion: 将经典算法先验嵌入GNN中提供了有用的归纳偏置，能够提升在复杂真实世界图数据上的性能。
...
### [252] [An unsupervised tour through the hidden pathways of deep neural networks](https://arxiv.org/abs/2510.21582)
*Diego Doimo*

Main category: cs.LG

TL;DR: 该论文通过开发无监督学习工具研究深度神经网络内部表示机制，包括提出Gride方法估计数据内在维度、分析隐藏层概率密度演化模式，以及解释神经网络泛化能力与参数冗余的关系。
...
Method: 开发了Gride方法估计数据内在维度；分析隐藏层概率密度演化；研究宽神经网络中参数冗余与泛化的关系。

Result: 发现初始层生成单峰概率密度去除无关结构，后续层出现分层密度峰值反映概念语义层次；宽神经网络通过冗余表示而非过拟合实现泛化提升。

Conclusion: 深度神经网络通过分层密度峰值结构编码语义关系，参数冗余在正则化条件下有助于泛化而非导致过拟合。
...
### [253] [REVE: A Foundation Model for EEG -- Adapting to Any Setup with Large-Scale Pretraining on 25,000 Subjects](https://arxiv.org/abs/2510.21585)
*Yassine El Ouahidi,Jonathan Lys,Philipp Thölke,Nicolas Farrugia,Bastien Pasdeloup,Vincent Gripon,Karim Jerbi,Giulia Lioi*

Main category: cs.LG

TL;DR: REVE是一个专门为处理多样化EEG信号设计的预训练模型，通过创新的4D位置编码方案和掩码自编码目标，在92个数据集上预训练超过60,000小时EEG数据，在10个下游任务上取得最先进结果。


<details>
  <summary>Details</summary>
Motivation: 现有EEG基础模型难以泛化到不同采集协议、设备和电极配置的异构数据集，通常限制在单一设置下预训练，导致性能不佳，特别是在线性探测下。

Method: 引入新颖的4D位置编码方案处理任意长度和电极排列的信号，使用掩码自编码目标在92个数据集上预训练超过60,000小时EEG数据。

Result: 在10个下游EEG任务上取得最先进结果，包括运动想象分类、癫痫检测、睡眠分期、认知负荷估计和情绪识别，展示出强大的泛化能力和精细的时空建模能力。

Conclusion: REVE支持标准化EEG研究并加速临床神经科学进展，代码、预训练权重和教程已发布。
...
### [254] [Accelerating Data Generation for Nonlinear temporal PDEs via homologous perturbation in solution space](https://arxiv.org/abs/2510.21592)
*Lei Liu,Zhenxin Huang,Hong Wang,huanshuo dong,Haiyang Xin,Hongwei Zhao,Bin Li*

Main category: cs.LG

TL;DR: 提出了一种名为HOPSS的新数据生成算法，通过同源扰动方法在解空间中直接生成训练数据集，显著减少了传统数值方法生成数据所需的时间步数，加速了数据集生成过程。


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动的深度学习方法需要大量解对（解函数和方程右端项），这些数据通常通过传统数值方法生成，需要数千个时间步迭代，远多于训练所需的几十个步数，造成沉重的计算和时间开销。

Method: 首先从可靠求解器获取一组基础解函数，通过降采样对齐时间步；然后提出"同源扰动"方法：将两个解函数（一个作为主函数，另一个作为同源扰动项）与随机噪声结合，高效生成具有可比精度的PDE数据点；最后用这些数据点计算原方程右端项的变化来形成新的解对。

Result: 理论和实验结果表明HOPSS降低了时间复杂度。例如在Navier-Stokes方程上，生成10,000个样本仅需传统方法约10%的时间，同时保持可比的模型训练性能。

Conclusion: HOPSS算法能够显著加速PDE训练数据集的生成过程，同时保持数据精度，为数据驱动的深度学习方法提供了更高效的数据生成解决方案。
...
### [255] [SHAP Meets Tensor Networks: Provably Tractable Explanations with Parallelism](https://arxiv.org/abs/2510.21599)
*Reda Marzouk,Shahaf Bassan,Guy Katz*

Main category: cs.LG

TL;DR: 本文提出了计算张量网络(TNs)SHAP解释的通用框架，证明对于张量链(TT)结构的TN，SHAP计算可在多对数时间内并行完成，并将此结果推广到多种ML模型，揭示了在二值化神经网络中宽度而非深度是SHAP计算的主要计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 虽然SHAP对于简单模型如决策树可在多项式时间内计算，但对于神经网络等更复杂的黑盒模型，SHAP计算变得NP难。本研究旨在分析更广泛的张量网络模型的SHAP计算问题，这类模型比现有精确SHAP算法适用的模型更具表达力。

Method: 提出了计算任意结构张量网络精确SHAP解释的通用框架。特别地，当TN限制为张量链结构时，SHAP计算可通过并行计算在多对数时间内完成。利用二值化神经网络到张量网络表示的约简来证明计算复杂性。

Result: 对于张量链结构的TN，SHAP计算可在多对数时间内并行完成。该结果可推广到决策树、树集成、线性模型和线性RNN等流行ML模型。在二值化神经网络中，当网络宽度固定时SHAP计算变得高效可解，而即使深度恒定计算仍然困难。

Conclusion: 宽度而非深度是二值化神经网络SHAP计算的主要计算瓶颈。张量链结构的SHAP高效计算框架为多种ML模型提供了更紧的计算复杂性结果，使得在更广泛模型类别中高效计算精确SHAP解释成为可能。
...
### [256] [Generalised Flow Maps for Few-Step Generative Modelling on Riemannian Manifolds](https://arxiv.org/abs/2510.21608)
*Oscar Davis,Michael S. Albergo,Nicholas M. Boffi,Michael M. Bronstein,Avishek Joey Bose*

Main category: cs.LG

TL;DR: 提出了广义流映射(GFM)，一种新的少步生成模型，将欧几里得空间中的流映射框架推广到任意黎曼流形上。通过三种自蒸馏训练方法，在多个几何数据集上实现了最先进的样本质量和竞争性的对数似然。


<details>
  <summary>Details</summary>
Motivation: 当前基于黎曼流形的几何生成模型在推理时计算成本高昂，需要复杂的数值模拟步骤。需要开发更高效的少步生成方法。

Method: 提出了广义流映射(GFM)框架，包含三种自蒸馏训练方法：广义拉格朗日流映射、广义欧拉流映射和广义渐进流映射。将欧几里得空间中的少步生成模型统一并提升到黎曼流形设置。

Result: 在包括地理空间数据、RNA扭转角和双曲流形等几何数据集上的基准测试表明，GFM在单步和少步评估中实现了最先进的样本质量，并使用隐式概率流获得了优越或竞争性的对数似然。

Conclusion: GFM为几何生成提供了一种高效的少步生成框架，显著降低了推理成本，同时在多个几何数据集上保持了高质量的生成性能。
...
### [257] [Optimal Graph Clustering without Edge Density Signals](https://arxiv.org/abs/2510.21669)
*Maximilien Dreveton,Elaine Siyu Liu,Matthias Grossglauser,Patrick Thiran*

Main category: cs.LG

TL;DR: 本文建立了在流行度调整块模型(PABM)下图聚类的理论极限，揭示了传统模型忽略的聚类可分离性维度：即使边缘密度信号消失，只要簇内和簇间流行度系数不同，聚类恢复仍然可能。


<details>
  <summary>Details</summary>
Motivation: 现有图聚类模型存在局限性：随机块模型(SBM)假设顶点度均匀，度校正块模型(DCBM)在簇间应用统一的度校正。PABM通过引入簇内和簇间连接的独立流行度参数，能更好地捕捉度异质性。

Method: 在PABM框架下分析聚类的最优错误率，研究谱嵌入方法。由于PABM的期望邻接矩阵秩在k到k²之间，传统使用前k个特征向量的谱聚类可能失效，需要纳入k²个特征向量。

Result: 理论分析表明PABM中聚类恢复在传统边缘密度信号消失时仍可能，数值实验证实使用k²个特征向量的谱聚类算法优于传统方法。

Conclusion: PABM捕捉了DCBM忽略的度异质性维度，局部连接模式差异可独立于全局边缘密度增强簇可分离性，需要扩展谱方法以充分利用PABM的丰富结构。
...
### [258] [On Uncertainty Calibration for Equivariant Functions](https://arxiv.org/abs/2510.21691)
*Edward Berman,Jacob Ginesin,Marco Pacini,Robin Walters*

Main category: cs.LG

TL;DR: 本文研究了等变性与不确定性估计之间的关系，通过理论分析和实验验证了等变模型在分类和回归任务中的校准误差界限，揭示了对称性不匹配如何导致模型校准不良。


<details>
  <summary>Details</summary>
Motivation: 在数据稀疏场景下，等变网络能改善对欠采样区域的建模，不确定性估计能防止过度自信，但等变性与模型置信度/校准的关系尚未被研究。

Method: 通过证明等变条件下不确定性校准误差（ECE和ENCE）的上下界，建立等变性与不确定性估计的理论框架，并用真实和模拟数据集进行数值实验验证。

Result: 理论分析阐明了等变模型的泛化界限，实验显示了等变性与不确定性的关系，包括对称性不匹配、群大小以及偶然性和认知性不确定性的影响趋势。

Conclusion: 等变性对模型校准有重要影响，对称性不匹配会导致校准不良，该研究为理解等变模型的不确定性估计提供了理论基础和实证支持。
...
### [259] [Mechanistic Interpretability for Neural TSP Solvers](https://arxiv.org/abs/2510.21693)
*Reuben Narad,Leonard Boussioux,Michael Wagner*

Main category: cs.LG

TL;DR: 该论文首次将稀疏自编码器应用于Transformer旅行商问题求解器，揭示了神经网络内部学习的几何特征，包括边界检测器、聚类敏感特征和分隔符特征，为理解神经网络在组合优化中的工作机制提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的旅行商问题求解器能在毫秒级获得近似最优解，但这些模型作为黑盒运行，无法提供关于它们学习的几何模式或启发式策略的见解。

Method: 使用强化学习训练指针网络处理100节点实例，然后在编码器的残差流上拟合稀疏自编码器，以发现可解释特征的过完备字典。

Result: 分析显示求解器自然发展出反映基本TSP概念的特征：在凸包节点上激活的边界检测器、对局部密集区域响应的聚类敏感特征，以及编码几何分割的分隔符特征。

Conclusion: 这些发现首次提供了神经网络TSP求解器在节点选择前计算内容的模型内部解释，证明了几何结构在没有显式监督的情况下自然涌现，并为结合神经效率和算法可解释性的透明混合系统指明了路径。
...
### [260] [Equivariance by Contrast: Identifiable Equivariant Embeddings from Unlabeled Finite Group Actions](https://arxiv.org/abs/2510.21706)
*Tobias Schmidt,Steffen Schneider,Matthias Bethge*

Main category: cs.LG

TL;DR: 提出Equivariance by Contrast (EbC)方法，从观察对(y, g·y)中学习等变嵌入，其中g来自有限群。该方法联合学习潜在空间和群表示，无需依赖群特定的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 旨在开发一种通用方法，仅从群作用观察中学习等变嵌入，无需群特定的先验知识，可处理非阿贝尔群和乘积群。

Method: 通过对比学习联合学习潜在空间和群表示，使群作用在潜在空间中对应可逆线性映射。在dSprites数据集上验证，群结构为G:=(R_m × Z_n × Z_n)。

Result: 在dSprites数据集上获得高保真等变性，群操作在潜在空间中忠实再现。在合成数据上成功验证了O(n)和GL(n)群。
