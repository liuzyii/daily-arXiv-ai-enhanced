...
TL;DR: 该论文提出了一种基于脉冲神经网络（SNN）的神经形态眼动追踪模型，通过用轻量级LIF层替换传统模型的循环和注意力模块，并利用深度可分离卷积降低复杂度，在保持精度的同时实现了20倍模型压缩和850倍计算量减少。


<details>
  <summary>Details</summary>
Motivation: 可穿戴系统的眼动追踪需要低延迟和毫瓦级功耗，但传统基于帧的管道存在运动模糊、计算成本高和时间分辨率有限的问题。神经形态传感器和脉冲神经网络（SNN）提供了有前景的替代方案，但现有SNN方法要么过于专门化，要么性能不及现代ANN架构。

Method: 开发了高性能事件驱动眼动追踪模型的神经形态版本，用轻量级LIF（泄漏积分发放）层替换其循环和注意力模块，并利用深度可分离卷积来降低模型复杂度。

Result: 模型获得3.7-4.1像素的平均误差，接近专用神经形态系统Retina（3.24像素）的精度，同时相比最接近的ANN变体，模型大小减少20倍，理论计算量减少850倍。这些高效变体预计在1kHz频率下以3.9-4.9mW功耗和3ms延迟运行。

Conclusion: 研究表明，高性能的事件驱动眼动追踪架构可以重新设计为SNN，在保持适合实时可穿戴部署精度的同时，获得显著的效率提升。
...
<details>
  <summary>Details</summary>
Motivation: 弱监督定向目标检测（WS-OOD）作为全监督方法的成本效益替代方案受到关注，其中水平边界框（HBox）监督方法能直接利用现有HBox标注并达到最高精度。但现有方法直接将GT HBox与预测RBox的最小外接矩形比较，导致尺度估计不准确，且当所有增强视图预测都错误时会出现学习崩溃问题。

Method: 提出ABBSPO框架，包含两个核心组件：1）自适应边界框缩放（ABBS），通过适当缩放GT HBox来优化每个预测RBox的尺寸，确保更准确的尺度预测；2）对称先验角度（SPA）损失，利用航空目标的固有对称性进行自监督学习，解决当原始、旋转和翻转三个增强视图预测都错误时的学习崩溃问题。

Result: 大量实验结果表明，ABBSPO实现了最先进的性能，超越了现有方法。

Conclusion: ABBSPO框架通过自适应缩放和对称先验学习，有效解决了弱监督定向目标检测中的尺度估计和学习稳定性问题，为航空图像分析提供了更准确和鲁棒的解决方案。
...
TL;DR: 研究发现，在生成式训练中，目标表示的空间结构（而非全局语义性能）是影响生成质量的关键因素，并提出简单改进方法iREPA来增强空间信息传递。


<details>
  <summary>Details</summary>
Motivation: 研究表示对齐（REPA）中目标表示的关键因素：是全局语义信息（如ImageNet准确率）还是空间结构（patch tokens间的余弦相似度）对生成性能更重要。
...
Conclusion: 空间结构是表示对齐中影响生成性能的关键因素，简单的空间信息增强方法能显著改进生成式模型的训练效果，需要重新审视表示对齐的基本工作机制。
...
TL;DR: SST（Show, Suggest and Tell）提出了一种新的范式，将扩散模型作为建议模块与自回归生成结合，在图像描述任务上取得了SOTA结果。
...
Method: 提出SST框架，使用扩散模型为自回归生成提供建议，而不是替代它。具体来说，扩散模型作为"建议模块"，为自回归解码器提供候选词汇或短语建议，引导其生成更高质量的描述。

Result: 在COCO数据集上，SST在无强化学习的情况下达到125.1 CIDEr-D分数，比自回归SOTA高1.5分，比扩散模型SOTA高2.5分。实验表明建议质量与描述质量呈正相关。

Conclusion: 将扩散模型作为建议模块与自回归生成结合是一个有前景的研究方向，在图像描述任务上取得了显著改进，为离散域生成任务提供了新的范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法通常针对特定预测方向和特定输入变量训练条件分布模型，需要为不同任务分别训练模型，限制了临床应用的灵活性和效率。

Method: 提出MetaVoxel框架，学习一个跨越所有变量的单一扩散过程来建模影像数据和临床元数据的联合分布，支持使用任意输入子集进行零样本推理。

Result: 使用超过10,000个T1加权MRI扫描和九个数据集的临床元数据，单个MetaVoxel模型在图像生成、年龄估计和性别预测任务上达到与任务特定基线相当的性能。

Conclusion: 联合多模态扩散为统一医学AI模型和扩展临床应用提供了有前景的方向，MetaVoxel展示了灵活推理的能力。
...
Method: 提出独立密度估计(IDE)方法，学习句子中单个单词与图像特征的连接；构建两种模型：一种使用完全解耦的视觉表示作为输入，另一种使用变分自编码器从原始图像获取部分解耦特征；提出基于熵的组合推理方法结合句子中每个单词的预测
...
TL;DR: TraceFlow：基于残差材料增强2D高斯溅射的动态镜面场景高保真渲染框架，通过精确反射方向估计和物理准确反射建模，在动态场景基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 动态镜面场景的高保真渲染面临两个关键挑战：精确反射方向估计和物理准确的反射建模。现有方法在处理复杂动态环境中的镜面反射时存在局限性，需要新的框架来提升渲染质量和物理准确性。

Method: 1. 提出残差材料增强2D高斯溅射表示，建模动态几何和材料属性以实现准确反射光线计算；2. 引入动态环境高斯和混合渲染流水线，将渲染分解为漫反射和镜面反射分量，通过光栅化和光线追踪实现物理基础的镜面合成；3. 设计从粗到精的训练策略，提高优化稳定性并促进物理意义分解。

Result: 在动态场景基准测试上的广泛实验表明，TraceFlow在定量和定性评估中都优于先前方法，能够在复杂动态环境中产生更清晰、更逼真的镜面反射效果。

Conclusion: TraceFlow通过创新的表示方法、混合渲染流水线和优化策略，成功解决了动态镜面场景渲染中的关键挑战，实现了高质量的物理准确渲染，为动态场景的高保真渲染提供了有效解决方案。
...
TL;DR: 提出层次实例跟踪新任务，建立首个包含2765个实体、552个视频、40个类别的基准数据集，评估了7种模型变体，发现任务具有挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪任务主要关注单个对象或实例，缺乏对层次关系的建模。需要开发能够同时跟踪对象及其部件、并维护它们之间层次关系的新任务。

Method: 提出层次实例跟踪任务，创建首个支持该任务的基准数据集，包含2765个独特实体、552个视频、40个类别（涵盖对象和部件）。评估了四种模型的七个变体。

Result: 新数据集具有挑战性，现有模型在该任务上表现有限。数据集已公开可用，为未来研究提供了基准。

Conclusion: 层次实例跟踪是一个重要且具有挑战性的新任务，提出的数据集为研究社区提供了有价值的资源，有助于推动对复杂视觉关系的理解。
...
TL;DR: 使用基于小波的持续性同调进行拓扑数据分析，将图像结构转换为空间多尺度特征图，通过通道拼接增强乳腺癌检测模型的外部泛化性能


<details>
  <summary>Details</summary>
Motivation: 乳腺癌筛查中，虽然乳腺X光检查能降低死亡率，但存在大量假阴性和假阳性问题，且模型在不同扫描仪、模态和患者群体间部署时性能会下降，需要提高外部泛化能力

Method: 提出基于小波持续性同调的拓扑数据分析方法，将图像结构转换为空间多尺度特征图，通过输入级通道拼接集成到两阶段检测管道中

Result: 在INbreast数据集上，使用小波持续性通道增强ConvNeXt Tiny模型，患者级AUC从0.55提升至0.75（有限训练预算下）

Conclusion: 基于拓扑数据分析的小波持续性特征能显著提升乳腺癌检测模型的外部泛化性能，在不同设备和人群间保持稳定表现
...
TL;DR: MPEG FCM标准通过压缩DNN中间特征，实现边缘-云协同推理，在保持精度的同时平均减少85.14%比特率


<details>
  <summary>Details</summary>
Motivation: 边缘设备部署DNN面临计算资源限制，传统全模型边缘推理或云推理在延迟、带宽和隐私方面存在权衡。边缘-云协同推理需要传输中间特征，但会带来带宽挑战。

Method: MPEG启动FCM标准，设计专门用于压缩中间特征的比特流语法和编解码器流水线，提出特征编码测试模型（FCTM）。

Result: FCTM在多个视觉任务中实现显著比特率降低（平均85.14%），同时保持模型精度，为带宽受限和隐私敏感应用提供高效可互操作的智能特征部署方案。

Conclusion: FCM标准为边缘-云协同推理提供了有效的特征压缩解决方案，平衡了计算效率、带宽利用和隐私保护，支持智能特征在消费应用中的可扩展部署。
...
TL;DR: LCDrive提出了一种用于自动驾驶的潜在语言思维链模型，用动作对齐的潜在空间替代自然语言进行推理，通过动作提议token和世界模型token的交替表达来统一推理和决策。
...
Method: 提出Latent-CoT-Drive模型，在动作对齐的潜在空间中表达思维链：1) 使用与输出动作相同词汇表的动作提议token；2) 基于学习到的潜在世界模型的世界模型token，表达动作的未来结果。通过真实未来场景的监督冷启动，然后通过闭环强化学习进行后训练。

Result: 在大规模端到端驾驶基准测试中，LCDrive实现了更快的推理速度、更好的轨迹质量，相比非推理和文本推理基线，从交互式强化学习中获得了更大的性能提升。

Conclusion: 潜在语言思维链比自然语言更有效地统一了推理和决策，在自动驾驶任务中实现了更好的性能和效率。
...
TL;DR: FCM（特征编码）通过压缩神经网络特征而非原始像素，显著降低机器间通信带宽，同时保持任务精度并增强隐私保护。


<details>
  <summary>Details</summary>
Motivation: 传统机器间视觉通信依赖人类优化的视频编码，导致带宽消耗大、扩展性差、隐私泄露风险高，需要为机器通信设计专用编码方案。

Method: 提出FCM（特征编码）框架，压缩神经网络中间特征而非原始像素；评估H.26X系列编解码器作为FCM内部编解码器的性能差异。

Result: FCM能保持接近边缘推理的精度并大幅降低比特率；H.265/HEVC与H.266/VVC在机器任务性能上几乎相同，H.264/AVC性能较差；跟踪任务中编解码器选择影响最小。

Conclusion: FCM为机器间通信提供了高效、隐私保护的解决方案，现有已部署的H.265/HEVC硬件可支持机器通信而不降低性能，无需等待新编解码器部署。
...
TL;DR: MCDPO提出多奖励条件DPO方法，解决标准DPO中因Bradley-Terry模型将多维度评估聚合成单一标量奖励导致的奖励冲突问题，实现各奖励维度的独立优化和推理时动态控制。


<details>
  <summary>Details</summary>
Motivation: 标准DPO方法存在根本性限制：它依赖Bradley-Terry模型将美学质量、语义对齐等多样化评估维度聚合成单一标量奖励，导致奖励冲突——模型被迫遗忘特定维度的理想特征，如果这些特征出现在全局非优样本中。

Method: 提出多奖励条件DPO（MCDPO），通过引入解耦的Bradley-Terry目标解决奖励冲突。方法明确注入偏好结果向量作为训练条件，使模型能在单一网络中独立学习各奖励维度的正确优化方向。进一步引入维度奖励dropout以确保跨维度平衡优化。

Result: 在Stable Diffusion 1.5和SDXL上的大量实验表明，MCDPO在基准测试中实现了优越性能。特别地，条件框架能够在推理时使用Classifier Free Guidance动态控制多个奖励维度，无需额外训练或外部奖励模型。

Conclusion: MCDPO有效解决了标准DPO中的奖励冲突问题，通过多奖励条件框架实现了各评估维度的独立优化，并为推理时动态、多轴控制提供了灵活机制，显著提升了扩散模型对齐的性能。
...
TL;DR: 本文提出SWIFT方法，通过简单的分类器初始化和温度调优技术，解决视觉语言模型在半监督少样本学习中表现不佳的问题，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 半监督少样本学习（SSFSL）在实际应用如"自动标注"中很重要，但现有研究忽视了开源视觉语言模型（VLM）及其预训练数据的潜力。虽然相关领域（如少样本学习）已成功利用这些资源，但SSFSL尚未有效整合。

Method: 提出SWIFT（Stage-Wise Finetuning with Temperature Tuning）方法：1）使用分类器初始化和温度调优解决VLM输出概率分布"平坦"的问题；2）分阶段微调VLM，利用有限的标注数据、大量未标注数据以及从VLM预训练集中检索的任务相关但带噪声的数据。

Result: 在五个SSFSL基准测试上的实验表明，SWIFT比最近的FSL和SSL方法准确率提升约5个百分点，甚至能与使用真实标签进行监督学习的性能相媲美。

Conclusion: 通过简单的技术解决了VLM在半监督少样本学习中的关键问题，使现有SSL方法能够有效利用开源VLM资源，为实际自动标注应用提供了实用解决方案。
...
Method: 构建包含6500个视频的数据集，分为四类：原始干净视频、原始视频添加假水印、带水印的生成视频、去水印的生成视频。设计两个评估任务：任务I测试对去水印AI视频的检测性能，任务II评估对带假水印原始视频的误报率。测试十种模型，包括专用AIGC检测器、Transformer架构和MLLM方法。

Result: 实验显示水印操作导致性能变化2-8个百分点。Transformer模型表现出中等依赖（6-8pp），MLLM显示多样化模式（2-8pp）。这表明现有检测器部分依赖水印模式。
...
TL;DR: THE-Pose：通过表面嵌入和混合图融合利用拓扑先验的类别级6D姿态估计新框架，显著提升对未见复杂物体的鲁棒性
...
Method: 提出THE-Pose框架：1) 从图像域提取一致且不变的拓扑特征；2) 通过混合图融合(HGF)模块自适应整合拓扑特征与点云特征，桥接2D图像上下文和3D几何结构

Result: 在REAL275数据集上，相比3D-GC基线(HS-Pose)提升35.8%，超越先前最佳方法7.2%。对未见或复杂物体（即使严重遮挡）保持稳定

Conclusion: THE-Pose通过拓扑先验和混合特征融合，有效克服了现有3D-GC方法的局限性，实现了更鲁棒的类别级6D姿态估计
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图序列中心腔的准确分割对于心脏功能的定量分析至关重要，但成像噪声、伪影以及心脏的变形和运动给分割算法带来挑战。现有方法在捕捉长距离时空依赖性和保持计算效率与细粒度特征表示之间存在权衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有NCD方法主要依赖视觉特征，存在特征判别性不足和数据长尾分布等问题，需要突破这一瓶颈。

Method: 提出多模态框架LLM-NCD，融合视觉-文本语义和原型引导聚类，通过联合优化已知类图像和文本特征建模聚类中心和语义原型，采用双阶段发现机制动态分离已知/新类样本。

Result: 在CIFAR-100数据集上，相比现有方法，未知类准确率提升高达25.3%，首次在NCD文献中展示了对长尾分布的独特鲁棒性。

Conclusion: LLM-NCD通过多模态语义融合有效解决了传统NCD方法的局限性，显著提升了新类发现性能，特别是在长尾分布场景下表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1）可泛化高斯泼溅方法（如Long-LRM）直接预测数百万高斯参数容易出错，导致模糊；2）隐式表示方法（如LVSM、LaCT）渲染质量高但需要计算密集的解码过程，无法实时渲染。研究目标是能否保留隐式表示优势的同时实现实时性能。

Method: 采用半显式场景表示结合轻量级解码器的设计。该方法避免了直接预测大量高斯参数的错误敏感性，同时减少了隐式方法中深度顺序解码的计算负担，实现了质量和速度的平衡。

Result: 在DL3DV数据集上达到与LaCT相当的渲染质量，同时在A100 GPU上实现14 FPS的实时渲染；支持扩展到64个950×540分辨率输入视图；在ScanNetv2上提供优于高斯泼溅直接深度渲染的新视角深度预测。

Conclusion: Long-LRM++成功解决了隐式表示方法的实时渲染瓶颈，通过半显式表示和轻量解码器设计，在保持高质量渲染的同时实现了实时性能，并展示了良好的可扩展性和深度预测能力。
...
TL;DR: 提出SAAD方法，通过样本级自适应权重调整解决对抗蒸馏中的鲁棒饱和问题，提升学生网络鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有对抗蒸馏方法常忽略使用最先进的鲁棒教师网络，且发现更强的教师不一定产生更鲁棒的学生（鲁棒饱和现象）。传统归因于容量差距的解释不完整，需要更深入理解鲁棒性传递机制。

Method: 提出样本级自适应对抗蒸馏（SAAD），通过测量学生生成的对抗样本对教师的有效性（对抗可转移性），以此重新加权训练样本，无需额外计算成本。

Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet数据集上的实验表明，SAAD相比先前方法能持续提升AutoAttack鲁棒性。

Conclusion: 对抗可转移性是鲁棒性成功传递的关键因素，SAAD通过样本级自适应权重调整有效解决了鲁棒饱和问题，提升了对抗蒸馏的效果。
...
TL;DR: MotionEdit是一个专注于运动中心图像编辑的新数据集和基准测试，用于评估和改进模型在修改主体动作和交互方面的能力，同时保持身份、结构和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑数据集主要关注静态外观变化，或者只包含稀疏、低质量的运动编辑。运动中心图像编辑任务在科学上具有挑战性，在实际应用中也很重要，可以支持帧控制视频合成和动画等下游应用。

Method: 1. 创建MotionEdit数据集：从连续视频中提取和验证高质量的运动变换图像对
2. 建立MotionEdit-Bench基准测试：使用生成式、判别式和基于偏好的指标评估模型性能
3. 提出MotionNFT框架：通过运动对齐奖励引导模型，确保输入图像和编辑后图像之间的运动流与真实运动匹配

Result: 基准测试显示现有最先进的扩散编辑模型在运动编辑任务上仍然面临很大挑战。MotionNFT框架在FLUX.1 Kontext和Qwen-Image-Edit模型上显著提高了编辑质量和运动保真度，同时不牺牲一般的编辑能力。

Conclusion: MotionEdit为运动中心图像编辑提供了首个高质量数据集和基准测试，而MotionNFT框架有效解决了现有模型在该任务上的不足，为运动编辑任务提供了实用的解决方案。
...
TL;DR: ShotDirector：通过参数级相机控制和分层编辑模式提示，实现电影级可控镜头转场的视频生成框架


<details>
  <summary>Details</summary>
Motivation: 现有多镜头视频生成方法主要关注低层次视觉一致性，忽视了镜头转场设计和电影语言对叙事连贯性的贡献，导致镜头变化缺乏有意的电影编辑模式

Method: 提出ShotDirector框架：1）采用包含6自由度位姿和内在设置的相机控制模块实现精确相机信息注入；2）使用镜头感知掩码机制引入专业编辑模式的分层提示，实现镜头内容的细粒度控制

Result: 构建了ShotWeaver40K数据集捕获电影编辑模式先验，开发了可控多镜头视频生成评估指标，实验证明了框架的有效性

Conclusion: ShotDirector通过结合参数级条件和高层次语义指导，实现了电影级可控镜头转场，解决了现有方法缺乏有意图编辑模式的问题
...
TL;DR: Disentangled360是一个创新的3D感知技术，通过方向解耦的体积渲染实现单图像360°独特视图合成，应用于医学成像和自然场景重建。
...
Method: 1. 在Gaussian Splatting骨干网络中区分各向同性和各向异性贡献；2. 双分支条件框架：一个针对CT强度驱动的体数据散射，另一个针对真实世界RGB场景；3. 混合姿态无关锚定方法解决尺度模糊并保持结构真实感；4. 集成术前放射学模拟和消费级360°渲染到单一推理管道。

Result: 在Mip-NeRF 360、RealEstate10K和DeepDRR数据集上表现出优越的SSIM和LPIPS性能，运行时评估确认其适用于交互式应用。

Conclusion: Disentangled360支持混合现实医学监督、机器人感知和沉浸式内容创作，无需场景特定微调或昂贵的光子模拟，实现了快速、逼真的视图合成。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉语言导航中面临两大训练开销问题：1) 处理长历史观察序列时的二次计算负担；2) DAgger方法中探索与效率的权衡问题（更多探索带来更长轨迹）。

Method: 提出Efficient-VLN：1) 渐进式记忆机制，动态分配更多token给近期观察；2) 可学习递归记忆，使用可学习token的键值缓存作为记忆状态；3) 动态混合策略平衡探索与效率。

Result: 在R2R-CE上达到64.2% SR，RxR-CE上达到67.0% SR的SOTA性能，仅消耗282 H800 GPU小时，相比现有方法大幅降低训练开销。

Conclusion: Efficient-VLN通过高效记忆机制和动态混合策略，在保持高性能的同时显著降低了多模态大语言模型在视觉语言导航中的训练成本，为实际应用提供了可行方案。
...
TL;DR: 提出基于原型驱动的视觉-语言对齐框架，通过双模态原型库（文本原型+图像原型）和多尺度金字塔模块，改进组织病理学弱监督语义分割，在BCSS-WSSS基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 组织病理学弱监督语义分割面临三大挑战：类间同质性、类内异质性以及CAM监督的区域收缩效应。现有方法依赖图像级标签，难以准确发现区域边界。

Method: 1. 采用CoOp风格可学习提示调优生成文本原型；2. 结合可学习图像原型形成双模态原型库；3. 引入多尺度金字塔模块缓解ViT表示过平滑问题；4. 利用文本语义和视觉原型学习的互补性。

Result: 在BCSS-WSSS基准测试中超越现有SOTA方法。分析显示文本描述多样性、上下文长度以及文本与图像原型的互补行为均有显著益处。

Conclusion: 联合利用文本语义和视觉原型学习能有效改进数字病理学中的弱监督语义分割，双模态原型库和多尺度增强是提升定位质量的关键。
...
<details>
  <summary>Details</summary>
Motivation: 组织病理学中的弱监督语义分割严重依赖分类骨干网络，但这些模型通常只定位最具判别性的区域，难以捕捉组织结构的完整空间范围。视觉语言模型如CONCH提供丰富的语义对齐和形态感知表示，而现代分割骨干网络如SegFormer保留细粒度空间线索，但结合这些互补优势在弱监督下仍具挑战。
...
Conclusion: 该框架成功整合了视觉语言模型的语义对齐能力、分割骨干的空间保留特性和文本引导的语义信息，为组织病理学弱监督语义分割提供了一种有效且高效的方法，无需像素级标注即可生成高质量伪掩码。
...
Conclusion: Point2Pose框架通过生成式方法有效解决了3D人体姿态估计的挑战，同时创建的多模态数据集MVPose3D为相关研究提供了重要资源
...
TL;DR: EchoingPixels是一个用于音频-视觉大语言模型（AV-LLMs）的令牌减少框架，通过跨模态语义筛选器（CS2）和同步增强RoPE（Sync-RoPE）技术，在保持性能的同时将令牌减少到原始的5-20%，实现2-3倍的速度提升和内存减少。


<details>
  <summary>Details</summary>
Motivation: AV-LLMs面临大量音频和视频令牌带来的计算开销问题。现有的单模态令牌减少方法无法利用音频-视觉跨模态协同作用，且静态的模态预算分配无法适应音频和视频不同的动态信息密度。如何在联合音频-视频流上进行令牌减少仍是一个未解决的瓶颈。

Method: 提出EchoingPixels框架，核心是跨模态语义筛选器（CS2），它通过共同关注联合多模态流，从整个音频-视觉令牌池中减少令牌，而不是使用固定的模态预算。同时设计同步增强RoPE（Sync-RoPE）来保持稀疏选择令牌的关键时间关系。

Result: 实验表明，EchoingPixels仅使用原始令牌的5-20%就能达到与强基线相当的性能，同时实现2-3倍的速度提升和内存减少。

Conclusion: EchoingPixels成功解决了AV-LLMs中令牌减少的瓶颈问题，通过跨模态协同和动态预算分配，在保持性能的同时显著提升了计算效率。
...
TL;DR: StainNet是一个专门针对特殊染色病理图像的基础模型，基于ViT架构，使用自蒸馏自监督学习在140万张特殊染色图像上训练，在肝脏恶性肿瘤分类等任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型主要基于H&E染色图像训练，但临床实践中常使用特殊染色（如免疫组化）图像，现有模型在特殊染色应用上存在局限。

Method: 提出StainNet模型，基于ViT架构，采用自蒸馏自监督学习方法，在HISTAI数据库的20,231张公开特殊染色WSI中裁剪的140万张patch图像上进行训练。

Result: 在内部肝脏恶性肿瘤分类任务和两个公开ROI级数据集上展示了强大性能，通过少样本学习和检索评估进一步验证优势，与近期更大PFMs相比表现突出。

Conclusion: StainNet填补了特殊染色病理基础模型的空白，为临床特殊染色图像分析提供了有效工具，模型权重已开源。
...
TL;DR: ISMVC提出了一种基于信息量的选择性插补多视图聚类方法，通过评估缺失位置的信息量，只在有足够支持时进行选择性插补，并结合变分自编码器和混合高斯先验学习聚类友好的潜在表示。


<details>
  <summary>Details</summary>
Motivation: 不完全多视图数据中，不同视图存在缺失和不平衡观测，给聚类带来挑战。现有插补方法会引入噪声和偏差，而免插补方法在严重不完全情况下因缺乏跨视图互补性而表现不佳。

Method: 基于信息量的选择性插补多视图聚类（ISMVC）：1）评估每个缺失位置的插补相关信息量（基于视图内相似性和跨视图一致性）；2）只在有足够支持时进行选择性插补；3）结合变分自编码器和混合高斯先验学习聚类友好的潜在表示；4）通过分布级插补稳定后验分布聚合并显式建模插补不确定性。

Result: 在多个基准数据集上，在更现实和具有挑战性的不平衡缺失场景下，ISMVC方法优于现有的基于插补和免插补方法。

Conclusion: ISMVC提供了一种轻量级、数据驱动、模型无关的选择性插补策略，可作为现有不完全多视图聚类模型的插件模块，有效平衡了插补的益处和风险。
...
TL;DR: 提出基于Pix2Pix的条件生成框架，从二值掩码生成逼真的显微镜图像中的丝状结构，以解决丝状结构分割任务中像素级标注数据稀缺的问题。
...
Result: 实验证明该方法有效，并且优于不使用合成数据训练的现有模型。

Conclusion: 提出的条件生成框架和结构损失函数能够生成逼真的丝状结构图像，有效解决丝状结构分割任务中的数据稀缺问题，提升分割性能。
...
TL;DR: HD-CLIP是一种分层退化CLIP方法，通过将低质量图像分解为语义嵌入和有序退化嵌入，为扩散模型提供更丰富的指导，提升真实世界图像超分辨率的性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像超分辨率面临复杂未知的退化问题，现有方法通常假设已知退化严重程度，并依赖无法捕捉数值严重性的CLIP文本编码器，限制了泛化能力。

Method: 提出HD-CLIP，将低质量图像分解为语义嵌入和有序退化嵌入，后者能捕捉有序关系并支持未见退化级别的插值。通过无分类器引导和无分类器投影引导集成到扩散模型中。

Result: HD-CLIP作为即插即用模块，无需训练即可集成到各种超分辨率框架中，显著提高了细节保真度和感知真实性，在多样化的真实世界数据集上表现优异。

Conclusion: HD-CLIP通过分层表示学习有效解决了真实世界图像超分辨率中的退化建模问题，提供了更丰富的指导信息，提升了生成恢复的质量和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型在复杂推理方面表现出色，但在视觉顺序规划（执行多步骤动作达成目标）方面研究不足，特别是处理包含非最优（错误）步骤的实际规划任务。

Method: 提出CoSPlan基准测试，评估模型在4个领域的错误检测和步骤完成能力；开发Scene Graph Incremental updates (SGI)方法，在初始状态和目标状态之间引入中间推理步骤。

Result: 现有VLMs（如Intern-VLM和Qwen2）在CoSPlan上表现不佳，即使使用Chain-of-Thought和Scene Graphs等技术也难以利用上下文线索达成目标；SGI方法平均提升5.2%性能，并能泛化到传统规划任务。

Conclusion: 视觉语言模型在错误检测和纠正的顺序规划任务中存在局限性，SGI方法通过引入中间推理步骤有效提升模型性能，增强了纠正顺序规划的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法大多依赖固定的骨骼模板，无法泛化到不同或扰动的骨骼拓扑结构，缺乏大规模异构动物运动数据和统一的生成框架

Method: 1) 构建OmniZoo数据集，包含140个物种的32,979个序列；2) 提出拓扑感知骨骼嵌入模块，将任意骨骼的几何和结构属性编码到共享标记空间；3) 建立通用自回归运动生成框架，结合文本语义生成运动

Result: 方法能够为任意骨骼拓扑生成时间连贯、物理合理、语义对齐的运动，并支持跨物种运动风格迁移

Conclusion: OmniZoo数据集和提出的框架解决了当前运动生成方法的局限性，实现了对任意骨骼拓扑的文本驱动运动生成，具有广泛的应用前景
...
TL;DR: TranSamba是一种混合Transformer-Mamba架构，用于弱监督体医学图像分割，通过跨平面Mamba块实现高效的三维上下文建模。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督医学图像分割方法通常依赖2D编码器，忽略了数据的体素本质，需要能够有效捕获三维上下文的方法。

Method: 提出TranSamba架构，在标准Vision Transformer骨干网络上增加跨平面Mamba块，利用状态空间模型的线性复杂度实现相邻切片间的高效信息交换，增强Transformer块在切片内的自注意力计算。

Result: 在三个数据集上的实验表明，TranSamba建立了新的最先进性能，在不同模态和病理上一致优于现有方法，且具有线性时间复杂度和恒定内存使用。

Conclusion: TranSamba通过结合Transformer和Mamba的优势，实现了高效的弱监督体医学图像分割，为三维医学图像分析提供了有效的解决方案。
...
TL;DR: mmCounter：利用毫米波雷达通过提取呼吸和微动作的超低频信号，实现在密集静态人群中准确计数的新系统


<details>
  <summary>Details</summary>
Motivation: 现有毫米波雷达在检测密集静态人群时存在困难，因为空间分辨率有限且依赖运动检测。需要一种能准确计数静态人群的方法，特别是在室内密集环境中。

Method: 采用多阶段信号处理流程：1) 提取超低频(<1Hz)信号（主要来自呼吸和微尺度身体移动）；2) 使用新颖的信号处理技术区分这些微弱信号与背景噪声和静态物体；3) 提取相关低频源及其空间信息；4) 将这些信号源映射到个体人员

Result: 在熟悉环境中达到87%的平均F1分数和0.6的平均绝对误差；在未测试环境中达到60%的平均F1分数和1.1的平均绝对误差；能在3平方米空间内计数最多7人（无并排间距，仅1米前后距离）

Conclusion: mmCounter通过提取超低频生理信号和微动作，成功解决了毫米波雷达在密集静态人群计数中的关键挑战，实现了比现有方法更准确的人员计数能力
...
TL;DR: 该论文提出了一个视频工具包和时空推理框架（STAR），通过增强多模态大语言模型（MLLMs）的时空推理能力来提升视频问答任务的性能。
...
Method: 1. 设计全面可扩展的视频工具包，确保工具数量与多样性的平衡；2. 提出STAR框架，策略性地调度时空工具，逐步定位视频中的关键区域，避免工具链捷径问题。
...
TL;DR: Visual Funnel：一种解决多模态大语言模型"上下文盲区"问题的训练免费方法，通过上下文锚定和熵缩放组合构建层次化视觉输入，显著提升细粒度视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在细粒度视觉细节感知上存在不足，现有裁剪方法导致"上下文盲区"问题——高保真细节与全局上下文的结构性脱节，限制了在精度要求高的任务中的应用。

Method: 提出Visual Funnel两阶段方法：1) 上下文锚定：单次前向传播识别感兴趣区域；2) 熵缩放组合：基于注意力熵动态确定裁剪尺寸并优化裁剪中心，构建从焦点细节到周围环境的层次化上下文组合。

Result: Visual Funnel显著优于简单的单裁剪和非结构化多裁剪基线。实验验证单纯增加非结构化裁剪带来的收益有限甚至有害，而层次化组合结构是解决上下文盲区的关键。

Conclusion: 视觉感知的关键不仅在于信息量，更在于输入的结构多样性。Visual Funnel通过层次化上下文组合有效解决了多模态大语言模型的上下文盲区问题，无需额外训练即可提升细粒度视觉理解能力。
...
Result: P2S是首个能在小时级长视频中进行时间定位的零样本框架，在MAD数据集上R5@0.1指标超过监督SOTA方法3.7%。

Conclusion: P2S通过创新的训练免费框架，有效解决了长视频时刻检索中搜索效率低和精炼成本高的核心挑战，在零样本设置下实现了超越监督方法的性能。
...
TL;DR: CoherentGS是一个从稀疏模糊图像进行高质量3D重建的新框架，通过结合去模糊网络和扩散模型的双重先验策略，解决了3DGS在稀疏模糊输入下的性能问题。


<details>
  <summary>Details</summary>
Motivation: 3DGS在稀疏模糊图像输入时性能严重下降，因为稀疏视图无法解决运动模糊，而运动模糊又抹去了对齐有限视图所需的高频细节，形成恶性循环，导致重建失败。

Method: 采用双重先验策略：结合专门去模糊网络恢复清晰细节提供光度指导，以及扩散模型提供几何先验填充未观察区域。关键技术包括一致性引导相机探索模块和深度正则化损失。

Result: 在合成和真实场景上使用3、6、9个输入视图进行评估，CoherentGS显著优于现有方法，为该挑战性任务设定了新的最先进水平。

Conclusion: CoherentGS通过双重先验策略成功打破了稀疏模糊图像重建的恶性循环，实现了高质量3D重建，代码和视频演示已公开。
...
TL;DR: 本文提出首个4D毫米波雷达与LiDAR联合的场景流估计框架RaLiFlow，构建了首个雷达-LiDAR场景流数据集，通过动态感知双向跨模态融合模块和专门设计的损失函数实现有效融合。


<details>
  <summary>Details</summary>
Motivation: 当前多模态融合方法主要关注图像与LiDAR点云的结合，而4D毫米波雷达与LiDAR的融合尚未探索。雷达相比LiDAR更便宜、在各种天气条件下更鲁棒，并能检测点级速度，是LiDAR的有价值补充。然而雷达数据存在噪声、低分辨率和稀疏性问题，且目前缺乏专门用于场景流估计的雷达-LiDAR数据集。

Method: 1) 基于公开真实世界汽车数据集构建首个雷达-LiDAR场景流数据集；2) 提出有效的雷达去噪预处理策略和场景流标签生成方法；3) 提出RaLiFlow框架，包含动态感知双向跨模态融合模块，将雷达动态线索集成到局部交叉注意力机制中；4) 设计专门损失函数减轻不可靠雷达数据的影响，增强两种模态在动态前景区域的实例级一致性。

Result: 在重新构建的场景流数据集上进行广泛实验，结果表明该方法显著优于现有的基于LiDAR和基于雷达的单模态方法。

Conclusion: 本文首次探索了4D毫米波雷达与LiDAR的联合场景流估计，通过构建数据集和提出有效的融合框架，证明了雷达-LiDAR融合在场景流估计中的优越性，为多模态感知提供了新方向。
...
TL;DR: 提出一个用于内窥镜图像特征匹配的深度学习框架，通过自监督学习和新视角合成生成真实对应关系，在SCARED数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 在微创手术中，内窥镜图像间的精确像素级对应关系对于3D重建、相机跟踪和场景理解至关重要。然而手术领域存在独特挑战：弱透视线索、非朗伯组织反射和复杂可变形解剖结构会降低传统计算机视觉技术的性能。深度学习模型虽然在自然场景中表现良好，但其特征并不天然适用于手术图像的细粒度匹配，需要针对该领域进行专门适配。

Method: 提出一个深度学习流水线用于建立内窥镜图像对的特征对应关系，以及一个自监督优化框架进行模型训练。方法利用新视角合成流水线生成真实内点对应关系，随后在对比学习范式中用于挖掘三元组。通过这种自监督方法，在DINOv2骨干网络上增加额外的Transformer层，专门优化以产生通过余弦相似度阈值直接匹配的嵌入向量。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注推理任务，忽略了细粒度识别能力，而细粒度识别在实际应用场景中至关重要。
...
Method: 1) 使用八叉树进行空间划分实现并行加速；2) 在叶节点内采用自适应体素占用统计和kNN密度估计快速去除孤立噪声点；3) 构建结合密度权重和自适应距离权重的引力评分函数精细区分噪声点和目标点

Result: 在Stanford 3D、CADC数据集和实验室FMCW LiDAR点云上的实验表明，相比现有方法，在多种噪声条件下F1、PSNR和Chamfer Distance指标均有提升，同时减少了单帧处理时间

Conclusion: 该方法在保持高去噪精度和边缘细节的同时实现了实时处理，验证了在多噪声场景下的高精度、鲁棒性和实时性能
...
### [45] [Are We Ready for RL in Text-to-3D Generation? A Progressive Investigation](https://arxiv.org/abs/2512.10949)
...
TL;DR: 该论文首次系统研究了强化学习在文本到3D自回归生成中的应用，提出了AR3D-R1模型，并引入了新的评估基准MME-3DR。
...
Method: 从四个维度系统研究RL在文本到3D生成中的应用：1) 奖励设计：评估奖励维度和模型选择；2) RL算法：研究GRPO变体，探索训练数据和迭代的扩展；3) 引入新基准MME-3DR；4) 提出Hi-GRPO分层优化方法，通过专用奖励集合优化全局到局部的3D生成。

Result: 基于研究洞察开发了AR3D-R1，这是首个RL增强的文本到3D模型，能够从粗粒度形状到纹理细化进行专家级生成。研究显示与人类偏好对齐至关重要，通用多模态模型能为3D属性提供稳健信号，令牌级优化效果显著。

Conclusion: 该研究为RL驱动的3D生成推理提供了重要见解，填补了RL在3D生成领域的空白。提出的AR3D-R1模型和MME-3DR基准为未来研究奠定了基础，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: TikTok和YouTube等平台视频内容快速增长，加剧了多模态仇恨言论的传播，其中有害线索在视觉、听觉和文本流中异步出现。现有研究主要关注视频级分类，而实际应用中关键的时序定位任务（识别仇恨片段何时发生）尚未解决，特别是在弱监督下仅有视频级标签时，静态融合或基于分类的架构难以捕捉跨模态和时序动态。
...
Conclusion: MultiHateLoc是首个为弱监督多模态仇恨言论定位设计的框架，通过创新的模态感知编码、动态融合和对比对齐策略，有效解决了现有方法在捕捉跨模态时序动态方面的不足，为多模态仇恨言论检测提供了实用的时序定位解决方案。
...
TL;DR: 提出WildRoad数据集和MaGRoad方法，解决越野道路提取难题，通过路径中心框架提升拓扑准确性


<details>
  <summary>Details</summary>
Motivation: 深度学习在城市道路提取方面已有进展，但越野环境仍缺乏探索且具有挑战性。现有模型在野外地形中失败的主要原因有两个：缺乏大规模矢量化数据集，以及主流方法的结构性弱点。节点中心范式在稀疏端点进行推理，对越野场景中的遮挡和模糊交叉口脆弱，导致拓扑错误。

Method: 1. 发布WildRoad数据集：使用专门的交互式标注工具构建的全球越野道路网络数据集；2. 提出MaGRoad方法：基于路径中心的框架，沿候选路径聚合多尺度视觉证据以鲁棒地推断连接性

Result: MaGRoad在WildRoad基准测试中达到最先进性能，同时在城市数据集上具有良好的泛化能力。简化后的推理流程使推理速度提升约2.5倍，提高了实际应用性。
...
TL;DR: TransLocNet：通过跨模态注意力融合LiDAR几何与航空语义，实现空中-地面定位，在CARLA和KITTI数据集上优于现有方法，定位误差减少达63%


<details>
  <summary>Details</summary>
Motivation: 地面LiDAR与航空图像之间存在大的视角和模态差异，使得空中-地面定位具有挑战性。需要一种能够有效融合两种模态信息的方法来解决这一难题。

Method: 提出TransLocNet跨模态注意力框架：将LiDAR扫描投影为鸟瞰图表示，通过双向注意力与航空特征对齐，使用似然图解码器输出位置和方向的空间概率分布，并加入对比学习模块增强跨模态对齐。

Result: 在CARLA和KITTI数据集上的实验表明，TransLocNet优于现有最先进方法，定位误差减少达63%，实现亚米级、亚度级精度，在合成和真实场景中都表现出鲁棒性和泛化能力。

Conclusion: TransLocNet通过有效融合LiDAR几何与航空语义上下文，为空中-地面定位提供了鲁棒且可泛化的解决方案，显著提升了定位精度。
...
TL;DR: 论文提出NCTTA方法，通过样本级特征-分类器对齐解决测试时适应中的性能下降问题，在ImageNet-C上比Tent提升14.52%


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法缺乏对域偏移下性能下降的理论洞察，需要从几何角度理解特征表示与分类器权重的关系

Method: 扩展神经坍缩到样本级，发现样本对齐坍缩现象，提出NCTTA方法：结合几何邻近性和预测置信度的混合目标特征-分类器对齐

Result: NCTTA显著提升域偏移下的鲁棒性，在ImageNet-C上比Tent方法提升14.52%，有效缓解伪标签不可靠问题

Conclusion: 样本级特征-分类器对齐是解决测试时适应性能下降的关键，NCTTA通过混合目标策略有效实现这一对齐
...
TL;DR: 提出基于移动设备的实时物理治疗运动识别与评估框架，通过姿态估计、角度特征提取和动态规划序列匹配实现实时运动分析与偏差检测。


<details>
  <summary>Details</summary>
Motivation: 解决远程物理治疗监督和移动健康应用中实时识别、分类和评估人体物理治疗运动的需求，实现客户端本地处理以确保可扩展性和实时性能。

Method: 将运动分解为静态姿态序列，使用姿态估计神经网络提取身体关键点，转换为三角函数角度特征，轻量级监督模型进行帧级预测，采用改进Levenshtein距离的动态规划算法进行序列匹配和偏差定位。

Result: 实验评估验证了方法的有效性，展示了在远程物理治疗监督和移动健康应用中的适用性，系统完全在客户端运行，确保实时性能。

Conclusion: 该框架为实时物理治疗运动分析提供了高效解决方案，通过客户端处理实现可扩展的远程监督，在移动健康领域具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有学习型视频压缩框架面临时间对齐不准确与误差传播之间的困境：分离变换框架在帧间压缩中表现良好但会导致误差传播，而统一变换框架消除了误差传播但在共享潜在域中的运动估计性能较差

Method: 1) 双域渐进时间对齐：结合粗粒度像素域对齐和细粒度潜在域对齐，前者用单参考帧光流处理简单运动，后者用流引导可变形Transformer在多个参考帧潜在域中实现长时运动细化；2) 质量条件专家混合模块：根据目标质量和内容动态分配不同专家调整每像素量化步长，实现连续比特率适配
...
TL;DR: 提出混合框架结合传统DDL核与轻量级GRU网络，通过多尺度聚焦体积和迭代优化实现更准确的深度估计
...
Method: 1) 使用手工设计的DDL核计算多尺度聚焦体积；2) 轻量级多尺度GRU深度提取模块迭代优化低分辨率初始深度估计；3) 学习凸上采样模块重建高分辨率深度图

Result: 在合成和真实数据集上的实验表明，该方法在准确性和泛化能力方面优于最先进的深度学习和传统方法

Conclusion: 提出的混合框架通过结合传统DDL核的鲁棒聚焦体积提取和轻量级GRU网络的迭代优化，实现了更准确、更泛化的深度估计，同时保持了计算效率
...
TL;DR: Pulse3DFace是首个用于估计3D血液脉动图的数据集，包含多视角RGB视频、脉搏参考测量和3D面部扫描，支持远程光电容积脉搏波成像方法的改进和验证。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏用于研究面部动态血液脉动的3D数据集，这限制了远程脉搏估计方法的发展。需要能够模拟真实面部血液脉动变化的数据来改进和验证光电容积脉搏波成像技术，并研究多视角方法以减轻光照对脉搏分析的影响。

Method: 收集了15名受试者的多视角RGB视频（23个视角，30Hz），同时记录血液脉搏参考测量值。使用单目运动结构技术生成面部3D扫描，并处理生成与FLAME 3D头部模型纹理空间兼容的3D脉动图，包含信噪比、局部脉搏幅度、相位等信息。

Result: 创建了首个包含原始视频、脉搏参考测量、3D面部扫描和处理的3D脉动图的综合数据集。脉动图提供了面部和颈部皮肤区域的生理学特征信息，包括信号质量、振幅和相位数据。

Conclusion: Pulse3DFace数据集填补了3D血液脉动研究的数据空白，为开发更准确的远程脉搏估计方法提供了重要资源，支持光照鲁棒性研究和合成视频数据生成，有助于推动光电容积脉搏波成像技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 少样本语义分割中，编码器对新类别特征提取能力有限是主要瓶颈，现有研究多关注解码器改进，而编码器适应性不足限制了性能提升

Method: 提出TaP方法，利用低秩适应（LoRA）在支持集上微调编码器，计算开销小，可快速适应新类别并减轻灾难性遗忘，方法模型无关且可无缝集成到现有FSS流程

Result: 在COCO 20^i、Pascal 5^i及跨域数据集（DeepGlobe、ISIC、Chest X-ray）上的实验表明，TaP能持续提升不同模型和shot设置的性能，在复杂多类别场景中表现尤为突出，即使低秩适应也能获得良好性能

Conclusion: TaP通过解决FSS中编码器对新类别泛化能力的关键限制，为构建更鲁棒、高效和可泛化的分割系统铺平了道路
...
TL;DR: Blink框架通过模拟人类眨眼过程，动态调整视觉token分辨率来增强多模态大语言模型的视觉感知能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉感知方面仍有局限，而人类通过动态扫描和聚焦显著区域的高效感知策略值得借鉴

Method: 提出Blink框架，包含显著性引导扫描和动态token分辨率两个模块，基于注意力图估计token显著性，通过token超分辨率模块扩展重要token，并在下一层失去焦点时丢弃扩展token

Result: 大量实验验证了Blink在增强视觉感知和多模态理解方面的有效性

Conclusion: Blink框架通过模拟人类感知策略，实现了对视觉token的动态分辨率调整，平衡了广泛探索和细粒度聚焦，从而高效地增强了多模态大语言模型的视觉感知能力
...
TL;DR: GETok提出了一种空间表示方法，通过可学习的网格标记和偏移标记来增强多模态大语言模型在2D空间中的物体定位能力，无需修改自回归架构。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）使用自回归Transformer架构，需要对输入图像进行标记化，这限制了模型在2D图像空间中准确定位物体的能力。需要解决如何改进序列语言标记以更好地在2D空间中对物体进行定位的问题。

Method: GETok方法将专门的可学习标记词汇集成到MLLMs中：1）使用网格标记将图像平面划分为结构化空间锚点；2）利用偏移标记实现精确且迭代的定位预测细化。通过直接将空间关系嵌入到标记中，增强MLLMs的原生2D空间推理能力。

Result: 大量实验表明，GETok在监督微调和强化学习设置下的各种指代任务中，都优于最先进的方法，取得了卓越的性能。

Conclusion: GETok通过将空间表示直接嵌入到语言标记中，显著提升了MLLMs在2D空间中的物体定位能力，无需修改自回归架构，为多模态理解提供了有效的空间推理解决方案。
...
TL;DR: 提出基于骨架的少样本原型网络框架，用于孤立手语识别，解决数据稀缺和长尾分布问题，在WLASL数据集上达到43.75% Top-1准确率，比基线提升13%以上。


<details>
  <summary>Details</summary>
Motivation: 孤立手语识别面临数据稀缺和词汇长尾分布问题，传统分类方法在数据不足时容易过拟合高频类别，难以泛化到罕见类别，需要新的方法来解决这一瓶颈。
...
Conclusion: 原型训练策略在数据稀缺情况下优于标准分类方法，提供了识别大规模手语词汇的可扩展途径，具有较强的零样本泛化能力。
...
TL;DR: AVI-Edit：音频同步视频实例编辑框架，通过粒度感知掩码精炼器和自反馈音频代理实现精确的实例级编辑和音频-视觉同步


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法忽视了音频-视觉同步，缺乏实例级编辑所需的细粒度时空控制能力，需要开发能够同时处理视觉实例编辑和音频同步的框架

Method: 提出AVI-Edit框架：1）粒度感知掩码精炼器迭代优化用户提供的粗掩码为精确实例区域；2）自反馈音频代理策划高质量音频指导，提供细粒度时间控制；3）构建大规模实例中心对应数据集

Result: 大量实验表明AVI-Edit在视觉质量、条件遵循和音频-视觉同步方面优于现有最先进方法

Conclusion: AVI-Edit通过创新的掩码精炼和音频代理机制，成功解决了视频编辑中的音频-视觉同步和实例级控制问题，为音频同步视频编辑提供了有效解决方案
...
TL;DR: 提出对称U-Net架构用于一体化图像修复，通过对称设计保留退化信号，无需复杂架构或退化提示策略，实现SOTA性能且计算成本更低
...
Method: 提出对称U-Net（SymUNet）架构，通过编码器-解码器间的特征尺度对齐和简化的跨尺度传播，在跳跃连接中使用简单加法融合即可有效保留内在退化信号；进一步提出语义增强变体SE-SymUNet，通过交叉注意力从冻结的CLIP特征直接注入语义信息以显式增强退化先验

Result: 在多个基准数据集上优于现有方法，同时降低了计算成本；SymUNet和SE-SymUNet都建立了更简单且更强的基础，为未来一体化图像修复的进展提供了新方向

Conclusion: 精心设计的特征提取本身已编码退化信息，对称U-Net架构足以有效利用这些线索，无需复杂架构或退化提示策略；提出的对称设计为一体化图像修复提供了更简单且更强的解决方案
...
Result: 在WXSOD数据集上进行了广泛实验，使用不同训练数据规模(100%、50%、30%)、三种编码器和七种解码器配置。结果表明，提出的SOD框架（特别是NIFM增强的特定编码器）相比普通编码器，在复杂天气条件下提高了分割精度。
...
TL;DR: 提出RSRT数据集和TRSLLaVA方法，通过文本到文本匹配实现无需训练的遥感图像语义检索，在多个基准测试中超越有监督模型。
...
Method: 引入RSRT数据集（每张图像多个结构化描述），提出TRSLLaVA方法，将跨模态检索重新定义为文本到文本匹配问题，在统一的文本嵌入空间中使用VLM生成的描述进行检索。
...
<details>
  <summary>Details</summary>
Motivation: 在遮挡、伪装或快速运动等挑战性视频条件下，理解视频更多地依赖于运动动态而非静态外观。现有方法通常需要用户查询，而TCAM旨在无需查询的情况下自动发现和描述视频中的运动模式。

Method: TCAM采用运动场注意力机制，自主观察视频、识别多个运动活动，并将每个自然语言描述空间地定位到对应的轨迹。通过将运动模式与对比视觉-语言表示对齐，结合全局视频-文本对齐和细粒度空间对应性的统一训练，实现多运动表达的无查询发现。

Result: 在MeViS基准测试中，TCAM实现了58.4%的视频到文本检索准确率，64.9 JF的空间定位分数，每个视频发现4.8个相关表达，准确率达84.7%，展示了强大的跨任务泛化能力。

Conclusion: TCAM通过运动中心的方法，在无需用户查询的情况下有效发现和描述视频中的运动模式，为自动视频理解提供了新的解决方案，特别是在挑战性视觉条件下表现出色。
...
TL;DR: 该论文提出了一种结合深度特征提取与可解释图像处理模块的深度学习架构，用于眼部疾病的自动化诊断，通过视网膜血管分割作为辅助任务来提高模型的可解释性和临床实用性。
...
Result: 通过将模型预测与临床相关的形态学特征相结合，减少了假阳性，提高了在临床环境中部署的可行性。

Conclusion: 该方法通过结合深度学习和可解释的图像处理，为眼部疾病的自动化诊断提供了一种更可靠、更可解释的解决方案，有助于在临床环境中实现更有效的部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注人体运动或视频合成，缺乏为任意物体生成显式轨迹的方法。本文旨在利用真实世界视频中的点跟踪提取运动，通过语言指导生成物体轨迹。

Method: 基于Transformer的自编码器通过双重监督学习轨迹表示：文本运动描述和渲染的轨迹可视化，两者都通过冻结的CLIP编码器映射到联合嵌入空间。

Result: 在文本到轨迹检索中达到34.2%的Recall@1，比视频方法高出12.5个百分点；运动准确性比视频生成基线提高33-52%（12.4 ADE vs 18.3-25.3）；仅用物体运动训练却在人体动作识别上达到88.3%的Top-1准确率。

Conclusion: Lang2Motion通过CLIP对齐的轨迹表示实现了有效的跨运动领域迁移，支持风格转移、语义插值和潜在空间编辑，为语言引导的轨迹生成提供了新框架。
...
TL;DR: DOCR-Inspector是一个基于视觉语言模型的文档解析质量评估系统，通过细粒度错误检测和分类来全面评估文档解析质量，超越了传统基准测试的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前文档解析任务中，虽然视觉语言模型取得了显著进展，但在实际场景中实现可靠、高质量的解析仍然具有挑战性。传统基准测试存在数据集特定偏差，导致模型排名不一致，且与真实世界性能相关性有限。基准指标通常只提供总体分数，掩盖了输出中的不同错误模式。

Method: 提出DOCR-Inspector，将文档解析评估形式化为细粒度错误检测和分析。利用VLM-as-a-Judge方法，分析文档图像及其解析输出，识别所有错误并将其分类到28种预定义类型中。构建DOCRcase-200K训练数据集，并提出Chain-of-Checklist推理范式来实现层次化的解析质量评估。

Result: 在DOCRcaseBench基准测试（包含882个真实世界文档解析案例）上，DOCR-Inspector-7B超越了Gemini 2.5 Pro等商业模型以及领先的开源模型。实验表明其质量评估能为解析结果优化提供有价值的指导。

Conclusion: DOCR-Inspector不仅是一个实用的评估工具，还能推动文档解析系统的大规模进步。该方法解决了传统基准测试的局限性，提供了更可靠、全面的文档解析质量评估方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的点跟踪器在边缘设备上部署面临计算资源限制问题，需要降低GPU推理成本以实现实时性能。

Method: 提出K-Track框架，采用混合策略：稀疏深度学习关键帧更新 + 轻量级卡尔曼滤波中间帧预测 + 贝叶斯不确定性传播保持时间一致性。

Result: 在NVIDIA Jetson Nano和RTX Titan等边缘平台上实现5-10倍加速，同时保持超过85%的原始跟踪器精度，达到实时性能。

Conclusion: K-Track为资源受限环境提供了高质量点跟踪的实用部署方案，弥合了现代跟踪算法与可部署视觉系统之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的进步使得伪造个人真实肖像变得容易，这对安全、沟通和公共信任构成严重威胁。检测这类人员驱动的操纵需要不仅能区分篡改内容与真实媒体，还能提供清晰可靠推理的系统。

Method: 引入TriDF基准，包含来自先进合成模型的高质量伪造内容，涵盖16种深度伪造类型。基准评估三个关键方面：感知（使用人工标注证据识别细粒度篡改伪影）、检测（评估跨不同伪造家族和生成器的分类性能）、幻觉（量化模型生成解释的可靠性）。
...
Method: 提出NaviHydra框架：1) 接受高级导航指令作为控制信号；2) 使用基于鸟瞰图(BEV)的轨迹收集方法增强轨迹特征提取；3) 引入新的导航合规性指标评估意图路线遵循度；4) 设计测试评估对各种导航指令的响应。

Result: 在NAVSIM基准测试中显著优于基线模型，达到最先进的结果，证明了在自动驾驶中的有效性。

Conclusion: NaviHydra通过结合规则模拟器的蒸馏和端到端学习，成功解决了自动驾驶中导航指令遵循的挑战，提高了可控性和导航安全性。
...
TL;DR: XDen-1K是首个大规模多模态数据集，专注于物体内部物理属性（特别是体积密度）估计，包含1000个真实物体及其3D模型、部件标注和双平面X射线扫描，通过优化框架恢复体积密度场，并应用于机器人任务。
...
Method: 1) 构建XDen-1K数据集：包含1000个真实物体、148个类别，提供高分辨率3D几何模型、部件级标注和双平面X射线扫描；2) 提出优化框架：从稀疏X射线视图恢复高保真体积密度场；3) 应用：将X射线图像作为条件信号加入现有分割网络进行体积分割，并在下游机器人任务中进行实验。

Result: 利用该数据集能有效提高重心估计的准确性和机器人操作的成功率。XDen-1K可作为基础资源和具有挑战性的新基准，推动物理基础视觉推理和具身AI的研究。

Conclusion: XDen-1K填补了大规模真实世界物理属性数据的空白，通过多模态数据和优化框架实现了物体内部物理属性的估计，在机器人操作等应用中展现出实用价值，有望成为该领域的重要资源。
...
TL;DR: Geo6DPose：一种轻量级、完全本地化、无需训练的零样本6D物体姿态估计方法，通过结合基础模型视觉特征与几何过滤策略，在单GPU上实现亚秒级推理，匹配更大零样本基线的性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本6D姿态估计方法依赖大规模模型和云端推理，导致高延迟、高能耗以及连接性、成本、数据治理等部署风险，不符合实际机器人应用中有限计算资源和本地推理的需求。

Method: 结合基础模型视觉特征与几何过滤策略：计算模板DINO描述符与场景补丁的相似度图，通过将场景补丁中心投影到3D空间、模板描述符投影到物体模型坐标系来建立相互对应关系，最后通过对应关系驱动的RANSAC恢复最终姿态，并使用加权几何对齐度量进行排序。

Result: 在单个消费级GPU上实现亚秒级推理（1.08 FPS），平均召回率达到53.7 AR，与显著更大的零样本基线性能相当，无需训练、微调或网络访问。

Conclusion: Geo6DPose通过以几何可靠性换取模型规模，实现了实用、完全本地的6D感知，适用于机器人部署，且与不断演进的基础模型骨干网络保持兼容。
...
TL;DR: 提出基于最优传输损失和迭代神经网络的新方法，用于单分子定位显微镜，无需非极大值抑制，提高高密度发射体下的定位精度
...
Method: 将SMLM训练目标重新表述为集合匹配问题，设计最优传输损失函数消除NMS需求；提出集成显微镜光学系统知识的迭代神经网络

Result: 在合成基准和真实生物数据上，新损失函数和架构在中高发射体密度下超越现有技术

Conclusion: 提出的方法实现了端到端训练，无需NMS，提高了SMLM在高密度发射体下的性能
...
TL;DR: SHARP：单张图像实现逼真视图合成，通过神经网络前向传播在1秒内回归3D高斯表示参数，支持实时渲染和度量相机运动


<details>
  <summary>Details</summary>
Motivation: 解决从单张图像进行逼真视图合成的挑战，传统方法需要大量计算时间或无法实现高质量合成

Method: 使用神经网络单次前向传播回归3D高斯表示参数，在标准GPU上1秒内完成，支持实时渲染和度量相机运动

Result: 在多个数据集上实现SOTA，LPIPS降低25-34%，DISTS降低21-43%，合成时间降低三个数量级

Conclusion: SHARP实现了从单张图像快速生成高质量3D表示，支持实时渲染和度量相机运动，具有强大的零样本泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割系统在临床部署中需要不确定性估计来识别不可靠预测并支持人工监督。虽然现有研究主要关注像素级不确定性，但基于标志点的分割具有固有的拓扑保证，其不确定性估计研究仍不足。

Method: 采用混合神经网络架构，结合标准图像卷积编码器和基于图的生成解码器，利用其变分潜在空间推导两种互补的不确定性度量：(1) 从学习分布参数直接捕获的潜在不确定性；(2) 通过从潜在样本生成多个随机输出预测获得的预测不确定性。

Result: 实验表明两种不确定性度量都随扰动严重程度增加而增加，反映了全局和局部退化。这些不确定性信号能够识别不可靠预测，并在CheXmask数据集上支持分布外检测。发布了包含657,566个胸部X光标志点分割及逐节点不确定性估计的大型数据集CheXmask-U。

Conclusion: 不确定性估计是增强基于标志点的解剖分割方法在胸部X光中鲁棒性和安全部署的有前景方向，发布的CheXmask-U数据集使研究人员能够在分割质量评估中考虑空间变化。
...
Method: 提出SpaceDrive框架，将空间信息作为显式位置编码处理：1）使用通用位置编码器处理来自多视角深度估计、历史自车状态和文本提示的3D坐标；2）将3D位置编码叠加到对应的2D视觉标记上；3）用任务无关的坐标表示替代数字标记作为VLM的输入输出，使模型能直接回归轨迹坐标而非逐数字生成。

Result: 在nuScenes数据集上实现了最先进的开环性能，在Bench2Drive闭环基准测试中获得了78.02的驾驶分数，在现有VLM方法中排名第二。

Conclusion: SpaceDrive通过将空间信息作为显式位置编码处理，有效提升了VLM在自动驾驶中的空间理解和轨迹规划能力，为解决VLM在细粒度3D空间关系理解上的局限性提供了有效方案。
...
TL;DR: VeloDepth：一种高效的在线视频深度估计方法，通过传播模块利用时空先验，实现时间一致性并提升效率，适合实时应用。


<details>
  <summary>Details</summary>
Motivation: 现有视频深度估计方法存在两个主要问题：1）逐帧单目模型导致时间不一致和不准确；2）基于时间建模的方法计算量大，不适合实时应用。这些限制影响了实际应用中的性能和通用性。

Method: 提出VeloDepth，一种高效的在线视频深度估计流程，利用先前深度预测的时空先验并进行深度特征传播。核心是传播模块，通过基于光流的扭曲和学习的残差校正来细化和传播深度特征与预测。结构上强制时间一致性。
...
TL;DR: IRMoGen提出通过评估和精炼任务连接运动理解与生成的新范式，IRG-MotionLLM首次实现运动生成、评估、精炼的交替推理，显著提升文本-运动对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有运动感知大语言模型通常将理解与生成任务分离，限制了任务间交互反馈带来的相互增益。研究发现运动评估和精炼任务可作为连接理解与生成的关键桥梁，实现双向知识流动。

Method: 提出IRMoGen范式，通过迭代的文本-运动对话将运动生成与评估、精炼紧密耦合。开发IRG-MotionLLM模型，采用三阶段渐进训练方案：初始化基础能力，随后增强交替推理能力。构建自动化数据引擎从现有文本-运动数据集合成交替推理标注。

Result: 实验表明：(1)评估和精炼任务显著改善文本-运动对齐；(2)交替执行生成、评估、精炼步骤在各训练阶段均带来持续性能提升；(3)IRG-MotionLLM明显优于基线模型，在标准文本到运动生成基准上达到先进性能。跨评估器测试进一步验证其有效性。

Conclusion: 通过评估和精炼任务连接运动理解与生成是有效的，IRMoGen范式及IRG-MotionLLM模型实现了交替推理，显著提升了运动生成质量，为运动感知大语言模型的发展提供了新方向。
...
Result: LDP在自动指标和临床专家评估（医师评分7.2/10）上均优于现有基线，相比全微调降低833倍训练计算成本，在IU-XRay数据集上验证了鲁棒性。

Conclusion: LDP为初级医疗保健提供了可扩展、临床可行的息肉诊断报告生成方案，显著提升诊断一致性和可靠性，同时大幅降低计算资源需求。
...
TL;DR: 开发了一个端到端管道，从CCTA自动提取冠状动脉几何形状，生成模拟数据，并使用扩散回归模型直接从CCTA特征预测冠状动脉血压，绕过传统CFD计算，实现快速、非侵入性的CAD诊断支持。


<details>
  <summary>Details</summary>
Motivation: 传统CFD模拟冠状动脉血流虽然能提供有价值的血流动力学标志物，但计算成本高、耗时且难以集成到大规模临床工作流中，限制了AI模型的训练数据和基于生理学的CAD评估的广泛应用。

Method: 开发了端到端管道：1) 从冠状动脉CT血管造影(CCTA)自动提取冠状动脉几何形状；2) 简化模拟数据生成；3) 引入基于扩散的回归模型，直接从CCTA特征预测冠状动脉血压分布，绕过推理时的CFD计算。

Result: 在模拟冠状动脉血流动力学数据集上评估，提出的模型达到最先进性能：R²为64.42%，均方根误差(RMSE)为0.0974，归一化RMSE为0.154，优于多个基线方法。

Conclusion: 这项工作为快速、非侵入性血压预测提供了一个可扩展且易于访问的框架，支持冠状动脉疾病诊断，解决了传统CFD工作流的局限性。
...
TL;DR: 提出GLAT+IRM方法，通过迭代优化选择关键病理区域，结合图拉普拉斯注意力机制提升前列腺癌WSI分级性能


<details>
  <summary>Details</summary>
Motivation: 现有WSI前列腺癌分级方法存在三个主要问题：1）WSI尺度巨大；2）组织结构异质性高；3）难以选择诊断相关区域。现有方法通常采用随机或静态补丁选择，导致包含冗余或非信息区域，降低性能。

Method: 提出GLAT（图拉普拉斯注意力Transformer）结合IRM（迭代优化模块）。IRM使用预训练ResNet50提取局部特征，基础模型进行重要性评分，迭代优化补丁选择。GLAT构建图结构（补丁为节点），通过图拉普拉斯约束确保空间一致性，可学习过滤机制增强判别性组织特征。凸聚合机制动态调整补丁重要性，生成稳健的WSI级表示。

Result: 在5个公共数据集和1个私有数据集上的实验表明，该方法优于现有最先进方法，获得更高性能和空间一致性，同时保持计算效率。

Conclusion: GLAT+IRM方法通过迭代优化补丁选择和图拉普拉斯注意力机制，有效解决了WSI前列腺癌分级中的关键挑战，显著提升了分级性能。
...
TL;DR: 提出SEPL方法，通过特征探测训练和预测集成推理，利用模型中间特征表示训练多个探测分类器，集成它们的预测来增强领域泛化在噪声标签下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当领域泛化遇到噪声标签时，噪声会加剧深层虚假特征的放大，导致现有算法性能下降。需要探索如何让现有方法在遇到噪声时仍然有效。

Method: 提出自集成后学习（SEPL）方法，包括特征探测训练和预测集成推理两部分。利用模型中间特征表示，训练多个探测分类器充分挖掘预训练模型能力，同时使用半监督算法训练探测分类器处理噪声标签，最后通过众包推理方式集成不同分类器的预测。

Result: 大量实验评估表明，该方法不仅增强了现有方法的鲁棒性，而且在真实世界应用中展现出显著潜力和高度灵活性。

Conclusion: SEPL方法通过利用模型内部潜在特征的判别能力，训练多个关注图像不同区域的探测分类器并集成它们的预测，有效解决了领域泛化在噪声标签下的性能下降问题。
...
TL;DR: PoseGAM：一种几何感知的多视角框架，无需显式特征匹配即可从查询图像和模板图像直接预测6D物体姿态，在未见物体上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有6D物体姿态估计方法通常依赖显式构建查询图像与物体模型或模板图像之间的特征对应关系，这限制了在未见物体上的性能。需要一种能直接预测姿态且无需显式匹配的方法。
...
Conclusion: PoseGAM通过几何感知的多视角框架成功实现了无需显式匹配的6D物体姿态估计，在未见物体上表现出优异的泛化性能，为物体姿态估计提供了新思路。
...
TL;DR: SWiT-4D：基于滑动窗口Transformer的视频转4D网格生成方法，利用图像到3D生成先验，仅需少量视频微调即可实现高质量时空一致的4D网格重建


<details>
  <summary>Details</summary>
Motivation: 当前4D内容生成面临两大挑战：1）单目视频转高质量动画3D资产（显式4D网格）困难；2）缺乏大规模自然捕获的4D网格数据集，难以纯数据驱动训练通用视频转4D模型。而图像到3D生成领域有丰富数据集和强大先验模型可供利用。

Method: 提出SWiT-4D（滑动窗口Transformer），无缝集成任何基于DiT的图像到3D生成器，通过时空建模跨视频帧处理，保留原始单图像前向过程，支持任意长度视频的4D网格重建。针对静态相机单目视频，引入基于优化的轨迹模块恢复全局平移。

Result: 仅需单个短视频（<10秒）微调即可实现高保真几何和稳定时间一致性。在域内zoo测试集和挑战性域外基准（C4D、Objaverse、野外视频）上均优于现有基线，在时间平滑性方面表现突出。

Conclusion: SWiT-4D通过有效利用图像到3D生成先验，在极有限的4D监督下实现高质量视频转4D网格生成，具有实际部署可行性，为4D内容创建提供了高效解决方案。
...
TL;DR: MMSI-Video-Bench：首个全面评估MLLM视频空间智能的基准，包含1,106个问题，覆盖感知、规划、预测和跨视频推理四个层次，揭示当前模型与人类存在近60%的差距。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏全面评估多模态大语言模型在连续视觉输入中空间理解能力的基准，这对于MLLM成为物理环境中的通用助手至关重要。

Method: 构建了完全人工标注的视频空间智能基准，采用四层框架（感知、规划、预测、跨视频推理），包含1,106个问题，基于1,278个视频片段，来自25个数据集和内部视频，每个问题由3DV专家精心设计和审查。

Result: 评估了25个开源和专有MLLM，发现显著的人机差距：许多模型表现接近随机水平，最佳推理模型落后人类近60%。空间微调模型在基准上泛化效果不佳，错误分析揭示了在几何推理、运动定位、长时预测和跨视频对应方面的系统性失败。

Conclusion: MMSI-Video-Bench为推进视频空间智能研究建立了坚实的测试平台，揭示了当前MLLM在空间理解方面的严重不足，需要新的方法来提升几何推理和跨视频理解能力。
...
TL;DR: 论文提出了微观空间智能(MiSI)概念，并开发了包含16.3万问答对和58.7万图像的MiSI-Bench基准测试框架，评估视觉语言模型在微观空间推理能力，发现当前SOTA模型远低于人类水平，但微调后的7B模型在空间转换任务上超越人类，而在科学任务如氢键识别上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 微观空间智能(MiSI)——感知和推理不可见微观实体空间关系的能力——对科学发现至关重要。当前缺乏评估视觉语言模型(VLMs)在此领域能力的系统性基准测试，需要建立评估框架来衡量模型在微观空间推理方面的表现。

Method: 提出了MiSI-Bench基准测试框架，包含超过163,000个问答对和587,000张图像，源自约4,000个分子结构。涵盖9个互补任务，评估从基础空间变换到复杂关系识别的能力。通过实验评估当前SOTA VLMs的表现，并对7B模型进行微调。

Result: 当前最先进的视觉语言模型在此基准测试上表现显著低于人类水平。然而，经过微调的7B模型显示出巨大潜力，在空间变换任务上甚至超越了人类表现。但该模型在科学基础任务（如氢键识别）上表现不佳，表明需要整合明确的领域知识。

Conclusion: 微观空间智能是科学发现的关键能力，当前VLMs在此领域仍有很大提升空间。虽然微调模型在空间变换任务上表现优异，但在科学推理任务上需要整合领域知识。该研究为评估和推进科学AGI的发展提供了重要基准。
...
TL;DR: MoCapAnything是一个类别无关的运动捕捉系统，能够从单目视频和任意3D资产提示中生成驱动特定资产的旋转动画（如BVH格式）。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉流程大多针对特定物种或模板，缺乏通用性。作者提出类别无关运动捕捉（CAMoCap）概念，旨在为任意3D资产生成可直接驱动的动画。

Method: 采用参考引导的分解框架：首先预测3D关节轨迹，然后通过约束感知逆向运动学恢复资产特定旋转。系统包含三个可学习模块：参考提示编码器、视频特征提取器、统一运动解码器，以及轻量级IK阶段。

Result: 在领域内基准测试和真实世界视频上，MoCapAnything能够生成高质量骨骼动画，并在异构骨架间实现有意义的跨物种重定向。创建了包含1038个运动片段的Truebones Zoo数据集。

Conclusion: MoCapAnything实现了可扩展的、提示驱动的3D运动捕捉，能够为任意资产生成动画，突破了传统运动捕捉的物种和模板限制。
...
Result: PubTables-v2是首个大规模多页表格结构识别基准。通过评估领域专用视觉语言模型展示了数据集的有效性，并开发了POTATR模型用于页面级表格提取。

Conclusion: PubTables-v2解决了表格提取领域的数据缺乏问题，支持多种挑战性任务，为未来研究提供了重要基准。POTATR模型展示了在页面级表格提取方面的潜力。
...
Result: 大量实验表明，该方法在广泛的应用场景中优于现有方法，能够生成视觉保真、语义对齐且语法清晰的SVG。

Conclusion: DuetSVG通过联合生成图像和SVG token，并利用视觉预测指导解码，有效解决了现有VLM-based方法在复杂语义处理和几何一致性方面的不足，显著提升了SVG生成质量。
...
Method: 1. 在视频中检测和跟踪物体以提取轨迹；2. 利用轨迹和视频帧，结合大语言模型生成细粒度描述和多样化的运动与空间推理问答对；3. 构建全自动数据构建管道FoundationMotion
...
TL;DR: BabyVLM-V2是一个基于儿童发展轨迹的视觉语言模型框架，通过纵向多模态预训练集和认知评估工具包，实现了从零开始训练的小模型在儿童认知任务上达到竞争性表现。
...
Method: 1. 构建纵向、多方面的婴儿中心化视听预训练集（视频-话语、图像-话语、多轮对话数据）；2. 开发DevCV Toolbox认知评估工具包，将NIH Baby Toolbox的视觉相关测量转化为10个多模态任务；3. 从零开始预训练紧凑模型。

Result: 从零开始预训练的紧凑模型在DevCV Toolbox上达到竞争性表现，在某些任务上甚至超过GPT-4o，验证了发展性预训练的有效性。

Conclusion: BabyVLM-V2提供了一个原则性、统一的框架，有望加速符合发展规律的视觉基础模型预训练研究。
...
TL;DR: Any4D是一个可扩展的多视角transformer，用于度量尺度、密集前馈4D重建，直接生成每像素的运动和几何预测，支持多种传感器模态，性能显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常专注于2视角密集场景流或稀疏3D点跟踪，而最近基于单目RGB视频的4D重建方法无法处理其他传感器模态。需要一种更灵活、更全面的4D重建框架。
...
Conclusion: Any4D提供了一个灵活、高效的4D重建框架，支持多模态输入，为多种下游应用开辟了新途径。
...
TL;DR: 提出一种基于3D Morphable Models映射Gaussian Splatting的方法，通过transformer从音频预测模型参数，实现实时、时间一致的人脸头像生成


<details>
  <summary>Details</summary>
Motivation: 当前语音驱动头像方法存在速度与质量矛盾：扩散方法真实但速度慢，Gaussian Splatting实时但面部跟踪不准确、高斯映射不一致，导致输出不稳定和视频伪影，影响实际应用

Method: 使用3D Morphable Models映射Gaussian Splatting生成个性化头像，引入基于transformer的模型参数预测直接从音频驱动时间一致性，仅需单目视频和独立音频输入

Result: 能够生成实时说话头像视频，在定量和定性评估中均表现出竞争力

Conclusion: 该方法解决了现有方法在速度与稳定性之间的权衡问题，实现了高质量、实时、时间一致的语音驱动头像生成
...
<details>
  <summary>Details</summary>
Motivation: 现有方法专注于4D一致性任务的特定子集（如新颖视图合成、带相机控制的文本到视频等），这些碎片化方法在不相交的3D/4D数据上进行训练，缺乏统一性。

Method: 提出OmniView框架，分别表示空间、时间和视角条件，使这些输入能够灵活组合。该方法能够从静态、动态和多视图输入合成新颖视图，在时间上向前和向后外推轨迹，以及从文本或图像提示创建具有完整相机控制的视频。

Result: 在多个基准测试中与任务特定模型竞争：在LLFF数据集上图像质量得分提高33%，在Neural 3D Video基准上提高60%，在RE-10K静态相机控制上提高20%，在文本条件视频生成中相机轨迹误差减少4倍。

Conclusion: OmniView展示了通用4D视频模型的可行性，通过一个模型实现强大的泛化能力，统一了多种4D一致性任务。
...
Method: 提出Mull-Tokens——模态无关的潜在令牌，通过监督学习从交错文本-图像轨迹中预训练，然后仅使用最终答案进行无监督微调，让模型在图像或文本模态中自由思考。

Result: 在四个具有挑战性的空间推理基准测试中，Mull-Tokens优于仅使用文本推理或交错图像-文本推理的基线方法，平均提升3%，在推理密集的谜题解决任务上提升高达16%。

Conclusion: Mull-Tokens为多模态抽象推理提供了一个简单有效的解决方案，推动了文本和视觉推理的融合。
...
TL;DR: VL-JEPA是一种基于联合嵌入预测架构的视觉语言模型，通过预测目标文本的连续嵌入而非生成token，实现了更强的性能、更少的参数和更高效的推理。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型（VLMs）采用自回归方式生成token，存在计算效率低、参数冗余的问题。VL-JEPA旨在通过预测连续嵌入而非离散token，在抽象表示空间中学习，从而关注任务相关语义而非表面语言变化，提高模型效率和性能。

Method: VL-JEPA采用联合嵌入预测架构（JEPA），在连续嵌入空间中预测目标文本的表示，而非生成离散token。模型包含视觉编码器和文本嵌入预测器，推理时仅在需要时调用轻量级文本解码器将预测嵌入转换为文本。支持选择性解码，减少解码操作。

Result: 在相同视觉编码器和训练数据条件下，VL-JEPA比标准token空间VLM性能更强，且可训练参数减少50%。选择性解码可将解码操作减少2.85倍。在8个视频分类和8个视频检索数据集上平均性能超过CLIP、SigLIP2和Perception Encoder。在4个VQA数据集上（GQA、TallyQA、POPE、POPEv2）与InstructBLIP、QwenVL等经典VLM性能相当，但仅有16亿参数。

Conclusion: VL-JEPA通过预测连续嵌入而非生成token，实现了更高效的视觉语言建模。模型不仅支持文本生成，还天然支持开放词汇分类、文本到视频检索和判别式VQA等任务，无需架构修改，为视觉语言模型提供了更高效、更通用的解决方案。
...
Method: 提出新颖的位置编码机制，支持时间区间编码（对应主题身份），并与预训练视频生成模型的位置嵌入无缝集成；引入主题描述性文本标记以增强视觉身份与视频描述之间的绑定；通过标记级连接避免额外交叉注意力模块，参数开销极小。

Result: AlcheMinT在视觉质量上与最先进的视频个性化方法相当，同时首次实现了视频内多主题生成的精确时间控制，在多个主题身份保持、视频保真度和时间遵循方面建立了基准评估。

Conclusion: AlcheMinT框架成功解决了主题驱动视频生成中缺乏细粒度时间控制的问题，实现了对多主题出现和消失的精确时序控制，为合成视频、故事板制作等应用提供了有效解决方案。
...
TL;DR: 提出MeViS大规模多模态数据集，专注于基于运动描述的视频目标分割与追踪，包含33,072个人工标注的运动表达，基准测试显示现有方法在处理运动表达引导的视频理解方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有参考视频分割数据集通常关注静态属性丰富的语言表达，可能仅凭单帧就能识别目标，忽视了视频和语言中运动信息的重要性。需要探索使用运动表达和运动推理线索进行像素级视频理解的可行性。

Method: 构建MeViS数据集，包含2,006个复杂场景视频中的8,171个对象，标注了33,072个文本和音频形式的运动表达。基准测试了15种现有方法，涵盖4个任务：参考视频对象分割(RVOS)、音频引导视频对象分割(AVOS)、参考多目标追踪(RMOT)和新提出的参考运动表达生成(RMEG)。

Result: 现有方法在处理运动表达引导的视频理解方面表现出弱点和局限性。提出的LMPM++方法在RVOS/AVOS/RMOT任务上取得了新的最先进结果。
...
TL;DR: Flex是一种高效的多摄像头场景编码器，使用可学习的场景token联合编码所有摄像头和时间步的图像信息，无需3D先验，显著提升推理吞吐量和驾驶性能


<details>
  <summary>Details</summary>
Motivation: 解决端到端自动驾驶中处理大量多摄像头数据的计算瓶颈问题，挑战现有方法依赖3D先验（如BEV、占用网格等）的假设

Method: 使用少量可学习的场景token联合编码所有摄像头和时间步的图像token，采用几何无关的设计，直接从数据中学习紧凑场景表示，无需3D归纳偏置
...
TL;DR: ClusIR是一个基于聚类引导的图像修复框架，通过可学习聚类显式建模退化语义，并在空间和频域传播聚类感知线索，实现自适应修复。
...
Method: 提出ClusIR框架，包含两个关键组件：概率聚类引导路由机制（PCGRM）和退化感知频率调制模块（DAFMM）。PCGRM将退化识别与专家激活解耦，实现判别性退化感知和稳定专家路由；DAFMM利用聚类引导先验进行自适应频率分解和目标调制。

Result: 在多个基准测试上的广泛实验验证了ClusIR在各种退化场景下达到竞争性性能，实现了卓越的修复结果。

Conclusion: ClusIR通过聚类引导的协同作用，将语义线索与频域调制无缝连接，能够处理广泛的退化类型，实现高保真度的图像修复。
...
TL;DR: E-RayZer是一种自监督大型3D视觉模型，直接从无标签图像学习真正3D感知表示，通过显式几何进行3D重建，超越现有方法


<details>
  <summary>Details</summary>
Motivation: 自监督预训练在语言、2D图像和视频领域已取得革命性进展，但在从多视角图像学习3D感知表示方面仍未被充分探索。现有方法如RayZer通过潜在空间视图合成间接推断3D，缺乏真正的几何基础

Method: E-RayZer直接在3D空间中操作，通过显式几何进行自监督3D重建，消除捷径解。引入细粒度学习课程，以从易到难的方式组织训练样本，并以完全无监督方式协调异构数据源

Result: E-RayZer在姿态估计上显著优于RayZer，匹配或有时超越全监督重建模型如VGGT。其学习到的表示在迁移到3D下游任务时，优于领先的视觉预训练模型（DINOv3、CroCo v2、VideoMAE V2、RayZer）

Conclusion: E-RayZer建立了3D感知视觉预训练的新范式，通过显式几何和细粒度课程学习，实现了真正3D感知表示的学习，为3D视觉基础模型开辟了新方向
...
TL;DR: GroupDiff：通过跨样本协作生成提升扩散模型推理质量的新方法


<details>
  <summary>Details</summary>
Motivation: 探索扩散模型推理中未被利用的信号，研究是否可以通过样本协作而非独立生成来提升图像质量

Method: 提出Group Diffusion方法，解锁注意力机制使其能在图像间共享，而不仅限于单个图像内的补丁，实现联合去噪并学习图像内和图像间的对应关系

Result: 观察到明显的缩放效应——更大的组规模产生更强的跨样本注意力和更好的生成质量；在ImageNet-256x256上实现高达32.2%的FID改进

Conclusion: 跨样本推理是生成建模中有效且先前未被探索的机制，为扩散模型提供了新的改进方向
...
Method: 1) 策划带有正负属性标注的语义链接图像对，明确教导编码器保留或抑制哪些信息；2) 采用双目标训练范式，平衡生成保真度和对比解缠结。

Result: 该编码器在开放词汇属性检索、个性化和组合生成方面表现优异，在多个基准测试中达到最先进的性能。
...
TL;DR: StereoWalker提出了一种增强导航基础模型的方法，通过引入立体视觉输入和显式中层视觉模块（深度估计、密集像素跟踪）来解决单目视觉的深度尺度模糊问题，显著减少了训练数据需求并提升了导航性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于单目视觉的导航基础模型（NFMs）完全依赖端到端学习，忽略了中层视觉模块。这种方法存在深度尺度模糊问题，在动态和非结构化环境中需要大量像素到动作的监督数据，且空间推理能力有限。作者认为单纯依赖单目视觉而忽略中层视觉先验是低效的。

Method: StereoWalker采用立体视觉输入来解决深度尺度模糊问题，并整合了显式中层视觉模块，包括深度估计和密集像素跟踪。同时，作者从互联网立体视频中构建了一个大型立体导航数据集，包含自动动作标注，以支持模型训练。

Result: 实验表明，中层视觉使StereoWalker仅使用1.5%的训练数据就能达到最先进模型的性能，使用全部数据时则超越了现有最佳模型。此外，立体视觉相比单目输入能带来更高的导航性能。

Conclusion: 在导航基础模型中整合立体视觉和显式中层视觉模块是更高效的方法，能够解决单目视觉的深度尺度模糊问题，减少数据需求，并在动态和非结构化环境中实现更鲁棒的导航性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在严重遮挡和开放集场景中难以同时生成高质量几何和准确姿态，主要原因是缺乏足够的开放集去遮挡和姿态估计先验知识

Method: 1. 将去遮挡模型与3D物体生成解耦，利用图像数据集和收集的去遮挡数据集增强多样性；2. 提出统一姿态估计模型，集成全局和局部机制的自注意力和交叉注意力；3. 构建开放集3D场景数据集扩展姿态估计模型的泛化能力

Result: 综合实验表明，该解耦框架在室内和开放集场景中具有优越性，代码和数据集已开源

Conclusion: SceneMaker通过解耦设计和改进的姿态估计，有效解决了严重遮挡和开放集场景中的3D场景生成问题，为相关领域提供了新的解决方案
...
TL;DR: WorldLens是一个评估生成世界模型的全方位基准测试，涵盖生成质量、重建能力、动作跟随、下游任务和人类偏好五个维度，旨在标准化评估世界模型的物理和行为真实性。


<details>
  <summary>Details</summary>
Motivation: 当前生成式世界模型能创建视觉上逼真的4D驾驶环境，但往往在物理和行为上不合理。领域缺乏统一方法来评估生成世界是否保持几何一致性、遵守物理规律、支持可靠控制。

Method: 提出WorldLens基准测试，涵盖五个方面：生成、重建、动作跟随、下游任务和人类偏好。构建WorldLens-26K大规模人类标注视频数据集，开发WorldLens-Agent评估模型从标注中蒸馏学习，实现可扩展、可解释的评分。

Result: 现有世界模型在五个维度上表现不均：纹理强的模型常违反物理规律，几何稳定的模型缺乏行为保真度。WorldLens生态系统提供了统一的评估框架，将客观指标与人类判断对齐。

Conclusion: WorldLens基准测试、数据集和评估模型形成了一个统一的生态系统，用于衡量世界保真度，标准化未来模型的评估标准——不仅评估它们看起来有多真实，还要评估它们的行为有多真实。
...
<details>
  <summary>Details</summary>
Motivation: 现有单目到立体合成方法通常依赖显式深度估计或变形操作，存在几何不准确和遮挡处理问题。本文旨在开发一种无需深度或变形的端到端解决方案，通过纯视点条件建模几何关系。

Method: 提出StereoSpace框架，基于扩散模型，使用规范矫正空间和视点条件引导生成器推断对应关系并填充遮挡区域。引入端到端评估协议，避免测试时使用真实或代理几何信息，强调下游相关指标。

Result: StereoSpace在感知舒适度(iSQoE)和几何一致性(MEt3R)方面优于变形修复、潜在变形和变形条件等类别的方法，在分层和非朗伯场景中表现出锐利的视差和强鲁棒性。

Conclusion: 视点条件扩散模型为立体生成提供了一种可扩展、无需深度的解决方案，证明了纯视点条件能够有效建模几何关系并生成高质量的立体图像。
...
<details>
  <summary>Details</summary>
Motivation: 探讨当前大语言模型的推理本质，澄清其看似具备溯因推理能力但实际上只是基于训练数据的模式匹配，这对于正确评估和应用LLMs至关重要。

Method: 通过分析LLMs的随机性本质和与人类溯因推理的相似性，使用具体例子展示LLMs如何在没有真实理解、验证或真理基础的情况下产生看似合理的想法、常识推理和解释性答案。

Result: LLMs本质上是随机的模式匹配系统，其看似推理的能力源于训练数据中的人类推理结构，无法真正识别真理或验证解释，但可用于辅助人类思维和生成想法。

Conclusion: LLMs具有随机基础但使用中看似溯因的双重特性，对评估和应用有重要影响。它们可辅助人类思考，但输出需批判性评估，因为它们无法识别真理或验证解释。文章还回应了五个反对观点并指出了分析的限制。
...
TL;DR: 使用小型语言模型(SLMs)进行自动问题生成，通过"生成-验证"策略和概率推理生成高质量问题


<details>
  <summary>Details</summary>
Motivation: 探索小型语言模型在自动问题生成中的应用，作为大型语言模型的补充，特别是在学习分析研究中

Method: 采用"生成-验证"策略的管道：首先生成大量候选问题，然后通过基于概率推理的选择性验证进行精炼

Result: 通过人类专家和大型语言模型评估，大多数评判者认为生成的问题答案清晰，与学习目标一致

Conclusion: 小型语言模型在精心设计的管道引导下，能够有效生成高质量问题，可作为大型语言模型的替代方案
...
TL;DR: DeepNews框架通过模拟金融记者的认知过程，解决了长文本生成中的"不可能三角"问题，在金融报道中实现了低幻觉、高逻辑一致性和个性化表达。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在垂直领域长文本生成中存在"不可能三角"瓶颈：难以同时实现低幻觉、深度逻辑一致性和个性化表达。研究发现这是由于现有生成范式陷入了"统计平滑陷阱"，忽略了专家写作所需的高熵信息获取和结构化认知过程。

Method: 提出DeepNews框架，模拟资深金融记者的隐式认知过程，包含三个核心模块：1)基于信息觅食理论的双粒度检索机制，强制10:1饱和信息输入比；2)基于领域专家知识库（叙事模式）和原子块的模式引导战略规划；3)使用节奏打破和逻辑迷雾等对抗性约束提示技术，破坏模型生成文本的概率平滑性。

Result: 实验发现深度金融报道中存在明显的"知识悬崖"：当检索上下文低于15,000字符时，内容真实性崩溃；而超过30,000字符的高冗余输入可将无幻觉率稳定在85%以上。在顶级中文科技媒体的生态效度盲测中，基于旧模型构建的DeepNews系统获得了25%的提交接受率，显著优于最先进模型GPT-5的零样本生成（0%接受率）。

Conclusion: DeepNews框架通过显式建模专家认知过程，有效解决了长文本生成的"不可能三角"问题，证明了结构化工作流在垂直领域文本生成中的优越性，为专业内容创作提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 在用户信息有限（如外卖平台）的场景中，LLM生成通用回复导致参与度和效果降低，需要从有限文本中提取用户特征以生成个性化回复

Method: 两阶段提示框架：1) 从短评文本推断显式（用户声明的偏好）和隐式（人口统计或风格线索）人设；2) 将推断的人设属性融入回复生成提示。通过调整解码温度鼓励多样且忠实的生成

Result: 在韩国外卖应用的真实数据集上评估，显示人设增强提示能有效提升自动回复的相关性和个性化，改善精确度、多样性和语义一致性

Conclusion: 人设增强提示框架能显著增强自动回复的相关性和个性化，无需模型微调，在用户信息有限的环境中具有实用价值
...
TL;DR: 该论文研究大语言模型在微调新任务时的安全性退化问题，将其归因于灾难性遗忘，并提出将安全保护视为持续学习问题。通过评估多种持续学习方法，发现DER方法在降低攻击成功率方面表现最佳，同时保持任务效用。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，其安全性对齐变得越来越重要。研究发现，当LLMs适应新任务时会出现安全性退化，这归因于灾难性遗忘。在微调即服务的场景下，用户上传数据给服务提供商以获得定制模型，需要解决如何在微调过程中保持模型安全性的问题。

Method: 将安全保护问题框架化为持续学习问题，评估了三种CL方法：基于正则化的方法、基于记忆的方法和模型合并方法。考虑了两种场景：良性用户数据和中毒用户数据。在三个下游任务（GSM8K、SST2、Code）和三个模型系列（LLaMA2-7B、Mistral-7B、Gemma-2B）上进行系统评估。

Result: 持续学习方法相比标准微调始终能获得更低的攻击成功率。其中DER方法优于其他CL方法和现有的安全保护基线，同时保持了任务效用。这些发现在不同的任务和模型家族中具有普遍性。

Conclusion: 持续学习是保护大语言模型在微调过程中安全性的实用解决方案，特别是DER方法在降低安全风险方面表现优异，为微调即服务场景下的模型安全提供了有效保障。
...
TL;DR: AutoMedic是一个多智能体仿真框架，可将静态医疗问答数据集转换为虚拟患者档案，用于自动化评估LLM作为临床对话代理的性能，使用CARE指标进行多维度评估。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在医疗领域的评估存在不足：1）主要依赖静态QA基准测试，缺乏对动态、交互式临床多轮对话场景的评估；2）评估策略单一，主要关注准确性，缺乏多维度评估；3）动态临床交互场景的组合空间巨大，难以标准化和量化评估。

Method: 提出AutoMedic多智能体仿真框架：1）将现成的静态QA数据集转换为虚拟患者档案；2）创建真实的、基于临床的多轮对话环境，让LLM智能体之间进行交互；3）使用CARE指标进行多维度评估，包括临床对话准确性、效率/策略、同理心和鲁棒性。

Result: AutoMedic被证明是临床对话代理的有效自动化评估框架，其评估结果经过人类专家验证，为LLM在对话式医疗应用中的有效开发提供了实用指南。

Conclusion: AutoMedic解决了医疗领域LLM评估的关键挑战，提供了一个标准化、可量化的多智能体仿真框架，能够全面评估LLM在动态临床对话场景中的表现，推动了LLM在医疗对话应用中的发展。
...
TL;DR: 本文探讨了将英语训练的视觉语言模型适配到其他语言的挑战，比较了翻译管道、LoRA微调和两阶段微调等方法，发现数据集翻译是主要瓶颈，建议未来应关注原生语言数据集收集和改进翻译策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有开源编码代理在处理大规模仓库、持久内存和复杂工具链方面不足，而专有代理虽性能强但缺乏可扩展性、可解释性和可控性。需要开发一个既透明又能在工业规模上工作的AI软件工程师。

Method: 基于Confucius SDK构建，该平台包含：统一编排器（分层工作内存用于长上下文推理）、持久笔记系统（跨会话持续学习）、模块化扩展模块（鲁棒工具使用）。元代理通过构建-测试-改进循环自动合成、评估和优化代理配置。

Result: CCA在SWE-Bench-Pro上实现了54.3%的Resolve@1性能，达到最先进水平，显著优于之前的编码代理。提供了透明、可扩展、可复现的AI代理基础。
...
<details>
  <summary>Details</summary>
Motivation: Transformer自注意力机制在长上下文推理中计算成本呈二次方增长，滑动窗口注意力(SWA)可降低为线性复杂度，但直接将SWA应用于全注意力预训练的LLM会导致性能严重下降，需要解决训练-推理不匹配问题。

Method: 提出滑动窗口注意力适配(SWAA)，包含五种方法组合：1)仅在预填充阶段使用SWA；2)保留"sink"令牌；3)交错FA/SWA层；4)思维链(CoT)；5)微调。通过协同组合这些方法实现有效适配。

Result: 实验表明SWA适配是可行但非平凡的：单一方法不足，但特定协同组合能有效恢复原始长上下文性能。分析了不同SWAA配置的性能-效率权衡，并为不同场景提供推荐方案。

Conclusion: SWAA方法成功解决了全注意力预训练LLM适配滑动窗口注意力的挑战，在保持性能的同时显著降低长上下文推理成本，为实际应用提供了实用方案。
...
TL;DR: CoopRAG是一个新颖的检索增强生成框架，通过检索器和LLM的协同工作，以及检索器模型不同层之间的协同，提升问答任务的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型倾向于生成事实不准确的输出，现有的RAG方法在简单和多跳问答中仍然容易产生错误检索和幻觉。需要解决这些限制。

Method: 1) 将问题分解为子问题和推理链，掩码不确定位置；2) 检索与问题、子问题和推理链相关的文档；3) 通过对比检索器不同层来重排文档；4) 通过LLM填充掩码位置来重构推理链。

Result: CoopRAG在三个多跳问答数据集和一个简单问答数据集上，在检索和问答性能方面均优于最先进的QA方法。

Conclusion: CoopRAG通过检索器和LLM之间的协同工作，以及检索器模型内部层的协同，有效提高了问答系统的准确性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 为俄语社区提供可访问的开源系统，用于构建和评估高效实用的俄语LLM应用，促进俄语推理研究和应用开发

Method: 使用西里尔密集分词器，采用适配的EAGLE推测解码管道降低延迟，支持直接回答和推理轨迹生成

Result: 发布了完整的开源资源包：模型权重、T-Wix 500k指令语料库、T-Math推理基准、EAGLE权重，并提供公开Web演示展示推理加速效果

Conclusion: T-pro 2.0作为一个可访问的开源系统，为构建和评估高效实用的俄语LLM应用提供了基础，支持可复现和可扩展的研究
...
TL;DR: SRAP框架通过两阶段方法检测和恢复对抗性抄袭中的"扭曲短语"，使用统计异常检测和基于源文档的语义重建，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 科学文献的完整性正受到对抗性文本生成技术的威胁，特别是使用自动转述工具掩盖抄袭行为。这些工具生成"扭曲短语"（统计上不太可能的同义词），在保持局部语法同时隐藏原始来源。现有检测方法依赖静态黑名单或通用语言模型，对新奇混淆检测效果差且无法确定抄袭来源。

Method: 提出语义重建对抗性抄袭（SRAP）框架，采用两阶段架构：1）使用领域特定掩码语言模型（SciBERT）进行基于伪困惑度的统计异常检测；2）使用密集向量检索（FAISS）和句子级对齐（SBERT）进行基于源文档的语义重建。

Result: 在对抗性科学文本平行语料库上的实验显示，零样本基线完全失败（0.00%恢复准确率），而检索增强方法达到23.67%恢复准确率，显著优于基线方法。静态决策边界在术语密集的科学文本中对于鲁棒检测是必要的。

Conclusion: SRAP框架不仅能检测对抗性抄袭异常，还能通过数学方法恢复原始术语，实现法医分析，将混淆表达链接回最可能的源文档，为科学文献完整性保护提供有效工具。
...
TL;DR: 通过将知识图谱与大型语言模型结合，使用KG-BERT增强模型的基于事实的推理能力，显著提升问答和实体链接等知识密集型任务的表现。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（如Claude、Mistral IA、GPT-4）虽然在自然语言处理方面表现出色，但缺乏结构化知识，导致事实不一致性问题。需要增强模型的事实可靠性和推理能力。

Method: 通过集成知识图谱（KGs），使用KG-BERT方法将结构化知识融入大型语言模型，增强模型的基于知识的推理和事实基础。

Result: 实验显示在知识密集型任务（如问答和实体链接）上取得显著提升，改善了事实可靠性，使模型更具上下文感知能力。

Conclusion: 知识图谱与大型语言模型的结合能够有效解决事实不一致问题，为下一代更具上下文感知能力的大型语言模型提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有教育聊天机器人通常只专注于辅导或情感支持，缺乏同时处理认知和情感状态的综合能力。需要开发能够实时理解学生认知和情感状态、并提供适应性教育干预的系统。

Method: 结合大型语言模型(LLMs)、知识图谱增强BERT(KG-BERT)和带注意力的双向LSTM，利用文本语义、语音韵律特征和时间行为趋势等多模态数据，实时分类学生的参与度、压力和概念理解状态。

Result: 对大学生进行的试点研究表明，相比基线方法，该系统提高了学习动机、降低了压力水平，并取得了适度的学业提升。

Conclusion: 整合语义推理、多模态融合和时间建模的方法，为支持适应性、以学生为中心的教育干预提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统生成语法中，语法性系统对比（如主助动词倒装和寄生空位许可）被视为内部层级语法的证据。本研究旨在测试仅基于表面形式训练的大语言模型（LLMs）是否能够复现这些对比，从而暗示其具有底层结构表征。

Method: 聚焦两个经典结构：主助动词倒装（测试主语边界识别）和寄生空位许可（测试抽象依赖结构）。使用GPT-4和LLaMA-3等模型，通过提示引出可接受性评分进行评估。

Result: LLMs能够可靠地区分两种结构中的语法正确与错误变体，表明它们对结构敏感，而不仅仅是线性顺序。结构概括（区别于认知知识）从表面形式的预测训练中涌现，表明对句法具有功能性敏感性，无需显式编码。

Conclusion: LLMs通过表面形式训练后能够发展出对句法结构的敏感性，支持结构表征可以从预测性语言建模中涌现的观点，而无需显式语法规则编码。
...
TL;DR: 本文提出XDoGE方法优化多语言LLM训练中的语言分布，通过代理模型确定最佳语言权重，并训练了专注于伊比利亚语言的7B模型


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型过度依赖英语等高资源语言，导致中低资源语言性能不佳，需要优化多语言训练中的语言分布

Method: 提出XDoGE方法：1) 扩展DoGE算法到多语言设置，用小代理模型优化语言分布；2) 用确定的最佳权重重新缩放数据，从头训练或持续预训练完整模型；针对6种不同资源水平的伊比利亚语言进行实验

Result: 研究了小语种数据重复和主导语言欠采样的影响，使用IberoBench框架评估，最终发布了专注于伊比利亚语言和英语的IberianLLM-7B-Instruct模型

Conclusion: 通过XDoGE方法优化多语言训练的语言分布，可以有效提升中低资源语言性能，并成功训练出专注于伊比利亚语言的高质量7B模型
...
TL;DR: 本文研究了不同架构LLM在因果推理任务中的表现，发现ICL单独使用不可靠，微调的编码器/编码器-解码器架构在中小规模下比仅解码器架构更稳健，后者需要大规模才能匹配性能。


<details>
  <summary>Details</summary>
Motivation: 研究ICL在大语言模型因果推理中的作用和性能，因果推理需要多跳组合和严格合取控制，而现有模型可能依赖输入中的虚假词汇关系导致误导结果。

Method: 比较微调后的编码器、编码器-解码器和仅解码器架构在零样本和少样本ICL设置下的表现，测试包括自然语言和非自然语言场景。

Result: ICL单独使用不足以进行可靠的因果推理，经常过度关注不相关的输入特征。仅解码器模型对分布变化特别脆弱，而微调的编码器和编码器-解码器模型在测试中（包括非自然语言场景）能更稳健地泛化。仅解码器架构需要大规模才能匹配或超越前两者。

Conclusion: 对于成本效益高、短时间范围内的稳健因果推理，带有针对性微调的编码器或编码器-解码器架构是更优选择，仅解码器架构需要大规模才能达到类似性能。
...
TL;DR: 论文提出了RoleRMBench基准和RoleRM奖励模型，用于解决角色扮演对话中现有奖励模型性能下降的问题，通过连续隐式偏好方法显著提升了叙事一致性和风格保真度。
...
Method: 提出了RoleRMBench基准（覆盖叙事管理、角色一致性等7个细粒度能力），并设计了RoleRM奖励模型，采用连续隐式偏好方法，将主观评估重构为多结构策略下的连续一致成对监督。

Result: RoleRM在RoleRMBench上平均超越现有开源和闭源奖励模型超过24%，在叙事连贯性和风格保真度方面取得显著提升，揭示了连续偏好表示和标注一致性的重要性。

Conclusion: 研究强调了连续偏好表示和标注一致性在主观对齐中的重要性，为人本对话系统的主观对齐奠定了基础，展示了专门的角色扮演奖励建模的价值。
...
TL;DR: AgriGPT-Omni是一个农业全模态框架，整合语音、视觉和文本，通过数据合成、三阶段训练和基准测试，显著提升多语言多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在农业应用中面临三大限制：缺乏多语言语音数据、统一的多模态架构和全面的评估基准。这些限制阻碍了农业智能系统的发展，特别是在低资源地区。

Method: 1. 构建可扩展的数据合成和收集管道，将农业文本和图像转化为训练数据，创建了最大的农业语音数据集（49.2万合成+1.4万真实样本，覆盖6种语言）
2. 采用三阶段训练范式：文本知识注入、渐进式多模态对齐、基于GRPO的强化学习
3. 提出AgriBench-Omni-2K基准测试，覆盖多样化的语音-视觉-文本任务和多语言切片

Result: AgriGPT-Omni在多项指标上显著优于通用基线模型，特别是在多语言多模态推理和真实世界语音理解方面表现出色。所有模型、数据、基准和代码都将开源。

Conclusion: 该研究提出了首个农业全模态框架，解决了农业AI中的数据稀缺和评估标准问题，促进了可重复研究、包容性农业智能和低资源地区的可持续AI发展。
...
TL;DR: 该研究以塞尔维亚语为例，探讨低资源语言在AI时代面临的技术发展挑战，提出基于CARE原则的Data Care框架，旨在构建更具包容性和文化敏感性的语言技术。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要基于英语等主导语言训练，对低资源语言的表示往往反映源语言材料的文化和语言偏见。研究以塞尔维亚语为案例，探讨AI时代低资源语言技术发展的结构、历史和社会技术因素。

Method: 采用半结构化访谈方法，采访了十位学者和实践者，包括语言学家、数字人文主义者和AI开发者，分析塞尔维亚语面临的挑战及其根源。

Result: 研究发现塞尔维亚语面临历史文本遗产破坏、当代问题导致的功能优先工程方法等问题，具体表现为表面化音译、依赖英语训练模型、数据偏见和缺乏文化特异性的数据集管理。

Conclusion: 研究提出基于CARE原则（集体利益、控制权、责任和伦理）的Data Care框架，将偏见缓解从技术修复转变为语料库设计、标注和治理的组成部分，为在传统LLM开发复制现有权力不平衡和文化盲点的背景下构建包容、可持续和文化基础的语言技术提供可复制模型。
...
TL;DR: 提出一个包含四个组件的全面数据偏见检测与缓解管道，针对表示偏见和刻板印象两种偏见类型，通过LLM生成词表、表示偏见量化、刻板印象过滤和反事实数据增强等方法，在性别、宗教和年龄等敏感属性上验证效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练数据存在多方面偏见表现，包括有害语言和倾斜的人口分布。欧盟AI法案等法规要求识别和减轻针对受保护群体的偏见，但缺乏实践指导和可操作性方法。

Method: 提出四组件管道：1) 基于质量标准的LLM生成相关群体标签词表；2) 使用人口统计表示分数量化表示偏见；3) 社会语言学知情过滤检测和缓解刻板印象；4) 语法和上下文感知的反事实数据增强补偿表示偏见。

Result: 在数据去偏见方面成功减少了表示偏见和显性刻板印象。但在模型偏见减少方面，使用去偏见数据微调的LLM在偏见基准测试中并未一致表现改善，暴露了当前评估方法的差距。

Conclusion: 虽然提出的方法能有效减少数据偏见，但数据去偏见不一定能转化为模型偏见的减少，这揭示了当前评估方法的局限性，并强调需要有针对性的数据操作来解决显现的模型偏见。
...
TL;DR: OPV（基于结果的流程验证器）通过验证长思维链中总结结果的推理过程，实现准确高效的验证，并通过主动学习框架降低标注成本，在多个基准测试中取得SOTA性能。
...
TL;DR: TRIDENT是一个三层调度员支持系统，通过加勒比口音优化的ASR、本地实体提取和生物声学痛苦检测，在ASR失败时仍能为调度员提供结构化输入，确保加勒比口音人群公平获得紧急分诊服务。


<details>
  <summary>Details</summary>
Motivation: 现有紧急语音识别系统对非标准英语变体（特别是加勒比口音）存在系统性性能下降，导致加勒比人群在紧急服务中存在关键差距。需要开发能够处理口音变化和压力诱导语音变化的系统。

Method: 三层架构：1) 加勒比口音优化的自动语音识别；2) 基于大语言模型的本地实体提取；3) 生物声学痛苦检测。系统提供三个互补信号：转录置信度、结构化临床实体和声音压力指标。关键洞察：低ASR置信度可作为队列优先级信号，特别是与升高的声音痛苦标记结合时。

Result: 建立了一个口音弹性的紧急AI框架，确保加勒比口音能够公平访问国家分诊协议。系统设计考虑了离线操作和灾难场景部署，但加勒比紧急呼叫的实证验证仍需未来工作。

Conclusion: TRIDENT为口音弹性的紧急AI建立了框架，通过将低ASR置信度重新定义为有价值的优先级信号，结合声音压力和语义分析，确保加勒比口音人群在紧急情况下能够获得公平的分诊服务。
...
TL;DR: 提出OPV验证器，通过总结长推理链的结果来验证推理过程，结合主动学习和专家标注实现高效准确验证


<details>
  <summary>Details</summary>
Motivation: 现有结果验证器无法检查长推理链中的不可靠中间步骤，而过程验证器因高质量标注稀缺难以可靠检测复杂长推理链中的错误

Method: 提出基于结果的流程验证器OPV，通过总结长推理链的结果来验证推理过程；采用迭代主动学习框架，通过专家标注不确定案例，使用拒绝微调和RLVR训练新OPV

Result: OPV在OPV-Bench上达到SOTA，F1分数83.1优于Qwen3-Max-Preview的76.3；能有效检测合成数据集中的假阳性；与策略模型协作可将DeepSeek-R1-Distill-Qwen-32B在AIME2025上的准确率从55.2%提升到73.3%

Conclusion: OPV实现了准确高效的验证，支持大规模标注，在多个基准测试中表现优异，并能显著提升策略模型的性能
...
TL;DR: 通过缩放策略提升语言模型在中等和低资源语言上的表现，发现缩放比持续预训练更数据高效，能减少灾难性遗忘，并探索了模型合并构建多语言系统的方法。


<details>
  <summary>Details</summary>
Motivation: 大规模多语言模型在中小规模时仍不如语言特定适配模型，尤其是在中等和低资源语言上表现不佳。需要寻找更有效的策略来适应新目标语言。

Method: 通过全面的缩放消融实验，在FLOP匹配的模型上测试缩放策略：将英语基础模型放大，与标准持续预训练方法对比。同时探索将缩放后的语言特定模型合并构建模块化多语言系统。

Result: 当接触足够的目标语言数据后，放大的模型能够匹配或超越在更多数据上持续预训练的小模型，显示缩放的数据效率优势。缩放还能保持基础模型在英语上的能力，减少灾难性遗忘。模型合并虽然不如联合多语言训练有效，但放大后的合并表现更好，且不同合并方法间存在较大性能差异。

Conclusion: 缩放是适应新目标语言的高效策略，比持续预训练更数据高效且能减少灾难性遗忘。模型合并构建多语言系统仍有改进空间，特别是需要开发专门针对语言层面整合的合并方法。
...
TL;DR: LLMs在印度临床应用中表现不佳，特别是当用户使用罗马化文本而非本地文字时，在母婴健康分诊任务中性能下降5-12个F1点，可能导致近200万额外错误。


<details>
  <summary>Details</summary>
Motivation: 在印度临床应用中，用户经常使用罗马化文本而非本地文字进行交流，但现有研究很少使用真实数据评估这种拼写变体对LLM可靠性的影响，特别是在母婴健康分诊这样的关键领域。

Method: 使用包含五种印度语言和尼泊尔语的真实用户生成查询数据集，对领先的LLM进行基准测试，比较罗马化文本和本地文字的性能差异，并分析性能差距的原因。

Result: 罗马化消息的性能持续下降，F1分数比本地文字低5-12点；在合作母婴健康组织中，这种差距可能导致近200万额外分诊错误；性能差距并非临床推理失败，LLM通常能正确推断罗马化查询的语义意图，但最终分类输出在面对罗马化输入的拼写噪声时仍然脆弱。

Conclusion: 研究揭示了基于LLM的健康系统中一个关键的安全盲点：看似能理解罗马化输入的模型可能仍然无法可靠地对其采取行动，这对在印度等多元语言环境中部署LLM的临床系统具有重要安全意义。
...
TL;DR: FACTS Leaderboard是一个综合性评估语言模型事实准确性的在线基准套件，包含四个子基准：多模态、参数化、搜索和文档基础，通过自动化评分模型评估模型在不同场景下生成事实准确文本的能力。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏全面评估语言模型事实准确性的统一基准，需要开发一个能衡量模型在不同场景下（包括图像问答、闭卷知识、信息搜索和文档基础）生成事实准确文本能力的综合评估体系。

Method: 创建包含四个子基准的在线领导板套件：FACTS Multimodal（图像问题回答）、FACTS Parametric（闭卷事实问题）、FACTS Search（搜索API使用）、FACTS Grounding v2（文档基础长文本生成）。使用自动化评分模型评估模型响应，最终得分为四个子基准的平均分。

Result: 开发了FACTS Leaderboard Suite，提供公开和私有数据集分割，支持外部参与同时保护基准完整性。该套件在Kaggle平台上线（https://www.kaggle.com/benchmarks/google/facts），并将持续维护更新。

Conclusion: FACTS Leaderboard提供了一个全面、平衡且稳健的语言模型事实准确性评估框架，通过多维度评估帮助研究者和开发者更好地理解和改进模型的事实生成能力。
...
TL;DR: LabelFusion是一个融合集成框架，通过结合传统Transformer分类器和大型语言模型，实现准确且成本感知的文本分类。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer分类器（如RoBERTa）虽然有效，但大型语言模型（LLM）在推理和语义理解方面有独特优势。然而，LLM使用成本高、延迟大。需要一种方法能结合两者的优势，同时考虑实际部署中的成本、延迟和准确性平衡。

Method: 1. 将传统Transformer分类器的嵌入向量与LLM生成的每类得分（通过结构化提示工程策略获得）进行拼接；2. 将拼接后的联合表示输入到紧凑的多层感知机（FusionMLP）中生成最终预测；3. 提供简单的高级接口（AutoFusionClassifier）进行端到端训练，同时为高级用户提供灵活API。

Result: 在AG News数据集上达到92.4%准确率，在10类Reuters 21578主题分类上达到92.3%准确率。该方法在多个领域展现出鲁棒性能，同时实现了准确性、延迟和成本之间的实用权衡。

Conclusion: LabelFusion成功融合了LLM推理能力和传统Transformer分类器的优势，提供了一种既能保持高性能又能进行成本控制的文本分类解决方案，为实际应用中的多类和多标签任务提供了实用框架。
...
TL;DR: 使用计算文本分析研究《霍比特人》对话的情感基调，发现整体积极、平静，随着故事进展逐渐增强控制感


<details>
  <summary>Details</summary>
Motivation: 探索如何通过计算工具揭示文学作品中的情感结构，特别是《霍比特人》对话的情感节奏和情感调节机制

Method: 使用正则表达式提取对话，预处理后应用NRC-VAD词典量化情感维度（效价、唤醒度、支配度），并通过情感轨迹图和词云进行可视化

Result: 对话整体保持积极（高效价）、平静（低唤醒度）的基调，随着故事进展控制感（支配度）逐渐增强；危险和兴奋时刻与幽默、友情和放松相平衡

Conclusion: 计算工具与文学解释相结合能够揭示文学作品中的微妙情感结构，展示了《霍比特人》中塑造叙事的稳定节奏和情感调节
...
TL;DR: 评估多模态大语言模型在政治视频情绪分析中的表现，发现在理想条件下表现良好，但在真实议会辩论场景中效果不佳
...
Result: 理想条件下mLLMs情绪唤起评分高度可靠且无人口统计偏差，但在真实议会辩论场景中表现不佳，可能影响下游统计推断
...
TL;DR: 该研究构建了一个多智能体语言习得计算实验室，通过母代理生成语言样本，子代理分析统计模式来习得抽象语法知识，验证了从语言输入中自动获取语法范畴的可行性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索如何通过计算模型模拟语言习得过程，特别是如何从语言输入中自动获取抽象语法知识，而不依赖先验的内部语言知识。

Method: 构建包含母语言模型和子语言模型的多智能体系统。母模型生成语言样本，子模型仅能访问这些样本。使用层次凝聚聚类分析对母模型生成的连续话语进行统计模式分析，提取离散语法规则并添加到子模型的语法知识中。

Result: 系统成功习得了类似于语言学家为自然语言提出的语法范畴结构，证实了非平凡语法知识的获取。通过训练数据确定的参数配置在测试集实验中得到了验证，同样实现了非平凡语法范畴的习得。

Conclusion: 该计算实验室环境能够有效模拟语言习得过程，证明仅通过语言输入样本的统计分析就能获取抽象语法知识，为语言习得研究提供了有效的计算工具。
...
TL;DR: ExaCraft是一个AI驱动的个性化示例生成系统，通过分析学习者的动态上下文（包括位置、教育背景、职业偏好等）和实时行为数据，为学习者提供文化相关且量身定制的学习示例。


<details>
  <summary>Details</summary>
Motivation: 现有教育AI工具未能专注于生成个性化示例，也无法适应学习者不断变化的理解水平、学习困难和技能增长。学习最有效的方式是将知识与学习者个人相关的、有共鸣的示例联系起来。

Method: 结合Google Gemini AI和Python Flask API，通过Chrome扩展访问。系统整合用户定义的配置文件（位置、教育、职业、复杂度偏好）和实时学习者行为分析，能够适应学习上下文的五个关键方面：困难指标、掌握模式、主题进展历史、会话边界和学习进展信号。

Result: 系统能够生成从基础概念到高级技术实现的演化示例，响应主题重复、重新生成请求和主题进展模式。示例既具有文化相关性，又针对个人学习需求进行定制。

Conclusion: ExaCraft通过动态适应学习者的上下文，提供个性化的学习示例，解决了现有教育AI工具在示例生成和适应性方面的不足，有望提高学习效果。
...
TL;DR: 提出"具身信息中心"概念，通过物理对话交互改善知识共享体验，开发了本地运行的软体AI代理Suzume-chan


<details>
  <summary>Details</summary>
Motivation: 现有数字工具虽然改善了信息获取，但缺乏深度理解所需的人际连接感。专家知识获取通常需要实时人际交流，而传统工具难以创造这种连接

Method: 基于社会临场感理论，提出"具身信息中心"概念，开发了Suzume-chan原型——一个本地运行的小型软体AI代理，结合语言模型和检索增强生成技术，通过语音对话进行知识学习与分享

Result: Suzume-chan能够通过对话学习专家知识并响应，减少了心理距离，使知识共享更加温暖和以人为本

Conclusion: 具身信息中心通过物理和对话交互，利用社会临场感理论改善了知识共享体验，为创建更人性化的知识传播系统提供了新途径
...
TL;DR: 提出一个两阶段框架用于健康信息检测：先预测证据一致性分数，若分数不足则进行多智能体辩论，结合自动化评分与协作推理提升验证效果。


<details>
  <summary>Details</summary>
Motivation: 随着健康相关虚假信息在网络上的泛滥，有效的事实核查变得至关重要。这需要高质量证据检索和严谨的推理过程，但目前的方法在这两方面存在不足。

Method: 采用两阶段框架：1) 一致性分数预测阶段：使用大语言模型独立评估检索到的文章，计算反映整体证据立场的一致性分数；2) 多智能体辩论阶段：当一致性分数低于预设阈值时，多个智能体进行结构化辩论，综合冲突证据并生成有明确理由的判断。

Result: 实验结果表明，这种两阶段方法相比基线方法取得了更优的性能，证明了将自动化评分与协作推理相结合对于复杂验证任务的价值。

Conclusion: 该研究提出的两阶段框架通过结合一致性评分和多智能体辩论，能够有效检测健康虚假信息，为复杂的事实核查任务提供了一种有效的解决方案。
...
TL;DR: Echo-CoPilot：一个基于大语言模型的多视图、多任务心脏超声智能代理，通过协调专用工具实现临床连贯的超声评估


<details>
  <summary>Details</summary>
Motivation: 心脏超声是心血管诊疗的核心，但全研究解读仍是需要人工完成的认知密集型多视图任务。现有的基础模型虽然能在单个感知子任务（如视图分类、分割、疾病预测）上表现良好，但通常孤立运行，无法提供统一、临床连贯的评估。

Method: 提出Echo-CoPilot，一个多视图、多任务代理，使用大语言模型协调一套专门的心脏超声工具。在ReAct风格循环中，代理分解临床医生查询，调用视图识别、心脏结构分割、测量和疾病预测、报告生成等工具，并将输出整合为指南感知的答案和叙述性总结。

Result: 在公开的MIMIC-EchoQA基准测试中，Echo-CoPilot达到50.8%的准确率，优于通用和生物医学视频视觉语言模型。定性分析显示，代理能利用定量测量和生理学上下文解决临床决策阈值附近的挑战性病例。

Conclusion: Echo-CoPilot通过大语言模型协调专门工具，实现了临床连贯的心脏超声评估，在基准测试中表现优异，并能处理临床决策阈值附近的复杂病例，有望提升心脏超声解读的效率和一致性。
...
TL;DR: 提出一个扩展FCM因果关系的模糊优化框架，利用动力学将数据映射到度量中，通过多重网络分析概念间的逻辑蕴含和层次结构，用于服务流程设计中的信息传输优化。
...
Method: 提出新的模糊优化框架，利用动力学将数据映射到度量空间，通过多重网络（multiplex）结构分析概念间的逻辑蕴含和层次关系，建立白盒理论模型分析框架的逻辑和数学基础。

Result: 建立了扩展FCM因果关系的理论框架，能够分析概念间的逻辑蕴含和层次结构，为服务流程设计中的信息传输优化提供了新的分析工具，并对FHM进行了简洁优雅的逻辑分析。

Conclusion: 该框架成功扩展了FCM的因果关系建模能力，为服务优化中的信息传输问题提供了有效的理论工具，通过多重网络结构和动力学映射实现了概念间关系的深入分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理科学文献时面临多重挑战：长上下文文档、多模态内容、跨多篇文献的信息不一致性，以及数据模式快速变化导致系统难以重新架构或微调。

Method: 提出SciEx模块化框架，将PDF解析、多模态检索、信息提取和聚合等关键组件解耦，支持新模型、提示策略和推理机制的灵活集成。

Result: 在三个科学主题的数据集上评估了SciEx提取细粒度信息的准确性和一致性，提供了关于当前基于LLM的提取流程优势和局限性的实用见解。

Conclusion: SciEx框架通过模块化设计简化了按需数据提取，同时支持扩展性和灵活性，为科学信息提取提供了更有效的解决方案。
...
Method: DynaMate是一个模块化多智能体框架，集成了动态工具使用、网络搜索、PaperQA和自校正行为。包含三个专门模块：实验规划、模拟执行和结果分析，能够自主设计并执行完整的MD工作流程，并提供MM/PB(GB)SA方法的结合自由能计算。
...
Result: 实验结果表明，包括视觉语言模型在内的最先进模型在SWR任务上表现不佳，缺乏城市环境所需的鲁棒感知、推理和规划能力。

Conclusion: SWR平台填补了城市环境机器人仿真研究的空白，提出的基准测试能够全面评估机器人在真实城市场景中的关键能力，为开发更强大的城市机器人系统提供了重要工具。
...
TL;DR: PDT是一种参数高效的并行解码架构，通过注入轻量级适配器实现并行流协调，无需修改预训练模型权重，解决了传统并行生成方法的连贯性漂移问题。
...
Method: 提出并行解码变换器（PDT），通过注入轻量级的"推测性笔记条件"适配器，允许并行解码流通过共享的动态潜在空间进行同步。将协调问题形式化为"推测共识"问题，其中兄弟流向全局总线广播语义"笔记"，由学习的验证头进行门控。
...
TL;DR: 通过大规模文献计量分析发现AI安全与AI伦理研究存在明显的结构性分裂，80%以上合作发生在各自社区内部，跨领域交流高度集中于少数关键研究者。


<details>
  <summary>Details</summary>
Motivation: AI能力快速发展的同时，对齐（alignment）研究却沿着两条平行轨道发展：安全（关注扩展智能、欺骗行为和存在风险）与伦理（关注当前危害、社会偏见和生产流程缺陷）。两个社区对"对齐"的定义和理解存在分歧，导致研究相对孤立，形成了不同的方法论、机构归属和学科谱系。

Method: 采用文献计量和合著网络分析方法，对2020-2025年间12个主要ML和NLP会议的6,442篇论文进行分析，研究安全与伦理社区之间的结构分裂程度。

Result: 1. 超过80%的合作发生在安全或伦理社区内部；2. 跨领域连通性高度集中：约5%的论文贡献了超过85%的桥梁链接；3. 移除少数关键中介者会显著增加社区隔离，表明跨学科交流依赖于少数关键参与者而非广泛分布的合作。

Conclusion: 安全与伦理的分裂不仅是概念上的，更是制度性的，对研究议程、政策和学术场所有重要影响。需要通过共享基准、跨机构平台和混合方法，将技术安全工作与规范伦理相结合，以构建既稳健又公正的AI系统。
...
TL;DR: LLMs在激活空间中形成用户社会人口统计特征的线性表示，这些表示会影响模型的下游行为，即使通过偏见基准测试的模型仍可能隐含地利用这些偏见。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs如何通过姓名、职业等间接线索编码对话伙伴的社会人口统计属性，以及这些隐含的人口统计表示如何影响模型行为，这对于大规模应用中的公平性有重要意义。

Method: 在四个开源transformer LLMs（Magistral 24B, Qwen3 14B, GPT-OSS 20B, OLMo2-1B）的残差流中探测人口统计表示，使用显式人口统计披露提示，然后验证这些探针能否从姓名和职业等隐含线索预测人口统计特征。

Result: LLMs在激活空间中形成用户人口统计特征的线性表示，姓名激活与人口普查一致的性别和种族表示，职业触发与现实世界劳动力统计数据相关的表示。这些隐含的人口统计表示会主动影响下游行为，如职业推荐。

Conclusion: 即使通过偏见基准测试的LLMs仍可能隐含地编码和利用人口统计偏见，这些偏见会影响模型行为，对大规模应用中的公平性有重要影响。
...
TL;DR: 稀疏自编码器（SAE）嵌入比LLM更经济可靠，比稠密嵌入更可控，可用于文本数据分析，发现数据集差异、概念关联等洞察。
...
Method: 提出使用稀疏自编码器（SAE）创建SAE嵌入，其维度映射到可解释概念，通过四个数据分析任务验证其有效性。

Result: SAE嵌入比LLM成本低2-8倍且更可靠，比稠密嵌入更可控，能发现数据集语义差异、概念关联，并在基于属性的检索中表现更好。

Conclusion: SAE是用于非结构化数据分析的多功能工具，强调通过数据解释模型的重要性，为模型行为分析提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 为了负责任地采用AI技术，了解AI安全和对齐鲁棒性的基本限制至关重要。哥德尔不完备性定理在数学和计算理论中具有深远影响，将其扩展到AI领域可以帮助我们识别和准备应对AI系统固有的局限性

Method: 通过将哥德尔不完备性定理扩展到人工智能领域，建立信息论框架来分析AI安全和对齐的鲁棒性限制。该方法从数学和计算理论的角度形式化地证明了AI系统存在的根本性限制

Result: 证明了AI安全和对齐存在固有的信息论限制，这些限制源于哥德尔不完备性定理的扩展。同时提供了应对这些挑战的实用方法，并证明了AI系统认知推理能力的更广泛限制

Conclusion: AI安全和对齐存在根本性的信息论限制，这些限制无法完全克服。了解这些限制对于负责任地采用AI技术至关重要，论文提供了应对这些挑战的实用方法，并揭示了AI系统认知推理能力的更广泛局限性
...
TL;DR: 开发了一个两阶段计算框架分析阴谋论内容：先用RoBERTa-large分类消息，再构建带符号的信念图神经网络(SiBeGNN)进行嵌入学习，发现阴谋论内容广泛融入日常讨论而非局限于孤立回音室。


<details>
  <summary>Details</summary>
Motivation: 阴谋论话语日益嵌入数字通信生态系统，但其结构和传播难以研究。现有研究常假设阴谋论内容局限于孤立回音室，但实际情况可能更复杂，需要新方法来分析其在日常讨论中的嵌入方式。

Method: 两阶段计算框架：1) 微调RoBERTa-large模型分类2000条专家标注消息(阴谋论/非阴谋论)，F1分数0.866；2) 构建带符号的信念图，节点代表消息，边符号反映信念标签对齐，权重为文本相似度；提出SiBeGNN模型，使用符号解缠损失学习分离意识形态对齐和风格特征的嵌入。
...
Conclusion: 阴谋论话语在普通社交互动中运作，挑战了关于网络激进化的常见假设。该框架推进了信念驱动话语分析的计算方法，可用于立场检测、政治传播研究和内容审核政策。
...
TL;DR: AgriRegion是一个专门为农业咨询设计的检索增强生成框架，通过地理空间元数据注入和区域优先重排机制，减少LLM在农业领域的上下文幻觉，提供区域准确的建议。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在农业领域经常出现上下文幻觉问题，提供非事实性建议，或者建议在某个地区科学合理但在另一个地区可能造成灾难性后果，因为存在土壤、气候和当地法规的差异。

Method: AgriRegion是一个检索增强生成框架，包含地理空间元数据注入层和区域优先重排机制。通过将知识库限制在已验证的当地农业推广服务，并在检索时强制执行地理空间约束，确保建议的本地准确性。

Result: 实验表明，AgriRegion相比最先进的LLM系统减少了10-20%的幻觉，并通过综合评估显著提高了信任分数。创建了包含12个农业子领域160个问题的基准数据集AgriRegion-Eval。

Conclusion: AgriRegion框架通过结合地理空间约束和区域优先检索，有效解决了农业咨询中的上下文幻觉问题，为不同地区提供准确可靠的农业建议。
...
<details>
  <summary>Details</summary>
Motivation: 随着基础模型开发商成为全球最重要的公司，需要持续评估其透明度实践的变化，为政策制定者提供参考，并揭示当前透明度状况及政策干预的必要性。

Method: 采用年度基础模型透明度指数（FMTI），引入新的评估指标（数据获取、使用数据、监控等），首次评估阿里巴巴、DeepSeek、xAI等公司，对主要基础模型开发商进行量化评估。

Result: 透明度整体恶化：平均分从2024年的58分降至2025年的40分；公司在训练数据、训练计算以及部署后使用和影响方面最不透明；IBM表现突出（95分），xAI和Midjourney最低（14分）；前沿模型论坛成员处于中间水平。

Conclusion: 基础模型开发商的透明度在下降，需要更积极的政策干预来解决关键信息缺失问题，特别是在训练数据和部署后影响等关键领域。
...
TL;DR: CP-Env是一个可控的智能医院环境，用于评估大语言模型在端到端临床路径中的表现，模拟真实医院生态系统，包含患者和医生智能体，支持从分诊到多学科会诊的完整临床流程评估。
...
Method: 构建CP-Env可控智能医院环境，模拟包含患者和医生智能体的医院生态系统。创建从分诊、专科会诊、诊断测试到多学科团队会议的场景。采用三层评估框架：临床效果、流程能力和专业伦理。

Result: 大多数模型在应对临床路径复杂性时表现不佳，存在幻觉问题且容易丢失关键诊断细节。有趣的是，过多的推理步骤有时会产生反效果，而顶级模型倾向于通过内化知识减少工具依赖。
...
TL;DR: 本文提出使用多目标强化学习来优化搜索算法中的算子序列选择，旨在解决多目标优化中算子选择效率问题


<details>
  <summary>Details</summary>
Motivation: 优化和搜索过程中的效率问题持续影响算法性能。虽然使用算子池代替单一算子处理邻域移动操作有前景，但最优或接近最优的算子序列选择仍需深入研究。多目标优化领域的算子序列选择问题尚未得到充分探索。

Method: 提出基于多目标强化学习的泛化方法，通过多阶段实现，已完成某些阶段，部分阶段仍在进行中。该方法旨在学习并泛化经验，优化算子序列选择。

Result: 论文概述了提出的方法框架，展示了已完成的工作阶段，并指出剩余待完成阶段。该方法有望为解决多目标优化中的算子序列选择问题提供有效解决方案。

Conclusion: 多目标强化学习为解决优化算法中算子序列选择问题提供了有前景的途径，特别是在多目标优化领域。提出的泛化方法框架展示了其潜力，需要进一步完成剩余阶段来验证其效率。
...
TL;DR: ID-PaS扩展了预测-搜索框架，使其能处理参数化混合整数规划问题，通过身份感知学习框架更有效地处理异构变量，在多个现实世界大规模问题上优于Gurobi和传统PaS方法。
...
Method: 提出了ID-PaS（身份感知学习框架），扩展了预测-搜索框架到参数化MIPs，使机器学习模型能更有效地处理异构变量。

Result: 在多个现实世界大规模问题上，ID-PaS始终表现出优于最先进求解器Gurobi和传统PaS方法的性能。

Conclusion: ID-PaS成功扩展了预测-搜索框架到参数化混合整数规划问题，通过身份感知学习有效处理异构变量，在实际应用中展现出显著优势。
...
TL;DR: 提出反向思维框架，通过逆向推理识别缺失信息，显著提升LLMs在信息缺失任务上的性能


<details>
  <summary>Details</summary>
Motivation: LLMs在处理缺失信息问题时存在不完整回答、事实错误和幻觉等问题，传统的前向推理方法（如CoT、ToT）无法系统识别和恢复被省略的信息

Method: 提出基于反向思维的新框架，引导LLMs通过逆向推理识别必要条件并定位缺失元素，将缺失信息识别任务转化为更易处理的逆向推理问题

Result: 实验结果表明，反向思维方法相比传统前向推理方法取得了显著的性能提升，为增强LLMs的逻辑完整性和推理鲁棒性提供了有前景的方向

Conclusion: 反向思维方法能有效提升LLMs在缺失信息检测任务上的表现，为解决LLMs在信息不完整情况下的推理问题提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 传统注意力机制虽然改进了RNN的表征学习能力，但其离散特性限制了连续时间建模。需要一种生物合理的连续时间注意力机制来克服这一限制。

Method: 将注意力logit计算重新表述为线性一阶ODE的解，利用非线性互连门控（源自C. elegans神经元电路策略）。采用稀疏感官门控替代密集投影，稀疏骨干网络计算内容-目标和可学习时间常数门控。提供三种计算模式：显式欧拉积分、精确闭式解和稳态近似。

Result: NAC在多个领域（不规则时间序列分类、自动驾驶车道保持、工业预测）中匹配或优于基线方法，在运行时间和内存效率方面处于中等水平。

Conclusion: NAC是一种有效的生物合理连续时间注意力机制，具有理论保证和实际应用价值，为连续时间建模提供了新方法。
...
TL;DR: 本文提出了一种新的可解释性框架来分析视觉语言模型（VLM）的内部机制，重点关注注意力头在多模态推理中的功能角色。通过CogVision数据集和探测方法，识别出专门执行特定认知功能的"功能头"，发现这些头具有稀疏性、层次组织等特性，并通过干预实验验证了它们对多模态推理的关键作用。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态基准测试中表现出色，但其内部机制仍然是一个黑箱。为了理解VLM的工作原理，特别是注意力头在多模态推理中的功能角色，需要开发系统的可解释性框架。

Method: 1. 提出CogVision数据集，将复杂的多模态问题分解为逐步子问题，模拟人类链式思维推理范式，每个子问题关联特定的接收或认知功能（如高级视觉接收和推理）。2. 采用基于探测的方法论，识别专门执行这些功能的注意力头，并将其表征为"功能头"。3. 在不同VLM家族中分析这些功能头的特性。4. 通过干预实验（移除或强调功能头）验证它们的作用。

Result: 1. 功能头具有普遍稀疏性，在不同功能间数量和分布各异。2. 这些头介导交互和层次组织。3. 干预实验表明：移除功能头会导致性能下降，强调它们则能提高准确性，证明功能头在多模态推理中起关键作用。

Conclusion: 该研究为VLM的认知组织提供了新见解，并指出了设计更具人类对齐感知和推理能力模型的有前景方向。功能头的发现有助于理解VLM的内部工作机制，为模型可解释性和改进提供了基础。
...
TL;DR: 提出可信编排AI的十大标准框架，将治理嵌入AI生态系统执行层，确保可验证、透明、可重现和人类可控
...
Method: 提出可信编排AI的十大标准框架，整合人类输入、语义一致性、审计和溯源完整性到统一的控制面板架构中。借鉴国际标准和澳大利亚国家AI保证框架，为整个AI组件、消费者和人类参与者提供治理覆盖

Result: 开发了一个全面的保证框架，通过工程方法将可信度系统性地融入AI系统，确保执行层保持可验证、透明、可重现和有意义的人类控制

Conclusion: 可信度可以通过工程方法系统性地融入AI系统，为整个AI生态系统提供治理架构，填补技术能力与制度问责之间的差距
...
<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法面临通信与性能的权衡问题，通常假设MB级数据传输，但在实际网络约束下可能失效。需要建立通信高效的协作感知理论基础来解决这一根本矛盾。

Method: 提出信息感知框架InfoCom，基于扩展的信息瓶颈原理，采用信息净化范式而非特征操作。包括：1）信息感知编码将特征压缩为最小消息；2）稀疏掩码生成以可忽略成本识别空间线索；3）多尺度解码通过掩码引导机制渐进恢复感知信息。

Result: 在多个数据集上的实验表明，InfoCom实现了近乎无损的感知性能，同时将通信开销从MB级降至KB级，相比Where2comm和ERMVP分别减少了440倍和90倍。

Conclusion: InfoCom为通信高效的协作感知建立了开创性的理论基础，通过信息瓶颈约束下的信息净化范式，在保持感知性能的同时大幅降低通信开销，解决了实际网络约束下的协作感知问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统的流行病应对计划制定依赖劳动密集型的手工方法，效率低下且耗时。需要一种自动化、智能化的解决方案来改进公共卫生应急准备。

Method: 设计并评估EpiPlanAgent系统，这是一个基于大语言模型的多智能体框架，集成了任务分解、知识基础和模拟模块。公共卫生专业人员使用真实世界疫情场景在受控环境中测试系统。

Result: EpiPlanAgent显著提高了计划的完整性和指南一致性，同时大幅减少了开发时间。专家评估确认AI生成内容与人工撰写内容高度一致，用户反馈显示系统具有很高的实用性。
...
TL;DR: 提出用户反馈驱动的视觉语言导航自适应框架，通过整合人类交互指导来增强GSA-VLN，利用记忆库热启动机制提升导航成功率和路径效率


<details>
  <summary>Details</summary>
Motivation: 当前GSA-VLN框架缺乏用户反馈机制，仅依赖无监督环境适应，而实际部署中用户反馈能提供有价值的监督信号，显著提升自适应质量

Method: 1) 将用户反馈（导航指令和纠正信号）转化为高质量环境对齐训练数据；2) 引入记忆库热启动机制重用先前获得的环境知识；3) 在GSA-R2R基准上验证框架效果

Result: 在GSA-R2R基准上超越GR-DUET等基线方法，提升导航成功率和路径效率；记忆库热启动稳定早期导航并减少更新后的性能下降；在持续和混合自适应设置下均表现稳健

Conclusion: 用户反馈驱动的自适应框架有效缩小了静态基准与真实世界部署之间的差距，通过整合人类交互实现了更高效、更现实的视觉语言导航系统自适应
...
<details>
  <summary>Details</summary>
Motivation: 现有的概率密度比方法在组合异构模型时存在边缘路径崩溃问题，导致中间密度不可归一化，这在分子设计等需要组合多个专家模型的场景中尤为严重
...
TL;DR: REMISVFU是一个用于垂直联邦学习的表示误导框架，通过将遗忘方的编码器输出坍缩到单位球上的随机锚点，实现快速客户端级遗忘，同时保持剩余方的模型效用。


<details>
  <summary>Details</summary>
Motivation: GDPR等数据保护法规赋予联邦系统参与者被遗忘权。现有遗忘技术主要针对水平联邦学习，而垂直联邦学习（VFL）中数据按特征划分，需要专门的遗忘方法。

Method: 提出表示误导框架REMISVFU：遗忘方将其编码器输出坍缩到单位球上的随机锚点，切断特征与全局模型的统计联系；服务器通过正交投影联合优化保留损失和遗忘损失，消除梯度干扰。

Result: 在公开基准测试中，REMISVFU将后门攻击成功率抑制到自然类别先验水平，仅牺牲约2.5%的干净准确率，优于现有最先进基线方法。

Conclusion: REMISVFU为垂直联邦学习提供了一种有效的即插即用遗忘框架，能够在保护遗忘权的同时保持模型效用，解决了VFL场景下的遗忘挑战。
...
TL;DR: EmerFlow：基于LLM的新兴物品推荐框架，通过LLM推理增强特征表示，与现有推荐模型嵌入空间对齐，结合元学习优化，能在有限交互下学习表达性嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有推荐方法通常假设新兴物品历史交互很少甚至没有，这简化了问题。但实际上，好的模型需要在保持新兴物品独特性的同时，利用其与成熟物品的共享模式。需要解决新兴物品交互随时间逐渐积累的动态过程。

Method: EmerFlow框架：1) 通过LLM推理增强新兴物品的原始特征；2) 将这些表示与现有推荐模型的嵌入空间对齐；3) 通过元学习结合新交互来优化嵌入表示。

Result: 在电影和医药等多个领域的广泛实验表明，EmerFlow始终优于现有方法，能够在有限交互下为新兴物品学习到表达性强的嵌入表示。

Conclusion: EmerFlow通过LLM增强特征表示、嵌入空间对齐和元学习优化，有效解决了新兴物品推荐问题，能够在保持物品独特性的同时利用共享模式，显著提升推荐性能。
...
Method: 提出AgentProg方法，将交互历史重构为包含变量和控制流的程序结构，基于程序结构决定哪些信息应保留、哪些可丢弃。同时整合了受信念MDP框架启发的全局信念状态机制，以处理部分可观测性并适应意外的环境变化。

Result: 在AndroidWorld和扩展的长时程任务套件上的实验表明，AgentProg在这些基准测试中达到了最先进的成功率。更重要的是，在长时程任务中保持稳健性能，而基线方法则出现灾难性性能下降。

Conclusion: AgentProg通过程序引导的上下文管理方法有效解决了移动GUI智能体在长时程任务中的上下文开销问题，同时保持了任务性能，为移动自动化任务提供了更高效的解决方案。
...
### [167] [Boosting RL-Based Visual Reasoning with Selective Adversarial Entropy Intervention](https://arxiv.org/abs/2512.10414)
*Yang Yu,Zhuangzhuang Chen,Siqi Wang,Lanqing Li,Xiaomeng Li*

Main category: cs.AI

TL;DR: 提出SaEI方法，通过选择性对抗熵干预增强视觉语言模型的推理能力，在RL采样阶段通过对抗性攻击视觉输入来增加策略熵，从而提升探索能力。
...
Method: 提出选择性对抗熵干预(SaEI)：1) 熵引导对抗采样(EgAS)：将采样响应的熵作为对抗目标，生成对抗梯度攻击视觉输入，产生对抗样本；2) token选择性熵计算(TsEC)：在不扭曲VLM事实知识的前提下最大化对抗攻击效果。

Result: 在领域内和领域外数据集上的广泛实验表明，该方法能通过熵干预显著提升策略探索能力，从而增强推理能力。

Conclusion: SaEI方法通过在RL采样阶段进行选择性对抗熵干预，有效提升了视觉语言模型的探索能力和推理性能。
...
### [168] [Representation of the structure of graphs by sequences of instructions](https://arxiv.org/abs/2512.10429)
...
Method: 将图的邻接矩阵转换为一串简单指令序列，这些指令逐步构建邻接矩阵，该转换是可逆的（图↔字符串）
...
### [169] [Targeted Data Protection for Diffusion Model by Matching Training Trajectory](https://arxiv.org/abs/2512.10433)
*Hojun Lee,Mijin Koo,Yeji Song,Nojun Kwak*

Main category: cs.AI

TL;DR: TAFAP是一种通过轨迹对齐实现有效目标数据保护的方法，能够控制整个训练过程，将模型输出重定向到用户指定的目标概念，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型的个性化微调技术虽然越来越普及，但也带来了未经授权数据使用和隐私侵犯的严重问题。现有的保护方法仅限于被动降低图像质量，无法实现稳定控制。虽然目标数据保护(TDP)提供了向用户指定目标概念主动重定向的有前景范式，但现有方法由于采用快照匹配方式，未能考虑完整学习动态，导致可控性差。

Method: TAFAP(通过对抗性扰动的微调轨迹对齐)是首个通过控制整个训练轨迹成功实现有效TDP的方法。与基于快照的方法不同，TAFAP采用受数据集蒸馏启发的轨迹匹配技术，在整个微调过程中强制执行持久、可验证的转换。该方法使用对抗性扰动来确保模型在整个训练过程中持续向目标概念重定向。

Result: TAFAP显著优于现有的TDP尝试，实现了对身份和视觉模式的同时控制，首次在扩散模型中成功实现目标转换。该方法能够实现向目标概念的稳健重定向，同时保持高图像质量。

Conclusion: 这项工作实现了可验证的安全保障，并为控制和追踪扩散模型输出的改变提供了新框架。TAFAP通过轨迹对齐方法解决了现有目标数据保护方法的局限性，为保护个人数据隐私提供了更有效的解决方案。
...
### [170] [When Reject Turns into Accept: Quantifying the Vulnerability of LLM-Based Scientific Reviewers to Indirect Prompt Injection](https://arxiv.org/abs/2512.10449)
*Devanshu Sahoo,Manish Prasad,Vasudev Majhi,Jahnvi Singh,Vinay Chamola,Yash Sinha,Murari Mandal,Dhruv Kumar*

Main category: cs.AI

TL;DR: 论文研究了科学同行评审中LLM评估系统的对抗性PDF操纵漏洞，开发了WAVS指标评估"拒绝"到"接受"的决策翻转成功率，发现多种攻击策略能有效操纵评分结果。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在科学同行评审中的广泛应用（包括个人使用和机构部署），需要评估这些"LLM-as-a-Judge"系统对对抗性PDF操纵的鲁棒性，特别是针对将"拒绝"决策翻转为"接受"的特定动机攻击。

Method: 收集200篇科学论文数据集，针对"拒绝到接受"的翻转动机开发WAVS评估指标，采用15种领域特定攻击策略，在包括GPT-5、Claude Haiku、DeepSeek在内的13个语言模型上进行评估。
...
### [171] [Planning, Living and Judging: A Multi-agent LLM-based Framework for Cyclical Urban Planning](https://arxiv.org/abs/2412.20505)
*Hang Ni,Yuzhi Wang,Hao Liu*

Main category: cs.AI

TL;DR: 提出CUP循环城市规划范式，利用多智能体LLM框架实现生成、评估、优化的闭环城市规划过程


<details>
  <summary>Details</summary>
Motivation: 城市化背景下城市更新面临挑战，需要适应性的方法来应对不断变化的需求，利用大语言模型的进展来改进城市规划
...
Conclusion: CUP框架为城市规划提供了动态响应的方法，能够持续生成、评估和优化城市计划，适应不断变化的需求
...
Conclusion: 现成LLM可有效重新用作任意PCG工具的通用代理，通过将负担从模型训练转向架构推理，提供了无需任务特定微调即可掌握复杂软件的可扩展框架。
...
TL;DR: InternGeometry是一个基于LLM的几何问题求解智能体，首次实现IMO金牌水平的几何解题能力，仅用13K训练样本（AlphaGeometry 2的0.004%）就解决了44/50个IMO几何问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在几何问题求解中由于辅助构造的启发式能力较弱，仍被AlphaGeometry 2等专家模型主导，这些模型依赖大规模数据合成和搜索。本研究旨在构建一个达到金牌水平的LLM几何智能体。

Method: 1. 迭代提出命题和辅助构造，用符号引擎验证，并根据反馈指导后续提案；2. 动态记忆机制支持每个问题与符号引擎进行200多次交互；3. 引入复杂度提升强化学习（CBRL），在训练阶段逐步增加合成问题的复杂度。

Result: 在基于InternThinker-32B构建的InternGeometry上：解决了2000-2024年50个IMO几何问题中的44个，超过金牌得主平均分（40.9）；仅用13K训练样本（AlphaGeometry 2的0.004%）；能够为IMO问题提出人类解法中未出现的新颖辅助构造。

Conclusion: InternGeometry首次展示了LLM智能体在专家级几何任务上的潜力，通过迭代推理和反馈机制克服了几何启发式限制，实现了高效的数据利用和金牌水平的性能。
...
TL;DR: NormCode是一种半形式化语言，用于构建推理计划，通过严格的数据隔离消除多步LLM工作流中的上下文污染问题。


<details>
  <summary>Details</summary>
Motivation: 多步LLM工作流存在上下文污染问题：随着信息在步骤间累积，模型会产生幻觉、混淆中间输出并丢失任务约束。需要一种方法来消除步骤间的交叉污染。

Method: NormCode是一种半形式化语言，创建推理计划的结构化分解，每个步骤在数据隔离中运行，只接收显式传递的输入。它严格分离语义操作（LLM驱动的推理，非确定性）和语法操作（确定性数据重构），支持三种同构格式：.ncds用于人工编写、.ncd用于机器执行、.ncn用于人工验证。

Result: 通过两个演示验证：1）基础X加法算法在任意长度输入上达到100%准确率；2）自托管执行NormCode自身的五阶段编译器流水线。工作编排器提供依赖驱动调度、SQLite支持的检查点和循环管理。

Conclusion: NormCode通过设计消除交叉步骤污染，使AI工作流可审计，解决了法律推理、医疗决策和金融分析等高风险领域对透明度的关键需求。
...
TL;DR: Phythesis框架结合大语言模型和物理引导的进化优化，自动化生成仿真就绪的数据中心场景，提升能效设计


<details>
  <summary>Details</summary>
Motivation: 传统数据中心设计方法依赖人工经验和专用仿真工具，难以应对日益复杂的系统需求。现有生成式AI方法缺乏物理考量，不适合需要量化运营目标和严格物理约束的数据中心设计

Method: 提出Phythesis框架，采用迭代双层优化架构：1) LLM驱动优化层生成物理合理的3D布局并进行自我批评以优化场景拓扑；2) 物理信息优化层识别最优资产参数并选择最佳资产组合

Result: 在三种规模实验中，相比纯LLM方案，Phythesis实现了57.3%的生成成功率提升和11.5%的电力使用效率(PUE)改进

Conclusion: Phythesis成功将LLM与物理引导优化相结合，为数据中心能效设计提供了自动化、仿真就续的场景合成解决方案
...
TL;DR: scRCL是一个用于单细胞组学无监督细胞类型识别的框架，通过结合细胞-基因相互作用和对比学习来提升细胞表示的质量。
...
Method: 提出scRCL框架，包含两个对比分布对齐组件来揭示可靠的细胞内在结构，以及一个整合基因相关性结构学习的精炼模块来增强细胞嵌入表示。

Result: 在多个单细胞RNA-seq和空间转录组基准数据集上的实验表明，该方法在细胞类型识别准确率上持续优于现有最先进方法。

Conclusion: scRCL通过有效利用细胞-基因相互作用，能够获得更具信息量的细胞表示，提高细胞类型识别性能，并且恢复的细胞群体表现出连贯的基因表达特征，验证了其生物学相关性。
...
### [177] [CAPTAIN: Semantic Feature Injection for Memorization Mitigation in Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.10655)
*Tong Zhang,Carlos Hinojosa,Bernard Ghanem*

Main category: cs.AI

TL;DR: CAPTAIN：一种无需训练的方法，通过在去噪过程中直接修改潜在特征来减少扩散模型的记忆化问题，同时保持与提示的语义对齐


<details>
  <summary>Details</summary>
Motivation: 扩散模型可能会无意中复制训练示例，引发隐私和版权问题。现有的推理时缓解方法（如操纵无分类器引导或扰动提示嵌入）通常难以在减少记忆化的同时保持与条件提示的对齐。
...
### [178] [On the Dynamics of Multi-Agent LLM Communities Driven by Value Diversity](https://arxiv.org/abs/2512.10665)
*Muhua Huang,Qinlin Zhao,Xiaoyuan Yi,Xing Xie*

Main category: cs.AI

TL;DR: 价值多样性增强AI多智能体系统的集体行为：促进价值稳定性、涌现行为和创造性原则，但过度异质性会导致不稳定性
...
### [179] [AEBNAS: Strengthening Exit Branches in Early-Exit Networks through Hardware-Aware Neural Architecture Search](https://arxiv.org/abs/2512.10671)
*Oscar Robben,Saeed Khalilian,Nirvana Meratnia*

Main category: cs.AI

TL;DR: 本文提出了一种硬件感知的神经架构搜索方法，用于优化早期退出网络的退出分支设计，通过考虑不同深度和层类型以及自适应阈值调整，在保持或降低计算量的同时提高模型精度。


<details>
  <summary>Details</summary>
Motivation: 早期退出网络通过根据输入复杂度调整计算量来降低能耗和延迟，但手动设计退出分支位置和结构耗时且困难。现有NAS方法主要关注退出位置和数量，而退出分支的深度和层类型对效率和精度同样重要。

Method: 采用硬件感知神经架构搜索（NAS）来强化退出分支设计，在优化过程中同时考虑精度和效率，探索退出分支的不同深度和层类型，并结合自适应阈值调整。

Result: 在CIFAR-10、CIFAR-100和SVHN数据集上的评估表明，所提框架设计的早期退出网络在相同或更低平均MACs（乘加运算）下，相比现有最优方法获得了更高的准确率。

Conclusion: 通过硬件感知NAS优化退出分支的深度和层类型，结合自适应阈值调整，能够设计出更高效的早期退出网络，在资源受限设备上实现更好的精度-效率平衡。
...
### [180] [Challenges of Evaluating LLM Safety for User Welfare](https://arxiv.org/abs/2512.10687)
*Manon Kempermann,Sai Suresh Macharla Vasu,Mahalakshmi Raveenthiran,Theo Farrell,Ingmar Weber*

Main category: cs.AI

TL;DR: 该研究指出当前大语言模型安全评估主要关注通用风险，但忽视了用户个人咨询中的情境依赖性风险，特别是对弱势群体的影响。通过实验发现，评估者需要了解用户背景才能准确评估安全性，而仅靠用户主动提供的情境信息并不足够。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估主要关注通用风险（如危险能力），但数百万用户将LLM用于个人高风险咨询（金融、健康），这些风险是情境依赖的而非通用的。虽然OECD等框架认识到需要评估个体风险，但用户福利安全评估仍然不成熟。

Method: 1. 评估GPT-5、Claude Sonnet 4和Gemini 2.5 Pro在金融和健康建议上的表现；2. 使用不同脆弱程度的用户档案；3. 比较有上下文背景和无上下文背景的评估者对相同LLM响应的安全评分；4. 重新评估包含用户报告会披露的情境信息的提示。

Result: 1. 评估者需要丰富的用户背景信息：无上下文背景的评估者对相同LLM响应的安全评分显著高于有上下文背景的评估者，高脆弱用户的安全评分从安全（5/7）降至有些不安全（3/7）；2. 仅靠用户主动提供的情境信息无法显著改善评估效果。

Conclusion: 有效的用户福利安全评估需要评估者针对多样化的用户档案评估响应，仅靠现实用户情境披露是不够的，特别是对弱势群体。该研究提供了情境感知评估的方法论，并证明评估个体福利需要与现有通用风险框架不同的方法。
...
TL;DR: ReMe框架通过多维度经验提炼、上下文自适应重用和基于效用的精炼，实现LLM智能体的动态记忆进化，显著提升性能并展示内存缩放效应。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体记忆框架主要采用"被动积累"范式，将记忆视为静态的只追加存档，无法实现静态存储与动态推理之间的有效衔接。

Method: 提出ReMe框架，包含三个核心机制：1) 多维度经验提炼，识别成功模式、分析失败触发因素并生成比较性见解；2) 上下文自适应重用，通过场景感知索引将历史见解适配到新情境；3) 基于效用的精炼，自主添加有效记忆并修剪过时记忆。

Result: 在BFCL-V3和AppWorld基准测试中达到最先进水平，观察到显著的内存缩放效应：配备ReMe的Qwen3-8B超越无记忆的Qwen3-14B，表明自我进化记忆为终身学习提供了计算高效的途径。

Conclusion: ReMe框架通过动态记忆进化机制有效解决了静态存储与动态推理之间的鸿沟，为LLM智能体的终身学习提供了计算高效的解决方案，并开源代码和数据集促进进一步研究。
...
### [182] [Enhancing Radiology Report Generation and Visual Grounding using Reinforcement Learning](https://arxiv.org/abs/2512.10691)
...
TL;DR: 该研究在胸部X光视觉语言模型上应用强化学习和显式推理，发现强化学习能提升报告生成和视觉定位性能，但显式推理没有额外帮助。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型主要依赖监督微调，缺乏对答案质量的评估。强化学习可以融入任务特定反馈，而显式推理在数学和编程任务中已证明有效，研究旨在探索这两种方法在胸部X光解释中的效果。

Method: 基于Qwen3-VL构建RadVLM模型，首先进行大规模监督微调，然后通过冷启动监督微调赋予基本推理能力。接着应用Group Relative Policy Optimization（GRPO）强化学习，使用临床相关的任务特定奖励进行报告生成和视觉定位优化。

Result: 研究发现：1）强监督微调对基础性能至关重要；2）强化学习在两个任务上都能提供额外提升；3）显式推理没有进一步改善结果。优化后的RadVLM模型在报告生成和视觉定位任务上达到最先进性能。

Conclusion: 临床对齐的强化学习是医学视觉语言模型监督微调的有力补充，能显著提升模型在胸部X光解释任务上的性能，但显式推理在当前设置下没有显示出额外优势。
...
TL;DR: SEAL-RAG提出"替换而非扩展"策略，通过搜索-提取-评估-循环机制动态替换检索文档中的干扰信息，解决多跳查询中的上下文稀释问题，在固定检索深度下显著提升答案正确性和证据精度。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在处理多跳查询时，当初始检索遗漏桥接事实时会失败。传统校正方法（如Self-RAG、CRAG、Adaptive-k）通常通过增加上下文或修剪列表来解决问题，但这会导致上下文稀释，即干扰信息挤占相关信息的空间。

Method: SEAL-RAG采用"替换而非扩展"策略，执行搜索→提取→评估→循环（SEAL）周期：1）进行实时的实体锚定提取构建间隙规范（缺失的实体/关系）；2）触发有针对性的微查询；3）使用实体优先排名主动用填补间隙的证据替换干扰信息。

Result: 在HotpotQA（k=3）上，SEAL比Self-RAG提升答案正确性3-13个百分点，证据精度提升12-18个百分点。在2WikiMultiHopQA（k=5）上，比Adaptive-k准确率提升8.0个百分点，证据精度保持96%（CRAG仅为22%）。所有提升在统计上显著（p<0.001）。
...
TL;DR: CA-GPT AI-OCT系统在OCT引导的PCI规划和评估中，显著优于通用ChatGPT-5和初级医师，提供标准化可靠的血管内成像解读方法。


<details>
  <summary>Details</summary>
Motivation: 血管内成像（特别是OCT）可改善PCI结果，但其解读依赖于操作者经验。通用AI虽有潜力但缺乏领域特异性可靠性，需要开发专门针对OCT引导PCI的AI系统。

Method: 单中心分析96例接受OCT引导PCI的患者，比较CA-GPT、ChatGPT-5和初级医师生成的程序决策与专家记录的一致性。使用10个预设指标评估术前规划和术后评估阶段。

Result: 术前规划：CA-GPT中位一致性评分（5[3.75-5]）显著高于ChatGPT-5（3[2-4]）和初级医师（4[3-4]）。CA-GPT在支架直径（90.3% vs 72.2%）和长度选择（80.6% vs 52.8%）上优于初级医师。术后评估：CA-GPT（5[4.75-5]）显著优于ChatGPT-5（4[4-5]）和初级医师（5[4-5]）。亚组分析显示CA-GPT在复杂场景中表现稳健。

Conclusion: 基于CA-GPT的AI-OCT系统在PCI规划和评估阶段均优于通用大语言模型和初级医师，为血管内成像解读提供了标准化可靠方法，有望增强操作者专业知识并优化OCT引导的PCI。
...
TL;DR: HAROOD是一个用于人类活动识别（HAR）在分布外（OOD）设置下的综合基准测试，定义了4种OOD场景，评估了16种方法，发现没有单一方法在所有场景中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的人类活动识别面临个体、设备、环境、时间等变化带来的分布偏移挑战，现有OOD算法只在特定场景中测试，缺乏全面的有效性评估。

Method: 提出HAROOD基准，定义4种OOD场景（跨人、跨位置、跨数据集、跨时间），构建包含6个数据集、16种比较方法（基于CNN和Transformer架构）的测试平台，采用两种模型选择协议。

Result: 通过大量实验发现：没有单一方法在所有场景中始终优于其他方法，这突显了该领域仍有很大改进空间；同时验证了OOD算法在HAR中的必要性。

Conclusion: HAROOD为OOD-based HAR研究提供了模块化、易扩展的代码库，有助于推动该领域发展；研究结果表明当前方法仍有局限性，需要进一步创新。
...
TL;DR: 提出Agile Deliberation框架，通过概念界定和迭代两个阶段，支持用户对模糊概念进行迭代式定义和细化，用于内容审核等需要主观视觉概念分类的应用。


<details>
  <summary>Details</summary>
Motivation: 现有的人机交互方法假设用户有清晰、稳定的概念理解，但现实中用户往往从模糊想法开始，需要通过"概念审议"逐步细化。内容审核专家通过结构化访谈揭示了这一实践需求。

Method: 将真实内容审核员的审议策略操作化为Agile Deliberation框架：1) 概念界定 - 将初始概念分解为结构化子概念层次；2) 概念迭代 - 展示语义边界案例供用户反思和反馈，迭代对齐图像分类器与用户意图。

Result: 通过18个1.5小时用户会话评估，Agile Deliberation比自动分解基线F1分数高7.5%，比手动审议高3%以上。参与者报告概念理解更清晰，认知努力更低。

Conclusion: Agile Deliberation有效支持主观、演化概念的定义，通过边界案例暴露帮助用户明确概念边界，在内容审核等需要主观视觉概念分类的应用中具有实用价值。
...
TL;DR: V-OCBF：从离线演示中学习神经控制屏障函数，实现无模型、无在线交互的安全控制器合成，相比基线方法显著减少安全违规。


<details>
  <summary>Details</summary>
Motivation: 现有安全离线强化学习方法通常只强制执行软预期成本约束，无法保证前向不变性；而控制屏障函数（CBFs）虽然提供严格安全保证，但依赖专家设计的屏障函数或完整的系统动力学知识。需要一种能从离线数据中学习安全屏障的方法。

Method: 提出价值引导离线控制屏障函数（V-OCBF）：1）从离线演示中学习神经CBF，无需动力学模型；2）推导递归有限差分屏障更新，实现无模型学习；3）采用期望分位数目标，避免在分布外动作上查询屏障；4）将学习到的屏障与二次规划（QP）结合合成实时安全控制。

Result: 在多个案例研究中，V-OCBF相比基线方法显著减少安全违规，同时保持强大的任务性能，展示了其在无在线交互或手工设计屏障的情况下合成安全关键控制器的可扩展性。

Conclusion: V-OCBF框架成功实现了从离线数据中学习神经控制屏障函数，为自主系统提供了无模型、无在线交互的安全控制器合成方法，在保证任务性能的同时显著提高了安全性。
...
TL;DR: LLMs可用于大型用户设施的提案筛选，提供比传统人工评审更可扩展、一致且经济高效的替代方案，在排名相关性上表现良好且成本降低两个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统人工提案评审存在提案间相关性弱、评审者偏见和不一致的问题。基于成对偏好的方法在逻辑上更优越，但二次工作量使其对人工评审不切实际。

Method: 利用LLMs进行提案排名，基于三个中子源束线的精心策划的提案和发表记录，采用成对偏好方法，并使用嵌入模型进行提案相似性定量评估。

Result: LLM排名与人工排名强相关（Spearman ρ≈0.2-0.8，去除10%异常值后≥0.5）。在识别高发表潜力提案方面，LLM表现不逊于人工评审，而成本降低两个数量级以上。

Conclusion: LLMs为提案筛选提供了可扩展、一致且经济高效的解决方案，不仅能有效排名，还能进行人类难以完成的定量相似性分析，为评审委员会提供关键信息。
...
TL;DR: 提出节点级剪枝框架用于电路发现，解决现有方法计算成本高和粒度粗的问题，通过多粒度可学习掩码和稀疏性惩罚实现单次微调中的全面压缩。
...
Method: 提出节点级剪枝框架，引入跨多个粒度（从整个块到单个神经元）的可学习掩码，在统一优化目标中使用粒度特定的稀疏性惩罚指导剪枝过程。
...
TL;DR: 该论文建立了部分可观察马尔可夫决策过程(POMDP)中的智能体与一输入过程函数之间的精确对应关系，揭示了AI与量子物理之间的深层联系。


<details>
  <summary>Details</summary>
Motivation: 探索AI中的决策智能体与量子物理中的高阶量子操作之间的数学对应关系，为理解智能体与环境交互提供新的理论框架。

Method: 通过将智能体的策略和记忆更新组合成过程函数w，使用链接积与POMDP环境交互，建立形式化对应。扩展到多智能体系统时，将观察独立的分散式POMDP识别为多输入过程函数的自然领域。

Result: 建立了POMDP智能体与一输入过程函数之间的精确数学对应，提供了双重解释：物理视角中过程函数作为环境，AI视角中过程函数编码智能体。该框架可自然扩展到多智能体系统。

Conclusion: 该研究揭示了AI决策理论与量子物理高阶操作之间的深刻联系，为理解智能体-环境交互提供了统一的理论框架，并开辟了跨学科研究的新方向。
...
<details>
  <summary>Details</summary>
Motivation: 异构图神经网络（HGNNs）在处理大规模图时面临挑战，因为结构冗余和高维节点特征导致可扩展性问题。现有的图压缩方法主要针对同构图设计，且依赖梯度匹配，带来巨大的计算、内存和优化开销。

Method: HGC-Herd采用免训练框架，集成轻量级特征传播来编码多跳关系上下文，并使用类内聚类机制为每个类别识别代表性节点，生成平衡且具有区分性的子集用于下游学习任务。

Result: 在ACM、DBLP和Freebase数据集上的实验表明，HGC-Herd在保持与全图训练相当或更优准确率的同时，显著降低了运行时间和内存消耗。

Conclusion: HGC-Herd为高效和可扩展的异构图表示学习提供了实用价值，能够在保持性能的同时大幅减少资源消耗。
...
TL;DR: BAMBO是一个自动化构建LLM帕累托集的贝叶斯自适应多目标块优化框架，通过混合最优块划分策略解决维度灾难问题，实现高效的能力-效率权衡探索。
...
Method: 提出BAMBO框架，引入混合最优块划分策略（作为1D聚类问题，使用动态规划平衡块内同质性和块间信息分布），在由qEHVI获取函数驱动的进化循环中自动化搜索。

Result: 实验表明BAMBO比基线方法发现更优越、更全面的帕累托前沿，能够根据不同的操作约束进行敏捷的模型选择。

Conclusion: BAMBO通过创新的块划分策略解决了现有方法的二分困境，实现了LLM能力-效率权衡的高效自动化探索，为实际部署提供了灵活的模型选择方案。
...
TL;DR: 提出了一种潜在动作世界模型，能够同时利用动作标注和无动作数据，显著减少对标注数据的需求，在DeepMind Control Suite上仅需十分之一的标注样本就能达到强大性能。


<details>
  <summary>Details</summary>
Motivation: 人类能够结合直接交互和无动作经验（如视频）学习，但标准世界模型通常依赖动作标注轨迹，当动作标签稀缺时效果受限。需要一种能够同时利用动作标注和无动作数据的方法。

Method: 引入潜在动作世界模型家族，学习共享的潜在动作表示。该潜在空间将观测到的控制信号与从被动观测推断的动作对齐，使单个动力学模型能够在大规模无标签轨迹上训练，同时仅需少量动作标注数据。使用潜在动作世界模型通过离线强化学习学习潜在动作策略。

Result: 在DeepMind Control Suite上，该方法在使用比纯动作标注基线少约一个数量级的动作标注样本情况下，实现了强大的性能表现。

Conclusion: 潜在动作使世界模型能够同时利用被动和交互数据进行训练，让世界模型学习更高效，弥合了离线强化学习（依赖动作标注数据）和无动作训练（很少用于后续强化学习）这两个传统分离的领域。
...
TL;DR: 提出基于Cluster-DAG先验知识的因果发现方法，通过Cluster-PC和Cluster-FCI算法在完全和部分观测场景下提升性能
...
Method: 引入Cluster-DAG作为先验知识框架，提出两种改进的基于约束的算法：Cluster-PC用于完全观测场景，Cluster-FCI用于部分观测场景

Result: 在模拟数据上的实证评估表明，Cluster-PC和Cluster-FCI算法在性能上优于没有先验知识的基线方法
...
### [195] [Robust Gradient Descent via Heavy-Ball Momentum with Predictive Extrapolation](https://arxiv.org/abs/2512.10033)
*Sarwan Ali*
...
<details>
  <summary>Details</summary>
Motivation: NAG等加速梯度方法在条件良好的问题上收敛快，但在病态或非凸问题上容易因动量积累而发散。需要一种既能保持加速效果又能在复杂地形上稳定的方法。

Method: HB-SGE结合重球动量和预测梯度外推，使用局部泰勒近似估计未来梯度方向，而不是简单累积历史梯度。这种方法提供自适应加速同时保持稳定性，内存开销仅为O(d)，超参数与标准动量方法相同。

Result: 在病态二次问题（条件数κ=50）上，HB-SGE在119次迭代收敛，而SGD和NAG都发散。在非凸Rosenbrock函数上，HB-SGE在2,718次迭代收敛，而经典动量方法在10步内发散。HB-SGE在强凸函数上有收敛保证。

Conclusion: 虽然NAG在条件良好的问题上更快，但HB-SGE提供了一个鲁棒的替代方案，在各种地形上都能保持稳定并加速收敛，特别适用于病态和非凸优化问题。
...
TL;DR: 该论文提出了四种新的参考权重设置策略来改进多参考偏好优化(MRPO)，但发现单参考DPO通常优于多参考方法，对多参考方法的实用性提出了质疑。


<details>
  <summary>Details</summary>
Motivation: 当前多参考偏好优化(MRPO)方法中的参考权重设置是临时且缺乏统计依据的，导致性能不可靠。需要更系统的方法来利用多个参考模型的集体优势。

Method: 提出了四种新的权重策略：两种离线方法利用验证集信号；一种在线方法使用滑动窗口估计器减少过拟合；另一种在线方法将参考权重设置视为K臂老虎机问题，使用Thompson Sampling。实验使用Qwen2.5-0.5B作为策略模型，7个不同家族的参考模型(0.5B-14B)。

Result: 所有四种新策略在UltraFeedback和SafeRLHF数据集上的偏好准确率都优于当前MRPO权重方法。但更令人深思的是，使用7个参考中任意6个的单参考DPO方法，持续优于所有测试的多参考方法。

Conclusion: 虽然新提出的权重策略改进了MRPO，但单参考DPO的持续优越性对多参考方法的实际吸引力提出了质疑，表明当前多参考方法可能不如预期有效。
...
### [197] [SEMDICE: Off-policy State Entropy Maximization via Stationary Distribution Correction Estimation](https://arxiv.org/abs/2512.10042)
*Jongmin Lee,Meiqi Sun,Pieter Abbeel*

Main category: cs.LG

TL;DR: SEMDICE是一种无监督强化学习预训练方法，通过状态熵最大化从任意离策略数据集中学习先验策略，在适应下游任务时表现优异。


<details>
  <summary>Details</summary>
Motivation: 在无监督强化学习预训练中，代理需要在不依赖任务特定奖励函数的情况下学习先验策略。状态熵最大化（SEM）旨在学习最大化状态平稳分布熵的策略，但现有方法在从任意离策略数据集中学习时存在挑战。
...
Conclusion: SEMDICE提供了一种有效的无监督预训练方法，能够从任意离策略数据集中学习状态熵最大化策略，为下游任务提供良好的先验知识，在适应效率方面表现优异。
...
### [198] [Local LLM Ensembles for Zero-shot Portuguese Named Entity Recognition](https://arxiv.org/abs/2512.10043)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言处理任务中表现出色，但在命名实体识别（尤其是低资源语言如葡萄牙语）中表现不佳。虽然开源权重模型支持本地部署，但没有单一模型在所有任务中占优，因此需要集成方法。现有的大语言模型集成主要关注文本生成或分类，NER领域研究不足。
...
Conclusion: 该工作通过有效结合多个小型大语言模型而不需要微调，推进了可扩展、低资源和零样本的命名实体识别。代码已开源，促进了可复现性和进一步研究。
...
### [199] [Detailed balance in large language model-driven agents](https://arxiv.org/abs/2512.10047)
*Zhuo-Yang Song,Qing-Hong Cao,Ming-xing Luo,Hua Xing Zhu*

Main category: cs.LG

TL;DR: 研究发现LLM驱动的智能体生成过程遵循最小作用量原理，揭示了LLM生成动态中的详细平衡现象，表明LLM不是学习规则集，而是隐式学习潜在函数


<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的智能体在解决复杂问题方面取得了经验性成功，但缺乏理解其宏观动态的理论框架。需要建立统一的理论基础，将AI智能体研究从工程实践提升为可预测、可量化的科学

Method: 基于最小作用量原理，通过实验测量LLM生成状态之间的转移概率，统计分析LLM生成过程中的详细平衡现象

Result: 统计发现了LLM生成转换中的详细平衡，表明LLM生成不是通过学习规则集和策略实现的，而是通过隐式学习一类潜在的势函数，这种势函数可能超越不同的LLM架构和提示模板

Conclusion: 这是首次在LLM生成动态中发现不依赖于具体模型细节的宏观物理定律，为建立复杂AI系统的宏观动力学理论迈出了第一步，旨在将AI智能体研究提升为基于有效测量的科学
...
### [200] [DB2-TransF: All You Need Is Learnable Daubechies Wavelets for Time Series Forecasting](https://arxiv.org/abs/2512.10051)
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型虽然擅长捕捉长期依赖关系，但其二次计算复杂度限制了在大规模高维时间序列预测中的可扩展性和适应性。需要一种既能保持预测精度又能降低计算成本的替代方案。

Method: 提出DB2-TransF架构，用可学习的Daubechies小波系数层替换传统的自注意力机制。这个小波模块能有效捕捉多尺度局部和全局模式，并增强多个时间序列之间的相关性建模。

Result: 在13个标准预测基准测试中，DB2-TransF取得了与传统Transformer相当或更优的预测精度，同时显著降低了内存使用量，实现了资源高效的时间序列预测。

Conclusion: DB2-TransF是一个可扩展且资源高效的时间序列预测框架，通过小波变换替代自注意力机制，在保持预测性能的同时解决了Transformer的计算瓶颈问题。
...
### [201] [Mitigating Exposure Bias in Risk-Aware Time Series Forecasting with Soft Tokens](https://arxiv.org/abs/2512.10056)
*Alireza Namazi,Amirreza Dolatpour Fathkouhi,Heman Shakeri*

Main category: cs.LG

TL;DR: SoTra方法通过传播连续概率分布来减轻曝光偏差，学习校准的、不确定性感知的轨迹，结合风险感知解码最小化临床危害，在血糖和血压预测中显著降低风险。
...
Method: 提出软令牌轨迹预测（SoTra），传播连续概率分布（"软令牌"）来减轻曝光偏差，学习校准的、不确定性感知的轨迹。然后使用风险感知解码模块最小化预期临床危害。

Result: 在血糖预测中，SoTra将基于区域的平均风险降低了18%；在血压预测中，将有效临床风险降低了约15%。

Conclusion: 这些改进支持SoTra在安全关键的预测控制中的应用，特别是在需要稳定多步预测和风险最小化的临床环境中。
...
### [202] [\textsc{Text2Graph}: Combining Lightweight LLMs and GNNs for Efficient Text Classification in Label-Scarce Scenarios](https://arxiv.org/abs/2512.10061)
*João Lucas Luz Lima Sarcinelli,Ricardo Marcondes Marcacini*

Main category: cs.LG

TL;DR: Text2Graph是一个开源Python包，通过结合LLM部分标注和GNN标签传播，实现高效、低能耗的零样本文本分类，显著降低计算成本和碳排放。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为零样本分类器虽然有效，但计算需求高、环境成本大，限制了在高性能计算环境中大规模标注的实用性。需要更可持续的工作流程。

Method: 开发Text2Graph开源框架，模块化实现文本到图的分类方法，结合LLM部分标注和GNN标签传播，支持灵活替换特征提取器、边构建方法和采样策略等组件。

Result: 在5个数据集（主题分类和情感分析）上基准测试显示，基于图的传播方法在性能上具有竞争力，同时显著降低能耗和碳排放，只需传统方法的一小部分成本。

Conclusion: Text2Graph框架提供了一种可持续的文本分类解决方案，在保持竞争力的同时大幅降低环境成本，为大规模标注任务提供了实用工具。
...
### [203] [MedXAI: A Retrieval-Augmented and Self-Verifying Framework for Knowledge-Guided Medical Image Analysis](https://arxiv.org/abs/2512.10098)
...
Result: 在10个多中心数据集上评估，在癫痫发作区定位和糖尿病视网膜病变分级任务中，实现跨域泛化提升3%，罕见类别F1分数提升10%，显著优于强深度学习基线。

Conclusion: MedXAI通过整合专家知识提供临床对齐的解释，同时实现优异的域内和跨域性能，特别适用于多模态医学AI中的罕见疾病诊断。
...
### [204] [CHyLL: Learning Continuous Neural Representations of Hybrid Systems](https://arxiv.org/abs/2512.10117)
...
Conclusion: CHyLL提供了一种无需轨迹分割或模式切换的连续混合系统学习方法，通过将状态空间重构为商流形，实现了连续流的准确学习和拓扑不变量的识别。
...
### [205] [Partitioning the Sample Space for a More Precise Shannon Entropy Estimation](https://arxiv.org/abs/2512.10133)
*Gabriel F. A. Bastos,Jugurta Montalvão*

Main category: cs.LG

TL;DR: 提出一种基于分解性、缺失质量和未见结果数估计的离散熵估计器，在小样本场景下优于经典方法，与先进方法相当


<details>
  <summary>Details</summary>
Motivation: 从小数据集中可靠估计香农熵是一个关键问题，特别是在样本数可能小于可能结果数的欠采样场景下

Method: 使用分解性属性，结合缺失质量和未见结果数的估计，来补偿由它们引起的负偏差

Result: 在欠采样场景下，提出的方法优于一些经典估计器，与一些成熟的最先进估计器性能相当

Conclusion: 该方法为小数据集下的熵估计提供了有效的解决方案，特别适用于样本数少于可能结果数的场景
...
### [206] [Sequence-to-Image Transformation for Sequence Classification Using Rips Complex Construction and Chaos Game Representation](https://arxiv.org/abs/2512.10141)
*Sarwan Ali,Taslim Murad,Imdadullah Khan*

Main category: cs.LG

TL;DR: 提出一种将分子序列转化为图像的新拓扑方法，结合混沌游戏表示和Rips复形构造，在抗癌肽数据集上优于现有方法。
...
Result: 在抗癌肽数据集上表现优异，在乳腺癌和肺癌数据集上分别达到86.8%和94.5%的准确率，优于基于向量、序列语言模型和现有图像方法。
...
### [207] [Murmur2Vec: A Hashing Based Solution For Embedding Generation Of COVID-19 Spike Sequences](https://arxiv.org/abs/2512.10147)
...
<details>
  <summary>Details</summary>
Motivation: COVID-19早期检测和表征对临床响应和公共卫生规划至关重要。现有方法存在局限性：基于系统发育树的方法计算密集，无法扩展到数百万序列数据集；现有嵌入方法要么依赖序列比对，要么预测性能不佳、运行成本高，难以进行大规模分析。

Method: 针对SARS-CoV-2刺突蛋白区域的最常见谱系，提出一种可扩展的嵌入方法，利用哈希技术生成紧凑的低维刺突序列表示，然后用这些嵌入训练多种机器学习模型进行监督谱系分类。

Result: 与多种基线和最先进的生物序列嵌入方法相比，所提嵌入方法在效率上有显著改进，达到86.4%的分类准确率，同时将嵌入生成时间减少高达99.81%。
...
### [208] [Rethinking Causal Discovery Through the Lens of Exchangeability](https://arxiv.org/abs/2512.10152)
*Tiago Brogueira,Mário Figueiredo*

Main category: cs.LG

TL;DR: 论文主张将传统i.i.d.因果发现框架重构为更一般的可交换性框架，提出概念和实证论证，并创建了仅基于可交换性假设的合成数据集，证明其比i.i.d.数据集更能反映真实世界统计结构。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现方法在i.i.d.和时间序列数据下采用不同的建模假设，作者认为i.i.d.设置应重构为更一般的可交换性框架，因为许多现有i.i.d.方法实际上基于可交换性假设，且真实世界"i.i.d."基准数据集主要由可交换而非i.i.d.样本组成。

Method: 1) 提出概念论证：将实验因果推断对可交换性的依赖扩展到因果发现；2) 提出实证论证：分析现有i.i.d.方法基于可交换性假设，分析Tübingen数据集的可交换性特征；3) 创建仅基于可交换性假设的合成数据集；4) 开发基于神经网络的因果发现算法，仅在该合成数据集上训练。

Result: 1) 可交换性合成数据集比所有i.i.d.合成数据集更能反映真实世界"i.i.d."数据集的统计结构；2) 仅在该合成数据集上训练的神经网络因果发现算法在真实世界基准测试中达到与其他最先进i.i.d.方法相似的性能。

Conclusion: 因果发现的i.i.d.设置应重构为更一般的可交换性框架，这不仅在概念上更合理，而且实证表明许多现有方法实际上依赖于可交换性假设。基于可交换性假设的合成数据集能更好地模拟真实世界数据，为因果发现方法开发提供了更合适的训练基础。
...
### [209] [CIEGAD: Cluster-Conditioned Interpolative and Extrapolative Framework for Geometry-Aware and Domain-Aligned Data Augmentation](https://arxiv.org/abs/2512.10178)
...
Method: 提出CIEGAD框架：1) 通过聚类条件构建领域轮廓；2) 使用集成类别频率和几何指标的分层频率-几何分配进行生成分配；3) 通过插值和外推合成共存精细控制生成方向；4) 通过几何约束过滤结合LLM-as-a-Judge机制进行质量控制。
...
### [210] [Assessing Neuromorphic Computing for Fingertip Force Decoding from Electromyography](https://arxiv.org/abs/2512.10179)
...
<details>
  <summary>Details</summary>
Motivation: HD-sEMG为非侵入式神经接口提供了可能，但将神经活动映射到用户运动意图仍具挑战性，需要探索更有效的解码方法

Method: 使用SNN和TCN两种架构解码从HD-sEMG分解得到的运动单元放电信息，数据来自单个参与者的10次试验，采用FastICA分解和端到端因果卷积训练

Result: TCN在测试集上达到4.44% MVC RMSE（Pearson r=0.974），SNN达到8.25% MVC（r=0.922），TCN更准确但SNN作为神经形态基准有改进空间

Conclusion: 虽然TCN解码精度更高，但SNN作为现实的神经形态架构基准，通过适度的架构和超参数优化可以缩小与TCN的性能差距
...
### [211] [MiniF2F-Dafny: LLM-Guided Mathematical Theorem Proving via Auto-Active Verification](https://arxiv.org/abs/2512.10187)
...
<details>
  <summary>Details</summary>
Motivation: miniF2F基准此前仅存在于交互式定理证明器中，需要探索其在自动定理证明器Dafny中的表现，并研究LLM如何辅助自动化证明

Method: 将miniF2F基准翻译到Dafny，评估自动化验证成功率，对无法自动验证的问题使用12个现成LLM提供证明提示，采用迭代错误修正策略

Result: Dafny自动化验证了测试集40.6%和验证集44.7%的问题（空证明），最佳LLM模型通过迭代错误修正达到55.7%的pass@4成功率

Conclusion: 展示了LLM提供高层指导与自动化处理底层细节的有效分工，为数学定理自动化证明提供了新思路
...
### [212] [Exact Recovery of Non-Random Missing Multidimensional Time Series via Temporal Isometric Delay-Embedding Transform](https://arxiv.org/abs/2512.10191)
...
<details>
  <summary>Details</summary>
Motivation: 非随机缺失数据是多维时间序列中普遍存在但处理不足的问题，严重威胁数据驱动分析和决策的可靠性。传统的低秩张量补全方法在处理非随机缺失时存在方法论和理论上的不足，而现有的Hankel结构张量补全方法又存在低秩性来源不明确、缺乏非随机缺失数据的精确恢复理论等问题。

Method: 提出时间等距延迟嵌入变换，构建Hankel张量，其低秩性由底层时间序列的平滑性和周期性自然诱导。在此基础上开发LRTC-TIDT模型，在张量奇异值分解框架下刻画低秩结构。在满足规定的非随机采样条件和温和的不相干假设下，模型能够实现精确恢复。

Result: 仿真实验验证了在各种非随机缺失模式下LRTC-TIDT模型能够实现精确恢复。在多个实际任务中（包括网络流量重建、城市交通估计和温度场预测），LRTC-TIDT始终优于现有的基于张量的方法。

Conclusion: LRTC-TIDT模型通过时间等距延迟嵌入变换有效解决了多维时间序列中非随机缺失数据的恢复问题，提供了明确的低秩性来源和精确恢复理论，在实际应用中表现出优越性能。
...
### [213] [Federated Domain Generalization with Latent Space Inversion](https://arxiv.org/abs/2512.10224)
*Ragja Palakkadavath,Hung Le,Thanh Nguyen-Tang,Svetha Venkatesh,Sunil Gupta*

Main category: cs.LG

TL;DR: 提出了一种新的联邦域泛化方法，通过潜在空间反转和重要权重聚合策略，在保护隐私的同时提升全局模型的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有的联邦域泛化方法在提升全局模型泛化能力的同时，往往通过共享客户端数据统计信息来损害隐私。需要一种既能保护隐私又能处理非独立同分布客户端的方法

Method: 1. 潜在空间反转：通过强制本地模型间的域不变性来增强隐私保护；2. 重要权重聚合策略：在聚合时优先考虑对本地模型预测有显著影响的参数，以保留本地适应性

Result: 实验表明该方法优于现有最先进方法，且通信开销更小

Conclusion: 提出的方法在联邦域泛化中有效平衡了隐私保护与模型性能，通过创新的本地训练和聚合策略解决了现有方法的局限性
...
### [214] [Adaptive Information Routing for Multimodal Time Series Forecasting](https://arxiv.org/abs/2512.10229)
...
TL;DR: 提出自适应信息路由（AIR）框架，利用文本信息动态指导时间序列模型，通过文本精炼管道将原始文本转换为适合多模态预测的形式，显著提升预测准确性。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测方法仅依赖历史数据，在实际场景中信息有限导致预测不准确。多模态方法虽然引入文本数据，但现有方法将文本数据与时间序列数据视为可互换的辅助特征，未能充分利用文本信息来动态指导时间序列模型。

Method: 提出自适应信息路由（AIR）框架，利用文本信息动态控制多元时间序列信息的组合方式和程度。开发文本精炼管道，使用大型语言模型将原始文本数据转换为适合多模态预测的形式。创建基准测试以促进基于该管道的多模态预测实验。

Result: 在原油价格和汇率等真实市场数据上的实验结果表明，AIR能够有效利用文本输入调节时间序列模型的行为，在各种时间序列预测任务中显著提高预测准确性。

Conclusion: AIR框架通过文本信息动态指导时间序列模型，解决了传统方法信息有限的问题，相比现有多模态方法能更有效地利用文本信息，显著提升预测性能。
...
### [215] [R^2-HGP: A Double-Regularized Gaussian Process for Heterogeneous Transfer Learning](https://arxiv.org/abs/2512.10258)
*Duo Wang,Xinming Wang,Chao Wang,Xiaowei Yue,Jianguo Wu*

Main category: cs.LG

TL;DR: 提出R²-HGP框架，通过可训练先验概率映射对齐异构输入空间，结合物理知识正则化和稀疏惩罚，解决多输出高斯过程在异构迁移学习中的挑战。


<details>
  <summary>Details</summary>
Motivation: 多输出高斯过程在迁移学习中面临三大挑战：1) 源域和目标域输入空间异构导致知识转移困难；2) 忽略先验知识和物理信息，导致映射不稳定；3) 不当的信息共享容易引发负迁移。传统方法无法统一解决这些问题。

Method: 提出双正则化异构高斯过程框架(R²-HGP)：1) 可训练先验概率映射模型对齐异构输入域；2) 将对齐结果作为隐变量构建多源迁移GP模型，集成到CVAE框架中；3) 加入物理知识作为正则项确保对齐符合物理规律；4) 对迁移系数施加稀疏惩罚，自适应选择信息量最大的源输出并抑制负迁移。

Result: 通过大量仿真和真实工程案例验证，R²-HGP在多种评估指标上均优于现有最先进基准方法，展现出显著的有效性和优越性。

Conclusion: R²-HGP框架成功解决了异构迁移学习中的关键挑战，通过输入对齐、物理知识整合和自适应源选择，实现了稳定高效的知识迁移，为多源异构迁移学习提供了有效的统一解决方案。
...
### [216] [A Kernel-based Resource-efficient Neural Surrogate for Multi-fidelity Prediction of Aerodynamic Field](https://arxiv.org/abs/2512.10287)
*Apurba Sarker,Reza T. Batley,Darshan Sarojini,Sourav Saha*

Main category: cs.LG

TL;DR: KHRONOS是一种基于变分原理、插值理论和张量分解的核神经网络代理模型，在计算资源受限条件下，通过融合稀疏高保真数据和低保真信息来预测气动场，相比传统密集神经网络需要更少的可训练参数且训练推理更快。


<details>
  <summary>Details</summary>
Motivation: 传统气动模拟计算成本高，代理模型在设计和优化中需要快速替代方案。现有方法在计算资源受限条件下表现不佳，需要一种能平衡精度和效率的多保真度气动场预测方法。

Method: 提出KHRONOS模型，基于变分原理、插值理论和张量分解构建核神经网络。使用AirfRANS数据集作为高保真基准，NeuralFoil生成低保真数据。比较KHRONOS与MLP、GNN、PINN三种架构，在不同高保真数据可用性（0%、10%、30%）和复杂几何参数化条件下，预测翼型表面压力系数分布。

Result: 虽然所有模型最终都能达到相当的预测精度，但KHRONOS在资源受限条件下表现优异：需要数量级更少的可训练参数，训练和推理速度远快于传统密集神经网络，在可比较的精度下实现更好的效率平衡。

Conclusion: KHRONOS及其类似架构在多保真度气动场预测中具有平衡精度和效率的潜力，特别适用于计算资源受限的应用场景。
...
TL;DR: 提出可解释的处方框架，通过最优策略树为主动脉瓣狭窄患者提供个体化的SAVR或TAVR治疗推荐，相比实际临床决策可显著降低5年死亡率。


<details>
  <summary>Details</summary>
Motivation: 目前临床实践中，对于低至中危严重主动脉瓣狭窄患者，选择外科主动脉瓣置换术（SAVR）还是经导管主动脉瓣置换术（TAVR）存在较大变异性，缺乏能够直接优化长期结果的个体化、可解释的治疗推荐模型。

Method: 开发可解释的处方框架，整合预后匹配、反事实结果建模和最优策略树（OPT）。通过预后匹配和样本加权模拟随机化，估计SAVR和TAVR两种治疗下的反事实死亡率，训练策略模型将患者划分为临床一致的亚组，推荐估计风险较低的治疗方案。

Result: 应用OPT推荐相比实际临床决策，在Hartford医院估计降低5年死亡率20.3%，在St. Vincent's医院降低13.8%，显示出良好的外部验证性能。学习到的决策边界与现实世界结果和临床观察一致。

Conclusion: 该可解释处方框架首次为TAVR vs SAVR提供透明、数据驱动的治疗推荐，在内部和外部队列中均能改善估计的长期结果，为结构性心脏病精准医疗提供更系统、循证的方法。
...
<details>
  <summary>Details</summary>
Motivation: 分布式机器学习系统需要强大的隐私保证、可验证的合规性以及在异构多云环境中的可扩展部署。现有系统在隐私保护、合规验证和跨平台治理方面存在不足。

Method: 提出一个云原生隐私保护架构，整合了：1) 联邦学习实现去中心化训练；2) 差分隐私提供形式化隐私保证；3) 零知识证明实现可验证合规；4) 强化学习驱动的自适应治理机制。该框架支持跨混合Kubernetes集群的安全模型训练和推理。

Result: 原型系统在多机构工作负载上的实验评估显示：1) 降低了成员推理攻击风险；2) 保持形式化隐私预算的一致性执行；3) 在差分隐私下保持稳定的模型性能；4) 在最小开销下保持效用；5) 提供连续的风险感知治理。
...
TL;DR: 提出几何框架分析智能体循环在语义嵌入空间中的动态行为，区分语言变换空间与几何测量空间，通过校准消除余弦相似度偏差，识别收缩与发散两种基本机制。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体系统通过递归反馈循环运行，但这些循环的几何行为（收敛、发散或复杂动态）缺乏系统理解，需要建立理论框架来分析智能体轨迹的动态特性。

Method: 引入几何框架分析智能体轨迹在语义嵌入空间中的行为，区分语言变换空间与几何测量空间，提出等渗校准消除余弦相似度的各向异性偏差，将迭代变换视为离散动力系统进行研究。

Result: 通过控制实验识别两种基本机制：收缩性重写循环收敛于稳定吸引子且分散度递减，探索性总结与否定循环产生无界发散且无聚类形成，两种机制展现出收缩与扩张的定性不同几何特征。

Conclusion: 提示设计直接控制智能体循环的动态机制，使人们能够系统控制迭代LLM变换中的收敛、发散和轨迹结构，为智能体系统的可预测性和可控性提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注有球防守动作（如拦截、抢断），但有效防守往往体现在阻止危险机会出现之前。这导致防守球员的真实影响无法被准确衡量，需要更全面的评估框架。

Method: 使用图注意力网络估计每个进攻选项的成功概率和期望价值，以及每个防守球员的责任分配。通过计算每次动作前后进攻队的期望控球价值变化，根据防守球员是否减少或增加对手的EPV来分配正负信用分。

Result: 在2023-24赛季训练、2024-25赛季荷甲数据评估中，DEFCON聚合的球员信用分与市场估值呈现强正相关。框架还展示了多种实际应用，包括比赛中防守贡献时间线、场地区域空间分析和攻防球员互动总结。
...
TL;DR: 提出了专门为基于Transformer的策略设计的广义策略梯度(GPG)定理，将标准策略梯度定理和GRPO作为其特例，并探索了在训练大语言模型中的实际应用。


<details>
  <summary>Details</summary>
Motivation: 针对Transformer架构的策略优化问题，现有策略梯度方法可能不是最优的。需要开发专门为Transformer设计的策略梯度理论框架，以更好地理解和优化基于Transformer的策略，特别是在大语言模型训练中。

Method: 提出了广义策略梯度(GPG)定理，专门为基于Transformer的策略设计。该框架将标准策略梯度定理和GRPO（可能指某种特定的策略优化方法）作为其特例，提供了一个统一的数学框架来分析Transformer策略的优化。

Result: 成功建立了专门针对Transformer策略的广义策略梯度理论框架，证明了现有方法（标准策略梯度定理和GRPO）都是该框架的特殊情况。这为理解Transformer策略优化提供了新的理论视角。

Conclusion: GPG定理为基于Transformer的策略优化提供了统一的理论框架，将现有方法纳入其中。该框架为大语言模型的高效策略优化提供了新的理论见解，有望改进Transformer策略的训练效果。
...
TL;DR: 该论文提出使用直线连分式来近似铁磁物质磁化强度的非线性函数，用于解释磁畴在增长和收缩过程中的非线性行为。


<details>
  <summary>Details</summary>
Motivation: 铁磁物质的磁化强度随外加磁场增强而增加，这是由于微观层面上磁畴逐渐与外场对齐。这种磁畴对齐形成了磁化非线性的物理基础，需要建立合适的数学模型来描述这种非线性行为。

Method: 将磁化强度的非线性函数近似为直线连分式的组合。直线连分式是一种代数表达式，可以通过非线性回归来估计参数。

Result: 得到的拟合结果可用于解释磁畴在增长和收缩过程中的非线性行为，为磁化过程的建模提供了数学工具。

Conclusion: 直线连分式方法能够有效近似铁磁物质的磁化非线性函数，为理解和分析磁畴动态变化提供了实用的数学框架。
...
TL;DR: 论文提出Eminence框架，通过理论分析稀疏决策边界，实现极低投毒率(<0.1%)的黑盒后门攻击，具有可解释性和理论保证。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络对后门攻击脆弱，但现有方法依赖启发式暴力搜索，缺乏理论分析，限制了攻击的可预测性和适应性。

Method: 通过理论分析稀疏决策边界，推导出模糊边界区域；提出Eminence框架，优化视觉隐蔽的通用触发器，战略性地利用脆弱决策边界。

Result: Eminence在<0.1%极低投毒率下保持>90%攻击成功率，清洁精度损失可忽略，在不同模型、数据集和场景中具有高迁移性。

Conclusion: 理论分析揭示了边界投毒与对抗性边界操纵的指数关系，Eminence为可解释、鲁棒的黑盒后门攻击提供了理论保证和实际有效性。
...
TL;DR: 该论文从算子理论角度统一描述神经网络训练动力学，推导出谱输运-耗散PDE，解释双下降现象，并连接NTK训练与特征学习两种极限情况。


<details>
  <summary>Details</summary>
Motivation: 现代深度网络在粗糙、有限正则性机制下运行，其Jacobian诱导的算子表现出重尾谱和强基漂移。需要从梯度下降出发，建立统一的算子理论框架来描述神经训练动力学。

Method: 从函数空间中的精确演化方程出发，应用Kato扰动理论获得耦合模式ODE系统。经过粗粒化后，这些动力学收敛到谱输运-耗散PDE。在弱耦合机制下，PDE允许自相似解，并推导出显式的标度律指数。

Result: 证明了神经训练保持函数正则性，迫使漂移取渐近幂律形式。推导出有效训练时间满足τ(t)=t^αL(t)。展示了NTK训练和特征学习是同一PDE的两个极限：v≡0恢复惰性动力学，而v≠0产生表示漂移。

Conclusion: 该研究提供了一个统一的谱框架，连接了算子几何、优化动力学和现代深度网络的通用标度行为，解释了双下降现象的几何结构，并为理解神经网络训练提供了理论基础。
...
TL;DR: 论文提出基于人类元认知的AI元认知评估框架，使用meta-d'度量模型置信度预测准确性的可靠性，并基于此进行动态模型选择，提高联合推理准确率。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然能表达预测置信度，但往往校准不佳，置信度不能反映真实能力。需要评估AI模型是否真正"知道它们知道什么"，借鉴人类认知科学来评估和利用AI的元认知能力。

Method: 引入心理学基础的元认知敏感性度量meta-d'，用于表征模型置信度预测自身准确性的可靠性。然后使用这个动态敏感性分数作为上下文，构建基于多臂赌博机的仲裁器，在测试时进行模型选择，学习在给定任务中信任哪个专家模型。
...
TL;DR: 提出了一种基于高斯过程回归的鲁棒混合物理-机器学习框架，用于准确预测正渗透水通量，并实现完整的预测不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 传统正渗透水通量预测面临挑战：机理模型受经验参数变化影响，纯数据驱动模型缺乏物理一致性和严格的不确定性量化。需要开发既准确又具有物理一致性且能量化不确定性的预测方法。

Method: 采用混合物理-ML框架，使用高斯过程回归训练物理模型预测值与实验水通量之间的残差误差。通过Delta方法将总预测方差分解为模型不确定性和输入不确定性，实现完整的不确定性量化。

Result: 仅使用120个数据点训练，在独立测试数据上取得了0.26%的平均绝对百分比误差和0.999的R²，实现了最先进的预测性能。

Conclusion: 该方法为高级正渗透工艺优化和数字孪生开发提供了一个真正鲁棒可靠的代理模型，特别适用于数据稀缺场景。
...
TL;DR: 提出T-SKM-Net框架，首次将SKM方法系统集成到神经网络约束满足中，实现高效处理混合约束问题，在电力系统优化等应用中实现25倍加速且零约束违反。


<details>
  <summary>Details</summary>
Motivation: 神经网络约束满足在安全关键应用中至关重要，但现有方法面临效率与适用性权衡。硬约束方法要么计算复杂度高，要么对约束结构有严格假设。SKM方法虽好，但其argmax操作不可微，难以应用于神经网络。
...
Conclusion: T-SKM-Net框架成功将SKM方法集成到神经网络约束满足中，克服了不可微操作挑战，支持标准反向传播。在电力系统优化等应用中实现高效、准确的约束满足，为安全关键应用提供可靠解决方案。
...
TL;DR: 提出UACER方法，通过多样化评论家集成和时间衰减不确定性机制，解决对抗强化学习中训练不稳定和收敛困难问题。


<details>
  <summary>Details</summary>
Motivation: 对抗强化学习在自动驾驶和机器人控制等序列决策领域有重要应用，但可训练的对手会导致学习动态的非平稳性，加剧训练不稳定和收敛困难，特别是在高维复杂环境中。

Method: 1) 多样化评论家集成：使用K个并行的多样化评论家网络稳定Q值估计，替代传统的单一评论家架构；2) 时间衰减不确定性机制：基于方差推导的Q值聚合策略，显式结合认知不确定性来动态调节探索-利用权衡，同时稳定训练过程。

Result: 在多个MuJoCo控制问题上进行综合实验，验证了UACER在整体性能、稳定性和效率方面优于最先进方法。

Conclusion: UACER通过评论家集成和不确定性感知机制，有效解决了对抗强化学习中的训练不稳定问题，提高了策略的鲁棒性和训练效率。
...
TL;DR: 本文提出自适应回放缓冲区（ARB），通过动态优先级采样解决离线到在线强化学习中的稳定性与性能权衡问题，无需复杂学习过程。
...
Method: 引入自适应回放缓冲区（ARB），基于轻量级指标"on-policyness"动态优先化数据采样。该方法无需复杂学习过程，易于实现，可无缝集成到现有O2O RL算法中。通过评估收集的轨迹与当前策略行为的对齐程度，为轨迹中的每个转移分配相应的采样权重。

Result: 在D4RL基准测试上的广泛实验表明，ARB能持续缓解早期性能下降，并显著提升各种O2O RL算法的最终性能。

Conclusion: 自适应、行为感知的回放缓冲区设计对于离线到在线强化学习至关重要，ARB通过简单的学习无关方法有效平衡了离线数据稳定性和在线经验相关性。
...
TL;DR: 提出DDE框架，通过师生蒸馏压缩VAE模型以减小OOD推理器尺寸，同时保持解缠结特性，适用于资源受限设备部署


<details>
  <summary>Details</summary>
Motivation: 现有基于解缠结VAE的OOD推理器模型较大，难以部署在资源受限设备上，需要在压缩模型的同时保持解缠结特性

Method: 提出解缠结蒸馏编码器(DDE)框架，将师生蒸馏形式化为带约束优化问题，通过解缠结约束保持解缠结特性，并基于Rademacher复杂度建立理论保证

Result: 在NVIDIA设备上部署压缩模型进行实证评估，证明DDE能有效减小模型尺寸同时保持解缠结性能

Conclusion: DDE框架成功解决了资源受限设备上部署解缠结OOD推理器的问题，为模型压缩与解缠结保持提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练扩散模型的后验采样或MAP估计方法存在建模近似和计算效率低的问题，需要更有效的方法来解决逆问题

Method: 提出变分模式寻求损失(VML)，通过最小化扩散后验与测量后验之间的KL散度来引导生成样本趋向MAP估计，特别针对线性逆问题有解析解

Result: VML-MAP算法在多个数据集上的多样化图像恢复任务中，在性能和计算时间上都优于现有方法

Conclusion: VML-MAP提供了一种有效且计算高效的解决方案，能够利用预训练扩散模型解决各种逆问题，无需特定任务训练或微调
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM的推理过程是顺序的：先思考再回答，这导致实时交互性差。而现实应用如语音助手需要模型能够实时响应和适应新信息，这与顺序交互不兼容。人类可以异步地听、想、说，论文旨在让LLM也能实现类似的异步能力

Method: 利用旋转嵌入（rotary embeddings）的特性，使原本设计用于顺序交互的LLM能够同时进行思考、聆听和生成输出。该方法不需要额外训练

Result: 在数学推理、常识推理和安全推理任务上评估，方法能够实时生成准确的思考增强答案，将第一个非思考token的生成时间从几分钟减少到≤5秒，整体实时延迟降低了6-11倍

Conclusion: 通过利用旋转嵌入的特性，成功实现了LLM的异步推理能力，显著提升了实时交互性能，使LLM能够像人类一样同时处理输入、思考和生成输出
...
<details>
  <summary>Details</summary>
Motivation: KV缓存是长上下文大语言模型的主要内存瓶颈，但通常被当作不透明的数值张量处理。需要一种方法既能减少内存占用，又能保持KV缓存的可解释性，同时维持注意力计算的准确性。

Method: 提出STA-Attention框架，使用Top-K稀疏自编码器（SAEs）分解KV缓存为语义原子。Top-K方法避免了标准L1正则化SAEs的收缩偏差，保持了注意力计算所需的精确点积几何。基于发现的键值不对称性（Key稀疏路由、Value密集内容），采用双预算策略选择性保留最有信息的语义成分。

Result: 在Yi-6B、Mistral-7B、Qwen2.5-32B等模型上的实验表明，语义重建能保持与原始模型相当的困惑度和零样本性能，有效弥合了机制可解释性与忠实注意力建模之间的差距。

Conclusion: STA-Attention框架成功地将KV缓存从数值张量转化为可解释的语义表示，通过Top-K SAEs和双预算策略解决了KV缓存的内存瓶颈问题，同时保持了模型性能，为长上下文LLMs的KV缓存提供了新的可解释性视角。
...
TL;DR: 本文提出Derf函数作为归一化层的替代方案，在多个领域超越LayerNorm、RMSNorm和DyT，成为无归一化Transformer架构的实用选择。


<details>
  <summary>Details</summary>
Motivation: 虽然归一化层长期以来被视为深度学习架构不可或缺的组成部分，但最近提出的Dynamic Tanh (DyT)表明存在替代方案。本研究旨在寻找超越DyT的函数设计，探索点函数的内在特性如何影响训练和性能。

Method: 首先研究点函数的内在特性对训练和性能的影响，然后进行大规模搜索寻找更有效的函数设计。最终提出Derf(x) = erf(αx + s)函数，其中erf(x)是重新缩放的高斯累积分布函数。

Result: Derf在视觉（图像识别和生成）、语音表示和DNA序列建模等多个领域都优于LayerNorm、RMSNorm和DyT。性能提升主要来自改进的泛化能力而非更强的拟合能力。

Conclusion: Derf的简单性和更强的性能使其成为无归一化Transformer架构的实用选择，为深度学习架构提供了归一化层的有力替代方案。
...
TL;DR: LaT-IB是一种抗标签噪声的信息瓶颈方法，通过引入"最小充分干净"准则和噪声感知潜在解缠，在标签噪声场景下实现鲁棒表示学习。


<details>
  <summary>Details</summary>
Motivation: 传统信息瓶颈方法严重依赖准确标签，在现实场景中普遍存在的标签噪声下容易性能下降和过拟合，需要一种抗标签噪声的表示学习方法。

Method: 提出LaT-IB方法：1)引入"最小充分干净"准则作为互信息正则化器；2)采用噪声感知潜在解缠，将潜在表示分解为干净标签空间和噪声空间对齐的分量；3)设计三阶段训练框架：预热、知识注入和鲁棒训练。

Result: 大量实验证明LaT-IB在标签噪声下实现了优越的鲁棒性和效率，显著增强了在真实标签噪声场景下的鲁棒性和适用性。
...
TL;DR: THeGAU：一种模型无关的异构图神经网络框架，通过类型感知图自编码器和引导图增强来解决类型信息丢失和结构噪声问题，提升节点分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有的异构图神经网络（HGNNs）在处理异质信息网络时存在类型信息丢失和结构噪声问题，这限制了模型的表示保真度和泛化能力。

Method: 提出THeGAU框架，包含两个核心组件：1）类型感知图自编码器，通过重构符合模式的边来保留节点类型语义；2）解码器驱动的增强机制，选择性优化噪声结构。

Result: 在IMDB、ACM和DBLP三个基准HIN数据集上的实验表明，THeGAU在多个骨干网络上均优于现有HGNN方法，达到了最先进的性能。

Conclusion: THeGAU通过联合设计类型感知重建和引导增强，有效解决了HGNN的类型信息丢失和结构噪声问题，提高了鲁棒性、准确性和效率，同时显著降低了计算开销。
...
TL;DR: 该论文在约束强化学习领域提出多个理论框架和算法，涵盖平均成本CMDPs、有限时域CMDPs、人类偏好学习以及大语言模型对齐，提供理论保证和实际应用工具。


<details>
  <summary>Details</summary>
Motivation: 当前约束强化学习在控制、偏好学习和语言模型对齐方面存在理论空白和实际挑战，需要开发具有理论保证且可扩展的算法来处理安全约束、人类偏好和模型对齐问题。

Method: 1) ACPO算法：结合敏感性分析和信任域更新处理平均成本CMDPs；2) e-COP算法：基于时域策略差异引理处理有限时域CMDPs；3) warmPref-PS：在线性赌博机中集成异构评分者离线偏好数据；4) PSPL算法：从轨迹比较中联合采样奖励模型和转移动态；5) MOPO算法：多目标约束优化方法用于大语言模型对齐。

Result: ACPO在平均成本CMDPs中达到最先进性能；e-COP在安全关键环境中提供可证明性能；warmPref-PS显著减少遗憾并提高数据收集效率；PSPL提供贝叶斯简单遗憾保证；MOPO可扩展到数十亿参数语言模型并在多种对齐设置中保持鲁棒性。

Conclusion: 该论文统一了平均成本、时域和偏好驱动的约束强化学习范式，为安全和对齐的决策制定提供了理论进展和实用工具，推动了约束RL在控制、偏好学习和语言模型对齐领域的应用。
...
TL;DR: 贝叶斯神经网络量化框架：提出三种量化策略（VPQ、SPQ、JQ），在4位精度下实现8倍内存压缩，保持不确定性估计能力


<details>
  <summary>Details</summary>
Motivation: 贝叶斯神经网络（BNNs）能提供原则性的不确定性量化，但相比确定性网络有显著的计算和内存开销。虽然量化技术已成功减少标准深度学习模型的资源需求，但在概率模型中的应用仍基本未被探索。

Method: 提出一个系统性的多级量化框架，基于随机变分推断的BNNs，区分三种量化策略：变分参数量化（VPQ）、采样参数量化（SPQ）和联合量化（JQ）。使用方差参数的对数量化和专门的激活函数来保持分布结构。

Result: 在Dirty-MNIST上的综合实验表明，BNNs可以量化到4位精度，同时保持分类准确性和不确定性解耦能力。在4位时，联合量化相比浮点实现实现高达8倍的内存减少，认知和偶然不确定性估计仅有最小退化。
...
<details>
  <summary>Details</summary>
Motivation: 应对移民模式突然变化和传统数据局限性的挑战，满足《欧盟移民与庇护公约》的预测需求，支持欧盟成员国战略决策和团结机制

Method: 混合方法整合机器学习技术与移民专家定性见解，创新性地引入人工评估协变量，在五个关键移民路线上进行一年期预测

Result: 方法经过已知数据测试验证，证明其在移民政策背景下的适用性和可靠性，能够提供政策相关的预测

Conclusion: 该研究通过数据驱动建模与专家判断的结合，开发了一种适用于欧盟移民治理的新型操作工具，符合学术建议并支持政策制定
...
TL;DR: 本文研究了注意力机制在超长序列下的收敛行为，提出了token-sample复杂度概念，分析了注意力映射在不同条件下的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型上下文窗口不断扩展，需要理解注意力机制在极端序列长度下的行为特征。现有研究缺乏对注意力在无限token极限下收敛行为的系统分析。

Method: 引入token-sample复杂度概念，从两个层面分析收敛性：1) 注意力映射的点一致收敛；2) 变换后token分布的矩收敛。针对紧支撑分布和次高斯分布，推导有限n下的收敛界。

Result: 对于紧支撑分布，注意力映射在半径R的球上以C(R)/√n速率一致收敛，C(R)随R指数增长。对于变换分布的矩收敛，速率为C'(R)/n^β，β<1/2，C'(R)与分布支撑大小呈多项式关系。在softmax趋近hardmax时，获得对数收敛速率。

Conclusion: 注意力机制在长序列下具有可量化的收敛行为，不同收敛度量对应不同的收敛速率。这些理论结果在合成高斯数据和真实BERT模型上得到验证，为理解超长上下文注意力提供了理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 异常检测需要可解释性，现有反事实解释方法大多未针对LOF等经典异常检测算法设计，LOF作为最流行的无监督异常检测方法缺乏解释性。

Method: 提出DCFO方法，将数据空间划分为LOF行为平滑的区域，实现高效的基于梯度的优化，为LOF异常点生成反事实解释。

Result: 在50个OpenML数据集上的实验验证表明，DCFO在生成反事实的接近度和有效性方面持续优于基准竞争对手。

Conclusion: DCFO成功解决了LOF异常检测算法的可解释性问题，为异常检测提供了有效的反事实解释方法，填补了现有研究的空白。
...
<details>
  <summary>Details</summary>
Motivation: 组合泛化能力使模型能够超越有限经验理解新概念组合，但现有方法的数据结构和原理尚不明确。作者认为组合泛化需要将高层概念分解为可跨相似上下文重组的低层概念，类似于人类通过类比理解新场景的方式。

Method: 引入基于因果模块化和最小变化原则的形式化框架，提出分层数据生成过程来编码不同层次概念及其交互机制。理论证明该方法支持复杂概念关系的组合泛化，并证明潜在分层结构可从文本-图像对等观测数据中可识别恢复。

Result: 理论框架支持超越简单加性效应的复杂概念关系组合泛化，潜在分层结构具有可识别性。应用理论见解在基准数据集上取得了显著改进。

Conclusion: 通过因果模块化和最小变化原则形式化组合泛化过程，分层数据生成过程能够编码概念层次结构并支持复杂交互，该框架不仅理论完备且在实践中有效，为理解组合泛化提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 垂直联邦学习(VFL)为移动健康诊断等边缘AI场景提供了隐私保护范式，但标准VFL系统由于简单的特征融合方法存在性能限制

Method: 提出HybridVFL框架，采用客户端特征解耦和服务器端跨模态Transformer进行上下文感知融合

Result: 在多模态HAM10000皮肤病变数据集上的系统评估表明，HybridVFL显著优于标准联邦基线

Conclusion: 验证了先进融合机制在鲁棒、隐私保护系统中的关键作用
...
### [244] [UniExtreme: A Universal Foundation Model for Extreme Weather Forecasting](https://arxiv.org/abs/2508.01426)
...
<details>
  <summary>Details</summary>
Motivation: 现有的天气预测基础模型要么关注一般天气条件，要么专门针对特定类型的极端事件，忽略了现实世界中多样化极端事件的大气模式。极端事件具有两个关键特征：与正常天气状态的光谱差异，以及多样化极端事件的分层驱动因素和地理混合。

Method: 提出了UniExtreme模型，包含两个核心模块：1) 自适应频率调制(AFM)模块，通过可学习的Beta分布滤波器和多粒度光谱聚合捕获正常与极端天气之间的区域光谱差异；2) 事件先验增强(EPA)模块，通过双级记忆融合网络整合区域特定的极端事件先验，解决极端事件的分层多样性和复合极端模式。

Result: 大量实验表明，UniExtreme在极端天气和一般天气预测方面都优于最先进的基线方法，展现出在各种极端场景下的卓越适应性。

Conclusion: UniExtreme通过整合光谱差异建模和事件先验增强，成功解决了多样化极端天气预测的挑战，为通用极端天气预测基础模型的发展提供了有效方案。
...
### [245] [Beyond the Black Box: Identifiable Interpretation and Control in Generative Models via Causal Minimality](https://arxiv.org/abs/2512.10720)
*Lingjing Kong,Shaoan Xie,Guangyi Chen,Yuewen Sun,Xiangchen Song,Eric P. Xing,Kun Zhang*

Main category: cs.LG

TL;DR: 基于因果最小化原则，为深度生成模型建立可解释的理论基础，实现潜在表示的可控因果解释


<details>
  <summary>Details</summary>
Motivation: 深度生成模型虽然革命性，但通常作为黑箱运行，阻碍人类理解、控制和对齐。现有方法如稀疏自编码器缺乏理论保证，存在主观解释风险

Method: 提出因果最小化原则（最简单的因果解释），建立层次选择模型的理论框架，在扩散视觉和自回归语言模型中应用稀疏性或压缩约束

Result: 在理论推导的最小化条件下，学习到的表示等价于数据生成过程的真实潜在变量，能够提取模型固有的层次概念图，实现细粒度模型控制

Conclusion: 因果最小化原则为可解释生成模型提供理论基础，实现透明可靠系统的因果解释和可控性
...
### [246] [Generalized Spherical Neural Operators: Green's Function Formulation](https://arxiv.org/abs/2512.10723)
...
<details>
  <summary>Details</summary>
Motivation: 神经算子在求解参数偏微分方程方面表现出色，但扩展到球面域时面临挑战：需要保持内在几何结构同时避免破坏旋转一致性的扭曲。现有球面算子依赖旋转等变性但缺乏处理现实世界复杂性的灵活性。

Method: 提出基于可设计球面格林函数及其谐波展开的通用算子设计框架，引入绝对和相对位置依赖的格林函数，平衡等变性和不变性。开发GSNO算子及新颖的谱学习方法，构建GSHNet层次架构，结合多尺度谱建模与球面上-下采样。

Result: 在扩散MRI、浅水动力学和全球天气预报等任务上，GSNO和GSHNet始终优于最先进方法，能够适应各向异性、约束丰富的系统同时保持谱效率。

Conclusion: GSNO为球面算子学习提供了原则性通用框架，将严格理论与现实世界复杂性相连接，为球面域上的参数偏微分方程求解提供了理论基础和实用工具。
...
### [247] [LGAN: An Efficient High-Order Graph Neural Network via the Line Graph Aggregation](https://arxiv.org/abs/2512.10735)
*Lin Du,Lu Bai,Jincheng Li,Lixin Cui,Hangyuan Du,Lichi Zhang,Yuting Chen,Zhao Li*

Main category: cs.LG

TL;DR: 提出LGAN（线图聚合网络），通过构建每个节点的诱导子图的线图进行高阶聚合，在保持较低计算复杂度的同时获得比2-WL更强的表达能力，并提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有GNN主要基于消息传递策略，表达能力受限于1-WL测试。虽然已有k-WL-based GNNs可以克服这一限制，但计算成本随k快速增加，且无法保留节点/边级别的细粒度语义，导致可解释性差。

Method: 提出LGAN（线图聚合网络），为每个节点构建其诱导子图的线图，在线图上进行高阶聚合。线图将原始图中的边转换为节点，从而捕获高阶结构信息。

Result: 理论证明LGAN在单射聚合假设下比2-WL具有更强的表达能力，且时间复杂度更低。在基准测试中，LGAN优于最先进的k-WL-based GNNs，同时提供更好的可解释性。
...
### [248] [Template-Free Retrosynthesis with Graph-Prior Augmented Transformers](https://arxiv.org/abs/2512.10770)
*Youjun Zhao*

Main category: cs.LG

TL;DR: 提出一种基于Transformer的无模板逆合成反应预测框架，通过注入分子图信息到注意力机制，结合SMILES序列和结构线索，并采用配对数据增强策略提升性能


<details>
  <summary>Details</summary>
Motivation: 现有逆合成反应预测模型在准确性和鲁棒性方面仍不足以满足实际部署需求，需要摆脱对手工反应模板或额外化学规则引擎的依赖

Method: 采用无模板的Transformer框架，将分子图信息注入注意力机制以同时利用SMILES序列和结构线索，并应用配对数据增强策略增加训练多样性和规模

Result: 在USPTO-50K基准测试中，该方法在无模板方法中达到最先进性能，显著优于基础的Transformer基线模型

Conclusion: 提出的无模板Transformer框架通过结合序列和结构信息以及数据增强，有效提升了逆合成反应预测的准确性和鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器(SAE)在LVLM中存在两个问题：1）多数神经元解释性或可操控性低，难以实际应用；2）无监督学习导致用户所需概念缺失。需要提升SAE的实际效用。

Method: 提出概念瓶颈稀疏自编码器(CB-SAE)：1）剪枝低效用神经元；2）在潜在空间添加轻量级概念瓶颈层，与用户定义的概念集对齐；3）引入两个计算成本低的解释性和可操控性度量指标。

Result: CB-SAE在LVLM和图像生成任务中，解释性提升32.1%，可操控性提升14.5%。代码和模型权重将开源。

Conclusion: CB-SAE通过概念瓶颈层有效解决了传统SAE的局限性，显著提升了特征的解释性和可操控性，为LVLM的机制解释、概念发现和模型操控提供了更实用的框架。
...
TL;DR: 论文发现二进制编码使神经网络能够外推周期性函数，提出NB2E编码方法让普通MLP成功外推各种周期信号


<details>
  <summary>Details</summary>
Motivation: 传统神经网络在处理周期性函数外推时存在困难，需要寻找能够使神经网络理解并外推周期信号结构的方法
...
Conclusion: 二进制编码为神经网络提供了理解周期信号结构的能力，使MLP能够进行有效的外推，这为处理周期性数据提供了新方法
...
TL;DR: 提出一个强化学习框架，无需人类游戏数据即可实现可控且多样化的玩家行为，通过行为向量空间和距离奖励机制实现风格控制。
...
Method: 将玩家行为定义为N维连续空间，从包含真实人类风格子集的区域均匀采样目标行为向量。训练时每个智能体接收当前和目标行为向量作为输入，奖励基于两者距离的归一化减少，使策略学习动作如何影响行为统计量。

Result: 在自定义多玩家Unity游戏中实验表明，该方法比仅以获胜为目标的基线产生显著更大的行为多样性，并能可靠匹配不同目标的行为向量。单个PPO多智能体策略无需重新训练即可重现新或未见过的游戏风格。

Conclusion: 该方法为自动化游戏测试、游戏平衡、人类行为模拟和在线游戏中替换断线玩家提供了可扩展的解决方案，实现了无需人类数据的行为可控性和多样性。
...
TL;DR: 提出基于序贯蒙特卡洛的贝叶斯符号回归框架，通过近似符号表达式的后验分布来增强噪声环境下的鲁棒性，实现不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 符号回归能从数据直接发现控制方程，但对噪声敏感限制了其广泛应用。需要开发更鲁棒的方法来处理噪声数据。

Method: 采用序贯蒙特卡洛框架，结合概率选择、自适应退火和归一化边缘似然，高效探索符号表达式搜索空间，得到简洁表达式。

Result: 相比传统遗传编程基线，该方法在噪声基准数据集上表现更好，减少过拟合趋势，能发现更准确、可解释的方程。

Conclusion: 该方法为科学发现和工程设计中更鲁棒的符号回归铺平了道路，通过贝叶斯框架提供不确定性量化能力。
...
TL;DR: 提出SCSI方法，通过迭代更新噪声数据与干净数据之间的传输映射，仅使用噪声数据集和黑盒访问噪声通道，构建干净数据的生成模型。


<details>
  <summary>Details</summary>
Motivation: 在科学和工程领域，通常只能获得被噪声、病态通道污染的测量数据，需要解决分布层面的逆问题来构建原始数据的生成模型。

Method: 基于随机插值方法，迭代更新噪声数据与干净数据样本之间的传输映射，仅需访问噪声数据集和黑盒访问噪声通道，最终收敛到自一致的传输映射。

Result: SCSI方法在计算效率上优于变分替代方案，能处理任意非线性前向模型，具有理论保证，在自然图像处理和科学重建的逆问题上表现出优越性能。

Conclusion: SCSI提供了一种高效、灵活且理论可靠的方法，仅通过噪声数据和黑盒访问噪声通道就能构建干净数据的生成模型，适用于各种逆问题场景。
...
TL;DR: 离散扩散语言模型（DLMs）的缩放行为研究：发现均匀扩散在数据受限场景下比掩码扩散更高效，并成功扩展到100亿参数模型


<details>
  <summary>Details</summary>
Motivation: 现代LLM预训练消耗大量计算资源和训练数据，不同模型的缩放行为成为关键区分因素。离散扩散语言模型（DLMs）作为自回归语言模型（ALMs）的替代方案被提出，但其缩放行为尚未充分探索，先前研究表明DLMs需要更多数据和计算才能达到ALMs的性能。

Method: 通过平滑插值掩码扩散和均匀扩散来研究不同噪声类型下DLMs的缩放行为，同时密切关注批量大小和学习率等关键超参数。实验分析了不同噪声类型在计算受限和数据受限场景下的表现差异。

Result: DLMs的缩放行为强烈依赖于噪声类型，与ALMs有显著差异。所有噪声类型在计算受限缩放中收敛到相似的损失值，但均匀扩散在参数效率方面表现更好，需要更多参数但更少数据，在数据受限场景下更具优势。研究将均匀扩散模型扩展到100亿参数，训练了10^22 FLOPs，验证了预测的缩放行为。

Conclusion: 均匀扩散DLMs在数据受限设置中是一种有前景的候选方案，其缩放行为与掩码扩散不同，需要更多参数但更少数据。成功扩展到100亿参数模型进一步证实了这一发现，使其成为目前已知最大的公开均匀扩散模型。
...
TL;DR: 线性模型（特别是DLinear）在仅使用历史温度数据的长期预测中表现优于Transformer系列模型
...
Method: 使用线性模型（Linear、NLinear、DLinear）和Transformer系列模型（Transformer、Informer、Autoformer）进行长期外生温度预测，在标准化的训练、验证和测试集上进行评估

Result: 线性基线模型（特别是DLinear）在所有数据集上都一致优于更复杂的Transformer架构，获得了最佳的整体预测精度

Conclusion: 精心设计的线性模型在仅使用历史数据的挑战性时间序列预测场景中仍然是强大的基线方法，不应忽视简单模型的有效性
...
TL;DR: 提出GTL方法，无需微调即可将预训练的离散扩散模型迁移到新领域，并通过高效采样器解决大词汇量长序列的计算问题


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型在语言等离散领域表现优异，但需要大量训练数据，迁移到新领域成本高且风险大。现有迁移方法需要微调整个模型，计算开销大且不实用

Method: 基于连续扩散的ratio-based迁移学习，提出GTL方法，通过引导机制在不修改预训练去噪器的情况下从目标分布采样。同时提出高效采样器，专注于规划器选择的位置和候选词，降低计算成本

Result: GTL在序列数据（包括合成马尔可夫链和语言建模）上进行了评估，展示了其有效性。高效采样器使大规模语言建模变得实用

Conclusion: GTL提供了一种无需微调预训练离散扩散模型的迁移学习方法，通过引导机制和高效采样器解决了计算效率问题，为大规模离散域迁移学习提供了实用解决方案
...
TL;DR: 论文提出一种利用反事实解释改进模型重构的方法，通过将原始数据与反事实样本结合，使用Wasserstein重心近似类别原型，从而提升代理模型质量并缓解决策边界偏移问题。


<details>
  <summary>Details</summary>
Motivation: 反事实解释不仅能提供可操作的见解，还可用于模型重构（训练代理模型复制目标模型行为）。然而，反事实样本通常靠近决策边界，作为训练样本时代表性不足，可能导致决策边界偏移问题。特别是在标记数据有限的情况下，如何有效利用反事实样本来改进模型重构是一个重要挑战。

Method: 提出一种将原始数据样本与反事实样本相结合的方法，使用Wasserstein重心来近似每个类别的原型，从而保留类别的分布结构。这种方法将反事实样本视为信息丰富但代表性较弱的样本，通过原型近似来平衡原始数据和反事实样本的贡献，避免将反事实样本简单视为普通训练实例。

Result: 在多个数据集上的实验结果表明，该方法显著提高了代理模型与目标模型之间的保真度，验证了其有效性。相比将反事实样本简单作为训练实例的朴素方法，该方法能更好地缓解决策边界偏移问题。

Conclusion: 通过将反事实样本与原始数据结合，并使用Wasserstein重心近似类别原型，可以有效改进模型重构的质量。这种方法特别适用于标记数据有限的场景，为利用反事实解释进行模型重构提供了更稳健的解决方案。
...
TL;DR: 提出一个物理信息学习框架，利用夜间均匀化时段从常规运行数据中推断槽式光热电站的回路质量流量比和接收器热损失系数


<details>
  <summary>Details</summary>
Motivation: 槽式光热电站存在大规模液压网络，由于光学性能、热损失和压降的空间异质性，需要保持均匀的出口温度。但回路质量流量和接收器热损失参数无法直接观测，导致难以诊断液压不平衡或接收器退化问题

Method: 利用夜间均匀化时段（无辐照时热油循环）分离液压和热损失效应，将可微分的共轭传热模型离散化并嵌入端到端学习管道，使用Andasol 3电站的历史数据进行优化

Result: 模型准确重建回路温度（RMSE <2°C），得到物理意义明确的回路不平衡和接收器热损失估计。与无人机红外热成像（QScan）对比显示强相关性，正确识别所有高损失接收器区域

Conclusion: 嘈杂的真实世界光热电站运行数据包含足够信息来恢复潜在物理参数，前提是结合适当的建模和可微分优化方法
...
TL;DR: 提出一种针对大语言模型的高效剪枝方法，通过解耦行约束和1-swap优化，显著降低剪枝误差并提升模型性能


<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法在大语言模型上效果不佳：完全重训练成本过高，全局幅度剪枝对Transformer架构不优，现有方法依赖近似或启发式算法，无法最优解决层级掩码选择问题

Method: 提出1-swap算法：1) 通过强制每行相同稀疏度解耦行约束；2) 利用校准数据的Gram矩阵高效计算最优1-swap（交换一个保留权重和一个剪枝权重）；3) 从任意剪枝掩码开始，在GPU上高效运行，基本无超参数

Result: 相比Wanda方法减少高达60%的每层剪枝误差，在GPT架构上持续改善困惑度和零样本准确率

Conclusion: 通过解耦行约束和高效1-swap优化，使大语言模型规模的掩码选择问题变得可行，提供了一种简单、高效且基本无超参数的剪枝解决方案
...
<details>
  <summary>Details</summary>
Motivation: 水下环境自主导航面临GPS缺失、能见度差和水下障碍物等挑战，需要开发鲁棒的导航方法来解决这些问题。

Method: 使用基于PPO算法的深度强化学习，观测空间结合目标导向导航信息、虚拟占用网格和操作区域边界的射线投射，并与DWA确定性运动规划器进行对比。

Result: PPO策略在高度杂乱环境中始终优于DWA，具有更好的局部适应性和更少的碰撞，同时验证了从仿真到真实世界的可迁移性。

Conclusion: 深度强化学习对于水下机器人自主导航具有实际应用价值，PPO方法在复杂环境中表现优异，仿真到实物的迁移能力得到证实。
...
TL;DR: 提出一种新算法，通过解耦评论家与策略的动作块长度，让策略操作更短的动作块，同时保留多步价值传播优势，解决传统块状评论家方法中策略需要开环输出长动作块的问题。


<details>
  <summary>Details</summary>
Motivation: 时间差分方法通过自举机制高效学习状态和动作价值，但容易产生自举偏差。现有块状评论家方法估计短动作序列的价值，加速价值传播，但从中提取策略面临挑战：策略需要开环输出整个动作块，这在需要策略反应性的环境中可能次优，且随着块长度增长建模困难。

Method: 提出解耦评论家与策略的动作块长度，让策略操作更短的动作块。通过优化策略对抗蒸馏评论家来评估部分动作块的价值，该蒸馏评论家通过从原始块状评论家乐观自举构建，近似部分动作块扩展为完整块时可实现的最大价值。

Result: 在具有挑战性的长时域离线目标条件任务上评估，该方法可靠地优于先前方法。

Conclusion: 该方法保留了多步价值传播的优势，同时避免了开环次优性和学习长动作块策略的困难，通过解耦评论家与策略的块长度实现了更好的性能。
...
TL;DR: 提出使用改进的Frank-Wolfe方法（投影自由方法）构建白盒对抗攻击，相比基于投影或几何直觉的标准方法更高效有效


<details>
  <summary>Details</summary>
Motivation: 神经网络对抗攻击构建对其部署至关重要，需要快速高效的方法来评估对抗鲁棒性。由于对抗攻击构建本质上是特定优化问题，需要从数值优化角度寻找高效有效的解决方案。

Method: 采用改进的Frank-Wolfe方法（投影自由方法）构建白盒对抗攻击。该方法避免了传统方法中的投影操作，在MNIST和CIFAR-10数据集上对多类逻辑回归模型、卷积神经网络和Vision Transformer进行了理论分析和数值实验。

Result: 通过理论分析和数值实验评估了改进的Frank-Wolfe方法，并与基于投影操作或几何直觉的标准方法进行了比较。实验表明该方法在构建对抗攻击方面具有优势。

Conclusion: 从数值优化角度提出的改进Frank-Wolfe方法为构建高效有效的对抗攻击提供了有前景的解决方案，有助于更好地评估神经网络的对抗鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中数据通常以离散数据集形式组织，不同数据集在相关性、质量和效用上存在差异。现有方法大多选择单个样本且将所有数据视为同等相关，忽略了数据集及其来源之间的差异，因此需要专门的数据集选择方法。

Method: 提出DaSH（Dataset Selection via Hierarchies）方法，在数据集和组（如集合、机构）两个层次上建模效用，通过层次化建模实现从有限观测中的高效泛化。

Result: 在两个公开基准测试（Digit-Five和DomainNet）上，DaSH比最先进的数据选择基线方法准确率提升高达26.2%，同时需要显著更少的探索步骤。消融实验表明DaSH在低资源设置和相关数据集缺乏的情况下具有鲁棒性。
...
TL;DR: BiFlow提出了一种双向归一化流框架，通过近似逆映射而非精确解析逆，解决了因果解码瓶颈，在ImageNet上实现了质量提升和百倍加速采样。


<details>
  <summary>Details</summary>
Motivation: 传统归一化流（NFs）需要精确的可逆变换，而TARFlow等基于Transformer的自回归流方法虽然复兴了NF，但因果解码成为主要瓶颈。需要更灵活的架构来克服这一限制。

Method: 提出双向归一化流（BiFlow）框架，放弃精确解析逆的要求，学习一个近似噪声到数据的逆映射模型。这使得可以使用更灵活的损失函数和架构设计。

Result: 在ImageNet上的实验表明，相比因果解码方法，BiFlow提升了生成质量，同时加速采样达两个数量级（百倍）。在NF方法中达到SOTA，在单评估（1-NFE）方法中具有竞争力。

Conclusion: BiFlow通过近似逆映射而非精确解析逆，解决了NFs的因果解码瓶颈，为这一经典范式带来了新的灵活性，有望进一步推动NFs的发展。
...
TL;DR: CC-GRMAS是一个利用卫星观测和环境信号提高滑坡预测准确性的框架，通过预测、规划和执行三个智能体协同工作，实现实时态势感知、响应规划和干预


<details>
  <summary>Details</summary>
Motivation: 滑坡是日益严重的气候诱发灾害，特别是在亚洲高山区，尽管卫星和时间数据集不断增加，但及时检测和灾害响应仍然不发达且分散，需要更准确、及时的滑坡预测和响应系统

Method: 提出CC-GRMAS框架，利用一系列卫星观测和环境信号，构建三个相互关联的智能体（预测、规划、执行），通过多智能体协调和本地环境因素整合，实现实时态势感知、响应规划和干预

Result: 该方法提供了一个可扩展的主动解决方案，能够增强滑坡预测准确性，支持实时灾害响应，提高气候适应型灾害准备能力

Conclusion: CC-GRMAS框架通过多智能体协作和本地环境因素整合，为脆弱山区地形提供了一个可扩展的、主动的气候适应型灾害准备解决方案
...
### [266] [Neural Hamiltonian Deformation Fields for Dynamic Scene Rendering](https://arxiv.org/abs/2512.10424)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态视图合成方法虽然能实现高质量渲染，但往往产生物理上不合理的运动。传统使用MLP预测变形场的方法存在固有偏差，导致不自然的动态效果。

Method: 1. 引入哈密顿力学框架建模高斯变形场，利用其相空间结构与高斯原语演化的一致性；2. 使用哈密顿神经网络隐式学习控制变形的物理规律；3. 提出玻尔兹曼平衡分解，基于时空能量状态自适应分离静态和动态高斯；4. 采用二阶辛积分和局部刚性正则化处理实际耗散；5. 扩展自适应流式传输，通过尺度感知mipmapping和渐进优化实现。

Result: NeHaD实现了物理合理的动态场景渲染，在渲染质量和效率之间取得良好平衡。实验证明该方法能产生更自然的动态效果。

Conclusion: 这是首次利用哈密顿力学进行神经高斯变形的研究，实现了物理真实的动态场景渲染并具备流式传输能力，为动态视图合成提供了新的物理约束框架。
...
### [267] [DeMapGS: Simultaneous Mesh Deformation and Surface Attribute Mapping via Gaussian Splatting](https://arxiv.org/abs/2512.10572)
*Shuyi Zhou,Shengze Zhong,Kenshi Takayama,Takafumi Taketomi,Takeshi Oishi*

Main category: cs.GR

TL;DR: DeMapGS：一种联合优化可变形表面和表面附着2D高斯斑点的结构化高斯斑点框架，通过将斑点锚定到可变形模板网格上，解决拓扑不一致问题并增强编辑灵活性。


<details>
  <summary>Details</summary>
Motivation: 传统高斯斑点方法将点独立处理，存在拓扑不一致和编辑灵活性不足的问题。需要一种能够结合可变形表面和高斯斑点优势的统一表示方法。

Method: 1. 将2D高斯斑点锚定到可变形模板网格上；2. 引入梯度扩散策略在表面传播监督；3. 采用交替2D/3D渲染方案处理凹区域；4. 支持提取高保真漫反射、法线和位移贴图。

Result: DeMapGS实现了最先进的网格重建质量，使重建的网格能够继承高斯斑点的照片级真实感渲染质量，并支持高斯斑点的编辑和跨对象操作等下游应用。

Conclusion: DeMapGS通过结构化高斯斑点框架成功解决了传统方法的局限性，实现了高质量网格重建和灵活编辑，为高斯斑点的实际应用提供了新途径。
