...
TL;DR: 提出一种基于高斯溅射的3D人脸重建方法，可从少量未标定图像生成可用于标准图形管线的三角网格表面和神经纹理


<details>
  <summary>Details</summary>
Motivation: 解决从少量未标定人脸图像重建可用于标准图形管线的3D模型的问题，同时保持高视觉保真度

Method: 使用高斯溅射构建3D神经表示，利用分割标注对齐人脸语义区域，通过软约束将高斯点绑定到三角网格表面，并将高斯溅射转换为纹理空间的视点依赖神经纹理

Result: 仅需11张图像即可重建中性姿态人脸，生成可用于标准图形管线的三角网格和高分辨率反照率纹理，支持文本驱动的资产创建

Conclusion: 该方法实现了从少量图像到标准图形管线可用资产的端到端转换，为高质量3D人脸资产创建提供了灵活高效的解决方案
...
TL;DR: MAGINet是一个用于人脸图像本征分解的多尺度注意力引导网络，能从单张RGB人像预测512×512的光照归一化漫反射反照率图，并通过RefinementNet和Pix2PixHD-based translator生成完整的六通道渲染层，实现高质量的重光照和材质编辑。


<details>
  <summary>Details</summary>
Motivation: 在无约束光照下准确分解人脸图像的本征属性是实现照片级真实感重光照、高保真数字替身和增强现实效果的前提。现有方法在反照率边界清晰度和光照不变性方面存在不足。

Method: 采用多尺度注意力引导网络架构：1) 编码器使用分层残差编码；2) 瓶颈层使用空间和通道注意力；3) 解码器采用自适应多尺度特征融合；4) 通过轻量级RefinementNet将512×512反照率上采样到1024×1024并细化；5) 基于细化反照率，使用Pix2PixHD-based translator预测另外五个物理渲染层。

Result: 在FFHQ-UV-Intrinsics数据集上训练，结合masked-MSE、VGG、边缘和patch-LPIPS损失函数，在漫反射反照率估计方面达到最先进性能，完整渲染堆栈的保真度相比先前方法显著提升。

Conclusion: MAGINet能够生成高质量的六通道本征分解，包括漫反射反照率、环境光遮蔽、表面法线、镜面反射、半透明度和原始漫反射颜色，为真实人脸的高质量重光照和材质编辑提供了有效解决方案。
...
TL;DR: 提出结合规则变形与GAN图像转换的两步生成式数据增强框架，用于生成逼真的戴口罩人脸样本，解决数据稀缺和分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和分布偏移是戴口罩人脸检测与识别的主要挑战。现有方法如纯合成变换或GAN生成方法（如IAMGAN）存在局限性，需要更有效的生成逼真戴口罩人脸样本的方法。

Method: 提出两步生成式数据增强框架：1) 基于规则的口罩变形；2) 使用GAN进行非配对图像到图像转换。引入非口罩保护损失和随机噪声注入来稳定训练并增强样本多样性。

Result: 相比单纯基于规则的变形方法，该框架在质量上获得一致改进，并能补充现有GAN方法如IAMGAN。实验验证了所提组件的有效性。

Conclusion: 该框架为面向数据增强的人脸识别任务提供了有效解决方案，并指出了未来改进方向，特别是在以数据为中心的增强策略方面。
...
TL;DR: FrameDiffuser是一个自回归神经渲染框架，通过结合G-buffer数据和自身先前输出来生成时间一致的光照真实帧，解决了现有方法在交互应用中时间不一致或计算成本过高的问题。


<details>
  <summary>Details</summary>
Motivation: 现有神经渲染方法存在关键限制：单图像模型缺乏时间一致性，视频模型计算成本过高且需要完整序列，都不适用于依赖用户输入的交互应用。

Method: 提出FrameDiffuser框架，采用自回归方式，结合G-buffer数据和先前生成帧进行时间引导。使用双条件架构：ControlNet提供结构指导，ControlLoRA确保时间一致性。采用三阶段训练策略实现稳定自回归生成，并针对特定环境进行专门化训练。

Result: FrameDiffuser能够在数百到数千帧中保持稳定的时间一致性生成，相比通用方法，环境特定训练实现了更优越的光照真实质量，包括准确的光照、阴影和反射。

Conclusion: FrameDiffuser为交互应用提供了一种高效、时间一致的神经渲染解决方案，通过自回归框架和特定环境训练，在保持计算效率的同时实现了高质量的光照真实图像生成。
...
TL;DR: JARVIS是一个基于JEPA框架的自监督视觉增强方法，用于提升多模态大语言模型在视觉推理任务上的性能，通过利用冻结的视觉基础模型作为编码器，训练LLM早期层作为预测器来学习图像的结构和语义规律。
...
Method: 提出JARVIS框架，将I-JEPA学习范式集成到标准视觉-语言对齐流程中。使用冻结的视觉基础模型作为上下文和目标编码器，训练LLM的早期层作为预测器，学习图像的结构和语义规律，而不完全依赖语言监督。

Result: 在标准MLLM基准测试上的广泛实验表明，JARVIS在不同LLM家族上都能持续提升视觉中心基准的性能，同时不会降低多模态推理能力。

Conclusion: JARVIS通过自监督视觉增强有效解决了MLLMs在视觉理解上的局限性，提供了一种不依赖语言监督的学习图像规律的方法，显著提升了视觉推理性能。
...
Method: 联合学习显式Voronoi图与隐式符号距离场（SDF），通过光线追踪优化场景，使用Eikonal正则化，使Voronoi单元面对齐到零水平集

Result: SDFoam显著改善了网格重建精度（Chamfer距离），在保持可比的光度质量（PSNR、SSIM）和训练速度的同时，减少了漂浮物并改善了拓扑结构

Conclusion: 提出的隐式-显式混合方法SDFoam在保持渲染效率的同时，显著提升了表面重建质量，为高质量网格生成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有评估基准过于语言中心化或依赖模拟环境，缺乏对真实世界知识密集型推理的评估，需要填补这一空白。

Method: 提出稀疏接地视觉导航任务，创建CityNav基准包含四个全球城市，要求智能体仅凭视觉输入和内部多模态推理进行导航。提出VoP方法，通过从MLLM中提取显式认知地图（关键地标和方向）来增强导航能力。

Result: 当前最先进的MLLM和标准推理技术（如思维链、反思）在这一挑战性设置中表现显著不佳。VoP方法大幅提升了导航成功率。

Conclusion: 稀疏接地视觉导航是评估MLLM在真实世界环境中决策能力的重要任务，VoP方法通过显式认知地图有效增强了导航性能，为开发更强大的具身智能体提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 受人类四维感知推理能力启发，希望让视觉语言模型具备类似的结构化、持久性记忆，能够编码语义、空间布局和时间动态信息，实现上下文依赖的推理。

Method: R4框架持续构建4D知识数据库，将物体级语义描述锚定在度量空间和时间中，形成持久世界模型。推理时将自然语言查询分解为语义、空间和时间键来检索相关观察，集成到VLM推理中。

Result: 在具身问答和导航基准测试中，R4相比基线方法在时空信息检索和推理方面有显著提升，无需训练即可实现情景式和协作式推理。

Conclusion: R4为动态环境中的具身4D推理提供了新范式，通过结构化4D记忆增强视觉语言模型的时空感知能力，实现了更接近人类的多模态记忆和推理。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能力不断增强，但其感知能力缺乏系统评估。大多数模型家族仅扩展语言组件而重用相同的视觉编码器，这引发了一个关键问题：模型进步是真正基于视觉基础，还是主要依赖互联网规模的文本世界知识。现有评估方法过于强调最终任务准确率，忽视了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了Perceptual Observatory框架，通过多个垂直维度系统评估MLLMs：1) 简单视觉任务（如人脸匹配和视觉文本理解）；2) 局部到全局理解（如图像匹配、网格指向游戏和属性定位），测试视觉基础能力。每个维度都使用真实数据集（人脸和文字），并通过像素级增强和基于扩散的风格化幻觉进行系统性扰动。

Result: 该框架超越了排行榜准确率，能够深入分析MLLMs在扰动下如何保持感知基础和关系结构，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: Perceptual Observatory为多模态大语言模型的感知能力评估提供了一个系统性框架，能够更全面地评估模型的视觉基础能力，而不仅仅是任务准确率，有助于理解模型进步的本质和局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单人类场景的自我中心视角行为预测，但许多机器人应用需要从第三人称视角理解多人类行为。缺乏合适的多人类行为预测数据集也是一个挑战。

Method: 提出CAMP-VLM框架，结合视觉输入的上下文特征和场景图的空间感知。使用光真实感模拟器生成合成人类行为数据进行微调，采用监督微调(SFT)和直接偏好优化(DPO)方法。
...
TL;DR: 该研究探索了基于视觉语言模型（VLMs）的少样本多光谱目标检测，通过适配Grounding DINO和YOLO-World处理多光谱输入，在FLIR和M3FD数据集上显著优于专用多光谱模型。


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全关键应用中至关重要，但标注数据稀缺限制了深度检测器的训练。文本类别信息可以作为有价值的语义监督源，而视觉语言模型在计算机视觉中的成功启发了其在少样本多光谱检测中的应用。

Method: 适配两种代表性的VLM检测器（Grounding DINO和YOLO-World）处理多光谱输入，提出有效机制整合文本、视觉和热模态信息，利用大规模VLMs学习的语义先验知识。

Result: 在FLIR和M3FD多光谱图像基准测试中，VLM检测器在少样本场景下显著优于使用可比数据训练的专业多光谱模型，在全监督设置下也达到竞争性或更优的结果。

Conclusion: 大规模VLMs学习的语义先验可以有效迁移到未见的光谱模态，为数据高效的多光谱感知提供了强大途径，展示了VLM在少样本多光谱目标检测中的潜力。
...
Method: 在AgML集合的27个农业分类数据集上（涵盖162个类别）对开源和闭源VLM进行基准测试，比较零样本性能与监督基线（YOLO11），分析不同提示策略（多项选择vs开放回答）和评估方法的影响。

Result: 零样本VLM在所有任务中都显著低于监督基线；最佳VLM（Gemini-3 Pro）在多项选择提示下平均准确率约62%，开放回答提示下通常低于25%；LLM语义判断可提升开放回答准确率；开源模型中Qwen-VL-72B表现最佳；植物和杂草分类比病虫害识别更容易。

Conclusion: 当前现成的VLM尚不适合作为独立的农业诊断系统，但可作为辅助组件，需配合约束界面、明确标签本体和领域感知评估策略使用。
...
TL;DR: 提出基于视觉感知和自适应决策的机器人割草框架，通过深度特征空间分析识别并保护视觉多样性植被斑块，选择性停用割草刀片以主动提升花园生物多样性。


<details>
  <summary>Details</summary>
Motivation: 传统草坪割草导致生态单一化，被动恢复方法效果有限。需要主动干预系统将生态价值低下的单一草坪转变为促进城市生物多样性的活跃生物栖息地。
...
Conclusion: 该系统能够将生态价值低下的单一草坪转变为促进城市生物多样性的活跃生物栖息地，广泛采用此类系统将显著提升城市生物多样性。
...
Conclusion: 该方法提供了一个可扩展的框架，能够利用大规模视频数据进行机器人学习，解决了动作标注缺乏的问题，实现了更好的跨模态信息共享。
...
TL;DR: 提出一个用于端到端自动驾驶的闭环评估平台，通过对抗性交互生成真实世界中的安全关键极端案例，以评估模型在复杂场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 安全关键极端案例在现实世界中难以收集，但对评估端到端自动驾驶至关重要。现有对抗性评估方法主要针对简化模拟环境中的模型，而针对真实世界端到端自动驾驶的对抗性评估研究较少。

Method: 构建闭环评估平台，包含基于流匹配的真实世界图像生成器和对抗性交通策略。生成器根据交通环境信息高效稳定地生成真实世界图像，对抗性策略设计挑战性交互并创建自动驾驶系统难以处理的极端案例。

Result: 平台能够高效生成逼真的驾驶图像。通过评估UniAD和VAD等端到端模型，基于对抗性策略的平台能够评估测试模型在极端案例中的性能下降，有效检测模型的潜在问题。
...
<details>
  <summary>Details</summary>
Motivation: 单壳低角分辨率dMRI（LAR-FOD）估计纤维取向分布精度有限，而多壳高角分辨率dMRI（HAR-FOD）需要长时间扫描限制应用。扩散模型在基于LAR-FOD估计HAR-FOD方面有潜力，但由于FOD中大量球谐系数，高效生成HAR-FOD具有挑战性。
...
Conclusion: 提出的3D多通道补丁扩散模型能够有效从低角分辨率扩散MRI预测高角分辨率纤维取向分布，通过创新的模块设计解决了球谐系数多、计算复杂的挑战。
...
TL;DR: 提出Auto-Vocabulary 3D目标检测(AV3DOD)，无需用户指定类别，自动生成检测对象的类别名称，并在ScanNetV2和SUNRGB-D数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D目标检测方法虽然在训练时能检测未见类别，但仍需用户在训练和推理时指定类别。本文旨在研究无需用户输入的自动词汇3D目标检测，让系统自动生成检测对象的类别名称。

Method: 提出AV3DOD框架：1)引入语义评分(SS)评估生成类别名称的质量；2)利用2D视觉语言模型通过图像描述、伪3D框生成和特征空间语义扩展来生成丰富的语义候选。

Result: 在ScanNetV2和SUNRGB-D数据集上取得SOTA性能：在ScanNetV2上比SOTA方法CoDA提升3.48整体mAP，语义评分相对提升24.5%。

Conclusion: AV3DOD首次实现了无需用户输入的自动词汇3D目标检测，通过创新的语义评分和基于2D VLM的语义生成框架，在定位精度和语义质量上都达到了最先进水平。
...
TL;DR: LAPX是一个基于Hourglass网络的自注意力轻量级姿态估计模型，在保持竞争力的精度下仅需230万参数，适合边缘设备实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有SOTA姿态估计方法参数量大、计算成本高；而轻量级变体要么不适合边缘设备高效部署，要么因设计过于简化导致精度受限。

Method: 基于LAP工作，提出LAPX：采用Hourglass网络结构，引入自注意力模块捕捉全局上下文信息，改进阶段设计并优化轻量级注意力模块。

Result: 在MPII和COCO两个基准数据集上取得有竞争力的结果，仅需230万参数，并展现出实时性能，证实了其边缘设备适用性。

Conclusion: LAPX在保持精度的同时实现了轻量化和实时性能，有效解决了姿态估计模型在边缘设备部署中的效率与精度平衡问题。
...
TL;DR: 提出一种基于准直器闪烁星点模式的事件相机标定方法，用于长距离高精度测量场景


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高动态范围和高时间分辨率等优势，但在长距离测量场景中的几何标定（包括内参和外参）仍然是一个重大挑战

Method: 使用带有闪烁星点模式的准直器，首先基于准直器的球面运动模型线性求解相机参数，然后通过非线性优化进行高精度参数细化

Result: 在不同条件下的综合真实世界实验中，该方法在准确性和可靠性方面始终优于现有的事件相机标定方法

Conclusion: 提出的基于准直器闪烁星点模式的事件相机标定方法能够有效解决长距离高精度测量需求，显著提升了标定性能
...
TL;DR: TurboDiffusion是一个视频生成加速框架，通过注意力加速、步数蒸馏和量化等技术，实现100-200倍的端到端扩散生成加速，同时保持视频质量。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在视频生成方面计算成本高昂，需要大量时间和计算资源。为了在实际应用中实现实时或近实时的视频生成，需要大幅提升生成速度。

Method: 采用三种主要技术：1) 注意力加速：使用低比特SageAttention和可训练稀疏线性注意力；2) 步数蒸馏：采用rCM进行高效步数蒸馏；3) W8A8量化：将模型参数和激活量化为8位以加速线性层和压缩模型。此外还包含其他工程优化。

Result: 在多个模型（Wan2.2-I2V-14B-720P等）上实验，在单张RTX 5090 GPU上实现100-200倍的视频生成加速，同时保持可比的视频质量。

Conclusion: TurboDiffusion框架成功实现了视频生成的显著加速，为实际应用提供了可行的解决方案，相关代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 相机标定是摄影测量和3D视觉应用中的关键步骤，需要更可靠、可控且灵活的标定方法。现有方法通常需要复杂的相机运动或多图像采集，限制了标定的灵活性和效率。

Method: 设计了准直仪系统，利用其独特的光学几何特性提出角度不变性约束，证明标定目标与相机之间的相对运动符合球面运动模型。基于此约束开发了多图像闭式线性求解器和双图像最小求解器，并提出了无需相机运动的单准直仪图像标定算法。

Result: 在合成和真实世界实验中验证了方法的可行性，证明使用准直仪系统进行标定是有效的，且性能优于现有基线方法。

Conclusion: 提出的准直仪相机标定方法通过角度不变性约束简化了标定过程，实现了无需相机运动的灵活快速标定，为摄影测量和3D视觉应用提供了新颖的标定解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 智能畜牧业需要自动检测牛群行为互动（如发情检测），但现有研究主要针对人类互动检测，牛群互动检测面临缺乏全面行为数据集和互动事件罕见的挑战

Method: 首先从大规模牛群动作数据集学习动作潜空间，然后通过对比学习在预训练潜空间上微调嵌入罕见互动，构建动作与互动的统一潜空间，并集成视频和GPS输入开发实用系统

Result: 在商业规模牧场上的实验表明，相比基线方法，该方法实现了准确的互动检测

Conclusion: 提出的CattleAct方法通过数据高效的方式解决了牛群互动检测的挑战，为智能畜牧业提供了实用的自动互动检测系统
...
<details>
  <summary>Details</summary>
Motivation: 双能CT重建面临通道不平衡和界面附近大伪影等挑战，需要开发能够同时利用物理知识和数据驱动优势的混合重建方法。

Method: 采用两阶段混合框架：1) 知识驱动阶段使用OPMT技术从投影数据快速重建基材料图像的中间解；2) 数据驱动阶段引入ResDynUNet++网络，基于UNet++架构，用残差动态卷积块替换标准卷积，结合动态卷积的自适应特征提取和残差连接的稳定训练优势。

Result: 在合成体模和真实临床数据集上的大量实验验证了该方法的有效性和优越性能，能够产生干净准确的重建结果。

Conclusion: 提出的混合重建框架成功整合了知识驱动和数据驱动方法的优势，有效解决了双能CT重建中的挑战，为医学成像提供了更准确的重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在将2D基础模型知识聚合到3D部件分割时，要么忽略3D几何结构学习，要么忽视SAM的高质量分组线索，导致欠分割和部件标签不一致

Method: 提出基于SAM分割图的传播方法SegGraph：构建分割图（节点为分割区域，边捕捉空间关系），通过图神经网络传播2D特征，使用视角方向加权融合将特征映射到3D点

Result: 在PartNet-E数据集上优于所有基线方法至少6.9% mIoU，在小部件和部件边界上表现尤其突出

Conclusion: SegGraph通过显式学习SAM分割掩码中的几何特征，有效解决了2D知识到3D的聚合问题，实现了更好的几何理解和部件分割性能
...
TL;DR: C-DGPA是一种用于无监督域适应的类中心双对齐生成提示适应方法，通过双分支架构同时优化边际分布对齐和条件分布对齐，在多个基准测试中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的提示调优方法主要关注边际分布对齐，但忽略了条件分布差异，导致类原型错位和语义判别性下降等问题。需要同时解决这两种分布差异以实现更好的域适应。

Method: 提出C-DGPA方法，包含两个分支：1）边际分布对齐分支使用动态对抗训练框架减少边际分布差异；2）条件分布对齐分支引入类映射机制（CMM）对齐条件分布差异，通过标准化语义提示理解和防止源域过依赖。

Result: 在OfficeHome、Office31和VisDA-2017三个基准数据集上进行广泛实验，C-DGPA在所有基准测试中都取得了新的最先进结果，验证了方法的优越性。

Conclusion: C-DGPA通过双对齐策略有效将领域知识整合到提示学习中，产生域不变且语义可判别的表示，解决了现有方法在条件分布对齐方面的不足。
...
TL;DR: 事件相机在自动驾驶中相比传统相机能更好地应对昼夜光照差异带来的域差距问题
...
Conclusion: 事件相机是应对自动驾驶中光照条件域差距问题的潜在替代方案，能保持跨域性能一致性
...
TL;DR: Avatar4D是一个可迁移的合成人类运动数据集生成管道，提供对身体姿态、外观、相机视角和环境背景的细粒度控制，无需人工标注。通过Syn2Sport数据集在体育领域验证，展示了合成数据在监督学习、零样本迁移和跨体育泛化方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注日常通用动作且灵活性有限，无法满足特定领域应用的需求。体育等特定领域的人类动作和运动模式对运动理解提出了独特挑战，需要可定制、可扩展的合成数据集。

Method: 提出Avatar4D管道，生成高保真4D（随时间变化的3D几何）人类运动序列，具有变化的玩家外观和多样化环境渲染。创建Syn2Sport大规模合成数据集，涵盖棒球和冰球等体育项目。

Result: 在Syn2Sport上评估了多个最先进的姿态估计模型，展示了合成数据在监督学习、零样本迁移到真实数据以及跨体育泛化方面的有效性。评估了生成合成数据与真实数据集在特征空间的对齐程度。

Conclusion: Avatar4D展示了在不依赖领域特定真实数据的情况下，生成可扩展、可控且可迁移的人类数据集用于多样化领域特定任务的潜力。
...
TL;DR: VALOR提出了一种基于强化学习的视觉对齐方法，通过两阶段训练提升医学视觉语言模型在放射学报告生成中的临床准确性和视觉基础性。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型在生成放射学报告时存在视觉基础性不足和临床准确性不高的问题，现有方法依赖大量标注数据、昂贵的任务特定偏好数据或检索方法，但未能有效缓解视觉与语言表示之间跨模态对齐不良导致的幻觉问题。
...
Conclusion: VALOR通过强化学习驱动的视觉对齐框架，有效解决了医学视觉语言模型在放射学报告生成中的幻觉问题，实现了更好的临床准确性和视觉基础性，为自动化医疗工作流程提供了更可靠的解决方案。
...
TL;DR: OAK模型通过可学习的上下文token和冻结CLIP，结合图像-文本对齐与视觉聚类目标，实现了自适应视觉场景分类，在Stanford和Clevr-4数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统AI系统使用固定的通用类别（如植物、动物），但现实任务需要动态创建的自适应类别（ad-hoc categories）。现有方法难以同时处理少量标注样本和大量未标注数据，无法有效发现底层上下文并扩展自适应类别。

Method: 提出OAK模型：在冻结的CLIP模型输入层引入少量可学习的上下文token，同时优化CLIP的图像-文本对齐目标和GCD的视觉聚类目标，实现语义扩展和视觉聚类。

Result: 在Stanford和Clevr-4数据集上达到SOTA：Stanford Mood数据集上新颖类别准确率达87.4%，比CLIP和GCD提升超过50%。模型生成可解释的显著性图，能聚焦于任务相关区域（如动作关注手部、情绪关注面部、位置关注背景）。

Conclusion: OAK证明了自适应类别与通用类别共享相似感知机制，通过简单有效的设计实现了自适应、可泛化的分类，同时提供透明可解释的决策过程，增强了AI系统的可信度。
...
TL;DR: 该论文提出了一种基于信息几何的3D点云形状分析方法，通过将点云表示为统计流形上的高斯混合模型，并引入具有理论上界和下界的修正对称KL散度，解决了传统几何度量的局限性。


<details>
  <summary>Details</summary>
Motivation: 三维点云在多个领域有重要应用，但比较点云面临挑战：传统几何度量（如Hausdorff和Chamfer距离）无法捕捉全局统计结构且对异常值敏感，而现有的高斯混合模型KL散度近似可能产生无界或不稳定的数值。

Method: 提出信息几何框架，将点云表示为统计流形上的高斯混合模型。证明GMM空间构成统计流形，并提出修正对称KL散度，该度量具有理论保证的上界和下界，确保所有GMM比较的数值稳定性。

Result: 在人体姿态判别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）的实验中，MSKL提供了稳定且单调变化的值，直接反映几何变化，优于传统距离和现有KL近似方法。

Conclusion: 提出的信息几何框架和修正对称KL散度为3D点云形状分析提供了数值稳定且几何意义明确的度量方法，解决了传统方法的局限性，在点云比较任务中表现出优越性能。
...
TL;DR: 该论文提出了一种基于编码器-解码器网络的学习框架，将随机噪声转换为高质量噪声，以提升单视图新视角合成模型的生成效果。


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的单视图新视角合成模型虽然能根据单张图像和相机姿态生成新视角图像，但缺乏专门的学习框架来学习高质量初始噪声，而高质量噪声能带来更好的生成结果。

Method: 1) 设计离散化欧拉反演方法，将图像语义信息注入随机噪声，构建随机噪声与高质量噪声的配对数据集；2) 提出基于编码器-解码器网络的学习框架，直接将随机噪声转换为高质量噪声。

Result: 提出的EDN框架可以无缝集成到SV3D、MV-Adapter等多种NVS模型中，在多个数据集上实现了显著的性能提升。

Conclusion: 通过专门学习高质量初始噪声，可以显著提升单视图新视角合成模型的生成质量，该方法具有通用性和有效性。
...
TL;DR: SVD低秩近似图像压缩效果不如JPEG等标准格式，在低误差时压缩文件甚至可能比原图更大
...
TL;DR: ARMFlow是一个基于MeanFlow的自回归框架，用于实时生成3D人体反应动作，解决高保真度、实时推理和自回归适应性三大挑战，在在线和离线场景均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时满足3D人体反应生成的三个关键要求：高动作保真度、实时推理能力以及在线场景的自回归适应性。需要一种能够兼顾这三方面性能的解决方案。

Method: 提出ARMFlow框架，包含因果上下文编码器和基于MLP的速度预测器。引入Bootstrap Contextual Encoding (BSCE)训练策略，使用生成的历史而非真实历史进行编码，减少自回归生成中的误差累积。还提出离线变体ReMFlow。

Result: ARMFlow在InterHuman和InterX数据集上，单步在线生成的FID指标比现有在线方法提升超过40%，同时匹配离线SOTA性能，尽管只使用部分序列条件。ReMFlow在离线方法中达到最快推理速度。

Conclusion: ARMFlow通过全局上下文编码器增强语义对齐、单步推理实现高精度低延迟、BSCE减少累积误差，成功解决了在线3D人体反应生成的关键限制，在在线和离线场景均表现卓越。
...
TL;DR: 开发了一个结合深度学习图像分析和临床数据（包括家族史）的多模态AI框架，用于提高皮肤病诊断准确性，特别针对遗传性皮肤病。


<details>
  <summary>Details</summary>
Motivation: 皮肤病影响全球19亿人，但准确诊断面临挑战：专科医生有限、临床表现复杂。家族史对皮肤病易感性和治疗反应有重要影响，但在诊断过程中常被忽视。需要AI系统整合家族史数据和临床影像来增强诊断，同时支持临床试验验证和实际应用。

Method: 开发了多模态AI框架，结合深度学习图像分析和结构化临床数据（包括详细的家族史模式）。采用可解释的卷积神经网络与整合遗传风险因素的临床决策树相结合。方法包括在不同医疗环境中进行前瞻性临床试验，验证AI辅助诊断与传统临床评估的对比。

Result: 整合家族史数据的AI系统显示出更高的诊断准确性，特别是对黑色素瘤、银屑病和特应性皮炎等遗传性皮肤病。专家反馈表明有潜力改善早期检测和提供更个性化的建议。框架设计为可集成到临床工作流程中，同时通过可解释AI机制保持可解释性。

Conclusion: AI系统整合家族史数据能显著提高皮肤病诊断准确性，特别是遗传性皮肤病。该框架具有临床实用性，计划进行正式临床试验验证其效果。
...
TL;DR: 提出两种基于模型排序的半监督多视角人群计数框架，通过约束不同视角数量下的模型预测或不确定性来减少对标注数据的需求


<details>
  <summary>Details</summary>
Motivation: 多视角人群计数可以解决大范围场景中的严重遮挡问题，但多视角数据的收集和标注困难，导致数据集规模有限。需要减少对标注数据的依赖。

Method: 提出两种半监督多视角人群计数框架：1）基于预测排序：约束较少视角的预测不应多于较多视角的预测；2）基于不确定性排序：约束较多视角的模型不确定性不应大于较少视角的不确定性。这些约束以半监督方式引入模型训练。
...
TL;DR: 提出FLIM_PSR_k深度学习框架，通过像素超分辨率技术将荧光寿命成像的空间分辨率提升5倍，实现25倍空间带宽积增加，解决FLIM临床应用中分辨率-速度权衡问题。


<details>
  <summary>Details</summary>
Motivation: 荧光寿命成像（FLIM）具有强大的代谢和分子对比能力，但临床应用受到长像素驻留时间和低信噪比的限制，导致分辨率与速度之间的严格权衡，需要新的技术突破。

Method: 基于条件生成对抗网络（cGAN）的深度学习框架FLIM_PSR_k，通过多通道像素超分辨率技术，从像素尺寸增加5倍的低分辨率数据中重建高分辨率FLIM图像。

Result: 在患者来源的肿瘤组织样本盲测中，FLIM_PSR_k可靠实现k=5的超分辨率因子，输出图像空间带宽积增加25倍，揭示低分辨率输入中丢失的精细结构特征，各项图像质量指标均有统计学显著改善。

Conclusion: FLIM_PSR_k通过提高FLIM的有效空间分辨率，推动寿命成像向更快、更高分辨率、硬件灵活的方向发展，兼容低数值孔径和小型化平台，更好地促进FLIM的转化应用。
...
<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍未被充分探索。现有方法需要生成可读字符的同时保持语义、几何和上下文一致性，这是一个未被满足的研究需求。

Method: 提出了TextEditBench评估基准，专注于图像中的文本中心区域，强调需要物理合理性、语言意义和跨模态依赖理解的推理密集型编辑场景。引入了语义期望（SE）这一新评估维度，衡量模型在文本编辑过程中保持语义一致性、上下文连贯性和跨模态对齐的推理能力。
...
TL;DR: GFLAN是一个两阶段生成框架，通过拓扑规划和几何实现分解楼层平面图生成，使用卷积网络分配房间中心点，再用图神经网络回归房间边界。
...
Method: 两阶段分解方法：阶段A使用具有双编码器的专用卷积架构，分离不变空间上下文和演化布局状态，通过离散概率图在建筑包络内顺序分配房间中心点；阶段B构建连接房间节点和边界顶点的异构图，应用Transformer增强的图神经网络联合回归房间边界。

Result: 论文提出了GFLAN框架，通过显式分解为拓扑规划和几何实现来重构楼层平面图合成，解决了传统方法在捕捉建筑推理方面的局限性。

Conclusion: GFLAN通过将楼层平面图生成分解为拓扑规划和几何实现两个阶段，提供了一种原则性的方法，能够更好地捕捉建筑推理过程，超越了直接的像素到像素或墙体追踪生成方法。
...
Method: MACL（多标签自适应对比学习）扩展了对比学习框架，集成了三个关键技术：1）标签感知采样 - 考虑标签语义关系进行样本选择；2）频率敏感加权 - 根据类别频率调整权重以平衡常见和稀有类别；3）动态温度缩放 - 自适应调整对比损失的温度参数。

Result: 在三个基准数据集（DLRSD、ML-AID、WHDLD）上的实验表明，MACL持续优于基于对比损失的基线方法，有效缓解了语义不平衡问题，并在大规模遥感档案中提供了更可靠的检索性能。

Conclusion: MACL通过自适应对比学习机制成功解决了多标签遥感图像检索中的关键挑战，为大规模遥感档案的可靠检索提供了有效解决方案，代码和预训练模型将在接受后开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的图像生成基准主要关注美学质量而非细粒度生成能力。现有评测方法无法客观评估模型在像素级精度上的生成智能。

Method: 提出PixelArena基准，使用语义分割任务来评估模型的细粒度生成能力。通过零样本设置测试模型生成语义掩码的能力，并进行定性和定量比较分析。

Result: Gemini 3 Pro Image展现出新兴的图像生成能力，在零样本设置下能生成高保真语义掩码，表现出前所未有的视觉智能和真正的任务泛化能力。研究还提供了与其他模型的比较结果和失败案例。
...
TL;DR: 提出LaverNet，一个仅362K参数的轻量级一体化视频修复网络，通过选择性传播退化无关特征来应对时变退化问题，性能可比甚至优于现有大模型。


<details>
  <summary>Details</summary>
Motivation: 现有的一体化视频修复方法在处理时变退化时面临两个挑战：1）退化会主导时序建模，使模型更关注伪影而非视频内容；2）当前方法通常依赖大模型来掩盖底层困难。

Method: 提出LaverNet轻量级网络，引入新颖的传播机制，仅选择性传输退化无关特征跨帧传播，从而减轻退化对时序建模的影响。

Result: LaverNet仅362K参数（不到现有模型的1%），但在多个基准测试中取得了可比甚至更优的性能。

Conclusion: 通过LaverNet证明了强大的全合一视频修复可以通过紧凑网络实现，为轻量级视频修复提供了新方向。
...
TL;DR: 提出基于岭估计的融合定位方法，结合序列图像丰富场景信息和激光测距高精度，提升无人机目标定位准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在无人机多传感器目标跟踪测量中，传统最小二乘算法在远距离、小交会角、大倾角等有限观测条件下，设计矩阵列向量存在严重多重共线性，导致病态问题和定位不稳定。
...
Conclusion: 基于岭估计的融合定位方法能够有效解决有限观测条件下的多重共线性问题，提高无人机多传感器目标定位的准确性和鲁棒性。
...
TL;DR: QUIDS系统通过质量感知的激励机制，在非专用车载移动群智感知中联合优化覆盖度和可靠性，提高感知质量38%，降低地图重建误差39-74%
...
Result: 在真实城市部署数据上，QUIDS比无调度场景提高ASQ 38%，比最先进方法提高10%，地图重建误差降低39-74%
...
TL;DR: 提出一个协作的边缘到服务器视觉语言模型推理框架，通过选择性重传机制在保持推理精度的同时显著降低通信成本。


<details>
  <summary>Details</summary>
Motivation: 传统部署中，边缘设备将视觉数据传输到服务器进行VLM推理，但将原始图像调整为视觉编码器输入分辨率时会丢失细粒度细节，导致精度下降。同时，传输完整高分辨率图像会产生高昂通信成本。

Method: 设计两阶段框架：第一阶段服务器在全局图像上进行推理，利用VLM内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度；若最小熵超过阈值，则请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行精炼推理。

Result: 在多个VLM架构上的实验表明，该框架在保持推理精度的同时显著降低了通信成本。

Conclusion: 提出的协作边缘到服务器推理框架通过选择性重传机制，在视觉语言模型部署中实现了通信效率与推理精度的良好平衡。
...
TL;DR: GMODiff：基于增益图的单步扩散框架，用于多曝光HDR重建，比传统LDM方法快100倍


<details>
  <summary>Details</summary>
Motivation: 预训练的潜在扩散模型（LDMs）在低层视觉任务中显示出强大的感知先验，但直接应用于HDR重建面临三个挑战：1）8位潜在压缩导致动态范围表示有限；2）多步去噪推理成本高；3）生成性质导致内容幻觉
...
Result: GMODiff在性能上优于多个最先进方法，比之前的LDM方法快100倍

Conclusion: 通过增益图驱动的单步扩散框架，成功解决了LDM在HDR重建中的动态范围限制、计算成本和内容幻觉问题，实现了高效高质量的HDR重建
...
Method: 提出身份匹配图（IMG）将生成帧和参考帧中的角色建模为加权完全二分图，使用掩码查询注意力计算边权重。将身份对应正确性形式化为图结构度量并在训练中优化。还提出身份嵌入引导、多尺度匹配策略和预分类采样等针对性策略。

Result: 构建了专门用于多角色身份对应评估的基准测试，大量实验表明EverybodyDance在身份对应和视觉保真度方面显著优于现有最先进基线方法。

Conclusion: EverybodyDance通过系统化的图建模和优化方法，有效解决了多角色动画中的身份对应问题，为复杂多角色场景提供了可靠的解决方案。
...
TL;DR: FVG通过三阶段分解（推理、构图、时序合成）解决T2V模型在复杂场景和逻辑时序指令上的失败问题，显著提升生成质量并加速采样


<details>
  <summary>Details</summary>
Motivation: 当前T2V扩散模型虽然能生成视觉上令人印象深刻的结果，但经常无法构建复杂场景或遵循逻辑时序指令。许多错误源于模型无法构建语义正确或逻辑一致的初始帧

Method: 提出Factorized Video Generation (FVG)管道，将T2V生成分解为三个专门阶段：1) 推理阶段：LLM重写视频提示，仅描述初始场景，解决时序歧义；2) 构图阶段：T2I模型从新提示合成高质量、构图正确的锚定帧；3) 时序合成阶段：微调的视频模型专注于动画化场景并遵循提示

Result: 在T2V CompBench基准测试中达到新的SOTA，在VBench2上显著改进所有测试模型。视觉锚定允许将采样步骤减少70%而不损失性能，实现采样速度大幅提升
...
TL;DR: 提出AFDAN网络，通过频域特征对齐解决医学图像分割数据稀缺问题，在白癜风和视网膜血管分割上取得SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要高质量标注数据，但人工标注耗时耗力导致数据稀缺。现有方法难以有效进行跨域知识迁移。

Method: 提出自适应频域对齐网络(AFDAN)，包含三个核心模块：对抗域学习模块实现源域到目标域特征迁移；源-目标频域融合模块混合跨域频域表示；空间-频域集成模块结合频域和空间特征提升分割精度。

Result: 在新构建的VITILIGO2025数据集上达到90.9%的IoU，在视网膜血管分割基准DRIVE上达到82.6%的IoU，超越现有SOTA方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于token序列的大语言模型无法直接处理包含复杂几何和拓扑信息的3D边界表示模型，存在3D几何数据与自然语言之间的模态鸿沟。
...
Conclusion: BrepLLM成功实现了LLM对原始Brep数据的解析和推理，弥合了结构化3D几何与自然语言之间的模态差距，为3D几何理解开辟了新途径。
...
TL;DR: CountZES：一种无需训练、通过零样本范例选择进行物体计数的框架，通过三个阶段渐进式发现多样化范例，在零样本物体计数任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有零样本物体计数方法存在局限性：基于开放词汇检测器的方法通常产生多实例候选，而随机补丁采样方法无法准确描绘物体实例。需要一种能精确选择单实例范例的方法来解决这些挑战。

Method: 提出CountZES框架，包含三个协同阶段：1) DAE阶段：精炼开放词汇检测结果以隔离精确的单实例范例；2) DGE阶段：通过密度驱动的自监督范式识别统计一致且语义紧凑的范例；3) FCE阶段：通过特征空间聚类增强视觉一致性。这三个阶段共同产生多样化、互补的范例集。

Result: 在多个数据集上的实验表明，CountZES在零样本物体计数方法中表现优异，并能有效泛化到自然场景、航拍图像和医学图像等多个领域。

Conclusion: CountZES通过渐进式范例选择框架解决了零样本物体计数的关键挑战，在保持文本接地性的同时平衡了计数一致性和特征代表性，展示了跨领域的强大泛化能力。
...
Method: 从几何角度出发，通过精炼文本嵌入来抑制不需要的语义，解决语义纠缠问题，无需训练即可提升主题一致性和文本对齐

Result: 实验证明该方法在主题一致性和文本对齐方面显著优于现有基线方法

Conclusion: 提出了一种简单有效的无需训练方法，通过几何视角解决语义纠缠问题，显著提升了多图像生成中的主题一致性和文本对齐性能
...
TL;DR: 提出首个包含23.7K视线引导人体运动序列的数据集，用于训练扩散模型生成自然的"注视-接近-抓取"运动


<details>
  <summary>Details</summary>
Motivation: 现有运动生成模型难以模拟人类自然的"注视引导"行为（先注视目标物体/位置，再接近并抓取），需要专门的数据集和评估方法

Method: 从5个公开数据集（HD-EPIC、MoGaze、HOT3D、ADT、GIMO）中整理23.7K视线引导运动序列，预训练文本条件扩散模型，然后用目标姿态/位置条件进行微调

Result: 在HD-EPIC数据集上，模型在目标物体位置条件下达到60%的Prime Success（注视引导成功率）和89%的Reach Success（抓取成功率）

Conclusion: 通过专门的数据集和评估指标，成功生成了更自然的视线引导人体运动，为模拟真实人类行为提供了有效方法
...
TL;DR: SNOW是一个无需训练、与主干网络无关的4D场景理解框架，将视觉语言模型的语义知识与点云几何和时序一致性相结合，构建可查询的4D场景图用于机器人推理。
...
Method: 处理同步的RGB图像和3D点云，使用HDBSCAN聚类生成物体级提议，指导SAM2分割。通过提出的STEP编码产生多模态token，捕获局部语义、几何和时序属性。这些token被增量整合到4D场景图中，轻量级SLAM后端提供空间锚定。

Result: 在多个基准测试中，SNOW实现了精确的4D场景理解和空间grounded推理，在多个设置中达到了新的state-of-the-art性能。

Conclusion: SNOW展示了结构化4D先验对于具身推理和自主机器人的重要性，通过整合语义、几何和时序信息，为机器人提供了可查询的统一世界模型。
...
TL;DR: StageVAR：针对视觉自回归模型的阶段感知加速框架，通过分析不同生成阶段的重要性差异，在保持早期关键步骤完整的同时，对后期细节优化步骤进行剪枝和近似，实现高效加速。
...
Method: StageVAR通过分析发现早期步骤对语义和结构一致性至关重要，而后期步骤主要细化细节。基于此，提出即插即用的加速策略，利用后期计算的语义无关性和低秩特性进行剪枝和近似，无需额外训练。

Result: StageVAR实现了最高3.4倍的加速，在GenEval上仅下降0.01分，在DPG上下降0.26分，始终优于现有加速基线方法。

Conclusion: 阶段感知设计是高效视觉自回归图像生成的强大原则，StageVAR为VAR模型提供了一种系统化、智能化的加速框架，在保持生成质量的同时显著提升效率。
...
TL;DR: 该论文提出了一种通过强化学习让模型获得人类感知-推理能力的盲图像质量评估方法，不仅预测评分，还能生成类似人类的解释性推理链。


<details>
  <summary>Details</summary>
Motivation: 人类评估图像质量是通过感知-推理的级联过程，结合感官线索和隐含推理形成自洽判断。现有盲图像质量评估模型缺乏这种人类式的推理能力和可解释性。

Method: 首先收集捕捉人类感知-推理过程多个方面的人类评估数据，然后采用强化学习，以人类标注作为奖励信号，引导模型获得人类式感知和推理能力。特别设计奖励机制使模型能够从自生成的描述中推断图像质量，实现自洽推理。

Result: 在通用指标（Pearson和Spearman相关系数）上，该方法达到了与最先进盲图像质量评估系统相当的预测性能。在1000多个人类标注样本上，模型在ROUGE-1指标上达到0.512分（基线为0.443），表明模型生成内容与人类解释有显著覆盖。

Conclusion: 该方法在盲图像质量评估中实现了与人类感知-推理链的更好对齐，标志着向人类式可解释推理迈出了一步，为开发更具解释性的图像质量评估系统提供了新方向。
...
TL;DR: 该论文提出通过眼动行为来弥补面部表情识别与真实情感识别之间的差距，构建了包含眼动行为的多模态情感识别数据集EMER，并设计了EMERT模型来提升情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 当前情感识别领域过度依赖面部表情识别，但面部表情常被用作社交工具而非真实内心情感的表达，导致FER与真实ER之间存在差距。作者认为眼动行为是重要的情感线索，可以弥补这一差距。

Method: 1) 构建EMER数据集：采用自发情感诱导范式收集真实情感数据，同时采集眼动序列、注视点图等眼动行为数据和面部表情视频；2) 设计EMERT模型：采用模态对抗特征解耦和多任务Transformer，将眼动行为作为面部表情的补充。

Result: 1) 创建了包含多模态数据的EMER数据集；2) 提出了七个多模态基准协议用于全面评估；3) EMERT模型显著优于其他最先进的多模态方法，证明了眼动行为建模对鲁棒情感识别的重要性。

Conclusion: 眼动行为在情感识别中具有重要作用，能够有效弥补面部表情识别与真实情感识别之间的差距。EMER数据集和EMERT模型为更鲁棒的情感识别研究提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 360度全景图像处理面临空间畸变、大视场和超高分辨率等挑战，传统检测器（如YOLO）针对标准图像尺寸优化，难以处理4K及以上分辨率的全景图像，计算需求过高

Method: 1. 引入多尺度检测头，包含P2层以提高对小物体的检测灵敏度；2. 采用GhostConv骨干网络降低计算复杂度而不损失表征能力；3. 构建CVIP360数据集，包含6,876个帧级边界框标注，为4K全景场景提供公开检测基准

Result: YOLO11-4K在0.50 IoU下达到0.95 mAP，每帧推理时间28.3毫秒，相比YOLO11（112.3毫秒）延迟降低75%，同时精度提升（从0.908提升至0.95）

Conclusion: 该框架在效率和精度间取得良好平衡，适用于360度大范围环境的鲁棒目标检测，可应用于自动驾驶、监控和增强现实等高分辨率全景应用场景
...
TL;DR: 提出PoseMoE，一种基于专家混合网络的单目3D人体姿态估计方法，通过分离2D姿态和深度特征编码来提升精度


<details>
  <summary>Details</summary>
Motivation: 传统基于提升的方法将检测到的2D姿态和未知深度在纠缠的特征空间中编码，深度不确定性会显式影响2D姿态特征，限制了整体估计精度。研究发现深度表示对估计过程至关重要：当深度处于完全未知状态时，与2D姿态特征联合编码是有害的；而当深度经过网络初步细化后，与2D姿态信息联合编码则是有益的。

Method: 提出PoseMoE方法：1) 专家混合网络，其中专门化的专家模块分别细化检测良好的2D姿态特征和学习深度特征，解耦2D姿态和深度的特征编码过程；2) 跨专家知识聚合模块，通过2D姿态和深度之间的双向映射来聚合跨专家的时空上下文信息

Result: 在三个广泛使用的数据集（Human3.6M、MPI-INF-3DHP和3DPW）上，PoseMoE优于传统的基于提升的方法

Conclusion: 通过分离2D姿态和深度特征编码的专家混合网络设计，有效减少了不确定深度特征对2D姿态特征的显式影响，提升了单目3D人体姿态估计的精度
...
TL;DR: VenusBench-GD是一个跨平台的双语GUI元素定位基准，包含大规模数据集、高质量标注流程和分层任务分类，评估发现通用多模态模型在基础任务上已媲美专用GUI模型，但高级任务仍需专用模型。


<details>
  <summary>Details</summary>
Motivation: 现有GUI定位基准存在数据量不足、领域覆盖窄、平台单一或需要专业知识等问题，需要构建更全面、实用的评估框架来推动GUI代理发展。

Method: 1) 构建大规模跨平台基准，覆盖多种应用、UI元素和丰富标注数据；2) 建立高质量数据构建流程，标注准确率更高；3) 提出分层任务分类法，将定位分为基础和高级两类，包含六个互补的子任务。

Result: 实验发现：通用多模态模型在基础定位任务上已匹配甚至超越专用GUI模型；而高级任务仍偏向GUI专用模型，但这些模型存在明显过拟合和鲁棒性差的问题。

Conclusion: 需要全面、多层次评估框架来准确评估GUI定位能力，VenusBench-GD为此提供了基准，并揭示了当前模型的局限性和未来发展方向。
...
TL;DR: 提出一种用于骨架时序动作定位的自监督预训练方法，通过片段区分任务和U形模块增强特征分辨率，在多个数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 骨架动作识别已有自监督预训练成功案例，但时序动作定位仍具挑战。动作边界检测需要时间敏感的特征来捕捉相邻帧间的细微差异，而现有方法对此关注不足。

Method: 1) 设计片段区分预训练任务：将骨架序列密集投影到非重叠片段，通过对比学习区分不同视频的片段；2) 构建U形模块：融合骨架动作识别模型的中间特征，增强帧级定位的特征分辨率。

Result: 在BABEL数据集上，方法显著提升了现有骨架对比学习方法在动作定位任务上的性能；在PKUMMD上通过NTU RGB+D和BABEL预训练实现了最先进的迁移学习性能。

Conclusion: 提出的片段区分自监督预训练方法和U形特征融合模块能有效学习时间敏感特征，提升骨架时序动作定位性能，为这一挑战性任务提供了有效解决方案。
...
TL;DR: 提出弱监督深度学习框架，使用Grad-CAM进行肺炎分类和定位，无需像素级标注，仅需图像级标签，评估7个预训练模型，获得高分类准确率
...
Method: 使用Grad-CAM生成热力图定位肺炎区域，评估7个预训练深度学习模型（包括Vision Transformer），采用focal loss和患者级数据划分防止数据泄露，仅需图像级标签

Result: 所有模型达到高分类准确率（96-98%），ResNet-18和EfficientNet-B0表现最佳，MobileNet-V3作为轻量级替代方案。Grad-CAM热力图可视化显示模型聚焦于临床相关肺区域
...
TL;DR: TTP是一种轻量级防御框架，通过测试时填充实现对抗样本检测和针对性适应，在保持干净准确率的同时显著提升CLIP模型的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（如CLIP）在零样本识别方面表现出色，但对对抗扰动高度敏感，存在安全风险。现有训练时防御需要标注数据和昂贵重训练，而测试时策略无法可靠区分干净和对抗输入，无法同时优化对抗鲁棒性和干净准确率。
...
Result: 在多种CLIP骨干网络和细粒度基准测试上的实验表明，TTP持续超越最先进的测试时防御方法，在保持干净准确率的同时显著提升对抗鲁棒性。

Conclusion: TTP为视觉语言模型提供了一种轻量级、有效的测试时防御方案，无需重训练即可同时实现对抗鲁棒性和干净准确率的优化，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型只能基于2D图像回答问题，缺乏内在的3D物体感知能力，限制了它们在3D场景中理解空间关系和深度线索的能力。

Method: 提出N3D-VLM框架，通过可扩展的数据构建流程，利用深度估计将大规模2D标注提升到3D空间，生成3D物体定位和空间问答数据集，支持3D物体定位和3D空间推理的联合训练。

Result: 实验结果表明，该框架在3D定位任务上达到最先进性能，在3D空间推理方面也持续超越现有视觉语言模型方法，构建的数据集比现有最大单图像3D检测数据集大6倍以上。

Conclusion: N3D-VLM成功将原生3D物体感知与3D感知视觉推理相结合，实现了更精确的3D定位和更可解释的结构化空间理解，为3D感知多模态模型的发展提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见场景部分，无法保持对先前观察部分的记忆，缺乏对动态场景的完整时空理解。

Method: 将场景分解为刚性3D基元，通过估计的密集2D对应关系，联合优化推断基元的刚性运动，实现4D重建；引入运动外推机制处理不可见物体，使用运动分组技术保持连续性。

Result: 在物体扫描和多物体数据集上，系统在定量和定性评估中显著优于现有方法，实现了可回放的3D重建、多物体扫描和物体持久性等能力。

Conclusion: 提出的动态重建系统能够从单目视频中实现完整持久的4D时空重建，为动态场景理解提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在微调视觉基础模型时忽略了预训练模型中的artifacts（非因果因素），这些因素阻碍了有价值表示的利用，降低了域泛化性能

Method: 使用离散余弦变换提取特征频谱，通过高斯带通滤波器分离因果与非因果成分，引入因果感知可学习token在频域细化因果成分，丢弃非因果成分

Result: 在多种跨域任务上验证了有效性，特别是在恶劣天气条件下表现优异，在雪天条件下比基线提升了4.8% mIoU

Conclusion: 通过因果机制分析特征中的因果与非因果因素，并有效分离它们，能够显著提升视觉基础模型在域泛化语义分割中的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有模型要么依赖单一先验扫描，要么使用固定网格时间，要么针对全局标签，这限制了不规则采样下的体素级预测。需要一种能够处理多上下文、连续时间预测的统一框架。
...
TL;DR: SkiLa提出了一种新的多模态推理范式，通过让MLLMs在推理过程中原生生成连续的视觉嵌入（潜在草图标记）作为视觉思维，实现统一的视觉-文本想象和交互。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要视觉想象的场景中表现不足，而人类能够在统一的大脑空间中构建灵活的视觉-文本思维过程。受此启发，作者认为既然当前MLLMs已经在同一特征空间中编码视觉和文本信息，视觉标记应该能够无缝插入到文本标记承载的推理过程中。

Method: 提出Sketch-in-Latents (SkiLa)范式，扩展MLLMs的自回归能力以原生生成连续的视觉嵌入（潜在草图标记）。在推理过程中，模型动态切换文本思维模式（生成文本思考标记）和视觉草图模式（生成潜在草图标记），并通过潜在视觉语义重建机制确保这些潜在草图标记具有语义基础。

Result: 大量实验表明，SkiLa在视觉中心任务上实现了优越性能，同时在多样化的通用多模态基准测试中表现出强大的泛化能力。

Conclusion: SkiLa通过统一的潜在空间实现视觉-文本推理，为MLLMs提供了更接近人类思维的视觉想象能力，在多模态任务中展现出显著优势。
...
TL;DR: 提出Yuan-TecSwin，一种基于Swin-transformer的文本条件扩散模型，在ImageNet生成基准上达到SOTA的FID 1.37分，人类难以区分生成图像与真实图像。
...
Method: 用Swin-transformer块替换CNN块构建U形架构，改进文本编码器选择和文本嵌入的有效利用，精心设计文本条件融合方式，并采用自适应时间步搜索策略。

Result: 在ImageNet生成基准上达到SOTA的FID 1.37分，推理性能提升10%，人类评估中难以区分生成图像与真实图像。

Conclusion: Swin-transformer在扩散模型中能有效提升非局部建模能力，结合精心设计的文本条件融合策略，实现了高质量的文本到图像生成。
...
TL;DR: Hazedefy是一个轻量级、面向应用的实时去雾管道，专注于在消费级硬件上部署，基于暗通道先验和大气散射模型，通过优化计算实现实时视频和直播增强。


<details>
  <summary>Details</summary>
Motivation: 现有去雾算法通常计算复杂，难以在消费级硬件上实时运行，特别是对于视频流和直播应用。需要一种轻量级、实用的解决方案，能够在移动和嵌入式设备上实现实时去雾增强。
...
Conclusion: Hazedefy提供了一种计算简单、可部署的实时去雾解决方案，平衡了去雾效果和计算效率，特别适合消费级硬件上的视频增强应用。
...
TL;DR: LLSA提出了一种对数线性复杂度的稀疏注意力机制，通过分层结构解决了DiTs中二次注意力成本的问题，在保持生成质量的同时大幅加速训练和推理。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）在视觉生成中达到最先进水平，但其二次自注意力成本从根本上限制了向长令牌序列的扩展。现有的Top-K稀疏注意力方法虽然通过压缩令牌和选择相关关键块来减少计算，但仍存在两个问题：(1) 压缩令牌上的二次选择成本；(2) 随着序列增长，需要增加K值来维持模型质量。作者发现这种低效性源于单层设计，因为单个粗略层级不足以表示全局结构。

Method: 提出对数线性稀疏注意力（LLSA），这是一种可训练的稀疏注意力机制，通过利用分层结构将选择和注意力成本从二次降低到对数线性复杂度。LLSA执行分层Top-K选择，逐步采用稀疏Top-K选择并使用前一层级找到的索引，并引入分层KV丰富机制，在注意力计算中使用不同粒度的更少令牌来保留全局上下文。为了支持高效训练，开发了高性能GPU实现，仅使用稀疏索引进行前向和反向传播，无需密集注意力掩码。

Result: 在256×256像素令牌序列上，LLSA将注意力推理加速28.27倍，将DiT训练加速6.09倍，同时保持生成质量。该方法在不使用分块化和VAE编码的情况下评估了高分辨率像素空间图像生成。

Conclusion: LLSA为高效训练长序列DiTs提供了一个有前景的方向。结果表明，通过分层结构设计，可以在保持模型质量的同时显著降低计算复杂度，解决了DiTs扩展到长序列的根本限制。
...
TL;DR: 提出一个基于电源插座的三阶段深度学习管道，用于室内多媒体地理位置定位，通过检测插座、分类插座类型、映射到国家来辅助数字取证调查。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理位置定位在打击人口贩运、儿童剥削等严重犯罪中具有重要应用价值，但现有方法面临室内布局相似、装修频繁、视觉模糊、光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战。
...
Result: 在Hotels-50K数据集（特别是TraffickCam子集）上评估，该数据集包含真实世界条件下的酒店图像（光照差、业余拍摄角度）。管道在插座检测、分类和国家映射方面均表现出色，展示了实际应用的可行性。

Conclusion: 该框架利用电源插座作为一致的室内标记物，为解决室内地理位置定位挑战提供了实用方法，是迈向实际数字取证应用的重要一步。代码、训练模型和数据已开源。
...
TL;DR: DeContext：一种通过注入微小扰动来破坏多模态注意力层，从而保护输入图像免遭未经授权的上下文编辑的新方法


<details>
  <summary>Details</summary>
Motivation: 上下文扩散模型虽然功能强大，但存在严重的隐私问题：个人图像可能被轻易操纵用于身份冒充、虚假信息等恶意用途。现有研究主要关注个性化文本到图像生成的输入扰动保护，但对现代大规模基于DiT的上下文模型的鲁棒性研究不足。

Method: DeContext通过注入微小、有针对性的扰动来削弱多模态注意力层的跨注意力路径，从而打破输入与输出之间的链接。研究发现早期去噪步骤和特定Transformer块主导上下文传播，因此可以将扰动集中在最关键的位置。

Result: 在Flux Kontext和Step1X-Edit上的实验表明，DeContext能持续阻止不需要的图像编辑，同时保持视觉质量。注意力基扰动被证明是对抗图像操纵的有效防御手段。

Conclusion: DeContext提供了一种高效且鲁棒的方法来保护图像免遭未经授权的上下文编辑，通过破坏注意力机制中的上下文传播路径，在保持图像质量的同时有效防止恶意操纵。
...
TL;DR: SARMAE：首个面向SAR图像的噪声感知掩码自编码器，通过构建百万级SAR数据集SAR-1M，结合斑点噪声注入和光学先验对齐，实现自监督SAR表征学习。
...
Method: 1) 构建首个百万级SAR数据集SAR-1M，包含配对的光学图像；2) 设计斑点感知表征增强(SARE)，在掩码自编码器中注入SAR特有斑点噪声；3) 引入语义锚点表征约束(SARC)，利用光学先验对齐SAR特征。

Result: 在多个SAR数据集上的分类、检测和分割任务中达到最先进性能，证明了SARMAE在噪声鲁棒性和语义一致性方面的优势。

Conclusion: SARMAE通过噪声感知的自监督学习框架有效解决了SAR图像数据稀缺和斑点噪声问题，为SAR遥感应用提供了强大的基础模型。
...
TL;DR: REGLUE是一种新的潜在扩散模型框架，通过联合建模VAE潜在空间、局部VFM语义和全局CLS标记，并引入非线性语义压缩器和外部对齐损失，显著提升了图像生成质量和训练效率。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型通过去噪目标间接学习语义，需要长时间训练且样本质量有限。现有方法未能充分利用视觉基础模型丰富的多层空间语义信息，要么仅外部对齐，要么内部建模过于局限。

Method: 提出REGLUE框架：1) 使用轻量级卷积语义压缩器非线性聚合多层VFM特征为低维空间结构化表示；2) 在单个SiT骨干中联合建模VAE潜在、局部VFM语义和全局CLS标记；3) 引入外部对齐损失正则化内部表示。

Result: 在ImageNet 256x256上，REGLUE持续改进FID并加速收敛，优于SiT-B/2、SiT-XL/2基线以及REPA、ReDi、REG等方法。实验表明空间VFM语义、非线性压缩、全局标记和外部对齐都至关重要。

Conclusion: REGLUE通过全局-局部-潜在联合建模框架，充分利用视觉基础模型的丰富语义信息，显著提升了潜在扩散模型的性能和训练效率，为语义增强的图像生成提供了有效解决方案。
...
TL;DR: 使用ResNet-50和三元组损失进行主体指纹识别，通过将同一主体的所有图像映射到潜在空间的特定区域，解决开源数据集合并时的数据泄露问题，在MRI和X-ray数据上实现了高精度的少样本主体重识别。


<details>
  <summary>Details</summary>
Motivation: 合并开源数据集时，如果同一主体出现在多个数据集中会导致数据泄露，从而夸大模型性能。需要一种方法来识别和避免这种数据泄露问题。

Method: 提出主体指纹识别方法，将同一主体的所有图像映射到潜在空间的特定区域。使用ResNet-50架构，采用三元组边界损失进行训练，通过相似性匹配实现主体重识别。在3D MRI和2D X-ray数据上评估少样本指纹识别性能，包括标准（20-way 1-shot）和挑战性（1000-way 1-shot）场景。
...
Conclusion: 主体指纹识别方法能有效解决数据集合并时的数据泄露问题，在少样本设置下实现了高精度的主体重识别，为医学图像分析中的数据集质量控制提供了有效工具。
...
TL;DR: 系统评估了为完全合成图像设计的深度伪造检测器在局部修复检测任务上的泛化能力，发现这些模型对中等和大面积修复具有部分可迁移性，并优于现有专门检测方法。
...
Method: 系统评估了为深度伪造检测训练的最先进检测器在局部修复检测任务上的表现。研究使用多个数据集，涵盖不同的生成器、掩码大小和修复技术。

Result: 实验表明，在大量生成器上训练的模型对基于修复的编辑具有部分可迁移性，能够可靠地检测中等和大面积操作或再生式修复，性能优于许多现有的专门检测方法。

Conclusion: 为完全合成图像设计的检测器在局部修复检测任务上表现出一定的泛化能力，特别是在检测中等和大面积修复时效果显著，这为开发更通用的图像操作检测系统提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 胎儿生长评估依赖于超声生物测量，但手动标注标志点耗时、操作者依赖性强，且不同扫描设备和站点间存在差异，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备胎儿超声图像基准数据集，包含4,513张去标识化超声图像，来自1,904名受试者，涵盖三个临床站点和七种不同超声设备。提供标准化的受试者分离训练/测试划分、评估代码和基线结果。

Result: 数据集包含所有主要胎儿生物测量指标（双顶径、枕额径、腹横径、腹前后径、股骨长度）的专家标注。通过自动生物测量模型量化了领域偏移，证明单中心训练评估会显著高估性能，多中心测试才能反映真实泛化能力。

Conclusion: 这是首个公开的多中心、多设备、标志点标注的胎儿生物测量数据集，为领域适应和多中心泛化提供了稳健基准，有助于开发更可靠的跨中心AI辅助胎儿生长评估系统。
...
TL;DR: 提出OMG-Bench首个大规模骨架在线微手势识别基准，包含40个精细手势类别和13,948个实例，并开发HMATr框架统一检测与分类，性能优于现有方法7.6%


<details>
  <summary>Details</summary>
Motivation: 在线微手势识别对VR/AR交互至关重要，但面临公开数据集稀缺和任务特定算法的挑战。微手势涉及细微运动模式，使得构建具有精确骨架和帧级标注的数据集困难

Method: 开发多视角自监督流程自动生成骨架数据，结合启发式规则和专家精炼实现半自动标注。提出HMATr框架，使用分层记忆库存储帧级细节和窗口级语义以保持历史上下文，并通过可学习的位置感知查询隐式编码手势位置和语义

Result: 创建OMG-Bench基准，包含40个精细手势类别、13,948个实例、1,272个序列，具有细微运动、快速动态和连续执行特点。HMATr在检测率上优于最先进方法7.6%

Conclusion: OMG-Bench为骨架在线微手势识别提供了首个大规模公开基准，HMATr框架通过分层记忆库和位置感知查询有效统一手势检测与分类，为在线微手势识别建立了强基线
...
<details>
  <summary>Details</summary>
Motivation: 遥感领域需要扩展标注数据集，但可控生成的语义掩码控制复杂性和采样质量不确定性限制了合成数据在下游语义分割任务中的效用。

Method: 1) 基于DiT的多模态扩散变换器(MM-DiT)采用文本-图像-掩码联合注意力机制；2) 控制-校正流匹配(CRFM)方法，在早期高可塑性阶段通过语义损失动态调整采样方向；3) 任务反馈引导的即插即用采样策略。

Result: 在少样本和复杂场景下显著提升遥感语义分割数据合成效果，比现有可控生成方法表现更优，生成更稳定、面向任务的合成数据。

Conclusion: TODSynth框架通过改进的控制方案和任务导向的采样策略，有效解决了遥感语义分割数据合成的挑战，为下游任务提供了高质量的合成数据。
...
TL;DR: TreeNet提出了一种基于二叉树结构的低复杂度图像压缩模型，在低比特率下比JPEG AI提升4.83% BD-rate，同时减少87.82%模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 学习型图像压缩方法虽然性能优越，但计算复杂度高限制了其广泛应用。需要开发既保持高性能又显著降低复杂度的解决方案。

Method: 采用二叉树结构的编码器-解码器架构实现高效表示和重建，使用注意力特征融合机制整合多分支特征。

Result: 在三个基准数据集上评估，低比特率下比JPEG AI平均提升4.83% BD-rate，模型复杂度降低87.82%。通过消融研究深入分析了潜在表示对重建的影响。

Conclusion: TreeNet成功实现了高性能与低复杂度的平衡，为学习型图像压缩的实际应用提供了有效解决方案。
...
TL;DR: Make-It-Poseable是一个新颖的前馈框架，将3D角色摆姿重新定义为潜在空间变换问题，通过操纵潜在表示而非直接变形网格顶点来实现高质量摆姿。
...
Method: 1. 将角色摆姿重新定义为潜在空间变换问题；2. 使用潜在摆姿变换器基于骨骼运动操纵形状标记；3. 采用密集姿态表示实现精确控制；4. 引入潜在空间监督策略和自适应补全模块确保高保真几何和拓扑变化适应。

Result: 该方法在摆姿质量上表现出优越性能，并自然扩展到3D编辑应用，如部件替换和精炼。

Conclusion: Make-It-Poseable通过潜在空间变换方法有效解决了传统3D角色摆姿方法的局限性，实现了高质量、鲁棒的摆姿效果，并具有良好的扩展性。
...
TL;DR: FlowDet：首个使用条件流匹配技术进行目标检测的方法，相比基于扩散的方法学习更简单、更直的传输路径，在推理步骤增加时检测性能提升更快。


<details>
  <summary>Details</summary>
Motivation: 将目标检测重新表述为生成式传输问题，超越扩散模型的随机传输路径，寻求更简单、更直的路径以实现更快的性能扩展。

Method: 采用现代条件流匹配技术，将目标检测框空间中的生成问题从扩散去噪推广到更广泛的生成传输问题，保持可变框数量和推理步骤的特性。

Result: 在COCO和LVIS数据集上分别比DiffusionDet提升+3.6% AP和+4.2% AP$_{rare}$，在多种特征骨干网络和数据集上均优于基于扩散的检测系统和非生成式基线。

Conclusion: 流匹配为生成式目标检测提供了更优的传输路径，在召回约束设置下特别有效，无需大量提议框即可展示生成传输的优势。
...
Method: 采用端到端架构，将多模态输入处理为统一的表示。构建了全面的数据系统作为多模态视频创作的基础。通过高效的大规模预训练策略和推理基础设施优化来增强框架能力。

Result: 综合评估显示Kling-Omni在上下文生成、基于推理的编辑和多模态指令跟随方面表现出卓越能力。能够创作电影质量的智能视频内容。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集存在局限性：真实数据集缺乏精确标注，合成数据集缺乏真实车辆运动轨迹。需要结合两者优势，为交通分析和道路安全研究提供更好的数据支持。

Method: 创建合成3D环境，并从SinD（无人机拍摄的鸟瞰数据集）中提取真实世界轨迹，将这些真实轨迹集成到合成环境中，生成既具有精确标注又包含真实车辆运动的数据集。

Result: R3ST数据集填补了合成数据与真实轨迹之间的空白，提供了准确的多模态地面实况标注和真实的人类驾驶车辆轨迹，推动了道路车辆轨迹预测的研究。

Conclusion: R3ST通过结合合成环境的标注优势和真实轨迹的运动真实性，为计算机视觉模型的训练和评估提供了更高质量的数据集，特别是在轨迹预测领域具有重要价值。
...
TL;DR: KineST是一个基于运动学引导状态空间模型的全身体姿重建方法，通过双向扫描和混合时空表示学习，在轻量级框架下实现高精度和时序一致性的运动重建。


<details>
  <summary>Details</summary>
Motivation: AR/VR应用中，基于头戴设备稀疏信号重建真实多样的全身体姿具有挑战性。现有方法计算成本高或需分别建模时空依赖，难以平衡精度、时序一致性和效率。

Method: 提出KineST模型：1) 在状态空间对偶框架中引入运动学引导的双向扫描策略，嵌入运动学先验；2) 采用混合时空表示学习紧密耦合时空上下文；3) 引入几何角速度损失施加物理约束。

Result: 大量实验表明KineST在轻量级框架下，在精度和时序一致性方面均表现出优越性能。

Conclusion: KineST通过运动学引导的状态空间模型有效解决了AR/VR场景中基于稀疏信号的全身体姿重建问题，在精度、平滑性和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型虽然在机器人操作中表现出良好的泛化能力，但主要基于反应式方法和2D中心化设计，在需要精确3D推理的任务中可靠性不足。为了解决这一问题，需要开发能够进行3D几何推理的增强框架。

Method: GeoPredict包含两个核心模块：1) 轨迹级模块，编码运动历史并预测机器人手臂的多步3D关键点轨迹；2) 预测性3D高斯几何模块，预测工作空间几何，并沿未来关键点轨迹进行轨迹引导的精炼。这些模块仅作为训练时的监督信号，通过基于深度的渲染实现，推理时只需轻量级的额外查询令牌，无需任何3D解码。

Result: 在RoboCasa Human-50、LIBERO和真实世界操作任务上的实验表明，GeoPredict在几何密集和空间要求高的场景中，持续优于强大的VLA基线模型。

Conclusion: GeoPredict通过整合预测性运动学和几何先验，成功增强了VLA模型的3D推理能力，在复杂机器人操作任务中实现了更可靠和精确的性能，特别是在需要深度空间理解的任务中。
...
TL;DR: 提出DenseBEV方法，使用BEV特征单元直接作为锚点，通过两阶段锚点生成和BEV-NMS解决多查询注意力扩展问题，在nuScenes和Waymo数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有BEV-based transformer多使用随机查询作为锚点，或依赖辅助网络检测。本文提出更直观高效的方法，直接使用BEV特征单元作为锚点，避免额外计算开销。

Method: 1) 使用BEV特征单元直接作为对象查询锚点；2) 提出两阶段锚点生成方法；3) 引入BEV-based NMS解决大量查询的注意力扩展问题；4) 集成混合时序建模，利用先验检测增强性能。

Result: 在nuScenes数据集上NDS和mAP显著提升，小物体检测效果明显（行人检测mAP提升3.8%）；在Waymo数据集上达到SOTA性能，LET-mAP 60.7%，比之前最佳提升5.4%。

Conclusion: DenseBEV通过直接使用BEV特征作为锚点，结合两阶段锚点生成和BEV-NMS，实现了高效的多摄像头3D目标检测，在稀疏BEV网格下仍能取得优异性能，特别是在小物体检测方面表现突出。
...
TL;DR: 该研究评估了YOLOv8不同变体在车牌识别和字符识别任务上的性能，提出了一个优化管道：使用YOLOv8 Nano进行车牌检测，YOLOv8 Small进行字符识别，并引入基于x轴位置的字符排序方法，实现了高精度和计算效率的平衡。


<details>
  <summary>Details</summary>
Motivation: 在智能交通系统中，车牌识别是关键技术，但现有方法在多样化环境中的实时准确性和一致性仍存在问题。需要开发既准确又高效的车牌识别系统，特别是针对边缘设备的部署。

Method: 使用两个独立数据集分别训练和评估YOLOv8不同变体（Nano和Small）在车牌识别和字符识别任务上的性能。引入基于x轴位置的字符排序方法，并提出了一个优化管道：YOLOv8 Nano用于车牌检测，YOLOv8 Small用于字符识别。

Result: YOLOv8 Nano在车牌识别任务上达到0.964的精确度和0.918的mAP50；YOLOv8 Small在字符识别任务上达到0.92的精确度和0.91的mAP50。提出的优化管道在保持计算效率的同时确保了高精度。

Conclusion: 该研究提出的YOLOv8 Nano和Small组合管道为智能交通系统中的车牌识别提供了高效准确的解决方案，特别适合边缘设备部署，为更智能高效的城市基础设施建设奠定了基础。
...
TL;DR: 提出一种紧凑的图像到文本架构，仅使用单张正面X光图像生成胸部X光报告的发现部分，通过解剖注意力机制提升临床相关区域的关注度，在资源受限环境下实现显著性能提升。
...
Method: 结合冻结的DINOv3 ViT编码器和GPT-2解码器，通过分层解剖注意力机制增强。该机制利用肺部和心脏分割掩码，通过分层高斯平滑引导注意力到临床相关区域，不增加可训练参数。模型仅使用单张正面胸部X光图像生成报告发现部分。

Result: 在MIMIC-CXR数据集上评估，5种关键病理的CheXpert Macro-F1提升168%（0.083→0.238），Micro-F1提升146%（0.137→0.337）。14种观察结果的性能提升86%（0.170→0.316）。结构连贯性方面，RadGraph F1提升9.7%。模型虽小且仅依赖图像，但解剖引导显著改善了空间定位和临床相关区域的连贯性。

Conclusion: 解码器级别的解剖引导能够有效改善空间定位和临床相关区域的连贯性，即使在小规模、纯图像条件下也能实现显著性能提升。这为资源受限环境下的自动放射学报告生成提供了可行的解决方案。
...
TL;DR: OpenTouch是首个野外环境下第一人称全手触觉数据集，包含5.1小时同步的视频-触觉-姿态数据，用于研究视觉感知与物理交互的联系。


<details>
  <summary>Details</summary>
Motivation: 尽管人手是我们与物理世界的主要交互界面，但第一人称感知很少知道何时、何地以及如何施加接触力。目前缺乏鲁棒的穿戴式触觉传感器，也没有野外数据集能对齐第一人称视频与全手触觉信息。

Method: 创建OpenTouch数据集，包含5.1小时同步的视频-触觉-姿态数据，以及2,900个精选片段和详细文本标注。基于该数据集建立了检索和分类基准任务，探索触觉如何支撑感知和行动。

Result: 触觉信号为抓握理解提供了紧凑而强大的线索，增强了跨模态对齐，并且可以从野外视频查询中可靠地检索。数据集和基准的发布将推动多模态第一人称感知、具身学习和接触丰富的机器人操作。

Conclusion: OpenTouch填补了视觉感知与物理交互之间的空白，通过提供首个野外环境下的同步视频-触觉-姿态数据集，为多模态感知和机器人操作研究提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 自动化文本到图像模型评估面临挑战：需要评分模型判断正确性，测试提示需要足够挑战性但又不超出评分模型能力。这可能导致基准随时间漂移，静态基准无法跟上新模型能力的发展。

Method: 1) 分析GenEval基准的漂移问题；2) 通过大规模人类研究验证基准饱和；3) 提出新基准GenEval 2，改进视觉基元概念覆盖和组合性；4) 提出Soft-TIFA评估方法，结合视觉基元判断，更符合人类判断且不易漂移。

Result: GenEval基准已严重漂移，与人类判断的绝对误差高达17.7%，表明基准已饱和。GenEval 2对当前模型更具挑战性，Soft-TIFA评估方法更符合人类判断且抗漂移性更强。

Conclusion: 基准漂移是文本到图像模型评估的重要问题，需要持续审计和改进。GenEval 2和Soft-TIFA提供了更好的解决方案，但避免基准漂移仍需持续努力。
...
Conclusion: RePlan通过规划-执行框架有效解决了复杂指令图像编辑问题，在区域定位精度和编辑保真度方面表现优异，为指令-视觉复杂场景下的图像编辑提供了有效解决方案
...
Method: 1) 提出仅对抗训练范式，消除不可靠的逐像素不可感知性损失；2) 引入三阶段训练计划，通过解耦鲁棒性和不可感知性来稳定收敛；3) 通过高分辨率适应解决分辨率差距，采用基于JND的衰减和训练时推理模拟来消除上采样伪影；4) 通过时间水印池化适应视频。

Result: Pixel Seal在不同图像类型和广泛变换下表现出卓越的鲁棒性和不可感知性，明显优于现有最先进方法。模型能有效适应视频，为实际图像和视频场景提供了实用且可扩展的可靠溯源解决方案。

Conclusion: Pixel Seal通过创新的训练范式、稳定的训练计划和有效的高分辨率适应技术，为图像和视频水印设立了新的最先进标准，解决了现有方法的根本问题，提供了实用且可扩展的数字内容溯源解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中手术器械的准确分割对于计算机辅助干预至关重要，但由于频繁遮挡、快速运动、镜面伪影和长期器械重新进入等挑战，现有方法性能有限。SAM3虽然提供了强大的时空框架，但在手术场景中存在内存更新不区分、内存容量固定和遮挡后身份恢复能力弱等问题。

Method: 提出了ReMeDI-SAM3，包含三个核心组件：1）相关性感知内存过滤，配备专门的遮挡感知内存存储遮挡前帧；2）分段插值方案扩展有效内存容量；3）基于特征的重新识别模块，通过时间投票实现可靠的遮挡后身份消歧。该方法无需额外训练。

Result: 在EndoVis17和EndoVis18数据集上的零样本评估显示，相比原始SAM3分别获得约7%和16%的绝对mcIoU提升，甚至超越了先前需要训练的方法。

Conclusion: ReMeDI-SAM3通过增强内存管理和身份恢复机制，有效解决了手术器械分割中的遮挡问题，在零样本设置下显著提升了分割性能，为计算机辅助手术提供了更可靠的工具。
...
TL;DR: 提出M-PhyGs方法，从视频中估计复杂自然物体（以花朵为例）的材料组成和物理参数，解决了现有方法对单材料、预学习动力学或简单拓扑的限制。


<details>
  <summary>Details</summary>
Motivation: 现实世界物体通常具有复杂的材料组成和几何形状，而现有方法假设物体为单一材料、需要预学习动力学或只能处理简单拓扑，无法准确估计真实物体的物理材料参数。

Method: 提出Multi-material Physical Gaussians (M-PhyGs)方法，通过新引入的级联3D和2D损失函数，结合时间小批量处理，从短视频中联合分割物体为相似材料区域并恢复其连续介质力学参数，同时考虑重力影响。

Result: 在提出的Phlowers数据集（人们与花朵互动的视频）上实验，证明了M-PhyGs方法及其组件在多材料物理参数估计这一挑战性任务中的准确性和有效性。

Conclusion: M-PhyGs能够从自然场景的短视频中准确估计复杂自然物体的材料组成和物理参数，为多材料物理参数估计提供了有效解决方案。
...
TL;DR: LinkedOut：首个基于VLLM的视频推荐方法，直接从原始帧提取世界知识表示，无需人工标注，实现快速推理和多视频输入支持


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（VLLMs）在下游任务如视频推荐中面临挑战：解码生成延迟高、不支持多视频输入、语言输出丢弃视觉细节。需要一种能保留像素级细节同时利用世界知识的表示方法

Method: LinkedOut从原始帧提取语义基础、知识感知的token，使用可提示查询和可选辅助模态。引入跨层知识融合MoE，从丰富的VLLM特征中选择适当的抽象层次，实现个性化、可解释、低延迟推荐

Result: 在标准基准测试中达到最先进结果，是首个无需手工标签、基于原始帧的VLLM视频推荐方法。可解释性研究和消融实验证实了层多样性和分层融合的好处

Conclusion: LinkedOut为充分利用VLLM世界知识先验和视觉推理进行下游视觉任务（如推荐）提供了实用路径，解决了现有VLLM在部署中的关键限制
...
TL;DR: 本文提出Instant4D，通过从2D扩散模型蒸馏知识到前馈编码器，实现单图到3D一致、快速且富有表现力的可动画表示，达到107.31 FPS的高帧率


<details>
  <summary>Details</summary>
Motivation: 现有2D肖像动画方法缺乏3D一致性且速度慢，而3D面部动画方法虽保证3D一致性和快速推理，但表情细节不足。需要结合两者优势，实现既保持3D一致性又富有表情细节的快速动画

Method: 1) 从2D扩散模型蒸馏知识到前馈编码器；2) 动画表示与3D表示解耦，从数据中隐式学习运动；3) 采用轻量级局部融合策略而非计算密集的全局融合机制；4) 消除对预定义参数模型的依赖

Result: 方法达到107.31 FPS的动画和姿态控制速度，动画质量与最先进方法相当，在速度和质量之间取得良好平衡

Conclusion: Instant4D成功结合了2D扩散模型的表达能力和3D方法的效率与一致性，实现了高质量、快速、3D一致的肖像动画，适用于数字孪生和远程呈现等实际应用
...
TL;DR: FlashPortrait：一种端到端的视频扩散Transformer，能生成保持身份一致性的无限长度肖像动画，推理速度提升6倍
...
Method: 1. 使用现成提取器计算身份无关的面部表情特征；2. 引入归一化面部表情块对齐面部特征与扩散潜变量；3. 采用动态滑动窗口方案和加权融合确保平滑过渡；4. 基于潜变量变化率和导数幅度比，使用高阶潜变量导数直接预测未来时间步潜变量，跳过多个去噪步骤

Result: 在基准测试中，FlashPortrait在质量和数量上都表现出有效性，能生成身份保持的无限长度视频，推理速度提升高达6倍
...
TL;DR: Alchemist是一个基于元梯度的自动数据选择框架，用于提升文本到图像生成模型的训练效率和质量，通过数据评分和剪枝两阶段方法从大规模文本-图像对中筛选出最有价值的子集。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型（如Imagen、Stable Diffusion、FLUX）的性能受限于训练数据质量。网络爬取和合成的图像数据集常包含低质量或冗余样本，导致视觉保真度下降、训练不稳定和计算效率低下。现有方法依赖昂贵的人工筛选或基于单维特征的启发式评分，而元学习方法在LLM中已有探索但未适配图像模态。

Method: Alchemist采用两阶段框架：1) 数据评分：训练轻量级评分器，基于梯度信息评估每个样本的影响力，增强多粒度感知；2) 数据剪枝：使用Shift-G采样策略选择信息丰富的子集用于高效模型训练。这是首个面向文本到图像模型训练的自动、可扩展、基于元梯度的数据选择框架。

Result: 在合成和网络爬取数据集上的实验表明，Alchemist能持续提升视觉质量和下游性能。使用Alchemist选择的50%数据进行训练，其效果可优于使用完整数据集训练的结果。

Conclusion: Alchemist为解决文本到图像生成模型训练中的数据质量问题提供了有效的自动解决方案，通过元梯度方法实现了高效的数据选择，显著提升了模型训练效率和质量。
...
TL;DR: VIVA是一个基于指令的视频编辑框架，通过VLM引导编码和奖励优化来解决现有方法泛化能力不足的问题，在指令遵循、泛化能力和编辑质量方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频编辑方法通常在简单编辑操作的配对数据上训练，这限制了它们对多样复杂真实世界指令的泛化能力，需要解决这个泛化差距。

Method: 1) 引入基于VLM的指导器，将文本指令、源视频首帧和可选参考图像编码为视觉基础指令表示；2) 提出后训练阶段Edit-GRPO，将组相对策略优化应用于视频编辑领域；3) 设计数据构建流程生成多样高质量的基础编辑操作配对视频-指令数据。

Result: 大量实验表明，VIVA在指令遵循、泛化能力和编辑质量方面优于最先进的方法。

Conclusion: VIVA通过VLM引导编码和奖励优化的可扩展框架，成功解决了指令视频编辑中的泛化问题，实现了对复杂真实世界指令的有效处理。
...
Method: 1）创建EgoMAN数据集：大规模第一人称数据集，包含219K 6DoF轨迹和3M结构化QA对，用于语义、空间和运动推理；2）提出EgoMAN模型：推理到运动框架，通过轨迹令牌接口连接视觉语言推理和运动生成，采用渐进式训练对齐推理与运动动态

Result: 该方法能够生成准确且具有阶段感知的轨迹，并在真实世界场景中展现出良好的泛化能力

Conclusion: 通过结合语义推理和运动生成，EgoMAN框架解决了现有方法的局限性，实现了更准确、更具阶段感知的3D手部轨迹预测
...
<details>
  <summary>Details</summary>
Motivation: 解决在不同时间拍摄的同一场景图像/视频中检测物体添加、移除或移动的问题，这对机器人整理、施工进度监控等应用很重要。主要挑战是视角变化可能导致物体被错误检测为变化

Method: 提出SceneDiff方法：1）在3D中对齐捕获场景；2）提取物体区域；3）比较空间和语义区域特征来检测变化。该方法利用预训练的3D、分割和图像编码模型，无需额外训练

Result: 在多视角和双视角基准测试中大幅超越现有方法，相对AP提升分别达到94%和37.4%。建立了包含350个多样化视频对、数千个变化物体的SceneDiff Benchmark

Conclusion: 提出的SceneDiff Benchmark填补了多视角变化检测基准的空白，SceneDiff方法通过3D对齐和特征比较有效解决了视角变化带来的挑战，在多个测试中表现出色，代码和基准将公开
...
TL;DR: MomaGraph：用于家庭移动操作机器人的统一场景图表示，包含数据集、基准测试和7B视觉语言模型，在任务规划中达到SOTA效果


<details>
  <summary>Details</summary>
Motivation: 家庭移动操作机器人需要同时导航和操作，现有场景表示方法存在以下问题：1）分离空间和功能关系；2）将场景视为静态快照，缺乏对象状态和时序更新；3）忽略当前任务最相关的信息

Method: 提出MomaGraph统一场景表示，整合空间功能关系和部件级交互元素；创建MomaGraph-Scenes大规模数据集和MomaGraph-Bench评估套件；开发MomaGraph-R1（7B视觉语言模型），通过强化学习训练，采用Graph-then-Plan框架进行零样本任务规划

Result: MomaGraph-R1在基准测试中达到71.6%准确率（比最佳基线提升11.4%），在开源模型中达到SOTA效果，能泛化到公共基准测试并有效迁移到真实机器人实验

Conclusion: MomaGraph为具身智能体提供了统一的场景表示方法，通过数据集、基准测试和模型的三位一体框架，显著提升了家庭环境中移动操作机器人的任务规划和场景理解能力
...
<details>
  <summary>Details</summary>
Motivation: 当前离散分词器在图像重建质量上仍落后于连续分词器，限制了其在多模态系统中的采用。需要开发既能保持计算效率又能提升重建精度的离散分词方法。

Method: 提出SFTok离散分词器，采用多步迭代重建机制，结合自强制引导视觉重建和去偏拟合训练策略，解决多步过程中的训练-推理不一致问题。

Result: 在ImageNet上达到rFID=1.21的最优重建质量，在类别到图像生成任务中取得gFID=2.29的优异表现，仅用64个token实现高压缩率。

Conclusion: SFTok通过创新的训练策略和多步迭代机制，显著提升了离散分词器的图像重建能力，为高效的多模态图像生成系统提供了有力工具。
...
TL;DR: 提出了一个全景度量深度基础模型，通过数据循环范式结合大规模数据集和伪标签处理，采用DINOv3-Large骨干网络和多种优化技术，在多个基准测试中表现出色并具有零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度估计方法在处理不同场景距离（室内/室外、合成/真实数据）时存在泛化能力不足的问题，需要开发一个能够跨场景距离泛化的全景度量深度基础模型。

Method: 采用数据循环范式，结合公开数据集、UE5模拟器合成数据、文本到图像模型生成数据和网络真实全景图像构建大规模数据集；通过三阶段伪标签处理减少领域差距；使用DINOv3-Large作为骨干网络，引入即插即用的范围掩码头、锐度中心优化和几何中心优化技术。

Result: 在Stanford2D3D、Matterport3D和Deep360等多个基准测试中表现出强大的性能，特别是在不同真实场景中具有鲁棒和稳定的度量预测能力，展示了优秀的零样本泛化能力。

Conclusion: 提出的全景度量深度基础模型通过创新的数据循环范式和模型设计，成功实现了跨场景距离的泛化，为全景深度估计提供了有效的解决方案。
...
TL;DR: 提出UniStereo数据集和StereoPilot模型，用于单目到立体视频转换，无需显式深度图或迭代扩散采样，支持不同立体格式并提升视觉质量和计算效率。


<details>
  <summary>Details</summary>
Motivation: 立体显示设备快速增长，但高质量立体视频制作成本高、复杂度大。传统的多阶段"深度-变形-修复"流水线存在误差传播、深度模糊和立体格式不一致等问题。

Method: 1) 创建UniStereo数据集，统一覆盖平行和汇聚两种立体格式；2) 提出StereoPilot前馈模型，直接合成目标视图，无需显式深度图或迭代扩散采样；3) 使用可学习域切换器和循环一致性损失，适应不同立体格式并提升一致性。

Result: StereoPilot在视觉保真度和计算效率方面显著优于现有最先进方法，能够无缝适应不同立体格式并实现更好的视觉一致性。

Conclusion: 通过UniStereo数据集和StereoPilot模型，解决了传统立体视频转换方法的局限性，为高质量、高效的立体视频生成提供了有效解决方案。
...
TL;DR: 提出AdaTooler-V多模态大语言模型，通过自适应工具使用机制，仅在视觉问题真正需要时调用视觉工具，避免不必要的工具调用开销和性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有开源多模态大语言模型存在盲目工具使用问题，即使不需要视觉工具也会调用，这增加了推理开销并降低了模型性能。需要一种自适应工具使用机制来优化工具调用决策。

Method: 1. 提出AT-GRPO强化学习算法，基于每个样本的工具效益分数自适应调整奖励尺度，鼓励模型仅在工具能真正提升性能时调用。2. 构建两个训练数据集：AdaTooler-V-CoT-100k用于SFT冷启动，AdaTooler-V-300k用于带可验证奖励的强化学习训练。

Result: 在12个基准测试中表现出强大的推理能力，在多样视觉推理任务中超越现有方法。AdaTooler-V-7B在V*高分辨率基准上达到89.8%准确率，超过商业专有模型GPT-4o和Gemini 1.5 Pro。

Conclusion: AdaTooler-V通过自适应工具使用机制有效解决了多模态大语言模型中盲目工具调用问题，在保持高性能的同时显著降低推理开销，为高效多模态推理提供了新方案。
...
<details>
  <summary>Details</summary>
Motivation: 目前自动驾驶领域缺乏能够适应不同场景和相机配置的密集几何感知模型。传统方法依赖精确相机参数，限制了灵活性和适应性。

Method: 使用DINO骨干网络提取视觉特征，通过交替的视图内局部注意力、跨视图空间注意力和跨帧时间注意力推断图像间的几何关系，然后解码生成第一帧自车坐标系下的全局点云地图和每帧的自车姿态。

Result: DVGT在nuScenes、OpenScene、Waymo、KITTI和DDAD等多个驾驶数据集上训练，在各种场景下显著优于现有模型，能够直接预测度量尺度的几何信息。
...
TL;DR: EasyV2V是一个简单有效的基于指令的视频编辑框架，通过数据构建、架构简化和控制统一三方面创新，实现了灵活输入和SOTA视频编辑效果
...
Method: 1) 数据方面：组合现有专家与快速逆变换构建多样视频对，通过单帧监督和共享仿射运动将图像编辑对提升为视频，挖掘密集标注的视频片段，添加过渡监督；2) 模型方面：利用预训练文本到视频模型的编辑能力，采用简单序列连接和轻量LoRA微调；3) 控制方面：通过单一掩码机制统一时空控制，支持可选参考图像

Result: EasyV2V在视频编辑方面达到最先进水平，超越了同期和商业系统，支持灵活输入格式（视频+文本、视频+掩码+文本、视频+掩码+参考图像+文本）

Conclusion: 通过系统探索数据、架构和控制的设计空间，EasyV2V提供了一个简单而强大的视频编辑框架，解决了视频编辑中的关键挑战，实现了高质量、灵活的视频编辑效果
...
TL;DR: AuditDM：通过强化学习训练审计员模型，自动发现并修复多模态大语言模型的失败模式，显著提升模型性能


<details>
  <summary>Details</summary>
Motivation: 传统多模态大语言模型评估方法缺乏可解释性，且不足以充分揭示模型间的能力差距。需要一种能够主动发现模型失败模式并用于改进的自动化框架。

Method: 通过强化学习微调一个MLLM作为审计员，生成能够最大化目标模型间分歧的挑战性问题和反事实图像。训练后的审计员能够发现多样化的、可解释的失败案例。

Result: 在Gemma-3和PaliGemma-2等SOTA模型上发现了20多种不同的失败类型。基于这些发现进行微调后，所有模型在16个基准测试中均得到一致提升，甚至使3B模型超越了其28B对应模型。

Conclusion: 随着数据扩展收益递减，有针对性的模型审计为模型诊断和改进提供了有效途径。AuditDM框架能够自动发现模型弱点并生成无需标注的改进数据。
...
TL;DR: 提出NEPA方法，通过预测未来图像块嵌入而非像素重建，实现视觉自监督学习，在ImageNet上达到83.8%-85.3%准确率


<details>
  <summary>Details</summary>
Motivation: 受自然语言生成式预训练成功启发，探索是否相同原则能产生强大的视觉自监督学习器，从学习表示转向学习模型
...
<details>
  <summary>Details</summary>
Motivation: 景深控制在摄影中很重要，但获得完美对焦通常需要多次尝试或特殊设备。当前单图像重对焦方法存在显著缺陷：需要全焦输入、依赖模拟器合成数据、光圈控制有限。

Method: 提出两阶段Generative Refocusing方法：1) DeblurNet从各种输入恢复全焦图像；2) BokehNet生成可控散景。采用半监督训练，结合合成配对数据和未配对的真实散景图像，利用EXIF元数据捕捉真实光学特性。

Result: 在散焦去模糊、散景合成和重对焦基准测试中达到最佳性能。Generative Refocusing还支持文本引导调整和自定义光圈形状。

Conclusion: 提出的Generative Refocusing方法通过创新的半监督训练策略，有效解决了单图像重对焦的挑战，实现了高质量、可控的散景效果，超越了现有方法的局限性。
...
TL;DR: WorldCanvas是一个多模态框架，通过结合文本、轨迹和参考图像，实现用户可提示的丰富世界事件模拟，支持多智能体交互、对象进出、参考引导外观和反直觉事件生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：纯文本方法表达能力有限，现有轨迹控制的图像到视频方法缺乏语义意图和视觉基础。需要一种能够生成连贯、可控世界事件的方法，将世界模型从被动预测器转变为交互式、用户可塑造的模拟器。

Method: 采用多模态方法，结合三种输入：轨迹（编码运动、时序和可见性）、自然语言（表达语义意图）和参考图像（为对象身份提供视觉基础）。这种组合使得能够生成包含多智能体交互、对象进出、参考引导外观和反直觉事件的连贯可控事件。

Result: 生成的视频不仅具有时间连贯性，还表现出涌现一致性，即使在对象暂时消失的情况下也能保持对象身份和场景一致性。框架能够支持丰富表达的世界事件生成。

Conclusion: WorldCanvas通过将文本、轨迹和参考图像相结合，实现了用户可提示的丰富世界事件模拟，将世界模型从被动预测器推进为交互式、用户可塑造的模拟器，为可控视频生成提供了新方向。
...
TL;DR: TabReX：一个基于图推理的无参考表格生成评估框架，通过知识图谱对齐和可解释的评分机制，提供结构化评估和错误追踪


<details>
  <summary>Details</summary>
Motivation: 现有表格生成评估方法存在局限：要么将表格扁平化为文本忽略结构信息，要么依赖固定参考限制泛化能力。需要一种无参考、能捕捉结构特性的评估框架

Method: 将源文本和生成表格转换为规范知识图谱，通过LLM引导的匹配过程对齐，计算可解释的、基于评分标准的分数，量化结构和事实保真度

Result: TabReX在专家排名相关性上表现最佳，在更难的扰动下保持稳定，支持细粒度的模型与提示分析，建立了结构化生成系统可信可解释评估的新范式

Conclusion: TabReX提供了一个可控的敏感性与特异性权衡的评估框架，通过TabReX-Bench基准验证了其鲁棒性，为结构化生成系统的评估建立了新标准
...
TL;DR: SocialStoryFrames：一种基于对话上下文和叙事理论的读者响应形式化框架，包含生成和分类模型，用于大规模分析社交媒体故事


<details>
  <summary>Details</summary>
Motivation: 当前计算模型对读者在阅读故事时的丰富解释、情感和评价响应（如叙事意图推断、角色判断）建模能力有限，需要更细致的分析方法
...
Result: 通过382名参与者的人类调查和专家标注验证模型有效性，展示了该框架在分析不同社区叙事实践多样性、故事意图频率和相互依赖性方面的应用价值

Conclusion: SocialStoryFrames通过将细粒度、上下文敏感的建模与通用的读者响应分类法相结合，为在线社区叙事研究提供了新的研究方法
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在性能、成本和token使用之间存在非线性关系，需要更高效的推理框架来优化自主代理系统的推理效率

Method: 提出BRAID（有界推理框架），使用基于Mermaid的指令图，让模型进行结构化推理而非无限制的自然语言token扩展

Result: 在AdvancedIF、GSM-Hard和SCALE MultiChallenge基准数据集上，结构化机器可读提示显著提高了推理准确性和成本效率

Conclusion: BRAID是一种有效且可扩展的技术，可用于优化自主代理系统中的推理效率
...
TL;DR: 研究探讨如何利用个人信息（如人口统计、态度、关系、经历）来预测标注者对社交规范的判断，发现人口统计信息最具影响力，少量相关评论即可，且多样化的标注者样本效果最佳。


<details>
  <summary>Details</summary>
Motivation: 以往研究使用个人信息（如人物描述或自我披露）来改进个体特征建模和主观任务标注预测，但对何种信息最有效缺乏深入探索。本研究旨在分析不同类型自我披露信息对预测标注者社交规范判断的影响。

Method: 将自我披露句子分类，构建标注者模型预测社交规范判断。进行多项消融实验和分析，考察不同类型信息（人口统计、态度、关系、经历）对预测标注模式的影响，比较理论方法与自动聚类效果。

Result: 人口统计信息比态度、关系和经历更具影响力；理论方法优于自动聚类；与先前研究不同，仅需少量相关评论即可；多样化的标注者自我披露样本带来最佳性能。

Conclusion: 在预测标注者社交规范判断时，应优先考虑人口统计信息，采用理论驱动的分类方法，收集少量但多样化的标注者自我披露数据，而非依赖大量自动聚类信息。
...
TL;DR: Sage是一个无需人工标注的LLM-as-a-Judge评估套件，通过局部自一致性和全局逻辑一致性来评估LLM评判的可靠性，发现当前顶级LLM在近四分之一困难案例中存在偏好不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-Judge基准主要依赖人工标注的真实标签，这引入了人类偏见，影响可靠性评估并限制了可扩展性。需要一种无需人工标注的方法来评估LLM评判的质量。

Method: 基于理性选择理论公理，提出两个新评估维度：局部自一致性（成对偏好稳定性）和全局逻辑一致性（完整偏好集的传递性）。构建了650个问题的数据集，结合结构化基准问题和真实用户查询。

Result: 实验表明Sage指标稳定且与监督基准（如LLMBar和RewardBench2）高度相关。当前最先进的LLM（包括Gemini-2.5-Pro和GPT-5）在近四分之一困难案例中无法保持一致的偏好。发现情境偏好现象，并验证微调LLM-as-a-Judge、基于面板的评判和深度推理能提升一致性。

Conclusion: Sage提供了一个无需人工标注的可靠评估框架，揭示了当前LLM评判的显著可靠性问题。人类判断也存在实质性不一致，表明人工标注可能不是可靠的金标准。明确规则或标准能帮助模型在不同答案对中保持一致的评判。
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络在捕捉文本局部、位置不变特征方面很成功，但其建模语言中复杂变换的能力仍有探索空间。李群操作能够捕捉复杂的非欧几里得对称性，这为语言建模提供了新的可能性。

Method: 提出了两种模型SCLie和DPCLie，将李卷积集成到基于卷积的句子分类器中。利用李群操作来捕捉语言中不常见的复杂变换和非欧几里得对称性。

Result: SCLie和DPCLie模型在实验上超越了传统的基于卷积的句子分类器。李基模型通过捕捉语言中不常见的变换，相对提高了分类准确率。

Conclusion: 研究结果表明李卷积在语言建模中具有潜力，能够捕捉传统方法难以处理的复杂变换。这为语言建模的新范式探索提供了动机。
...
TL;DR: 该论文提出了一种语义驱动的强化学习方法（SRL），用于医学报告生成，通过优化报告级别的临床正确性奖励而非传统的词元级监督，显著提升了生成报告的临床准确性。


<details>
  <summary>Details</summary>
Motivation: 现有医学报告生成方法通常模仿放射科医生的语言风格，但无法保证临床正确性，因为它们基于词元级目标训练，关注词汇选择和句子结构而非实际的医学准确性。

Method: 提出语义驱动强化学习（SRL）方法，采用组相对策略优化（GRPO）来鼓励临床正确性导向的学习。优化报告级奖励：基于边缘的余弦相似度（MCCS），计算生成报告和参考报告中提取的关键放射学发现之间的相似性。还引入轻量级推理格式约束，指导模型生成结构化的"思考报告"输出。

Result: 在IU X-Ray数据集上达到CE-F1 51.88，在MIMIC-CXR数据集上达到40.39，实现了最先进的性能。研究发现标签语义强化优于传统的词元级监督。

Conclusion: 优化基于临床的报告级奖励而非词元重叠，能显著改善临床正确性。这项工作是在医学大型视觉语言模型（Med-LVLM）训练中探索语义强化监督医学正确性的先驱。
...
TL;DR: 本文介绍了针对Faux-Hate共享任务的系统，该任务旨在检测由虚假叙事驱动的仇恨言论，特别是在印地语-英语混合社交媒体文本中。系统处理两个子任务：二元Faux-Hate检测（虚假和仇恨言论分类）以及目标和严重性预测。


<details>
  <summary>Details</summary>
Motivation: 社交媒体平台已成为有害内容（包括仇恨言论和虚假叙事）快速传播的中心。Faux-Hate现象特指由虚假叙事驱动的仇恨言论生成，需要专门技术来检测这种复杂内容，尤其是在多语言混合的社交媒体环境中。

Method: 结合先进的自然语言处理技术与领域特定的预训练方法，采用多任务学习框架来处理二元分类（虚假和仇恨言论）以及目标和严重性预测两个子任务。

Result: 系统在共享任务中取得了有竞争力的结果，证明了多任务学习方法在处理这种复杂问题上的有效性。

Conclusion: 通过结合NLP技术和领域特定预训练，多任务学习框架能够有效检测由虚假叙事驱动的仇恨言论，为社交媒体有害内容检测提供了有前景的解决方案。
...
TL;DR: 基于Qwen2.5-7B模型，通过LoRA微调和提示工程，构建了从警方通报中提取15个关键字段的信息抽取系统，在死亡检测、死亡人数和省级位置提取等任务上达到95%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 警方事件通报（特别是社交媒体上的非正式文本）的结构化信息抽取对及时准确的数据处理至关重要，但由于文本的多样性和非正式性，这一任务面临重大挑战。

Method: 开发了领域适应的抽取流程，结合针对性提示工程和基于LoRA的参数高效微调Qwen2.5-7B模型。使用从27,822条中国微博警方通报（2019-2020年）中手动标注的4,933个实例构建高质量数据集。

Result: LoRA微调显著优于基础模型和指令微调模型，死亡检测准确率超过98.36%，死亡人数精确匹配率达95.31%，省级位置提取精确匹配率达95.54%。

Conclusion: 该流程为专业领域的多任务结构化信息抽取提供了经过验证的高效解决方案，为社会科学研究中将非结构化文本转化为可靠结构化数据提供了实用框架。
...
TL;DR: 提出一个独立于LLM的事实核查模块和领域特定的摘要模型，用于减少医疗领域LLM输出的幻觉，在MIMIC-III数据集上取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 在医疗健康领域，LLM生成的输出必须可靠准确，特别是在决策和患者安全方面。然而，LLM存在幻觉风险，导致关键领域的输出不可靠。

Method: 1) 提出独立的事实核查模块，使用数值测试和自然语言处理中的离散逻辑进行细粒度逻辑检查，验证事实与电子健康记录的一致性；2) 开发领域特定的摘要模型，使用LoRa在MIMIC-III数据集上进行微调；3) 将两者结合使用。

Result: 事实核查模块在3,786个命题上达到精确率0.8904、召回率0.8234、F1分数0.8556；LLM摘要模型在摘要质量上达到ROUGE-1分数0.5797和BERTScore 0.9120。

Conclusion: 提出的独立事实核查模块和领域特定摘要模型能有效减少医疗领域LLM输出的幻觉，提高输出的可靠性和准确性，有助于医疗决策和患者安全。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在科学和社会中应用广泛，但其中的错误或过时信息会影响准确性并限制安全部署。现有编辑技术泛化能力有限，容易产生意外后果，需要开发不依赖全模型重训练的高效知识更新方法

Method: 基于信息瓶颈理论，压缩和隔离实现可泛化知识校正所需的关键信息，最小化对无关模型行为的影响。提出信息瓶颈知识编辑器（IBKE），利用紧凑的潜在表示指导基于梯度的更新
...
TL;DR: LoPA是一种无需训练、即插即用的算法，通过优化token填充顺序(TFO)来提升扩散大语言模型(dLLMs)的并行推理速度，将D2F-Dream模型的每前向传递token数(TPF)从1-3提升到10.1，同时保持性能优于基准。


<details>
  <summary>Details</summary>
Motivation: 当前扩散大语言模型(dLLMs)的置信度驱动解码策略受限于有限的并行性，通常每前向传递只能生成1-3个token。研究发现dLLM推理的并行度对token填充顺序(TFO)高度敏感，需要找到更好的TFO来加速推理。

Method: 提出Lookahead PArallel Decoding LoPA算法：1) 通过并行分支同时探索不同的候选TFO；2) 基于分支置信度选择具有最高未来并行潜力的TFO；3) 开发支持分支并行(BP)的多设备推理系统，实现多GPU部署下的高效推理。

Result: 1) LoPA将D2F-Dream在GSM8K数据集上的TPF提升到10.1，性能仍优于Dream基准；2) 多GPU部署下实现单样本吞吐量1073.9 tokens/秒；3) 代码已开源。

Conclusion: LoPA通过优化token填充顺序显著提升了扩散大语言模型的推理并行度，实现了训练免费、即插即用的加速方案，为dLLMs的高效推理提供了有效解决方案。
...
TL;DR: Sigma-MoE-Tiny是一个极稀疏的MoE语言模型，总参数200亿但仅激活5亿，通过细粒度专家分割（每层96个专家）实现最高稀疏度，解决了极端稀疏下的专家负载均衡问题。


<details>
  <summary>Details</summary>
Motivation: 混合专家（MoE）模型因其高效可扩展性成为基础模型的有前景范式，但现有开源模型的稀疏度仍有提升空间。本文旨在探索更高稀疏度的MoE架构，同时解决极端稀疏带来的专家负载均衡挑战。

Method: 采用细粒度专家分割（每层最多96个专家），每个token仅激活一个专家；提出渐进稀疏化调度策略解决负载均衡问题；在多样化高质量语料上进行预训练，然后进行后训练；使用负载均衡损失并发现其在底层失效的问题。

Result: Sigma-MoE-Tiny在仅激活5亿参数的情况下，达到了与可比或更大规模模型相媲美的顶级性能；整个训练过程非常稳定，没有出现不可恢复的损失尖峰；实现了现有开源模型中最高的稀疏度。

Conclusion: 通过渐进稀疏化调度策略，可以在极端稀疏的MoE模型中有效解决专家负载均衡问题，实现稳定训练和高性能。这项工作为未来MoE架构中推进稀疏度提供了重要见解。
...
TL;DR: 本研究比较了OpenAI的GPT模型中推理与非推理架构在芬兰语与四种低资源乌拉尔语翻译任务上的表现，发现推理模型拒绝翻译率低16个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型翻译评估主要关注高资源语言，对低资源和濒危语言的性能了解不足，特别是乌拉尔语系语言。

Method: 使用文学文本平行语料库，分析GPT模型中推理与非推理架构在芬兰语与科米-兹良语、莫克沙语、埃尔齐亚语、乌德穆尔特语翻译时的拒绝率差异。

Result: 推理模型比非推理模型的拒绝翻译率低16个百分点，显示出在低资源语言翻译任务上的显著性能优势。

Conclusion: 推理模型在低资源乌拉尔语翻译中表现更好，为濒危语言保护提供了有价值的见解，有助于理解推理模型在语言保存任务中的能力。
...
TL;DR: 该论文提出了一种寻找对抗性文本的方法，这种文本无论测试用例如何都能被评估为高质量，揭示了基于嵌入的神经文本评估指标（如COMET）的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 基于嵌入的神经文本评估指标（如COMET）在研究和开发领域被广泛使用，但由于神经网络的"黑盒"特性，无法保证其评估结果的可靠性和安全性。作者旨在揭示这类评估指标的脆弱性，提高对其可靠性和安全性的关注。

Method: 提出了一种在离散空间中寻找单个对抗性文本（hub text）的方法，这种文本无论测试用例如何都能被一致评估为高质量。该方法通过识别评估指标中的漏洞来发现这种对抗性文本。

Result: 在WMT'24英日翻译任务中，找到的对抗性文本获得了79.1 COMET%的评分，在英德翻译任务中获得了67.8 COMET%的评分，甚至超过了使用通用翻译模型M2M100为每个源句子单独生成的翻译。此外，该方法发现的对抗性文本在多个语言对（如日英和德英）上都具有泛化能力。

Conclusion: 该研究揭示了基于嵌入的神经文本评估指标的严重脆弱性，表明这些指标可能产生不可靠的评估结果，从而对生成模型的开发和改进构成风险。这强调了需要更可靠和鲁棒的评估方法。
...
TL;DR: SpeechLLMs在语音翻译任务中表现不如级联系统，级联系统仍然是当前最可靠的语音翻译方案。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型扩展到多模态领域，将语音作为原生模态整合的SpeechLLMs应运而生，旨在直接翻译口语而绕过传统的转录流程。然而，这种整合是否比已建立的级联架构在语音到文本翻译质量上有所提升，仍然是一个未解决的问题。
...
Result: 在广泛的评估中，级联系统总体上仍然是最可靠的，而当前的SpeechLLMs仅在特定设置下与级联系统相当，语音基础模型则落后于两者。这表明整合大语言模型（无论是内部整合还是管道整合）对于高质量的语音翻译至关重要。

Conclusion: 虽然SpeechLLMs作为新兴技术有发展潜力，但当前阶段级联系统仍然是语音翻译的最可靠方案。整合大语言模型（无论是通过SpeechLLMs内部整合还是级联系统外部整合）对于实现高质量的语音翻译是必要的。
...
TL;DR: 提出隐私保护、边缘设备适应的ASR框架，在临床音频上实现17.1%的WER相对改进，减少47%的灾难性遗忘


<details>
  <summary>Details</summary>
Motivation: 自动语音识别在临床文档数字化方面潜力巨大，但面临数据隐私限制、计算资源有限和声学领域偏移等技术障碍，现有模型在真实临床音频上WER高达40.94%，无法实际应用

Method: 提出高效隐私保护适应框架，采用低秩适应（LoRA）在边缘设备上实现持续学习，结合多领域经验回放减少灾难性遗忘

Result: 在目标领域实现17.1%的相对WER改进，与朴素适应相比减少47%的灾难性遗忘

Conclusion: 展示了在高影响现实环境中构建可靠、自我改进ASR系统的可行路径，能够在资源受限的医疗环境中有效运行
...
TL;DR: 研究探索了使用大语言模型简化生物医学文本以提升健康素养，比较了提示模板、双AI代理和微调三种方法，发现gpt-4o-mini表现最佳，微调方法表现不佳，G-Eval评估指标与人工评估结果一致。


<details>
  <summary>Details</summary>
Motivation: 生物医学文本通常专业性强、难以理解，这限制了公众的健康素养。研究旨在利用大语言模型简化这些文本，使其更易于普通读者理解，从而提升健康信息的可及性。

Method: 使用包含生物医学摘要的公开数据集，开发并评估了三种方法：1）基于提示模板的基线方法；2）双AI代理方法；3）微调方法。以OpenAI的gpt-4o和gpt-4o-mini作为基线模型，采用Flesch-Kincaid、SMOG、SARI、BERTScore、G-Eval等定量指标，以及基于5点李克特量表的简单性、准确性、完整性、简洁性等定性指标进行评估。

Result: gpt-4o-mini模型表现最佳，而微调方法表现不佳。G-Eval这一基于大语言模型的定量评估指标显示出与定性评估相似的结果，能够有效区分不同方法的性能。

Conclusion: 大语言模型在简化生物医学文本方面具有潜力，其中gpt-4o-mini表现突出。G-Eval作为一种自动评估指标，能够可靠地评估文本简化质量，为未来研究提供了有效的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 科学文本通常包含复杂术语和结构，难以被非专业读者理解。CLEF 2025 SimpleText Track旨在开发自动简化科学文本的方法，以提高科学知识的可访问性。该研究旨在探索不同规模的语言模型在句子和文档级简化任务中的表现。

Method: 使用OpenAI的GPT-4.1、GPT-4.1-mini和GPT-4.1-nano模型，采用两种方法：1）无上下文方法：通过提示工程直接简化文本；2）微调方法：对模型进行特定任务的微调。在句子和文档两个粒度上评估简化效果。

Result: GPT-4.1-mini模型在无上下文设置下在句子和文档级简化中都表现出稳健性能。微调模型的结果参差不齐，其中GPT-4.1-nano-ft在一个案例中在文档级简化方面表现突出，表明不同粒度文本简化的复杂性。

Conclusion: 提示工程方法（无上下文）在科学文本简化任务中表现稳定，而微调方法的效果因模型规模和简化粒度而异。研究强调了针对不同简化粒度需要采用不同策略的重要性，为未来科学文本简化研究提供了有价值的见解。
...
TL;DR: 提出Refusal Steering方法，在推理时精细控制LLM对政治敏感话题的拒绝行为，无需重新训练。使用LLM作为评判员分配拒绝置信度，通过正则化变体计算更好的拒绝-顺从方向向量。


<details>
  <summary>Details</summary>
Motivation: 现有基于模式的拒绝检测方法脆弱且不够精细，需要一种能在推理时控制LLM对政治敏感话题拒绝行为的方法，同时保持对有害内容的安全性。

Method: 1) 用LLM作为评判员替代脆弱的模式检测，分配拒绝置信度分数；2) 提出岭正则化变体计算steering vectors，更好隔离拒绝-顺从方向；3) 在Qwen3-Next-80B等模型上应用激活导向技术。

Result: 在Qwen3-Next-80B-A3B-Thinking上，成功移除模型对政治敏感话题的拒绝行为，同时在JailbreakBench上保持安全性，在通用基准上接近基线性能。方法可推广到4B和80B模型，也能诱导定向拒绝。分析显示拒绝信号集中在transformer深层且分布在多个维度。

Conclusion: 激活导向技术可以在移除政治拒绝行为的同时保留对有害内容的安全对齐，为推理时可控、透明的审核提供了实用路径。
...
TL;DR: JustRL：一种极简的单阶段强化学习方法，使用固定超参数在1.5B推理模型上达到SOTA性能，计算量减少2倍，挑战了当前RL训练中复杂性的必要性。
...
Conclusion: 当前领域可能为了解决本不存在的复杂问题而增加了不必要的复杂性；简单的稳定基线方法可能已经足够；提供了开源代码和模型作为社区的简单验证基线。
...
TL;DR: 提出GinSign框架，通过分层分类方法将自然语言映射到系统签名，解决现有NL到时序逻辑翻译中原子命题接地准确率低的问题。


<details>
  <summary>Details</summary>
Motivation: 现有NL到时序逻辑翻译框架要么需要准确原子接地，要么接地翻译准确率低，这限制了在自主系统中无需手动编写形式化规范的能力。

Method: 提出GinSign框架，引入接地模型学习将NL片段映射到系统签名的抽象任务。采用分层分解方法：先预测谓词标签，再选择适当类型的常量参数，将自由形式生成问题转化为结构化分类问题。

Result: 实验表明，省略接地的框架会产生语法正确但语义不等价的LTL，而GinSign支持下游模型检查，接地逻辑等价分数达95.5%，比SOTA提升1.4倍。

Conclusion: GinSign通过结构化分类方法有效解决NL到时序逻辑翻译中的接地问题，显著提升翻译准确率，支持系统验证。
...
TL;DR: 提出推理轨迹增强的RAG框架，通过三阶段结构化推理处理检索文档中的冲突、过时和主观信息，引入冲突感知信任评分管道评估系统性能。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成(RAG)在处理检索到的冲突、过时或主观信息时存在局限，现有方法缺乏统一的推理监督机制。

Method: 提出推理轨迹增强的RAG框架，包含三个结构化推理阶段：文档级裁决、冲突分析和基于证据的合成；引入冲突感知信任评分(CATS)管道，使用LLM作为评判者评估系统性能。
...
TL;DR: 研究多模态检索增强生成在生物医学QA中的视觉处理策略：将图表转换为文本 vs OCR-free视觉检索，发现在中等模型上文本转换更可靠，而在前沿模型上视觉检索变得有竞争力。


<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成(MM-RAG)在生物医学QA中面临关键决策：何时将图表转换为文本，何时使用OCR-free视觉检索。特别是在糖生物学这种视觉密集领域，需要明确这两种策略的适用场景和性能差异。

Method: 构建了包含120个多选题的糖生物学基准，按检索难度分层。实现了四种增强策略：无增强、文本RAG、多模态转换、OCR-free视觉检索(ColPali)。使用Docling解析和Qdrant索引，评估了中等规模开源模型和前沿专有模型，并进行了统计显著性分析。

Result: 对于Gemma-3-27B-IT，文本和多模态转换优于OCR-free检索(0.722-0.740 vs 0.510)。对于GPT-4o，多模态最佳(0.808)，文本(0.782)和ColPali(0.745)接近。在GPT-5实验中，ColPali和ColFlor提升至0.828，不同检索器统计上无差异。GPT-5-nano落后较大模型8-10%。

Conclusion: 策略选择取决于模型能力：中等模型更适合将视觉内容转换为文本以降低理解负担，而前沿模型下OCR-free视觉检索变得有竞争力。ColFlor检索器在保持性能的同时具有较小计算开销，是强生成器下的高效默认选择。
...
TL;DR: GraFT框架通过限制每一步有效输出token来降低NL到TL翻译的复杂度，相比现有方法提升端到端翻译准确率5.49%，域外翻译准确率14.06%
...
Method: 提出Grammar Forced Translation (GraFT)框架，通过限制每一步有效输出token来降低翻译复杂度，利用每个问题的独特属性减少解空间

Result: 在CW、GLTL和Navi基准测试中，相比SOTA方法，GraFT提升端到端翻译准确率5.49%，域外翻译准确率14.06%

Conclusion: GraFT通过解空间缩减显著提升NL到TL翻译性能，为机器人系统的人机交互提供了更有效的解决方案
...
TL;DR: 本文提出了一种信息论方法来量化语音中韵律单独传达的信息量及其内容，发现对于讽刺和情感，音频通道比文本通道传递的信息量高一个数量级。


<details>
  <summary>Details</summary>
Motivation: 韵律（语音的旋律）传达了通常不被文字或文本捕获的关键信息。需要量化韵律单独表达的信息量以及这些信息的具体内容。

Method: 使用大型语音和语言模型来估计话语意义特定维度（如情感）与其任何通信通道（如音频或文本）之间的互信息。应用该方法量化音频和文本在讽刺、情感和疑问性方面的信息传递。

Result: 对于讽刺和情感，音频通道（通过韵律）传递的信息量比单独文本通道高一个数量级以上，特别是在缺乏超出当前句子的长期上下文时。对于疑问性，韵律提供的额外信息相对较少。

Conclusion: 该方法可以扩展到更多意义维度、通信通道和语言，为理解韵律在沟通中的作用提供了量化框架。
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型在推理时存在高延迟问题，现有缓存机制（如token级KV缓存）适用范围有限，需要更通用、高效的缓存方案来支持实时大规模部署
...
Conclusion: LLMCache是一个实用且通用的Transformer推理优化方案，为实际应用中的实时大规模部署提供了有效解决方案
...
TL;DR: AdaSearch：一个两阶段强化学习框架，通过解耦问题解决和搜索决策，提高LLM搜索代理的自知能力，减少不必要搜索调用，同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM搜索代理存在过度依赖搜索（增加成本、暴露于噪声/恶意内容）或仅依赖参数知识（导致幻觉）的问题。现有方法通过惩罚工具调用次数来缓解搜索过度使用，但这需要大量奖励工程、信用分配模糊，且可能被表面减少调用的代理利用。评估仅通过调用次数会混淆必要和不必要的搜索。

Method: 提出AdaSearch：一个简单的两阶段、结果驱动的强化学习框架。它将问题解决与是否调用搜索的决策解耦，使决策过程变得明确和可解释。首先量化现有搜索代理的自知意识，然后设计框架提高知识边界意识。

Result: 实验表明，AdaSearch显著提高了知识边界意识，减少了不必要的搜索调用，保持了强大的任务性能，并提供了更透明、可解释的决策行为。在多个模型家族和规模上都有效。

Conclusion: AdaSearch通过解耦决策过程，解决了现有搜索代理的局限性，为金融和医疗等高风险领域提供了更透明、可解释的搜索代理解决方案。
...
TL;DR: MMRB2是首个针对多模态奖励模型的综合基准，涵盖图像生成、编辑、交错生成和推理四个任务，包含4000个专家标注的偏好对，用于评估多模态奖励模型的性能。


<details>
  <summary>Details</summary>
Motivation: 奖励模型对训练大语言模型至关重要，但针对处理交错图像和文本序列的全能模型（omni models）的奖励模型研究不足，缺乏全面的评估基准。

Method: 构建MMRB2基准：1）设计实用且具有挑战性的提示；2）收集最先进模型和智能体的响应；3）通过集成过滤策略创建具有强人类专家共识的偏好对，涵盖四个任务各1000个偏好对。
...
Conclusion: MMRB2基准填补了多模态奖励模型评估的空白，揭示了当前模型与人类专家之间的显著差距，为未来奖励模型的改进提供了关键方向，且其性能与下游任务成功强相关。
...
TL;DR: Transformer在变量符号含义不固定的算术任务中，发展出符号推理机制而非几何嵌入，包括可交换复制、单位元识别和闭包消去三种机制。


<details>
  <summary>Details</summary>
Motivation: 先前研究发现Transformer在固定符号含义的算术任务中会发展几何嵌入反映代数结构，但本研究探索当符号作为变量、其含义随序列变化时，Transformer会发展何种机制。

Method: 设计新任务：符号到代数群元素的分配随序列变化；创建针对性数据分布进行因果测试；分析模型学习到的机制。

Result: Transformer在挑战性设置下达到接近完美的准确率，甚至能泛化到未见过的代数群；识别出三种一致学习的机制：可交换复制、单位元识别和闭包消去。

Conclusion: 与固定符号设置中的几何表示互补，当训练模型在上下文推理中处理含义不固定的变量时，模型会发展符号推理机制。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究发现LLM中存在负责特定任务的稀疏子网络（电路），且微调通常通过增强现有电路来提升性能。这启发我们直接干预这些电路进行精确的任务定向更新。

Method: 提出Constructive Circuit Amplification方法：1）从模型推理轨迹中识别关键token；2）识别负责目标任务的模型组件；3）仅更新这些识别出的组件。

Result: 在数学推理任务上，准确率提升高达11.4%，仅修改1.59%的模型组件，在MMLU、TriviaQA和TruthfulQA等基准测试中对其他能力影响最小。

Conclusion: 通过选择性更新稀疏的模型组件，可以可靠地增强目标能力，这为精确的任务定向模型更新提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 量子光学实验设计通常需要专业编程知识和物理专业知识，限制了非专家用户的参与。该系统旨在民主化计算实验设计，使研究人员和教育工作者能够通过自然语言交互设计和模拟量子光学实验。

Method: 系统采用多智能体架构，通过语义检索从三层工具箱中组合光学组件，使用意图路由、知识增强生成和双模式验证（QuTiP和FreeSim物理模拟器）进行设计验证和收敛性优化。

Result: 在13个量子光学实验中（包括基础光学、量子信息协议和先进技术），系统达到8-9/10的设计-模拟对齐分数，模拟结果忠实地建模了预期物理现象。自由形式模拟在11/13实验中优于约束框架。

Conclusion: 系统成功民主化了量子光学实验设计，产生用户可通过对话迭代优化的强大初始设计。关键发现区分了结构正确性与定量准确性：高对齐确认了正确的物理架构，而数值预测需要专家审查。量子光学的多样性需要灵活的数学表示。
...
<details>
  <summary>Details</summary>
Motivation: 传统伦理框架在处理不确定性决策时存在局限，通常将其简单视为行动约束。需要一种能动态建模道德责任如何随认知状态变化的新框架。

Method: 提出比例责任原则(PPD)框架，用数学方程D_total = K[(1-HI) + HI * g(C_signal)]建模总责任，其中K为知识，HI为不确定性，C_signal为情境信号强度。通过蒙特卡洛模拟验证，并在临床伦理、接受者权利法、经济治理和人工智能四个领域应用。

Result: 模拟显示保持基线谦逊系数(lambda>0)的系统能产生更稳定的责任分配，降低过度自信决策风险。PPD为可审计AI决策系统提供了数学上可处理的道德责任建模方法。
...
TL;DR: 提出一个从自然语言描述生成物理可实现的装配指令框架，使用LDraw作为中间表示，通过约束词汇表确保几何有效性和可建造性，实现超过3000个零件的砖块原型装配。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如基于像素的扩散方法或CAD模型）无法支持复杂的装配指令或组件交换，需要一种能够连接语义设计意图与可制造输出的桥梁，实现从自然语言规范到物理原型制作。

Method: 使用LDraw作为文本丰富的中间表示，通过大型语言模型结合工具引导，在离散零件词汇表中操作，强制执行几何有效性、连接约束和建造顺序约束，生成有效的逐步建造序列和装配指令。

Result: 开发了Python库用于程序化模型生成，在复杂卫星、飞机和建筑领域评估可建造输出，展示了超过3000个装配零件的砖基原型，证明了方法的可扩展性、模块化和保真度。

Conclusion: 该方法作为一种"物理API"，通过约束词汇表将精确定向的砖块位置与"词袋"连接，使任意功能需求能够编译为物质现实，为制造和工程原型中的自然语言实现开辟了新的设计选项。
...
TL;DR: LLMs存在特权信息偏见，导致知识丰富的领导者无法有效指导感知受限的跟随者，主动查询协议比标准指令更有效


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在具身环境中的符号落地问题，特别是信息不对称分布时的特权信息偏见（知识诅咒），领导者因缺乏心智理论而无法有效指导感知受限的跟随者

Method: 提出非对称辅助推理框架，在AI2-THOR环境中进行实验，比较推式（标准指令）和拉式（主动查询）两种通信协议

Result: 发现显著的成功差距：领导者能感知目标的比例为35.0%，但协作团队成功率仅17.0%，近50%可行计划因通信落地错误而失败；拉式协议比推式指令更稳健，成功场景中澄清请求频率是2倍

Conclusion: 主动不确定性减少机制是安全人机协作和机器人间协作的前提条件，拉式通信协议能有效缓解特权信息偏见问题
...
TL;DR: AI Epidemiology 是一个通过应用群体层面监测方法来治理和解释高级AI系统的框架，类比流行病学方法，通过统计证据而非理解内部机制来预测AI输出失败。


<details>
  <summary>Details</summary>
Motivation: 当前的可解释性方法（如SHAP和机制可解释性）在面对大规模部署模型时，受限于模型复杂性而难以有效工作。需要一种能够绕过模型复杂性、提供有效AI治理的框架。

Method: 通过标准化捕获AI-专家交互为结构化评估字段：风险等级、对齐分数和准确度分数。这些作为暴露变量，通过统计关联预测输出失败，类似于胆固醇和血压预测心脏事件。然后通过专家覆盖和真实世界结果验证输出失败关联。

Result: 该框架为零专家负担，通过被动跟踪专家与AI建议的趋同和分歧提供自动审计追踪。通过分析输出而非内部计算，在机构更新模型和切换供应商时提供治理连续性。提供可靠性分数和语义评估，使专家和机构能在AI输出造成危害前检测不可靠输出。

Conclusion: AI Epidemiology 通过使领域专家无需机器学习专业知识即可治理AI系统，实现了AI监督的民主化，为复杂AI系统提供了一种实用的治理和解释框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前以模型为中心的代理架构部署后难以自我进化，需要持续模型重训练/微调，计算开销大且存在准确性与推理效率的权衡

Method: 提出MOBIMEM内存中心代理系统：1) 三种专用内存原语：Profile Memory（轻量级距离图结构）、Experience Memory（多级模板）、Action Memory（细粒度交互序列）；2) 操作系统风格服务：调度器、AgentRR机制、上下文感知异常处理

Result: 在AndroidWorld和top-50应用上评估：83.1%配置文件对齐，23.83ms检索时间（比GraphRAG基线快280倍），任务成功率提升高达50.3%，端到端延迟降低高达9倍

Conclusion: MOBIMEM通过内存中心架构实现LLM代理的迭代自进化，无需模型重训练，显著提升个性化、能力和效率，为移动设备上的代理系统提供了新范式
...
TL;DR: 提出一种基于状态增强拆卸序列规划图的确定性求解器框架，用于优化循环经济产品分拣决策，以电动汽车电池为例展示其灵活性


<details>
  <summary>Details</summary>
Motivation: 循环经济产品分拣需要平衡保留价值与处理成本，现有方法缺乏统一的决策框架来处理复杂约束和条件感知评估

Method: 建立状态增强的拆卸序列规划图，将拆卸历史编码到状态中以确保马尔可夫性，通过递归评估在继续拆卸或选择循环经济选项间做出决策

Result: 框架能够处理电动汽车电池等复杂产品的分层分拣决策，适应不同的机械复杂度、安全要求和经济驱动因素

Conclusion: 该统一形式化为优化循环经济分拣决策提供了可处理且可推广的基础，适用于多种产品和操作场景
...
TL;DR: 大型语言模型在儿科咨询中的安全性评估：研究发现LLMs在面对现实世界家长焦虑压力时存在安全隐患，对抗性查询会降低模型安全性，模型规模影响安全表现但所有模型都存在漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地被家长用于儿科咨询，但其在真实世界对抗性压力下的安全性了解不足。焦虑家长常使用紧急语言可能破坏模型安全防护，导致有害建议。
...
Result: 平均安全得分为5.50/15（标准差2.41）。70B模型优于8B模型（6.26 vs 4.95，p<0.001），关键失败率更低（4.8% vs 12.0%，p=0.02）。对抗性查询使安全性降低8%（p=0.03），紧迫性导致最大降幅（-1.40）。在癫痫发作（33.3%不适当诊断）和疫苗接种后查询中存在漏洞。谨慎表达与安全性强相关（r=0.68，p<0.001），而紧急情况识别完全缺失。
...
TL;DR: Darth Vecdor (DV) 是一个将大语言模型知识提取为结构化SQL知识图谱的系统，旨在解决直接查询LLM的成本、速度、安全性和可靠性问题，特别针对医疗领域设计。


<details>
  <summary>Details</summary>
Motivation: 虽然可以直接查询大语言模型获取知识，但在高容量操作中面临成本、速度、安全性和置信度等问题。将LLM知识预提取到结构化知识图谱中可以缓解这些问题，特别在医疗等关键领域。

Method: DV系统通过专门设计的功能处理LLM响应的错误、离题、自由文本、过于笼统和不一致等问题，支持多元素响应。提供基于浏览器的图形用户界面，便于领域专家进行提示工程。

Result: DV已作为免费、开源、可扩展的软件发布，采用"按现状"原则，不提供任何保证。系统旨在帮助改善医疗保健，但用户需自行承担使用风险。

Conclusion: DV系统通过将LLM知识结构化到SQL知识图谱中，为医疗等领域提供更可靠、高效的知识查询方案，但用户需谨慎评估风险并负责任地使用。
...
TL;DR: 提出基于传播激活算法的RAG框架，通过自动构建知识图谱连接文档，提升大语言模型在复杂推理任务上的表现


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统将所有检索信息视为同等可靠，忽略了文本语料库中信息的可信度和相互关联性。GraphRAG虽然通过知识图谱改进，但依赖高质量图谱构建且存在与标准RAG类似的挑战

Method: 提出新型RAG框架，使用传播激活算法从自动构建的知识图谱连接的文档语料库中检索信息，增强大语言模型在复杂任务（如多跳问答）上的性能

Result: 实验显示该方法达到或优于迭代RAG方法，与思维链迭代检索结合时相比朴素RAG获得39%绝对正确率提升，且能在资源受限环境下使用小型开源模型

Conclusion: 提出的传播激活RAG框架有效解决复杂推理任务中的多步证据检索问题，可作为即插即用模块与多种RAG方法集成，在资源受限环境中表现优异
...
TL;DR: 研究探索用优化的小语言模型替代大语言模型工作流，通过领域适应训练在特定任务上实现高性能且大幅降低成本


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI规模化应用，大语言模型的高计算成本成为企业可持续性和可访问性的关键瓶颈，需要探索更经济高效的小语言模型解决方案

Method: 使用facebook/opt-350m模型，通过Hugging Face TRL的监督微调方法进行单轮训练，针对文档摘要、查询回答和结构化数据解释等任务进行领域适应

Result: 微调后的小语言模型在ToolBench评估中获得77.55%通过率，显著优于ChatGPT-CoT(26.00%)、ToolLLaMA-DFS(30.18%)和ToolLLaMA-CoT(16.27%)等基线模型

Conclusion: 精心设计和针对性训练的小语言模型能显著降低采用门槛，实现经济高效的大规模生成式AI生产系统集成，为成本优化和运营效率提供可行方案
...
<details>
  <summary>Details</summary>
Motivation: 人类智能能够动态合成新的目标函数，但人工智能系统缺乏这种能力。论文旨在回答目标函数从何而来、如何选择追求目标等根本问题，探索如何赋予人工系统类似的能力。

Method: 提出主观函数概念——一种内生于智能体的高阶目标函数（相对于智能体特征而非外部任务定义）。以期望预测误差作为具体的主观函数示例进行研究。

Result: 建立了主观函数的概念框架，展示了期望预测误差作为主观函数的具体应用。该框架与心理学、神经科学和机器学习中的多个概念建立了联系。

Conclusion: 主观函数为理解智能体如何内生地合成目标函数提供了新视角，有望帮助人工系统获得类似人类的目标生成能力，在多个学科领域具有广泛的理论联系和应用潜力。
...
TL;DR: 提出使用LLM作为智能法官来协调时间序列基础模型集成，通过R1风格微调和SHAP引导训练，在GIFT-Eval基准上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 当前时间序列基础模型众多，但没有单一方法能始终最优，且缺乏可解释性。LLM具有强大推理能力但直接用于时间序列预测效果不佳，需要将其重新定位为协调者角色

Method: 将LLM重新定位为智能法官，通过R1风格微调过程（基于SHAP忠实度分数指导）教授模型将集成权重解释为时间动态的因果陈述，然后通过多轮对话进行前瞻性评估、提供因果解释并自适应优化策略

Result: 在GIFT-Eval基准的23个数据集、97个设置上验证，在CRPS和MASE指标上显著优于领先的时间序列基础模型，建立了新的最先进结果

Conclusion: 通过将LLM重新定位为智能法官来协调时间序列基础模型集成，结合R1微调和SHAP引导训练，实现了既高性能又具有可解释性的时间序列预测系统
...
TL;DR: LLMs在预测未来事件时普遍存在系统性过度自信问题，即使最先进的模型也表现出显著的校准误差，增强推理能力反而可能恶化校准性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在各种任务上表现出色，但其认知校准能力（即置信度与实际准确度的匹配程度）尚未得到充分理解。研究者希望评估模型对真正未知的未来事件的量化不确定性能力，而不仅仅是静态知识的准确性。

Method: 引入KalshiBench基准测试，包含300个来自受监管交易所Kalshi的预测市场问题，这些问题的真实结果发生在模型训练截止日期之后。评估了五个前沿模型（Claude Opus 4.5, GPT-5.2, DeepSeek-V3.2, Qwen3-235B, Kimi-K2），使用预期校准误差（ECE）和Brier技能分数等指标衡量校准性能。

Result: 所有模型都表现出系统性过度自信。即使校准最好的模型（Claude Opus 4.5，ECE=0.120）也有显著校准误差。增强推理的模型（如GPT-5.2-XHigh，ECE=0.395）校准更差，尽管准确性相当。只有一个模型获得了正的Brier技能分数，表明大多数模型的表现还不如简单地预测基准率。

Conclusion: 模型规模的扩展和推理能力的增强并不会自动带来校准性能的提升，认知校准是一种需要针对性开发的独立能力。这凸显了在开发更可靠、更值得信赖的AI系统时，需要专门关注校准问题。
...
TL;DR: 开发自动化系统来发现和分类课程大纲中的AI相关政策，结合主题建模和LLM技术分析政策文本，帮助学生在教育中安全、公平地使用生成式AI。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育中的广泛应用，学生使用AI工具辅助学习的同时也面临抄袭、错误信息和批判性思维削弱等问题。各教育机构的AI政策差异大且不断变化，导致学生不清楚使用规范和最佳实践。

Method: 设计自动化系统，结合无监督主题建模技术识别关键政策主题，使用大语言模型（如GPT-4.0）对政策文本进行分类，分析生成式AI的允许程度和其他要求。

Result: 系统主题发现的一致性得分为0.73。基于GPT-4.0的政策分类在八个识别主题上达到0.92-0.97的精确率和0.85-0.97的召回率。
...
TL;DR: WeMusic-Agent是一个基于LLM的对话式音乐推荐训练框架，通过知识内化和代理边界学习，让模型智能决定何时使用内部知识、何时调用外部工具，并在真实数据上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前对话式音乐推荐方法难以平衡专业领域知识和灵活工具集成，需要更智能的决策机制来处理用户偏好和音乐上下文。

Method: 提出WeMusic-Agent训练框架，结合知识内化和代理边界学习。具体实现WeMusic-Agent-M1模型，通过500亿音乐相关语料持续预训练内化音乐知识，同时学习在必要时调用外部工具（如音乐检索API、推荐系统）。还构建了基于微信听书真实数据的开源基准测试。

Result: 在真实世界数据上的实验表明，WeMusic-Agent相比现有模型取得了显著改进。构建的基准测试支持从相关性、个性化、多样性等多个维度进行全面评估。

Conclusion: WeMusic-Agent框架通过智能结合内部知识和外部工具，有效提升了对话式音乐推荐的性能，同时提供了开源基准测试以促进该领域的研究。
...
TL;DR: ToolForge是一个自动合成框架，仅需少量虚拟工具即可生成高质量工具调用训练数据，无需真实API调用，使8B参数模型在多个基准测试中超越GPT-4o
...
Method: 基于(问题、黄金上下文、答案)三元组合成大规模工具学习数据，通过多跳推理和自反思机制丰富数据，采用多层验证框架（规则和模型评估）确保数据质量

Result: 仅用8B参数的模型在合成数据上训练后，在多个基准测试中超越GPT-4o，证明方法的有效性
...
TL;DR: 基于网络的AI工具，通过问卷、智能填充、研究指导推荐和原型构建四个核心组件，帮助从业者选择和实施最有效的AI建模策略。


<details>
  <summary>Details</summary>
Motivation: AI解决方案开发中，从业者面临选择合适建模策略的挑战，需要工具来加速开发过程并确保方案有效性。

Method: 采用四个核心组件：1) 问卷收集需求，2) 智能填充自动补全信息，3) 基于文献的研究指导推荐，4) 原型构建器生成实施方案。

Result: 该工具能够为产品经理、软件开发者和研究人员等不同用户群体加速AI解决方案的开发过程。

Conclusion: Science Consultant Agent是一个有效的AI建模策略选择工具，通过结构化流程帮助从业者快速实施AI解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统K-means和约束K-means在无线网络应用中存在局限性，需要一种既能确保数值稳定性又能实现软分配的聚类算法，该算法在无线网络中应具有直接的解释性，能够同时处理无线节点放置和用户关联问题。

Method: 提出加权K-harmonic means (WKHM)算法，通过逆距离加权实现软分配，权重在无线网络中解释为基于接收信号强度的分数用户关联。算法建立了确定性和随机设置下的严格收敛保证。

Result: 证明了固定初始化下的单调下降收敛到局部最小值、二项点过程初始化下的概率收敛、以及温和衰减条件下的几乎必然收敛。通过多样化用户分布的模拟实验，WKHM在最小信号强度和负载公平性之间实现了优于经典和现代基线的权衡。

Conclusion: WKHM为基于谐均值的聚类提供了首个随机收敛保证，在无线网络中作为联合无线节点放置和用户关联的原则性工具，在信号强度和负载公平性之间实现了优越的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 传统PDE求解方法依赖人工设置和领域专业知识，现有PINN和DeepXDE等方法仍需专家知识且缺乏完全自主性。需要一种更自动化的PDE求解范式。

Method: 提出PDE-Agent框架，包含两个关键创新：1) 具有图记忆的Prog-Act框架支持多智能体协作，通过双循环机制（局部修复和全局修订）实现动态规划和错误修正；2) 集成工具-参数分离机制的Resource-Pool支持多工具协作，集中管理运行时工件并解决工具间依赖问题。

Result: 开发了PDE-Bench多类型PDE基准测试集，并提出多级指标评估工具协调能力。评估验证PDE-Agent在复杂多步骤、跨步骤依赖任务中表现出优越的适用性和性能。
...
TL;DR: SPRITE框架通过模拟器和大模型程序化合成高质量空间推理数据，解决现有方法在可扩展性、多样性和精确性之间的困境，显著提升视觉语言模型的空间理解能力。
...
Conclusion: SPRITE框架成功解决了空间数据合成的可扩展性-多样性-精确性困境，证明克服传统模板方法的低多样性对构建鲁棒、可泛化的空间智能至关重要。
...
TL;DR: AlignMerge：一种几何感知的LLM融合框架，在保持对齐性的同时合并多个微调模型，避免传统融合方法破坏安全对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型融合方法（如线性权重平均、任务向量、Fisher加权平均）虽然能保持损失函数性能，但会悄悄破坏模型的安全对齐性。作者认为融合不应只是数值技巧，而应是在已对齐锚点周围的几何约束操作。

Method: 在指令调优基模型的局部Fisher图表中，估计对齐子空间并引入投影器P_A。优化目标包含三个损失项：L_geo（保持Fisher-Rao几何接近性）、L_align（惩罚对齐敏感方向运动）、L_bud（强制执行软对齐预算）。使用对齐质量指数（AQI）作为对齐功能度量。

Result: 在五个模型家族（LLaMA-3 8B、Mistral 7B、Qwen 2、Phi-3.5、Gemma 2）上测试，AlignMerge在保持对齐指标（AQI、毒性、LLM-judge对齐）的同时，在指令遵循、推理和帮助性方面匹配或超越最佳专家模型。相比Fisher soups、TIES、SafeMerge和MergeAlign，显示更小的对齐子空间漂移和更少的预算违规。

Conclusion: 该研究使对齐保持融合成为首要设计目标，为未来基础模型的几何感知组合提供了路径，强调融合应尊重安全几何而非事后验证。
...
TL;DR: AMUSE是一个评估多模态大语言模型在多说话人对话场景中代理推理能力的基准，RAFT是一个数据高效的代理对齐框架，能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（如GPT-4o、Qwen3-Omni）在感知方面表现强劲，但在需要代理推理的多说话人对话场景中表现不佳，这些场景要求模型跟踪说话者、维持角色、跨时间定位事件，对音频-视频理解应用至关重要。

Method: 1. 提出AMUSE基准：围绕代理性任务设计，要求模型将复杂音频-视觉交互分解为规划、定位和反思步骤，评估三种模式（零样本、引导、代理）和六个任务族；2. 提出RAFT框架：数据高效的代理对齐框架，整合奖励优化与内在多模态自评估作为奖励，并进行选择性参数适应以实现数据和参数高效更新。

Result: 当前模型在所有评估模式下都表现出弱的多说话人推理能力和不一致行为；使用RAFT框架后，在基准上实现了高达39.52%的相对准确率提升。

Conclusion: AMUSE和RAF共同为研究多模态模型中的代理推理提供了实用平台，并有效提升了模型在多说话人对话场景中的能力，解决了当前模型在代理推理方面的不足。
...
Method: 将代码即行动范式扩展到时间领域，代理利用语义先验和上下文学习来预测精确的等待时间（time.sleep(t)），从而与异步环境同步，无需详尽检查。

Result: 在模拟Kubernetes集群中的实验表明，代理能够精确校准内部时钟，最小化查询开销和执行延迟，验证了时间意识是可学习的能力。

Conclusion: 时间意识是自主代理在开放环境中进化的重要可学习能力，代理端方法能够有效解决异步环境中的时间同步问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在解决复杂任务时使用工具、多步规划和智能体间通信，会产生安全风险。部署者用自然语言编写的策略存在模糊性和上下文依赖性，难以映射到机器可检查的规则，导致运行时执行不可靠。

Method: 将安全策略表达为sequent（序列式逻辑公式），提出QuadSentinel四智能体防护系统：状态跟踪器、策略验证器、威胁监视器和裁判。系统将这些策略编译为基于可观察状态谓词的机器可检查规则，并在线执行。裁判逻辑和高效的top-k谓词更新器通过优先级检查和分层冲突解决来降低成本。
...
TL;DR: OS-Oracle：一个用于GUI计算机使用代理的批评模型框架，包含数据合成管道、两阶段训练方法和跨平台基准测试


<details>
  <summary>Details</summary>
Motivation: 随着VLM驱动的计算机使用代理在GUI导航和操作方面能力增强，可靠的步骤级决策成为实际部署的关键瓶颈。长流程任务中错误快速累积，不可逆操作可能导致意外后果，因此需要批评模型在执行前评估每个动作。然而，缺乏多样化的高质量GUI反馈数据和公开的批评基准限制了批评模型的发展。

Method: 提出OS-Oracle框架，包含三个核心贡献：(1) 可扩展的跨平台GUI批评数据合成管道；(2) 结合监督微调(SFT)和一致性保持组相对策略优化(CP-GRPO)的两阶段训练范式；(3) OS-Critic Bench，一个跨移动端、网页端和桌面端平台的批评模型性能评估基准。

Result: 构建了包含31万个批评样本的高质量数据集。得到的批评模型OS-Oracle-7B在OS-Critic Bench上达到开源VLM中的最先进性能，在移动端领域超越专有模型。作为预批评模型使用时，能提升UI-TARS-1.5-7B等原生GUI代理在OSWorld和AndroidWorld环境中的性能。

Conclusion: OS-Oracle通过提供可扩展的数据合成、有效的训练方法和全面的评估基准，解决了GUI批评模型发展的关键瓶颈，为计算机使用代理的可靠步骤级决策提供了实用解决方案。
...
TL;DR: 提出ForenAgent框架，通过多轮交互让MLLM自主生成和执行Python低级工具进行图像伪造检测，结合冷启动和强化微调训练，在FABench数据集上展现工具使用和反思推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像伪造检测方法要么依赖低级语义无关特征，要么依赖MLLM高级语义知识，这两种信息流在范式和推理上高度异构，难以统一建模跨层次交互。

Method: 提出ForenAgent多轮交互框架，让MLLM围绕检测目标自主生成、执行和迭代优化Python低级工具；采用冷启动+强化微调两阶段训练；设计包含全局感知、局部聚焦、迭代探测和整体裁决的动态推理循环；构建FABench数据集（10万图像，20万交互问答对）。

Result: 实验表明ForenAgent在低级工具辅助下，在挑战性IFD任务上展现出涌现的工具使用能力和反思推理能力，为通用IFD开辟了有前景的路径。

Conclusion: ForenAgent通过让MLLM自主操作低级工具，实现了更灵活可解释的伪造分析，统一了低级特征和高级语义知识，为图像伪造检测提供了新范式。
...
TL;DR: 本文提出了一个系统化框架，将智能体AI系统的适应机制统一为智能体适应和工具适应两大类，并进一步细分为不同信号触发和监督形式，为构建更强大、高效、可靠的智能体AI系统提供概念基础和实践路线图。


<details>
  <summary>Details</summary>
Motivation: 随着基于基础模型的智能体AI系统能力不断增强、应用范围不断扩大，适应机制成为提升性能、可靠性和泛化性的核心手段。然而，快速扩展的研究领域缺乏系统化框架来统一理解各种适应策略，需要建立清晰的概念基础来指导系统设计和策略选择。

Method: 提出一个系统化框架，将智能体AI系统的适应机制分为两大维度：智能体适应和工具适应。智能体适应进一步分为工具执行信号触发和智能体输出信号触发两种形式；工具适应则分为智能体无关和智能体监督两种形式。通过这一框架分析各种适应策略的设计空间、权衡取舍，并提供实践指导。

Result: 该框架澄清了智能体AI中适应策略的设计空间，明确了各种策略的权衡关系，为系统设计中选择或切换策略提供了实用指导。通过对各类代表性方法的回顾分析，揭示了它们的优势和局限性，并指出了关键开放挑战和未来机遇。

Conclusion: 本文为研究人员和实践者构建更强大、高效、可靠的智能体AI系统提供了概念基础和实践路线图。统一的适应框架有助于系统化理解智能体AI的适应机制，指导策略选择，并推动该领域的进一步发展。
...
TL;DR: 本文提出了一种成本感知的PoQ框架，将显式效率测量整合到推理节点和评估节点的奖励机制中，通过实验证明该框架能有效奖励高质量低成本的模型，为去中心化LLM推理提供经济可持续的基础。


<details>
  <summary>Details</summary>
Motivation: 去中心化大语言模型推理需要透明和抗审查的访问，但现有验证方法难以扩展到现代模型。原始的PoQ忽略了推理节点和评估节点之间的异构计算成本差异，需要一种能考虑成本因素的改进方案。

Method: 提出成本感知PoQ框架，将显式效率测量整合到奖励机制中。设计结合了真实标记级F1、轻量级学习评估器和GPT判断的统一评估流程，采用平衡标准化质量和成本的线性奖励函数。

Result: 实验表明：1）语义文本相似性双编码器比交叉编码器与真实值和GPT分数相关性更高；2）池中最大模型在单位延迟的质量效率方面最高；3）蒙特卡洛模拟显示成本感知奖励方案能持续奖励高质量低成本的推理模型和高效评估器。

Conclusion: 成本感知PoQ为经济可持续的去中心化LLM推理提供了实用基础，通过整合效率测量到奖励机制中，能够有效激励高质量低成本的计算节点。
...
TL;DR: 论文认为当前AI的实际影响有限，而物理学能为AI发展提供更多价值，提出"大AI"概念结合理论严谨性与机器学习灵活性。


<details>
  <summary>Details</summary>
Motivation: 尽管AI被广泛宣传为变革性技术，但经过十多年炒作后，其可衡量的影响仍局限于少数高调的科学和商业成功案例。2024年诺贝尔化学和物理学奖虽然认可了AI的潜力，但更广泛的评估表明当前AI的影响更多是宣传性的而非技术性的。作者认为，虽然当前AI可能影响物理学，但物理学能为这一代AI提供更多价值。

Method: 论文回顾了当前AI架构（大语言模型、推理模型和代理AI）的局限性批判，强调了量子AI和模拟计算中的机遇，并提出了"大AI"的路线图。

Result: 当前AI架构存在多个根本问题：依赖数万亿无意义的参数、存在分布偏差、缺乏不确定性量化、无法提供机制性见解、甚至无法捕捉基本的科学定律。这些限制阻碍了AI在科学领域的实质性影响。

Conclusion: 需要发展"大AI"——一种将理论基础的严谨性与机器学习的灵活性相结合的新范式。物理学能为AI发展提供重要价值，特别是在量子AI和模拟计算领域，这有望解决当前AI架构的根本缺陷。
...
TL;DR: 提出了一种新的元启发式优化算法PCIA，受人类构建和使用路径的行为启发，在53个数学优化问题和13个约束优化问题上表现出色。


<details>
  <summary>Details</summary>
Motivation: 受人类路径构建行为的启发：人们通常偏好热门交通路线，当路径关闭时会智能地混合现有路径构建新路线，并且会随机选择不同路径探索未知目的地。

Method: PCIA模拟人类路径构建行为，生成随机种群寻找最佳路径（类似群体智能算法），每个粒子代表一条通向目标的路径，通过智能混合现有路径和随机探索来优化。
...
TL;DR: Synthelite是一个基于大语言模型的合成规划框架，能够生成端到端合成路线，并通过自然语言提示允许专家干预，在约束条件下达到95%的成功率。
...
Result: Synthelite能够灵活适应各种用户指定约束，在策略约束和起始材料约束的合成任务中达到95%的成功率，并在路线设计中考虑化学可行性。
...
TL;DR: TIB AIssistant是一个AI支持的研究平台，通过多个专门助手和工具支持整个研究生命周期，促进研究透明度和可重复性。


<details>
  <summary>Details</summary>
Motivation: 随着AI和大型语言模型的普及，学术界需要系统化的AI支持工具来协助整个研究生命周期，提高研究效率和可重复性。

Method: 开发TIB AIssistant平台，包含多个专门负责特定研究任务的助手，集成外部学术服务工具，使用RO-Crate格式存储和导出数据以确保透明度。

Result: 通过助手间的顺序交互演示了平台功能，能够生成研究论文草稿的各个部分，为AI支持研究提供了社区维护平台的基础。

Conclusion: TIB AIssistant为AI支持研究奠定了基础，旨在建立社区维护的平台，促进研究透明度和可重复性，支持整个研究生命周期。
...
TL;DR: 提出SC2BA环境，用于多智能体强化学习算法的对抗式基准测试，解决现有SMAC基准中对手固定、多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 当前深度多智能体强化学习（MARL）算法评估中，SMAC基准使用固定的内置AI作为对手，导致对手多样性不足，算法评估缺乏全面性。需要建立算法对算法的对抗环境来刷新MARL基准测试。

Method: 基于星际争霸II构建SC2BA环境，专门设计用于算法间对抗，考虑公平性、可用性和可定制性。开发APyMARL库提供易用接口。采用两种对抗模式：双算法配对对抗和多算法混合对抗。

Result: 对经典MARL算法进行了广泛的基准测试，揭示了这些算法在有效性、敏感性和可扩展性方面存在一些值得思考的问题和观察。

Conclusion: SC2BA环境为MARL领域提供了新的对抗式基准测试平台，能够更全面地评估算法性能，有望推动该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI和大型语言模型的快速发展有望改变研究方式，但将AI有效集成到研究中面临挑战，包括领域需求差异、AI素养有限、工具和代理协调复杂，以及生成式AI在研究中的准确性不明确。
...
Conclusion: TIB AIssistant平台为跨学科研究者提供了一个有前景的人机协作解决方案，通过模块化设计支持整个研究生命周期，有望推动AI在研究中的有效集成和应用。
...
TL;DR: TS2R是一个将电池运行时间序列数据转换为结构化报告的提示框架，使LLM能够在电池储能系统管理中执行推理、预测和决策任务，无需重新训练即可达到专家级性能。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在解释多元时间序列数据方面具有潜力，但在实际电池储能系统运维中的应用尚未充分探索。需要一种方法将原始电池运行数据转化为LLM可理解的形式，以支持智能决策。

Method: 提出TimeSeries2Report（TS2R）提示框架，通过分割、语义抽象和基于规则的解释，将短期时间动态编码为自然语言，将低级传感器信号与高级上下文洞察连接起来。

Result: 在实验室和真实数据集上的基准测试表明，相比视觉、嵌入和文本提示基线，TS2R在异常检测、荷电状态预测和充放电管理等任务中，在准确性、鲁棒性和可解释性方面持续提升LLM性能。
...
<details>
  <summary>Details</summary>
Motivation: CUDA内核优化需要硬件-软件协同设计专业知识且高性能内核库通常闭源，现有LLM结合进化算法的方法因智能体设计不佳和演化表示不匹配而性能不足。

Method: 提出策略协调的多智能体框架cuPilot，包括：1) 策略协调演化算法；2) 屋顶线引导提示；3) 策略级种群初始化，将策略作为内核演化的中间语义表示。

Result: 在100个内核基准测试中，cuPilot生成的内核相比PyTorch平均加速3.09倍；在GEMM任务中展示了复杂的优化技术并实现了关键硬件单元的高利用率。

Conclusion: cuPilot通过引入策略作为中间表示解决了现有方法的表示不匹配问题，显著提升了CUDA内核自动优化的性能，生成的代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶安全验证依赖合成数据进行虚拟测试，但研究发现像素级视觉保真度并不能保证系统在仿真和真实环境中基于相同的因果证据做出决策。现有方法缺乏能够衡量系统决策机制一致性的行为基础保真度度量。

Method: 提出决定性特征保真度（DFF）度量方法，利用可解释AI技术识别和比较系统在匹配的真实-合成数据对中的决定性特征。基于反事实解释提出实用估计器，并设计了DFF引导的仿真器校准方案来提升保真度。

Result: 在2126个匹配的KITTI-VirtualKITTI2数据对上的实验表明，DFF能够揭示传统输出值保真度忽略的差异。DFF引导的校准在提升决定性特征和输入级保真度的同时，不会牺牲不同系统间的输出值保真度。

Conclusion: DFF为自动驾驶虚拟测试提供了一种系统特定的行为基础保真度度量，能够更准确地评估合成数据与真实数据在决策机制层面的一致性，有助于提升仿真到真实世界的可靠迁移。
...
<details>
  <summary>Details</summary>
Motivation: 元算法研究（如算法选择、配置和调度）依赖大量计算实验，实验设置自由度大且存在多种错误来源，威胁研究的可扩展性和有效性。现有最佳实践分散在不同出版物和领域，需要系统整合。

Method: 收集COSEAL社区各子领域的良好实践，涵盖整个实验周期：从研究问题提出和实验设计选择，到实验执行，再到结果分析和公正呈现。

Result: 建立了元算法研究领域当前最先进的实践标准，为研究人员和从业者提供了全面的指导框架。

Conclusion: 该报告系统整合了元算法研究的最佳实践，为新研究人员和从业者提供了实用指南，有助于提高研究的严谨性和可重复性。
...
TL;DR: 提出ParamExplorer框架，通过强化学习启发的方法帮助探索生成艺术算法的高维参数空间，减少手动试错，并集成到p5.js项目中


<details>
  <summary>Details</summary>
Motivation: 生成艺术系统通常具有高维复杂的参数空间，其中美学上有趣的输出只占据很小且分散的区域。由于组合爆炸，艺术家通常依赖大量手动试错，导致许多潜在有趣的配置未被发现

Method: 引入ParamExplorer框架，这是一个受强化学习启发的交互式模块化框架，支持人工参与或自动化反馈引导的参数空间探索。框架可无缝集成到现有p5.js项目中，并在框架内实现和评估多种探索策略（称为智能体）

Result: 论文提出了ParamExplorer框架并实现了多种探索策略，但摘要中未提供具体的实验结果数据

Conclusion: ParamExplorer框架通过智能探索策略帮助艺术家更有效地发现生成艺术参数空间中有趣的区域，减少手动试错的工作量
...
TL;DR: 本文提出了Needle in the Web基准测试，专门评估LLM和搜索代理在模糊探索性查询下的网页检索与推理能力，发现现有系统在此类任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试如BrowseComp和xBench-DeepSearch主要关注需要多跳推理的复杂搜索，但忽略了模糊探索性搜索（Fuzzy Exploratory Search）。这类查询具有模糊性和多面性，用户寻求最相关的网页而非单一事实答案。需要专门评估现代搜索代理和LLM系统在此类任务上的能力。

Method: 提出Needle in the Web基准测试，包含7个不同领域的663个问题。采用灵活的方法论，基于网页内容的事实声明可靠地生成可控难度的查询，确保查询质量和答案唯一性。评估了三个领先的LLM和三个基于代理的搜索系统。

Result: 大多数模型表现不佳：许多准确率低于35%，没有模型能在所有领域或难度级别上持续表现出色。这表明Needle in the Web对当前搜索系统构成了显著挑战。

Conclusion: Needle in the Web基准测试揭示了当前搜索系统在模糊探索性查询下的局限性，突出了在语义模糊性下进行有效模糊检索这一开放性问题。该基准为评估和改进LLM及搜索代理在真实世界模糊查询场景中的能力提供了重要工具。
...
TL;DR: 该论文系统研究了CPU-only环境下本地大语言模型和视觉语言模型的推理计算规律，发现了文本长度与计算成本近似线性关系、视觉模型存在分辨率阈值效应，并提出量子启发压缩技术能显著降低计算和能耗。


<details>
  <summary>Details</summary>
Motivation: 虽然GPU主导AI部署，但大多数消费硬件（笔记本电脑、桌面电脑、工业控制器和嵌入式系统）依赖CPU。然而，CPU-only推理本地语言和视觉语言工作负载的计算规律尚未得到充分探索。

Method: 系统化基准测试两个代表性CPU层级：MacBook Pro M2（主流笔记本级）和Raspberry Pi 5（低功耗嵌入式）。采用基于处理器和内存使用连续采样及曲线下面积积分的统一方法，分析计算负载如何随输入文本长度（语言模型）和图像分辨率（视觉语言模型）变化。

Result: 发现两个经验缩放规律：1）语言模型推理计算成本与token长度近似线性关系；2）视觉语言模型存在预处理驱动的"分辨率拐点"，计算在内部分辨率钳位以上保持恒定，以下急剧下降。量子启发压缩技术可将处理器和内存使用降低高达71.9%，能耗降低高达62%，同时保持或提高语义准确性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然记忆增强的个性化LLM代理能提升连续性和相关性，但可能引入偏见风险，这在招聘等关键应用中尤其值得关注

Method: 以招聘为用例，模拟记忆增强的个性化代理行为，研究偏见在不同操作阶段如何被引入和放大

Result: 实验发现，即使使用经过安全训练的LLM，偏见仍会通过个性化过程被系统性地引入和强化
...
TL;DR: 开发了一个基于强化学习和语义嵌入的伊斯兰教法合规聊天机器人，用于回答伊斯兰问题，在功能测试中达到87%的语义准确性。


<details>
  <summary>Details</summary>
Motivation: 在工业4.0时代，需要一种能够提供经过验证的伊斯兰知识的数字工具，以增强宗教素养和数字宣教，弥合传统伊斯兰学术与现代AI咨询之间的差距。

Method: 采用CRISP-DM方法学，结合强化学习（Q-Learning）和Sentence-Transformers进行语义嵌入，处理来自古兰经、圣训和学者法特瓦的25,000个问答对数据集，使用Flask API后端和Flutter移动前端开发原型系统。

Result: 聊天机器人在涵盖教法、信仰、崇拜和交易等多样化主题的功能测试中实现了87%的语义准确性，能够有效处理封闭领域查询。

Conclusion: 该系统展示了增强宗教素养和数字宣教的潜力，但存在静态学习和数据集依赖等限制，未来需要持续适应性和多轮对话支持等改进，作为传统伊斯兰学术与现代AI咨询之间的桥梁。
...
<details>
  <summary>Details</summary>
Motivation: 尽管具备显式推理能力的大语言模型在数学推理方面表现出色，但仍存在过程错误，如计算错误、脆弱的逻辑和表面合理但无效的步骤。需要一种方法来增强推理质量并减少这些错误。

Method: 提出Generative Adversarial Reasoner框架，采用策略联合训练方法，通过对抗强化学习共同进化LLM推理器和基于LLM的判别器。使用计算高效的审查机制将推理链划分为逻辑完整的片段，判别器评估每个片段的合理性并提供结构化理由。学习过程结合互补信号：推理器因逻辑一致的步骤和正确答案获得奖励，判别器因正确检测错误或区分推理轨迹获得奖励。

Result: 在多个数学基准测试中，该方法相比标准RL后训练的强基线模型取得一致提升。在AIME24上，将DeepSeek-R1-Distill-Qwen-7B从54.0提升到61.3（+7.3），将DeepSeek-R1-Distill-Llama-8B从43.7提升到53.7（+10.0）。模块化判别器还支持灵活奖励塑造，适用于教师蒸馏、偏好对齐和基于数学证明的推理等目标。

Conclusion: Generative Adversarial Reasoner框架通过对抗强化学习提供密集、校准良好的步骤级奖励，补充稀疏的精确匹配信号，改善了信用分配，提高了样本效率，并增强了LLM的整体推理质量。该方法在数学推理任务中表现出显著改进，并为多种推理目标提供了灵活的奖励塑造能力。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在现实安全敏感应用中面临检测准确性、推理延迟和部署成本的三方权衡。现有方法通常需要额外模型或多阶段推理，导致高延迟和高成本。

Method: 1. 黑盒检测方法，比较"同意/执行"与"拒绝/安全"前缀的条件对数概率；2. 利用前缀缓存减少检测开销至接近首词延迟；3. 设计高效前缀构造算法自动发现高信息量前缀；4. 推理时只需单次对数概率计算和阈值判断。

Result: 实验表明Prefix Probing检测效果与主流外部安全模型相当，同时计算成本极低，无需额外模型部署，具有强实用性和高效性。

Conclusion: Prefix Probing通过创新的前缀探测方法，有效解决了大语言模型在安全检测中的准确性、延迟和成本权衡问题，为实际应用提供了高效实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI技术快速融入社会，但教育框架未能有效应对，导致危险的素养鸿沟。功能性AI工具使用技能的发展正在掩盖对AI的批判性和伦理推理能力的培养，需要系统性转变。

Method: 提出以人类能动性为核心的AI素养框架，包括AI素养、流畅性和能力三个层次。强调将技术视为可选择而非必然采纳的工具，通过批判性思维和认识论理解来培养教育生态系统所有利益相关者的决策能力。

Result: 通过提出的AI素养、流畅性和能力框架，教育者和学生将成为以人为中心的AI方法的能动者，能够清晰表达决策意图、态度，并理解这些决策对学术、职业和社会的影响。

Conclusion: 需要从功能性AI技能教育转向全面的AI素养教育，以人类能动性为核心，培养批判性思维和伦理决策能力，使技术成为可选择而非必然采纳的工具，确保教育生态系统所有利益相关者都能做出负责任的选择。
...
TL;DR: 使用Apriori算法对印尼语布哈里圣训进行无监督主题关联分析，自动发现圣训文本中的语义关系模式
...
<details>
  <summary>Details</summary>
Motivation: 探究多智能体与单智能体在几何问题求解中的性能差异，明确多智能体设计的实际价值

Method: 系统比较单智能体与多智能体流水线在四个视觉数学基准上的表现：Geometry3K、MathVerse、OlympiadBench和We-Math

Result: 开源模型（如Qwen-2.5-VL）在多智能体模式下性能显著提升，闭源模型（如Gemini-2.0-Flash）在经典基准上单智能体表现更好，仅在新数据集We-Math上有适度改进

Conclusion: 多智能体流水线对开源模型有明显益处，对强大的专有系统在新基准上有辅助作用，但智能体分解并非普遍最优
...
TL;DR: 提出"教育中的赛博人文主义"框架，旨在生成式AI重塑教育的背景下，通过反思能力、算法公民权和对话式设计三大支柱，重新确立人类在AI驱动学习环境中的主体性。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正在从根本上改变教育中知识生产和验证的方式，将阅读、写作和编程重构为混合的人机工作流程。这引发了关于认知自动化、认知卸载和教师去专业化的担忧。需要一个新的框架来在教育中重新确立人类的主体性。

Method: 提出"教育中的赛博人文主义"框架，将AI赋能的学习环境概念化为由人类和机器共同构建的社会技术基础设施。提出三大设计支柱：反思能力、算法公民权和对话式设计，并将其与国际数字和AI能力框架相关联。通过高等教育案例研究（基于提示的学习和"对话式AI教育者"认证）来实施这些理念。

Result: 研究发现，这些实践可以增强认知主体性，同时揭示了关于工作量、公平性和治理方面的紧张关系。为AI丰富、以人为本教育的未来发展提供了启示。

Conclusion: 赛博人文主义框架为在AI驱动的教育环境中重新确立人类主体性提供了理论基础和实践路径，将教育者和学习者定位为具有塑造这些基础设施权利和责任的认知主体和算法公民。
...
TL;DR: 该论文形式化了算法智能的两个独立计算限制：形式不完备性和动态不可预测性，并证明它们共同限制了智能体对其自身预测能力的推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究算法智能的根本计算限制，特别是智能体如何推理自身预测能力的问题。作者旨在理解形式不完备性（哥德尔不完备定理）和动态不可预测性（混沌理论）如何共同约束智能系统的能力。

Method: 通过形式化分析，将形式不完备性和动态不可预测性作为两个独立的计算限制进行建模。作者展示了这两个极端如何共同作用，对智能体的自我推理能力施加结构性限制。

Result: 证明算法智能体通常无法计算自身的最大预测范围。形式不完备性限制了推理系统的演绎能力，而动态不可预测性限制了有限精度下的长期预测能力，这两者共同约束了智能体对其自身预测能力的分析。

Conclusion: 智能系统中存在推理、预测和自我分析之间的固有权衡。算法智能体无法完全理解自身的预测能力极限，这揭示了智能系统的基本结构限制。
...
TL;DR: 该论文提出了一种使用PDDL风格表示来高效学习和建模黑盒AI系统规划能力的方法，通过蒙特卡洛树搜索生成测试任务并学习符号模型，能够描述AI的能力、执行条件和概率结果。
...
Conclusion: PDDL风格表示能够有效学习和建模黑盒AI系统的规划能力，为理解这些系统的能力提供了可靠且可解释的框架。
...
<details>
  <summary>Details</summary>
Motivation: 肺癌患者中高达91%会经历突破性疼痛，需要及时干预。现有方法难以有效预测疼痛发作，特别是结合临床文本等非结构化数据时存在挑战。

Method: 开发混合机器学习与大型语言模型管道：1) 机器学习模块捕捉时间性药物趋势；2) 大型语言模型解析模糊剂量记录和自由文本临床笔记；3) 整合两种模态分析266名住院患者的回顾性队列数据，包括人口统计学、肿瘤分期、生命体征和WHO分级镇痛药使用。

Result: 框架在48小时预测准确率达0.874，72小时达0.917。大型语言模型增强使敏感性分别提升8.6%和10.4%。整合多模态数据改善了敏感性和可解释性。

Conclusion: 这种混合方法提供了临床可解释且可扩展的早期疼痛发作预测工具，有望提高肿瘤护理中的治疗精确性和资源分配优化。
...
TL;DR: CitySeeker是一个评估视觉语言模型在动态城市环境中处理隐含需求导航能力的新基准，包含6440条轨迹，发现当前最佳模型仅能完成21.1%的任务，揭示了长时推理错误累积、空间认知不足和体验回忆缺陷等关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在显式指令导航方面取得显著进展，但在动态城市环境中解释隐含人类需求（如"我渴了"）的能力尚未充分探索。需要评估模型的空间推理和决策能力，以解决"最后一公里"导航挑战。

Method: 提出CitySeeker基准，包含6440条轨迹，覆盖8个城市和7个目标驱动场景。采用BCR策略（回溯机制、丰富空间认知、基于记忆的检索），受人类认知映射中迭代观察-推理循环和自适应路径优化的启发。

Result: 实验显示即使最佳模型（如Qwen2.5-VL-32B-Instruct）任务完成率仅为21.1%。识别出三个关键瓶颈：长时推理中的错误累积、空间认知不足、体验回忆缺陷。BCR策略为改进提供了可操作的见解。

Conclusion: CitySeeker基准揭示了当前视觉语言模型在隐含需求导航方面的严重不足，为开发具有强大空间智能的模型提供了重要见解，有助于解决"最后一公里"导航挑战。
...
TL;DR: TOGGLE：首个将形式化方法（信号时序逻辑）集成到LLM压缩中的框架，通过贝叶斯优化探索量化与剪枝配置，在保证语言属性约束下实现最高3.3倍计算成本降低和68.8%模型压缩。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在边缘设备部署面临计算资源限制，现有压缩技术（如量化和剪枝）会损害关键语言属性且缺乏形式化保证，需要一种能保持模型语言行为的形式化压缩方法。

Method: 提出TOGGLE框架，利用信号时序逻辑（STL）形式化指定语言属性约束，采用STL鲁棒性引导的贝叶斯优化系统探索层级的量化和剪枝配置，生成满足形式化约束的压缩模型，无需重新训练或微调。

Result: 在GPT-2、DeepSeek-V2 7B、LLaMA 3 8B和Mistral 7B四个LLM架构上评估，实现最高3.3倍计算成本（FLOPs）降低和68.8%模型大小压缩，同时满足所有指定的语言属性约束。

Conclusion: TOGGLE是首个将形式化方法集成到LLM压缩中的框架，通过形式化保证语言属性，实现了高效、可验证的LLM边缘部署，为资源受限设备上的可靠模型压缩提供了新途径。
...
TL;DR: 论文提出"拼凑式AGI"假说，认为通用智能可能首先通过多个子AGI智能体协调实现，而非单一AGI系统。为此需要超越个体对齐，建立分布式AGI安全框架，包括虚拟智能体沙盒经济和市场治理机制。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全研究主要关注单一AGI系统的对齐，但忽视了"拼凑式AGI"假说——即通用能力可能首先通过多个子AGI智能体协调实现。随着具有工具使用和协调能力的AI智能体快速部署，这种分布式智能带来的安全风险需要紧急关注。

Method: 提出分布式AGI安全框架，核心是设计和实施虚拟智能体沙盒经济（不透或半透），通过稳健的市场机制治理智能体间交易，配合适当的可审计性、声誉管理和监督来缓解集体风险。

Result: 论文提出了一个超越个体智能体评估和对齐的分布式AGI安全框架，强调需要通过市场机制、审计和声誉系统来管理智能体群体协调带来的集体风险。

Conclusion: "拼凑式AGI"假说需要认真考虑，应开发相应的安全保障措施。分布式AGI安全框架为解决智能体群体协调带来的新安全挑战提供了方向，是当前AI安全研究的紧迫课题。
...
Method: 提出社会责任堆栈(SRS)框架，包含六层架构，采用基于约束的统一表述，引入安全包络和反馈解释，将责任建模为技术社会系统的闭环监督控制问题。

Result: 展示了SRS如何将公平性、自主性、认知负担和解释质量等规范目标转化为可操作的工程控制，并通过临床决策支持、协作自动驾驶车辆和公共部门系统的案例研究进行了验证。

Conclusion: SRS框架连接了伦理学、控制理论和AI治理，为可问责、自适应和可审计的技术社会AI系统提供了实用基础，将规范性目标转化为可执行的工程和操作控制。
...
<details>
  <summary>Details</summary>
Motivation: 需要开发一个易于部署、支持野外操作和灵活相机配置的快速体积捕获系统，能够处理不同输入类型并生成多种3D表示格式，同时保持高质量重建效果。

Method: 基于改进的GPS-Gaussian回归器，支持RGB-D或RGB输入，生成点云和高斯溅射3D表示。系统设计支持野外操作、不受控光照和任意背景，以及灵活的相机配置（稀疏设置、任意相机数量和基线）。

Result: 系统实现5-10 FPS的实时预览，支持PLY、MPEG V-PCC、SPLAT等标准格式导出，提供基于Web的查看器和Unity/Unreal插件，开源框架便于复现和进一步研究。

Conclusion: 提出了一个快速高效的体积捕获与重建系统，通过改进GPS-Gaussian回归器实现了高质量重建，支持灵活部署和多种输出格式，为3D重建研究提供了开源工具。
...
TL;DR: 提出一种基于分箱的照明模型，将轨迹结构从密度中解耦，通过局部自适应照明突出主要趋势和异常轨迹，同时保持原始色彩映射。


<details>
  <summary>Details</summary>
Motivation: 传统密度图在处理轨迹或时间序列等线基数据集时存在不足，会破坏路径连续性，模糊平滑趋势和罕见异常。需要一种方法既能显示密度信息，又能保持路径结构和突出异常模式。

Method: 提出基于分箱的照明模型：1) 引入基于分箱的异常度度量对轨迹进行排序；2) 根据排序构建结构法线图；3) 在亮度通道应用局部自适应照明来突出选定模式（从主要趋势到异常路径）；4) 保持可接受的色彩失真。

Result: 方法在多个真实数据集上验证：1) 能揭示简单替代方法遗漏的细节；2) 相比标准着色实现显著更低的CIEDE2000色彩失真；3) 支持高达10,000条线的交互式更新。

Conclusion: 该交互式方法使分析人员能够在主要趋势和异常检测之间取得平衡，有效增强线基数据集的可视化，在保持色彩保真度的同时突出结构特征和稀疏异常。
...
### [204] [DiscoverDCP: A Data-Driven Approach for Construction of Disciplined Convex Programs via Symbolic Regression](https://arxiv.org/abs/2512.15721)
*Sveinung Myhre*

Main category: cs.LG

TL;DR: DiscoverDCP：结合符号回归与DCP规则的框架，自动发现全局凸的模型表达式，避免后验凸性验证的计算负担


<details>
  <summary>Details</summary>
Motivation: 传统固定参数凸表达式（如二次函数）形式受限且不够准确，而系统辨识中需要可验证的凸模型用于安全关键的控制和优化任务
...
Conclusion: DiscoverDCP框架通过数据驱动方式自动构建全局凸模型，避免了后验凸性验证的计算困难，适用于安全关键的控制和优化应用
...
### [205] [Hybrid Quantum-Classical Ensemble Learning for S\&P 500 Directional Prediction](https://arxiv.org/abs/2512.15738)
*Abraham Itzhak Weinberg*

Main category: cs.LG

TL;DR: 提出混合集成框架，结合量子情感分析、决策Transformer和策略模型选择，在S&P500预测中达到60.14%方向准确率，比单个模型提升3.10%


<details>
  <summary>Details</summary>
Motivation: 金融市场预测具有挑战性，大多数模型难以超过55-57%准确率，因为存在高噪声、非平稳性和市场效率问题。现有方法存在三个主要局限：架构多样性不足、情感分析能力有限、集成策略不够智能

Method: 1) 混合集成框架结合多种学习算法(LSTM、决策Transformer、XGBoost、随机森林、逻辑回归)；2) 使用4量子比特变分量子电路增强情感分析；3) 智能过滤排除准确率低于52%的弱预测器；4) 基于置信度的过滤策略(6+模型共识)

Result: 在2020-2023年市场数据上，框架达到60.14%方向准确率，比相同架构模型在多个数据集上的52.80%显著提升。量子情感分析为每个模型带来0.8-1.5%增益。Top-7模型集成性能(60.14%)远优于全部35个模型(51.2%)。McNemar检验统计显著(p<0.05)。回测显示夏普比率1.2 vs 买入持有0.8

Conclusion: 架构多样性比数据集多样性更重要，量子增强情感分析能有效提升预测性能，智能模型选择对集成学习至关重要。该框架在真实市场条件下表现出实用交易潜力
...
### [206] [SHARe-KAN: Holographic Vector Quantization for Memory-Bound Inference](https://arxiv.org/abs/2512.15742)
*Jeff Smith*

Main category: cs.LG

TL;DR: SHARe-KAN框架通过增益-形状-偏置向量量化解决KANs的内存瓶颈问题，实现88倍运行时内存减少，同时保持原始精度。


<details>
  <summary>Details</summary>
Motivation: Kolmogorov-Arnold Networks (KANs)面临基本的内存墙问题：其学习的基础函数导致参数数量激增，产生极高的带宽需求，阻碍了在内存受限环境中的部署。传统剪枝方法在KANs上失效，因为KANs具有全息拓扑结构，信息分布在样条干涉中而非局部化到特定边。

Method: 提出SHARe-KAN框架，采用增益-形状-偏置向量量化(Gain-Shape-Bias Vector Quantization)来利用函数冗余同时保持密集拓扑结构。结合LUTHAM硬件感知编译器进行静态内存规划。

Result: 在PASCAL VOC上实现88倍运行时内存减少（从1.13 GB降至12.91 MB），同时匹配未压缩基线的准确率。在NVIDIA Ampere架构上分析显示>90%的L2缓存驻留，证明工作负载已从DRAM带宽约束中解耦。

Conclusion: SHARe-KAN通过创新的量化方法和硬件感知编译技术，有效解决了KANs的内存瓶颈问题，使其能够在内存受限环境中部署，同时保持模型性能。
...
### [207] [How Do Graph Signals Affect Recommendation: Unveiling the Mystery of Low and High-Frequency Graph Signals](https://arxiv.org/abs/2512.15744)
*Feng Liu,Hao Cang,Huanhuan Yuan,Jiaqing Fan,Yongjing Hao,Fuzhen Zhuang,Guanfeng Liu,Pengpeng Zhao*

Main category: cs.LG

TL;DR: 论文提出图信号在推荐系统中的新视角，证明低频和高频图信号在推荐任务中具有等效作用，并提出可插拔的频率信号缩放器和空间翻转方法增强GNN推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究对低频和高频图信号在推荐系统中的作用存在争议，传统观点认为GNN的成功主要源于低频滤波，但近期研究表明高频信号也很重要。本文旨在澄清不同频率图信号在推荐任务中的实际作用。

Method: 1) 理论证明低频和高频图信号在推荐任务中等效，都通过平滑用户-物品对相似性发挥作用；2) 提出频率信号缩放器，可调整图信号滤波函数以微调相似性平滑度；3) 引入空间翻转方法，解决图嵌入方法无法完全捕捉图信号特性的问题。

Result: 实验证明：1) 仅使用低频或高频图信号就能实现有效推荐；2) 提出的方法在四个公开数据集上表现优异；3) 频率信号缩放器与任何GNN模型兼容，能显著提升性能。

Conclusion: 低频和高频图信号在推荐任务中具有等效作用，都通过平滑用户-物品相似性来提升推荐性能。提出的频率信号缩放器和空间翻转方法能有效增强GNN推荐系统的表达能力。
...
### [208] [LLaDA2.0: Scaling Up Diffusion Language Models to 100B](https://arxiv.org/abs/2512.15745)
...
<details>
  <summary>Details</summary>
Motivation: 论文旨在建立前沿规模部署的新范式，避免从头训练的高成本，通过继承预训练模型的知识，实现高效的知识迁移和模型转换。

Method: 采用3阶段块级WSD训练方案：1）渐进增加块大小的块扩散（预热）；2）大规模全序列扩散（稳定）；3）回归紧凑块扩散（衰减）。结合SFT和DPO进行后训练对齐，得到16B和100B的MoE变体。

Result: 开发了LLaDA2.0-mini（16B）和LLaDA2.0-flash（100B）两个指令调优的MoE变体，保持了并行解码优势，在性能与效率方面表现优异，模型已开源。

Conclusion: LLaDA2.0通过系统化转换自回归模型为离散扩散模型，建立了前沿规模部署的新范式，实现了知识继承、渐进适应和效率优化的设计原则，为大规模语言模型部署提供了高效解决方案。
...
### [209] [A Unified Generative-Predictive Framework for Deterministic Inverse Design](https://arxiv.org/abs/2512.15746)
*Reza T. Batley,Sourav Saha*

Main category: cs.LG

TL;DR: Janus框架统一生成与预测，通过解缠的潜在空间实现异构材料微结构的实时物理引导逆向设计


<details>
  <summary>Details</summary>
Motivation: 异构材料微结构的逆向设计是一个不适定且计算昂贵的问题，传统生成模型难以支持快速、稳定的确定性物理引导逆向求解

Method: 提出Janus框架，结合深度编码器-解码器架构与可分离的KHRONOS预测头，学习同时支持生成逆向和物理预测的等距潜在流形，通过联合目标诱导潜在空间解缠

Result: 在MNIST数据集上实现高保真重建、准确分类和多样逆向生成；在热导率标记的微结构逆向设计中，前向预测精度R²=0.98，重建误差低于5%，逆向解满足目标属性1%相对误差

Conclusion: Janus通过统一预测和生成于单一潜在空间，实现了实时、物理引导的微结构逆向生成，计算成本远低于传统优化方法
...
### [210] [D3G: Diverse Demographic Data Generation Increases Zero-Shot Image Classification Accuracy within Multimodal Models](https://arxiv.org/abs/2512.15747)
*Javon Hickmon*

Main category: cs.LG

TL;DR: 提出D3G方法，利用生成模型为CLIP等预训练多模态模型提供多样化人口统计数据，提升零样本图像分类准确率并减少人口统计偏见。


<details>
  <summary>Details</summary>
Motivation: 现有图像分类模型面临容量不足导致细粒度分类表现差、数据质量要求高、人口统计不平衡导致偏见等问题，特别是在零样本分类中这些偏见可能造成危害。

Method: 提出D3G方法：基于CLIP作为多模态基础模型，使用Stable Diffusion XL作为生成模型，在推理时生成多样化人口统计数据来提升分类性能，无需额外训练。

Result: 实验表明，在推理时提供多样化人口统计数据能显著提升模型性能，并探讨了不同人口统计特征对准确率指标的影响。

Conclusion: D3G是一种有效的训练免费、零样本方法，能够同时提升预训练多模态模型的分类准确率并减少人口统计偏见。
...
### [211] [Surely Large Multimodal Models (Don't) Excel in Visual Species Recognition?](https://arxiv.org/abs/2512.15748)
*Tian Liu,Anwesha Basu,James Caverlee,Shu Kong*

Main category: cs.LG

TL;DR: 提出POC方法，利用大语言多模态模型对少样本学习专家模型的预测进行后验校正，在视觉物种识别任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 视觉物种识别需要专业标注，但专家只能标注少量样本。虽然大语言多模态模型在通用识别任务表现出色，但在专业物种识别任务中表现不佳，甚至不如简单的少样本学习专家模型。然而发现LMMs可以有效校正专家模型的错误预测。

Method: 提出后验校正方法，使用LMMs重新排序专家模型的top预测。方法包括：1) 获取专家模型的top预测和置信度分数；2) 构建包含置信度分数和少样本视觉示例的丰富提示；3) 使用LMMs重新排序预测

Result: 在五个具有挑战性的VSR基准测试中，POC方法比现有少样本学习方法准确率提升+6.4%，无需额外训练、验证或人工干预。方法具有通用性，适用于不同的预训练骨干网络和LMMs

Conclusion: POC方法作为即插即用模块，能显著增强现有少样本学习方法，在视觉物种识别任务中实现专家模型与LMMs的优势互补
...
### [212] [A Special Case of Quadratic Extrapolation Under the Neural Tangent Kernel](https://arxiv.org/abs/2512.15749)
*Abiel Kim*
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注ReLU MLP的线性外推机制，但在NTK机制下，对于原点附近的外推分析仍不充分。由于NTK特征映射不具有平移不变性，原点附近的外推与远离原点的外推是不同的特殊情况，需要专门研究

Method: 在神经正切核（NTK）机制下，分析ReLU多层感知机在原点附近的外推行为。利用NTK特征映射的旋转不变性，研究原点附近外推的特殊情况

Result: 发现ReLU MLP在NTK机制下，对于靠近原点的外推点表现出二次外推行为，这与远离原点时的线性外推形成对比

Conclusion: ReLU MLP在NTK机制下的外推行为取决于外推点与训练数据分布的距离：远离原点时表现为线性外推，靠近原点时表现为二次外推。这为理解神经网络外推行为提供了更完整的理论框架
...
### [213] [GLOW: Graph-Language Co-Reasoning for Agentic Workflow Performance Prediction](https://arxiv.org/abs/2512.15751)
*Wei Guan,Jian Cao,Jinyu Cai,Qiqi Cai,Jianqi Gao,See-Kiong Ng*

Main category: cs.LG

TL;DR: GLOW是一个结合GNN图结构建模能力和LLM语义推理能力的统一框架，用于预测Agentic Workflows的性能表现，解决了现有方法无法同时捕捉拓扑依赖和语义逻辑的问题。


<details>
  <summary>Details</summary>
Motivation: Agentic Workflows自动化生成的可扩展性受到执行评估高成本和延迟的严重制约。现有的AW性能预测方法作为替代方案，但无法同时捕捉AW中复杂的拓扑依赖关系和深层语义逻辑。

Method: 提出GLOW统一框架：1）引入经过图任务指令调优的图导向LLM，提取拓扑感知的语义特征；2）将LLM提取的语义特征与GNN编码的结构表示进行融合；3）采用对比对齐策略进一步优化潜在空间以区分高质量AW。

Result: 在FLORA-Bench上的大量实验表明，GLOW在预测准确性和排序效用方面优于最先进的基线方法。

Conclusion: GLOW通过结合GNN的图结构建模能力和LLM的推理能力，成功解决了AW性能预测中同时捕捉拓扑依赖和语义逻辑的挑战，为AW自动化生成提供了有效的性能预测解决方案。
...
### [214] [TAO-Net: Two-stage Adaptive OOD Classification Network for Fine-grained Encrypted Traffic Classification](https://arxiv.org/abs/2512.15753)
*Zihao Wang,Wei Peng,Junming Zhang,Jian Li,Wenxin Fang*

Main category: cs.LG

TL;DR: TAO-Net：两阶段自适应OOD加密流量分类网络，通过混合OOD检测和LLM语义增强提示，实现ID/OOD区分和细粒度OOD分类，显著提升新兴应用识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有加密流量分类方法依赖预定义类别，无法有效处理不断出现的新应用产生的OOD流量模式。虽然有些方法将未知流量简单归为"其他"类别，但缺乏细粒度分类能力。

Method: 提出两阶段自适应OOD分类网络：第一阶段采用混合OOD检测机制，结合transformer层间变换平滑度和特征分析区分ID/OOD流量；第二阶段利用大语言模型和语义增强提示策略，将OOD分类转化为生成任务，实现无需预定义标签的细粒度分类。

Result: 在三个数据集上，TAO-Net达到96.81-97.70%宏精度和96.77-97.68%宏F1分数，显著优于先前方法（仅44.73-86.30%宏精度），特别在识别新兴网络应用方面表现突出。

Conclusion: TAO-Net通过创新的两阶段设计有效解决了加密流量分类中的OOD问题，实现了对已知和未知流量的准确分类，为处理不断变化的应用环境提供了灵活解决方案。
...
### [215] [KAN-Matrix: Visualizing Nonlinear Pairwise and Multivariate Contributions for Physical Insight](https://arxiv.org/abs/2512.15755)
*Luis A. De la Fuente,Hernan A. Moreno,Laura V. Alvarez,Hoshin V. Gupta*

Main category: cs.LG

TL;DR: 提出基于Kolmogorov-Arnold Networks (KANs)的两种可视化工具PKAN和MKAN，用于增强高维数据解释性，超越传统相关性分析


<details>
  <summary>Details</summary>
Motivation: 解决复杂数据集解释的挑战，特别是高维度和变量共线性问题，传统相关性分析在非线性关系和特征重要性评估方面存在局限

Method: 开发两种基于KANs的可视化工具：PKAN（成对KAN矩阵）用于表征变量间的非线性关联，MKAN（多元KAN贡献矩阵）作为非线性特征排序工具量化输入变量对目标变量的相对贡献

Result: 实验比较显示PKAN和MKAN比Pearson相关性和互信息提供更稳健和信息丰富的结果，能捕捉关系的强度和函数形式

Conclusion: PKAN和MKAN工具支持模型开发工作流程的预处理和后处理，促进隐藏物理模式的发现和领域知识驱动的模型开发
...
### [216] [ReactorFold: Generative discovery of nuclear reactor cores via emergent physical reasoning](https://arxiv.org/abs/2512.15756)
*Yoonpyo Lee*

Main category: cs.LG

TL;DR: ReactorFold：将核反应堆燃料组件设计转化为序列建模问题，使用语言模型生成新型设计拓扑，能自主调整设计参数并发现传统方法无法触及的高性能非对称配置。


<details>
  <summary>Details</summary>
Motivation: 传统核反应堆堆芯设计方法（确定性、元启发式、机器学习辅助）在固定的人工定义配置空间中搜索，限制了发现全新设计拓扑的能力。需要一种能超越人为约束、发现根本性新设计的生成式方法。

Method: 将燃料组件设计重新定义为语言模型的序列建模问题。使用蒙特卡洛数据、参数高效微调和直接偏好优化（DPO）训练模型，学习压水堆组件的潜在结构，并通过单次前向传播生成候选布局。

Result: DPO对齐的模型展现出设计空间扩展的涌现能力：尽管仅在固定数量钆可燃吸收体（Gd）棒配置上训练，却能自主调整Gd库存以满足严格的功率峰值约束。模型还发现了挑战传统对称装载启发式的高性能非对称配置，进入了传统搜索方法无法触及的设计区域。

Conclusion: 语言模型能够内化因果物理关系并超越人为施加的设计约束，为核反应堆设计开辟了新的可能性，展示了生成式AI在复杂工程优化问题中的潜力。
...
### [217] [Twin Restricted Kernel Machines for Multiview Classification](https://arxiv.org/abs/2512.15757)
*A. Quadir,M. Sajid,Mushir Akhtar,M. Tanveer*

Main category: cs.LG

TL;DR: 提出了一种新的多视图双限制核机(TMvRKM)，通过正则化最小二乘方法高效确定最优分离超平面，解决了传统核方法计算复杂和泛化性能不足的问题。


<details>
  <summary>Details</summary>
Motivation: 传统多视图支持向量机(MvSVM)在捕获高维空间决策边界时面临挑战，容易产生误差且难以处理视图不一致问题。现有方法需要解决大规模二次规划问题，计算效率低下。

Method: 提出TMvRKM模型，结合核机与多视图框架的优势。采用正则化最小二乘方法替代传统二次规划，包含耦合项平衡多视图误差。集成早期和晚期融合策略，充分利用所有视图信息同时保持对单个视图变化的灵活性。

Result: 在UCI、KEEL和AwA基准数据集上的实验和统计分析显示，TMvRKM在所有场景中都优于基线模型，展现出卓越的泛化性能。

Conclusion: TMvRKM有效解决了传统多视图核方法的计算效率和泛化性能问题，通过创新的融合策略和优化方法在多视图学习中取得了显著改进。
...
### [218] [Yantra AI -- An intelligence platform which interacts with manufacturing operations](https://arxiv.org/abs/2512.15758)
*Varshini Krishnamurthy*

Main category: cs.LG

TL;DR: 开发XRIT智能生产系统，集成机器学习模型和AI虚拟助手，提升生产效率、能源管理和预测性维护
...
Method: 构建智能生产系统，集成随机森林分类器用于预测性维护、隔离森林用于异常检测，使用Streamlit实现实时数据可视化，并集成GPT-4驱动的虚拟助手

Result: 系统通过模拟数据测试，显著提高了工作效率、能源管理和维修规划能力，具备可扩展性，可应用于XRIT的实际生产环境

Conclusion: 开发的智能生产系统有效解决了工业4.0环境下的关键生产问题，未来将专注于实时数据集成和系统优化改进
...
### [219] [Semantic-Constrained Federated Aggregation: Convergence Theory and Privacy-Utility Bounds for Knowledge-Enhanced Distributed Learning](https://arxiv.org/abs/2512.15759)
*Jahidul Arafat*

Main category: cs.LG

TL;DR: SCFA框架将领域知识约束融入联邦学习聚合过程，通过理论证明约束能减少数据异构性、改善隐私-效用权衡，并在制造业预测性维护数据上验证了22%的收敛加速和41.3%的模型分歧减少。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习解决方案在非IID数据条件下收敛缓慢，且对所有客户端更新采用相同处理方式，忽略了语义有效性。需要将领域知识约束融入分布式优化以改善性能。

Method: 提出语义约束联邦聚合(SCFA)框架，将领域知识约束纳入分布式优化。使用知识图谱编码约束（从ISA-95和MASON本体中提取3000个约束），在理论分析中证明收敛率O(1/sqrt(T) + rho)，其中rho表示约束违反率。

Result: 理论分析显示约束减少41%的有效数据异构性，隐私-效用权衡改善因子θ=0.37。在ε=10的差分隐私下，约束正则化保持效用仅比非隐私基线下降3.7%，而标准联邦学习下降12.1%。在Bosch生产数据实验中实现22%收敛加速、41.3%模型分歧减少。

Conclusion: SCFA是首个为基于约束的联邦学习建立收敛理论框架的方法，通过领域知识约束显著改善非IID数据下的联邦学习性能，理论预测与实证结果高度一致(R^2>0.90)。
...
### [220] [A Tutorial on Dimensionless Learning: Geometric Interpretation and the Effect of Noise](https://arxiv.org/abs/2512.15760)
...
<details>
  <summary>Details</summary>
Motivation: 传统实验数据分析中，从测量数据中发现物理定律和标度关系具有挑战性。研究者需要一种系统的方法，能够从实验数据中自动发现无因次数和标度律，同时保证结果的物理可解释性。

Method: 该方法结合经典量纲分析与机器学习：首先从物理量测量值中识别基本变量组合成无因次群的方法，然后使用神经网络发现哪些组合最能预测实验输出。关键创新是采用正则化技术，使学习到的系数取简单可解释的值（如整数或半整数）。

Result: 该方法能够成功处理单个或多个无因次数的情况，揭示不同但等效的表示如何捕捉相同的基础物理。正则化方法对测量噪声和离散采样具有鲁棒性，能够发现既准确又具有物理意义的定律。

Conclusion: 无因次学习为从实验数据中发现物理定律提供了强大框架，但仍面临计算成本、数据特性影响、变量选择自动化和用户友好工具开发等挑战。本教程既是教育资源，也是研究人员应用该方法的实用指南。
...
### [221] [Machine Learning Framework for Thrombosis Risk Prediction in Rotary Blood Pumps](https://arxiv.org/abs/2512.15761)
*Christopher Blum,Michael Neidlin*

Main category: cs.LG

TL;DR: 提出一个基于可解释机器学习的空间血栓评估框架，直接从CFD流场特征预测血栓风险区域，相比传统计算模型更高效透明。
...
Method: 使用逻辑回归模型结合结构化特征选择流程，从CFD导出的流动特征中提取紧凑且物理可解释的特征集（包括非线性特征组合），基于已验证的宏观血栓模型的空间风险模式进行训练。

Result: 模型成功复现了标注的风险分布，识别出与血栓风险增加相关的不同流动特征集。即使仅在单个轴向泵工况上训练，当应用于离心泵时仍能预测合理的血栓易发区域。

Conclusion: 可解释机器学习能够将局部流动特征与血栓风险联系起来，同时保持计算高效和机理透明。该框架为将可解释机器学习集成到CFD驱动的血栓分析和设备设计工作流程提供了方法基础。
...
### [222] [Cross-Sample Augmented Test-Time Adaptation for Personalized Intraoperative Hypotension Prediction](https://arxiv.org/abs/2512.15762)
*Kanxue Li,Yibing Zhan,Hua Jin,Chongchong Qi,Xu Lin,Baosheng Yu*

Main category: cs.LG

TL;DR: 提出CSA-TTA框架，通过跨样本增强解决术中低血压预测中测试时训练数据稀缺问题，提升个性化预测性能


<details>
  <summary>Details</summary>
Motivation: 术中低血压预测面临患者特异性差异大、低血压事件罕见导致测试时训练不可靠的挑战

Method: 构建跨样本库，采用粗到细检索策略（K-Shape聚类+语义相似度检索），结合掩码重建和序列预测的自监督训练

Result: 在VitalDB和院内数据集上，CSA-TTA显著提升Recall和F1分数，在零样本场景下提升尤为明显（+7.46% Recall）

Conclusion: CSA-TTA通过跨样本增强有效解决罕见事件导致的测试时训练不可靠问题，展现出强大的鲁棒性和泛化能力
...
### [223] [AdaGradSelect: An adaptive gradient-guided layer selection method for efficient fine-tuning of SLMs](https://arxiv.org/abs/2512.15764)
*Anshul Kumar,Gagan Raj Gupta,Manisha Chawla*

Main category: cs.LG

TL;DR: AdaGradSelect是一种针对小型语言模型的自适应参数高效微调方法，通过基于梯度选择要更新的Transformer块，在保持接近全微调性能的同时显著减少训练时间和内存使用。


<details>
  <summary>Details</summary>
Motivation: 全微调大型语言模型成本高昂且内存需求大，而现有的参数高效微调方法（如LoRA）将训练限制在有限子空间中，有时会降低性能。对于小型语言模型，效率提升更为重要，因此需要一种更有效的微调方法。

Method: AdaGradSelect是一种自适应方法，基于梯度选择要更新的Transformer块。它结合了Dirichlet采样（基于历史更新频率）和epsilon-greedy探索策略，早期探索不同块，后期逐渐聚焦于最重要的块。

Result: AdaGradSelect训练速度提升约12%，GPU内存使用减少35%，性能接近全微调。在GSM8K数据集上，相比LoRA（rank 256）平均提升约3%，在MATH数据集上达到相似准确率。

Conclusion: AdaGradSelect为传统微调方法提供了更有效和资源高效的替代方案，特别适用于小型语言模型，在保持性能的同时显著提升效率。
...
### [224] [Data Valuation for LLM Fine-Tuning: Efficient Shapley Value Approximation via Language Model Arithmetic](https://arxiv.org/abs/2512.15765)
*Mélissa Tamine,Otmane Sakhi,Benjamin Heymann*

Main category: cs.LG

TL;DR: 本文提出了一种针对使用DPO训练的大语言模型的高效数据价值评估方法，利用DPO的数学结构简化了Shapley值的计算


<details>
  <summary>Details</summary>
Motivation: 数据是训练大语言模型的关键资产，但数据价值评估（特别是使用Shapley值）通常计算成本高昂，需要大量模型重训练。对于大模型来说，这种计算负担尤为严重，阻碍了数据所有者做出明智的投资决策和公平的利益分配。

Method: 利用直接偏好优化（DPO）训练大语言模型的特定数学结构，开发可扩展的Shapley值计算方法。DPO的特殊形式使得数据价值评估的计算复杂度大幅降低。

Result: 证明了对于使用DPO训练的LLMs，数据价值评估的计算挑战显著简化，能够实现可扩展的Shapley值计算，为数据价值评估与大语言模型的交叉应用打开了可能性。

Conclusion: DPO的数学结构为LLMs的数据价值评估提供了高效解决方案，这一发现有望促进数据所有者之间的合作投资和公平利益分配，推动数据价值评估在大型语言模型领域的实际应用。
...
### [225] [Bridging Data and Physics: A Graph Neural Network-Based Hybrid Twin Framework](https://arxiv.org/abs/2512.15767)
*M. Gorpinich,B. Moya,S. Rodriguez,F. Meraghni,Y. Jaafra,A. Briot,M. Henner,R. Leon,F. Chinesta*

Main category: cs.LG

TL;DR: 提出基于图神经网络的混合孪生方法，用少量数据学习物理模型与真实系统间的差距（无知模型），提升仿真精度


<details>
  <summary>Details</summary>
Motivation: 传统有限元等物理模型存在未建模效应和简化假设，与真实系统存在差距；纯数据驱动方法需要大量高质量数据，而实际场景中数据稀疏且难以获取

Method: 采用混合孪生方法，结合物理模型与数据驱动的无知模型；使用图神经网络表示无知模型，即使测量点稀疏也能学习空间模式；在不同网格、几何和载荷位置的非线性热传导问题上验证

Result: GNN成功捕捉无知模型，在不同空间配置间泛化修正，提高仿真精度和可解释性，同时最小化数据需求

Conclusion: GNN混合孪生方法能有效弥补物理模型与真实系统的差距，在数据稀疏条件下提升仿真性能，为复杂非稳态物理现象建模提供可靠方案
...
### [226] [TENG++: Time-Evolving Natural Gradient for Solving PDEs With Deep Neural Nets under General Boundary Conditions](https://arxiv.org/abs/2512.15771)
*Xinjie He,Chenggong Zhang*

Main category: cs.LG

TL;DR: 将时间演化自然梯度（TENG）框架扩展到处理狄利克雷边界条件，通过结合自然梯度优化与数值时间步进方案（欧拉和Heun方法），在热方程上实现了高精度求解。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法在处理高维或复杂PDE问题时存在困难，而物理信息神经网络（PINNs）虽然提供了替代方案，但在实现高精度和处理复杂边界条件方面仍有挑战。需要开发更有效的方法来精确处理边界条件。

Method: 扩展时间演化自然梯度（TENG）框架以处理狄利克雷边界条件，将自然梯度优化与数值时间步进方案（欧拉和Heun方法）相结合，通过在损失函数中加入边界条件惩罚项来精确强制狄利克雷约束。

Result: 在热方程上的实验表明，Heun方法由于其二阶修正而具有更高的精度，而欧拉方法在简单场景中计算效率更高。该方法成功实现了狄利克雷边界条件的精确执行。
...
### [227] [TS-DP: Reinforcement Speculative Decoding For Temporal Adaptive Diffusion Policy Acceleration](https://arxiv.org/abs/2512.15773)
*Ye Li,Jiahe Feng,Yuan Meng,Kangye Ji,Chen Tang,Xinwan Wen,Shutao Xia,Zhi Wang,Wenwu Zhu*

Main category: cs.LG

TL;DR: TS-DP是一种针对扩散策略的时序感知强化学习推测解码框架，通过Transformer草稿器和RL调度器实现动态计算调整，在保持性能的同时将推理速度提升4.17倍，达到25Hz实时控制频率。


<details>
  <summary>Details</summary>
Motivation: 扩散策略在具身控制中表现出色，但存在推理延迟高、计算成本大的问题。现有的静态加速方法（如量化）无法处理动态的具身任务，而推测解码虽然无损且自适应，但在扩散策略中尚未得到充分探索。需要解决如何在任务难度随时间变化的具身环境中，以更低成本匹配基础模型的去噪质量，以及如何根据任务难度动态调整计算的问题。

Method: 提出TS-DP框架：1）使用Transformer-based草稿器模仿基础模型，替代昂贵的去噪调用以处理动态环境；2）采用基于强化学习的调度器，通过调整推测参数来适应时变任务难度，在保持准确性的同时提高效率。

Result: 在多种具身环境中的实验表明，TS-DP实现了高达4.17倍的推理加速，草稿接受率超过94%，推理频率达到25Hz，实现了无性能下降的实时扩散控制。

Conclusion: TS-DP是首个为扩散策略提供时序自适应推测解码的框架，成功解决了动态具身任务中的计算效率问题，实现了高性能的实时控制，为扩散策略的实际应用提供了有效解决方案。
...
### [228] [Adversarial Robustness in Financial Machine Learning: Defenses, Economic Impact, and Governance Evidence](https://arxiv.org/abs/2512.15780)
*Samruddhi Baviskar*
...
<details>
  <summary>Details</summary>
Motivation: 金融决策中的表格机器学习模型（如信用评分和欺诈检测）对对抗攻击的鲁棒性尚未充分研究，需要评估其对金融风险指标的影响

Method: 使用信用评分和欺诈检测数据，应用基于梯度的攻击方法，测量对歧视性、校准性和金融风险指标的影响

Result: 小扰动导致显著性能下降，对抗训练可部分恢复模型性能

Conclusion: 金融表格模型对对抗攻击脆弱，对抗训练是有效的防御策略但只能部分恢复性能，需要进一步研究提升鲁棒性
...
### [229] [Boosting t-SNE Efficiency for Sequencing Data: Insights from Kernel Selection](https://arxiv.org/abs/2512.15900)
*Avais Jan,Prakash Chourasia,Sarwan Ali,Murray Patterson*

Main category: cs.LG

TL;DR: 本研究系统评估了t-SNE中九种不同核函数在生物序列数据降维中的应用，发现余弦相似度核在可视化质量和下游分析任务中均优于传统高斯核和隔离核，特别适合大规模生物序列分析。


<details>
  <summary>Details</summary>
Motivation: t-SNE广泛用于高维生物测序数据的可视化分析，但传统高斯核缺乏数据依赖性且计算开销大，限制了其在分类生物序列上的可扩展性和效果。虽然已有研究提出隔离核作为替代，但它可能无法最优地捕捉序列相似性，因此需要系统评估不同核函数在生物序列分析中的性能。

Method: 研究评估了九种不同核函数在t-SNE中的应用，使用三种嵌入方法：One-Hot Encoding、Spike2Vec和minimizers。通过主观可视化评估和客观指标（包括邻域保持分数）进行分析，并在六个不同的生物数据集（Spike7k、Host、ShortRead、Rabies、Genome和Breast Cancer）上进行了广泛的分类和聚类实验，使用多种机器学习算法和评估指标进行验证。

Result: 余弦相似度核总体上优于其他核函数（包括高斯核和隔离核），在运行效率和低维空间中的成对距离保持方面表现更优。核函数选择不仅显著影响可视化质量，还影响下游分析任务，余弦相似度核在不同数据类型和嵌入策略中提供了最稳健的性能。

Conclusion: 核函数选择对生物序列数据的t-SNE分析至关重要，余弦相似度核在可视化质量和下游分析任务中均表现最佳，特别适合大规模生物序列分析，为生物信息学领域提供了更有效的降维工具选择。
...
### [230] [Introduction to Symbolic Regression in the Physical Sciences](https://arxiv.org/abs/2512.15920)
*Deaglan J. Bartlett,Harry Desmond,Pedro G. Ferreira,Gabriel Kronberger*

Main category: cs.LG

TL;DR: 该特刊介绍了符号回归在物理科学中的应用，包括自动方程发现、涌现现象建模和计算仿真替代模型构建，总结了方法学挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 符号回归作为一种从数据中发现可解释数学关系的方法，为科学发现和高效经验建模提供了新途径。该特刊基于2025年4月皇家学会讨论会议，旨在展示符号回归在物理科学中的最新进展和应用。

Method: 特刊包含综述文章，介绍了符号回归的概念基础，与传统回归方法的对比，以及在物理科学中的主要应用场景。方法学考虑包括搜索空间设计、算子选择、复杂度控制、特征选择，以及与现代AI方法的整合。

Result: 特刊收集了从自动方程发现、涌现现象建模到计算昂贵仿真替代模型构建等多个应用领域的贡献。展示了符号回归在物理科学中的加速进展和日益增长的相关性。

Conclusion: 符号回归在物理科学中显示出巨大潜力，但仍面临可扩展性、噪声鲁棒性、过拟合和计算复杂度等挑战。未来方向包括整合对称性约束、渐近行为和其他理论信息。
...
### [231] [A Unification of Discrete, Gaussian, and Simplicial Diffusion](https://arxiv.org/abs/2512.15923)
...
TL;DR: 该论文提出了一个统一框架，将三种离散扩散方法（离散空间扩散、欧几里得空间高斯扩散、单纯形扩散）统一为同一底层过程的不同参数化形式，即Wright-Fisher种群遗传模型，并展示了在不同域之间切换的能力。


<details>
  <summary>Details</summary>
Motivation: 当前离散序列建模（如DNA、蛋白质、语言）存在三种主要扩散方法，它们具有不同的算法、理论结构和权衡取舍。理想情况下需要一个统一框架将这些方法视为同一底层过程的不同实例，使实践者能够在不同模型间切换。

Method: 基于Wright-Fisher种群遗传模型构建统一理论，将三种离散扩散方法视为该模型的不同参数化形式，发现单纯形扩散和高斯扩散是两种大种群极限。利用数十年数学遗传学文献解锁稳定的单纯形扩散。

Result: Wright-Fisher单纯形扩散比以前的单纯形扩散模型更稳定，在条件DNA生成任务上表现更好。能够训练单个模型在测试时执行三种域中的任意一种扩散，并且在多个域上训练的模型与在单个域上训练的模型具有竞争力。

Conclusion: 该研究建立了统一理论框架，将三种离散扩散方法统一为Wright-Fisher模型的不同参数化形式，解决了单纯形扩散的数值不稳定问题，并实现了在单一模型中支持多种扩散域的能力，为实践者提供了更大的灵活性。
...
### [232] [DSO: Direct Steering Optimization for Bias Mitigation](https://arxiv.org/abs/2512.15926)
*Lucas Monteiro Paes,Nivedha Sivakumar,Yinong Oliver Wang,Masha Fedzechkina Donaldson,Luca Zappella,Nicholas Apostoloff*

Main category: cs.LG

TL;DR: 提出DSO方法，通过强化学习优化激活转向，在推理时控制模型偏见与性能的权衡，在VLMs和LLMs上实现最先进的公平性-能力平衡。


<details>
  <summary>Details</summary>
Motivation: 生成模型决策受人口属性偏见影响（如VLMs难以识别女性医生），现有激活转向方法难以纠正需要跨群体等概率结果的偏见，且用户对偏见缓解与性能损失有不同需求，需要推理时可控制的偏见减少方法。

Method: 提出直接转向优化（DSO），使用强化学习寻找线性变换来转向激活，专门针对缓解偏见同时保持对模型性能的控制。

Result: DSO在VLMs和LLMs上实现了最先进的公平性与能力权衡，为从业者提供推理时对权衡的控制。
...
### [233] [BarcodeMamba+: Advancing State-Space Models for Fungal Biodiversity Research](https://arxiv.org/abs/2512.15931)
*Tiancheng Gao,Scott C. Lowe,Brendan Furneaux,Angel X Chang,Graham W. Taylor*

Main category: cs.LG

TL;DR: BarcodeMamba+是一个基于状态空间模型架构的真菌DNA条形码分类基础模型，采用预训练-微调范式，通过层次标签平滑、加权损失函数和多头输出层等增强技术，在数据稀疏的真菌分类任务中显著优于传统监督学习方法。


<details>
  <summary>Details</summary>
Motivation: 真菌DNA条形码分类面临标签稀疏和长尾分布等极端挑战，传统监督学习方法难以泛化到未见物种，也无法有效捕捉数据的层次结构特征。
...
### [234] [In-Context Semi-Supervised Learning](https://arxiv.org/abs/2512.15934)
*Jiashuo Fan,Paul Rosu,Aaron T. Wang,Michael Li,Lawrence Carin,Xiang Cheng*

Main category: cs.LG

TL;DR: 论文研究Transformer在上下文半监督学习中的能力，证明其能利用未标记数据提升低标签场景下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有理论研究主要关注有明确标签对的监督学习，但实践中Transformer在标签稀疏或缺失时仍表现良好，这表明未标记上下文演示中存在重要结构。研究者希望理解Transformer如何利用未标记上下文进行表示学习。

Method: 引入并研究上下文半监督学习，其中少量标记示例伴随大量未标记点，展示Transformer能利用未标记上下文学习鲁棒的、上下文相关的表示。

Result: Transformer能够利用未标记上下文学习稳健的表示，实现准确预测，在低标签场景下显著提升性能。

Conclusion: 该研究为理解Transformer如何利用未标记上下文进行表示学习提供了基础性见解，揭示了其在上下文学习框架中利用未标记数据的能力。
...
### [235] [SALVE: Sparse Autoencoder-Latent Vector Editing for Mechanistic Control of Neural Networks](https://arxiv.org/abs/2512.15938)
*Vegard Flovik*
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络性能优异但难以解释和控制，需要一种系统方法来发现特征、验证其有效性并进行精确控制

Method: 使用ℓ1正则化自编码器学习稀疏特征基，通过Grad-FAM验证特征，利用自编码器结构进行权重空间干预，推导临界抑制阈值α_crit

Result: 在ResNet-18和ViT-B/16模型上验证了框架有效性，实现了对类别定义特征和跨类别特征的连续调制和精细鲁棒性诊断
...
### [236] [AIE4ML: An End-to-End Framework for Compiling Neural Networks for the Next Generation of AMD AI Engines](https://arxiv.org/abs/2512.15946)
*Dimitrios Danopoulos,Enrico Lupi,Chang Sun,Sebastian Dittmeier,Michael Kagan,Vladimir Loncar,Maurizio Pierini*

Main category: cs.LG

TL;DR: AIE4ML：首个将AI模型自动转换为针对AMD AIE-ML架构优化固件的综合框架，实现接近架构峰值的性能，支持全片上执行，在粒子物理触发系统等超低延迟环境中提供GPU级吞吐量。


<details>
  <summary>Details</summary>
Motivation: 在AMD Versal AI Engine上进行高效AI推理具有挑战性，因为其紧密耦合的VLIW执行、显式数据路径和本地内存管理。先前工作仅关注第一代AIE内核优化，未能解决跨2D阵列的完整神经网络执行问题。

Method: 提出AIE4ML框架：1）在单内核级别实现接近架构峰值的性能；2）在图和系统级别提供结构化并行化方法，可跨2D AIE-ML架构扩展并利用专用内存瓦片实现全片上执行；3）设计高效线性层实现，支持融合偏置加法和ReLU激活；4）通过新颖的图放置和搜索算法，系统化推导确定性、紧凑且拓扑优化的布局；5）无缝接受来自hls4ml或PyTorch等高级工具的量化模型。

Result: 在层扩展基准测试中，相对于单内核基线达到98.6%的效率，使用304个AIE瓦片中的296个（97.4%），实现全片上数据移动。在实际模型拓扑评估中，在微秒级延迟约束下提供GPU级吞吐量。

Conclusion: AIE4ML是首个针对AMD AIE-ML架构的全面AI模型优化框架，在超低延迟环境中（如粒子物理实验的触发系统）具有实际应用价值，能够提供GPU级性能同时保持全片上执行。
...
### [237] [Governance by Evidence: Regulated Predictors in Decision-Tree Models](https://arxiv.org/abs/2512.15955)
*Alexios Veskoukis,Dimitris Kalles*

Main category: cs.LG

TL;DR: 该研究分析已发表决策树论文中使用的预测变量，发现许多属于隐私法规监管的数据类别，支持隐私保护方法和治理检查。


<details>
  <summary>Details</summary>
Motivation: 决策树方法广泛用于结构化表格数据，其可解释性受到重视。但研究中使用的预测变量（如年龄、诊断代码、位置）越来越多地受到隐私法规监管。需要了解实际应用中这些数据类型的法律合规情况。

Method: 收集决策树研究文献，将每个报告的预测变量分配到受监管的数据类别（如健康数据、生物识别标识符、儿童数据、财务属性、位置轨迹、政府ID等），然后将每个类别与欧盟和美国隐私法律的具体条款对应。

Result: 研究发现许多报告的预测变量属于受监管类别，其中医疗保健领域占比最大，不同行业存在明显差异。分析了流行程度、行业构成和时间模式，并总结了每个法规框架参考年份下的监管对齐时间。

Conclusion: 研究证据支持隐私保护方法和治理检查，可以为超越决策树的机器学习实践提供信息，帮助应对日益严格的隐私法规环境。
...
### [238] [Tracking Wildfire Assets with Commodity RFID and Gaussian Process Modeling](https://arxiv.org/abs/2512.15956)
...
<details>
  <summary>Details</summary>
Motivation: 解决森林环境中商品RFID系统因信号衰减、多径效应和环境变化导致的定位精度差问题，特别是在无法预先标记已知位置的情况下，为野火响应应用提供低成本、可扩展的资产追踪方案

Method: 使用高斯过程建模不同环境的RF信号响应特征，构建环境模型字典；提出加权对数似然方法将未知环境与字典中最接近的环境模型匹配，实现RFID标签定位

Result: 能够实现接近GPS精度的定位（约GPS量级），可同时追踪数十个野火响应资产，无需预先标记已知位置，成本远低于GPS系统

Conclusion: 该方法为森林环境中的资产追踪提供了一种成本效益高、可扩展的解决方案，特别适用于野火响应等应用场景，突破了传统RFID定位需要预先标记位置的限制
...
### [239] [Dynamic Rank Reinforcement Learning for Adaptive Low-Rank Multi-Head Self Attention in Large Language Models](https://arxiv.org/abs/2512.15973)
*Caner Erden*

Main category: cs.LG

TL;DR: DR-RL：通过强化学习动态优化LLM中多头自注意力的低秩分解，平衡注意力保真度和计算延迟


<details>
  <summary>Details</summary>
Motivation: 传统低秩近似方法使用静态秩假设，在不同输入上下文和硬件约束下缺乏灵活性，需要更自适应且理论严谨的优化方法

Method: 提出动态秩强化学习框架，将秩选择建模为序列策略优化问题，使用RL代理基于实时序列动态、层敏感性和硬件约束选择秩，结合在线矩阵扰动理论实现增量更新

Result: 在保持与全秩注意力统计等效的下游准确性的同时，显著减少FLOPs（尤其在长序列L>4096场景），实现了自适应效率与理论严谨性的平衡

Conclusion: DR-RL为资源受限深度学习中的启发式秩缩减技术提供了原则性、数学基础坚实的替代方案，在多头自注意力中实现了自适应效率与理论严谨性的统一
...
### [240] [Provably Extracting the Features from a General Superposition](https://arxiv.org/abs/2512.15987)
*Allen Liu*

Main category: cs.LG

TL;DR: 提出一种高效查询算法，能从噪声查询访问中恢复叠加特征方向和非退化响应函数
...
Method: 开发一种在傅里叶空间中搜索的算法，通过迭代细化搜索空间来定位隐藏特征方向。算法从噪声查询访问中识别所有非退化响应的特征方向并重构函数。

Result: 算法能在比所有相关先前结果更一般的设置下工作：允许任意叠加，只要求特征方向不近似相同，支持一般响应函数。

Conclusion: 该算法为从叠加中学习特征提供了有效的解决方案，突破了过完备机制下的算法挑战，具有更广泛的适用性。
...
### [241] [Higher-Order LaSDI: Reduced Order Modeling with Multiple Time Derivatives](https://arxiv.org/abs/2512.15997)
*Robert Stephany,William Michael Anderson,Youngsoo Choi*

Main category: cs.LG

TL;DR: 提出结合高阶有限差分格式和Rollout损失函数的新方法，解决传统降阶模型在长时间预测中精度下降的问题


<details>
  <summary>Details</summary>
Motivation: 传统偏微分方程数值解法计算成本高，现有降阶模型虽然能加速求解参数化PDE族，但在长时间预测中精度会显著下降

Method: 1) 引入灵活、高阶且计算成本低的有限差分格式；2) 提出Rollout损失函数，训练降阶模型在任意时间范围内做出准确预测

Result: 在二维Burgers方程上验证了方法的有效性

Conclusion: 提出的方法能够显著提升降阶模型在长时间预测中的精度和稳定性
...
### [242] [Surrogate Neural Architecture Codesign Package (SNAC-Pack)](https://arxiv.org/abs/2512.15998)
*Jason Weitz,Dmitri Demler,Benjamin Hawks,Nhan Tran,Javier Duarte*

Main category: cs.LG

TL;DR: SNAC-Pack是一个自动化神经网络设计与FPGA部署的框架，通过硬件感知的神经架构搜索实现多目标优化，在保持精度的同时优化资源利用和延迟。


<details>
  <summary>Details</summary>
Motivation: 现有神经架构搜索方法难以准确优化真实硬件性能，通常依赖代理指标（如位操作），无法直接针对FPGA部署进行优化。

Method: 结合神经架构协同设计的多阶段搜索能力和资源利用率与延迟估计器，实现精度、FPGA资源利用和延迟的多目标优化，无需为每个候选模型进行耗时的综合。
...
### [243] [Towards Fine-Tuning-Based Site Calibration for Knowledge-Guided Machine Learning: A Summary of Results](https://arxiv.org/abs/2512.16013)
*Ruolei Zeng,Arun Sharma,Shuai An,Mingzhou Yang,Shengya Zhang,Licheng Liu,David Mulla,Shashi Shekhar*

Main category: cs.LG

TL;DR: FTBSC-KGML是一个基于预训练-微调、空间变异感知的知识引导机器学习框架，用于准确量化农业生态系统碳循环，通过利用迁移学习和空间异质性来提高区域预测精度。


<details>
  <summary>Details</summary>
Motivation: 农业生态系统碳循环在决策相关尺度上的准确且经济有效的量化对于气候减缓和可持续农业至关重要。传统方法通常依赖位置无关的参数化和独立训练，未能充分利用迁移学习和输入数据的空间异质性，限制了在具有显著变异性的区域的适用性。

Method: 提出FTBSC-KGML框架，结合预训练-微调过程、空间变异感知和知识引导机器学习。使用多站点遥感GPP、气候和土壤协变量数据进行全局预训练，然后在各州或站点进行微调，学习位置感知表示。框架包含空间异质性感知的迁移学习方案和站点特定参数。

Result: FTBSC-KGML在验证误差上低于纯全局模型，解释能力更一致，能更好地捕捉跨州的空间变异性。在有限数据条件下提高了局部准确性，同时不牺牲可解释性。

Conclusion: 该工作扩展了先前的SDSA-KGML框架，通过预训练-微调过程和空间变异感知的迁移学习，为农业生态系统碳循环量化提供了更准确、可扩展的解决方案，特别适用于具有显著空间变异性的区域。
...
### [244] [Topic Modelling Black Box Optimization](https://arxiv.org/abs/2512.16445)
*Roman Akramov,Artem Khamatullin,Svetlana Glazyrina,Maksim Kryzhanovskiy,Roman Ischenko*

Main category: cs.LG

TL;DR: 该研究将LDA主题模型的主题数量选择问题形式化为离散黑盒优化问题，比较了四种优化器：遗传算法、进化策略、PABBO和SABBO，发现摊销优化器在样本和时间效率上显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 在LDA主题模型中，主题数量T的选择是一个关键设计决策，它强烈影响模型的统计拟合度和可解释性。传统方法通常需要大量试错，研究者希望找到更高效的优化方法来选择最佳主题数量。

Method: 将主题数量选择问题形式化为离散黑盒优化问题，每个函数评估对应训练一个LDA模型并测量其验证困惑度。在固定评估预算下，比较了四种优化器：两种手工设计的进化方法（遗传算法和进化策略）和两种学习的摊销方法（PABBO和SABBO）。

Result: 实验表明，虽然所有方法最终都能达到相似的困惑度范围，但摊销优化器在样本和时间效率上显著更优。SABBO通常只需一次评估就能识别接近最优的主题数量，PABBO在几次评估内就能找到有竞争力的配置，而GA和ES需要几乎全部预算才能接近相同区域。

Conclusion: 摊销黑盒优化方法在LDA主题数量选择问题上比传统进化方法更高效，特别是SABBO能够以极少的评估次数找到接近最优的配置，这为实际应用中的超参数优化提供了更实用的解决方案。
...
### [245] [Techno-economic optimization of a heat-pipe microreactor, part I: theory and cost optimization](https://arxiv.org/abs/2512.16032)
*Paul Seurin,Dean Price,Luis Nunez*

Main category: cs.LG

TL;DR: 提出一种结合强化学习的几何设计优化方法，用于降低热管微反应器的平准化电力成本(LCOE)，通过代理模型和约束优化实现成本降低57%以上。


<details>
  <summary>Details</summary>
Motivation: 热管微反应器(HPMRs)适合偏远地区供电，但存在规模不经济问题，财务可行性不足。需要早期设计阶段就考虑经济与物理分析的整合方法。

Method: 采用基于强化学习的统一几何设计优化框架：1) 生成随机样本训练高斯过程和多层感知机代理模型；2) 在强化学习框架中优化平准化电力成本(LCOE)，同时约束燃料寿命、停堆裕度、峰值热通量和棒积分峰值因子。

Result: 研究了轴向反射体成本高和低两种情况。发现运行维护成本和资本成本是LCOE主要贡献者，特别是轴向反射体(第一种情况)和控制鼓材料成本。优化器巧妙调整设计参数，在满足约束条件下最小化成本，两种情况下LCOE均降低超过57%。

Conclusion: 该方法有效整合技术经济考虑，显著降低热管微反应器成本。未来将进行燃料和热管性能与多目标优化的全面集成，以深入理解约束与成本性能的相互作用。
...
### [246] [Explainable AI in Big Data Fraud Detection](https://arxiv.org/abs/2512.16037)
...
<details>
  <summary>Details</summary>
Motivation: 大数据在金融、保险和网络安全应用中变得至关重要，但自动化分析的依赖性引发了透明度、监管合规性和信任问题。需要将可解释AI集成到大数据分析中以解决这些问题。

Method: 回顾大数据关键特征和主要分析工具，包括分布式存储系统、流处理平台和欺诈检测模型；调查广泛使用的XAI方法（LIME、SHAP、反事实解释、注意力机制）；分析这些方法在大规模部署时的优缺点；提出集成可扩展大数据基础设施与上下文感知解释机制的概念框架。

Result: 识别了与可扩展性、实时处理以及图和时序模型可解释性相关的研究空白；提出了解决这些挑战的概念框架；明确了可扩展XAI、隐私感知解释和标准化评估方法等开放研究方向。

Conclusion: 可解释AI对于大数据驱动的欺诈检测和风险管理至关重要，需要开发可扩展、实时且能处理复杂模型（如图和时序模型）的XAI方法，同时考虑隐私保护和标准化评估。
...
### [247] [DataFlow: An LLM-Driven Framework for Unified Data Preparation and Workflow Automation in the Era of Data-Centric AI](https://arxiv.org/abs/2512.16676)
...
TL;DR: DataFlow：一个统一的、可扩展的LLM驱动数据准备框架，通过系统级抽象和模块化操作符实现可调试、可优化的数据流，提升下游LLM性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM数据准备主要依赖临时脚本和松散的工作流程，缺乏原则性抽象、可重复性差，且对模型在环数据生成支持有限，需要更系统化的解决方案。

Method: 提出DataFlow框架，包含系统级抽象、PyTorch风格管道构建API、近200个可重用操作符和六个领域通用管道。引入DataFlow-Agent通过自然语言规范自动生成可执行管道。

Result: 在六个代表性用例中，DataFlow持续提升下游LLM性能：数学、代码和文本管道优于人工策划数据集和专用合成基线，Text-to-SQL执行准确率提升3%，代码基准平均提升7%，MATH、GSM8K和AIME提升1-3分。统一10K样本数据集使基础模型超越使用1M Infinity-Instruct数据的对应模型。

Conclusion: DataFlow为可靠、可重复、可扩展的LLM数据准备提供了实用高性能基础，为未来以数据为中心的AI发展建立了系统级基础。
...
### [248] [CauSTream: Causal Spatio-Temporal Representation Learning for Streamflow Forecasting](https://arxiv.org/abs/2512.16046)
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在径流预测中虽然性能良好，但忽略了物理过程，限制了可解释性和泛化能力。现有的因果学习方法通常依赖固定的因果图，无法适应数据变化。

Method: 提出CauStream框架，联合学习两个因果结构：(1)气象强迫与径流之间的因果图，(2)站点间动态依赖的路由图。在非参数设置下建立了这些因果结构的可识别性条件。

Result: 在三个美国主要河流流域和三个预测时间尺度上评估，CauStream始终优于现有最先进方法，且预测窗口越长性能优势越明显。学习的因果图与领域知识高度一致，提供了流域动态的可解释见解。

Conclusion: CauStream为因果时空建模提供了理论基础，具有扩展到广泛科学和环境应用的潜力，实现了预测性能与物理可解释性的平衡。
...
### [249] [Impacts of Racial Bias in Historical Training Data for News AI](https://arxiv.org/abs/2512.16901)
*Rahul Bhargava,Malene Hornstrup Jespersen,Emily Boardman Ndulue,Vivica Dsouza*

Main category: cs.LG

TL;DR: 研究分析了基于《纽约时报》语料库训练的AI分类器中"blacks"标签的偏见问题，发现该标签部分充当"种族主义检测器"，但在现代语境下表现不佳，揭示了新闻编辑室采用AI工具时面临的历史偏见风险。


<details>
  <summary>Details</summary>
Motivation: AI模型在训练过程中会编码历史数据中的态度和刻板印象，当这些模型应用于新闻编辑室和研究环境时，可能无意中传播历史偏见。本研究旨在调查基于《纽约时报》语料库训练的分类器中"blacks"标签的具体偏见表现及其对实际应用的影响。

Method: 通过定量和定性方法分析训练语料库中"blacks"标签的使用情况，应用可解释AI技术探究该标签在训练分类器中编码的概念，并测试其在现代示例（如COVID-19时期反亚裔仇恨报道和"黑人的命也是命"运动报道）上的表现。

Result: 研究发现"blacks"标签在一定程度上充当了针对某些少数群体的"种族主义检测器"，但在现代语境下表现不佳，特别是在处理反亚裔仇恨报道和"黑人的命也是命"运动报道时。这揭示了模型可能传播历史偏见的问题。

Conclusion: 新闻编辑室在采用AI工作流程工具时面临根本性矛盾：如何在利用AI技术优势的同时，降低在新闻报道中复制历史偏见的风险。该案例研究表明，需要仔细审查AI模型中的嵌入偏见，以避免在故事发现、受众定位、摘要生成等多种应用中产生意外输出。
...
### [250] [In-Context Multi-Operator Learning with DeepOSets](https://arxiv.org/abs/2512.16074)
*Shao-Ting Chiu,Aditya Nambiar,Ali Syed,Jonathan W. Siegel,Ulisses Braga-Neto*

Main category: cs.LG

TL;DR: DeepOSets架构通过结合DeepSets和DeepONets，展示了无需权重更新的上下文学习能力，能够从提示中的示例对中学习新的PDE解算子，并证明了其在科学机器学习中的通用逼近性质。


<details>
  <summary>Details</summary>
Motivation: 探索非自回归、非注意力机制的神经网络架构是否也能实现上下文学习能力，特别是在科学机器学习中解决PDE问题。传统的上下文学习主要与自回归变换器架构相关，本研究旨在展示其他架构同样具备这种能力。

Method: 提出DeepOSets架构，结合DeepSets的集合学习和DeepONets的算子学习能力。通过适当的修改，使其成为多算子上下文学习器，能够从提示中的参数-解示例对中学习新的PDE解算子，无需权重更新。

Result: DeepOSets能够准确预测训练中未见PDE的参数对应解，在Poisson和反应扩散正反边值问题实验中表现良好。更重要的是，证明了DeepOSets是连续算子类上的通用一致逼近器，这是科学机器学习文献中的首创结果。

Conclusion: DeepOSets架构不仅展示了强大的上下文学习能力，能够从示例中学习新的PDE解算子，而且具有理论上的通用逼近性质，为科学机器学习中的算子学习提供了新的架构选择。
...
### [251] [Exploration v.s. Exploitation: Rethinking RLVR through Clipping, Entropy, and Spurious Reward](https://arxiv.org/abs/2512.16912)
...
<details>
  <summary>Details</summary>
Motivation: 最近研究表明RLVR可以通过两种看似矛盾的机制激发LLMs的数学推理能力：虚假奖励（通过奖励与真实结果无关的结果来抑制利用）和熵最小化（通过推动模型产生更自信、确定性的输出来抑制探索）。这两种机制都能提升推理性能，但其背后的原理尚不清楚。

Method: 研究聚焦两个基本问题：(1)策略熵与性能的关系，(2)虚假奖励是否通过裁剪偏差和模型污染的相互作用产生收益。通过分析虚假奖励下的裁剪偏差如何降低策略熵，以及提出奖励错配模型来解释虚假奖励在污染环境之外的性能提升。

Result: 研究发现虚假奖励下的裁剪偏差会降低策略熵，导致更自信和确定性的输出，而单独的熵最小化不足以带来改进。奖励错配模型解释了为什么虚假奖励能够在污染环境之外提升性能。

Conclusion: 研究结果阐明了虚假奖励带来益处的机制，并为更有效的RLVR训练提供了原则指导，有助于理解探索-利用权衡在提升LLMs推理能力中的作用。
...
### [252] [Privacy Blur: Quantifying Privacy and Utility for Image Data Release](https://arxiv.org/abs/2512.16086)
...
<details>
  <summary>Details</summary>
Motivation: 野外收集的图像数据常包含人脸、车牌等隐私信息，负责任的数据发布需要隐藏这些信息，同时保持数据对模型训练的有效性。当前标准的高斯模糊方法存在隐私风险。

Method: 比较了四种隐私保护算法：高斯模糊、像素化、像素化加噪声(DP-Pix)和裁剪。通过逆向攻击和识别攻击评估隐私性，通过在模糊化数据上训练模型评估实用性。

Result: 最常用的工业标准方法高斯模糊隐私性最差，在低精度实现中易受逆向攻击。像素化和像素化加噪声在适当粒度下能在多个计算机视觉任务中同时提供隐私保护和实用性。

Conclusion: 高斯模糊不应作为隐私保护的标准方法，像素化和像素化加噪声是更好的替代方案。作者开发了Privacy Blur软件包提供建议参数。
...
### [253] [AIMM: An AI-Driven Multimodal Framework for Detecting Social-Media-Influenced Stock Market Manipulation](https://arxiv.org/abs/2512.16103)
*Sandeep Neela*
...
<details>
  <summary>Details</summary>
Motivation: 市场操纵现在经常源自协调的社交媒体活动，而非孤立的交易。零售投资者、监管机构和经纪商需要能够将在线叙事和协调模式与市场行为联系起来的工具。

Method: 开发AIMM框架，融合Reddit活动、机器人和协调指标、OHLCV市场特征，生成每日操纵风险评分。由于Reddit API限制，使用校准的合成社交特征匹配事件特征；市场数据使用真实历史数据。构建AIMM-GT数据集（33个标记的股票-交易日），实施前向评估和预测记录。

Result: AIMM在GME事件中提前22天发出预警（在2021年1月挤压峰值前）。尽管标记数据集较小（33个股票-交易日，3个正面事件），但结果显示初步的判别能力和对GME事件的早期预警。

Conclusion: AIMM框架展示了检测社交媒体驱动市场操纵的潜力，特别是在GME事件中提前预警。发布代码、数据集架构和仪表板设计，支持社交媒体驱动的市场监控研究。
...
### [254] [BUILD with Precision: Bottom-Up Inference of Linear DAGs](https://arxiv.org/abs/2512.16111)
...
<details>
  <summary>Details</summary>
Motivation: 从观测数据中学习有向无环图(DAG)结构是因果发现、统计信号处理和机器学习中的核心问题。在等噪声方差的线性高斯结构方程模型下，该问题是可识别的，但需要有效的算法来准确重建DAG

Method: 提出BUILD算法：利用观测数据精度矩阵的特殊结构，自底向上逐步识别叶节点及其父节点，然后剪除叶节点及其相关边，重复此过程直至完全重建DAG。为处理有限数据估计的精度矩阵误差，采用周期性重新估计策略

Result: 在具有挑战性的合成基准测试中，BUILD算法与最先进的DAG学习算法相比表现良好，同时提供了明确的复杂度控制

Conclusion: BUILD算法通过利用等噪声方差线性高斯SEM下精度矩阵的结构特性，提供了一种确定性的自底向上DAG学习方法，在准确性和计算效率之间取得了良好平衡
...
### [255] [Dual-View Inference Attack: Machine Unlearning Amplifies Privacy Exposure](https://arxiv.org/abs/2512.16126)
...
<details>
  <summary>Details</summary>
Motivation: 机器遗忘技术虽然能保护请求遗忘的用户权利，但引入了新的隐私风险。先前研究主要关注已遗忘数据的隐私，而保留数据的隐私风险尚未充分探索。本文旨在填补这一空白，研究机器遗忘在双视图设置下对保留数据的隐私威胁。
...
Result: 在不同数据集和模型架构上的实验验证了DVIA的有效性，并突显了双视图设置固有的隐私风险。攻击者通过同时访问原始模型和遗忘模型，能够更有效地推断保留数据的成员信息。

Conclusion: 机器遗忘在双视图设置下会放大隐私泄露风险，攻击者通过查询两个模型可以获得比单独查询任一模型更多的隐私信息。这揭示了机器遗忘技术中一个被忽视的隐私漏洞，需要开发更安全的遗忘机制来保护保留数据的隐私。
...
### [256] [INTELLECT-3: Technical Report](https://arxiv.org/abs/2512.16144)
...
TL;DR: INTELLECT-3是一个106B参数的MoE模型（12B激活），通过大规模强化学习训练，在数学、代码、科学和推理基准测试中达到同规模模型的最先进性能，并开源了完整的RL基础设施栈。


<details>
  <summary>Details</summary>
Motivation: 构建一个在数学、代码、科学和推理任务上具有卓越性能的大型语言模型，同时开源完整的训练基础设施，促进社区研究和开发。

Method: 使用端到端RL基础设施栈训练106B参数的MoE模型，基于GLM-4.5-Air-Base模型进行SFT和RL训练，引入prime-rl框架支持大规模异步强化学习，最高扩展到512个H200 GPU。

Result: INTELLECT-3在其规模级别上在数学、代码、科学和推理基准测试中达到最先进性能，超越了许多更大的前沿模型。

Conclusion: 通过大规模强化学习训练和开源完整基础设施栈，成功开发了高性能的MoE模型INTELLECT-3，为社区提供了可复现的训练框架和工具。
...
### [257] [A Multimodal Approach to Alzheimer's Diagnosis: Geometric Insights from Cube Copying and Cognitive Assessments](https://arxiv.org/abs/2512.16184)
*Jaeho Yang,Kijung Yoon*

Main category: cs.LG

TL;DR: 该研究提出了一种基于图神经网络的阿尔茨海默病检测方法，通过将手绘立方体转换为图结构表示，并结合人口统计学和神经心理学测试数据进行多模态分类。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期和可及性检测仍然是一个关键的临床挑战。立方体复制任务提供了简单但信息丰富的视觉空间功能评估，但传统方法缺乏对几何和拓扑特征的深入分析。

Method: 将手绘立方体草图转换为图结构表示，捕捉几何和拓扑特性。使用图神经网络处理节点特征（空间坐标、局部图元拓扑、角度几何），并与年龄、教育程度、神经心理学测试分数进行后期融合。

Result: 基于图的表示提供了强大的单模态基线，显著优于基于像素的卷积模型。多模态集成进一步提高了性能和对类别不平衡的鲁棒性。SHAP可解释性分析识别出特定的图元基序和几何扭曲是关键预测因子。

Conclusion: 基于图的立方体复制分析为阿尔茨海默病筛查提供了一种可解释、非侵入性和可扩展的方法，与临床观察到的AD患者立方体绘图紊乱现象高度一致。
...
### [258] [A Multi-scale Fused Graph Neural Network with Inter-view Contrastive Learning for Spatial Transcriptomics Data Clustering](https://arxiv.org/abs/2512.16188)
...
<details>
  <summary>Details</summary>
Motivation: 空间转录组学能在原生组织背景下进行全基因组表达分析，但识别空间域仍然具有挑战性，因为存在复杂的基因-空间相互作用。现有方法通常分别处理空间和特征视图，仅在输出层融合，这种"分别编码、后期融合"的范式限制了多尺度语义捕获和跨视图交互。

Method: stMFG是一种多尺度交互融合图网络，引入层间跨视图注意力机制，在每次卷积后动态整合空间和基因特征。模型结合跨视图对比学习和空间约束，以增强可区分性同时保持空间连续性。

Result: 在DLPFC和乳腺癌数据集上，stMFG优于最先进的方法，在某些切片上实现了高达14%的ARI（调整兰德指数）提升。

Conclusion: stMFG通过层间跨视图融合机制有效解决了空间转录组学中空间域识别的挑战，为复杂基因-空间相互作用的建模提供了新思路。
...
### [259] [Explicit and Non-asymptotic Query Complexities of Rank-Based Zeroth-order Algorithms on Smooth Functions](https://arxiv.org/abs/2512.16200)
*Haishan Ye*
...
<details>
  <summary>Details</summary>
Motivation: 基于排序的零阶优化方法（如CMA-ES、自然进化策略等）在实践中广泛应用且表现出色，但现有理论分析仅提供渐近结果，缺乏显式收敛速率分析。本文旨在填补这一理论空白。

Method: 分析一个简单的基于排序的零阶算法，采用新颖的分析方法，避免传统的漂移和信息几何技术，首次建立显式、非渐近的查询复杂度界限。

Result: 对于d维问题：1）L-光滑且μ-强凸函数，达到ε-次优解需要Õ(dL/μ·log(dL/μδ)·log(1/ε))查询；2）光滑非凸目标，达到O(dL/ε·log(1/ε))查询。这些结果以至少1-δ的概率成立。

Conclusion: 本文首次为基于排序的零阶优化算法提供了显式、非渐近的收敛速率分析，揭示了基于排序的启发式方法为何能实现高效零阶优化的新见解。
...
### [260] [Neural emulation of gravity-driven geohazard runout](https://arxiv.org/abs/2512.16221)
*Lorenzo Nava,Ye Chen,Maximillian Van Wyk de Vries*

Main category: cs.LG

TL;DR: 该研究开发了一个机器学习模型，用于快速预测地质灾害（如滑坡、雪崩）的运移范围和沉积厚度，计算速度比传统数值求解器快100-10,000倍，并在真实地形上表现出高准确性。


<details>
  <summary>Details</summary>
Motivation: 地质灾害（滑坡、雪崩等）的快速运移预测对保护生命、基础设施和生态系统至关重要。现有方法面临计算速度与物理真实性之间的根本性权衡，难以实现大规模、快速且准确的预测。

Method: 训练机器学习模型（神经仿真模型）来预测地质灾害运移。模型基于超过100,000个数值模拟数据，覆盖10,000多个真实世界数字高程模型（DEM）区域，能够预测流动范围和沉积厚度。

Result: 模型在预测流动范围和沉积厚度方面表现出高准确性，计算速度比数值求解器快100-10,000倍。模型能够再现关键物理行为（如分流和沉积模式），并能泛化到不同的流动类型、规模和地形。

Conclusion: 神经仿真技术能够实现跨多样化真实地形的快速、空间分辨率的运移预测，为灾害风险降低和基于影响的预报开辟了新途径，有望将物理真实的地质灾害建模扩展到适用于大规模预警系统的时空尺度。
...
### [261] [Coarse-to-Fine Open-Set Graph Node Classification with Large Language Models](https://arxiv.org/abs/2512.16244)
*Xueqi Ma,Xingjun Ma,Sarah Monazam Erfani,Danilo Mandic,James Bailey*

Main category: cs.LG

TL;DR: 提出CFC框架，利用大语言模型进行图数据的开放集分类，将OOD检测扩展为OOD分类，无需真实标签信息


<details>
  <summary>Details</summary>
Motivation: 现有方法将所有OOD样本视为单一类别，但实际应用（如欺诈检测和医疗诊断）需要更深入了解OOD样本的潜在标签。需要解决能否在没有真实标签信息的情况下将OOD检测扩展到OOD分类的问题。

Method: 提出粗到细开放集分类框架（CFC），包含三个关键组件：1）粗分类器使用LLM提示进行OOD检测和异常标签生成；2）基于GNN的细分类器，使用粗分类器识别的OOD样本进行训练，增强OOD检测和ID分类；3）通过LLM提示和后处理的OOD标签实现精细化的OOD分类。

Result: 实验结果显示，CFC在图和文本领域的OOD检测性能比最先进方法提高10%，在图数据集上的OOD分类准确率达到70%。

Conclusion: CFC框架成功将OOD检测扩展到OOD分类，利用语义OOD实例提高可解释性和实用性，无需合成或辅助OOD样本。
...
### [262] [Sharpness-aware Federated Graph Learning](https://arxiv.org/abs/2512.16247)
*Ruiyu Li,Peige Zhao,Guangxia Li,Pengcheng Wu,Xingyu Gao,Zhiqiang Xu*

Main category: cs.LG

TL;DR: SEAL提出了一种面向联邦图学习的锐度感知优化方法，通过同时最小化损失函数及其锐度，并引入基于相关矩阵的正则化器，解决数据异构性和维度坍塌问题，提升GNN模型的分类准确率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习面临两大核心挑战：1）基于经验风险最小化的优化器导致本地模型陷入尖锐谷，削弱了对分布外图数据的泛化能力；2）本地图数据表示学习中普遍存在的维度坍塌问题对GNN模型的分类能力产生负面影响。

Method: 提出SEAL算法：1）制定锐度感知优化目标，同时最小化损失函数及其锐度，寻找平坦区域中的模型参数；2）引入基于本地表示相关矩阵的正则化器，放松单个本地图样本生成表示之间的相关性，缓解维度坍塌问题。

Result: 在多个图分类基准测试中，SEAL始终优于最先进的联邦图学习基线方法，并为更多参与者提供性能增益，显著提升了本地GNN模型的分类准确率和泛化能力。

Conclusion: SEAL通过锐度感知优化和相关矩阵正则化，有效解决了联邦图学习中的数据异构性和维度坍塌问题，为大规模真实图数据的隐私保护协作学习提供了有效解决方案。
...
### [263] [Sharpness-aware Second-order Latent Factor Model for High-dimensional and Incomplete Data](https://arxiv.org/abs/2512.16277)
*Jialiang Wang,Xueyan Bao,Hao Wu*

Main category: cs.LG

TL;DR: 提出SSLF模型，通过Hessian-vector乘积获取二阶信息，并在曲率中注入锐度项，改善SLF模型的优化问题
...
Method: 提出锐度感知SLF模型，包含两个关键思想：1)通过Hessian-vector乘积获取二阶信息；2)通过设计的Hessian-vector乘积在曲率中注入锐度项

Result: 在多个工业数据集上的实验表明，所提出的模型始终优于最先进的基线方法

Conclusion: SSLF模型通过结合二阶信息和锐度感知优化，有效解决了SLF模型的优化难题，提高了模型性能
...
### [264] [CKA-Guided Modular Quantization: Beyond Bit-Width to Algorithmic Diversity](https://arxiv.org/abs/2512.16282)
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的大语言模型后训练量化方法通常对所有网络层采用统一的量化策略，忽视了不同层在算法适用性上的显著差异。这种一刀切的方法限制了量化性能的进一步提升。

Method: 提出CKA引导的模块化量化框架：1）独立评估多个PTQ算法在每个层上的表现；2）使用线性中心核对齐（CKA）作为度量标准自动选择每层的最优量化策略；3）将个体优化策略整合构建混合量化模型。该方法无需微调，即插即用。

Result: 实验表明，该方法在LLaMA和Qwen等主流大语言模型上，在困惑度（PPL）和下游任务性能方面，一致优于均匀量化基线和最先进的混合精度方法。

Conclusion: 通过逐层选择最优量化策略的异构量化方法能够显著提升大语言模型的量化性能，CKA引导的模块化量化为后训练量化提供了有效的解决方案。
...
### [265] [Feature-Selective Representation Misdirection for Machine Unlearning](https://arxiv.org/abs/2512.16297)
*Taozhao Chen,Linghan Huang,Kim-Kwang Raymond Choo,Huaming Chen*

Main category: cs.LG

TL;DR: SRMU提出了一种选择性表征误导的遗忘框架，通过特征感知和方向控制的激活编辑，在高度纠缠的数据分布中实现安全有效的知识遗忘。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在安全关键和受监管领域的应用增加，模型中保留的敏感或禁止知识带来了隐私泄露、监管不合规和潜在滥用等风险。现有遗忘技术假设遗忘集和保留集完全分离，但在实际运营环境中数据分布高度纠缠，传统扰动方法会损害模型效用或无法保证安全。

Method: 提出选择性表征误导遗忘框架，采用结构化的误导向量和激活重要性图，进行特征感知和方向控制的激活编辑。与无差别的模型权重扰动不同，SRMU选择性地抑制有害表征，同时保留对良性知识的效用。
...
### [266] [Pretrained Battery Transformer (PBT): A battery life prediction foundation model](https://arxiv.org/abs/2512.16334)
*Ruifeng Tan,Weixiang Hong,Jia Li,Jiaqiang Huang,Tong-Yi Zhang*

Main category: cs.LG

TL;DR: 提出了首个用于电池寿命预测的基础模型PBT，通过领域知识编码的专家混合层，在13个锂离子电池数据集上学习可迁移表示，平均性能提升19.8%，并在15个不同数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 电池循环寿命的早期预测对加速电池研究、制造和部署至关重要。现有机器学习方法受限于数据稀缺性和不同老化条件导致的异质性，而其他领域的基础模型通过迁移学习实现了广泛泛化，但电池寿命预测领域尚未有基础模型。

Method: 开发了预训练电池变压器(PBT)，这是首个电池寿命预测基础模型。采用领域知识编码的专家混合层架构，在最大的公共电池寿命数据库上进行验证，从13个锂离子电池数据集中学习可迁移表示。
...
### [267] [Multivariate Uncertainty Quantification with Tomographic Quantile Forests](https://arxiv.org/abs/2512.16383)
*Takuya Kanazawa*
...
Method: TQF学习方向投影𝐧ᵀ𝐲的条件分位数作为输入𝐱和单位方向𝐧的函数。在推理时，它聚合多个方向的分位数，并通过高效交替方案（具有凸子问题）最小化切片Wasserstein距离来重建多变量条件分布。
...
### [268] [Quantitative Verification of Fairness in Tree Ensembles](https://arxiv.org/abs/2512.16386)
*Zhenjiang Zhao,Takahisa Toda,Takashi Kitamura*

Main category: cs.LG

TL;DR: 提出了一种针对树集成模型的公平性定量验证方法，能够提供实时上下界估计，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 传统公平性验证仅返回单个反例，而定量验证能估计所有反例的比例并定位其区域，这对诊断和缓解偏见至关重要。现有定量验证方法主要针对深度神经网络，对树集成模型存在局限性

Method: 利用树集成模型的离散结构，提出高效的量化技术，提供实时上下界估计，克服了传统反例引导抽象精化框架只能提供下界且性能扩展性差的问题

Result: 在五个广泛使用的数据集上验证了方法的有效性和效率，在公平性测试方面显著优于最先进的测试技术

Conclusion: 该方法为树集成模型的公平性定量验证提供了高效解决方案，能够提供更全面的偏见分析信息
...
### [269] [Kascade: A Practical Sparse Attention Method for Long-Context LLM Inference](https://arxiv.org/abs/2512.16391)
*Dhruv Deshmukh,Saurabh Goyal,Nipun Kwatra,Ramachandran Ramjee*

Main category: cs.LG

TL;DR: Kascade是一种无需训练的稀疏注意力方法，通过选择性地在锚定层计算Top-k索引并在中间层重用，显著加速长上下文LLM推理，同时保持与密集注意力相近的准确性。


<details>
  <summary>Details</summary>
Motivation: 长上下文LLM推理中注意力机制是主要延迟来源，特别是在推理模型和RAG应用中。现有方法需要解决注意力计算效率问题，同时保持模型准确性。

Method: 基于两个关键观察：1) softmax后的注意力本质稀疏，2) 高权重键的标识在相邻层间稳定。Kascade在锚定层计算精确Top-k索引，然后在中间重用层复用这些索引。通过动态规划算法选择锚定层以最大化跨层相似性，支持头感知的Top-k选择和重用，并针对预填充和解码注意力进行高效实现。

Result: 在H100 GPU上，相比FlashAttention-3基准，Kascade实现了解码注意力最高4.1倍加速和预填充注意力2.2倍加速。在LongBench和AIME-24等长上下文基准测试中，准确性与密集注意力非常接近。

Conclusion: Kascade提供了一种无需训练、易于部署的稀疏注意力方法，显著加速长上下文LLM推理，同时保持高准确性，为实际应用提供了有效的解决方案。
...
### [270] [NDRL: Cotton Irrigation and Nitrogen Application with Nested Dual-Agent Reinforcement Learning](https://arxiv.org/abs/2512.16408)
*Ruifeng Xu,Liang He*

Main category: cs.LG

TL;DR: 提出嵌套双智能体强化学习(NDRL)方法优化棉花水氮管理，通过父智能体宏观决策和子智能体动态微调，提高产量和资源利用效率


<details>
  <summary>Details</summary>
Motivation: 现有水氮组合优化研究存在两个主要问题：1) 作物生长期间水氮组合优化复杂度高且产量优化效果差；2) 轻度胁迫信号难以量化且反馈延迟，导致水氮动态调控不精准和资源利用效率低

Method: 提出嵌套双智能体强化学习(NDRL)方法，包含父智能体和子智能体。父智能体基于预期累积产量效益识别有前景的宏观灌溉施肥行动；子智能体奖励函数纳入量化的水分胁迫因子(WSF)和氮素胁迫因子(NSF)，使用混合概率分布动态优化每日策略。使用2023-2024年田间实验数据校准验证DSSAT模型模拟真实环境

Result: 与最佳基线相比，模拟产量在2023和2024年均提高4.7%，灌溉水生产力分别提高5.6%和5.1%，氮素偏生产力分别提高6.3%和1.0%

Conclusion: NDRL方法推进了棉花灌溉和氮肥管理的发展，为解决农业资源管理的复杂性和精准性问题以及可持续农业发展提供了新思路
...
### [271] [Geometric Laplace Neural Operator](https://arxiv.org/abs/2512.16409)
*Hao Tang,Jiongyu Zhu,Zimeng Feng,Hao Li,Chao Li*

Main category: cs.LG

TL;DR: 提出基于极点-残差分解和指数基函数的广义算子学习框架GLNO，将拉普拉斯谱表示嵌入拉普拉斯-贝尔特拉米算子的特征基中，扩展算子学习到任意黎曼流形，无需周期性或均匀网格。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子方法在处理非周期激励、瞬态响应以及不规则或非欧几何上的信号时存在困难，需要一种能够处理非周期衰减动态并扩展到任意流形的算子学习框架。

Method: 提出基于极点-残差分解和指数基函数的广义算子学习框架，引入几何拉普拉斯神经算子(GLNO)，将拉普拉斯谱表示嵌入拉普拉斯-贝尔特拉米算子的特征基中，并设计网格不变网络架构GLNONet实现GLNO。

Result: 在PDEs/ODEs和真实世界数据集上的广泛实验表明，该方法相比其他最先进模型具有鲁棒的性能优势。

Conclusion: 提出的GLNO框架成功解决了现有算子学习方法在处理非周期动态和任意黎曼流形上的局限性，为算子学习提供了更通用的解决方案。
...
### [272] [Multi-Fidelity Delayed Acceptance: hierarchical MCMC sampling for Bayesian inverse problems combining multiple solvers through deep neural networks](https://arxiv.org/abs/2512.16430)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于物理模型的逆不确定性量化计算成本高，特别是涉及偏微分方程时，全阶模型使MCMC采样不可行。数据驱动代理模型受限于高保真数据生成成本，而低保真数据单独使用会降低精度。

Method: 扩展多级延迟接受框架，引入多保真度神经网络，在离线阶段训练结合不同精度求解器的网络，在线阶段仅使用粗求解器输出通过神经网络获得似然评估，避免额外高保真模拟。

Result: 方法提高了低保真求解器的近似精度，实现更长的子链长度、更好的混合特性，加速后验推断。在两个基准反问题（稳态各向同性地下水流、非稳态反应扩散系统）上获得显著计算节省。

Conclusion: 提出的多保真度延迟接受方案通过多保真度神经网络有效结合不同精度求解器，在避免在线高保真模拟的同时提高精度，为计算密集型贝叶斯反问题提供灵活高效的解决方案。
...
### [273] [Emergent Bias and Fairness in Multi-Agent Decision Systems](https://arxiv.org/abs/2512.16433)
*Maeve Madigan,Parameswaran Kamalaruban,Glenn Moynihan,Tom Kempton,David Sutton,Stuart Burrell*

Main category: cs.LG

TL;DR: 论文提出需要为多智能体预测系统开发公平性评估方法，特别是在金融表格数据领域，通过大规模模拟发现多智能体系统会出现无法追溯到单个组件的涌现性偏见，强调必须将多智能体决策系统作为整体实体进行评估。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统通过协作决策在各种预测任务中表现出性能提升，但缺乏有效的评估方法使得难以估计偏见风险，在消费者金融等高风险领域部署不安全，因为偏见决策可能导致监管违规和财务损失。

Method: 开发多智能体预测系统的公平性评估方法，在金融表格数据领域测量这些系统的公平性特征。通过大规模模拟检查不同多智能体配置（具有不同通信和协作机制）的公平性指标。

Result: 揭示了金融决策中涌现性偏见的模式，这些偏见无法追溯到单个智能体组件，表明多智能体系统可能表现出真正的集体行为。发现金融多智能体系统中的公平性风险是模型风险的重要组成部分，对信用评分和收入估计等任务有实际影响。

Conclusion: 多智能体决策系统必须作为整体实体进行评估，而不是通过对其组成组件的还原主义分析。公平性风险是金融多智能体系统模型风险的重要部分，需要专门的评估方法。
...
### [274] [A Novel Proposal in Wind Turbine Blade Failure Detection: An Integrated Approach to Energy Efficiency and Sustainability](https://arxiv.org/abs/2512.16437)
...
<details>
  <summary>Details</summary>
Motivation: 风力涡轮机叶片故障检测对于提高系统可靠性和维护效率至关重要，需要开发有效的计算方法来早期识别故障模式。

Method: 提出了一种新颖的方法论，评估了两种模型：第一种使用逻辑回归（优于神经网络、决策树和朴素贝叶斯），第二种采用聚类技术进行数据分割。

Result: 逻辑回归在识别故障相关模式方面表现优异，而聚类方法在精度和数据分割方面实现了更优性能，表明聚类能更好地捕捉数据底层特征。

Conclusion: 该方法为风力涡轮机叶片早期故障检测提供了新途径，展示了集成不同计算学习技术提升系统可靠性的潜力，未来将结合这些方法进一步提高检测精度并扩展到其他能源基础设施组件。
...
### [275] [IoMT-based Automated Leukemia Classification using CNN and Higher Order Singular Value](https://arxiv.org/abs/2512.16448)
*Shabnam Bagheri Marzijarani,Mohammad Zolfaghari,Hedieh Sajedi*

Main category: cs.LG

TL;DR: 本文提出了一种基于卷积神经网络和高阶奇异值分解的急性淋巴细胞白血病细胞分类方法，应用于医疗物联网架构，实现了98.88%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 急性淋巴细胞白血病（ALL）是一种致命性疾病，早期诊断至关重要。传统人工检测存在人为错误风险且耗时，需要基于人工智能的自动化诊断方法。医疗物联网（IoMT）的发展为实时医疗监测提供了可能。

Method: 结合卷积神经网络（CNN）提取特征，使用新型分类器高阶奇异值分解（HOSVD）对ALL细胞和正常细胞进行分类。该方法部署在IoMT架构中，实现快速安全的白血病识别。

Result: 在急性淋巴细胞白血病图像数据库（ALL-IDB2）上测试，模型在测试阶段达到了98.88%的平均准确率。

Conclusion: 提出的CNN+HOSVD分类框架能够有效识别白血病细胞，结合IoMT架构可实现患者与临床医生的实时通信，为白血病早期诊断提供了高效可靠的自动化解决方案。
...
### [276] [Batch Normalization-Free Fully Integer Quantized Neural Networks via Progressive Tandem Learning](https://arxiv.org/abs/2512.16476)
*Pengfei Sun,Wenyu Jiang,Piew Yoong Chee,Paul Devos,Dick Botteldooren*

Main category: cs.LG

TL;DR: 提出了一种无批归一化、完全整数量化神经网络训练方法，通过渐进式逐层蒸馏实现整数推理，无需BN操作


<details>
  <summary>Details</summary>
Motivation: 量化神经网络虽然通过低比特算术缩小模型并降低推理能耗，但大多数仍依赖批归一化层，这阻碍了真正的整数部署。现有方法去除BN后难以恢复其稳定性和准确性

Method: 采用渐进式逐层蒸馏方案：从预训练的带BN教师模型开始，使用逐层目标和渐进补偿训练学生模型，实现完全整数算术推理且不含BN操作

Result: 在ImageNet上使用AlexNet，无BN模型在激进量化下获得有竞争力的Top-1准确率

Conclusion: 该方法可直接集成到标准量化工作流中，为边缘和嵌入式设备等资源受限环境实现端到端整数推理
...
### [277] [Persistent Multiscale Density-based Clustering](https://arxiv.org/abs/2512.16558)
*Daniël Bot,Leland McInnes,Jan Aerts*

Main category: cs.LG

TL;DR: PLSCAN是一种新的基于密度的聚类算法，通过识别HDBSCAN*产生稳定叶簇的所有最小簇大小，无需先验知识选择超参数，在真实数据集上表现优于HDBSCAN*。


<details>
  <summary>Details</summary>
Motivation: 密度聚类算法在探索性数据分析中很重要，但实际应用时需要选择超参数（如DBSCAN的密度阈值、HDBSCAN*的最小簇大小），这在没有数据分布先验知识时很困难。

Method: 提出PLSCAN算法，基于尺度空间聚类原理，等价于新度量空间上的持久同调，能高效识别HDBSCAN*产生稳定叶簇的所有最小簇大小。

Result: 在多个真实数据集上，PLSCAN比HDBSCAN*获得更高的平均ARI，对互可达邻居数量的变化更不敏感；低维数据集上计算成本与k-Means竞争，高维时与HDBSCAN*相似。

Conclusion: PLSCAN是一种无需超参数选择的密度聚类算法，通过识别所有稳定簇配置，在性能和计算效率上都表现出色，特别适合探索性数据分析。
...
### [278] [Abacus: Self-Supervised Event Counting-Aligned Distributional Pretraining for Sequential User Modeling](https://arxiv.org/abs/2512.16581)
...
<details>
  <summary>Details</summary>
Motivation: 展示广告系统中用户购买行为建模面临挑战：正样本稀疏、用户行为随机性导致类别严重不平衡和事件时间不规则。现有方法依赖手工"计数器"特征，忽略了用户意图的细粒度时序演化，而当前序列模型又缺失有用的事件计数统计信息。
...
Conclusion: Abacus方法有效解决了展示广告中用户行为建模的挑战，通过结合频率分布预测和序列建模，显著提升了预测性能，为实时竞价系统提供了更好的用户意图理解。
...
### [279] [Stackelberg Learning from Human Feedback: Preference Optimization as a Sequential Game](https://arxiv.org/abs/2512.16626)
*Barna Pásztor,Thomas Kleine Buening,Andreas Krause*

Main category: cs.LG

TL;DR: SLHF是一种新的偏好优化框架，将对齐问题建模为领导者（Leader）和跟随者（Follower）之间的序贯博弈，相比RLHF和NLHF能捕捉更丰富的偏好结构，支持推理时优化。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法如RLHF（基于标量奖励）和NLHF（基于同时博弈均衡）存在局限性，无法充分捕捉人类偏好的复杂结构。需要一种能更好地建模序贯决策和不对称偏好的框架。

Method: 将偏好优化问题建模为Stackelberg博弈：领导者策略先行动并承诺一个动作，跟随者策略根据领导者的动作条件性地响应。将问题分解为跟随者的精炼问题和领导者对抗对手的优化问题。

Result: 在大型语言模型上的实验表明，SLHF在多种偏好数据集上实现了强大的对齐效果，参数规模从0.5B到8B都能有效扩展，并且推理时精炼能力可以跨模型家族迁移而无需微调。

Conclusion: SLHF通过序贯博弈设计提供了比RLHF和NLHF更优越的偏好优化框架，在一致性、数据敏感性和处理非传递性偏好方面具有优势，支持推理时优化并具有良好的可扩展性。
...
### [280] [Exploiting Radio Frequency Fingerprints for Device Identification: Tackling Cross-receiver Challenges in the Source-data-free Scenario](https://arxiv.org/abs/2512.16648)
*Liu Yang,Qiang Li,Luxiong Wen,Jian Yang*

Main category: cs.LG

TL;DR: 提出MS-SHOT方法解决射频指纹识别中的源数据自由跨接收器适应问题，通过动量中心引导的软伪标签和全局结构约束提升性能。


<details>
  <summary>Details</summary>
Motivation: 边缘计算中射频指纹识别(RFFI)对设备认证很重要，但深度学习模型在不同硬件特性的接收器间性能会因分布偏移而显著下降。现有方法在目标域存在标签偏移或非均匀类别分布时效果有限。

Method: 首先建立基于约束伪标签的SCRFFI适应框架，然后提出MS-SHOT方法：结合动量中心引导的软伪标签和全局结构约束，鼓励自信且多样化的预测，特别处理目标域的标签偏移问题。

Result: 在真实数据集上的大量实验表明，MS-SHOT在准确性和鲁棒性上持续优于现有方法，为RFFI中的源数据自由跨接收器适应提供了实用且可扩展的解决方案。

Conclusion: MS-SHOT通过理论分析和创新设计有效解决了SCRFFI问题，特别是在处理目标域标签偏移方面表现出色，为实际部署中的跨接收器射频指纹识别提供了可靠方案。
...
### [281] [Blog Data Showdown: Machine Learning vs Neuro-Symbolic Models for Gender Classification](https://arxiv.org/abs/2512.16687)
*Natnael Tilahun Sinshaw,Mengmei He,Tadesse K. Bahiru,Sudhir Kumar Mohapatra*

Main category: cs.LG

TL;DR: 该研究比较了传统机器学习算法（SVM、NB、LR等）与神经符号AI（NeSy）在文本分类任务上的性能，并探索了不同文本表示方法和特征提取技术的影响。


<details>
  <summary>Details</summary>
Motivation: 文本分类（如博客性别分类）是成熟的研究领域，在市场分析、客户推荐等应用中有重要价值。研究旨在比较传统机器学习方法与新兴的神经符号AI方法的效果。

Method: 采用比较分析方法，评估SVM、Naive Bayes、Logistic Regression、AdaBoost、XGBoost、SVM_R等传统算法，并与神经符号AI（NeSy）对比。同时探索TF-IDF、Universal Sentence Encoder、RoBERTa等文本表示方法，以及Chi-Square、Mutual Information、PCA等特征提取技术。

Result: 实验结果显示，神经符号AI方法在有限数据集上取得了与多层感知器（MLP）相当的结果，表明NeSy在文本分类任务中具有竞争力。

Conclusion: 神经符号AI在文本分类任务中表现出潜力，未来研究将扩展知识库、嵌入类型范围和超参数配置，以进一步探索NeSy方法的有效性。
...
### [282] [CLARiTy: A Vision Transformer for Multi-Label Classification and Weakly-Supervised Localization of Chest X-ray Pathologies](https://arxiv.org/abs/2512.16700)
*John M. Statheros,Hairong Wang,Richard Klein*

Main category: cs.LG

TL;DR: CLARiTy是一个基于视觉Transformer的模型，用于胸部X光片的多标签病理分类和弱监督定位，通过类别特定token和SegmentCAM模块实现背景抑制，在NIH ChestX-ray14数据集上取得了先进的分类和定位性能。


<details>
  <summary>Details</summary>
Motivation: 胸部X光片解读面临多标签病理分类和空间定位的挑战，通常受限于区域级（密集）标注的稀缺性。需要开发能够在仅有图像级标签的情况下同时实现准确分类和定位的方法。

Method: CLARiTy采用视觉Transformer架构，使用多个类别特定token生成判别性注意力图，并引入SegmentCAM模块利用解剖先验进行前景分割和背景抑制。模型从ConvNeXtV2教师模型进行知识蒸馏以提高效率，使用正交类别token损失和注意力池化等技术。
...
Conclusion: CLARiTy超越了CNN-ViT混合方法，通过利用ViT自注意力获取全局上下文和类别特定定位，并通过卷积背景抑制进行精炼，生成精确、噪声减少的热力图，在低资源环境下具有应用潜力。
...
### [283] [Towards Reproducibility in Predictive Process Mining: SPICE - A Deep Learning Library](https://arxiv.org/abs/2512.16715)
...
<details>
  <summary>Details</summary>
Motivation: 当前预测性流程挖掘方法缺乏可复现性、决策透明度、对新数据集的适应性以及基准测试能力，导致不同实现之间难以比较。

Method: 在PyTorch中重新实现三种流行的深度学习基线方法，设计具有严格可配置性的通用基础框架，支持对过去和未来建模方法进行可复现比较。

Result: 在11个数据集上将SPICE与原始报告指标以及公平指标进行比较，验证了框架的有效性。

Conclusion: SPICE框架解决了预测性流程挖掘领域的方法比较难题，为可复现研究和未来方法开发提供了标准化平台。
...
### [284] [Phishing Detection System: An Ensemble Approach Using Character-Level CNN and Feature Engineering](https://arxiv.org/abs/2512.16717)
*Rudra Dubey,Arpit Mani Tripathi,Archit Srivastava,Sarvpal Singh*

Main category: cs.LG

TL;DR: 提出结合字符级CNN和LightGBM的集成模型进行钓鱼URL检测，在测试集上达到99.819%准确率，并通过FastAPI实现实时检测服务。
...
Method: 采用集成方法结合字符级卷积神经网络（CNN）和LightGBM：1）从URL中提取36个词汇、结构和基于域名的特征；2）使用字符级CNN提取序列特征；3）将两种方法集成（LightGBM贡献40%，字符CNN贡献60%）。

Result: 在19,873个URL的测试集上，集成模型达到：准确率99.819%、精确率100%、召回率99.635%、ROC-AUC 99.947%，优于单个模型，且保持极低的误报率。

Conclusion: 提出的集成方法能有效检测现代钓鱼技术，通过FastAPI服务提供实时检测，在保持低误报率的同时实现高性能检测。
...
### [285] [Polyharmonic Spline Packages: Composition, Efficient Procedures for Computation and Differentiation](https://arxiv.org/abs/2512.16718)
*Yuriy N. Bakhvalov*

Main category: cs.LG

TL;DR: 提出一种基于多谐样条包的级联架构，解决先前方法在计算复杂度和高维输入空间下的限制问题


<details>
  <summary>Details</summary>
Motivation: 先前基于随机函数理论的机器学习回归解决方案存在两个主要限制：1) O(N^3)的计算复杂度，2) 当输入空间维度过高时原始理论假设失效。需要一种既能扩展计算规模又能处理未知内在低维问题的解决方案。

Method: 提出级联架构，由多谐样条包构建而成。该架构同时解决可扩展性问题，并为具有未知内在低维性的问题提供理论依据。提供了高效矩阵过程用于前向计算和通过级联的端到端微分。

Result: 该方法理论上适用于具有未知内在低维性的问题，并提供了可扩展的计算框架，克服了先前方法的计算复杂度限制。

Conclusion: 级联多谐样条架构为机器学习回归问题提供了可扩展且理论合理的解决方案，特别适用于高维输入空间中存在未知低维结构的情况。
...
### [286] [KOSS: Kalman-Optimal Selective State Spaces for Long-Term Sequence Modeling](https://arxiv.org/abs/2512.16723)
*Lei Wang,Xin Tan,Mingwei Wang,Ying Zhang*

Main category: cs.LG

TL;DR: KOSS提出了一种基于卡尔曼最优选择的状态空间模型，通过最小化潜在状态不确定性来实现上下文感知的选择机制，在序列建模任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的选择性状态空间模型（如Mamba和Mamba-2）虽然性能良好，但缺乏理论依据，且无法支持基于潜在状态动态的上下文感知选择。需要一种理论上有依据的选择机制。

Method: KOSS将选择机制建模为潜在状态不确定性最小化问题，采用基于卡尔曼增益的连续时间潜在状态更新，通过全局谱微分进行频域导数估计，并使用分段扫描实现硬件高效处理。

Result: 在选择性复制任务中，KOSS达到79%以上准确率，而基线模型低于20%。在9个长期预测基准测试中，MSE降低2.92-36.23%，在准确性和稳定性方面均优于最先进模型。在二次监视雷达跟踪案例中表现出对不规则间隔和噪声条件的鲁棒性。

Conclusion: KOSS通过卡尔曼最优选择机制提供了理论上有依据的上下文感知选择，在多个任务中表现出优越性能，并通过补充实验验证了卡尔曼增益收敛和谱微分的频率响应，为闭环设计提供了理论支持。
...
### [287] [Machine Learning Algorithms: Detection Official Hajj and Umrah Travel Agency Based on Text and Metadata Analysis](https://arxiv.org/abs/2512.16742)
*Wisnu Uriawan,Muhamad Veva Ramadhan,Firman Adi Nugraha,Hasbi Nur Wahid,M Dantha Arianvasya,Muhammad Zaki Alghifari*

Main category: cs.LG

TL;DR: 本研究针对印尼朝觐服务数字化中的虚假应用诈骗问题，采用机器学习算法自动验证应用真实性，SVM算法在混合特征提取方法下达到92.3%准确率。


<details>
  <summary>Details</summary>
Motivation: 印尼朝觐和副朝服务的快速数字化虽然便利了朝觐者，但也为通过假冒移动应用进行的数字诈骗开辟了途径。这些欺诈应用不仅造成经济损失，还通过收集敏感个人数据带来严重的隐私风险。

Method: 使用包含官方应用和非官方应用的综合数据集，比较SVM、随机森林和朴素贝叶斯三种分类器的性能。采用混合特征提取方法，结合应用描述的文本分析（TF-IDF）和敏感访问权限的元数据分析。

Result: SVM算法表现最佳，准确率达到92.3%，精确率91.5%，F1分数92.0%。特征分析显示，与合法性相关的特定关键词和高风险权限（如读取手机状态）是最重要的区分特征。

Conclusion: 该系统可作为主动、可扩展的解决方案，增强宗教旅游领域的数字信任，有望作为国家验证系统的原型，有效打击数字欺诈和保护朝觐者隐私。
...
### [288] [NRGPT: An Energy-based Alternative for GPT](https://arxiv.org/abs/2512.16762)
*Nima Dehmamy,Benjamin Hoover,Bishwajit Saha,Leo Kozachkov,Jean-Jacques Slotine,Dmitry Krotov*

Main category: cs.LG

TL;DR: 提出NRGPT模型，将GPT架构与能量基模型框架统一，通过能量景观探索进行推理，在多种任务上表现良好且具有抗过拟合特性


<details>
  <summary>Details</summary>
Motivation: GPT架构是当前最流行的语言建模设计，而能量基模型（EBM）将推理视为能量景观上的动态过程。本文旨在通过最小修改将GPT与EBM框架统一起来

Method: 提出eNeRgy-GPT（NRGPT）模型，将推理步骤概念化为在能量景观上对token的探索。在某些情况下，这种探索会变成梯度下降，但不一定产生最佳性能模型

Result: 模型在简单语言（莎士比亚数据集）、代数ListOPS任务以及更丰富的OpenWebText语言建模任务中表现良好。同时观察到模型可能更抗过拟合，只在非常长时间的训练中才会出现过拟合

Conclusion: 成功将GPT架构与能量基模型框架统一，提出的NRGPT模型在多种语言建模任务中表现良好，并展现出抗过拟合的优势
...
### [289] [Pattern recognition in complex systems via vector-field representations of spatio-temporal data](https://arxiv.org/abs/2512.16763)
...
TL;DR: 提出基于离散测度空间上向量场理论的几何框架，用于分析复杂系统的时空数据，引入适合机器学习的双参数度量族，有效解决维度约简、模态分解、相空间重构等挑战。


<details>
  <summary>Details</summary>
Motivation: 复杂系统（如大脑、气候、经济）具有高维非线性动力学特性，传统建模方法面临挑战。虽然信息技术进步带来了数据驱动方法，但时空数据的海量性和复杂性阻碍了传统降维、相空间重构等技术的应用。

Method: 基于离散测度空间上向量场理论构建几何框架，提出适用于数据分析和机器学习的双参数度量族。该框架支持时间相关图像、图像梯度以及定义在图和单纯复形上的实值或向量值函数。

Result: 在生物和物理系统的数值模拟数据（包括平坦和弯曲域）上验证了该框架。结果表明，所提出的度量结合多维缩放能有效解决关键分析挑战：实现维度约简、模态分解、相空间重构和吸引子表征。

Conclusion: 该几何框架为理解复杂动力系统提供了稳健途径，特别适用于传统建模不切实际但实验数据丰富的场景，为复杂系统分析提供了新的数学工具。
...
### [290] [MEPIC: Memory Efficient Position Independent Caching for LLM Serving](https://arxiv.org/abs/2512.16822)
...
<details>
  <summary>Details</summary>
Motivation: 现代LLM应用（如深度研究助手、编码代理和RAG系统）需要重复处理包含共享文档或代码块的长提示历史，给KV缓存带来巨大压力。现有前缀缓存和位置无关缓存（PIC）存在限制：前缀缓存需要严格前缀匹配，PIC虽然支持任意位置重用但需要选择性重计算和位置编码调整，导致相同块的KV在不同请求中发散，内存布局不一致，无法实现页面共享，HBM节省有限。

Method: MEPIC采用四种关键技术：1）将块KV对齐到分页存储；2）将重计算从令牌级转移到块级，只有第一个块是请求特定的；3）通过注意力内核中的RoPE融合移除位置编码；4）使剩余块完全可共享。这些技术消除了HBM中的大部分重复块KV。

Result: MEPIC在保持可比延迟和准确性的情况下，相比最先进的PIC系统减少高达2倍的HBM使用，对于长提示可减少高达5倍，且无需任何模型更改。

Conclusion: MEPIC通过创新的内存对齐、计算优化和位置编码融合技术，有效解决了LLM应用中KV缓存的内存效率问题，实现了跨位置、请求和批次的块级KV重用，显著提升了内存利用率。
...
### [291] [Tiny Recursive Control: Iterative Reasoning for Efficient Optimal Control](https://arxiv.org/abs/2512.16824)
*Amit Jain,Richard Linares*

Main category: cs.LG

TL;DR: TRC是一种基于递归推理的神经控制架构，通过迭代深度而非参数数量来提升容量，在保持小参数量（约1.5M）的同时实现高性能控制。
...
Method: TRC采用紧凑网络（约1.5M参数），通过两级分层潜在结构进行递归应用，通过模拟轨迹并根据跟踪误差进行修正来细化控制序列。相同权重处理每个细化步骤，增加迭代只增加计算而不增加内存。

Result: 在非线性控制问题（包括振荡器稳定和带燃料约束的动力下降）上，TRC实现了接近最优的控制成本，在GPU上仅需毫秒级推理时间，内存占用低于10MB，比语言模型基线小两个数量级。

Conclusion: 递归推理（之前仅限于离散任务）可以有效地转移到连续控制合成中，为资源受限的嵌入式系统提供了可行的神经控制解决方案。
...
### [292] [Meta-RL Induces Exploration in Language Agents](https://arxiv.org/abs/2512.16848)
*Yulun Jiang,Liangze Jiang,Damien Teney,Michael Moor,Maria Brbic*

Main category: cs.LG

TL;DR: LaMer是一个元强化学习框架，让大语言模型代理在测试时主动探索环境并从反馈中学习，显著提升多步长时程任务的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习训练的大语言模型代理在需要主动探索的任务中表现不佳，难以从试错经验中高效适应环境。

Method: LaMer包含两个关键组件：1）跨回合训练框架鼓励探索和长期奖励优化；2）通过反思进行上下文策略适应，让代理无需梯度更新就能从任务反馈信号中调整策略。

Result: 在多种环境中，LaMer相比强化学习基线显著提升性能，在Sokoban、MineSweeper和Webshop上分别获得11%、14%和19%的性能增益，并且在更具挑战性或未见任务上表现出更好的泛化能力。

Conclusion: 元强化学习为语言代理提供了诱导探索的原则性方法，通过学习探索策略实现对新颖环境更鲁棒的适应。
...
### [293] [Semi-Supervised Online Learning on the Edge by Transforming Knowledge from Teacher Models](https://arxiv.org/abs/2512.16866)
*Jiabin Xue*

Main category: cs.LG

TL;DR: 论文提出知识转换(KT)方法，结合知识蒸馏、主动学习和因果推理，为在线边缘机器学习中未见过数据生成伪标签，解决标签获取难题。


<details>
  <summary>Details</summary>
Motivation: 在线边缘机器学习需要持续更新模型以适应新数据，但未来未见过数据的标签获取是主要挑战。现有方法假设静态模型和集中式训练，无法有效处理动态边缘环境中的未知数据。

Method: 提出知识转换(KT)方法，将知识蒸馏、主动学习和因果推理相结合。KT作为主动学习中的"预言机"，从教师模型转换知识来为学生模型生成伪标签，从而训练学生模型。

Result: 仿真实验表明：当使用相对稳定的教师模型时，学生模型最终能达到预期最大性能。KT在教师任务通用（可使用预训练模型）和学生任务标签难以获取的场景下特别有益。

Conclusion: KT方法有效解决了在线边缘机器学习中未见过数据的标签获取问题，特别适用于教师任务通用且学生任务标签获取困难的场景，为边缘设备上的持续学习提供了可行方案。
...
### [294] [Sequencing to Mitigate Catastrophic Forgetting in Continual Learning](https://arxiv.org/abs/2512.16871)
...
<details>
  <summary>Details</summary>
Motivation: 持续学习系统需要在整个生命周期中增量获取、更新和利用知识，但灾难性遗忘（CF）是主要挑战。大多数现有方法分为五类，本文从不同角度考虑任务排序对缓解CF的作用。

Method: 提出基于任务排序的方法来缓解灾难性遗忘，利用受神经架构搜索（NAS）启发的零样本评分算法确定最优任务顺序。该方法可与传统持续学习策略结合使用。

Result: 智能任务排序能显著减少灾难性遗忘。当与传统持续学习策略结合时，排序方法能提供增强的性能和更强的抗遗忘鲁棒性。

Conclusion: 任务排序是缓解灾难性遗忘的有效策略，提出的方法也可应用于课程学习等其他领域。智能任务排序为持续学习提供了新的优化维度。
...
### [295] [Training Together, Diagnosing Better: Federated Learning for Collagen VI-Related Dystrophies](https://arxiv.org/abs/2512.16876)
...
TL;DR: 利用联邦学习平台对胶原VI相关肌营养不良症进行跨机构诊断，通过分布式数据训练模型，显著提升诊断性能


<details>
  <summary>Details</summary>
Motivation: 罕见病诊断面临数据稀缺且分散的挑战，跨机构数据共享存在隐私、监管和物流障碍，需要一种能保护隐私的协作学习方案

Method: 采用Sherpa.ai联邦学习平台，在两个国际组织的分布式数据集上训练模型，使用患者来源成纤维细胞培养物的胶原VI免疫荧光显微镜图像

Result: 模型能准确分类COL6-RD的三种主要致病机制组（外显子跳跃、甘氨酸替换、假外显子插入），F1分数达0.82，显著优于单机构模型（0.57-0.75）

Conclusion: 联邦学习相比孤立机构模型能显著提升诊断效用和泛化能力，不仅支持更准确诊断，还能帮助解读意义未明变异和指导测序策略优先排序
...
### [296] [Posterior Behavioral Cloning: Pretraining BC Policies for Efficient RL Finetuning](https://arxiv.org/abs/2512.16911)
*Andrew Wagenmaker,Perry Dong,Raymond Tsao,Chelsea Finn,Sergey Levine*

Main category: cs.LG

TL;DR: 提出Posterior Behavioral Cloning (PostBC)方法，通过建模演示数据的后验分布而非直接模仿动作，为RL微调提供更好的初始化，显著提升机器人控制性能


<details>
  <summary>Details</summary>
Motivation: 现有实践通常先在大规模演示数据上预训练策略，然后通过RL微调提升性能。然而，虽然微调算法研究很多，但很少有人关注如何确保预训练策略能为RL微调提供有效的初始化。标准行为克隆(BC)可能无法覆盖演示者的所有动作，这是有效RL微调的必要条件。
...
Result: 理论证明标准BC可能无法覆盖演示者动作，而PostBC能确保这种覆盖。在实际机器人控制基准测试和真实世界机器人操作任务中，PostBC相比标准BC能显著提升RL微调性能。

Conclusion: PostBC通过建模演示数据的后验分布，为RL微调提供了更好的策略初始化方法，在保持预训练性能的同时显著提升微调效果，为从演示学习到强化学习的过渡提供了更有效的解决方案。
