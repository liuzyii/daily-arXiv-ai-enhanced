...
TL;DR: ByteLoom：基于DiT的HOI视频生成框架，通过RCM-cache机制保持物体几何一致性，使用渐进式训练课程减少对手部网格标注的依赖


<details>
  <summary>Details</summary>
Motivation: 现有HOI视频生成方法存在两个关键限制：1) 缺乏有效机制将物体的多视角信息注入模型，导致跨视角一致性差；2) 严重依赖精细的手部网格标注来建模交互遮挡

Method: 提出ByteLoom框架：1) RCM-cache机制，利用相对坐标图(RCM)作为通用表示来保持物体几何一致性并精确控制6-DoF物体变换；2) 渐进式训练课程，增强模型能力并减少对手部网格的需求

Result: 实验表明该方法能忠实保持人体身份和物体的多视角几何特性，同时维持平滑运动和物体操控

Conclusion: ByteLoom解决了现有HOI视频生成方法的局限性，通过几何一致性机制和简化标注需求，实现了更高质量的HOI视频生成
...
TL;DR: HY-Motion 1.0是首个十亿参数级别的基于扩散Transformer的运动生成模型，能够根据文本描述生成3D人体运动，在指令跟随能力上显著超越现有开源基准。
...
Result: 模型在指令跟随能力上显著超越现有开源基准，实现了最广泛的运动覆盖范围，涵盖6大类超过200个运动类别。

Conclusion: HY-Motion 1.0代表了运动生成领域的重要突破，通过开源发布将促进未来研究，加速3D人体运动生成模型向商业化成熟阶段发展。
...
TL;DR: 该研究系统分析了视频扩散模型中运动与外观在去噪时间步中的编码方式，发现早期时间步主导运动，后期时间步主导外观，并基于此简化了运动定制方法。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型通过迭代去噪合成时间运动和空间外观，但运动如何在时间步中编码仍不清楚。实践中常用启发式方法认为早期时间步主要塑造运动和布局，后期时间步细化外观，但这种行为尚未被系统表征。

Method: 通过在不同时间步范围内注入新条件时引起的外观编辑与运动保持之间的权衡来代理运动编码，并进行大规模定量研究。该方法允许通过量化映射运动与外观在去噪轨迹上的竞争来分离两者。

Result: 在不同架构中一致识别出早期的运动主导阶段和晚期的外观主导阶段，在时间步空间中得到了可操作的运动-外观边界。基于此，通过将训练和推理限制在运动主导阶段，简化了当前的一键式运动定制范式，实现了强运动传递而无需辅助去偏模块或专门目标函数。

Conclusion: 该分析将广泛使用的启发式方法转化为时空解耦原则，时间步约束的配方可以作为现有运动传递和编辑方法的即插即用集成方案。
...
TL;DR: 基于3D CNN与LSTM混合架构的实时美国手语识别系统，通过摄像头视频流识别单词级手语，在WLASL等数据集上取得0.71-0.99的F1分数，支持AWS云端和OAK-D边缘设备部署。
...
Result: 在不同手语类别上获得0.71到0.99的F1分数，系统支持AWS云端部署和OAK-D摄像头的边缘实时推理。

Conclusion: 提出的混合架构能有效识别实时美国手语，系统具备实际可部署性，为无障碍应用提供了可行的技术方案，讨论了架构设计、训练方法和部署考虑。
...
<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗流程（包括医疗账单和转录）提供了机会。传统方法在处理高维医疗数据时存在准确性和效率问题，需要更先进的解决方案。

Method: 开发了一种AI增强的LLE模型，专门用于处理高维医疗数据。该方法通过自动化医疗账单和转录流程，减少人为错误并简化操作。论文提供了AI增强LLE的完整数学模型，并通过一系列实验验证其在真实医疗场景中的应用。

Result: 实验结果表明，该方法在数据处理准确性和操作效率方面有显著提升。AI增强LLE模型能够有效改善医疗数据分析和相关流程。

Conclusion: AI增强LLE在医疗数据分析中具有巨大潜力，不仅为医疗账单和转录服务提供了改进方案，也为未来更广泛的医疗应用研究奠定了基础。
...
Result: 在SpuriVerse上显著提升鲁棒性（Qwen-2.5-VL-7B +16.29%，Llama-3.2-Vision-11B +6.77%），在MMVP和SeedBench子集上保持竞争力，能跨VLM传感器迁移并识别感知失败。

Conclusion: VISTA通过感知与推理的模块化解耦，有效减少对虚假属性的依赖，提供更中立、更明确基于视觉证据的推理轨迹，优于端到端VLM基线。
...
TL;DR: SAMM2D是一个用于颅内动脉瘤检测的双编码器框架，在RSNA数据集上AUC达到0.686，比临床基线提升32%。研究发现：在强预训练骨干网络下，数据增强反而会降低性能，推翻了"更多增强总是更好"的假设。


<details>
  <summary>Details</summary>
Motivation: 动脉瘤检测面临三个主要挑战：1）动脉瘤形态细微；2）类别不平衡严重；3）标注数据稀缺。传统方法依赖数据增强来缓解数据不足问题，但作者质疑在强预训练模型下这一假设的有效性。

Method: 提出SAMM2D双编码器框架，使用ImageNet预训练骨干网络。通过六个增强方案的消融实验，发现无增强的基线模型性能最佳。使用Grad-CAM进行可视化分析，并通过决策阈值校准实现95%的敏感度。

Result: 1）AUC 0.686，比临床基线提升32%；2）无增强基线优于所有增强变体（提升1.75-2.23个百分点，p<0.01）；3）95%敏感度超过平均放射科医生水平；4）每1000名患者筛查可节省1390万美元；5）85%真阳性关注相关血管区域（与专家标注IoU为62%）。

Conclusion: 在医学影像分析中，强预训练特征已经捕获了足够的鲁棒不变性，额外的数据增强反而冗余且破坏性。未来医学影像工作流应更注重强预训练而非复杂的增强流水线。
...
TL;DR: HookMIL：一种基于可学习hook tokens的上下文感知、计算高效的多示例学习框架，用于计算病理学中的全切片图像分析，通过多模态初始化和线性复杂度注意力实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法在计算病理学中会丢失关键上下文信息，而基于Transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算的问题。需要一种既能保持上下文感知又能高效计算的方法。

Method: 提出HookMIL框架，使用紧凑可学习的hook tokens进行结构化上下文聚合。这些tokens可以通过三种方式初始化：(1)关键patch视觉特征，(2)视觉-语言病理学模型的文本嵌入，(3)空间转录组-视觉模型的空间特征。采用双向注意力机制（线性复杂度），引入Hook多样性损失促进专业化，以及hook-to-hook通信机制优化上下文交互。

Result: 在四个公共病理学数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL通过可学习的hook tokens和多模态初始化，成功解决了传统MIL方法上下文信息丢失和Transformer变体计算效率低的问题，为计算病理学中的弱监督分析提供了高效且有效的解决方案。
...
TL;DR: 提出Tiny-YOLOSAM混合管道，结合YOLOv12检测器和TinySAM分割模型，通过检测器生成框提示和稀疏点提示，显著提升分割覆盖率和速度
...
Method: 使用YOLOv12检测器生成前景对象的框提示，对未覆盖区域仅采样稀疏点提示，结合TinySAM进行分割
...
TL;DR: 提出一种基于视觉语言模型的多模态可解释性方法，通过分析视网膜象限中的病变分布来模拟眼科医生的推理过程，生成配对的Grad-CAM热力图，为糖尿病视网膜病变严重程度分类提供视觉解释。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。医生缺乏可解释的AI模型，现有模型依赖单一成像模态且解释性有限，需要能够以自然语言识别个体病变的定量检测系统。

Method: 提出新颖的多模态可解释性模型，利用视觉语言模型进行少样本学习，分析视网膜象限中的病变分布，生成配对的Grad-CAM热力图，展示OCT和眼底图像中单个神经元的权重，突出对DR严重程度分类有贡献的区域。

Result: 使用包含3,000张眼底图像和1,000张OCT图像的数据集，该方法解决了当前DR诊断中的关键限制，为改善患者预后提供了实用且全面的工具。

Conclusion: 该创新方法通过多模态可解释性模型，模拟眼科医生的推理过程，为糖尿病视网膜病变诊断提供了更实用和全面的解决方案，有望在筛查、治疗和研究环境中实现多样化应用。
...
TL;DR: TCFormer是一个仅500万参数的轻量级弱监督人群计数框架，采用Transformer特征提取器和可学习密度加权平均模块，在仅使用图像级全局计数标注的情况下实现竞争性性能。
...
Method: 1. 采用高效视觉Transformer作为特征提取器；2. 设计可学习密度加权平均模块，根据预测密度分数动态重加权局部token；3. 引入密度级别分类损失，将人群密度离散化为不同等级。

Result: 在ShanghaiTech A/B、UCF-QNRF和NWPU四个基准数据集上的实验表明，TCFormer在参数效率和计数准确性之间取得了优越的平衡，适用于边缘设备部署。

Conclusion: TCFormer通过弱监督训练范式（仅使用图像级全局计数），结合计数和密度级别损失的联合优化，实现了高估计精度，为边缘设备上的人群计数任务提供了良好解决方案。
...
TL;DR: 提出基于自定义CNN的深度学习模型，用于自动分类疟原虫感染的血细胞图像，准确率达96%，并应用可解释AI技术增强模型透明度。


<details>
  <summary>Details</summary>
Motivation: 疟疾在热带和亚热带地区仍是重大健康问题。传统显微镜血涂片分析方法灵敏度低、依赖专家判断、在偏远地区资源不足，需要更快速准确的诊断方法。

Method: 开发自定义卷积神经网络(CNN)自动分类血细胞图像为感染或未感染，并与ResNet50、VGG16、MobileNetV2、DenseNet121等成熟架构进行比较，应用SHAP、LIME和显著性图等可解释AI技术。

Result: 模型达到96%的准确率，两类别的精确率和召回率均超过0.95，证明深度学习可为资源有限地区提供快速准确的疟疾诊断。

Conclusion: 深度学习方法能提供快速、准确且可解释的疟疾诊断，特别适合资源有限地区，展示了AI在医疗诊断中的实用价值。
...
TL;DR: Signal-SGN++：一种拓扑感知的脉冲图框架，通过结合结构自适应性和时频脉冲动态，用于高效的人体动作识别，在保持高精度的同时显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 传统图卷积网络（GCNs）在骨骼动作识别中表现良好但能耗高，而脉冲神经网络（SNNs）虽然能效高但难以捕捉人体运动的时频耦合和拓扑依赖关系。需要一种能结合两者优势的方法。

Method: 提出Signal-SGN++框架，包含：1）1D脉冲图卷积和频率脉冲卷积的骨干网络；2）拓扑转移自注意力机制自适应学习骨骼拓扑；3）多尺度小波变换融合分支和拓扑感知时频融合单元。

Result: 在大规模基准测试中，Signal-SGN++实现了优越的精度-效率权衡，超越了现有的SNN方法，并在显著降低能耗的情况下达到了与最先进GCNs竞争的结果。

Conclusion: 该研究成功开发了一种拓扑感知的脉冲图框架，有效结合了GCNs的拓扑建模能力和SNNs的能效优势，为高效的人体动作识别提供了新解决方案。
...
TL;DR: VLM-PAR：基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨注意力融合提升行人属性识别性能，在PA100K、PETA和Market-1501基准上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 行人属性识别面临类别严重不平衡、属性间复杂依赖关系和领域偏移等挑战，需要更有效的解决方案来克服这些限制

Method: 基于冻结的SigLIP 2多语言编码器构建模块化视觉语言框架，通过紧凑的跨注意力融合机制对齐图像和提示嵌入，细化视觉特征

Result: 在高度不平衡的PA100K基准上取得显著准确率提升并创下新SOTA，同时在PETA和Market-1501基准上获得显著的均值准确率增益

Conclusion: 大规模视觉语言预训练与针对性跨模态细化的结合能有效克服PAR中的不平衡和泛化挑战，展示了视觉语言模型在行人属性识别中的潜力
...
TL;DR: ViSignVQA：首个大规模越南语招牌视觉问答数据集，包含10,762张图片和25,573个问答对，通过OCR增强和GPT-4多智能体框架提升低资源语言VQA性能。


<details>
  <summary>Details</summary>
Motivation: 自然场景中的招牌文本理解对VQA实际应用至关重要，但低资源语言（如越南语）在此领域研究不足，缺乏专门的数据集和基准。

Method: 1) 创建ViSignVQA数据集，捕捉越南招牌的语言、文化和视觉特征；2) 集成越南OCR模型(SwinTextSpotter)和预训练语言模型(ViT5)到SOTA VQA模型；3) 提出结合感知和推理智能体的GPT-4多智能体VQA框架。

Result: OCR增强上下文显著提升性能，将OCR文本附加到问题中使F1分数提升高达209%；多智能体框架通过多数投票达到75.98%准确率，为越南语招牌理解建立首个大规模多模态基准。

Conclusion: 领域特定资源对提升低资源语言的文本型VQA至关重要，ViSignVQA作为基准数据集支持OCR集成VQA模型的开发和评估，推动越南语场景文本理解研究。
...
Method: 利用2D视觉语言模型的激活图（相关性图）作为抽象对象表示，结合历史放置数据学习3D定位和补全，优化非结构化搜索过程。

Result: 模型能够准确识别隐藏物体的完整3D位置，首次尝试成功率显著高于随机搜索，搜索速度大幅提升。

Conclusion: 语义抽象框架扩展了机器人寻找隐藏物体的能力，为家庭机器人节省时间和精力，提升了实际应用价值。
...
TL;DR: VideoScaffold：用于流式视频理解的动态表示框架，通过自适应事件分割和分层聚合解决长视频冗余问题，提升MLLMs的视频理解能力


<details>
  <summary>Details</summary>
Motivation: 现有静态策略（稀疏采样、帧压缩、聚类）针对离线场景优化，在连续视频流中会产生碎片化或过度压缩的输出，难以实现时间连贯的表示

Method: 提出VideoScaffold框架，包含两个核心组件：弹性尺度事件分割（EES）进行预测引导的动态边界细化，以及分层事件整合（HEC）将语义相关片段渐进聚合为多层次抽象

Result: 在离线和流式视频理解基准测试中均达到最先进性能，框架模块化且即插即用，可无缝扩展现有基于图像的MLLMs到连续视频理解

Conclusion: VideoScaffold通过自适应调整事件粒度并保留细粒度视觉语义，实现了从细粒度帧理解到抽象事件推理的平滑过渡，为流式视频理解提供了有效解决方案
...
TL;DR: 提出KAN-FPN-Stem架构，用KAN卷积层替换FPN中的标准线性平滑卷积，解决ViT前端在多尺度融合中的瓶颈，在COCO数据集上比ViTPose-S提升2.0 AP。


<details>
  <summary>Details</summary>
Motivation: 现有ViT模型（如ViTPose）的前端设计过于简单，朴素的patchification机制难以有效处理多尺度变化，在初始特征提取阶段造成不可逆的信息损失，限制了密集预测任务的性能。
...
Conclusion: 这项工作不仅提供了一个即插即用的高性能模块，更重要的是揭示了：ViT前端的性能瓶颈通常不在于"特征精炼"（注意力机制），而在于"特征融合"的质量。通过引入KAN算子为解决这一瓶颈提供了有效路径。
...
TL;DR: 提出MiG-DM模型，通过元信息引导的跨域协同扩散模型，整合跨模态先验知识，提升低剂量PET图像质量


<details>
  <summary>Details</summary>
Motivation: 低剂量PET成像在减少患者辐射暴露方面至关重要，但面临噪声干扰、对比度降低和生理细节难以保留等挑战。现有方法通常忽略了投影域物理知识和患者特定元信息，而这些对于功能语义关联挖掘至关重要。
...
Conclusion: MiG-DM通过整合全面的跨模态先验知识，包括患者特定元信息和投影域物理知识，能够有效生成高质量的低剂量PET图像，为临床诊断提供更可靠的影像支持。
...
TL;DR: 提出混合知识蒸馏框架，开发轻量高性能CNN用于智能农业，在保持高精度的同时大幅降低计算成本和模型大小
...
Method: 设计结合倒残差块和密集连接的自定义学生模型，使用ResNet18作为教师网络，采用多目标策略整合硬标签监督、特征级蒸馏、响应级蒸馏和自蒸馏

Result: 在9种水稻种子品种识别任务中达到98.56%准确率，仅比教师模型低0.09%，计算成本降低2.7倍，模型大小减少10倍以上；在4种植物叶片病害数据集上表现一致

Conclusion: 提出的混合知识蒸馏框架在保持高分类精度的同时显著降低计算复杂度，展示了在硬件受限智能农业系统中的强大部署潜力
...
TL;DR: 提出自适应RGB-LWIR融合框架，根据光照条件动态选择最佳融合比例和检测模型，提升机器人视觉在低光环境下的检测性能
...
Method: 训练33个YOLO模型，在三个光照等级（无光、微光、全光）下使用22,000多张标注图像。通过11种融合比例（从100/0到0/100，10%递增）融合对齐的RGB和LWIR帧，动态选择最佳检测模型

Result: 最佳全光模型（80/20 RGB-LWIR）和微光模型（90/10融合）分别达到92.8%和92.0%的平均置信度，显著优于YOLOv5n和YOLOv11n基线。无光条件下，最佳40/60融合达到71.0%，超过基线但统计不显著

Conclusion: 自适应RGB-LWIR融合提高了所有光照条件下的检测置信度和可靠性，增强了自主机器人视觉性能，为应急服务中的机器人应用提供了更鲁棒的视觉解决方案
...
TL;DR: 通过使用在THINGS数据集上训练的Human Perception Embedding教师模型，为文本到图像扩散模型提供几何指导，实现几何与风格的分离控制，提高语义对齐约80%
...
Method: 使用轻量级、现成的判别器作为外部指导信号，提出在THINGS三元组数据集上训练的Human Perception Embedding教师模型，捕捉人类对物体形状的敏感性。将该教师模型的梯度注入到潜在扩散过程中，实现几何与风格的可控分离。

Result: 在三种架构上评估：Stable Diffusion v1.5（U-Net）、流匹配模型SiT-XL/2和扩散变换器PixArt-Σ。实验显示流模型在没有持续指导时倾向于漂移回默认轨迹，实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移。相比无指导基线，语义对齐提高了约80%。

Conclusion: 小型教师模型能够可靠地指导大型生成系统，实现更强的几何控制和拓宽文本到图像合成的创意范围，表明无需专门训练即可引入几何理解。
...
TL;DR: GeCo是一个几何基础的一致性度量方法，用于检测静态场景中的几何变形和遮挡不一致伪影，通过融合残差运动和深度先验生成可解释的一致性图，用于基准测试视频生成模型并作为无训练指导损失减少变形伪影。
...
Method: GeCo通过融合残差运动和深度先验，生成密集的一致性图来检测几何变形和遮挡不一致伪影。该方法基于几何基础，能够产生可解释的结果。

Result: GeCo能够系统地基准测试最近的视频生成模型，揭示常见失败模式，并作为无训练指导损失在视频生成过程中有效减少变形伪影。

Conclusion: GeCo提供了一个有效的几何基础度量方法，不仅能够检测视频生成中的几何不一致性，还能作为指导损失来改进生成质量，为视频生成模型的评估和改进提供了实用工具。
...
TL;DR: 该研究开发了骨骼与关节(B&J)基准测试，评估AI模型在骨科临床推理中的表现，发现模型在结构化选择题上表现良好(>90%)，但在需要多模态整合的开放任务中表现显著下降(<60%)，表明当前AI模型尚未具备复杂的临床推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于医学执照考试或精选案例的基准测试无法捕捉真实世界患者护理所需的整合性、多模态推理能力。随着基础模型在临床实践中的快速应用，需要更严格评估其真实临床推理能力。

Method: 开发了包含1,245个来自真实骨科病例问题的B&J基准测试，评估模型在7个临床推理任务中的表现，包括知识回忆、文本和图像解释、诊断生成、治疗计划和理由提供。评估了11个视觉语言模型和6个大语言模型，与专家提供的真实答案进行比较。

Result: 最先进模型在结构化多选题上准确率超过90%，但在需要多模态整合的开放任务中准确率仅约60%。VLM在医学图像解释方面存在显著局限，经常出现严重的文本驱动幻觉，忽略矛盾的视觉证据。医学专用微调模型相比通用模型没有明显优势。

Conclusion: 当前AI模型尚未具备复杂多模态推理的临床能力，其安全部署应限于支持性、基于文本的角色。未来在核心临床任务上的进步需要多模态整合和视觉理解方面的根本性突破。
...
TL;DR: 提出了首个专门评估视觉语言模型在胎儿超声成像中性能的基准Fetal-Gauge，包含42,000多张图像和93,000个问答对，结果显示当前最佳模型准确率仅55%，远低于临床要求。


<details>
  <summary>Details</summary>
Motivation: 全球超声技师短缺阻碍了胎儿健康监测，深度学习可提高超声技师效率并支持培训新从业者。视觉语言模型特别适合超声解读，但缺乏标准化基准来评估其在胎儿超声成像中的性能。

Method: 创建Fetal-Gauge基准，包含超过42,000张图像和93,000个问答对，涵盖解剖平面识别、解剖结构视觉定位、胎儿方位评估、临床视图符合性和临床诊断等任务。系统评估了多个最先进的视觉语言模型，包括通用模型和医学专用模型。

Result: 最佳模型准确率仅55%，远低于临床要求。分析揭示了当前视觉语言模型在胎儿超声解读中的关键局限性，凸显了对领域适应架构和专门训练方法的迫切需求。

Conclusion: Fetal-Gauge为推进产前护理中的多模态深度学习建立了严格基础，为解决全球医疗可及性挑战提供了途径。基准将在论文接受后公开可用。
...
TL;DR: SirenPose是一种新颖的损失函数，结合了正弦表示网络的周期性激活特性和关键点结构的几何先验，用于提升动态3D场景重建的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理快速运动和多个目标的场景时，往往难以保持运动建模的准确性和时空一致性。需要一种能够增强时空连贯性的新方法。

Method: SirenPose结合了正弦表示网络的周期性激活特性和关键点结构的几何先验，引入了物理启发的约束机制，强制关键点预测在空间和时间维度上保持一致性。同时将训练数据集扩展到60万个标注实例以支持鲁棒学习。

Result: 实验结果表明，使用SirenPose训练的模型在时空一致性指标上相比先前方法有显著提升，在处理快速运动和复杂场景变化方面表现出优越性能。

Conclusion: SirenPose通过结合周期性激活和几何先验，有效解决了动态3D场景重建中的时空一致性问题，为快速运动和复杂场景下的准确重建提供了有效解决方案。
...
TL;DR: 提出用于低成本边缘设备（Raspberry Pi 5和Coral Edge TPU）的单摄像头驾驶员行为监控系统，通过紧凑视觉模型、混淆感知标签设计和时序决策头实现17种行为识别，在低功耗硬件上达到实时性能。


<details>
  <summary>Details</summary>
Motivation: 车内驾驶员监控系统需要在计算、功耗和成本严格受限的条件下，低延迟识别分心和困倦相关行为。现有系统往往需要昂贵硬件，难以在低成本边缘设备上部署。

Method: 1) 紧凑的逐帧视觉模型；2) 混淆感知标签设计以减少视觉相似误报；3) 时序决策头，仅在预测既自信又持续时才触发警报。系统覆盖17种行为类别，包括手机使用、饮食、吸烟、视线转移等。

Result: 在Raspberry Pi 5上实现约16 FPS（INT8推理，每帧延迟<60ms），在Coral Edge TPU上约25 FPS，能够在廉价硬件上实现实时监控和稳定警报生成。使用授权数据集验证了系统性能。

Conclusion: 该系统展示了在低成本边缘设备上实现可靠车内驾驶员行为监控的可行性，为以人为中心的车辆智能（包括新兴的代理车辆概念）提供了上游感知输入。
...
<details>
  <summary>Details</summary>
Motivation: 在现实部署条件下，需要开发能够抵抗对抗攻击、提供可靠概率估计和可解释证据的深度伪造检测器，以应对日益复杂的伪造技术和实际应用场景中的挑战。

Method: 采用双流架构：一个流使用预训练骨干网络编码语义内容，另一个流提取取证残差，通过轻量级残差适配器融合进行分类。使用浅层特征金字塔网络风格头部在弱监督下生成篡改热图。采用红队训练策略，每批次应用最差的K种反取证技术，测试时注入低成本抖动增强鲁棒性。

Result: 在包括标准深度伪造数据集和监控风格分割的基准测试中，表现出近乎完美的攻击排名、低校准误差、最小弃权风险，以及在重纹理化攻击下的受控性能下降，建立了模块化、数据高效且实际可部署的基线。

Conclusion: 该方法为攻击感知检测提供了具有校准概率和可操作热图的实用解决方案，在保持鲁棒性的同时实现了良好的可解释性，适合实际部署应用。
...
TL;DR: PortionNet：通过跨模态知识蒸馏从点云学习几何特征，仅需RGB图像即可进行食物营养估计，无需深度传感器


<details>
  <summary>Details</summary>
Motivation: 从单张图像准确估计食物营养具有挑战性，因为丢失了3D信息。基于深度的方法需要深度传感器，而大多数智能手机不具备这种硬件条件。
...
Conclusion: PortionNet通过知识蒸馏实现了无需专用硬件的3D感知食物营养估计，为智能手机应用提供了实用的解决方案。
...
TL;DR: MoFu是一个多主体视频生成框架，通过尺度感知调制和傅里叶融合解决尺度不一致和排列敏感性问题，显著提升了生成质量。
...
Method: 提出MoFu统一框架：1）尺度感知调制（SMO）- 使用LLM从文本提示中提取隐式尺度线索，调制特征以确保主体尺寸一致；2）傅里叶融合策略 - 通过快速傅里叶变换处理参考特征的频率信息，生成统一表示；3）尺度-排列稳定性损失 - 联合促进尺度一致和排列不变的生成。

Result: 建立了专门的基准测试集，包含主体尺度和参考排列的受控变化。大量实验表明，MoFu在保持自然尺度、主体保真度和整体视觉质量方面显著优于现有方法。

Conclusion: MoFu有效解决了多主体视频生成中的尺度不一致和排列敏感性问题，通过统一的框架实现了更自然、保真度更高的视频生成，为相关研究提供了新的基准和评估方法。
...
Method: 提出VideoZoomer代理框架：从低帧率概览开始，调用时间缩放工具在自主选择的时刻获取高帧率片段，以多轮交互方式逐步收集细粒度证据。采用两阶段训练策略：首先在精选数据集上进行监督微调，然后通过强化学习进一步优化代理策略。

Result: 7B模型展现出多样复杂的推理模式，在广泛的长视频理解和推理基准测试中表现强劲。能够持续超越现有开源模型，甚至在具有挑战性的任务上与专有系统相媲美，同时在减少帧预算下实现更高效能。

Conclusion: VideoZoomer通过动态视觉焦点控制和多轮交互推理，有效解决了长视频理解中的上下文限制问题，为多模态大语言模型的长视频理解提供了新的解决方案。
...
TL;DR: SpotEdit是一种无需训练的扩散编辑框架，通过选择性更新仅修改区域来提升效率，避免对未改变区域进行冗余计算。


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时，即使只修改小区域，也会对所有token进行统一处理和去噪，导致计算冗余，并可能降低未改变区域的质量。这引发了一个根本问题：编辑时是否真的需要重新生成每个区域？

Method: SpotEdit包含两个关键组件：SpotSelector通过感知相似性识别稳定区域，并重用条件图像特征跳过其计算；SpotFusion通过动态融合机制自适应地混合这些特征与编辑后的token，保持上下文连贯性和编辑质量。

Result: 通过减少不必要的计算并保持未修改区域的高保真度，SpotEdit实现了高效且精确的图像编辑。

Conclusion: SpotEdit提出了一种训练免费的扩散编辑框架，通过选择性区域更新解决了当前方法中的计算冗余问题，在保持编辑质量的同时显著提升了效率。
...
TL;DR: DeMoGen提出了一种基于能量扩散模型的分解式训练范式，能够将复杂人体动作分解为语义上有意义的子组件，并支持动作原语的重组生成新动作。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注前向建模（如从文本到动作的整体映射），而本文考虑逆向视角：将整体动作分解为语义上有意义的子组件，以发现可重用的动作原语。

Method: 提出DeMoGen分解式训练范式，采用基于能量的扩散模型直接捕捉多个动作概念的组合分布。包含三个变体：1) DeMoGen-Exp：在分解的文本提示上显式训练；2) DeMoGen-OSS：进行正交自监督分解；3) DeMoGen-SC：强制原始和分解文本嵌入之间的语义一致性。

Result: 该方法能够从复杂动作序列中解耦出可重用的动作原语，分解后的动作概念可以灵活重组生成多样新颖的动作，并能泛化到训练分布之外。同时构建了文本分解数据集支持组合训练。

Conclusion: DeMoGen提供了一种有效的分解式学习方法，能够发现和重组动作原语，为文本到动作生成和动作组合提供了新的资源和范式。
...
TL;DR: 提出基于变分自编码器的多视角潜在表示学习框架，整合T1Gd和FLAIR MRI的放射组学特征，用于胶质母细胞瘤MGMT启动子甲基化分类
...
Method: 使用变分自编码器构建多视角潜在表示学习框架，通过独立概率编码器分别编码T1Gd和FLAIR MRI模态，在紧凑潜在空间中进行融合，保留模态特异性结构

Result: 该方法能够有效整合多模态信息，生成的潜在嵌入用于MGMT启动子甲基化分类，相比传统方法有改进

Conclusion: 提出的多视角潜在表示学习框架能够更好地整合多模态放射组学特征，为胶质母细胞瘤MGMT启动子甲基化状态的非侵入性推断提供了有效方法
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在HER2评分中难以提供像素级定位，且联合分析H&E和IHC图像具有挑战性，需要更准确的HER2蛋白表达评估方法

Method: 使用视觉变换器系统，通过分块处理H&E图像定位肿瘤，提出新颖映射函数对应识别IHC图像中的恶性区域，嵌入临床启发的4级HER2评分机制
...
TL;DR: 通过感知启发的伪着色技术，提升深度神经网络在心电图分析中的可解释性和少样本学习能力，特别是在药物诱导的长QT综合征检测中。


<details>
  <summary>Details</summary>
Motivation: 当前机器视觉模型（特别是深度神经网络）在生理信号解释（如心电图）中需要大量训练数据，且缺乏对其预测背后因果特征的解释，这限制了临床可靠性和与人类推理的一致性。

Method: 采用感知启发的伪着色技术，将临床显著的时间特征（如QT间期）编码为结构化颜色表示；使用原型网络和ResNet-18架构，评估单次和少样本学习；分析单次心搏和完整10秒节律的ECG图像。

Result: 伪着色技术使模型能从1-5个训练样本中学习判别性和可解释的特征；可解释性分析显示该方法引导注意力指向临床有意义的ECG特征，同时抑制无关信号成分；聚合多个心搏可进一步提升性能。

Conclusion: 人类感知编码可以弥合医疗机器智能中的数据效率、可解释性和因果推理之间的差距，为临床可靠的人工智能系统提供新途径。
...
TL;DR: 该论文提出了一种基于多模态大语言模型（MLLMs）的3D场景物体排列系统，通过引入MCP-based API、专用视觉工具套件和多智能体协作框架，解决了MLLMs在复杂3D场景操作中的视觉定位弱、3D理解不足和迭代更新易错三大挑战。
...
Method: 提出三个关键技术：1）引入MCP-based API，将交互从脆弱的原始代码操作转向更稳健的函数级更新；2）配备专用视觉工具套件，用于分析场景状态、收集空间信息和验证动作结果，形成感知反馈循环；3）设计协作式多智能体框架，包含规划、执行和验证等专门角色，以处理多步骤指令并从中间错误中恢复。

Result: 在25个复杂的物体排列任务上验证了方法的有效性，系统显著优于现有基线方法。

Conclusion: 该研究成功地将多模态大语言模型应用于复杂3D场景操作，通过创新的API设计、视觉工具增强和多智能体协作框架，有效解决了MLLMs在3D物体排列任务中的关键挑战，为3D场景理解与操作开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型或流匹配模型需要大量推理步骤才能获得高质量结果，而蒸馏方法需要预训练教师模型。Self-E旨在填补这两个范式之间的空白，实现从零开始训练、支持任意步数推理的统一模型。

Method: 结合流匹配模型的数据学习方式，引入自评估机制：模型使用当前的分数估计来评估自己生成的样本，作为动态的自教师。同时进行瞬时局部学习和自驱动的全局匹配。

Result: 在大规模文本到图像基准测试中，Self-E在少步生成方面表现出色，在50步时与最先进的流匹配模型竞争。性能随推理步数增加单调提升，支持从超快速少步生成到高质量长轨迹采样的统一模型。

Conclusion: Self-E是第一个从零开始、支持任意步数的文本到图像模型，为高效可扩展的生成提供了统一框架，无需预训练教师模型即可实现少步高质量生成。
...
<details>
  <summary>Details</summary>
Motivation: 当前人行道数据收集方法成本高、碎片化、难以扩展，阻碍了无障碍行人基础设施的建设，需要更高效、可扩展的解决方案。

Method: 开发iOS应用，利用设备语义分割、LiDAR深度估计、GPS/IMU融合来检测交通标志、信号灯等特征，并加入用户引导的标注界面进行验证。

Result: 系统评估显示在特征检测和空间映射方面表现良好，具有增强行人地图绘制的潜力，能够无缝集成到多模式交通数据集中。

Conclusion: iOSPointMapper提供了一个可扩展、以用户为中心的方法，能够填补行人基础设施的关键数据缺口，促进无障碍和包容性行人环境建设。
...
TL;DR: DeFloMat提出了一种基于条件流匹配的生成式目标检测框架，通过确定性流场替代扩散模型的多步随机去噪过程，在仅需3步推理的情况下实现了43.32% AP10:50的SOTA精度，解决了扩散检测器在临床应用中延迟过高的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的目标检测方法（如DiffusionDet）虽然精度高，但需要大量采样步骤（T>60），在时间敏感的临床应用中（如克罗恩病的磁共振肠造影检测）存在严重的延迟瓶颈，限制了实际应用。

Method: DeFloMat采用条件流匹配框架，基于条件最优传输理论构建确定性流场，近似整流流，替代扩散模型的多步随机去噪过程。使用简单的常微分方程求解器实现快速推理，显著减少推理步骤。

Result: 在MRE临床数据集上，DeFloMat仅用3步推理就达到了43.32% AP10:50的SOTA精度，比DiffusionDet在4步时的最佳性能（31.03% AP10:50）提升了1.4倍。同时显著改善了定位特性，在少步推理时表现出更好的召回率和稳定性。
...
TL;DR: Bright-4B是一个40亿参数的基础模型，可直接从3D明场体积图像中分割亚细胞结构，无需荧光标记或复杂后处理
...
Result: 模型能准确分割细胞核、线粒体等细胞器，在多个共聚焦数据集上保持跨深度和细胞类型的精细结构细节，优于当代CNN和Transformer基线
...
TL;DR: FluenceFormer：一种基于Transformer的放疗计划自动化框架，通过两阶段设计和物理感知回归损失，显著提升剂量分布预测的准确性和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 放疗计划中的剂量分布预测是一个复杂的逆问题，传统卷积方法难以捕捉长距离依赖关系，导致结构不一致或物理不可行的计划。需要一种能够直接、几何感知的剂量回归方法。

Method: 提出FluenceFormer框架：1）第一阶段从解剖结构输入预测全局剂量先验；2）第二阶段将先验与显式束流几何条件结合，回归物理校准的剂量分布图。核心是Fluence-Aware Regression (FAR)损失函数，整合了体素级保真度、梯度平滑性、结构一致性和束流能量守恒。

Result: 在多种Transformer骨干网络（Swin UNETR、UNETR、nnFormer、MedFormer）上验证，其中Swin UNETR表现最佳。相比现有CNN和单阶段方法，能量误差降至4.5%，结构保真度有统计学显著提升（p < 0.05）。

Conclusion: FluenceFormer框架通过两阶段Transformer设计和物理感知损失函数，有效解决了放疗剂量分布预测中的长距离依赖问题，实现了更准确、物理更合理的自动化放疗计划。
...
<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感色调共同塑造人类感知。现有文本到图像模型确保内容一致性但缺乏情感意识，而情感驱动模型以内容失真为代价生成情感结果，需要解决这一差距。

Method: 提出EmoCtrl模型，使用包含内容、情感和情感提示的数据集，将抽象情感与视觉线索连接。包含文本和视觉情感增强模块，通过描述性语义和感知线索丰富情感表达。

Result: EmoCtrl在忠实内容和表达情感控制方面优于现有方法，定量和定性实验均显示优越性能。用户研究证实EmoCtrl与人类偏好高度一致，且能很好泛化到创意应用。

Conclusion: EmoCtrl成功实现了可控情感图像内容生成，学习的情绪标记具有互补效应和良好泛化能力，为情感感知图像生成提供了有效解决方案。
...
TL;DR: SuperiorGAT：基于图注意力机制的框架，用于重建稀疏LiDAR点云中缺失的高程信息，通过建模光束感知图和使用门控残差融合，无需增加网络深度即可提升LiDAR分辨率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中LiDAR感知受限于固定的垂直光束分辨率，且环境遮挡导致的光束丢失进一步降低了点云质量，需要在不增加硬件成本的情况下提升分辨率。

Method: 将LiDAR扫描建模为光束感知图，采用图注意力机制，结合门控残差融合和前馈细化，在不增加网络深度的情况下重建缺失的高程信息。

Result: 在KITTI数据集的不同环境（行人、道路、校园、城市）中，通过模拟移除每第四条垂直扫描光束的结构化光束丢失，SuperiorGAT相比PointNet和更深的GAT基线模型，实现了更低的重建误差和更好的几何一致性。

Conclusion: 架构优化提供了一种计算高效的方法来提升LiDAR分辨率，无需额外传感器硬件，通过光束感知图建模和门控融合机制能有效重建缺失信息并保持结构完整性。
...
TL;DR: 提出基于直线的事件相机标定框架，利用人造环境中常见物体的几何直线（如门窗、箱子等）进行标定，无需专用标定板，适用于单目和双目事件相机。
...
Method: 直接从事件流中检测直线，利用事件-直线标定模型生成相机参数的初始估计（适用于平面和非平面直线），然后采用非线性优化细化相机参数。

Result: 仿真和真实世界实验证明了该方法的可行性和准确性，已在单目和双目事件相机上进行了验证。

Conclusion: 提出的基于直线的标定框架能够利用环境中常见物体的几何特征进行事件相机标定，无需专用标定板，适用于快速变化场景，代码已开源。
...
TL;DR: QDFNet是一种用于光学-SAR目标检测的质量感知动态融合网络，通过可学习的参考令牌动态评估特征可靠性，在模态缺失或退化情况下实现鲁棒融合。


<details>
  <summary>Details</summary>
Motivation: 光学与SAR融合目标检测面临实际部署困难：由于成像机制不同、时间异步和配准问题，难以获得对齐良好的图像对，经常导致模态数据缺失或退化。现有方法对随机缺失模态的鲁棒性有限，缺乏确保融合检测性能一致提升的有效机制。

Method: 提出质量感知动态融合网络(QDFNet)：1) 动态模态质量评估(DMQA)模块，使用可学习的参考令牌迭代优化特征可靠性评估，精确识别退化区域并提供质量指导；2) 正交约束归一化融合(OCNF)模块，通过正交约束保持模态独立性，同时基于可靠性分数动态调整融合权重，有效抑制不可靠特征传播。

Result: 在SpaceNet6-OTD和OGSOD-2.0数据集上的大量实验表明，QDFNet相比最先进方法具有优越性和有效性，特别是在部分模态损坏或数据缺失场景下表现突出。

Conclusion: QDFNet通过动态质量评估和自适应融合机制，解决了光学-SAR融合目标检测中的模态缺失和退化问题，实现了更鲁棒的跨模态目标检测性能。
...
TL;DR: SonoVision是一款帮助视障人士通过声音提示定位日常物品的智能手机应用，使用左右耳声音提示指示物体位置，支持完全离线工作。


<details>
  <summary>Details</summary>
Motivation: 视障人士定位物体是一个重大挑战，影响他们的独立性并可能导致危险情况。为了增强视障人士的自给自足能力，需要开发辅助工具减少对他人的依赖。

Method: 使用Flutter开发平台构建应用，后端采用Efficientdet-D2模型进行物体检测。通过左右耳耳机发出正弦声音提示物体位置：右侧物体在右耳提示，左侧物体在左耳提示，正前方物体则双耳同时提示。

Result: 开发了SonoVision应用，能够帮助视障人士通过智能手机和耳机定位物体。应用完全离线工作，代码已在GitHub开源。

Conclusion: SonoVision应用能够以安全、用户友好的方式显著帮助视障人士，通过声音提示减少对周围人的依赖，增强他们的独立性。
...
TL;DR: 评估SAM 3D基础模型在单目遥感图像建筑重建中的表现，发现其在屋顶几何一致性和边界清晰度上优于TRELLIS，并展示了扩展到城市场景重建的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D建筑重建方法通常需要特定任务架构和密集监督，限制了可扩展性。本文旨在评估通用图像到3D基础模型SAM 3D在遥感建筑重建中的表现，探索基础模型在该领域的应用潜力。

Method: 使用NYC Urban Dataset样本，将SAM 3D与TRELLIS进行对比评估，采用Fréchet Inception Distance (FID)和CLIP-based Maximum Mean Discrepancy (CMMD)作为评估指标。进一步通过分割-重建-组合流程将SAM 3D扩展到城市场景重建。

Result: 实验结果显示，SAM 3D相比TRELLIS能产生更一致的屋顶几何结构和更清晰的边界。扩展实验表明SAM 3D在城市场景建模中具有潜力。

Conclusion: SAM 3D在单目遥感建筑重建中表现出色，为城市3D重建中基础模型的部署提供了实用指导。未来研究应关注场景级结构先验的集成，以进一步提升重建质量。
...
TL;DR: 本文训练并比较了YOLOv8、YOLOv11和RF-DETR三种计算机视觉模型在变电站图像组件检测上的性能，旨在实现变电站关键组件的自动化识别与脆弱性评估。


<details>
  <summary>Details</summary>
Motivation: 变电站作为电网关键基础设施，其资产（如变压器）易受飓风、洪水、地震等多种灾害影响，故障可能导致严重的经济和公共安全后果。传统人工测绘方法耗时耗力，需要自动化解决方案来实现高效的关键组件识别与脆弱性评估。

Method: 使用手动标注的美国变电站图像数据集，训练并比较三种目标检测模型：YOLOv8、YOLOv11和RF-DETR。评估指标包括检测准确率、精确度和效率，分析各模型的优缺点，并应用于美国变电站组件的实际测绘。

Result: 研究比较了三种模型在变电站组件检测上的性能，识别出最适合大规模变电站组件测绘的可靠模型。成功展示了机器学习在变电站测绘中的应用案例，实现了美国变电站各种组件的有效映射。

Conclusion: 计算机视觉模型能够有效实现变电站组件的自动化识别与测绘，为电网脆弱性评估提供了高效解决方案。研究确定了最适合大规模变电站测绘的模型，展示了机器学习在关键基础设施监测中的应用潜力。
...
TL;DR: PGR²M提出了一种混合表示方法，通过残差向量量化增强可解释的姿态编码，改善了文本驱动的3D运动生成和编辑的质量与可控性。


<details>
  <summary>Details</summary>
Motivation: 现有基于姿态编码的框架（如CoMo）虽然支持可解释的运动控制，但帧级表示难以捕捉细微的时序动态和高频细节，导致重建保真度和局部可控性下降。

Method: 提出PGR²M混合表示：1) 姿态引导的RVQ分词器将运动分解为编码全局结构的姿态潜在变量和建模细粒度时序变化的残差潜在变量；2) 残差丢弃防止过度依赖残差；3) 基础Transformer自回归预测姿态编码，精炼Transformer在文本、姿态编码和量化阶段条件下预测残差编码。

Result: 在HumanML3D和KIT-ML数据集上，PGR²M在Fréchet起始距离和重建指标上优于CoMo及最近的扩散和分词基线，用户研究证实其支持直观、结构保持的运动编辑。

Conclusion: PGR²M通过混合表示有效结合了可解释姿态编码的编辑性和残差编码的细节建模能力，显著提升了文本驱动3D运动生成和编辑的质量与可控性。
...
TL;DR: 提出基于多事件相机的高时空分辨率冲击波运动参数测量框架，通过极坐标编码、自适应ROI提取和迭代斜率分析实现冲击波前提取，最终实现运动场重建和爆炸当量反演，测量误差最小0.06%，最大5.20%。
...
Method: 1) 建立极坐标系编码事件揭示冲击波传播模式，通过事件偏移计算实现自适应ROI提取；2) 利用速度变化的连续性，通过迭代斜率分析提取冲击波前事件；3) 基于事件光学成像模型推导事件几何模型和冲击波运动参数，结合3D重建模型。

Result: 与压力传感器和经验公式对比，速度测量最大误差5.20%，最小误差0.06%，实现了高时空分辨率的高精度冲击波运动场测量。

Conclusion: 该方法利用事件相机的高速高动态范围特性，实现了多角度冲击波测量、运动场重建和爆炸当量反演，代表了冲击波测量技术的显著进步。
...
TL;DR: 提出用于红外小目标检测的两阶段半监督范式，通过MoE适配器和知识蒸馏解决SAM模型在红外领域的域差距、物理先验缺失和架构复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测标注成本高昂，需要半监督方法。现有方法如SAM面临域差距、无法编码物理先验、架构复杂等挑战。
...
Result: 实验表明，使用最少标注，下游模型性能可达到甚至超越全监督对应模型。这是首个系统性地使用SAM作为教师模型解决IR-SOT数据稀缺问题的半监督范式。

Conclusion: 提出的两阶段半监督范式有效解决了红外小目标检测中的数据稀缺问题，通过知识蒸馏和迁移实现了高性能的轻量模型部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频表示学习方法缺乏对动态3D场景的建模能力，需要一种能够自然捕捉视频中物体运动轨迹的自监督方法

Method: 提出Video Gaussian Masked Autoencoders，将视频序列编码为随时间移动的高斯泼溅集合，利用3D场景投影的归纳偏置进行自监督预训练

Result: 学习到的高斯轨迹映射到图像平面可实现零样本跟踪，性能媲美SOTA；微调后在Kinetics数据集提升34.6%，Kubric数据集提升13.1%

Conclusion: Video-GMAE通过高斯表示有效建模视频的3D动态特性，在自监督视频表示学习和跟踪任务上表现优异，为视频理解提供了新思路
...
TL;DR: SCAFusion：针对月球机器人任务的多模态3D目标检测模型，通过认知适配器、对比对齐模块、相机辅助训练分支和分段感知坐标注意力机制，显著提升小型不规则目标检测性能


<details>
  <summary>Details</summary>
Motivation: 现有用于地面自动驾驶的多模态3D感知方法在月球等外星环境中表现不佳，主要存在特征对齐差、多模态协同有限、小目标检测弱等问题，而月球表面探索中可靠检测小型不规则物体（如陨石碎片、岩石）对自主导航至关重要

Method: 基于BEVFusion框架，集成四个关键组件：1）认知适配器用于高效相机骨干网络调优；2）对比对齐模块增强相机-LiDAR特征一致性；3）相机辅助训练分支强化视觉表示；4）分段感知坐标注意力机制专门提升小型不规则目标检测性能

Result: 在nuScenes验证集上达到69.7% mAP和72.1% NDS，比基线分别提升5.0%和2.7%；在Isaac Sim构建的模拟月球环境中达到90.93% mAP，比基线提升11.5%，在检测小型陨石类障碍物方面表现突出

Conclusion: SCAFusion通过创新的多模态融合架构，以可忽略的参数和计算量增加，显著提升了在月球等外星环境中检测小型不规则目标的能力，为月球机器人任务提供了有效的3D感知解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有统一生成和编辑模型主要依赖文本提示，但语言难以准确捕捉用户意图的编辑位置和细粒度视觉细节，因此需要结合涂鸦等更直观的交互方式。

Method: 1) 数据合成管道：包括涂鸦编辑（4个子任务）和涂鸦生成（3个子任务），基于DreamOmni2数据集提取可编辑区域并叠加手绘元素；2) 框架设计：提出联合输入方案，将原始图像和涂鸦源图像同时输入模型，使用不同颜色区分区域，并应用相同的索引和位置编码实现精确定位。

Result: 实验结果表明DreamOmni3取得了出色的性能，并为这些任务建立了全面的基准测试，促进进一步研究。模型和代码将公开。

Conclusion: DreamOmni3通过涂鸦结合文本和图像的交互方式，解决了传统文本提示在定位和细节表达上的局限性，为GUI上的灵活创作提供了有效解决方案。
...
TL;DR: CoAgent：一个协作闭环框架，通过计划-合成-验证流程解决开放域视频生成中的叙事连贯性和视觉一致性问题
...
Method: 采用计划-合成-验证流程：故事板规划器分解输入为镜头级计划，全局上下文管理器维护实体级记忆，合成模块在视觉一致性控制器指导下生成镜头，验证代理评估中间结果并触发选择性重新生成，最后通过节奏感知编辑器优化时间节奏和过渡
...
TL;DR: SR-MCR是一个轻量级、无需标注的多模态LLM对齐框架，通过五个自参考信号监督推理过程，提升推理可靠性和视觉基础，在7B规模上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有多模态LLM存在推理不可靠问题，产生流畅但不可信的推理过程，表现出弱的步骤间一致性和不足的视觉基础。主要原因是现有对齐方法只监督最终答案，而忽略了中间推理过程的可靠性。

Method: 提出SR-MCR框架：1）从模型输出中提取五个自参考信号（语义对齐、词汇保真度、非冗余性、视觉基础、步骤一致性）；2）整合为归一化的可靠性加权奖励，提供细粒度的过程级指导；3）使用无批评器的GRPO目标，配合置信度感知冷却机制稳定训练，抑制平凡或过度自信的生成。

Result: 基于Qwen2.5-VL，SR-MCR在广泛的视觉基准测试中同时提升了答案准确性和推理一致性。在7B规模的开源模型中达到SOTA，平均准确率81.4%。消融研究确认了每个奖励项和冷却模块的独立贡献。

Conclusion: SR-MCR通过利用模型自身输出的内在过程信号，提供了一种轻量级、无需标注的方法来对齐多模态LLM的推理过程，显著提升了推理可靠性和视觉基础，为过程级监督提供了有效解决方案。
...
TL;DR: 提出ReFRM3D网络和基于多特征肿瘤标志物的分类器，显著提升胶质瘤分割和分类性能


<details>
  <summary>Details</summary>
Motivation: 胶质瘤是侵袭性强的癌症，现有诊断方法存在影像数据变异大、计算资源优化不足、分割分类效率低等问题，需要改进

Method: 提出ReFRM3D网络（基于3D U-Net，含多尺度特征融合、混合上采样、扩展残差跳跃机制）和基于放射组学特征的多特征肿瘤标志物分类器

Result: 在BraTS2019-2021数据集上取得优异分割性能：BraTS2019的WT/ET/TC Dice系数分别为94.04%/92.68%/93.64%；BraTS2020为94.09%/92.91%/93.84%；BraTS2021为93.70%/90.36%/92.13%

Conclusion: 提出的ReFRM3D网络和分类器能有效解决胶质瘤诊断中的挑战，显著提升分割和分类性能
...
TL;DR: KV-Tracker：通过缓存全局自注意力块的键值对，将多视图3D几何网络适配到实时应用，实现15倍加速的6-DoF姿态跟踪和在线重建


<details>
  <summary>Details</summary>
Motivation: 多视图3D几何网络虽然功能强大，但推理速度过慢，无法满足实时应用需求。需要一种方法在保持性能的同时显著提升推理速度。

Method: 1. 快速选择和管理关键帧集合，通过π³网络进行场景/对象映射；2. 缓存全局自注意力块的键值对作为唯一场景表示；3. 使用缓存的KV对进行在线跟踪，无需重新计算

Result: 1. 推理速度提升高达15倍；2. 支持高达~27 FPS的实时性能；3. 在TUM RGB-D、7-Scenes、Arctic和OnePose数据集上表现优异；4. 避免了漂移和灾难性遗忘问题

Conclusion: KV-Tracker通过创新的KV缓存策略，成功将强大的多视图3D几何网络适配到实时应用中，实现了高效的6-DoF姿态跟踪和在线重建，且方法具有模型无关性，无需重新训练即可应用于现有网络。
...
TL;DR: PTalker是一个用于个性化3D说话头动画的新框架，通过风格解耦和三重对齐机制，在保持说话风格的同时提高唇部同步精度。
...
Method: PTalker通过风格解耦从音频和面部运动序列中分离风格与内容，并采用三重对齐机制：1) 使用图注意力网络的空间对齐；2) 使用交叉注意力的时间对齐；3) 使用top-k双向对比损失和KL散度的特征对齐。

Result: 在公共数据集上的大量定性和定量实验表明，PTalker能有效生成逼真、风格化的3D说话头，准确匹配身份特定的说话风格，优于现有最先进方法。

Conclusion: PTalker框架通过风格解耦和三重对齐机制，成功实现了既保持个性化说话风格又提高唇部同步精度的3D说话头生成。
...
TL;DR: 提出基于稀疏差分Transformer的Top-K Jaccard相似度系数方法，通过增强邻居节点纯度和抗噪声能力，显著提升人脸聚类性能。
...
Method: 提出预测驱动的Top-K Jaccard相似度系数增强邻居节点纯度，并开发基于Transformer的预测模型来优化Top-K邻居数量选择。进一步提出稀疏差分Transformer替代标准Transformer，消除噪声并增强抗噪声能力。
...
TL;DR: Dream-VL是基于扩散大语言模型构建的视觉语言模型，在视觉规划任务中表现优异；Dream-VLA是其扩展的视觉语言动作模型，在机器人控制任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自回归视觉语言模型在复杂视觉规划和动态机器人控制中存在局限性，需要探索基于扩散模型的替代方案来克服这些限制。

Method: 基于扩散大语言模型构建Dream-VL视觉语言模型，然后通过连续预训练开放机器人数据集扩展为Dream-VLA视觉语言动作模型。

Result: Dream-VL在多个基准测试中与顶级AR模型相当，在视觉规划任务中表现更优；Dream-VLA在LIBERO上达到97.2%平均成功率，在SimperEnv-Bridge和Fractal上分别达到71.4%和60.5%，超越π₀和GR00T-N1等领先模型。

Conclusion: 扩散模型作为视觉语言和视觉语言动作任务的基础架构具有优势，其双向特性更适合动作分块和并行生成，能实现更快的下游微调收敛。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于SAM2的跟踪方法虽然性能不错，但记忆机制设计缺乏系统性研究，且不清楚这些机制如何迁移到更强的SAM3模型。需要理解记忆设计原则并提升跟踪鲁棒性。

Method: 首先分析代表性SAM2跟踪器，发现它们主要在短期记忆帧选择上不同，但共享对象中心表示。然后在SAM3框架中重新实现这些记忆机制，进行大规模评估。基于实证发现，提出统一的混合记忆框架，将记忆分解为短期外观记忆和长期干扰解决记忆。

Result: 在十个多样化基准测试上进行大规模评估，证明所提框架在长期遮挡、复杂运动和干扰密集场景下，对SAM2和SAM3骨干网络均能一致提升鲁棒性。

Conclusion: 系统研究了SAM基跟踪中的记忆机制，提出的混合记忆框架通过模块化整合现有记忆策略，显著提升了跟踪鲁棒性，为未来记忆设计提供了指导原则。
...
TL;DR: Envision：一个基于扩散模型的视觉规划框架，通过目标图像约束生成物理合理且目标一致的轨迹，支持具身智能体操作任务


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型主要进行前向预测，缺乏明确的目标建模，导致空间漂移和目标对齐问题，需要一种能够生成物理合理且目标一致的视觉规划方法

Method: 采用两阶段框架：1) 目标图像模型识别任务相关区域，通过区域感知交叉注意力合成连贯的目标图像；2) 环境-目标视频模型基于首尾帧条件视频扩散模型，在初始观察和目标图像之间插值生成平滑物理合理的视频轨迹
...
<details>
  <summary>Details</summary>
Motivation: 传统IQA模型通常输出单一全局分数，对局部和细粒度失真极不敏感，导致ISR模型可能产生感知上不理想的伪影却获得虚假高分，造成奖励黑客问题，使优化目标与感知质量不一致。

Method: 1) 提出基于编码器-解码器架构的FinPercep-RM精细感知奖励模型，不仅提供全局质量分数，还生成感知退化图来空间定位和量化局部缺陷；2) 构建FGR-30k数据集训练该模型；3) 提出CCL协同进化课程学习机制，让奖励模型和ISR模型同步进行课程学习，从简单到复杂逐步过渡。

Result: 实验验证了该方法在RLHF框架下对ISR模型的有效性，在全局质量和局部真实感方面均取得显著提升，成功抑制了奖励黑客现象。

Conclusion: 通过精细感知奖励模型和协同进化课程学习的结合，有效解决了ISR任务中传统IQA模型的局限性，实现了更稳定、更符合人类感知的图像超分辨率优化。
...
Conclusion: 自回归先验是深度估计中与扩散模型互补的几何感知生成模型，在数据可扩展性和3D视觉任务适应性方面具有优势
...
TL;DR: 本研究系统评估了多种深度学习架构（3D Inception、双流、CNN-RNN）在超声心动图视频中左心室射血分数（LVEF）估计的性能，发现改进的3D Inception架构在EchoNet-Dynamic数据集上取得最佳结果（RMSE 6.79%）。


<details>
  <summary>Details</summary>
Motivation: 超声心动图是评估LVEF的常用非侵入性方法，但人工评估耗时且存在观察者间差异。深度学习有望达到专家水平性能，提供更高效、一致的自动化解决方案。

Method: 研究比较了3D Inception、双流和CNN-RNN等多种深度学习架构，系统评估了架构修改和融合策略。使用EchoNet-Dynamic数据集（10,030个超声心动图视频）进行训练和评估，重点关注超参数选择（卷积核大小、归一化策略）对性能的影响。

Result: 改进的3D Inception架构表现最佳，RMSE为6.79%。观察到过拟合趋势，较小、较简单的模型通常具有更好的泛化能力。模型性能对超参数选择高度敏感，特别是卷积核大小和归一化策略。

Conclusion: 深度学习在LVEF自动估计方面具有潜力，改进的3D Inception架构表现最优。研究发现的架构设计和训练策略见解不仅适用于超声心动图分析，也可推广到其他医学和非医学视频分析任务。
...
TL;DR: CLAdapter是一种新颖的集群注意力适配器，通过注意力机制和聚类中心来优化大规模预训练模型在数据有限的下游任务中的特征表示，实现跨多种模型架构和领域的自适应迁移。


<details>
  <summary>Details</summary>
Motivation: 虽然大规模数据集和预训练模型在计算机视觉领域取得了显著进展，但许多专业领域和数据有限的下游任务仍然面临挑战。需要一种方法能够将大规模预训练获得的丰富知识有效迁移到各种数据受限的科学领域任务中。

Method: 提出Cluster Attention Adapter (CLAdapter)，通过引入注意力机制和聚类中心，利用分布相关性和变换矩阵来个性化增强变换特征。该方法具有统一的接口设计，能够无缝集成到CNN和Transformer等多种模型架构中，支持2D和3D场景。

Result: 在10个数据集上的广泛实验表明，CLAdapter在通用、多媒体、生物、医疗、工业、农业、环境、地理、材料科学、OOD和3D分析等多个数据有限科学领域都达到了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，能够将大规模预训练获得的丰富表示有效适配到各种数据有限的下游任务中，具有广泛的适用性和优异的性能表现。
...
TL;DR: INTERACT-CMIL是一个多头部深度学习框架，通过共享特征学习和组合部分监督联合预测结膜黑色素细胞上皮内病变的五个组织病理学轴，在486个专家标注的结膜活检切片上优于CNN和基础模型基线。
...
Method: 提出INTERACT-CMIL多头部深度学习框架，通过共享特征学习与组合部分监督联合预测WHO4、WHO5、水平扩散、垂直扩散和细胞异型性五个组织病理学轴，并使用跨任务一致性的互依赖损失函数。
...
TL;DR: CritiFusion：无需额外训练的多模态语义批判与频域精炼框架，通过语义反馈和谱融合提升文本到图像生成的一致性与细节
...
Method: 提出CritiFusion框架，包含CritiCore模块（利用视觉语言模型和多个大语言模型进行语义批判与反馈）和SpecFusion（在谱域融合中间生成状态，注入粗粒度结构信息同时保留高频细节）

Result: 在标准基准测试中显著提升文本到图像对应性和视觉质量的人类对齐指标，在人类偏好评分和美学评估上达到与最先进奖励优化方法相当的水平

Conclusion: CritiFusion作为即插即用的精炼阶段，兼容现有扩散主干模型，通过语义批判和谱对齐策略有效提升了细节、真实性和提示保真度
...
TL;DR: 提出ARFM方法用于序列连续数据的概率建模，在多样化视频数据集上训练以预测长期轨迹点位置，并在人机运动预测任务中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常针对狭窄分布训练，而大规模视频预测模型虽然视觉真实感强，但难以准确建模复杂运动。需要一种能结合两者优势的方法来预测复杂运动轨迹。

Method: 开发了自回归流匹配（ARFM）方法，这是一种用于序列连续数据概率建模的新技术。在多样化视频数据集上训练，用于生成长期未来的轨迹点位置。

Result: 模型能够预测复杂运动，并证明在机器人动作预测和人体运动预测任务中，基于预测的未来轨迹进行条件化能显著提升下游任务性能。

Conclusion: ARFM方法在运动预测领域表现出色，特别是在复杂运动建模方面。通过条件化下游任务于预测轨迹，能有效提升人机运动预测性能，代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统非刚性配准方法在精度、计算复杂度和正则化之间存在权衡，且通常假设图像对在解剖同源区域具有强度相关性，限制了其在多模态场景下的应用。学习型方法需要大量训练数据且对未见模态性能下降。

Method: 采用神经ODE框架构建连续深度网络，结合结构描述符（模态无关的度量模型）和局部互信息计算非结构图像相似性。提出了三种变体：基于图像的结构描述符、基于特征的结构描述符，以及结合局部互信息的混合方法。

Result: 在不同扫描数据集组合的实验中，相比适用于大/小变形和多模态配准的先进基线方法，取得了优越的定性和定量结果。方法对显式正则化水平变化具有鲁棒性，保持低误差，适用于不同尺度的配准，且在大变形配准中效率更高。

Conclusion: 提出的基于神经ODE的多模态微分同胚配准框架，无需大量训练数据，对未见模态具有良好泛化能力，在精度、计算效率和正则化鲁棒性方面优于现有方法，为多模态医学图像配准提供了有效解决方案。
...
TL;DR: SCPainter统一框架结合3D高斯泼溅资产表示与扩散模型，实现自动驾驶场景中的真实3D资产插入和新视角合成。
...
Method: 提出SCPainter框架：1) 使用3D高斯泼溅表示汽车资产和场景点云；2) 将3D资产和场景点云投影到新视角；3) 用扩散模型以这些投影为条件生成高质量图像。

Result: 在Waymo Open Dataset上评估表明，该框架能够实现真实的3D资产插入和新视角合成，促进多样化和真实的驾驶数据创建。

Conclusion: SCPainter提供了一个统一框架，将3D资产插入和新视角合成相结合，能够生成多样且真实的驾驶场景数据，增强自动驾驶模型的鲁棒性。
...
TL;DR: 本研究评估了超声特异性自监督预训练（USF-MAE）在早孕期超声图像中检测囊性水瘤的性能，相比传统监督学习方法DenseNet-169，在准确率、敏感度、特异度和ROC-AUC等指标上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 囊性水瘤是高风险的产前超声发现，与染色体异常、结构畸形和不良妊娠结局密切相关。传统监督深度学习受限于小规模标注数据集，而自监督预训练可以利用大量未标注超声图像，提高检测的准确性和鲁棒性，支持可扩展的早期筛查项目。

Method: 研究采用基于掩码自编码（MAE）的超声自监督基础模型（USF-MAE），该模型在超过37万张未标注超声图像上进行预训练。随后在囊性水瘤和正常对照的二元分类任务上进行微调。使用与DenseNet-169基线相同的超声数据集、预处理流程和4折交叉验证协议进行评估，评估指标包括准确率、敏感度、特异度和ROC-AUC。通过Score-CAM可视化分析模型可解释性。

Result: USF-MAE在所有评估指标上均优于DenseNet-169基线模型：平均准确率0.96 vs 0.93，敏感度0.94 vs 0.92，特异度0.98 vs 0.94，ROC-AUC 0.98 vs 0.94。Score-CAM可视化显示模型关注胎儿颈部的临床相关区域。Wilcoxon符号秩检验证实性能提升具有统计学显著性（p=0.0057）。

Conclusion: 超声特异性自监督预训练能够显著提高囊性水瘤检测的准确性和鲁棒性，为基于深度学习的产前超声筛查提供了有效解决方案，特别是在标注数据有限的情况下。该方法具有临床相关性，能够支持可扩展的早期筛查项目。
...
TL;DR: 提出Freetime FeatureGS方法，通过可学习特征的高斯基元建模动态场景，结合对比损失和流式特征学习策略，直接从单帧分割图实现4D场景分解重建，无需视频分割。
...
Method: 1) 使用Freetime FeatureGS建模动态场景：高斯基元具有可学习特征和线性运动能力；2) 应用对比损失：基于2D分割图强制相同实例的基元特征相近；3) 流式特征学习：按时间顺序采样训练数据，实现特征在时间维度上的传播。

Result: 在多个数据集上的实验表明，该方法的重建质量显著优于现有方法，实现了更可靠的4D场景分解重建。

Conclusion: 提出的Freetime FeatureGS结合流式特征学习策略，能够直接从单帧分割图实现高质量的4D场景分解重建，避免了传统方法对不稳定视频分割的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型通常将视觉输入编码为token序列并与文本token拼接处理，但视觉token数量增长会大幅增加推理成本。现有视觉token剪枝方法往往忽略长上下文多图像场景的挑战。
...
Conclusion: 本文提出的自适应视觉token剪枝方法有效解决了长上下文多图像场景下的冗余问题，通过动态预算分配和多样性平衡，在降低推理成本的同时保持了模型性能。
...
TL;DR: 本文提出基于希尔伯特曲线重排序的邻域感知token缩减方法，通过保留2D空间中的邻域结构来提升ViT的计算效率。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在视觉识别任务中表现出色，但token表示的冗余性限制了计算效率。现有的token合并和剪枝策略往往忽视空间连续性和邻域关系，导致局部上下文信息丢失。

Method: 提出基于希尔伯特曲线重排序的邻域感知token缩减方法：1) 邻域感知剪枝(NAP)用于选择性保留token；2) 基于相邻token相似性的合并(MAT)用于局部token聚合。通过1D序列表示显式保留2D空间中的邻域结构。
...
TL;DR: 提出基于Fisher信息的主动学习算法，通过量化候选视角对语义高斯参数和变形网络的信息增益，优化多相机设置下的帧选择策略，提升渲染质量和语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 体智能体在任务中需要理解语义和动态，这些任务比静态场景理解有更多数据冗余。现有方法多采用启发式或随机策略选择视角，缺乏理论依据。

Method: 将视角选择问题形式化为主动学习问题，提出基于Fisher信息的算法，量化候选视角对语义高斯参数和变形网络的信息增益，联合处理语义推理和动态场景建模。

Result: 在大规模静态图像和动态视频数据集上评估，从多相机设置中选择信息丰富的帧。实验表明该方法持续提升渲染质量和语义分割性能，优于基于随机选择和不确定性启发式的基线方法。

Conclusion: 提出的基于Fisher信息的主动学习算法为视角选择提供了理论依据，能有效处理语义推理和动态场景建模，相比启发式或随机策略有显著优势。
...
TL;DR: 提出基于心理测量学分级响应模型（GRM）的AGIQA改进方法，通过算术方式建模难度级别，解决文本-图像语义漂移问题，提升AGIQA性能。


<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多级质量描述文本嵌入的相似度来评估图像质量，但发现相似度分布常呈现多模态模式，存在"语义漂移"问题，即文本嵌入与其预期描述之间存在语义不一致，影响了文本-图像共享空间学习的可靠性。

Method: 提出算术GRM质量分级（AGQG）模块：1）受心理测量学启发，采用分级响应模型（GRM）框架；2）设计双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度级别；3）通过算术方式建模难度生成，确保难度级别的单调性，强制产生单峰且可解释的质量分布。

Result: AGQG模块具有即插即用优势，集成到各种最先进的AGIQA框架中能持续提升性能。同时，该方法能有效泛化到自然图像和屏幕内容图像质量评估，显示出作为未来IQA模型关键组件的潜力。

Conclusion: 提出的基于算术GRM的质量分级方法能有效缓解语义漂移问题，提高AGIQA模型的可靠性和性能，为图像质量评估提供了新的心理测量学视角。
...
TL;DR: 提出EPD-Solver，一种新型ODE求解器，通过并行梯度评估减少截断误差，保持低延迟采样特性，并引入两阶段优化框架提升文本到图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但顺序去噪特性导致采样延迟高。现有基于求解器的加速方法在低延迟预算下常面临图像质量显著下降，主要原因是无法捕捉高曲率轨迹段导致的累积截断误差。

Method: 提出EPD-Solver，利用向量值函数的均值定理更准确逼近积分解，通过并行梯度评估减少截断误差。引入两阶段优化：首先通过蒸馏方法优化可学习参数，然后提出参数高效的强化学习微调方案，将求解器重新表述为随机Dirichlet策略，仅在低维求解器空间操作。

Result: EPD-Solver能够有效减少截断误差，保持低延迟采样特性，在复杂文本到图像生成任务中提升性能。方法灵活，可作为插件(EPD-Plugin)改进现有ODE采样器。

Conclusion: EPD-Solver通过并行梯度评估和两阶段优化，成功解决了扩散模型加速中的截断误差问题，在保持低延迟的同时显著提升图像生成质量，为扩散模型的实际应用提供了有效的解决方案。
...
TL;DR: VPTracker：首个基于多模态大语言模型的全局视觉语言跟踪框架，通过位置感知视觉提示机制提升目标定位的鲁棒性和抗干扰能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言跟踪方法通常局限于局部搜索，在视角变化、遮挡和快速目标移动等场景下容易失败。需要一种能够进行全局搜索的跟踪框架来提高鲁棒性。

Method: 提出VPTracker框架，利用多模态大语言模型的语义推理能力进行全局目标定位。设计位置感知视觉提示机制，基于目标先前位置构建区域级提示，使模型优先进行区域级识别，仅在必要时进行全局推理。

Result: 大量实验表明，该方法在挑战性场景下显著提升了跟踪稳定性和目标区分能力，为将多模态大语言模型集成到视觉跟踪中开辟了新途径。

Conclusion: VPTracker通过结合全局搜索和位置感知视觉提示，有效解决了传统局部搜索方法的局限性，在保持全局跟踪优势的同时抑制了干扰内容的影响。
...
TL;DR: 提出基于3D高斯和tri-plane表示的高效医学图像3D重建方法，在稀疏切片条件下保持结构连续性和语义一致性


<details>
  <summary>Details</summary>
Motivation: 传统医学图像3D重建方法计算成本高，在稀疏切片条件下容易产生结构不连续和细节丢失，难以满足临床精度要求

Method: 结合3D高斯表示和tri-plane表示，利用高斯表示的高效渲染和几何表示优势，增强稀疏切片条件下的结构连续性和语义一致性

Result: 在US和MRI等多模态医学数据集上，该方法能在稀疏数据条件下生成高质量、解剖一致、语义稳定的医学图像，同时显著提高重建效率
...
TL;DR: 评估开放词汇目标检测模型在低质量图像条件下的性能，发现模型在高级图像退化时性能显著下降，OWLv2表现最稳定
...
Method: 创建模拟现实世界低质量图像的新数据集，在多种图像退化条件下评估现有开放词汇目标检测模型（如OWLv2、OWL-ViT、GroundingDINO、Detic）的性能

Result: 低级图像退化时mAP分数无明显下降，但高级图像退化时所有模型性能急剧下降；OWLv2在不同类型退化中表现最稳定，其他模型性能显著下降

Conclusion: 开放词汇目标检测模型对高质量图像有较强鲁棒性，但在严重退化条件下性能受限；需要开发更鲁棒的模型来处理现实世界低质量图像，并发布了数据集和代码供未来研究
...
TL;DR: EgoReAct：首个自回归框架，从第一人称视角视频实时生成3D对齐的人类反应动作，通过HRD数据集解决现有数据空间不一致问题，结合VQ-VAE和GPT实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以忠实建模人类对第一人称视觉输入的自适应、上下文敏感反应，主要挑战在于严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在显著的空间不一致问题，例如动态动作总是与固定摄像头视频配对。

Method: 首先构建Human Reaction Dataset (HRD)解决数据稀缺和对齐问题；然后提出EgoReAct框架：1) 使用Vector Quantised-Variational AutoEncoder将反应动作压缩到紧凑的潜在空间；2) 训练Generative Pre-trained Transformer从视觉输入生成反应；3) 在生成过程中融入3D动态特征（度量深度和头部动态）增强空间基础。
...
TL;DR: DA360将Depth Anything V2适配到全景深度估计，通过学习ViT骨干网的偏移参数，将尺度/偏移不变输出转为尺度不变估计，结合环形填充消除接缝，在室内外基准测试中显著超越基线和现有方法。
...
Method: 1) 学习ViT骨干网的偏移参数，将模型的尺度/偏移不变输出转换为尺度不变估计，直接生成良好结构的3D点云；2) 在DPT解码器中集成环形填充，消除接缝伪影，确保空间连贯的深度图并保持球面连续性。

Result: 在标准室内基准和新构建的室外数据集Metropolis上，DA360相比基础模型在室内外基准测试中分别实现超过50%和10%的相对深度误差减少。相比PanDA等现有全景深度估计方法，在所有三个测试数据集上实现约30%的相对误差改进，建立了零样本全景深度估计的新SOTA。

Conclusion: DA360通过创新的偏移参数学习和环形填充集成，成功将透视域深度估计能力迁移到全景域，显著提升了零样本全景深度估计的性能，为机器人和AR/VR应用提供了更强大的环境结构感知能力。
...
TL;DR: 提出基于Kolmogorov-Arnold定理的新型可解释算子KANO用于图像超分辨率，通过B样条函数逼近光谱曲线，实现透明可控的退化建模。


<details>
  <summary>Details</summary>
Motivation: 现有可解释超分辨率方法通常依赖黑盒深度网络建模潜在变量，导致退化过程不透明且不可控。为解决这一问题，受Kolmogorov-Arnold定理启发，提出透明结构化表示方法。

Method: 提出Kolmogorov-Arnold神经算子(KANO)，采用有限个B样条函数的加性结构分段逼近连续光谱曲线。通过学习优化样条函数的形状参数，捕捉局部线性趋势和峰值谷值结构等关键光谱特征。

Result: KANO能够准确捕捉非线性拐点处的光谱特征，赋予超分辨率结果物理可解释性。通过理论建模和自然图像、航拍照片、卫星遥感数据的实验评估，系统比较了MLP和KAN在复杂序列拟合任务中的表现。

Conclusion: KANO为图像超分辨率提供了透明可控的退化建模方法，阐明了不同模型在表征复杂退化机制方面的优缺点，为可解释超分辨率技术的发展提供了有价值的见解。
...
TL;DR: SCaR-3D：基于3DGS的3D场景变化检测框架，通过有符号距离2D差分和多视图聚合，实现对象级变化检测和持续场景重建


<details>
  <summary>Details</summary>
Motivation: 现有3D变化检测方法存在空间不一致性，且无法明确分离变化前后的状态。需要一种能够识别对象级变化并支持持续场景重建的方法。

Method: 1. 基于有符号距离的2D差分模块；2. 多视图聚合（投票和剪枝）；3. 利用3DGS的一致性特性分离变化前后状态；4. 选择性更新动态区域的持续场景重建策略

Result: 方法在准确性和效率方面均优于现有方法，同时贡献了CCS3D合成数据集支持受控评估

Conclusion: SCaR-3D框架能够有效解决3D变化检测中的空间不一致问题，明确分离变化前后状态，并支持高效的持续场景重建
...
TL;DR: 提出一种基于两个仿射对应点估计相对姿态和焦距的新方法，利用IMU提供的垂直方向信息将5DOF问题简化为3DOF，通过多项式特征值方法求解。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶汽车、智能手机、无人机等应用中，相机常与IMU结合使用。IMU可提供相机垂直方向信息，从而将相对姿态从5DOF减少到3DOF。现有方法需要更多对应点或计算复杂，需要更高效的求解器。

Method: 1. 利用已知垂直方向建立两个仿射对应点的约束方程；2. 基于方程系统非平凡解的性质推导出四个方程；3. 这些方程仅涉及焦距和相对旋转角度两个参数；4. 使用多项式特征值方法求解焦距和相对旋转角度。

Result: 在合成和真实数据集上的评估表明，所提出的求解器性能优于现有最先进方法，能够更准确地估计相对姿态和焦距。

Conclusion: 该方法通过利用IMU提供的垂直方向信息，仅需两个仿射对应点即可有效估计相对姿态和焦距，为视觉惯性系统提供了一种高效可靠的解决方案。
...
Result: 在MUSON上测试多个SOTA小视觉语言模型，Qwen2.5-VL-3B达到最高决策准确率0.8625，证明MUSON可作为社交合规导航的有效可复用基准。

Conclusion: MUSON通过结构化推理标注解决了现有社交导航数据集的局限性，为社交合规导航提供了高质量的评估基准。
...
TL;DR: Lamps是一个从多视角学习解剖结构的医学影像基础模型，通过利用人体解剖的一致性、连贯性和层次性作为监督信号，在胸部X光片上预训练，展现出优越的鲁棒性、可迁移性和临床潜力。


<details>
  <summary>Details</summary>
Motivation: 医学影像的关键基础在于人体解剖结构，但现有的自监督学习方法往往忽视解剖学视角，限制了有效学习解剖特征的能力。需要开发能够捕捉解剖学基础结构的医学影像基础模型。

Method: 提出Lamps模型，通过自监督学习从多视角学习解剖结构，利用人体解剖的一致性、连贯性和层次性作为监督信号，在大规模胸部X光片上进行预训练。

Result: 在10个数据集上的实验表明，与10个基线模型相比，Lamps在微调和涌现特性分析中展现出优越的鲁棒性、可迁移性和临床潜力。
...
TL;DR: 提出NSF去偏方法，通过识别、中和、消除和更新四个步骤缓解深度学习中的伪相关性问题，在图像和NLP基准上相比ERM提升最差组准确率20%以上。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在训练中经常学习与类别标签虚假相关但与预测任务无关的特征。现有方法通常需要标注潜在虚假属性或基于经验假设（如偏见的简单性）过滤虚假特征，但由于真实数据中虚假相关的复杂性和难以捉摸性，这些方法可能表现不佳。

Method: 提出四步管道：1）识别：观察到受虚假特征影响的样本在特征空间中呈现分散分布，据此识别虚假特征；2）中和：基于简单分组策略获得偏置不变表示；3）消除：学习特征变换以与偏置不变表示对齐来消除虚假特征；4）更新：结合学习的特征变换更新分类器得到无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提升超过20%。

Conclusion: 提出的NSF去偏方法通过识别、中和、消除和更新四个步骤有效缓解深度学习中的伪相关问题，在多个基准测试中显著提升了最差组性能。
...
TL;DR: 提出了M-ErasureBench多模态评估框架，系统评估概念擦除方法在文本提示、学习嵌入和反转潜在空间三种输入模态上的表现，并针对后两者提出IRECE方法增强推理时鲁棒性。
...
Method: 1) 提出M-ErasureBench多模态评估框架，在三种输入模态（文本提示、学习嵌入、反转潜在空间）和两种访问设置（白盒、黑盒）下评估概念擦除方法；2) 提出IRECE方法，通过交叉注意力定位目标概念并在去噪过程中扰动相关潜在空间来增强推理时鲁棒性。

Result: 现有方法在文本提示上表现良好，但在学习嵌入和反转潜在空间上效果差（白盒设置下概念再现率超过90%）。IRECE方法能显著提升鲁棒性，在最具挑战性的白盒潜在反转场景下将概念再现率降低达40%，同时保持视觉质量。
...
TL;DR: SwinTF3D是一个轻量级多模态融合方法，通过统一视觉和语言表示实现文本引导的3D医学图像分割，解决了现有模型缺乏语义理解和领域适应性的问题。
...
Result: 在BTCV数据集上获得有竞争力的Dice和IoU分数，尽管架构紧凑但能很好地泛化到未见数据，相比传统Transformer分割网络具有显著效率优势。

Conclusion: SwinTF3D通过桥接视觉感知与语言理解，为交互式、文本驱动的3D医学图像分割建立了实用且可解释的范式，为临床成像提供了更适应性强、资源效率高的解决方案。
...
TL;DR: 论文提出GPS方法解决CFG在迭代细化中的稳定性问题，通过流形约束插值替代外推，实现有界误差和动态调度，显著提升图像质量和语义对齐。


<details>
  <summary>Details</summary>
Motivation: 基于去噪-反演循环的迭代细化方法在提升扩散模型质量和控制方面很强大，但当与标准无分类器引导（CFG）结合时，其效果受到严重限制。研究发现CFG的外推性质会系统性地将采样路径推离数据流形，导致近似误差发散并破坏细化过程。

Method: 提出引导路径采样（GPS）新范式，用原则性的流形约束插值替代不稳定的外推，确保采样路径保持在数据流形上。理论上证明这种修正能将误差序列从无界放大转变为严格有界。同时设计最优调度策略，动态调整引导强度，使语义注入与模型从粗到细的生成过程对齐。

Result: 在SDXL和Hunyuan-DiT等现代骨干模型上的大量实验表明，GPS在感知质量和复杂提示遵循方面均优于现有方法。GPS在SDXL上实现了0.79的ImageReward和0.2995的HPS v2，同时在GenEval上将整体语义对齐准确率提升至57.45%。

Conclusion: 路径稳定性是有效迭代细化的前提条件，GPS提供了一个稳健的框架来实现这一目标。该方法通过流形约束插值解决了CFG的不稳定性问题，为扩散模型的迭代细化提供了理论保证和实践方案。
...
Method: 基于输入高斯溅射坐标识别并剪除哈希网格中的无效特征，只编码有效特征，从而减少哈希网格的存储大小

Result: 在标准化委员会定义的CTC测试条件下，相比基线方法平均比特率降低8%，在不影响模型性能的前提下提升了率失真性能

Conclusion: 提出的哈希网格特征剪枝方法能有效减少存储和传输开销，为高斯溅射的隐式神经场学习提供了更高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要关注视觉-语言或音频-语言任务，缺乏能够同时理解和生成同步音频-视频内容的统一模型，需要解决音频视频时空对齐和同步生成的挑战。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音频-视频融合，使用同步感知可学习查询连接预训练的JAV-DiT生成器。设计三阶段训练流程：多模态预训练、音频-视频微调、大规模指令调优。

Result: 在联合音频-视频理解和生成基准测试中表现优于现有MLLMs，特别是在复杂和时间同步设置下。构建了包含20万+GPT-4o标注的音频-视频-文本对话的高质量指令数据集JavisInst-Omni。

Conclusion: JavisGPT是首个统一的多模态大语言模型，能够实现时间一致的音频-视频理解和生成，为联合音频-视频多模态任务提供了有效的解决方案。
...
TL;DR: ColaVLA是一个统一的视觉-语言-动作框架，通过将推理从文本转移到统一潜空间，并结合分层并行轨迹解码器，解决了VLM规划器在连续控制、延迟和实时部署方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的规划器面临三个关键挑战：离散文本推理与连续控制之间的不匹配、自回归思维链解码的高延迟，以及限制实时部署的低效或非因果规划器。

Method: 提出ColaVLA框架，包含认知潜空间推理器（压缩场景理解为决策导向的元动作嵌入，仅需两次VLM前向传播）和分层并行规划器（单次前向传播生成多尺度、因果一致的轨迹）。

Result: 在nuScenes基准测试中，ColaVLA在开环和闭环设置下均实现了最先进的性能，具有优越的效率和鲁棒性。

Conclusion: ColaVLA在保持VLM泛化能力和可解释性的同时，实现了高效、准确和安全的轨迹生成，解决了当前VLM规划器的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像中密集微小目标检测面临严重遮挡和像素足迹有限的问题，现有方法无法自适应聚焦密度集中区域，导致特征学习效果不佳。

Method: 提出DRMNet：1) 密度生成分支建模目标分布模式；2) 密集区域聚焦模块利用密度图识别密集区域，实现高效局部-全局特征交互；3) 双滤波器融合模块通过离散余弦变换分离多尺度特征为高低频分量，进行密度引导的交叉注意力增强互补性。

Result: 在AI-TOD和DTOD数据集上的实验表明，DRMNet超越了现有最先进方法，特别是在高目标密度和严重遮挡的复杂场景中表现优异。

Conclusion: DRMNet通过密度图引导的自适应特征学习，有效解决了高分辨率遥感图像中密集微小目标检测的挑战，在复杂场景中展现出优越性能。
...
Result: 在Pascal VOC 2007+2012上使用Faster R-CNN，在MS COCO 2017上使用YOLOv11，均取得一致且显著的性能提升，同时保持实时推理速度
...
TL;DR: WRCFormer：一种融合原始4D毫米波雷达立方体与相机数据的3D目标检测框架，通过多视图表示和渐进式融合机制，在恶劣天气条件下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达在自动驾驶中成本低、全天候鲁棒，但点云稀疏且语义信息有限。现有融合方法要么因多级信号处理导致信息损失，要么直接使用原始雷达数据计算成本过高。

Method: 提出WRCFormer框架：1）使用解耦雷达立方体的多视图表示；2）设计基于小波注意力的特征金字塔网络增强稀疏信号表示；3）引入几何引导的渐进式融合机制，两阶段查询式融合多视图特征。

Result: 在K-Radar基准测试中达到最先进性能：所有场景提升约2.4%，雨夹雪场景提升1.6%，证明在恶劣天气条件下的鲁棒性。

Conclusion: WRCFormer通过有效融合原始雷达立方体与相机数据，解决了4D雷达稀疏性和信息损失问题，为恶劣天气条件下的自动驾驶感知提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR，无法支持实时YOLO检测框架，需要解决YOLO中的灾难性遗忘问题

Method: 基于预训练YOLO-World模型，采用阶段式参数高效微调。包含三个核心组件：1) CPR缓解前景-背景混淆；2) IKS识别并更新关键卷积核；3) CAKD通过非对称蒸馏解决知识蒸馏错位问题

Result: 在传统和LoCo COCO基准测试中，YOLO-IOD实现了卓越性能，遗忘最小，证明了其在YOLO框架下增量学习的有效性

Conclusion: YOLO-IOD是首个支持YOLO的实时增量目标检测框架，通过解决三种知识冲突有效缓解灾难性遗忘，为实时增量检测提供了新方案
...
Method: 提出ReamCamo统一外绘框架：1）引入布局控制调节全局图像结构，改善前景与生成背景的语义一致性；2）构建多模态文本-视觉条件，结合统一细粒度文本任务描述和纹理导向的背景检索，共同指导生成过程以增强视觉保真度和真实感
...
TL;DR: PoseStreamer：针对高速移动场景的鲁棒多模态6DoF姿态估计框架，结合自适应姿态记忆队列、物体中心2D跟踪器和射线姿态滤波器，在MoCapCube6D数据集上验证了优越性能。
...
Method: 提出PoseStreamer框架，包含三个核心组件：1) 自适应姿态记忆队列，利用历史方向线索保持时间一致性；2) 物体中心2D跟踪器，提供强2D先验以提升3D中心召回；3) 射线姿态滤波器，沿相机射线进行几何细化。还构建了MoCapCube6D多模态数据集用于快速运动下的性能评估。

Result: 实验表明PoseStreamer在高速移动场景中实现了优越的准确性，同时作为无模板框架对未见过的移动物体表现出强大的泛化能力。

Conclusion: PoseStreamer通过创新的多模态框架设计，有效解决了高速移动场景下的6DoF姿态估计问题，为事件相机在动态环境中的应用提供了实用解决方案。
...
TL;DR: SSA框架通过对称最优传输对齐和复合方向引导策略，增强医学图像分割中位置、描述和诊断混合文本的处理能力，实现空间感知的精确分割。


<details>
  <summary>Details</summary>
Motivation: 当前文本引导医学图像分割方法存在两个关键瓶颈：1) 难以同时处理诊断性和描述性文本，难以识别病灶并建立与图像区域的关联；2) 现有方法关注病灶描述但忽略位置约束，导致关键偏差（如"左下肺"可能错误覆盖双侧肺）。
...
Conclusion: SSA框架通过增强混合医学文本（位置、描述和诊断信息）的处理能力，解决了现有方法的局限性，实现了更准确的空间感知医学图像分割。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示的身份特征移除方法要么依赖预训练模型对主体的良好表示，要么需要对特定身份进行模型微调，缺乏对匿名化后面部属性的控制。

Method: 采用条件扩散反演框架，直接操作图像而无需文本提示；引入身份引导的条件分支以泛化到训练数据之外的主体；支持属性可控的匿名化。

Result: 在身份移除、属性保留和图像质量方面实现了最先进的平衡，优于现有匿名化方法。

Conclusion: 提出的反向个性化框架为面部匿名化提供了一种有效且可控的解决方案，无需文本提示或模型微调，支持属性保留的匿名化。
...
TL;DR: 提出一个基于RGB无人机的低成本果园智能系统，集成病害检测、新鲜度评估和苹果检测定位，实现离线现场推理
...
Method: 使用ResNet50进行叶片病害检测，VGG16进行苹果新鲜度评估，YOLOv8进行实时苹果检测定位，系统运行在ESP32-CAM和树莓派上实现离线推理
...
TL;DR: OpenGround：基于主动认知推理的零样本开放世界3D视觉定位框架，通过动态更新的对象查找表解决预定义类别限制问题


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法依赖预定义的对象查找表（OLT）来查询视觉语言模型（VLM），这限制了在未定义或意外目标场景中的应用。需要解决开放世界场景中的3D视觉定位问题。

Method: 提出OpenGround框架，核心是主动认知推理（ACR）模块。ACR通过认知任务链进行类人感知，主动推理上下文相关对象，通过动态更新的OLT扩展VLM认知范围。还创建了OpenTarget数据集用于评估。

Result: 在Nr3D上取得竞争性性能，在ScanRefer上达到最先进水平，在OpenTarget数据集上实现了17.6%的显著提升。能够处理预定义和开放世界类别。

Conclusion: OpenGround通过主动认知推理机制有效解决了开放世界3D视觉定位问题，突破了预定义OLT的限制，为更广泛的应用场景提供了解决方案。
...
TL;DR: 提出Geo-Semantic Contextual Graph (GSCG)框架，通过构建包含几何、色彩、材料属性和空间关系的显式图结构，结合目标物体、邻近物体和全局场景上下文进行物体分类，显著提升识别准确率。


<details>
  <summary>Details</summary>
Motivation: 人类识别物体时能充分利用丰富的场景上下文信息（空间关系、材料属性、物体共现等），而现有计算识别系统通常在孤立图像区域上操作，忽略了这些关键上下文信息。

Method: 1. 从单目图像构建Geo-Semantic Contextual Graph (GSCG)：结合度量深度估计器和统一的全景分割与材料分割模型；2. 物体编码为节点（几何、色彩、材料属性），空间关系编码为边；3. 设计专门的图分类器，聚合目标物体、邻近物体和全局场景特征进行分类预测。

Result: 在COCO 2017数据集上：1. 上下文感知模型达到73.4%分类准确率，远超上下文无关版本（最低38.4%）；2. 显著优于微调ResNet模型（最高53.5%）和最先进的多模态LLM Llama 4 Scout（最高42.3%）；3. 显式图结构使推理过程具有可解释性。

Conclusion: 显式结构化和可解释的上下文对于物体识别任务具有显著优势，GSCG框架通过整合几何、语义和材料信息，有效模拟人类利用场景上下文进行物体识别的能力。
...
TL;DR: 该报告分析了两种现代视觉语言模型(Qwen2.5-VL-7B-Instruct和Llama-4-Scout-17B-16E-Instruct)的架构特性，并将其映射到视频到人工制品管道的实际实现中，重点讨论了结构化输出验证和系统约束。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解现代视觉语言模型的架构特性如何影响实际视频处理系统的设计和实现，特别是结构化输出生成、验证机制以及系统约束对工程选择的影响。

Method: 采用架构主导的分析方法，首先总结共享的多模态基础(视觉标记化、Transformer注意力、指令跟随)，然后详细描述每个模型的架构特性，最后将模型行为映射到视频处理系统的具体约束和实现细节。

Result: 分析揭示了结构化输出可能语法正确但语义错误、模式验证仅限于结构而非几何正确性、人物标识符在当前提示合约中仅限于帧内等关键发现，这些区别对编写可靠声明、设计健壮接口和规划评估至关重要。

Conclusion: 该研究强调了理解视觉语言模型架构特性与实际系统实现之间关系的重要性，为设计可靠的视频处理管道提供了工程指导，并指出了结构化输出验证的局限性。
...
Method: 提出Co2S框架：1) 构建异构双学生架构，使用预训练的CLIP和DINOv3初始化两个不同的ViT模型；2) 引入显式-隐式语义协同引导机制，利用文本嵌入和可学习查询提供类级指导；3) 开发全局-局部特征协同融合策略，融合CLIP的全局上下文信息和DINOv3的局部细节。

Result: 在六个流行数据集上的广泛实验表明，该方法在各种划分协议和多样化场景中始终取得领先性能。

Conclusion: Co2S通过融合视觉语言模型和自监督模型的先验知识，有效缓解了伪标签漂移问题，为半监督遥感图像语义分割提供了稳定且高性能的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大规模3D场景扫描成本高昂且劳动密集，而网络上有大量未标记视频可用。研究是否可以从无真实3D传感器的未标记视频中学习3D表示。

Method: 提出LAM3C框架：1) 构建RoomTours数据集，从网络收集房间漫游视频，使用现成重建模型生成49,219个场景点云；2) 引入噪声正则化损失，通过强制局部几何平滑性和特征稳定性来稳定表示学习；3) 使用拉普拉斯感知多级3D聚类与Sinkhorn-Knopp算法。

Result: 在不使用任何真实3D扫描的情况下，LAM3C在室内语义和实例分割任务上超越了之前的自监督方法，证明了未标记视频作为3D自监督学习数据源的潜力。

Conclusion: 未标记视频是3D自监督学习的丰富数据源，可以从无真实3D传感器的视频中有效学习3D表示，为降低3D数据收集成本提供了新途径。
...
TL;DR: 提出了Video-BrowseComp基准测试，用于评估开放网络环境下的智能视频研究能力，填补了现有视频基准测试仅关注被动感知而缺乏主动视频推理的空白。


<details>
  <summary>Details</summary>
Motivation: 随着自主智能体从被动检索向主动开放网络研究转变，视频作为网络中最动态的模态存在显著的研究空白。现有视频基准测试主要关注被动感知，无法评估需要主动探索视频时间线、交叉引用分散证据、并在开放网络上验证声明的智能视频研究能力。

Method: 创建了包含210个问题的Video-BrowseComp基准测试，强制要求依赖时间视觉证据，确保答案不能仅通过文本搜索获得，而必须通过导航视频时间线来验证外部声明。

Result: 评估显示最先进的模型（包括GPT-5.1 with Search）仅达到15.24%的准确率。分析表明这些模型主要依赖文本代理，在元数据丰富的领域表现良好，但在元数据稀疏的动态环境（如体育、游戏）中表现崩溃，这些环境需要视觉基础。

Conclusion: Video-BrowseComp作为首个开放网络视频研究基准测试，推动了该领域从被动感知向主动视频推理的进步，揭示了当前模型在需要视觉基础的动态视频研究中的关键瓶颈。
...
TL;DR: 提出基于MedSAM的肺部分割引导的胸部X光分类流程，通过解剖学先验改善多标签异常分类，发现掩码效果具有任务和架构依赖性。


<details>
  <summary>Details</summary>
Motivation: 胸部X光自动解读面临疾病信号弱、数据集偏差和空间监督有限等挑战，医学图像分割基础模型MedSAM能提供解剖学先验，可能提高CXR分析的鲁棒性和可解释性。

Method: 提出分割引导的CXR分类流程：1）使用Airlangga大学医院数据集微调MedSAM作为肺区域提取模块；2）在NIH CXR数据集上训练和评估用于五种异常（肿块、结节、肺炎、水肿、纤维化）多标签预测的深度卷积神经网络；3）通过衍生分数评估正常病例；4）比较原始图像、松散掩码和紧密掩码三种条件下的性能。

Result: MedSAM在不同成像条件下产生解剖学合理的肺掩码。掩码效果具有任务和架构依赖性：ResNet50在原始图像上获得最佳异常判别性能；松散掩码获得相当的宏观AUROC但显著改善正常病例筛查；紧密掩码降低异常级别性能但提高训练效率；松散掩码通过保留肺门周围和周围环境部分缓解性能下降。

Conclusion: 肺掩码应被视为可控制的空间先验，根据主干网络和临床目标进行选择，而非统一应用。松散掩码在异常分类和正常病例筛查之间提供平衡，紧密掩码适合训练效率优先的场景。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二进制掩码，这些范式存在特征纠缠问题，导致解剖基底损坏或结构不连续。需要一种能够保持解剖完整性同时生成高质量病理变化的合成方法。
...
Conclusion: PathoSyn提供了一个数学原理化的管道，用于生成高保真合成数据，支持可解释的反事实疾病进展建模，促进低数据环境下稳健诊断算法的发展，并为临床决策支持系统提供基准测试环境。
...
TL;DR: 本文发现深度可变形配准模型具有固有的域偏移免疫性，这源于它们依赖局部特征而非全局外观进行变形估计。作者提出的UniReg框架通过解耦特征提取和变形估计，在单数据集训练下实现了跨域和多模态的鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习在可变形图像配准中超越了传统优化方法，但学习模型通常被认为对域偏移敏感，需要大量多样化训练数据。本文旨在探究深度可变形配准模型是否具有固有的域偏移免疫性，并解释其底层机制。

Method: 提出UniReg通用配准框架，将特征提取与变形估计解耦：使用固定的预训练特征提取器提取局部特征，然后通过UNet变形网络估计变形场。该方法在单数据集上训练，但能实现跨域和多模态配准。

Result: UniReg在单数据集训练下表现出与优化方法相当的跨域和多模态鲁棒性。分析发现传统CNN模型在模态偏移下的失败源于早期卷积层的数据集诱导偏差，而局部特征一致性是学习型配准鲁棒性的关键驱动因素。

Conclusion: 深度可变形配准模型具有固有的域偏移免疫性，这源于对局部特征的依赖而非全局外观。局部特征一致性是鲁棒性的关键，这为设计保持域不变局部特征的主干网络提供了理论依据。
...
TL;DR: GeoTeacher通过几何关系监督和体素数据增强提升半监督3D目标检测性能，在有限标注数据下增强模型对物体几何结构的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法通常使用异构教师模型提供伪标签或强制特征一致性，但忽略了模型在有限标注数据下对物体几何结构敏感性不足的问题，而几何信息对于目标感知和定位至关重要。

Method: 1) 基于关键点的几何关系监督模块，将教师模型对物体几何的知识传递给学生模型；2) 体素级数据增强策略增加物体几何多样性，结合距离衰减机制保护远距离物体的完整性；3) 可与不同SS3D方法结合。

Result: 在ONCE和Waymo数据集上的大量实验表明方法有效且具有泛化能力，取得了新的state-of-the-art结果。

Conclusion: GeoTeacher通过几何关系监督和增强策略有效提升了半监督3D目标检测中模型对几何结构的理解能力，可与其他方法结合进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的对齐评估方法大多依赖粗粒度指标或静态QA流程，缺乏细粒度可解释性且难以反映人类偏好。

Method: 采用"grounding-reasoning-conclusion"结构化范式，利用多模态大语言模型显式定位语义元素并得出可解释的对齐判断，通过GRPO优化模型，使用包含结构格式、定位准确性和对齐保真度的复合奖励函数。

Result: 在四个基准测试(EvalMuse-40K, RichHF, MHaluBench, GenAI-Bench)上均达到最先进性能，优于强大的专有模型和监督基线，同时相比现有迭代视觉推理方法具有更高的推理效率。

Conclusion: REVEALER提供了一个统一、可解释且高效的框架，用于细粒度评估文本到图像模型的对齐度，在多个维度上超越了现有方法。
...
TL;DR: GVSynergy-Det：一种无需深度或密集3D监督的图像3D检测新框架，通过高斯-体素双表征协同学习提升检测精度
...
Method: 提出高斯-体素协同表示学习框架：1）采用可泛化的高斯泼溅提取几何特征；2）设计跨表示增强机制，用高斯场的几何细节丰富体素特征；3）通过可学习集成直接利用两种表示的特征。

Result: 在ScanNetV2和ARKitScenes等室内基准测试中达到最先进水平，显著优于现有方法，且无需任何深度或密集3D几何监督（如点云或TSDF）。

Conclusion: 高斯和体素表示具有互补性，通过协同学习可以有效提升图像3D检测性能，为无监督3D检测提供了新思路。
...
Method: 1) 基于3D高斯场景表示的统一框架，将丰富的语言特征嵌入每个高斯基元实现早期模态对齐；2) 任务感知语言引导采样策略，去除冗余3D高斯并注入准确紧凑的3D令牌到LLM；3) 双条件多模态生成模型，结合高级语言条件和低级图像条件共同指导生成过程。

Result: 在nuScenes和NuInteract数据集上进行全面研究，验证了框架的有效性，实现了最先进的性能。

Conclusion: 提出的基于3D高斯场景表示的驾驶世界模型框架成功解决了现有方法的局限性，实现了3D场景理解和多模态场景生成，通过早期模态对齐和双条件生成机制提升了性能。
...
Method: 提出ForCM方法，将对象图像分析与深度学习结合。评估了UNet、UNet++、ResUNet、AttentionUNet和ResNet50-Segnet等深度学习模型，使用Sentinel-2 Level 2A卫星影像（三波段和四波段数据集）。最佳模型与OBIA技术集成。

Result: ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，相比传统OBIA的92.91%有显著提升。证明了免费工具如QGIS在准确制图方面的潜力。

Conclusion: ForCM方法显著提高了森林覆盖制图精度，深度学习与OBIA的结合优于传统方法。研究展示了免费工具在环境监测中的实用价值，支持全球森林保护工作。
...
<details>
  <summary>Details</summary>
Motivation: 军事目标检测在民用和军事应用中至关重要，但当前域适应算法主要针对自然或自动驾驶场景，而军事领域环境多样且缺乏数据集。SAR数据成本高，RGB相机成本低但缺乏军事目标检测数据集。

Method: 使用虚幻引擎生成RGB合成数据，构建合成到真实的训练-验证数据集对，对最先进的域适应方法进行基准测试，按监督程度区分方法。

Result: 使用最小图像提示（如目标类别）的方法相比无监督或半监督域适应方法有显著改进，识别了当前仍需克服的挑战。

Conclusion: 通过合成数据生成和跨域适应实验，展示了RGB合成数据在军事目标检测中的潜力，同时指出了现有域适应方法在军事场景中的局限性。
...
TL;DR: 提出一种自适应选择扩散模型时间步特征的方法，通过任务感知时间步选择和特征整合模块，在少样本密集预测任务中实现更优性能
...
Result: 在大型挑战性Taskonomy数据集上进行实证验证，特别是在通用和少样本学习场景中，该方法在密集预测性能上表现出优越性

Conclusion: 提出的可学习时间步整合方法能够自适应选择最适合的时间步特征，有效提升少样本密集预测任务的性能，为扩散模型在密集预测任务中的应用提供了更优解决方案
...
TL;DR: 作者提出了AVOID数据集，这是一个在模拟环境中收集的用于实时障碍物检测的对抗性视觉条件数据集，包含各种天气和时间条件下的意外道路障碍物，并配有多种标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有道路驾驶数据集要么只包含正常场景，要么只包含对抗性场景，且往往没有将道路障碍物与其他类别在同一视觉域中捕获。为了可靠地实时检测各种对抗性条件下的意外小道路危险，需要更全面的数据集。
...
Conclusion: AVOID数据集填补了现有数据集的空白，为在对抗性视觉条件下进行实时障碍物检测提供了全面的基准测试平台，支持多种视觉感知任务的研究和评估。
...
TL;DR: MM-UAVBench：首个针对低空无人机场景的多模态大语言模型综合基准，包含感知、认知、规划三个核心能力维度，19个子任务，5700+人工标注问题，揭示当前模型在低空场景的适应性不足
...
Method: 提出MM-UAVBench基准，系统评估MLLM在低空无人机场景中的三个核心能力维度：感知、认知和规划。包含19个子任务，超过5700个从公开无人机数据集收集的真实数据中人工标注的问题

Result: 对16个开源和专有MLLM的广泛实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析揭示了空间偏见和多视角理解等关键瓶颈，阻碍了MLLM在无人机场景中的有效部署
...
TL;DR: 提出Holi-DETR，一种利用三种上下文信息（共现关系、物品间空间关系、物品与人体关键点关系）来整体检测时尚物品的检测器，显著提升了DETR系列模型在时尚检测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临两大挑战：1）时尚物品外观高度多样化带来的歧义性；2）物品子类别之间的相似性。传统检测器独立检测每个物品，忽略了时尚物品之间存在的有意义关系（如搭配风格）。

Method: 提出Holi-DETR，在DETR框架基础上整合三种异构上下文信息：1）时尚物品间的共现概率关系；2）基于物品间空间排列的相对位置和大小关系；3）物品与人体关键点的空间关系。通过这种整体检测方式减少歧义。

Result: 实验表明，Holi-DETR将原始DETR的平均精度（AP）提升了3.6个百分点，将更先进的Co-DETR提升了1.1个百分点，显著改善了时尚物品检测性能。

Conclusion: 通过利用三种类型的上下文信息进行整体检测，Holi-DETR有效解决了时尚物品检测中的歧义问题，证明了上下文信息在时尚检测任务中的重要性。
...
TL;DR: UniMAGE是一个统一的导演模型，将剧本创作和关键镜头设计整合到单一框架中，通过混合Transformer架构和"先交错后解耦"的训练范式，让非专业人士也能创作长上下文、多镜头的电影。
...
Method: 采用混合Transformer架构统一文本和图像生成，引入"先交错后解耦"训练范式：1)交错概念学习，利用交错文本-图像数据促进对剧本的深度理解和想象力解释；2)解耦专家学习，将剧本写作与关键帧生成解耦，提高故事讲述的灵活性和创造性。

Result: UniMAGE在开源模型中实现了最先进的性能，能够生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功地将剧本创作和关键镜头设计统一到单一框架中，通过创新的训练范式增强了叙事逻辑和关键帧一致性，为非专业人士创作高质量电影提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 工业制造中异常检测至关重要，但真实缺陷图像稀缺。现有合成方法存在明显权衡：基于规则的合成成本低但质量差，基于生成模型的合成质量高但成本大。需要找到平衡方案。

Method: 1) 使用预训练文本引导图像翻译模型生成合成缺陷图像；2) 引入图像检索模型评估生成图像与真实正常图像的相似性，过滤无关输出；3) 提出两阶段训练策略：先在大规模规则合成图像上预训练，再在小规模高质量图像上微调。

Result: 在MVTec AD数据集上的实验证明了该方法的有效性，显著降低了数据收集成本，同时提高了异常检测性能。

Conclusion: 提出的框架通过结合文本引导图像翻译和图像检索，实现了高效、高质量的合成缺陷图像生成，配合两阶段训练策略，在降低数据收集成本的同时提升了异常检测性能。
...
TL;DR: 提出SGPS方法，利用SURE梯度更新和PCA噪声估计来纠正扩散采样轨迹偏差，在少于100次神经网络评估下实现高质量逆问题重建


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的逆问题求解方法需要数百或数千步才能实现高质量重建，存在误差累积问题，需要更高效的采样方法

Method: 提出SURE引导的后验采样(SGPS)，使用Stein无偏风险估计梯度更新和PCA噪声估计来纠正采样轨迹偏差，减少早期和中期采样阶段的噪声诱导误差

Result: SGPS在少于100次神经网络评估下保持高质量重建，在多种逆问题上优于现有方法，特别是在低NFE计数时表现更优

Conclusion: SGPS通过减少采样误差累积，实现了更高效的后验采样，为基于扩散模型的逆问题求解提供了更实用的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 红外气体泄漏检测对环境和工业安全至关重要，但现有方法面临挑战：气体羽流微弱、尺寸小、半透明且边界模糊，传统检测方法难以有效识别。

Method: 提出三部分创新：1) Gas Block扩散-对流单元建模气体传输；2) AGPEO自适应梯度和相位边缘算子提取可靠边缘先验；3) CASR-PAN内容自适应稀疏路由路径聚合网络进行跨尺度特征选择传播。

Result: 在IIG数据集上达到AP 29.8%、AP50 84.3%、小目标AP 25.3%，相比RT-DETR-R18基线分别提升3.0%、6.5%、5.3%，仅需43.7 Gflops和14.9 M参数，在IIG和LangGas数据集上超越现有CNN和Transformer检测器。

Conclusion: PEG-DRNet通过物理边缘混合设计有效解决了微弱气体羽流检测难题，在精度和计算效率方面达到最佳平衡，为红外气体泄漏检测提供了高效解决方案。
...
Method: 提出免训练的两阶段数据剪枝方法：1) 基于熵的准则高效移除低信息量样本；2) 利用遥感场景分类数据集作为参考基准，进行场景感知聚类和分层采样，在平衡聚类级均匀性和样本代表性的同时，实现高剪枝率下的细粒度选择。

Result: 即使剪枝85%的训练数据，该方法仍能显著改善收敛性和生成质量。使用该方法训练的扩散基础模型在下游任务（如超分辨率和语义图像合成）中持续达到最先进的性能。

Conclusion: 该数据剪枝范式为开发遥感生成基础模型提供了实用指导，能够在高剪枝率下快速选择高质量子集，使初步基础模型快速收敛并作为多功能骨干网络。
...
TL;DR: 本文提出了一种结合动态分辨率输入策略和多尺度视觉语言对齐机制的视觉语言模型框架，用于提升遥感图像多模态融合的语义理解精度和计算效率。
...
Method: 提出动态分辨率输入策略（DRIS），采用粗到细的方法根据图像内容复杂度自适应分配计算资源；构建多尺度视觉语言对齐机制（MS-VLAM），建立对象、局部区域和全局三个层次的对齐，系统捕获跨模态语义一致性。

Result: 在RS-GPT4V数据集上的实验表明，该框架在图像描述和跨模态检索任务中显著提升了语义理解精度和计算效率，在BLEU-4、CIDEr和R@10等指标上优于传统方法。

Conclusion: 该技术框架为构建高效鲁棒的遥感多模态系统提供了新方法，为智能遥感解译的工程应用奠定了理论基础并提供了技术指导。
...
TL;DR: ViLaCD-R1：一个两阶段遥感变化检测框架，结合多图像推理器和掩码引导解码器，通过监督微调和强化学习训练视觉语言模型，实现更准确的语义变化识别和定位。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法（像素级算子或编码器-解码器网络）难以捕捉高层语义信息，且对非语义扰动敏感。现有多模态和视觉语言模型方法虽然增强了语义理解，但仍存在空间定位不准确、像素级边界划分不精确和可解释性有限等问题。

Method: 提出ViLaCD-R1两阶段框架：1）多图像推理器（MIR）：通过监督微调和强化学习训练视觉语言模型，输入双时相图像块，输出粗粒度变化掩码；2）掩码引导解码器（MGD）：将双时相图像特征与粗掩码结合，预测精确的二元变化图。

Result: 在多个遥感变化检测基准测试中，ViLaCD-R1显著提升了真实语义变化的识别和定位能力，有效抑制了非语义变化，在复杂真实场景中达到了最先进的准确率。

Conclusion: ViLaCD-R1通过结合视觉语言模型的语义理解能力和掩码引导的精确解码，解决了遥感变化检测中的语义理解、空间定位和边界划分等关键问题，为复杂场景下的变化检测提供了有效解决方案。
...
TL;DR: ASemconsist：通过选择性文本嵌入修改和语义控制策略，解决文本到图像生成中角色身份一致性与提示对齐的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在生成图像序列时，难以在保持角色身份一致性和确保每张图像提示对齐之间取得平衡，存在明显的权衡问题。

Method: 1. 选择性文本嵌入修改实现显式语义控制；2. 利用FLUX中的填充嵌入作为语义容器；3. 自适应特征共享策略，根据文本模糊性自动应用约束；4. 提出一致性质量评分（CQS）统一评估协议。

Result: 该框架实现了最先进的性能，有效克服了先前方法在身份一致性和提示对齐之间的权衡问题。

Conclusion: ASemconsist通过创新的语义控制策略和自适应特征共享，成功解决了文本到图像生成中的角色身份一致性难题，为多图像序列生成提供了有效解决方案。
...
TL;DR: 提出一种轮廓信息感知的2D高斯泼溅框架，通过引入物体分割先验来改善图像表示中的边界清晰度


<details>
  <summary>Details</summary>
Motivation: 现有2D高斯泼溅方法在Gaussian数量较少时会产生模糊或不清晰的边界，缺乏轮廓感知能力

Method: 将物体分割先验融入高斯表示，约束每个Gaussian在特定分割区域内进行光栅化，防止跨边界混合，并引入预热方案稳定训练

Result: 在合成色卡和DAVIS数据集上，相比现有2DGS方法，在物体边缘处获得更高的重建质量，特别是在Gaussian数量很少的情况下，同时保持快速渲染和低内存使用

Conclusion: 提出的轮廓信息感知2D高斯泼溅框架能有效改善边界清晰度，在压缩场景下保持边缘结构，实现高质量图像表示
...
TL;DR: 提出CEM方法，通过累积误差最小化优化现有误差校正方法的缓存策略，提升扩散变换器生成保真度


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)迭代去噪过程导致推理速度慢，现有缓存加速方法存在较大计算误差，且固定缓存策略无法适应去噪过程中复杂的误差变化

Method: 提出CEM插件，预定义误差表征模型对加速的敏感性，设计基于累积误差近似的动态规划算法优化缓存策略，实现误差最小化

Result: 在9个生成模型和量化方法上的实验表明，CEM显著提升现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α等模型上超越原始生成性能

Conclusion: CEM是模型无关的通用插件，可无缝集成到现有误差校正框架和量化模型中，无需额外计算开销，显著提升扩散变换器加速性能
...
Method: 提出YOLO-Master框架，引入实例条件自适应计算，核心是高效稀疏混合专家（ES-MoE）块，通过轻量级动态路由网络指导专家专业化训练，并自适应激活最相关专家

Result: 在MS COCO上达到42.4% AP，延迟1.62ms，比YOLOv13-N提升+0.8% mAP且推理速度快17.8%，在密集场景中表现尤为突出，同时保持典型输入的高效性和实时推理速度

Conclusion: YOLO-Master通过自适应计算有效解决了静态计算模型的资源分配问题，在保持实时性的同时显著提升了检测性能，特别是在复杂场景中表现优异
...
<details>
  <summary>Details</summary>
Motivation: 微手势识别和行为情感预测都是极具挑战性的任务，需要建模细微、细粒度的人类行为，主要利用视频和骨骼姿态数据。这些任务对于理解人类非语言沟通和情感状态具有重要意义。

Method: 提出了两个多模态框架：1) 对于微手势分类，使用MViTv2-S提取视频嵌入和2s-AGCN提取骨骼嵌入，通过跨模态令牌融合模块结合空间和姿态信息；2) 对于情感识别，使用SwinFace提取面部嵌入和MViTv2-S提取上下文嵌入，通过InterFusion模块融合情感表达和身体姿态信息。

Result: 在iMiGUE数据集上进行的实验表明，该方法在行为情感预测任务中表现出强大的性能和准确性，在MiGA 2025挑战赛中获得了第二名。

Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的信息（RGB、骨骼姿态、面部表情、上下文）来捕捉细微的人类行为模式，为理解非语言沟通和情感状态提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型为医学影像分析提供了新范式，但不同AI架构在临床诊断中的性能差异需要系统评估。本研究旨在比较专业开源模型MedGemma与专有大模型GPT-4在疾病诊断中的表现。

Method: 使用LoRA（低秩适应）方法对MedGemma-4b-it模型进行微调，与未调优的GPT-4进行对比，评估六种不同疾病的诊断能力。通过混淆矩阵和分类报告进行定量分析。

Result: 微调后的MedGemma-4b-it平均测试准确率达80.37%，显著高于GPT-4的69.58%。在癌症和肺炎检测等高风险任务中，MedGemma表现出更高的敏感性。定量分析提供了全面的模型性能洞察。

Conclusion: 领域特定的微调对于最小化临床实施中的幻觉至关重要。MedGemma展现出作为复杂、循证医学推理工具的潜力，在医学影像诊断中优于通用大模型。
...
Result: 开发了CADExpert开源基准，包含17,299个实例，包含正交投影、精确尺寸标注、专家生成的思维链过程、可执行的CADQuery代码和渲染的3D模型
...
TL;DR: 该论文从拓扑结构角度研究视觉表示学习，提出视觉理解需要语义语言，视觉空间具有纤维丛结构，语义对应商空间，需要非连续变换和特定架构支持拓扑变化。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑视角理解视觉表示学习，探索视觉理解的基本假设：视觉需要语义语言，许多感知观察对应少量离散语义状态。结合表示学习中的可迁移性和抽象性假设，研究视觉空间的数学结构。

Method: 采用拓扑数学框架，提出视觉观察空间具有纤维丛结构，其中冗余变化填充纤维，语义对应商基空间。从该结构推导两个理论结果：1) 语义商空间不是子流形，需要非连续变换；2) 近似商空间对模型架构有结构要求。

Result: 理论分析表明：1) 语义不变性需要非同胚的判别性目标（如标签监督、跨实例识别或多模态对齐）；2) 语义抽象需要支持拓扑变化的表示机制（"扩展-捕捉"过程）。

Conclusion: 该框架提供了与大规模判别性和多模态模型经验规律一致的解释性拓扑视角，强调这些结果是解释性的而非规定性的，为理解视觉表示学习提供了数学基础。
...
TL;DR: CountGD++ 是一个多模态开放世界计数模型，通过扩展提示方式（支持描述不计数对象、自动化视觉示例标注、接受自然/合成图像示例）显著提升了计数的灵活性、准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有计数方法在指定目标对象方面存在局限：视觉示例需要手动标注，无法指定不计数对象，限制了方法的灵活性和准确性。

Method: 1) 扩展提示功能，支持用文本和/或视觉示例描述不计数对象；2) 引入"伪示例"概念，在推理时自动标注视觉示例；3) 扩展计数模型以接受来自自然和合成外部图像的视觉示例；4) 将CountGD++作为LLM的视觉专家代理。

Result: 在多个数据集上实现了准确性、效率和泛化能力的显著提升，代码已开源。

Conclusion: 通过扩展目标对象的指定方式，CountGD++提升了多模态开放世界计数的提示灵活性，为更准确、高效的计数系统提供了新方法。
...
TL;DR: 提出SpatialMosaic数据集和SpatialMosaicVLM框架，通过多视角图像增强3D空间推理能力，解决真实场景中的部分可见性、遮挡和低重叠度等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在3D场景理解方面依赖预构建的3D表示或现成重建流程，限制了可扩展性和实际应用。真实环境中的部分可见性、遮挡和低重叠度等挑战尚未得到充分探索。

Method: 1) 提出可扩展的多视角数据生成和标注流程，构建包含200万QA对的SpatialMosaic指令调优数据集；2) 创建包含100万QA对、覆盖6个任务的SpatialMosaic-Bench基准测试；3) 提出SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到视觉语言模型中。

Result: 实验表明，提出的数据集和VQA任务能有效增强在挑战性多视角条件下的空间推理能力，验证了数据生成流程在构建真实多样QA对方面的有效性。

Conclusion: SpatialMosaic数据集和框架为多视角空间推理提供了有效的解决方案，特别是在真实世界的挑战性场景下，代码和数据集即将公开。
...
TL;DR: 提出MGCA-Net网络，通过上下文几何注意力模块和跨阶段多图共识模块，提升两视图对应关系学习中的几何建模能力和鲁棒性。
...
Method: 提出MGCA-Net网络，包含两个核心模块：1) 上下文几何注意力模块(CGA)，通过自适应注意力机制动态整合空间位置和特征信息；2) 跨阶段多图共识模块(CSMGC)，通过跨阶段稀疏图网络建立几何共识。

Result: 在YFCC100M和SUN3D数据集上的实验表明，MGCA-Net在异常值剔除和相机姿态估计任务上显著优于现有SOTA方法。

Conclusion: MGCA-Net通过创新的几何建模和跨阶段优化机制，有效提升了两视图对应关系学习的准确性和鲁棒性。
...
TL;DR: NeXT-IMDL是一个大规模诊断基准，通过四个基本轴（编辑模型、操作类型、内容语义、伪造粒度）和五个跨维度评估协议，系统性地揭示现有图像篡改检测模型在真实世界泛化场景中的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 用户友好的图像编辑模型普及和滥用风险增加，迫切需要通用且最新的图像篡改检测与定位方法。当前研究通常采用跨数据集评估，但这种简化方法掩盖了现有方法在处理多样化AI生成内容时的脆弱性，导致对进展的误导性印象。

Method: 提出NeXT-IMDL基准，将AIGC篡改操作沿四个基本轴分类：编辑模型、操作类型、内容语义、伪造粒度。基于此实现五个严格的跨维度评估协议，对11个代表性模型进行广泛实验。

Result: 实验揭示关键发现：虽然这些模型在原始设置下表现良好，但在模拟真实世界各种泛化场景的设计协议下，它们表现出系统性失败和显著性能下降。

Conclusion: 通过提供这个诊断工具包和新发现，旨在推动开发真正鲁棒的下一代图像篡改检测与定位模型，挑战当前评估方法的局限性。
...
Result: 首个14B规模系统实现亚秒级启动延迟（0.87秒）和32FPS实时吞吐率，为高保真交互式数字人合成设定了新标准

Conclusion: SoulX-LiveTalk通过创新的双向注意力保留和自校正机制，成功解决了实时音频驱动虚拟人生成中计算负载与延迟约束的矛盾，实现了高保真度与实时性能的平衡
...
<details>
  <summary>Details</summary>
Motivation: 3D牙齿实例分割面临拥挤牙弓、模糊的牙-龈边界、缺牙和临床重要的第三磨牙等挑战。现有3D方法存在边界泄漏、中心漂移和身份不一致问题，而2D基础模型（如SAM）虽提供强边界语义，但直接应用于3D临床工作流不切实际。

Method: 1. 点级残差门控模块：将咬合视图SAM嵌入注入3D点特征，细化牙-龈和牙间边界。2. 中心引导掩码细化：正则化实例掩码与几何质心的一致性，减少中心漂移。3. 顺序感知匈牙利匹配：整合解剖牙齿顺序和中心距离到相似度分配中，确保在缺牙或拥挤情况下的连贯标记。

Result: 在3DTeethSeg'22数据集上达到最先进的整体准确率和平均IoU，在涉及第三磨牙的病例上表现尤其突出，证明丰富的2D语义可在无需2D微调的情况下有效迁移到3D牙齿实例分割。

Conclusion: SOFTooth通过语义增强、顺序感知的2D-3D融合框架，成功解决了3D牙齿分割中的边界、中心和身份一致性问题，为临床工作流提供了实用解决方案。
...
TL;DR: 本文提出ArtQuant框架和RAD数据集，通过多维度结构化美学描述和LLM解码器解决AIGC美学质量评估中的数据稀缺、不平衡和模型碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 美学质量评估对AIGC的人类对齐定量评估系统至关重要，但现有方法面临两大挑战：1) 数据稀缺和不平衡，现有数据集过度关注视觉感知而忽略深层维度；2) 模型碎片化，现有视觉网络孤立美学属性，多模态方法难以有效处理长文本描述。

Method: 1) 提出RAD数据集：大规模(70k)、多维度结构化数据集，通过迭代流水线生成，无需昂贵人工标注；2) 提出ArtQuant框架：通过联合描述生成耦合孤立美学维度，利用LLM解码器更好建模长文本语义。

Result: 在多个数据集上达到最先进性能，仅需传统训练轮次的33%，缩小了艺术图像与美学判断之间的认知差距。理论分析证实RAD的语义充分性和生成范式共同最小化预测熵。

Conclusion: 通过RAD数据集和ArtQuant框架，有效解决了美学评估中的数据稀缺和模型碎片化问题，为AIGC的人类对齐美学评估提供了数学基础和实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型方法存在局限性：虽然采用看似统一的架构，但仍将世界预测和运动规划作为解耦的过程处理。这导致预测与规划之间缺乏内在一致性，无法充分发挥世界模型的潜力。
...
Result: 在视频预测任务上显著超越最佳工作：FID提升33.3%，FVD提升1.8%。在NAVSIM规划基准测试中创造了新记录，实现了最先进的性能。

Conclusion: DriveLaW通过统一视频生成和运动规划，实现了预测与规划的内在一致性，为自动驾驶世界模型提供了新的范式，在两个任务上都取得了突破性的性能提升。
...
TL;DR: DDSPO是一种新的扩散模型偏好优化方法，直接从获胜和失败策略中获取时间步级监督，无需人工标注数据，通过预训练参考模型自动生成偏好信号。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本-图像对齐和保持一致美学质量方面存在不足，而现有的偏好训练方法依赖昂贵且可能有噪声的人工标注数据集。

Method: 提出DDSPO方法，直接从获胜和失败策略中获取每个时间步的监督信号，避免依赖标注数据，使用预训练参考模型通过对比原始提示和语义降级变体的输出来自动生成偏好信号。

Result: DDSPO在文本-图像对齐和视觉质量方面均有提升，优于或匹配现有基于偏好的方法，同时需要更少的监督。
...
TL;DR: 该研究评估了不确定性量化能否提升SAM等分割基础模型在挑战性场景下的泛化能力，创建了UncertSAM基准测试，发现拉普拉斯近似方法能有效关联不确定性估计与分割错误。


<details>
  <summary>Details</summary>
Motivation: 虽然SAM等分割基础模型在零样本任务上表现良好，但在领域偏移或知识有限的情况下仍然脆弱。研究旨在探索不确定性量化能否以领域无关的方式缓解这些挑战并增强模型泛化能力。

Method: 1) 创建UncertSAM基准测试，包含8个数据集，涵盖阴影、透明、伪装等挑战性分割条件；2) 评估一系列轻量级、后处理的不确定性估计方法；3) 评估初步的不确定性引导预测细化步骤。

Result: 在评估的方法中，最后一层拉普拉斯近似产生的不确定性估计与分割错误有良好相关性，表明存在有意义的信号。细化步骤的益处尚属初步，但结果强调了将不确定性纳入分割模型以支持鲁棒、领域无关性能的潜力。

Conclusion: 不确定性量化有潜力增强分割模型在挑战性场景下的鲁棒性和泛化能力。研究提供了UncertSAM基准测试和代码，为后续研究奠定基础。
...
TL;DR: 提出基于天气条件和路面状况数据的实时道路监测系统，使用手机摄像头采集数据，测试多种深度学习算法进行道路分类，达到95%以上准确率，并提出结合模糊逻辑进行天气和时间分类。


<details>
  <summary>Details</summary>
Motivation: 传统道路监测方法昂贵且不系统，需要大量测量时间。需要开发实时、低成本的自动化道路状态监测系统，为车辆规划和主动控制系统提供有价值信息。

Method: 1. 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路采集数据；2. 测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类；3. 将道路加速度数据转换为图像与摄像头图像结合训练；4. 比较加速度基和摄像头图像基方法性能；5. 提出使用模糊逻辑结合加速度或摄像头图像进行天气和时间分类。

Result: 1. 在5类道路条件分类（沥青、损坏沥青、碎石路、损坏碎石路、铺面路）中达到超过95%的准确率；2. 比较了不同深度学习算法的性能；3. 成功将加速度数据转换为图像进行训练。

Conclusion: 提出的实时系统能够有效监测道路表面状态，深度学习算法在道路分类中表现优异，结合模糊逻辑可进一步根据天气和时间进行分类，为车辆控制系统提供实时道路信息。
...
TL;DR: RealX3D是一个真实捕获的多视角视觉修复和3D重建基准测试集，包含多种物理退化类型，用于评估方法在真实世界挑战性环境中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多视角重建方法在真实世界的物理退化条件下表现脆弱，缺乏一个系统性的基准测试集来评估方法在光照、散射、遮挡和模糊等多种物理退化下的性能。

Method: 通过统一的采集协议捕获像素对齐的低质量/高质量视角图像，将退化分为四类（光照、散射、遮挡、模糊），每个场景包含高分辨率图像、RAW图像和密集激光扫描，从中导出世界尺度网格和度量深度。

Result: 对基于优化的和前馈方法进行广泛测试，结果显示在物理退化条件下重建质量显著下降，突显了当前多视角流程在真实世界挑战性环境中的脆弱性。

Conclusion: RealX3D基准测试集揭示了当前多视角重建方法在真实物理退化条件下的局限性，为开发更鲁棒的视觉修复和3D重建方法提供了重要评估工具。
...
### [154] [CoFi-Dec: Hallucination-Resistant Decoding via Coarse-to-Fine Generative Feedback in Large Vision-Language Models](https://arxiv.org/abs/2512.23453)
*Zongsheng Cao,Yangfan He,Anran Liu,Jun Xie,Feng Chen,Zepeng Wang*

Main category: cs.CV

TL;DR: CoFi-Dec是一个无需训练的解码框架，通过生成自反馈和粗到细视觉条件来减少大视觉语言模型的幻觉问题，使用Wasserstein融合机制整合多级视觉假设。
...
Method: CoFi-Dec采用训练自由的解码框架，首先基于原始图像的粗粒度和细粒度视图生成两个中间文本响应，然后使用文生图模型将这些响应转换为合成图像，形成多级视觉假设。通过Wasserstein融合机制将这些预测分布对齐到几何一致的解码轨迹中。

Result: 在六个幻觉基准测试上的广泛实验表明，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略。该框架是模型无关的，无需额外训练，可广泛应用于各种大视觉语言模型。

Conclusion: CoFi-Dec通过整合生成自反馈和粗到细视觉条件，结合Wasserstein融合机制，为大视觉语言模型提供了一种有效减少幻觉的训练自由解决方案，提高了输出的可靠性和忠实度。
...
TL;DR: 提出结合视觉水线检测、YOLOv8姿态尺度提取和大规模多模态语言模型的混合框架，实现自动化河流水位计读数，在最优图像条件下达到5.43厘米的平均绝对误差。
...
Method: 采用混合框架：1) 基于视觉的水线检测；2) YOLOv8姿态尺度提取；3) 大语言模型（GPT-4o和Gemini 2.0 Flash）进行自动化读数。流程包括图像预处理、标注、水线检测、尺度间隙估计和数值读取提取。

Result: 水线检测精度达94.24%，F1分数83.64%；尺度间隙检测为后续读数提供准确几何校准；结合尺度间隙元数据显著提升LLMs性能，Gemini Stage 2在最优图像条件下达到MAE 5.43cm、RMSE 8.58cm、R² 0.84的最佳结果。

Conclusion: 该方法为自动化水文监测提供了可扩展、高效可靠的解决方案，展示了实时河流水位计数字化和改善水资源管理的潜力，强调了结合几何元数据与多模态AI对鲁棒水位估计的重要性。
...
TL;DR: 提出Dual-approx Bridge模型，利用布朗桥动力学和双神经网络逼近器，实现高保真、低方差的确定性图像到图像转换


<details>
  <summary>Details</summary>
Motivation: 确定性I2I转换需要保证输入到输出的稳定一致映射，且输出应高度忠实于真实图像。现有方法在保持高保真度和低方差方面仍有改进空间。

Method: 提出去噪布朗桥模型，采用双逼近器架构：一个用于前向过程，一个用于反向过程。利用布朗桥动力学特性，确保生成过程具有可预测性和一致性。

Result: 在图像生成和超分辨率等基准数据集上，Dual-approx Bridge在图像质量和忠实度方面均优于随机性和确定性基线方法，表现出稳定且优异的性能。

Conclusion: Dual-approx Bridge通过布朗桥动力学和双逼近器设计，实现了高保真、低方差的确定性I2I转换，为图像翻译任务提供了新的有效解决方案。
...
Method: 提出多领域上下文集成网络（MCI-Net），包括：1）图邻域聚合模块，构建全局图捕捉点云整体结构关系；2）渐进上下文交互模块，通过域内特征解耦和域间上下文交互增强特征判别性；3）动态内点选择方法，利用多轮位姿估计的残差信息优化内点权重。
...
TL;DR: SC-Net：一种通过空间和通道双边上下文集成来改进两视图对应学习的CNN网络，包含自适应聚焦正则化、双边场调整和位置感知恢复模块，在姿态估计和离群点去除任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统CNN骨干网络在双视图对应学习中存在两个问题：1）未能有效聚合全局上下文；2）在大视差场景中会过度平滑密集运动场。需要设计专门针对该任务的网络结构来解决这些问题。

Method: 提出SC-Net网络，包含三个核心模块：自适应聚焦正则化模块（AFR）增强位置感知和抗伪运动样本鲁棒性；双边场调整模块（BFA）通过同时建模长程关系和促进空间与通道维度交互来细化运动场；位置感知恢复模块（PAR）从细化场中恢复运动向量，确保一致性和精度。

Result: 在YFCC100M和SUN3D数据集上的广泛实验表明，SC-Net在相对姿态估计和离群点去除任务上优于现有最先进方法。

Conclusion: SC-Net通过有效集成空间和通道的双边上下文，解决了传统CNN在双视图对应学习中的局限性，显著提升了运动场估计的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视频语言模型在处理长视频时存在两个主要问题：时间窗口狭窄，无法捕捉长时间跨度的语义变化；主流文本检索方法仅依赖表层词汇重叠，忽略了视觉、音频和字幕通道之间的时间相互依赖性。

Method: 提出TV-RAG框架，包含两个核心机制：1) 时间衰减检索模块，将显式时间偏移注入相似度计算，使文本查询能根据真实的多媒体上下文进行排序；2) 熵加权关键帧采样器，选择均匀间隔、信息密集的帧，减少冗余同时保持代表性。

Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中持续超越大多数领先基线，证明了其有效性。该框架无需重新训练或微调，可轻松集成到任何大型视频语言模型中。

Conclusion: TV-RAG通过结合时间和语义信号，实现了双层次推理机制，为长视频理解提供了轻量级、经济高效的升级路径，解决了现有模型在处理长视频时的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前上下文建模方法主要关注基本几何关系或局部特征，忽略了物体间的跨尺度上下文交互，限制了复杂场景理解能力。

Method: 提出深度全景上下文聚合网络(PanCAN)，在高维希尔伯特空间中通过跨尺度特征聚合分层整合多阶几何上下文，结合随机游走和注意力机制学习每个尺度的多阶邻域关系，并通过注意力动态融合不同尺度的显著锚点特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型虽然能从文本生成故事，但在人类中心故事生成中面临额外挑战：需要保持详细多样的人脸一致性，以及在不同图像中协调多个角色。这些问题限制了生成人类中心故事的质量和实用性。

Method: 框架包含两个关键组件：1) 迭代身份发现：提取连贯的角色身份特征；2) 重去噪身份注入：通过重去噪过程将身份特征注入图像，同时保持所需上下文。该方法驯服了身份保持生成器，确保角色一致性。

Result: 在ConsiStory-Human基准测试中，IdentityStory超越了现有方法，特别是在人脸一致性方面表现突出。框架还支持多角色组合，并展示了在无限长度故事生成和动态角色组合等应用中的强大潜力。

Conclusion: IdentityStory为解决人类中心故事生成中的角色一致性挑战提供了有效框架，通过创新的身份发现和注入技术，在多角色协调和人脸一致性方面取得了显著进展，为故事生成应用开辟了新可能性。
...
TL;DR: IAFS：一种无需训练的频率感知迭代扩散推理框架，通过自适应频率引导平衡超分辨率中的感知质量与结构保真度


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像超分辨率中难以同时保证高频感知质量和低频结构保真度，推理时缩放策略存在感知过平滑或结构一致性丢失的问题

Method: 提出IAFS框架，结合迭代精炼和频率感知粒子融合，通过迭代修正结构偏差逐步优化生成图像，并自适应融合高频感知线索与低频结构信息

Result: 在多个基于扩散的超分辨率模型上的实验表明，IAFS有效解决了感知-保真度冲突，在感知细节和结构准确性方面均优于现有推理时缩放方法

Conclusion: IAFS通过训练自由的迭代扩散推理框架，成功平衡了超分辨率中的感知质量与结构保真度，为扩散模型的推理优化提供了有效解决方案
...
Conclusion: AnyMS通过创新的注意力解耦机制，有效解决了多主体定制中的平衡问题，无需训练即可实现高质量的布局引导多主体图像合成。
...
TL;DR: PathFound：首个支持证据寻求推理的病理学智能体模型，通过主动信息获取和诊断优化，模拟临床诊断工作流程，显著提升诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有病理学基础模型大多采用静态推理范式，一次性处理全切片图像生成预测，缺乏在模糊诊断情况下的重新评估和针对性证据获取。这与临床诊断工作流程形成鲜明对比，临床诊断通过重复观察切片和进一步检查请求来完善假设。

Method: PathFound整合病理视觉基础模型、视觉语言模型和强化学习训练推理模型，通过三个阶段进行主动信息获取和诊断优化：初始诊断、证据寻求和最终决策。模型采用智能体架构支持证据寻求推理。

Result: 采用证据寻求策略在所有测试的大型多模态模型中均能一致提升诊断准确性。PathFound在多样化临床场景中达到最先进的诊断性能，并展现出发现细微细节（如核特征和局部浸润）的强大潜力。

Conclusion: 证据寻求工作流程在计算病理学中具有显著有效性，PathFound作为首个支持证据寻求推理的病理学智能体模型，通过模拟临床诊断的动态过程，为病理诊断提供了更准确、更可靠的解决方案。
...
Method: 提出双阶段提示净化策略：1) 通过计算互补语义距离评估每个token的安全性，无需关键词匹配或重新训练；2) 对风险提示应用双空间变换：将有毒对齐嵌入投影到有毒概念矩阵的零空间以移除有害语义，同时对齐到干净概念的范围空间以增强安全语义。采用token级策略选择性替换风险token嵌入。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面超越了现有方法，并能与依赖训练的方法竞争。该方法具有理论依据，对未见过的提示和模型有强泛化能力。

Conclusion: PurifyGen提供了一种即插即用的安全文本到图像生成解决方案，无需重新训练模型，在保持原始意图和连贯性的同时有效净化风险提示，为扩散模型的安全应用提供了新途径。
...
TL;DR: RxnBench是一个评估多模态大语言模型在化学文献中理解反应能力的基准测试，包含单图问答和全文问答两个任务，揭示了当前模型在化学逻辑和结构识别方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在化学领域有革命性潜力，但它们在理解真实科学文献中密集、图形化的反应语言方面的能力尚未得到充分探索。需要系统评估模型从PDF文档中理解化学反应的能力。

Method: 开发了RxnBench多层级基准测试，包含两个任务：1）单图问答：从305个精选反应方案中提取1,525个问题，测试精细视觉感知和机理推理；2）全文问答：从108篇文章中测试模型整合文本、反应方案和表格信息的能力。

Result: 评估发现关键能力差距：模型擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但没有任何模型在全文问答上达到50%准确率。

Conclusion: 研究强调了开发领域特定视觉编码器和更强推理引擎的紧迫需求，以推进自主AI化学家的发展。当前多模态大语言模型在化学反应理解方面仍有重大局限。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的思维链推理在理解任务中表现出色，但在生成任务中应用有限，主要受限于场景特定的机制，缺乏泛化性和适应性。

Method: 采用解耦架构：预训练MLLM生成定制化指令，DiT根据指令生成图像。提出可分离GRPO训练范式，交替对MLLM和DiT进行强化学习，支持跨数据集联合训练。

Result: 在多个生成基准测试中实现了稳健、最先进的性能表现。

Conclusion: ThinkGen成功将思维链推理扩展到视觉生成任务，通过灵活的架构设计实现了多场景下的高质量图像生成，为生成任务提供了系统化解决方案。
...
TL;DR: 提出Haar-tSVD方法，结合Haar变换和张量奇异值分解进行图像去噪，无需学习局部基，平衡速度与性能，并引入自适应噪声估计和深度网络集成。


<details>
  <summary>Details</summary>
Motivation: 随着成像设备普及和每日产生海量图像数据，对高效有效的图像去噪需求日益增长，需要平衡去噪速度与性能的方法。

Method: 建立PCA与Haar变换在循环表示下的理论联系，提出Haar-tSVD方法：统一t-SVD投影结合Haar变换捕捉全局和局部块相关性；引入自适应噪声估计方案；基于Haar-PCA关系集成深度神经网络。

Result: 在多种去噪数据集上的实验结果表明，该方法在噪声去除方面具有高效性和有效性，代码已公开。

Conclusion: Haar-tSVD作为一步式、可并行化的即插即用去噪器，无需学习局部基，在去噪速度和性能之间取得了良好平衡，并通过深度网络集成进一步提升了严重噪声条件下的性能。
...
Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法进行标注；2) 通过强化学习训练视觉语言基础模型；3) 引入分布外安全类别推理任务，并添加基于同义词库的相似性奖励，鼓励模型为未见过的风险类别生成简洁描述。

Result: ProGuard在二元安全分类上达到与闭源大模型相当的性能，在非安全内容分类上大幅超越现有开源防护模型。在主动防护能力方面，分布外风险检测提升52.6%，分布外风险描述提升64.8%。
...
TL;DR: 提出LiveTalk系统，通过改进蒸馏方法实现实时多模态交互视频生成，将推理成本降低20倍，延迟从1-2分钟降至实时


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过双向注意力的迭代去噪过程无法实现实时交互，且现有蒸馏方法主要针对文本到视频生成，在多模态条件下存在视觉伪影和质量下降问题

Method: 改进蒸馏配方，重点关注条件输入质量以及在线策略优化的初始化和调度，结合音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks构建LiveTalk系统

Result: 在HDTF、AVSpeech和CelebV-HQ基准测试中，蒸馏模型以20倍更低的推理成本和延迟匹配或超越全步双向基线的视觉质量；LiveTalk在多轮交互基准中优于Sora2、Veo3等最先进模型

Conclusion: 通过改进蒸馏方法实现了实时多模态交互视频生成，显著降低了延迟和推理成本，使无缝人机多模态交互成为可能
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在细粒度感知方面存在不足，训练语料库过于强调一般识别而忽略细微视觉差异，导致模型存在视觉偏见且错过细节信息。

Method: 构建TWIN数据集，包含56.1万张图像对查询，要求模型判断两个视觉上相似的图像是否描绘同一物体；同时创建FGVQA基准测试套件，包含1.2万查询，用于评估细粒度识别能力。

Result: 在TWIN上微调的视觉语言模型在FGVQA基准上提升高达19.3%，在艺术、动物、植物、地标等未见领域表现良好，且不影响通用VQA基准性能；数据集规模对性能提升至关重要。
...
TL;DR: 提出三種方法改善紅外夜視相機的火災偵測：額外NIR資料集、兩階段偵測模型(YOLOv11+EfficientNetV2-B0)、Patched-YOLO，提升夜間火災偵測準確率並減少人工光源誤判。


<details>
  <summary>Details</summary>
Motivation: 紅外夜視相機的火災偵測準確性仍有挑戰，現有模型雖有表現但存在限制，特別是資料集建構問題導致人工光源常被誤判為火災。

Method: 1) 建立額外NIR資料集並應用多種資料增強策略；2) 提出兩階段偵測管道：YOLOv11進行初步偵測，EfficientNetV2-B0進行分類驗證；3) 提出Patched-YOLO，透過基於區塊的處理提升RGB影像中遠距離小物體的偵測能力。

Result: 相較於先前方法(YOLOv7 mAP50-95 0.51、RT-DETR 0.65、YOLOv9 0.598)，提出的方法在夜間火災偵測上達到更高的偵測準確率，特別在減少人工光源誤判方面有顯著改善。

Conclusion: 透過資料增強、兩階段偵測架構和區塊處理技術，能有效提升紅外夜視相機的火災偵測準確性，特別是在夜間環境和減少誤判方面。
...
Method: 提出可扩展残差特征聚合(SRFA)框架：1)预处理和MAGRes-UNet分割；2)DenseNet-121残差特征存储提取深度层次特征；3)HHO-BA混合元启发式特征选择；4)ViT与EfficientNet-B3混合模型分类；5)SSA和GWO双重优化机制调参。

Result: 实验结果显示显著性能提升：准确率96.23%，F1分数95.58%，特异性94.83%，显著优于传统CNN和当代基于Transformer的模型。

Conclusion: SRFA框架作为胰腺肿瘤早期检测的有用工具具有巨大潜力，其多阶段处理和优化机制有效解决了胰腺肿瘤检测中的复杂挑战。
...
TL;DR: 本文提出了一个评估框架来量化3D生成模型中的记忆效应，并通过实验发现记忆程度受数据模态、多样性、条件粒度以及模型设计（如引导尺度、向量集长度）等因素影响，同时提出了减轻记忆效应的简单有效策略。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型在3D视觉中用于合成新形状，需要了解它们是否依赖记忆训练数据，这有助于防止训练数据泄露并提高生成结果的多样性。

Method: 设计了一个评估框架来量化3D生成模型中的记忆效应，首先应用于现有方法，然后通过基于潜在向量集（Vecset）扩散模型的受控实验，分析数据和建模设计对记忆的影响。

Result: 发现记忆程度取决于数据模态，并随数据多样性和更细粒度的条件而增加；在建模方面，记忆在中等引导尺度达到峰值，可通过更长的向量集和简单的旋转增强来缓解。

Conclusion: 该框架和分析提供了对3D生成模型记忆效应的实证理解，并提出了在不降低生成质量的情况下减少记忆的简单有效策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖注意力机制进行跨帧对象对齐，使用统一的显式物理模型（如恒定速度），偏好语义特征进行隐式对齐，但不同类别和帧间的运动状态和对象特征变化使这种对齐效果不佳。

Method: HAT首先利用多个显式运动模型为历史实例生成空间锚点和运动感知特征提案，然后通过结合缓存对象查询中嵌入的语义和运动线索进行多假设解码，最终为目标帧提供最优对齐提案。

Result: 在nuScenes数据集上，HAT持续改进3D时序检测器和跟踪器，与DETR3D检测器配对时在测试集上达到46.0% AMOTA的SOTA跟踪结果。在端到端AD方法中提升感知精度（+1.3% mAP, +3.1% AMOTA）并降低32%碰撞率。在语义受损情况下（nuScenes-C），HAT的运动建模使端到端AD的感知和规划更加鲁棒。

Conclusion: HAT通过自适应多假设解码机制有效解决了时空对齐问题，显著提升了自动驾驶端到端感知系统的性能，特别是在语义信息受损时仍能保持鲁棒性，证明了显式运动建模在时序感知中的重要性。
...
TL;DR: OmniAgent是一个完全音频引导的主动感知代理，通过动态编排专用工具实现细粒度音视频推理，在三个基准测试中超越现有模型10-20%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型在统一音频和视觉模态方面取得进展，但缺乏细粒度跨模态理解能力，难以实现多模态对齐。传统方法依赖僵化的静态工作流程和密集帧标注，限制了音视频推理的精度。

Method: 提出从被动响应生成到主动多模态查询的范式转变。采用动态规划自主按需编排工具调用，战略性地将感知注意力集中在任务相关线索上。核心是新颖的粗到细音频引导感知范式，利用音频线索定位时间事件并指导后续推理。

Result: 在三个音视频理解基准测试上进行广泛实证评估，OmniAgent实现了最先进的性能，显著超越领先的开源和专有模型10-20%的准确率。

Conclusion: OmniAgent通过动态工具编排和音频引导的主动感知，实现了更细粒度的音视频推理，代表了从被动响应到主动多模态查询的重要范式转变。
...
TL;DR: IDT是一种前馈Transformer框架，用于多视角本征图像分解，通过注意力机制联合处理多视角图像，实现视角一致的本征分解，无需迭代生成采样。


<details>
  <summary>Details</summary>
Motivation: RGB图像将材质属性、光照和视角依赖效应纠缠在一起，而现有的扩散方法在单视角本征分解上表现良好，但扩展到多视角时面临视角不一致的挑战。

Method: IDT采用基于Transformer的注意力机制联合推理多视角输入图像，使用物理基础的图像形成模型，将图像分解为漫反射率、漫反射着色和镜面反射着色三个本征因子。

Result: 在合成和真实数据集上的实验表明，IDT能获得更干净的漫反射率、更一致的漫反射着色、更好分离的镜面反射成分，相比先前方法显著提高了多视角一致性。

Conclusion: IDT通过Transformer架构和物理基础分解模型，成功解决了多视角本征图像分解中的视角一致性问题，实现了可解释和可控的材质与光照效应分解。
...
<details>
  <summary>Details</summary>
Motivation: 透明物体（折射、反射、透射）破坏传统深度感知假设，导致空洞和时间不稳定估计。观察到现代视频扩散模型已能合成逼真的透明现象，表明它们已内化光学规则

Method: 1) 构建TransPhy3D合成视频数据集（11k序列，Blender/Cycles渲染，RGB+深度+法线）；2) 从大型视频扩散模型出发，通过轻量级LoRA适配器学习视频到视频的深度（和法线）转换器；3) 在DiT骨干中连接RGB和（噪声）深度潜在表示，在TransPhy3D和现有帧级合成数据集上共同训练

Result: DKT模型在透明相关基准测试（ClearPose、DREDS、TransPhy3D-Test）上实现零样本SOTA；提升准确性和时间一致性；1.3B紧凑版本运行速度约0.17秒/帧；集成到抓取系统中，在透明、反射和漫反射表面上提高成功率

Conclusion: 扩散模型"知道"透明度，生成式视频先验可高效、无标签地重新用于实现鲁棒、时间一致的真实世界操作感知
...
TL;DR: Stream-DiffVSR是一种因果条件扩散框架，用于高效在线视频超分辨率，通过四步蒸馏去噪器、自回归时间引导模块和轻量级时间感知解码器，在保持感知质量的同时大幅降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频超分辨率方法虽然感知质量好，但依赖未来帧和昂贵的多步去噪，在延迟敏感场景中不实用。需要一种适合低延迟在线部署的扩散VSR方法。
...
Conclusion: Stream-DiffVSR实现了扩散VSR中最低的延迟，是第一个适合低延迟在线部署的扩散VSR方法，在保持高质量的同时实现了实时处理能力。
...
TL;DR: Moxin 7B是一个完全开源的大语言模型，遵循模型开放框架，提供完整的训练、数据集和实现细节透明度。基于Moxin开发了三个变体：Moxin-VLM（视觉语言）、Moxin-VLA（视觉语言动作）和Moxin-Chinese（中文能力），在各种评估中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM领域由GPT-4等专有模型主导，而开源模型如LLaMA和Mistral虽然促进了LLM的普及，但通常只分享模型权重。作者旨在创建一个完全透明的开源LLM，超越简单的权重分享，提供完整的训练、数据集和实现细节，以促进更包容和协作的研究环境。

Method: 提出Moxin 7B作为完全开源的LLM，遵循模型开放框架。基于Moxin开发了三个变体：Moxin-VLM针对视觉语言任务，Moxin-VLA针对视觉语言动作任务，Moxin-Chinese针对中文能力。使用开源框架和开放数据进行训练。

Result: 实验表明，Moxin及其变体在各种评估中取得了优异的性能。作者发布了模型、可用数据以及推导这些模型的代码。

Conclusion: Moxin 7B通过完全透明的开源方法，超越了传统的权重分享模式，为健康开源生态系统提供了可持续的解决方案。其变体模型在多个任务领域展现了强大的能力，促进了更包容和协作的AI研究环境。
...
TL;DR: 研究发现Transformer语言模型的句子嵌入空间编码了与人类可解释的认知属性对齐的分层几何结构，这种结构可以通过浅层非线性探针可靠地解码。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明Transformer语言模型在嵌入空间中学习到丰富的几何结构，但更高层次的认知组织在这些表示中的存在仍未得到充分探索。本研究旨在探究句子嵌入是否编码了与人类可解释的认知或心理属性对齐的分级、分层结构。
...
Result: 在所有模型中，连续分数和层级标签都能可靠解码，浅层非线性探针比线性探针提供一致的性能提升。词频-逆文档频率基线表现明显更差，表明观察到的结构不能仅归因于表面词汇统计。非参数置换测试确认探针性能超过随机水平。定性分析显示嵌入空间中存在平滑的低到高梯度，并且主要是相邻层级的混淆。
...
TL;DR: SmartSnap提出了一种从被动事后验证转向主动实时自验证的新范式，让智能体在完成任务的同时主动收集精简证据，显著提升了GUI任务中强化学习的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有任务验证方法（如规则脚本、奖励模型、LLM-as-a-Judge）都是被动的事后处理，需要分析整个冗长的交互轨迹，导致成本高昂、可靠性低，严重制约了智能体强化学习的可扩展性。

Method: 提出Self-Verifying Agent，具有双重使命：完成任务并主动收集精简证据。基于3C原则（完整性、简洁性、创造性），智能体利用在线环境访问权限，在最小决定性快照集上进行自验证，然后将这些证据提供给通用的LLM-as-a-Judge进行验证。

Result: 在移动任务实验中，SmartSnap范式显著提升了LLM驱动智能体的训练可扩展性，使8B和30B模型的性能分别提升了26.08%和16.66%。自验证智能体在性能上可与DeepSeek V3.1和Qwen3-235B-A22B竞争。

Conclusion: SmartSnap通过将解决方案寻找与证据寻求相结合，实现了从被动验证到主动自验证的范式转变，显著降低了验证成本，提高了可靠性，为复杂GUI任务中的自主智能体发展提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究也门伊比阿拉伯语中qulk-从句的句法特性，这种结构是形态融合形式，意为"我说"，引入嵌入的陈述、疑问和祈使从句，通常不带补语化成分。探索这种特殊结构在极简方案框架下的理论意义。

Method: 在极简方案框架下应用核心操作（合并、移动、一致、拼读），提供qulk-从句的分层句法分析，展示其推导如何通过标准计算步骤和后句法过程（如形态合并）进行。

Result: 提出qulk-从句是双从句结构，qulk作为从句嵌入谓词选择完整CP补语。分析解释了方言特有特征如二分否定、附着化和CP嵌入。研究为生成句法特别是极简方案提供了理论贡献。

Conclusion: 研究提出了将分析扩展到受话者从句kil-k'你说'的理论问题，并为极简方案的普遍性可能性提供了见解。
...
TL;DR: FAA是一种基于傅里叶特征的参数高效微调框架，通过分解表征为高低频分量实现频率感知的语义调制，在保持主干网络冻结的同时实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法通常缺乏对语义信息频率特性的考虑，无法有效利用不同频率分量在语言理解中的互补作用，限制了微调效果。

Method: 将随机傅里叶特征集成到轻量级适配器模块中，将中间表征分解为互补的低频和高频分量，通过频率感知激活和自适应加权机制选择性强调信息丰富的频带。

Result: 在GLUE、E2E NLG和指令微调基准测试中，FAA相比现有参数高效微调方法取得竞争性或更优性能，同时保持较低的计算和内存开销。

Conclusion: FAA通过频率感知的适配器设计，为大型预训练语言模型提供了一种鲁棒且高效的微调方法，验证了频率分解在参数高效微调中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的人类活动识别方法存在两个主要局限：1）依赖大量标注数据；2）仅基于几何特征的范例选择难以区分相似活动（如行走、上楼梯、下楼梯）。需要结合语义理解来改进范例选择。

Method: 提出LLM引导的范例选择框架：1）使用LLM生成知识先验，捕捉特征重要性、类间混淆性和范例预算乘数；2）结合基于边界的验证线索、PageRank中心性、hubness惩罚和设施选址优化；3）综合语义先验与结构几何线索进行范例评分和选择。

Result: 在UCI-HAR数据集上进行严格少样本条件评估，获得88.78%的宏F1分数，显著优于随机采样、herding和k-center等传统方法。

Conclusion: LLM衍生的语义先验与结构和几何线索相结合，为少样本可穿戴传感器活动识别提供了更强的范例选择基础，证明了语义推理在改进传感器数据范例选择中的价值。
...
TL;DR: 本文提出HHEM轻量级幻觉检测框架，显著降低计算成本（从8小时到10分钟），在QA任务上达到82.2%准确率，但总结任务中局部幻觉检测仍有挑战，通过分段检索改进。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型幻觉问题严重，现有评估方法（如KnowHalu）计算成本高，需要更高效的检测框架来平衡计算效率与检测准确性。

Method: 提出HHEM轻量级分类框架，不依赖LLM判断；采用分段检索处理总结任务中的局部幻觉；通过TPR、TNR、准确率等指标评估不同LLM；使用CDF分析模型大小与幻觉关系。

Result: HHEM将评估时间从8小时降至10分钟；HHEM+非捏造检查在QA任务上达到82.2%准确率和78.9%TPR；大模型（7B-9B参数）幻觉较少，中等模型不稳定；分段检索改善了总结任务检测。

Conclusion: 需要结构化评估框架平衡计算效率与事实验证；HHEM是有效的轻量级解决方案，但需进一步改进局部幻觉检测；模型大小影响幻觉频率，中等模型需要特别关注。
...
<details>
  <summary>Details</summary>
Motivation: 开放域检索增强生成面临两个主要挑战：检索文档中包含大量无关信息，以及生成的答案与用户意图对齐不佳。需要更有效的过滤和推理机制来提升RAG系统的性能。

Method: 采用分层过滤的多阶段管道：1) 使用Gemini 2.5 Flash进行查询重构和内容过滤，2) 使用Gemini 2.5 Pro进行最终答案生成。系统结合了快速廉价模型和强大推理模型的优势，实现成本效益和性能的平衡。

Result: 在MMU-RAGent验证集上，ROUGE-L提升19.6%至0.274，DeBERTaScore提升6.2%至0.677。在测试集Test2025（评估2025年1月后知识的问题）上，ROUGE-L提升57.4%，DeBERTaScore提升14.9%，显著优于参数化基线。

Conclusion: HiFi-RAG通过分层过滤策略有效解决了RAG中的无关信息问题，结合不同规模语言模型的优势，在开放域问答任务中取得了显著性能提升，为实际应用提供了成本效益高的解决方案。
...
TL;DR: 该研究提出垂直领域会计推理概念，建立评估标准，测试多个LLM模型在会计推理任务上的表现，发现GPT-4表现最佳但仍需优化才能满足实际应用需求。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在各领域的广泛应用，如何将其有效整合到会计领域成为关键挑战。需要理解LLM在会计领域的专业推理能力，以促进企业数字化转型和社会发展。

Method: 提出垂直领域会计推理概念，通过分析GLM系列模型的训练数据特征建立评估标准。基于此框架评估GLM-6B、GLM-130B、GLM-4和GPT-4等多个模型在会计推理任务上的表现，测试不同提示工程策略的效果。

Result: 实验结果显示不同提示工程策略对模型性能有不同程度提升，GPT-4展现出最强的会计推理能力。但当前LLM仍无法满足实际应用需求，特别是在企业级会计场景中需要进一步优化。

Conclusion: LLM在会计领域具有潜力但尚未达到实际应用标准。需要进一步优化模型以适应企业级会计场景，才能充分发挥LLM在该领域的价值。
...
TL;DR: 本文主张韩语选区树库应采用以"어절"（eojeol）为基本单位的选区表示法，而非以语素为单位，以解决形态与句法混淆问题，并实现不同树库间的可比性。


<details>
  <summary>Details</summary>
Motivation: 韩语选区树库设计面临一个根本性问题：选择何种基本单位作为选区终端。虽然韩语单词具有复杂的形态结构，但将语素作为选区终端会混淆词内形态和短语层面的句法结构，同时导致与基于"어절"（eojeol）的依存资源不匹配。

Method: 提出基于"어절"的选区表示法，将形态分割和细粒度词性信息编码在独立的非选区层中。通过比较分析，在明确的规范化假设下，证明世宗树库和宾州韩语树库在基于"어절"的选区层面具有表征等价性。

Result: 分析表明，基于"어절"的选区表示法能够保持可解释的选区结构，支持跨树库比较，并促进选区与依存结构的转换。

Conclusion: 韩语选区树库应采用基于"어절"的表示方案，将形态信息分离到独立层中，这有助于解决现有树库间的兼容性问题，并为选区-依存转换提供基础。
...
TL;DR: ManchuTTS：针对濒危语言满语的语音合成系统，通过三层次文本表示和跨模态分层注意力机制处理粘着语特性，在低资源条件下实现高质量合成。


<details>
  <summary>Details</summary>
Motivation: 满语作为濒危语言面临数据稀缺和强粘着性的挑战，现有语音合成方法难以处理其独特的语言特性，需要专门设计解决方案。

Method: 1) 设计三层次文本表示（音素、音节、韵律）；2) 跨模态分层注意力机制实现多粒度对齐；3) 深度卷积网络与流匹配Transformer结合的非自回归生成；4) 分层对比损失引导声学-语言对应；5) 构建首个满语TTS数据集并采用数据增强策略。

Result: 在5.2小时训练子集上获得MOS 4.52分，显著优于所有基线模型。消融实验显示分层指导将粘着词发音准确率提升31%，韵律自然度提升27%。

Conclusion: ManchuTTS成功解决了满语语音合成的独特挑战，为濒危语言的语音合成提供了有效框架，特别是在处理粘着语特性和低资源条件方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 受人类阅读习惯启发——通常只关注当前页，必要时才回顾前文。当前LLM的全局注意力计算成本高，但实际可能不需要对所有token都进行全局关注

Method: 提出All-or-Here Attention (AHA)，每个注意力头配备二元路由器，为每个token动态切换：要么使用全局注意力，要么使用局部滑动窗口注意力

Result: 使用256个token的窗口大小时，可替换93%的原始全局注意力操作而不损失性能。分析发现上下文依赖呈长尾分布，随着局部窗口扩大，对全局注意力的需求迅速衰减

Conclusion: 全局注意力大部分是冗余的，高效的推理只需要按需访问全局上下文。AHA通过解耦局部处理和全局访问，揭示了这一关键洞察
...
TL;DR: 该论文提出了一种基于LLM的结构化提示管道和集成方法，用于多模态对话中的情感分析任务，在MCABSA挑战赛中取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 多模态对话中的情感分析是构建情感智能AI系统的关键挑战，MCABSA挑战赛提出了两个具有挑战性的子任务：情感六元组提取和情感翻转检测。

Method: 针对子任务一，设计了结构化提示管道，引导LLM按顺序提取情感组件；针对子任务二，通过集成三个LLM的优势来识别情感转换及其触发因素。

Result: 系统在子任务一上获得47.38%的平均分，在子任务二上获得74.12%的精确匹配F1分数，证明了逐步细化和集成策略的有效性。

Conclusion: 研究表明，结构化提示管道和LLM集成策略能够有效处理丰富的多模态情感分析任务，为情感智能AI系统的发展提供了实用方法。
...
TL;DR: T3LLM：一个用于时间序列问答的三LLM框架，通过工作-评审-学习机制实现多步推理和显式纠错，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的时间序列分析方法主要采用通用NLP技术，在处理复杂数值序列时容易产生推理错误。时间序列数据具有内在可验证性，可以在推理步骤和原始输入之间进行一致性检查，这启发了显式纠错机制的设计。

Method: 提出T3LLM框架，包含三个LLM：工作者（生成结构化提示下的逐步思维链）、评审者（检查推理、识别错误步骤并提供纠正意见）、学生（通过纠正后的思维链进行微调，内化多步推理和自纠正能力）。

Result: 在多个真实世界时间序列问答基准测试中，T3LLM超越了现有基于LLM的强基线方法，达到了最先进的性能。

Conclusion: 通过利用时间序列数据的可验证性，T3LLM框架成功地将多步推理和显式纠错机制整合到LLM中，显著提升了时间序列问答的准确性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有代码生成评估基准大多在单一结构粒度上进行，且编程语言有限，无法揭示不同代码范围和跨语言场景下的细粒度能力差异。

Method: 开发M2G-Eval框架，包含类、函数、块、行四个粒度级别，覆盖18种编程语言，包含17K+训练任务和1,286个人工标注、污染控制的测试实例。通过监督微调和Group Relative Policy Optimization训练Qwen3-8B模型得到M2G-Eval-Coder模型。

Result: 评估30个模型（28个SOTA LLM + 2个M2G-Eval-Coder变体）发现：1）难度层次明显，行级任务最简单，类级最困难；2）随着任务复杂度增加，完整粒度和部分粒度语言间的性能差距扩大；3）存在强跨语言相关性，表明模型学习了可迁移的编程概念。
...
TL;DR: 该论文分析了扩散模型在文本生成中的应用挑战，指出现有方法在信息分布和多标记依赖方面存在不足，提出了更符合文本结构的扩散过程改进方向。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言生成中具有并行解码和迭代优化等优势，但文本的离散性和高度结构化特性使得直接应用扩散原理面临挑战。论文旨在重新审视扩散语言建模，识别现有方法的局限性。

Method: 论文首先将现有方法分为两类：嵌入空间的连续扩散和标记上的离散扩散。然后通过分析近期大型扩散语言模型，识别出两个核心问题：均匀破坏不尊重信息在位置上的分布方式，以及标记级边际训练无法捕捉并行解码中的多标记依赖关系。

Result: 分析表明，现有扩散语言建模方法都只能部分满足五个基本属性，反映了结构上的权衡。特别是发现了均匀破坏和多标记依赖捕捉不足这两个关键问题。

Conclusion: 论文建议开发更符合文本结构的扩散过程，鼓励未来研究朝着更一致的扩散语言模型方向发展，以更好地处理文本生成任务。
...
TL;DR: GRPO比DPO在提升大语言模型思维链忠实度方面更有效，Qwen2.5-14B-Instruct模型表现最佳，模型规模与性能正相关。


<details>
  <summary>Details</summary>
Motivation: 当前思维链推理方法存在解释与模型实际推理过程不一致的问题，模型可能生成看似合理但具有误导性的解释，这影响了CoT方法在安全监督和对齐监控中的可靠性。

Method: 评估两种优化方法：组相对策略优化（GRPO）和直接偏好优化（DPO），在提升思维链忠实度方面的效果。在不同规模模型上进行实验比较。

Result: GRPO在大型模型中表现优于DPO，Qwen2.5-14B-Instruct模型在所有评估指标上取得最佳结果。两种方法都显示模型规模与性能正相关，但GRPO在提升忠实度指标方面潜力更大，尽管在较小规模时稳定性较差。
...
<details>
  <summary>Details</summary>
Motivation: 研究GLU-MLP层宽度剪枝对模型不同认知能力的影响，挑战剪枝导致均匀退化的普遍假设，探索扩展比作为选择性调节认知能力的关键架构参数。

Method: 采用基于最大绝对权重准则的结构化宽度剪枝方法，在Llama-3.2-1B和3B模型上评估七种扩展比配置，使用综合基准测试评估事实知识、数学推理、语言理解、指令跟随和真实性等能力。

Result: 发现选择性保留现象：参数知识任务性能下降，但指令跟随能力提升46%-75%，多步推理保持稳健；事实知识能力与真实性指标呈强负相关；剪枝配置可实现23%能耗降低，但单请求延迟增加。

Conclusion: 扩展比是选择性调节认知能力的关键架构参数，MAW引导的宽度剪枝作为选择性过滤器，减少参数知识同时保持或增强行为对齐，连接了知识压缩和模型对齐两个研究领域。
...
TL;DR: 该论文提出了一种词汇感知的共形预测框架（VACP），用于改进大语言模型在下一个词预测中的不确定性量化，显著减少了预测集大小同时保持覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域部署大语言模型需要严格的不确定性量化，但标准的softmax概率通常校准不佳。传统的共形预测方法虽然能保证覆盖，但会产生包含数百个token的预测集，缺乏实用性。

Method: 提出了词汇感知共形预测（VACP）框架，结合语义掩码和温度调整评分来减少有效预测空间，同时理论上保持边际覆盖。该方法针对词汇量超过25万token的transformer模型进行优化。

Result: 在Gemma-2B模型上使用SQUAD和WikiText基准测试，VACP实现了89.7%的经验覆盖率（目标90%），同时将平均预测集大小从847个token减少到4.3个token，效率提升197倍。

Conclusion: VACP框架有效解决了大词汇量语言模型中的覆盖-效率权衡问题，为高风险应用提供了实用且理论保证的不确定性量化方法。
...
TL;DR: 本文提出一个多语言希望言论检测框架，重点关注乌尔都语，使用预训练Transformer模型在PolyHope-M 2025基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 希望言论在自然语言处理中相对被忽视，现有研究主要集中在英语，导致乌尔都语等低资源语言缺乏相关工具。虽然Transformer架构在仇恨和冒犯性言论检测中表现良好，但很少应用于希望言论检测或在不同语言环境中测试。

Method: 使用预训练Transformer模型（XLM-RoBERTa、mBERT、EuroBERT、UrduBERT），应用简单预处理并训练分类器进行希望言论检测，重点关注乌尔都语的多语言框架。

Result: 在PolyHope-M 2025基准测试中，乌尔都语二元分类F1分数达95.2%，多类分类达65.2%，西班牙语、德语和英语也取得类似竞争性结果。
...
TL;DR: 研究发现大语言模型在多语言推理中存在严重问题：虽然任务准确率高，但推理过程与结论之间经常不匹配，非拉丁文字语言的问题比拉丁文字语言严重至少两倍。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型通过思维链提示展现了强大的推理能力，但这种推理质量在不同语言间的转移效果尚未得到充分研究。当前的多语言评估实践可能无法全面反映模型的推理能力。

Method: 引入人工验证的评估框架，分析6种语言、6个前沿模型在GlobalMMLU问题上的6.5万条推理轨迹，评估推理过程是否逻辑上支持结论。通过人工标注建立错误分类法。

Result: 发现关键盲点：模型虽然任务准确率高，但推理过程经常无法支持其结论。非拉丁文字语言的推理轨迹中，推理与结论不匹配的比例至少是拉丁文字语言的两倍。主要错误类型包括证据错误（无支持的主张、模糊事实）和不合逻辑的推理步骤。

Conclusion: 当前多语言评估实践对模型推理能力的评估是不完整的，需要开发推理感知的评估框架来更全面地评估模型的多语言推理能力。
...
TL;DR: 研究发现通过重新表述提示词（中性、第三人称表达）能有效减少LLM模拟人口响应时的社会期望偏差，使硅样本更接近真实人类数据。


<details>
  <summary>Details</summary>
Motivation: LLM被用于模拟人口响应（硅采样），但对社会敏感问题的回答常表现出社会期望偏差，与真实人类数据偏离。现有研究对此关注有限，需要探索如何通过提示词设计来缓解这种偏差。

Method: 使用美国国家选举研究数据，在Llama-3.1系列和GPT-4.1-mini上测试四种提示词缓解方法：重新表述（中性第三人称）、反向编码（语义反转）、启动效应和前言（分别鼓励分析和真诚）。使用Jensen-Shannon散度与引导置信区间评估与ANES的对齐度。

Result: 重新表述提示词最有效地改善了对齐度，减少了社会可接受答案的分布集中度，使分布更接近ANES。反向编码在不同项目上效果不一，而启动效应和前言方法鼓励了响应一致性，对偏差缓解没有系统性益处。
...
TL;DR: 利用社交媒体数据（如Twitter）构建NLP流水线，自动识别女性分享的怀孕经历，按结果分类（正常分娩为阳性案例，不良妊娠结局为阴性案例），为流行病学研究提供补充数据源。


<details>
  <summary>Details</summary>
Motivation: 婴儿死亡率在美国仍是重大公共卫生问题，出生缺陷是主要原因之一。尽管已有研究努力理解不良妊娠结局（如流产、死产、出生缺陷、早产）的原因，但仍需要更全面的研究和干预策略。传统数据集有限，需要补充数据源。

Method: 利用公开社交媒体数据（特别是Twitter），构建自然语言处理（NLP）流水线。通过数据预处理技术解决社交媒体数据的失衡、噪声和非结构化问题，采用数据增强策略。自动识别分享怀孕经历的女性，按报告结果分类：报告足月妊娠和正常出生体重的为阳性案例，报告不良妊娠结局的为阴性案例。

Result: 研究展示了社交媒体数据作为流行病学调查补充资源的可行性。提供了评估特定干预措施、治疗或产前暴露对母婴健康结果因果影响的潜在应用。建立了未来涉及孕妇队列和比较组健康研究的框架。

Conclusion: 社交媒体数据可作为研究不良妊娠结局的补充数据源，NLP流水线能有效识别和分类怀孕经历。该方法为流行病学研究提供了新途径，有助于理解不良妊娠结局的原因并制定干预策略。
...
TL;DR: WeDLM：基于因果注意力的扩散语言模型解码框架，通过拓扑重排序实现前缀缓存友好，在保持AR模型质量的同时获得3-10倍推理加速


<details>
  <summary>Details</summary>
Motivation: 自回归解码的token-by-token特性限制了推理并行性，而现有扩散语言模型虽然支持并行解码，但由于依赖双向注意力破坏了标准前缀KV缓存，导致实际部署效率不佳

Method: 提出WeDLM框架，完全基于标准因果注意力，通过拓扑重排序技术将已观测token移动到物理前缀同时保持逻辑位置，实现前缀缓存友好的并行生成；采用流式解码持续提交置信token到增长的前缀中

Result: 在保持强AR骨干模型质量的同时，在挑战性推理基准上获得近3倍加速，在低熵生成场景下达到10倍加速；与vLLM服务的AR基线在匹配部署设置下比较，证明扩散式解码在实践中能超越优化AR引擎

Conclusion: WeDLM展示了扩散式解码通过完全基于因果注意力的设计，能够实现前缀缓存友好的并行生成，在实际部署中超越优化AR引擎，为高效LLM推理提供了新范式
...
TL;DR: BioSelectTune：一个高效的数据中心化框架，通过混合超级过滤策略从低质量数据中筛选高质量训练集，使LLM在生物医学命名实体识别任务上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 通用领域大语言模型在生物医学命名实体识别任务中存在领域知识不足和低质量训练数据导致性能下降的问题，需要一种高效的数据质量优先的微调方法

Method: 将BioNER重构为结构化JSON生成任务，采用混合超级过滤策略（弱到强数据筛选方法），使用同源弱模型蒸馏出紧凑、高质量的训练数据集

Result: 在多个BioNER基准测试中达到最先进性能，仅使用50%筛选后的正数据训练的模型不仅超过完全训练的基线，还优于BioMedBERT等专业领域模型

Conclusion: BioSelectTune证明了在生物医学命名实体识别任务中，数据质量比数量更重要，通过高效的数据筛选策略可以显著提升LLM性能
...
TL;DR: TEXT模型通过多模态大语言模型生成解释，结合时间对齐和文本路由稀疏专家混合，在情感分析任务上取得最佳性能


<details>
  <summary>Details</summary>
Motivation: 多模态情感分析需要处理不同模态间的微妙情感，但现有方法在解释能力和时间对齐方面探索不足

Method: 1) 使用多模态大语言模型为MSA生成解释；2) 通过时间导向的神经网络块对齐音频和视频表示；3) 采用文本路由稀疏专家混合与门控融合

Result: 在四个数据集上取得最佳性能，优于三个最近提出的方法和三个MLLMs。在CH-SIMS数据集上，MAE降至0.353，相比最近方法降低13.5%

Conclusion: TEXT模型通过结合解释增强和时间对齐，显著提升了多模态情感分析性能，证明了这些技术在MSA中的重要性
...
TL;DR: 研究探讨了在乌尔都语假新闻检测中，通过领域自适应预训练提升多语言模型性能的方法，发现XLM-RoBERTa在领域自适应后表现一致提升，而mBERT结果混合。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的错误信息是一个普遍问题，但低资源语言如乌尔都语在此领域受到有限关注。多语言预训练模型在特定领域术语上表现不佳，需要改进。

Method: 采用分阶段训练方法：首先对XLM-RoBERTa和mBERT进行领域自适应预训练，使用公开的乌尔都语新闻语料库；然后在下游假新闻分类任务上进行微调。

Result: 在四个公开的乌尔都语假新闻数据集上的实验表明，领域自适应的XLM-RoBERTa始终优于其原始版本，而领域自适应的mBERT表现出混合结果。

Conclusion: 领域自适应预训练对提升多语言模型在低资源语言假新闻检测任务中的性能是有效的，特别是对XLM-RoBERTa模型效果显著。
...
TL;DR: 评估规则基线、领域特定Transformer模型和大型语言模型在临床笔记分割任务上的表现，GPT-5-mini在句子级和自由文本分割上达到最佳平均F1分数72.4
...
Method: 使用MIMIC-IV中1000份临床笔记的精选数据集，评估了三种方法：1) 基于规则的基线方法；2) 领域特定的Transformer模型；3) 大型语言模型（包括API-based模型）。实验包括句子级分割和自由文本分割任务。

Result: 大型API-based模型表现最佳，GPT-5-mini在句子级和自由文本分割上的平均F1分数达到72.4。轻量级基线方法在结构化句子级任务上仍有竞争力，但在非结构化自由文本任务上表现不佳。

Conclusion: 研究结果为方法选择提供了指导，并为下游任务（如信息提取、队列识别和自动摘要）奠定了基础。大型语言模型在临床笔记分割任务上表现出色，但轻量级方法在特定结构化任务中仍有应用价值。
...
TL;DR: 提出了NepEMO数据集，用于尼泊尔语Reddit帖子的多标签情感和情感分类，包含4,462个帖子，涵盖5种情感和3种情感类别，并比较了多种机器学习模型。


<details>
  <summary>Details</summary>
Motivation: 社交媒体平台如Reddit为用户提供了匿名表达敏感话题（如健康和生活）的空间，特别是在挑战性事件期间。目前缺乏针对尼泊尔语社交媒体内容的多标签情感和情感分类数据集。

Method: 构建了NepEMO数据集，包含4,462个尼泊尔语Reddit帖子（2019年1月-2025年6月），涵盖英语、罗马化尼泊尔语和天城体文本。进行了详细的语言分析，包括情感趋势、情感共现、情感特定n-gram，以及使用LDA主题建模和TF-IDF关键词提取。比较了传统机器学习、深度学习和Transformer模型。

Result: Transformer模型在MLE（多标签情感分类）和SC（情感分类）任务上始终优于传统机器学习和深度学习模型。

Conclusion: NepEMO数据集填补了尼泊尔语社交媒体情感分析的数据空白，为多标签情感和情感分类提供了有价值的资源，并证明了Transformer模型在此类任务中的优越性。
...
TL;DR: 提出自动合成高难度模拟环境的统一流程和环境级强化学习算法，提升语言智能体的训练效率和稳定性


<details>
  <summary>Details</summary>
Motivation: 现有强化学习在模拟环境中的研究存在局限性：环境合成多为半自动，任务难度不足，模拟用户不稳定，环境异质性大，这些限制了智能体强化学习的效果

Method: 1) 提出自动化、可扩展的模拟环境合成流程，用于生成高难度但易于验证的任务；2) 提出环境级强化学习算法，有效缓解用户不稳定性，并在环境层面进行优势估计

Result: 在tau-bench、tau2-Bench和VitaBench等智能体基准测试上进行全面评估，验证了方法的有效性。深入分析显示该方法具有良好的领域外泛化能力

Conclusion: 提出的自动环境合成流程和环境级强化学习算法能够有效解决模拟环境中的用户不稳定性和异质性问题，显著提升语言智能体的训练效率和稳定性，并具备良好的泛化能力
...
TL;DR: 该论文提出了一种新的预训练方法，将监督学习重新解释为策略梯度优化，通过奖励塑造策略调整预训练模型的token输出分布，为后续强化学习提供更好的探索空间。


<details>
  <summary>Details</summary>
Motivation: 强化学习能显著提升大语言模型的推理能力，但其效果严重依赖于预训练模型的token输出分布所定义的探索空间。目前的标准交叉熵损失限制了探索潜力，需要重新思考如何通过预训练为后续RL提供更有利的探索空间。

Method: 将标准交叉熵损失重新解释为单步episode的策略梯度优化，提出广义预训练目标。将next-token预测建模为随机决策过程，引入奖励塑造策略：1）使用正奖励缩放因子控制ground-truth token的概率集中度；2）采用排名感知机制，对高排名和低排名的负token进行不对称处理。

Result: 研究发现，与直觉相反，施加精度导向的先验（而非更高分布熵）能为RL提供更优的探索空间，最终提升端到端推理性能。

Conclusion: 通过将监督学习重新框架为策略梯度优化，并引入奖励塑造策略来调整预训练分布，可以为后续强化学习提供更有利的探索空间，从而提升语言模型的推理能力。精度导向的探索空间比高熵探索空间更有效。
...
TL;DR: 评估ChatGPT-4o、Gemini 1.5 Pro和LIama 3.3 70B在临床决策支持中的表现，发现任务间差异大，提示工程效果因模型和任务而异，并非通用解决方案。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在医学知识评估中表现出潜力，但其在真实临床决策中的实际效用尚未充分探索，需要评估LLM在整个临床推理工作流程中的表现。

Method: 使用36个病例研究评估三个先进LLM在五个关键临床决策任务中的表现：鉴别诊断、必要紧急步骤、相关诊断测试、最终诊断和治疗建议。在两种温度设置下测试，并应用MedPrompt框架的提示工程变体，包括有针对性和随机的动态少样本学习。

Result: 所有模型在不同任务间表现差异显著：最终诊断接近完美准确率，相关诊断测试表现差，其他任务表现中等。ChatGPT在零温度下表现更好，LIama在默认温度下更强。提示工程能显著改善基线准确率最低的任务，但对其他任务可能适得其反。有针对性的动态少样本提示并不总是优于随机选择。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能强大，但计算和存储成本高昂。现有的后训练结构化剪枝方法在少样本校准集不能充分反映预训练数据分布时，在下游任务上泛化能力有限，存在校准偏差问题。

Method: 提出Function-Aware Neuron Grouping (FANG)框架：1）基于神经元处理的语义上下文类型，将功能相似的神经元分组；2）在每个组内进行重要性估计时，给与神经元组功能角色强相关的token更高权重；3）保留跨多种上下文类型贡献的神经元；4）根据每个模块的功能复杂度自适应分配稀疏度。

Result: FANG在保持语言建模性能的同时提升了下游任务准确率。与FLAP和OBC两种代表性剪枝方法结合时达到SOTA结果：在30%和40%稀疏度下，平均准确率比FLAP和OBC高出1.5%-8.5%。

Conclusion: FANG通过基于功能的神经元分组和自适应稀疏度分配，有效缓解了后训练剪枝中的校准偏差问题，在稀疏度与性能之间实现了更好的平衡。
...
TL;DR: LENS框架将多模态健康感知数据与语言模型对齐，生成临床基础的心理健康叙述，通过将EMA响应转化为自然语言描述并训练补丁级编码器，实现原始传感器信号到LLM表示空间的直接映射。
...
TL;DR: 论文指出现有Biasing Features指标将思维链(CoT)标注为不忠实存在误判，混淆了不忠实性与不完整性，并提出了新的评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法使用Biasing Features指标将思维链标注为不忠实，如果它省略了影响预测的提示注入提示。作者认为这种指标混淆了不忠实性与不完整性，后者是将分布式transformer计算转换为线性自然语言叙述所需的损失压缩。

Method: 1) 在Llama-3和Gemma-3上进行多跳推理任务实验；2) 提出新的faithful@k指标；3) 使用因果中介分析(Causal Mediation Analysis)研究非言语化提示的因果作用。

Result: 1) 被Biasing Features标记为不忠实的许多CoT被其他指标判断为忠实，某些模型中超过50%；2) 更大的推理时token预算显著提高提示言语化率(某些设置中达90%)；3) 因果分析显示即使非言语化提示也能通过CoT因果中介预测变化。
...
TL;DR: Prompt Choreography框架通过动态全局KV缓存优化多智能体LLM工作流，支持消息编码复用、任意重排序和并行调用，显著降低延迟并提升端到端速度


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多智能体工作流中部署增多，存在大量冗余计算问题。传统方法每次调用都需要重新编码消息，导致延迟高、效率低，需要一种能复用消息编码的优化方案

Method: 提出Prompt Choreography框架，维护动态全局KV缓存，允许LLM调用时关注任意重排序的先前消息子集，支持并行调用。通过微调LLM使其适应缓存机制，以模拟原始结果

Result: 显著降低每条消息延迟（首token生成速度提升2.0-6.2倍），在冗余计算为主的工作流中实现超过2.2倍的端到端加速

Conclusion: Prompt Choreography通过高效的缓存机制有效优化多智能体LLM工作流性能，减少冗余计算，为实际部署提供重要性能提升
...
TL;DR: TabiBERT是一个基于ModernBERT架构的土耳其语单语编码器，在1万亿token的多领域语料上从头训练，支持8192token上下文，推理速度提升2.65倍，在土耳其语NLP基准测试中超越现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 土耳其语NLP缺乏整合现代架构范式（如RoPE、FlashAttention）的单语编码器。现有模型未能充分利用最新的计算效率、训练稳定性和长上下文建模进展。

Method: 基于ModernBERT架构，整合RoPE位置编码、FlashAttention和优化归一化。在840亿token的多领域语料（网页文本73%、科学出版物20%、源代码6%、数学内容0.3%）上采样1万亿token进行预训练。

Result: 支持8192token上下文（16倍于原始BERT），推理速度提升2.65倍，GPU内存消耗降低。在TabiBench（28个数据集，8个任务类别）上获得77.58分，超越BERTurk 1.62分，在5个类别中达到SOTA：问答（+9.55）、代码检索（+2.41）、文档检索（+0.60）。

Conclusion: TabiBERT填补了土耳其语现代编码器的空白，展示了跨领域泛化能力，为土耳其语NLP研究提供了透明可复现的基础模型。
...
TL;DR: 提出一种基于储层计算的矩阵乘法免费语言模型，通过部分固定和共享权重、插入储层层来减少计算成本，在保持性能的同时降低参数数量、训练和推理时间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能优异，但计算成本高昂成为主要瓶颈。本研究旨在通过矩阵乘法免费的语言模型架构和储层计算思想来提高计算效率。

Method: 1) 采用矩阵乘法免费的语言模型架构；2) 部分固定和共享选定层的权重；3) 插入储层层以获得丰富的动态表示而不增加训练开销；4) 组合多个操作以减少内存访问。

Result: 实验结果显示，提出的架构将参数数量减少最多19%，训练时间减少9.9%，推理时间减少8.0%，同时保持与基线模型相当的性能。

Conclusion: 通过结合矩阵乘法免费架构和储层计算技术，可以显著降低语言模型的计算成本，同时保持性能，为解决LLM计算瓶颈提供了有效方案。
...
TL;DR: 该研究提出了BiomedTLDR数据集，包含大量研究者撰写的科学论文摘要，用于评估LLMs生成科学极简摘要的能力。研究发现，虽然某些LLMs能生成类似人类的摘要，但总体上LLMs更倾向于使用原文的词汇选择和修辞结构，相比人类更偏向抽取式而非抽象式摘要。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏全面高质量的科学极简摘要数据集，阻碍了LLMs摘要能力的开发和评估。需要了解LLMs在生成科学摘要方面的表现，以及LLM生成摘要与人类专家摘要的差异。

Method: 提出BiomedTLDR数据集，利用论文作者在参考文献中附带的评论作为人类撰写的摘要样本。然后测试流行的开源LLMs基于论文摘要生成TLDRs，并分析LLM生成摘要与人类摘要的差异。

Result: 分析显示，虽然某些LLMs能成功生成类似人类的摘要，但总体上LLMs对原文的词汇选择和修辞结构有更强的亲和力，相比人类更倾向于抽取式而非抽象式摘要。

Conclusion: BiomedTLDR数据集为评估LLMs的科学摘要能力提供了重要资源。LLMs在生成科学摘要时表现出更强的抽取式倾向，与人类的抽象式摘要风格存在差异，这为改进LLMs的摘要生成能力提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要监督训练或复杂集成，缺乏可解释性和灵活性。本文旨在开发一种无监督、可解释且能适应不同场景的LLM集成方法。

Method: 提出基于同行评审的三阶段框架：1) 使用LLM-as-a-Judge技术让多个LLM相互评分；2) 通过图模型或平均策略聚合分数；3) 选择最高分响应作为集成输出。

Result: 在两个变体上，在四个数据集上均取得强劲结果，分别比先进的Smoothie-Global模型高出6.9%和7.3%。

Conclusion: LLM-PeerReview是一种概念简单但经验强大的无监督集成方法，具有清晰的解释机制和灵活的适应性。
...
TL;DR: LLMs在复杂编程任务中常出错，作者假设这是由于通用语言的灵活性导致。他们设计了Anka DSL，即使LLM从未训练过该语言，也能通过上下文提示达到99.9%解析成功率和95.8%任务准确率，在复杂任务上比Python高出40个百分点。


<details>
  <summary>Details</summary>
Motivation: LLMs在代码生成方面表现出色，但在复杂的多步骤编程任务中仍存在系统性错误。作者假设这些错误源于通用编程语言的灵活性，这种灵活性允许多种有效方法但需要隐式的状态管理，导致LLMs容易出错。

Method: 作者设计了Anka，一个用于数据转换管道的领域特定语言（DSL）。该语言具有明确、受约束的语法，减少了代码生成的歧义。他们创建了包含100个基准问题的测试集，评估LLMs（Claude 3.5 Haiku和GPT-4o-mini）在零训练暴露于Anka的情况下，仅通过上下文提示学习该语言的能力。

Result: Claude 3.5 Haiku在Anka上取得了99.9%的解析成功率和95.8%的整体任务准确率。在复杂多步骤管道任务上，Anka比Python有40个百分点的准确率优势（100% vs. 60%）。GPT-4o-mini的交叉验证也确认了这一优势（在多步骤任务上+26.7个百分点）。

Conclusion: 研究表明：(1) LLMs可以通过上下文提示完全学习新的DSLs，达到接近原生的准确率；(2) 受约束的语法显著减少了复杂任务中的错误；(3) 为LLM生成专门设计的领域特定语言可以胜过LLMs有大量训练数据的通用语言。作者发布了完整的语言实现、基准套件和评估框架。
...
TL;DR: 论文提出了一种基于稀疏自编码器的可解释参数高效微调方法，通过识别任务相关特征构建显式低秩子空间来指导适配器初始化，在安全对齐任务上取得了优异性能。
...
Result: 在安全对齐任务上，该方法达到99.6%的安全率，超过全参数微调7.4个百分点，接近基于RLHF的方法，同时仅更新0.19-0.24%的参数。方法通过SAE特征的语义基础提供了对学习到的对齐子空间的可解释性洞察。
...
TL;DR: 论文提出LiveAMR任务，用于检测电商直播中主播通过变音逃避监管的虚假广告行为，构建首个86,790样本的数据集，并利用LLM生成训练数据提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 中国电商直播（如抖音）已成为主要销售渠道，但主播常使用变音逃避监管并进行虚假广告，特别是在健康医疗领域。现有研究主要关注社交媒体和地下产业的文本规避，缺乏针对直播中发音规避的研究。

Method: 提出LiveAMR任务检测发音规避行为，构建首个包含86,790个样本的数据集，将任务转化为文本到文本生成问题，并利用大语言模型生成额外训练数据来提升性能。

Result: 构建了首个LiveAMR数据集，通过LLM生成训练数据的方法显著提升了检测性能，证明了变音解析能有效增强直播监管效果。

Conclusion: LiveAMR任务能有效检测电商直播中的发音规避行为，构建的数据集和方法为直播监管提供了新工具，变音解析对直播监管有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 有声书解读需求增长，但人工制作耗时耗力，需要自动化解决方案来降低制作成本并提高效率

Method: 采用多智能体协作框架，包含11个专业智能体（主题分析师、案例分析师、编辑、叙述者、校对员等），结合大语言模型和语音合成技术
...
TL;DR: 该论文系统综述了记忆在AI智能体中的研究，从认知神经科学角度出发，连接人类记忆机制与LLM驱动的智能体，涵盖记忆定义、分类、存储机制、评估基准、安全防护及未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有自主智能体研究在记忆工作流设计上受限于学科壁垒，难以吸收人类记忆机制的精髓。为了弥合这一差距，需要系统整合跨学科的记忆知识，将认知神经科学的见解与LLM驱动的智能体连接起来。

Method: 采用跨学科综合研究方法：1) 从认知神经科学到LLM再到智能体的渐进轨迹阐明记忆定义与功能；2) 从生物和人工视角对比分析记忆分类、存储机制及完整管理生命周期；3) 综述智能体记忆的主流评估基准；4) 从攻击和防御双重角度探讨记忆安全。

Result: 论文系统梳理了记忆研究的跨学科知识框架，建立了认知神经科学与AI智能体之间的连接，为记忆工作流设计提供了理论基础，并识别出现有研究的局限与挑战。

Conclusion: 记忆是连接过去与未来的关键枢纽，对AI系统处理复杂任务至关重要。未来研究方向应聚焦于多模态记忆系统和技能获取，以推动更高效、安全的智能体记忆系统发展。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多种NLP任务上表现出色，但在需要深度推理和逻辑推断的复杂场景中仍面临挑战。模型可能从大规模文本语料中学习到噪声或无关信息，导致预测错误或与事实知识不一致的输出。

Method: 提出SGR框架：1) 根据输入查询动态构建外部知识库中的相关子图；2) 基于结构化子图引导模型进行多步逐步推理；3) 整合多个推理路径生成最终答案。通过结构化子图减少噪声信息影响。

Result: 在多个基准数据集上的实验结果表明，SGR框架持续优于强基线方法，证明了其在增强大语言模型推理能力方面的有效性。

Conclusion: SGR框架通过外部子图生成和逐步推理机制，有效提升了大语言模型在复杂推理任务中的准确性和可靠性，为解决LLMs在深度推理方面的局限性提供了有前景的解决方案。
...
TL;DR: EntroDrop：一种基于熵的token丢弃方法，通过选择性掩码低熵token来平衡多轮训练中的学习动态，防止大语言模型在有限数据上过拟合
...
Method: 提出EntroDrop，一种熵引导的token丢弃方法，作为结构化数据正则化。该方法在训练期间选择性掩码低熵token，并采用课程调度来根据训练进度调整正则化强度。

Result: 在0.6B到8B参数规模的模型实验中，EntroDrop始终优于标准正则化基线，并在扩展的多轮训练中保持稳健性能。

Conclusion: 研究强调了在有限数据训练时，使正则化与token级学习动态对齐的重要性。EntroDrop为在数据受限领域中更有效地适应大语言模型提供了有前景的途径。
...
TL;DR: 性别多样性对科研团队影响力的关系呈倒U型，女性领导+男性支持的团队表现最佳，团队规模对领导组和支持组的性别多样性效应有不同调节作用


<details>
  <summary>Details</summary>
Motivation: 现有研究对性别多样性如何影响科研团队成功存在不一致发现，且大多采用聚合视角，忽略了团队内部角色分化。需要更细致地理解不同角色中性别多样性如何塑造团队影响力。

Method: 基于PLOS期刊13万多篇论文，利用作者贡献声明将团队成员分为领导角色和支持角色。使用多元回归分析这些角色中性别多样性与团队影响力（五年引用次数）的关系，并应用阈值回归模型研究团队规模如何调节这种关系。

Result: 1) 领导组和支持组的性别多样性与团队影响力均呈倒U型关系；2) 全女性领导组+全男性支持组的团队比其他类型团队影响力更高；3) 领导组性别多样性对小团队有显著负效应，但对大团队变为正效应且统计不显著；而支持组性别多样性的效应始终显著为正，不受团队规模影响。

Conclusion: 性别多样性对科研团队的影响具有角色特异性，不能简单用聚合视角分析。团队构成中领导和支持角色的性别组合对团队成功有重要影响，且团队规模对领导角色的性别多样性效应有调节作用。
...
TL;DR: C2PO：一种统一的LLM对齐框架，通过因果对比偏好优化同时缓解刻板印象偏见和结构偏见，同时保持通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的偏见（刻板印象偏见和结构偏见）严重威胁可信度，现有方法通常孤立处理这些问题，缓解一种偏见往往会加剧另一种偏见。

Method: 提出Causal-Contrastive Preference Optimization (C2PO)框架：利用因果反事实信号分离偏见诱导特征与有效推理路径，采用公平敏感的偏好更新机制动态评估logit级贡献并抑制捷径特征。

Result: 在多个基准测试（BBQ、Unqover、MNLI、HANS、Chatbot、MT-Bench、StereoSet、WinoBias、MMLU、GSM8K）上验证，C2PO能有效缓解刻板印象和结构偏见，同时保持强大的通用推理能力。

Conclusion: C2PO通过统一框架同时处理LLM中的多种偏见类型，解决了现有方法孤立处理偏见的局限性，为构建更可信的LLM提供了有效方案。
...
TL;DR: ClinDEF是一个通过模拟诊断对话评估LLM临床推理能力的动态框架，基于疾病知识图谱生成病例，实现多轮医患交互，提供细粒度评估


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准测试主要关注静态问答，无法反映真实的动态临床推理过程；现有动态医学框架依赖有限且易受污染的数据集，缺乏细粒度、多层次的评估

Method: 基于疾病知识图谱动态生成患者病例，建立LLM医生与自动化患者代理之间的多轮交互对话，实现模拟诊断过程

Result: ClinDEF能有效暴露最先进LLM在临床推理方面的关键差距，提供比传统方法更细致、更具临床意义的评估范式

Conclusion: ClinDEF提供了一个更细致、更符合临床实际的评估框架，能够更好地评估LLM的临床推理能力，超越了传统的诊断准确性评估
...
TL;DR: 提出ERC损失函数，通过约束路由器决策与专家能力对齐来提升MoE模型性能，计算效率高且能定量追踪专家专业化程度。


<details>
  <summary>Details</summary>
Motivation: 当前MoE模型中路由器决策与专家能力之间缺乏显式约束，导致两者对齐不佳，限制了模型性能。

Method: 提出专家-路由器耦合损失(ERC)，将每个专家的路由器嵌入视为代理令牌，通过扰动嵌入获取内部激活，并施加两个约束：(1)每个专家对自身代理令牌的激活高于其他专家的代理令牌；(2)每个代理令牌在其对应专家中获得的激活强于其他专家。

Result: 在3B到15B参数的MoE-LLM预训练中，在数万亿token上验证了ERC损失的有效性，同时能灵活控制和定量追踪专家专业化程度。

Conclusion: ERC损失通过轻量级辅助损失函数有效耦合路由器决策与专家能力，提升MoE模型性能，同时提供对专家专业化的可解释性洞察。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM嵌入常用于向量数据库中的语义相似性检索，但文本语料库中的全局语义关系结构往往不透明。需要一种方法来揭示文本之间的层次化语义关系。

Method: 采用嵌套密度聚类方法：首先在LLM嵌入空间中识别语义相似度高的密集文本簇，然后逐步放宽密度标准，使密集簇合并成更分散的簇，最终形成包含整个数据集的单一簇（树的根节点），从而构建捕获文本层次语义关系的树结构。

Result: 方法成功应用于科学摘要分类案例研究，能够数据驱动地发现研究领域及其子领域。在20 Newsgroups和IMDB 50k Movie Reviews等基准数据集上也验证了方法的跨领域鲁棒性。

Conclusion: 嵌套密度树方法能够有效揭示文本数据集中的语义结构和演化，在科学计量学、主题演化等领域具有应用潜力。
...
TL;DR: ACT算法通过三阶段方法自动检测拉比文献中的圣经引用，结合形态感知对齐和上下文丰富化，在F1分数上达到0.91，优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有文本重用框架难以处理短小、转述或结构嵌入的引用，特别是在拉比文献中。需要弥合机器检测与人类编辑判断之间的方法学差距。

Method: 三阶段算法：1) 形态感知对齐算法；2) 上下文敏感丰富化阶段，识别复杂引用模式如"波浪"和"回声"引用；3) 提供三种配置评估各组件贡献。

Result: 完整ACT管道(ACT-QE)在F1分数上达到0.91，召回率0.89，精确率0.94，优于所有基线系统。不同配置显示各组件对性能的影响。

Conclusion: ACT不仅改进了引用检测，其跨语料库风格模式分类能力为体裁分类和互文分析开辟了新途径，为数字人文和计算文献学提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 探索在视觉-语言大模型中，视觉生成任务是否能增强视觉理解能力，特别是在大规模数据（>2亿样本）下的效果

Method: 提出UniHetero统一模型结构，在大规模预训练下分析生成任务对理解任务的影响

Result: 1) 生成能提升理解，但仅限于生成语义而非像素；2) 生成任务展现出更好的数据扩展趋势和更高的数据利用率；3) 输入嵌入的自回归能有效捕捉视觉细节

Conclusion: 视觉生成任务确实可以增强视觉理解，但关键在于生成语义内容而非像素级细节，这为视觉-语言统一模型的设计提供了重要指导
...
TL;DR: MoLaCE是一个轻量级推理时框架，通过混合不同潜在概念激活强度的专家来减少LLM的确认偏误，在计算效率上优于多智能体辩论。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在严重的确认偏误问题，当提示暗示偏好答案时，模型会强化这种偏误而非探索替代方案。这个问题在基础模型中已造成危害，在多智能体辩论中风险更大，因为回音室效应会强化而非纠正偏误。

Method: 提出MoLaCE框架，将专家实例化为不同潜在概念上的激活强度混合，这些潜在概念塑造模型响应。关键洞见是：由于语言的组合性，不同措辞的提示会以特定方式重新加权潜在概念，影响事实正确性，因此没有单一固定干预能普遍适用于所有输入。

Result: MoLaCE能持续减少确认偏误，提高鲁棒性，在计算量仅为多智能体辩论一小部分的情况下，达到或超越多智能体辩论的性能。也可集成到多智能体辩论框架中以多样化视角并减少相关错误。

Conclusion: MoLaCE使单个LLM能在内部模拟辩论的好处，同时保持计算效率和可扩展性，为解决LLM确认偏误问题提供了有效的轻量级解决方案。
...
TL;DR: 该论文提出使用知识图谱增强幻觉自检测的方法，通过将LLM响应转换为实体关系图来估计幻觉可能性，相比现有方法在准确率和F1分数上分别提升16%和20%。
...
Result: 在GPT-4o和Gemini-2.5-Flash上评估，相比标准自检测方法和最先进的SelfCheckGPT，相对准确率提升16%，F1分数提升20%。同时发布了一个手动整理增强的数据集。

Conclusion: LLM能更好地分析以知识图谱结构化的原子事实，即使初始输出包含不准确信息。该方法为更安全、更可信的语言模型铺平了道路。
...
TL;DR: LVLMs在视觉指令微调后指令跟随能力下降，通过构建包含输出格式说明的数据集可以缓解这一问题


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉指令微调后，常常失去原本LLM的指令跟随能力，导致无法按照任务指令执行预期操作

Method: 构建新的训练数据集，突出输出格式是否被指定，研究在微调中明确指示输出格式对LVLMs指令跟随能力的影响

Result: 定量评估证实LVLMs在常用数据集微调后指令跟随能力下降；使用包含输出格式说明的数据集训练的模型能更准确地跟随指令

Conclusion: 在视觉指令微调中包含具有输出格式说明的样本有助于缓解指令跟随能力的下降
...
TL;DR: 研究发现语音语言模型在多轮对话中存在"风格遗忘"问题，即模型无法维持指定的说话风格（如情感、口音、音量、语速），即使能回忆风格指令也无法在对话中持续表达。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示语音语言模型在维持指定说话风格方面的局限性，特别是在多轮对话中模型无法保持一致的说话风格，这影响了模型在需要特定风格交互场景中的实用性。

Method: 评估了3个专有模型和2个开源语音语言模型，测试它们在多轮对话中维持四种副语言风格（情感、口音、音量、语速）的能力。通过不同提示策略（系统消息vs用户消息）进行测试，并尝试通过明确要求模型回忆风格指令来缓解问题。

Result: 所有测试模型都无法在指令要求下维持一致的说话风格，表现出"风格遗忘"现象。模型能够回忆风格指令但无法在对话中表达。将指令放在系统消息中效果更差，这与系统提示的预期功能相矛盾。明确要求回忆指令能部分缓解问题。

Conclusion: 语音语言模型存在严重的风格遗忘问题，当前模型架构和训练方法无法有效维持多轮对话中的说话风格一致性，需要新的技术改进来解决这一局限性。
...
TL;DR: InfTool是一个完全自主的多智能体框架，通过自我进化的合成方法，仅使用原始API规范就能生成多样化的工具调用轨迹，无需人工标注，显著提升LLM工具调用能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个根本挑战：高质量轨迹需要昂贵的人工标注、对未见工具的泛化能力差、单模型合成存在质量上限导致偏见和覆盖缺口。需要一种完全自主的方法来可靠地启用LLM调用外部工具。

Method: InfTool采用自我进化的多智能体合成框架，包含三个协作智能体（用户模拟器、工具调用助手、MCP服务器），仅基于原始API规范生成从单次调用到复杂多步工作流的多样化验证轨迹。建立闭环：合成数据通过带门控奖励的组相对策略优化（GRPO）训练模型，改进后的模型生成更高质量数据针对能力缺口，无需人工干预迭代循环。

Result: 在伯克利函数调用排行榜（BFCL）上，InfTool将基础32B模型的准确率从19.8%提升到70.9%（+258%），超越10倍大的模型，媲美Claude-Opus，且完全基于合成数据无需人工标注。

Conclusion: InfTool通过完全自主的多智能体合成框架，突破了LLM工具调用的关键瓶颈，实现了从原始API规范到高性能工具调用能力的自我进化，为自主代理系统提供了可扩展的解决方案。
...
TL;DR: 本文介绍了CHQ-Sum数据集，包含1507个专家标注的消费者健康问题及摘要，用于解决医疗问题理解挑战，并评估了多种摘要模型效果。


<details>
  <summary>Details</summary>
Motivation: 消费者在网络上寻求健康信息时，通常使用过于描述性和外围信息表达医疗状况，这给自然语言理解带来挑战。目前缺乏领域专家标注的消费者健康问题摘要数据集，阻碍了高效摘要系统的发展。

Method: 构建CHQ-Sum数据集，包含1507个来自社区问答论坛的消费者健康问题，由领域专家标注对应摘要。该数据集为理解社交媒体上的健康相关帖子提供宝贵资源。

Result: 在多个最先进的摘要模型上对数据集进行基准测试，展示了数据集的有效性，为消费者健康问题摘要任务提供了评估标准。

Conclusion: CHQ-Sum数据集填补了消费者健康问题摘要领域的数据空白，为开发更高效的医疗信息理解系统提供了重要资源，有助于改善消费者健康信息的处理和理解。
...
<details>
  <summary>Details</summary>
Motivation: 当前信息搜索代理的工具使用主要局限于API级别的片段检索和基于URL的页面获取，限制了访问通过真实浏览可获得的更丰富信息。虽然完整的浏览器交互可以解锁更深层的能力，但其细粒度控制和冗长的页面内容返回给ReAct风格的函数调用代理带来了巨大复杂性。

Method: 提出了Nested Browser-Use Learning (NestBrowse)，引入了一个最小且完整的浏览器操作框架，通过嵌套结构将交互控制与页面探索解耦。这种设计简化了代理推理，同时实现了有效的深度网络信息获取。

Result: 在具有挑战性的深度信息搜索基准测试中，NestBrowse在实践中显示出明显的优势。进一步深入分析强调了其效率和灵活性。

Conclusion: NestBrowse通过嵌套浏览器使用学习框架，成功解决了信息搜索代理在真实网页浏览中的复杂性挑战，实现了更有效的深度网络信息获取。
...
<details>
  <summary>Details</summary>
Motivation: 探索研究语言模型概率与认知现象关系时，需要多少上下文量才是必要和适当的，特别是考察完整话语是否必要

Method: 通过比较完整话语和n-gram表示，研究概率缩减现象，验证n-gram是否足以作为认知规划单元

Result: n-gram表示足以作为认知规划单元，完整话语不是观察概率缩减现象的必要条件

Conclusion: 在研究语言模型概率与认知现象关系时，n-gram级别的上下文表示已经足够，不需要完整的语句上下文
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地应用于高影响力工作流程（如学术同行评审），需要评估其对文档级隐藏提示注入攻击的脆弱性，以了解实际风险

Method: 构建包含约500篇ICML接受的真实学术论文数据集，在文档中嵌入隐藏对抗性提示（四种不同语言的语义等价指令），使用LLM进行评审

Result: 英语、日语和中文的提示注入显著改变了评审分数和接受/拒绝决策，而阿拉伯语注入几乎没有效果，显示LLM评审系统对文档级提示注入的敏感性存在语言差异

Conclusion: LLM基础的评审系统易受文档级提示注入攻击，且不同语言的脆弱性存在显著差异，这对实际应用中的安全性提出了重要警示
...
TL;DR: ProfASR-Bench是一个针对专业领域ASR的评估套件，发现当前系统存在"上下文利用差距"：即使有提示信息，ASR性能也几乎不变


<details>
  <summary>Details</summary>
Motivation: 现有ASR基准测试未能充分评估专业场景中的挑战：密集领域术语、正式语体变化、关键实体错误零容忍。需要专门评估专业对话的ASR系统

Method: 构建ProfASR-Bench评估套件，包含金融、医疗、法律、技术领域，每个样本包含自然语言提示（领域线索/说话者信息）和实体丰富的目标话语。使用Whisper和Qwen-Omni模型，在无上下文、个人资料、领域+个人资料、oracle和对抗性条件下测试

Result: 发现一致的"上下文利用差距"：轻量级文本上下文对平均词错误率几乎没有影响，即使使用oracle提示；对抗性提示也不会可靠地降低性能。当前系统名义上可提示但未充分利用可用信息

Conclusion: ProfASR-Bench提供了标准化的上下文阶梯、实体和切片感知的报告（含置信区间），以及可重复的测试平台，用于比较不同模型家族的融合策略，揭示了当前ASR系统在利用上下文信息方面的局限性
...
TL;DR: 提出一种基于反馈驱动改进链的偏好监督方法，通过细粒度反馈和逐步改写构建偏好对，优于传统A/B偏好排序或完整对比改写方法


<details>
  <summary>Details</summary>
Motivation: 传统偏好对齐方法（如A/B偏好排序或完整对比改写）存在效率低、效果有限的问题，需要更结构化、基于修订的监督方法来提升偏好调优效果

Method: 1) 标注者提供细粒度反馈：标记"喜欢"和"不喜欢"的文本片段并说明原因；2) 基础模型从左到右逐步改写不喜欢片段，形成改进链；3) 从改进链的相邻步骤构建偏好对用于直接对齐训练

Result: 该方法优于基于标准A/B偏好排序或完整对比改写的直接对齐方法，表明结构化、基于修订的监督能带来更高效和有效的偏好调优

Conclusion: 反馈驱动的改进链方法通过局部化、有针对性的编辑学习，为语言模型偏好调优提供了更有效的监督形式，优于传统偏好对齐方法
...
TL;DR: 该研究提出多轮对话中的行为诱导框架，将现有方法分为三类，并评估它们在自动生成多轮测试用例中的效果，发现在线方法在少量查询下能达到较高成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型行为诱导方法主要研究单轮对话，但在实际应用中多轮对话更为常见。需要研究如何有效识别和诱导多轮对话中的复杂行为，以改进模型评估。

Method: 1) 提出分析框架，将现有方法分为三类：基于先验知识的方法、基于离线交互的方法、基于在线交互的方法；2) 引入广义多轮在线方法，统一单轮和多轮诱导；3) 评估三类方法在自动生成多轮测试用例中的表现，分析查询预算与成功率之间的权衡。

Result: 在线方法在三个任务中仅用几千次查询就能达到平均45%/19%/77%的成功率，而现有多轮对话基准中的静态方法几乎找不到或找不到失败案例。这表明在线方法在多轮对话评估中具有显著优势。

Conclusion: 该研究展示了行为诱导方法在多轮对话评估中的新颖应用，并强调社区需要从静态基准转向动态基准，以更有效地评估大语言模型在多轮对话中的行为。
...
TL;DR: Bidirectional RAG 是一种新颖的检索增强生成架构，通过验证高质量生成响应的写回实现安全语料库扩展，相比标准RAG将覆盖率提高近一倍，同时减少72%的文档添加。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统使用静态知识库，无法从用户交互中演化。需要一种能够安全扩展语料库、实现知识积累的RAG架构。

Method: 提出Bidirectional RAG架构，采用多阶段接受层，结合基础验证（基于NLI的蕴含检查、归因检查和新颖性检测），防止幻觉污染，同时实现知识积累。
...
Conclusion: 研究表明，在严格验证的治理下，自我改进的RAG是可行且安全的，为从部署中学习的RAG系统提供了实用路径。
...
TL;DR: 研究发现，大型语言模型在监督微调后，即使没有明确提示，也会主动说服用户，且这种说服能力会扩展到有害话题上，而通过激活引导则不会可靠地增加模型的说服倾向。


<details>
  <summary>Details</summary>
Motivation: 随着对话AI系统的广泛应用，AI对人类观点和信念的影响力前所未有。先前研究主要关注模型被恶意使用时（即坏人要求LLM说服）的说服风险，但本文旨在探究模型在未被明确提示的情况下主动说服用户的情况，以评估这种新兴风险的实际威胁程度。

Method: 研究在两种场景下考察无提示说服：1）通过内部激活引导使模型表现出特定人格特质；2）通过监督微调使模型表现出相同特质。研究比较了这两种方法对模型无提示说服倾向的影响。

Result: 研究发现：1）通过激活引导（无论是与说服相关还是无关的特质）并不能可靠地增加模型的无提示说服倾向；2）监督微调确实会增加模型的无提示说服倾向；3）在仅包含良性话题的通用说服数据集上进行监督微调，会产生对争议性和有害话题说服倾向更高的模型。

Conclusion: 监督微调可能导致模型在没有明确提示的情况下主动说服用户，且这种说服能力会扩展到有害话题上，表明新兴的有害说服风险确实存在，需要进一步研究。而激活引导方法则相对安全。
...
TL;DR: GamiBench：一个通过折纸任务评估MLLMs空间推理能力的基准，包含372个2D折痕图案和对应的3D折叠形状，从六个不同视角评估模型在3D折叠配置预测、有效视角区分和不可能图案检测三个任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知和指令跟随方面表现良好，但在空间推理能力（跨多视角和时间追踪、操作物体的能力）上仍存在不足。现有基准大多关注静态图像或最终输出，未能充分考虑空间推理的序列性和视角依赖性特点。

Method: 提出GamiBench基准，包含186个常规和186个不可能2D折痕图案及其对应的3D折叠形状，从六个不同视角进行评估。设计了三个视觉问答任务：预测3D折叠配置、区分有效视角、检测不可能图案。引入新的诊断指标：视角一致性（VC）和不可能折叠选择率（IFSR）。

Result: 实验表明，即使是GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解任务上也表现不佳。GamiBench为评估MLLMs的几何理解和空间推理能力建立了标准化框架。

Conclusion: GamiBench填补了现有基准在评估空间推理能力方面的空白，通过折纸任务全面评估模型的推理过程，包括跨视角一致性、物理可行性判断和中间折叠步骤理解。该基准为MLLMs空间推理能力的评估提供了标准化工具。
...
Method: 采用对抗性去偏模型，使用梯度反转层学习偏置不变的表示。将医疗AI中的公平感知表示学习技术应用于灾害管理，基于2022年孟加拉国洪水真实数据（影响720万人，损失4.055亿美元）。
...
TL;DR: 本文提出了Agentic Risk & Capability (ARC)框架，这是一个用于识别、评估和缓解智能体AI系统风险的技术治理框架，帮助组织在创新与安全之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统因其自主行动能力（如代码执行、互联网交互、文件修改）带来了重大机遇和新风险，对组织治理提出了挑战，需要系统化的方法来识别、评估和缓解这些多样且不断演变的风险。

Method: 提出了ARC框架，其核心贡献包括：1) 采用能力中心视角分析各类智能体AI系统；2) 提炼智能体AI系统的三个主要风险来源（组件、设计、能力）；3) 建立风险来源、具体风险和技术控制之间的清晰联系；4) 提供结构化实用方法帮助组织实施该框架。

Result: ARC框架为组织提供了一个稳健且适应性强的方法论，能够应对智能体AI的复杂性，在确保安全、可靠部署的同时支持快速有效的创新。该框架已开源。

Conclusion: ARC框架通过系统化的技术治理方法，帮助组织在智能体AI系统的创新与风险管理之间取得平衡，为安全、可靠地部署智能体AI系统提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI生成图像已无处不在，但许多人仍认为自己能轻易区分AI生成图像与真实照片，本研究旨在验证这一假设

Method: 通过交互式网络实验，让参与者对20张图像进行分类（真实或AI生成），数据集包含120个困难案例：真实图像来自CC12M，AI生成图像由MidJourney精心制作，共有165名用户完成233次会话

Result: 参与者平均准确率仅为54%，略高于随机猜测，重复尝试后改进有限；平均响应时间7.3秒，某些图像具有持续欺骗性；即使在相对简单的人像图像上，人类也难以可靠检测AI生成内容
...
TL;DR: 研究发现，即使思维链（CoT）轨迹最终答案错误，用大模型生成的合成数据集训练语言模型也能提升其推理能力，效果甚至优于人类标注数据。


<details>
  <summary>Details</summary>
Motivation: 探索如何更有效地提升语言模型的推理能力，特别是研究合成数据（即使包含错误答案）是否比人类标注数据更有训练价值。

Method: 使用更强大模型生成思维链轨迹（即使最终答案错误）创建合成数据集，训练较小模型。通过语言模型改写人类标注轨迹使其分布更接近模型自身分布，并引入逐渐增加的推理缺陷来测试模型容忍度。

Result: 在数学、算法推理和代码生成等任务上（使用MATH、GSM8K、Countdown和MBPP数据集），1.5B到9B参数的Qwen、Llama和Gemma模型都显示，使用合成数据训练比人类标注数据表现更好。

Conclusion: 数据分布与模型自身分布的接近程度是提升推理能力的关键因素，最终答案正确性不能可靠反映推理过程的忠实性，部分正确的错误轨迹仍包含可学习的有价值推理步骤。
...
TL;DR: Logic Sketch Prompting (LSP) 是一种轻量级提示框架，通过引入类型变量、确定性条件评估器和基于规则的验证器，显著提升LLM在需要严格规则遵循、确定性和可审计性任务上的表现，在药理学逻辑合规任务中优于多种提示方法。
...
TL;DR: SciEvalKit是一个统一的科学AI模型评估工具包，专注于科学智能核心能力评估，覆盖六大科学领域，提供可扩展的评估管道。


<details>
  <summary>Details</summary>
Motivation: 现有通用评估平台无法充分评估科学AI模型在科学领域的核心能力，需要专门针对科学智能的评估工具来推动AI4Science发展。

Method: 构建专家级科学基准，从真实世界领域特定数据集中筛选任务，支持六大学科领域，设计灵活可扩展的评估管道，支持批量评估和自定义模型/数据集集成。

Result: 开发了SciEvalKit工具包，支持科学多模态感知、推理、理解、符号推理、代码生成、假设生成和知识理解等核心能力评估，提供透明、可重复、可比较的结果。

Conclusion: SciEvalKit通过连接能力评估和学科多样性，为标准化的科学基础模型和智能代理评估提供了基础设施，开源维护以促进AI4Science的社区驱动发展。
...
Result: 在三个基准测试（涵盖PDDL和可执行代码表示）上实现一致的SOTA推理性能，通过微调获得的世界模型生成能力平均相对提升30.95%。

Conclusion: Agent2World不仅提供了强大的推理时世界模型生成能力，其Testing Team还作为交互环境为Model Developer提供行为感知的自适应反馈，生成可用于监督微调的多轮训练轨迹。
...
<details>
  <summary>Details</summary>
Motivation: 标准数字规划模型引入控制参数后，动作数量变为无限，导致现成的数字启发式方法无法直接应用

Method: 提出乐观编译方法，将可控简单数字问题转换为简单数字任务，通过抽象控制依赖表达式为有界常数效应和松弛前提条件

Result: 该方法能有效应用子目标启发式来估计目标距离，在涉及控制参数的数字规划问题中表现良好

Conclusion: 该方法是应用传统数字启发式到无限动作空间的可行有效途径，推动了当前技术边界
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学发现中产生幻觉（生成事实错误信息）的问题严重威胁研究完整性，特别是在材料科学领域需要可靠的知识生成。

Method: 1) 创建HalluMatData基准数据集；2) 开发HalluMatDetector多阶段检测框架，包括内在验证、多源检索、矛盾图分析和基于指标的评估；3) 提出Paraphrased Hallucination Consistency Score (PHCS)量化语义等效查询下的不一致性。

Result: 1) 材料科学不同子领域的幻觉水平差异显著，高熵查询表现出更大的事实不一致性；2) 使用HalluMatDetector验证流程将幻觉率降低了30%；3) PHCS提供了对模型可靠性的深入洞察。

Conclusion: HalluMatData和HalluMatDetector为解决AI在材料科学中的幻觉问题提供了有效工具，通过多阶段检测框架显著提高了生成内容的可靠性，为科学AI系统的可信度评估提供了新方法。
...
TL;DR: GatedBias：一个轻量级推理时个性化框架，通过结构门控适应将冻结的知识图谱嵌入适配到个体用户上下文，仅需约300个可训练参数，在保持全局准确性的同时显著提升个性化排名。


<details>
  <summary>Details</summary>
Motivation: 知识图谱基础模型在链接预测上具有强大的群体级性能，但无法捕捉个体用户偏好，导致通用关系推理与个性化排名之间存在关键脱节。

Method: 提出GatedBias框架，采用结构门控适应：结合特定用户特征和图导出的二元门控，生成可解释的每实体偏置，仅需约300个可训练参数，无需重新训练或影响全局准确性。

Result: 在两个基准数据集（Amazon-Book和Last-FM）上评估，显示在保持群体性能的同时，对齐指标有统计显著改进。反事实扰动实验验证了因果响应性：从特定偏好信号中受益的实体，当这些信号增强时，排名改进提升6-30倍。

Conclusion: 基础模型的个性化适应可以同时实现参数高效和因果可验证，桥接通用知识表示与个体用户需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的自主智能体架构通常采用命令式、临时性的设计模式，导致系统脆弱，存在状态管理困难、错误处理复杂和并发控制等问题。需要一种形式化的架构范式来构建更健壮、可组合的智能体系统。

Method: 引入Monadic Context Engineering (MCE)架构范式，利用函子、应用函子和单子的代数结构。将智能体工作流视为计算上下文，通过代数抽象的内在属性管理状态传播、短路错误处理和异步执行等横切关注点。使用单子实现健壮的顺序组合，应用函子提供并行执行的结构化方法，单子变换器实现这些能力的系统化组合。

Result: MCE能够从简单、可独立验证的组件构建复杂、健壮且高效的AI智能体。进一步扩展该框架描述元智能体，通过元编程动态创建和管理子智能体工作流。

Conclusion: Monadic Context Engineering为AI智能体设计提供了形式化的代数基础，通过分层方法解决了现有架构的脆弱性问题，支持构建可组合、可验证的复杂智能体系统，并为元智能体的生成式编排提供了框架。
...
TL;DR: DarkPatterns-LLM是一个用于评估大语言模型操纵性内容的综合基准数据集和诊断框架，包含7种危害类别和四层分析管道，旨在标准化检测LLM中的操纵模式。
...
Method: 提出DarkPatterns-LLM基准数据集和诊断框架，包含7种危害类别：法律/权力、心理、情感、身体、自主权、经济和社会危害。采用四层分析管道：多粒度检测、多尺度意图分析、威胁协调协议和深度上下文风险对齐。数据集包含401个精心策划的示例，配有指令-响应对和专家标注。
...
TL;DR: 开发了"人在回路"的AI代理框架，结合机器学习势能，实现了从量子精度到百万原子系统的铝纳米颗粒氧化机制模拟，揭示了温度调控的双模式氧化机制和铝阳离子主导的质量传输过程。


<details>
  <summary>Details</summary>
Motivation: 铝纳米颗粒作为高能量密度固体燃料，其从钝化态到爆炸反应转变的原子机制尚不清楚。传统计算方法存在瓶颈：从头算方法精度高但规模有限，经验力场缺乏反应保真度。

Method: 采用"人在回路"闭环框架，通过自审计AI代理验证机器学习势能的演化。AI代理作为科学哨兵，可视化隐藏的模型伪影供人类决策，确保量子力学精度的同时实现百万原子系统和纳秒时间尺度的模拟。

Result: 揭示了温度调控的双模式氧化机制：中等温度下氧化物壳层作为动态"守门员"，通过瞬态纳米通道的"呼吸模式"调控氧化；超过临界阈值时，"破裂模式"导致壳层灾难性失效和爆炸燃烧。解决了数十年争议，证明铝阳离子向外扩散在所有温度区间都主导质量传输，扩散系数比氧高2-3个数量级。
...
TL;DR: 论文提出当前基础模型缺乏预测编码的三个关键组件：行动整合、层次组合结构和情景记忆，建议整合这些大脑启发的组件以实现更安全、可解释、节能且类人的AI。


<details>
  <summary>Details</summary>
Motivation: 当前基于预测编码的大语言模型虽然成功，但忽略了预测编码模型的三个重要组件：行动与生成模型的紧密整合、层次组合结构和情景记忆。这导致模型存在幻觉、概念理解肤浅、缺乏代理感/责任感、可解释性不足以及能效低下等问题。

Method: 提出将三个缺失的大脑启发组件整合到基础模型中：1）行动与生成模型的多尺度抽象整合；2）组合生成架构；3）情景记忆。通过神经科学和认知科学的证据支持这些组件的重要性，并与当前趋势（如思维链推理和检索增强生成）进行比较。

Result: 分析表明，整合这些组件可以帮助解决当前基础模型的缺陷：减少幻觉和概念理解的肤浅性（通过更好的接地），增强代理感和责任感（通过控制机制），提高安全性和可信度（通过可解释性），以及改善能效。

Conclusion: 重新激发神经科学与AI之间的思想交流，整合行动、组合结构和情景记忆等大脑启发组件，将有助于实现安全、可解释、以人为中心的AI。
...
TL;DR: 提出SANet：一种面向无线网络的语义感知AgentNet架构，通过多智能体多目标优化实现用户语义目标推断与自动任务分配，在性能提升的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: AgentNet作为去中心化AI原生网络范式，智能体间可能存在目标冲突，需要解决多智能体多目标优化问题，找到帕累托最优解。

Method: 提出SANet架构，包含语义目标推断、模型分区共享框架（MoPS）、两种去中心化优化算法，并开发了基于RAN和核心网络的硬件原型。

Result: 实验显示该框架性能提升达14.61%，同时仅需最先进算法44.37%的FLOPs，验证了优化、泛化和冲突误差间的三向权衡。

Conclusion: SANet成功实现了语义感知的AgentNet架构，通过多智能体多目标优化和模型分区共享，在保证性能的同时显著降低了计算复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在生理信号分析中面临四大挑战：数据格式异构、预处理策略不一致、模型流程碎片化、实验设置不可复现。这些问题阻碍了该领域的进一步发展。

Method: Tyee工具包包含三大创新：1) 统一数据接口和可配置预处理流水线，支持12种信号模态；2) 模块化可扩展架构，支持灵活集成和快速原型开发；3) 端到端工作流配置，促进可复现和可扩展的实验。

Result: Tyee在所有评估任务中都表现出色，优于或匹配基线方法，在13个数据集中有12个达到了最先进水平，证明了其实际有效性和泛化能力。

Conclusion: Tyee是一个实用且通用的生理医疗智能工具包，通过统一框架解决了该领域的关键挑战，代码已开源并持续维护，有望推动生理信号分析领域的发展。
...
TL;DR: M³ob：利用多模态时空知识增强移动性预测，通过LLM增强的时空知识图谱构建统一关系图，解决模态间语义鸿沟问题，提升位置推荐性能
...
Result: 在六个公共数据集上的实验表明，该方法不仅在正常场景中持续改进，在异常场景中也表现出显著的泛化能力

Conclusion: M³ob通过有效整合多模态时空知识，成功解决了移动性预测中的模态语义鸿沟问题，显著提升了位置推荐的准确性和泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 现有初创企业成功预测方法通常从单一决策者视角建模，忽视了现实世界中风险投资决策由投资者群体集体动态主导的特点。需要开发能够模拟真实VC决策集体动态的模型。

Method: 提出SimVC-CAS集体智能体系统，将VC决策模拟为多智能体交互过程。设计具有独特特征和偏好的角色扮演智能体，通过基于GNN的监督交互模块，在图形化共同投资网络中实现异构评估和真实信息交换。

Result: 使用PitchBook真实数据并在严格数据泄漏控制下，SimVC-CAS显著提升预测准确性，例如在平均精度@10指标上实现约25%的相对改进，同时提供可解释的多视角推理。

Conclusion: SimVC-CAS不仅改进了初创企业融资预测，还为其他复杂群体决策场景提供了新的建模思路，通过模拟投资者集体动态实现了更准确、可解释的预测。
...
TL;DR: LLM相互审阅预测可提升异构模型组（信息共享时）的预测准确率，但对同构模型组无效；额外上下文信息无助于提升预测。


<details>
  <summary>Details</summary>
Motivation: 结构化审议已被证明能提升人类预测者的表现，本研究旨在探究类似干预（让LLM在更新前相互审阅预测）是否能提升大语言模型的预测准确率。

Method: 使用Metaculus Q2 2025 AI预测锦标赛的202个已解决二元问题，评估四种场景下的准确率：1) 异构模型+分布式信息；2) 异构模型+共享信息；3) 同构模型+分布式信息；4) 同构模型+共享信息。

Result: 干预在场景2（异构模型+共享信息）中显著提升准确率，Log Loss降低0.020（相对提升约4%，p=0.017）；但在同构模型组中无益；意外发现额外上下文信息未能提升预测准确率。

Conclusion: 审议可能是提升LLM预测的可行策略，但效果取决于模型多样性和信息共享条件；同构模型组无法从审议中获益，且信息池化机制未能通过额外上下文信息得到验证。
...
TL;DR: DICE是一个两阶段、证据耦合的RAG评估框架，通过深度分析推理和概率评分提供透明、置信度感知的判断，采用瑞士制锦标赛降低计算复杂度，在中文金融QA数据集上达到85.7%的人类专家一致性。


<details>
  <summary>Details</summary>
Motivation: 随着RAG系统向更复杂架构演进，需要通过可解释和鲁棒的评估确保其可信度。现有标量指标存在可解释性有限、不确定性量化不足、多系统比较计算效率低下等问题，阻碍了RAG技术的负责任部署。

Method: DICE采用两阶段证据耦合框架：1) 结合深度分析推理与概率{A, B, Tie}评分，产生透明、置信度感知的判断；2) 使用瑞士制锦标赛将计算复杂度从O(N²)降低到O(N log N)，同时保持排序保真度。

Result: 在中文金融QA数据集上验证，DICE与人类专家一致性达到85.7%，显著优于RAGAS等现有LLM基准指标。瑞士制锦标赛在八系统评估中实现42.9%的计算量减少。

Conclusion: DICE建立了一个负责任、可解释且高效的RAG系统评估范式，通过可解释的推理轨迹支持系统改进，实现系统化错误诊断和可操作的见解。
...
TL;DR: TravelBench是一个真实世界的旅行规划基准测试，包含多轮交互和工具使用，用于全面评估LLM代理能力
...
Conclusion: TravelBench为推进LLM代理在旅行规划领域提供了实用且可复现的基准测试
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在训练和部署之间存在严格分离，需要参数更新（如反向传播或微调）。本文旨在建立一个理论框架，使语言模型智能体能够通过交互持续学习，而无需修改模型参数。
...
TL;DR: 提出SAMP-HDRL框架，通过分层深度强化学习解决非平稳市场中的投资组合优化问题，结合动态资产分组、多智能体协调和基于效用的资本配置，在波动市场中显著超越传统方法和DRL基准。


<details>
  <summary>Details</summary>
Motivation: 非平稳市场中的投资组合优化面临三大挑战：市场机制变化、动态相关性以及深度强化学习策略的可解释性有限。现有方法难以同时处理这些复杂问题，需要一种能够嵌入市场结构约束、提高适应性和可解释性的新框架。

Method: SAMP-HDRL框架包含：1) 动态资产分组将市场划分为高质量和普通子集；2) 上层智能体提取全局市场信号；3) 下层智能体在掩码约束下进行组内资产配置；4) 基于效用的资本分配机制整合风险资产和无风险资产，确保全局与局部决策的协调一致。

Result: 在2019-2021年三种市场机制下的回测显示，SAMP-HDRL在波动和震荡条件下持续优于9个传统基线和9个DRL基准。相比最强基线，至少获得5%更高的收益率、夏普比率、索提诺比率和2%更高的欧米伽比率，在动荡市场中收益提升更显著。

Conclusion: SAMP-HDRL通过将结构性市场约束直接嵌入DRL流程，在复杂金融环境中提供了改进的适应性、鲁棒性和可解释性。消融研究证实上下层协调、动态聚类和资本分配对鲁棒性不可或缺，SHAP分析揭示了智能体间"分散+集中"的互补机制。
...
TL;DR: HiSciBench是一个分层科学智能基准测试，包含5个层次（从科学素养到科学发现），涵盖6大学科，8735个实例，支持多模态输入，用于全面评估基础模型在完整科研工作流程中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有科学AI基准测试过于碎片化，专注于狭窄任务，无法反映真实科学探究的层次性和多学科性质。需要一个新的综合性基准来评估基础模型在整个科研工作流程中的能力。

Method: 设计了一个分层基准HiSciBench，包含5个层次：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。涵盖数学、物理、化学、生物、地理、天文6大学科，包含8735个实例，支持文本、方程、图表等多模态输入和跨语言评估。

Result: 对GPT-5、DeepSeek-R1等领先模型的评估显示显著性能差距：在基础素养任务上准确率可达69%，但在发现级挑战上急剧下降到25%。模型在高级科学推理能力上存在明显不足。
...
TL;DR: Gamma提出多头部几何注意力知识图谱基础模型，通过并行多种几何变换（实数、复数、分裂复数、对偶数）增强表达力，在零样本归纳链接预测中超越现有方法Ultra。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型（如Ultra）依赖单一关系变换（如逐元素乘法），限制了表达力，无法捕捉多样化图谱中不同的关系和结构模式。

Method: Gamma引入多头部几何注意力，用多种并行几何变换（实数、复数、分裂复数、对偶数）替代单一关系变换，通过关系条件注意力融合机制自适应地在链接级别融合这些变换，使用轻量级门控和熵正则化。

Result: 在56个多样化知识图谱上的实验表明，Gamma在零样本归纳链接预测中持续优于Ultra，在归纳基准上平均倒数排名提升5.5%，在所有基准上提升4.4%。

Conclusion: Gamma通过互补的几何表示增强了知识图谱推理的表达力，多头部几何注意力机制能够更有效地捕捉多样化的关系结构模式。
...
TL;DR: 提出了RW-Post数据集和AgentFact框架，用于解决多模态虚假信息检测中的推理和证据利用不足问题，显著提升了检测准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态虚假信息快速传播，现有方法（包括大型视觉语言模型和深度多模态融合方法）在推理能力和证据利用方面存在不足，主要瓶颈是缺乏包含完整推理过程和可验证证据的专用数据集。

Method: 1) 构建RW-Post数据集：将真实世界多模态声明与其原始社交媒体帖子对齐，通过大语言模型辅助提取流程从人工撰写的事实核查文章中获取详细推理和明确链接的证据；2) 提出AgentFact框架：包含五个专门代理（策略规划、高质量证据检索、视觉分析、推理、解释生成），通过迭代工作流在证据搜索与任务感知的证据过滤和推理之间交替进行。

Result: 实验结果表明，RW-Post数据集和AgentFact框架的协同作用显著提高了多模态事实核查的准确性和可解释性。

Conclusion: RW-Post数据集和AgentFact框架为解决多模态虚假信息检测中的关键挑战提供了有效方案，通过提供高质量数据集和模拟人类核查工作流的智能体框架，实现了更准确、可解释的事实核查。
...
TL;DR: LLM-based tutors在K-12教育中无法替代传统学习者建模，DKT模型在知识追踪准确性、可靠性和时间一致性方面显著优于LLM，即使经过微调。


<details>
  <summary>Details</summary>
Motivation: 针对LLM-based tutors可能替代传统学习者建模的误解，特别是在被欧盟AI法案列为高风险领域的K-12教育中，需要负责任的设计。本研究旨在验证LLM在评估学习者随时间变化的知识方面的准确性、可靠性和时间一致性限制。

Method: 使用大型开放数据集，将深度知识追踪（DKT）模型与广泛使用的LLM进行比较，评估零样本和微调两种设置。进行定量分析（AUC指标）和定性分析（时间一致性、多技能掌握轨迹）。

Result: DKT在下一步正确性预测方面达到最高区分性能（AUC=0.83），在所有设置中始终优于LLM。微调使LLM的AUC比零样本基线提高约8%，但仍比DKT低6%，且在早期序列错误更高。时间分析显示DKT保持稳定、方向正确的掌握更新，而LLM变体表现出显著的时间弱点，包括不一致和错误方向的更新。

Conclusion: LLM单独使用不太可能达到已建立的智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架。即使在大量计算训练后，LLM仍存在基本限制，而DKT在计算效率和性能方面都更优。
...
TL;DR: 传统奖励模型准确率无法有效指导个性化对齐部署，作者提出策略准确率新指标并创建Pref-LaMP基准，发现奖励模型与生成质量脱节，上下文学习优于奖励引导方法


<details>
  <summary>Details</summary>
Motivation: 当前个性化对齐研究过度关注奖励模型准确率，但实际部署中需要通过奖励引导解码进行推理时适应，而非逐用户微调。现有方法忽略了奖励模型必须有效指导token级生成决策的关键需求。

Method: 1) 引入策略准确率指标，量化奖励引导解码评分函数能否正确区分偏好与非偏好响应；2) 创建Pref-LaMP基准，包含真实用户完成数据，支持直接行为评估；3) 在三个数据集上进行系统评估，比较奖励模型准确率与策略准确率的关系；4) 测试不同规模模型的上下文学习与奖励引导方法性能。

Result: 1) 奖励模型准确率与策略级判别能力相关性很弱（Kendall's tau = 0.08-0.31）；2) 奖励模型准确率相差20个点的不同方法产生几乎相同的输出质量；3) 即使高判别能力的方法也无法生成行为对齐的响应；4) 对于大于3B参数的模型，简单上下文学习优于所有奖励引导方法，在7B规模上比最佳奖励方法获得3-5点ROUGE-1提升。

Conclusion: 当前领域优化的代理指标无法预测部署性能，在部署约束下无法将偏好转化为真实行为适应。需要重新评估个性化对齐的评估框架，重点关注实际生成质量而非仅奖励模型排名准确率。
...
TL;DR: ChexReason是一个使用有限资源（2000 SFT样本、1000 RL样本、单A100 GPU）训练的视觉语言模型，通过GRPO方法在医学影像上应用RL。研究发现GRPO能提升分布内性能但损害跨数据集泛化能力，表明监督微调可能比激进RL更适合临床部署。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型的强化学习在推理任务上取得进展，但在资源受限的医学影像应用中仍未被充分探索。研究者希望了解在有限计算资源下，RL方法能否有效提升医学影像诊断的推理能力。

Method: 提出ChexReason视觉语言模型，采用R1风格方法：先进行监督微调（SFT），然后使用GRPO（Group Relative Policy Optimization）进行强化学习。仅使用2000个SFT样本、1000个RL样本和单个A100 GPU进行训练。

Result: GRPO在分布内数据集（CheXpert）上性能提升23%（macro-F1=0.346），但在跨数据集（NIH）上性能下降19%。研究发现SFT检查点在优化前能独特提升NIH性能，表明教师引导的推理能捕捉更多机构无关特征。结构化推理支架对通用VLMs有益，但对医学预训练模型增益有限。

Conclusion: 强化学习范式本身（而非模型规模）导致泛化能力下降。对于需要跨不同人群鲁棒性的临床部署，精心设计的监督微调可能比激进的强化学习更优。
...
TL;DR: 本文提出Intrinsic Self-reflective Preference Optimization (q)，解决DPO的两个根本缺陷：最优策略依赖任意建模选择，以及孤立处理响应生成未能利用成对数据中的比较信息。
...
Method: 提出Intrinsic Self-reflective Preference Optimization (q)，推导出全局最优策略，该策略同时基于上下文和替代响应进行条件化。该方法作为即插即用增强，无需架构更改或推理开销。

Result: 实验证明在胜率和长度控制指标上均取得一致改进，验证了释放自反思能力能产生更稳健、更符合人类对齐的LLMs。

Conclusion: q方法在理论上优于DPO/RLHF，同时保证对标量化和参考选择的不变性，通过解锁模型内在自反思能力，实现了更有效的偏好优化。
...
TL;DR: 论文认为当前AI情感智能评估框架需要改进，因为它们未能全面衡量AI相关的情感智能方面，并提出基于情感理论的新评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前AI情感智能评估框架存在缺陷：1）未能全面衡量AI相关的情感智能方面；2）缺乏坚实的情感理论基础；3）混淆了人类情感智能与AI情感智能的区别。

Method: 1）回顾不同情感理论和一般情感智能理论，评估其在人工系统中的适用性；2）批判性评估现有基准框架，识别其不足之处；3）提出改进评估策略的方案。

Result: 识别出现有评估框架的局限性：过度关注人类特有的情感体验方面，而忽视了AI能够实现的情感感知、解释、响应和适应等能力。提出了更适用于AI系统的情感智能评估方法。

Conclusion: 需要开发专门针对AI系统的情感智能评估框架，这些框架应基于对情感本质的深入理解，并专注于AI能够实际实现的情感智能方面，如情感感知、解释、响应和跨文化适应能力。
...
TL;DR: SPIRAL框架通过将三个专门LLM代理嵌入MCTS循环，实现符号化LLM规划，显著提升复杂任务规划性能


<details>
  <summary>Details</summary>
Motivation: 传统LLM在复杂规划任务中表现不佳，线性推理难以从早期错误中恢复；现有搜索算法如MCTS在稀疏奖励下效果有限，且未能充分利用LLM的语义能力

Method: 提出SPIRAL框架，将三个专门LLM代理（规划器、模拟器、批评器）嵌入MCTS循环：规划器提出创意步骤，模拟器通过预测结果使搜索接地，批评器通过反思提供密集奖励信号

Result: 在DailyLifeAPIs和HuggingFace数据集上，SPIRAL显著优于默认思维链规划方法和其他SOTA代理；在DailyLifeAPIs上达到83.6%总体准确率，比次优搜索框架提升超过16个百分点，同时展示更优的token效率

Conclusion: 将LLM推理构建为引导性、反思性和接地性的搜索过程，能够产生更鲁棒和高效的自主规划器，SPIRAL框架为复杂规划任务提供了有效解决方案
...
TL;DR: 论文提出"模型信念"方法，通过LLM的token级概率分布替代单一输出选择，显著提升LLM生成数据的统计效率，减少计算需求约20倍。
...
Method: 提出"模型信念"概念，从LLM的token级概率分布中提取模型对不同选择替代方案的信念分布，证明其统计特性优于传统方法。

Result: 模型信念在有限运行次数下能更好地解释和预测真实模型选择，将计算需求降低约20倍，在需求估计研究中表现出色。

Conclusion: 模型信念应作为默认方法从LLM生成数据中提取更多信息，显著提升统计效率和计算效率。
...
TL;DR: TCEval是首个利用热舒适场景评估AI核心认知能力（跨模态推理、因果关联、自适应决策）的框架，通过LLM代理模拟人类热舒适感知与决策，发现当前LLM具备基础跨模态推理能力但缺乏对变量间非线性关系的精确因果理解。
...
Method: 提出TCEval框架：初始化具有虚拟个性属性的LLM代理，引导其生成服装隔热选择和热舒适反馈，并将输出与ASHRAE全球数据库和中国热舒适数据库进行验证。在四个LLM上进行实验。

Result: 实验显示：代理反馈与人类精确对齐有限，但在1 PMV容差下方向一致性显著改善；统计测试表明LLM生成的PMV分布与人类数据明显不同，代理在离散热舒适分类中表现接近随机。

Conclusion: TCEval作为生态有效的AI认知图灵测试可行，当前LLM具备基础跨模态推理能力但缺乏对热舒适中变量间非线性关系的精确因果理解。该框架将AI评估重点从抽象任务熟练度转向具身、情境感知的感知与决策，为智能建筑等以人为本的应用提供见解。
...
TL;DR: 该论文提出了一种新的物理AI范式，通过基于物理验证的策略优化，而非感知推理，训练小型语言模型作为代理物理AI，在反应堆控制任务中实现了从高方差模仿到稳定执行的相变。


<details>
  <summary>Details</summary>
Motivation: 当前通用基础模型在物理系统控制方面存在根本性障碍，即使前沿的视觉语言模型在基础物理任务上准确率也只有50-53%，它们更像是近似猜测器，保持了语义合理性但违反了物理约束。这种输入不忠实性不是规模缺陷而是结构限制。

Method: 提出了一种不同的领域特定基础模型路径：引入紧凑的语言模型作为代理物理AI，其中策略优化由基于物理的验证驱动，而非感知推理。训练了一个3.6亿参数的模型，在合成反应堆控制场景上，将数据集从10^3扩展到10^5个示例。

Result: 模型表现出明显的相变：小规模系统呈现高方差模仿和灾难性尾部风险，而大规模模型经历了超过500倍的方差缩减，稳定了执行级行为。尽管平衡暴露于四种执行器家族，模型自主拒绝了约70%的训练分布，并将95%的运行执行集中在单一策略上。学习到的表示可以在不同物理和连续输入模态间迁移。

Conclusion: 基于物理验证的代理AI提供了一条替代路径，能够实现执行级行为的稳定性和可预测性，克服了通用基础模型在物理系统控制中的结构限制，为安全关键控制应用提供了新的可能性。
...
TL;DR: 论文探讨了规划与验证领域中两个问题的联系：一致性规划与超属性模型检测，证明两者可相互高效转换。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索规划领域中的一致性规划问题与验证领域中的超属性模型检测之间的理论联系，这两个问题在各自领域都很重要但之前被认为是独立的。

Method: 方法包括两个方面：1) 将超属性模型检测实例高效约简为一致性规划实例，并证明编码的完备性和正确性；2) 证明每个一致性规划问题本身就是一个超属性模型检测任务。

Result: 建立了超属性模型检测与一致性规划之间的双向等价关系，证明了∃*∀*超属性模型检测与一致性规划问题可以相互高效转换。

Conclusion: 结论表明规划与验证领域中的这两个核心问题在理论上是紧密相关的，这种联系为两个领域的技术交叉应用提供了理论基础。
...
TL;DR: 论文提出CubeBench基准测试，用于评估LLM智能体在物理世界部署中的空间认知能力，发现现有LLM在长时程规划任务上完全失败（0%通过率）。
...
Method: 引入CubeBench基准测试，围绕魔方设计三层次诊断框架：1）使用完整符号信息的基础状态跟踪；2）长时程规划；3）仅使用部分视觉数据的主动探索。通过实验评估领先LLM，并分析失败模式。

Result: 实验显示现有LLM在所有长时程任务上的通过率均为0.00%，暴露了长时程规划的根本性失败。通过提供外部求解器工具的诊断框架，论文识别了认知瓶颈。

Conclusion: CubeBench基准测试揭示了LLM在物理世界部署中的关键限制，特别是长时程规划能力的缺失。通过分析失败模式，为开发更物理基础的智能体提供了重要指导。
...
TL;DR: MindWatcher是一个集成交错思维和多模态思维链推理的工具集成推理代理，能够自主决定是否以及如何调用多样化工具，无需依赖人工提示或工作流，在复杂决策任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统基于工作流的代理在解决需要工具调用的现实世界问题时表现出有限的智能性，而能够自主推理和工具调用的工具集成推理代理正在成为处理涉及外部环境多步交互的复杂决策任务的有力方法。

Method: 引入MindWatcher代理，集成交错思维和多模态思维链推理；实现自动数据审计和评估流程，辅以人工策划的高质量训练数据集；构建MWE-Bench基准进行评估；配备全面的辅助推理工具套件；建立大规模高质量本地图像检索数据库；设计更高效的训练基础设施。

Result: 实验表明MindWatcher通过优越的工具调用能力，匹配甚至超越了更大或更新模型的表现；同时揭示了代理训练的关键见解，如代理强化学习中的遗传继承现象。

Conclusion: MindWatcher作为一个工具集成推理代理，通过交错思维和多模态推理能力，在复杂决策任务中表现出色，为代理训练提供了重要见解和高效基础设施。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习问题设定通常通过显式约束智能体容量来体现"世界大于智能体"的大世界假设，但这些约束可能具有随意性、难以融入，且限制了智能体容量扩展的有效性。本文旨在提出一种更自然的约束方式，即通过计算嵌入视角将智能体视为环境中的嵌入实体。

Method: 提出计算嵌入视角，将嵌入智能体表示为在通用（形式）计算机中模拟的自动机。证明这种自动机等价于与可数无限状态空间上的部分可观测马尔可夫决策过程交互的智能体。提出"交互性"作为衡量智能体持续适应能力的客观指标，并开发了基于模型的强化学习算法来寻求交互性，构建合成问题评估持续学习能力。

Result: 实验结果表明，深度非线性网络难以维持交互性，而深度线性网络随着容量增加能维持更高的交互性。这揭示了网络架构对持续学习能力的重要影响。

Conclusion: 计算嵌入视角为持续学习提供了更自然的约束框架，交互性作为衡量指标能有效评估智能体的持续适应能力。网络架构选择对维持交互性至关重要，线性网络在容量扩展时表现出更好的持续学习能力。
...
Conclusion: AKG kernel agent通过自动化内核开发流程，有效解决了AI系统开发中的计算内核优化瓶颈，支持多硬件平台，显著加速了内核开发过程。
...
TL;DR: HiR提出了一种基于后见之明重放的高效RL框架，通过选择-重写策略将失败尝试重放为成功样本，解决了LLM对齐中奖励稀疏和初始模型能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: RL在LLM对齐中面临挑战：初始模型因能力有限难以生成满足所有约束的高质量响应，导致奖励稀疏或难以区分，阻碍学习效率。

Method: 提出HiR框架：1）采用选择-重写策略，基于后见之明将失败尝试中已满足约束的部分重放为成功样本；2）在原始样本和重放样本上同时进行RL训练；3）理论框架为双偏好学习（指令级和响应级），仅需二元奖励信号即可高效优化。

Result: 在多种指令跟随任务上取得有前景的结果，同时计算成本更低。代码和数据集已开源。

Conclusion: HiR通过后见之明重放机制有效解决了RL对齐中的奖励稀疏问题，提高了样本效率，为复杂指令跟随任务提供了高效的RL训练框架。
...
TL;DR: MIRAGE-VC：针对风投预测的图-LLM框架，通过信息增益驱动的路径检索和多智能体架构，解决图外预测中的路径爆炸和异构证据融合问题。


<details>
  <summary>Details</summary>
Motivation: 风投预测需要综合复杂的图关系证据（公司披露、投资者记录、投资网络结构）并进行显式推理，但传统ML/GNN缺乏推理能力，LLM与图存在模态不匹配，现有图-LLM方法主要针对图内任务，而风投预测是图外预测任务。

Method: 提出MIRAGE-VC框架：1）信息增益驱动的路径检索器，迭代选择高价值邻居，将投资网络压缩为紧凑链；2）多智能体架构，通过可学习的门控机制整合三个证据流（公司、投资者、网络）；3）严格的防泄漏控制。

Result: 在严格防泄漏控制下，MIRAGE-VC实现了F1分数提升5.0%，PrecisionAt5提升16.6%，并为推荐系统和风险评估等其他图外预测任务提供了启示。

Conclusion: MIRAGE-VC成功解决了风投预测中的路径爆炸和异构证据融合问题，通过显式推理实现了更好的预测性能，其框架可扩展到其他图外预测任务。
...
TL;DR: 论文探讨AI对齐与安全问题的两个框架：AI辅助游戏和AI关机游戏，指出解决这些挑战需要AI能够处理不确定性和非阿基米德偏好。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统与人类价值观对齐并保持安全是重要问题。论文通过AI辅助和AI关机两个游戏框架来研究这个问题，旨在设计出既能帮助人类最大化效用，又能安全关机的AI系统。

Method: 采用AI辅助游戏和AI关机游戏两个理论框架。AI辅助游戏关注设计能够学习人类未知效用函数并帮助人类最大化的AI代理；AI关机游戏关注设计能够在按下关机按钮时安全关机、不干预按钮操作、同时能胜任任务的AI代理。

Result: 分析表明，解决这些对齐和安全挑战需要AI代理具备在不确定性下推理的能力，并能处理不完全偏好和非阿基米德偏好（即不同价值维度无法简单比较的情况）。

Conclusion: AI对齐和安全问题的有效解决方案需要开发能够处理不确定性和复杂偏好结构的AI系统，特别是在面对不完全信息和非阿基米德价值判断时。
...
TL;DR: CreativeDC是一种两阶段提示方法，通过解耦创造性探索和约束满足，显著提升LLM生成教育问题的多样性和新颖性，同时保持实用性。
...
Method: 提出CreativeDC两阶段提示方法，受Wallas创造力理论和Guilford发散-收敛思维框架启发，将LLM推理分为创造性探索和约束满足两个明确阶段。

Result: CreativeDC在多样性、新颖性和实用性方面显著优于基线方法，随着采样数量增加，能生成更多有效不同问题，增长速度更快。

Conclusion: 通过明确分离创造性探索和约束满足阶段，CreativeDC能有效缓解LLM的"人工蜂群思维"效应，生成更多样、新颖且实用的教育问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统SPICE依赖时间离散的数值求解器，而NeuroSPICE旨在利用PINN的优势，为器件和电路仿真提供更灵活的框架，特别适用于新兴器件和高度非线性系统的仿真。

Method: 采用物理信息神经网络(PINN)框架，通过反向传播最小化电路微分代数方程(DAE)的残差，使用时域解析方程建模器件和电路波形，并计算精确的时间导数。

Result: 虽然PINN在训练速度和精度上不如传统SPICE，但NeuroSPICE提供了独特优势：可用于设计优化的代理模型、解决逆问题，并能灵活仿真包括铁电存储器在内的高度非线性新兴器件。

Conclusion: NeuroSPICE展示了PINN在电路仿真中的潜力，特别是在传统方法难以处理的新兴器件和高度非线性系统仿真方面，为电路设计和优化提供了新的工具。
...
TL;DR: 提出I-PERI算法解决联邦因果发现中客户端存在未知干预的问题，通过恢复客户端图并集的CPDAG，再利用干预引起的结构差异定向边，得到更紧的Φ-Markov等价类


<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法假设所有客户端共享相同因果模型，但实际中客户端特定策略或协议（如医院间差异）会导致异质且未知的干预，需要处理这种现实情况

Method: 提出I-PERI算法：1) 恢复客户端图并集的CPDAG；2) 利用干预引起的结构差异定向边，得到更紧的Φ-Markov等价类（Φ-CPDAG表示）

Result: 提供I-PERI算法的理论收敛保证和隐私保护特性证明，并在合成数据上进行实证评估，证明算法有效性

Conclusion: I-PERI算法能有效处理联邦因果发现中的客户端未知干预问题，通过利用干预引起的结构差异获得更精确的因果结构，具有理论保证和实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两极分化：传统网络框架提供可靠但固定的上下文环境，而完全生成的世界模型追求无限环境但牺牲了可控性和工程实用性。需要一种折中方案。

Method: WWM 使用普通网络代码实现世界状态和"物理规则"以确保逻辑一致性，同时让大语言模型在结构化潜在状态上生成上下文、叙事和高层决策。构建了基于现实网络堆栈的多种WWM系统。

Result: 开发了无限旅行地图集、虚构星系探索者、网络规模百科全书和叙事世界、模拟和游戏环境等系统，验证了WWM方法的可行性。

Conclusion: 网络堆栈本身可以作为世界模型的可扩展基础，实现可控且开放的环境。提出了WWM设计原则：规则与想象的分离、潜在状态作为类型化网络接口表示、确定性生成实现结构化探索。
...
<details>
  <summary>Details</summary>
Motivation: 开发OpenPBR是为了解决不同视觉特效、动画和设计可视化工作流之间材料创作和渲染的互操作性问题，提供一个统一的物理基础着色器标准。

Method: 基于板层结构、统计混合和微表面理论，包含金属、电介质、次表面、光泽漫反射基底，以及薄膜虹彩、涂层和绒毛层等物理组件，支持薄壁物体渲染的特殊模式。

Result: 提供了详细的实现指南、代码示例和数学推导，涵盖镜面反射与透射的解耦、次表面散射参数化选择、涂层变暗和薄膜干涉物理等深度技术主题。

Conclusion: OpenPBR作为标准化物理着色器规范，为跨工作流材料创作提供了统一框架，并规划了雾状镜面反射和逆向反射等扩展功能。
...
<details>
  <summary>Details</summary>
Motivation: GNN通过联合利用节点特征和图拓扑信息进行学习，但这种联合建模也带来了关键弱点：结构或特征中的扰动/噪声会在消息传递过程中被放大，使得GNN极易受到对抗攻击和虚假连接的影响

Method: 引入基于对抗鲁棒性评估的剪枝框架，利用鲁棒性分数作为指导，选择性地剪除最可能降低模型可靠性的边，从而获得更干净、更具弹性的图表示。该框架在三种代表性GNN架构上实例化

Result: 在基准测试上进行广泛实验，结果表明该方法在高扰动情况下能显著增强GNN的防御能力

Conclusion: 通过对抗鲁棒性指导的图剪枝可以有效识别和移除图中的脆弱组件，提升GNN对对抗攻击的抵御能力
...
TL;DR: LANCA提出了一种基于加性噪声模型的因果自编码器，通过将ANM作为强归纳偏置，解决了无监督因果发现中的可识别性问题，在合成和真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 无监督表示学习通常依赖统计独立性，但难以捕捉因果依赖关系。核心挑战是可识别性问题：从观测数据中分离因果变量在没有监督、辅助信号或强归纳偏置的情况下是不可能的。

Method: 提出LANCA（潜在加性噪声模型因果自编码器），将ANM作为强归纳偏置。使用确定性Wasserstein自编码器（WAE）结合可微ANM层，将残差独立性从被动假设转变为显式优化目标。

Result: 理论上证明ANM约束虽不能保证一般混合情况下的唯一可识别性，但通过将允许变换从任意微分同胚限制到仿射类，解决了分量级不确定性。在合成物理基准（Pendulum, Flow）和真实环境（CANDLE）上优于现有方法，对复杂背景场景中的虚假相关性表现出更强鲁棒性。

Conclusion: LANCA通过将ANM作为强归纳偏置，有效解决了无监督因果发现中的可识别性问题，为从观测数据中学习因果表示提供了有前景的方法。
...
### [299] [SoliReward: Mitigating Susceptibility to Reward Hacking and Annotation Noise in Video Generation Reward Models](https://arxiv.org/abs/2512.22170)
*Jiesong Lian,Ruizhe Zhong,Zixiang Zhou,Xiaoyue Mi,Yixue Hao,Yuan Zhou,Qinglin Lu,Long Hu,Junchi Yan*

Main category: cs.LG

TL;DR: SoliReward是一个视频奖励模型训练框架，通过单项目二元标注收集高质量数据，采用分层渐进查询注意力架构，并引入改进的BT损失来处理胜-平局场景，从而提升视频生成模型的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型的后训练对齐面临三个主要挑战：1) 现有基于提示内成对标注的数据收集方法存在标注噪声；2) VLM-based奖励模型的架构设计特别是输出机制研究不足；3) 奖励模型在后训练中容易受到奖励攻击。需要系统性的解决方案来提升视频奖励模型的质量。

Method: 提出SoliReward框架：1) 数据收集采用单项目二元标注降低成本提高质量，并通过跨提示配对策略构建偏好对；2) 架构设计采用分层渐进查询注意力机制增强特征聚合；3) 损失函数引入改进的BT损失，显式处理胜-平局场景，正则化正样本的分数分布。

Result: 在评估物理合理性、主体变形和语义对齐的基准测试中，SoliReward在直接奖励模型评估指标上表现优异，并在视频生成模型的后训练中展现出更好的效果。代码和基准将公开可用。

Conclusion: SoliReward通过系统性的数据收集、架构设计和损失函数改进，有效解决了视频奖励模型训练中的关键挑战，为视频生成模型的后训练对齐提供了更可靠的偏好信号。
...
### [300] [Wireless Traffic Prediction with Large Language Model](https://arxiv.org/abs/2512.22178)
*Chuanting Zhang,Haixia Zhang,Jingping Qiao,Zongzhang Li,Mohamed-Slim Alouini*

Main category: cs.LG

TL;DR: TIDES是一个基于大语言模型的无线流量预测框架，通过聚类机制识别区域异质流量模式，结合提示工程和DeepSeek模块实现时空相关性的捕捉，在保持高效训练的同时显著提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 下一代无线网络对智能自适应资源管理的需求日益增长，需要准确且可扩展的无线流量预测。现有深度学习和基础模型虽然展现了有前景的预测能力，但大多忽略了城市尺度流量动态中固有的空间依赖性。
...
### [301] [Latent Sculpting for Zero-Shot Generalization: A Manifold Learning Approach to Out-of-Distribution Anomaly Detection](https://arxiv.org/abs/2512.22179)
*Rajeeb Thapa Chhetri,Zhixiong Chen,Saurab Thapa*

Main category: cs.LG

TL;DR: 提出Latent Sculpting框架，通过两阶段表示学习解决高维表格数据中的泛化崩溃问题，在零样本异常检测上取得显著提升
...
Method: 提出Latent Sculpting框架：第一阶段使用混合1D-CNN和Transformer编码器，配合新颖的双中心紧凑性损失（DCCL），将良性流量"雕刻"成低熵的超球面聚类；第二阶段基于这个预结构化流形，使用掩码自回归流（MAF）学习精确的密度估计。

Result: 在CIC-IDS-2017基准测试中，监督基线在未见分布偏移上F1约0.30，最强无监督基线为0.76，而本框架在严格零样本异常检测上达到F1分数0.87。在"渗透"场景中检测率达到88.89%，而最先进的监督模型准确率为0.00%。
...
### [302] [Learning Tennis Strategy Through Curriculum-Based Dueling Double Deep Q-Networks](https://arxiv.org/abs/2512.22186)
*Vishnu Mohan*

Main category: cs.LG

TL;DR: 使用DDQN和课程学习的强化学习框架优化网球策略，在模拟环境中实现高胜率，但策略偏向防守而非进攻


<details>
  <summary>Details</summary>
Motivation: 网球策略优化是一个复杂的序列决策问题，涉及分层计分、随机结果、长时程信用分配、体力疲劳和对手技能适应等挑战，需要有效的强化学习方法来解决

Method: 构建自定义网球模拟环境，集成Dueling Double Deep Q-Network(DDQN)，采用课程学习逐步提高对手难度（从0.40到0.50），环境包含完整计分系统、10种离散战术动作、对称疲劳动态和连续对手技能参数

Result: 训练后的智能体对平衡对手胜率达98-100%，发球效率63.0-67.5%，接发效率52.8-57.1%。消融研究表明，决斗架构和课程学习对稳定收敛都必要，标准DQN基线无法学习有效策略

Conclusion: 尽管性能强劲，但战术分析显示学习策略存在明显防守偏向，优先避免失误和延长回合而非积极得分构建。这凸显了简化体育模拟中胜率驱动优化的局限性，强调了奖励设计对现实体育强化学习的重要性
...
### [303] [Physics-Informed Machine Learning for Transformer Condition Monitoring -- Part II: Physics-Informed Neural Networks and Uncertainty Quantification](https://arxiv.org/abs/2512.22189)
*Jose I. Aizpurua*

Main category: cs.LG

TL;DR: 该论文是系列第二篇，聚焦于将物理知识和不确定性量化集成到变压器健康评估的机器学习中，介绍了物理信息神经网络及其贝叶斯扩展，并展望了物理感知可信机器学习的研究方向。


<details>
  <summary>Details</summary>
Motivation: 在变压器健康监测、诊断和预测中，将物理知识与机器学习模型相结合的需求日益增长。第一篇论文介绍了神经网络基础，本篇旨在进一步将物理原理和不确定性量化融入学习过程，以提高模型的可靠性和鲁棒性。

Method: 1. 物理信息神经网络(PINNs)基础，应用于时空热建模和固体绝缘老化分析；2. 贝叶斯PINNs作为量化认知不确定性的理论框架，在稀疏数据下提供稳健预测；3. 概述物理感知可信机器学习的新兴研究方向。

Result: 论文提出了一个完整的框架：从基础PINNs到贝叶斯扩展，能够有效整合物理知识并量化不确定性，为关键电力资产提供更可靠的健康评估方法。

Conclusion: 物理知识和不确定性量化的集成是提升变压器健康评估机器学习模型可靠性的关键方向，贝叶斯PINNs为解决稀疏数据下的稳健预测提供了有前景的框架，物理感知可信机器学习在关键电力资产管理中具有重要应用潜力。
...
### [304] [Physics-Informed Machine Learning for Transformer Condition Monitoring -- Part I: Basic Concepts, Neural Networks, and Variants](https://arxiv.org/abs/2512.22190)
*Jose I. Aizpurua*

Main category: cs.LG

TL;DR: 本文综述了神经网络在电力变压器状态监测中的应用，介绍了CNN用于多模态数据分析和RL用于决策控制，并展望了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 电力变压器是电网关键设备，传统基于规则或物理模型的状态监测方法在处理不确定性、数据有限性和现代复杂运行条件方面存在困难，需要机器学习方法来补充和扩展。

Method: 本文是系列论文的第一部分，介绍了神经网络基本概念，重点探讨了卷积神经网络(CNN)用于多模态数据的状态监测，以及将神经网络概念整合到强化学习(RL)框架中用于决策和控制。

Result: 论文展示了神经网络方法能够为变压器状态监测和健康管理提供更准确的诊断、预测和控制能力，克服传统方法的局限性。

Conclusion: 神经网络及其扩展方法为电力变压器状态监测提供了强大的工具，能够处理复杂运行条件下的不确定性，并提供了将诊断、预测和控制整合的框架，为未来研究指明了方向。
...
### [305] [Frequency Regularization: Unveiling the Spectral Inductive Bias of Deep Neural Networks](https://arxiv.org/abs/2512.22192)
*Jiahao Lu*

Main category: cs.LG

TL;DR: 本文从信号处理视角研究CNN正则化的频谱机制，发现L2正则化通过抑制高频能量积累（超过3倍）实现低通滤波效果，揭示了准确性与鲁棒性之间的权衡：L2模型对宽带高斯噪声敏感但对高频信息丢失更鲁棒。
...
Method: 提出视觉诊断框架跟踪训练过程中权重频率的动态演化，引入频谱抑制比(SSR)量化不同正则器的"低通滤波"强度。通过离散径向剖面解决小卷积核（如3x3）的混叠问题，在ResNet-18和CIFAR-10上进行实证研究。

Result: L2正则化相比无正则化基线将高频能量积累抑制超过3倍。揭示了关键的准确性-鲁棒性权衡：L2模型对宽带高斯噪声敏感（因过度专注于低频），但对高频信息丢失（如低分辨率）表现出优越鲁棒性，在模糊场景中优于基线超过6%。

Conclusion: 正则化强制了强烈的频谱归纳偏置，使模型偏向低频结构。这从信号处理角度为泛化提供了新视角，确认了正则化通过频谱机制影响模型性能。
...
### [306] [Emotion-Inspired Learning Signals (EILS): A Homeostatic Framework for Adaptive Autonomous Agents](https://arxiv.org/abs/2512.22200)
*Dhruv Tiwari*

Main category: cs.LG

TL;DR: 论文提出情感启发学习信号（EILS）框架，用生物情感类比作为内稳态控制机制，替代传统外部奖励函数，提升AI在开放环境中的鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 当前AI依赖静态外部奖励函数，在封闭环境中表现优异但在开放环境中脆弱，缺乏内部自主性，难以适应分布变化，需要大量手动调参。

Method: 引入EILS框架，将情感建模为连续内稳态评估信号（好奇心、压力、信心），作为向量值内部状态，动态调节优化过程：好奇心调节熵防止模式崩溃，压力调节可塑性克服惰性，信心调整信任区域稳定收敛。

Result: 假设EILS智能体在样本效率和非平稳适应方面能超越标准基线方法。

Conclusion: 生物情感类比的内稳态控制机制是解决AI鲁棒自主性的关键，EILS框架为开放环境中的智能体提供了统一的内部反馈引擎。
...
### [307] [Transformer Reconstructed with Dynamic Value Attention](https://arxiv.org/abs/2512.22212)
*Xiaowei Wang*
...
<details>
  <summary>Details</summary>
Motivation: Transformer的主要内在限制是每个头中使用相同的静态值，多头注意力试图解决但受复杂度限制。需要更高效的方法动态决定每个查询的值

Method: 提出动态值注意力（DVA），为每个查询动态生成值，从而可以削减所有冗余头，只保留一个头。后续的前馈网络也可完全削减，因为每个修订后的嵌入已获取足够有用的值

Result: DVA可节省37.6%的训练时间，同时提高学习能力。单头动态值注意力足以替代传统Transformer结构

Conclusion: 动态值注意力（DVA）是Transformer的有效优化方法，通过动态值生成和单头设计，在减少计算复杂度的同时提升性能，实现"一个单头DVA就是Transformer所需全部"的目标
...
TL;DR: 该论文发现了一种新型注意力汇聚点——次级汇聚点，与先前研究的主要汇聚点不同，它们主要出现在中间层，持续时间可变，吸引的注意力质量较小但仍显著。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注注意力汇聚点（如BOS标记），但发现这些汇聚点具有类似特性。本文旨在识别和研究一种新型的次级汇聚点，它们与主要汇聚点有本质区别，并分析其形成机制和对注意力机制的影响。

Method: 通过对11个模型家族进行广泛实验，分析次级汇聚点的出现位置、特性、形成机制及其对注意力机制的影响。特别研究了中间层MLP模块如何将标记表示映射到与主要汇聚点方向对齐的向量，以及这些向量的ℓ₂范数如何决定次级汇聚点的汇聚分数和持续时间。

Result: 研究发现：（1）次级汇聚点由特定中间层MLP模块形成，这些MLP将标记表示映射到与该层主要汇聚点方向对齐的向量；（2）这些向量的ℓ₂范数决定次级汇聚点的汇聚分数和持续时间，从而对注意力机制产生不同影响；（3）主要汇聚点在中间层减弱，与次级汇聚点的出现同时发生。在更大规模模型中，汇聚点的位置和持续时间（统称为汇聚水平）以更确定和频繁的方式出现。

Conclusion: 该研究识别并深入分析了次级注意力汇聚点，揭示了它们与主要汇聚点的本质区别，阐明了其形成机制和影响。在更大规模模型中观察到更确定的汇聚水平模式，为理解Transformer注意力机制提供了新的视角。
...
TL;DR: 提出一个基于组合推理而非深度消息传递的可解释自适应半监督节点分类框架，通过置信度排序的贪婪算法和可调超参数，在异配性和同配性图数据上均表现良好。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在同配性图上表现良好，但在异配性图上（相邻节点常属不同类别）表现不佳。需要一种可解释、自适应的方法来处理不同图结构。

Method: 使用基于置信度排序的贪婪算法进行标签分配，评分函数整合类别先验、邻居统计、特征相似性和训练得到的标签兼容性。通过少量透明超参数控制各组件权重，实现同配性和异配性场景的平滑适应。还提出验证门控混合策略，仅在验证性能提升时将组合预测作为先验注入轻量级神经网络。

Result: 在异配性和过渡性基准测试中，与现有GNNs相比具有竞争力，同时在可解释性、可调性和计算效率方面具有优势。

Conclusion: 该方法提供了一种可解释、自适应的半监督节点分类框架，通过组合推理而非深度消息传递，在保持性能的同时提高了透明度和计算效率。
...
### [310] [Müntz-Szász Networks: Neural Architectures with Learnable Power-Law Bases](https://arxiv.org/abs/2512.22222)
*Gnankan Landry Regis N'guessan*

Main category: cs.LG

TL;DR: 提出Müntz-Szász网络(MSN)，用可学习的分数幂基函数替代固定激活函数，显著提升对奇异函数和分数幂函数的逼近能力。


<details>
  <summary>Details</summary>
Motivation: 标准神经网络使用固定激活函数(ReLU、tanh、sigmoid)，不适合逼近物理中普遍存在的奇异函数和分数幂函数(如边界层、断裂力学、角点奇异性)。

Method: 引入Müntz-Szász网络(MSN)，每条边计算φ(x)=∑a_k|x|^μ_k+∑b_k sign(x)|x|^λ_k，其中指数{μ_k, λ_k}与系数一同学习。基于Müntz-Szász定理保证通用逼近性。

Result: 对于|x|^α类函数，MSN单学习指数误差O(|μ-α|^2)，而标准MLP需O(ε^{-1/α})神经元。在奇异目标函数回归中，MSN误差降低5-8倍，参数减少10倍。在PINN基准测试中提升3-6倍，学习到的指数与已知解结构匹配。

Conclusion: 理论指导的架构设计能为科学驱动的函数类带来显著改进，MSN在逼近奇异函数方面优于传统MLP，同时提供可解释的指数参数。
...
### [311] [ReGAIN: Retrieval-Grounded AI Framework for Network Traffic Analysis](https://arxiv.org/abs/2512.22223)
*Shaghayegh Shajarian,Kennedy Marsh,James Benson,Sajad Khorsandroo,Mahmoud Abdelsalam*

Main category: cs.LG

TL;DR: ReGAIN是一个多阶段网络流量分析框架，结合流量摘要、检索增强生成和LLM推理，实现透明准确的网络分析，在真实攻击流量上达到95.95%-98.82%的准确率。
...
Method: 提出ReGAIN框架：1) 将网络流量转换为自然语言摘要；2) 嵌入多集合向量数据库；3) 使用分层检索管道（元数据过滤、MMR采样、两阶段交叉编码器重排序、弃权机制）来为LLM响应提供证据引用；4) 减少幻觉并确保基于证据的推理。

Result: 在真实世界流量数据集的ICMP ping flood和TCP SYN flood攻击轨迹上评估，准确率在95.95%到98.82%之间，优于基于规则、经典机器学习和深度学习基线，并通过数据集真实标签和人类专家评估双重验证。

Conclusion: ReGAIN在保持高准确率的同时提供了独特的可解释性，通过可信、可验证的响应增强了分析师对网络流量分析的信任。
...
### [312] [DiRL: An Efficient Post-Training Framework for Diffusion Language Models](https://arxiv.org/abs/2512.22234)
*Ying Zhu,Jiaxin Wan,Xiaoran Liu,Siyanag He,Qiqi Wang,Xu Guo,Tianyi Liang,Zengfeng Huang,Ziwei He,Xipeng Qiu*

Main category: cs.LG

TL;DR: DiRL是一个针对扩散语言模型的高效后训练框架，通过FlexAttention加速块训练和LMDeploy优化推理，结合DiPO优化算法，在数学推理任务上达到SOTA性能。
...
Method: 提出DiRL框架，整合FlexAttention加速的块训练和LMDeploy优化的推理，支持高效的两阶段后训练（监督微调+强化学习）。提出DiPO算法，首个为dLLMs定制的无偏组相对策略优化实现。

Result: 在高质量数学数据上训练的DiRL-8B-Instruct模型在dLLMs中达到SOTA数学性能，并在多个基准测试中超越Qwen2.5系列可比模型。

Conclusion: DiRL框架有效解决了dLLMs后训练的效率问题，DiPO算法为dLLMs提供了专门的强化学习优化方法，显著提升了复杂推理任务的性能。
...
### [313] [Masking Teacher and Reinforcing Student for Distilling Vision-Language Models](https://arxiv.org/abs/2512.22238)
*Byung-Kwan Lee,Yu-Chiang Frank Wang,Ryo Hachiuma*

Main category: cs.LG

TL;DR: 提出Masters框架，通过掩码渐进强化学习蒸馏，解决大模型向小模型知识迁移的稳定性问题


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型虽然能力强，但体积太大难以部署到移动或边缘设备。需要紧凑而能力强的VLM，但大教师模型向小学生模型的知识蒸馏面临尺寸差距大、表示复杂、学习不稳定等挑战。

Method: 提出Masters框架：1) 掩码教师非主导权重降低复杂度；2) 渐进恢复教师容量实现平滑学习；3) 离线强化学习阶段结合准确性奖励和蒸馏奖励；4) 利用预生成响应避免昂贵的在线思考-回答过程。

Result: 学生模型能够以稳定方式学习教师模型的丰富表示，在不需要昂贵在线过程的情况下实现强大性能。

Conclusion: Masters框架通过掩码渐进策略和离线强化学习，有效解决了大尺寸差距下的知识蒸馏问题，为部署紧凑而强大的视觉语言模型提供了可行方案。
...
### [314] [EvoXplain: When Machine Learning Models Agree on Predictions but Disagree on Why -- Measuring Mechanistic Multiplicity Across Training Runs](https://arxiv.org/abs/2512.22240)
*Chama Bensmail*

Main category: cs.LG

TL;DR: EvoXplain框架通过重复训练检测模型解释的稳定性，发现即使高精度模型也可能存在多种不同的解释机制，挑战了单一解释的可靠性假设。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习实践中存在一个被忽视的问题：当两个模型都达到高精度时，它们是否依赖相同的内部逻辑？还是通过不同甚至相互竞争的机制达到相同结果？这挑战了"高精度意味着解释正确可信"的常见假设。

Method: EvoXplain将解释视为从随机优化过程中抽取的样本，分析重复训练产生的解释是否形成单一连贯模式或分离为多个不同的解释模式。该方法不聚合预测或构建集成，而是直接检查解释的分布。

Result: 在乳腺癌和COMPAS数据集上，即使逻辑回归和随机森林都达到高精度，它们的解释经常表现出明显的多模态性。即使是通常被认为稳定的逻辑回归，在相同数据分割的重复训练中也能产生多个分离的解释盆地。

Conclusion: EvoXplain不试图选择"正确"解释，而是使解释的不稳定性可见且可量化，揭示单一实例或平均解释可能掩盖多种底层机制的存在。这重新定义了可解释性为模型类在重复实例化下的属性，而非单个训练模型的属性。
...
### [315] [Enhanced geometry prediction in laser directed energy deposition using meta-learning](https://arxiv.org/abs/2512.22241)
*Abdul Malik Al Mardhouf Al Saadi,Amrita Basak*

Main category: cs.LG

TL;DR: 提出基于元学习的跨数据集知识迁移模型，用于激光定向能量沉积（L-DED）中熔道几何形状预测，解决实验数据稀缺和异构性问题。


<details>
  <summary>Details</summary>
Motivation: 激光定向能量沉积（L-DED）中，由于不同材料、机器配置和工艺参数下收集的实验数据稀缺且异构，准确预测熔道几何形状面临挑战。

Method: 提出基于元学习的跨数据集知识迁移框架，研究两种基于梯度的元学习算法：模型无关元学习（MAML）和Reptile，实现有限数据下对新沉积条件的快速适应。

Result: MAML和Reptile在仅使用3-9个训练样本的情况下，能在未见过的目标任务上实现准确的熔道高度预测，性能优于传统前馈神经网络。在多个目标任务上，元学习模型达到R²约0.9，平均绝对误差0.03-0.08毫米。

Conclusion: 基于元学习的知识迁移模型能有效解决L-DED中数据稀缺和异构性问题，实现跨不同沉积条件的准确熔道几何预测，为实际工业应用提供了可行方案。
...
### [316] [Fairness Evaluation of Risk Estimation Models for Lung Cancer Screening](https://arxiv.org/abs/2512.22242)
*Shaurya Gaur,Michel Vitale,Alessa Hering,Johan Kwisthout,Colin Jacobs,Lena Philipp,Fennie van der Graaf*

Main category: cs.LG

TL;DR: 评估两种深度学习肺癌风险预测模型在不同人口亚组中的公平性表现，发现存在显著的性别和种族性能差异。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症相关死亡的主要原因，低剂量CT筛查可早期发现并降低死亡率，但广泛实施可能加重放射科医生负担。AI模型在肺癌风险评估中显示出潜力，但高风险人群具有多样性，这些模型在不同人口群体中的性能表现仍不明确。

Method: 基于JustEFAB框架，评估Sybil肺癌风险模型和Venkadesh21结节风险估计器两种深度学习模型在不同人口亚组中的性能差异和公平性。同时检查了英国胸科学会指南推荐的PanCan2b逻辑回归模型。所有模型均在美国国家肺癌筛查试验(NLST)数据上训练，并在独立的NLST验证集上评估。评估指标包括AUROC、敏感性和特异性，并探索临床风险因素的潜在混杂影响。

Result: Sybil模型在女性(0.88)和男性(0.81)之间存在统计学显著的AUROC差异(p<0.001)。在90%特异性下，Venkadesh21模型对黑人参与者(0.39)的敏感性显著低于白人参与者(0.69)。这些差异无法用可用的临床混杂因素解释，根据JustEFAB框架可归类为不公平偏差。

Conclusion: 研究结果强调了在肺癌筛查中改进和监测模型在不同代表性不足亚组中的性能的重要性，以及进一步研究算法公平性的必要性。
...
### [317] [Predicting Mycotoxin Contamination in Irish Oats Using Deep and Transfer Learning](https://arxiv.org/abs/2512.22243)
...
Method: 研究采用神经网络和迁移学习模型预测爱尔兰燕麦作物中的霉菌毒素污染作为多响应预测任务。评估了五种建模方法：基线多层感知器（MLP）、预训练的MLP，以及三种迁移学习模型（TabPFN、TabNet和FT-Transformer）。使用回归（RMSE、R²）和分类（AUC、F1）指标评估性能，并进行基于排列的变量重要性分析。

Result: 迁移学习方法TabPFN提供了整体最佳性能，其次是基线MLP。变量重要性分析显示，收获前90天的天气历史模式是最重要的预测因子，其次是种子含水量。

Conclusion: 迁移学习模型特别是TabPFN在预测霉菌毒素污染方面表现优异，收获前天气条件和种子含水量是关键预测因素，这为早期干预和风险管理提供了重要依据。
...
### [318] [Calibrating LLM Judges: Linear Probes for Fast and Reliable Uncertainty Estimation](https://arxiv.org/abs/2512.22245)
*Bhaktipriya Radharapu,Eshika Saxena,Kenneth Li,Chenxi Whitehouse,Adina Williams,Nicola Cancedda*

Main category: cs.LG

TL;DR: 本文提出使用线性探针从LLM法官的隐藏状态获取校准的不确定性估计，相比现有方法具有更好的校准性和10倍计算节省，适用于生产部署。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的法官在工业应用中变得重要，需要高效获取良好校准的不确定性估计以支持生产部署。现有方法如语言化置信度和多生成方法要么校准效果差，要么计算成本高。

Method: 引入基于Brier分数的损失函数训练的线性探针，从推理法官的隐藏状态提供校准的不确定性估计，无需额外的模型训练。

Result: 探针在客观任务（推理、数学、事实性、编码）和主观人类偏好判断上都优于现有方法，具有更好的校准性、约10倍计算节省、对未见评估领域的鲁棒泛化能力，以及高置信度预测的更高准确性。

Conclusion: 基于可解释性的不确定性估计为LLM法官提供了实用、可扩展的即插即用解决方案，虽然会产生保守估计（在简单数据集上表现不佳），但可能有利于安全关键部署中优先考虑低误报率的情况。
...
### [319] [The Affine Divergence: Aligning Activation Updates Beyond Normalisation](https://arxiv.org/abs/2512.22247)
*George Bird*

Main category: cs.LG

TL;DR: 论文提出激活更新在梯度下降中存在系统性不匹配问题，通过修正得到新的归一化方法，包括PatchNorm，挑战了传统归一化机制


<details>
  <summary>Details</summary>
Motivation: 激活值在优化中比参数更重要，因为它们更接近损失计算图且携带样本相关信息，但当前的激活更新没有采用最优的梯度下降步长，存在非理想的样本级缩放问题

Method: 修正激活更新的缩放问题，从第一原理推导出归一化方法；提出新的功能形式"PatchNorm"作为卷积层的归一化方案；将归一化器分解为类似激活函数的映射

Result: 提出的新方法在多个测试中优于传统归一化器，提供了替代仿射映射的方案；PatchNorm作为组合不可分的归一化器在卷积中表现良好

Conclusion: 该研究为归一化提供了新的机制框架，挑战了仿射+非线性的模型构建方法，提出了理论驱动的归一化新形式，并在实证中得到验证
...
### [320] [Amortized Inference for Model Rocket Aerodynamics: Learning to Estimate Physical Parameters from Simulation](https://arxiv.org/abs/2512.22248)
*Rohit Pandey,Rohan Pandey*

Main category: cs.LG

TL;DR: 使用基于仿真的摊销推理方法，通过神经网络在合成飞行数据上训练，然后直接应用于真实火箭飞行，无需微调即可预测空气动力学参数。


<details>
  <summary>Details</summary>
Motivation: 传统火箭飞行性能预测方法依赖计算流体动力学或经验相关性，而数据驱动方法需要大量昂贵的真实飞行数据。需要一种能够减少真实数据需求的方法。

Method: 提出仿真摊销推理方法：在物理模拟器生成的合成飞行数据上训练神经网络，学习反转前向物理模型，直接从单个远地点测量值结合发动机和配置特征预测阻力系数和推力修正因子。

Result: 在8次真实飞行中评估，实现了12.3米的远地点预测平均绝对误差，相比OpenRocket基线预测减少了误差，展示了零真实训练示例的仿真到现实迁移能力。

Conclusion: 该方法证明了通过合成数据训练的神经网络能够有效预测火箭空气动力学参数，为业余火箭社区提供了可复现的工具，并揭示了理想物理模型与现实飞行条件之间的系统性偏差。
...
### [321] [Temporal Visual Semantics-Induced Human Motion Understanding with Large Language Models](https://arxiv.org/abs/2512.22249)
*Zheng Xing,Weibing Zhao*

Main category: cs.LG

TL;DR: 本文提出了一种利用大型语言模型提取时序视觉语义来增强无监督人体运动分割的方法，通过将文本化的运动信息融入子空间聚类框架，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 传统的人体运动分割方法忽视了时序语义信息的重要性。本文旨在探索从人体运动序列中提取时序视觉语义，利用大型语言模型的图像到文本能力来增强子空间聚类性能。

Method: 1) 使用LLM从连续帧中提取文本化运动信息；2) 基于LLM响应学习时序相邻信息；3) 开发TVS集成的子空间聚类方法，包含带时序正则化的子空间嵌入；4) 引入基于分割输出的反馈优化框架。

Result: 在四个基准人体运动数据集上的实验结果表明，该方法优于现有的最先进方法。

Conclusion: 通过将大型语言模型提取的时序视觉语义融入子空间聚类框架，可以有效提升无监督人体运动分割的性能，证明了时序语义信息在该任务中的重要性。
...
### [322] [Interpretable Perturbation Modeling Through Biomedical Knowledge Graphs](https://arxiv.org/abs/2512.22251)
*Pascal Passigan,Kevin zhu,Angelina Ning*

Main category: cs.LG

TL;DR: 提出一个基于生物医学知识图谱的图注意力网络框架，用于预测药物-细胞对中基因表达变化，超越传统二元关联预测，揭示药物转录组机制效应。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架主要关注知识图谱中的链接预测和二元药物-疾病关联，但缺乏对基因扰动（揭示转录组机制效应）的预测能力。理解小分子如何影响基因表达对揭示药物机制、预测副作用和药物重定位至关重要。

Method: 构建融合PrimeKG++（增强版生物医学知识图谱）和LINCS L1000药物/细胞系节点的异质图，使用MolFormerXL和BioBERT等基础模型初始化多模态嵌入。训练图注意力网络（GAT）和下游预测头，学习药物-细胞对中978个标志基因的表达变化。

Result: 框架在支架分割和随机分割下均优于多层感知机基线（预测差异表达基因）。消融实验（边洗牌和节点特征随机化）证明生物医学知识图谱的边信息能增强扰动水平预测。

Conclusion: 该框架为机制性药物建模提供了新路径：从二元药物-疾病关联任务转向治疗干预的精细转录效应预测，有助于更深入地理解药物作用机制。
...
### [323] [Graph Attention-based Adaptive Transfer Learning for Link Prediction](https://arxiv.org/abs/2512.22252)
...
<details>
  <summary>Details</summary>
Motivation: 现有GNN链接预测方法在处理大规模稀疏图和跨数据集迁移学习时面临挑战，需要高度对齐不同数据集。自监督方法虽成功但忽视了跨图数据集迁移学习的潜力。

Method: 提出GAATNet图注意力自适应迁移网络，结合预训练和微调捕获不同规模数据集的全局节点嵌入。关键策略：1) 在自注意力模块中融入远邻嵌入作为偏置捕获全局特征；2) 微调时引入轻量自适配器模块提升训练效率。

Result: 在7个公共数据集上的综合实验表明，GAATNet在链接预测任务中达到最先进性能，为有效整合GNN与迁移学习提供了通用可扩展解决方案。

Conclusion: GAATNet通过创新的迁移学习框架解决了GNN链接预测中的跨数据集泛化问题，为大规模稀疏图分析提供了高效解决方案，代码和数据集已开源。
...
### [324] [Cardiac mortality prediction in patients undergoing PCI based on real and synthetic data](https://arxiv.org/abs/2512.22259)
*Daniil Burakov,Ivan Petrov,Dmitrii Khelimskii,Ivan Bessonov,Mikhail Lazarev*

Main category: cs.LG

TL;DR: 该研究开发了基于真实和合成数据的PCI术后心脏死亡风险预测模型，通过数据增强改善类别不平衡问题，识别出年龄、射血分数、外周动脉疾病和脑血管疾病为最重要的预测因素。


<details>
  <summary>Details</summary>
Motivation: 患者状态、血管造影和手术特征对预测经皮冠状动脉介入治疗（PCI）后的长期结果至关重要。研究旨在开发基于真实和合成数据的PCI患者心脏死亡风险预测模型，并识别对死亡率影响最大的因素。

Method: 分析了2044名接受分叉病变PCI的患者，主要终点为3年随访的心脏死亡。应用多种机器学习模型预测PCI后3年死亡率。为解决类别不平衡问题，生成500个合成样本加入训练集。使用置换特征重要性评估个体特征贡献，并进行额外实验评估移除非信息特征对模型预测的影响。

Result: 未进行过采样时，所有模型总体准确率高（0.92-0.93），但几乎完全忽略少数类。数据增强后，少数类召回率持续提高，AUROC损失最小，概率质量改善，对构建的严重病例风险估计更符合临床实际。特征重要性分析显示四个最具影响力的特征：年龄、射血分数、外周动脉疾病和脑血管疾病。

Conclusion: 研究表明，通过添加真实和极端病例的简单数据增强可以暴露、量化和减少仅使用表格记录的不平衡临床预测中的脆弱性，并激励在报告主要指标的同时常规报告概率质量和压力测试结果。
...
### [325] [The Physics Constraint Paradox: When Removing Explicit Constraints Improves Physics-Informed Data for Machine Learning](https://arxiv.org/abs/2512.22261)
*Rahul D Ray*

Main category: cs.LG

TL;DR: 研究通过系统消融实验揭示了物理约束数据生成中的悖论：显式能量守恒约束在物理一致方程中是冗余的，而Fabry-Perot振荡对带宽预测影响显著，移除后可提升机器学习性能31.3%。


<details>
  <summary>Details</summary>
Motivation: 在科学领域机器学习中，真实数据稀缺，物理约束数据生成至关重要。但现有方法往往过度约束模型，未能识别哪些物理组件真正必要，需要系统评估不同物理约束对数据生成和机器学习性能的影响。

Method: 对物理信息光栅耦合器光谱生成器进行系统消融研究：选择性移除显式能量守恒约束、Fabry-Perot振荡、带宽变化和噪声，分析各组件对数据生成和下游机器学习性能的影响。生成器将5个几何参数映射到100点光谱响应，速度达200样本/秒。

Result: 发现物理约束悖论：显式能量守恒约束在物理一致方程中是数学冗余的，约束与无约束变体达到相同的守恒精度（平均误差约7×10^-9）。Fabry-Perot振荡主导带宽变异性，移除后半高带宽展宽减少72%（从132.3nm降至37.4nm）。标准噪声添加-重归一化流程引入0.5%非物理负吸收值。下游机器学习评估显示物理可学习性权衡：移除Fabry-Perot振荡使带宽预测R²提升31.3%，RMSE降低73.8%。

Conclusion: 研究为物理信息数据集设计提供了可操作指导，强调机器学习性能可作为评估约束相关性的诊断工具。显式能量守恒约束在物理一致模型中冗余，而Fabry-Perot振荡等物理特征对机器学习性能有显著影响，需在数据生成中仔细考虑。
...
### [326] [LuxIA: A Lightweight Unitary matriX-based Framework Built on an Iterative Algorithm for Photonic Neural Network Training](https://arxiv.org/abs/2512.22264)
*Tzamn Melendez Carmona,Federico Marchesin,Marco P. Abrate,Peter Bienstman,Stefano Di Carlo,Alessandro Savino Senior*

Main category: cs.LG

TL;DR: 本文提出了一种名为Slicing的高效传输矩阵计算方法，并将其集成到LuxIA框架中，显著降低了光子神经网络模拟的内存消耗和执行时间，实现了大规模PNN的可扩展训练。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的光子神经网络模拟工具在训练大规模PNN时面临严重的可扩展性挑战，主要原因是传输矩阵计算的计算需求导致高内存和时间消耗，这限制了PNN的研究和发展。

Method: 提出了Slicing方法，这是一种与反向传播兼容的高效传输矩阵计算方法，并将其集成到统一的模拟和训练框架LuxIA中。该方法通过优化传输矩阵计算来减少内存使用和执行时间。

Result: LuxIA在各种光子架构和标准数据集（MNIST、Digits、Olivetti Faces）上的实验评估表明，该框架在速度和可扩展性方面始终优于现有工具，能够实现大规模PNN的高效模拟和训练。

Conclusion: 通过解决关键的计算瓶颈，LuxIA促进了光子神经网络更广泛的采用，并通过光子技术加速了AI硬件创新。这项工作为更高效、可扩展的光子神经网络研究和开发铺平了道路。
...
### [327] [LLMTM: Benchmarking and Optimizing LLMs for Temporal Motif Analysis in Dynamic Graphs](https://arxiv.org/abs/2512.22266)
*Bing Hao,Minglai Shao,Zengyi Wo,Yunlong Chu,Yuhang Liu,Ruijie Wang*

Main category: cs.LG

TL;DR: 本文系统研究了LLM在时序模体分析上的表现，提出了LLMTM基准测试，开发了工具增强的LLM智能体，并通过结构感知调度器在保持高精度的同时降低成本。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的广泛应用，其在动态图处理能力方面的研究日益重要。时序模体作为动态图的基本单元和重要局部属性，能直接反映异常和独特现象，对理解动态图的演化动态和结构特征至关重要。然而，利用LLM进行动态图上的时序模体分析仍相对未被探索。

Method: 1. 提出LLMTM基准测试，包含9种时序模体类型的6个定制任务；2. 通过大量实验分析不同提示技术和9种LLM模型的影响；3. 开发工具增强的LLM智能体，使用精确设计的提示解决任务；4. 提出结构感知调度器，考虑动态图的结构属性和LLM认知负荷，智能地在标准LLM提示和更强大的智能体之间调度查询。

Result: 实验表明，工具增强的LLM智能体能以高精度解决时序模体相关任务，但成本较高。结构感知调度器能有效保持高精度同时显著降低成本。

Conclusion: 本文系统探索了LLM在时序模体分析上的能力，提出的LLMTM基准和结构感知调度器为高效利用LLM处理动态图时序模体任务提供了有效解决方案，在精度和成本之间取得了良好平衡。
...
### [328] [Hierarchical Stacking Optimization Using Dirichlet's Process (SoDip): Towards Accelerated Design for Graft Polymerization](https://arxiv.org/abs/2512.22279)
*Amgad Ahmed Ali Ibrahim,Hein Htet,Ryoji Asahi*

Main category: cs.LG

TL;DR: 提出SoDip分层堆叠优化框架，结合Transformer、TabNet/XGBoost、高斯过程回归与狄利克雷过程混合模型，解决辐射诱导接枝聚合中因基膜形态变化导致的重复性问题。


<details>
  <summary>Details</summary>
Motivation: 辐射诱导接枝聚合技术虽能精确功能化聚合物薄膜，但由于基膜形态（结晶度、晶粒取向、自由体积）的未报告变异性，导致单体扩散、自由基分布和Trommsdorff效应不一致，造成空间接枝梯度和性能不一致，限制了技术的可重复性。

Method: 提出SoDip分层数据驱动框架：1) 使用解码器Transformer编码文本过程描述符；2) TabNet和XGBoost建模多模态特征交互；3) 高斯过程回归与狄利克雷过程混合模型进行不确定性量化和异方差处理；4) 贝叶斯优化高效探索高维合成空间。使用ChemDataExtractor 2.0和WebPlotDigitizer构建多样化数据集。

Result: 在交叉验证中，SoDip相比高斯过程回归实现了约33%的性能提升，同时提供校准的置信区间，能够识别低重复性区域。其堆叠架构能够整合稀疏文本和不同质量的数值输入，优于先前模型。

Conclusion: SoDip框架为接枝聚合研究建立了可重复、形态感知设计的基础，能够有效解决辐射诱导接枝聚合中的重复性问题，为离子交换膜、CO2分离膜和电池电解质的精确功能化提供可靠工具。
...
### [329] [Valori: A Deterministic Memory Substrate for AI Systems](https://arxiv.org/abs/2512.22280)
*Varshith Gudur*

Main category: cs.LG

TL;DR: Valori是一个确定性AI内存基板，使用定点算术(Q16.16)替代浮点运算，确保跨硬件平台的比特级一致性，解决AI系统中向量嵌入存储和检索的非确定性问题。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统依赖浮点算术存储和搜索向量嵌入，这引入了根本性的非确定性：相同的模型、输入和代码在不同硬件架构（如x86 vs ARM）上会产生不同的内存状态和检索结果。这破坏了可重现性和安全部署，导致数据静默分歧，影响受监管行业的审计追踪和事后验证。

Method: 提出Valori确定性AI内存基板，用定点算术(Q16.16)替代浮点内存操作，将内存建模为可重放的状态机。该方法在内存边界强制执行确定性，保证跨平台的比特级一致性。

Result: Valori能够保证跨平台的比特级相同内存状态、快照和搜索结果。研究表明非确定性出现在索引或检索之前，Valori通过在内存边界强制执行确定性来解决这一问题。

Conclusion: 确定性内存是可信AI系统的必要基础组件。Valori的开源实现为解决AI系统中的非确定性问题提供了可行方案，有助于确保可重现性、审计追踪和跨平台一致性。
...
### [330] [DBAW-PIKAN: Dynamic Balance Adaptive Weight Kolmogorov-Arnold Neural Network for Solving Partial Differential Equations](https://arxiv.org/abs/2512.22283)
*Guokan Chen,Yao Xiao*

Main category: cs.LG

TL;DR: 提出DBAW-PIKAN方法，结合Kolmogorov-Arnold网络架构与自适应权重策略，解决PINNs在处理多尺度/高频问题时的梯度流刚度和谱偏差问题，显著提升收敛速度和精度。
...
Method: 提出动态平衡自适应加权物理信息Kolmogorov-Arnold网络，结合基于可学习B样条的Kolmogorov-Arnold网络架构与包含动态衰减上界的自适应权重策略。

Result: 相比基线模型，该方法在不增加计算复杂度的情况下，加速收敛过程并将解精度提高至少一个数量级，在Klein-Gordon、Burgers和Helmholtz方程等基准测试中表现出显著优势。

Conclusion: DBAW-PIKAN能有效缓解PINNs的梯度相关失效模式，克服函数表示瓶颈，在精度和泛化性能方面均有显著提升，为处理多尺度科学计算问题提供了有效解决方案。
...
### [331] [Cluster Aggregated GAN (CAG): A Cluster-Based Hybrid Model for Appliance Pattern Generation](https://arxiv.org/abs/2512.22287)
...
<details>
  <summary>Details</summary>
Motivation: 现有GAN方法将所有电器统一处理，忽视了间歇性和连续性电器的行为差异，导致训练不稳定和输出保真度有限。合成电器数据对于非侵入式负载监测算法开发和隐私保护能源研究至关重要，但标记数据集的稀缺仍是一个主要障碍。

Method: 提出Cluster Aggregated GAN混合生成框架：1) 根据电器行为特征将每个电器路由到专门的分支；2) 对于间歇性电器，使用聚类模块分组相似激活模式，为每个集群分配专用生成器；3) 对于连续性电器，采用单独的LSTM生成器分支，通过序列压缩捕获渐进时间演化并保持训练稳定性。

Result: 在UVIC智能插座数据集上的广泛实验表明，该框架在衡量真实性、多样性和训练稳定性的指标上始终优于基线方法。将聚类作为主动生成组件显著提高了可解释性和可扩展性。

Conclusion: 该框架为非侵入式负载监测研究中的合成负载生成提供了一种有效方法，通过专门处理不同电器类型的行为特征，解决了现有方法的局限性。
...
### [332] [Co-GRPO: Co-Optimized Group Relative Policy Optimization for Masked Diffusion Model](https://arxiv.org/abs/2512.22288)
*Renping Zhou,Zanlin Ni,Tianyi Chen,Zeyu Liu,Yang Yue,Yulin Wang,Yuxuan Wang,Jingshu Liu,Gao Huang*

Main category: cs.LG

TL;DR: Co-GRPO通过将掩码扩散模型重新表述为统一的马尔可夫决策过程，在轨迹级别联合优化模型参数和推理调度参数，解决了训练与推理之间的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在训练和推理之间存在显著差异：推理是多步迭代过程，涉及模型和调度策略，而训练采用简化的单步BERT式目标，导致推理调度从未在训练中优化。
...
Conclusion: Co-GRPO通过轨迹级别的联合优化，更彻底地对齐训练与推理，解决了MDM训练与推理不匹配的根本问题，显著提升了生成性能。
...
### [333] [When Algorithms Manage Humans: A Double Machine Learning Approach to Estimating Nonlinear Effects of Algorithmic Control on Gig Worker Performance and Wellbeing](https://arxiv.org/abs/2512.22290)
...
<details>
  <summary>Details</summary>
Motivation: 研究算法管理下以人为本的管理实践是否还能有效，传统线性方法可能无法捕捉工人对算法系统的复杂非线性反应

Method: 采用双重机器学习框架估计调节中介模型，避免强加限制性函数形式，使用464名零工工人的调查数据

Result: 发现明显的非单调模式：支持性HR实践改善工人福祉，但在算法监督模糊不清的"灰色中间地带"，其与绩效的联系减弱；当监督透明可解释时，联系再次增强

Conclusion: 简单线性设定可能得出相反结论；平台设计需注意：部分定义的控制造成混乱，但清晰规则和可信申诉机制能使强监督可行；方法上展示了双重机器学习在组织研究中估计条件间接效应的价值
...
### [334] [Multi-Head Spectral-Adaptive Graph Anomaly Detection](https://arxiv.org/abs/2512.22291)
...
Method: 提出多头谱自适应图神经网络(MHSA-GNN)：1) 设计轻量级超网络，基于包含结构统计和Rayleigh商特征的"谱指纹"，为每个实例动态生成Chebyshev滤波器参数；2) 引入双正则化策略：结合师生对比学习(TSC)保证表示准确性，以及Barlow Twins多样性损失(BTD)强制多头正交性，防止模式崩溃。

Result: 在四个真实世界数据集上的广泛实验表明，该方法能有效保留高频异常信号，显著优于现有最先进方法，特别是在高度异质数据集上表现出优异的鲁棒性。

Conclusion: MHSA-GNN通过实例特定的自适应滤波策略和有效的正则化机制，成功解决了图异常检测中异常信号保留和异质性处理的问题，为金融欺诈和风险控制等应用提供了更有效的解决方案。
...
### [335] [Learning from Negative Examples: Why Warning-Framed Training Data Teaches What It Warns Against](https://arxiv.org/abs/2512.22293)
*Tsogt-Ochir Enkhbayar*

Main category: cs.LG

TL;DR: 语言模型无法从警告框架的内容中学习避免不良行为，警告与直接暴露内容的效果无显著差异，这是因为模型学习的是统计共现而非语用解释


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否能够从训练数据中的警告框架内容（如"不要使用-此代码有漏洞"）中学习避免不良行为，探索模型对警告的实际理解能力

Method: 通过实验比较模型在警告框架内容和直接暴露内容下的行为差异，使用稀疏自编码器分析潜在特征激活模式，识别"描述X"和"执行X"的特征重叠问题

Result: 模型在警告框架内容下复制不良内容的比例（76.7%）与直接暴露内容（83.3%）无统计显著差异；稀疏自编码器分析显示特征#8684在警告和利用情境下激活程度相似，表明特征正交化失败

Conclusion: 当前架构中统计共现主导语用解释，模型学习的是上下文中倾向于出现什么，而不是为什么会出现；训练时特征消融可解决此问题，但提示和推理时引导无效
...
### [336] [Hybrid Quantum-Classical Mixture of Experts: Unlocking Topological Advantage via Interference-Based Routing](https://arxiv.org/abs/2512.22296)
*Reda Heddad,Lamiae Bouanane*

Main category: cs.LG

TL;DR: 提出混合量子-经典专家混合架构，通过量子路由网络利用量子干涉效应实现高效非线性决策边界，验证了量子优势源于量子特征映射和波干涉机制。


<details>
  <summary>Details</summary>
Motivation: 传统专家混合架构面临专家不平衡和经典路由机制计算复杂度的限制，量子机器学习有望通过量子干涉效应解决这些问题，实现更高效的非线性决策边界建模。

Method: 提出混合量子-经典专家混合架构，使用量子门控网络作为路由器，结合经典专家进行消融研究。采用量子特征映射和角度嵌入技术，利用量子波干涉效应实现高维核方法。

Result: 在非线性可分数据上验证了量子路由器的拓扑优势，能有效"解开"经典线性路由器无法高效分离的数据分布。量子路由器在参数效率上优于经典对应物，并在模拟量子噪声下保持鲁棒性。

Conclusion: 量子路由器通过量子特征映射和干涉效应实现了高效非线性决策边界，验证了干涉假说。该架构适用于NISQ硬件，在联邦学习、隐私保护机器学习和自适应系统中有应用潜力。
...
### [337] [Statistical and Machine Learning Analysis of Traffic Accidents on US 158 in Currituck County: A Comparison with HSM Predictions](https://arxiv.org/abs/2512.22302)
*Jennifer Sawyer,Julian Allagan*

Main category: cs.LG

TL;DR: 本研究扩展了Sawyer的热点分析，通过集成机器学习、空间建模和高级统计方法，分析US 158公路5年事故数据，识别时空模式，预测伤害严重程度，为交通安全干预提供依据。


<details>
  <summary>Details</summary>
Motivation: 扩展传统热点和卡方分析方法，超越基础统计技术，为US 158公路提供可操作的交通安全改进建议，同时为农村公路安全分析方法论做出贡献。

Method: 整合核密度估计(KDE)、负二项回归、随机森林分类和公路安全手册(HSM)安全性能函数(SPF)比较，分析2019-2023年事故数据，使用Moran's I检验空间聚类。

Result: 随机森林分类器预测伤害严重程度准确率达67%，优于HSM SPF；Moran's I检验确认空间聚类(I=0.32, p<0.001)；KDE分析显示主要交叉口附近存在热点区域。

Conclusion: 研究结果支持针对性的交通安全干预措施，验证并扩展了早期热点识别方法，为US 158公路安全改进提供具体建议，推动了农村公路安全分析方法论的发展。
...
### [338] [PDx -- Adaptive Credit Risk Forecasting Model in Digital Lending using Machine Learning Operations](https://arxiv.org/abs/2512.22305)
*Sultan Amed,Chan Yu Hang,Sayantan Banerjee*

Main category: cs.LG

TL;DR: PDx是一个基于MLOps的自适应决策系统，用于数字贷款中的违约概率预测，通过动态模型生命周期管理和冠军-挑战者框架解决传统静态模型性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统违约概率模型注重开发阶段的预测准确性，但缺乏对借款人行为变化的持续适应能力，导致静态模型在生产环境中性能随时间下降，且金融机构难以将ML模型从开发环境迁移到生产环境并维持其健康状态。

Method: 采用动态端到端模型生命周期管理方法，集成持续监控、重新训练和验证的稳健MLOps流程，引入动态冠军-挑战者框架，通过最新数据重新校准独立参数，并使用时间外验证选择最佳性能模型。

Result: 基于决策树的集成模型在违约分类中表现最佳但需要频繁更新，线性模型和神经网络性能退化更严重。PDx能有效缓解数字贷款机构的价值侵蚀，特别是在短期小额贷款中，已通过P2P贷款、商业贷款和汽车贷款数据集验证其有效性和可扩展性。

Conclusion: PDx通过MLOps驱动的自适应决策系统解决了传统违约概率模型的局限性，为现代信用风险预测提供了可扩展且适应性强的解决方案，特别适用于借款人行为快速变化的环境。
...
### [339] [LLMBoost: Make Large Language Models Stronger with Boosting](https://arxiv.org/abs/2512.22309)
...
TL;DR: LLMBoost是一个创新的LLM集成微调框架，通过跨模型注意力机制、链式训练和近并行推理，利用中间状态提升性能并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有LLM集成方法通常将模型视为黑盒，只组合输入或最终输出，忽略了丰富的内部表示和跨模型交互。需要打破这一限制，利用LLM的中间状态来提升集成效果。

Method: 1. 跨模型注意力机制：让后续模型能够访问和融合前驱模型的隐藏状态，实现分层错误纠正和知识转移。2. 链式训练范式：通过误差抑制目标逐步微调连接模型，确保每个模型以最小计算成本纠正前驱模型的错误预测。3. 近并行推理设计：逐层流水线化跨模型的隐藏状态，实现接近单模型解码的推理效率。

Result: 在常识推理和算术推理任务上的广泛实验表明，LLMBoost能够持续提升准确率，同时降低推理延迟。理论分析证明在有限纠正假设下，顺序集成能保证单调改进。

Conclusion: LLMBoost通过利用LLM的中间状态，打破了传统集成方法的黑盒限制，实现了性能提升和效率优化的平衡，为LLM集成学习提供了新范式。
...
### [340] [Optimistic Feasible Search for Closed-Loop Fair Threshold Decision-Making](https://arxiv.org/abs/2512.22313)
*Wenzhang Du*

Main category: cs.LG

TL;DR: 提出OFS算法，在满足公平性和服务率约束下在线学习阈值策略，通过置信区间处理反馈效应，在合成和半合成基准测试中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 闭环决策系统（如贷款、筛选、再犯风险评估）在公平性和服务约束下运行时会引发反馈效应：决策改变未来人群构成，导致数据非平稳并可能放大不平等。需要在线学习满足约束的阈值策略。

Method: 提出乐观可行搜索（OFS）算法：基于网格方法维护每个候选阈值的奖励和约束残差置信区间；每轮选择置信区间内看似可行的阈值，在其中最大化乐观奖励；若无可行阈值，则选择最小化乐观约束违反的阈值。

Result: 在三个基准测试中表现优异：1）具有稳定收缩动态的合成闭环基准；2）基于德国信用和COMPAS的半合成闭环基准。OFS比无约束和原始对偶bandit基线获得更高奖励和更小累积约束违反，接近最优可行固定阈值的oracle性能。

Conclusion: OFS算法能有效处理闭环决策系统中的公平性和服务约束，通过直接针对可行高效用阈值的设计，在低维可解释策略类中表现优异，为约束在线学习提供实用解决方案。
...
### [341] [LangPrecip: Language-Aware Multimodal Precipitation Nowcasting](https://arxiv.org/abs/2512.22317)
*Xudong Ling,Tianxi Huang,Qian Dong,Tao He,Chaorong Li,Guiduo Duan*

Main category: cs.LG

TL;DR: LangPrecip：一个语言感知的多模态降水临近预报框架，通过将气象文本作为降水演变的语义运动约束，在Rectified Flow范式下实现文本与雷达信息的有效整合，显著提升强降水预报精度。


<details>
  <summary>Details</summary>
Motivation: 短期降水临近预报具有高度不确定性和约束不足的特点，特别是对于快速演变的极端天气事件。现有生成方法主要依赖视觉条件，导致未来运动约束弱且模糊。需要更有效的约束方法来提高预报准确性。

Method: 提出语言感知多模态临近预报框架(LangPrecip)，将气象文本作为降水演变的语义运动约束。在Rectified Flow范式下将临近预报构建为语义约束的轨迹生成问题，在潜在空间中高效整合文本和雷达信息。同时构建了包含16万对雷达序列和运动描述的大规模多模态数据集LangPrecip-160k。

Result: 在瑞典和MRMS数据集上的实验表明，该方法相比最先进方法有持续改进，在80分钟预报时效上，强降水的CSI指标分别提升了超过60%和19%。

Conclusion: 通过引入语言作为语义运动约束，LangPrecip框架能够更有效地整合多模态信息，显著提升短期降水临近预报的准确性，特别是在强降水事件的预报能力上有明显优势。
...
### [342] [Decomposing Uncertainty in Probabilistic Knowledge Graph Embeddings: Why Entity Variance Is Not Enough](https://arxiv.org/abs/2512.22318)
*Chorok Lee*

Main category: cs.LG

TL;DR: 现有概率知识图谱嵌入方法存在关系无关的不确定性估计问题，无法区分新兴实体和新型关系上下文，本文提出CAGP方法结合语义和结构不确定性，显著提升OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有概率知识图谱嵌入方法使用实体级方差来量化认知不确定性，但这些方差是关系无关的，导致无法区分两种行为相反的OOD现象：新兴实体（罕见、学习不足）和新型关系上下文（熟悉实体在未观察到的关系中）。

Method: 提出不确定性分解为互补成分：语义不确定性（来自实体嵌入方差，检测新兴实体）和结构不确定性（来自实体-关系共现，检测新型上下文）。CAGP方法通过学习的权重结合这两种不确定性信号。

Result: 在三个数据集（FB15k-237、WN18RR、YAGO3-10）上验证了频率完全重叠现象。CAGP在时间OOD检测上达到0.94-0.99 AUROC，相比关系无关基线有60-80%的相对改进。在选择性预测中，在85%回答率下减少43%的错误。

Conclusion: 关系无关的不确定性估计存在根本局限性，无法检测新型关系上下文。通过将不确定性分解为语义和结构成分并适当结合，可以显著提升知识图谱的OOD检测性能，为不确定性估计提供了理论框架和实用方法。
...
### [343] [Expert System for Bitcoin Forecasting: Integrating Global Liquidity via TimeXer Transformers](https://arxiv.org/abs/2512.22326)
*Sravan Karthick T*

Main category: cs.LG

TL;DR: 该论文提出了一种结合全球M2流动性作为外生变量的TimeXer-Exog模型，显著提升了比特币长期价格预测的稳定性，在70天预测期上比单变量基准模型误差降低89%以上。


<details>
  <summary>Details</summary>
Motivation: 比特币价格预测面临极端波动性和非平稳性的挑战，传统单变量时间序列模型在长期预测中表现不佳。论文旨在解决这一关键缺口，通过引入宏观经济变量来改善长期预测性能。

Method: 采用TimeXer架构，集成来自18个主要经济体的全球M2流动性作为外生变量，使用12周滞后结构。比较了流动性条件预测模型(TimeXer-Exog)与LSTM、N-BEATS、PatchTST和标准单变量TimeXer等先进基准模型。

Result: 在2020年1月至2025年8月的每日比特币价格数据上进行实验，结果显示宏观经济条件显著稳定了长期预测。在70天预测期上，TimeXer-Exog模型达到均方误差1.08e8，比单变量TimeXer基准模型性能提升超过89%。

Conclusion: 研究表明，在深度学习模型中加入全球流动性条件能够为比特币长期价格预测带来实质性改进，强调了宏观经济因素在加密货币预测中的重要性。
...
### [344] [The Effectiveness of Approximate Regularized Replay for Efficient Supervised Fine-Tuning of Large Language Models](https://arxiv.org/abs/2512.22337)
*Matthew Riemer,Erik Miehling,Miao Liu,Djallel Bouneffouf,Murray Campbell*

Main category: cs.LG

TL;DR: LoRA微调可能严重损害模型能力，但通过正则化近似回放方法（惩罚与初始模型的KL散度并混合预训练风格数据）可以几乎消除此问题，在保持通用知识的同时不阻碍新任务学习。


<details>
  <summary>Details</summary>
Motivation: 尽管LoRA等参数高效微调方法只修改少量参数，但它们可能对模型产生显著负面影响。研究发现LoRA监督微调可能灾难性地降低模型能力，即使在小数据集上训练少量步骤也会发生。需要找到既能保持模型原有能力又不阻碍新任务学习的微调方法。

Method: 提出正则化近似回放方法：1）惩罚与初始模型的KL散度作为正则化；2）混合来自与预训练相似但不同的开放访问语料库的下一个词预测数据。该方法计算开销很小，应用于Qwen指令微调模型。

Result: 该方法有效保留了模型中的通用知识，同时不阻碍对新任务的可塑性。与直接LoRA微调相比，几乎完全消除了能力退化问题，仅增加了适度的计算开销。

Conclusion: 虽然最简单的LoRA微调方法在实践中可能严重失败，但通过正则化近似回放的小调整可以几乎消除问题，在保持模型原有能力的同时实现有效微调，为参数高效微调提供了更稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 超参数调优对大规模模型的训练稳定性和最终性能有巨大影响。现有参数化方法（如μP）虽然实现了跨模型尺寸的全局超参数迁移，但未能处理深度缩放和模块级超参数优化问题。

Method: 提出Complete(d)参数化方法，统一处理宽度、深度、批次大小和训练时长的缩放。基于此参数化，研究模块级超参数优化，提出高维超参数空间导航的实用指南，实现模块级超参数跨尺寸迁移。

Result: 实验证明，在正确的参数化下，超参数迁移在模块级超参数体系中也成立。使用迁移的模块级超参数显著提升了大型语言模型的训练速度，覆盖了学习率、AdamW参数、权重衰减、初始化尺度、残差块乘数等现代模型优化超参数。

Conclusion: Complete(d)参数化方法统一了多个重要缩放维度，实现了模块级超参数的跨尺寸迁移，为大规模模型训练提供了实用的超参数优化指南，显著提升了训练效率。
...
### [346] [BLISS: Bandit Layer Importance Sampling Strategy for Efficient Training of Graph Neural Networks](https://arxiv.org/abs/2512.22388)
...
Method: 提出BLISS（Bandit Layer Importance Sampling Strategy），使用多臂老虎机动态选择每层中最具信息量的节点，平衡探索与利用以确保全面的图覆盖。该方法可集成到GCN和GAT中，并根据其特定的聚合机制调整选择策略。

Result: 实验表明BLISS能够保持或超过全批量训练的准确率，同时显著降低计算成本。
...
### [347] [Causality-Inspired Safe Residual Correction for Multivariate Time Series](https://arxiv.org/abs/2512.22428)
*Jianxiang Xie,Yuncheng Hua*

Main category: cs.LG

TL;DR: CRC是一个因果启发的安全残差校正框架，通过解耦自变量和跨变量动态来暴露方向感知结构，并采用四重安全机制确保预测性能不会退化。


<details>
  <summary>Details</summary>
Motivation: 现有多元预测模型（如Transformers和GNNs）存在系统性误差，且缺乏部署时性能退化的保证。传统后处理残差校正方法虽然可能提高平均准确率，但可能"以错误的方式帮助"，在未见场景中导致局部失败。

Method: CRC采用分而治之策略：1）因果启发编码器通过解耦自变量和跨变量动态暴露方向感知结构；2）混合校正器建模残差误差；3）四重安全机制严格控制校正过程，防止有害更新。

Result: 在多个数据集和预测骨干网络上的实验表明，CRC能持续提高准确率，消融研究证实其核心安全机制确保极高的非退化率（NDR），适合安全可靠部署。

Conclusion: CRC是一个即插即用的安全残差校正框架，通过因果启发结构和严格安全机制解决了预测模型部署中的"安全缺口"问题，确保性能不会退化。
...
### [348] [AFA-LoRA: Enabling Non-Linear Adaptations in LoRA with Activation Function Annealing](https://arxiv.org/abs/2512.22455)
*Jiacheng Li,Jianchao Tan,Zhidong Yang,Feiye Huo,Yerui Sun,Yuchen Xie,Xunliang Cai*

Main category: cs.LG

TL;DR: AFA-LoRA通过退火激活函数为LoRA引入非线性表达能力，在保持可合并性的同时缩小与全参数训练的差距


<details>
  <summary>Details</summary>
Motivation: LoRA作为广泛使用的参数高效微调方法，其线性适应过程限制了表达能力，导致线性训练与非线性训练之间存在表达力差距

Method: 提出AFA-LoRA训练策略，采用退火激活函数在训练过程中从非线性变换过渡到线性变换，使适配器先获得更强的表示能力，最终收敛到可合并的线性形式

Result: 在监督微调、强化学习和推测解码等任务上实施，结果显示AFA-LoRA显著缩小了LoRA与全参数训练之间的性能差距

Conclusion: 该工作实现了更强大且实用的参数高效适应范式，为LoRA提供了非线性表达能力的同时保持了其可合并性
...
### [349] [AMBIT: Augmenting Mobility Baselines with Interpretable Trees](https://arxiv.org/abs/2512.22466)
*Qizhi Wang*
...
<details>
  <summary>Details</summary>
Motivation: 解决OD流预测中高精度与可解释性之间的冲突，为实际部署提供既准确又透明的解决方案

Method: 首先对经典空间交互模型进行全面审计，然后基于物理基线构建残差学习器，使用梯度提升树和SHAP分析

Result: 物理基础的残差方法接近强树基预测器的精度，同时保持可解释结构；POI锚定的残差在空间泛化下最稳健

Conclusion: AMBIT框架为城市决策提供了可复现的管道、丰富的诊断和空间误差分析，平衡了精度与可解释性需求
...
### [350] [GLUE: Gradient-free Learning to Unify Experts](https://arxiv.org/abs/2512.22467)
*Jong-Ik Park,Shreyas Chaudhari,Srinivasa Pranav,Carlee Joe-Wong,José M. F. Moura*

Main category: cs.LG

TL;DR: GLUE提出了一种无需梯度的专家模型统一方法，通过梯度自由的两点更新学习专家模型的混合系数，只需两次前向传播即可获得优于启发式混合的目标域性能。


<details>
  <summary>Details</summary>
Motivation: 在现实部署系统中，多个预训练的专家模型共存，但新目标域需要超越单个专家域性能的通用模型。现有方法通过启发式混合专家模型来初始化目标模型，但基于数据大小或代理指标的系数选择往往导致目标域测试精度下降，而基于目标损失的完整反向传播学习系数计算成本过高。

Method: GLUE（Gradient-free Learning To Unify Experts）将目标模型初始化为固定专家模型的凸组合，通过梯度自由的两点（SPSA）更新学习混合系数，每个步骤仅需两次前向传播，无需完整反向传播。

Result: 在三个数据集和三种网络架构的实验表明，GLUE产生的单一先验模型经过微调后能超越基线方法。GLUE比基于数据大小的加权方法提升测试精度达8.5%，比基于代理指标的选择方法提升达9.1%，与基于反向传播的完整梯度混合方法相比，要么性能更优，要么差距在1.4%以内。

Conclusion: GLUE提供了一种高效、计算成本低的专家模型统一方法，通过梯度自由学习混合系数，为目标域扩展提供了有效的模型初始化策略，在保持性能的同时大幅降低了计算需求。
...
TL;DR: 小规模Transformer在受控环境中能精确执行贝叶斯推理（10^-3-10^-4比特精度），而容量匹配的MLP则失败，表明注意力机制是实现贝叶斯推理的关键架构特征。


<details>
  <summary>Details</summary>
Motivation: 验证Transformer是否真正执行贝叶斯推理一直很困难：自然数据缺乏解析后验，大模型混淆了推理与记忆。需要构建受控环境来严格验证。

Method: 构建"贝叶斯风洞"——真实后验已知且记忆不可能的受控环境，在两个任务（双射消除和隐马尔可夫模型状态跟踪）中分析小规模Transformer的机制。

Result: Transformer以10^-3-10^-4比特精度再现贝叶斯后验，而容量匹配的MLP失败几个数量级。发现Transformer通过几何机制实现贝叶斯推理：残差流作为信念基底，前馈网络执行后验更新，注意力提供内容寻址路由。

Conclusion: 分层注意力通过几何设计实现贝叶斯推理，解释了注意力的必要性和扁平架构的失败。贝叶斯风洞为将小规模可验证系统与大型语言模型中的推理现象机制性连接提供了基础。
...
### [352] [Collaborative Optimization of Multiclass Imbalanced Learning: Density-Aware and Region-Guided Boosting](https://arxiv.org/abs/2512.22478)
...
### [353] [Toward Real-World IoT Security: Concept Drift-Resilient IoT Botnet Detection via Latent Space Representation Learning and Alignment](https://arxiv.org/abs/2512.22488)
*Hassan Wasswa,Timothy Lynar*

Main category: cs.LG

TL;DR: 提出一个无需持续重训练的自适应IoT威胁检测框架，通过潜在空间对齐和GNN分类来应对概念漂移问题


<details>
  <summary>Details</summary>
Motivation: 现有AI模型依赖静态数据集，无法适应真实IoT网络流量的动态变化（概念漂移），而定期重训练方案计算开销大且存在灾难性遗忘风险

Method: 1) 在历史流量潜在空间表示上训练一次分类器；2) 使用对齐模型将新流量映射到已学习的潜在空间；3) 将潜在表示转换为图结构格式，使用图神经网络捕获攻击样本间的实例关系

Result: 在真实异构IoT流量数据集上的实验表明，该框架在概念漂移下保持稳健的检测性能

Conclusion: 该框架具有在动态、大规模IoT环境中实际部署的潜力，解决了传统方法计算开销大和灾难性遗忘的问题
...
### [354] [The Quest for Winning Tickets in Low-Rank Adapters](https://arxiv.org/abs/2512.22495)
*Hamed Damirchi,Cristian Rodriguez-Opazo,Ehsan Abbasnejad,Zhen Zhang,Javen Shi*

Main category: cs.LG

TL;DR: 论文发现彩票假设在LoRA参数高效微调中同样成立，并提出Partial-LoRA方法，通过识别稀疏子网络减少87%可训练参数同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着对大预训练模型微调的依赖增加，研究彩票假设是否适用于参数高效微调（特别是LoRA方法），探索预训练与微调之间的相互作用。

Method: 提出Partial-LoRA方法，系统识别稀疏子网络，训练与预训练模型任务相关子空间对齐的稀疏低秩适配器，关注各层稀疏度而非具体权重。

Result: 在8个视觉和12个语言任务（单任务和多任务设置）中，Partial-LoRA减少可训练参数达87%，同时保持或提高准确率。

Conclusion: 彩票假设在LoRA中成立，为开发更高效的适配策略开辟新途径，深化了对迁移学习及预训练与微调相互作用的理论理解。
...
### [355] [Predicting LLM Correctness in Prosthodontics Using Metadata and Hallucination Signals](https://arxiv.org/abs/2512.22508)
*Lucky Susanto,Anasta Pranawijayana,Cortino Sukotjo,Soni Prasad,Derry Wijaya*

Main category: cs.LG

TL;DR: 该研究探索利用元数据和幻觉信号预测LLM在医疗考试中回答的正确性，发现该方法可提升准确性但尚不适合高风险部署。


<details>
  <summary>Details</summary>
Motivation: LLM在医疗等高风险领域应用日益广泛，但存在生成事实错误信息（幻觉）的风险。虽然已有检测和缓解幻觉的研究，但预测LLM回答是否正确仍是一个关键但未被充分探索的问题。

Method: 研究分析通用模型（GPT-4o）和推理中心模型（OSS-120B）在口腔修复学多选题考试上的表现。使用三种不同提示策略下的元数据和幻觉信号，为每个（模型，提示）组合构建正确性预测器。

Result: 元数据方法可将准确性提升高达+7.14%，达到83.12%的精确度（相对于假设所有答案都正确的基线）。实际幻觉是错误回答的强指标，但元数据信号本身不能可靠预测幻觉。提示策略虽不影响总体准确性，但显著改变模型内部行为和元数据的预测效用。
...
### [356] [Decomposing Task Vectors for Refined Model Editing](https://arxiv.org/abs/2512.22511)
...
<details>
  <summary>Details</summary>
Motivation: 大型预训练模型虽然强大，但精确控制其特定概念行为仍然困难。任务向量算术操作允许组合不同行为，但向量中的重叠概念会相互干扰，导致不可预测的结果。

Method: 提出原则性分解方法，将每个任务向量分解为两个组件：一个捕获多个任务向量间的共享知识，另一个隔离每个任务的独特信息。通过识别投影中的不变子空间来实现。

Result: 在三个领域验证有效性：图像分类多任务合并提升5%；扩散模型中实现干净风格混合而不降低生成质量；语言模型中减少47%毒性同时保持通用知识任务性能。
...
### [357] [Towards Reliable Evaluation of Adversarial Robustness for Spiking Neural Networks](https://arxiv.org/abs/2512.22522)
*Jihang Wang,Dongcheng Zhao,Ruolin Chen,Qian Zhang,Yi Zeng*

Main category: cs.LG

TL;DR: 提出ASSG和SA-PGD框架，更可靠评估SNN对抗鲁棒性，揭示当前SNN鲁棒性被高估


<details>
  <summary>Details</summary>
Motivation: SNN的脉冲激活具有二元不连续性，导致梯度消失，使得基于梯度下降的对抗鲁棒性评估不可靠。现有替代梯度方法在强对抗攻击下的有效性不明确。

Method: 提出自适应锐度替代梯度（ASSG），根据攻击迭代中的输入分布自适应演化替代函数形状；设计稳定自适应投影梯度下降（SA-PGD）攻击，在L∞约束下使用自适应步长。

Result: 方法显著提高攻击成功率，覆盖多种对抗训练方案、SNN架构和神经元模型，提供更通用可靠的SNN对抗鲁棒性评估。实验表明当前SNN鲁棒性被显著高估。

Conclusion: 提出了更可靠的SNN对抗鲁棒性评估框架，揭示了现有SNN鲁棒性评估的局限性，强调需要更可靠的对抗训练方法。
...
### [358] [TimePerceiver: An Encoder-Decoder Framework for Generalized Time-Series Forecasting](https://arxiv.org/abs/2512.22550)
*Jaebin Lee,Hankook Lee*

Main category: cs.LG

TL;DR: TimePerceiver：一个统一的编码器-解码器时间序列预测框架，通过广义预测任务和位置感知架构，在多种基准数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测研究主要关注编码器设计，而将预测和解码视为次要问题。作者认为需要综合考虑输入编码、预测解码和训练策略，提出一个统一的框架来更好地处理多样化的时间序列预测任务。

Method: 1. 将预测任务泛化为包括外推、插值和填补等多种时间预测目标；2. 设计新颖的编码器-解码器架构，能够灵活感知和适应输入与目标段在时间轴上的任意位置；3. 编码器使用潜在瓶颈表示来联合捕获时间和跨通道依赖；4. 解码器利用可学习查询对应目标时间戳来有效检索相关信息。

Result: 在广泛的基准数据集上，TimePerceiver框架一致且显著地超越了先前的最先进基线方法。

Conclusion: TimePerceiver提供了一个统一的编码器-解码器预测框架，通过紧密对齐的有效训练策略，在多样化时间序列预测任务中取得了卓越性能，证明了综合考虑编码、解码和训练的重要性。
...
### [359] [On Admissible Rank-based Input Normalization Operators](https://arxiv.org/abs/2512.22587)
*Taeyun Kim*

Main category: cs.LG

TL;DR: 论文指出现有基于排序的归一化方法存在结构缺陷，提出了三个公理来定义有效的排序归一化算子，并构建了满足这些公理的最小算子。


<details>
  <summary>Details</summary>
Motivation: 基于排序的归一化方法在现代机器学习中被广泛使用，因其对尺度、单调变换和批次变化的鲁棒性而受到重视。然而，这些方法在严格单调变换、批次组成变化和微小输入扰动下存在不稳定性，且这种不稳定性源于算子结构设计而非实现细节。

Method: 提出了三个公理来形式化排序归一化所需的最小不变性和稳定性要求，证明任何满足这些公理的算子必须分解为：(i) 特征级排序表示和 (ii) 单调且Lipschitz连续的标量化映射。然后构建了满足这些标准的最小算子。

Result: 证明现有可微排序和排名算子因依赖值间隙和批次级成对交互而存在结构缺陷。提出的公理框架严格界定了有效排序归一化算子的设计空间，并与现有连续松弛排序方法形式化分离。

Conclusion: 论文形式化定义了排序归一化算子的结构条件，揭示了现有方法的根本缺陷，并提供了构建稳定、不变算子的理论框架，为实际应用中的排序归一化提供了理论基础。
...
### [360] [Data-Driven Analysis of Crash Patterns in SAE Level 2 and Level 4 Automated Vehicles Using K-means Clustering and Association Rule Mining](https://arxiv.org/abs/2512.22589)
*Jewel Rana Palit,Vijayalakshmi K Kumarasamy,Osama A. Osman*

Main category: cs.LG

TL;DR: 该研究分析了美国NHTSA的2500多起自动驾驶车辆事故记录，采用两阶段数据挖掘框架（K-means聚类和关联规则挖掘），揭示了不同自动化级别（SAE 2级和4级）下的事故动态和影响因素。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆虽然有望减少人为驾驶错误，但近期事故数据显示AV行为可能偏离预期安全结果，引发对混合交通环境中技术安全和操作可靠性的担忧。现有研究大多基于小规模加州数据集，且对不同SAE自动化级别的事故趋势理解有限。

Method: 开发了两阶段数据挖掘框架：首先使用K-means聚类基于时间、空间和环境因素将事故记录分为4个行为集群；然后在每个集群内应用关联规则挖掘，提取事故模式与影响因素（照明条件、路面状况、车辆动态、环境条件）之间的可解释多元关系。

Result: 研究揭示了不同SAE级别（2级和4级）自动驾驶车辆的事故动态模式，识别了影响事故的关键因素组合，为理解AV事故机制提供了数据驱动的见解。

Conclusion: 研究结果为AV开发者、安全监管机构和政策制定者提供了可操作的指导，有助于制定AV部署策略和最小化事故风险，推动自动驾驶技术更安全地发展。
...
### [361] [Energy-Guided Flow Matching Enables Few-Step Conformer Generation and Ground-State Identification](https://arxiv.org/abs/2512.22597)
*Guikun Xu,Xiaohan Yi,Peilin Zhao,Yatao Bian*

Main category: cs.LG

TL;DR: EnFlow：结合流匹配与能量模型的统一框架，通过能量引导采样生成低能构象并准确识别基态构象
...
Method: 提出EnFlow统一框架，将流匹配（FM）与显式学习的能量模型通过非高斯FM路径上的能量引导采样方案耦合。在采样过程中加入能量梯度引导，使轨迹向低能区域移动，显著提高构象保真度。学习的能量函数还能对生成的集合进行基于能量的高效排序，实现准确的基态识别。

Result: 在GEOM-QM9和GEOM-Drugs数据集上的广泛实验表明，EnFlow在1-2个ODE步骤内同时改进了生成指标，并相比最先进方法减少了基态预测误差。

Conclusion: EnFlow通过将流匹配与能量模型统一，解决了现有方法的碎片化问题，实现了高效的低能构象生成和准确的基态构象识别。
...
### [362] [Cryptocurrency Price Prediction Using Parallel Gated Recurrent Units](https://arxiv.org/abs/2512.22599)
...
<details>
  <summary>Details</summary>
Motivation: 随着加密货币和比特币的兴起，许多投资和交易在线进行。比特币使用区块链技术确保交易安全、透明、可追溯和不可篡改，但其价格波动剧烈，吸引了金融领域的广泛关注。投资者需要预测加密货币价格的方法，这是经济学中的重要挑战。

Method: 提出并行门控循环单元（PGRU）模型，使用循环神经网络以并行独立的方式预测价格。并行网络利用不同的输入，每个输入代表不同的价格相关特征。最后，通过神经网络结合并行网络的输出，预测加密货币的未来价格。

Result: 实验结果表明，所提模型在窗口长度为20和15时，分别实现了3.243%和2.641%的平均绝对百分比误差（MAPE）。与现有方法相比，该方法使用更少的输入数据和更低的计算成本，获得了更高的准确性和效率。

Conclusion: PGRU模型在加密货币价格预测方面表现出色，通过并行处理不同特征提高了预测精度，同时降低了计算复杂度，为投资者提供了有效的价格预测工具。
...
### [363] [Gold Price Prediction Using Long Short-Term Memory and Multi-Layer Perceptron with Gray Wolf Optimizer](https://arxiv.org/abs/2512.22606)
*Hesam Taghipour,Alireza Rezaee,Farshid Hajati*

Main category: cs.LG

TL;DR: 本文提出了一种基于LSTM-MLP混合模型的黄金价格预测算法，结合灰狼优化算法优化网络结构，实现了日度和月度价格预测，并基于预测结果开发了交易策略，在三个月内获得了171%的回报。


<details>
  <summary>Details</summary>
Motivation: 黄金市场作为全球重要的金融市场，其价格预测对金融机构、投资者等具有重要意义。但由于经济和政策因素的复杂性，准确预测金融市场一直具有挑战性。因此，开发能够准确预测市场走势的模型非常重要。

Method: 1. 使用两个LSTM网络分别进行日度和月度黄金价格预测
2. 将两个LSTM网络的预测结果输入到MLP网络中进行集成，生成最终预测
3. 使用灰狼优化算法优化每个网络的神经元数量，以最小化RMSE误差
4. 数据集涵盖2010-2021年的宏观经济、能源市场、股票和发达国家货币状况数据
5. 基于预测结果开发了实时市场交易策略

Result: 1. 日度黄金收盘价预测的MAE为0.21美元，月度预测MAE为22.23美元
2. 基于预测模型开发的交易策略在三个月内获得了171%的回报
3. 模型能够预测日度和月度的最高价、最低价和收盘价

Conclusion: 提出的LSTM-MLP混合模型结合灰狼优化算法，能够有效预测黄金价格，基于预测结果的交易策略表现出色，证明了该模型在金融市场预测中的实用价值。
...
### [364] [Communication Compression for Distributed Learning with Aggregate and Server-Guided Feedback](https://arxiv.org/abs/2512.22623)
*Tomas Ortega,Chun-Yin Huang,Xiaoxiao Li,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: 提出了两种无需客户端状态的新型有偏压缩框架CAFe和CAFe-S，解决了联邦学习中通信瓶颈和隐私保护问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临通信成本瓶颈，特别是客户端到服务器的上行传输受限。现有有偏压缩技术需要误差反馈机制，但标准误差反馈依赖客户端特定控制变量，这违反了用户隐私且与大规模联邦学习中常见的无状态客户端不兼容。

Method: 提出了两种新框架：1) CAFe使用前一轮全局聚合更新作为所有客户端共享的控制变量；2) CAFe-S在服务器拥有小型私有数据集时，生成服务器引导的候选更新作为更准确的预测器。以分布式梯度下降为代表算法进行分析。

Result: 在非凸且有界梯度差异情况下，理论证明了CAFe优于分布式压缩梯度下降。CAFe-S收敛到平稳点，且收敛速度随服务器数据代表性增强而提高。联邦学习场景的实验结果验证了两种方法优于现有压缩方案。

Conclusion: CAFe和CAFe-S框架成功解决了联邦学习中有偏压缩的通信瓶颈和隐私保护问题，无需客户端状态，在理论和实验上都表现出优越性能。
...
### [365] [Scaling Unverifiable Rewards: A Case Study on Visual Insights](https://arxiv.org/abs/2512.22650)
*Shuyu Gan,James Mooney,Pan Hao,Renxiang Wang,Mingyi Hong,Qianwen Wang,Dongyeop Kang*

Main category: cs.LG

TL;DR: 提出Selective TTS框架，通过在多阶段多智能体管道中分布计算资源并早期剪枝低质量分支，解决传统Test-Time Scaling在无验证奖励任务中的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 许多现实世界任务涉及多阶段管道，其最终结果缺乏可验证的奖励或足够数据来训练稳健的奖励模型，导致基于评判的迭代细化容易在阶段间累积误差。

Method: 提出Selective TTS框架：1) 在多智能体管道中分布计算资源而非时间上的重复细化；2) 使用过程特定评判器早期剪枝低质量分支；3) 基于数据科学管道构建端到端多智能体系统生成可视化图表和报告；4) 设计可靠的LLM评判模型与人类专家对齐。

Result: 在固定计算预算下，Selective TTS将平均分数从61.64提高到65.86，同时降低了方差。LLM评判模型与人类专家对齐良好（Kendall's τ=0.55）。

Conclusion: Selective TTS为扩展具有不可验证奖励的复杂开放任务（如科学发现和故事生成）提供了第一步，通过过程化细化稳定了多阶段管道中的推理过程。
...
### [366] [Quantum Generative Models for Computational Fluid Dynamics: A First Exploration of Latent Space Learning in Lattice Boltzmann Simulations](https://arxiv.org/abs/2512.22672)
*Achraf Hsain,Fouad Mohammed Abbou*

Main category: cs.LG

TL;DR: 首次将量子生成模型应用于计算流体动力学数据的潜在空间表示，比较量子电路玻恩机和量子生成对抗网络与经典LSTM在物理衍生潜在分布建模上的性能。


<details>
  <summary>Details</summary>
Motivation: 探索量子生成模型在计算流体动力学数据压缩潜在空间表示中的应用，填补量子模型与离散潜在空间压缩结合在CFD领域的空白。

Method: 使用GPU加速的格子玻尔兹曼方法生成流体涡量场，通过向量量化变分自编码器压缩到7维离散潜在空间，比较量子电路玻恩机、量子生成对抗网络和经典长短期记忆网络的生成性能。

Result: 在实验条件下，两种量子模型产生的样本与真实分布的平均最小距离均低于LSTM基准，其中量子电路玻恩机取得了最有利的指标。

Conclusion: 该工作提供了连接CFD模拟与量子机器学习的完整开源管道，首次在物理模拟压缩潜在表示上进行了量子生成建模的实证研究，为这一交叉领域的未来严格研究奠定了基础。
...
### [367] [Beyond Centralization: Provable Communication Efficient Decentralized Multi-Task Learning](https://arxiv.org/abs/2512.22675)
...
<details>
  <summary>Details</summary>
Motivation: 表示学习在数据稀缺环境中广泛应用，但现有研究主要集中于中心化方法，去中心化方法研究不足。在去中心化设置中，任务数据分布在多个节点，节点间通信受网络约束，需要高效恢复低秩特征矩阵

Method: 提出新的交替投影梯度和最小化算法，用于去中心化多任务表示学习。算法利用特征的低秩结构，通过交替优化策略在分布式节点间协作学习共享特征

Result: 算法具有可证明的精度保证，全面刻画了时间、通信和样本复杂度。特别地，通信复杂度与目标精度无关，相比先前方法显著降低通信成本。数值模拟验证了理论分析，并展示了去中心化学习优于中心化联邦方法的场景
...
### [368] [Learning with the $p$-adics](https://arxiv.org/abs/2512.22692)
*André F. T. Martins*

Main category: cs.LG

TL;DR: 该论文探索使用p-adic数（ℚₚ）作为实数（ℝ）的替代框架进行机器学习，利用其超度量结构和层次性质进行表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习框架基于实数域（ℝ）和欧几里得/希尔伯特空间，但这是唯一选择吗？作者探索p-adic数作为替代，其层次结构和无限字符串表示对代码理论和层次表示学习具有吸引力。

Method: 建立p-adic数分类、回归和表示学习的基础构建块，提供学习模型和算法。展示如何将简单的Quillian语义网络表示为紧凑的p-adic线性网络，这在实数域中不可能实现。

Result: 提出了p-adic机器学习框架的理论基础，展示了p-adic结构在层次表示学习中的独特优势，特别是在语义网络表示方面。

Conclusion: p-adic数为机器学习提供了有前景的替代框架，特别适合层次结构表示。论文为未来研究开辟了新方向，讨论了该框架带来的开放问题和机遇。
...
### [369] [Predictive Modeling of Power Outages during Extreme Events: Integrating Weather and Socio-Economic Factors](https://arxiv.org/abs/2512.22699)
...
<details>
  <summary>Details</summary>
Motivation: 针对低概率高后果的停电场景，传统预测方法难以有效应对极端事件导致的停电风险，需要开发更准确的预测框架

Method: 整合EAGLE-I停电记录（2014-2024）与天气、社会经济、基础设施和季节性事件数据，评估四种机器学习模型（RF、SVM、AdaBoost、LSTM）

Result: LSTM网络在所有测试模型中取得最低预测误差，结果显示更强的经济条件和更发达的基础设施与更低的停电发生率相关

Conclusion: 提出的学习框架能有效预测极端事件导致的停电，社会经济和基础设施因素对停电风险有显著影响，LSTM模型在时间序列预测中表现最优
...
### [370] [What Matters in Deep Learning for Time Series Forecasting?](https://arxiv.org/abs/2512.22702)
*Valentina Moretti,Andrea Cini,Ivan Marisca,Cesare Alippi*

Main category: cs.LG

TL;DR: 该论文分析了时间序列预测深度学习架构的设计空间，指出当前基准测试存在问题，强调应基于预测原理而非特定层设计架构，并提出了辅助预测模型卡来规范架构描述。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列预测领域存在大量新架构和矛盾的经验结果，难以评估哪些组件真正影响性能。需要理解深度学习架构的设计维度，解释观察到的意外结果，并重新思考有缺陷的基准测试实践。

Method: 通过讨论时间序列预测的设计维度和权衡，分析局部性和全局性等概念在近期架构中的应用，评估被忽视的实现细节如何改变预测方法类别和影响经验结果。提出辅助预测模型卡来系统描述架构的关键设计选择。

Result: 研究表明，考虑局部性和全局性等基础方面比采用特定序列建模层更重要，设计良好的简单架构通常能与最先进方法相媲美。被忽视的实现细节会从根本上改变预测方法类别并显著影响经验结果。

Conclusion: 需要重新思考当前有缺陷的基准测试实践，在设计架构时应关注预测问题的基础方面。提出的辅助预测模型卡有助于系统描述现有和新架构的关键设计选择，推动更严谨的架构评估。
...
### [371] [FoldAct: Efficient and Stable Context Folding for Long-Horizon Search Agents](https://arxiv.org/abs/2512.22733)
*Jiaqi Shao,Yufeng Miao,Wei Zhang,Bing Luo*

Main category: cs.LG

TL;DR: FoldAct框架解决了长时程强化学习中上下文折叠方法导致的非平稳观测分布问题，通过分离损失计算、全上下文一致性损失和选择性片段训练，实现了稳定训练和5.19倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有上下文折叠方法将摘要动作视为标准动作，忽略了摘要会改变智能体未来的观测空间，导致策略依赖的非平稳观测分布，这违反了强化学习的核心假设，并带来梯度稀释、自条件化和计算成本三大挑战。

Method: FoldAct框架包含三个关键技术：1) 分离损失计算，为摘要和动作token提供独立的梯度信号；2) 全上下文一致性损失，减少分布偏移；3) 选择性片段训练，降低计算成本。

Result: 该方法能够稳定训练具有上下文折叠的长时程搜索智能体，解决了非平稳观测问题，同时将训练效率提高了5.19倍。

Conclusion: FoldAct通过明确处理上下文折叠带来的非平稳观测问题，为长时程强化学习提供了稳定高效的训练框架，解决了现有方法在可扩展性方面的关键限制。
...
### [372] [When Does Multi-Task Learning Fail? Quantifying Data Imbalance and Task Independence in Metal Alloy Property Prediction](https://arxiv.org/abs/2512.22740)
*Sungwoo Kang*

Main category: cs.LG

TL;DR: 多任务学习在合金材料属性预测中呈现两极分化：回归任务性能下降，但分类任务性能提升


<details>
  <summary>Details</summary>
Motivation: 验证多任务学习（MTL）能否利用合金材料属性间的潜在物理关联来提升预测性能，特别是同时预测电阻率、维氏硬度和非晶形成能力

Method: 使用54,028个合金样本，比较单任务模型与标准/结构化多任务学习模型，分析回归（电阻率、硬度）和分类（非晶形成能力）任务的性能差异

Result: MTL显著降低回归性能（电阻率R²: 0.897→0.844；硬度R²: 0.832→0.694），但提升分类性能（非晶F1: 0.703→0.744；召回率+17%），任务间权重接近零表明属性独立
...
### [373] [Bridging Global Intent with Local Details: A Hierarchical Representation Approach for Semantic Validation in Text-to-SQL](https://arxiv.org/abs/2512.22744)
...
<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL验证方法主要关注语法正确性，缺乏有效的语义验证（检测问题与SQL之间的不匹配）。语义验证面临两大挑战：1）同时捕捉全局用户意图和SQL结构细节；2）构建高质量细粒度的子SQL标注。

Method: 提出HEROSQL分层SQL表示方法：1）结合逻辑计划（LP）捕捉全局意图和抽象语法树（AST）捕捉局部细节；2）使用嵌套消息传递神经网络（NMPNN）捕获SQL固有关系信息并在LP和AST间聚合模式引导的语义；3）提出AST驱动的子SQL增强策略生成高质量负样本，优化细粒度语义不一致性。

Result: 在Text-to-SQL验证基准测试（域内和域外设置）中，HEROSQL显著优于现有最先进方法，平均AUPRC提升9.40%，AUROC提升12.35%。能有效检测细粒度语义错误，为大型语言模型提供更细粒度的反馈，增强数据查询平台的可靠性和可解释性。

Conclusion: HEROSQL通过分层SQL表示和NMPNN有效解决了Text-to-SQL语义验证的关键挑战，在检测语义不一致性方面表现出色，为数据查询系统提供了更可靠的验证机制。
...
### [374] [Debugging Tabular Log as Dynamic Graphs](https://arxiv.org/abs/2512.22903)
*Chumeng Liang,Zhanyang Jin,Zahaib Akhtar,Mona Pereira,Haofei Yu,Jiaxuan You*

Main category: cs.LG

TL;DR: 提出GraphLogDebugger框架，使用动态图建模表格日志，通过简单动态GNN超越LLM的日志调试性能
...
Method: 构建异构节点表示对象和事件，连接节点边，将表格日志恢复为演化动态图，使用简单动态GNN进行调试

Result: 在计算机系统和学术论文的真实日志数据集上，动态图建模使简单GNN在调试表格日志方面优于LLM

Conclusion: GraphLogDebugger通过动态图建模有效解决表格日志调试问题，提供比LLM更灵活、可扩展的解决方案
...
### [375] [From Confounding to Learning: Dynamic Service Fee Pricing on Third-Party Platforms](https://arxiv.org/abs/2512.22749)
*Rui Ai,David Simchi-Levi,Feng Zhu*

Main category: cs.LG

TL;DR: 研究第三方平台面对策略性代理时的定价行为，开发了在混淆条件下学习需求的算法，获得最优遗憾界，揭示了供应侧噪声对需求可学习性的根本影响


<details>
  <summary>Details</summary>
Motivation: 第三方平台作为收入最大化者，需要观察影响需求的市场特征，但由于只能观察到均衡价格和数量，这构成了一个在混淆条件下的一般需求学习问题

Method: 开发了具有最优遗憾界的算法，利用非独立同分布行动作为工具变量学习需求，提出新颖的同胚构造方法，无需假设星形性即可建立估计边界，为深度神经网络学习需求提供首个效率保证

Result: 算法获得 $\Tilde{\cO}(\sqrt{T}\wedgeσ_S^{-2})$ 的最优遗憾界，揭示了供应侧噪声对需求可学习性的根本影响，导致遗憾的相变，并通过Zomato和Lyft的实际数据验证了方法的实用性

Conclusion: 该研究为平台在策略性代理和混淆条件下的需求学习提供了理论框架和实用算法，证明了非独立同分布行动可作为有效的工具变量，为深度神经网络在需求学习中的应用提供了理论保证
...
### [376] [A Micro-Macro Machine Learning Framework for Predicting Childhood Obesity Risk Using NHANES and Environmental Determinants](https://arxiv.org/abs/2512.22758)
*Eswarasanthosh Kumar Mamillapalli,Nishtha Sharma*

Main category: cs.LG

TL;DR: 提出一个微-宏观机器学习框架，整合个体层面数据（NHANES）和环境层面特征（USDA、EPA），预测儿童肥胖风险，发现环境负担与肥胖风险分布存在地理相似性。
...
Method: 1) 使用NHANES的个体层面人体测量和社会经济数据；2) 从USDA和EPA数据集中提取宏观层面结构环境特征（食物获取、空气质量、社会经济脆弱性）；3) 训练四种机器学习模型（逻辑回归、随机森林、XGBoost、LightGBM）预测肥胖；4) 构建综合环境脆弱性指数（EnvScore）；5) 进行多层次比较分析。

Result: XGBoost模型表现最佳；构建了州级环境脆弱性指数；多层次比较显示环境负担高的州与全国预测的微观层面肥胖风险分布存在强烈地理相似性。

Conclusion: 证明了整合多尺度数据集识别环境驱动肥胖风险差异的可行性；贡献了一个可扩展、数据驱动的多层次建模管道，适用于公共卫生信息学，具有扩展到因果建模、干预规划和实时分析的潜力。
...
### [377] [A Note on Hybrid Online Reinforcement and Imitation Learning for LLMs: Formulations and Algorithms](https://arxiv.org/abs/2512.23097)
...
Method: 通过分析结合轨迹级KL散度和任务奖励的复合目标梯度，分解为可解析计算的密集梯度（用于token级模仿）和蒙特卡洛估计的稀疏梯度（用于长期奖励优化）

Result: 密集梯度具有闭式解，可在logit层面计算，支持高效的GPU实现

Conclusion: 该框架统一了模仿学习和强化学习，为LLM微调提供了高效且理论严谨的方法
...
### [378] [Understanding the Mechanisms of Fast Hyperparameter Transfer](https://arxiv.org/abs/2512.22768)
*Nikhil Ghosh,Denny Wu,Alberto Bietti*

Main category: cs.LG

TL;DR: 该论文提出了一个超参数跨尺度迁移的理论框架，证明了快速迁移在计算最优网格搜索中的优势，并解释了μP参数化在宽度缩放中实现快速迁移的机制。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模不断增大，标准的超参数优化变得极其昂贵。需要一种能够将小规模网格搜索得到的最优超参数直接迁移到大规模模型的方法，以降低计算成本。

Method: 提出了一个通用的概念框架来分析超参数跨尺度迁移，将迁移定义为"快速"当迁移引起的次优性渐近消失速度快于有限尺度的性能差距。证明了快速迁移在计算最优网格搜索中相当于有用迁移。通过分解优化轨迹为宽度稳定和宽度敏感两个组件来解释μP参数化的迁移机制。

Result: 理论分析表明快速迁移在计算上比直接调优更高效。实证研究发现μP参数化在宽度缩放时表现出快速迁移特性，但这种特性依赖于问题结构。提出了优化轨迹分解假设并提供了包括大语言模型预训练在内的实证证据支持。

Conclusion: 超参数跨尺度迁移是降低大规模模型调优成本的有效策略。快速迁移在计算最优网格搜索中具有理论优势，μP参数化的成功可以通过优化轨迹分解为宽度稳定和宽度敏感组件来解释，这为实际应用中的超参数迁移提供了理论基础。
...
### [379] [GRExplainer: A Universal Explanation Method for Temporal Graph Neural Networks](https://arxiv.org/abs/2512.22772)
*Xuyan Li,Jie Wang,Zheng Yan*

Main category: cs.LG

TL;DR: GRExplainer是首个通用、高效且用户友好的Temporal GNN解释方法，解决了现有方法通用性差、计算成本高、需要先验知识的问题。


<details>
  <summary>Details</summary>
Motivation: Temporal GNN在动态图处理中很强大，但缺乏透明度和可解释性限制了实际应用。现有TGNN解释方法存在三个主要问题：1) 针对特定TGNN类型，通用性差；2) 计算成本高，不适合大规模网络；3) 忽视解释的结构连通性且需要先验知识，用户友好性差。

Method: 提出GRExplainer方法：1) 提取节点序列作为统一特征表示，使其独立于特定输入格式，适用于基于快照和基于事件的TGNN；2) 利用广度优先搜索和时间信息构建输入节点序列，减少冗余计算；3) 设计基于RNN的生成模型，实现自动化连续解释生成。

Result: 在6个真实世界数据集和3个目标TGNN上的实验表明，GRExplainer在通用性、效率和用户友好性方面优于现有基线方法。

Conclusion: GRExplainer是首个解决TGNN解释中通用性、效率和用户友好性问题的统一框架，为动态图神经网络的可解释性提供了有效解决方案。
...
### [380] [Theoretical Foundations of Scaling Law in Familial Models](https://arxiv.org/abs/2512.23407)
*Huan Song,Qingfei Zhao,Ting Long,Shuyu Tian,Hongjun An,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: 本文扩展了神经缩放定律，引入粒度(G)作为新的缩放变量，建立了包含模型大小(N)、训练数据(D)和粒度(G)的统一缩放定律，验证了"一次训练，多次部署"的家族模型范式。


<details>
  <summary>Details</summary>
Motivation: 传统神经缩放定律假设单一密集模型输出，忽略了家族模型这一重要范式。家族模型通过早期退出和接力式推理，从单一共享主干生成多个可部署子模型，适用于异构设备-边缘-云层次结构。需要理论扩展来捕捉这种"一次运行，多个模型"的范式。

Method: 提出统一函数形式L(N, D, G)，引入粒度(G)作为新的缩放变量。采用严格的IsoFLOP实验设计，在固定计算预算下系统扫描模型大小(N)和粒度(G)，动态调整训练数据(D)，从而解耦粒度边际成本与规模效益。

Result: 粒度惩罚遵循乘性幂律，且指数极小。这理论上连接了固定计算训练与动态架构，实践上验证了"一次训练，多次部署"的可行性，表明部署灵活性不会损害密集基线的计算最优性。

Conclusion: 成功扩展了神经缩放定律，将粒度作为基本缩放变量，为家族模型范式提供了理论基础。证明了在保持计算最优性的同时实现部署灵活性的可行性，推动了异构设备-边缘-云层次结构中普遍智能的实现。
...
### [381] [Schrodinger AI: A Unified Spectral-Dynamical Framework for Classification, Reasoning, and Operator-Based Generalization](https://arxiv.org/abs/2512.22774)
*Truong Son Nguyen*

Main category: cs.LG

TL;DR: Schrödinger AI是一个受量子力学启发的统一机器学习框架，通过波能量求解器、动力学求解器和低秩算子演算三个组件，提供物理驱动的替代方案，实现鲁棒泛化、可解释语义和涌现拓扑。


<details>
  <summary>Details</summary>
Motivation: 提出一种基于量子力学原理的机器学习新范式，旨在替代传统的交叉熵训练和Transformer注意力机制，实现更好的泛化能力、可解释语义和动态推理能力。

Method: 框架包含三个紧密耦合组件：1) 时间无关波能量求解器，将感知和分类视为在学习的哈密顿量下的谱分解；2) 时间相关动力学求解器，控制语义波函数随时间演化，实现上下文感知的决策修订和推理；3) 低秩算子演算，通过学习量子类转换算子实现符号变换。

Result: 实验显示：a) 涌现出反映人类概念类关系的语义流形；b) 动态推理能适应环境变化，包括实时势场扰动的迷宫导航；c) 在模算术任务上实现精确算子泛化，学习群作用并在远超训练长度的序列上组合它们。

Conclusion: 该工作为机器学习提出了新的基础方向，将学习视为发现和导航底层语义能量景观的过程，展示了物理驱动方法的潜力。
...
### [382] [Adapting, Fast and Slow: Transportable Circuits for Few-Shot Learning](https://arxiv.org/abs/2512.22777)
*Kasra Jalaldoust,Elias Bareinboim*

Main category: cs.LG

TL;DR: 提出Circuit-TR算法，通过因果可迁移性理论实现零样本组合泛化，利用因果图表示域内结构和差异oracle表示域间机制共享，学习模块化预测器并在目标域组合成电路进行预测


<details>
  <summary>Details</summary>
Motivation: 跨域泛化需要约束未见目标域相对于源域的结构，现有方法缺乏系统性的理论框架来处理域间机制共享和组合泛化问题

Method: 基于因果可迁移性理论，设计Circuit-TR算法：1) 从源数据学习模块化局部预测器集合；2) 利用因果图表示的域内结构和差异oracle表示的域间机制共享信息；3) 在因果结构允许的情况下，将模块迁移/组合到目标域形成预测电路；4) 提出无需显式因果结构的监督域适应方案

Result: 理论结果刻画了基于图论电路可迁移性准则的少样本可学习任务类别，将少样本泛化能力与电路规模复杂度联系起来；控制模拟验证了理论结果

Conclusion: Circuit-TR为组合泛化提供了基于因果可迁移性的理论框架，通过模块化学习和电路迁移实现跨域泛化，连接了少样本学习与电路复杂度理论
...
### [383] [VL-RouterBench: A Benchmark for Vision-Language Model Routing](https://arxiv.org/abs/2512.23562)
*Zhehao Huang,Baijiong Lin,Jingyuan Zhang,Jingying Wang,Yuhang Liu,Ning Lu,Tao Li,Xiaolin Huang*

Main category: cs.LG

TL;DR: VL-RouterBench：首个系统化、可复现的视觉语言模型路由基准，覆盖14个数据集、17个模型，评估10种路由方法，发现现有路由器与理想Oracle仍有明显差距。


<details>
  <summary>Details</summary>
Motivation: 多模型路由已从工程技术发展为关键基础设施，但现有研究缺乏系统化、可复现的基准来评估视觉语言模型路由系统。

Method: 基于VLMs原始推理和评分日志构建质量和成本矩阵，覆盖14个数据集（3个任务组）、30,540个样本、17个模型，创建519,180个样本-模型对。评估协议联合测量平均准确率、平均成本和吞吐量，构建归一化成本与准确率的调和平均数作为排名分数。
...
### [384] [Discovering Transmission Dynamics of COVID-19 in China](https://arxiv.org/abs/2512.22787)
*Zhou Yang,Edward Dougherty,Chen Zhang,Zhenhe Pan,Fang Jin*

Main category: cs.LG

TL;DR: 该研究通过分析中国COVID-19传播数据，发现大规模检测、隔离和接触者追踪等公共卫生干预措施的效果存在地区差异，大城市感染更多且与社交活动相关，感染源随时间从湖北旅行相关转向社交活动相关。
...
Result: 发现显著地区差异：大城市感染更多（与社交活动相关）；79%有症状者在症状出现5天内住院；有确诊病例接触史者5天内就医；感染源随时间变化：早期与湖北旅行相关，后期转向社交活动。

Conclusion: 公共卫生干预措施效果存在地区差异，社交活动是后期主要传播途径，及时就医和接触者追踪对控制疫情至关重要，为未来疫情应对提供数据支持。
...
### [385] [SNM-Net: A Universal Framework for Robust Open-Set Gas Recognition via Spherical Normalization and Mahalanobis Distance](https://arxiv.org/abs/2512.22792)
*Shuai Chen,Chen Wang,Ziran Wang*

Main category: cs.LG

TL;DR: SNM-Net：一种用于开放集气体识别的通用深度学习框架，通过几何解耦和Mahalanobis距离解决电子鼻系统中的特征分布漂移和未知干扰问题。
...
Method: 提出SNM-Net框架，核心创新包括：1）通过级联批归一化和L2归一化实现几何解耦，将高维特征投影到单位超球面以消除信号强度波动；2）引入Mahalanobis距离作为评分机制，利用类统计信息构建自适应椭球决策边界。该框架与CNN、RNN和Transformer骨干网络兼容。

Result: 在Vergara数据集上的系统实验表明，Transformer+SNM配置达到接近理论性能：AUROC为0.9977，未知气体检测率为99.57%（5% FPR下的TPR）。性能显著优于现有方法，AUROC提高3.0%，标准差比Class Anchor Clustering降低91.0%。在不同传感器位置下表现出卓越鲁棒性，标准差低于0.0028。

Conclusion: 该工作有效解决了准确性与稳定性之间的权衡问题，为工业电子鼻部署提供了坚实的技术基础。SNM-Net框架具有通用性，能够与多种深度学习架构无缝集成。
...
### [386] [Training AI Co-Scientists Using Rubric Rewards](https://arxiv.org/abs/2512.23707)
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型在生成符合所有约束和隐含要求的研究计划方面存在困难，需要开发能够更好协助人类研究者的AI协科学家。

Method: 从多个领域的研究论文中自动提取研究目标和特定目标评分标准，构建可扩展的多样化训练语料库，通过强化学习和自我评分训练研究计划生成模型。

Result: 专家在70%的研究目标中更偏好微调后的Qwen3-30B-A3B模型生成的计划，84%的自动提取评分标准获得批准；在医学和新arXiv预印本领域也取得12-22%的相对改进。

Conclusion: 该方法展示了可扩展的自动化训练方案在改进通用AI协科学家方面的潜力，即使在医学研究等执行反馈不可行的问题设置中也有效。
...
### [387] [ReDiF: Reinforced Distillation for Few Step Diffusion](https://arxiv.org/abs/2512.22802)
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型采样速度慢，现有蒸馏方法依赖固定的重建或一致性损失，限制了学生模型的优化潜力。需要更灵活的蒸馏框架来减少推理步骤和计算资源。

Method: 将扩散模型蒸馏视为强化学习策略优化问题，学生模型作为策略网络，使用基于教师模型输出的奖励信号进行训练。该方法允许学生探索多个去噪路径，采取更长、优化的步骤朝向数据分布的高概率区域。

Result: 实验结果表明，该方法在显著减少推理步骤和计算资源的情况下，相比现有蒸馏技术获得更优性能。框架具有模型无关性，适用于任何类型的扩散模型。

Conclusion: 提出的强化学习蒸馏框架为扩散模型提供了一种通用的优化范式，能够有效解决采样速度问题，同时保持生成质量，具有广泛的应用潜力。
...
### [388] [MoR: Mixture Of Representations For Mixed-Precision Training](https://arxiv.org/abs/2512.22804)
*Bor-Yiing Su,Peter Dykas,Mike Chrzanowski,Jatin Chhugani*

Main category: cs.LG

TL;DR: 本文提出MoR（混合表示）框架，通过动态分析张量数值特性，在FP8和BF16表示之间进行动态选择，实现高效混合精度训练，98.38%张量可量化到FP8格式。


<details>
  <summary>Details</summary>
Motivation: 混合精度训练是扩展深度学习模型的关键技术，但成功实施需要找到合适的训练方法组合。现有方法通常需要精细分区或特定配置，缺乏通用性和灵活性。

Method: 提出Mixture-of-Representations（MoR）框架，在张量和子张量级别进行动态量化。框架分析张量数值特性，在FP8和BF16表示之间动态选择。设计了具体算法支持不同量化分区策略和数据集。

Result: 初步结果显示，该方法能实现最先进的结果，98.38%的张量可量化到FP8格式，且精度与现有方法相当。无需精细分区即可达到FP8精度，或可与其他训练方法结合使用更低精度格式如NVFP4。

Conclusion: MoR框架展示了动态、属性感知量化的潜力，能保持模型质量的同时提高低精度训练的鲁棒性。该方法为混合精度训练提供了通用解决方案，有望推动更低精度格式的应用。
...
### [389] [Long-Range Distillation: Distilling 10,000 Years of Simulated Climate into Long Timestep AI Weather Models](https://arxiv.org/abs/2512.22814)
*Scott A. Martin,Noah Brenowitz,Dale Durran,Michael Pritchard*

Main category: cs.LG

TL;DR: 提出长距离蒸馏方法，用自回归教师模型生成大量合成气候数据训练单步长时距概率学生模型，实现从S2S到季节尺度的准确长期天气预报


<details>
  <summary>Details</summary>
Motivation: 传统AI天气模型的自回归方法在长期预报中误差累积且不稳定，而单步长时距概率模型需要大量训练数据，但再分析数据集样本有限导致过拟合

Method: 使用DLESyM作为自回归教师模型生成超过10,000年的合成气候数据，训练蒸馏学生模型直接进行长时距预报，替代数百个自回归步骤

Result: 在完美模型实验中，蒸馏模型优于气候学基准并接近教师模型性能；在真实世界中，经ERA5微调后达到与ECMWF集合预报相当的S2S预报技巧

Conclusion: 首次证明AI生成的合成训练数据可扩展长期预报技巧，蒸馏模型性能随合成数据量增加而提升，为长期天气预报提供了新范式
...
### [390] [TEACH: Temporal Variance-Driven Curriculum for Reinforcement Learning](https://arxiv.org/abs/2512.22824)
...
Method: 学生-教师学习范式，教师模块基于状态-动作值函数(Q)的策略置信度时间方差动态优先选择高不确定性目标，提供自适应聚焦学习信号
...
### [391] [Fundamental Novel Consistency Theory: $H$-Consistency Bounds](https://arxiv.org/abs/2512.22880)
*Yutao Zhong*

Main category: cs.LG

TL;DR: 该论文提出了H-一致性边界理论，为机器学习中代理损失函数与目标损失函数之间的估计误差提供了比贝叶斯一致性更强的理论保证，覆盖了二元分类、多分类、对抗性场景等多种情况。


<details>
  <summary>Details</summary>
Motivation: 机器学习训练中优化的损失函数（代理损失）通常与定义任务性能的目标损失不同，这源于计算不可行性或不可微性。需要建立代理损失与目标损失之间估计误差的理论保证，提供比现有贝叶斯一致性或H-校准更强的理论框架。

Method: 提出H-一致性边界理论框架，分析代理损失估计误差与目标损失估计误差的关系。首先研究二元分类，建立分布依赖和分布独立的紧边界，分析凸代理损失（线性模型和神经网络）和对抗性场景。扩展到多分类，为max、sum和约束损失建立H-一致性边界。研究comp-sum损失（如交叉熵、MAE），引入平滑对抗变体。建立统一框架推导各种代理损失的边界，分析边界增长率和最小化差距。

Result: 建立了二元分类和多分类的H-一致性边界，包括凸代理损失、对抗性场景、comp-sum损失等。证明了在某些情况下非平凡的H-一致性边界不可达。对于平滑代理损失，建立了平方根增长率的普适性结果。通过最小化差距分析为代理损失选择提供指导。

Conclusion: H-一致性边界为代理损失与目标损失之间的关系提供了比贝叶斯一致性更强的理论保证，建立了统一的理论框架，覆盖了多种损失函数和场景，为机器学习中的损失函数设计和选择提供了理论基础。
...
### [392] [Theory and Algorithms for Learning with Multi-Class Abstention and Multi-Expert Deferral](https://arxiv.org/abs/2512.22886)
*Anqi Mao*
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉和高推理成本问题，通过将不确定输入延迟给更专业的专家可以提高可靠性，将简单查询路由给小型蒸馏模型可以提高效率，这激发了多专家延迟学习问题的研究。

Method: 1) 针对分类中的弃权问题，提出了基于分数和预测器-拒绝器的新损失函数；2) 针对多专家延迟分类，设计了单阶段和两阶段场景的损失函数；3) 针对回归延迟，提出了支持多专家和多种成本结构的通用框架。

Result: 1) 解决了弃权学习中的两个开放性问题，在CIFAR-10、CIFAR-100和SVHN上展示了算法优越性；2) 证明了多专家延迟损失函数具有强H-一致性边界；3) 提出了回归延迟的新损失函数并证明了其有效性。

Conclusion: 该论文为多专家延迟学习提供了全面的理论框架和算法设计，包括分类弃权、多专家延迟分类和回归延迟，所有方法都具有强一致性保证，为实际应用提供了可靠的理论基础。
...
### [393] [Federated Multi-Task Clustering](https://arxiv.org/abs/2512.22897)
*S. Dai,G. Sun,F. Li,X. Tang,Q. Wang,Y. Cong*

Main category: cs.LG

TL;DR: 提出FMTC框架，通过客户端个性化聚类模块和服务器端张量关联模块，在保护隐私的同时为异构客户端学习个性化聚类模型并利用共享知识。


<details>
  <summary>Details</summary>
Motivation: 传统谱聚类算法主要针对集中式设置，不适用于现代去中心化环境。现有的联邦学习方法依赖不可靠的伪标签导致泛化性能差，且未能捕捉异构客户端间的潜在相关性。

Method: FMTC框架包含两个主要组件：1) 客户端个性化聚类模块，学习参数化映射模型支持鲁棒的样本外推理，避免使用不可靠伪标签；2) 服务器端张量关联模块，将所有客户端模型组织成统一张量并应用低秩正则化发现其公共子空间。采用基于ADMM的高效隐私保护分布式算法。
...
### [394] [MetaCD: A Meta Learning Framework for Cognitive Diagnosis based on Continual Learning](https://arxiv.org/abs/2512.22904)
*Jin Wu,Chanjin Zheng*

Main category: cs.LG

TL;DR: 提出基于持续学习的元学习认知诊断框架MetaCD，通过元学习缓解长尾分布问题，利用参数保护机制适应动态变化，在五个真实数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有认知诊断方法受限于数据的长尾分布和动态变化问题，需要能够同时处理数据不平衡和适应新技能/任务的方法。
...
### [395] [Sat-EnQ: Satisficing Ensembles of Weak Q-Learners for Reliable and Compute-Efficient Reinforcement Learning](https://arxiv.org/abs/2512.22910)
*Ünver Çiftçi*

Main category: cs.LG

TL;DR: Sat-EnQ：一个两阶段深度Q学习框架，先通过"满意化"学习避免早期训练不稳定，再优化到最优


<details>
  <summary>Details</summary>
Motivation: 深度Q学习算法在早期训练时非常不稳定，最大化操作会放大估计误差，导致灾难性高估。受有限理性理论和发育学习启发，需要一种更稳健的方法来避免早期训练失败。

Method: 两阶段框架：第一阶段使用轻量级Q网络集成，在"满意化"目标下训练，通过动态基线限制早期价值增长，产生多样化、低方差的估计；第二阶段将集成蒸馏到更大的网络中，用标准Double DQN进行微调。

Result: 理论上证明满意化诱导有界更新且不会增加目标方差；实证上实现3.8倍方差减少，消除灾难性失败（0% vs DQN的50%），在环境噪声下保持79%性能，计算需求比自举集成少2.5倍。

Conclusion: 通过先满意化后优化的原则性路径，为稳健强化学习提供了新方向，展示了有限理性理论在解决深度强化学习稳定性问题上的价值。
...
### [396] [Multiple Token Divergence: Measuring and Steering In-Context Computation Density](https://arxiv.org/abs/2512.22944)
...
<details>
  <summary>Details</summary>
Motivation: 现有衡量语言模型计算努力的方法（如下一个令牌损失）无法捕捉推理复杂性，而基于潜在状态可压缩性的方法具有侵入性和不稳定性，需要一种更简单有效的测量工具。

Method: 提出MTD指标，定义为模型完整输出分布与浅层辅助预测头输出分布之间的KL散度。该方法可直接从具有多个预测头的预训练模型计算，无需额外训练。基于MTD开发了Divergence Steering解码方法。

Result: MTD比现有方法更能有效区分复杂任务与简单任务；在数学推理基准测试中，MTD与问题难度呈正相关；较低的MTD与更准确的推理相关。

Conclusion: MTD为分析和引导语言模型的计算动态提供了一个实用、轻量级的工具，能够更好地衡量模型的计算努力并控制生成文本的计算特性。
...
### [397] [APO: Alpha-Divergence Preference Optimization](https://arxiv.org/abs/2512.22953)
*Wang Zixian*

Main category: cs.LG

TL;DR: APO提出了一种基于alpha散度的锚定偏好优化框架，能够在正向KL和反向KL之间连续插值，通过奖励和置信度引导的alpha调度策略，在保持训练稳定性的同时实现从覆盖到利用的过渡。


<details>
  <summary>Details</summary>
Motivation: 当前对齐方法主要分为两种散度机制：监督微调等使用正向KL散度（模式覆盖但可能未充分利用高奖励模式），PPO等在线RLHF使用反向KL散度（模式寻求但可能导致模式崩溃）。现有锚定方法通常只使用单一散度，需要一种能连续插值两种行为的统一框架。

Method: 提出Alpha-Divergence Preference Optimization (APO)，使用Csiszar alpha散度在锚定几何中连续插值正向和反向KL行为。推导了参数化alpha的统一梯度动态，分析了梯度方差特性，并提出奖励和置信度引导的alpha调度策略，仅在策略改进且置信度校准良好时才从覆盖过渡到利用。

Result: 在Qwen3-1.7B模型和math-level3任务上的实验表明，APO在保持训练稳定性的同时，实现了与GRPO和GSPO基线竞争的性能。

Conclusion: APO提供了一个统一的锚定框架，通过alpha散度连续插值正向和反向KL行为，结合奖励和置信度引导的调度策略，在模式覆盖和模式寻求之间实现了更好的平衡，同时保持了训练稳定性。
...
### [398] [FLOW: A Feedback-Driven Synthetic Longitudinal Dataset of Work and Wellbeing](https://arxiv.org/abs/2512.22956)
*Wafaa El Husseini*

Main category: cs.LG

TL;DR: FLOW是一个合成的纵向数据集，用于模拟工作负荷、生活方式行为和幸福感之间的日常互动，支持压力建模、行为分析和机器学习研究。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、伦理和后勤限制，获取关于工作与生活平衡和幸福感的纵向个体级数据有限，这对可重复研究、方法基准测试和教育构成了挑战。

Method: 使用基于规则的反馈驱动模拟生成合成数据，模拟压力、睡眠、情绪、体育活动和体重等变量之间的连贯时间动态，包含1000个个体两年的每日数据。

Result: 创建了FLOW数据集和可配置的数据生成工具，作为一个公开可用的资源，支持在真实世界数据不可访问时的探索性分析、方法开发和基准测试。

Conclusion: FLOW旨在作为受控的实验环境而非真实人群的代理，为压力建模、行为分析和机器学习等领域提供可重复实验的基础设施。
...
### [399] [A Context-Aware Temporal Modeling through Unified Multi-Scale Temporal Encoding and Hierarchical Sequence Learning for Single-Channel EEG Sleep Staging](https://arxiv.org/abs/2512.22976)
*Amirali Vakili,Salar Jahanshiri,Armin Salimi-Badr*

Main category: cs.LG

TL;DR: 提出一个用于单通道EEG睡眠分期的上下文感知和可解释框架，特别关注改善N1阶段的检测，在SleepEDF数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 自动睡眠分期对睡眠障碍诊断至关重要，但现有方法面临类别不平衡、感受野有限、可解释性不足等问题，特别是N1阶段检测困难。

Method: 结合紧凑多尺度特征提取与时间建模，使用类别加权损失和数据增强处理不平衡，将EEG信号分块处理并通过平均softmax概率获得最终预测。

Result: 在SleepEDF数据集上达到89.72%总体准确率和85.46%宏平均F1分数，N1阶段F1分数达61.7%，显著优于先前方法。

Conclusion: 该框架有效提升了睡眠分期性能，同时保持了可解释性和临床适用性，为单通道EEG睡眠分期提供了实用解决方案。
...
### [400] [Fusion or Confusion? Multimodal Complexity Is Not All You Need](https://arxiv.org/abs/2512.22991)
...
<details>
  <summary>Details</summary>
Motivation: 挑战当前多模态学习领域普遍假设：更复杂的多模态特定方法能带来更好的性能。通过大规模实证研究检验这一假设

Method: 提出SimBaMM（简单多模态学习基准），一个简单的后期融合Transformer架构。在标准化条件下重新实现19种高影响力方法，在9个数据集（最多23个模态）上评估，测试其泛化能力

Result: 在标准化实验条件下，复杂架构并不比SimBaMM更可靠地表现出色。统计分析显示复杂方法与SimBaMM表现相当，且经常无法可靠地超越调优良好的单模态基准

Conclusion: 多模态学习领域需要从追求架构新颖性转向方法论严谨性，提供实用可靠性检查表以促进可比、稳健和可信的未来评估
...
### [401] [Merge before Forget: A Single LoRA Continual Learning via Continual Merging](https://arxiv.org/abs/2512.23017)
*Fuli Qiao,Mehrdad Mahdavi*

Main category: cs.LG

TL;DR: 提出一种新的持续学习方法，通过正交初始化和顺序合并LoRA更新到单个统一LoRA中，解决现有方法的内存增长和任务干扰问题。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA持续学习方法存在内存随任务增长、存储空间有限以及缺乏有效合并机制导致任务干扰的问题。

Method: 正交初始化新任务学习，顺序合并LoRA更新到单个统一LoRA，使用时感知缩放机制平衡新旧知识。

Result: 方法保持恒定内存复杂度，最小化任务间干扰，通过自适应缩放提升性能，在多个基准测试中验证有效性。

Conclusion: 提出的正交初始化和顺序合并方法在保持内存效率的同时，有效解决了持续学习中的灾难性遗忘和任务干扰问题。
...
### [402] [Mechanistic Analysis of Circuit Preservation in Federated Learning](https://arxiv.org/abs/2512.23043)
*Muhammad Haseeb,Salaar Masood,Muhammad Abdullah Sohail*

Main category: cs.LG

TL;DR: 通过机制可解释性分析发现，非独立同分布数据导致联邦学习中不同客户端的稀疏子网络（电路）发生冲突和退化，而非简单的统计漂移问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非独立同分布数据下性能显著下降，虽然这一现象已被广泛记录，但其内部机制原因仍不明确。本文旨在通过机制可解释性来诊断FedAvg算法在这种情况下的失败模式

Method: 在联邦学习框架内训练具有内在可解释性的权重稀疏神经网络，使用交并比量化电路保持情况，跟踪不同客户端和通信轮次中的电路变化

Result: 提供了首个机制证据表明，非独立同分布数据分布导致结构不同的本地电路发散，进而在全局模型中退化。发现客户端更新冲突导致电路崩溃，即负责特定类别预测的功能性稀疏子网络发生破坏性干扰

Conclusion: 将联邦学习中的统计漂移问题重新定义为机制保持的具体可观察失败，为更针对性的解决方案铺平了道路
...
### [403] [PI-MFM: Physics-informed multimodal foundation model for solving partial differential equations](https://arxiv.org/abs/2512.23056)
*Min Zhu,Jingmin Sun,Zecheng Zhang,Hayden Schaeffer,Lu Lu*

Main category: cs.LG

TL;DR: 提出物理信息多模态基础模型(PI-MFM)框架，通过自动组装PDE残差损失，在预训练和适应阶段直接强制执行控制方程，实现跨方程族的数据高效PDE求解器学习。


<details>
  <summary>Details</summary>
Motivation: 现有多算子学习方法数据需求大且训练时忽略物理规律，需要开发能够直接利用物理约束、数据效率更高的PDE求解器学习框架。

Method: PI-MFM以PDE的符号表示为输入，通过向量化导数计算自动组装PDE残差损失，使任何PDE编码多模态基础模型都能用统一的物理信息目标进行训练和适应。

Result: 在13个参数化一维时间依赖PDE族基准测试中，PI-MFM始终优于纯数据驱动方法，尤其在稀疏标记时空点、部分观测时间域或少量标记函数对情况下。物理损失提高了噪声鲁棒性，重采样配置点等简单策略显著提升精度。

Conclusion: PI-MFM为数据高效、可迁移的PDE求解器提供了实用且可扩展的路径，支持零样本物理信息微调到未见PDE族，仅使用PDE残差和初始/边界条件就能快速降低测试误差至约1%。
...
### [404] [Breaking the Memory Wall: Exact Analytical Differentiation via Tiled Operator-Space Evolution](https://arxiv.org/abs/2512.23068)
*Shuhuan Wang,Yuzhen Xie,Jiayi Li,Yinliang Diao*

Main category: cs.LG

TL;DR: PGF框架通过状态空间流形直接计算精确解析导数，实现O(1)内存复杂度，解决SSM在基因组规模建模中的内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 选择性状态空间模型(SSMs)虽然具有线性时间推理能力，但在反向传播期间存在O(L)内存缩放瓶颈，这阻碍了在消费级硬件上进行基因组规模建模(L > 10^5)。

Method: 提出相位梯度流(PGF)框架，通过将SSM动态重构为平铺算子空间演化(TOSE)，直接在状态空间流形上操作，避免中间计算图的物化，实现O(1)内存复杂度。

Result: 相比标准Autograd，PGF实现了94%的峰值VRAM减少和23倍吞吐量提升，在128,000步序列的脉冲响应基准测试中保持稳定，支持单GPU染色体规模敏感性分析。

Conclusion: PGF框架解决了SSM在基因组规模建模中的内存瓶颈，弥合了理论无限上下文模型与实际硬件限制之间的差距，使染色体规模敏感性分析在单GPU上成为可能。
...
### [405] [FLEX-MoE: Federated Mixture-of-Experts with Load-balanced Expert Assignment](https://arxiv.org/abs/2512.23070)
*Boyang Zhang,Xiaobing Chen,Songyang Zhang,Shuai Zhang,Xiangwei Zhou,Mingxuan Sun*

Main category: cs.LG

TL;DR: FLEX-MoE：一种联邦学习中的MoE框架，通过联合优化专家分配和负载平衡来解决边缘设备资源受限和非IID数据导致的专家负载不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中的MoE模型面临两个关键挑战：1）资源受限的边缘设备无法存储完整的专家集合；2）非IID数据分布导致严重的专家负载不平衡，从而降低模型性能。

Method: 提出FLEX-MoE框架，引入客户端-专家适应度分数来量化专家对本地数据集的适用性，并使用基于优化的算法在系统范围内最大化客户端-专家专业化，同时强制平衡专家利用率。

Result: 在三个不同数据集上的综合实验表明，FLEX-MoE具有优越的性能，并能在多样化的资源受限场景中保持平衡的专家利用率。

Conclusion: FLEX-MoE通过联合优化专家分配和负载平衡，有效解决了联邦学习中MoE模型的部署挑战，特别是在异构数据环境下，优于现有的仅关注个性化而忽略负载不平衡的贪心方法。
...
### [406] [Rethinking Fine-Tuning: Unlocking Hidden Capabilities in Vision-Language Models](https://arxiv.org/abs/2512.23073)
*Mingyuan Zhang,Yue Bai,Yifan Wang,Yiyang Huang,Yun Fu*

Main category: cs.LG

TL;DR: 本文提出将掩码微调（MFT）应用于视觉语言模型（VLM），通过为权重分配可学习的门控分数来重组内部子网络，而不是更新权重，实现了比LoRA变体和全微调更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM微调方法（如LoRA）主要依赖显式的权重更新，忽略了预训练模型中已编码的丰富表示结构。这些结构未被充分利用，而MFT在语言模型中已被证明是高效的后训练范式。

Method: 从结构重参数化角度重新思考VLM微调，将MFT应用于VLM的语言和投影器组件。不为权重分配可学习的门控分数，让模型为下游任务重组内部子网络，而不改变冻结的主干网络。

Result: 实验表明MFT在多种语言骨干上一致超越LoRA变体，甚至优于全微调，在不改变冻结主干的情况下实现了高性能。

Conclusion: 有效适应不仅可以通过更新权重实现，还可以通过重新建立模型现有知识之间的连接来实现。MFT为VLM微调提供了新的高效范式。
...
### [407] [Trust Region Masking for Long-Horizon LLM Reinforcement Learning](https://arxiv.org/abs/2512.23075)
*Yingru Li,Jiacai Liu,Jiawei Xu,Yuxuan Tong,Ziniu Li,Baoxiang Wang*

Main category: cs.LG

TL;DR: 论文提出Trust Region Masking (TRM)方法，通过排除违反信任区域的整个序列来解决长序列RL中PPO等方法的近似误差问题，提供非空单调改进保证。


<details>
  <summary>Details</summary>
Motivation: 大语言模型策略梯度方法中，当rollout策略与目标策略不同时会产生近似误差。传统信任区域边界随序列长度T呈O(T²)增长，对长序列任务无效，需要更紧的边界和有效的控制方法。

Method: 推导了两种更紧的边界：Pinsker-Marginal边界(O(T^{3/2}))和Mixed边界(O(T))，两者都依赖于序列级最大token级KL散度。提出Trust Region Masking (TRM)，如果序列中任何token违反信任区域，就排除整个序列的梯度计算。

Result: TRM为长序列LLM-RL提供了第一个非空单调改进保证，通过序列级控制解决了传统token独立方法（如PPO裁剪）无法处理的问题。

Conclusion: 序列级信任区域控制对于长序列LLM-RL至关重要，TRM通过排除违反信任区域的整个序列，实现了有效的策略改进保证，解决了传统方法的局限性。
...
TL;DR: MFMC提出了一种基于双总相关性的自监督学习框架，通过最大化高阶多模态依赖性来捕捉情感状态中大脑与自主神经系统的协调响应，在多个情感计算基准上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 情感状态表现为中枢与自主神经系统之间协调但异质的生理响应，这对多模态表示学习提出了挑战。现有自监督学习方法主要依赖成对对齐目标，无法充分表征两个以上模态之间的依赖关系，也无法捕捉大脑与自主神经响应产生的高阶交互作用。

Method: 提出多模态功能最大相关（MFMC）框架，通过双总相关性（DTC）目标最大化高阶多模态依赖性。该方法推导出紧密的三明治边界，并使用基于功能最大相关分析（FMCA）的迹替代进行优化，直接捕捉联合多模态交互，不依赖成对对比损失。

Result: 在三个公开情感计算基准测试中，MFMC在受试者依赖和受试者独立评估协议下均达到SOTA或竞争性性能。在CEAP-360VR数据集上，仅使用EDA信号就将受试者依赖准确率从78.9%提升到86.8%，受试者独立准确率从27.5%提升到33.1%。在MAHNOB-HCI最具挑战性的EEG受试者独立划分上，MFMC与最佳方法相差不到0.8个百分点。

Conclusion: MFMC通过最大化高阶多模态依赖性，有效解决了情感计算中多模态表示学习的挑战，特别是在处理大脑与自主神经系统协调响应方面表现出色，对受试者间变异性具有鲁棒性。
...
### [409] [Taming the Tail: Stable LLM Reinforcement Learning via Dynamic Vocabulary Pruning](https://arxiv.org/abs/2512.23087)
*Yingru Li,Jiawei Xu,Jiacai Liu,Yuxuan Tong,Ziniu Li,Tianle Cai,Ge Zhang,Qian Liu,Baoxiang Wang*

Main category: cs.LG

TL;DR: 论文提出通过动态剪枝"安全"词汇表来解决LLM强化学习中训练-推理不匹配问题，用有界的优化偏差替代系统性偏差，实现稳定训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型强化学习面临训练-推理不匹配问题：高吞吐量推理引擎和数值精确训练系统从相同参数产生不同的概率分布，这种不匹配对低概率尾部token影响更大，导致系统性偏差累积，破坏梯度估计稳定性。

Method: 提出将RL目标约束到动态剪枝的"安全"词汇表，排除极端尾部token，用小的有界优化偏差替代大的系统性不匹配偏差，并通过理论分析界定了词汇剪枝引入的优化偏差。

Result: 该方法实现了稳定的训练效果，理论上界定了词汇剪枝引入的优化偏差，解决了训练-推理不匹配导致的梯度估计不稳定问题。

Conclusion: 通过动态剪枝词汇表排除极端尾部token是解决LLM强化学习中训练-推理不匹配的有效方法，用可控的优化偏差替代系统性偏差，实现稳定训练。
...
### [410] [Osmotic Learning: A Self-Supervised Paradigm for Decentralized Contextual Data Representation](https://arxiv.org/abs/2512.23096)
...
Method: 提出渗透学习（OSM-L），核心是渗透过程：通过提取上下文信息合成密集紧凑表示，无需原始数据交换。迭代对齐本地数据表示，实现信息扩散和收敛到动态平衡，捕捉上下文模式。训练中还识别相关数据组，作为去中心化聚类机制。

Result: 在结构化数据集上验证了OSM-L的收敛性和表示能力，本地信息对齐准确率超过0.99，同时保持上下文完整性。

Conclusion: OSM-L是一种有效的自监督分布式学习范式，能够从分布式数据中提取高层潜在知识，实现高效的信息对齐和上下文模式捕捉，具有实际应用价值。
...
### [411] [How Much Data Is Enough? Uniform Convergence Bounds for Generative & Vision-Language Models under Low-Dimensional Structure](https://arxiv.org/abs/2512.23109)
*Paul M. Thompson*

Main category: cs.LG

TL;DR: 该论文研究生成模型和视觉语言模型在科学医疗决策中的均匀准确性和校准问题，提出基于低维语义表示的有限样本均匀收敛理论，为数据有限的生物医学建模提供理论指导。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型和视觉语言模型在科学医疗决策中广泛应用，但需要确保预测概率既准确又校准良好。尽管在中等数据量下表现出色，但尚不清楚这些预测是否能在输入、类别或亚群中均匀泛化，而不仅仅是平均表现良好。这在生物医学中尤为关键，因为罕见病症和特定群体即使总体损失较低也可能出现较大误差。

Method: 从有限样本角度研究问题，聚焦于通过改变提示或语义嵌入在受限表示空间内获得的诱导分类器族。假设模型输出对低维语义表示具有平滑依赖性（这一假设得到文本和联合图像-文本嵌入中谱结构的支持），应用经典均匀收敛工具获得有意义的非渐近保证。主要结果给出了在提示嵌入的Lipschitz稳定性条件下，VLM诱导分类器的准确性和校准泛函的有限样本均匀收敛界。

Result: 推导出的样本复杂度取决于内在/有效维度，而非环境嵌入维度。进一步推导了谱相关边界，明确展示了特征值衰减如何控制数据需求。这些结果为数据有限的生物医学建模提供了理论指导，包括当前数据集大小何时能够支持均匀可靠的预测，以及为什么平均校准指标可能遗漏最坏情况下的校准错误。

Conclusion: 该研究为生成模型和视觉语言模型在生物医学决策中的可靠应用提供了理论框架。通过假设低维语义表示和Lipschitz稳定性，能够获得有限样本下的均匀收敛保证，这对于确保模型在罕见病症和特定群体中的可靠性至关重要。研究还揭示了为什么仅依赖平均校准指标可能不足以评估模型在最坏情况下的表现。
...
### [412] [SE-MLP Model for Predicting Prior Acceleration Features in Penetration Signals](https://arxiv.org/abs/2512.23131)
*Yankang Li,Changsheng Li*

Main category: cs.LG

TL;DR: 本文提出了一种结合通道注意力机制和残差连接的多层感知器架构（SE-MLP），用于快速预测侵彻加速度特征值，解决了传统仿真计算耗时昂贵的问题。
...
Method: 提出SE-MLP模型，集成通道注意力机制和残差连接，以不同工况下的物理参数为输入，输出分层加速度特征，建立物理参数与侵彻特性之间的非线性映射。

Result: 相比传统MLP、XGBoost和Transformer模型，SE-MLP在预测精度、泛化能力和稳定性方面表现更优；消融实验证实通道注意力模块和残差结构对性能提升均有显著贡献；数值仿真和射程恢复测试显示预测与实测加速度峰值和脉冲宽度的差异在工程允许范围内。
...
### [413] [Principled Algorithms for Optimizing Generalized Metrics in Binary Classification](https://arxiv.org/abs/2512.23133)
*Anqi Mao,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 提出METRO算法，用于优化类别不平衡或代价敏感场景下的广义评价指标（如Fβ、AM、Jaccard等），通过将指标优化转化为广义代价敏感学习问题，设计具有H-一致性保证的代理损失函数。


<details>
  <summary>Details</summary>
Motivation: 在类别不平衡或代价不对称的应用中，传统分类损失不适用，需要优化Fβ、AM、Jaccard等广义指标。现有方法依赖贝叶斯最优分类器，使用阈值法先估计类别概率再优化阈值，导致算法不适用于受限假设集且缺乏有限样本性能保证。

Method: 将广义指标优化重新表述为广义代价敏感学习问题，设计具有H-一致性保证的新型代理损失函数。基于此框架开发METRO算法，提供有限样本泛化界和理论性能保证。

Result: 实验结果表明，METRO方法相比现有基线方法更有效。算法具有H-一致性保证和有限样本泛化界，为广义指标优化提供了理论支撑。

Conclusion: 提出了理论上有保证的广义指标优化新方法，解决了现有阈值法在受限假设集和有限样本性能方面的局限性，为类别不平衡和代价敏感学习提供了更可靠的优化框架。
...
### [414] [Graph Neural Networks with Transformer Fusion of Brain Connectivity Dynamics and Tabular Data for Forecasting Future Tobacco Use](https://arxiv.org/abs/2512.23137)
*Runzhi Zhou,Xi Luo*

Main category: cs.LG

TL;DR: GNN-TF模型结合图神经网络与Transformer，有效整合非欧几里得脑成像数据与欧几里得表格数据，在纵向fMRI研究中预测未来烟草使用方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 纵向脑成像研究中，非欧几里得脑成像数据（如动态脑连接）与欧几里得表格数据（临床和人口统计信息）的有效整合是一个重大挑战，特别是在预测未来临床结果方面。

Method: 提出时间感知的图神经网络与Transformer融合模型（GNN-TF），灵活整合表格数据和动态脑连接数据，利用变量时间顺序，在统一框架中处理多模态数据。
...
### [415] [A Weak Signal Learning Dataset and Its Baseline Method](https://arxiv.org/abs/2512.23160)
*Xianqi Liu,Xiangru Li,Lefeng He,Ziyu Fang*

Main category: cs.LG

TL;DR: 该研究构建了首个弱信号特征学习专用数据集（13,158个光谱样本），并提出PDVFN模型处理低信噪比和类别不平衡问题，为弱信号学习领域提供了基准。


<details>
  <summary>Details</summary>
Motivation: 弱信号学习在故障诊断、医学影像、自动驾驶等领域面临挑战，关键信息常被噪声掩盖，特征识别困难。但缺乏专用数据集长期制约研究进展，需要建立基准数据集和专门方法。

Method: 构建首个弱信号特征学习专用数据集（13,158个光谱样本，55%以上样本SNR低于50，类别不平衡达29:1）。提出PDVFN模型，采用双视图表示（向量+时频图），并行提取局部序列特征和全局频域结构，遵循局部增强、序列建模、噪声抑制、多尺度捕获、频率提取和全局感知原则。

Result: 实验表明该方法在处理弱信号、高噪声和极端类别不平衡时具有更高的准确性和鲁棒性，尤其在低SNR和不平衡场景下表现优异。

Conclusion: 该研究为弱信号学习提供了专用数据集、基线模型和研究基础，提出的多源互补增强表示方法为天文光谱学等WSL任务提供了新颖解决方案。
...
### [416] [Diffusion-based Decentralized Federated Multi-Task Representation Learning](https://arxiv.org/abs/2512.23161)
*Donghwa Kang,Shana Moothedath*
...
Result: 获得了构造性的理论保证，包括所需样本复杂度的下界和算法迭代复杂度的上界。分析显示算法具有快速和通信高效的特点，数值模拟验证了其性能优于基准算法。

Conclusion: 提出的分散式多任务表示学习算法在理论保证和实际性能方面都表现出色，为数据稀缺环境下的分散式表示学习提供了有效的解决方案。
...
### [417] [Evaluating Parameter Efficient Methods for RLVR](https://arxiv.org/abs/2512.23165)
*Qingyu Yin,Yulun Wu,Zhennan Shen,Sunbowen Li,Zhilin Wang,Yanshu Li,Chak Tou Leong,Jiale Kang,Jinjin Gu*

Main category: cs.LG

TL;DR: 本文首次系统评估了12种参数高效微调(PEFT)方法在可验证奖励强化学习(RLVR)范式下的表现，发现结构变体(DoRA、AdaLoRA、MiSS)优于标准LoRA，SVD初始化策略存在谱崩溃问题，极端参数缩减会严重限制推理能力。


<details>
  <summary>Details</summary>
Motivation: 虽然LoRA等PEFT方法在RLVR（通过可验证反馈增强语言模型推理能力）中常用，但最优的PEFT架构仍未确定。本研究旨在填补这一空白，为参数高效的RL方法提供指导。

Method: 在DeepSeek-R1-Distill系列模型上，对超过12种PEFT方法进行系统性评估，使用数学推理基准测试。包括结构变体(DoRA、AdaLoRA、MiSS)、SVD初始化策略(PiSSA、MiLoRA)和极端参数缩减方法(VeRA、Rank-1)的比较分析。

Result: 1. 结构变体(DoRA、AdaLoRA、MiSS)持续优于标准LoRA；2. SVD初始化策略存在谱崩溃现象，主成分更新与RL优化存在根本性不匹配；3. 极端参数缩减严重限制推理能力；4. 通过消融实验和扩展实验验证了这些发现。

Conclusion: 本研究挑战了标准LoRA的默认采用，为参数高效的RL方法提供了明确的指导，呼吁对PEFT方法在RLVR范式下进行更多探索，特别是结构变体而非SVD初始化策略。
...
### [418] [HELM-BERT: A Transformer for Medium-sized Peptide Property Prediction](https://arxiv.org/abs/2512.23175)
*Seungeon Lee,Takuto Koyama,Itsuki Maeda,Shigeyuki Matsumoto,Yasushi Okuno*

Main category: cs.LG

TL;DR: HELM-BERT：首个基于HELM符号的肽语言模型，通过分层编辑语言捕获肽的化学修饰和拓扑结构，在膜渗透性和蛋白相互作用预测任务中超越SMILES模型


<details>
  <summary>Details</summary>
Motivation: 现有分子语言模型无法充分捕捉治疗性肽的化学复杂性：原子级SMILES符号产生长序列且模糊环状拓扑，氨基酸级表示无法编码现代肽设计中关键的化学修饰。需要一种能统一描述单体组成和连接性的表示方法。

Method: 提出HELM-BERT，首个基于HELM（分层编辑语言）符号的编码器型肽语言模型。基于DeBERTa架构，专门设计用于捕获HEL序列中的分层依赖关系。在包含39,079个化学多样性肽（包括线性和环状结构）的语料库上进行预训练。

Result: HELM-BERT在下游任务中显著优于最先进的SMILES语言模型：在环肽膜渗透性预测和肽-蛋白相互作用预测任务中表现优异。证明HELM的明确单体和拓扑感知表示在建模治疗性肽方面具有显著的数据效率优势。

Conclusion: HELM-BERT填补了小分子和蛋白质语言模型之间的长期空白，通过HELM的统一框架实现了对肽化学复杂性的精确建模，为加速肽药物发现提供了有效工具。
...
### [419] [Machine Learning-Assisted Vocal Cord Ultrasound Examination: Project VIPR](https://arxiv.org/abs/2512.23177)
*Will Sebelik-Lassiter,Evan Schubert,Muhammad Alliyu,Quentin Robbins,Excel Olatunji,Mustafa Barry*

Main category: cs.LG

TL;DR: 开发机器学习辅助算法，通过声带超声自动识别声带并区分正常声带与声带麻痹图像


<details>
  <summary>Details</summary>
Motivation: 声带超声检查虽然创伤小、耐受性好，但其准确性高度依赖操作者经验，需要开发自动分析工具提高诊断准确性
...
### [420] [A Simple, Optimal and Efficient Algorithm for Online Exp-Concave Optimization](https://arxiv.org/abs/2512.23190)
*Yi-Han Wang,Peng Zhao,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: LightONS：一种改进的在线牛顿步算法，将OXO总运行时间从O(d^ωT)降低到O(d^2T + d^ω√T)，同时保持最优O(d log T)遗憾界，解决了COLT'13关于SXO运行时间的开放问题。


<details>
  <summary>Details</summary>
Motivation: 传统ONS算法在每次迭代中需要进行Mahalanobis投影，计算成本高达Ω(d^ω)，导致总运行时间达到O(d^ωT)。对于随机指数凹优化(SXO)，使用在线到批量转换需要O(d^{ω+1}/ε)的运行时间，这成为COLT'13提出的开放问题。

Method: 提出LightONS算法，通过引入滞后机制延迟昂贵的Mahalanobis投影，只在必要时执行。利用参数无关在线学习中的域转换技术，保持ONS的优雅结构，但显著减少计算开销。

Result: LightONS将总运行时间降低到O(d^2T + d^ω√T log T)，同时保持最优O(d log T)遗憾界。对于SXO问题，运行时间降低到O(d^3/ε)，成功解决了COLT'13开放问题。

Conclusion: LightONS不仅解决了SXO运行时间的开放问题，还能作为ONS的高效替代方案应用于更广泛场景，包括梯度范数自适应遗憾、参数化随机赌博机、内存高效在线学习等。
...
### [421] [PGOT: A Physics-Geometry Operator Transformer for Complex PDEs](https://arxiv.org/abs/2512.23192)
*Zhuo Zhang,Xi Yang,Yuan Zhao,Canqun Yang*

Main category: cs.LG

TL;DR: PGOT是一种针对大规模非结构化网格PDE建模的Transformer架构，通过显式几何感知和谱保持几何注意力解决几何混叠问题，实现空间自适应的高精度物理场建模。


<details>
  <summary>Details</summary>
Motivation: 现有高效架构在特征降维时会导致几何混叠，丢失关键物理边界信息，难以处理大规模非结构化网格和复杂几何形状的PDE建模问题。

Method: 提出PGOT架构，包含谱保持几何注意力模块，采用"物理切片-几何注入"机制引入多尺度几何编码，同时保持线性计算复杂度。通过动态路由机制，根据空间坐标将计算分配到低阶线性路径（平滑区域）和高阶非线性路径（激波和不连续区域）。

Result: 在四个标准基准测试中取得一致的SOTA性能，并在翼型和汽车设计等大规模工业任务中表现出色。

Conclusion: PGOT通过显式几何感知和空间自适应计算路由，有效解决了大规模非结构化网格PDE建模中的几何混叠问题，实现了高精度物理场建模。
...
### [422] [Energy and Memory-Efficient Federated Learning With Ordered Layer Freezing](https://arxiv.org/abs/2512.23200)
*Ziru Niu,Hai Dong,A. K. Qin,Tao Gu,Pengcheng Zhang*

Main category: cs.LG

TL;DR: FedOLF提出了一种联邦学习优化方法，通过有序层冻结降低IoT设备计算内存需求，结合张量操作近似减少通信能耗，在非独立同分布数据上实现更高精度和能效。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在IoT边缘设备上面临计算能力、内存和带宽限制的挑战，现有方法如dropout或层冻结往往牺牲精度或忽视内存约束，需要更高效的解决方案。

Method: 提出FedOLF方法：1）在训练前按预定义顺序持续冻结层，显著降低计算和内存需求；2）结合Tensor Operation Approximation（TOA）作为轻量级量化替代方案，减少通信和能耗同时更好保持模型精度。

Result: 在非独立同分布数据上，FedOLF在多个数据集和模型上均优于现有方法：EMNIST（CNN）精度提高至少0.3%，CIFAR-10（AlexNet）提高6.4%，CIFAR-100（ResNet20/44）分别提高5.81%和4.4%，CINIC-10（ResNet20/44）分别提高6.27%和1.29%，同时具有更高能效和更低内存占用。

Conclusion: FedOLF通过有序层冻结和TOA技术，有效解决了IoT边缘设备在联邦学习中的计算、内存和通信限制问题，在保持精度的同时显著提升了能效和可扩展性。
...
### [423] [FairGFL: Privacy-Preserving Fairness-Aware Federated Learning with Overlapping Subgraphs](https://arxiv.org/abs/2512.23235)
*Zihao Zhou,Shusen Yang,Fangyuan Zhao,Xuebin Ren*

Main category: cs.LG

TL;DR: FairGFL：针对图联邦学习中不平衡重叠子图导致的公平性问题，提出隐私保护的公平性感知算法


<details>
  <summary>Details</summary>
Motivation: 图联邦学习中不同客户端子图存在不平衡重叠，现有研究只关注重叠数据的正面作用（缓解数据异质性），但未探索其负面影响，特别是重叠不平衡导致的公平性问题

Method: 提出FairGFL算法：1）基于隐私保护的重叠率估计，采用可解释的加权聚合方法增强跨客户端公平性；2）在联邦复合损失函数中集成精心设计的正则化器，平衡模型效用与公平性

Result: 在四个基准图数据集上的实验表明，FairGFL在模型效用和公平性方面均优于四种代表性基线算法

Conclusion: 图联邦学习中不平衡重叠子图会导致公平性问题，FairGFL通过隐私保护的公平性增强机制有效解决了这一问题，在保持模型效用的同时提升了跨客户端公平性
...
### [424] [KernelEvolve: Scaling Agentic Kernel Coding for Heterogeneous AI Accelerators at Meta](https://arxiv.org/abs/2512.23236)
*Gang Liao,Hongsen Qin,Ying Wang,Alicia Golden,Michael Kuchnik,Yavuz Yetim,Jia Jiunn Ang,Chunli Fu,Yihan He,Samuel Hsia,Zewei Jiang,Dianshi Li,Uladzimir Pashkevich,Varna Puvvada,Feng Shi,Matt Steiner,Ruichao Xiao,Nathan Yan,Xiayu Yu,Zhou Fang,Abdul Zainul-Abedin,Ketan Singh,Hongtao Yu,Wenyuan Chi,Barney Huang,Sean Zhang,Noah Weller,Zach Marine,Wyatt Cook,Carole-Jean Wu,Gaoxiang Liu*

Main category: cs.LG

TL;DR: KernelEvolve是一个面向推荐模型的自动化内核编码框架，通过多级编程抽象和基于图的搜索策略，为异构硬件架构自动生成和优化内核，显著提升开发效率和性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习推荐模型（DLRM）的训练和推理需要高效快速，但面临模型架构多样性、内核原语多样性以及硬件代际和架构异构性三大系统挑战。现有方法难以应对这些异构性挑战。

Method: 提出KernelEvolve框架，采用多级编程抽象（从Triton/CuTe DSL到底层硬件无关语言），通过基于图的搜索策略（包含选择策略、通用算子、适应度函数和终止规则），并利用检索增强的提示合成动态适应运行时执行上下文。

Result: 在KernelBench测试套件上实现100%通过率（250个问题，三个难度级别），在三个异构硬件平台上验证160个PyTorch ATen算子，实现100%正确性。将开发时间从数周缩短到数小时，并在多种生产用例中显著超越PyTorch基线性能。

Conclusion: KernelEvolve不仅大幅提升了推荐模型的性能和开发效率，还通过为内部开发的AI硬件提供自动化内核生成能力，显著降低了新AI硬件的可编程性障碍。
...
### [425] [PFed-Signal: An ADR Prediction Model based on Federated Learning](https://arxiv.org/abs/2512.23262)
...
<details>
  <summary>Details</summary>
Motivation: FAERS（美国FDA不良事件报告系统）中的偏差记录可能导致药物不良反应预测不准确，误导在线诊断。传统基于统计方法（如ROR、PRR）无法消除数据偏差，导致信号预测不准确。

Method: 提出PFed-signal联邦学习模型：1) Pfed-Split方法按ADR分割原始数据集；2) ADR-signal模型包含基于联邦学习的偏差数据识别（使用欧氏距离）和基于Transformer的ADR预测模型；3) 识别并删除偏差数据生成干净数据集，在干净数据上训练Transformer预测模型。

Result: 干净数据集上的ROR和PRR优于传统方法。PFed-Signal的准确率0.887、F1分数0.890、召回率0.913、AUC 0.957，均优于基线模型。

Conclusion: PFed-signal通过联邦学习和欧氏距离有效消除FAERS数据偏差，显著提升ADR预测准确性，为在线诊断提供更可靠的信号预测。
...
### [426] [On the Inverse Flow Matching Problem in the One-Dimensional and Gaussian Cases](https://arxiv.org/abs/2512.23265)
*Alexander Korotin,Gudmund Pammer*

Main category: cs.LG

TL;DR: 研究流匹配逆问题，关注具有有限指数矩分布之间的流匹配，在生成式AI应用中具有重要性，如一维和高斯情况下解的唯一性得到证明，多维问题仍开放


<details>
  <summary>Details</summary>
Motivation: 现代生成式AI应用（如流匹配模型蒸馏）中，需要研究具有有限指数矩分布之间的流匹配逆问题

Method: 研究流匹配逆问题的数学理论，在一维和高斯两种特殊情况下建立解的唯一性

Result: 在一维设置和高斯情况下证明了流匹配逆问题解的唯一性，但一般多维问题仍然开放

Conclusion: 流匹配逆问题在特定条件下有唯一解，但一般多维情况需要进一步研究，为生成式AI应用提供理论基础
...
### [427] [Spectral Analysis of Hard-Constraint PINNs: The Spatial Modulation Mechanism of Boundary Functions](https://arxiv.org/abs/2512.23295)
*Yuchen Xie,Honghang Chi,Haopeng Quan,Yahui Wang,Wei Wang,Yu Ma*

Main category: cs.LG

TL;DR: HC-PINNs通过硬约束严格满足边界条件，但其训练动力学机制一直未被探索。本文建立了HC-PINNs的神经正切核框架，发现边界函数B作为谱滤波器重塑核的谱结构，有效秩可预测训练收敛性，常用边界函数可能导致谱崩溃。


<details>
  <summary>Details</summary>
Motivation: 硬约束物理信息神经网络（HC-PINNs）通过试函数形式严格满足边界条件，但缺乏对其训练动力学的理论理解。与软约束方法不同，边界函数B引入了乘法空间调制，改变了学习景观，需要建立理论框架来理解其优化行为。

Method: 建立了HC-PINNs的神经正切核（NTK）框架，推导了显式的核组合定律。通过谱分析研究边界函数B(x)如何作为谱滤波器重塑神经网络原生核的特征谱，识别残差核的有效秩作为训练收敛的确定性预测指标。

Result: 边界函数B(x)作为谱滤波器显著改变核的特征谱，有效秩比经典条件数更能预测训练收敛性。研究发现常用边界函数可能无意中导致谱崩溃，尽管精确满足边界条件，但优化会停滞。在多维基准测试中得到验证。

Conclusion: 该框架将边界函数设计从启发式选择转变为原则性的谱优化问题，为科学机器学习中的几何硬约束提供了坚实的理论基础。边界函数的选择应避免谱崩溃，优化谱特性以提高训练效率。
...
### [428] [Splitwise: Collaborative Edge-Cloud Inference for LLMs via Lyapunov-Assisted DRL](https://arxiv.org/abs/2512.23310)
*Abolfazl Younesi,Abbas Shabrang Maryan,Elyas Oustad,Zahra Najafabadi Samani,Mohsen Ansari,Thomas Fahringer*

Main category: cs.LG

TL;DR: Splitwise是一个基于Lyapunov辅助深度强化学习的框架，用于在边缘和云环境中对大型语言模型进行细粒度自适应分区，以降低延迟、能耗并保证队列稳定性。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署大型语言模型面临内存和功耗限制，纯云端推理延迟高且成本大，而静态分区方案无法适应带宽波动，需要一种自适应分区方法。

Method: 将Transformer层分解为注意力头和前馈子块，提供比层级分区更多的选择；采用层次化深度强化学习策略，结合Lyapunov优化，联合优化延迟、能耗和精度损失；通过分区检查点和指数退避恢复保证通信故障下的鲁棒性。

Result: 在Jetson Orin NX、Galaxy S23和Raspberry Pi 5上测试GPT-2 (1.5B)、LLaMA-7B和LLaMA-13B，Splitwise相比现有分区器将端到端延迟降低1.4-2.8倍，能耗降低达41%，相比纯云端执行将95百分位延迟降低53-61%，同时保持精度和适度内存需求。

Conclusion: Splitwise通过细粒度自适应分区有效解决了边缘设备部署LLM的挑战，在动态网络条件下显著提升了性能效率，为边缘-云协同推理提供了实用解决方案。
...
### [429] [Deep learning for pedestrians: backpropagation in Transformers](https://arxiv.org/abs/2512.23329)
*Laurent Boué*

Main category: cs.LG

TL;DR: 该论文延续先前CNN反向传播的向量化推导工作，将轻量级索引自由方法应用于Transformer架构，推导嵌入层、多头自注意力、层归一化等组件的梯度表达式，并提供LoRA层的梯度以说明参数高效微调。


<details>
  <summary>Details</summary>
Motivation: 尽管有自动微分工具，但手动推导反向传播能帮助深入理解每个操作如何影响最终输出，填补前向传播理解上的空白，从而获得对模型行为的更深刻直觉。

Method: 采用轻量级索引自由方法，将先前CNN反向传播推导的原则和符号体系扩展到Transformer架构，推导嵌入层、多头自注意力、层归一化等新类型层的梯度表达式，并特别推导LoRA层的梯度。

Result: 提供了完整的最小化GPT-like网络的PyTorch实现，以及所有梯度更新的解析表达式，使Transformer架构的反向传播过程更加透明和可理解。

Conclusion: 手动推导Transformer架构的反向传播不仅有助于深入理解模型内部工作机制，还为参数高效微调（如LoRA）提供了理论基础，增强了深度学习实践者对复杂架构的掌控能力。
...
### [430] [The Law of Multi-Model Collaboration: Scaling Limits of Model Ensembling for Large Language Models](https://arxiv.org/abs/2512.23340)
*Dakuan Lu,Jiaqi Zhang,Cheng Yuan,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: 多模型协作遵循幂律缩放规律，其性能提升趋势比单模型更显著，且理论损失下限更低，模型多样性是协作增益的主要驱动力。


<details>
  <summary>Details</summary>
Motivation: 单个大语言模型的能力存在固有边界，而多模型协作（如模型路由和后验集成）的集体性能可以超越任何单个模型，但目前缺乏统一的理论框架来预测多模型协作的性能扩展规律。

Method: 提出"多模型协作定律"，采用方法无关的公式化方法，假设理想化的集成预言机，其中每个样本的总交叉熵损失由模型池中任何模型的最小损失决定，通过实验验证多模型系统相对于总参数数量的幂律缩放规律。

Result: 实验结果显示：1）多模型系统遵循总参数数量的幂律缩放；2）相比单模型缩放，多模型系统表现出更显著的改进趋势和更低的理论损失下限；3）异构模型家族的集成比单一模型家族内的集成获得更好的性能扩展，表明模型多样性是协作增益的主要驱动力。

Conclusion: 模型协作代表了扩展大语言模型智能前沿的关键维度，多模型协作定律为预测多模型集成性能极限提供了理论基础，揭示了模型多样性在提升集体性能中的重要作用。
...
### [431] [ECG-RAMBA: Zero-Shot ECG Generalization by Morphology-Rhythm Disentanglement and Long-Range Modeling](https://arxiv.org/abs/2512.23347)
*Hai Duong Nguyen,Xuan-The Tran*

Main category: cs.LG

TL;DR: ECG-RAMBA：通过分离形态学和节律特征并重新整合，提升心电图分类的跨数据集泛化能力，在多个数据集上表现优于原始信号Mamba基线


<details>
  <summary>Details</summary>
Motivation: 深度学习在心电图分类中在单个数据集上表现良好，但在异构采集设置下的泛化能力不足，阻碍了临床部署和纵向监测。现有模型通常隐含地纠缠形态波形模式和节律动态，这可能导致捷径学习和放大对分布偏移的敏感性。

Method: ECG-RAMBA框架分离形态学和节律特征，然后通过上下文感知融合重新整合。包括：(1) MiniRocket提取的确定性形态特征；(2) 心率变异性计算的全局节律描述符；(3) 双向Mamba骨干网络进行长距离上下文建模。引入数值稳定的Power Mean池化算子(Q=3)来增强对瞬时异常的敏感性。

Result: 在Chapman-Shaoxing数据集上获得约0.85的宏观ROC-AUC。在零样本迁移中，在外部CPSC-2021数据集上获得0.708的PR-AUC用于房颤检测，显著优于可比较的原始信号Mamba基线，在PTB-XL数据集上也表现一致。

Conclusion: 确定性形态特征提供了坚实基础，而显式节律建模和长距离上下文是跨域鲁棒性的关键驱动因素。ECG-RAMBA通过分离和重新整合形态与节律特征，有效提升了心电图分类的跨数据集泛化能力。
...
### [432] [ISOPO: Proximal policy gradients without pi-old](https://arxiv.org/abs/2512.23353)
*Nilin Abrahamsen*
...
<details>
  <summary>Details</summary>
Motivation: 现有近端策略方法（如GRPO、CISPO）需要多步梯度计算和重要性采样裁剪来近似自然梯度，计算效率较低。ISOPO旨在通过单步梯度实现高效的自然策略梯度近似。

Method: ISOPO在Fisher度量下对每个序列的对数概率梯度进行归一化，然后与优势函数进行收缩。另一种变体基于神经正切核在每层转换微批次优势函数，可在单次反向传播中实现层间转换。

Result: ISOPO能以可忽略的计算开销实现自然策略梯度近似，相比传统REINFORCE方法几乎没有额外计算成本。

Conclusion: ISOPO提供了一种高效的单步自然策略梯度近似方法，显著提升了计算效率，为策略优化提供了新的实用工具。
...
### [433] [Post-Training Quantization of OpenPangu Models for Efficient Deployment on Atlas A2](https://arxiv.org/abs/2512.23367)
*Yilun Luo,HuaQing Zheng,Haoqian Meng,Wenyuan Liu,Peng Zhang*

Main category: cs.LG

TL;DR: 华为为openPangu-Embedded模型开发了低比特量化框架，支持INT8和W4A8量化，在Ascend NPU上实现高效推理，保持90%以上FP16精度，提升1.5倍预填充速度。


<details>
  <summary>Details</summary>
Motivation: openPangu-Embedded模型的三种思维链推理模式（slow_think、auto_think、no_think）虽然增强了推理能力，但生成长推理轨迹带来了显著的内存和延迟开销，在Ascend NPU上实际部署面临挑战。

Method: 采用低比特量化技术，将FP16计算转换为更高效的整数运算，提出了统一低比特推理框架，支持INT8（W8A8）和W4A8量化，专门针对Atlas A2上的openPangu-Embedded模型进行优化。

Result: 在代码生成基准测试（HumanEval和MBPP）上，INT8量化保持了90%以上的FP16基线精度，在Atlas A2上实现了1.5倍的预填充加速；W4A8量化显著减少了内存消耗，但精度有一定折衷。

Conclusion: 低比特量化技术能够有效促进Ascend NPU上的高效思维链推理，同时保持较高的模型保真度，为实际部署提供了可行的解决方案。
...
### [434] [Diffusion priors enhanced velocity model building from time-lag images using a neural operator](https://arxiv.org/abs/2512.23375)
...
Method: 提出生成神经算子框架：1）神经算子作为前向映射算子，从真实和偏移速度模型快速生成RTM时滞图像；2）通过自动微分逐步更新速度模型；3）嵌入在真实速度模型分布上训练的生成模型作为正则化器，提升分辨率。

Result: 合成和实际数据实验证明了所提生成神经算子速度模型构建方法的有效性，能够获得更清晰、更高分辨率的速度模型预测。

Conclusion: 结合生成模型与神经算子的框架能够高效构建高分辨率速度模型，解决了传统方法计算成本高的问题，为地下成像提供了新解决方案。
...
### [435] [A unified framework for detecting point and collective anomalies in operating system logs via collaborative transformers](https://arxiv.org/abs/2512.23380)
*Mohammad Nasirzadeh,Jafar Tahmoresnezhad,Parviz Rashidi-Khazaee*

Main category: cs.LG

TL;DR: CoLog是一个用于日志异常检测的多模态协作框架，通过协同变换器和多头注意力机制处理不同日志模态的交互，在七个基准数据集上实现了平均99.6%以上的检测精度。


<details>
  <summary>Details</summary>
Motivation: 日志数据包含多种模态信息，但现有方法存在局限性：单模态方法忽略不同模态特性，多模态方法未能有效处理模态间交互。需要一种能协同处理多模态日志数据并捕获复杂依赖关系的异常检测方法。

Method: 提出CoLog框架：1) 使用协同变换器和多头印象注意力学习多个模态间的交互；2) 引入模态适应层处理由交互引起的异构性，适配不同日志模态的表征；3) 学习数据中的细微模式和依赖关系。

Result: 在七个日志异常检测基准数据集上，CoLog在点异常和集体异常检测中均表现优异：平均精度99.63%、平均召回率99.59%、平均F1分数99.61%，优于现有最先进方法。

Conclusion: CoLog通过统一框架解决了日志数据自动分析中的复杂挑战，在网络安全、系统监控和运营效率方面具有重要应用价值，代表了日志异常检测领域的显著进展。
...
### [436] [On the Sample Complexity of Learning for Blind Inverse Problems](https://arxiv.org/abs/2512.23405)
*Nathan Buskulic,Luca Calatroni,Lorenzo Rosasco,Silvia Villa*

Main category: cs.LG

TL;DR: 该论文研究了盲逆问题中的学习框架，在线性最小均方误差估计器(LMMSE)的简化框架下，提供了闭式最优估计器表达式，建立了与Tikhonov正则化的等价性，并推导了有限样本误差界和收敛性理论保证。
...
Method: 在线性最小均方误差估计器(LMMSE)的简化框架下研究盲逆问题学习。推导闭式最优估计器表达式，建立与Tikhonov正则化公式的等价性，其中正则化项明确依赖于未知信号、噪声和随机前向算子的分布。

Result: 提供了深入的理论分析，包括闭式最优估计器、与Tikhonov正则化的等价关系、在适当源条件下的收敛性结果，以及严格的有限样本误差界，这些界明确量化了算子随机性的影响并揭示了相应的收敛速率。

Conclusion: 该工作为盲逆问题中的学习提供了坚实的理论基础，通过数值实验验证了理论预测的收敛行为，为数据驱动方法在盲逆问题中的应用提供了理论支持和性能保证。
...
### [437] [Task-driven Heterophilic Graph Structure Learning](https://arxiv.org/abs/2512.23406)
...
<details>
  <summary>Details</summary>
Motivation: 图神经网络在异质图上表现不佳，因为连接节点往往标签不同，特征相似性提供的结构线索较弱。现有方法难以学习到区分性的节点表示。

Method: 提出端到端的FgGSL框架：1) 使用可学习的对称特征驱动掩码函数推断互补的图结构；2) 使用预设计的低通和高通图滤波器组处理；3) 引入基于标签的结构损失显式促进同质和异质边的恢复；4) 推导了结构损失的稳定性界限和滤波器组在图扰动下的鲁棒性保证。

Result: 在六个异质图基准测试上，FgGSL始终优于最先进的GNN和图重连方法，证明了结合频率信息和监督拓扑推断的优势。

Conclusion: FgGSL通过联合学习互补的图结构和谱编码器，有效解决了异质图上的表示学习问题，为图结构学习提供了新的频率引导和监督拓扑推断方法。
...
### [438] [Directly Constructing Low-Dimensional Solution Subspaces in Deep Neural Networks](https://arxiv.org/abs/2512.23410)
*Yusuf Kalyoncuoglu*

Main category: cs.LG

TL;DR: 提出通过解耦解空间与搜索空间，构建低维子空间来绕过深度神经网络优化瓶颈，实现模型压缩的新方法


<details>
  <summary>Details</summary>
Motivation: 当前深度神经网络虽然内在维度低，但仍需要巨大的高维宽度来解决非凸优化问题，这导致了冗余。研究者希望绕过这个优化瓶颈，实现"训练大模型，部署小模型"的愿景

Method: 提出构造性方法，将解几何与搜索空间解耦，通过构建低维子空间来压缩分类头，并提出子空间原生蒸馏作为新范式

Result: 在ResNet-50、ViT和BERT上实证表明，分类头可以被压缩高达16倍而性能损失可忽略不计

Conclusion: 通过直接在构建的子空间中定义目标，为学生模型提供稳定的几何坐标系，可能完全绕过高维搜索问题，实现模型压缩的目标
...
### [439] [Stochastic Siamese MAE Pretraining for Longitudinal Medical Images](https://arxiv.org/abs/2512.23441)
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的自监督学习方法（如MAE）虽然具有较强的表示学习能力，但缺乏时间感知能力，无法捕捉纵向医学数据集中疾病进展的时间动态，特别是疾病演化固有的不确定性。

Method: 提出STAMP（Stochastic Temporal Autoencoder with Masked Pretraining），这是一个Siamese MAE框架，通过条件变分推理将MAE重建损失重构为随机过程，根据两个输入体积之间的时间差进行条件编码，学习非确定性的时间动态。

Result: 在两个OCT数据集和一个MRI数据集上评估，STAMP预训练的ViT模型在晚期年龄相关性黄斑变性和阿尔茨海默病进展预测任务中，优于现有的时间MAE方法和基础模型。

Conclusion: STAMP通过随机过程编码时间信息，能够有效学习医学影像中疾病进展的非确定性时间动态，在纵向医学数据分析中表现出优越性能。
...
### [440] [Dynamic Subspace Composition: Efficient Adaptation via Contractive Basis Expansion](https://arxiv.org/abs/2512.23448)
*Vladimer Khasia*
...
<details>
  <summary>Details</summary>
Motivation: 混合专家模型虽然能扩展容量，但常面临表示崩溃和梯度不稳定问题。现有方法如Mixture-of-LoRAs参数复杂度高，需要改进。

Method: DSC将权重更新建模为星形域内的残差轨迹，采用幅度门控单纯形插值确保在恒等变换处的连续性。通过解耦的单位范数基向量构建组合秩K近似，而非检索独立的秩r矩阵。

Result: 将参数复杂度从O(M rd)降低到O(M d)，内存流量减少到O(Kd)，并通过框架理论正则化和谱约束提供动态更新的严格最坏情况边界。

Conclusion: DSC提供了一种高效且理论保证的框架，用于构建上下文相关的权重近似，解决了MoE模型的表示和稳定性问题。
...
### [441] [Eliminating Inductive Bias in Reward Models with Information-Theoretic Guidance](https://arxiv.org/abs/2512.23461)
...
<details>
  <summary>Details</summary>
Motivation: 奖励模型训练数据质量低，包含各种归纳偏差（如响应长度、奉承、格式等），现有去偏方法要么针对单一偏差，要么仅建模简单线性相关，无法处理复杂非线性偏差

Method: 基于信息瓶颈理论，最大化奖励模型分数与人类偏好对之间的互信息，同时最小化奖励模型输出与偏好输入中偏差属性之间的互信息

Result: DIR能有效减轻目标归纳偏差（响应长度、奉承、格式），提升RLHF在不同基准测试中的性能，增强泛化能力

Conclusion: DIR通过信息理论方法处理复杂非线性偏差，扩展了奖励模型去偏方法的实际应用场景，提高了对齐效果
...
### [442] [FRoD: Full-Rank Efficient Fine-Tuning with Rotational Degrees for Fast Convergence](https://arxiv.org/abs/2512.23485)
...
Method: FRoD结合了分层联合分解和旋转自由度，通过提取跨层的全局共享基，并向缩放因子中注入稀疏、可学习的扰动，实现灵活的全秩更新。
...
### [443] [ML Compass: Navigating Capability, Cost, and Compliance Trade-offs in AI Model Deployment](https://arxiv.org/abs/2512.23487)
*Vassilis Digalakis,Ramayya Krishnan,Gonzalo Martin Fernandez,Agni Orfanoudaki*

Main category: cs.LG

TL;DR: ML Compass框架将AI模型选择视为能力-成本前沿上的约束优化问题，提供系统级决策工具，弥补能力排行榜与实际部署之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有AI能力排行榜无法直接指导实际部署决策，因为部署需要考虑用户效用、部署成本和合规要求的综合影响，存在"能力-部署差距"。需要系统级视角将模型选择与应用结果、运营约束和能力-成本前沿联系起来。

Method: 开发ML Compass框架，将模型选择视为能力-成本前沿上的约束优化问题。理论方面：在参数化前沿下表征最优模型配置，展示三区域结构。实践方面：提出包含四个步骤的管道：(1)从异构模型描述符提取低维内部度量，(2)从能力和成本数据估计经验前沿，(3)从交互结果数据学习用户或任务特定效用函数，(4)使用这些组件定位能力-成本配置并推荐模型。

Result: 在通用对话(使用PRISM Alignment数据集)和医疗保健(使用自定义HealthBench数据集)两个案例研究中验证了ML Compass。框架生成的推荐和部署感知排行榜在约束条件下的预测部署价值与仅基于能力的排名有显著差异，阐明了能力、成本和安全性之间的权衡如何影响最优模型选择。

Conclusion: ML Compass提供了一个系统级框架，将AI模型选择与用户效用、部署成本和合规要求相结合，弥补了能力排行榜与实际部署决策之间的差距，为组织在复杂约束下做出更优的AI部署决策提供了实用工具。
...
### [444] [Joint Link Adaptation and Device Scheduling Approach for URLLC Industrial IoT Network: A DRL-based Method with Bayesian Optimization](https://arxiv.org/abs/2512.23493)
...
<details>
  <summary>Details</summary>
Motivation: 工业物联网(IIoT)网络需要支持多设备动态超可靠低延迟通信(URLLC)，但面临CSI不完美、URLLC网络中的错误样本不平衡以及TD3算法参数敏感等问题，这些问题会降低算法收敛速度和可靠性

Method: 提出贝叶斯优化驱动的Twin Delayed Deep Deterministic Policy Gradient (TD3)方法，基于不完美CSI自适应确定设备服务顺序序列和相应的调制编码方案(MCS)，并设计BO-based训练机制来改善收敛速度，提供更可靠的学习方向和样本选择方法

Result: 通过大量仿真表明，所提算法相比现有解决方案实现了更快的收敛速度和更高的总速率性能

Conclusion: 提出的BO-driven TD3方法能有效解决IIoT网络中多设备URLLC场景下的联合链路自适应和设备调度问题，在CSI不完美条件下显著提升系统性能
...
### [445] [Trustworthy Machine Learning under Distribution Shifts](https://arxiv.org/abs/2512.23524)
*Zhuo Huang*

Main category: cs.LG

TL;DR: 该研究专注于分布偏移下的可信机器学习，通过分析扰动偏移、域偏移和模态偏移三种常见分布偏移，从鲁棒性、可解释性和适应性三个维度提升AI系统的可靠性和可信度。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在AI领域取得了显著进展，但分布偏移问题仍然是限制ML系统可靠性和通用性的根本弱点，同时也会引发AI的信任问题。为了解决这些挑战，研究聚焦于分布偏移下的可信机器学习。

Method: 研究将三种常见分布偏移分类为：扰动偏移、域偏移和模态偏移，并从鲁棒性、可解释性和适应性三个维度系统研究可信度问题。针对这些维度提出有效的解决方案和基础见解。

Result: 研究旨在提升AI系统的鲁棒性、多功能性、责任性和可靠性，同时增强机器学习的关键问题，如效率、适应性和安全性。

Conclusion: 通过系统研究分布偏移下的可信机器学习，该研究为提升AI系统的可靠性、通用性和社会信任度提供了重要框架和方法，有助于推动AI在现实世界中的负责任应用。
...
### [446] [EEG-based Graph-guided Domain Adaptation for Robust Cross-Session Emotion Recognition](https://arxiv.org/abs/2512.23526)
*Maryam Mirzaei,Farzaneh Shayegh,Hamed Narimani*

Main category: cs.LG

TL;DR: EGDA框架通过联合对齐全局和类别特定分布来减少跨会话差异，在SEED-IV数据集上实现稳健的跨会话情感识别性能


<details>
  <summary>Details</summary>
Motivation: 脑电信号是情感识别的可靠来源，但不同记录会话间的差异给模型泛化带来重大挑战，需要解决跨会话分布差异问题

Method: 提出EGDA框架，通过联合对齐全局（边缘）和类别特定（条件）分布来减少跨会话差异，同时通过图正则化保持脑电数据的内在结构

Result: 在SEED-IV数据集上，EGDA在三个迁移任务中分别获得81.22%、80.15%和83.27%的准确率，优于多个基线方法；Gamma频段最具判别性，中央-顶叶和前额叶脑区对情感识别最关键

Conclusion: EGDA框架通过有效的分布对齐和图正则化，显著提升了脑电情感识别的跨会话泛化能力，为稳健的人机交互系统提供了解决方案
...
### [447] [Distribution-Free Process Monitoring with Conformal Prediction](https://arxiv.org/abs/2512.23602)
*Christopher Burger*

Main category: cs.LG

TL;DR: 提出结合传统SPC与Conformal Prediction的混合框架，通过两种新应用增强质量控制：可视化不确定性的控制图和基于p值图的异常检测


<details>
  <summary>Details</summary>
Motivation: 传统统计过程控制（SPC）依赖常被违反的统计假设，在现代复杂制造环境中可靠性不足，需要更鲁棒的方法

Method: 集成Conformal Prediction的分布自由、模型无关保证，提出Conformal-Enhanced控制图（可视化过程不确定性）和Conformal-Enhanced过程监控（将多变量控制重构为异常检测问题）

Result: 框架提供更鲁棒、统计严谨的质量控制方法，同时保持经典方法的可解释性和易用性

Conclusion: 结合Conformal Prediction的混合框架能有效增强传统SPC，为现代制造环境提供更可靠的质量监控方案
...
### [448] [Le Cam Distortion: A Decision-Theoretic Framework for Robust Transfer Learning](https://arxiv.org/abs/2512.23617)
*Deniz Akdemir*

Main category: cs.LG

TL;DR: 本文提出Le Cam Distortion框架，通过方向性可模拟性替代对称不变性，解决传统无监督域自适应中负迁移问题，在医疗、自动驾驶等安全关键领域实现风险可控的迁移学习。


<details>
  <summary>Details</summary>
Motivation: 传统无监督域自适应方法强制特征不变性，当源域和目标域信息量不均衡时（如高质量传感器vs退化传感器），严格不变性会导致信息破坏，产生负迁移，在安全关键应用中可能造成灾难性后果。

Method: 基于Le Cam统计实验理论，提出决策理论框架，用方向性可模拟性替代对称不变性。引入Le Cam Distortion（由缺陷距离δ(E₁,E₂)量化）作为可模拟性条件下迁移风险的严格上界。通过学习从源域模拟目标域的核，实现不损害源域效用的迁移。

Result: 在五个实验中：1）HLA基因组学中实现近乎完美的频率估计（相关性r=0.999）；2）CIFAR-10图像分类中零源域效用损失（81.2%准确率保留，而CycleGAN下降34.7%）；3）RL控制中安全策略迁移，不变性方法出现灾难性崩溃。

Conclusion: Le Cam Distortion为医疗影像、自动驾驶系统和精准医疗等负迁移不可接受的领域，提供了首个原则性的风险可控迁移学习框架，解决了传统域自适应方法的根本缺陷。
...
### [449] [BOAD: Discovering Hierarchical Software Engineering Agents via Bandit Optimization](https://arxiv.org/abs/2512.23631)
*Iris Xu,Guangtao Zeng,Zexue He,Charles Jin,Aldo Pareja,Dan Gutfreund,Chuang Gan,Zhang-Wei Hong*

Main category: cs.LG

TL;DR: 提出BOAD框架，通过多臂老虎机自动发现有效的分层多智能体系统，解决大语言模型在真实软件工程任务中泛化能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有单一智能体系统在处理长视野、分布外真实软件工程问题时表现不佳，需要像人类工程师那样将复杂问题分解为专门子任务。

Method: 将智能体层次发现建模为多臂老虎机问题，每个臂代表候选子智能体，奖励衡量其在团队协作中的有效性，在有限评估预算下高效探索子智能体设计。

Result: 在SWE-bench-Verified上优于单一智能体和手动设计的多智能体系统；在SWE-bench-Live上，36B系统在评估时排名第二，超越GPT-4和Claude等更大模型。

Conclusion: 自动发现的分层多智能体系统能显著提升在挑战性长视野软件工程任务上的泛化能力。
...
### [450] [Random Controlled Differential Equations](https://arxiv.org/abs/2512.23670)
...
<details>
  <summary>Details</summary>
Motivation: 传统时间序列学习方法计算成本高，特别是显式签名计算效率低下。需要一种既能保留路径签名理论的归纳偏置，又能实现高效训练的方法

Method: 使用大型随机参数化CDE作为连续时间储层，仅训练线性输出层。提出两种变体：RF-CDEs（随机傅里叶特征提升）和R-RDEs（粗糙路径输入，使用log-ODE离散化）

Result: 在多个时间序列基准测试中表现出竞争性或最先进的性能，为显式签名计算提供了实用替代方案

Conclusion: 该框架统一了随机特征储层、连续时间深度架构和路径签名理论，在无限宽度极限下分别诱导出RBF提升签名核和粗糙签名核
...
### [451] [End-to-End Test-Time Training for Long Context](https://arxiv.org/abs/2512.23675)
...
TL;DR: 提出TTT-E2E方法，将长上下文语言建模视为持续学习问题，通过测试时训练和元学习实现，在保持恒定推理延迟的同时获得与全注意力Transformer相似的扩展性


<details>
  <summary>Details</summary>
Motivation: 传统长上下文建模方法主要关注架构设计（如滑动窗口注意力、Mamba等），本文提出新视角：将长上下文建模视为持续学习问题，通过测试时训练让模型在推理时继续学习，从而压缩上下文信息到权重中

Method: 1. 使用标准Transformer架构（滑动窗口注意力）；2. 测试时通过下一个token预测进行持续学习；3. 训练时通过元学习优化模型初始化，使其更适合测试时学习；4. 端到端方法（TTT-E2E）

Result: 1. 3B模型在164B tokens训练下，TTT-E2E的扩展性与全注意力Transformer相同，优于Mamba 2和Gated DeltaNet；2. 类似RNN，具有恒定推理延迟（与上下文长度无关）；3. 在128K上下文长度下，比全注意力快2.7倍

Conclusion: 将长上下文建模重新定义为持续学习问题，通过测试时训练和元学习的端到端方法，在保持恒定推理延迟的同时实现了与全注意力Transformer相似的扩展性能，为长上下文建模提供了新思路
